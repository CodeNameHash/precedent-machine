...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { provision_id, id } = req.query;
    if (id) {
  ...
  if (req.method === 'POST') {
    const { provision_id, phrase, favorability, note, user_id, is_ai_generated, overrides_id } = req.body;
    const { data, error } = await sb.from('annotations')
      .insert({ provision_id, phrase, favorability, note, user_id, is_ai_generated: is_ai_generated || false, overrides_id })
      .select().single();
    if (error) return res.status(500).json({ error: error.message });
    return res.json({ annotation: data });
  }
