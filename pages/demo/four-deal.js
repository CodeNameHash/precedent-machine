...
function rowSummary(rows = []) {
  return rows.reduce((summary, row) => {
    if (row.review_state === 'OPEN_WORLD') summary.openWorld += 1;
    else if (row.review_state === 'REVIEW_REQUIRED') summary.review += 1;
    else summary.reviewed += 1;
    return summary;
  }, { reviewed: 0, review: 0, openWorld: 0 });
}
      ...
      <main className="mtx min-h-screen bg-[#F2F0EA] text-[#1F1F1F]">
        ...
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6">
          <header className="border-b-2 border-[#1F1F1F] pb-4">
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#66625C]">TopBuild, Skechers and Modiv use the current immutable M3 execution artefact. Metsera uses the sealed Process Product result. This page does not connect to a database or write data.</p>
          </header>
...
function Review({ deal, result }) {
  ...
  return <div><DealHeader deal={adapted} view="summary" onToggleView={() => {}} hasAgreementText={false} extracted={{ consideration: deal.consideration_type }} /><div className="p-5"><ResultNotice result={result} /><div className="mt-4 grid gap-3 sm:grid-cols-3"><Result label="Governed values" value={rowSummary(result.rows).reviewed} /><Result label="Review required" value={rowSummary(result.rows).review} /><Result label="Open-world warnings" value={rowSummary(result.rows).openWorld} /></div>{result.product_component ? <div className="mt-5"><h2 className="text-sm font-bold uppercase tracking-wide text-[#4E4B46]">Process Product result</h2><div className="mt-3"><ProcessPassageCard slot={result.product_component} /></div></div> : null}<ResultRows rows={result.rows} title="Reviewed output" /></div></div>;
}

function Compare({ deals, resultContract }) {
  return <div className="overflow-x-auto p-5"><h2 className="text-lg font-bold">Compare four deals</h2><p className="mt-2 text-sm text-[#66625C]">Each column uses the same frozen result contract as Review, Corpus Context and Query.</p><table className="mt-4 min-w-full border-collapse text-left text-sm"><thead><tr className="border-b border-[#D9D7D2] text-xs uppercase tracking-wide text-[#77736C]"><th className="p-2">Deal</th><th className="p-2">Result domain</th><th className="p-2">Governed values</th><th className="p-2">Review required</th><th className="p-2">Open-world</th></tr></thead><tbody>{deals.map((deal) => { const result = resultContract.deals.find((entry) => entry.deal_id === deal.id); const summary = rowSummary(result.rows); return <tr key={deal.id} className="border-b border-[#E6E4DF]"><td className="p-2 font-semibold">{title(deal)}</td><td className="p-2">{stateLabel(result.result_domain)}</td><td className="p-2">{summary.reviewed}</td><td className="p-2">{summary.review}</td><td className="p-2">{summary.openWorld}</td></tr>; })}</tbody></table></div>;
}

function CorpusContext({ deal, result, resultContract }) {
  const citations = [...new Set(result.rows.map((row) => row.source_citation))];
  return <div className="grid gap-4 p-5 md:grid-cols-2"><Panel title="Deal context"><FactList deal={deal} /><Result label="Result domain" value={stateLabel(result.result_domain)} /><Result label="Result state" value={stateLabel(result.result_state)} /></Panel><Panel title="Immutable source binding"><Result label="M3 execution result" value={resultContract.m3_artifact.execution_result_id} mono /><Result label="Source artefact" value={resultContract.m3_artifact.relative_path} /><Result label="Source citations" value={citations.length} /><p className="mt-3 text-xs text-[#66625C]">Each output row below carries its retained source quote and citation. No source reader, corpus write or database query is used.</p></Panel><div className="md:col-span-2"><ResultRows rows={result.rows} title="Exact source context" limit={8} /></div></div>;
}
...
function ResultNotice({ result }) { return <p className="border border-[#D9D7D2] bg-[#F7F5F0] px-3 py-2 text-xs leading-5 text-[#4E4B46]">{stateLabel(result.result_domain)} · {stateLabel(result.result_state)} · exact quotes and citations are bound from the frozen preview contract.</p>; }

function ResultRows({ rows, title, limit = 16 }) { const [showAll, setShowAll] = useState(false); const visible = showAll ? rows : rows.slice(0, limit); return <section className="mt-5"><div className="flex items-baseline justify-between gap-4"><h2 className="text-sm font-bold uppercase tracking-wide text-[#4E4B46]">{title}</h2><span className="text-xs text-[#77736C]">{rows.length} rows</span></div><div className="mt-3 space-y-3">{visible.map((row) => <article key={row.row_id} className="rounded border border-[#E6E4DF] bg-[#FBFAF7] p-3"><div className="flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#77736C]">{stateLabel(row.review_state)}</span><span className="text-xs font-semibold text-[#34312D]">{row.governed_field}</span><span className="text-xs text-[#66625C]">{row.governed_value}</span></div><blockquote className="mt-2 whitespace-pre-wrap border-l-2 border-[#1F1F1F] pl-3 text-sm leading-6 text-[#34312D]">{row.source_quote}</blockquote><p className="mt-2 text-xs font-medium text-[#66625C]">{row.source_citation}</p>{row.warning ? <p className="mt-2 text-xs font-semibold text-[#9D3B21]">Warning: {row.warning}</p> : null}{row.review_reasons.length ? <p className="mt-2 text-xs text-[#77736C]">Review basis: {row.review_reasons.join(', ')}</p> : null}</article>)}</div>{rows.length > limit ? <button type="button" onClick={() => setShowAll((value) => !value)} className="mt-4 text-xs font-semibold underline">{showAll ? 'Show fewer rows' : `Show all ${rows.length} rows`}</button> : null}</section>; }
