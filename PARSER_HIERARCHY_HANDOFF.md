...
- WP-SCHEMA Phase 1 and Phase 2 are already on `origin/main`:
  ...
  - `lib/schema/*`
  - `tests/schema/*`
- WP-SCHEMA Phase 3 is implemented and merged:
...
- Admin/gaps stored metrics sidecar is merged:
  ...
  - Hotfix PR `#122` (`https://github.com/CodeNameHash/precedent-machine/pull/122`) merged at `3a7ff06` to remove `provisions.updated_at` from the production query.
  - Production `deal_quality_metrics` SQL was applied with Supabase CLI.
  - Production warm pass succeeded: `/api/admin/gaps?limit=100&refresh_metrics=1` recomputed and wrote `25` metric rows.
  - Production stored pass succeeded: `/api/admin/gaps?limit=100` returned `25` stored rows, `0` recomputed, schema available.
  - Live `public.provisions` columns are `id`, `deal_id`, `type`, `category`, `full_text`, `prohibition`, `exceptions`, `ai_favorability`, `created_at`, `ai_metadata`; do not select repo-schema-only optional columns from production API paths.
