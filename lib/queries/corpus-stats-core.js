...
// `${deal_id}|${excerpt_id}|${provision_instance_id}` -> card id.
// provision_instance_id added (2026-08-05 ruling): excerpt_id is not a
// unique card identity -- two cards in a deal can legitimately share one,
// so buildCorpusBase() below binds identity on deal_id + excerpt_id +
// provision_instance_id instead. Falls back to the legacy deal_id|
// excerpt_id key when the richer one misses -- both for a claim whose
// provision_instance_id is unresolved (unresolvable is not evidence of a
// mismatch; both live claim-writers always stamp it) and for any caller
// still building cardIdByKey the old way (e.g.
// tests/query/relative-periods.test.js's hand-built fixture). No-op on
// today's corpus: excerpt_id alone is already unique per card, so the rich
// key always resolves directly once a caller populates it.
function cardIdForClaim(claim, cardIdByKey) {
  ...
  return cardIdByKey.get(richKey) || cardIdByKey.get(legacyKey) || null;
}
...
// endpoint) -- safe either way since it's keyed by deal_id+excerpt_id+
// provision_instance_id, which is unique per card regardless of how many
// codes' cards got fetched together. excerpt_id alone is NOT a unique card
// identity (2026-08-05 ruling: two cards in a deal can legitimately share
// one) -- provision_instance_id is what disambiguates them; see
// cardIdForClaim's fallback for callers still on the legacy key.
function buildCorpusBase({ deals, cards, subjectDealId, filters }) {
  ...
  for (const c of cards || []) {
    if (c.excerpt_id) cardIdByKey.set(`${c.deal_id}|${c.excerpt_id}|${c.provision_instance_id || ''}`, c.id);
  }
