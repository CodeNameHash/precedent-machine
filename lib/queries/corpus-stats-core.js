...
// surfaced as an explicit "none captured" bucket rather than silently
...
function buildCategoricalDealDistribution(attribute, peerClaims, subjectDealId, dealsById, cardIdByKey, { presentDealEntries = null } = {}) {
  ...
  const noneLabel = `None captured — no ${attributeLabel(attribute).toLowerCase()}`;
...
function buildNumericAttributeSummary(attribute, claimsForAttribute, peerIds, peerSet) {
  ...
  if (unit === 'usd') {
    ...
    if (percentValues.length) {
      ...
      percentOfDeal = {
        min: pStats.min, p25: pStats.p25, median: pStats.median, p75: pStats.p75, max: pStats.max,
