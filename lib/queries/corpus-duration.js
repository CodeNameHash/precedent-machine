...
// excerpt_id is not a unique card identity (2026-08-05 ruling): two cards
// in a deal can legitimately share one, so the ideal key also binds on
// provision_instance_id. Tries that richer key first, then falls back to
// the legacy deal_id|excerpt_id key -- both for a claim whose
// provision_instance_id is unresolved, and for any caller still building
// evidenceByCardKey the old way (e.g. tests/corpus-duration.test.js's
// hand-built fixture). No-op on today's corpus: excerpt_id alone is
// already unique per card, so the rich key resolves directly whenever a
// caller populates it with provision_instance_id.
function linkedEvidenceForClaim(claim, evidenceByCardKey) {
  if (!claim?.excerpt_id || !(evidenceByCardKey instanceof Map)) return null;
  const richKey = `${claim.deal_id}|${claim.excerpt_id}|${claim.provision_instance_id || ''}`;
  const legacyKey = `${claim.deal_id}|${claim.excerpt_id}`;
  return evidenceByCardKey.get(richKey) || evidenceByCardKey.get(legacyKey) || null;
}
...
function durationInDays(duration) {
  ...
  if (duration.unit === 'elapsed_hours') return { value: duration.value / 24, unit: 'days_equivalent' };
  if (duration.unit === 'business_days' || duration.unit === 'calendar_days' || duration.unit === 'days_equivalent') {
    return { value: duration.value, unit: 'days_equivalent' };
  }
  return null;
}
...
function selectDurationCohort({ claims, subjectDealId, requestedCode, evidenceByCardKey }) {
  ...
  const subjectDayValue = durationInDays(subject.duration);
  const normaliseToDays = Boolean(subjectDayValue);
  const matchingByDeal = new Map();
  for (const claim of triggerClaims) {
    const rawDuration = normalizeDurationClaim(claim, linkedEvidenceForClaim(claim, evidenceByCardKey));
    const duration = normaliseToDays ? durationInDays(rawDuration) : rawDuration;
    if (!duration || duration.unit !== (normaliseToDays ? 'days_equivalent' : subject.duration.unit)) continue;
  ...
  return {
    cohort: {
      entries,
      unit: normaliseToDays ? 'days_equivalent' : subject.duration.unit,
      triggerCode,
      subjectValue: normaliseToDays ? subjectDayValue.value : subject.duration.value,
