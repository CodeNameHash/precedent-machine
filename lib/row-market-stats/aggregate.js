...
function semanticMismatch(value, semantics) {
  ...
  for (const [dimension, actual, expected] of checks) {
    ...
    if (!actual && expected && required.has(contractDimension)) return `missing_${dimension}`;
    if (actual && expected && actual !== expected) return dimension;
  }
  return null;
}
...
function analyseScalar(entry, spec) {
  ...
  return { state: 'comparable', value: values[0].value };
}

function analyseMultiSelect(entry) {
  if (!entry.values.length) return { state: 'value_unknown' };
  const values = distinctValues(entry.values).map((item) => item.value);
  return values.length ? { state: 'comparable', values } : { state: 'value_unknown' };
}
...
function analyseNumeric(entry, spec) {
  ...
  for (const item of entry.values) {
    ...
    compatible.push({ value: number, cohort: semanticCohort(candidate, semantics) });
  }
  ...
  for (const item of compatible) {
    distinct.set(`${semanticCohortKey(item.cohort)}|${item.value}`, item);
  }
  if (distinct.size !== 1) return { state: 'excluded', reason: 'multiple_values_for_scalar' };
  const [value] = distinct.values();
  return { state: 'comparable', value: value.value, cohort: value.cohort };
}

function analyseMoney(entry, spec) {
  if (!entry.values.length) return { state: 'value_unknown' };
  const numeric = analyseNumeric(entry, spec);
  if (numeric.state !== 'comparable') return numeric;
  if (normaliseUnit(numeric.cohort.unit) !== 'usd') {
    return { state: 'excluded', reason: 'incompatible_unit', rawValue: numeric.value };
  }
  ...
  const proportion = numeric.value / basis.dealValueUsd;
  return {
    state: 'comparable',
    rawValue: numeric.value,
    rawUnit: 'usd',
...
function analysePresentEntry(entry, spec) {
  ...
  if (kind === 'presence') return { state: 'comparable' };
...
function categoricalDistribution(spec, analyses, coverage) {
  ...
  for (const analysis of analyses.values()) {
    ...
    if (!counts.has(key)) counts.set(key, { value: analysis.value, count: 0 });
  ...
  return {
    ...
    values: [...counts.values()]
      .map((item) => ({ ...item, rate: ratio(item.count, coverage.presentCount) }))
...
function numericDistribution(kind, analyses, coverage) {
  ...
  for (const analysis of analyses.values()) {
    ...
    if (!cohorts.has(key)) cohorts.set(key, { semantics: analysis.cohort, values: [] });
    cohorts.get(key).values.push(analysis.value);
  }
  return {
    kind,
    denominator: 'present_deals',
    ...
    cohorts: [...cohorts.values()]
      .map((cohort) => ({ semantics: cohort.semantics, stats: numericSummary(cohort.values) }))
...
function moneyDistribution(spec, analyses, coverage) {
  ...
  for (const analysis of analyses.values()) {
    if (analysis.rawUnit === 'usd' && Number.isFinite(analysis.rawValue)) raw.push(analysis.rawValue);
    if (analysis.state !== 'comparable') continue;
    if (!byBasis.has(analysis.basis)) byBasis.set(analysis.basis, { percent: [], basisPoints: [] });
    byBasis.get(analysis.basis).percent.push(analysis.percentOfDealValue);
    byBasis.get(analysis.basis).basisPoints.push(analysis.basisPointsOfDealValue);
  }
  return {
    kind: 'money',
    denominator: 'present_deals',
...
function subjectObservation(subjectEntry, spec) {
  ...
  const analysis = analysePresentEntry(subjectEntry, spec);
  if (spec.comparison.kind === 'presence') return { ...base, valueState: 'comparable' };
