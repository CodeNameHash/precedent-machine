...
// card identity -- two cards in a deal can legitimately share one (see
// indexDataset() in lib/row-market-stats/observations.js). Card/claim
// identity keys throughout this module and observations.js bind on
// deal_id + excerpt_id + provision_instance_id, so it must be fetched here.
const PROVISION_CARD_COLUMNS = 'id, deal_id, excerpt_id, provision_instance_id, provision_type, provision_subtype, short_title, defined_term, primary_quote';
...
async function loadMarketDataset(supabase, specs, {
  ...
  try {
    ...
    const claimTasks = featureChunks.map((values) => () => loadPagedRows(
      (from, to) => supabase
        .from('claims')
        .select('id, deal_id, excerpt_id, provision_instance_id, attribute, canonical, verbatim, evidence_quote, provenance')
        .in('attribute', values)
        .order('id')
        .range(from, to),
      'claims',
    ...
    // 2026-08-05 ruling: excerpt_id is not a unique card identity, so the
    // "did we already load this claim's card" check binds on
    // provision_instance_id too (falling back to '' when absent, matching
    // however the claim's own provision_instance_id degrades below --
    // today's excerpt_id-unique corpus makes this a no-op). The follow-up
    // backfill fetch itself stays keyed by bare excerpt_id (line below) --
    // it deliberately pulls EVERY card at that excerpt_id, siblings
    // included, and the dedupe() below sorts identity out afterward.
    const loadedCardKeys = new Set(cards.map((card) => `${card.deal_id}|${card.excerpt_id}|${card.provision_instance_id || ''}`));
    const missingClaimExcerptIds = [...new Set(claims
      .filter((claim) => claim.excerpt_id && !loadedCardKeys.has(`${claim.deal_id}|${claim.excerpt_id}|${claim.provision_instance_id || ''}`))
    ...
    return {
      deals,
      // provision_instance_id added (2026-08-05 ruling): excerpt_id is not
      // a unique card identity, so two genuinely distinct sibling cards
      // fetched into the same `cards` array must not collapse into one --
      // only exact re-fetches of the SAME card (identical on every column,
      // including provision_instance_id) should dedupe away. No-op on
      // today's corpus, where provision_type/provision_subtype already
      // made the old key unique per real card.
      cards: dedupe(cards, (card) => `${card.deal_id}|${card.excerpt_id}|${card.provision_instance_id || ''}|${card.provision_type}|${card.provision_subtype || ''}`),
      // provision_instance_id added for the same reason on the claims
      // fallback key (only used when claim.id is falsy): two sibling
      // cards' auto-derived claims can carry the identical attribute/
      // canonical/verbatim (Ben's example -- a non-reliance acknowledgment
      // and its auto-derived sibling quoting the same sentence), which
      // would otherwise collide on this key and silently drop one.
      claims: dedupe(claims, (claim) => claim.id || `${claim.deal_id}|${claim.excerpt_id}|${claim.provision_instance_id || ''}|${claim.attribute}|${claim.canonical || ''}|${claim.verbatim || ''}`),
      source: {
