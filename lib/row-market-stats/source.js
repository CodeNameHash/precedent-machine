...
function collectSourceRequirements(specs) {
  ...
  for (const spec of comparableSpecs(specs)) {
    ...
    if (presence.strategy === 'definition_term') provisionFamilies.add('DEFINITION');
  }
...
async function loadMarketDataset(supabase, specs) {
  ...
  try {
    const dealPromise = loadPagedRows(
      (from, to) => supabase
        .from('deals')
    ...
    const cardPromises = [
      ...familyChunks.map((values) => loadPagedRows(
        (from, to) => supabase
          ...
          .select('deal_id, excerpt_id, provision_type, provision_subtype, short_title, defined_term, primary_quote')
    ...
    const claimPromises = featureChunks.map((values) => loadPagedRows(async (from, to) => {
      let result = await supabase
        .from('claims')
        .select('id, deal_id, excerpt_id, attribute, canonical, canonical_numeric, verbatim, evidence_quote, provenance')
        .in('attribute', values)
        .order('id')
        .range(from, to);
      const missingColumn = result?.error && (
        ...
        || /canonical_numeric/i.test(String(result.error.message || ''))
      );
      if (missingColumn) {
    ...
    const [dealResult, cardResults, claimResults] = await Promise.all([
      dealPromise,
