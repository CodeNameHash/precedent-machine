...
// Card/claim identity keys in this function prefer deal_id + excerpt_id +
// provision_instance_id (2026-08-05 ruling): excerpt_id is not a unique
// card identity on its own -- two cards in a deal can legitimately share
// one (a non-reliance acknowledgment and its auto-derived sibling quoting
// the same sentence; several representation-qualifier cards on one
// provision). Every rich key here has a legacy deal_id|excerpt_id
// fallback alongside it, because not every claims producer stamps
// provision_instance_id onto its claim rows the way the live ingest
// writers do (lib/parser-v2/store-claims.js,
// scripts/backfill/claims-from-normalized.js) -- e.g.
// lib/canonical-v2/material-contracts-product-projection.js mints claims
// with no provision_instance_id at all, even though its cards have one.
// Preferring the rich key and falling back to the legacy one keeps both
// producers correct: a claim that DOES carry provision_instance_id
// resolves precisely to its own sibling card; one that doesn't falls back
// to the same (pre-ruling) excerpt-only resolution as before -- a no-op on
// today's corpus either way, since excerpt_id is already unique per card.
function cardKeyVariants(entity) {
  const base = `${entity.deal_id}|${entity.excerpt_id}`;
  return entity.provision_instance_id
    ? [`${base}|${entity.provision_instance_id}`, base]
    : [base];
}

function indexDataset(dataset) {
  const recoveredClaims = [...(dataset.claims || [])];
  const existingBringDown = new Set();
  for (const claim of recoveredClaims) {
    if (claim.attribute !== 'bringDownTiers') continue;
    const code = token(claim?.provenance?.code) || '';
    for (const variant of cardKeyVariants(claim)) existingBringDown.add(`${variant}|${code}`);
  }
  for (const card of dataset.cards || []) {
    ...
    const keyVariants = cardKeyVariants(card).map((variant) => `${variant}|${recovered.provisionCode}`);
    if (keyVariants.some((variant) => existingBringDown.has(variant))) continue;
    recoveredClaims.push({
      ...
      // Mirrors the live claim-writers (lib/parser-v2/store-claims.js,
      // scripts/backfill/claims-from-normalized.js), which always stamp a
      // real claim's provision_instance_id from its owning card row --
      // this synthesized claim must carry the same so cardByExcerpt
      // lookups below (claimMatchesScope, cardIdForClaim, valuesForDeal)
      // resolve it back to THIS card, not a sibling.
    ...
    for (const variant of keyVariants) existingBringDown.add(variant);
  }
  ...
  for (const card of dataset.cards || []) {
    ...
    // Both variants point at the SAME card. The legacy (excerpt-only)
    // variant can be overwritten by a later sibling sharing that
    // excerpt_id -- exactly today's pre-ruling ambiguity, and only ever
    // consulted as a fallback when a claim's rich-key lookup misses.
    if (card.excerpt_id) for (const variant of cardKeyVariants(card)) cardByExcerpt.set(variant, card);
  }
...
// Resolves the card a claim belongs to, preferring the rich
// deal_id|excerpt_id|provision_instance_id key and falling back to the
// legacy deal_id|excerpt_id key -- see indexDataset's header comment.
function cardForClaim(index, claim) {
  for (const variant of cardKeyVariants(claim)) {
    const card = index.cardByExcerpt.get(variant);
    if (card) return card;
  }
  return null;
}
...
function claimMatchesScope(claim, spec, index) {
  ...
  const card = cardForClaim(index, claim);
  if (cohort.scope === 'provision_family') {
...
function definitionCards(cards, term, claims = []) {
  ...
  const matchingClaimExcerpts = new Set(claims
    ...
    .flatMap((claim) => cardKeyVariants(claim)));
  return cards.filter((card) => token(card.provision_type) === 'DEFINITION'
    && (
      ...
      || cardKeyVariants(card).some((variant) => matchingClaimExcerpts.has(variant))
    ));
}

function cardIdForClaim(index, claim) {
  if (!claim) return null;
  const cardId = cardForClaim(index, claim)?.id;
