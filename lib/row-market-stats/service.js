...
const { hasDarkAuthority } = require('../query/dark-authority-fence');
...
function calculateMarketStats(request, dataset, validateMetricResult) {
  if ([...(dataset?.cards || []), ...(dataset?.claims || [])].some(hasDarkAuthority)) {
    throw new MarketStatsError(
      'DARK_AUTHORITY_NOT_SERVABLE',
      'Market statistics cannot admit VALIDATED_NOT_SERVED cards or claims.',
    );
  }
