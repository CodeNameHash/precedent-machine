...
function lastStdoutErrorMessage(out) {
  ...
  for (const line of String(out || '').split('\n')) {
    try {
      ...
      // Claude Code's stream-json ends a refused turn with a result event
      // carrying is_error and the reason ("You've hit your session limit ·
      // resets 8:40pm (UTC)", Metsera generation 7 at 19:29 UTC,
      // 2026-09-14: five attempts in twenty seconds recorded as "claude
      // exited 1: " and the run FAILED with no reason).
      const candidate = event?.type === 'error' ? event.message
        : event?.type === 'turn.failed' ? event.error?.message
          : event?.type === 'item.completed' && event.item?.type === 'error' ? event.item.message
            : event?.type === 'result' && event.is_error === true && typeof event.result === 'string' ? event.result : null;
...
const USAGE_LIMIT = /usage limit|session limit|purchase more credits|rate limit|limit reached|out of extra usage/i;
