...
async function assertCodexChatgptAuth(timeoutMs = DEFAULT_TIMEOUT_MS) {
  let status;
  try {
    status = (await runProcess('codex', ['login', 'status'], '', timeoutMs, true)).trim();
  } catch (error) {
    throw new Error(
...
function runProcess(cmd, args, stdinText, timeoutMs, includeStderrOnSuccess = false) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      settle(() => {
        ...
        if (stdinError) return reject(new Error(`${cmd} stdin failed: ${stdinError.message}`));
...
function buildCodexExecArgs(opts = {}) {
  ...
  return [
    ...
    ...(opts.isolated ? ['--skip-git-repo-check', '-C', os.tmpdir()] : []),
    '-',
  ];
}
...
function createCodexCliClient(opts = {}) {
  ...
  const execArgs = buildCodexExecArgs(opts);
    ...
    messages: {
      create: async (params) => {
        if (opts.skipAuthPreflight !== true) {
          if (!authPreflight) authPreflight = assertCodexChatgptAuth(timeoutMs);
          await authPreflight;
        }
        const prompt = flattenMessages(params);
        await acquire();
