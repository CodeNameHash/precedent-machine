...
function claudeJsonResult(raw) {
  ...
  if (!parsed || typeof parsed !== 'object' || parsed.type !== 'result') {
    throw new Error('CLAUDE_CLI_JSON: the result is not a result object');
  }
  return parsed;
}
      ...
      create: async (params) => {
        ...
        try {
          ...
          return {
            ...
            usage: result.usage || {},
            claude_completion: { status: 'COMPLETE', subtype: result.subtype, stop_reason: result.stop_reason ?? null },
