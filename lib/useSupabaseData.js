...
export function useProvisions(filters = {}) {
  ...
  // If filters were specified but none resolved (e.g. deal_id is undefined while loading),
  // skip the fetch to avoid pulling ALL provisions across every deal.
  const hasFilterKeys = Object.keys(filters).length > 0;
  const url = hasFilterKeys && !qs ? null : `/api/provisions${qs ? `?${qs}` : ''}`;
