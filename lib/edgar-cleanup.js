...
export function rejoinEdgarLines(text) {
  // Heading-specific wrap repairs BEFORE the line loop. Older EDGAR HTML
  // (2015-era: Whole Foods, Dyax) hard-wraps INSIDE headings — "Section\n6.1"
  // and "6.1 Interim\nOperations." — which the loop below cannot fix: its
  // <40-char guard treats short heading fragments as intentional breaks, and
  // its structural guard refuses to join a line starting with a bare section
  // number. Wrapped headings then fail line-start heading detection and whole
  // articles collapse into INTRO blobs (Dyax coverage 26%).
  text = text
