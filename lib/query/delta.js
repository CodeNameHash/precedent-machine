...
function scalarDelta(field, a, b, kind) {
  ...
  // 'numeric' is the NORMALIZED kind that shared.js fieldKind() emits (and the
  // only value deal-compare passes here); the rest are raw def.type literals
  // kept for any direct callers. Missing 'numeric' silently routed every real
  // numeric delta to the string branch below (a $2M-vs-$20M gap scored MINOR).
  if (kind === 'numeric' || kind === 'number' || kind === 'decimal' || kind === 'int' || kind === 'currency' || kind === 'percentage' || kind === 'duration' || kind === 'usd' || kind === 'percent') {
    const x = Number(a);
    const y = Number(b);
