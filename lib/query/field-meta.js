...
// "object { ... }" was the only lead-in this covered; the rubric also
// annotates array-shaped fields as "array of { ... } objects, ..." / "list
// of { ... }" -- same schema-shape leak, different noun, sometimes with
// trailing prose after the closing brace ("... one per tier. Use a
// single-element array if ..."). `verbatim` widened the same way: extraction
// guidance doesn't stop at the word "language" ("verbatim language and any
// exceptions" is still an instruction to the extractor, not part of the
// field's meaning).
const ANNOTATION_SUFFIX_RE = new RegExp(
  '\\s+-\\s+(?:(?:object|array|list)(?:\\s+of)?\\s*\\{[^}]*\\}.*|\\[\\{[^\\]]*\\]|verbatim\\b.*|'
...
function fieldsForProvisionType(provisionType) {
  const defs = [
    ...
    ...Object.entries(DERIVED_FIELDS)
      .filter(([, def]) => def.provisionType === provisionType)
      .map(([key]) => ({ key })),
  ];
  ...
  // The pass above disambiguates fields of DIFFERENT shapes that clean to the
  // same label, but two same-shape siblings (e.g. two booleans that both
  // clean to "Standstill waiver permitted") get the identical type suffix
  // and still collide. Break anything still shared with each field's own
  // key, which is unique by construction (deduped on canonical key above).
  const stillCollided = new Map();
  for (const f of out) stillCollided.set(f.label.toLowerCase(), (stillCollided.get(f.label.toLowerCase()) || 0) + 1);
  for (const f of out) {
    if (stillCollided.get(f.label.toLowerCase()) < 2) continue;
