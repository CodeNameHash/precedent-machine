...
const { humanizeKey, lowerFirst, sentenceForFilter, isBooleanFilter } = require('./filter-labels');
...
function resultTitle(result) {
  ...
  if (result.kind === 'MARKET_RANGE') return `Market range — ${humanizeKey(result.field_path)}`;
  if (result.kind === 'DEAL_COMPARE') return `Compare: ${(result.columns || []).map((col) => col.deal_name).join(' vs ')}`;
  if (result.kind === 'PROVISION_CROSS_CUT') return `${humanizeKey(result.provision_type)} across deals`;
