  ...
  const rows = provisionTypes.map((provisionType) => {
    const cells = deals.map((deal) => {
      ...
      const fields = fieldsForCompareCell(provisionType, payload.included_field_groups).map((field) => {
        const fieldProvision = provisionType === 'CLOSING_CONDITION'
          ? firstProvisionWithField(context.provisions || [], deal.id, provisionType, field, null, deal)
          : provision;
        const result = fieldProvision ? provisionFieldValue(fieldProvision, provisionType, field, deal) : { value: null, def: null };
        return {
