...
const { dealColumn, firstProvision, fieldKind, provisionFieldValue, primaryQuote } = require('./shared');
  ...
  const rows = provisionTypes.map((provisionType) => {
    const cells = deals.map((deal) => {
      ...
      const fields = fieldsForCompareCell(provisionType, payload.included_field_groups).map((field) => {
        const result = provision ? provisionFieldValue(provision, provisionType, field) : { value: null, def: null };
        return {
