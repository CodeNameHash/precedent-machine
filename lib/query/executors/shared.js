...
// All family provisions carrying a value for the field, in agreement
// order, each with its resolved value/quote — the obligation-level view
// (r10c). Callers that need one value per deal (market-range,
// deal-to-market) keep firstProvisionWithField below.
function provisionsWithField(provisions, dealId, provisionType, fieldPath, deal) {
  const rows = provisionsForDealAndType(provisions, dealId, provisionType);
  const seen = new Set();
  const out = [];
  for (const row of rows) {
    ...
    seen.add(row.id);
    const resolved = provisionFieldValue(row, provisionType, fieldPath, deal);
    if (resolved.value === null || resolved.value === undefined || resolved.value === '') continue;
    const ownQuote = typeof resolved.quote === 'string' && resolved.quote.trim() && resolved.quote !== row.full_text;
    out.push({ provision: row, resolved, value: resolved.value, ownQuote });
  }
  return out;
}
...
function firstProvisionWithField(provisions, dealId, provisionType, fieldPath, subtype, deal) {
  ...
  // r10 (Ben, force-the-vote query): a section-level boolean can be stamped
  // on SEVERAL provisions of the family (QXO: forceTheVote=true on both the
  // Change-of-Recommendation card AND the Rule 14d-9 disclosure carve-out) —
  // "first with any value" then anchored the hit to whichever card happened
  // to sort first, quoting an unrelated clause. Prefer the provision whose
  // resolved value carries its OWN claim-level evidence quote (result.quote
  // that isn't just the full_text fallback); fall back to first-with-value,
  // then first at all — unchanged behavior when no claim quotes exist.
  // r10b (conflict reconciliation): several fields carry CONFLICTING values
  // across cards of one family (corpus audit: antitrust effortsStandard on
  // 10 deals holds both RBE and CRE; governingLaw on 11 deals holds both
  // the main choice-of-law and the financing-suit carve-out) — "first with
  // any value" made the query's answer depend on card order. Deterministic
  // rule instead: among evidence-backed candidates (falling back to all
  // candidates when none carry claim quotes), take the MAJORITY value, and
  // anchor to that value's first evidence-backed carrier. Ties keep
  // agreement order — still deterministic, and the limitation is visible in
  // reports/query-field-conflicts.md for targeted data fixes.
  const seen = new Set();
  const candidates = [];
  for (const row of ordered) {
    ...
    const ownQuote = typeof result.quote === 'string' && result.quote.trim() && result.quote !== row.full_text;
    candidates.push({ row, value: result.value, ownQuote });
  }
  if (!candidates.length) return ordered[0] || null;
  const pool = candidates.some((c) => c.ownQuote) ? candidates.filter((c) => c.ownQuote) : candidates;
  ...
  for (const c of pool) {
    const key = JSON.stringify(c.value);
    if (counts.get(key) > best) { best = counts.get(key); majorityKey = key; }
  }
  const winner = pool.find((c) => JSON.stringify(c.value) === majorityKey);
  return winner.row;
}
...
function comparisonDeals(deals, filter) {
  ...
  return (deals || []).filter((deal) => {
    const search = Array.isArray(f.search) ? f.search[0] : f.search;
    if (search) {
      const haystack = [dealName(deal), resolveBuyerDisplay(deal), resolveTargetDisplay(deal), deal.sector]
        .filter(Boolean).join(' ').toLowerCase();
      if (!haystack.includes(String(search).trim().toLowerCase())) return false;
    }
    ...
    if (Array.isArray(f.lawyer) && f.lawyer.length) {
      const lawyers = dealLawyers(deal);
      if (!f.lawyer.some((lawyer) => lawyers.includes(String(lawyer)))) return false;
    }
    if (Array.isArray(f.merger_form) && f.merger_form.length) {
      const meta = deal.metadata && typeof deal.metadata === 'object' ? deal.metadata : {};
      const mergerForm = deal.merger_form || meta.merger_form || null;
      if (!f.merger_form.includes(mergerForm)) return false;
    }
    return true;
  });
}
...
function dealLawyers(deal) {
  const adv = getDisplayAdvisors(deal && deal.metadata);
  return [...new Set([...(adv.buyerLawyers || []), ...(adv.sellerLawyers || [])])];
}
...
function dealColumn(deal) {
  ...
  return {
    ...
    buyer: resolveBuyerDisplay(deal),
    target: resolveTargetDisplay(deal),
    signing_date: row.signing_date,
    total_deal_value: row.total_deal_value,
    sector: deal.sector || null,
    merger_form: deal.merger_form || metadata.merger_form || null,
    law_firm: dealLawFirms(deal),
    lawyer: dealLawyers(deal),
  };
}
