  ...
  for (const provisionType of provisionTypes) {
    for (const field of SCORE_FIELDS[provisionType] || []) {
      ...
      for (const peer of comparison) {
        const provision = firstProvisionWithField(context.provisions || [], peer.id, provisionType, field);
        if (!provision) continue;
        const result = provisionFieldValue(provision, provisionType, field);
      ...
      const dealProvision = deal ? firstProvisionWithField(context.provisions || [], deal.id, provisionType, field) : null;
      const dealResult = dealProvision ? provisionFieldValue(dealProvision, provisionType, field) : { value: null, quote: null };
