...
const { comparisonDeals, dealColumn, dealMetaValue, firstProvisionWithField, provisionFieldValue, buildProv, hasValue } = require('./shared');
...
function executeFilterThenList(payload, context) {
  ...
  for (const deal of deals) {
    ...
    for (const filter of payload.filters || []) {
      const provision = firstProvisionWithField(context.provisions || [], deal.id, filter.provision_type, filter.field, null, deal);
      const value = provision ? provisionFieldValue(provision, filter.provision_type, filter.field, deal).value : null;
      if (!testOp(value, filter.op, filter.value)) {
        ok = false;
        break;
      }
      const resolved = provision ? provisionFieldValue(provision, filter.provision_type, filter.field, deal) : { key: filter.field, value };
      hits.push({
        provision_type: filter.provision_type,
        card_id: provision && provision.id,
        field: resolved.key || filter.field,
        value,
        quote: provision ? hitQuote(resolved, provision, filter) : null,
        ...(hasValue(value) ? { _prov: buildProv(resolved, provision) } : {}),
      });
    }
    if (!ok) continue;
    const columns = {};
