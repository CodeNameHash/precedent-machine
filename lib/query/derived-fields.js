...
// precise is worse than no percentage.
//
// 2026-08-05, later the same day: consolidated onto lib/parse-money.js, the
// one shared implementation for what were six independent copies of this
// exact rule (see that file's header for the full backstory). One BEHAVIOR
// CHANGE falls out of that consolidation and is worth stating plainly: the
// shared parser's ambiguity rule is "$"-sign-aware (see lib/parse-money.js
// "AMBIGUITY RULE") rather than counting every bare numeral the way this
// function used to. On the real Modiv PARENT/reverse fee headline, "Lesser
// of $15,000,000 (§7.3(c)) and the REIT Requirements cap", the old any-
// numeral rule saw two numbers (15000000 and the "7.3" citation) and
// returned null; the shared parser sees one dollar figure and one unrelated
// citation and resolves to 15000000 -- matching parseFeeAmountUsd's
// (components/review/table-configs/termination-fees.config.js) already-
// shipped, already-tested behavior for this exact string, which the review
// page's termination-fees table already rendered as "0.75% of deal value"
// before this file agreed with it. See tests/derived-fields.test.js for the
// updated pins and lib/parse-money.js's header "FINDING" note for the full
// writeup. The core rule -- more than one genuine dollar figure, or a range,
// is not one clean figure -- is unchanged; this file no longer implements it.
function parseUsdAmount(value) {
  if (value == null) return null;
  if (typeof value === 'number') return Number.isFinite(value) ? value : null;
  return parseMoneyAmount(String(value));
}
