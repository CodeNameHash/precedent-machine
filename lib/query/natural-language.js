...
const DEFAULT_COMPARE_TYPES = ['CONSIDERATION', 'TERMINATION_FEE', 'COVENANT_NO_SOLICITATION'];
...
const FIELD_PHRASES = [
  ...
  {
    provisionType: 'TERMINATION_FEE', field: 'reverseFeePercentage', label: 'Reverse termination fee (% of deal value)',
...
const PREFERRED_FIELDS = {
  ...
  TERMINATION_FEE: ['companyTerminationFee', 'feePctOfDealValue', 'reverseFeeAmount', 'tailFeeWindowMonths'],
...
function summaryFor(kind, payload, catalog, deals) {
  ...
  if (kind === 'DEAL_COMPARE') {
    const names = payload.deal_ids.map((id) => {
      const deal = deals.find((candidate) => String(candidate.id) === String(id));
      return deal ? dealName(deal) : id;
    });
    return `Compare ${names.join(' · ')}`;
  }
  const deal = deals.find((candidate) => String(candidate.id) === String(payload.deal_id));
  return `${deal ? dealName(deal) : payload.deal_id} vs market`;
}
...
function validateReadyPayload(kind, payload, { deals = [], provisions = [], catalog = buildFieldCatalog() } = {}) {
  ...
  } else if (kind === 'DEAL_COMPARE') {
    const ids = resolveDealIds(payload.deal_ids, deals);
    if (ids.length < 2 || ids.length > 4) throw new IntentValidationError('DEAL_COMPARE requires 2-4 deals');
    if (!Array.isArray(payload.provision_types) || !payload.provision_types.length) throw new IntentValidationError('DEAL_COMPARE requires provision types');
    resolveProvisionTypes(payload.provision_types);
    if (typeof payload.highlight_deltas !== 'boolean' || !Array.isArray(payload.included_field_groups)) {
      throw new IntentValidationError('DEAL_COMPARE settings are incomplete');
    }
  } else if (kind === 'DEAL_TO_MARKET') {
    resolveDealIds([payload.deal_id], deals);
    if (!payload.comparison_set_filter || typeof payload.comparison_set_filter !== 'object') {
      throw new IntentValidationError('DEAL_TO_MARKET comparison set is missing');
    }
    if (payload.provision_types !== null && payload.provision_types !== undefined) resolveProvisionTypes(payload.provision_types);
  }
...
function compileIntent(intent, { deals = [], provisions = [], catalog = buildFieldCatalog() } = {}) {
  ...
  if (kind === 'MARKET_RANGE') {
    ...
    const dealIds = resolveDealIds(intent.deal_ids, deals);
    if (dealIds.length < 2 || dealIds.length > 4) return clarificationResponse('Select two to four deals to compare.');
    const provisionTypes = resolveProvisionTypes(intent.provision_types && intent.provision_types.length ? intent.provision_types : DEFAULT_COMPARE_TYPES);
    payload = {
      deal_ids: dealIds,
    ...
    const dealIds = resolveDealIds(intent.deal_ids || intent.deal_id, deals);
    if (dealIds.length !== 1) return clarificationResponse('Select one deal to compare with the market.');
    const provisionTypes = asArray(intent.provision_types).length
      ? resolveProvisionTypes(intent.provision_types)
      : null;
    payload = {
      deal_id: dealIds[0],
  ...
  return { status: 'ready', kind, payload, summary: summaryFor(kind, payload, catalog, deals) };
}
...
function inferKind(question, fieldMatches, matchedDeals) {
  ...
  if (/\b(vs|versus|against) (the )?market\b/.test(text) || /\bcompare\b.*\b(to|with) (the )?market\b/.test(text) || (/\bhow does\b/.test(text) && /\bmarket\b/.test(text))) {
    return 'DEAL_TO_MARKET';
  }
  if (matchedDeals.length >= 2 && /\b(compare|difference|differences|versus|vs|side by side)\b/.test(text)) return 'DEAL_COMPARE';
  ...
  if (matchedDeals.length >= 2) return 'DEAL_COMPARE';
...
function interpretDeterministically(questionInput, { deals = [], provisions = [], catalog = buildFieldCatalog() } = {}) {
  ...
  if (requiresRowChoice(question, catalog) && !['DEAL_COMPARE', 'DEAL_TO_MARKET'].includes(broadDealKind)) {
  ...
  if (kind === 'DEAL_COMPARE') {
    base.provision_types = provisionTypes.length ? provisionTypes : DEFAULT_COMPARE_TYPES;
  } else if (kind === 'DEAL_TO_MARKET') {
    base.provision_types = provisionTypes.length ? provisionTypes : null;
  } else if (kind === 'FILTER_THEN_LIST') {
