...
const REGISTRY_PATH = path.join(process.cwd(), 'lib/query/serving-registry-v1.json');
...
// These generated-registry alias groups merge fields with different legal
// meanings. A unique alias must not fall through to its semantic neighbour.
const DISAMBIGUATED_ALIASES = Object.freeze({
  pull_refile: Object.freeze(['pull_refile', 'pullRefile', 'rubric.anti.pull_refile']),
  clear_skies_parent: Object.freeze(['clear_skies_parent', 'clearSkiesParent', 'rubric.anti.clear_skies_parent']),
  clear_skies_company: Object.freeze(['clear_skies_company', 'clearSkiesCompany', 'rubric.anti.clear_skies_company']),
});
...
function resolveFeatureValue(key, provision) {
  ...
  // aliasesForKey(entry.key) — the serving registry has at
  ...
  const requested = String(key || '').trim();
  const aliases = DISAMBIGUATED_ALIASES[requested] || entryAliases(entry);
...
// WP-3 (M4-02) normalizer badges: `_meta.version` is copied from the source
// registry. Read through the same cached registry parse
// readRegistry() already keeps warm, so this is a per-run read, not a
// per-cell one, even though every value-bearing query cell calls it.
function registryVersion() {
  const { parsed } = readRegistry();
  return (parsed && parsed._meta && parsed._meta.version) || null;
}
