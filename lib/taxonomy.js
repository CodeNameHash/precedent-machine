...
const EXCEPTION_CODES = {
  ...
  // Per user (MetsFB2, reversing the earlier composite rule): when the
  // preamble strings together "Disclosure Letter ... or otherwise expressly
  // required by this Agreement ... or required by Law ... or with Parent's
  // consent", each carve-out is its OWN pill — the Metsera framing yields
  // FOUR exceptions, not three. COMPANY_DISCLOSURE_LETTER covers only the
  // disclosure-letter cite; REQUIRED_BY_AGREEMENT covers the
  // Agreement-required half even when they appear in the same sentence.
  ...
  // IOC monetary-threshold exceptions — frequent across litigation, capex,
  // settlements, contract caps. Splitting these out of OTHER lets the UI
  // surface the threshold numerically (individual + aggregate) instead of
  // rendering them as a flat "Other specified exception" pill.
  MONETARY_THRESHOLD: 'Below monetary threshold',
  BUDGET_EXCEPTION: 'Within budget / capex plan',
...
// ---------------------------------------------------------------------------
// Per-rep knowledge qualifier (TAX-1 survey 2026-07-18: the extractor emits
// one tagged code for this feature 243 times corpus-wide; it was never
// registered, so every claim flagged. The code asserts the rep IS
// knowledge-qualified — the qualifier's substance (standard, persons) lives
// in knowledgeScope.)
// ---------------------------------------------------------------------------

const KNOWLEDGE_QUALIFIER_CODES = {
  KNOWLEDGE_QUALIFIED: 'Knowledge-qualified',
};

// ---------------------------------------------------------------------------
// "No Undisclosed Liabilities" rep carve-outs. Canonical set curated from
// the corpus-wide TAX-1 survey (2026-07-18) + standard NUL drafting: the
// rep excepts liabilities (a) on/reserved against the balance sheet,
// (b) incurred in the ordinary course since the balance-sheet date,
// (c) incurred in connection with the transactions, (d) future performance
// under existing contracts, (e) below an MAE threshold, (f) since paid or
// discharged, (g) disclosed in SEC financial statements/notes. Model
// synonyms map via aliases at the validation layer's case-insensitive
// lookup; the near-duplicate codes observed in the wild are listed as
// their own keys pointing at the same label so existing stored claims
// resolve without rewriting.
// ---------------------------------------------------------------------------

const UNDISCLOSED_LIABILITIES_EXCEPTION_CODES = {
  BALANCE_SHEET_DISCLOSED: 'Reflected or reserved on the balance sheet',
  BALANCE_SHEET_RESERVED: 'Reflected or reserved on the balance sheet',
  ORDINARY_COURSE_SINCE_DATE: 'Incurred in the ordinary course since the balance-sheet date',
  GENERAL_ORDINARY_COURSE: 'Incurred in the ordinary course since the balance-sheet date',
  TRANSACTION_RELATED: 'Incurred in connection with the transactions',
  TRANSACTION_EXPENSES: 'Incurred in connection with the transactions',
  CONTRACT_OBLIGATIONS: 'Future performance under existing contracts',
  CONTRACTS: 'Future performance under existing contracts',
  MATERIAL_CONTRACT_PERFORMANCE: 'Future performance under existing contracts',
  MAE_THRESHOLD: 'Would not reasonably be expected to have an MAE',
  MAE: 'Would not reasonably be expected to have an MAE',
  MAE_EXCEPTION: 'Would not reasonably be expected to have an MAE',
  // LOOKUP ALIAS ONLY for stored V1 undisclosed-liabilities exception claims.
  // Surviving meaning HERE: same MAE-exception gloss as MAE / MAE_THRESHOLD /
  // MAE_EXCEPTION above. The key name collides with MATERIALITY_CODES.
  // MAT_MAE_AGGREGATE below (different dictionary, different meaning — the
  // legacy "individually or in aggregate" materiality standard). Do not
  // treat this entry as that materiality code. Step 2X-D / DECISIONS.md
  // entry 14 (2026-08-08) documented the collision; left in place so
  // existing stored exception claims still resolve without rewrite.
  MAT_MAE_AGGREGATE: 'Would not reasonably be expected to have an MAE',
  PAID_OR_DISCHARGED: 'Since paid or discharged',
  FINANCIAL_STATEMENTS: 'Disclosed in SEC financial statements or notes',
  // Codes that validated via the generic EXCEPTION_CODES tags before these
  // features got their own dictionaries — kept so stored claims resolve.
  ORDINARY_COURSE: 'Incurred in the ordinary course since the balance-sheet date',
  DISCLOSURE_SCHEDULE: 'As disclosed',
  OTHER: 'Other exception',
};

// "Absence of Changes" rep carve-outs (same survey). HYBRID observed once
// and deliberately NOT registered — it is model noise, not a carve-out.
const ABSENCE_OF_CHANGES_EXCEPTION_CODES = {
  GENERAL_ORDINARY_COURSE: 'Operations in the ordinary course',
  ORDINARY_COURSE: 'Operations in the ordinary course',
  SPECIFIED_IOCS: 'Actions permitted or required by the interim operating covenants',
  OTHER: 'Other exception',
};
...
const MATERIALITY_CODES = {
  ...
  // Legacy V1 materiality code: "individually or in the aggregate" drafting
  // of an MAE qualifier. Surviving meaning HERE: that materiality standard
  // label for stored V1 claims / review UI. RETIRED for V2 classification
  // per Ben's 2026-08-08 ruling (DECISIONS.md entry 14 / Step 2X-D) — MAE is
  // inherently aggregate, so V2 resolves both forms as MAT_MAE_QUALIFIED.
  // Name collides with UNDISCLOSED_LIABILITIES_EXCEPTION_CODES.MAT_MAE_AGGREGATE
  // above (MAE-exception lookup alias); the two dictionaries must not be
  // conflated. Do not emit this code from the V2 lexicon or admit it on
...
const EFFORTS_STANDARDS = {
  ...
  // FB3 item 2: the unqualified case — an affirmative obligation stated with
  // NO "use ... efforts to" qualifier at all (e.g. "conduct its business in
  // the ordinary course", "maintain ... in all material respects"). This is
  // the single canonical "standard" value for such a limb — it must never be
  // paired with a separate, independently-derived materiality code on the
  // same limb (that produced the "CRE and Flat on one limb" contradiction).
  FLAT: 'Flat (unqualified obligation)',
};

// ---------------------------------------------------------------------------
// Representatives standard (how the company must control its Representatives
// under the no-solicit "cease discussions" covenant) and Intervening Event
// scope (whether the Intervening Event definition covers only positive events
// or both positive and negative ones). Fix batch item 4: these enums rendered
// as bare codes (RBE_NOT_TO, POSITIVE_ONLY) on live pages because no taxonomy
// dictionary existed for them.
// ---------------------------------------------------------------------------

const REPRESENTATIVES_STANDARDS = {
  CAUSE_NOT_TO: 'Company must cause Representatives not to engage',
  RBE_NOT_TO: 'Company must use reasonable best efforts to cause Representatives not to engage',
  INSTRUCT_NOT_TO: 'Company must instruct Representatives not to engage',
  NA: 'Not applicable',
};

const INTERVENING_EVENT_SCOPES = {
  POSITIVE_ONLY: 'Positive events only (excludes Acquisition Proposal-related events)',
  BOTH: 'Both positive and negative events',
  NA: 'Not applicable',
};
...
const ANTITRUST_CONTROL = {
  ...
  BUYER_WITH_SETTLEMENT_GAG: 'Buyer controls; seller barred from offers/settlement except as directed',
  BUYER_LEAD: 'Buyer takes the lead',
  PRINCIPAL_WITH_VETO: 'Principal party controls subject to counterparty veto',
  JURISDICTION_SPLIT: 'Control split by jurisdiction',
  SELLER_LED: 'Seller / Target leads strategy',
};

const BURDEN_COMMITMENT = {
  ANTI_HOHW: 'Anti-hell-or-high-water',
  NO_LITIGATION_ONLY: 'No obligation to litigate',
  CAPPED_QUANTITATIVE: 'Divestiture cap: quantitative (revenue/EBITDA/units/NPV/formula)',
  BURDENSOME_CONDITION: 'Burdensome-condition / MAE-style cap',
  PRECEDENT_COMPARABILITY: 'Capped by precedent-deal remedies / consent decree',
  BUYER_DISCRETION: 'Subjective buyer-discretion cap',
  NAMED_ASSET_CARVEOUT: 'Named-asset carve-out / carve-in (modifier)',
  EXPRESS_HOHW: 'Hell-or-high-water (express take-all-steps)',
  SILENT_NO_CAP: 'No express limitation (efforts standard governs)',
};

const BURDEN_BASELINE = {
  TARGET_ONLY: 'Target only',
  BUYER_ONLY: 'Buyer only',
  COMBINED_ENTITY: 'Combined entity',
  SIZE_NORMALIZED: 'Target deemed buyer-sized',
  WITH_SYNERGIES: 'With synergies',
  DE_MINIMIS_FLOOR: 'De minimis floor',
};

const LITIGATION_OBLIGATION = {
  MANDATORY_DEFEND: 'Must defend (incl. appeals/final judgment)',
  RIGHT_NOT_OBLIGATION: 'Right but not obligation (elective offense / mandatory defense)',
  MERITS_GATED: 'Merits-gated',
  TIME_BOUNDED: 'Time-bounded',
  EXPRESS_NONE: 'Expressly no obligation',
  SILENT: 'Silent',
};

const CLEAR_SKIES_FAMILY = {
  RESTRAINT_IN_EFFECT: 'Fails only on restraint in effect',
  SUIT_SEEKING: 'Fails on suit merely seeking relief',
  AGENCIES_NAMED: 'Agencies named',
  US_EU: 'US / EU',
  SCHEDULED_JURISDICTIONS: 'Scheduled jurisdictions',
  ANY_AUTHORITY: 'Any authority',
  THREATENED_INCLUDED: 'Threatened included',
  MATERIALITY_GATED: 'Materiality gated',
  ANTI_DOUBLE_COUNT_CARVEOUT: 'Antitrust double-count carve-out',
};

const CONSULTATION_TIER = {
  NOTICE_CONSULT: 'Notice and consultation',
  GOOD_FAITH_VIEWS: 'Consider views in good faith',
  INCORPORATE_COMMENTS: 'Incorporate reasonable comments',
  PARTICIPATE: 'Reasonable opportunity to participate',
  CONSENT_NOT_UNREASONABLE: 'Consent not to be unreasonably withheld',
  CONSENT_SOLE: 'Consent in sole discretion',
};

const PULL_REFILE = {
  BUYER_UNILATERAL_GF: 'Buyer may pull-and-refile (good-faith gated)',
  SELLER_CONSENT_GATED: 'Seller consent gated',
  MUTUAL_CONSENT: 'Mutual consent',
  SILENT: 'Silent',
};

const TIMING_AGREEMENT = {
  BARRED_MUTUAL_CONSENT: 'Barred absent mutual consent',
  NOT_UNREASONABLY_WITHHELD: 'Consent not to be unreasonably withheld',
  BUYER_ONLY_RESTRICTED: 'Buyer only restricted',
  CAPPED_DISCRETION: 'Capped discretion',
  BUYER_OPTION_CONSULT: 'Buyer option after consultation',
  SILENT: 'Silent',
};

// ---------------------------------------------------------------------------
// Antitrust closing-condition approvals (COND-M-REG / "Antitrust" condition)
// — FB3 item 4(d): the condition bundles TWO conceptually distinct approvals
// (the HSR waiting period, and any OTHER approvals/clearances the parties
// scheduled on the disclosure letter) into one free-text field. Lawyers want
// these as two separate pills, with NO section/schedule cross-reference in
// the label (that belongs in "text" only, same discipline as TERMF triggers).
// ---------------------------------------------------------------------------

const ANTITRUST_APPROVAL_CODES = {
  HSR: 'HSR Act waiting period expired or terminated',
  SCHEDULED_APPROVALS: 'Other scheduled regulatory approvals obtained',
};
...
const MATERIAL_CONTRACT_BUCKET_META = {
  ...
  IP_LICENSES_IN: {
    ...
    synonyms: [/in-?bound\s+licens/i, /licens(?:es?|ed)\s+(?:from|in)/i],
  },
  IP_LICENSES_OUT: {
    label: 'Outbound IP licenses',
    // Same hyphen gap as IP_LICENSES_IN, mirrored: Modiv 3.17(b)(x) says
    // "out-bound licenses".
    synonyms: [/out-?bound\s+licens/i, /licens(?:es?|ed)\s+(?:to|out)/i],
  },
  SUPPLY: {
...
// ---------------------------------------------------------------------------
// IOC affirmative-covenant STANDARD — the qualifier on each affirmative
// obligation. "Flat" = no qualifier (the obligation is unconditioned),
// distinct from a real efforts standard. Covers the few common qualifiers
// IOC affirmatives carry (Material / In all material respects / Reasonable
// best efforts / Commercially reasonable efforts).
// ---------------------------------------------------------------------------

const IOC_AFFIRMATIVE_STANDARD_META = {
  FLAT: { label: 'Flat', synonyms: [/^\s*$/, /unqualified/i, /no\s+qualifier/i, /\bflat\b/i] },
  MATERIAL: { label: 'Material', synonyms: [/^material$/i, /\bmaterial\b(?!\s+respects)/i] },
  MATERIAL_RESPECTS: { label: 'Material respects', synonyms: [/material\s+respects/i, /in\s+all\s+material\s+respects/i] },
  REASONABLE_BEST_EFFORTS: { label: 'Reasonable best efforts', synonyms: [/reasonable\s+best\s+efforts?/i] },
  COMMERCIALLY_REASONABLE_EFFORTS: { label: 'Commercially reasonable efforts', synonyms: [/commercially\s+reasonable\s+efforts?/i] },
  BEST_EFFORTS: { label: 'Best efforts', synonyms: [/best\s+efforts?/i] },
  ORDINARY_COURSE: { label: 'Ordinary course of business', synonyms: [/ordinary\s+course/i] },
};

// ---------------------------------------------------------------------------
// IOC affirmative-covenant SCOPE — what each obligation "applies to". A small
// canonical set keeps the cross-deal comparison consistent (rather than free
// text per provision). Multiple codes can apply to one obligation.
// ---------------------------------------------------------------------------

const IOC_AFFIRMATIVE_SCOPE_META = {
  BUSINESS: { label: 'Business / operations', synonyms: [/\bbusiness\b/i, /\boperations?\b/i] },
  BUSINESS_ORGANIZATION: { label: 'Business organization', synonyms: [/business\s+organization/i] },
  ASSETS: { label: 'Assets', synonyms: [/\bassets?\b/i, /properties?\s+and\s+assets/i] },
  PROPERTIES: { label: 'Properties', synonyms: [/\bproperties\b/i, /\breal\s+(?:property|estate)\b/i] },
  OFFICERS_KEY_EMPLOYEES: { label: 'Officers / key employees', synonyms: [/officers?\b/i, /key\s+employees?/i] },
  CUSTOMERS: { label: 'Customers', synonyms: [/customers?/i] },
  SUPPLIERS: { label: 'Suppliers', synonyms: [/suppliers?/i] },
  EMPLOYEES: { label: 'Employees', synonyms: [/employees?(?!\s*[/-]\s*key)/i, /\bworkforce\b/i] },
  GOVERNMENTAL_ENTITIES: { label: 'Governmental entities', synonyms: [/government(?:al)?\s+(?:entit|authoriti|bod)/i, /regulators?/i] },
  LICENSORS_LICENSEES: { label: 'Licensors / licensees', synonyms: [/licensors?/i, /licensees?/i] },
  COLLABORATORS: { label: 'Collaborators / partners', synonyms: [/collaborators?/i, /partners?/i] },
  OTHER_RELATIONSHIPS: { label: 'Other business relationships', synonyms: [/business\s+relationships?/i] },
};

// ---------------------------------------------------------------------------
// Common cross-rep / IOC EXCEPTION CODES — canonical pills for the most-cited
// carve-outs ("Ordinary course", "Required by law", "Disclosed in schedule",
// etc.). Used wherever an exceptions/permittedExceptions list renders so the
// cross-deal compare reads as pills rather than free text. The OTHER_SPECIFIC
// code is the deliberate catch-all the user requested for "other specific
// exceptions" — it is itself a canonical pill.
// ---------------------------------------------------------------------------

const COMMON_EXCEPTION_META = {
  ORDINARY_COURSE: { label: 'Ordinary course of business', synonyms: [/ordinary\s+course/i] },
  ORDINARY_COURSE_PAST_PRACTICE: { label: 'Ordinary course consistent with past practice', synonyms: [/consistent\s+with\s+past\s+practice/i] },
  REQUIRED_BY_LAW: { label: 'Required by law', synonyms: [/required\s+by\s+(?:applicable\s+)?law/i, /to\s+comply\s+with\s+law/i] },
  PERMITTED_BY_LAW: { label: 'Permitted by law', synonyms: [/permitted\s+by\s+law/i] },
  DISCLOSED_IN_SCHEDULE: { label: 'Disclosed in Disclosure Letter / Schedule', synonyms: [/disclosure\s+letter/i, /\bschedule\b/i, /set\s+forth\s+in\s+section/i] },
  WITH_CONSENT: { label: 'With Parent / Buyer consent', synonyms: [/consent\s+of\s+(?:parent|buyer|acquir)/i, /written\s+consent/i] },
  CONTEMPLATED_BY_AGREEMENT: { label: 'Contemplated by this Agreement', synonyms: [/contemplated\s+by\s+(?:this\s+)?agreement/i] },
  EXISTING_CONTRACTS: { label: 'Existing contracts', synonyms: [/existing\s+contracts?/i, /required\s+by\s+(?:any\s+)?contract/i] },
  EXISTING_BENEFIT_PLANS: { label: 'Existing benefit plans', synonyms: [/benefit\s+plans?/i, /\berisa\b/i] },
  COMPLIANCE_WITH_ORDER: { label: 'Required by court order / governmental order', synonyms: [/court\s+order/i, /government(?:al)?\s+order/i] },
  PANDEMIC: { label: 'Pandemic / health emergency response', synonyms: [/pandemic/i, /covid/i, /public\s+health/i] },
  COMPANY_DISCLOSURE_LETTER: { label: 'Company Disclosure Letter', synonyms: [/company\s+disclosure\s+letter/i] },
  // Monetary-threshold and budget carve-outs — per user feedback, exceptions
  // that are fundamentally dollar caps (litigation settlements, capex) should
  // read as their own category rather than collapsing into "Other specific".
  // The synonyms match BOTH the canonical UPPER_SNAKE codes the extractor now
  // emits AND the free-text / label form of legacy data (a dollar figure, or
  // "in the aggregate" / "individually", or budget references) so the pills
  // improve even before a re-extract.
  MONETARY_THRESHOLD: { label: 'Below monetary threshold', synonyms: [/\$[\d,]+/, /monetary\s+(?:threshold|obligation|amount|relief)/i, /individually\s+or\s+in\s+the\s+aggregate/i, /in\s+the\s+aggregate\b/i, /de\s+minimis/i] },
  BUDGET_EXCEPTION: { label: 'Within budget / capex plan', synonyms: [/\bbudget\b/i, /capital\s+expenditure\s+plan/i, /capex\s+plan/i] },
  OTHER_SPECIFIC: { label: 'Other specific exceptions', synonyms: [] },
};
...
// ---------------------------------------------------------------------------
// Termination-fee triggers — canonical codes for WHEN a fee becomes payable.
// The label is the PLAIN-ENGLISH default a partner would say out loud, NOT a
// section cross-reference. Codes are stable cross-deal identifiers: add new
// ones, never rename existing ones.
// ---------------------------------------------------------------------------

const TERMF_TRIGGER_META = {
  SUPERIOR_PROPOSAL: {
    label: 'Company terminates to accept a Superior Proposal',
    synonyms: [/superior\s+proposal/i],
  },
  RECOMMENDATION_CHANGE: {
    label: 'Parent terminates after the Board changes its recommendation',
    synonyms: [/recommendation\s+change/i, /change\s+(?:in|of)\s+recommendation/i, /adverse\s+recommendation/i],
  },
  NO_SOLICIT_BREACH: {
    label: 'Company breaches the no-shop / non-solicitation covenant',
    synonyms: [/breach\s+of\s+(?:the\s+)?(?:no[\s-]*shop|no[\s-]*solicit\w*|non[\s-]*solicit\w*)/i, /no[\s-]*solicit\w*\s+breach/i],
  },
  NAKED_NO_VOTE: {
    label: 'Stockholders vote the deal down with no competing proposal on the table',
    synonyms: [/naked\s+no[\s-]*vote/i],
  },
  TAIL: {
    label: 'Deal terminated, and the Company signs or closes an alternative deal within the tail period',
    synonyms: [/\btail\b/i, /subsequent\s+transaction/i, /within\s+(?:twelve|\d+)\s+months\s+(?:of|after|following)\s+(?:such\s+)?termination/i],
  },
  NO_VOTE: {
    label: 'Stockholders fail to approve the deal',
    synonyms: [/no[\s-]*vote/i, /stockholder\s+approval\s+(?:is\s+)?not\s+obtained/i, /fail\w*\s+to\s+(?:approve|obtain\s+the\s+\w*\s*approval)/i],
  },
  OUTSIDE_DATE: {
    label: 'Deal not closed by the outside date',
    synonyms: [/outside\s+date/i, /\bend\s+date\b/i, /\btermination\s+date\b/i],
  },
  COMPANY_BREACH: {
    label: "Parent terminates for the Company's uncured breach",
    synonyms: [/breach\s+by\s+(?:the\s+)?(?:company|target)/i, /(?:company|target)(?:'|’)?s?\s+(?:uncured\s+)?breach/i],
  },
  PARENT_BREACH: {
    label: "Company terminates for Parent's uncured breach",
    synonyms: [/breach\s+by\s+(?:the\s+)?(?:parent|buyer|acquir\w+)/i, /(?:parent|buyer|acquir\w+)(?:'|’)?s?\s+(?:uncured\s+)?breach/i],
  },
  ANTITRUST_FAILURE: {
    label: 'Antitrust / regulatory approval not obtained',
    synonyms: [/antitrust/i, /regulatory\s+(?:approval|clearance)/i, /\bhsr\b/i],
  },
  FINANCING_FAILURE: {
    label: 'Parent fails to close when required / financing not funded',
    synonyms: [/financing\s+fail/i, /fail\w*\s+to\s+(?:close|consummate|fund)/i],
  },
  OTHER: { label: 'Other trigger', synonyms: [] },
};
...
// ── Stage 4: canonical layer, Section B ────────────────────────────────────
// New enums seeded from the whole-corpus mining pass (40 deals). These are the
// canonical VALUES for concepts previously summarized by deal-specific regex at
// render time. Labels here are the reviewer-facing DISPLAY labels. Scalars that
// accompany these (interest spread, superior-proposal %, notice deadlines,
// tail windows) stay as separate numeric riders -- never enumerated.

// Interest-rate basis on a late termination-fee payment (concept 16). The
// spread (+2%, +5%) and the published source are scalar/entity riders.
const INTEREST_RATE_BASIS = {
  PRIME_WSJ: 'Prime rate (as published in The Wall Street Journal)',
  PRIME_BANK: 'Prime rate (a named bank)',
  US_TREASURY_6MO: '6-month U.S. Treasury rate',
  LIBOR: '3-month LIBOR',
  FIXED: 'Fixed rate per annum',
};

// Governing-law state (concept 17). SPLIT_FINANCING = primary state, but New
// York governs Financing-Source claims.
const GOVERNING_LAW = {
  DELAWARE: 'Delaware',
  NEW_YORK: 'New York',
  MARYLAND: 'Maryland',
  SPLIT_FINANCING: 'Split (primary state; New York for Financing-Source claims)',
  FOREIGN: 'Non-U.S.',
};

// Shared solicitation-act vocabulary (concepts 3 + 4). ONE code set for the
// acts a no-solicitation clause prohibits AND the acts a change-of-
// recommendation clause forbids, because the overlapping ones (enter an
// alternative agreement / approve-endorse a proposal / resolve to do the
// foregoing) appear in BOTH -- a single vocabulary keyed by context prevents
// the two lists drifting apart. The `context ∈ {nosol_prohibit, nosol_cease,
// cor}` discriminator lives on each list item, not in the code. Scalar riders
// (reaffirmation deadline, return-information look-back) stay separate.
const SOLICITATION_ACT = {
  // Prohibited acts (no-solicitation)
  SOLICIT_INITIATE: 'Solicit / initiate / knowingly encourage or facilitate an Acquisition Proposal',
  DISCUSS_NEGOTIATE: 'Engage in or continue discussions or negotiations',
  FURNISH_INFORMATION: 'Furnish non-public information / afford diligence access',
  ENTER_ALT_AGREEMENT: 'Enter into an LOI / acquisition / merger agreement (other than an ACA)',
  APPROVE_ENDORSE: 'Approve / endorse / recommend / declare advisable a proposal',
  WAIVE_STANDSTILL: 'Waive / terminate / modify or fail to enforce a standstill',
  SUBMIT_TO_VOTE: 'Submit a competing proposal to the stockholder vote',
  RESOLVE_AGREE_FOREGOING: 'Resolve / agree / authorize / publicly propose to do any of the foregoing',
  // Cease obligations (affirmative)
  CEASE_EXISTING_ACTIVITY: 'Immediately cease existing solicitations / discussions / negotiations',
  TERMINATE_DATAROOM: 'Terminate physical / electronic data-room access',
  REQUEST_RETURN_DESTROY: 'Request return or destruction of previously furnished information',
  // Change-of-recommendation acts
  WITHDRAW_QUALIFY_REC: 'Withhold / withdraw / qualify / modify the Board Recommendation adverse to Parent',
  FAIL_TO_INCLUDE_REC: 'Fail to include the Board Recommendation in the Proxy / 14D-9 / Info Statement',
  FAIL_TO_REAFFIRM: 'Fail to publicly reaffirm the Recommendation on request',
  TENDER_OFFER_ACTION: 'Fail to recommend against a tender / exchange offer within the required period',
};

// Who decides a Superior Proposal, and with what advice (concept 10). The
// qualifying percentage is a separate scalar.
const SUPERIOR_DETERMINER = {
  BOARD_LEGAL_AND_FINANCIAL: 'Board, in good faith, after outside legal counsel and financial advisor',
  BOARD_LEGAL_ONLY: 'Board, after outside legal counsel only',
  BOARD_ONLY: 'Board’s good-faith judgment (no advisor reference)',
  COMMITTEE: 'A committee (Special / Strategic Review) decides',
};

// Force-the-Vote codebook (feature key `forceTheVoteType`, NOSOL). A
// force-the-vote (FTV) clause makes the company's obligation to submit the
// agreement to its stockholders SURVIVE an Adverse Recommendation Change
// (ARC): the board may flip its recommendation, but the meeting still
// happens and the stockholders still vote on the signed deal.
//
// The HARD/SOFT discriminator is NOT the ARC itself (both hard and soft FTV
// allow the board to change its recommendation). It is the existence of a
// COMPANY SELF-TERMINATION RIGHT FOR A SUPERIOR PROPOSAL — a right,
// anywhere in the agreement (typically the termination article, often
// cross-referenced from the no-shop as "unless this Agreement has been
// validly terminated"), for the COMPANY itself to terminate pre-vote and
// enter an Alternative Acquisition Agreement (paying the termination fee).
// If that escape valve exists, the FTV is soft: the vote covenant binds
// only while the agreement is alive, and the company can moot the vote by
// terminating for the better deal. If no such right exists, the FTV is
// hard: the target can NEVER escape the vote by walking away for a
// competing bid — the only path to the competing deal runs through a
// failed vote.
//
// FTV_NONE covers the ordinary "shall submit this Agreement to its
// stockholders for approval" covenant with NO ARC-override language tied
// to it. That boilerplate is near-universal and is NOT force-the-vote —
// only the explicit vote-survives-ARC construction counts (e.g.
// "Notwithstanding any Adverse Recommendation Change ... shall nonetheless
// submit ..." / "a Change in the Company Recommendation shall not limit
// the Company's obligation to submit this Agreement ...").
// STRENGTH only — HARD (no self-termination-for-Superior escape) vs SOFT (escape
// exists) vs NONE. Force-the-vote is a PER-PARTY posture: the *party* whose vote is
// locked is NOT baked into this code. Party is carried on the provision row (the
// Company- vs Parent-scoped NOSOL row convention, surfaced via `appliesToParty`).
// A deal where both sides are force-voted is TWO rows, each independently HARD/SOFT
// — which a single per-deal code could not express (e.g. buyer HARD + seller SOFT).
const FORCE_THE_VOTE = {
  FTV_HARD: 'Hard force-the-vote — the obligated party\'s stockholder vote survives an Adverse Recommendation Change AND that party has no self-termination right to accept a Superior Proposal (cannot escape the vote by walking away for a competing deal)',
  FTV_SOFT: 'Soft force-the-vote — the obligated party\'s vote survives an Adverse Recommendation Change but that party may terminate to accept a Superior Proposal',
  FTV_NONE: 'No force-the-vote (no vote-survives-recommendation-change covenant)',
};

// Assignment: Parent-side exceptions to the no-assignment baseline (concept 14).
// Intervening Event — grade of the intervening-event fiduciary out
// (Ben-decided codebook, 2026-07-20). The question the code answers: does
// the DEFINITION of "Intervening Event" allow a change of recommendation
// only for POSITIVE developments (events making the company MORE valuable
// — the classic narrow shape, e.g. "any positive material event or
// development"), or for positive OR negative material developments (the
// neutral "any material event, development or change ... not known or
// reasonably foreseeable" shape), or is there no intervening-event concept
// at all. The carve-out list (what can NEVER constitute an Intervening
// Event) is coded separately as interveningEventExceptions via
// INTERVENING_EVENT_EXCEPTION_META below — exceptions are NOT baked into
// this grade.
//
// NOTE vs the legacy INTERVENING_EVENT_SCOPES dict above: its POSITIVE_ONLY
// code conflated "excludes Acquisition Proposal-related events" (a
// near-universal carve-out) with genuine positive-only drafting, so the
// stored interveningEventScope stamps read POSITIVE_ONLY on decks whose
// definitions are actually direction-neutral. This dict supersedes it for
// coding; the old dict stays for pre-existing stored values.
const INTERVENING_EVENT_TYPE = {
  IE_POSITIVE_ONLY: 'Positive developments only — the definition is expressly limited to positive/value-increasing events ("any positive material event or development"); negative developments cannot support a recommendation change',
  IE_POSITIVE_OR_NEGATIVE: 'Positive or negative developments — the definition covers any material unknown/unforeseeable event or development, without a positivity limiter',
  IE_NONE: 'No intervening event concept — the agreement has no intervening-event fiduciary out (recommendation change, if any, is tied only to a Superior Proposal)',
};

// Intervening Event exceptions — the excluded-event list ("in no event
// shall ... constitute an Intervening Event"), rendered as a carve-out pill
// list like the MAE carve-outs. Vocabulary derived from the 11 corpus decks
// with stored definitions (2026-07-20 corpus pass) — nothing speculative:
// a "general market/economic conditions" exclusion, though plausible, does
// NOT appear in the corpus and is deliberately absent. The common
// "underlying cause may be considered / taken into account" carveback rides
// the market-metric exclusions in 8 of 11 decks; it is a proviso, not an
// excluded event, and is not tagged.
const INTERVENING_EVENT_EXCEPTION_META = {
  ACQUISITION_PROPOSAL_RELATED: {
    label: 'Receipt, existence, or terms of an Acquisition Proposal (or related matters/consequences)',
    synonyms: [
      /receipt,?\s+existence\s+(?:of\s+)?or\s+terms\s+of\s+(?:a|an|any)\s+(?:company\s+)?(?:acquisition|takeover|superior)\s+proposal/i,
      /(?:acquisition|takeover)\s+proposal\s+or\s+any\s+(?:matter|inquiry)\s+relating\s+thereto/i,
      /excluding\s+any\s+(?:company\s+)?takeover\s+proposal/i,
      /does\s+not\s+relate\s+to\s+.{0,30}(?:acquisition|takeover)\s+proposal/i,
    ],
  },
  MEETS_EXCEEDS_PROJECTIONS: {
    label: 'Meeting or exceeding projections, forecasts, guidance, milestones, or budgets',
    synonyms: [
      /meets\s+or\s+exceeds\s+any\s+internal\s+or\s+publi(?:c|shed)/i,
      /(?:projections|forecasts|estimates|predictions|guidance|milestones|budgets)/i,
    ],
  },
  STOCK_PRICE_CHANGE: {
    label: 'Changes in stock price or trading volume',
    synonyms: [
      /changes?\s+in\s+the\s+(?:market\s+|stock\s+)?price\s+or\s+trading\s+volume/i,
      /changes?\s+in\s+the\s+stock\s+price/i,
    ],
  },
  CREDIT_RATING_CHANGE: {
    label: 'Changes in credit rating',
    synonyms: [/credit\s+rating/i],
  },
  AGREEMENT_COMPLIANCE_ACTIONS: {
    label: 'Actions taken pursuant to and in compliance with the Agreement (and their consequences)',
    synonyms: [/action\s+taken\s+by\s+either\s+party\s+pursuant\s+to\s+and\s+in\s+compliance\s+with/i],
  },
};

// Information Sharing — the multi-field codebook (Ben-decided, 2026-07-20)
// coding the NOSOL notice/disclosure provision's information-flow terms.
// Five independent fields, not a single grade:
//   informationSharing (primary grade)  — what the Company must hand over
//     about an inbound proposal: full terms + copies of bid documents, terms
//     described without documents, or bare notice a proposal arrived.
//   copiesScope   — when copies ARE required (TERMS_AND_COPIES), whether
//     they must be unredacted, the agreement is silent on redaction, or the
//     bidder may redact its own confidential terms.
//   bidderIdentity — whether the bidder's identity must be disclosed at
//     receipt, is carved out (typically by a pre-signing NDA/confidentiality
//     agreement already in effect), or is not required at receipt.
//   ongoingUpdates — the standard governing the Company's continuing duty to
//     keep Parent apprised as the competing process develops.
//   engagementNotice — whether the Company must notify Parent BEFORE it
//     starts providing information/engaging with the bidder (advance), only
//     AFTER it has already decided to engage (prompt-after), or not at all.
// NOTICE_ONLY is a deliberate floor value: as of the 2026-07 corpus pass, no
// deck in the corpus codes to it — every deck with a notice/disclosure
// provision requires at least a description of material terms. Kept in the
// dict because a bare "notice a proposal arrived" clause is a recognized
// market floor, not because any deck currently occupies it.
const INFORMATION_SHARING_GRADE = {
  TERMS_AND_COPIES: 'Terms + copies of bid documents',
  TERMS_ONLY: 'Material terms described, no documents',
  NOTICE_ONLY: 'Bare notice a proposal arrived',
};

const COPIES_SCOPE = {
  UNREDACTED: 'Unredacted copies',
  UNSPECIFIED: 'Copies (redaction silent)',
  REDACTION_PERMITTED: 'Copies, bidder-confidential redactions allowed',
};

const BIDDER_IDENTITY_REQUIREMENT = {
  REQUIRED: 'Identity required',
  CARVEOUT: 'Identity unless pre-signing NDA blocks',
  NOT_REQUIRED: 'Identity not required at receipt',
};

// financingDocsIncluded rides the pre-existing infoRequiredFinancingPapers
// boolean (lib/schema/features.js) — not duplicated here.
const ONGOING_UPDATES_STANDARD = {
  DEADLINE: 'Updates due within a fixed deadline of each material development',
  REASONABLY_INFORMED: 'Company must keep Parent reasonably informed on a current basis',
  ON_REQUEST: 'Updates provided upon Parent\'s request',
  NONE: 'No ongoing-update obligation',
};

const ENGAGEMENT_NOTICE_TIMING = {
  ADVANCE: 'Advance notice required before the Company begins providing information or engaging with the bidder',
  PROMPT_AFTER: 'Notice required promptly (typically within 24-48 hours) after the Company has already begun engaging',
  NONE: 'No separate notice of the decision to engage',
};

// Appraisal Rights Status (Ben-decided codebook, 2026-07-20) — what THIS
// AGREEMENT says about appraisal/dissenters' rights, coded onto the deal's
// CONSID-DISSENT provision (or, where the drafting embeds the appraisal
// clause inside the share-conversion mechanics rather than a standalone
// section — e.g. Heinz/Kraft Sec. 2.01(c) — the CONSID-CONVERT/CONSID-
// EXCHANGE provision carrying that text).
//
// AVAILABLE requires affirmative provision-FOR language: dissenting-shares
// carve-out mechanics, a defined "Dissenting Shares"/"Appraisal Shares"
// term, and/or express Section 262 (DGCL) / Section 238 (Cayman) / Chapter
// 10 Subch. H (TBOC) / MGCL procedures. NOT_AVAILABLE requires an express
// negative statement ("No appraisal rights shall be available ...") — Ben's
// two anchor examples are Heinz/Kraft Sec. 2.01(c) ("No appraisal rights
// shall be available to the holders of Kraft Common Stock in connection
// with the Merger") and ENDRA/Renergen Sec. 3.5 ("No appraisal rights shall
// be available with respect to the Merger Sub Units or the Company Units").
// SILENT means the corpus pass found no appraisal/dissent language
// anywhere in the stored provisions AND the deal structure does not make
// appraisal near-certain (e.g., an asset purchase, where DGCL Sec. 262
// does not customarily attach the way it does to a stock-for-stock or cash
// merger of a Delaware corporation).
//
// IMPORTANT — this codebook captures what the AGREEMENT says, not the
// statutory answer. Delaware's Sec. 262(b)(2) "market-out" exception can
// eliminate STATUTORY appraisal for stock-for-stock mergers of listed
// companies even where the agreement's own dissenting-shares mechanics
// (coded AVAILABLE here) are boilerplate that would only matter if the
// market-out exception's conditions are not met at closing (e.g., a cash
// component, appraisal properly demanded and perfected, etc.). Do not
// infer the statutory answer from this field, and do not infer this field
// from the statutory answer — read the agreement.
const APPRAISAL_RIGHTS_STATUS = {
  AVAILABLE: 'Appraisal rights available',
  NOT_AVAILABLE: 'Agreement states no appraisal rights',
  SILENT: 'Agreement silent — statutory default governs',
};

const ASSIGNMENT_PARENT_EXCEPTION = {
  AFFILIATE_SUBSIDIARY: 'Parent may assign to a wholly-owned subsidiary / affiliate',
  FINANCING_SOURCE_PLEDGE: 'Parent may collaterally assign / pledge to financing sources',
  POST_CLOSING_SUCCESSOR: 'Assignment permitted from and after the Effective Time (successor)',
};
const ASSIGNMENT_PROVISO = {
  NO_RELEASE: 'Assignment does not relieve Parent of its obligations',
  NO_DELAY_IMPEDE: 'Assignment must not prevent / impede / delay the closing',
};
...
const TERMF_TRIGGER_CODES = metaToDict(TERMF_TRIGGER_META);
...
const IOC_AFFIRMATIVE_STANDARDS = metaToDict(IOC_AFFIRMATIVE_STANDARD_META);
const IOC_AFFIRMATIVE_SCOPE_CODES = metaToDict(IOC_AFFIRMATIVE_SCOPE_META);
const COMMON_EXCEPTION_CODES = metaToDict(COMMON_EXCEPTION_META);
const INTERVENING_EVENT_EXCEPTION_CODES = metaToDict(INTERVENING_EVENT_EXCEPTION_META);
...
function isValidTaxonomyCode(code, dict) {
  ...
  // A feature key with no registered dictionary (taxonomyForFeatureKey ->
  // null) must degrade to "unknown code", never throw: a TypeError here
  // propagates up through section builders and silently blanks the entire
  // section (2026-07-19 pre-demo audit finding B1 — three deals lost their
  // whole No-Solicitation section via ceaseDiscussionsExceptions).
...
function taxonomyForFeatureKey(featureKey) {
  switch (featureKey) {
    ...
    case 'representativesStandard':
    case 'ceaseDiscussionsAffiliateStandard':
      return REPRESENTATIVES_STANDARDS;
    case 'interveningEventScope':
      return INTERVENING_EVENT_SCOPES;
    case 'appliesToParty':
      return APPLIES_TO_PARTY;
    case 'controllingParty':
    case 'regulatoryStrategyControlTagged':
      return ANTITRUST_CONTROL;
    // 'burdenCommitment' was renamed to 'divestitureCapDescription' during
    // Phase 0-C (docs/schema-shape/reconciliation-log.jsonl); keep the old
    // key alongside the new one so pre-rename extraction data (still present
    // in provenance.feature_key on older triples) resolves too.
    case 'burdenCommitment':
    case 'divestitureCapDescription':
      return BURDEN_COMMITMENT;
    case 'burdenBaseline':
      return BURDEN_BASELINE;
    case 'litigationObligation':
      return LITIGATION_OBLIGATION;
    // 'consultationTier' -> 'regulatoryCooperationScope'.
    case 'consultationTier':
    case 'regulatoryCooperationScope':
      return CONSULTATION_TIER;
    case 'pullRefile':
      return PULL_REFILE;
    // 'timingAgreement' -> 'timingAgreementsProhibited'.
    case 'timingAgreement':
    case 'timingAgreementsProhibited':
      return TIMING_AGREEMENT;
    case 'partyWhoCanTerminate':
      return TERMINATION_PARTY;
    case 'knowledgeQualifier':
      return KNOWLEDGE_QUALIFIER_CODES;
    case 'undisclosedLiabilitiesExceptions':
      return UNDISCLOSED_LIABILITIES_EXCEPTION_CODES;
    case 'absenceOfChangesExceptions':
      return ABSENCE_OF_CHANGES_EXCEPTION_CODES;
    // cvrMilestonePayments is deliberately NOT registered: the
    // consideration-hero config drops code-only milestone items rather than
    // rendering a generic label (pinned by tests) — registering the code
    // would flip that behavior. Its ~5 flags stay visible.
    ...
    case 'iocAffirmativeStandard':
      return IOC_AFFIRMATIVE_STANDARDS;
    case 'iocAffirmativeScope':
    case 'appliesTo':
      return IOC_AFFIRMATIVE_SCOPE_CODES;
    case 'antitrustApprovals':
      return ANTITRUST_APPROVAL_CODES;
    // ── Stage 4 (canonical layer, Section B) ──
    case 'interestRateBasis':
      return INTEREST_RATE_BASIS;
    case 'governingLaw':
      return GOVERNING_LAW;
    case 'ceaseDiscussionsProhibitedList':
    case 'changeOfRecommendationItems':
      return SOLICITATION_ACT;
    case 'superiorProposalDeterminer':
      return SUPERIOR_DETERMINER;
    case 'forceTheVoteType':
      return FORCE_THE_VOTE;
    case 'interveningEventType':
      return INTERVENING_EVENT_TYPE;
    case 'interveningEventExceptions':
      return INTERVENING_EVENT_EXCEPTION_CODES;
    case 'informationSharing':
      return INFORMATION_SHARING_GRADE;
    case 'copiesScope':
      return COPIES_SCOPE;
    case 'bidderIdentity':
      return BIDDER_IDENTITY_REQUIREMENT;
    case 'ongoingUpdates':
      return ONGOING_UPDATES_STANDARD;
    case 'engagementNotice':
      return ENGAGEMENT_NOTICE_TIMING;
    case 'appraisalRightsStatus':
      return APPRAISAL_RIGHTS_STATUS;
    case 'parentAssignmentConditions':
      return ASSIGNMENT_PARENT_EXCEPTION;
    case 'assignmentProvisos':
      return ASSIGNMENT_PROVISO;
    default:
      return null;
  }
}
...
const LIST_TAXONOMY_KEYS = new Set([
  ...
  'antitrustApprovals',
  // Stage 4 (canonical layer, Section B): LIST features
...
module.exports = {
  ...
  SUPERIOR_DETERMINER,
  FORCE_THE_VOTE,
  INTERVENING_EVENT_TYPE,
  INTERVENING_EVENT_EXCEPTION_META,
  INTERVENING_EVENT_EXCEPTION_CODES,
  ...
  CONSENT_STANDARDS,
  EFFORTS_STANDARDS,
  REPRESENTATIVES_STANDARDS,
  INTERVENING_EVENT_SCOPES,
  APPLIES_TO_PARTY,
  ANTITRUST_CONTROL,
  BURDEN_COMMITMENT,
  BURDEN_BASELINE,
  ...
  TERMINATION_PARTY,
  EQUITY_INSTRUMENTS,
  KNOWLEDGE_QUALIFIER_CODES,
  UNDISCLOSED_LIABILITIES_EXCEPTION_CODES,
  ABSENCE_OF_CHANGES_EXCEPTION_CODES,
  EQUITY_TREATMENT,
  VESTING_STATUS,
  COMP_STANDARDS,
  COMP_ITEMS,
