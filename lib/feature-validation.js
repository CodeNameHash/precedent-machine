...
function isTaggedObject(v) {
  return isPlainObject(v) && typeof v.code === 'string' && v.code.trim() !== '';
}
...
function taxonomyHasCode(key, code) {
  const dict = taxonomyDict(key);
  if (!dict || !code) return false;
  return Object.prototype.hasOwnProperty.call(dict, String(code).trim().toUpperCase());
}

function enumLikeScalar(entry, raw) {
  const v = unwrap(raw);
  if (isTaggedObject(v)) return String(v.code).trim().toUpperCase();
...
function isTaxonomyTagged(entry, raw) {
  const v = unwrap(raw);
  return isTaggedObject(v) && taxonomyHasCode(entry.key, v.code);
}

function checkDeclaredType(entry, raw) {
  ...
  switch (declared) {
    ...
    case 'duration':
    case 'number':
      if (declared === 'text' && isTaxonomyTagged(entry, raw)) return null;
    ...
    case 'enum':
      if (isTaxonomyTagged(entry, raw)) return null;
      if (isPlainObject(v) || Array.isArray(v)) {
    ...
    case 'tagged':
      if (isTaggedObject(v)) return null;
      if (typeof v === 'string' && taxonomyHasCode(entry.key, v)) {
        return { kind: 'stringly-tagged', severity: 'warn', detail: `tagged declared, got code string "${v}"` };
      }
      return { kind: 'shape', severity: 'error', detail: `tagged declared, got ${Array.isArray(v) ? 'array' : typeof v}` };
    case 'object':
...
function validateFeatures(provisionType, features) {
  ...
  for (const [key, raw] of Object.entries(features)) {
    ...
    const typeViolation = checkDeclaredType(entry, raw);
    if (typeViolation) push(typeViolation, key);
    if (entry.type === 'enum' && Array.isArray(entry.options)) {
      const v = enumLikeScalar(entry, raw);
      if (typeof v === 'string' && v && !entry.options.includes(v) && !taxonomyHasCode(entry.key, v)) {
