...
function numericValue(raw) {
  ...
  if (typeof inner === 'string') {
    ...
    // A string naming more than one number is not one clean figure -- e.g. a
    // branch-conditional headline ("the lesser of $10,000,000 and a cap") or
    // a range ("30-45 days"). Taking the first would silently plot a
    // cross-deal distribution point, or flag a deal as a market outlier, off
    // an arbitrary substring: a wrong number that looks precise is worse
    // than no number (same principle as parseFeeAmountUsd in
    // components/review/table-configs/termination-fees.config.js, fixed for
    // the identical defect). Zero matches is the existing "nothing to parse"
    // case and also yields null, unchanged from before.
