...
function projectClaimSlice(route, familyId, resolution, provisionId, dealId, claimRevisionId) {
  ...
  try {
    projection = route.projectionArgs === 'RESOLVED_ENTRIES'
      ? project({ resolved_entries: slice.resolved })
      : project({ resolution: slice, deal_id: dealId });
  } catch (error) {
...
// defect. This returns every row the slice renders and marks which one the
// claim's own definition key matches, rather than guessing or failing.
function previewClaimSection({ run, resolved_entry: resolvedEntry, claim_revision_id: claimRevisionId, deal_id: dealId, deal = {}, scope = 'CLAIM' } = {}) {
  ...
  // Build the view from the CLAIM's own card, not the whole provision's cards.
  //
  // One provision commonly yields many claims -- a single "Material Contracts"
  // section lists a dozen buckets, each its own claim -- and the projection
  // emits a card per bucket. Rendering all of them answers "what does this
  // SECTION show", which is a different question and reads as though unrelated
  // provisions had bled into each other. `scope: 'PROVISION'` keeps the old
  // behaviour for callers that want the section view.
  const scopedCards = scope === 'PROVISION' ? projection.cards : cards;
  const reviewDeal = toReviewDeal(effectiveDealId, scopedCards);
  ...
  const claimKeyNormal = normalise(claimKey);
  const renderedRows = rows.map((row) => {
    ...
    return deepFreeze({
      ...
      matches_claim_key: Boolean(claimKeyNormal) && normalise(rowId).includes(claimKeyNormal),
      cells,
    });
  });
  return deepFreeze({
