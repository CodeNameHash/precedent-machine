...
// defect. This returns every row the slice renders and marks the row selected
// by the route's governed key. Material Contracts uses the exact bucket code;
// other routes retain their definition-key match.
  ...
  const renderedRows = rows.map((row) => {
    ...
    return deepFreeze({
      ...
      matches_claim_key: route.rowMatch === 'BUCKET_CODE'
        ...
        : Boolean(claimKeyNormal) && normalise(rowId).includes(claimKeyNormal),
      cells,
    });
  });
  if (route.rowMatch === 'BUCKET_CODE'
