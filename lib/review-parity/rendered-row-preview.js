...
function previewClaimSection({ run, resolved_entry: resolvedEntry, claim_revision_id: claimRevisionId, deal_id: dealId, deal = {} } = {}) {
  if (!run?.manifest || !run?.resolution || !Array.isArray(run.resolution.resolved)) {
    fail('RUN_INVALID', 'run must contain manifest and resolution.resolved.');
  }
  const familyId = run.manifest.section_family;
  ...
  const reviewDeal = toReviewDeal(effectiveDealId, projection.cards);
