...
function resolveSectionReference(sectionRef, allProvisions) {
  ...
  if (match) {
    ...
    if (cat) {
      // E (truncation sweep): drop the literal "…" -- this bracketed
      // category tag is a supplementary label on the section-ref chip, not
      // the primary text; cut cleanly rather than dangle an ellipsis.
      const short = cat.length > 60 ? cat.slice(0, 57).trim() : cat;
      label = `${label} [${short}]`;
    }
  }
