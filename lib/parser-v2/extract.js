...
async function strategyA(sections, client) {
  ...
  const tasks = Object.entries(byType).map(([typeKey, typeSections]) => async () => {
    ...
    for (const p of preambles) {
      ...
      // For IOC: emit "Affirmative Covenants" (with limbs as features) and
      // "General Exceptions" as separate provisions. Skip the legacy
      // "General / Preamble" so the UI shows the two new buckets only.
      if (typeKey === 'IOC' && split && split.consolidated && split.consolidated.affirmativeCovenants) {
        const aff = split.consolidated.affirmativeCovenants;
        const affProv = makeProvision({
          type: 'IOC',
          code: null,
          ...
          favorability: 'neutral',
          features: { affirmativeLimbs: aff.limbs },
          relatedDefinitions: findRelatedDefinitions(aff.text),
          isNewCode: true,
          proposedCode: aff.key,
          proposedLabel: aff.category,
        });
        preambleProvisions.push(affProv);
        provisions.push(affProv);

        if (split.consolidated.generalExceptions) {
          ...
          const gexProv = makeProvision({
            ...
            isNewCode: true,
            proposedCode: gex.key,
            proposedLabel: gex.category,
          });
...
function expandConsidEquityByInstrument(provisions) {
  ...
  for (let idx = provisions.length - 1; idx >= 0; idx--) {
    ...
    const rows = insts.map((inst, i) => {
      const treatment = treatments[i] || null;
      const rowFeatures = {
        ...f,
