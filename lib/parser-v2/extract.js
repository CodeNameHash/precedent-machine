...
- parentRemedyObligation (tagged from REMEDY_TYPES): What remedies Parent must accept (DIVEST / CONDUCT / LITIGATE / MULTIPLE / NONE). Emit as { code, label, text } from REMEDY_TYPES.
...
PAIRING RULE: Never set nakedNoVoteFeePresent=true without filling nakedNoVoteFeeAmount.
`;
  } else if (typeKey === 'DEF') {
    typeSpecific = `
...
- fundamentalRep (boolean, clause-scope): true if THIS rep is a "fundamental rep" (typically Organization, Authority, Capitalization, Brokers — anything that survives indefinitely / is bring-down at "true in all respects").
...
Field semantics:
- "mergerForm" (STRUCT-MERGER) — ONE short canonical phrase, e.g. "Reverse triangular merger", "Forward triangular merger", "Direct merger", "Two-step tender". Do NOT write a paragraph.
...
`;
  ...
  const citationRule = anyCitable
    ? `\nCITATION RULE: For any field whose type is described as "object { value, text }", ALWAYS include the verbatim quote that supports the value in the "text" field. Copy the agreement text directly — do NOT paraphrase. Keep the quote to one or two sentences. The "text" inside { value, text } is the EVIDENCE QUOTE that proves the value; any companion *Scope / *Details / *Conditions / *Language field describes the OBLIGATION and is separate (do not duplicate). If you cannot find a supporting quote, set the entire field to null instead of emitting an empty text.\n`
    : '';
...
function findInlineDefinitions(sectionText) {
  ...
  // Anchors:
  //   - Start of line, OR after a paragraph marker like "(i)", "(a)", "(1)",
  //     OR after a sentence-ending ". ".
  //   - Term is 1-6 Title-Case words (each beginning with a capital letter),
  //     length 4-80 chars total.
  //   - Term is immediately followed by " means" / " shall mean" /
  //     " has the meaning" / " shall have the meaning".
  // The look-behind has to be done in code (not regex) for portability.
  const unquotedPattern =
    /(^|\n|\.\s+|\([a-z]\)\s+|\([ivx]+\)\s+|\(\d+\)\s+)([A-Z][A-Za-z]+(?:[\s\-/&,][A-Z][A-Za-z]+){0,5})\s+(means?|shall\s+mean|has\s+the\s+meaning|shall\s+have\s+the\s+meaning)\b/g;
  let u;
  while ((u = unquotedPattern.exec(sectionText)) !== null) {
