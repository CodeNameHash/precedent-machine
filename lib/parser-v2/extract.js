...
`;
    ...
    typeSpecific = `
CRITICAL FEATURE EXTRACTION RULES for ${typeKey}:
- Each sub-clause (a)/(b)/(c)/(d) of the conditions article is its OWN closing condition and gets its OWN canonical code and OWN provision. Do NOT bundle multiple sub-clauses under one code.
    * Mutual conditions article (typically 7.01 or 6.01) — split into: COND-M-STOCKHOLDER (a) stockholder vote, COND-M-LEGAL (b) no legal restraints / injunctions / laws, COND-M-REG (c) regulatory approvals / HSR / waiting periods.
    * Buyer conditions article (typically 7.02 or 6.03) — split into: COND-B-REP (a) representations bring-down, COND-B-COV (b) covenant performance ("covenants required to be performed at or prior to Closing have been performed in all material respects"), COND-B-MAE if a standalone no-MAE condition exists, COND-B-CERT (c) closing certificate from Company officer, COND-B-DISSENT if a dissenting-shares cap is a separate condition.
    * Target / Seller conditions article (typically 7.03 or 6.02) — split into: COND-S-REP (a) representations bring-down by Parent / Merger Sub, COND-S-COV (b) covenant performance by Parent / Merger Sub, COND-S-CERT (c) closing certificate from Parent officer, COND-S-FUNDS only if Parent's available funds is itself a condition.
