...
- environmentalLawsList (list, clause): Environmental laws referenced ("CERCLA", "Clean Water Act").
- environmentalPermitsHeld (list, clause): Environmental permits / authorizations held by the Company.
- environmentalLiabilities (text, clause): Description of environmental liabilities / remediation obligations.
- hazardousMaterialsScope (text, clause): Scope of "Hazardous Materials" or "Hazardous Substances" definition.
- ipRegistrations (list, clause): Registered IP (patents, trademarks, copyrights) listed or referenced.
- ipLicensesIn (list, clause): Material inbound IP licenses ("licenses to the Company").
- ipLicensesOut (list, clause): Material outbound IP licenses ("licenses from the Company").
- ipInfringementClaims (text, clause): Pending or threatened IP infringement claims.
- ipTradeSecretsProtection (text, clause): Trade-secrets protection measures (NDAs, access controls).
...
- taxReturnsFiled (text, clause): "all material Tax Returns have been filed" anchor.
- taxAuditsPending (text, clause): Pending tax audits / examinations.
- taxClosingAgreements (text, clause): Closing agreements / rulings.
- taxNoNotices (boolean, clause): true if the rep states no notices of tax deficiency.
- taxSection355 (text, clause): Section 355 / spin-off language.
- taxAttributes (text, clause): NOLs, credits, basis description.
- itSystemsScope (text, clause): IT-systems scope description ("Company IT Systems are sufficient...").
- cybersecurityIncidents (text, clause): Cybersecurity incidents anchor ("no material breach of IT Systems since").
- dataPrivacyCompliance (text, clause): Data-privacy compliance ("in compliance with applicable Privacy Laws").
- personalInfoBreaches (text, clause): Personal-information breach disclosures.
- pendingLitigation (text, clause): "no pending litigation" anchor.
- governmentInvestigations (text, clause): Government investigations / inquiries.
- outstandingOrders (text, clause): Outstanding orders / judgments / consent decrees.` : `- sufficientFundsRepPresent (boolean, clause): true if Parent reps it has sufficient funds. sufficientFundsRepDetails = verbatim language ("at Closing, Parent will have ... sufficient cash on hand").
