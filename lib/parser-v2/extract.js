...
const { canonicalIocCategoryFromHeading, iocCategoryLabel } = require('../vocab/ioc-categories');
...
function iocSubClauseFallbackCategory(effectiveType, text) {
  ...
  const key = canonicalIocCategoryFromHeading(String(text || ''));
  if (!key) return null;
  ...
  "results": [
    ...
    try {
      ...
      for (const sc of classifiable) {
        const fallbackCategory = iocSubClauseFallbackCategory(typeKey, sc.text);
        provisions.push(makeProvision({
          type: typeKey,
          code: null,
