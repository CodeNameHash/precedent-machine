...
function extractTitledSubclauses(text) {
  ...
  // Keep only the MONOTONIC a,b,c,… top-level run so a stray titled "(d)" deep
  // inside a rep body can't fragment it: accept a mark only when its letter is
  // exactly the next one expected in sequence.
  const seq = [];
  let expect = 'a'.charCodeAt(0);
  for (const mk of marks) {
    if (mk.letter.charCodeAt(0) === expect) { seq.push(mk); expect += 1; }
  }
  const parts = [];
