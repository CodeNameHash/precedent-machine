...
${formatDict(FORCE_THE_VOTE)}
   ...
   * "a Change in the [Company] Recommendation shall not limit the Company's obligation to submit this Agreement to the stockholders ... for the purpose of obtaining the ... Stockholder Approval"
   ...
   HARD vs SOFT — the discriminator is the existence of a COMPANY SELF-TERMINATION RIGHT FOR A SUPERIOR PROPOSAL (a right for the COMPANY itself to terminate this Agreement pre-vote to enter an Alternative Acquisition Agreement, paying the termination fee — usually in the termination article, often cross-referenced from this section). It is NOT whether the board may change its recommendation: both hard and soft FTV permit an ARC.
   * FTV_SOFT — the vote-survives-ARC covenant coexists with such a company self-termination right. Signals: "unless this Agreement has been validly terminated in accordance with its terms" qualifying the vote covenant; a company right to terminate to accept a Superior Proposal (see companyTerminationForSuperior). If companyTerminationForSuperior is true, forceTheVoteType MUST NOT be FTV_HARD.
   * FTV_HARD — the vote-survives-ARC covenant exists AND the company has NO self-termination right to accept a Superior Proposal anywhere in the agreement: the board may flip its recommendation, but the target can never escape the vote by terminating for a competing deal.
   * FTV_NONE — no vote-survives-ARC covenant at all (even if a company self-termination-for-Superior-Proposal right exists — an ordinary superior-proposal-out without FTV language is FTV_NONE).
