...
async function strategyA(sections, client) {
  ...
  const tasks = Object.entries(byType).map(([typeKey, typeSections]) => async () => {
    ...
    for (const section of typeSections) {
      // Material Contracts rep: do NOT split its (i)/(ii)/(iii) enumeration —
      // those sub-clauses ARE the buckets and the extractor must see the WHOLE
      // list at once to enumerate all ~21 into materialContractsBuckets.
      // Splitting would feed the AI one bucket at a time (the cause of "only a
      // handful show up").
      const secTitle = String(section.title || section.heading || section.category || '');
      const isMaterialContractsRep = (typeKey === 'REP-T' || typeKey === 'REP-B')
        && /material\s+contracts/i.test(secTitle);
      const parts = isMaterialContractsRep ? null : splitSubClauses(section.text, typeKey);
      if (parts) {
