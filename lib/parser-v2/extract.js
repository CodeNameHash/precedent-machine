 ...
 *
 * Strategies (see STRATEGY_A_TYPES / STRATEGY_B_TYPES / STRATEGY_C_TYPES
 * below for the current, authoritative type lists: a type's strategy has
 * moved before, e.g. TERMF was section-level AI until it needed overlapping-
 * span handling like NOSOL/ANTI, so treat this summary as a map, not the
 * source of truth):
 *   A  Regex-split → AI-classify  (IOC, COND-M/B/S, TERMR types)
 *   B  AI multi-code extraction   (NOSOL, ANTI, TERMF: overlapping spans)
 *   C  Section-level AI           (REP, STRUCT, CONSID, COV, MISC, OTHER)
...
async function extractProvisions(classifiedSections, client, fullCleanedText, dealMeta = {}, opts = {}) {
  ...
  const transactionStepModel = extractTransactionSteps(classifiedSections, {
    fullCleanedText,
    dealMeta,
    resolvedModelClaims: opts.resolvedModelClaims || [],
  });
