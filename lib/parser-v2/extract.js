...
- secFilingsExceptionLookback (text): The lookback phrase ("since January 1, 2023" / "since the day prior to signing" / "filed since [DATE]").
- secFilingsExceptionLookbackDate (text, ISO YYYY-MM-DD): If the lookback is expressed as a date, normalize to ISO. Otherwise null.
- secFilingsExceptionExclusions (list of short strings): Sections EXCLUDED from the SEC-filings exception. Look for the parenthetical "(other than disclosures under the heading 'Risk Factors' / other than forward-looking statements / other than the Note disclosures)". One short label per exclusion (e.g. ["Risk Factors", "Forward-Looking Statements", "Cautionary Statements"]). Return [] if absent.
...
- disclosureLetterReference (text): Reference to the Company / Parent Disclosure Letter (e.g. "Section 4.X of the Company Disclosure Letter").
...
- absenceOfChangesType (enum SPECIFIED_IOCS/GENERAL_ORDINARY_COURSE/HYBRID, clause): On Absence-of-Changes reps, classify the rep's structure:
    * SPECIFIED_IOCS — the rep enumerates a list of specific restricted actions / changes (look for sub-clauses (i), (ii), (iii) listing concrete prohibited or to-be-disclosed acts: amendments to charter, dividends, equity issuances, material acquisitions, etc.).
    * GENERAL_ORDINARY_COURSE — a single ordinary-course-of-business statement ("the Company has conducted its business only in the ordinary course consistent with past practice") with no enumerated list.
    * HYBRID — both an ordinary-course statement AND an enumerated list. Pick the MOST specific code.
- absenceOfChangesExceptions (list-tagged, clause): On Absence-of-Changes reps, extract each enumerated sub-clause (the specific changes/actions the rep calls out) as one tagged item. Examples: "any amendment to its certificate of incorporation", "any declaration, setting aside, or payment of any dividend", "any material change in accounting methods", "any incurrence of Indebtedness". Use the verbatim sub-clause text. REQUIRED when the rep type is SPECIFIED_IOCS or HYBRID.
...
MATERIAL CONTRACTS REP — bucket + per-bucket dollar threshold extraction (REP-T only):
The Material Contracts rep enumerates categories ("buckets") of contracts the target must disclose, with each bucket typically scoped by its own dollar threshold. Populate BOTH of these fields when the provision is the Material Contracts rep:

- materialContractsBuckets (list-tagged, clause): One TAGGED item per bucket the rep enumerates, drawn from MATERIAL_CONTRACT_BUCKET_CODES. Each item MUST be { code, label, text, threshold } where:
    * code = canonical MATERIAL_CONTRACT_BUCKET_CODES code (e.g. AGGREGATE_PAYMENTS, INDEBTEDNESS, JV_PARTNERSHIPS, MA_AGREEMENTS, IP_LICENSES_IN, IP_LICENSES_OUT, SUPPLY, MANUFACTURE, DISTRIBUTION, COLLABORATION, SETTLEMENT, NONCOMPETE, REAL_ESTATE, EMPLOYMENT_KEY, GOVERNMENT_CONTRACTS, OTHER).
    * label = the canonical label for that code.
    * text = the verbatim sub-clause that introduces this bucket (e.g. "(i) any Contract that involves aggregate annual payments by the Company or any Company Subsidiary of more than \$5,000,000").
    * threshold = the dollar (or other) threshold expressed for THIS bucket as a short string (e.g. "$5,000,000", "$10 million per year", "$25,000,000 aggregate"), or null if no threshold is stated.

- materialContractsDollarThresholds (list, clause): Parallel/fallback list. One object per bucket carrying { bucket, threshold } (bucket = the canonical MATERIAL_CONTRACT_BUCKET_CODES code, threshold = short string). Populate this list in ADDITION to setting threshold on each bucket item so downstream consumers can resolve thresholds via either path.

THRESHOLD ANCHOR PHRASES — search the Material Contracts rep sub-clauses for these patterns and extract the dollar (or other) figure into the matching bucket's threshold:
    * "any Contract that involves aggregate annual payments by the Company or any Company Subsidiary of more than \$[X]" → AGGREGATE_PAYMENTS bucket, threshold = "\$[X]"
    * "any Indebtedness in excess of \$[X]" / "any Contract relating to Indebtedness having an outstanding principal amount in excess of \$[X]" → INDEBTEDNESS bucket
    * "for the avoidance of doubt, [bucket name] above includes [Contract type] involving payments greater than \$[X]" → match to that bucket
    * "Contracts involving capital expenditures exceeding \$[X]" / "capital expenditures in excess of \$[X]" → AGGREGATE_PAYMENTS or OTHER bucket (whichever the sub-clause sits under)
    * "any Contract requiring payments by or to the Company in excess of \$[X] in any 12-month period" → AGGREGATE_PAYMENTS
    * "any Contract for the acquisition or disposition of … assets with a value in excess of \$[X]" → MA_AGREEMENTS
    * "any settlement … providing for monetary payment in excess of \$[X]" → SETTLEMENT
    * "any Real Property lease with annual rent in excess of \$[X]" → REAL_ESTATE
    * "any Government Contract with annual revenues in excess of \$[X]" → GOVERNMENT_CONTRACTS
Always extract the threshold AS WRITTEN (preserve "$" and the figure exactly). Do not normalize "$5,000,000" to "5M".

PAIRING RULE for Material Contracts: every materialContractsBuckets[i] populated MUST also produce a materialContractsDollarThresholds entry { bucket: <same code>, threshold: <same string or null> }.` : `- sufficientFundsRepPresent (boolean, clause): true if Parent reps it has sufficient funds. sufficientFundsRepDetails = verbatim language ("at Closing, Parent will have ... sufficient cash on hand").
...
// a REP-T-FINSTMT provision when the body contains the classic anchor. Mutates
// the `provisions` array in place.
