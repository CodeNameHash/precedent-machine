...
function filterFeaturesToCodeSchema(features, code) {
  ...
  // Always allow post-pass-stamped keys (linkBringDownToReps,
  // linkKnowledgeScopeToReps, resolveAocCovenantCitations).
...
2. "label" must NEVER be a bare section cross-reference. "Termination pursuant to Section 8.01(c)(ii)" is WRONG — say WHAT Section 8.01(c)(ii) is. Section numbers belong in "text", which is copied VERBATIM (character-for-character, including the cross-references and parentheticals).
    ...
    try {
      ...
      for (let i = 0; i < classifiable.length; i++) {
        ...
        if (!features.sectionNumber) {
          ...
          if (baseNum) {
            features.sectionNumber = sc.letter && sc.letter !== '_whole'
              ? `${baseNum}(${sc.letter})`
              : baseNum;
          }
        }
...
function resolveAocCovenantCitations(provisions) {
  ...
  // Cite → name lookup from every provision that carries a section number.
  ...
  for (const p of provisions) {
    ...
    if (!name) continue;
    const isIoc = typeof p.type === 'string' && p.type.startsWith('IOC');
    const existing = nameByCite.get(key);
    if (!existing || (isIoc && !existing.isIoc)) {
      nameByCite.set(key, { name, isIoc });
    }
  }
  ...
  for (const rep of provisions) {
    ...
    // between the endpoints. Only single-letter ranges on the same base
    // section expand; everything else passes through as written. The scan is
    // deliberately limited to the recorded limb + cite list — never the whole
    // rep text, which may cross-reference unrelated section ranges.
    ...
    let rm;
    while ((rm = rangeRe.exec(rangeSource)) !== null) {
      const base = rm[1];
      const from = LETTERS.indexOf(rm[2].toLowerCase());
      const to = LETTERS.indexOf(rm[3].toLowerCase());
      if (from === -1 || to === -1 || to <= from) continue;
      for (let i = from; i <= to; i++) pushCite(`${base}(${LETTERS[i]})`);
    }

    // Natural section order (5.1 before 5.2; (a) before (b)).
    const sortKey = (key) => {
...
/**
 * Stamp features.knowledgeScope — the verbatim core of the deal's "Knowledge"
 * definition (named individuals / "actual knowledge of the persons listed on
 * Schedule X") — onto every rep whose text is knowledge-qualified. REP-T reps
 * get the Company-side definition, REP-B reps the Parent-side one; a single
 * generic "Knowledge" definition serves both. Deterministic; no AI calls; a
 * rep with no knowledge qualifier is left untouched (absence is meaningful).
 */
function linkKnowledgeScopeToReps(provisions) {
  ...
  if (!companyScope && !parentScope) return;

  for (const p of provisions) {
    if (!p || !p.text) continue;
    if (p.type !== 'REP-T' && p.type !== 'REP-B') continue;
    if (p.features && p.features.knowledgeScope) continue;
    if (!KNOWLEDGE_QUALIFIER_RE.test(p.text)) continue;
    const scope = p.type === 'REP-T' ? companyScope : parentScope;
    if (!scope) continue;
    if (!p.features) p.features = {};
    p.features.knowledgeScope = scope;
  }
}
 ...
 * CONSID provision. Deterministic — no LLM call.
 */
function backfillCvrMaxFromExhibit(provisions, fullText) {
  ...
  // Respect an LLM-extracted max if one already exists on any CONSID row.
  if (consids.some((p) => p.features && (p.features.maxPayment || p.features.cvrMaxPayment))) return;
  ...
  const defRe = /["“]([^"“”\n]{0,60}Milestone[^"“”\n]{0,40}Payment(?:\s+Amount)?)["”]\s*(?:\[\[\/DEFINED\]\])?\s*means\s*(?:an\s+amount(?:\s+in\s+cash)?\s+equal\s+to\s*)?\$\s?([\d,]+(?:\.\d+)?)/gi;
  const seen = new Map();
  let m;
 ...
 * provision's text but missing from outstandingInstruments gets a minimal
 * entry (verbatim naming sentence, treatment left null → UI renders "—"),
 * so the row can never silently disappear.
 */
function backfillMissingInstrumentMentions(provisions) {
  ...
  for (const p of provisions) {
    ...
    for (const [code, re] of Object.entries(INSTRUMENT_MENTION_RES)) {
      ...
      if (!m) continue;
      // Verbatim sentence containing the first mention (capped).
    ...
    p.features = {
      ...
      // Treatments/vesting stay index-aligned: pad with nulls so the pairing
      // pass in expandConsidEquityByInstrument falls back cleanly ("—").
