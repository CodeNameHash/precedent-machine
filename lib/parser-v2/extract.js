...
${buildFeatureInstructions('DEF')}
MAE DEFINITION:
${mp.text}

Return ONLY valid JSON (no markdown): { "features": { "carveouts": [ { "code": "<MAE_CARVEOUT_CODE>", "label": "...", "text": "<verbatim clause>" } , ... ], "disproportionateImpactCarveouts": [ ... ] } }
You MUST list EVERY enumerated carve-out (a)/(b)/(c)/(i)/(ii)… — an empty carveouts array is an error for an MAE definition.`;
      ...
      const cv = parsed && parsed.features && parsed.features.carveouts;
      if (Array.isArray(cv) && cv.length > 0) {
        mp.features = { ...mp.features, carveouts: cv };
        if (Array.isArray(parsed.features.disproportionateImpactCarveouts)) {
          mp.features.disproportionateImpactCarveouts = parsed.features.disproportionateImpactCarveouts;
        }
      }
    } catch (err) {
