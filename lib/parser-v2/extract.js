  ...
  "results": [
    ...
    try {
      ...
      for (let i = 0; i < typeSections.length; i++) {
        ...
        // For STRUCT, OTHER, CONSID-EQUITY filter to the code's specific
        // schema so we don't carry irrelevant fields.
        if (isValidCode(code) && (code.startsWith('STRUCT-') || code === 'CONSID-EQUITY')) {
          features = filterFeaturesToCodeSchema(features, code);
        }
...
function linkBringDownToReps(provisions) {
  ...
  for (const { condCode, repType } of condMappings) {
    ...
    // If no explicit catch-all was found but there ARE tiers, fall back to
    // the LAST tier as the catch-all — drafters often list the "all other
    // reps" tier last without using the literal phrase "all other".
    if (!catchAll && lastTier) catchAll = lastTier;
