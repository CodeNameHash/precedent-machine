...
const STRATEGY_C_TYPES = new Set([
  'REP-T', 'REP-B', 'STRUCT', 'CONSID', 'COV', 'TERMF', 'MISC', 'OTHER',
]);
...
function buildFeatureInstructions(typeKey, opts = {}) {
  ...
  // ── GLOBAL BREVITY PRINCIPLE — applies to EVERY provision type ──────────
  // The "summary" fields (mainConcept / mainObligation / mainCondition /
  // summary) must each be a clean one-line summary — never a paragraph and
  // never a verbatim copy of the source. Lawyers comparing across deals want
  // to scan dozens of provisions at a glance, so canonical, short, plain-English
  // summaries are the highest-value extraction the model can produce.
  const globalBrevity = `
GLOBAL SUMMARY-FIELD RULE (applies to EVERY field below):
- "mainConcept", "mainObligation", "mainCondition", and "summary" MUST each be
  a single clean sentence or short phrase that conveys the CORE obligation in
  plain English. They are NOT paragraphs and NOT verbatim copies of the source.
- Strip "Except as..." / "Subject to..." / "Notwithstanding..." framing language;
  state the bottom-line obligation directly.
- Aim for canonical phrasing so the same concept across deals reads the same way.
  Examples of the target style:
    * "HSR filing within 15 business days of signing."
    * "Buyer may not take any action that would impede regulatory clearance."
    * "Target reps true and correct except where failure would not have an MAE."
    * "Mutual termination by written consent."
    * "Outside date is 12 months after signing."
    * "Target cannot incur indebtedness over $25M without buyer consent."
- If you cannot summarize in one line because the provision genuinely has two
  obligations, use a single sentence with a semicolon. Never more than two clauses.
`;

  // Type-specific extraction guard rails
  let warnings = globalBrevity;
  if (typeKey === 'IOC') {
    if (scope === 'preamble') {
      warnings = `
...
- "permittedExceptions" — the IOC preamble almost always lists 3-5 SECTION-WIDE carve-outs in a single "Except as ... or with ..." framing. Extract EACH ONE as a tagged item { code, label, text } drawn from EXCEPTION_CODES. Look explicitly for these 5 standard carve-outs (omit any that aren't actually present):
    ...
    * PRIOR_WRITTEN_CONSENT — "with the prior written consent of Parent (not to be unreasonably withheld, conditioned, or delayed)"
...
- Map by content: mutual consent → TERMR-MUTUAL; outside-date / drop-dead-date language → TERMR-OUTSIDE; standalone extension provision → TERMR-EXTENSION; legal-restraint / order / injunction → TERMR-LEGAL; failure to obtain stockholder vote → TERMR-VOTE; target breach uncured → TERMR-BREACH-T; buyer/parent breach uncured → TERMR-BREACH-B; superior-proposal termination by target → TERMR-SUPERIOR; adverse recommendation change → TERMR-RECOMMEND.

PER-CODE FEATURE FIELDS — once you choose a code for a sub-clause, ONLY extract the fields listed for that code. Leave irrelevant fields out of the features object. Common fields (mainConcept, partyWhoCanTerminate, terminationTriggers, faultBasedExclusion) apply to ALL codes:
${codeMenu}

Field semantics:
- "partyWhoCanTerminate" identifies who can invoke THIS specific termination right (buyer / target / either / mutual).
- "terminationTriggers" for a SINGLE sub-clause is a SHORT list (often one item) describing the specific trigger condition for that sub-clause. Do NOT roll up triggers from other sub-clauses.
...
- "outsideDate" / "outsideDateMonths" / "outsideDateExtension" / "outsideDateExtensionConditions" / "extensionPeriod" / "extensionTrigger" / "tickingFee" apply ONLY to TERMR-OUTSIDE / TERMR-EXTENSION.
- "breachCurePeriod" / "breachMaterialityStandard" apply ONLY to TERMR-BREACH-T / TERMR-BREACH-B.
- "superiorProposalConditions" / "superiorProposalTermination" / "simultaneousFeePayment" apply ONLY to TERMR-SUPERIOR.
- "recommendationChangeConditions" / "preVoteOnlyWindow" apply ONLY to TERMR-RECOMMEND.
- "legalRestraintScope" / "restraintFinality" apply ONLY to TERMR-LEGAL.
- "voteRequiredThreshold" / "voteFailureContext" apply ONLY to TERMR-VOTE.
- "writtenConsentRequired" applies ONLY to TERMR-MUTUAL.
...
const IOC_PREAMBLE_PROVISIONS = [
  ...
  },
  {
    ...
    // "shall use [commercially reasonable / reasonable best] efforts to (i)
    // preserve ... (ii) retain ..." — stop at sentence end OR at the start of
    // the next obligation ("In addition" / "Without limiting" / "Further").
    re: /(?:shall\s+use\s+(?:its\s+)?(?:commercially\s+reasonable|reasonable\s+best|reasonable|best)\s+efforts\s+to[^.]{0,800})/i,
  },
  {
    key: 'IOC-NOACTION',
  ...
  for (const { condCode, repType } of condMappings) {
    ...
    for (const rep of provisions) {
      ...
      if (!stamp) {
        unstampedCount++;
        console.log(
          `[linkBringDownToReps]   REP "${rep.category || ''}" — section="${repSection || '(none)'}" — NO MATCH (no tier and no catch-all)`,
        );
        continue;
      }
    ...
    console.log(
      `[linkBringDownToReps] ${condCode} → ${repType}: stamped ${stampedCount} reps, ${unstampedCount} unmatched`,
    );
  }
}
