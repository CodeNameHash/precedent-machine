...
const { convertConsiderationEquityProvisions } = require('./consideration-equity');
...
${formatDict(FORCE_THE_VOTE)}
   ...
   * "a Change in the [Company] Recommendation shall not limit the Company's obligation to submit this Agreement to the stockholders ... for the purpose of obtaining the ... Stockholder Approval"
   ...
   The code carries TWO axes — resolve BOTH:

   AXIS 1 — STRENGTH (HARD vs SOFT): the discriminator is the existence of a SELF-TERMINATION RIGHT FOR A SUPERIOR PROPOSAL held by the OBLIGATED party (a right for that party itself to terminate this Agreement pre-vote to enter an Alternative Acquisition Agreement, paying the termination fee — usually in the termination article, often cross-referenced from this section). It is NOT whether the board may change its recommendation: both hard and soft FTV permit an ARC.
   * SOFT — the vote-survives-ARC covenant coexists with such a self-termination right for the obligated party. Signals: "unless this Agreement has been validly terminated in accordance with its terms" qualifying the vote covenant; a right to terminate to accept a Superior Proposal (see companyTerminationForSuperior). If companyTerminationForSuperior is true for the obligated party, the code MUST NOT be a HARD code.
   * HARD — the vote-survives-ARC covenant exists AND the obligated party has NO self-termination right to accept a Superior Proposal anywhere in the agreement: the board may flip its recommendation, but that party can never escape the vote by terminating for a competing deal. NOTE: a termination right held by the COUNTERPARTY (e.g. the other side may walk away if this party changes its recommendation) does NOT make it soft — only a self-termination right held by the OBLIGATED party does.

   AXIS 2 — PARTY (whose stockholder vote is force-locked): read the SUBJECT of the vote covenant — whose stockholders/meeting must be held regardless of the recommendation change.
   * TARGET — the company being acquired (the seller). This is the default in a normal cash/stock acquisition ("the Company shall ... hold the Company Stockholders' Meeting").
   * ACQUIRER — the buyer/issuer's OWN stockholders (a share-issuance vote, or a reverse merger where the public issuer's — e.g. "PubCo's" — stockholders vote). If the forced meeting is the acquirer's/issuer's own, the party is ACQUIRER even though the drafting may call that entity "the Company".
   * BOTH — each side's stockholder vote is independently force-locked (dual-approval structure / merger of equals).

   COMBINE the two axes into one code: FTV_HARD_TARGET, FTV_HARD_ACQUIRER, FTV_HARD_BOTH, FTV_SOFT_TARGET, FTV_SOFT_ACQUIRER, FTV_SOFT_BOTH.
   * FTV_NONE — no vote-survives-ARC covenant at all on any party (even if a self-termination-for-Superior-Proposal right exists — an ordinary superior-proposal-out without FTV language is FTV_NONE). FTV_NONE is party-free.
   "text" = the verbatim FTV clause for any HARD/SOFT code; for FTV_NONE leave the whole field null rather than tagging boilerplate. Consistency: forceTheVote (boolean) true ⇔ forceTheVoteType is any FTV_HARD_* or FTV_SOFT_* code.
...
function classifyIocRestrictionComponents(text) {
  ...
  for (const [code, meta] of Object.entries(IOC_CATEGORY_META || {})) {
    ...
    if ((meta.synonyms || []).some((syn) => syn.test(t))) codes.push(code);
  }
  return codes;
}
    ...
    try {
      ...
      for (let i = 0; i < typeSections.length; i++) {
        ...
        const stampedCode = section.provisionCode && isValidCode(section.provisionCode)
          ...
          : null;
        const code = stampedCode || aiResult.code || null;
        const codeEntry = code ? CODES[code] : null;
 ...
 */
async function extractProvisionsForType(classifiedSections, type, client, fullCleanedText, dealMeta = {}) {
  if (!type) return [];
...
async function extractProvisions(classifiedSections, client, fullCleanedText, dealMeta = {}) {
