...
// NOTE: this function also contains REP-side kept-whole logic (isRepSide /
// isMaterialContractsRep) that is currently DEAD — REP-T/REP-B route to
// Strategy C, not here. See the strategy-mapping review notes.
// ---------------------------------------------------------------------------
...
async function strategyA(sections, client) {
  ...
  const tasks = Object.entries(byType).map(([typeKey, typeSections]) => async () => {
    ...
    for (const section of typeSections) {
      // Certain REP sub-sections must be extracted WHOLE (do NOT split their
      // (i)/(ii)/(iii) enumeration into separate fragment-provisions), because
      // the enumerated sub-clauses ARE the data the per-rep "specific features"
      // extraction needs to see together:
      //   - Material Contracts → the sub-clauses are the ~21 buckets.
      //   - Absence of (Certain) Changes → the sub-clauses are the enumerated
      //     changes/exceptions (absenceOfChangesExceptions).
      //   - (No) Undisclosed Liabilities → the sub-clauses are the carve-outs.
      //   - Employee Benefits / ERISA → multi-prong rep (plans/Title IV/etc).
      // Splitting fed the AI one fragment at a time, which is why these
      // specific-features were never populated.
      const secTitle = String(section.title || section.heading || section.category || '');
      const secBody = String(section.text || '');
      const isRepSide = (typeKey === 'REP-T' || typeKey === 'REP-B');
      // Material Contracts rep: match by title OR by the body signature (the
      // rep references "Company Material Contracts" / "Material Contract" and
      // enumerates contract types). Title-only matching missed deals where the
      // section is titled just "Contracts" or the classifier labelled it
      // generically — leaving the rep split and the bucket count short.
      const isMaterialContractsRep =
        /material\s+contracts?\b/i.test(secTitle) ||
        (/\bcontracts?\b/i.test(secTitle) && /material\s+contract/i.test(secBody)) ||
        (/material\s+contract/i.test(secBody) && /\(\s*[ivx]+\s*\)|\([a-z]\)/i.test(secBody));
      const keepWhole = isRepSide && (
        isMaterialContractsRep ||
        /absence\s+of\s+(?:certain\s+)?changes/i.test(secTitle) ||
        /undisclosed\s+liabilities|no\s+(?:undisclosed\s+)?liabilities/i.test(secTitle) ||
        /employee\s+benefit|\berisa\b/i.test(secTitle)
      );
      const parts = keepWhole ? null : splitSubClauses(section.text, typeKey);
      if (parts) {
