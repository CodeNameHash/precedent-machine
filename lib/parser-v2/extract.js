...
// Span accounting spec Part 2 (docs/handoffs/SPAN-ACCOUNTING-SPEC-2026-07-18.md)
...
// section text. INERT unless opts.spanClaims === true — no default call
// site passes that, so this is a no-op in the ingest path today.
function attachStrategySpanClaims(sections, provisions, opts) {
...
function detectIocPartyFlipLetter(subClauseParts) {
  ...
  let firstName = null;
  for (const part of subClauseParts) {
    ...
    if (name !== firstName) return letter;
  }
  return null;
}
...
async function strategyA(sections, client, opts = {}) {
  ...
  const tasks = Object.entries(byType).map(([typeKey, typeSections]) => async () => {
    ...
    for (const section of typeSections) {
      ...
      // Modiv/GNL, CSRA/General Dynamics) carry BOTH parties' conduct
      // covenants under one section-level heading. See
      // detectIocPartyFlipLetter above — flipLetter is the top-level letter
      // (and everything after it) that actually belongs to the OTHER party
      // from what classify.js stamped on the whole section.
      const flipLetter = (parts && (typeKey === 'IOC-T' || typeKey === 'IOC-B'))
        ? detectIocPartyFlipLetter(parts)
        : null;
      if (parts) {
        for (const part of parts) {
          const topLetter = part.letter && part.letter !== '_preamble'
            ? String(part.letter).split('.')[0]
            : null;
          const iocPartyFlip = !!(flipLetter && topLetter && topLetter >= flipLetter);
          allSubClauses.push({
            text: part.text,
            letter: part.letter,
