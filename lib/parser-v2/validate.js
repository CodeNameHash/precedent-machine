...
// the parser as { value, text }. If the AI returns a bare value (legacy data
// or under-tuned prompt), wrap it as { value: <bareValue>, text: "" } so the
// UI can still render the value AND display an "(no evidence captured)"
// placeholder in the evidence slot. Re-ingests under the new prompt will fill
// the text.
// ---------------------------------------------------------------------------

const WRAPPED_KEYS = ['value', 'text'];

function isAlreadyWrapped(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  // Accept exactly { value, text } (text optional but key must exist OR be
  // implicitly empty). A tagged item has `code`, which means we must NOT
  // touch it.
...
function wrapCitableFeatures(features, schema, soft) {
  ...
  for (const fdef of schema) {
    ...
    if (isAlreadyWrapped(v)) continue;
    // Auto-wrap a bare value into the citable shape.
    features[key] = { value: v, text: '' };
    wraps++;
    if (soft && wraps < 4) {
      // Keep a soft log per provision so debugging is possible.
      // eslint-disable-next-line no-console
      console.warn(`[validate] auto-wrapped citable field "${key}" (bare value -> { value, text: "" })`);
    }
  }
