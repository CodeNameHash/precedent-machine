 ...
 *
 * Validates extracted provisions against the canonical rubric AND fixes up
 * their shape before storage:
 *   - Checks each provision's code is valid; flags duplicates (same type +
 *     code); identifies missing universal-frequency codes; collects
 *     proposed new codes; generates a validation report.
 *   - Auto-wraps any `citable: true` feature the model returned as a bare
 *     value into the { value, quotes } shape (normalizing the legacy
 *     { value, text } form too), so storage never has to special-case an
 *     under-wrapped citable field.
 *   - Backfills an OTHER provision for any classified section that didn't
 *     make it into the final provisions list, guaranteeing 100% section
 *     coverage in the stored output (backfillOrphanSections).
 *   - Optionally (opts.spanResidual === true, off by default, inert
 *     otherwise) computes the Part 3 span-residual report; see
 *     computeSpanResidualReport and lib/parser-v2/span-residual.js.
 *
 * CommonJS. Reached from Next.js API routes only indirectly today, via
 * lib/parser-v2/run-extract.js; see that module's header for the current
 * containment status of the ingest API surface.
 */
...
// Part 3 — REPORT-ONLY. computeSpanResidualReport is INERT unless a caller
// passes opts.spanResidual === true; no default validateProvisions() call
// site does, and even when enabled it only appends a report field — it
// never fails validation or blocks ingest. See
// scripts/span-residual-baseline.js for the corpus-wide report run.
const { computeSectionResidual } = require('./span-residual');
...
// ---------------------------------------------------------------------------
// Stage 5: citable-field auto-wrap
//
// Any feature whose schema entry is marked `citable: true` must be emitted by
// the parser as { value, quotes: [string] }. If the AI returns a bare value
// (legacy / under-tuned prompt), wrap it as { value, quotes: [] }. The legacy
// shape { value, text } is normalized to { value, quotes: [text] } so older
// data continues to render under the multi-quote UI. Re-ingests under the new
// prompt will fill multiple verbatim quotes.
//
// Backward compat: both readers and writers accept `text` OR `quotes`. The
// UI prefers `quotes` and falls back to `[text]` when only the legacy field
// is present.
// ---------------------------------------------------------------------------

const WRAPPED_KEYS = ['value', 'text', 'quotes'];

function isAlreadyWrapped(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  // A tagged item has `code`, which means we must NOT touch it.
  if ('code' in v) return false;
  return 'value' in v;
}

function wrapCitableFeatures(features, schema, soft) {
  if (!features || typeof features !== 'object') return { features, wraps: 0 };
  if (!Array.isArray(schema) || schema.length === 0) return { features, wraps: 0 };
  let wraps = 0;
  for (const fdef of schema) {
    if (!fdef || !fdef.citable) continue;
    const key = fdef.key;
    if (!(key in features)) continue;
    const v = features[key];
    if (v === null || v === undefined) continue;
    if (isAlreadyWrapped(v)) {
      // Normalize legacy { value, text } → { value, quotes: [text] } so
      // downstream consumers can rely on `quotes` being present.
    ...
    // Auto-wrap a bare value into the multi-quote citable shape.
    features[key] = { value: v, quotes: [] };
    wraps++;
    if (soft && wraps < 4) {
      // eslint-disable-next-line no-console
      console.warn(`[validate] auto-wrapped citable field "${key}" (bare value -> { value, quotes: [] })`);
    }
  }
  return { features, wraps };
}
...
/**
 * Part 3, REPORT-ONLY: per-classified-section residual report.
 *
 * For each classified section, gathers the claimedSpans of every validated
 * provision that came from it (via features.spanClaims — see
 * span-claims.js's attachSpanClaimsToProvisions; empty if Part 2 wiring
 * wasn't enabled, which conservatively flags the WHOLE section as residual
 * rather than silently under-reporting) and runs computeSectionResidual.
 * Returns only sections that were actually flagged EXTRACTION_INCOMPLETE —
 * this is a report, not an enforcement gate; nothing here throws or blocks.
 */
function computeSpanResidualReport(classifiedSections, provisions) {
  if (!Array.isArray(classifiedSections) || !classifiedSections.length) return null;
  const claimsBySection = new Map();
  for (const prov of provisions || []) {
    const key = (prov && (prov.sectionStartChar ?? prov.startChar)) ?? null;
    if (key === null) continue;
    const spans = (prov.features && prov.features.spanClaims && prov.features.spanClaims.claimedSpans) || [];
    if (!claimsBySection.has(key)) claimsBySection.set(key, []);
    claimsBySection.get(key).push(...spans);
  }

  const flaggedSections = [];
  for (const section of classifiedSections) {
    const sectionText = section && section.text;
    if (!sectionText || sectionText.length < 200) continue;
    const claimedSpans = claimsBySection.get(section.startChar) || [];
  ...
  return {
    sectionsChecked: classifiedSections.filter((s) => s && s.text && s.text.length >= 200).length,
 ...
 * @param {Object} [opts] — opts.spanResidual === true enables the Part 3
 *   report-only residual check (see computeSpanResidualReport below).
 *   Omitted/false by default — inert, no ingest behavior change.
 * @returns {{ valid: boolean, provisions: Array<Object>, report: Object }}
 */
function validateProvisions(provisions, fullText, classifiedSections, opts = {}) {
  ...
  // ── Citable-field auto-wrap (soft) ──
  // Walk every provision and wrap any bare citable values into { value, quotes }.
  // This is non-fatal and idempotent. Re-ingests under the new prompt will
  // populate the text; existing data renders cleanly via the UI fallback.
  let citableAutoWraps = 0;
  for (const prov of provisions) {
    if (!prov || !prov.features) continue;
    const schema = prov.code
      ? getFeaturesForCode(prov.code) || getFeaturesForType(prov.type)
      : getFeaturesForType(prov.type);
    const { wraps } = wrapCitableFeatures(prov.features, schema, true);
    citableAutoWraps += wraps;
  }
  if (citableAutoWraps > 0) {
    warnings.push(
      `Auto-wrapped ${citableAutoWraps} citable field value(s) lacking evidence quotes — re-ingest to populate verbatim quotes`,
    );
  }
  ...
  // Part 3, REPORT-ONLY (span accounting spec, rollout gate 1). INERT unless
  // opts.spanResidual === true — never affects `valid` or throws.
  const spanResidual = opts && opts.spanResidual === true
    ? computeSpanResidualReport(classifiedSections, validated)
    : null;
