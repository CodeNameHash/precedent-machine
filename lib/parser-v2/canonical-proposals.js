...
function validateReadOnlyNoShopProposalBatch({
  ...
  if (!proposalBatch || typeof proposalBatch !== 'object' || Array.isArray(proposalBatch)) {
    throw new TypeError('a source-backed no-shop proposal batch is required');
  }
  if (Array.isArray(proposalBatch.quarantined_proposals)
    && proposalBatch.quarantined_proposals.length > 0) {
    throw new TypeError('source-backed no-shop proposal batch contains quarantined input');
  }
  const expected = buildReadOnlyNoShopProposalBatch({
    contract_bundle: contractBundle,
    source,
    source_admission: sourceAdmission,
    governed_deal_key: governedDealKey,
  ...
  if (canonicalJson(proposalBatch) !== canonicalJson(expected)) {
    throw new TypeError('source-backed no-shop proposal batch identity or exact source mapping mismatch');
  }
  return true;
}
