...
async function storeProvisionCards(dealId, extractorOutput, sb, options = {}) {
  ...
  if (options.replaceDeal) {
    // TODO(claims-writer WP): this is the seam where a future ingest-time
    // claims writer should (re)write public.claims for this deal
    // atomically with cards -- upserting claims for the surviving
    // provision_instance_id/excerpt_id set and removing claims for the
    // orphaned ids computed below, in the same transaction as the orphan
    // delete. Not implemented here; claims backfill/writer today lives in
    // scripts/backfill/claims-from-normalized.js (see
    // supabase/schema-05-claims.sql for the anchoring contract).
