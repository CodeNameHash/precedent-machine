...
const { partyScopeFromCode } = require('../party-scope');
...
function partyScope(prov, cardType) {
  ...
  // No explicit scope: derive from the party token on the provision type/code.
  // prov.type still carries IOC-T / IOC-B / REP-T / REP-B (the party the card's
  // mapped provision_type drops), and prov.code carries TERMF-TARGET etc. This
  // replaces the old blanket 'MUTUAL' default that mislabeled every party-typed
  // card (lib/party-scope.js is the shared derivation).
  const derived = partyScopeFromCode(prov.type || prov.provision_type)
    || partyScopeFromCode(prov.code || prov.canonical_code);
...
// primary_quote_start/primary_quote_end — REGION-relative (an offset into
// `source`, i.e. the card's own region_full_text), NEVER a full-document
// offset despite the column names (see lib/parser-v2/resolve-source-span.js's
// header for the live-data finding this comment records: raw offsets
// "essentially never address the right span" when read as full-document
// coordinates). supabase/schema-03-card-model.sql makes both columns
// `NOT NULL` with a `end > start >= 0` CHECK, so there is no null/absent
// state to fall back to — SOME pair must always be written, even when
// `quote` cannot be found in `source` at all. `located: false` is the
// actual answer in that case; `{0, quote.length}` is a placeholder, not a
// location. Returns `{ start, end, located }` — callers must use `located`
// (see initialCardRow's provenance stamp) rather than trusting the numbers
// on their own, exactly as resolve-source-span.js already does for every
// live consumer of these two columns.
function quoteSpan(prov, quote, source) {
  ...
  if (
    Number.isInteger(explicitStart) && Number.isInteger(explicitEnd) && explicitEnd > explicitStart
    && src.slice(explicitStart, explicitEnd) === quote
  ) {
    return { start: explicitStart, end: explicitEnd, located: true };
  }
  const index = src.indexOf(quote);
  if (index >= 0) return { start: index, end: index + quote.length, located: true };
  return { start: 0, end: quote.length, located: false };
}
...
function buildProvenance(prov, options, span) {
  ...
  return {
    ...
    // primary_quote_start/primary_quote_end (this ROW's own columns, set
    // below in initialCardRow) are region_full_text-relative, NOT this
    // full-document offset pair, and NOT the section-relative coordinates a
    // provision's features.spanClaims.textSpan carries (span-claims.js) —
    // three separate coordinate spaces; never average or substitute between
    // them. This pair is always freshly recomputed from `span` (never
    // deferred to `existing`, matching primary_quote_start/end themselves,
    // which initialCardRow also always recomputes fresh) because it
    // describes THIS write's span, not a fact fixed at first-write time.
    primary_quote_span_coordinate_space: 'region_full_text',
    // False exactly when quoteSpan() fell through to its {0, quote.length}
    // placeholder — the one machine-checkable signal that
    // primary_quote_start/end on this row are not a real location. See
    // quoteSpan's header.
    primary_quote_span_verified: Boolean(span && span.located),
  };
}
