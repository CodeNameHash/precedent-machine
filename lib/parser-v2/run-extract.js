/* ─────────────────────────────────────────────────────────────────────────
   ...
   orchestration could run from two entry points:
   ...
   The API-route entry point is currently DORMANT: pages/api/ingest/
   extract-type.js now resolves to createBroadCorpusContainedHandler('POST')
   (a 503 stub), same containment as the from-url ingest route (see
   lib/ingest/deal-metadata-prompt.js's header for that story). Today this
   module's only live callers are local scripts (scripts/extract-local.js,
   scripts/ingest-worker.js, scripts/reprocess.js). The two-entry-point
   design this header describes remains correct architecture and the API
   route's real logic would call straight back into this module the moment
   containment lifts; it just is not live right now.
...
const { cleanText } = require('./structural');
...
const { isValidCode } = require('../rubric');

function isExemptMissingCodeProvision(provision) {
  if (!provision) return true;
  if (provision.type === 'OTHER' || provision.type === 'SECTION-LEFTOVER') return true;
  if (provision.backfilled) return true;
  if (provision.features?.v1Disposition === 'OPEN_WORLD') return true;
  const category = String(provision.category || '').toLowerCase();
  return category === 'general / preamble' || category === 'preamble';
}

function preserveOpenWorldDisposition(provisions) {
  for (const provision of provisions || []) {
    if (provision?.features?.v1Disposition !== 'OPEN_WORLD') continue;
    provision.code = null;
    provision.isNewCode = false;
    provision.proposedCode = null;
    provision.proposedLabel = null;
  }
  return provisions;
}

function assertOpenWorldDispositions(sections, provisions) {
  const expected = (sections || [])
    .filter((section) => section?.v1Disposition === 'OPEN_WORLD')
    .map((section) => String(section.number || section.sectionNumber || ''));
  for (const sectionNumber of expected) {
    const matches = (provisions || []).filter((provision) =>
      String(provision?.features?.sectionNumber || '') === sectionNumber &&
      provision?.features?.v1Disposition === 'OPEN_WORLD',
    );
    if (matches.length === 0 || matches.some((provision) => provision.code)) {
      throw new Error(`OPEN_WORLD disposition was not preserved for section ${sectionNumber || '(unnumbered)'}`);
    }
  }
}

function assertCanonicalFamilyQuality(type, provisions, validation) {
  if (type === 'OTHER' || type === 'SECTION-LEFTOVER') return;
  const report = (validation && validation.report) || {};
  const finalProvisions = Array.isArray(provisions) ? provisions : [];
  const uncoded = finalProvisions.filter((provision) => {
    if (isExemptMissingCodeProvision(provision)) return false;
    if (provision.isNewCode && provision.proposedCode) return false;
    return !(provision.code && isValidCode(provision.code));
  });
  const invalid = finalProvisions.filter((provision) =>
    provision && provision.code && !isValidCode(provision.code),
  );
  const enforcement = report.code_enforcement || {};
  const failures = Array.isArray(enforcement.failures) ? enforcement.failures : [];
  const uncodedRemaining = Number(enforcement.uncodedRemaining || 0);
  if (uncoded.length === 0 && invalid.length === 0 && failures.length === 0 && uncodedRemaining === 0) return;

  const preview = uncoded.slice(0, 5).map((item) => {
    const label = item.category || item.sourceCategory || 'Unclassified';
    return `${item.type || type}:${label}`;
  }).join('; ');
  const parts = [];
  if (uncoded.length) parts.push(`${uncoded.length} uncoded provision(s)`);
  if (invalid.length) parts.push(`${invalid.length} invalid code(s)`);
  if (uncodedRemaining) parts.push(`${uncodedRemaining} enforcement uncoded remaining`);
  if (failures.length) parts.push(`${failures.length} enforcement failure(s)`);
  const err = new Error(
    `Canonical code quality failed for ${type}: ${parts.join(', ')}${preview ? ` (${preview})` : ''}`,
  );
  err.statusCode = 422;
...
function storedDefinitionForPostPass(row) {
  return {
    type: row.type,
    code: row.ai_metadata?.code || row.provision_subtype || row.code || null,
    category: row.category || null,
    text: row.full_text || row.text || '',
    features: row.ai_metadata?.features || row.features || {},
  };
}

async function fetchStoredDefinitionsForPostPass(sb, dealId) {
  const { data, error } = await sb
    .from('provisions')
    .select('type, category, full_text, ai_metadata')
    .eq('deal_id', dealId)
    .eq('type', 'DEF');
  if (error) throw new Error(`Definition lookup failed: ${error.message}`);
  return data || [];
}

function hydratePerTypeCrossTypeFeatures(type, extracted, storedDefinitions) {
  if (type !== 'REP-T' && type !== 'REP-B') return extracted;
  const definitions = (storedDefinitions || []).map(storedDefinitionForPostPass);
  linkKnowledgeScopeToReps([...definitions, ...extracted]);
  return extracted;
}
...
async function runExtractTypePhase({ dealId, type, sb, client, dryRun = false }) {
  ...
  // linkKnowledgeScopeToReps here — those operate across REP+DEF/COND and
  // only make sense in the all-types path.
  try {
    extracted._codeEnforcementReport = await enforceCanonicalCodes(extracted, client);
    extracted._codeMergeReport = await consolidateProposedCodes(extracted, client);
  } catch (err) {
    console.warn('[run-extract] code enforcement failed:', err.message);
    extracted._codeEnforcementReport = {
      failures: [{ reason: `exception: ${err.message}`, count: extracted.length }],
    };
  }
  preserveOpenWorldDisposition(extracted);

  if (type === 'REP-T' || type === 'REP-B') {
    const definitions = await fetchStoredDefinitionsForPostPass(sb, dealId);
    hydratePerTypeCrossTypeFeatures(type, extracted, definitions);
  }
  ...
  assertOpenWorldDispositions(typeSections, finalProvisions);
  assertCanonicalFamilyQuality(type, finalProvisions, validation);
  ...
  await updateExtractionMetadata(sb, dealId, type, {
    status: 'done',
...
async function updateExtractionMetadata(sb, dealId, type, statusEntry, runRecord) {
  const { data: fresh, error: fetchErr } = await sb
    ...
    .single();
  if (fetchErr) {
    console.warn('[run-extract] metadata refresh failed:', fetchErr.message);
    return;
  }
  const latest = fresh?.metadata || {};
  const nextStatus = {
    ...(latest.extract_status || {}),
    [type]: statusEntry,
  };
  const nextMetadata = appendRunRecord({ ...latest, extract_status: nextStatus }, runRecord);
