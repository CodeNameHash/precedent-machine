...
function storedDefinitionForPostPass(row) {
  return {
    type: row.type,
    code: row.provision_subtype || row.code || null,
...
async function runExtractTypePhase({ dealId, type, sb, client, dryRun = false }) {
  ...
  if (type === 'REP-T' || type === 'REP-B') {
    const { data: definitions, error: definitionsError } = await sb
      .from('provisions')
      .select('type, provision_subtype, category, full_text, ai_metadata')
      .eq('deal_id', dealId)
      .eq('type', 'DEF');
    if (definitionsError) {
      throw new Error(`Definition lookup failed: ${definitionsError.message}`);
    }
