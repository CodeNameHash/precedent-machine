...
const PARTY_ANCHORS = [
  ...
  // ── Notices-block address headers ("if to Parent:", "if to the Company:")
  //    These precede the "with a copy to: <Firm>" advisor lines and give the
  //    party for any firm hit within the same notice block. ──
  { re: /if\s+to\s+(?:the\s+)?(?:parent|buyer|acqui|purchaser|merger\s+sub)/i, party: 'parent' },
  { re: /if\s+to\s+(?:the\s+)?(?:company|target|seller)/i, party: 'company' },
];
...
const PARTNER_CUE = /(?:attention|attn|lead\s+counsel|principal\s+contact|primary\s+contact|email)\s*:?\s*([A-Z][A-Za-z\.\-']{1,20}(?:\s+[A-Z][A-Za-z\.\-']{1,25}){1,3})/gi;
...
function getScanRegions(text) {
  ...
  // ── Notices region ──
  //  The Notices section is where advisor firms most reliably appear, in the
  //  "with a copy to: <Firm>, Attention: <Partner>" blocks under each party's
  //  notice address. Locate the "Notices" heading and scan the ~6000 chars
  //  that follow it. This is the body region the preamble/signature scan
  //  misses. The "with a copy to" lines explicitly sit under "if to Parent" /
  //  "if to the Company", giving a strong party signal.
  const noticeRe = /\bNotices?\b[^.\n]{0,40}(?:\.|\n|shall\s+be|must\s+be|to\s+be\s+(?:given|effective))/i;
  const nm = noticeRe.exec(text);
  if (nm) {
    const start = nm.index;
    regions.push({ name: 'notices', text: text.slice(start, start + 6000), offset: start });
  }
  return regions;
}
...
function nearestParty(regionText, firmStart, firmEnd) {
  // Look further BACK than forward: in a Notices block the "if to the
  // Company:" header sits above the address + "with a copy to: <Firm>" line,
  // often 300-500 chars before the firm name.
  const windowStart = Math.max(0, firmStart - 600);
  ...
  for (const anchor of PARTY_ANCHORS) {
    ...
    while ((m = localRe.exec(slice)) !== null) {
      ...
      const before = absPos < firmStart;
      const distance = before
        ? firmStart - (absPos + m[0].length)
        : absPos - firmEnd;
      // A firm belongs to the notice block whose header PRECEDES it, so a
      // following anchor (the next party's block) is heavily penalized — it
      // only wins if there's no preceding anchor at all in the window.
      const safeDist = Math.max(0, distance) + (before ? 0 : 5000);
...
function nearestPartner(regionText, firmStart, firmEnd) {
  ...
  let best = null;
  let bestDistance = Infinity;
  const reGlobal = new RegExp(PARTNER_CUE.source, 'gi');
  let m;
  while ((m = reGlobal.exec(slice)) !== null) {
    // Trim trailing sentence punctuation captured from the source ("Schoen.").
