function detectDoubleDummyEquity(section) {
  ...
  const buyerSignals = /(Parent|Buyer|Acquiror|New\s+Parent|Merger\s+Sub)\s+(?:Stock|Common\s+Stock|RSU|PSU|Option|Equity)/i;
  const targetSignals = /(Company|Target)\s+(?:Stock|Common\s+Stock|RSU|PSU|Option|Equity)/i;
