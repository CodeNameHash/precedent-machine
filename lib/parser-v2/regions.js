const REGION_TYPES = Object.freeze({
  ...
  BACKMATTER_UNCLASSIFIED: 'backmatter.unclassified',
  BACKMATTER_INDEX: 'backmatter.index',
  BACKMATTER_OTHER: 'backmatter.unclassified',
});
...
function isProvisionType(type) {
  return type === REGION_TYPES.BODY_SECTION_PROVISION || type === REGION_TYPES.BODY_SECTION_DEFINITION;
}

function isPreambleType(type) {
  return typeof type === 'string' && type.startsWith('preamble.');
}

function isBackmatterType(type) {
  return typeof type === 'string' && type.startsWith('backmatter.');
}

function isClassifiable(type) {
  return isClassifiableRegionType(type);
}
