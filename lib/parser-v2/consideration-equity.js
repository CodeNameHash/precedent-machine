...
function buildConsiderationEquityProvision(prov) {
  ...
  // Collapse duplicate (instrumentType, span) rows before the invariant: two
  // AI-emitted instrument entries for the SAME instrument (differing only in
  // mention text) resolve to the same clause span — redundant, not corrupt
  // (CSRA RSUs, 2026-07-17). Cross-instrument span conflicts still throw.
  const seenKeys = new Set();
  const dedupedRows = treatmentRows.filter((t) => {
    const key = `${t.instrumentType}:${t.sourceSpanStart}`;
