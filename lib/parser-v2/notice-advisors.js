...
function findFirmCandidates(blockText, partyNames) {
  ...
  for (let i = 0; i < physical.length; i++) {
    ...
    // Firm name wrapped across the line break BEFORE an "&" ("Davis Polk\n&
    // Wardwell LLP") — the continuation line starts with the ampersand
    // instead of the prior line ending with one. Join the same way.
    const continuesWithAmpersand = /^&/.test(p.line.trim());
    if (last && (/[,&]\s*$/.test(last.text) || continuesWithAmpersand) && !CONTACT_LINE_RE.test(p.line)) {
