...
const { registryHasTagCode, taxonomyHasCode } = require('../schema/validation');
...
//   5. FLAG-ONLY freeze gate (TAX-1): a tagged-object's model-assigned `code`
//      is still stored verbatim in `canonical` exactly as before -- this
//      module never drops, rejects, or rewrites a stored value. But if that
//      code isn't in the feature's registry tag family
//      (lib/schema/validation.js registryHasTagCode) or its taxonomy
//      dictionary (taxonomyHasCode), it's an invented code (e.g.
//      PRIME_BLOOMBERG, INFORM) that will later humanize into a
//      plausible-looking pill on read (lib/queries/claims-adapter.js). Such
//      codes are console.warn'd and tallied in `invalidCodes` so a caller
//      can surface counts -- purely observational, not a gate on writes.
// ---------------------------------------------------------------------------
...
// THE claim identity, for every writer. A claim is identified by the card it
// is anchored to, the feature it came from, and its ordinal inside that
// feature's atomicValues() expansion -- so the same logical claim gets the
// same id no matter which path wrote it. scripts/backfill/
// claims-from-normalized.js and scripts/reprocess/rematerialize-claims.js
// both mint through this function for exactly that reason: public.claims.id
// is the upsert conflict target, so a second id scheme for the same logical
// claim does not produce a conflict -- it produces a duplicate row.
...
// Derived, non-claim entries that extract.js may attach to a provision's
// feature bag. They are not features the model asserted, so they are neither
// materialised as claims nor counted as registry-unknown -- reporting them
// as unknown attributes would put a permanent false signal in every deal's
// ingest summary. Currently: `spanClaims`, the span-accounting payload
// written by lib/parser-v2/span-claims.js when that pass is enabled (see
// its COORDINATE SPACE note; it is read by validate.js's residual report,
// never by the claims layer).
const DERIVED_FEATURE_KEYS = new Set(['spanClaims']);
...
// A claim atom's canonical was assigned from a tagged object's own `code`
// field (as opposed to `canonicalForScalar`'s exact registry enumSet match,
// which is valid by construction). Only these atoms need the TAX-1 freeze
// check -- the model invents `code` values freely, unlike enum scalars.
function isTaggedCodeAtom(atom) {
  return atom.canonical !== null && atom.canonical !== undefined
    && isPlainObject(atom.rawFeatureValue) && 'code' in atom.rawFeatureValue;
}

// TAX-1 freeze gate: is `code` a real member of this feature's tag family,
// taxonomy dictionary, or (for enum-valued features whose codes are tagged
// rather than bare-scalar, e.g. effortsStandard's dash-case enumSet, which
// isn't mirrored in lib/taxonomy.js) its own registry enumSet? FLAG-ONLY --
// callers use this to warn/tally, never to reject or alter what gets stored
// (see file header, rule 5).
function isRegisteredCode(registryEntry, code) {
  if (registryHasTagCode(registryEntry, code) || taxonomyHasCode(registryEntry, code)) return true;
  if (registryEntry && Array.isArray(registryEntry.enumSet)) {
    const normalized = String(code).trim().toLowerCase();
    return registryEntry.enumSet.some((candidate) => String(candidate).trim().toLowerCase() === normalized);
  }
  return false;
}
...
function buildClaimRowsForCard(dealId, cardRow, prov, options = {}) {
  ...
  for (const [attribute, rawValue] of Object.entries(featureBag)) {
    if (DERIVED_FEATURE_KEYS.has(attribute)) continue;
    ...
    atoms.forEach((atom, index) => {
      if (isTaggedCodeAtom(atom) && !isRegisteredCode(registryEntry, atom.canonical)) {
        invalidCodes.push({ dealId, attribute, code: atom.canonical, excerptId: cardRow.excerpt_id });
        // eslint-disable-next-line no-console
        console.warn(`[store-claims] deal ${dealId}: invalid taxonomy code "${atom.canonical}" for feature "${attribute}" (excerpt ${cardRow.excerpt_id}) -- stored as-is, flagged only`);
      }
      rows.push({
  ...
  return { rows, unknownAttributes, invalidCodes };
}
...
function buildClaimRowsForDeal(dealId, cardRows, provisions, options = {}) {
  ...
  for (let i = 0; i < cardRows.length; i += 1) {
    const { rows: cardClaims, unknownAttributes, invalidCodes: cardInvalidCodes } = buildClaimRowsForCard(dealId, cardRows[i], provisions[i], options);
    ...
    invalidCodes.push(...cardInvalidCodes);
  }
  return { rows, unknownAttributeCounts, invalidCodes };
}
...
async function storeClaimsForDeal(dealId, cardRows, provisions, sb, options = {}) {
  ...
  const { rows, unknownAttributeCounts, invalidCodes } = buildClaimRowsForDeal(dealId, cardRows, provisions, options);
