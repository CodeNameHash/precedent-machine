...
const { validateFeatures, validationSummary } = require('../feature-validation');
...
async function storeProvisions(dealId, provisions, agreementText, title, sb, extras = {}) {
  ...
  const provisionsForInsert = dedupeProvisions(provisions);
  ...
  if (provisionsForInsert.length > 0) {
    ...
    const buildRichRow = (prov) => {
      // Flag malformed feature shapes at the door (see lib/feature-validation).
      const validation = validationSummary(validateFeatures(prov.type, prov.features || {}));
      return {
        ...buildCoreRow(prov),
        ai_metadata: {
