 ...
 *
 * Persistence for everything extraction can produce, not just core
 * provisions:
 *   - Core provisions: store raw agreement text, delete existing provisions
 *     + annotations for the deal (clean slate), batch-insert the new set
 *     (type/code-aware dedup: see dedupeProvisions).
 *   - Deal topology + transaction steps, for multi-step mergers (tender
 *     offer into back-end merger, double mergers, etc.).
 *   - Election mechanisms, proration rules, and consideration-equity
 *     provisions/treatments, archiving the prior rows before replacement.
 * The topology/election/consideration-equity paths each degrade gracefully
 * (return null / skip) rather than throwing when their own optional schema
 * (WP-SCHEMA-02, the consideration-equity tables) hasn't been migrated in a
 * given environment, so core-provisions storage keeps working regardless.
 *
