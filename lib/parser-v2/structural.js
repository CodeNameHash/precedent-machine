...
function parseArticles(body, bodyStart) {
  ...
  while ((m = artPattern.exec(body)) !== null) {
    ...
    if (sameLine.length > 2) {
      ...
      // Case 2: title on the next line
      const afterMatch = body.substring(matchStart + m[0].trimStart().length);
...
function parseSections(body, bodyStart, bodyEnd, articles) {
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    // Extract title: strip "Section X.XX" prefix and punctuation
    const titleMatch = headingLine.match(
      /^(?:SECTION|Section)\s+\d+\.\d{1,2}\s*[.:\s]\s*(.*)/
    );
    let title = titleMatch ? titleMatch[1].trim() : headingLine;
    // Remove trailing period from title
    title = title.replace(/\.\s*$/, '').trim();
    // If the title has body text after it (sentence continuation), truncate at first period
    const periodIdx = title.indexOf('.');
    if (periodIdx > 0 && periodIdx < title.length - 1) {
      title = title.substring(0, periodIdx).trim();
    }
...
module.exports = { parseStructure, cleanText };
