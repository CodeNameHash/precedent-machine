...
const { coverageDiagnostics } = require('./coverage');
const { detectPreambleRegions } = require('./regions/preamble');
const { detectBackmatterRegions } = require('./regions/backmatter');
const { withAtomicMetadata } = require('./regions/atomic');
const { analyzeDefinitionCompleteness } = require('./regions/definitions');
const { detectDoubleDummyEquity } = require('./regions/double-dummy');
const { normalizeBoundaryNoise } = require('./text-layers');
const { stripDefinedTermsToc } = require('./detectors/defined-terms-toc');
const { bodyStartAfterEnactingHardWall } = require('./invariants/enacting-hard-wall');
...
function cleanText(rawText) {
  ...
  text = normalizeBoundaryNoise(text);

  // Repair headings that 2015-era EDGAR HTML hard-wrapped mid-heading
  // ("7.1\nConditions to...", "9.5 GOVERNING LAW AND VENUE;\nSPECIFIC
  // PERFORMANCE.") BEFORE structural parsing — the same repairs run inside
  // cleanSectionText, but that is post-segmentation and cannot rescue a
  // heading the section scanner never saw. Runs after cleanEdgarText so
  // page-number lines between heading fragments are already gone.
  ...
  text = stripDefinedTermsToc(text);
...
function stripDisplayMarkers(text) {
  ...
  // FIRST: rescue any runaway [[TOC_START]] block before the TOC-dropping
  // passes below delete it wholesale. See repairRunawayTocBlock.
  text = repairRunawayTocBlock(text);
  ...
  text = text.replace(/\[\[\/?TOC_[A-Za-z0-9_ -]+\]\]/g, '');
  ...
  text = text.replace(/\[\[([A-Za-z0-9_ -]+)\]\]([\s\S]*?)\[\[\/\1\]\]/g, '$2');
...
/**
 * Rescue a runaway [[TOC_START]] block before stripDisplayMarkers deletes it.
 *
 * A [[TOC_START]]...[[/TOC_START]] block is only ever supposed to wrap the
 * front-of-document table of contents. Early versions of the display
 * formatter (markTableOfContents) computed the block's end with a
 * findBodyStart that did not understand bare-numbered body headings
 * ("1.1. The Merger." with no "Section" prefix), so on those documents the
 * closing tag landed hundreds of KB too late and the block swallowed most of
 * the agreement body (Whole Foods: preamble + Articles I-VI, 81% of the
 * document; Dyax: 58%). Worse, the formatter's TOC-entry walker joined every
 * line of the swallowed spans with single spaces, collapsing whole articles
 * into giant one-line [[TOC_ENTRY]] blocks.
 *
 * Deleting such a block (the normal handling — a real TOC is redundant with
 * parseStructure's own TOC detection) would discard the body. Instead:
 *   1. Detect the runaway: a genuine TOC block never contains body markers
 *      ([[DEFINED]] / [[REF]] / [[ARTICLE]] / [[SECTION]]) and never runs
 *      anywhere near this long.
 *   2. Unwrap [[TOC_ARTICLE]]ARTICLE X -- Title[[/TOC_ARTICLE]] back into a
 *      blank-line-anchored "ARTICLE X" heading + title line (the body
 *      articles that were mislabelled as TOC articles need to look like real
 *      headings again; the genuine TOC copies become plain stub lines, which
 *      findBodyStart already knows how to skip).
 *   3. Restore [[TOC_ENTRY]]NUM|TEXT|PAGE[[/TOC_ENTRY]] blocks whose "title"
 *      is clearly swallowed body prose (very long), and drop the short ones
 *      (genuine TOC entries).
 *   4. Re-break the collapsed one-line spans: reinsert a blank line before
 *      heading-shaped tokens ("ARTICLE VI", "5.2 Representations and
 *      Warranties ... Sub.") that ended up mid-line, so the structural
 *      parser's blank-line-anchored heading patterns can see them again.
 *
 * Documents with well-formed (small, marker-free) TOC blocks never take this
 * path — the block is left untouched for the normal drop below.
 */
function repairRunawayTocBlock(text) {
  const m = text.match(/\[\[TOC_START\]\]([\s\S]*?)\[\[\/TOC_START\]\]/);
  if (!m) return text;
  const block = m[1];

  // Size is the discriminator, not markers alone: healthy TOC blocks run
  // 3-13k chars (2-4% of the document) but can contain the odd stray
  // [[REF]]/[[DEFINED]] marker (Anadarko, Pharmasset). Runaway blocks run
  // 190-206k chars (58-81% of the document).
  const implausiblyLarge = block.length > Math.max(30000, text.length * 0.25);
  // POSITION is a second, independent discriminator: a genuine TOC block
  // only ever wraps the FRONT-of-document table of contents, so it starts
  // within the first quarter of the text. A block that starts deep in the
  // document is bogus regardless of size — Kraft's stored text has an 18.7k
  // block at 53% of the document, where markTableOfContents anchored on the
  // in-prose "table of contents ... are for reference purposes only"
  // sentence in Section 9.04 (Interpretation) and swallowed Sections
  // 9.05-9.15 plus the signature block as [[TOC_ENTRY]]s. Deleting it (the
  // normal handling) discarded those sections and the body-end anchor.
  const startsLate = m.index > Math.max(30000, text.length * 0.25);
  if (!implausiblyLarge && !startsLate) return text;

  let repaired = block;

  // (2) TOC_ARTICLE → standalone heading + title line.
  repaired = repaired.replace(
    /\[\[TOC_ARTICLE\]\]([\s\S]*?)\[\[\/TOC_ARTICLE\]\]/g,
    (_, inner) => '\n' + inner.trim().replace(/\s+--\s+/, '\n\n') + '\n'
  );

  // (3) TOC_ENTRY → drop genuine entries, restore swallowed body text.
  //
  // Inside a runaway block these entries are NOT TOC entries at all: the
  // formatter matched a line-leading "Section X.Y" that was really a WRAPPED
  // CROSS-REFERENCE continuation ("...within the meaning of Treasury
  // Regulation\nSection 1.6011-4(b)(2)..." — its regex split the number as
  // "Section 1.60|11-4(b)(2)...") and then swallowed everything up to the
  // next page-number-looking line into the entry's "title". Restore them as
  // MID-LINE continuations of the preceding text (joining the newline away),
  // so the "Section X.Y" token reads as the cross-reference it originally
  // was rather than a line-leading phantom heading.
  repaired = repaired.replace(/[ \t]*\n[ \t]*(?=\[\[TOC_ENTRY\]\])/g, ' ');
  repaired = repaired.replace(
    /\[\[TOC_ENTRY\]\]([\s\S]*?)\[\[\/TOC_ENTRY\]\]/g,
    (_, inner) => {
      const firstPipe = inner.indexOf('|');
      const lastPipe = inner.lastIndexOf('|');
      if (firstPipe < 0) return inner + '\n';
      const num = inner.substring(0, firstPipe).trim();
      const body = (lastPipe > firstPipe
        ? inner.substring(firstPipe + 1, lastPipe)
        : inner.substring(firstPipe + 1)
      ).trim();
      // Short "title" → a real TOC entry; drop it like the normal path would.
      if (body.length <= 160) return '';
      // The formatter's entry regex ate an optional "." + whitespace after
      // the number. If the remainder starts with a non-letter the number was
      // simply split mid-token ("Section 1.60" + "11-4(b)(2)", "Section 6.2"
      // + "(g)") — rejoin with nothing; otherwise a period/space was eaten —
      // rejoin with a space.
      // A swallowed SECTION HEADING (Kraft: "SECTION 9.05|Severability. If
      // any term ...") — the entry's "number" is a full "SECTION X.YZ" token
      // and its "title" opens with a Title-Case heading phrase ending in a
      // period. Restore it as a blank-line-anchored heading line so the
      // section scanner can see it again. (Wrapped cross-references —
      // "Section 2.02|(c) and cash in lieu ..." / "Section 9.15|shall not
      // ..." — don't take this shape and keep the mid-line join below.)
      if (
        /^(?:SECTION|Section)\s+\d{1,2}\.\d{1,2}$/.test(num) &&
        /^[A-Z][A-Za-z0-9 ,;:'’&()\-\/]{1,90}\.(?:\s|$)/.test(body)
      ) {
        return '\n\n' + num + '. ' + body + '\n';
      }
      const sep = /^[A-Za-z]/.test(body) ? ' ' : '';
      return num + sep + body + '\n';
    }
  );

  // Strip any leftover TOC tags inside the block.
  repaired = repaired.replace(/\[\[\/?TOC_[A-Za-z0-9_ -]+\]\]/g, '');

  // Re-anchor the attestation clause. When a swallowed block reaches the end
  // of the agreement (Kraft), the signature lead-in ends up glued mid-line by
  // the entry joins above — and findBodyEnd's line-anchored
  // /\n\s*IN WITNESS WHEREOF/ can no longer see it, so the body overruns
  // into the bundled exhibits (charter, by-laws, registration rights).
  repaired = repaired.replace(/([^\n])[ \t]+(?=IN WITNESS WHEREOF\b)/, '$1\n\n');

  // (4) Re-break heading tokens the formatter collapsed mid-line. Only very
  // long lines can be collapsed spans, so leave normal lines untouched.
  repaired = repaired
    .split('\n')
    .map((line) => {
      if (line.length < 300) return line;
      let out = line;
      // "... plans). ARTICLE VI Covenants 6.1 Interim Operations. ..."
      out = out.replace(
        /([.;:)"'’”]) (ARTICLE [IVXLCDM]+\b)/g,
        '$1\n\n$2'
      );
      // "... Exchange Act. 5.2 Representations and Warranties of Parent. ..."
      // Sentence-boundary + bare "X.Y[.]" + Title-Case phrase ending in a
      // period — the same shape parseSections' strict bare-heading pattern
      // accepts. Cross-references ("pursuant to Section 6.2") are preceded by
      // a letter, not punctuation, so they never match. An optional short
      // Title-Case run between the sentence boundary and the number is a
      // collapsed ARTICLE TITLE line ("... ratably adjusted. Representations
      // and Warranties 5.1 Representations and Warranties of the Company.",
      // "... cost-related plans). Covenants 6.1 Interim Operations.") — break
      // it onto its own line too.
      out = out.replace(
        /([.;:)"'’”]) ((?:[A-Z][A-Za-z'’&-]*(?: (?:and|of|the|for|[A-Z][A-Za-z'’&-]*)){0,5} )?)((?:Section )?\d{1,2}\.\d{1,2}\.? +(?=[A-Z][^\n]{2,90}\.))/g,
        (_, punct, title, head) =>
          title ? `${punct}\n\n${title.trim()}\n\n${head}` : `${punct}\n\n${head}`
      );
      return out;
  ...
  return text.substring(0, m.index) + repaired + text.substring(m.index + m[0].length);
}
...
function stripDefinedTermsIndex(text) {
  ...
  // Acceptable refs: 1.02, 9.03, 2.01(d), 3.11(i)(i), Preamble, Corpuss,
  // Section X.XX, Article X, sometimes with sub-clause letters.
  const refLineRe = /^(?:(?:Section\s+|Article\s+)?\d{1,2}\.\d{1,2}(?:\([a-z0-9]+\))*|Preamble|Corpuss|Article\s+(?:[IVXLCDM]+|\d+))$/i;
...
function findBodyStart(fullText) {
  const legacyStart = findBodyStartLegacy(fullText);

  // TOC-LESS BARE-DECIMAL FALLBACK: agreements with NO "TABLE OF CONTENTS",
  // no surviving "ARTICLE" heading line, and no "SECTION"/"Section" keyword
  // on their very first section open directly with a bare "1.1 Title." (or
  // "1.01 Title.") line right after the cover page — no preamble/corpuss
  // ("WHEREAS", "is entered into as of", ...) survives cleanup either, so
  // every path above (which all anchor on a preamble, a TOC, or a keyworded
  // "SECTION 1.0?1") comes up empty on the REAL body. When the stored text
  // also has an APPENDED exhibit that carries its own genuine TOC/preamble/
  // ARTICLE structure (a CVR agreement, bylaws, a certificate of
  // incorporation), the legacy cascades happily anchor on THAT instead —
  // Dyax's stored text anchors 266k chars in, inside the appended "CONTINGENT
  // VALUE RIGHTS AGREEMENT" exhibit's own Article I, discarding the entire
  // merger agreement (Articles I-IX).
  //
  // Only distrust the legacy result once it lands implausibly deep into the
  // document — mirror repairRunawayTocBlock's discriminator for the same
  // shape of bug (a later, unrelated structure hijacking an anchor meant for
  // the front of the document).
  const tooLate = legacyStart > Math.max(30000, fullText.length * 0.25);
  if (tooLate) {
    const bareTop = findBareDecimalBodyStart(fullText);
    if (bareTop !== null && bareTop < legacyStart) return bareTop;
  }
...
/**
 * Detect a bare "1.1 Title." / "1.01 Title." heading near the top of the
 * document — the first-section shape used by TOC-less agreements whose
 * ARTICLE heading and "SECTION"/"Section" keyword did not survive cleanup
 * (Dyax: body opens "1.1 The Merger. Upon the terms and subject to the
 * conditions set forth in this Agreement, ..." immediately after the cover
 * page, with no WHEREAS recitals surviving either).
 *
 * Validated exactly like parseSections' strictBare pattern validates a bare
 * heading: blank-line anchored, then a Title-Case phrase ending in a period
 * on the same line. Both constraints matter for rejecting prose:
 *   - "in exchange for 1.1 million shares" never sits right after a blank
 *     line at the top of a merger agreement, and even where a stray blank
 *     line precedes it, "million" is lowercase — the required
 *     `[A-Z][^\n]{2,90}\.` right after the number can't match lowercase
 *     body prose.
 *   - A bare cross-reference number is never followed by a title-case,
 *     period-terminated phrase on its own line.
 *
 * Restricted to the FRONT of the document (same 25k window the preamble
 * anchor below uses for "front matter") — a "1.1"/"1.01" match found deep in
 * the document is far more likely to be an appended exhibit's own Article I
 * than the real merger-agreement body, which is exactly the confusion this
 * fallback exists to correct, not reproduce.
 */
function findBareDecimalBodyStart(fullText) {
  const NEAR_TOP_LIMIT = 25000;
  const window = fullText.substring(0, NEAR_TOP_LIMIT + 200);
  const bareTopRe = /\n[ \t]*\n[ \t]*(1\.0?1)\.?[ \t]+(?=[A-Z][^\n]{2,90}\.)/;
  const m = bareTopRe.exec(window);
  if (!m) return null;
  if (m.index > NEAR_TOP_LIMIT) return null;
  // Keep the leading blank line inside the returned offset (mirrors the
  // preamble path's bareM handling below) so parseSections' strict
  // bare-heading pattern — which itself requires a blank line before the
  // number — can still recognise section 1.1 as a heading once the body
  // slice starts here.
  return m.index;
}

function findPreambleMatch(searchText) {
  const patterns = [
    /\bNOW,\s+THEREFORE\b[\s\S]{0,900}?\bagree\s+as\s+follows\b/i,
    /\bNOW\s+THEREFORE\b[\s\S]{0,900}?\bagree\s+as\s+follows\b/i,
    /\bIT\s+IS\s+HEREBY\s+AGREED\b/i,
    /\bTHE\s+PARTIES\s+HEREBY\s+AGREE\b/i,
    /\bTHE\s+PARTIES\s+AGREE\s+AS\s+FOLLOWS\b/i,
    /\bThe\s+Parties,\s+intending\s+to\s+be\s+legally\s+bound\b[\s\S]{0,500}?\bagree\s+as\s+follows\b/i,
    /\bW\s?I\s?T\s?N\s?E\s?S\s?S\s?E\s?T\s?H\b/i,
    /\n[ \t]*WHEREAS\b/i,
    /AGREEMENT[^\n]{0,160}?\bdated\s+as\s+of\b/i,
    /is\s+made\s+and\s+entered\s+into\s+as\s+of/i,
    /is\s+entered\s+into\s+as\s+of/i,
  ];

  for (const pattern of patterns) {
    const match = pattern.exec(searchText);
    if (match && match.index < 25000) return match;
  }
  return null;
}

function hasSubstantiveInlineSectionContent(rest) {
  const text = String(rest || '')
    .replace(/^\s*\.?\s*/, '')
    .replace(/\.{3,}\s*\d{1,4}\s*$/, '')
    .trim();
  if (!text) return false;
  return /^[A-Z][^\n.]{2,180}\.\s+(?:[A-Z(]|["“])/.test(text);
}

function findBodyStartLegacy(fullText) {
  // Require the TOC header to appear as a standalone-line heading (preceded
  // by a blank line, modulo leading whitespace). The old plain
  // /TABLE\s+OF\s+CONTENTS/i match also fired on the in-body prose
  // reference "the table of contents and headings herein are for convenience
  // of reference" which most agreements include in their miscellaneous
  // section. In agreements whose front-of-doc TOC has no visible "TABLE OF
  // CONTENTS" header (Landos — the original HTML used a table without a
  // header row), that prose match landed near EOF, the post-TOC scan failed
  // to find a sane body section, and the fallback then picked the FIRST
  // "Section 1.1" line in the file — which was the TOC stub. Anchoring to
  // `\n\n` rules out the prose reference (preceded by " (b) The ") while
  // still matching real headings (preceded by blank lines).
  const tocMatch = fullText.match(/\n\n[ \t]*TABLE\s+OF\s+CONTENTS\b/i);

  // HIGH-PRECISION PATH: anchor on the agreement PREAMBLE. Every merger
  // agreement opens its body with a preamble sentence ("AGREEMENT AND PLAN
  // OF MERGER, dated as of ...", "... is made and entered into as of ...")
  // and/or corpuss ("W I T N E S S E T H", "WHEREAS, ..."), which appear
  // exactly ONCE between the front matter and Article I and never inside a
  // TOC or a defined-terms index. The same-line-content cascades below are
  // brittle in both directions — CSRA's body articles have short section
  // titles on the header line ("Section 2.1 The Offer.") so every article
  // until IX was rejected as a TOC stub (96% of the body discarded);
  // Covance's TOC entries carry long same-line titles ("Section 1.05.
  // Certificate of Incorporation and Bylaws") so the body start landed
  // INSIDE the TOC and every "section" was a stub. The preamble sidesteps
  // that entire class: body start = the first ARTICLE (or Section 1.x)
  // heading AFTER the preamble. Falls through to the legacy cascades when no
  // preamble matches.
  //
  // Runs whether or not a "TABLE OF CONTENTS" header exists. Kraft's filing
  // has a HEADERLESS stub TOC (cover page, then straight into "SECTION
  // 1.01.\n\nThe Merger" stubs) and body headings whose titles are wrapped
  // onto the next line ("SECTION 1.01.\nThe Merger. (a) On the terms ...") —
  // no same-line content anywhere — so the sec-1.01 fallback scanned all the
  // way into the mid-document defined-terms index and anchored the body 274k
  // chars in, discarding every REP/COND/TERM section. When a TOC header
  // exists the search starts there (skipping cover-page noise); otherwise it
  // starts at the top of the document.
  {
    const searchBase = tocMatch ? tocMatch.index : 0;
    const searchText = tocMatch ? fullText.substring(searchBase) : fullText;
    {
      const pm = findPreambleMatch(searchText);
      // PROXIMITY GUARD: a true preamble sits directly after the TOC (or the
      // cover page) — the longest front-of-document TOC in the corpus runs
      // ~13k chars. A match far beyond that is a body cross-reference to
      // another contract ("... the Confidentiality Agreement, dated as of
      // April 27, 2017 ..." 170k into Whole Foods, whose real preamble was
      // lost at ingest); anchoring on it would discard everything before
      // that point.
      // NO-TOC GUARD: without a TOC header the search starts at the very
      // top of the document, and a defined term deep in a definitions
      // article can false-match the preamble regex ('"Confidentiality
      // Agreement" means the non-disclosure agreement, dated as of
      // December 4, 2017' — Bioverativ, whose real preamble was lost at
      // ingest and whose body opens directly with "Section 1.01.
      // Definitions."). A true preamble always PRECEDES the first
      // substantive body heading. TOC stubs (Kraft's headerless stub TOC:
      // "SECTION 1.01.\n\nThe Merger") carry no same-line content, so they
      // don't count as substantive and don't block the preamble anchor.
      let falsePreamble = false;
      if (pm && !tocMatch) {
        const headRe = /\n[ \t]*\n[ \t]*(?:SECTION|Section)\s+\d{1,2}\.\d{1,2}\b\.?[ \t]*([^\n]*)/g;
        let hm;
        while ((hm = headRe.exec(searchText)) !== null) {
          if (hm.index >= pm.index) break;
          if (hasSubstantiveInlineSectionContent(hm[1])) { falsePreamble = true; break; }
        }
      }
      if (pm && pm.index < 25000 && !falsePreamble) {
        const afterPreamble = searchText.substring(pm.index);
        // First blank-line-anchored ARTICLE heading after the preamble …
        const artM = afterPreamble.match(/\n\s*\n[ \t]*(ARTICLE\s+(?:[IVXLCDM]+|\d+))\b/i);
        // … or, for agreements with no ARTICLE level, the first Section 1.x …
        const secM = afterPreamble.match(/\n\s*\n[ \t]*(?:SECTION|Section)\s+1\.\d{1,2}\b/);
        // Redfin-style EDGAR HTML can collapse the body article and section
        // headings to single-newline spacing:
        //   AGREEMENT
        //   The Parties, intending to be legally bound...
        //   Article I
        //   DESCRIPTION OF TRANSACTION
        //   Section 1.1 The Merger. Upon the terms...
        // The blank-line anchored artM/secM probes above intentionally avoid
        // TOC stubs, but after a high-confidence preamble anchor we can accept
        // this tighter single-newline shape when Section 1.1 carries body
        // prose on the same line.
        let inlineSecM = null;
        {
          const section11 = afterPreamble.match(/\n[ \t]*(?:SECTION|Section)\s+1\.0?1\b[^\n]*/);
          const longInline = afterPreamble.match(/\n[ \t]*(?:SECTION|Section)\s+1\.\d{1,2}\b[^\n]{40,}/);
          if (section11 && (longInline && longInline.index >= section11.index)) {
            const beforeInline = afterPreamble.substring(0, section11.index);
            const inlineArtRe = /\n[ \t]*(Article\s+(?:I|1))\b/gi;
            let lastInlineArt = null;
            let iam;
            while ((iam = inlineArtRe.exec(beforeInline)) !== null) lastInlineArt = iam;
            inlineSecM = lastInlineArt
              ? { index: lastInlineArt.index, offset: 1 }
              : { index: section11.index, offset: 1 };
          }
        }
        // … or a bare-numbered "1.1 The Merger. Upon the terms..." heading
        // carrying substantive same-line content. Needed when the document's
        // early ARTICLE heading lines were lost entirely (Dyax's stored text
        // has section 1.1 4k after the preamble but its first surviving
        // "ARTICLE" line is ARTICLE VII, 186k later — anchoring on artM alone
        // discarded Articles I-VI).
        let bareM = null;
        const bareRe = /\n[ \t]*\n[ \t]*1\.0?1\.?[ \t]+([^\n]{40,})/g;
        let bm;
        while ((bm = bareRe.exec(afterPreamble)) !== null) {
          const rest = bm[1].replace(/\.{3,}\s*\d{1,4}\s*$/, '').trim();
          if (rest.length > 60 && /^[A-Z]/.test(rest)) { bareM = bm; break; }
        }
        // bareM keeps a 0 offset (not +1): its match starts at the first of
        // the TWO newlines before the number, and the strict bare-heading
        // pattern in parseSections needs that full blank line inside the
        // body slice to recognise the heading. artM/secM keep the historic
        // +1 (their heading patterns don't require a leading blank line).
        const candidates = [
          artM ? { index: artM.index, offset: 1 } : null,
          secM ? { index: secM.index, offset: 1 } : null,
          inlineSecM,
          bareM ? { index: bareM.index, offset: 0 } : null,
        ].filter(Boolean);
        if (candidates.length) {
          const best = candidates.reduce((a, b) => (b.index < a.index ? b : a));
          return searchBase + pm.index + best.index + best.offset;
        }
      }
    }
  }
  ...
  if (tocMatch) {
    ...
    // BARE-NUMBER PATH: agreements drafted without "Section" prefixes and
    // without a recoverable preamble (Whole Foods' stored text — the
    // preamble was lost at original ingest). The body opens with
    // "1.1. The Merger. Upon the terms and subject to the conditions..." —
    // a blank-line-anchored bare number carrying the title AND opening
    // prose on the same line. TOC stubs in the same drafting style put the
    // title on the NEXT line ("1.1.\n\nThe Merger"), and even same-line TOC
    // titles stay short, so requiring >60 chars of same-line content
    // separates body from TOC. Without this path the generic Section-scan
    // below matches a mid-body cross-reference ("Section 4.3 ...") and
    // walks back to a mid-body ARTICLE, discarding everything before it.
    {
      const bareRe = /\n[ \t]*\n[ \t]*1\.0?1\.?[ \t]+([^\n]+)/g;
      let bm;
      while ((bm = bareRe.exec(afterToc)) !== null) {
        const rest = bm[1].replace(/\.{3,}\s*\d{1,4}\s*$/, '').trim();
        if (rest.length > 60 && /^[A-Z]/.test(rest)) {
          // Walk back for the nearest preceding ARTICLE heading (body
          // ARTICLE I), capped like the no-TOC fallback's look-back.
          const lookbackStart = Math.max(0, bm.index - 8000);
          const before = afterToc.substring(lookbackStart, bm.index + 1);
          const artRe = /\n[ \t]*\n[ \t]*ARTICLE\s+(?:[IVXLCDM]+|\d+)\b/gi;
          let lastArtStart = -1;
          let am;
          while ((am = artRe.exec(before)) !== null) {
            lastArtStart = am.index + am[0].search(/ARTICLE/i);
          }
          if (lastArtStart >= 0) return tocMatch.index + lookbackStart + lastArtStart;
          // No offset: keep the blank line before the number inside the body
          // slice so the strict bare-heading pattern can match section 1.1.
          return tocMatch.index + bm.index;
        }
      }
    }
    ...
    while ((sm = secPattern.exec(afterToc)) !== null) {
      ...
      // BUT a stray cross-reference inside the corpuss (e.g. "Section 3.1(b)
  ...
  // Find the FIRST Section 1.0?1 heading that has substantive inline
  // content on the same line. Body sections render as "Section 1.1. The
  // Merger. On the terms and subject to the conditions..." (title and prose
  // all inline); TOC stubs render as "Section 1.1" on a line by itself with
  // the title on the next non-empty line. Without this discriminator, an
  // implicit front-of-doc TOC (no "TABLE OF CONTENTS" header — Landos)
  // makes bodyStart land at the first TOC entry and every extracted
  // provision ends up being a TOC stub.
  const sec101Re = /\n\s*(?:SECTION|Section)\s+1\.0?1\b/g;
  let firstSec101 = null;
  let sm101;
  while ((sm101 = sec101Re.exec(fullText)) !== null) {
    // Find the end of the line containing the section number. Look from the
    // position RIGHT AFTER the matched "Section 1.X" (the match may have
    // consumed multiple leading newlines via `\s*`, so we can't measure from
    // sm101.index itself).
    const afterNumIdx = sm101.index + sm101[0].length;
    const lineEnd = fullText.indexOf('\n', afterNumIdx);
    const rest = fullText.substring(afterNumIdx, lineEnd === -1 ? fullText.length : lineEnd);
    if (hasSubstantiveInlineSectionContent(rest)) {
      firstSec101 = sm101;
      break;
    }
  }
  ...
  if (firstSec101) {
    // Walk backwards from the substantive Section 1.X to find the nearest
    // preceding ARTICLE I heading. We anchor on the substantive section
    // (not on a free-floating ARTICLE match) because the front-of-doc TOC
    // also contains "ARTICLE II"-style headings on their own line, and a
    // free firstArt match would land on those TOC stubs. Cap the look-back
    // to 8k chars so we can't accidentally pick a TOC article that's
    // hundreds of KB earlier in a document with an unusual TOC layout.
    const lookbackStart = Math.max(0, firstSec101.index - 8000);
    const before = fullText.substring(lookbackStart, firstSec101.index);
    const artRe = /\n\n[ \t]*Article\s+(?:[IVXLCDM]+|\d+)\b/gi;
    let lastArtIdx = -1;
    let am;
    while ((am = artRe.exec(before)) !== null) lastArtIdx = am.index;
    if (lastArtIdx >= 0) return lookbackStart + lastArtIdx + 2;
    return firstSec101.index + 1;
  }

  // Last resort — no Section 1.0?1 found anywhere. Try the standalone
  // ARTICLE heading match (preserves prior behaviour for malformed
  // agreements that drop Section numbering entirely).
  const firstArt = fullText.match(/\n\n[ \t]*Article\s+(?:[IVXLCDM]+|\d+)\b(?=\s*(?:\n|--|—))/i);
  if (firstArt) return firstArt.index + 2;
  return 0;
}
...
function findBodyEnd(fullText, bodyStart) {
  ...
  const sigPatterns = [
    ...
    // Trailing-exhibit markers. These must look like HEADINGS — blank-line
    // anchored, with the line carrying nothing after the designator except a
    // dash/colon or a Title-Case tail. A merely line-LEADING "Exhibit B; and"
    // is a hard-wrapped in-body cross-reference: Catalent §1.05 ("...amended
    // and restated to conform to\nExhibit B; and") matched the old pattern at
    // char 7264 of a 337k-char agreement, truncating the parse to Article I
    // and gutting the whole deal's extraction (2026-07-17).
    /\n[ \t]*\n[ \t]*(?:EXHIBIT|Exhibit)\s+[A-Z0-9.-]+\s*(?:\n|[—–:-]\s|[A-Z][^\n]*)/,
    /\n[ \t]*\n[ \t]*(?:FORM|Form)\s+OF\s+(?:CERTIFICATE|CHARTER|BYLAWS?|VOTING|SUPPORT|LETTER|CVR|CONTINGENT\s+VALUE|REGISTRATION\s+RIGHTS)\b[^\n]*/,
  ];
...
function parseSections(body, bodyStart, bodyEnd, articles) {
  ...
  // Bare "X.Y Title." headings with NO "Section" prefix — the Pharmasset /
  // Whole Foods drafting style ("5.1 Interim Operations."). These documents
  // still contain plenty of "Section X.XX" strings as CROSS-REFERENCES, so
  // the sparse (<5) fallback below never fired and whole articles collapsed
  // into INTRO blobs. Run a STRICT variant unconditionally: blank-line
  // anchored, Title-Case start, period-terminated heading phrase — a shape
  // cross-reference lists don't take. The heading/cross-ref classifier in
  // Step 2 still vets every match.
  {
    // The number may carry a trailing period of its own: Pharmasset writes
    // "5.1 Interim Operations." but Whole Foods writes "1.1. The Merger.".
    // Three accepted title shapes after the number:
    //   1. Period-terminated on the same line ("5.1 Interim Operations.").
    //   2. NO period, line ends, and the body opens on a "(a)" sub-clause
    //      after a blank line (Forest City 9.6 "Payment of Parent
    //      Termination Payment") — the "(" anchor keeps a bare number
    //      followed by arbitrary prose from matching.
    //   3. Hard-wrapped mid-title with the period on the continuation line
    //      (Prometheus 8.2 "Notice of Termination; Effect\nof
    //      Termination."). repairWrappedHeadings only rejoins wraps whose
    //      continuation starts with a capital, so lowercase continuations
    //      ("of ...") must be accepted here.
    const strictBare = /\n[ \t]*\n[ \t]*(\d{1,2}\.\d{1,2})\.?[ \t]*(?=[A-Z](?:[^\n]{2,130}\.|[^\n]{2,80}\n[ \t]*\n[ \t]*\(|[^\n]{2,80}\n[ \t]*[^\n]{1,80}\.))/g;
    let sm;
    while ((sm = strictBare.exec(body)) !== null) {
      if (bodyStart + sm.index >= bodyEnd) break;
      const idx = sm.index + sm[0].indexOf(sm[1]);
      if (!allMatches.some((a) => a.number === sm[1] && Math.abs(a.index - idx) < 20)) {
        allMatches.push({ index: idx, absIndex: bodyStart + idx, number: sm[1], fullMatch: sm[1] });
      }
    }
    allMatches.sort((a, b) => a.index - b.index);
  }

  // Some EDGAR table conversions preserve the heading line but remove the
  // separator after its bare section number ("4.3No Solicitation."). These
  // headings may follow the article title after only one newline, so they do
  // not satisfy strictBare's blank-line anchor. Admit only the exact no-space
  // shape at a line start with a bounded, period-terminated Title-Case heading;
  // Step 2 still applies isHeading and every cross-reference guard.
  {
    const noSpaceBare = /(?:^|\n)[ \t]*(\d{1,2}\.\d{1,2})(?=[A-Z][^\n]{2,130}\.)/g;
    let nm;
    while ((nm = noSpaceBare.exec(body)) !== null) {
      if (bodyStart + nm.index >= bodyEnd) break;
      const idx = nm.index + nm[0].indexOf(nm[1]);
      if (!allMatches.some((a) => a.number === nm[1] && Math.abs(a.index - idx) < 20)) {
        allMatches.push({
          index: idx,
          absIndex: bodyStart + idx,
          number: nm[1],
          fullMatch: nm[1],
        });
      }
  ...
  const headings = allMatches.filter(match => {
    ...
    const prevNonEmptyLine = (lookback.split('\n').map((line) => line.trim()).filter(Boolean).pop() || '');
    ...
    // Also catches list-continuation conjunctions ("and" / "or" / ","): a
    // wrapped cross-reference list "Section 4.1, Section 4.3 (...) and\n
    // Section 4.8 (...)" puts the trailing reference at the start of the next
    // line — without an "and" / "or" terminator here, the parser misidentified
    // that line-leading "Section 4.8" as a real body heading and prematurely
    // closed the current section. (Bug surfaced on Landos Section 6.2, which
    // was being truncated at sub-clause (a)(i) with sub-clauses (b) and (c)
    // never reaching the extractor.)
    // Bare "by" covers wrapped passives ("...would be prohibited by\n
    // Section 5.2 (other than ...)" — Skechers 3.12(b)); bare "this" covers
    // wrapped self-references ("...the provisions of this\nSection 6.3." —
    // Verve 6.3, which lost sub-clauses (b)-(d) of its no-shop to the
    // phantom). Prose never ends a paragraph on "by" or "this".
    if (/\b(?:in|under|of|to|from|by|this|and|or|pursuant\s+to|set\s+forth\s+in|described\s+in|defined\s+in|referenced\s+in|subject\s+to|accordance\s+with|provided\s+in|specified\s+in|required\s+by|referred\s+to\s+in|see|per|comply\s+with|violation\s+of|provisions\s+of|obligations\s+under|requirements\s+of|terms\s+of|meaning\s+of|contemplated\s+by)\s*[,;]?\s*$/i.test(prevLine)) {
      return false;
    }
    ...
    // A lowercase-word parenthetical right after the number is a scope
    // carve-out inside a cross-reference ("... prohibited by Section 5.2
    // (other than subsections 5.2(c), ...)" — Skechers 3.12(b)). A real
    // heading's title never opens with a lowercase parenthetical. Without
    // this, the phantom "5.2" heading inside Article III shadowed the REAL
    // 5.2 Forbearance Covenants section via the keep-first number dedupe,
    // orphaning its entire ~12k-char body.
    if (/^\s*\(\s*[a-z]/.test(afterNum)) return false;
    // Number terminated by a period at END OF LINE with no title after it
    // ("...of the provisions of this\nSection 6.3.\n\n (b) ...") is usually a
    // sentence-ending self-reference — UNLESS the next non-empty line opens
    // with a period-terminated Title-Case phrase, which is a real heading
    // whose title wrapped to its own line (Kraft "SECTION 2.02.\nExchange of
    // Certificates. (a) Exchange Agent. ...", Verve "Section 9.9.\nHeadings.
    // ..." — repairWrappedHeadings does not rejoin across the wrap when the
    // title line carries body text). Sub-clauses ("(b) Notwithstanding ...")
    // and prose continuations fail the title-phrase test.
    if (/^\s*\.\s*(?:\n|$)/.test(afterNum)) {
      const rest = body.substring(match.index + match.fullMatch.length);
      const restLines = rest.split('\n');
      let nextLine = '';
      for (let li = 1; li < Math.min(restLines.length, 4); li++) {
        if (restLines[li].trim()) { nextLine = restLines[li].trim(); break; }
      }
      if (!/^[A-Z][A-Za-z'’,()&\/ -]{2,90}\./.test(nextLine)) return false;
    }
    // A close-paren right after the number is a cross-reference inside a
    // parenthetical ("... treated in accordance\n\nwith Section 4.3) issued
    // and outstanding ..." / "... governed by\nSection 7.14);"). A real
    // heading never opens with ")". Without this, Forest City grew phantom
    // sections 4.3 and 7.14 — the latter swallowing 32k chars of Section 7.1.
    if (/^\s*\)/.test(afterNum)) return false;
    // A semicolon/colon/comma/period right after a wrapped Section number is
    // a cross-reference continuation, not a real heading. Conoco 6.1 contains
    // "... contemplated by\nSection 3.2; (D) pay..." and the parser must keep
    // that inside 6.1 rather than creating a phantom 3.2 section.
    if (/^\s*[;:,]/.test(afterNum)) return false;
    if (/^\s*(?:\n|$)/.test(afterNum) && /^Section\s+\d+\.\d{1,2}$/i.test(match.fullMatch.trim())) {
      const rest = body.substring(match.index + match.fullMatch.length, match.index + match.fullMatch.length + 160);
      const nextNonEmptyLine = rest.split('\n').map((line) => line.trim()).filter(Boolean)[0] || '';
      const tableCellRe = /^[A-Z0-9][A-Za-z0-9()\/&'’., -]{1,90}$/;
      const tableContext = body.substring(Math.max(0, match.index - 3000), match.index);
      if (
        /terms?\s+defined\s+elsewhere|defined\s+terms?\s+index|defined\s+in\s+this\s+Agreement\s+as\s+referenced|Section\s+set\s+forth\s+opposite|Location\s+of\s+Definition|Definition\s+Section/i.test(tableContext) &&
        tableCellRe.test(prevNonEmptyLine) &&
        tableCellRe.test(nextNonEmptyLine)
      ) {
        return false;
      }
    }
    ...
    if (/^\s*,\s*[a-z]/.test(afterNum)) return false;
    // "Section X.XX (Title-Case Words)" — the parenthetical-title shape is a
    // hallmark of cross-references inside a wrapped list of rep references
    // (e.g. "Section 3.2 (Certificate of Incorporation; Bylaws)"). Real body
    // headings render as "Section X.XX Certificate of Incorporation" without
    // parens. Combine with prev-line ending in "," / "and" / "or" / ";" to
    // limit the heuristic to actual list continuations.
    if (/^\s*\(\s*[A-Z][A-Za-z][^)]{2,80}\)/.test(afterNum) && /(?:,|;|\band\b|\bor\b)\s*$/i.test(prevLine)) {
      return false;
    }
...
function enrichSectionHierarchy(section) {
  const regionType = regionTypeForSection(section);
  const atomic = withAtomicMetadata(section);
  const definitionCompletenessWarnings = analyzeDefinitionCompleteness({ ...section, regionType });
  const needsDoubleDummyModel = detectDoubleDummyEquity(section);
  ...
  return {
    ...
    ...(needsDoubleDummyModel ? { needsDoubleDummyModel: true } : {}),
  };
}

function articleHeaderEnd(fullText, article, sections, bodyEnd) {
  const start = article && typeof article.startChar === 'number' ? article.startChar : null;
  if (start == null) return null;
  let stop = bodyEnd;
  let firstSectionStart = null;
  for (const section of sections || []) {
    if ((section.startChar || 0) > start && (section.startChar || 0) < stop) {
      stop = section.startChar;
      firstSectionStart = section.startChar;
    }
  }
  if (firstSectionStart != null) return firstSectionStart;

  const sliceEnd = Math.min(stop, start + 500);
  const slice = fullText.substring(start, sliceEnd);
  const secM = slice.match(/\n\s*(?:SECTION|Section)\s+\d{1,2}\.\d{1,2}\b|\n\s*\d{1,2}\.\d{1,2}\b/);
  if (secM) return start + secM.index;
  ...
  for (const line of lines) {
    ...
    pos = nextPos;
    if (nonEmpty >= 2) break;
    if (/^ARTICLE\b/i.test(trimmed) && /[-:—.]\s*\S/.test(trimmed)) break;
  }
  return Math.min(pos, stop);
}

function subtractOccupied(region, occupied) {
  let fragments = [{ start: region.start, end: region.end }];
  for (const occ of occupied) {
    ...
    for (const frag of fragments) {
      ...
      if (occ.start > frag.start) next.push({ start: frag.start, end: occ.start });
      if (occ.end < frag.end) next.push({ start: occ.end, end: frag.end });
    }
    fragments = next;
    if (fragments.length === 0) break;
  }
  const { start, end, length, ...extra } = region;
  return fragments.map((frag) => makeRegion(region.type, frag.start, frag.end, extra));
}

function nonOverlappingSectionRegions(sections) {
  const regions = [];
  let cursor = 0;
  for (const section of [...(sections || [])].sort((a, b) => (a.startChar || 0) - (b.startChar || 0))) {
    const start = Math.max(cursor, Number(section.startChar) || 0);
    const end = Math.max(start, Number(section.endChar) || 0);
    if (end <= start) continue;
    regions.push(makeRegion(section.regionType || regionTypeForSection(section), start, end, {
      source: 'section',
...
function fillRegionHoles(regions, length, bodyStart, bodyEnd) {
  const out = [...regions];
  const ordered = sortedRegions(out);
  let cursor = 0;
  const addGap = (start, end) => {
    if (end <= start) return;
    if (end <= bodyStart) {
      out.push(makeRegion(REGION_TYPES.PREAMBLE_COVER, start, end, { source: 'coverage-fill' }));
      return;
    }
    if (start >= bodyEnd) {
      out.push(makeRegion(REGION_TYPES.BACKMATTER_OTHER, start, end, { source: 'coverage-fill' }));
      return;
    }
    if (start < bodyStart) {
      addGap(start, bodyStart);
      addGap(bodyStart, end);
      return;
    }
    if (end > bodyEnd) {
      addGap(start, bodyEnd);
      addGap(bodyEnd, end);
      return;
    }
    out.push(makeRegion(REGION_TYPES.BODY_SECTION_UNASSIGNED, start, end, { source: 'coverage-fill' }));
  };

  for (const region of ordered) {
    addGap(cursor, region.start);
    cursor = Math.max(cursor, region.end);
  }
  addGap(cursor, length);
  return mergeAdjacentRegions(out);
}

function buildTypedRegions(fullText, { bodyStart, bodyEnd, articles, sections }) {
  const sectionRegions = nonOverlappingSectionRegions(sections);
  const occupied = sortedRegions(sectionRegions);

  const preambleRegions = detectPreambleRegions(fullText, bodyStart);
  const backmatterRegions = detectBackmatterRegions(fullText, bodyEnd);
  const articleHeaderRegions = [];
  for (const article of articles || []) {
    if (!article || typeof article.startChar !== 'number') continue;
    if (article.startChar < bodyStart || article.startChar >= bodyEnd) continue;
    const end = articleHeaderEnd(fullText, article, sections, bodyEnd);
    if (end && end > article.startChar) {
      articleHeaderRegions.push(makeRegion(REGION_TYPES.BODY_ARTICLE_HEADER, article.startChar, end, {
        source: 'article',
        articleNumber: article.number || null,
        title: article.title || article.articleTitle || null,
      }));
    }
  }

  const baseRegions = [...preambleRegions, ...articleHeaderRegions, ...backmatterRegions];
  const nonOverlappingBase = [];
  for (const region of baseRegions) {
    nonOverlappingBase.push(...subtractOccupied(region, occupied));
  }

  return fillRegionHoles([...sectionRegions, ...nonOverlappingBase], fullText.length, bodyStart, bodyEnd);
}
...
function exactContentsTitleForGap(fullText, label, bodyPosition, bodyLine) {
  const prefix = fullText.substring(0, bodyPosition);
  const contentsStart = prefix.lastIndexOf('TABLE OF CONTENTS');
  if (contentsStart < 0) return null;
  const lines = prefix.substring(contentsStart).split('\n').map((line) => line.trim());
  for (let index = 0; index < lines.length - 1; index += 1) {
    if (lines[index] !== label) continue;
    const title = lines[index + 1] || '';
    if (/^[A-Z][^\n]{2,200}$/.test(title)
      && (bodyLine === title || bodyLine.startsWith(`${title} `))) return title;
  }
  return null;
}
...
function recoverGaps(gaps, fullText, sections, bodyStart, bodyEnd) {
  ...
  for (const gap of gaps) {
    ...
    while ((match = pattern.exec(body)) !== null) {
      const token = new RegExp(`(?:(?:SECTION|Section)\\s+)?${escapedNum}\\b`).exec(match[0]);
      if (!token) continue;
      const absPos = bodyStart + match.index + token.index;
      const headingLine = match[0].slice(token.index).trim();
      const titleText = headingLine.replace(new RegExp(`^(?:(?:SECTION|Section)\\s+)?${escapedNum}\\s*\\.?\\s*`), '');
      const isPrefixed = /^(?:SECTION|Section)\s+/.test(headingLine);
      const prefixedHeading = isPrefixed
        && /^[A-Z][A-Za-z0-9 ,;:'’&()\/-]{2,200}\.\s+[A-Z]/.test(titleText);
      const contentsTitle = isPrefixed ? null : exactContentsTitleForGap(fullText, gap.label, absPos, titleText);
      if (!prefixedHeading && !contentsTitle) continue;
      const containing = sections.find((section) => absPos > section.startChar && absPos < section.endChar);
      const originalEnd = containing ? containing.endChar : null;

      // Determine end: next known section boundary or capped at 5000 chars
      let endChar = originalEnd || Math.min(absPos + 5000, bodyEnd);
      for (const s of sections) {
      ...
      const title = contentsTitle || extractSectionTitle(text, gap.label);

      if (containing) {
        containing.endChar = absPos;
        containing.text = cleanSectionText(fullText.substring(containing.startChar, absPos).trim());
        containing.subItemCount = countSubItems(containing.text);
      }
 ...
 *
 * Annex/Schedule headings are conservative; Exhibit headings are only emitted
 * when they contain operative offer conditions or defined terms.
 */
function parseAnnexes(fullText, bodyEnd, articles) {
  ...
  // Match annex/schedule/exhibit headings like "Annex I", "Annex A",
  // "Schedule I", or "Exhibit A".
  // Conoco keeps its integral defined-term appendix at "ANNEX A / Certain
  // Definitions"; that is operative parser input, not an ancillary exhibit.
  const annexHeadingRe = /(?:^|\n)\s*((?:(Annex|ANNEX|Schedule|SCHEDULE)\s+([A-Z]|[IVXLCDM]+)|(?:Exhibit|EXHIBIT)\s+([A-Z0-9.-]+))\b[^\n]*|(?:Form|FORM)\s+OF\s+[^\n]{3,140})/g;
  const out = [];
  const matches = [];
  let m;
  while ((m = annexHeadingRe.exec(tail)) !== null) {
    const kind = m[2] ? m[2].toUpperCase() : (m[4] ? 'EXHIBIT' : 'FORM');
    matches.push({
      kind,
      num: (m[3] || m[4] || '').toUpperCase(),
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (!/^(?:ANNEX|SCHEDULE|EXHIBIT)$/.test(cur.kind)) continue;
    ...
    // Only surface annexes that contain agreement-level provisions. The two
    // operative cases in the corpus are offer conditions and defined terms.
    // Other typical annexes — Certificate of Incorporation, Bylaws, CVR form —
    ...
    const isDefinitionsAnnex = /certain\s+definitions|defined\s+terms|definitions?/i
      .test(cur.headerLine + ' ' + title);
    if (!isOfferConditions && !isDefinitionsAnnex) continue;

    const absStart = bodyEnd + segStart;
    const absEnd = bodyEnd + segEnd;
    const heading = lines[0].substring(0, 200).trim();
    const numberPrefix = cur.kind === 'EXHIBIT' ? 'Exhibit' : 'Annex';
    out.push({
      number: `${numberPrefix}-${cur.num}`,
      heading,
      title: isDefinitionsAnnex ? (title || 'Certain Definitions') : (title || cur.headerLine),
      text: segText,
      articleNumber: cur.num,
      articleTitle: isDefinitionsAnnex ? (title || 'Certain Definitions') : (title || 'Offer Conditions'),
      startChar: absStart,
      endChar: absEnd,
      level: 'annex',
      subItemCount: countSubItems(segText),
      isAnnex: true,
      ...(isDefinitionsAnnex ? { isDefinitionsAnnex: true, regionType: REGION_TYPES.BODY_SECTION_DEFINITION } : {}),
    });
  }
...
function parseStructure(fullText) {
  ...
  const bodyStart = bodyStartAfterEnactingHardWall(fullText, findBodyStart(fullText));
  ...
  for (const articleInt of seenArticleNumsFromSections) {
    ...
    // short non-empty line followed by a blank line. The window must be wide
    // enough to reach past a reps-article chapeau: Kraft's article title
    // ("Representations and Warranties of Heinz, ...") sits ~1300 chars
    // before SECTION 3.01, separated by the full disclosure-letter lead-in
    // paragraph. A 400-char window only saw the chapeau prose and fell back
    // to "Article III", which the classifier can't type.
    const beforeWindow = fullText.substring(Math.max(0, firstSecOffset - 1500), firstSecOffset);
    // Find lines (split on \n\n)
  ...
  // Step 5.5: TENDER-OFFER ANNEX CONDITIONS carve-in. Tender deals put the
  // buyer's offer conditions (minimum condition, MAE condition, bring-downs)
  // in an "Annex A — Tender Offer Conditions" / "Annex I — Conditions to the
  // Offer" AFTER the signature block, where body parsing stops — so the
  // deal's most important conditions were never captured (Pharmasset COND=4,
  // all mutual). Locate the annex header in the post-body tail and append it
  // as a section; the classifier's existing offer-conditions rule types it
  // COND and Strategy A splits its (a)/(b)… limbs.
  {
    const alreadyHasOfferConditionsAnnex = sections.some((section) =>
      section && section.isAnnex && /conditions?\s+(?:to|of)\s+(?:the\s+)?offer|offer\s+conditions/i.test(
        `${section.title || ''} ${section.articleTitle || ''} ${String(section.text || '').slice(0, 200)}`,
      )
    );
    const tail = fullText.substring(bodyEnd);
    // Header shape varies: "Annex A — Tender Offer Conditions" on one line, or
    // "Annex A\n\nTENDER OFFER CONDITIONS" (title on its own line after a blank).
    const annexM = tail.match(/\n[ \t]*(?:Annex\s+(?:[A-Z]|[IVX]+))\b[\s\S]{0,80}?(?:Tender\s+Offer\s+Conditions|Conditions\s+(?:to|of)\s+the\s+Offer|Offer\s+Conditions)/i);
    if (annexM && !alreadyHasOfferConditionsAnnex) {
      const start = bodyEnd + annexM.index + 1;
      const rest = fullText.substring(start + annexM[0].length);
      const nextHdr = rest.search(/\n[ \t]*(?:Annex\s+(?:[A-Z]|[IVX]+)\b|EXHIBIT\s+[A-Z]\b)/i);
      const end = nextHdr >= 0 ? start + annexM[0].length + nextHdr : fullText.length;
      const text = fullText.substring(start, end);
      if (text.length > 1500) {
        sections.push({
          number: 'ANNEX-COND',
          title: 'Conditions to the Offer',
          heading: 'Conditions to the Offer',
          articleNumber: null,
          articleTitle: 'CONDITIONS TO THE OFFER',
          startChar: start,
          endChar: end,
          text,
        });
  ...
  for (let i = 0; i < sections.length; i++) {
    sections[i] = enrichSectionHierarchy(sections[i]);
  }

  const regions = buildTypedRegions(fullText, {
    bodyStart,
    bodyEnd,
    articles,
    sections,
  });
  const regionCoverage = coverageDiagnostics(regions, fullText);
  ...
  return {
    ...
    diagnostics: {
      ...
      delimiter,
      coveragePct,
      regionCoverageComplete: regionCoverage.complete,
      regionCoverageGaps: regionCoverage.gaps,
      regionCoverageOverlaps: regionCoverage.overlaps,
      gaps: remainingGaps.map(g => g.label),
    },
  };
}

function compactReviewText(text) {
  return String(text || '').replace(/\s+/g, ' ').trim();
}

function previewReviewText(text, max = 240) {
  const s = compactReviewText(text);
  if (s.length <= max) return s;
  return `${s.slice(0, Math.max(0, max - 3)).trim()}...`;
}

function formatReviewId(prefix, index) {
  return `${prefix}-${String(index).padStart(3, '0')}`;
}

function regionText(cleanedText, region) {
  const start = Math.max(0, Math.min(cleanedText.length, Number(region && region.start) || 0));
  const end = Math.max(start, Math.min(cleanedText.length, Number(region && region.end) || start));
  return cleanedText.slice(start, end);
}

function buildParserReview(sourceText, opts = {}) {
  const cleanedText = cleanText(sourceText || '');
  const parsed = parseStructure(cleanedText);
  const minStructuralGapChars = opts.minStructuralGapChars == null ? 30 : Number(opts.minStructuralGapChars);

  const structuralGaps = (parsed.regions || [])
    .filter((region) => isGapRegionType(region.type))
    .map((region) => {
      const text = regionText(cleanedText, region);
      return { region, text, compactText: compactReviewText(text) };
    })
    .filter((item) => item.compactText.length >= minStructuralGapChars)
    .map((item, index) => ({
      id: formatReviewId('PG', index + 1),
      start: item.region.start,
      end: item.region.end,
      length: item.region.length,
      region_type: item.region.type,
      source: item.region.source || null,
      preview: previewReviewText(item.text),
      text: item.text.trim(),
      full_text: item.text.trim(),
      coordinate_space: 'cleaned_structural_text',
    }));

  const definition_warnings = [];
  for (const section of parsed.sections || []) {
    for (const warning of section.definitionCompletenessWarnings || []) {
      definition_warnings.push({
        id: formatReviewId('DW', definition_warnings.length + 1),
        ...
        term: warning.term || null,
        message: warning.message || null,
        preview: previewReviewText(section.text || ''),
        text: String(section.text || '').trim(),
        full_text: String(section.text || '').trim(),
        coordinate_space: 'cleaned_structural_text',
      });
    }
  }

  const data_model_flags = [];
  for (const section of parsed.sections || []) {
    if (!section.needsDoubleDummyModel) continue;
    data_model_flags.push({
      id: formatReviewId('DM', data_model_flags.length + 1),
      flag: 'needsDoubleDummyModel',
      section_number: section.number || null,
      section_title: section.title || section.heading || null,
      start: section.startChar || null,
      end: section.endChar || null,
  ...
  const atomic_sections = (parsed.sections || [])
    .filter((section) => section.atomic)
    .map((section, index) => ({
      id: formatReviewId('AS', index + 1),
      section_number: section.number || null,
      section_title: section.title || section.heading || null,
      atomic_reason: section.atomicReason || null,
      subclause_count: Array.isArray(section.subClauses) ? section.subClauses.length : 0,
      start: section.startChar || null,
      end: section.endChar || null,
    }));
  ...
  return {
    summary: {
      ...
      body_start: parsed.diagnostics.bodyStart,
      body_end: parsed.diagnostics.bodyEnd,
      section_count: parsed.sections.length,
      article_count: parsed.articles.length,
      region_coverage_complete: parsed.diagnostics.regionCoverageComplete,
      structural_gap_count: structuralGaps.length,
      definition_warning_count: definition_warnings.length,
      data_model_flag_count: data_model_flags.length,
      atomic_section_count: atomic_sections.length,
    },
...
module.exports = { parseStructure, cleanText, displayCleanText, buildParserReview };
