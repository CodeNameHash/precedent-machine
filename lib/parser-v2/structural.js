...
function stripDefinedTermsIndex(text) {
  ...
  if (headerIdx >= 0) {
    ...
    // page/header noise until we hit a "real" body line (ARTICLE heading,
    // a paragraph of prose, "This AGREEMENT", "IN WITNESS WHEREOF", etc.)
    let i = headerIdx + 1;
    let lastIdx = headerIdx;
    while (i < lines.length) {
      const trimmed = lines[i].trim();
      if (!trimmed) { i++; continue; }
      // Stop at the start of a substantive body block
      if (/^ARTICLE\s+(?:[IVXLCDM]+|\d+)\b/i.test(trimmed)) break;
      if (/^This\s+AGREEMENT/i.test(trimmed)) break;
      if (/^IN\s+WITNESS\s+WHEREOF/i.test(trimmed)) break;
      if (/^WHEREAS\b/i.test(trimmed)) break;
      if (/^SECTION\s+\d+\.\d{1,2}\.\s+\S+/i.test(trimmed)) {
        // SECTION X.XX. Title. body — actual body section (not a TOC stub)
        // Heuristic: if the line is long, it's body.
