...
function stripDisplayMarkers(text) {
  if (!text) return text;
  // Drop TOC block entirely — content was just the index, body follows.
  text = text.replace(/\[\[TOC_START\]\][\s\S]*?\[\[\/TOC_START\]\]/g, '');
  // Drop standalone TOC_ARTICLE / TOC_ENTRY tags if any escaped the block.
  text = text.replace(/\[\[\/?TOC_[A-Z_]+\]\]/g, '');
  // Unwrap all other [[FOO]]...[[/FOO]] pairs — keep the inner text.
  // Examples: [[ARTICLE]]ARTICLE I[[/ARTICLE]] -> ARTICLE I
  text = text.replace(/\[\[([A-Z_]+)\]\]([\s\S]*?)\[\[\/\1\]\]/g, '$2');
  // Strip any orphan open/close tags that the pair-strip missed.
