...
function findBodyStart(fullText) {
  ...
  //   (a) first uppercase ARTICLE heading on its own line
  ...
  // body to start at section 1.01), and the case-insensitive ARTICLE regex
  // used to misdetect inline cross-references like "Article VII," inside
  // section 1.02's body text.
  //
  // Requirements that defeat the cross-reference false-positive:
  //   - No `i` flag (ARTICLE must be uppercase, not "Article")
  //   - Followed by uppercase roman/digits + whitespace + EOL/dash
  //     (not a comma or other prose)
  let candidate = -1;

  const firstArt = fullText.match(/\n\s*ARTICLE\s+(?:[IVXLCDM]+|\d+)\b(?=\s*(?:\n|--|—))/);
  if (firstArt) candidate = firstArt.index + 1;
