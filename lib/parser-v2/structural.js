...
function repairRunawayTocBlock(text) {
  ...
  repaired = repaired.replace(/\[\[\/?TOC_[A-Z_]+\]\]/g, '');
...
function findBodyStartLegacy(fullText) {
  ...
  // starts at the top of the document.
  {
    ...
    {
      const preambleRe = /(?:AGREEMENT[^\n]{0,160}?\bdated\s+as\s+of\b|is\s+made\s+and\s+entered\s+into\s+as\s+of|is\s+entered\s+into\s+as\s+of|\bW\s?I\s?T\s?N\s?E\s?S\s?S\s?E\s?T\s?H\b|\n[ \t]*WHEREAS\b)/i;
      const pm = searchText.match(preambleRe);
      ...
      if (pm && !tocMatch) {
        ...
        while ((hm = headRe.exec(searchText)) !== null) {
          ...
          const rest = hm[1].replace(/\.{3,}\s*\d{1,4}\s*$/, '').trim();
          if (rest.length > 3) { falsePreamble = true; break; }
        }
      }
...
function parseAnnexes(fullText, bodyEnd, articles) {
  ...
  // Match annex/schedule headings like "Annex I", "Annex A", "Schedule I".
  ...
  const annexHeadingRe = /(?:^|\n)\s*((Annex|ANNEX|Schedule|SCHEDULE)\s+([A-Z]|[IVXLCDM]+)\b[^\n]*|(?:Exhibit|EXHIBIT)\s+[A-Z0-9.-]+\b[^\n]*|(?:Form|FORM)\s+OF\s+[^\n]{3,140})/g;
  const out = [];
  const matches = [];
  let m;
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (!/^(?:Annex|ANNEX|Schedule|SCHEDULE)$/.test(cur.kind)) continue;
    ...
    const isDefinitionsAnnex = /certain\s+definitions|defined\s+terms|definitions?/i
      .test(title + ' ' + segText.substring(0, 300));
