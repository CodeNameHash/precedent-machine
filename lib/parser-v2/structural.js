...
function repairRunawayTocBlock(text) {
  ...
  if (!implausiblyLarge) return text;
...
function findBodyStart(fullText) {
  ...
  if (tocMatch) {
    ...
    // exactly ONCE between the TOC and Article I and never inside a TOC or a
    // defined-terms index. The same-line-content cascade below is brittle in
    // both directions — CSRA's body articles have short section titles on the
    // header line ("Section 2.1 The Offer.") so every article until IX was
    // rejected as a TOC stub (96% of the body discarded); Covance's TOC
    // entries carry long same-line titles ("Section 1.05. Certificate of
    // Incorporation and Bylaws") so the body start landed INSIDE the TOC and
    // every "section" was a stub. The preamble sidesteps that entire class:
    // body start = the first ARTICLE (or Section 1.x) heading AFTER the
    // preamble. Falls through to the legacy cascade when no preamble matches.
    {
      ...
      const pm = afterToc.match(preambleRe);
      // PROXIMITY GUARD: a true preamble sits directly after the TOC — the
      // longest front-of-document TOC in the corpus runs ~13k chars. A match
      // far beyond that is a body cross-reference to another contract
      // ("... the Confidentiality Agreement, dated as of April 27, 2017 ..."
      // 170k into Whole Foods, whose real preamble was lost at ingest);
      // anchoring on it would discard everything before that point.
      if (pm && pm.index < 25000) {
        const afterPreamble = afterToc.substring(pm.index);
        ...
        if (candidates.length) {
          ...
          return tocMatch.index + pm.index + best.index + best.offset;
        }
      }
    }
