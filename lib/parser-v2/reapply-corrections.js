...
//     wrong sibling. 0.15 was originally roughly half the gap between the
//     two similarity floors when MIN_SIM_WITH_CATEGORY was still 0.3 (half
//     of the 0.3-vs-0.6 gap); raising MIN_SIM_WITH_CATEGORY to 0.45 above
//     shrank that gap to 0.15 without MIN_MARGIN being re-derived, so today
//     MIN_MARGIN equals the FULL current gap (0.45 vs 0.6), not half of it.
//     Still does the intended job -- a genuinely dominant match (the common
//     case -- one provision is a clear textual descendant of the
//     correction's `before` snapshot) clears it easily, while two
//     structurally similar siblings scored within noise of each other do
//     not -- but "roughly half the gap" is no longer the right mental model
//     for why 0.15 is safe; re-derive if either floor moves again.
