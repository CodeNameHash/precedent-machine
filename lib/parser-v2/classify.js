...
const DETERMINISTIC_RULES = [
  ...
  { pattern: /covenants?\s+of\s+(?:the\s+)?(?:company|target|seller)|conduct\s+of\s+(?:the\s+)?(?:company|target|seller)['’]?s?\s+business|interim\s+oper/i, type: 'IOC', overrideArticle: true },
...
function tryDeterministic(section, articleType) {
  ...
  const STRONG_ARTICLE_TYPES = new Set(['REP-T', 'REP-B', 'IOC', 'COV', 'STRUCT', 'CONSID', 'DEF', 'MISC']);
