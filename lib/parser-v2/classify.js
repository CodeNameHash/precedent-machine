 ...
 * Deterministic-first pipeline, layered well past a simple two-pass split as
 * real deals exposed edge cases (see the numbered section banners below for
 * each stage; do not assume "two passes" describes this file):
 *   1. Article-level classification, then section-level deterministic regex
 *      rules: three independently-sized rule tables (section-type rules,
 *      article-type rules, sub-code refinement rules), each grown by adding
 *      one narrow, title-anchored pattern per deal-specific misclassification
 *      found. Count the arrays in the code for today's total; a number here
 *      would just go stale again the next time a deal needs a new rule.
 *   2. A prior-snapshot cache reuses a section's last classification when its
 *      text is unchanged, ahead of any AI call.
 *   3. AI classification (Claude) for whatever neither of the above resolved.
 *   4. Several document-order positional/content fixups that run AFTER
 *      classification and CAN still override it: REP article- and
 *      section-ordering by drafting convention, codename conduct sections, a
 *      NOSOL content-based fallback. These are deterministic too, not AI.
 *
...
const { PROVISION_TYPES, CODES, isRetiredCode } = require('../rubric');
...
//    A large, continuously-growing table of narrow, title-anchored patterns
//    that never misfire (each is scoped with guardNot/notInArticle where a
//    looser version could collide with a stronger type): not a short fixed
//    list. Most entries exist because one real deal's section title needed
//    one. Everything the tables below (and the prior-snapshot cache) don't
//    resolve goes to AI.
// ---------------------------------------------------------------------------
...
const DETERMINISTIC_RULES = [
  ...
  { pattern: /no[\s-]*(?:solicitation|shop)|unsolicited\s+proposals?|acquisition\s+proposals?|takeover\s+proposals?|alternative\s+(?:transactions?|proposals?)|company\s+(?:adverse|board)\s+recommendation|adverse\s+recommendation\s+change/i, type: 'NOSOL', overrideArticle: true },
  ...
  // \bsolicitation\b (no "no"/"non-" gate) catches these title variants
  // (the hyphen in "Non-Solicitation" is a word boundary, so plain
  // "Non-Solicitation" also lands here — the rule above needs a literal
  // "no"); title-only anchoring means unrelated prose can't misfire.
  // guardNot: bare "solicitation" is the ONLY alternative loose enough to
  // hit NON-deal-protection solicitation sections — an employee/personnel
  // non-solicit ("Non-Solicitation of Employees", "No Hire") or a proxy
  // covenant ("Solicitation of Proxies") would misclassify as the no-shop,
  // a wrong legal answer that reads as correct. The guard is scoped to
  // THIS rule only: the stronger signals above ("no shop", "acquisition
  // proposals", ...) stay ungated. Verified against all 40 deals' section
  // titles (see tests/nosol-title-solicitation.test.js and the corpus-wide
  // safety-check script) — only Frontier's "Solicitation; Change in
  // Recommendation" title newly matches; no other section flips.
  { pattern: /\bsolicitation\b/i, type: 'NOSOL', overrideArticle: true, guardNot: /employee|personnel|no[\s-]*hire|proxy|proxies/i },
...
const SUBCODE_REFINEMENT_RULES = [
  ...
  // REP-T family
  // v1 reclassification (2026-08-02, R1/R2 — mechanism pinned per audit
  // A-M3): SUBCODE_REFINEMENT_RULES entries with whenType: 'REP-T' (the
  // NOREP precedent above), NOT DETERMINISTIC_RULES — DETERMINISTIC_RULES
  // never fires inside strong-typed REP articles without overrideArticle
  // and would be a silent no-op here. Title rules ONLY where the corpus
  // shows regular titles; everything else stays AI-classified against the
  // new CODES vocabulary (retired REP-T-CONSENT / REP-T-REGSTATUS excluded
  // from the extract prompt — see getCodesForType).
  { pattern: /^requisite\s+stockholder\s+approval|company\s+stockholder\s+approval|required\s+stockholder\s+vote/i, whenType: 'REP-T', code: 'REP-T-STOCKAPPROVAL' },
  { pattern: /requisite\s+governmental\s+approvals?|governmental\s+authorizations?|government\s+approvals?/i, whenType: 'REP-T', code: 'REP-T-GOVAPPROVAL' },
  { pattern: /investment\s+company\s+act/i, whenType: 'REP-T', code: 'REP-T-40ACT' },
...
function buildTypeReference() {
  return PROVISION_TYPES.map(t => {
    const codes = Object.entries(CODES)
      .filter(([, v]) => v.type === t.key && !v.retired) // retired codes never shown as AI prompt vocabulary
 ...
 * @param {Object} client - Anthropic SDK client instance
 * @param {Object} [opts] - { prior?: Map<textHash, {provisionType, provisionCode, confidence}>, rerunPriorCodes?: Set<string> }
...
async function classifySections(sections, articles, client, opts = {}) {
  ...
  const rerunPriorCodes = opts && opts.rerunPriorCodes instanceof Set ? opts.rerunPriorCodes : null;
  ...
  for (let i = 0; i < sections.length; i++) {
    const priorHit = prior?.get(sectionTextHash(sections[i].text || sections[i].body || ''));
    if (rerunPriorCodes && priorHit?.provisionType && !rerunPriorCodes.has(priorHit.provisionCode)) {
      cached.push({ index: i, hit: priorHit });
      continue;
    }
  ...
  if (prior && prior.size > 0 && needsAI.length > 0) {
    for (let n = needsAI.length - 1; n >= 0; n--) {
      ...
      if (!hit || !hit.provisionType) continue;
      // v1 reclassification (2026-08-02, retired-code cache invalidation —
      // audit A-C2): a cached provisionCode that is now retired must NOT be
      // carried forward by `--classify-only --apply` — that would silently
      // re-stamp a code the classify/extract layers no longer accept. Treat
      // this as a cache MISS (bypass) for the affected section so it falls
      // through to the AI pass below and gets reclassified against the
      // CURRENT rules/vocabulary instead.
      if (hit.provisionCode && isRetiredCode(hit.provisionCode)) continue;
