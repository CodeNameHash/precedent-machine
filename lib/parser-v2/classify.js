...
const DETERMINISTIC_RULES = [
  ...
  // so "Expense Reimbursement" (TERMF territory) can't match. NOT inside a
  // TERMINATION article: there, "Fees and Expenses" is where PE-style deals
  // bury the termination fee (Prometheus 8.3, Skechers 8.3) — the
  // TERMINATION-article branch below decides TERMF vs MISC from the body.
  { pattern: /^fees?\s+and\s+expenses\s*\.?\s*$|^expenses\s*\.?\s*$/i, type: 'MISC', code: 'MISC-EXPENSES', overrideArticle: true, notInArticle: ['TERMINATION'] },
  // Termination fee sections parked in the MISCELLANEOUS article (Anadarko
  // 10.5 "Company Termination Fee", Mr. Cooper 10.5 "Termination Fees" —
  // both were typed MISC via the article, leaving the fee data under a
  // generic MISC view). Exact anchored title; "Payment of Parent Termination
  // Payment"-style TERMR mechanics sections don't start with "Termination".
  { pattern: /^(?:company\s+|parent\s+)?termination\s+(?:fees?|payments?)\s*\.?\s*$/i, type: 'TERMF', overrideArticle: true, notInArticle: ['REP-T', 'REP-B'] },
  ...
  { pattern: /effect\s+(?:of|on)\s+(?:the\s+)?merger\s+on|^effect\s+on\s+(?:the\s+)?capital\s+stock\b|conversion\s+of\s+(?:the\s+)?(?:shares|stock|securit|capital\s+stock)|exchange\s+of\s+certificates|(?:mixed|cash|stock)\s+election|lost,?\s+stolen\s+or\s+destroyed\s+certificates|no\s+further\s+ownership\s+rights|no\s+dividends?\s+or\s+distributions|withholding\s+rights?|required\s+withholding|^(?:merger\s+)?consideration\s*\.?\s*$/i, type: 'CONSID', overrideArticle: true },
...
function tryDeterministic(section, articleType) {
  ...
  if (articleType) {
    ...
    if (articleType === 'TERMINATION') {
      // Generic expense-allocation titles: TERMF only when the body actually
      // carries a termination fee. PE-style deals bury the fee in "Fees and
      // Expenses" (Prometheus 8.3, Skechers 8.3 — both ~9k chars of Company/
      // Parent Termination Fee mechanics), but Metsera 8.03 with the same
      // title is 323 chars of genuine each-party-bears-its-own boilerplate
      // and must stay MISC.
      if (/^fees?\s+and\s+expenses\s*\.?\s*$|^expenses\s*\.?\s*$/i.test(title)) {
        const bodyHasFee = /termination\s+(?:fee|payment)|break[\s-]*up\s+fee/i.test(section.text || '');
        return bodyHasFee
          ? { type: 'TERMF', confidence: 'high' }
          : { type: 'MISC', code: 'MISC-EXPENSES', confidence: 'high' };
      }
