...
const DETERMINISTIC_RULES = [
  ...
  // ── Stage 2: new sub-code routing rules ──
  // Each rule must match the SECTION title only (not body) so it doesn't
  // false-fire on cross-references. Order matters — more specific titles
  // come before generic catch-alls (already enforced by the rule order).
  // CONSID sub-codes
  { pattern: /contingent\s+value\s+rights?|\bcvr\b/i, type: 'CONSID', code: 'CONSID-CVR' },
  { pattern: /\bcollar\b/i, type: 'CONSID', code: 'CONSID-COLLAR' },
  { pattern: /ticking\s+fee|per[\s-]*diem\s+fee/i, type: 'CONSID', code: 'CONSID-TICKING' },
  { pattern: /exchange\s+ratio/i, type: 'CONSID', code: 'CONSID-EXCHANGE-RATIO' },
  { pattern: /walkaway|market[\s-]*out/i, type: 'CONSID', code: 'CONSID-WALKAWAY' },
  // COV sub-codes
  { pattern: /appraisal(?:\s+rights?|\s+proceedings?)?|dissenters?\s+rights?/i, type: 'COV', code: 'COV-APPRAISAL' },
  { pattern: /paying\s+agent|exchange\s+agent|disbursing\s+agent/i, type: 'COV', code: 'COV-PAYAGENT' },
  { pattern: /marketing\s+period/i, type: 'COV', code: 'COV-MARKETING' },
  { pattern: /indemnification\s+(?:of|and)\s+(?:directors|officers)|d\s*&\s*o\s+(?:indemnification|insurance)|directors?\s+and\s+officers?\s+(?:indemnification|insurance)/i, type: 'COV', code: 'COV-DO' },
  { pattern: /^proxy(?:\s+statement)?$|preparation\s+of\s+(?:the\s+)?proxy|special\s+meeting/i, type: 'COV', code: 'COV-PROXY', matchOn: 'title' },
  // TERMF sub-codes
  { pattern: /antitrust\s+(?:reverse\s+)?termination\s+fee|regulatory\s+termination\s+fee|reverse\s+termination\s+fee.*antitrust/i, type: 'TERMF', code: 'TERMF-RTF-ANTI' },
  { pattern: /(?:acquirer|buyer|parent)\s+expense\s+reimburse|expense\s+reimbursement/i, type: 'TERMF', code: 'TERMF-REIMBURSE' },
  // REP-B sub-codes
  { pattern: /sufficient\s+funds|available\s+funds|sufficient\s+cash/i, type: 'REP-B', code: 'REP-B-FUNDS' },
  { pattern: /\bsolvenc/i, type: 'REP-B', code: 'REP-B-SOLVENCY' },
  { pattern: /anti[\s-]*reliance|exclusivity\s+of\s+representations|no\s+other\s+representations/i, type: 'REP-B', code: 'REP-B-ANTIRELIANCE' },
  // REP-T sub-codes
  { pattern: /sufficiency\s+of\s+assets/i, type: 'REP-T', code: 'REP-T-SUFFICIENCY' },
  { pattern: /top\s+customers|top\s+suppliers|significant\s+customers|major\s+customers/i, type: 'REP-T', code: 'REP-T-TOP-CUSTOMERS' },
  { pattern: /material\s+contracts|significant\s+contracts/i, type: 'REP-T', code: 'REP-T-MATERIAL-CONTRACTS' },
];
