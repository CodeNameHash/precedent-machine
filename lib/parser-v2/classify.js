...
const DETERMINISTIC_RULES = [
  // No-solicitation / No-shop — unmistakable language
  { pattern: /no[\s-]*(?:solicitation|shop)/i, type: 'NOSOL' },

  // Antitrust / HSR — distinctive legal terms
  { pattern: /antitrust|HSR\b/i, type: 'ANTI' },

  // Definitions — exact title match only
  { pattern: /^definitions?\s*$/i, type: 'DEF', matchOn: 'title' },

  // Governing law — always MISC
  { pattern: /governing\s+law/i, type: 'MISC' },

  // Boilerplate MISC — severability, counterparts, jury waiver
  { pattern: /severab|counterpart|waiver\s+of\s+jury/i, type: 'MISC' },

  // Specific performance — always MISC
  { pattern: /specific\s+performance/i, type: 'MISC' },

  // "The Merger" as exact section title — STRUCT
 ...
 * Returns { type } or null if no deterministic match.
 */
function tryDeterministic(section) {
...
async function classifySections(sections, articles, client) {
  ...
  for (let i = 0; i < sections.length; i++) {
    const result = tryDeterministic(sections[i]);
    if (result) {
      deterministic.push({ index: i, type: result.type });
    } else {
      needsAI.push({ index: i, section: sections[i] });
    }
  }

  // ── Pass 2: AI classification for remaining sections ──
  const aiSections = needsAI.map(item => item.section);
  const aiResults = await classifyWithAI(aiSections, articles, client);

  // ── Assemble results ──
  const classified = new Array(sections.length);

  // Apply deterministic results
  for (const { index, type } of deterministic) {
    classified[index] = {
      ...sections[index],
      provisionType: type,
      confidence: 'high',
