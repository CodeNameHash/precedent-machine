/* ─────────────────────────────────────────────────────────────────────────
   ...
   2. CHILD-OPEN — the token is the first value of its style ("a"/"i"/"A"/
      "1"/"x") AND one of: (i) it is the very first marker in the section;
      (ii) it is immediately adjacent to the prior structural marker's
      closing paren (only whitespace between, as in "(a)\n\n(i)…"); or (iii)
      it is COLON-INTRODUCED — the nearest non-whitespace character before
      the bracket, whitespace aside, is ':' (as in Concho's "…involving,
   ...
   ── "(x)"/"(y)"/"(z)" as a fourth marker style, and why it needs its own
   corroboration rule ──
   Lawyers use "(x)/(y)/(z)" for short inline enumerations, deliberately, to
   avoid colliding with "(i)/(ii)/(iii)" and "(a)/(b)/(c)" ("...other than
   (x) Permitted Encumbrances or (y) as would not reasonably be expected to
   ..."). "x" is TRIPLY ambiguous — alpha value 23, roman value 9, and xyz
   value 0 — so CHILD-OPEN for the xyz style additionally requires
   CORROBORATION: a matching "(y)" must actually be found within the same
   enclosing span (a bounded forward scan stopping at the next paragraph
   break), or the "(x)" opens nothing at all. One "(x)" alone never opens a
   list; this is the same discipline the module already applies to
   ambiguous single-char roman/alpha tokens ("i"/"v"/"x" needing a STRONG
   position to SIBLING-match), generalised to a case corroboration can
   settle outright rather than merely narrow. Because no OTHER style ever
   resolves "x" to value 0, ordinary CHILD-OPEN and corroborated xyz
   CHILD-OPEN are mutually exclusive on the same token, so precedence is:
   SIBLING first (a genuine alpha list running "(w)" -> "(x)" continues as
   alpha and is never reached by the xyz rule), then corroborated xyz
   CHILD-OPEN, then ordinary CHILD-OPEN for every other style. SIBLING
   continuation of an ALREADY-OPEN xyz frame ("y" after "x", "z" after "y")
   is exempt from the ambiguous-token strong-position guard — that guard
   stops a stray token hijacking an unrelated ACTIVE frame at a weak
   position, but an xyz frame can only exist because its "(x)" was already
   corroborated, and the target drafting is inline BY DESIGN, so requiring a
   strong position for "(y)"/"(z)" to continue would defeat the rule for its
   only real use. Only "x" can ever CHILD-OPEN an xyz frame (it is the only
   xyz value that is ever 0); a lone "(y)" or "(z)" with no preceding
   corroborated "(x)" therefore cannot open anything via this style either,
   and "(x)" immediately followed by "(z)" with no "(y)" in between does NOT
   continue as xyz (the SIBLING value test requires exactly value 1 next) —
   both stay conservatively unsplit, the same as any other un-opened
   enumeration.
   ...
   SEGMENTER_VERSION is stamped on Step 2X-A governing-structure output so
   derived annotation can be invalidated when this module's outline rules
   change. findStructuralMarkers also returns `style` per marker (the frame
   style that accepted the token) so same-style parent-child links are
   detectable without re-deriving style from labels alone.
   ───────────────────────────────────────────────────────────────────────── */
...
// "x" is TRIPLY ambiguous — alpha 23, roman 10, and xyz 0 — which is exactly
// why an xyz CHILD-OPEN additionally requires corroboration (see below)
...
function candidateStyles(content) {
  ...
  if (Object.prototype.hasOwnProperty.call(XYZ_VALUE, content)) {
    out.push({ style: 'xyz', value: XYZ_VALUE[content] });
  }
  return out;
}

// Corroboration for an "(x)" CHILD-OPEN: true only if a matching "(y)"
// actually follows within the same enclosing span. Approximated as a
// bounded forward scan that stops at the first paragraph break (blank
// line), which is the cheapest reliable signal that we have left the
// chapeau's own enumeration. Deliberately does NOT bail merely because some
// other marker-shaped token appears first ("clause (i) of Section 3.1"
// between the "(x)" and its "(y)" must not block corroboration) — the only
// question is "does a (y) exist nearby", not "is (y) the very next marker
// of any kind". One "(x)" alone, with no "(y)" anywhere in the span, never
// opens anything — this is what stops a lone triply-ambiguous "(x)" from
// being guessed into a new list.
function xyzCorroborated(text, tokenStart) {
  const searchFrom = tokenStart + 3; // past "(x)"
  const window = text.slice(searchFrom, Math.min(text.length, searchFrom + 2000));
  const paragraphBreak = window.search(/\n\s*\n/);
  const scoped = paragraphBreak === -1 ? window : window.slice(0, paragraphBreak);
  return /\(y\)/.test(scoped);
}
...
// Raised from 3 to 5, 2026-08-08, on evidence from segmenting every resolved
// section of all 213 evidence runs across the 7-deal corpus (docs/codex-
// program/notes/subclauses-depth-and-xyz.md): real drafting reaches four
// levels ((a) -> (i) -> (A) -> (1)) routinely, and a genuine, clean five-
// level enumeration exists in the corpus (TopBuild 3.1's 18-item alphaUpper
// list, e.g. "...vii.F.f.vii.E.A" through ".R" -- untainted at depth 5).
// MAX_DEPTH=6 was measured and rejected: no section in the corpus reaches a
// genuine depth 6, and enabling it corrupts that same TopBuild list with a
// spurious same-style child layer (an 18-marker mis-nest). Going from 3 to 4
// costs one small, real mis-nest (Skywater Annex-A gains a single same-style
// restart at the new depth) that going on to 5 does not add to -- depth 5
// is "free" once depth 4 is accepted, and is what a "five occurs" corpus
// actually needs.
const MAX_DEPTH = 5;
// Bump when outline rules, depth, or marker styles change in a way that
// moves leaf boundaries or paths. Consumed by governing-structure.js.
const SEGMENTER_VERSION = 'SEGMENT_SUBCLAUSES/V1+MAX_DEPTH_5+XYZ';
const MARKER_TOKEN_RE = /\(([a-zA-Z]{1,3}|[0-9]{1,3})\)/g;

// "(x)", "(y)", "(z)" as a short inline enumeration style, distinct from
// alpha/roman/digit -- see the CHILD-OPEN section below and the header note
// on the corroboration rule. Values start at 0 like every other style.
const XYZ_VALUE = { x: 0, y: 1, z: 2 };
 ...
 *
 * Returns [{ path, depth, tokenStart, tokenEnd, label, style }...].
 */
function findStructuralMarkers(text) {
  ...
  while ((m = MARKER_TOKEN_RE.exec(text)) !== null) {
    ...
    // 1. SIBLING at current top frame. One deliberate exemption from the
    // ambiguous+weak-position guard just below: continuing an ALREADY-OPEN
    // xyz frame ("y" after "x", "z" after "y") is trusted even mid-paragraph.
    // The guard exists to stop a stray token from hijacking an unrelated
    // ACTIVE frame at a weak position; an xyz frame can only ever have been
    // opened by a corroborated "(x)" in the first place (step 2 below), so
    // by the time "(y)"/"(z)" is tested here the context was already vetted
    // once. The real drafting this rule targets is inline BY DESIGN
    // ("...(x) Permitted Encumbrances or (y) as would not..."), so requiring
    // a strong position for "(y)"/"(z)" to continue would silently defeat
    // the rule for its only real use.
    let matched = null;
    if (top) {
      const hit = candidates.find((c) => c.style === top.style && c.value === top.value + 1);
      if (hit) {
        const exemptFromAmbiguityGuard = hit.style === 'xyz';
        if (exemptFromAmbiguityGuard || !(ambiguous && !strongPosition)) {
          top.value = hit.value;
          matched = top;
        }
      }
    ...
    //    structural marker (or the very first marker overall). For the xyz
    //    style specifically, this ALSO requires corroboration: a matching
    //    "(y)" must actually follow within the same enclosing span, or the
    //    triply-ambiguous "(x)" (alpha 23 / roman 10 / xyz 0) never opens a
    //    list on its own. Precedence is SIBLING (above) first, then this
    //    corroborated xyz open, then ordinary CHILD-OPEN — "x" only ever
    //    resolves to xyz's value 0 here (no other style gives "x" value 0),
    //    so the corroboration check below is reached only for "x", and only
    //    once SIBLING has already failed to continue a genuine roman/alpha
    //    list running through it (e.g. "(w)" -> "(x)").
    if (!matched && stack.length < MAX_DEPTH) {
      const firstHit = candidates.find((c) => c.value === 0);
      if (firstHit && !(firstHit.style === 'xyz' && !xyzCorroborated(text, tokenStart))) {
