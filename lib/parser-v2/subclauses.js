/* ─────────────────────────────────────────────────────────────────────────
   ...
   2. CHILD-OPEN — the token is the first value of its style ("a"/"i"/"A"/
      "1") AND is immediately adjacent to the prior structural marker's
      closing paren (only whitespace between, as in "(a)\n\n(i)…"), or it is
      the very first marker in the section. This is deliberately strict: a
      chapeau sentence ("shall include: (i) …") does NOT open a child unless
      the chapeau is empty/whitespace. The consequence is conservative, not
      lossy — un-split enumerations simply stay inside their parent leaf
      (still covered, just less finely divided). See Redfin §2.10(d)/(h),
      whose internal (i)-(v) / (i)-(iii) runs are NOT line-adjacent to their
      parent and are correctly left unsplit.
  ...
  while ((m = MARKER_TOKEN_RE.exec(text)) !== null) {
    ...
    if (!matched && stack.length < MAX_DEPTH) {
      ...
      if (firstHit) {
        ...
        if (isFirstMarkerOverall || adjacentToPrior) {
          const frame = {
