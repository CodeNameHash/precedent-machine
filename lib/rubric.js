 ...
 * compatibility.
 *
 * RUBRIC.md, the prose walkthrough this used to point to, has moved to
 * archive/RUBRIC.md and is stale against this file (it describes 6 deals
 * and 16 provision types; PROVISION_TYPES below now has 22). This file is
 * the reference -- there is no external doc to chase.
 */
...
const PROVISION_TYPES = [
  ...
  },
  {
    ...
    description: 'Family bucket for interim operating covenant concepts shared by target and buyer-side IOC sections',
    classificationMode: 'single',
    party: null,
  },
  {
    ...
    description: 'Restrictions and affirmative obligations on target / company conduct between signing and closing',
    classificationMode: 'single',
    party: 'target',
  },
  {
    ...
    description: 'Restrictions and affirmative obligations on buyer / parent conduct between signing and closing',
    classificationMode: 'single',
    party: 'buyer',
  },
  {
  ...
  {
    key: 'NOSOL-T',
    label: 'No-Solicitation / No-Shop (Target)',
    description: 'Solicitation prohibition, fiduciary out, matching rights, go-shop windows applicable to the target/company',
    classificationMode: 'multi',
    party: 'target',
  },
  {
    key: 'NOSOL-B',
    label: 'No-Solicitation / No-Shop (Buyer)',
    description: 'Solicitation prohibition, fiduciary out, matching rights, go-shop windows applicable to the buyer/parent (reverse no-shop / reciprocal deals)',
    classificationMode: 'multi',
    party: 'buyer',
  },
  {
    key: 'NOSOL-M',
    label: 'No-Solicitation / No-Shop (Mutual)',
    description: 'Mutual / reciprocal no-solicitation obligations binding both parties equally',
    classificationMode: 'multi',
    party: 'mutual',
  },
  {
...
const CODES = {
  ...
  'STRUCT-OFFER': {
    type: 'STRUCT',
    label: 'Tender Offer Mechanics',
    description: 'Two-step tender-offer mechanics: offer launch, SEC filings, acceptance/payment, extensions, and back-end merger path',
    aliases: ['The Offer', 'Tender Offer', 'Offer Mechanics', 'Company Actions', 'Schedule 14D-9'],
    frequency: 'common',
    industries: [],
  },
  'STRUCT-CLOSING': {
  ...
  // v1 reclassification (2026-08-02, R3): REP-T-NOREP retired in favor of
  // the four-element mirrored split below (element split IS Ben-ruled — see
  // FAMILY-MAPPING-RULINGS-2026-08-02.md). Stays registered — historic rows
  // reference it; classify.js's SUBCODE_REFINEMENT_RULES still stamps this
  // FAMILY-LEVEL code at the section-title stage (the classify layer cannot
  // emit multiple cards per section), and the extract-phase element scan
  // (lib/parser-v2/extract.js) replaces the whole-section card with one
  // element-scoped card per element found.
  'REP-T-NOREP': {
    type: 'REP-T',
    ...
    industries: [],
    retired: '2026-08-02',
    superseded_by: ['REP-T-NOOTHERREPS', 'REP-T-NONRELIANCE', 'REP-T-INDEPINVEST', 'REP-T-FRAUDCARVEOUT'],
  },
  // R3 element-split successors (target/company side). Party mirroring is
  // deliberate: store-cards.js's partyScopeFromCode derives party scope from
  // the code prefix, so a shared prefix-less family would break party
  // attribution — these codes exist in BOTH REP-T- and REP-B- prefixes.
  'REP-T-NOOTHERREPS': {
    type: 'REP-T',
    label: 'No Other Representations (Disclaimer)',
    description: 'Disclaimer that no representations or warranties were made beyond those expressly set forth in the agreement',
    aliases: ['No Other Representations', 'Disclaimer of Representations'],
    frequency: 'near-universal',
    industries: [],
  },
  'REP-T-NONRELIANCE': {
    type: 'REP-T',
    label: 'Non-Reliance on Extra-Contractual Statements',
    description: 'Affirmative acknowledgment of non-reliance on statements, projections, or materials outside the agreement',
    aliases: ['Non-Reliance', 'Non-Reliance Acknowledgment'],
    frequency: 'common',
    industries: [],
  },
  'REP-T-INDEPINVEST': {
    type: 'REP-T',
    label: 'Independent Investigation Acknowledgment',
    description: 'Acknowledgment that the party conducted its own independent investigation, review, and analysis',
    aliases: ['Independent Investigation'],
    frequency: 'common',
    industries: [],
  },
  'REP-T-FRAUDCARVEOUT': {
    ...
    description: 'Express preservation of fraud remedies notwithstanding the non-reliance / no-other-reps disclaimers',
    aliases: ['Fraud Carve-Out', 'Fraud Exception'],
    frequency: 'uncommon',
    industries: [],
  },
  'REP-T-PRODUCT': {
  ...
  // v1 reclassification (2026-08-02, R1/R2 — docs/superpowers/specs/
  // 2026-08-02-v1-reclassification-design.md): REP-T-REGSTATUS and
  // REP-T-CONSENT are retired in favor of the split codes below. Both stay
  // REGISTERED (never deleted) — historic rows and fixtures reference them,
  // and the extract prompt vocabulary (getCodesForType) excludes retired
  // codes so the model never re-emits them; enforceCanonicalCodes remaps an
  // incoming retired code to its superseded_by target when unambiguous, else
  // routes to review. `industries` on both retired entries was stale
  // ('energy' — the corpus evidence shows footwear/REIT deals, not energy);
  // left uncorrected here since the entries are retired, corrected instead
  // on the successor codes below.
  'REP-T-REGSTATUS': {
    type: 'REP-T',
    ...
    industries: ['energy'],
    retired: '2026-08-02',
    superseded_by: ['REP-T-40ACT', 'REP-T-ADVISERSACT', 'REP-T-INSREG', 'REP-T-CFIUS'],
  },
  'REP-T-CONSENT': {
    type: 'REP-T',
    ...
    industries: ['energy'],
    retired: '2026-08-02',
    superseded_by: ['REP-T-STOCKAPPROVAL', 'REP-T-GOVAPPROVAL'],
  },
  // R1 split successors. Corpus evidence (FAMILY-MAPPING-RULINGS-2026-08-02):
  // 18/19 REP-T-CONSENT cards were purely governmental approvals; ONE deal
  // (af4940e1/Skechers) drafted "Requisite Stockholder Approval" (§3.4) and
  // "Requisite Governmental Approvals" (§3.6) as separate sections — v1
  // collapsed a distinction the drafters themselves made. `industries`
  // corrected to [] (footwear/REIT corpus, not energy).
  'REP-T-STOCKAPPROVAL': {
    type: 'REP-T',
    label: 'Requisite Stockholder Approval',
    description: 'Target stockholder vote required to approve the merger (mirrors REP-B-VOTE on the buyer side)',
    aliases: ['Requisite Stockholder Approval', 'Company Stockholder Approval', 'Required Stockholder Vote'],
    frequency: 'industry-specific',
    industries: [],
  },
  'REP-T-GOVAPPROVAL': {
    type: 'REP-T',
    label: 'Governmental Approvals',
    description: 'Governmental consents, filings, and approvals required of the target for the transaction',
    aliases: ['Requisite Governmental Approvals', 'Governmental Authorizations', 'Governmental Authorization', 'Government Approvals'],
    frequency: 'industry-specific',
    industries: [],
  },
  // R2 split successors — REP-T-REGSTATUS was a per-industry grab-bag (Ben:
  // "I'd call this 40Act", but 4/10 cards were the DISTINCT Investment
  // Advisers Act, not the Investment Company Act). `industries` corrected to
  // reflect the actual regime each successor covers rather than the stale
  // blanket 'energy' tag.
  'REP-T-40ACT': {
    type: 'REP-T',
    label: 'Investment Company Act Status',
    description: 'Target is not required to register as an investment company under the Investment Company Act of 1940',
    aliases: ['Investment Company Act', 'Investment Company Act of 1940', '1940 Act'],
    frequency: 'industry-specific',
    industries: ['reit', 'financial-services'],
  },
  'REP-T-ADVISERSACT': {
    type: 'REP-T',
    label: 'Investment Advisers Act / Broker-Dealer Registration',
    description: 'Target investment adviser or broker-dealer registration status under the Investment Advisers Act',
    aliases: ['Investment Advisers Act', 'Advisers Act', 'Broker-Dealer Registration'],
    frequency: 'industry-specific',
    industries: ['financial-services'],
  },
  'REP-T-INSREG': {
    type: 'REP-T',
    label: 'Insurance Licensing / Regulatory Status',
    description: 'Target state insurance licensing and regulatory status',
    aliases: ['Insurance Licensing', 'Insurance Regulatory Status', 'Insurance Laws'],
    frequency: 'industry-specific',
    industries: ['insurance'],
  },
  'REP-T-CFIUS': {
    type: 'REP-T',
    label: 'CFIUS / National Security Status',
    description: 'Target status under CFIUS and critical-technologies national-security review',
    aliases: ['CFIUS', 'National Security Status', 'Critical Technologies', 'Foreign Investment Review'],
    frequency: 'industry-specific',
    industries: [],
  },
  ...
  // v1 reclassification (2026-08-02, R3): REP-B-NOREP retired — it folds
  // into REP-B-NOOTHERREPS (a drafting-convention alias, not a distinct
  // concept, per the ruling doc). Stays registered for historic rows.
  'REP-B-NOREP': {
    type: 'REP-B',
    label: 'No Other Representations or Warranties',
    ...
    industries: [],
    retired: '2026-08-02',
    superseded_by: ['REP-B-NOOTHERREPS'],
  },
  'REP-B-CAP': {
    type: 'REP-B',
  ...
  'REP-B-SUBSIDIARIES': {
    type: 'REP-B',
    label: 'Subsidiaries (public buyer)',
    description: 'Buyer representations regarding ownership, validity, and liens on significant subsidiaries',
    aliases: ['Buyer Subsidiaries', 'Subsidiaries'],
    frequency: 'common',
    industries: [],
  },
  'REP-B-REALPROPERTY': {
    type: 'REP-B',
    label: 'Real Property (public buyer)',
    description: 'Buyer real property ownership, leasehold, condemnation, and lease compliance representations',
    aliases: ['Buyer Real Property', 'Real Property'],
    frequency: 'industry-specific',
    industries: [],
  },
  ...
  'IOC-EXISTENCE': {
    type: 'IOC',
    label: 'Maintain Corporate Existence',
    description: 'Maintain corporate existence and good standing during the interim period',
    aliases: ['Maintain Existence', 'Maintain Good Standing', 'Corporate Existence'],
    frequency: 'common',
    industries: [],
  },
  'IOC-PRESERVE': {
  ...
  // P7 item 10: explicit positive- and negative-preamble pseudo-codes so the
  // two halves of the IOC section preamble can be captured separately. The
  // UI compares positive-side carve-outs vs negative-side carve-outs for
  // asymmetry.
  'IOC-POSITIVE-PREAMBLE': {
    type: 'IOC',
    label: 'IOC Positive Preamble (affirmative obligations + general exceptions)',
    description: 'The first half of the IOC preamble — the affirmative-duty paragraph and the section-wide "Except as ..." carve-outs that scope all affirmative covenants',
    aliases: [],
    frequency: 'common',
    industries: [],
  },
  'IOC-NEGATIVE-PREAMBLE': {
    type: 'IOC',
    label: 'IOC Negative Preamble (lead-in to negative covenants list)',
    description: 'The second half of the IOC preamble — the "In addition, without limiting the generality of the foregoing, the Company shall not, and shall not permit any Subsidiary to, do any of the following ..." lead-in that scopes the negative covenants list',
    aliases: [],
    frequency: 'common',
    industries: [],
  },
  ...
  'IOC-MERGE': {
    type: 'IOC',
    label: 'Acquisition / Disposition Covenant',
  ...
  'COND-M-REG': {
    type: 'COND-M',
    // Renamed from 'Regulatory Approvals' per review: the canonical concept is
    // ANTITRUST clearance — HSR expiration/termination plus any scheduled
    // (disclosure-schedule-listed) approvals. Code kept stable for existing data.
    label: 'Antitrust',
    description: 'Antitrust clearance: HSR waiting-period expiration/termination and scheduled regulatory approvals (those listed on the disclosure schedule)',
    aliases: ['Regulatory Approvals', 'HSR Clearance', 'Antitrust Approval', 'Regulatory Clearance', 'Scheduled Approvals'],
    frequency: 'universal',
    industries: [],
  },
  'COND-M-STOCKHOLDER': {
  ...
  // Preambles: the lead-in of each conditions section ("The obligations of …
  // are subject to the satisfaction or waiver of the following conditions").
  // Legally meaningful for WHO may waive and the satisfaction standard —
  // canonical, not an uncoded remainder.
  'COND-M-PREAMBLE': {
    type: 'COND-M',
    label: 'Conditions Preamble (satisfaction / waiver)',
    description: 'Lead-in to the mutual conditions: satisfaction-or-waiver standard and waiver mechanics',
    aliases: ['General / Preamble', 'Conditions Lead-in'],
    frequency: 'universal',
    industries: [],
  },
  'COND-B-PREAMBLE': {
    type: 'COND-B',
    label: 'Conditions Preamble (satisfaction / waiver)',
    description: "Lead-in to Parent/Merger Sub's conditions: satisfaction-or-waiver standard and who may waive",
    aliases: ['General / Preamble', 'Conditions Lead-in'],
    frequency: 'universal',
    industries: [],
  },
  'COND-S-PREAMBLE': {
    type: 'COND-S',
    label: 'Conditions Preamble (satisfaction / waiver)',
    description: "Lead-in to the Company's conditions: satisfaction-or-waiver standard and who may waive",
    aliases: ['General / Preamble', 'Conditions Lead-in'],
    frequency: 'universal',
    industries: [],
  },
  ...
  'TERMR-PREAMBLE': {
    type: 'TERMR',
    label: 'Termination Preamble (lead-in)',
    description: 'Lead-in to the termination rights ("This Agreement may be terminated … whether before or after receipt of stockholder approval")',
    aliases: ['General / Preamble', 'Termination Lead-in'],
    frequency: 'universal',
    industries: [],
  },
  'TERMR-MUTUAL': {
  ...
  'DEF-MAE-CLINICAL': {
    ...
    description: 'A clinical/FDA/pipeline carve-out clause that appears WITHIN the "Material Adverse Effect" definition itself (e.g. "failure to obtain FDA approval … shall not constitute a Material Adverse Effect"). This code is ONLY for clauses inside the MAE definition. Do NOT use it for standalone defined terms whose name happens to mention clinical/FDA/trial/milestone/CVR concepts (e.g. "Initiate", "Clinical Trial Milestone", "Combination FDA Approval", "CVR Product") — those are ordinary or CVR-consideration definitions and belong to DEF-GENERAL.',
  ...
  'DEF-ENVIRONMENTAL-CLAIMS': {
    ...
    description: 'Definition of environmental claims, notices, proceedings, and liabilities',
    aliases: ['Environmental Claims', 'Environmental Claim Definition'],
    frequency: 'industry-specific',
    industries: [],
  },
  ...
  // P7 item 17: parent's adoption of the merger agreement — typically a single
  // section titled "Approval of the Merger" / "Adoption of this Agreement" in
  // a sole-stockholder context (Parent owns 100% of MergerSub and adopts on
  // its behalf), distinct from COV-MEETING (Company stockholders' meeting).
  'COV-SHAPRV-PARENT': {
    type: 'COV',
    label: 'Parent Adoption of Merger Agreement',
    description: 'Parent (as sole stockholder of MergerSub) adopts the agreement, typically by written consent immediately after signing',
    aliases: ['Approval of the Merger', 'Adoption of this Agreement', 'Adoption by Sole Stockholder'],
    frequency: 'common',
    industries: [],
  },
  'COV-PUBLICITY': {
  ...
  'MISC-EXPENSES': {
    ...
    description: 'Each party bears its own fees and expenses, subject to enumerated exceptions (termination fee, regulatory filing fees, expense reimbursement)',
    aliases: ['Fees and Expenses', 'Expenses', 'Transaction Expenses'],
    frequency: 'universal',
    industries: [],
  },
  'MISC-COUNTER': {
  ...
  // v1 reclassification (2026-08-02, R3): REP-B-ANTIRELIANCE retired — the
  // ruling doc's corpus evidence found it mixed FOUR distinct elements
  // (no-other-reps, non-reliance, independent-investigation, fraud
  // carve-out) under one code. Stays registered for historic rows; classify
  // still stamps this family-level code at the title-match stage (see
  // classify.js SUBCODE_REFINEMENT_RULES), and the extract-phase element
  // scan (lib/parser-v2/extract.js) does the actual element split.
  'REP-B-ANTIRELIANCE': {
    type: 'REP-B',
    ...
    industries: [],
    retired: '2026-08-02',
    superseded_by: ['REP-B-NOOTHERREPS', 'REP-B-NONRELIANCE', 'REP-B-INDEPINVEST', 'REP-B-FRAUDCARVEOUT'],
  },
  // R3 element-split successors (buyer/parent side) — mirrors the REP-T-
  // prefixed set above; see that block's comment for the party-mirroring
  // rationale.
  'REP-B-NOOTHERREPS': {
    type: 'REP-B',
    label: 'No Other Representations (Disclaimer)',
...
const FEATURES = {
  IOC: [
    ...
    // FB3 missed item 2: cross-deal comparability hook. Deterministic
    // post-pass (stampIocRestrictionComponents in lib/parser-v2/extract.js)
    // keyword-matches EACH sub-clause's own text against the IOC_CATEGORY_CODES
    // synonym regexes and stamps every canonical category it hits — a bundled
    // sub-clause (e.g. one clause barring both new indebtedness AND
    // guaranteeing another Person's indebtedness) carries BOTH tags. Never
    // LLM-derived; array of plain code strings, not tagged objects.
    {
    ...
    },
    {
      ...
      // Tagged item {code,label,text} resolved against the MAT_* taxonomy —
      // was declared boolean before the taxonomy existed.
      type: 'object',
      scope: 'preamble',
    },
    ...
    //
    //    FB3 item 2: each limb carries EXACTLY ONE standard field —
    //    "efforts_standard", drawn from EFFORTS_STANDARDS (which now includes
    //    FLAT for an unqualified, mandatory duty with no "use ... efforts to"
    //    language). There is NO separate per-limb materialityQualifier —
    //    a limb previously carrying both an efforts_standard AND an
    //    independently-derived materialityQualifier (e.g. efforts_standard:
    //    COMMERCIALLY_REASONABLE_EFFORTS *and* materialityQualifier: FLAT on
    //    the SAME limb) rendered as two contradictory pills in the UI. A
    //    deterministic post-pass (normalizeIocLimbEffortsStandards) derives
    //    efforts_standard from the limb's own text and strips any legacy
    //    materialityQualifier/materiality key so this can never recur.
    //
    //    Each limb is an object:
    //      { obligation, efforts_standard, appliesTo, includedObligations? }
    //      - obligation: verbatim or near-verbatim short phrase describing the
    //        affirmative duty
    //      - efforts_standard: ONE EFFORTS_STANDARDS code — FLAT when the
    //        duty is unqualified (e.g. "shall maintain ... in all material
    //        respects", "conduct its business in the ordinary course"), or
    //        the actual named effort standard when the duty is phrased as
    //        "use commercially reasonable efforts to ..."
    //      - appliesTo: list of IOC_AFFIRMATIVE_SCOPE codes
    //      - includedObligations: optional child limbs introduced by
    //        "including" language under the parent duty
    {
  ...
  'COND-M': [
    ...
    // ── FB3 item 4(d): antitrust condition — HSR + scheduled approvals as
    //    two SEPARATE tagged pills (no section/schedule cite in the label —
    //    same discipline as TERMF trigger labels). Supersedes the bundled
    //    free-text regulatoryApprovals for display purposes; regulatoryApprovals
    //    stays for backward compat / free-text detail.
    { key: 'antitrustApprovals', label: 'Antitrust approvals — tagged items from ANTITRUST_APPROVAL_CODES (HSR / SCHEDULED_APPROVALS), one per approval type actually present', type: 'list-tagged' },
    // ── FB3 item 4(c): stockholder-approval condition — deterministic
    //    post-pass pulls the DEF "Company Stockholder Approval" core.
    { key: 'approvalDefinition', label: 'Stockholder-approval definition (verbatim core, from the DEF provision)', type: 'text', source: 'post-pass' },
    // ── FB3 item 4(b): bring-down rep cites / consents cross-refs resolved
    //    to provision names — deterministic post-pass (resolveCondCitedProvisionNames),
    //    reusing the resolveAocCovenantCitations cite-index/range-expansion pattern.
    { key: 'citedProvisionNames', label: 'Cited provisions resolved to names — [{ section, name }]', type: 'list', source: 'post-pass' },
  ],
  ...
  'COND-B': [
    ...
    { key: 'regulatoryApprovals', label: 'Required regulatory approvals (jurisdictions / agencies, free text)', type: 'text' },
    { key: 'hsrClearance', label: 'HSR clearance is a closing condition', type: 'boolean' },
    // ── FB3 item 4(a): No-MAE closing condition — the modern form reads
    //    "there shall not have occurred any Company Material Adverse Effect
    //    that IS CONTINUING" (a standalone, negotiated qualifier — NOT every
    //    No-MAE condition has it). Capture presence + the verbatim anchor.
    { key: 'continuingRequirement', label: 'No-MAE condition requires the MAE to be continuing (verbatim anchor, e.g. "that is continuing")', type: 'boolean' },
    // ── FB3 item 4(b): see the matching COND-M entry above.
    { key: 'citedProvisionNames', label: 'Cited provisions resolved to names — [{ section, name }]', type: 'list', source: 'post-pass' },
  ],
  ...
  NOSOL: [
    ...
    {
      key: 'noticePeriod',
      // Units vary by deal (hours for the inbound-notice duty, business days
      // for the match-period notice) — the quote/text carries the real unit
      // verbatim; do not hardcode one here (audit-2 item 2: a hardcoded
      // "(hours)" label matched the display bug that mislabeled Metsera's
      // and Skechers' business-day periods as hours).
      label: 'Notice Period',
      type: 'duration',
    },
    {
    ...
    // Coded strength sibling to the forceTheVote boolean (which stays for
    // backward compatibility). Codebook: FORCE_THE_VOTE in lib/taxonomy.js.
    // STRENGTH only (HARD/SOFT/NONE — does the obligated party have a
    // self-termination-for-Superior escape?). Party (whose vote is locked) rides
    // the row via appliesToParty; both-sided deals get one FTV row per party.
    {
      key: 'forceTheVoteType',
      label: 'Force the Vote — strength code (FTV_HARD / FTV_SOFT / FTV_NONE)',
      type: 'tagged',
    },
    {
    ...
    { key: 'ceaseDiscussionsProhibitedList', label: 'Prohibited acts during cease-discussions period (list)', type: 'list-tagged' },
    ...
    { key: 'changeOfRecommendationItems', label: 'What constitutes a Change of Recommendation (list)', type: 'list-tagged' },
    ...
    { key: 'superiorProposalDeterminer', label: 'Superior Proposal determiner (Board only / Board with advisors)', type: 'tagged' },

    // ── P7 item 12: Discussion-Initiation Notice (typically 48h notification
    //    of any inbound contact) + No-Conflicting-Agreements duty ─────────
    { key: 'discussionInitiationNoticePresent', label: 'Discussion-initiation notice present (e.g. 48h notice of inbound contact)', type: 'boolean' },
    { key: 'discussionInitiationNoticeHours', label: 'Discussion-initiation notice deadline (hours)', type: 'duration' },
    { key: 'discussionInitiationNoticeText', label: 'Discussion-initiation notice — verbatim language', type: 'text' },
    { key: 'noConflictingAgreementsPresent', label: 'No-conflicting-agreements duty present', type: 'boolean' },
    { key: 'noConflictingAgreementsScope', label: 'No-conflicting-agreements scope (verbatim)', type: 'text' },

    // ── P7 item 13: Information-Sharing / Equal-Information obligation ──
    { key: 'informationSharingObligationPresent', label: 'Information-sharing / equal-info duty present (Company must share inbound info with existing buyer)', type: 'boolean' },
    { key: 'informationSharingObligationScope', label: 'Information-sharing scope (verbatim)', type: 'text' },
    { key: 'informationSharingObligationTiming', label: 'Information-sharing timing (e.g. "promptly", "concurrently", "within 24h")', type: 'text' },

    // ── P7 item 14: ARC re-affirmation deadline ───────────────────────────
    { key: 'arcReaffirmDeadlineDays', label: 'ARC re-affirmation deadline (business days) — for "fail to publicly recommend against ... within X days" sub-item', type: 'duration' },

    // ── P7 item 16: Tender-offer / safe-disclosure carve-outs ─────────────
    { key: 'tenderOfferDisclosurePermitted', label: 'Tender-offer disclosure (Rule 14d-9 "stop, look and listen") permitted without triggering ARC', type: 'boolean' },
    { key: 'tenderOfferDisclosureScope', label: 'Tender-offer disclosure scope (verbatim)', type: 'text' },
    { key: 'legallyRequiredDisclosurePermitted', label: 'Legally-required-disclosure carve-out permitted', type: 'boolean' },
    { key: 'safeDisclosureCarveoutLanguage', label: 'Safe-disclosure carve-out — verbatim language', type: 'text' },
  ],
  ...
  ANTI: [
    ...
    { key: 'burdenCommitment', label: 'Burden commitment', type: 'tagged' },
    { key: 'capDetail', label: 'Cap detail', type: 'text' },
    { key: 'burdenBaseline', label: 'Burden measurement baseline', type: 'tagged' },
    {
      key: 'appliesToParty',
    ...
    { key: 'regulatoryStrategyControlTagged', label: 'Regulatory strategy control', type: 'tagged' },
    {
    ...
    { key: 'hsrFilingDeadline', label: 'HSR filing deadline', type: 'object' },
    { key: 'exHsrFilingDeadline', label: 'Ex-HSR filing deadline', type: 'object' },
    {
    ...
    { key: 'pullRefile', label: 'Pull-and-refile gating', type: 'tagged' },
    { key: 'pullRefileText', label: 'Pull-and-refile text', type: 'text' },
    { key: 'timingAgreementsProhibited', label: 'Timing agreements with regulators prohibited', type: 'boolean' },
    { key: 'timingAgreement', label: 'Timing agreement restriction', type: 'tagged' },
    { key: 'timingAgreementText', label: 'Timing agreement text', type: 'text' },
    ...
    { key: 'clearSkies', label: 'Clear-skies by party', type: 'object' },
    { key: 'consultationTier', label: 'Consultation tier', type: 'tagged' },
  ...
  'ANTI-FILING': [
    ...
    { key: 'exHsrFilingDeadline', label: 'Ex-HSR filing deadline', type: 'object' },
    { key: 'hsrFilingDeadlineBusinessDays', label: 'HSR filing deadline (business days from signing)', type: 'duration' },
    { key: 'otherRegulatoryFilingDeadlines', label: 'Other regulatory filing deadlines', type: 'text' },
  ],
  ...
  'ANTI-BURDEN': [
    ...
    { key: 'burdensomeConditionPresent', label: 'Burdensome Condition Present', type: 'boolean' },
    { key: 'burdensomeConditionScope', label: 'Burdensome Condition Scope', type: 'text' },
    { key: 'burdensomeConditionInTerminationTriggers', label: 'Burdensome condition in termination triggers', type: 'text' },
  ],
  ...
  'ANTI-COOPERATE': [
    ...
    { key: 'regulatoryStrategyControlTagged', label: 'Regulatory strategy control', type: 'tagged' },
    { key: 'regulatoryStrategyControl', label: 'Regulatory strategy control (legacy enum)', type: 'enum', options: ['PARENT_CONTROL', 'COMPANY_CONTROL', 'JOINT', 'NA'] },
    { key: 'consultationTier', label: 'Consultation tier', type: 'tagged' },
    { key: 'regulatoryCooperationScope', label: 'Regulatory information / cooperation covenant — scope', type: 'text' },
  ],
  ...
  TERMF: [
    ...
    },
    {
      ...
      type: 'tagged',
      description: 'Basis of the late-payment interest rate — the published rate the late fee accrues at (WSJ prime / named-bank prime / 6-month Treasury / LIBOR / fixed). The spread and named source stay on interestOnLatePayment.',
    },
    {
      key: 'terminationFees',
    ...
    { key: 'triggers', label: 'Per-trigger array — list of { code, name, terminationClauses, feeAmount, feeAmountPct } (code from TERMF_TRIGGER_CODES; name is the plain-English trigger)', type: 'list' },
  ],
  ...
  'TERMF-TARGET': [
    ...
    { key: 'interestRateBasis', label: 'Interest-rate basis on late payment', type: 'tagged', description: 'Basis of the late-payment interest rate — the published rate the late fee accrues at (WSJ prime / named-bank prime / 6-month Treasury / LIBOR / fixed). The spread and named source stay on interestOnLatePayment.' },
  ...
  //   This is the ONLY TERMR code that carries the outside-date VALUE fields
  //   (outsideDate, outsideDateMonths, outsideDateISO, extendedOutsideDateISO).
  //   Per fix #3, those keys MUST NOT appear on any other TERMR-* code.
  //   outsideDateExtension (the boolean flag) is the one deliberate exception:
  //   its sibling TERMR-EXTENSION also carries it -- see that code's own
  //   comment below.
  'TERMR-OUTSIDE': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    {
    ...
    // ── FB3 item 5: clean ISO dates (separate from the narrative "outsideDate"
    //    text above, which often bundles the initial + extended date into one
    //    prose string) so a deterministic post-pass can do pure date math.
    { key: 'outsideDateISO', label: 'Outside Date, normalized (ISO YYYY-MM-DD)', type: 'text' },
    { key: 'extendedOutsideDateISO', label: 'Extended Outside Date, normalized (ISO YYYY-MM-DD)', type: 'text' },
    // Deterministic post-pass (computeOutsideDateMonths): pure date math off
    // the deal's signing date + the two ISO dates above. Null when either
    // date is missing — never invented.
    { key: 'outsideDateMonthsPostSigning', label: 'Outside Date — months after signing (computed)', type: 'duration', source: 'post-pass' },
    { key: 'extensionMonths', label: 'Outside Date extension length (months, computed)', type: 'duration', source: 'post-pass' },
  ],
  ...
  STRUCT: [
    ...
    // ── P7 item 2: STRUCT-EFFECTS / STRUCT-EFFTIME single-fact extracts ────
    { key: 'effectsOfMergerReference', label: 'Effects of Merger — statute cited (e.g. "DGCL § 259")', type: 'text' },
    { key: 'effectiveTimeShort', label: 'Effective Time — one-sentence summary (e.g. "Upon filing of the Certificate of Merger with the Delaware Secretary of State")', type: 'text' },
  ],
  ...
  'STRUCT-OFFER': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'dealStructure', label: 'Deal structure', type: 'enum', options: ['TWO_STEP_TENDER_OFFER'] },
    { key: 'mergerForm', label: 'Merger Form', type: 'tagged' },
    { key: 'offerCommencementDeadline', label: 'Offer commencement deadline', type: 'text' },
    { key: 'offerPrice', label: 'Offer price', type: 'text' },
    { key: 'offerConsideration', label: 'Offer consideration form', type: 'text' },
    { key: 'offerConditionsReference', label: 'Offer conditions reference', type: 'text' },
    { key: 'offerExpirationAndExtension', label: 'Offer expiration and extension mechanics', type: 'text' },
    { key: 'acceptanceAndPaymentMechanics', label: 'Acceptance and payment mechanics', type: 'text' },
    { key: 'scheduleTOFiling', label: 'Schedule TO / offer documents filing mechanics', type: 'text' },
    { key: 'schedule14D9Filing', label: 'Schedule 14D-9 filing mechanics', type: 'text' },
    { key: 'stockholderListCovenant', label: 'Stockholder-list / holder-communications covenant', type: 'text' },
    { key: 'buyerBoardDesignation', label: 'Buyer board-designation rights after acceptance', type: 'text' },
    { key: 'section251h', label: 'DGCL Section 251(h) back-end merger', type: 'boolean' },
    { key: 'shortFormMergerMechanic', label: 'Short-form merger / 90% ownership mechanic', type: 'text' },
    { key: 'backendMergerMechanic', label: 'Back-end merger mechanic', type: 'text' },
  ],
  ...
  CONSID: [
    ...
    },
    {
      ...
      // Tagged item {code,text} resolved against EQUITY_TREATMENTS in practice.
      type: 'object',
    },
  ...
  'REP-T': [
    ...
    {
      // Stamped deterministically (post-pass, linkKnowledgeScopeToReps) off
      // the same regex that already detects "to the Knowledge of the
      // Company" / "to Parent's Knowledge" phrasing in the rep's OWN text —
      // an LLM-authored article-wide boolean was unreliable and, worse,
      // could only ever live on the preamble row, never on the individual
      // rep rows the review table actually renders this pill for.
    ...
    // ── MetsFB2: Absence-of-Changes "No MAE since [date]" limb + IOC-cite limb ──
    { key: 'aocNoMaePresent', label: 'AoC rep — "no MAE since [date]" limb present', type: 'boolean', scope: 'clause' },
    { key: 'aocNoMaeSinceDate', label: 'AoC rep — "no MAE since" date (verbatim, e.g. "December 31, 2024")', type: 'text', scope: 'clause' },
    { key: 'aocOrdinaryCourseLimb', label: 'AoC rep — ordinary-course limb (verbatim)', type: 'text', scope: 'clause' },
    { key: 'aocCitedCovenantSections', label: 'AoC rep — covenant sections cited by the ordinary-course limb', type: 'list', scope: 'clause' },
    // Resolved deterministically (post-pass) against the deal's own IOC
    // sub-clause provisions — array of { section, name }; never AI-populated.
    { key: 'aocCitedCovenantNames', label: 'AoC rep — cited covenant sections resolved to names', type: 'list', scope: 'clause', source: 'post-pass' },
    ...
    { key: 'lookbackPeriod', label: 'Lookback period (years and date since which reps are made)', type: 'text', scope: 'clause' },
    ...
    // ── MetsFB2: No-Other-Reps / Non-Reliance (Abry) fields — populated ONLY
    //    on the "No Other Representations and Warranties" / "Non-Reliance"
    //    section; null/false everywhere else. Silence on the fraud carve-out
    //    is legally meaningful and stays null.
    { key: 'noOtherRepsPresent', label: 'No-Other-Reps / Non-Reliance provision present', type: 'boolean', scope: 'clause' },
    { key: 'noOtherRepsParty', label: 'Whose reps are disclaimed beyond the agreement', type: 'enum', options: ['COMPANY', 'PARENT', 'BOTH'], scope: 'clause' },
    { key: 'nonRelianceClause', label: 'Non-reliance disclaimer (verbatim core)', type: 'text', scope: 'clause' },
    { key: 'extraContractualClaimsWaived', label: 'Reliance on extra-contractual statements expressly disclaimed (Abry)', type: 'boolean', scope: 'clause' },
    { key: 'fraudCarveout', label: 'Fraud claims expressly preserved (verbatim; null if silent)', type: 'text', scope: 'clause' },
    // ── MetsFB2: Knowledge-definition scope, stamped deterministically from
    //    the deal's DEF "Knowledge" provision onto knowledge-qualified reps.
    { key: 'knowledgeScope', label: 'Knowledge definition (verbatim core, from the DEF provision)', type: 'text', scope: 'clause', source: 'post-pass' },
    { key: 'knowledgeScopeType', label: 'Knowledge qualifier scope (ENTIRE_REP / PARTIAL)', type: 'enum', options: ['ENTIRE_REP', 'PARTIAL'], scope: 'clause' },
    // ── FB3 item 1: materiality-qualifier scope, mirroring knowledgeScopeType
    //    exactly. LLM-populated (no `source`) whenever this rep carries a
    //    materiality qualifier; a deterministic post-pass (linkMaterialityScopeToReps)
    //    folds it into features.materialityQualifier.scope — never invented
    //    when the model didn't populate it.
    { key: 'materialityScopeType', label: 'Materiality qualifier scope (ENTIRE_REP / PARTIAL)', type: 'enum', options: ['ENTIRE_REP', 'PARTIAL'], scope: 'clause' },
  ],
  ...
  'REP-B': [
    ...
    {
      // See the matching REP-T entry above: stamped deterministically
      // (post-pass, linkKnowledgeScopeToReps) per rep row, not LLM-authored.
      key: 'knowledgeQualifier',
      label: 'Knowledge Qualifier Used (per rep)',
      type: 'object',
      scope: 'clause',
    ...
    { key: 'knowledgeScope', label: 'Knowledge definition (verbatim core, from the DEF provision)', type: 'text', scope: 'clause', source: 'post-pass' },
    { key: 'knowledgeScopeType', label: 'Knowledge qualifier scope (ENTIRE_REP / PARTIAL)', type: 'enum', options: ['ENTIRE_REP', 'PARTIAL'], scope: 'clause' },
    // ── FB3 item 1: see the matching REP-T entry above.
    { key: 'materialityScopeType', label: 'Materiality qualifier scope (ENTIRE_REP / PARTIAL)', type: 'enum', options: ['ENTIRE_REP', 'PARTIAL'], scope: 'clause' },
  ],
  ...
  MISC: [
    ...
    // ── FB3 item 6(b): the governing-law/jurisdiction provision must name
    //    the actual forum court(s) and any fallback — never just a bare
    //    boolean. "forumCourts" is the named court(s) verbatim (e.g. "the
    //    Court of Chancery of the State of Delaware, or, if such court lacks
    //    subject matter jurisdiction, any federal court located in the State
    //    of Delaware"); "forumFallback" is the fallback forum verbatim when
    //    the primary court lacks jurisdiction (often folded into the same
    //    sentence — extract the fallback clause separately when present).
    {
      key: 'forumCourts',
    ...
    { key: 'parentAssignmentConditions', label: 'Parent assignment — conditions text', type: 'list-tagged' },
    { key: 'assignmentProvisos', label: 'Assignment provisos (list)', type: 'list-tagged' },
    ...
    // ── No Other Reps / Fraud (Abry) — MISC-embedded landing ──
    // Many agreements fold the anti-reliance / no-other-reps language INTO
    // the Entire Agreement / Integration section (MISC-ENTIRE) instead of
    // giving it its own titled section (Metsera §9.07: "Entire Agreement;
    // Third-Party Beneficiaries; No Other Representations or Warranties").
    // The classify-time subcode rules only stamp the family-level codes
    // (REP-T-NOREP / REP-B-NOREP / REP-B-ANTIRELIANCE -- retired 2026-08-02
    // in favour of the eight R3 element successors, REP-T-/REP-B- x
    // NOOTHERREPS/NONRELIANCE/INDEPINVEST/FRAUDCARVEOUT, that extract.js's
    // element scan re-codes a title match into) from a SECTION TITLE match,
    // so a compound title like Metsera's — or embedded language with no
    // dedicated title at all — never reaches any of them. Registering the
    // same fields directly on the generic MISC schema means the MISC
    // extraction prompt (which runs at the section level, before a specific
    // code is chosen — see the COV-EMPLOYEE merge above) can populate them
    // wherever the language actually lives, and getFeaturesForType('MISC',
    // 'MISC-ENTIRE') picks them up via its bare-type fallback. Keys match
    // every one of those landing codes exactly (the three retired family
    // codes and their eight successors) so the review page's Abry summary
    // (lib/abry.js) can scan every provision type/code uniformly.
    { key: 'noOtherRepsPresent', label: 'No-Other-Reps / Non-Reliance provision present', type: 'boolean' },
    { key: 'noOtherRepsParty', label: 'Whose reps are disclaimed beyond the agreement', type: 'enum', options: ['COMPANY', 'PARENT', 'BOTH'] },
    { key: 'nonRelianceClause', label: 'Non-reliance disclaimer (verbatim core)', type: 'text' },
    { key: 'extraContractualClaimsWaived', label: 'Reliance on extra-contractual statements expressly disclaimed (Abry)', type: 'boolean' },
    { key: 'fraudCarveout', label: 'Fraud claims expressly preserved (verbatim; null if silent)', type: 'text' },
    // v1 reclassification (2026-08-02, R3): mirrors the field added to the
    // REP-B-INDEPINVEST / REP-T-INDEPINVEST schemas — added here too so the
    // MISC-embedded landing spot (Metsera §9.07 pattern, see comment above)
    // can populate it just like a dedicated titled section can.
    { key: 'independentInvestigationAcknowledged', label: 'Party acknowledges its own independent investigation, review, and analysis', type: 'boolean' },
  ],
  ...
  'COV-PROXY': [
    { key: 'boardRecommendationInclusion', label: 'Board recommendation included in proxy statement', type: 'boolean' },
    { key: 'meetingConveneObligation', label: 'Stockholder meeting convene-and-hold obligation', type: 'boolean' },
    { key: 'meetingAdjournmentMaxCount', label: 'Maximum number of meeting adjournments', type: 'number' },
    { key: 'meetingAdjournmentMaxDays', label: 'Maximum meeting adjournment period in days', type: 'duration' },
    { key: 'proxyFilingDeadline', label: 'Proxy / S-4 filing deadline object { text, days, unit: BUSINESS_DAYS | CALENDAR_DAYS | null, trigger: SIGNING | AGREEMENT_DATE | null }', type: 'object' },
    { key: 'mailingDeadline', label: 'Mailing deadline object { text, days, unit: BUSINESS_DAYS | CALENDAR_DAYS | null, trigger: SEC_CLEARANCE | NO_SEC_COMMENT_PERIOD_END | EFFECTIVENESS | FILING | null }', type: 'object' },
    { key: 'meetingDeadline', label: 'Meeting deadline object { text, days, unit: BUSINESS_DAYS | CALENDAR_DAYS | null, trigger: MAILING | EFFECTIVENESS | CLEARANCE | null }', type: 'object' },
    { key: 'adjournmentRights', label: 'Adjournment rights array of { party: COMPANY | PARENT | MUTUAL, reasons: array of tagged { code, label, text } with codes QUORUM_ABSENT, INSUFFICIENT_VOTES, SUPPLEMENTAL_DISCLOSURE, LEGAL_REQUIREMENT, OTHER, maxAdjournments, maxDaysPerAdjournment, maxDaysTotal, text }', type: 'object-list' },
    { key: 'meetingControlNotes', label: 'Meeting-control notes (verbatim)', type: 'text' },
  ],
  ...
  // P7 item 17: Parent adoption of the merger agreement (typically
  // Parent-as-sole-stockholder-of-MergerSub adopting by written consent
  // contemporaneously with signing).
  'COV-SHAPRV-PARENT': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'parentAdoptionTiming', label: 'When Parent adopts (e.g. "concurrently with execution", "promptly after the date hereof")', type: 'text' },
    { key: 'parentAdoptionMechanism', label: 'Adoption mechanism', type: 'enum', options: ['SOLE_STOCKHOLDER_ADOPTION', 'WRITTEN_CONSENT', 'BOARD_ONLY'] },
  ],
  ...
  'REP-B-ANTIRELIANCE': [
    ...
    // MetsFB2 Abry fields (shared with REP-T-NOREP / REP-B-NOREP below) —
    // anti-reliance sections land under several codes depending on where the
    // drafters put them; all of them carry the same structured analysis.
    { key: 'noOtherRepsPresent', label: 'No-Other-Reps / Non-Reliance provision present', type: 'boolean' },
    { key: 'noOtherRepsParty', label: 'Whose reps are disclaimed beyond the agreement', type: 'enum', options: ['COMPANY', 'PARENT', 'BOTH'] },
    { key: 'nonRelianceClause', label: 'Non-reliance disclaimer (verbatim core)', type: 'text' },
    { key: 'extraContractualClaimsWaived', label: 'Reliance on extra-contractual statements expressly disclaimed (Abry)', type: 'boolean' },
  ...
  'REP-T-NOREP': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'language', label: 'Verbatim no-other-reps language', type: 'text' },
    { key: 'noOtherRepsPresent', label: 'No-Other-Reps / Non-Reliance provision present', type: 'boolean' },
    { key: 'noOtherRepsParty', label: 'Whose reps are disclaimed beyond the agreement', type: 'enum', options: ['COMPANY', 'PARENT', 'BOTH'] },
    { key: 'nonRelianceClause', label: 'Non-reliance disclaimer (verbatim core)', type: 'text' },
    { key: 'extraContractualClaimsWaived', label: 'Reliance on extra-contractual statements expressly disclaimed (Abry)', type: 'boolean' },
  ...
  // v1 reclassification (2026-08-02, R3) — element-split successors of
  // REP-B-ANTIRELIANCE / REP-B-NOREP / REP-T-NOREP. Each reuses the shared
  // Abry-style schema block verbatim (the registered pattern above), except
  // the INDEPINVEST pair which adds ONE new field
  // (independentInvestigationAcknowledged) — no existing field represented
  // it. That field is also added to the shared MISC block below (Abry
  // fields land there when the language is embedded in an Entire Agreement
  // section with no dedicated title — Metsera §9.07 pattern).
  'REP-B-NOOTHERREPS': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'language', label: 'Verbatim no-other-reps language', type: 'text' },
    { key: 'noOtherRepsPresent', label: 'No-Other-Reps / Non-Reliance provision present', type: 'boolean' },
  ...
  // R1 codes — deliberately thin per-code minimal schemas (growth is a
  // later adjudication, not an implementer guess).
  'REP-T-STOCKAPPROVAL': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'voteStandard', label: 'Vote standard required (e.g. majority of outstanding shares)', type: 'text' },
    { key: 'shareClassesEntitled', label: 'Share classes entitled to vote', type: 'list' },
  ],

  'REP-T-GOVAPPROVAL': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'regimesCited', label: 'Regulatory regimes cited', type: 'list' },
  ],

  // R2 codes — deliberately thin per-code minimal schemas.
  'REP-T-40ACT': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'registrationRequired', label: 'Registration required under the statute', type: 'enum', options: ['YES', 'NO', 'UNCLEAR'] },
    { key: 'statuteCited', label: 'Statute cited (verbatim)', type: 'text' },
  ],
  ...
  // P7 item 10: per-code feature schemas for the two IOC preamble halves so
  // the UI can compare carve-outs across the positive/negative sides. Both
  // inherit the general IOC schema; the negative side adds two preamble-only
  // fields for the affiliate / no-permit standard and the negative-side
  // exception list.
  'IOC-NEGATIVE-PREAMBLE': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'negativePreambleAffiliateStandard', label: 'Negative preamble — affiliate / subsidiary standard (e.g. "shall not, and shall not permit any Subsidiary to")', type: 'text' },
    { key: 'negativePreambleExceptions', label: 'Negative preamble — section-wide exceptions (tagged list)', type: 'list-tagged' },
  ],

  // P7 item 18: Frustration-of-Conditions is a single meta-rule, not a
  // collection of sub-clauses. Treat it as one provision with structured
  // {test, applies, language} fields rather than splitting on (a)/(b)/(c).
  'COND-FRUSTRATE': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'frustrationOfConditionsPresent', label: 'Frustration-of-conditions rule present', type: 'boolean' },
    { key: 'frustrationTest', label: 'Frustration test', type: 'enum', options: ['PRIMARY_CAUSE', 'PRINCIPAL_CAUSE', 'MATERIAL_BREACH', 'WILLFUL_BREACH', 'ANY_BREACH', 'OTHER'] },
    { key: 'frustrationApplies', label: 'Who is bound', type: 'enum', options: ['MUTUAL', 'PARENT_ONLY', 'COMPANY_ONLY'] },
    { key: 'frustrationLanguage', label: 'Verbatim frustration language', type: 'text' },
  ],

  // P7 item 23: REP-T-NOLIAB ("No Undisclosed Liabilities") deserves its own
  // per-row entry — Financial Statements + No Undisclosed Liabilities have
  // historically been collapsed. Schema gives the row its undisclosed-liabilities
  // exceptions back-reference; REP_SPECIFIC_FEATURE_SPECS already targets this
  // category via the /undisclosed\s+liabilities|no\s+liabilities/ regex.
  'REP-T-NOLIAB': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    { key: 'undisclosedLiabilitiesExceptions', label: 'Undisclosed-liabilities exceptions (list of tagged items)', type: 'list-tagged' },
  ],
  ...
  // P7 item 20: dropped materialityScrapePresent + materialityScrapeLanguage.
  // The closing-condition-level materiality scrape lives on COND, not on the
  // reps preamble.
  'REP-T-PREAMBLE': [
    { key: 'mainConcept', label: 'Provision', type: 'text' },
    ...
    { key: 'secFilingsExceptionCutoffStandard', label: 'SEC-filings cutoff (normalized: ONE_BUSINESS_DAY_PRE_SIGNING / DATE_OF_AGREEMENT / ABSOLUTE_DATE / OTHER)', type: 'enum', options: ['ONE_BUSINESS_DAY_PRE_SIGNING', 'DATE_OF_AGREEMENT', 'ABSOLUTE_DATE', 'OTHER'] },
...
const CITABLE_FEATURE_KEYS = new Set([
  ...
  'burdenCommitment', 'capDetail', 'burdenBaseline',
  'litigationObligationQualification', 'regulatoryStrategyControlTagged',
  'consultationTier', 'pullRefile', 'pullRefileText',
  'timingAgreement', 'timingAgreementText', 'hsrFilingDeadline',
  'exHsrFilingDeadline', 'clearSkies',
  ...
  'adsPresent',
  'offerCommencementDeadline', 'offerPrice', 'offerConsideration',
  'offerConditionsReference', 'offerExpirationAndExtension',
  'acceptanceAndPaymentMechanics', 'scheduleTOFiling', 'schedule14D9Filing',
  'stockholderListCovenant', 'buyerBoardDesignation', 'shortFormMergerMechanic',
  'backendMergerMechanic',
  // P7 item 2: STRUCT-EFFECTS / STRUCT-EFFTIME mini-fields
  'effectsOfMergerReference', 'effectiveTimeShort',
  // P7 item 18: COND-FRUSTRATE
  'frustrationOfConditionsPresent', 'frustrationTest', 'frustrationApplies',
  'frustrationLanguage',

  // P3: NOSOL additions
  'interveningEventScope', 'superiorProposalThresholdPct',

  // P7 items 12 / 13 / 14 / 16: NOSOL additions
  'discussionInitiationNoticePresent', 'discussionInitiationNoticeHours',
  'noConflictingAgreementsPresent',
  'informationSharingObligationPresent',
  'arcReaffirmDeadlineDays',
  'tenderOfferDisclosurePermitted', 'legallyRequiredDisclosurePermitted',
 ...
 * retired codes are EXCLUDED by default. This is the extract-prompt
 * vocabulary builder (buildCodesList in lib/parser-v2/extract.js reads this
 * list) — a retired code must never be shown to the model as an option to
 * emit. Pass { includeRetired: true } for call sites that need the full
 * registry (e.g. admin/back-office tooling, historic-data audits).
 */
function getCodesForType(typeKey, opts = {}) {
  let lookupType = typeKey === 'IOC-T' || typeKey === 'IOC-B' ? 'IOC' : typeKey;
  if (lookupType === 'NOSOL-T' || lookupType === 'NOSOL-B' || lookupType === 'NOSOL-M') lookupType = 'NOSOL';
  const includeRetired = !!(opts && opts.includeRetired);
  const results = [];
  for (const [code, entry] of Object.entries(CODES)) {
    if (entry.type !== lookupType) continue;
    if (entry.retired && !includeRetired) continue;
...
/**
 * Returns true if the given code string exists in the rubric. Retired codes
 * are still VALID (they stay registered — historic rows and fixtures
 * reference them); this only checks registry membership, not currency. Use
 * isRetiredCode / getSupersededBy to check/act on retirement.
 */
...
/**
 * Returns true if `code` is a retired canonical code.
 */
function isRetiredCode(code) {
  const entry = CODES[code];
  return !!(entry && entry.retired);
}

/**
 * Returns the superseded_by array for a retired code, or null if the code
 * isn't retired / doesn't exist / has no successor recorded.
 */
function getSupersededBy(code) {
  const entry = CODES[code];
  if (!entry || !entry.retired || !Array.isArray(entry.superseded_by)) return null;
...
function getFeaturesForType(typeKey, code) {
  ...
  if (code && FEATURES[code]) {
    ...
    if (code === 'COV-PROXY' || code === 'COV-MEETING') return codeFeats;
