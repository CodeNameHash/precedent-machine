...
const FEATURES = {
  ...
  MISC: [
    ...
    // The classify-time subcode rules only stamp REP-T-NOREP / REP-B-NOREP /
    // REP-B-ANTIRELIANCE from a SECTION TITLE match, so a compound title like
    // Metsera's — or embedded language with no dedicated title at all — never
    // reaches those codes. Registering the same five fields directly on the
    // generic MISC schema means the MISC extraction prompt (which runs at the
    // section level, before a specific code is chosen — see the COV-EMPLOYEE
    // merge above) can populate them wherever the language actually lives,
    // and getFeaturesForType('MISC', 'MISC-ENTIRE') picks them up via its
    // bare-type fallback. Keys match REP-T-NOREP / REP-B-NOREP /
    // REP-B-ANTIRELIANCE exactly so the review page's Abry summary
