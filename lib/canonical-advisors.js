...
function getDisplayAdvisors(metadata) {
  ...
  const v2 = meta.advisors_v2;
  const empty = { buyerFirm: null, buyerFirms: [], buyerLawyers: [], sellerFirm: null, sellerFirms: [], sellerLawyers: [] };
  if (!v2 || typeof v2 !== 'object') {
    const facts = meta.deal_facts && meta.deal_facts.advisors && typeof meta.deal_facts.advisors === 'object'
      ? meta.deal_facts.advisors
      : null;
    if (facts) {
      const buyerFirms = dedupeFirms(facts.buyer_firms);
      const sellerFirms = dedupeFirms(facts.seller_firms);
      return { ...empty, buyerFirm: buyerFirms[0] || null, buyerFirms, sellerFirm: sellerFirms[0] || null, sellerFirms };
    }
    const legacy = Array.isArray(meta.advisors) ? meta.advisors : [];
    if (legacy.length === 0) return empty;
    const buyerRaw = [];
    const sellerRaw = [];
    for (const row of legacy) {
      const firm = row && row.firm;
      const party = String((row && row.party) || '').toLowerCase();
      if (!firm) continue;
      if (/buyer|parent|acquir/.test(party)) buyerRaw.push(firm);
      else if (/seller|target|company/.test(party)) sellerRaw.push(firm);
    }
    const buyerFirms = dedupeFirms(buyerRaw);
    const sellerFirms = dedupeFirms(sellerRaw);
    return { ...empty, buyerFirm: buyerFirms[0] || null, buyerFirms, sellerFirm: sellerFirms[0] || null, sellerFirms };
  }
