...
const FINAL_REVIEW_PACKET_PATH = 'final-review-v5/sealed-final-pilot-review-packet.json';
const FINAL_LEGAL_FINDING_PATHS = Object.freeze([
  'final-review-v5/sealed-corrected-independent-legal-re-review-findings.json',
]);
const FINAL_STRICT_INDEPENDENT_REVIEW_INPUT_PATH = 'final-review-v5/sealed-strict-independent-legal-review-input-source-bound-v5.json';
...
function m3Rows(workItems, legalFindingsByWorkItem) {
  ...
  for (const work of workItems) {
    ...
    for (const entry of resolution.conditional_termination_fee_values || []) {
      rows.push({
        row_id: entry.conditional_termination_fee_value_id,
    ...
    for (const entry of resolution.structured_per_share_cash_values || []) {
      rows.push({
        row_id: entry.structured_per_share_cash_value_id,
        ...
        source_quote: entry.raw_formula,
        source_citation: entry.defined_term_lineage.join(' → '),
        legal_review_state: legalReviewState,
        legal_review_reason: legalFinding?.reason || null,
        resolver_state: 'STRUCTURED_INACTIVE_CANDIDATE',
        resolver_reasons: [],
...
function finalLegalFindings(paths, { finalReviewPacketId, strictIndependentReviewInputId }) {
  ...
  for (const artifact of artifacts) {
    ...
    validateArtifactContentId(artifact.content, 'independent_legal_review_findings_id', 'final legal findings');
    if (artifact.content.final_review_packet_id !== finalReviewPacketId
      || artifact.content.strict_independent_review_input_id !== strictIndependentReviewInputId) {
      throw new Error(`four-deal local preview final legal findings are not bound to the sealed review input: ${artifact.path}`);
    }
...
function getFrozenFourDealLocalDemoResult({
  ...
  const legal = finalLegalFindings(finalLegalFindingPaths, {
    finalReviewPacketId: artifact.content.final_review_packet_id,
    strictIndependentReviewInputId: strictReviewInput?.content.strict_independent_review_input_id,
  });
