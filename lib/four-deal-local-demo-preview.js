...
const { validateBoundM3FinalReviewArtifacts } = require('./canonical-v2/m3-final-sol-adversarial-audit');
...
const FINAL_REVIEW_PACKET_PATH = 'final-review-v8/sealed-final-pilot-review-packet-v8.json';
const FINAL_LEGAL_FINDING_PATHS = Object.freeze([
  'final-review-v8/sealed-corrected-independent-legal-re-review-findings-v2.json',
]);
const FINAL_STRICT_INDEPENDENT_REVIEW_INPUT_PATH = 'final-review-v8/sealed-strict-independent-legal-review-input-source-bound-v8.json';
...
function rowReviewStatus(legalFinding, rowId) {
  return legalFinding?.row_review_status_by_id?.[rowId] || null;
}

function legalReviewFields(legalFinding, rowId) {
  return {
    legal_review_state: rowReviewStatus(legalFinding, rowId),
    legal_review_reason: null,
    work_item_legal_review_state: legalFinding?.status || 'PENDING_INDEPENDENT_REVIEW',
    work_item_legal_review_reason: legalFinding?.reason || null,
  };
}
...
function m3Rows(workItems, legalFindingsByWorkItem) {
  ...
  for (const work of workItems) {
    ...
    for (const entry of resolution.resolved || []) {
      ...
      const rowId = entry.claim?.claim_revision_id || `${work.work_item_id}:resolved:${rows.length}`;
      rows.push({
        row_id: rowId,
        ...
        ...legalReviewFields(legalFinding, rowId),
    ...
    for (const entry of (resolution.review_queue || []).filter((item) => !item.has_resolution)) {
      const rowId = entry.original_claim_occurrence_id || `${work.work_item_id}:review:${rows.length}`;
      rows.push({
        row_id: rowId,
        family: entry.concept_key || 'M3',
        work_item_id: work.work_item_id,
    ...
    for (const entry of resolution.open_world || []) {
      const rowId = entry.closure_id || `${work.work_item_id}:open-world:${rows.length}`;
      rows.push({
        row_id: rowId,
        family: entry.concept_key || 'M3',
        work_item_id: work.work_item_id,
    ...
    for (const entry of resolution.conditional_termination_fee_values || []) {
      const rowId = entry.conditional_termination_fee_value_id;
      rows.push({
        row_id: rowId,
        family: 'TERMINATION_FEE',
        work_item_id: work.work_item_id,
        source_kind: work.source_kind,
        result_type: 'STRUCTURED_FORMULA',
        governed_field: 'CONDITIONAL_TERMINATION_FEE_VALUE',
        governed_value: displayValue({
          fee_side: entry.fee_side,
          triggering_branch: entry.triggering_branch,
          operator: entry.operator,
          base_amount: entry.base_amount,
          currency: entry.currency,
          cap_term: entry.reit_limit_cap_term_ref,
        }),
        source_quote: entry.raw_formula,
        source_citation: (entry.source_citations || []).join(' + ') || 'Published citation unavailable',
        ...legalReviewFields(legalFinding, rowId),
        resolver_state: 'STRUCTURED_INACTIVE_CANDIDATE',
        resolver_reasons: [],
        warning: 'This formula is preserved as structured operands. It is not flattened to one fee amount.',
      });
    }
    for (const entry of resolution.structured_per_share_cash_values || []) {
      const rowId = entry.structured_per_share_cash_value_id;
      rows.push({
        row_id: rowId,
        ...
        source_kind: work.source_kind,
        result_type: 'STRUCTURED_FORMULA',
        governed_field: 'STRUCTURED_PER_SHARE_CASH_VALUE',
        governed_value: displayValue({
          operator: entry.operator,
          base_amount: entry.base_amount,
          currency: entry.currency,
          variable_component: entry.variable_component,
        }),
        source_quote: entry.raw_formula,
        source_citation: (entry.source_citations || []).join(' + ') || 'Published citation unavailable',
        ...legalReviewFields(legalFinding, rowId),
        resolver_state: 'STRUCTURED_INACTIVE_CANDIDATE',
        resolver_reasons: [],
        warning: 'This consideration is preserved as a base amount plus a variable component. It is not flattened to cash alone.',
      });
    }
  }
  return rows;
...
function validateArtifactContentId(content, idKey, label) {
  if (!content || typeof content.schema_version !== 'string' || typeof content[idKey] !== 'string') {
    throw new Error(`four-deal local preview ${label} lacks a sealed content identity`);
  }
  const body = { ...content };
  delete body[idKey];
  if (contentId(content.schema_version, body) !== content[idKey]) {
    throw new Error(`four-deal local preview ${label} does not match its sealed content identity`);
  }
}

function finalLegalFindings(paths, { finalReviewPacket, strictIndependentReviewInput }) {
  ...
  validateBoundM3FinalReviewArtifacts({
    ...
    strict_review_input: strictIndependentReviewInput,
    final_finding_artifacts: artifacts,
  });
  for (const artifact of artifacts) {
    ...
    for (const finding of artifact.content.findings) {
      ...
      const rowReviewStatusById = {};
      for (const [collection, idKeys] of [
        [finding.claim_reviews, ['claim_revision_id']],
        [finding.review_queue_reviews, ['row_id']],
        [finding.open_world_reviews, ['row_id']],
        [finding.conditional_termination_fee_value_reviews, ['conditional_termination_fee_value_id']],
        [finding.structured_per_share_cash_value_reviews, ['structured_per_share_cash_value_id']],
      ]) {
        for (const review of collection || []) {
          const rowId = idKeys.map((key) => review?.[key]).find(Boolean);
          if (rowId && ['PASS', 'FAIL'].includes(review.status)) rowReviewStatusById[rowId] = review.status;
        }
      }
      findings.set(finding.work_item_id, Object.freeze({
        status: finding.status,
        reason: finding.reason
          || artifact.content.finding_notes?.[finding.work_item_id]
          || (finding.reason_codes || []).join(', '),
        row_review_status_by_id: Object.freeze(rowReviewStatusById),
      }));
    }
  }
...
function metseraDealResult(deal) {
  ...
  return {
    ...
    rows: [{
      ...
      legal_review_reason: null,
      work_item_legal_review_state: null,
      work_item_legal_review_reason: null,
...
function getFrozenFourDealLocalDemoResult({
  artifact_root: suppliedArtifactRoot,
  final_review_packet_path: finalReviewPacketPath = process.env.CANONICAL_V2_M3_FINAL_REVIEW_PACKET_PATH
    || FINAL_REVIEW_PACKET_PATH,
  final_strict_independent_review_input_path: finalStrictIndependentReviewInputPath
    = process.env.CANONICAL_V2_M3_STRICT_INDEPENDENT_REVIEW_INPUT_PATH || FINAL_STRICT_INDEPENDENT_REVIEW_INPUT_PATH,
  final_legal_finding_paths: suppliedFinalLegalFindingPaths,
  } = {}) {
  const artifactRoot = resolveDurableArtifactRoot({
    root: suppliedArtifactRoot === undefined
      ? process.env.CANONICAL_V2_M3_PREVIEW_ARTIFACT_ROOT
      : suppliedArtifactRoot,
    environmentKey: 'CANONICAL_V2_M3_PREVIEW_ARTIFACT_ROOT',
    certificationMode: true,
    requireExisting: true,
  });
  const finalLegalFindingPaths = suppliedFinalLegalFindingPaths === undefined
    ? (process.env.CANONICAL_V2_M3_FINAL_LEGAL_FINDINGS_PATHS
      ? process.env.CANONICAL_V2_M3_FINAL_LEGAL_FINDINGS_PATHS.split(path.delimiter).filter(Boolean)
      : FINAL_LEGAL_FINDING_PATHS.map((relativePath) => path.join(artifactRoot, relativePath)))
    : suppliedFinalLegalFindingPaths;
  const demo = getFourDealLocalDemo();
  const artifact = readJsonArtifact(path.join(artifactRoot, finalReviewPacketPath), 'final review packet');
  ...
  validateArtifactContentId(artifact.content, 'final_review_packet_id', 'final review packet');
  const strictReviewInput = finalLegalFindingPaths.length === 0
    ? null
    : readJsonArtifact(path.join(artifactRoot, finalStrictIndependentReviewInputPath), 'strict independent review input');
  if (strictReviewInput) {
    validateArtifactContentId(strictReviewInput.content, 'strict_independent_review_input_id', 'strict independent review input');
    if (strictReviewInput.content.final_review_packet_id !== artifact.content.final_review_packet_id) {
      throw new Error('four-deal local preview strict independent review input is not bound to the sealed final review packet');
    }
  }
  const legal = finalLegalFindingPaths.length === 0
    ? Object.freeze({ findings: new Map(), artifacts: [] })
    : finalLegalFindings(finalLegalFindingPaths, {
      finalReviewPacket: artifact,
      strictIndependentReviewInput: strictReviewInput,
    });
  ...
  const body = {
    ...
    m3_artifact: {
      root: artifactRoot,
      relative_path: finalReviewPacketPath,
      sha256: artifact.sha256,
      final_review_packet_id: artifact.content.final_review_packet_id,
      strict_independent_review_input: strictReviewInput ? {
        relative_path: finalStrictIndependentReviewInputPath,
        sha256: strictReviewInput.sha256,
        strict_independent_review_input_id: strictReviewInput.content.strict_independent_review_input_id,
      } : null,
      final_legal_findings: legal.artifacts.map((entry) => ({
        path: entry.path,
        sha256: entry.sha256,
        independent_legal_review_findings_id: entry.content.independent_legal_review_findings_id,
      })),
    },
