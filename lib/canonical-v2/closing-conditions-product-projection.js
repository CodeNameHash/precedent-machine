...
const TITLES = Object.freeze({
  'COND-M-REG': 'Antitrust / Regulatory Clearance',
  'COND-B-COV': 'Target Covenant Compliance',
  'COND-S-COV': 'Buyer Covenant Compliance',
  'COND-B-MAE': 'No Target Material Adverse Effect',
...
const SUPPORTED_DEFINITIONS = new Set([
  'REGULATORY_APPROVAL_CONDITION', 'COVENANT_COMPLIANCE_STANDARD',
...
function resolvedParty(entry) {
  ...
  if (entryParty && provisionParty && canonicalJson(entryParty) !== canonicalJson(provisionParty)) {
    throw new TypeError('resolved Closing Conditions party conflicts with its provision party');
  }
  ...
  if (!party
    ...
    throw new TypeError('resolved Closing Conditions party is ambiguous');
  }
  return party;
}
...
function projectedFeatures(entry) {
  ...
  const features = {};
  if (definition === 'REGULATORY_APPROVAL_CONDITION') {
    const approvalKind = attrs.approval_kind;
    addFeature(features, 'antitrustApprovals', [{
      code: approvalKind,
      label: approvalKind === 'HSR' ? 'HSR waiting period expired or terminated' : 'Scheduled regulatory approvals',
      text: quote,
    }]);
    if (approvalKind === 'HSR') addFeature(features, 'hsrClearance', true);
  } else if (definition === 'COVENANT_COMPLIANCE_STANDARD') {
    addFeature(features, 'covenantComplianceStandard', claim.canonical_value);
  } else if (definition === 'NO_MAE_CONDITION_CONTINUING') {
    addFeature(features, 'continuingRequirement', true);
  } else if (definition === 'STOCKHOLDER_APPROVAL_CONDITION') {
...
function reviewConcept(entry, party) {
  if (entry?.concept_key === 'COND-REG') return 'COND-M-REG';
  if (entry?.concept_key === 'COND-COV' && party.capacity === 'TARGET') return 'COND-B-COV';
  if (entry?.concept_key === 'COND-COV' && party.capacity === 'BUYER') return 'COND-S-COV';
  if (entry?.concept_key === 'COND-MAE' && party.capacity === 'TARGET') return 'COND-B-MAE';
  return entry?.concept_key;
}
...
function projectClosingConditionProductSurfaces({ resolution, deal_id: dealId, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId, publication_evaluation_time: publicationEvaluationTime } = {}) {
  ...
  for (const entry of filterResolvedEntriesForPublication(resolution.resolved, publicationFilter, releaseReceiptId, publicationEvaluationTime)) {
    ...
    const conceptKey = reviewConcept(entry, party);
    ...
    if (canonicalJson(group.party) !== canonicalJson(party)) {
      throw new TypeError('resolved Closing Conditions claims for one provision have conflicting parties');
    }
