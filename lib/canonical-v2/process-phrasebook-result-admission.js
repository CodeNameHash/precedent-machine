...
function validatePredicateWitnessRevision(
  ...
  if (successorPredicateKeys.has(value.predicate_key)) {
    fail(
      code,
      'The predicate witness needs the governed version-2 witness contract before it can retain successor machine-rule semantics.',
    );
  }
  if (
    value.predicate_key
    ...
    fail(
      code,
      'The predicate witness needs the governed version-2 witness contract before it can retain a response union.',
    );
  }
  const subjectCodes =
...
function validateCandidateReleaseMembership(
  ...
  if (
    ...
    || value.membership_state !== 'CANDIDATE_MEMBER'
    || value.validation_state !== 'EXTERNALLY_VALIDATED'
    || value.authority_state !== 'NOT_GRANTED'
  ) {
...
function buildAdmissionReceipt(input) {
  ...
  const body = {
    ...
    release_membership_state: 'BOUND_CANDIDATE_MEMBER',
    exact_detail_reference_state: 'BOUND_AUTH_REQUIRED',
    admission_state: 'VALIDATED_NOT_MATERIALISED',
