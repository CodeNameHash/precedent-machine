...
function validateActiveReviewContextResult(result, request) {
  ...
  if (isRejectedServingContractFingerprint(result.contract_fingerprint)) {
    throw new ReviewContextError('INVALID_RESPONSE', 'The rejected F3 contract cannot be served.');
  }
  const seen = new Set();
  ...
  for (const [index, row] of result.rows.entries()) {
    ...
    if (row.provenance
      && row.provenance.contract_fingerprint !== result.contract_fingerprint) {
      safeRows[index] = Object.freeze({
        row_serving_key: key,
        corpus_release_id: row.corpus_release_id,
        governed_deal_key: row.governed_deal_key,
        row_kind: 'CONTRACT_FINGERPRINT_MISMATCH',
      });
    }
    seen.add(key);
    previous = key;
  ...
  return safeRows.some((row, index) => row !== result.rows[index])
    ? Object.freeze({ ...result, rows: Object.freeze(safeRows) })
    : result;
}
...
function buildActiveReviewContextResult({ pointer, directoryRecord, rows, request }) {
  ...
  const matching = [...rows].filter((row) => (
    ...
    && row.provenance?.contract_fingerprint === directoryRecord.contract_fingerprint
  ...
  return validateActiveReviewContextResult({
    ...
    contract_fingerprint: directoryRecord.contract_fingerprint,
