...
const FEE_TRIGGER_LABELS = Object.freeze({
  ...
  CHANGE_IN_RECOMMENDATION_TERMINATION: 'Parent terminates following a Change in Recommendation',
  ACQUISITION_PROPOSAL_TAIL: 'Specified termination followed by a qualifying acquisition transaction within 12 months',
  // PROPOSAL-TERMF-TRIGGER-VOCABULARY-2026-07-23 / SPEC-QXO-TERMF-AMENDMENT-2026-07-23
  // (Step 1): the seven Ben-approved trigger_code additions. Labels are plain legal
  // English, matching the existing three above.
  INTERVENING_EVENT_RECOMMENDATION_CHANGE_TERMINATION: 'Board changed its recommendation for an intervening event',
  NO_SOLICIT_BREACH_TERMINATION: 'Termination for material breach of the no-solicitation covenant',
  STOCKHOLDER_APPROVAL_FAILURE_TERMINATION: 'Stockholder vote taken and approval not obtained',
  COUNTERPARTY_COVENANT_BREACH_TERMINATION: 'Termination for breach of another covenant or agreement',
  OUTSIDE_DATE_TERMINATION: 'Termination because the outside date passed',
  ANTITRUST_FAILURE_TERMINATION: 'Termination for failure to obtain required antitrust/regulatory clearance',
  FINANCING_FAILURE_TERMINATION: 'Termination for failure of financing',
});

const FEE_TIMING_LABELS = Object.freeze({
  CONCURRENT_WITH_TERMINATION: 'payable concurrently with termination',
  TWO_BUSINESS_DAYS_AFTER_TERMINATION: 'payable within 2 business days after termination',
  TWO_BUSINESS_DAYS_AFTER_EARLIER_SIGNING_OR_CONSUMMATION: 'payable within 2 business days after the earlier of signing or consummation',
  // SPEC-QXO-TERMF-AMENDMENT-2026-07-23 (Step 1).
  UPON_EARLIER_OF_SIGNING_OR_CONSUMMATION: 'payable upon the earlier of entering into a definitive agreement or consummation',
});
...
function adaptSharedServingRow(row) {
  ...
  if (market.metric_key === 'SELLER_TERMINATION_FEE_PERCENT_OF_DEAL_VALUE') {
    legalTerms.push(
      ...
      { key: 'deal_value_basis', label: 'Deal-value basis', value: 'SEC-reported closing transaction value' },
      ...feeTriggerEffects.map((trigger, index) => ({
        key: `trigger_${index + 1}`,
        label: index === 0 ? 'Triggers' : '',
        value: `${FEE_TRIGGER_LABELS[trigger.trigger_code]}: ${FEE_TIMING_LABELS[trigger.payment_timing]}`,
      })),
    );
  }
