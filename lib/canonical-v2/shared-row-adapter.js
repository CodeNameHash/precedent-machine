...
function adaptSharedServingRow(row) {
  ...
  if (market.metric_key === 'SELLER_TERMINATION_FEE_PERCENT_OF_DEAL_VALUE') {
    legalTerms.push(
      { key: 'payer', label: 'Payer', value: 'Company (target)' },
      { key: 'payee', label: 'Payee', value: 'Parent (buyer)' },
  ...
  if (market.metric_key === 'MATERIAL_CONTRACT_CASH_FLOW_THRESHOLD_PERCENT_OF_DEAL_VALUE') {
    legalTerms.push(
      { key: 'criterion', label: 'Criterion', value: 'Payments by or to the Company' },
      { key: 'period', label: 'Measured over', value: 'FY2023 or any single fiscal year thereafter' },
      { key: 'threshold_rule', label: 'Threshold rule', value: 'Amount in excess of the stated threshold' },
      { key: 'deal_value_basis', label: 'Deal-value basis', value: 'SEC-reported closing transaction value' },
    );
  }
