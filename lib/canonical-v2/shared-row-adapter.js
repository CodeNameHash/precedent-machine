...
function formattedPercentLabel(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) throw new TypeError('percentage is not numeric');
  return `${parsed.toFixed(2).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1')}% of headline deal value`;
}

function moneyValueLabel(value, denominatorPrecision) {
  return qualifyDerivedValue(formattedPercentLabel(value), denominatorPrecision);
}
...
function adaptReviewedSourceSpecificRow(row) {
  ...
  const sourceSpecific = Object.freeze({
    ...
    cohortMembership: Object.freeze({
      status: 'excluded',
      exclusionReason: body.market_comparability,
    }),
...
function adaptIncompleteCanonicalResultRow(row) {
  ...
  const denominatorPrecision = resolveDenominatorPrecision(component, attributes.denominator_precision);
  const legalTerms = Object.freeze([
    Object.freeze({
      key: 'primary_value',
      label: presentation.metric_label,
      value: moneyValueLabel(component.canonical_value, denominatorPrecision),
    }),
    Object.freeze({
      key: 'raw_amount',
    ...
    Object.freeze({
      ...
      value: qualifyDealValueBasis('SEC-reported headline transaction value', denominatorPrecision),
    }),
  ]);
  ...
  const metricResult = Object.freeze({
    ...
    subject: Object.freeze({
      ...
      label: moneyValueLabel(component.canonical_value, denominatorPrecision),
...
function adaptSharedServingRow(row) {
  ...
  const claimAttributes = component.claim_attributes;
  const contextComponents = result.components.slice(1);
  const market = result.market_context;
  const governedBinding = governedReviewBinding(result, market, component);
  ...
  const denominatorPrecision = presentation.comparison_kind === 'money'
    ? resolveDenominatorPrecision(component, claimAttributes.denominator_precision)
    : null;
  const counts = market.cohort.counts;
  const effect = component.bounded_relationship_effects[0]?.effect || null;
  const feeTriggerEffects = component.bounded_relationship_effects
  ...
  if (presentation.comparison_kind === 'money') {
    legalTerms[0] = {
      ...legalTerms[0],
      value: moneyValueLabel(market.subject_observation.canonical_value, denominatorPrecision),
    };
  }
  ...
  const subjectLabel = presentation.comparison_kind === 'money'
    ? moneyValueLabel(market.subject_observation.canonical_value, denominatorPrecision)
    : valueLabel(
      presentation,
      market.subject_observation.canonical_value,
      market.subject_observation.basis_key,
  ...
  const metricResult = {
    ...
    subject: {
      status: 'present',
      cohortMembership: {
        status: market.subject_cohort_membership?.status === 'INCLUDED'
          ? 'included'
          : market.subject_cohort_membership?.status === 'EXCLUDED'
            ? 'excluded'
            : 'unavailable',
        exclusionReason: market.subject_cohort_membership?.exclusion_reason || null,
      },
