...
function employeeFeatures(entry) {
  ...
  if (definition === 'EMPLOYEE_COMP_ITEM_STANDARD') {
    ...
    else if (attrs.relief_kind === 'EXPENSE_CREDIT') addFeature(features, 'welfarePlanExpenseCredit', true);
    else throw new TypeError(`unsupported Employee relief_kind: ${attrs.relief_kind || 'missing'}`);
...
function dnoFeatures(entry) {
  ...
  else if (definition === 'INDEMNIFICATION_SURVIVAL_YEARS') {
    addFeature(features, 'indemnificationPeriod', fixedDuration(
      Number(entry.claim.canonical_value),
      'years',
      'effective_time',
    ));
  }
  else if (definition === 'TAIL_POLICY_OBLIGATION') addFeature(features, 'doInsurance', true);
  else if (definition === 'TAIL_POLICY_PERIOD_YEARS') {
    addFeature(features, 'insurancePeriod', fixedDuration(
      Number(entry.claim.canonical_value),
      'years',
      'effective_time',
    ));
...
function projectEmployeeDnoProductSurfaces({ resolution, deal_id: dealId, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId, publication_evaluation_time: publicationEvaluationTime } = {}) {
  ...
  for (const entry of filterResolvedEntriesForPublication(resolution.resolved, publicationFilter, releaseReceiptId, publicationEvaluationTime)) {
    ...
    if (!groups.has(provisionId)) groups.set(provisionId, {
      ...
      features: {}, featureClaimRevisionIds: {}, quotes: [], claimRevisionIds: [],
    });
