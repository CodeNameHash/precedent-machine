...
function projectEntry(entry) {
  ...
  if (key === 'MAE_DISPROPORTIONALITY_CARVEBACK'
    && (typeof claim.raw_value !== 'string'
      ...
      || attributes.applies_to_clause_labels.some((label) => !claim.raw_value.includes(label)))) {
    fail('INVALID_GOVERNED_ATTRIBUTES', 'Disproportionality relationships must retain their exact labels and comparison baseline in the claim quote.');
  }
...
function buildMaeDisproportionalityReviewCards(rollups) {
  return rollups.map((rollup) => {
    ...
    return Object.freeze({
      ...
      features: {
        ...
        maeDisproportionalityRollup: rollup.review,
      },
      ai_metadata: { features: { carveouts, maeCarveouts: carveouts, maeDisproportionalityRollup: rollup.review } },
...
function projectKeyTermsMaeClaims({ resolved_entries: resolvedEntries, open_world_entries: openWorldEntries = [] } = {}) {
  ...
  const uniqueResolvedEntries = dedupeResolvedAssertions(resolvedEntries);
  ...
  const records = uniqueResolvedEntries
    .filter((entry) => !quarantinedClaimRevisionIds.has(entry?.claim?.claim_revision_id))
    .map(projectEntry)
  ...
  const maeReviewCards = buildMaeDisproportionalityReviewCards(maeDisproportionalityRollups);
  const body = {
