...
const M3_FINAL_SOL_AUDIT_FAILURE_EVIDENCE_SCHEMA = 'M3_FINAL_SOL_ADVERSARIAL_AUDIT_FAILURE_EVIDENCE/V1';
const STRICT_REVIEW_INPUT_SCHEMA = 'M3_12_CALL_FINAL_PILOT_STRICT_INDEPENDENT_REVIEW_INPUT/V3';
const FINAL_FINDINGS_SCHEMA = 'M3_12_CALL_FINAL_PILOT_RE_REVIEW_FINDINGS/V2';
const PRODUCTION_PLAN_SCHEMA = 'CANONICAL_V2_M3_PRODUCTION_EXTRACTION_PLAN/V4';
...
function requireSealedArtifact(artifact, { schema, idKey, label, code }) {
  if (artifact.content?.schema_version !== schema || typeof artifact.content?.[idKey] !== 'string') {
    fail(code, `${label} does not use the required sealed contract.`);
  }
  const body = { ...artifact.content };
  const observedId = body[idKey];
  delete body[idKey];
  if (contentId(schema, body) !== observedId) fail(code, `${label} does not match its content identity.`);
}

function requireFinalReviewPacket(artifact) {
  requireSealedArtifact(artifact, {
    schema: 'M3_12_CALL_FINAL_PILOT_REVIEW_PACKET/V1',
    idKey: 'final_review_packet_id',
    label: 'final review packet',
    code: 'FINAL_REVIEW_PACKET_ID_MISMATCH',
  });
...
function requireStrictReviewInput(artifact, finalReviewPacket) {
  requireSealedArtifact(artifact, {
    schema: STRICT_REVIEW_INPUT_SCHEMA,
    idKey: 'strict_independent_review_input_id',
    label: 'strict independent review input',
    code: 'STRICT_REVIEW_INPUT_ID_MISMATCH',
  });
  const reviewItemIds = artifact.content.review_items?.map((item) => item?.work_item_id);
  const packetWorkItemIds = finalReviewPacket.content.work_items.map((item) => item.work_item_id);
  if (artifact.content.final_review_packet_id !== finalReviewPacket.content.final_review_packet_id
    || !Array.isArray(reviewItemIds) || reviewItemIds.length !== 12
    || new Set(reviewItemIds).size !== 12
    || !same([...reviewItemIds].sort(), [...packetWorkItemIds].sort())) {
    fail('STRICT_REVIEW_INPUT_BINDING_MISMATCH', 'The strict review input is not bound to the supplied final review packet.');
  }
}

function requireFinalFindingsBinding(artifact, finalReviewPacket, strictReviewInput, label) {
  if (artifact.content?.schema_version !== FINAL_FINDINGS_SCHEMA
    || artifact.content?.final_review_packet_id !== finalReviewPacket.content.final_review_packet_id
    || artifact.content?.strict_independent_review_input_id
      !== strictReviewInput.content.strict_independent_review_input_id) {
    fail('FINAL_FINDINGS_BINDING_MISMATCH', `${label} is not bound to the supplied final packet and strict review input.`);
  }
  requireSealedArtifact(artifact, {
    schema: FINAL_FINDINGS_SCHEMA,
    idKey: 'independent_legal_review_findings_id',
    label,
    code: 'FINAL_FINDINGS_ID_MISMATCH',
  });
}

function validateBoundM3FinalReviewArtifacts({
  final_review_packet: finalReviewPacket,
  strict_review_input: strictReviewInput,
  final_finding_artifacts: finalFindingArtifacts,
}) {
  requireFinalReviewPacket(finalReviewPacket);
  requireStrictReviewInput(strictReviewInput, finalReviewPacket);
  if (!Array.isArray(finalFindingArtifacts) || ![1, 2].includes(finalFindingArtifacts.length)) {
    fail('INPUT_COVERAGE_MISSING', 'Provide one twelve-item or two six-item final independent legal finding files.');
  }
  finalFindingArtifacts.forEach((artifact, index) => requireFindings(
    artifact,
    finalFindingArtifacts.length === 1 ? 12 : 6,
    `final independent legal findings ${index + 1}`,
  ));
  finalFindingArtifacts.forEach((artifact, index) => requireFinalFindingsBinding(
    artifact,
    finalReviewPacket,
    strictReviewInput,
    `final independent legal findings ${index + 1}`,
  ));
  const finalFindingWorkItemIds = finalFindingArtifacts.flatMap(
    (artifact) => artifact.content.findings.map((finding) => finding?.work_item_id),
  );
  const finalPacketWorkItemIds = finalReviewPacket.content.work_items.map((item) => item.work_item_id);
  if (new Set(finalFindingWorkItemIds).size !== 12
    || !same([...finalFindingWorkItemIds].sort(), [...finalPacketWorkItemIds].sort())) {
    fail('INPUT_COVERAGE_MISSING', 'Final independent legal findings must cover each final review work item exactly once.');
  }
  return true;
}

function projectV4ProductionPlan(artifact, finalReviewPacket, strictReviewInput) {
  const plan = artifact.content;
  const bound = plan?.bound_pilot_evidence;
  const packet = bound?.packet;
  const strict = bound?.strict_review_input;
  const findings = bound?.final_findings;
  const qualityFacts = bound?.quality_facts;
  const qualityFactKeys = [
    'resolved_claim_count', 'unresolved_review_count', 'open_world_count',
    'missing_resolved_citation_count', 'missing_party_evidence_count',
    'conditional_fee_formula_count', 'structured_cash_formula_count', 'formula_review_row_count',
  ];
  const sealedBody = plan && { ...plan };
  if (sealedBody) delete sealedBody.content_hash;
  if (plan?.schema_version !== PRODUCTION_PLAN_SCHEMA
    || plan?.status !== 'PROPOSED_NOT_AUTHORISED'
    || typeof plan?.content_hash !== 'string'
    || contentId(PRODUCTION_PLAN_SCHEMA, sealedBody) !== plan.content_hash
    || packet?.contract !== 'M3_12_CALL_FINAL_PILOT_REVIEW_PACKET/V1'
    || packet?.final_review_packet_id !== finalReviewPacket.content.final_review_packet_id
    || strict?.contract !== STRICT_REVIEW_INPUT_SCHEMA
    || strict?.strict_independent_review_input_id
      !== strictReviewInput.content.strict_independent_review_input_id
    || findings?.contract !== FINAL_FINDINGS_SCHEMA
    || typeof findings?.id !== 'string' || !findings.id.trim()
    || findings?.binding_state !== 'SEALED_FINAL_FINDINGS_BOUND'
    || !qualityFacts || typeof qualityFacts !== 'object' || Array.isArray(qualityFacts)
    || qualityFactKeys.some((key) => !Number.isInteger(qualityFacts[key]) || qualityFacts[key] < 0)
    || !Array.isArray(plan?.proposed_execution) || plan.proposed_execution.length === 0
    || !Array.isArray(plan?.release_gates) || plan.release_gates.length === 0) {
    fail('PRODUCTION_PLAN_SHAPE_UNSUPPORTED', 'Production extraction plan must be a sealed, source-bound V4 proposal.');
  }
  return {
    ...
    bound_pilot_evidence: {
      packet,
      strict_review_input: strict,
      final_findings: findings,
      quality_facts: qualityFacts,
    },
    proposed_execution: plan.proposed_execution,
    release_gates: plan.release_gates,
  };
}
...
function resolutionForFinalWorkItem(workItem) {
  const source = ['REPAIRED_REPLAY', 'REPLAY_ONLY_CURRENT_RESOLVER_REPLAY',
    'PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY'].includes(workItem.source_kind)
    ? workItem.repaired_replay
    : workItem.source_kind === 'ADJUDICATED_FIRST_PASS_REPLAY'
      ? workItem.repaired_replay
...
function sourceCitation(entry) {
  if (entry.source_citation) return entry.source_citation;
  if (entry.section_reference) return `Published citation pending; governed scope ${entry.section_reference}`;
  return 'Published citation unavailable';
}
...
function projectFinalReviewPacket(packet) {
  return packet.work_items.map((workItem) => {
    ...
    return {
      ...
      adjudication_review_state: ['ADJUDICATED_FIRST_PASS', 'ADJUDICATED_FIRST_PASS_REPLAY'].includes(workItem.source_kind)
      ...
      conditional_termination_fee_values: resolution.conditional_termination_fee_values || [],
      structured_per_share_cash_values: resolution.structured_per_share_cash_values || [],
    };
  });
}
...
function buildSealedM3FinalSolAuditInput(options = {}) {
  ...
  const strictReviewInput = readJsonArtifact(options.strict_review_input_path, 'strict independent review input', options.read_file);
  ...
  if (![1, 2].includes(finalFindingArtifacts.length)) {
    fail('INPUT_COVERAGE_MISSING', 'Provide one twelve-item or two six-item final independent legal finding files.');
  }
  ...
  validateBoundM3FinalReviewArtifacts({
    final_review_packet: finalReviewPacket,
    strict_review_input: strictReviewInput,
    final_finding_artifacts: finalFindingArtifacts,
  });
  ...
  const productionPlanProjection = projectV4ProductionPlan(
    productionPlan,
    finalReviewPacket,
    strictReviewInput,
  );
  ...
  const auditProjection = {
    final_review_packet_id: finalReviewPacket.content.final_review_packet_id,
    final_review_packet: projectFinalReviewPacket(finalReviewPacket.content),
    strict_independent_review_input: {
      final_review_packet_id: strictReviewInput.content.final_review_packet_id,
      strict_independent_review_input_id: strictReviewInput.content.strict_independent_review_input_id,
      work_item_ids: strictReviewInput.content.review_items.map((item) => item.work_item_id),
    },
    final_findings_bindings: finalFindingArtifacts.map((artifact) => ({
      final_review_packet_id: artifact.content.final_review_packet_id,
      strict_independent_review_input_id: artifact.content.strict_independent_review_input_id,
      independent_legal_review_findings_id: artifact.content.independent_legal_review_findings_id,
      work_item_ids: artifact.content.findings.map((finding) => finding.work_item_id),
    })),
    ...
    production_extraction_plan: productionPlanProjection,
  };
  const baseBody = {
    ...
    audit_artifact_seals: {
      ...
      strict_independent_review_input: artifactSeal(strictReviewInput),
...
function validateSealedM3FinalSolAuditInput(value) {
  ...
  const projection = value.audit_projection;
  const strictBinding = projection?.strict_independent_review_input;
  const findingsBindings = projection?.final_findings_bindings;
  const packetWorkItemIds = projection?.final_review_packet?.map((item) => item?.work_item_id);
  const findingsWorkItemIds = Array.isArray(findingsBindings)
    ? findingsBindings.flatMap((binding) => binding?.work_item_ids || [])
    : [];
  if (typeof projection?.final_review_packet_id !== 'string'
    || typeof strictBinding?.strict_independent_review_input_id !== 'string'
    || strictBinding.final_review_packet_id !== projection.final_review_packet_id
    || !Array.isArray(packetWorkItemIds) || packetWorkItemIds.length !== 12
    || !Array.isArray(strictBinding.work_item_ids)
    || !same([...strictBinding.work_item_ids].sort(), [...packetWorkItemIds].sort())
    || ![1, 2].includes(findingsBindings?.length)
    || findingsBindings.some((binding) => (
      binding?.final_review_packet_id !== projection.final_review_packet_id
      || binding?.strict_independent_review_input_id !== strictBinding.strict_independent_review_input_id
      || typeof binding?.independent_legal_review_findings_id !== 'string'
      || !Array.isArray(binding?.work_item_ids)
    ))
    || new Set(findingsWorkItemIds).size !== 12
    || !same([...findingsWorkItemIds].sort(), [...packetWorkItemIds].sort())
    || !value.audit_artifact_seals?.strict_independent_review_input) {
    fail('AUDIT_REVIEW_BINDING_MISMATCH', 'The sealed audit input does not bind one final packet, strict review input and final findings set.');
  }
  return true;
}
...
function auditPrompt(input) {
  const prompt = [
    ...
    `Allowed topic slugs (exactly): ${AUDIT_TOPICS.join(', ')}`,
    'Allowed severities (exactly): CRITICAL, HIGH, MEDIUM, LOW',
    'Every finding_id must be a unique, non-empty, trimmed string.',
    'Every finding must have at least one non-empty, trimmed evidence_refs string.',
...
function invalidOutputPath(outputPath) {
  return outputPath.endsWith('.json')
    ? `${outputPath.slice(0, -'.json'.length)}.invalid.json`
    : `${outputPath}.invalid.json`;
}

function writeInvalidProviderOutputEvidence({ input, outputPath, rawProviderOutput, error }) {
  const body = {
    schema_version: M3_FINAL_SOL_AUDIT_FAILURE_EVIDENCE_SCHEMA,
    failure_state: 'INVALID_PROVIDER_OUTPUT_NOT_A_VALID_AUDIT',
    audit_input_id: input.audit_input_id,
    audit_model_profile: SOL_HIGH_PROFILE,
    raw_provider_output: rawProviderOutput,
    raw_provider_output_sha256: sha256Hex(Buffer.from(rawProviderOutput, 'utf8')),
  ...
  const evidence = {
    ...body,
    failure_evidence_id: contentId(M3_FINAL_SOL_AUDIT_FAILURE_EVIDENCE_SCHEMA, body),
  };
  writeFileSync(invalidOutputPath(outputPath), `${canonicalJson(evidence)}\n`, { encoding: 'utf8', flag: 'wx' });
}
...
async function runSealedM3FinalSolAdversarialAudit({ input, output_path: outputPath, client_factory: clientFactory = createCodexCliClient }) {
  ...
  try {
    ...
    if (error instanceof M3FinalSolAdversarialAuditError) {
      writeInvalidProviderOutputEvidence({ input, outputPath, rawProviderOutput, error });
    }
    throw error;
  }
...
module.exports = {
  ...
  M3_FINAL_SOL_AUDIT_FAILURE_EVIDENCE_SCHEMA,
