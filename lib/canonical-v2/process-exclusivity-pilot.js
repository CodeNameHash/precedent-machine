...
const {
  ...
  RELATIONSHIP_TYPE_CODE: BIDDER_TRACK_EVENT_MEMBERSHIP_TYPE,
  buildProcessBidderTrackEventMembershipRevision,
} = require('./process-bidder-track-event-membership');
const {
...
function validateSourceDocument(value, index) {
  ...
  if (
    ...
    fail(code, `Source document ${index} identity is invalid.`);
  }
}

function verifiedSourceBytes(
  sourceBytesByIdentity,
  sourceDocumentIdentity,
  expectedDigest,
  code,
) {
  if (!(sourceBytesByIdentity instanceof Map)) {
    fail(code, 'Source bytes must be supplied as a Map keyed by source document identity.');
  }
  const bytes = sourceBytesByIdentity.get(sourceDocumentIdentity);
  if (!Buffer.isBuffer(bytes)) {
    fail(code, 'Source bytes are missing for the source document identity.');
  }
  if (sha256Hex(bytes) !== expectedDigest) {
    fail(code, 'Source bytes do not match source_text_digest.');
  }
  return bytes;
}

function sourceContext(sourceDocuments, sourceBytesByIdentity) {
  ...
  return { sourceDocuments, byIdentity, sourceBytesByIdentity };
}
...
function resolveInterval(reference, sources, label) {
  ...
  const sourceBytes = verifiedSourceBytes(
    sources.sourceBytesByIdentity,
...
function exactSourceSlicesFromEdges(edges, sources) {
  ...
  for (const edge of edges.flat()) {
    ...
    const bytes = verifiedSourceBytes(
      sources.sourceBytesByIdentity,
      source.source_document_identity,
      source.source_text_digest,
      'INVALID_PROCESS_PILOT_EVIDENCE',
...
function buildMaterialisation(input, sourceBytesByIdentity) {
  validatePredicateCatalogueContract();
  validateInputShape(input);
  const sources = sourceContext(input.source_documents, sourceBytesByIdentity);
  ...
  const frozenRelationshipEvidenceBySlot = new Map(
    input.frozen_scope.relationship_slots.map((slot, index) => [
      slot.process_relationship_slot_key,
      evidence(
        slot.relationship_evidence,
        `Relationship slot ${index} frozen evidence`,
      ),
    ]),
  );
  const typedRelationshipDescriptors = typed.relationships.map(
    (value, index) => {
      ...
      if (!identity) {
        fail(code, `Relationship typed value ${index} has no frozen identity.`);
      }
      ...
      if (
        canonicalJson(edges)
          !== canonicalJson(frozenRelationshipEvidenceBySlot.get(
            value.process_relationship_slot_key,
          ))
      ) {
        fail(
          'PROCESS_PILOT_RELATIONSHIP_EVIDENCE_SUBSTITUTION',
          `Relationship typed value ${index} changed its frozen exact evidence.`,
        );
      }
      return {
        index,
  ...
  const membershipPairKeys = new Set();
  const membershipRevisionBySlot = new Map();
  for (const descriptor of typedRelationshipDescriptors) {
    if (
      descriptor.value.relationship_type_code
        !== BIDDER_TRACK_EVENT_MEMBERSHIP_TYPE
    ) {
      continue;
    }
    const source = descriptor.identity.source_process_object_kind_and_id;
    const target = descriptor.identity.target_process_object_kind_and_id;
    const pairKey = `${source.stable_object_id}:${target.stable_object_id}`;
    if (membershipPairKeys.has(pairKey)) {
      fail(
        'DUPLICATE_PROCESS_BIDDER_TRACK_EVENT_MEMBERSHIP',
        'The pilot contains more than one membership for a stable BidderTrack and ProcessEvent pair.',
      );
    }
    membershipPairKeys.add(pairKey);
    if (
      descriptor.evidence_edges.some(
      ...
      fail(
        ...
        'Membership evidence must use the exact governed membership role.',
      );
    }
    const revision = buildProcessBidderTrackEventMembershipRevision({
      schema_version: MEMBERSHIP_INPUT_SCHEMA,
      process_relationship_identity: descriptor.identity,
      bidder_track_id: source.stable_object_id,
      process_event_id: target.stable_object_id,
      relationship_state: descriptor.value.relationship_state,
      relationship_effect: descriptor.value.relationship_effect,
      evidence_edges: descriptor.evidence_edges,
      temporal_expression_revision_id:
        descriptor.value.temporal_slot_key,
    });
    membershipRevisionBySlot.set(
      descriptor.value.process_relationship_slot_key,
      revision,
    );
  }
  if (membershipRevisionBySlot.size < 1) {
    fail(
      'MISSING_PROCESS_BIDDER_TRACK_EVENT_MEMBERSHIP',
      'The exclusivity pilot requires an exact BidderTrack-to-ProcessEvent membership.',
    );
  }
  const membershipRevisionIdsForEndpoint = (
    endpointKey,
    stableObjectId,
  ) => typedRelationshipDescriptors
    .filter((descriptor) => (
      membershipRevisionBySlot.has(
        descriptor.value.process_relationship_slot_key,
      )
      && descriptor.identity[endpointKey].stable_object_id
        === stableObjectId
    ))
    .map((descriptor) => membershipRevisionBySlot.get(
      descriptor.value.process_relationship_slot_key,
  ...
  const bidderTrackRevisions = typed.bidder_tracks.map((value, index) => {
    ...
    const membershipRevisionIds = membershipRevisionIdsForEndpoint(
      'source_process_object_kind_and_id',
      identity.bidder_track_id,
    );
    if (membershipRevisionIds.length < 1) {
      fail(
        'PROCESS_PILOT_BIDDER_TRACK_MEMBERSHIP_UNRESOLVED',
        `Bidder-track typed value ${index} has no exact event membership.`,
      );
    }
  ...
  const eventRevisions = typed.events.map((value, index) => {
    ...
    return buildRevision('event', {
      ...
      process_relationship_revision_ids: membershipRevisionIds,
      temporal_expression_revision_id:
  ...
  const passageRevisions = typed.passages.map((value, index) => {
    ...
    const exactBytes = edges.map((edge) => verifiedSourceBytes(
      sources.sourceBytesByIdentity,
      source.source_document_identity,
      source.source_text_digest,
      code,
  ...
  const relationshipRevisions = typedRelationshipDescriptors.map(
    (descriptor) => {
      const {
        index,
        value,
        identity,
        evidence_edges: evidenceEdgesValue,
      } = descriptor;
      const membershipRevision = membershipRevisionBySlot.get(
        value.process_relationship_slot_key,
      );
      if (membershipRevision) return membershipRevision;
      const code = 'INVALID_PROCESS_PILOT_TYPED_VALUE';
      const temporalRevision = value.temporal_slot_key === null
        ? null
        : temporalRevisionBySlot.get(value.temporal_slot_key);
      ...
      return buildRevision('relationship', {
        ...
        source_process_object_kind_and_id:
          identity.source_process_object_kind_and_id,
        target_process_object_kind_and_id:
          identity.target_process_object_kind_and_id,
        temporal_expression_revision_id:
          temporalRevision?.temporal_expression_revision_id || null,
      });
    },
...
function compileProcessExclusivityPilotMaterialisation(
  ...
  return deepFreeze(buildMaterialisation(input, sourceBytesByIdentity));
}
...
function validateProcessExclusivityPilotMaterialisation(
  ...
  const rebuilt = buildMaterialisation(input, sourceBytesByIdentity);
  if (canonicalJson(value) !== canonicalJson(rebuilt)) {
