...
function buildMaterialisation(input) {
  ...
  const predicateWitnessRevisions = typed.predicate_witnesses.map(
    (value, index) => {
      ...
      if (value.predicate_state !== 'PRESENT') {
        fail(
          'PROCESS_PILOT_NON_PRESENT_PREDICATE_UNSUPPORTED',
          `Predicate-witness typed value ${index} needs the governed version-2 witness contract before it can retain a non-present state.`,
        );
      }
      if (successorPredicateKeys.has(value.predicate_key)) {
        fail(
          'PROCESS_PILOT_SUCCESSOR_PREDICATE_UNSUPPORTED',
          `Predicate-witness typed value ${index} needs the governed version-2 witness contract before it can retain the successor machine-rule semantics.`,
        );
      }
      if (
        value.predicate_key
        ...
        fail(
          'PROCESS_PILOT_RESPONSE_UNION_UNSUPPORTED',
          `Predicate-witness typed value ${index} needs the governed version-2 witness contract before it can retain a response union.`,
        );
      }
