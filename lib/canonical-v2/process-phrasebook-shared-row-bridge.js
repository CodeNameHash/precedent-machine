...
function validateContractBinding() {
  ...
  requireExactValue(
    definition.validation_carrier_contract
      .allowed_process_admission_state_bindings,
    [
      {
    ...
    'Process adapter pre-write state bindings',
    code,
  );
  requireExactValue(
  ...
  if (
    ...
    || definition.validation_carrier_contract.authority_state
      !== 'NOT_GRANTED'
    || definition.validation_carrier_contract
      .process_receipt_remains_authoritative_for_lineage_and_prewrite_state
      !== true
    || definition.validation_carrier_contract
      .process_receipt_candidate_membership_authority !== false
    || definition.validation_carrier_contract
      .product_result_admission_receipt_owns_candidate_membership !== true
...
function validateProcessInputAndReceipt(
  ...
  const releaseMembershipState =
    processAdmissionReceipt.release_membership_state;
  const processStateIsBoundCandidate = (
    processAdmissionReceipt.admission_state
      === 'VALIDATED_NOT_MATERIALISED'
    && releaseMembershipState === 'BOUND_CANDIDATE_MEMBER'
    && processAdmissionInput.candidate_release_membership
      .membership_state === 'CANDIDATE_MEMBER'
  );
  const processStateIsPrewrite = (
    processAdmissionReceipt.admission_state
      === 'VALIDATED_NOT_RELEASE_BOUND'
    && releaseMembershipState === 'NOT_RELEASE_BOUND'
    && processAdmissionInput.candidate_release_membership
  ...
  if (
    ...
    || (!processStateIsBoundCandidate && !processStateIsPrewrite)
...
function buildExactCitation(
  ...
  const sourceLabel = admittedSourceCitation === undefined
    ? detail.human_readable_source_label
    : formatAdmittedSourceLabel(admittedSourceCitation);
  return {
    ...
    source_filing_type: admittedSourceCitation?.form_type || null,
    source_filing_date: admittedSourceCitation?.filed_on || null,
...
function formatAdmittedSourceLabel(citation) {
...
function validateAdmittedSourceCitation(citation, preview, detail) {
  ...
  requireExactKeys(citation, [
    ...
    'event_date',
  ], 'Admitted source citation', 'INVALID_PROCESS_PHRASEBOOK_SHARED_ROW_SOURCE_CITATION');
  if (
    citation.source_document_identity !== preview.source_document_identity
    || citation.source_revision_id !== preview.source_revision_id
    || !/^[A-Z][A-Z0-9-]*$/.test(citation.form_type)
    || !/^\d{4}-\d{2}-\d{2}$/.test(citation.filed_on)
    || !/^\d{4}-\d{2}-\d{2}$/.test(citation.event_date)
    || citation.event_date === citation.filed_on
    || typeof citation.issuer_name !== 'string'
    || citation.issuer_name.length === 0
    || typeof citation.source_location_label !== 'string'
    || citation.source_location_label.length === 0
    || detail.human_readable_source_label
      !== formatAdmittedSourceLabel(citation)
  ) {
    fail(
      'INVALID_PROCESS_PHRASEBOOK_SHARED_ROW_SOURCE_CITATION',
      'The admitted source citation does not bind the exact Process source or derived label.',
    );
  }
  return citation;
}
...
function validateCompiledMappings(input, domainResult, productResult) {
  ...
  const sourceCitation = validateAdmittedSourceCitation(
    input.admitted_source_citation,
    preview,
    detail,
  );
  if (
    ...
    || productResult.exact_citation.human_readable_source_label
      !== detail.human_readable_source_label
    || productResult.exact_citation.source_filing_type
      !== (sourceCitation?.form_type || null)
    || productResult.exact_citation.source_filing_date
      !== (sourceCitation?.filed_on || null)
...
function buildProcessPhrasebookSharedRowBridge(input) {
  ...
  if (Object.hasOwn(input, 'admitted_source_citation')) {
    requiredInputKeys.push('admitted_source_citation');
  }
  requireExactKeys(
    input,
    requiredInputKeys,
    'Process phrasebook shared-row bridge input',
    code,
  );
  requireObject(
  ...
  const sourceCitation = validateAdmittedSourceCitation(
    ...
    input.process_admission_input.exact_detail_reference,
  );
