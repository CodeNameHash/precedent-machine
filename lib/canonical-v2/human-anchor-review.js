...
const MACHINE_PACKET_SCHEMA = 'CANONICAL_V2_HUMAN_ANCHOR_MACHINE_PACKET/V2';
const REVIEW_PACKET_SCHEMA = 'CANONICAL_V2_HUMAN_ANCHOR_REVIEW_PACKET/V2';
const KEY_SCHEMA = 'CANONICAL_V2_HUMAN_ANCHOR_KEY/V2';
const DECISION_LEDGER_SCHEMA = 'CANONICAL_V2_HUMAN_ANCHOR_DECISION_LEDGER/V2';
const DECISION_SCHEMA = 'CANONICAL_V2_HUMAN_ANCHOR_DECISION/V2';
...
function sourceRow({ run, manifest, receipt, adapter, resolved, index, governingContext }) {
  const claim = resolved.claim;
...
function sourceRows({ runs }) {
  ...
  for (const run of runs) {
    const governingContext = createCandidateGoverningContext({
      admittedSourceContext: run.adapter?.admitted_source_contexts?.[0],
      sectionsByReference: new Map((run.receipt?.resolved_sections || []).map((section) => [section.section_reference, section])),
    });
    for (const [index, resolved] of (run.resolution.resolved || []).entries()) {
      try {
        rows.push(sourceRow({ ...run, resolved, index, governingContext }));
      } catch (error) {
...
function buildHumanAnchorMachinePacket({ runs, cards_per_error_class = 16, baseline_seeds_per_hard_class = 4, seeds_per_hard_class = 12 } = {}) {
...
function validateHumanAnchorMachinePacket(packet) {
  ...
  for (const [index, card] of packet.cards.entries()) {
    exactKeys(card, ['card_number', 'claim_definition_key', 'deal', 'decision_key', 'error_class', 'evidence', 'family', 'governing_context', 'identity', 'proposed_analysis', 'question', 'run', 'section_reference', 'seed_extension', 'seed_type', 'seeded_wrong', 'source_id'], 'INVALID_MACHINE_PACKET');
...
function validateHumanAnchorReviewPacket(packet) {
  ...
  const keys = [];
  for (const [index, card] of packet.cards.entries()) {
    exactKeys(card, ['card_number', 'claim_definition_key', 'deal', 'decision_key', 'error_class', 'evidence', 'family', 'governing_context', 'identity', 'proposed_analysis', 'question', 'section_reference'], 'INVALID_REVIEW_PACKET');
    ...
    validateIdentity(card.identity, 'INVALID_REVIEW_PACKET'); validateEvidence(card.evidence, 'INVALID_REVIEW_PACKET'); validateGoverningContext(card.governing_context, 'INVALID_REVIEW_PACKET'); validateProposedAnalysis(card.proposed_analysis, 'INVALID_REVIEW_PACKET'); validateQuestion(card.question, card.error_class, 'INVALID_REVIEW_PACKET');
