...
const INITIAL_IMPORT_TRANSACTION_IDS = Object.freeze(Array.from(
  { length: INITIAL_IMPORT_COUNT },
  (_, index) => `TX-${String(index + 1).padStart(6, '0')}`,
));
...
function isAdoptedInitialImportSeed(seed) {
  return seed?.kind === 'BEN_APPROVED_IMPORT_IDENTITY'
    && seed.governed_source_namespace === INITIAL_IMPORT_NAMESPACE
    && seed.seed_schema_version === INITIAL_IMPORT_SEED_SCHEMA_VERSION
    && INITIAL_IMPORT_TRANSACTION_IDS.includes(seed.immutable_submitted_transaction_identifier);
}
...
function buildUnsignedDealIdentityApprovalSigningRequest({
  ...
  if (isAdoptedInitialImportSeed(seed)) {
    fail('IDENTITY_INITIAL_IMPORT_REQUIRES_BATCH_MAPPING_APPROVAL', 'The adopted 40-deal initial cohort requires the one batch mapping-signature path, not per-deal Ben approval.');
  }
...
function validateV2DealIdentityAuthorityEvidence(input = {}) {
  ...
  if (isAdoptedInitialImportSeed(manifest.immutable_deal_seed)) {
    fail('IDENTITY_INITIAL_IMPORT_REQUIRES_BATCH_MAPPING_APPROVAL', 'The adopted initial cohort requires the governed batch mapping, machine receipt, bridge and current-head chain.');
  }
...
function buildInitialImportProposalPacket({ initial_import_source_inventory_digest: sourceInventoryDigest } = {}) {
  ...
  const rows = INITIAL_IMPORT_TRANSACTION_IDS.map((identifier, index) => {
    return Object.freeze({
  ...
  const body = {
    ...
    status: 'ADOPTED_IDENTITY_SEED_POLICY_NO_ISSUANCE_OR_ADMISSION_AUTHORITY',
...
function validateInitialImportProposalPacket(packet) {
  ...
  if (!exactKeys(packet, keys) || packet.schema_version !== INITIAL_IMPORT_PACKET_SCHEMA
    || packet.status !== 'ADOPTED_IDENTITY_SEED_POLICY_NO_ISSUANCE_OR_ADMISSION_AUTHORITY' || packet.governed_source_namespace !== INITIAL_IMPORT_NAMESPACE
