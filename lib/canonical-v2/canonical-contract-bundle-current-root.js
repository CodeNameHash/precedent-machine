...
const MEMBER_KIND_BY_OBJECT_KIND = Object.freeze({
  ...
  PROCESS_PILOT_ADMISSION_ADAPTER_CONTRACT_INPUT: 'COMPOSITION_CATALOGUE',
  ...
  PRODUCT_CANDIDATE_WRITE_CONTRACT_INPUT: 'CORE_CANONICAL_CONTRACT',
...
function isContractReferencePath(valuePath) {
  return !/(^|\.)(required_)?value_slot_key$/u.test(valuePath);
}
...
function classifyCanonicalContractMemberReferences(member, membersByStableId) {
  ...
  for (const [valuePath, stableId] of collectStringLeaves(member.canonical_value)) {
    if (!isContractReferencePath(valuePath)) continue;
