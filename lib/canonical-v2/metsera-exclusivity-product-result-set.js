...
function validateInputs(admission, row) {
  if (
...
function buildResultSet(admission, row) {
  validateInputs(admission, row);
...
function compileMetseraExclusivityProductResultSet(admission, row) {
  return deepFreeze(buildResultSet(clone(admission), clone(row)));
}
...
function validateMetseraExclusivityProductResultSet(
  ...
  const rebuilt = buildResultSet(clone(admission), clone(row));
