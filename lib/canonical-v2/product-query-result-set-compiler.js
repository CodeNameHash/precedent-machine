...
function expectedProductResultIdentity(queryIr, domainResultIdentity) {
  const release = queryIr.release_contract;
  const resultDefinition = queryIr.semantic_contract.result_definition;
  return contentId(PRODUCT_QUERY_RESULT_SCHEMA, {
    ...
    domain_result_definition_stable_id:
      resultDefinition.stable_id,
    domain_result_definition_version:
      resultDefinition.version,
    domain_result_identity: domainResultIdentity,
  });
}

function validateAgreementOrder({
  queryIr,
  candidates,
  orderingProjection,
  orderingReceipt,
  ...
  if (
    ...
    fail(code, 'The Agreement ordering validator is not admitted.');
  }
  try {
    validateAgreementComparableResultOrderProjection(orderingProjection);
  } catch (error) {
    fail(code, 'The Agreement ordering projection is invalid.', {
      cause: error.code || error.message,
    });
  }
  const release = queryIr.release_contract;
  ...
  if (
    orderingProjection.product_query_definition_id
      !== queryIr.query_definition_id
    || orderingProjection.approved_pm_data_version_id
      !== release.approved_pm_data_version_id
    || orderingProjection.candidate_release_manifest_id
      !== release.candidate_release_manifest_id
    || orderingProjection.candidate_release_manifest_payload_digest
      !== release.candidate_release_manifest_payload_digest
    || orderingProjection.product_field_catalogue_id
      !== release.product_field_catalogue_manifest_id
    || orderingProjection.product_field_catalogue_payload_digest
      !== release.product_field_catalogue_payload_digest
    || canonicalJson(orderingProjection.ordered_query_sort_vector)
      !== canonicalJson(queryIr.presentation_contract.sort)
    || orderingProjection.query_diversity_definition_id
      !== diversity.definition_id
    || orderingProjection.query_diversity_payload_digest
      !== diversity.payload_digest
    || orderingProjection.page_size
      !== queryIr.pagination_contract.page_size
    || orderingReceipt.domain_ordering_projection_identity
      !== orderingProjection.ordering_projection_id
    ...
    fail(code, 'The Agreement ordering projection does not match the query.');
  }
  if (
    orderingProjection.candidate_count !== candidates.length
    || orderingProjection.ordered_agreement_result_identities.length
      !== candidates.length
  ) {
    fail(code, 'The Agreement ordering projection candidate count is stale.');
  }
  ...
  const factByDomainResult = new Map();
  for (const fact of orderingProjection.validated_ordering_facts) {
    if (
      factByDomainResult.has(
        fact.agreement_comparable_result_identity,
      )
    ) {
      fail(code, 'Two Agreement ordering facts bind the same result.');
    }
    factByDomainResult.set(
      fact.agreement_comparable_result_identity,
      fact,
    );
  }
  if (factByDomainResult.size !== byDomainResult.size) {
    fail(code, 'The Agreement ordering facts are not a complete set.');
  }

  const projected = orderingProjection
    .ordered_agreement_result_identities
    .map((identity) => {
      const candidate = byDomainResult.get(identity);
      const fact = factByDomainResult.get(identity);
      if (!candidate || !fact) {
        fail(code, 'The Agreement ordering projection substituted a candidate.');
      }
      const expectedProductIdentity = expectedProductResultIdentity(
        queryIr,
        identity,
      );
      if (
        fact.product_query_result_identity
          !== expectedProductIdentity
        || (
          candidate.slot.slot_state === 'VALID'
          && candidate.slot.slot_identity
            !== expectedProductIdentity
        )
      ) {
        fail(
          code,
          'The Agreement ordering fact does not bind the Product result.',
        );
      }
      return candidate.slot.slot_identity;
    });
  if (
    new Set(
      orderingProjection.ordered_agreement_result_identities,
    ).size
      !== orderingProjection
        .ordered_agreement_result_identities.length
    || projected.length !== byDomainResult.size
  ) {
    fail(code, 'The Agreement ordering projection is not a complete set.');
  }

  const expectedComplete = [...bySlot.keys()].sort(compareText);
  const emittedCount = orderingProjection
    .first_page_agreement_result_identities.length;
  const expectedOrdered = projected.slice(0, emittedCount);
  const expectedExcluded = projected.slice(emittedCount);
  if (
    canonicalJson(
      orderingProjection.first_page_agreement_result_identities,
    )
      !== canonicalJson(
        orderingProjection
          .ordered_agreement_result_identities
          .slice(0, emittedCount),
      )
    || canonicalJson(orderingReceipt.complete_candidate_slot_identities)
      !== canonicalJson(expectedComplete)
    || canonicalJson(orderingReceipt.ordered_slot_identities)
...
function compileProductQueryResultSet({
  ...
  if (!['AGREEMENT', 'PROCESS'].includes(domainKey)) {
    fail(
