...
//      `source-reference.json` (`admitted_source_capture_inputs`), so the
//      rebuild is exact within one environment. Across environments the
//      compressed-digest dependence still bites, and the durable fix is to
//      key identity on `source_map_digest` -- already computed, already
//      stable -- rather than on compressor output. That is a schema change
//      to a closed V2 contract and is recorded as an open decision in
//      PLAN.md Step 2B rather than made here.
//
