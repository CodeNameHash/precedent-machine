...
// converts the same document, so twenty-four runs share one 1.5 MB file
// rather than committing thirty-five megabytes of identical bytes.
//
// WHAT INFLATION EQUALITY DOES NOT PROVE, since it is easy to over-read.
// zlib stops at the end of a deflate stream and ignores trailing bytes, and
// the same map recompressed at a different level inflates identically. So
// inflation equality alone does not identify the payload -- which is why the
// recorded digest is REQUIRED rather than optional, and why the adopted
// digest still has to reproduce the run's committed
// `immutable_source_document_id` downstream. Three checks, not one.
...
} = require('./source-map-payload-store');
...
function adoptPersistedSourceMapPayload({ conversion, payloadBytes, expectedSha256 }) {
  ...
  // REQUIRED, not merely checked-if-present. Inflation equality alone will
  // accept a payload with trailing bytes after the deflate stream, because
  // zlib stops at the stream's end and ignores the rest; it will equally
  // accept the same map recompressed at another level. Both are harmless
  // here -- the adopted digest still has to match the run's committed
  // source_references, so a substitution only survives if the evidence is
  // edited too -- but "harmless because something downstream catches it" is
  // not a property to rely on in a function whose whole job is to decide
  // whether these bytes may be trusted. The run records the digest; require it.
  if (!expectedSha256) {
    throw new AdmittedSourceChainError(
      'PERSISTED_SOURCE_MAP_UNVERIFIABLE',
      'a persisted source map was supplied with no recorded digest to check it against. The run '
      + 'records source_map_compressed_sha256 precisely so these bytes can be verified rather than '
      + 'assumed.',
    );
  }
...
function rebuildAdmittedSourcePrimitives({ runDirectory, repoRoot = path.join(__dirname, '..', '..') }) {
  ...
  const locallyValidatedConversion = convertSecHtmlToCanonicalText(capture);
  let conversion = locallyValidatedConversion;
  ...
  if (inputs.source_map_payload_path) {
    assertRecordedSourceMapProvenance(
      sourceReference.admitted_source_capture_inputs,
      locallyValidatedConversion.canonical_text_id,
    );
    const payloadPath = resolveSourceMapPayloadPath({
      repoRoot,
      canonicalTextId: locallyValidatedConversion.canonical_text_id,
    });
  ...
  const verification = verifySecHtmlCanonicalText({ capture, conversion: locallyValidatedConversion });
  ...
  const bundle = inputs.source_map_payload_path
    ? buildVerifiedSecSourceAdmissionForRecordedSourceMapPayload({
      capture,
      locally_validated_conversion: locallyValidatedConversion,
      adopted_conversion: conversion,
      verification,
      recorded_source_map_provenance: sourceReference.admitted_source_capture_inputs,
    })
