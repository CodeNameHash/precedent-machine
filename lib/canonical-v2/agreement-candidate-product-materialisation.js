...
function buildExactCitation(domainResult, query, source) {
  ...
  if (source.release) {
    ...
    sourceDocumentIdentity = source.ioc.agreementSource.document_hash;
    sourceEvidenceIdentity = source.ioc.excerpts.capex.excerpt_id;
  ...
  return {
    ...
    result_component_evidence_identity: sourceEvidenceIdentity,
...
function compileAgreementCandidateProductMaterialisation(input = {}) {
  ...
  const citation = buildExactCitation(domainResult, query, source);
  ...
  const body = {
    ...
    evidence_sidecar: {
      ...
      exact_detail_package: source.ioc ? buildIocDetailPackage(
        source.ioc,
