...
const DEFINITION_DIGEST =
  '20faeff3f9b1c9f93b003bd95149a35ea22cd3f3b3815035981d08b3838eeb9c';
...
const ADMITTED_TOPICS = Object.freeze([
  Object.freeze({
    ...
    exact_detail_action_stable_id: 'RESULT_COMPOSITION_EVIDENCE',
...
function validateDetailAction(action, topic) {
  const code = 'AGREEMENT_NAVIGATION_DETAIL_ACTION_DEPENDENCY_INVALID';
  const isComposition = topic.exact_detail_action_stable_id
  ...
  if (
    ...
    || action.stable_id !== topic.exact_detail_action_stable_id
    || action.action_slot_key !== topic.exact_detail_action_stable_id
    || action.action_version !== topic.exact_detail_action_version
    || action.parent_kind !== 'RESULT_ROW'
    || action.detail_kind !== (isComposition
      ? 'RESULT_COMPOSITION_EVIDENCE'
      : 'CLAIM_EVIDENCE')
    || action.object_authorisation_predicate
      !== (isComposition
        ? 'PARENT_SELECTED_RESULT_COMPOSITION_ONLY'
        : 'PARENT_SELECTED_CLAIM_EVIDENCE_ONLY')
...
function validatePredicateCatalogue(predicateCatalogue, topic) {
  ...
  if (
    ...
    || admission.exact_detail_action_stable_id
      !== topic.exact_detail_action_stable_id
    || admission.exact_detail_action_version
      !== topic.exact_detail_action_version
  ) {
