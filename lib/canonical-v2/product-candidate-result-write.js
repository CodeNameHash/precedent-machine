...
function buildProcessPhrasebookProductChain(writeSet) {
  const body = {
...
function validateWriteEnvelope(value) {
  ...
  if (value.adapter_identifier === 'PROCESS_PHRASEBOOK_PRODUCT_CHAIN') {
    ...
    return { adapterIdentifier: value.adapter_identifier, writeSet: carrier.complete_write_set };
  }
...
function validateProductCandidateResultWriteSet(writeSet) {
  ...
  const record = buildProductCandidateResultRecord(envelope.writeSet);
  return {
    adapterIdentifier: envelope.adapterIdentifier,
    materialisable: true,
    counts: {
