...
const SOURCE_PACK_SCHEMA = 'CLOSING_CONDITIONS_FOLLOW_ON_SOURCE_PACK/V1';
...
const APPROVED_RETIRED_PENDING_BEN_APPROVAL = 'APPROVED_RETIRED_PENDING_BEN_APPROVAL';
...
function buildClosingConditionsFollowOnSourcePack(snapshot) {
  ...
  const body = {
    ...
    dispositions: {
      ...
      DISSENT_THRESHOLD: dissentThresholdSources.length
        ? GROUNDED_REPOSITORY_PRODUCTION_SNAPSHOT : OPEN_NO_GROUNDED_PRODUCTION_EVIDENCE,
    },
  };
...
function recommendedDissentThresholdDisposition(pack) {
  if (!pack || pack.dispositions?.DISSENT_THRESHOLD !== OPEN_NO_GROUNDED_PRODUCTION_EVIDENCE) return null;
  return APPROVED_RETIRED_PENDING_BEN_APPROVAL;
}
...
module.exports = {
  ...
  APPROVED_RETIRED_PENDING_BEN_APPROVAL,
