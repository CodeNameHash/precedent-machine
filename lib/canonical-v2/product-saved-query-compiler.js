...
function validateCurrentQxoCapitalisationQueryIr(queryIr) {
  ...
  const isQxoCapitalisation = semantic?.domain_key === 'AGREEMENT'
    && (
      semantic.predicate_key === 'TARGET_CAPITALISATION_BRING_DOWN'
  ...
  if (
    ...
    && (
      semantic.result_definition?.stable_id
        !== 'TARGET_CAPITALISATION_BRING_DOWN'
      || semantic.result_definition?.version !== 3
    )
  ) {
    fail(
      'RETIRED_QXO_CAPITALISATION_F27_NOT_RELEASE_ADMITTED',
      'Current QXO capitalisation saved queries must use the F28 result definition.',
    );
  }
}
...
function validateProductSavedQueryDefinition(definition) {
  ...
  validateCurrentQxoCapitalisationQueryIr(definition.source_query_ir);
...
function compileProductSavedQueryDefinition({
  ...
  validateRuntimeContractBinding();
  canonicalProductQueryIrBytes(queryIr);
  validateCurrentQxoCapitalisationQueryIr(queryIr);
