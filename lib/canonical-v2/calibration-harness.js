...
function machineReviewCardBinding(machineCard, reviewCard) {
  return canonicalJson({
    ...
    governing_context: machineCard.governing_context,
    rendered_row: machineCard.rendered_row,
    rendered_row_absent: machineCard.rendered_row_absent,
    error_class: machineCard.error_class,
    proposed_analysis: machineCard.proposed_analysis,
    question: machineCard.question,
...
function buildAnchorSetFromHumanAnchorLedger({
  ...
  const anchors = reviewPacket.cards.map((reviewCard) => {
    ...
    if (decision.verdict === 'CANT_JUDGE') {
      fail('HUMAN_ANCHOR_UNJUDGEABLE', 'CANT_JUDGE cards cannot become calibration truth');
    }
    const humanVerdict = decision.verdict === 'CORRECT' ? 'CORRECT' : 'ERROR';
    if (seed.seeded_wrong && (seed.planted_expected_verdict !== 'ERROR' || humanVerdict !== 'ERROR')) {
...
function validateAuthorityBinding(value) {
  const binding = object(value, 'authority_binding');
  const fields = [
    'corpus_digest',
    'source_digest',
    'prompt_digest',
  ...
  if (canonicalJson(Object.keys(binding).sort()) !== canonicalJson(fields.slice().sort())) {
    fail('INVALID_INPUT', 'authority binding fields do not match the contract');
  }
  for (const field of fields) sha256(binding[field], `authority_binding.${field}`);
  return cloneFreeze(binding);
}
...
function validateCalibration(calibration) {
  const body = exactContentId(calibration, 'calibration_id', CALIBRATION_SCHEMA, 'calibration');
  if (body.schema_version !== CALIBRATION_SCHEMA) {
    fail('UNCALIBRATED_ADJUDICATOR', 'calibration schema is unknown');
  }
  validateCalibrationContract(body);
  return calibration;
}

function validateCalibrationHarnessReportIdentity(report) {
  const body = exactContentId(
    report,
    'calibration_harness_report_id',
    CALIBRATION_HARNESS_SCHEMA,
    'calibration harness report',
  );
  if (body.schema_version !== CALIBRATION_HARNESS_SCHEMA
    || body.publication_authorisation !== 'NONE'
    || !Array.isArray(body.selected_rung_results)
    || !Array.isArray(body.tuple_reports)
    || !Array.isArray(body.disagreement_claim_ids)) {
    fail('INVALID_INPUT', 'calibration harness report schema is invalid');
  }
  sha256(body.calibration_id, 'calibration harness report.calibration_id');
  sha256(body.joint_confirmation_id, 'calibration harness report.joint_confirmation_id');
  sha256(body.input_digest, 'calibration harness report.input_digest');
  validateAuthorityBinding(body.authority_binding);
  return report;
}
...
function buildCalibrationHarnessReport({ catalogue, calibration, claims, sampling, confidence_interval: confidenceInterval, joint_confirmation: jointConfirmation, authority_binding: authorityBinding } = {}) {
  const maps = catalogueMaps(catalogue);
  const calibrationBody = validateCalibration(calibration);
  const confidenceLevel = validateConfidenceInterval(confidenceInterval);
  const boundAuthority = validateAuthorityBinding(authorityBinding);
  ...
  const body = {
    ...
    input_digest: sha256Hex(Buffer.from(canonicalJson({ catalogue, claims, sampling, confidence_interval: confidenceInterval, joint_confirmation: jointConfirmation, authority_binding: boundAuthority }), 'utf8')),
    authority_binding: boundAuthority,
...
module.exports = {
  ...
  buildCalibration,
  validateCalibration,
  validateCalibrationHarnessReportIdentity,
