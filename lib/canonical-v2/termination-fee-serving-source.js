...
// VERCEL FILE TRACING (production incident, 2026-08-05): this path is
// assembled at RUNTIME in readPinnedSource() below (path.join(__dirname,
// '..', '..', ...relativeParts), relativeParts arriving through a function
// parameter) rather than named in a require()/import. Next's static file
// tracer only follows real module edges -- a path built from a spread
// parameter is invisible to it -- so this file was silently excluded from
// the /api/review/[id]/cards Vercel function bundle even though
// .vercelignore allows it through. Local runs (dev, npm start, node --test)
// never noticed because they read the full repo checkout with no pruning.
// See next.config.js's outputFileTracingIncludes entry for this route,
// which force-includes it -- confirmed necessary by diffing
// `.next/server/pages/api/review/[id]/cards.js.nft.json` after a build:
// this file was absent from the trace while review-preview-assembly.js's
// sibling __fixtures__/canonical-v2/preview/*.json requires (genuine static
// imports) were present. Keep the two in sync: if this relative path ever
// changes, next.config.js's entry has to change with it.
//
// The failure this produced: readPinnedSource() threw ENOENT,
// canonicalTerminationFeeCardsForDeal() caught it and returned [], and the
// served page told a reviewer "Canonical V2 has no termination-fee data for
// this deal" -- false; the data exists and is registered below, the
// deployment just could not read it. describeCanonicalTerminationFeeSource()
// below now reports that distinction explicitly (state FAILED, never
// silently folded into NOT_REGISTERED).
...
const TERMINATION_FEE_SOURCE_STATE = Object.freeze({
  ...
  ATTACHED: 'ATTACHED',
  // A builder IS registered but threw -- an unreadable/missing pinned source
  // (the file-tracing incident documented at QXO_EXCERPTS_RELATIVE_PATH
  // above), a sha256 digest mismatch, or a drifted/ambiguous quote. This
  // deal is NOT one with "no data": Canonical V2 has data it could not read
  // or verify. Must never render, or log, the same as NOT_REGISTERED.
  FAILED: 'FAILED',
});
