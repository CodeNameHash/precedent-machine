...
//
// This module started out owning three things and nothing else -- that
// became four with the compare mode below, and this header is the second
// place in this codebase where a closed "N things" count has already gone
// stale once, so treat the number as a target, not a guarantee, and recount
// from the exports at the bottom before repeating it:
...
//   4. the compare-mode gate (isCanonicalV2TerminationFeeCompareEnabled,
//      added by Ben's ruling of 2026-08-05 -- see "Fourth mode" below) that
//      additionally allows both sources to render side by side. Strictly
//      narrower than #1: it can only be on when the serving gate is also on.
//
...
// Step 2C. Only the Modiv database-backed builder below constructs a Client
// -- QXO/TopBuild stays a pinned in-memory fixture, unchanged. `pg` is
// already an ordinary dependency (scripts/canonical-v2-local-durable-
// write.js and siblings), not new surface for this module to introduce.
...
const { readDealFromLocalCanonicalV2Staging } = require('./local-staging-deal-reader');
...
// ---------------------------------------------------------------------------
// Step 2C: Modiv, database-backed. The first entry in this registry that
// reads canonical_v2_staging instead of a pinned in-memory fixture.
// ---------------------------------------------------------------------------
//
// The real production deal id -- verified, not assumed: it is the exact
// string lib/four-deal-local-demo.js and lib/generated/home-deal-directory-
// v1.json both register for "Global Net Lease / Modiv Industrial", and it is
// the deal_id every committed tests/fixtures/review-parity/cases/*/dfaa71fa-
// modiv.case.json fixture uses for this same document.
const MODIV_DEAL_ID = 'dfaa71fa-9723-4794-825d-bd5024aa0b5d';

// The one document_hash every committed Modiv evidence directory's
// source-reference.json carries (raw_bytes_sha256 / document_hash), pinned
// independently in tests/fixtures/canonical-v2/modiv-first-live-run/intake-
// pin.json. Not a guess: it is the same value scripts/canonical-v2-live-
// extraction-run.mjs's Modiv DEAL_PINS entry and every replay run agree on.
const MODIV_DOCUMENT_HASH = '659bcfaa017718ac735811861565fa2cd4e212657ba68e06ff1eab53e3729968';

// Same env var name scripts/canonical-v2-local-durable-write.js and its
// siblings already use for an identical purpose (a local/preview Postgres
// connection string), so a developer who has already written Modiv data
// through those scripts does not need a second variable to read it back.
// Server-only, deliberately not NEXT_PUBLIC_: never reaches the client
// bundle, and isCanonicalV2TerminationFeeServingEnabled()'s own runtime gate
// (isPermittedCanonicalV2Runtime) still governs whether this builder ever
// runs at all -- this variable only says WHERE to connect, never WHETHER.
const CANONICAL_V2_LOCAL_STAGING_DB_URL_ENV_KEY = 'LOCAL_CANONICAL_V2_DB_URL';

// canonical_v2_staging.conditional_termination_fee_values has no
// document_hash or other deal-scoping column at all yet (PLAN.md Step 2B1,
// open item 4: its own CHECK constraints already hard-code Modiv's clause
// numbers -- 7.3(b)(i)..7.3(c) -- into the global schema). So this reads
// EVERY row in the table, which is correct only because Modiv is the sole
// deal that has ever written to it. A second deal's conditional-fee rows
// would need the migration Step 2B1 names before this could stay correct --
// this function does not invent scoping the schema does not have, and the
// gap is recorded here rather than silently assumed away.
async function readModivConditionalTerminationFeeValues(client) {
...
// The database-backed builder. Unlike buildQxoTopBuildTerminationFeeCards
// above, this one is async -- it opens a real connection, reads real rows
// written by scripts/canonical-v2-local-durable-write.js through
// public.canonical_v2_write, and closes the connection before returning.
// describeCanonicalTerminationFeeSource() below detects a thenable result
// and awaits it; every other registered builder keeps returning a plain
// array synchronously, so nothing about the QXO path changes.
async function buildModivTerminationFeeCardsFromDatabase(dealId, { env } = {}) {
  const resolvedEnv = env || process.env;
  const dbUrl = ownEnvValue(resolvedEnv, CANONICAL_V2_LOCAL_STAGING_DB_URL_ENV_KEY);
  if (!dbUrl) {
    throw new Error(
      `${CANONICAL_V2_LOCAL_STAGING_DB_URL_ENV_KEY} is not set -- the database-backed Modiv `
      + 'termination-fee source has nothing to connect to. Set it to a local Postgres container URL, '
      + 'e.g. the one scripts/canonical-v2-local-durable-write.js wrote through.',
    );
  }
  const client = new Client({ connectionString: dbUrl });
  await client.connect();
  try {
    const deal = await readDealFromLocalCanonicalV2Staging({
      client,
      documentHash: MODIV_DOCUMENT_HASH,
      env: resolvedEnv,
    });
    const conditionalTerminationFeeValues = await readModivConditionalTerminationFeeValues(client);
    return projectTerminationFeeProductSurfaces({
      resolution: {
        resolved: deal.resolved,
        open_world: deal.open_world,
...
const CANONICAL_TERMINATION_FEE_SOURCES = Object.freeze({
  ...
  [MODIV_DEAL_ID]: buildModivTerminationFeeCardsFromDatabase,
});
...
function finishSourceOutcome(built) {
...
function failedSourceOutcome(error) {
  return {
    cards: [],
...
//
// Step 2C, dual return shape, deliberately. Every builder registered before
// this step (QXO/TopBuild) builds its cards synchronously from an in-memory
// fixture, and for those this function keeps returning the plain
// { cards, outcome } object -- byte-identical to before, so every existing
// synchronous caller (canonicalTerminationFeeCardsForDeal, every test that
// destructures this return value without awaiting it) keeps working
// unchanged. Step 2C's Modiv builder is the first one that must open a real
// database connection, so IT returns a Promise; this function detects that
// (a thenable) and, only then, returns a Promise of the SAME { cards,
// outcome } shape -- rather than forcing every synchronous caller in this
// codebase to add `await` for a change that affects one deal's builder.
function describeCanonicalTerminationFeeSource(dealId, { sources = CANONICAL_TERMINATION_FEE_SOURCES, env } = {}) {
  ...
  let built;
  try {
    built = build(dealId, { env });
  } catch (error) {
    return failedSourceOutcome(error);
  }
  if (built && typeof built.then === 'function') {
    return built.then(finishSourceOutcome, failedSourceOutcome);
  }
  return finishSourceOutcome(built);
}
...
function finishAttach(reviewDeal, cards, outcome, logger, resolvedEnv) {
...
// Step 2C: this is the ONE call site cards.js reaches
// (pages/api/review/[id]/cards.js:55), and the ONLY place a database-backed
// source can now be attached. Stays synchronous end to end for every
// pre-2026-08-07 caller (see describeCanonicalTerminationFeeSource's own
// comment on why); for the new Modiv database source it returns a Promise
// of the exact same shape, and cards.js awaits it the same way it already
// awaits attachCanonicalV2Preview() one line above.
function attachCanonicalTerminationFeeServing(reviewDeal, { env, logger, sources } = {}) {
  ...
  const described = describeCanonicalTerminationFeeSource(reviewDeal.dealId, { sources, env: resolvedEnv });
  if (described && typeof described.then === 'function') {
    return described.then(({ cards, outcome }) => finishAttach(reviewDeal, cards, outcome, logger, resolvedEnv));
  }
  return finishAttach(reviewDeal, described.cards, described.outcome, logger, resolvedEnv);
}
