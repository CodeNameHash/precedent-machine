...
const { compileFixtureContractV4 } = require('./contract-bundle');
const {
...
function buildQxoReverseF4Candidate({
  agreementSourceContext,
  agreementSourceAdmission,
  dealValueSourceContext,
  dealValueSourceAdmission,
  contractBundle = compileFixtureContractV4(),
  correctionAuthoritySelection,
} = {}) {
  if (contractBundle.fingerprint !== QXO_REVERSE_F4_CONTRACT_FINGERPRINT) {
    throw new TypeError('The corrected QXO reverse candidate requires the frozen F4 contract.');
  }
  ...
  if (materialProbe.reviewed_mapping.reviewed_mapping_id !== QXO_MATERIAL_REVIEWED_MAPPING_ID) {
    throw new TypeError('The reviewed QXO material mapping has drifted before the F4 rebuild.');
  }

  const seed = buildQxoReverseF4CombinedCandidateSeed({
  ...
  const corpusReleaseId = qxoReverseF4CombinedCandidateReleaseId(seed);
  const servingNamespaceId = qxoReverseF4CombinedServingNamespaceId(seed);
  ...
  return Object.freeze({
    schema_version: 'QXO_REVERSE_F4_CANDIDATE_BUILD/V1',
    contract_bundle: contractBundle,
    correction_authority_selection: authoritySelection,
    seed,
    corpus_release_id: corpusReleaseId,
