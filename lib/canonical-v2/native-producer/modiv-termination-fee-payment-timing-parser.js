...
const { requireModivFeeBranchSupport } = require('./modiv-termination-fee-source-parser');
...
function resolveModivPaymentTimings({ source_text: sourceText, fee_trigger_candidates: feeTriggerCandidates } = {}) {
  ...
  const parsed = parseModivPaymentTimings(sourceText);
  requireModivFeeBranchSupport({
    sourceText,
    feeTriggerCandidates,
    sideMismatchCode: 'MODIV_PAYMENT_TIMING_TRIGGER_SIDE_MISMATCH',
    branchUnsupportedCode: 'MODIV_PAYMENT_TIMING_TRIGGER_BRANCH_UNSUPPORTED',
  });
  return parsed;
}
