 ...
 * id. `relationship_occurrence_id` is NOT re-derived: it does not depend on
 * evidence, so the "same relationship slot, new evidence revision"
 * semantics fall out for free. `claim_occurrence_id` is NOT re-derived FROM
 * EVIDENCE for the same reason, but IS re-derived on a separate,
 * evidence-independent trigger added after this paragraph was first
 * written: an assertion-node claim whose subject gets a newly-minted
 * component row (`COMPONENT_KEY_REPRESENTATION_LIMB`, below) changes
 * `subject_occurrence_id`, and `claim_occurrence_id` is content-derived
 * FROM `subject_occurrence_id` (claims-relationships.js's
 * `buildClaimRevision`), so it must be re-derived too when that happens --
 * see the `claimSubjectRekey` handling below for exactly when this fires.
 * `closure_id` is likewise NOT
 ...
 * `retained_residuals`/`quarantine`/`publication_state` (the canonical
 * validation state, not `publication_disposition`) are also carried
...
const { buildPublicationDispositionV2 } = require('../publication-disposition');
...
// context is supplied. Resolver-minted provisions are copied from
// `resolution.provisions`; this adapter does not invent either an obligation
// party or a structural provision kind.
//
// **Ben's ruling, DECISIONS.md decision 2 (2026-08-07): "emit them, with a
// flag."** Until this change, `resolution.open_world` -- 244 entries across
// the regenerated baseline, model proposals the resolver kept with a reason
// code and a verbatim quote, but no governed vocabulary slot -- was dropped
// here unconditionally, via all five open-world collections sitting in
// `EMPTY_COLLECTION_KEYS`. That is no longer true. `buildNativeOpenWorldWriteRows`
// below turns each `resolution.open_world` entry into one write object per
// row kind (`open_world_candidates`, `open_world_candidate_occurrences`,
// `open_world_evidence_references`, `open_world_candidate_dispositions`),
// sharing the resolver's own `closure_id` as the join key, evidence shifted
// to document-absolute coordinates and excerpted through the exact same
// `shiftEdge`/`excerptFor` machinery the main per-candidate loop uses for
// governed claims -- an open-world entry's `.evidence` is the same
// section-local shape as a compiled claim row's, by construction (see
// candidate-resolution.js's `pushOpenWorld`, which passes the candidate's own
// `claim` object through untouched).
//
// **The marker.** Every row this function mints carries
// `evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER` in its own
// payload -- on the row, not inferred from the table it lands in -- and no
// governed `claims`/`relationships` row can carry that field: their payload
// shape is a closed, content-hashed field list
// (`CLAIM_REVISION_PAYLOAD_FIELDS` above) that does not include it, and
// adding it would change every claim's identity, which this module never
// does. `validate-write-set.js`'s `openWorldGraphResiduals` recognises this
// shape by its own `schema_version` (`NATIVE_OPEN_WORLD_CANDIDATE/V1`,
// distinct from the QXO F16-F26 certification chain's
// `NOVEL_CONCEPT_CANDIDATE/V1`) and validates it with a lighter, purpose-built
// closure check -- see that module. **This adapter does not, and does not
// need to, produce a `SharedServingRow` terminal row
// (`reviewed_source_specific_rows`/`incomplete_canonical_result_rows`) or a
// `semantic_impact_closures` row for open-world evidence**: those are part of
// the QXO market-comparison certification chain DECISIONS.md decision 13
// rules is not extended past termination fees/no-shop, and Ben's decision 2
// asks only that the fact be written and flagged, not that it be certified
// against a canonical result.
//
// `definition_occurrences`, `condition_groups`, `validated_semantic_graphs`,
// `open_world_primitives`, `semantic_impact_closures`,
// `reviewed_source_specific_rows` and `incomplete_canonical_result_rows`
// remain genuinely empty from this path -- nothing here produces that kind of
// row, still. `open_world_primitives` in particular is QXO-specific (the
// decomposition of one open-world candidate into TIME/etc "legal primitives"
// -- see shared-serving-row.js): a general native-producer open-world entry
// does not decompose that way, and Ben's decision 2 does not ask for it to.
//
// **`conditional_termination_fee_values` is NOT in this list** (PLAN.md Step
// 4A3, Ben's ruling in DECISIONS.md decision 3): unlike everything above,
// `resolution.conditional_termination_fee_values` -- when
// `resolveModivConditionalFees` produced any -- is carried straight into the
// write set. See the dedicated comment ahead of that pass, further down.
...
// The on-row ungoverned-evidence marker (DECISIONS.md decision 2). Exported
// so callers (tests, projection/serving code) check against this constant
// rather than a hand-typed string.
const OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER = 'UNGOVERNED_OPEN_WORLD_EVIDENCE';
const OPEN_WORLD_CANDIDATE_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE/V1';
const OPEN_WORLD_OCCURRENCE_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE_OCCURRENCE/V1';
const OPEN_WORLD_EVIDENCE_REFERENCE_DOMAIN = 'NATIVE_OPEN_WORLD_EVIDENCE_REFERENCE/V1';
const OPEN_WORLD_DISPOSITION_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE_DISPOSITION/V1';
const OPEN_WORLD_DISPOSITION_CODE = 'UNGOVERNED_UNREVIEWED';
...
function requireRunReceipt(runReceipt) {
  ...
  if (!/^[a-f0-9]{64}$/.test(runReceipt.run_receipt_id || '')) {
    fail('INVALID_RUN_RECEIPT', 'run_receipt.run_receipt_id must be a lower-case SHA-256 content identity');
  }
  return runReceipt;
}
...
// For `claim` candidates this pipeline produces evidence whose (section-
// local) slice already equals the compiled row's own `raw_value` -- WITH
// ONE NAMED EXCEPTION `native-extraction-run.js`'s own `checkEvidenceScope`
// carves out and this function must mirror exactly, not just approximate:
// a defined-term proposal's evidence carries two independently byte-
// verified spans, its definition head (`evidence_role: 'DEFINITION'`) and
// its operative limb (`evidence_role: 'OPERATIVE_TEXT'`); `raw_value` is
// deliberately the limb's text, so `checkEvidenceScope` compares a
// DEFINITION edge against `proposal.attributes.definition_head_quote`
// instead. Found 2026-08-07 (docs/codex-program/notes/key-defined-terms-
// zero-claims.md): this function did not carry that exception, so every
// DEFINITION edge on a rebuilt defined-term claim failed the post-shift
// byte-verify against the wrong string and the whole claim was dropped as
// a residual, even though both edges shifted to real, correctly-located
// text -- `KEY_DEFINED_TERMS` resolving 10 real candidates and publishing
// zero claims traced to exactly this. Re-checking against the SAME
// role-appropriate text `checkEvidenceScope` already proved is the direct,
// byte-exact proof that the shift preserved what the section-local check
// established -- checking a DEFINITION edge against `raw_value` was never
// that proof, it was a false positive for single-edge claims that
// coincidentally became a false rejection the first time a claim carried
// a genuinely different DEFINITION span. Relationship proposals carry no
// single row-level text (`raw_scope` describes the relationship's scope,
// not each evidence edge), so for `relationship` candidates this falls
// back to the section-local slice at the SAME local offsets recorded on
// the compiled row -- still a genuine, non-tautological proof that
// shifting did not corrupt the edge, just anchored to the receipt's own
// previously-proven text rather than to a single label field.
function expectedTextForEdge({ kind, row, edge, sectionBytes }) {
  if (kind === 'claim' && typeof row.raw_value === 'string') {
    if (edge.evidence_role === 'DEFINITION' && typeof row.attributes?.definition_head_quote === 'string') {
      return row.attributes.definition_head_quote;
    }
...
// ---------------------------------------------------------------------------
// Open-world write objects (DECISIONS.md decision 2, PLAN.md Step 2B). One
// `resolution.open_world` entry becomes one candidate + one occurrence + one
// disposition + one-or-more evidence references, all sharing the candidate's
// own `closure_id` (already minted upstream by candidate-proposal-compiler.js
// at compile time -- reused here as the join key, not re-derived) so
// `validate-write-set.js`'s closure grouping finds all four together. Every
// row carries `evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER`.
//
// A single failed evidence edge quarantines only that open-world entry (a
// residual, not a thrown error) -- exactly the "fail closed, typed, never a
// silent drop" discipline this module's header already commits to for
// governed claims, applied here to entries that were never governed to begin
// with.
function buildOpenWorldWriteRows({
  openWorldEntries, sectionsByReference, fullBytes, admittedSourceContext, excerptFor, residualPush,
}) {
  const candidates = [];
  const occurrences = [];
  ...
  for (const owEntry of openWorldEntries) {
    if (!owEntry || typeof owEntry !== 'object') continue;
    const section = sectionsByReference.get(owEntry.section_reference);
    if (!section) {
      fail('SECTION_REFERENCE_UNRESOLVED_IN_RECEIPT', 'an open-world entry names a section the run receipt never resolved -- the receipt is internally inconsistent', {
        section_reference: owEntry.section_reference,
      });
    }
    const sectionBytes = fullBytes.subarray(section.start, section.end);
    const closureId = owEntry.closure_id;
    const documentHash = admittedSourceContext.document_hash;

    const candidateBody = {
      document_hash: documentHash,
      section_reference: owEntry.section_reference,
      attempted_claim_definition_key: owEntry.claim_definition_key ?? null,
      reason_code: owEntry.reason ?? null,
      raw_value: owEntry.raw_value ?? null,
      canonical_value: owEntry.canonical_value ?? null,
      closure_id: closureId,
    };
    const candidateId = contentId(OPEN_WORLD_CANDIDATE_SCHEMA, candidateBody);
    ...
    const occurrenceBody = {
      ...
      raw_value: owEntry.raw_value ?? null,
      canonical_value: owEntry.canonical_value ?? null,
      attributes: owEntry.attributes ?? null,
      closure_id: closureId,
    };
    const occurrenceId = contentId(OPEN_WORLD_OCCURRENCE_SCHEMA, occurrenceBody);

    const edges = Array.isArray(owEntry.evidence) ? owEntry.evidence : [];
    const finalEdges = [];
    let edgeFailure = null;
    for (const edge of edges) {
      const shifted = shiftEdge({
        kind: 'claim', row: owEntry, edge, section, sectionBytes, fullBytes,
      });
      if (!shifted.ok) {
        edgeFailure = { ...shifted, edge };
        break;
      }
      const built = excerptFor(shifted.absoluteStart, shifted.absoluteEnd);
      if (!built.ok) {
        edgeFailure = { ...built, edge };
        break;
      }
      finalEdges.push({
        evidence_role: edge.evidence_role,
    ...
    if (edgeFailure || finalEdges.length === 0) {
      residualPush({
        residual_type: 'OPEN_WORLD_EVIDENCE_COORDINATE_SHIFT_FAILED',
        section_reference: owEntry.section_reference,
        closure_id: closureId,
        reason_code: edgeFailure ? edgeFailure.reason : 'OPEN_WORLD_ENTRY_HAS_NO_EVIDENCE',
        expected: edgeFailure?.expected ?? null,
        actual: edgeFailure?.actual ?? null,
        message: edgeFailure?.message ?? null,
      });
      continue;
    }

    const evidenceRows = finalEdges.map((edge, ordinal) => {
      const body = {
      ...
      return Object.freeze({
        evidence_reference_id: contentId(OPEN_WORLD_EVIDENCE_REFERENCE_DOMAIN, body),
        open_world_candidate_occurrence_id: occurrenceId,
        evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER,
        evidence_role: edge.evidence_role,
        excerpt_id: edge.excerpt_id,
        document_ordinal: edge.document_ordinal,
        absolute_start: edge.absolute_start,
    ...
    const dispositionBody = {
      open_world_candidate_occurrence_id: occurrenceId,
      disposition_code: OPEN_WORLD_DISPOSITION_CODE,
      reason_code: owEntry.reason ?? null,
      closure_id: closureId,
    };
    const dispositionId = contentId(OPEN_WORLD_DISPOSITION_SCHEMA, dispositionBody);

    candidates.push(Object.freeze({
      schema_version: OPEN_WORLD_CANDIDATE_SCHEMA,
      candidate_id: candidateId,
      evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER,
      ...candidateBody,
    }));
    occurrences.push(Object.freeze({
      schema_version: OPEN_WORLD_OCCURRENCE_SCHEMA,
      open_world_candidate_occurrence_id: occurrenceId,
      evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER,
      ...occurrenceBody,
    }));
    evidenceReferences.push(...evidenceRows);
    dispositions.push(Object.freeze({
      schema_version: OPEN_WORLD_DISPOSITION_SCHEMA,
      final_disposition_id: dispositionId,
      evidence_governance: OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER,
      ...dispositionBody,
    }));
  }
  ...
  return {
    open_world_candidates: candidates,
    open_world_candidate_occurrences: occurrences,
    open_world_evidence_references: evidenceReferences,
    open_world_candidate_dispositions: dispositions,
  };
}
 ...
 *   limb_path). Also carries `conditional_termination_fee_values` (PLAN.md
 *   Step 4A3), copied into the write set unchanged when present and
 *   non-empty -- see the dedicated comment ahead of that pass, below. Absent,
 *   this module's behaviour is exactly as if this parameter did not exist --
 *   strictly additive.
 * @returns {{
...
function buildNativeWriteSet({
  ...
  resolution,
  // Optional and inert. An omitted gate creates audit sidecars which are
  // deterministically WITHHELD; it never loads an authority implicitly.
  ...
  // mintCitationTriggeredByRelationship). UNLIKE every other relationship/
  // claim in this module, these arrive from `resolution.relationships`
  // ALREADY document-absolute -- built directly against
  // `admitted_source_context` via `buildExcerpt`, never against a
  // section-local buffer -- so none of this module's own coordinate-shift
  // machinery runs on them; they are concatenated as-is. Absent or empty
  // `resolution.relationships` (every existing caller, every deal that
  // never follows a citation) leaves `relationships`/`excerpts`/
  // `provisions` byte-identical to today.
  const citationRelationshipBundles = resolutionProvided && Array.isArray(resolution.relationships)
    ? resolution.relationships
    : [];
  for (const bundle of citationRelationshipBundles) {
    if (!bundle || !bundle.relationship) continue;
    relationships.push(bundle.relationship);
    for (const excerpt of (Array.isArray(bundle.excerpts) ? bundle.excerpts : [])) {
      if (excerpt && excerpt.excerpt_id && !excerptsById.has(excerpt.excerpt_id)) {
        excerptsById.set(excerpt.excerpt_id, excerpt);
      }
    }
    if (bundle.cited_provision && bundle.cited_provision.provision_instance_id) {
      provisionsById.set(bundle.cited_provision.provision_instance_id, bundle.cited_provision);
    }
  }

  // Open-world evidence (DECISIONS.md decision 2). `resolution.open_world` is
  // absent for every caller that does not supply a `resolution` context
  // (component-row minting's own "strictly additive" rule, mirrored here):
  // no `resolution` -> no open-world rows, byte-identical to before this
  // change for every caller that never passed one.
  const openWorldEntries = resolutionProvided && Array.isArray(resolution.open_world)
    ? resolution.open_world
    : [];
  const openWorldRows = buildOpenWorldWriteRows({
    openWorldEntries,
    sectionsByReference,
    fullBytes,
  ...
  // Conditional termination-fee values (PLAN.md Step 4A3; Ben's ruling,
  // DECISIONS.md decision 3). Found missing by Step 4A2 itself: that step
  // gave `conditional_termination_fee_values` a table and a shape check in
  // `supabase/canonical-v2-foundation.sql`, and proved the SQL side against a
  // hand-built harness input -- but this adapter never carried the kind, so
  // nothing a real run produces ever reached that table. This closes that.
  //
  // UNLIKE every other collection this module builds, these rows carry no
  // coordinate-shiftable evidence at all. `resolveModivConditionalFees`
  // (modiv-termination-fee-source-parser.js) already builds each one as a
  // closed, content-addressed `CONDITIONAL_TERMINATION_FEE_VALUE/V1` row via
  // `buildConditionalTerminationFeeValue` (conditional-termination-fee-
  // value.js), with `source_citations` as verbatim clause-reference STRINGS
  // -- never excerpt ids, never a section-local offset. There is nothing
  // here for this module's `shiftEdge`/`excerptFor` machinery to touch, so
  // (unlike `claims`/`relationships`/open-world evidence) this pass does not
  // shift, excerpt, or re-derive identity: the resolver's rows are the
  // finished rows.
  //
  // Carried through UNCHANGED, the same "strictly additive, absent without a
  // `resolution` context" rule the open-world/component-row/citation-
  // relationship passes above already follow: no `resolution`, or a
  // `resolution` with none of these, leaves the write set byte-identical to
  // before this change. OMITTED from the write set (not an empty array) when
  // absent or empty -- mirroring candidate-resolution.js's own "OMITTED
  // entirely, not an empty array" choice for this exact field, and matching
  // `persisted_object_references`'s independently-optional top-level key in
  // `validate-write-set.js`'s `assertResolvedWriteSetShape` and
  // `supabase/canonical-v2-foundation.sql`'s OPTIONAL-key subtraction list
  // (Step 4A2): most deals never produce this kind, and an omitted key is
  // what "OPTIONAL" means there, not a present-but-empty array.
  ...
  const resolutionReceiptId = publicationGate?.resolution_receipt_id
    || resolution?.resolution_receipt?.resolution_receipt_id
    || contentId('CANONICAL_V2_ABSENT_RESOLUTION_RECEIPT/V1', { run_receipt_id: runReceipt.run_receipt_id });
  const reviewQueueArtifactId = publicationGate?.review_queue_artifact_id
    ?? resolution?.review_queue_artifact?.review_queue_artifact_id
    ?? null;
  const evaluatedAt = publicationGate?.evaluated_at || '1970-01-01T00:00:00.000Z';
  const publicationDispositions = claims.map((claim) => buildPublicationDispositionV2({
    ...
    publication_input: publicationGate?.publication_inputs_by_claim_revision_id?.[claim.claim_revision_id]
      || publicationGate?.publication_input,
    canonical_validation_state: claim.publication_state,
  ...
  const writeSet = {
    ...
    claims,
    publication_dispositions: publicationDispositions,
    ...(publicationGate?.calibration_authority
      ? { publication_calibration_authorities: [publicationGate.calibration_authority] }
      : {}),
    relationships,
    components,
  ...
  return Object.freeze({
    ...
    counts: Object.freeze({
      ...
      open_world_candidates_total: openWorldEntries.length,
      open_world_candidates_written: openWorldRows.open_world_candidates.length,
      open_world_candidates_rejected_by_coordinate_shift:
        openWorldEntries.length - openWorldRows.open_world_candidates.length,
      conditional_termination_fee_values_written: conditionalTerminationFeeValues.length,
      publication_dispositions_written: publicationDispositions.length,
    }),
  });
}
...
module.exports = {
  ...
  OPEN_WORLD_OCCURRENCE_SCHEMA,
  OPEN_WORLD_DISPOSITION_SCHEMA,
