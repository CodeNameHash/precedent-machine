...
//
// The other collections are present-but-empty. That was once because nothing
// produced that kind of row. **For the five open-world collections it is no
// longer true, and the gap is load-bearing.** `resolution.open_world` carries
// 275 entries across the 25 importable runs in the committed baseline
// (`evidence/canonical-v2/baseline-manifest.json`) -- model proposals the
// resolver kept, with `why_unmapped`, `structured_mechanic` and a verbatim
// quote -- and every one of them is dropped here.
//
// Two consequences worth knowing before relying on this:
//   - the open-world tables in `canonical_v2_staging` are empty by
//     construction from this path, so nothing served FROM the database can
//     ever show open-world evidence. Four of the ten cards the termination-fee
//     projection produces from a run are open-world evidence cards; those four
//     have no database-backed equivalent.
//   - a family whose output is entirely open-world -- and several are, see
//     PLAN.md's triage of the ten zero-claim families -- writes nothing at
//     all, which is indistinguishable from a family that found nothing.
//
// Left as-is deliberately: emitting these rows changes what the pipeline
// writes for every deal and is recorded as a decision in PLAN.md Step 2B
// rather than made here. This comment exists so the emptiness is not read as
