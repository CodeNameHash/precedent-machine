 ...
 * concatenated anywhere in the prose (see that module's header). A proposal
 * whose citation does not agree with the independently-derived one is never
 * silently compiled: it is excluded from `compiled_candidates` and recorded
 * as a typed `citation_residuals` entry (`CITATION_NOT_CONSTRUCTIBLE` or
 * `CITATION_DISAGREEMENT`), the same fail-closed treatment `scope_violations`
 * already gives an evidence span outside the governed scope. THE GENERAL
 * PRINCIPLE, for the next reference-like field this producer grows: check
 * for constructibility against independently-derived structure, never for
 * literal presence, and never take the model's word for it.
 */
...
async function runNativeExtraction({
  ...
  for (const { reference, node } of resolved) {
    ...
    // Citation constructibility (see file header). The derived citation for
    // every proposal produced under THIS governed scope is simply the
    // scope's own resolved node: defect 1's "one representation per
    // governing section" rule means every proposal's evidence necessarily
    // sits inside exactly that node's span, so deriving it via the general
    // deepest-containing-node walk over the FULL span this call examined is
    // both correct and (by construction) resolves back to `node` itself.
    const derivedNode = deriveCitationForSpan(tree, node.start, node.end);
    const citationCheckedProposals = [];
    for (const proposal of inScopeProposals) {
      ...
      if (citationCheck && citationCheck.status !== 'AGREEMENT') {
      ...
      citationCheckedProposals.push(proposal);
    }

    const compileResults = compileCandidateProposals({
      proposals: citationCheckedProposals,
      governed_scope: governedScope,
      producer_receipt: producerReceipt,
