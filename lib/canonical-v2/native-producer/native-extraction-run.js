...
const { classifySectionFamily } = require('./section-family-classifier');
...
async function runNativeExtraction({
  ...
  for (const { reference, node } of resolved) {
    ...
    const prompt = producerPromptModule({
      source_text: sectionText,
    ...
    const {
      ...
      governed_scope: Object.freeze({ ...governedScope, section_family: sectionFamily }),
      definitions,
      contract_bundle: contractBundle,
      section_family: sectionFamily,
      provider,
