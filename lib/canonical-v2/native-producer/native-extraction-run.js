 ...
 *   2. producer-prompt-registry.js     -- resolves and builds the prompt for
 *                                          each resolved governed scope's
 *                                          section family (recorded into the
 *                                          run receipt for audit; the
 *                                          provider is free to rebuild the
 *                                          identical prompt itself). Without
 *                                          a `section_family_classifier`
 *                                          this always resolves to
 *                                          capitalisation-producer-prompt.js,
 *                                          exactly as when this module hard-
 *                                          required that one file (see
 *                                          "PRODUCER DISPATCH THROUGH THE
 *                                          REGISTRY" below); with one
 *                                          supplied it can resolve to any of
 *                                          the families producer-prompt-
 *                                          registry.js registers.
 ...
 *
 * CITATION CONSTRUCTIBILITY, NOT LITERAL PRESENCE (docs/archive/handoffs/
 ...
 *
 * NEVER SILENTLY DISCARD (docs/archive/handoffs/F28-SECOND-LIVE-RUN.md). An earlier
...
// Deliberately its own schema_version, never RUN_RECEIPT_SCHEMA: a partial
// receipt (see buildPartialRunReceipt below) is missing a run_receipt_id and
// makes no claim about sections it never reached, so nothing downstream can
// mistake it for a complete, content-addressed receipt by accident.
const PARTIAL_RUN_RECEIPT_SCHEMA = 'NATIVE_EXTRACTION_RUN_PARTIAL_RECEIPT/V1';
...
const CODEX_SUBSCRIPTION_PROVIDER_ID = 'OPENAI_CODEX_CLI_SUBSCRIPTION';
...
// Capitalisation provider output carries useful limb labels, but it is not
// allowed to author a published citation. For a byte-verified proposal, the
// admitted tree supplies the parent node and the source sentence supplies the
// only child marker that can apply to the quoted span. The result is a
// deterministic published citation such as `3.7(a)(i)` or
// `III-INTRO(b)(i)(A)`, even where the rendered agreement never prints that
// concatenated string.
function deriveCapitalisationSourceTreeCitation({ sourceText, derivedNode, proposalSpan }) {
  if (!derivedNode || typeof derivedNode.reference !== 'string' || !proposalSpan) return null;
  const sourceBytes = Buffer.from(sourceText, 'utf8');
  if (proposalSpan.start < derivedNode.start || proposalSpan.end > derivedNode.end) return null;

  const prefix = sourceBytes.subarray(derivedNode.start, proposalSpan.start).toString('utf8');
  const suffix = sourceBytes.subarray(proposalSpan.end, derivedNode.end).toString('utf8');
  const sentenceBoundary = /[.;](?=\s+(?:[A-Z]|\())|[.;](?=$|\n)/g;
  let boundaryMatch;
  let sentenceStart = 0;
  while ((boundaryMatch = sentenceBoundary.exec(prefix)) !== null) sentenceStart = boundaryMatch.index + 1;
  const sentencePrefix = prefix.slice(sentenceStart);
  const markerPattern = /\(([A-Za-z0-9]{1,4})\)/g;
  let marker = null;
  let match;
  while ((match = markerPattern.exec(sentencePrefix)) !== null) marker = `(${match[1]})`;

  const rootMarkers = derivedNode.reference.match(/\([A-Za-z0-9]{1,4}\)/g) || [];
  const rootTerminalMarker = rootMarkers[rootMarkers.length - 1] || null;
  const childMarker = marker && marker !== rootTerminalMarker ? marker : null;
  const terminalMatch = sentenceBoundary.exec(suffix);
  const terminal = terminalMatch ? terminalMatch.index : -1;
  const contextQuote = `${sentencePrefix}${sourceBytes.subarray(proposalSpan.start, proposalSpan.end).toString('utf8')}${
    terminal === -1 ? suffix : suffix.slice(0, terminal + 1)
  }`.trim();

  return Object.freeze({
    published_citation: `${derivedNode.reference}${childMarker || ''}`,
    source_tree_parent_citation: derivedNode.reference,
    source_limb_path: childMarker ? Object.freeze([childMarker]) : Object.freeze([]),
    source_tree_context_quote: contextQuote,
  });
}
...
// A provider failure partway through a multi-section batch (see the
// runNativeExtraction loop below) used to discard every section resolved
// before it: nothing is returned until the whole loop finishes without
// error, so an exception thrown while dispatching section N erased sections
// 1..N-1's already-succeeded, already-paid-for work along with it. This
// builds a typed, frozen snapshot of everything resolved before the failure
// -- same field shape as the real run receipt below, so a caller can feed
// it through the same downstream code for the sections that DID succeed --
// plus which section failed and which were never attempted. It is attached
// to the thrown error (see the catch site below), never returned in place
// of a real receipt: this turns a total loss into a partial one without
// changing runNativeExtraction's success-path contract at all.
function buildPartialRunReceipt({
  documentHash, tree, references, resolvedSections, compiledCandidates, evidenceResiduals,
  scopeViolations, citationResiduals, coverageProxies, limbEnumerationScan, undispatchedSections,
  failedSectionReference, failureCode, failureMessage, unattemptedSectionReferences,
}) {
  return deepFreeze({
    schema_version: PARTIAL_RUN_RECEIPT_SCHEMA,
    complete: false,
    ...
    failed_section_reference: failedSectionReference,
    failure_code: failureCode,
    failure_message: failureMessage,
    unattempted_section_references: unattemptedSectionReferences,
 ...
 * @param {boolean} [args.source_tree_limb_citations=false]
 *   Enables deterministic capitalisation child citations for a sealed replay.
...
async function runNativeExtraction({
  ...
  source_tree_limb_citations: sourceTreeLimbCitations = false,
} = {}) {
  ...
  if (typeof sourceTreeLimbCitations !== 'boolean') {
    throw new NativeExtractionRunError('INVALID_INPUT', 'source_tree_limb_citations must be a boolean when supplied');
  }
  ...
  for (const [loopIndex, { reference, node }] of resolved.entries()) {
    ...
    let producerCallResult;
    try {
      // eslint-disable-next-line no-await-in-loop
      producerCallResult = await produceCandidateProposals({
        governed_scope: providerScope,
        definitions,
        contract_bundle: contractBundle,
        section_family: sectionFamily,
      ...
      // Crash 3 (docs/codex-program/notes/extraction-crashes.md): a
      // malformed/failed model response on THIS section must not silently
      // erase sections 1..loopIndex-1's already-succeeded, already-paid-for
      // work. Attach a partial receipt onto the SAME error object being
      // thrown -- never a new error type, message or code, so every
      // existing catch site keeps working exactly as before -- so a caller
      // that wants to recover the partial result can, instead of losing it
      // to the void. This never weakens fail-closed behaviour: the error
      // still propagates, sections after this one are never attempted, and
      // the partial receipt carries its own distinct schema_version so it
      // can never be mistaken for a complete run receipt.
      if (error && typeof error === 'object') {
        error.details = {
          ...(error.details && typeof error.details === 'object' ? error.details : {}),
          partial_run_receipt: buildPartialRunReceipt({
            documentHash,
            tree,
            references,
            resolvedSections,
            ...
            coverageProxies,
            limbEnumerationScan,
            undispatchedSections,
            failedSectionReference: reference,
            failureCode: (error && error.code) || null,
            failureMessage: (error && error.message) || String(error),
            unattemptedSectionReferences: references.slice(loopIndex + 1),
          }),
        };
      }
      throw error;
    ...
    // live-run defect: see docs/archive/handoffs -- native-extraction-run.js commit
    ...
    const citationChecks = inScopeProposals.map((proposal) => {
      ...
      const sourceTreeCitation = sourceTreeLimbCitations && sectionFamily === 'CAPITALISATION' && proposalSpan
        ? deriveCapitalisationSourceTreeCitation({
          sourceText,
          derivedNode,
          proposalSpan,
        })
        : null;
      if (sourceTreeCitation) {
        return {
          status: 'DERIVED_FROM_SOURCE_TREE_LIMB',
          accepted: true,
          validation_source: 'DERIVED_FROM_SOURCE_TREE_LIMB',
          model_citation: typeof modelCitation === 'string' ? modelCitation : null,
          normalized_citation: sourceTreeCitation.published_citation,
          derived_citation: sourceTreeCitation.published_citation,
          source_tree_parent_citation: sourceTreeCitation.source_tree_parent_citation,
          source_limb_path: sourceTreeCitation.source_limb_path,
          source_tree_context_quote: sourceTreeCitation.source_tree_context_quote,
        };
      }
    ...
    compileResults.forEach((result, index) => {
      ...
      const citationValidation = citationCheck ? {
        ...
        normalized_citation: citationCheck.normalized_citation,
        derived_citation: citationCheck.derived_citation,
        ...(citationCheck.validation_source === 'DERIVED_FROM_SOURCE_TREE_LIMB' ? {
          source_tree_parent_citation: citationCheck.source_tree_parent_citation,
          source_limb_path: citationCheck.source_limb_path,
          source_tree_context_quote: citationCheck.source_tree_context_quote,
        } : {}),
      } : null;
    ...
    resolvedSections.push({
      ...
      // Codex adds a provider-specific system wrapper. Its producer receipt
      // binds those wire bytes. Other providers retain the established run
      // receipt contract over the literal governed message array.
      prompt_digest: producerReceipt.provider_id === CODEX_SUBSCRIPTION_PROVIDER_ID
        ? producerReceipt.prompt_digest
        : promptDigest,
      producer_receipt: producerReceipt,
    });
  }
