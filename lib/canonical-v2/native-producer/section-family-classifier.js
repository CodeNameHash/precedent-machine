...
const SECTION_FAMILY_CLASSIFIER_VERSION = 6;
...
const APPRAISAL_STANDALONE_TITLE_PATTERN = /^(?:dissenting|appraisal) (?:shares|rights|proceedings)$/i;
const EMPLOYEE_MATTERS_TITLE_PATTERN = /employee\s+matters|continuing\s+employees?|employee\s+benefit\s+matters/i;
const DNO_TITLE_PATTERN = /directors?['’]?\s*(?:and|&)\s*officers?|d\s*&\s*o|d&o|indemnification\s+and\s+insurance|d&o\s+insurance|director\s+and\s+officer\s+liability/i;
...
function runStage1(title, articleContext = null) {
  for (const rule of STAGE_1_TITLE_RULES) {
    if (rule.test(title, articleContext)) {
      return {
...
async function classifySectionFamily({
  ...
  const stage1 = runStage1(title, articleContext);
  if (stage1) return stage1;
  if (typeof sourceText === 'string' && /"Marketing\s{1,3}Period"\s+(?:shall\s+)?means?\b/.test(sourceText)) {
    return {
      section_family: 'FINANCING_COVENANTS',
      provenance: 'SECTION_FAMILY_DEFINED_TERM_ANCHORED',
      classifier_version: SECTION_FAMILY_CLASSIFIER_VERSION,
      covenant_side: null,
      declined_reason: null,
    };
