...
const WORK_ITEM_TELEMETRY_SCHEMA = 'NATIVE_UNIFIED_RUN_WORK_ITEM_TELEMETRY/V1';
...
function validateRecomputedPromptBudgetSplitPreflights({
  extract_items: extractItems,
  admitted_by_id: admittedById,
  prompt_budget_split_preflights: promptBudgetSplitPreflights,
} = {}) {
  if (!Array.isArray(extractItems) || !(admittedById instanceof Map)
    || !Array.isArray(promptBudgetSplitPreflights)) {
    fail(
      'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
      'Prompt-budget split preflights require trusted work items and admitted sources for recomputation.',
      { reason: 'INVALID_RECOMPUTATION_INPUT' },
    );
  }
  const preflightByOrigin = new Map();
  for (const preflight of promptBudgetSplitPreflights) {
    try {
      validatePromptBudgetSplitPreflight(preflight);
    } catch (error) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        'A prompt-budget split preflight failed validation during trusted recomputation.',
        { reason: 'INVALID_SUPPLIED_PREFLIGHT', cause_code: error?.code || null },
      );
    }
    if (preflightByOrigin.has(preflight.origin_work_item_id)) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        'Trusted recomputation found duplicate prompt-budget split preflights for one work item.',
        { reason: 'DUPLICATE_ORIGIN', origin_work_item_id: preflight.origin_work_item_id },
      );
    }
    preflightByOrigin.set(preflight.origin_work_item_id, preflight);
  }
  if (preflightByOrigin.size !== extractItems.length) {
    fail(
      'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
      'Trusted recomputation requires one prompt-budget split preflight for every EXTRACT work item.',
  ...
  for (const item of extractItems) {
    const supplied = preflightByOrigin.get(item?.work_item_id);
    const admitted = admittedById.get(item?.source_id);
    const sectionReference = item?.section_reference ?? item?.section_pin?.section_reference;
    const sectionId = item?.section_id ?? item?.section_pin?.section_id;
    const sectionKind = item?.section_kind ?? item?.section_pin?.section_kind;
    const sectionTextSha256 = item?.section_text_sha256 ?? item?.section_pin?.section_text_sha256;
    if (!supplied || !admitted || admitted.source_id !== item.source_id
      || !admitted.context || typeof admitted.context.document_hash !== 'string'
      || typeof admitted.context.canonical_text?.text !== 'string'
      || !admitted.tree || !Array.isArray(admitted.tree.nodes)) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        `Trusted recomputation lacks an admitted source or preflight for ${item?.work_item_id || 'an EXTRACT work item'}.`,
        { reason: 'TRUSTED_INPUT_MISSING', origin_work_item_id: item?.work_item_id || null },
      );
    }
    const node = admitted.tree.nodes.find((candidate) => (
      candidate.reference === sectionReference
      && candidate.section_id === sectionId
      && candidate.kind === sectionKind
      && candidate.text_sha256 === sectionTextSha256
    ));
    if (!node) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        `The trusted section pin for ${item.work_item_id} does not identify one exact admitted tree node.`,
        { reason: 'SECTION_PIN_MISMATCH', origin_work_item_id: item.work_item_id },
      );
    }

    let recomputed;
    try {
      recomputed = buildPromptBudgetSplitPreflight({
        tree: admitted.tree,
        canonical_text: admitted.context.canonical_text.text,
        ...
        nodes: [node],
        prompt_budget_policy: { prompt_byte_ceiling: supplied.prompt_byte_ceiling },
        max_split_depth: supplied.max_split_depth,
      });
    } catch (error) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        `Prompt-budget recomputation failed for ${item.work_item_id}.`,
        {
          reason: 'RECOMPUTATION_FAILED',
          origin_work_item_id: item.work_item_id,
          cause_code: error?.code || null,
        },
      );
    }
    if (recomputed.status !== 'PASS' || canonicalJson(recomputed) !== canonicalJson(supplied)) {
      fail(
        'PROMPT_BUDGET_SPLIT_PREFLIGHT_RECOMPUTATION_MISMATCH',
        `The supplied prompt-budget split preflight for ${item.work_item_id} does not match trusted admitted bytes.`,
        {
          reason: recomputed.status === 'PASS' ? 'CANONICAL_MISMATCH' : 'RECOMPUTED_NON_PASS',
          origin_work_item_id: item.work_item_id,
          supplied_preflight_id: supplied.preflight_id,
          recomputed_preflight_id: recomputed.preflight_id,
          recomputed_status: recomputed.status,
        },
      );
    }
    recomputedIds.push(recomputed.preflight_id);
  }
  return Object.freeze({
    status: 'PASS',
    preflight_ids: Object.freeze(recomputedIds),
  });
}
...
function validateIteration2LiveLaunch({
  manifest,
  work_item_controls: workItemControls,
  live_work_item_profiles: liveWorkItemProfiles,
  live_work_item_context: liveWorkItemContext,
  live_work_item_controls: liveWorkItemControls,
  max_model_invocations: maxModelInvocations,
} = {}) {
  if (!liveWorkItemProfiles || typeof liveWorkItemProfiles !== 'object'
    || Array.isArray(liveWorkItemProfiles)
    || !liveWorkItemContext || typeof liveWorkItemContext !== 'object'
    || Array.isArray(liveWorkItemContext)
    || !liveWorkItemControls || typeof liveWorkItemControls !== 'object'
    || Array.isArray(liveWorkItemControls)
    || !Number.isSafeInteger(maxModelInvocations) || maxModelInvocations < 0) {
    fail('INVALID_ITERATION_2_POLICY', 'iteration 2 requires a sealed live-work policy and invocation limit.');
  }
  const expectedEntries = Object.entries(liveWorkItemProfiles)
  ...
  if (expectedEntries.length !== maxModelInvocations
    || expectedEntries.some(([, profileId]) => !PROFILES[profileId])
    || JSON.stringify(Object.keys(liveWorkItemContext).sort()) !== JSON.stringify(expectedIds)
    || JSON.stringify(Object.keys(liveWorkItemControls).sort()) !== JSON.stringify(expectedIds)
    || !manifest || !Array.isArray(manifest.work_items)
    || manifest.work_items.length !== expectedIds.length
    || JSON.stringify(actualIds) !== JSON.stringify(expectedIds)
    || extractItems.length !== expectedIds.length) {
    fail('ITERATION_2_WORK_ITEM_SET_MISMATCH', 'iteration 2 permits exactly its sealed declared live work items.');
  }
  for (const item of extractItems) {
    const expectedContext = liveWorkItemContext[item.work_item_id];
    if (!expectedContext || item.source_id !== expectedContext.source_id
      || item.family_id !== expectedContext.family_id) {
      fail('ITERATION_2_WORK_ITEM_SET_MISMATCH', 'iteration 2 work items must preserve the sealed source and family context.');
    }
  }
  ...
  for (const [workItemId, profileId] of expectedEntries) {
    ...
    const expectedControl = liveWorkItemControls[workItemId];
    if (!expectedControl || control?.profile_id !== profileId
      || expectedControl.profile_id !== profileId
      || control.covenant_side !== expectedControl.covenant_side) {
...
function jsonValueOrNull(value) {
  ...
  if (Array.isArray(value)) {
    const values = value.map(jsonValueOrNull);
    return values.some((entry) => entry === undefined) ? null : values;
  }
  if (!value || typeof value !== 'object' || Object.getPrototypeOf(value) !== Object.prototype) return null;
  const output = {};
  for (const [key, entry] of Object.entries(value)) {
    const normalised = jsonValueOrNull(entry);
...
function providerUsage(providerOutput) {
  const usage = providerOutput?.provider_usage ?? providerOutput?.usage ?? null;
  const normalised = jsonValueOrNull(usage);
  return normalised && typeof normalised === 'object' && !Array.isArray(normalised) ? normalised : null;
}

function providerRequestId(providerOutput) {
  for (const key of ['provider_request_id', 'request_id', 'requestId', 'response_id', 'responseId']) {
    if (typeof providerOutput?.[key] === 'string' && providerOutput[key].length > 0) {
      return providerOutput[key];
    }
  }
  return null;
}

function checkpointTelemetryContextId(context, workItemId) {
  return contentId(WORK_ITEM_TELEMETRY_SCHEMA, {
    validation_receipt_id: context.validation_receipt_id,
    semantic_manifest_id: context.semantic_manifest_id,
...
function buildWorkItemTelemetry({ item, workResult, checkpoint, context, timings, providerOutput, error }) {
  const failedProviderOutput = providerOutput || error?.details?.provider_output || null;
  const rawResponse = typeof failedProviderOutput?.raw_response_text === 'string'
    ? failedProviderOutput.raw_response_text
    : null;
  const attemptCount = Number.isSafeInteger(failedProviderOutput?.attempts) && failedProviderOutput.attempts > 0
    ? failedProviderOutput.attempts
    : (Number.isSafeInteger(error?.details?.attempts) && error.details.attempts > 0
      ? error.details.attempts : null);
  const body = {
    schema_version: WORK_ITEM_TELEMETRY_SCHEMA,
    work_item_id: item.work_item_id,
    work_result_id: workResult.work_result_id,
    resumable_checkpoint_id: checkpoint.checkpoint_id,
    checkpoint_context_id: checkpointTelemetryContextId(context, item.work_item_id),
    status: workResult.status,
    failure_code: workResult.failure_code,
    timing: {
      work_started_at: timings.work_started_at,
      work_completed_at: timings.work_completed_at,
      work_duration_ms: timings.work_duration_ms,
      provider_started_at: timings.provider_started_at,
      provider_completed_at: timings.provider_completed_at,
      provider_duration_ms: timings.provider_duration_ms,
    },
    attempts: {
      provider_invocation_count: timings.provider_invocation_count,
      provider_attempt_count: attemptCount,
      provider_retry_count: attemptCount === null ? null : attemptCount - 1,
    },
    provider: {
      provider_id: typeof failedProviderOutput?.provider_id === 'string'
        ? failedProviderOutput.provider_id : null,
      model_id: typeof failedProviderOutput?.model_id === 'string'
        ? failedProviderOutput.model_id : null,
      provider_request_id: providerRequestId(failedProviderOutput),
      provider_usage: providerUsage(failedProviderOutput),
    },
    raw_response: rawResponse === null ? null : {
      sha256: sha256Hex(rawResponse),
      byte_length: utf8ByteLength(rawResponse),
    },
    counts: {
      provider_candidate_count: Array.isArray(failedProviderOutput?.proposals)
        ? failedProviderOutput.proposals.length : null,
      provider_evidence_residual_count: Array.isArray(failedProviderOutput?.evidence_residuals)
        ? failedProviderOutput.evidence_residuals.length : null,
      compiled_candidate_count: workResult.run_receipt?.compiled_candidate_count ?? null,
      rejected_candidate_count: workResult.run_receipt?.rejected_candidate_count ?? null,
      evidence_residual_count: workResult.run_receipt?.evidence_residual_count ?? null,
      citation_residual_count: workResult.run_receipt?.citation_residual_count ?? null,
      scope_violation_count: workResult.run_receipt?.scope_violation_count ?? null,
    },
  };
  return Object.freeze({
    ...body,
    work_item_telemetry_id: contentId(WORK_ITEM_TELEMETRY_SCHEMA, body),
  });
}

function attachCheckpointTelemetry(checkpoint, telemetry) {
  return Object.freeze({ ...checkpoint, telemetry });
}

function validateCheckpointTelemetry(telemetry, checkpoint, item, context) {
  const keys = [
    'schema_version', 'work_item_id', 'work_result_id', 'resumable_checkpoint_id', 'checkpoint_context_id',
    'status', 'failure_code', 'timing', 'attempts', 'provider', 'raw_response', 'counts',
    'work_item_telemetry_id',
  ];
  if (!exactKeys(telemetry, keys)
    || telemetry.schema_version !== WORK_ITEM_TELEMETRY_SCHEMA
    || telemetry.work_item_id !== item.work_item_id
    || telemetry.work_result_id !== checkpoint.work_result.work_result_id
    || telemetry.resumable_checkpoint_id !== checkpoint.checkpoint_id
    || telemetry.checkpoint_context_id !== checkpointTelemetryContextId(context, item.work_item_id)
    || telemetry.status !== checkpoint.work_result.status
    || telemetry.failure_code !== checkpoint.work_result.failure_code) {
    fail('INVALID_RESUME_CHECKPOINT', `checkpoint for ${item.work_item_id} has invalid telemetry bindings.`);
  }
  const body = { ...telemetry };
  delete body.work_item_telemetry_id;
  if (telemetry.work_item_telemetry_id !== contentId(WORK_ITEM_TELEMETRY_SCHEMA, body)) {
    fail('INVALID_RESUME_CHECKPOINT', `checkpoint for ${item.work_item_id} has a telemetry identity mismatch.`);
  }
}
...
function validateResumeCheckpoints(checkpoints, extractItems, controlsById, context) {
  ...
  checkpoints.forEach((checkpoint, index) => {
    if (!exactKeys(checkpoint, ['schema_version', 'validation_receipt_id', 'semantic_manifest_id', 'execution_plan_id', 'contract_fingerprint', 'work_result', 'checkpoint_id', 'telemetry'])
    ...
    delete checkpointBody.telemetry;
    ...
    validateCheckpointTelemetry(checkpoint.telemetry, checkpoint, item, context);
...
async function executeUnifiedRun({
  ...
  if (!Number.isSafeInteger(maxModelInvocations) || maxModelInvocations < 0) {
    fail('INVALID_MAX_MODEL_INVOCATIONS', 'max_model_invocations must be an explicit non-negative safe integer derived from the sealed execution manifest.');
  }
  if (promptBudgetSplitPreflights !== undefined) {
    validatePromptBudgetSplitPreflightFence({
      manifest,
      prompt_budget_split_preflights: promptBudgetSplitPreflights,
    });
  ...
  if (promptBudgetSplitPreflights === undefined) {
    fail(
      ...
      'Trusted unified-run execution requires one PASS prompt-budget split preflight for every EXTRACT work item.',
    );
  }
  ...
  const newWorkResults = await mapConcurrent(pendingItems, maxConcurrency, async (item) => {
    ...
    const recordingProvider = async (input) => {
      ...
      timings.provider_invocation_count += 1;
      timings.provider_started_at = new Date().toISOString();
      const providerStartedMs = Date.now();
      try {
        providerOutput = await providerFor(control.profile_id)(input);
        return providerOutput;
      } finally {
        timings.provider_completed_at = new Date().toISOString();
        timings.provider_duration_ms = Date.now() - providerStartedMs;
      }
    };
    let workResult;
    try {
    ...
    timings.work_completed_at = new Date().toISOString();
    timings.work_duration_ms = Date.now() - workStartedMs;
    const coreCheckpoint = buildCheckpoint(workResult, context);
    const checkpoint = attachCheckpointTelemetry(coreCheckpoint, buildWorkItemTelemetry({
      item,
      workResult,
