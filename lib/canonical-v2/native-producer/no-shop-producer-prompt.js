 ...
 * Go-shop windows, intervening-event machinery, DADW/standstill waivers,
 * and superior-/acquisition-proposal DEFINITIONS are all expressly named
 * below as open-world-for-now (spec "Out of scope"). A sentence stating two
...
const PROMPT_ID = 'native-producer-no-shop/v1';
...
/**
 * The response contract (spec section 3): four top-level arrays. Enum-
...
PRESERVE THE NOVEL. Go-shop windows, intervening-event machinery, don't-ask-don't-waive/standstill-waiver provisions, and Superior Proposal / Acquisition Proposal DEFINITIONS themselves are all open-world-for-now -- do not force any of them into the arrays above. If the text asserts something you cannot express in the shape above without distorting it, put it in open_world_candidates with the verbatim text and say what it resembles. Promotion into a typed array narrows what gets governed; it never forces a doubtful reading into a controlled shape.

CONTROLLED CODES ONLY. Where a field takes a code (action_code, prerequisite_code, period_role), use one from the supplied vocabularies exactly, or null (for action_code/prerequisite_code only -- period_role has no null: if you cannot tell the role, use open_world_candidates instead). Never invent a code.
