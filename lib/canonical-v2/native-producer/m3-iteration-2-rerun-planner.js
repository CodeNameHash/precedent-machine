...
} = require('./recorded-provider-response-replay');
...
function validateCheckpointTelemetry(checkpoint, result) {
  if (checkpoint.telemetry === undefined) return;
  ...
  const body = telemetry && { ...telemetry };
  if (body) delete body.work_item_telemetry_id;
  if (!exactKeys(telemetry, keys)
    || telemetry.schema_version !== WORK_ITEM_TELEMETRY_SCHEMA
    || telemetry.work_item_id !== result.work_item_id
    || telemetry.work_result_id !== result.work_result_id
    || telemetry.resumable_checkpoint_id !== checkpoint.checkpoint_id
    || telemetry.status !== result.status
    || telemetry.failure_code !== result.failure_code
    || telemetry.work_item_telemetry_id !== contentId(WORK_ITEM_TELEMETRY_SCHEMA, body)) {
    fail('INVALID_CHECKPOINT', 'A first-pass checkpoint has invalid durable work-item telemetry.');
  }
}
...
function loadCurrentCheckpoints(directory, executionResult) {
  ...
  const checkpointRows = files.map((entry) => {
    ...
    const acceptedKeys = checkpoint && checkpoint.telemetry === undefined ? keys : [...keys, 'telemetry'];
    if (!exactKeys(checkpoint, acceptedKeys) || checkpoint.schema_version !== CHECKPOINT_SCHEMA
    ...
    validateCheckpointTelemetry(checkpoint, result);
...
function validateIteration2RerunPlan({ plan } = {}) {
  const keys = [
    'schema_version', 'pilot_work_item_set_id', 'first_execution_result_id', 'quality_gate_id',
    'preserved_checkpoint_set', 'archived_replaced_work_items', 'replay_only_work_items',
    'live_terra_work_items', 'live_sol_work_items', 'new_controls', 'controls',
    'iteration_2_rerun_plan_id',
  ];
  if (!exactKeys(plan, keys) || plan.schema_version !== ITERATION_2_RERUN_PLAN_SCHEMA
    || plan.pilot_work_item_set_id !== PILOT_WORK_ITEM_SET_ID
    || !isDigest(plan.iteration_2_rerun_plan_id)) {
    fail('INVALID_SEALED_RERUN_PLAN', 'The iteration 2 plan must match the sealed rerun-plan contract.');
  }
  const body = { ...plan };
  delete body.iteration_2_rerun_plan_id;
  if (contentId(ITERATION_2_RERUN_PLAN_SCHEMA, body) !== plan.iteration_2_rerun_plan_id) {
    fail('INVALID_SEALED_RERUN_PLAN', 'The iteration 2 plan identity does not match its contents.');
  }
  if (!exactKeys(plan.controls, [
    'first_pass_artifact_mutation', 'checkpoint_deletion', 'original_manifest_work_item_count',
    'replay_model_call_count', 'live_model_call_count',
    ...
    || !Number.isSafeInteger(plan.controls.live_model_call_count)
    || plan.controls.live_model_call_count < 0) {
    fail('INVALID_SEALED_RERUN_PLAN', 'The iteration 2 plan must forbid first-pass mutation and replay model calls.');
  }
  const liveProfiles = new Map();
  const liveContexts = new Map();
  function registerLive(rows, profileId, label) {
    if (!Array.isArray(rows)) fail('INVALID_SEALED_RERUN_PLAN', `${label} must be an array.`);
    for (const row of rows) {
      if (!exactKeys(row, ['work_item_id', 'source_id', 'family_id'])
        || !nonEmptyText(row.work_item_id) || !nonEmptyText(row.source_id)
        || !nonEmptyText(row.family_id) || liveProfiles.has(row.work_item_id)) {
        fail('INVALID_SEALED_RERUN_PLAN', `${label} must contain unique complete work items.`);
      }
      liveProfiles.set(row.work_item_id, profileId);
      liveContexts.set(row.work_item_id, Object.freeze({
        source_id: row.source_id,
        family_id: row.family_id,
      }));
    }
  }
  registerLive(plan.live_terra_work_items, 'TERRA_MEDIUM', 'live_terra_work_items');
  registerLive(plan.live_sol_work_items, 'SOL_HIGH', 'live_sol_work_items');
  if (plan.controls.live_model_call_count !== liveProfiles.size) {
    fail('INVALID_SEALED_RERUN_PLAN', 'The live model-call limit must equal the exact declared live work-item set.');
  }
  const replayIds = new Set();
  if (!Array.isArray(plan.replay_only_work_items)) {
    fail('INVALID_SEALED_RERUN_PLAN', 'replay_only_work_items must be an array.');
  }
  for (const row of plan.replay_only_work_items) {
    if (!exactKeys(row, [
      'work_item_id', 'source_id', 'family_id', 'first_work_result_id', 'preserved_checkpoint_id',
      'provider_recording_id', 'model_call_count',
    ]) || !nonEmptyText(row.work_item_id) || !nonEmptyText(row.source_id)
      || !nonEmptyText(row.family_id) || !isDigest(row.first_work_result_id)
      || !isDigest(row.preserved_checkpoint_id) || !isDigest(row.provider_recording_id)
      || row.model_call_count !== 0 || replayIds.has(row.work_item_id)
      || liveProfiles.has(row.work_item_id)) {
      fail('INVALID_SEALED_RERUN_PLAN', 'Replay-only items must be unique, sealed and excluded from live model calls.');
    }
    replayIds.add(row.work_item_id);
  }
  if (!exactKeys(plan.new_controls, ['schema_version', 'work_item_controls'])
    || plan.new_controls.schema_version !== CONTROLS_SCHEMA
    || !Array.isArray(plan.new_controls.work_item_controls)) {
    fail('INVALID_SEALED_RERUN_PLAN', 'The iteration 2 plan must retain its closed work-item controls.');
  }
  const controlsById = new Map();
  for (const control of plan.new_controls.work_item_controls) {
    if (!exactKeys(control, ['work_item_id', 'profile_id', 'covenant_side'])
      || !nonEmptyText(control.work_item_id) || controlsById.has(control.work_item_id)) {
      fail('INVALID_SEALED_RERUN_PLAN', 'The iteration 2 plan has invalid duplicate work-item controls.');
    }
    controlsById.set(control.work_item_id, control);
  }
  for (const [workItemId, profileId] of liveProfiles) {
    if (controlsById.get(workItemId)?.profile_id !== profileId) {
      fail('INVALID_SEALED_RERUN_PLAN', 'A live work-item profile does not match the sealed controls.');
    }
  }
  const liveWorkItemControls = Object.fromEntries([...liveProfiles].map(([workItemId, profileId]) => [
    workItemId,
    Object.freeze({ profile_id: profileId, covenant_side: controlsById.get(workItemId).covenant_side }),
  ]));
  return Object.freeze({
    iteration_2_rerun_plan_id: plan.iteration_2_rerun_plan_id,
    first_execution_result_id: plan.first_execution_result_id,
    live_work_item_profiles: Object.freeze(Object.fromEntries(liveProfiles)),
    live_work_item_context: Object.freeze(Object.fromEntries(liveContexts)),
    live_work_item_controls: Object.freeze(liveWorkItemControls),
    replay_work_item_ids: Object.freeze([...replayIds].sort()),
    max_model_invocations: plan.controls.live_model_call_count,
  });
}
...
async function replayRetainedProviderOutput({
  ...
  const runReceipt = await runNativeExtraction({
    ...
    source_tree_limb_citations: item.family_id === 'CAPITALISATION',
    contract_bundle: contractBundle,
    definitions,
    provider: async ({ governed_scope: governedScope }) => {
      try {
        return reshapeRecordedProviderOutput({
          provider_output: prior.provider_recording.provider_output,
          section_family: item.family_id,
          source_text: governedScope.source_text,
        });
      } catch (error) {
        if (error instanceof RecordedProviderReplayError) fail(error.code, error.message, error.details);
        throw error;
      }
    },
  });
