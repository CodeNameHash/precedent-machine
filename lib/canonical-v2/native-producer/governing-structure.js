...
function charToByteTable(text) {
  ...
  for (let i = 0; i < text.length;) {
    ...
    // The segmenter reports UTF-16 indices. Keep a table entry for each
    // code-unit boundary while charging a surrogate pair once as UTF-8.
    for (let boundary = 1; boundary <= width; boundary += 1) {
      table[i + boundary] = byteOffset;
    }
    i += width;
  }
  return table;
