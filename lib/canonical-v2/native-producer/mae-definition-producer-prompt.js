...
const PROMPT_ID = 'mae-definition-producer/v1';
 ...
 * Metsera), each carrying three nested lists (prong / carveout /
 * disproportionality assertions), plus the shared `open_world_candidates`.
 */
...
DISPROPORTIONALITY CARVEBACK -- ITS OWN OBJECT, EVEN IF ITS WORDS SIT INSIDE A CARVE-OUT CLAUSE'S OWN SENTENCE. Many agreements draft the disproportionality proviso as a tail clause of the very sentence that closes the carve-out enumeration ("... provided, however, that in the case of the foregoing clauses (a), (b) ... except to the extent such matters disproportionately and adversely impact the Company ..."). Even then, emit a SEPARATE disproportionality_assertions object -- do not treat it as already covered by the carve-out assertions above. applies_to_clause_labels is the array of verbatim clause labels the carveback claws back (may legitimately be empty if the carveback is drafted without any clause enumeration at all -- "except to the extent any of the foregoing ..."). comparison_baseline_phrase is REQUIRED whenever you emit this assertion at all -- the verbatim comparison class the disproportion is measured against. incremental_impact_phrase is OPTIONAL: include it only when the drafting expressly limits the carveback to the INCREMENTAL disproportionate impact; set it to null when that limitation is not present in the text -- its absence from a claim asserts nothing about the drafting (never invent or infer it).
