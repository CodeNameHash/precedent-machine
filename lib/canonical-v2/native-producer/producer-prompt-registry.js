 ...
 *
 * ONE ENTRY WHEN THIS SEAM WAS BUILT, 25 NOW. The existing capitalisation
 * producer was registered as the first, and at the time this header was
 * first written the slice's only, entry: `CAPITALISATION ->
 * buildCapitalisationProducerPrompt`. Its behavior was completely
 * unchanged by that move -- this module only relocated *how the prompt
 * builder is selected*, never *what it does*. Recorded fixture replays
 * (F28/TopBuild, Skechers, Modiv) still produce byte-identical output
 * routed through this registry; that was a pure refactor, not a behavior
 * change (see native-extraction-run.js's own header note on "PURE
 * REFACTOR THROUGH THE REGISTRY"). Every family registered since (see
 * REGISTRY below -- 25 entries as of this writing) followed the same
 * rule and this header has not always kept up with the count; check
 * REGISTRY itself, not this sentence, before relying on a total.
 *
 * Future families register their own prompt module here, in their own
 * reviewed diff, and nowhere else -- this file is the only place a
 * section_family is ever mapped to a producer.
 */
...
const { buildFinancingProducerPrompt } = require('./financing-producer-prompt');
const { buildGuarantyProducerPrompt } = require('./guaranty-producer-prompt');
const { buildEmployeeMattersProducerPrompt } = require('./employee-matters-producer-prompt');
const { buildDnoProducerPrompt } = require('./dno-producer-prompt');
const { buildTaxMattersProducerPrompt } = require('./tax-matters-producer-prompt');
const { buildDividendsProducerPrompt } = require('./dividends-producer-prompt');
const { buildAppraisalProducerPrompt } = require('./appraisal-producer-prompt');
const { buildRemediesMiscProducerPrompt } = require('./remedies-misc-producer-prompt');
const { buildSpecificPerformanceRemediesProducerPrompt } = require('./specific-performance-remedies-producer-prompt');
const { buildDefinedTermsProducerPrompt } = require('./defined-terms-producer-prompt');
const { buildRepresentationsProducerPrompt } = require('./representations-producer-prompt');
const { buildConsiderationProducerPrompt } = require('./consideration-producer-prompt');
const { buildClosingConditionsProducerPrompt } = require('./closing-conditions-producer-prompt');
const { buildProxyMeetingProducerPrompt } = require('./proxy-meeting-producer-prompt');
const { buildIocProducerPrompt } = require('./ioc-producer-prompt');
const { buildMaterialContractsProducerPrompt } = require('./material-contracts-producer-prompt');
const { buildNoOtherRepsFraudProducerPrompt } = require('./no-other-reps-fraud-producer');
const { buildGeneralCovenantsProducerPrompt } = require('./general-covenants-producer-prompt');
...
const REGISTRY = new Map([
  ...
  ['FINANCING_COVENANTS', buildFinancingProducerPrompt],
  ['GUARANTY_FINANCING_PARTY', buildGuarantyProducerPrompt],
  ['EMPLOYEE_MATTERS', buildEmployeeMattersProducerPrompt],
  ['DNO_INDEMNIFICATION', buildDnoProducerPrompt],
  ['TAX_MATTERS', buildTaxMattersProducerPrompt],
  ['DIVIDENDS', buildDividendsProducerPrompt],
  ['APPRAISAL_DISSENTERS_RIGHTS', buildAppraisalProducerPrompt],
  ['SPECIFIC_PERFORMANCE_REMEDIES', buildSpecificPerformanceRemediesProducerPrompt],
  ['MISC_BOILERPLATE', buildRemediesMiscProducerPrompt],
  ['KEY_DEFINED_TERMS', buildDefinedTermsProducerPrompt],
  ['REPRESENTATIONS', buildRepresentationsProducerPrompt],
  ['CONSIDERATION', buildConsiderationProducerPrompt],
  ['CLOSING_CONDITIONS', buildClosingConditionsProducerPrompt],
  ['PROXY_MEETING', buildProxyMeetingProducerPrompt],
  ['INTERIM_OPERATING', buildIocProducerPrompt],
  ['MATERIAL_CONTRACTS', buildMaterialContractsProducerPrompt],
  ['NO_OTHER_REPS_FRAUD', buildNoOtherRepsFraudProducerPrompt],
  ['GENERAL_COVENANTS', buildGeneralCovenantsProducerPrompt],
]);
