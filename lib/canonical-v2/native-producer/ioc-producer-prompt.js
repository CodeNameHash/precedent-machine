...
const PROMPT_ID = 'native-producer-interim-operating/v3';
const IOC_PROMPT_VERSION = 3;
...
ATTACHMENT. Shared qualifiers and exceptions attach to the narrowest restriction limb that their wording governs. Use PARENT_COVENANT only when the wording governs every limb in the parent covenant. Never duplicate one qualifier at both levels.

NUMERIC SHAPE. Preserve the exact value, unit, basis and period phrases. An explicit monetary scale word may be normalised for comparison: for example, $25 million means 25000000 USD while the exact "$25 million" text remains evidence. Do not calculate formula-derived amounts, annualise, aggregate an unstated period, or convert foreign currency without a governed rate and date.
