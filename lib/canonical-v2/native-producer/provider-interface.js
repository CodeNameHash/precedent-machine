 ...
 * @param {string} [args.section_family] registered family selected for this scope
 * @param {(input: {governed_scope, definitions, contract_bundle, section_family?}) =>
...
async function produceCandidateProposals({
  ...
  if (section_family !== undefined) {
    requireNonEmptyString(section_family, 'section_family');
  }
  ...
  const providerOutput = await provider({
    ...
    definitions,
    contract_bundle,
    ...(section_family === undefined ? {} : { section_family }),
  });
