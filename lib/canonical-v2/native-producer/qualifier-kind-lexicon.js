...
const QUALIFIER_KIND_LEXICON_VERSION = 1;

const QUALIFIER_KINDS = Object.freeze(['KNOWLEDGE', 'TEMPORAL', 'ACCURACY', 'THRESHOLD']);
...
const TEMPORAL_CALENDAR_DATE_PATTERN = new RegExp(
  `^${TEMPORAL_CALENDAR_MONTH}\\s+\\d{1,2},\\s*\\d{4}`,
  'i'
);
...
// DEVIATION (documented): the spec states the MAT_MATERIAL_TO_COMPANY and
// MAT_MAE_QUALIFIED pairs descriptively ("the whole-rep 'except as would
// not be material to the Company' form", "the MAE-qualified form") without
// pinning an exact phrase the way it does for MAT_ALL_RESPECTS /
// MAT_ALL_MATERIAL. The phrases below are this implementation's best-faith
// reading and are flagged here for Fable/Ben governance review -- per the
// module header, whitelist edits are identity-semantics changes.
...
function attemptSplit(quote, binding, effectiveOccurrences) {
  ...
  // Merge into ordered "regions" (consecutive occurrences already sorted).
  const regions = effectiveOccurrences;
  const parts = [];
  let cursor = 0;
  for (let i = 0; i < regions.length; i += 1) {
    ...
    if (!next) {
      ...
      // Look for a depth-0-relative comma/semicolon between region.end and
      // next.start; if present, that punctuation stays with THIS part.
      let boundary = null;
      for (let k = region.end; k < next.start; k += 1) {
        if (normalised[k] === ',' || normalised[k] === ';') {
          boundary = k + 1;
        }
      }
...
function classifyQualifierQuote(args) {
  ...
  if (familySet.size === 1) {
    ...
    // Disagreement: asymmetric routing.
    if (family === 'ACCURACY' || modelKind === 'ACCURACY') {
      return Object.freeze({
        outcome: 'REVIEW',
...
function doubtOutcome(families, modelKind) {
  const touchesAccuracy = families.includes('ACCURACY') || modelKind === 'ACCURACY';
  if (touchesAccuracy) {
    return Object.freeze({
      outcome: 'REVIEW',
      reason: QUALIFIER_KIND_DISAGREEMENT,
