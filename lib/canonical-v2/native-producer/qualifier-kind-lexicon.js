...
const QUALIFIER_KIND_LEXICON_VERSION = 3;
...
const ACCURACY_CODES = Object.freeze({
  ...
  // Stage 3: consumed from lib/taxonomy.js's own split. MAT_MATERIAL_INLINE
  // is "in any material respect" and other inline-modifier materiality
  // (materiality as an element of the rep's content); MAT_MAE_AGGREGATE is
  // the "individually or in the aggregate" MAE-qualifier form, kept
  // DISTINCT from the bare MAT_MAE_QUALIFIED per the same never-alias
  // ruling.
  MAT_MATERIAL_INLINE: 'MAT_MATERIAL_INLINE',
  MAT_MAE_AGGREGATE: 'MAT_MAE_AGGREGATE',
});
...
// Code split per the never-alias ruling: the "individually or in the
// aggregate" form is MAT_MAE_AGGREGATE, the bare form MAT_MAE_QUALIFIED --
// both consumed from lib/taxonomy.js, which already carries the pair.
...
const MAE_QUALIFIER_AGGREGATE_PATTERN = /individually\s+or\s+in\s+the\s+aggregate/i;
...
function classifyQualifierQuote(args) {
  ...
  // Step 2.5 (Stage 3, lexicon version 3): two ACCURACY front doors, both
  // whole-quote anchored, both BEFORE the binding pipeline for the same
  // reason the P2 doors are -- they must not perturb bound-clause
  // boundaries on legacy quotes. A definite, differing identity-bearing
  // model hint still routes to review, never overridden.
  const frontDoorAccuracyCode = (() => {
    ...
    if (MAE_QUALIFIER_IDIOM_PATTERN.test(normalisedTrimmed)) {
      return MAE_QUALIFIER_AGGREGATE_PATTERN.test(normalisedTrimmed)
        ? ACCURACY_CODES.MAT_MAE_AGGREGATE
        : ACCURACY_CODES.MAT_MAE_QUALIFIED;
    }
    return null;
  })();
