 ...
 *  containment on existing evidence spans. Review rows retain the source
 *  claim's exact evidence array at top level, so annotation never re-finds
 *  a quote in document text. Never mints claim identity,
 *  never rewrites claim_revision_id, never gates model input.
 *
 *  Step 2X free-phase (DECISIONS.md §15): `findTerminationLimbChapeau`'s
 *  structural half (paragraph-initial lettered marker + head-bound at
 *  `:`/`;`/newline) calls `resolveTerminationLimbChapeauViaStructure`.
 *  Fail-closed / UNDETERMINED from the service becomes null — never a
 *  guessed chapeau. Direction grammar (`parseTerminationLimbDirection`)
 *  and capacity comparison stay in this file.
 *
 ...
 *
 * Stage 2Y adds one explicit opt-in to that path: `corroboration_rung` may
 * select deterministic normalisation or anchor-stem corroboration for
 * General Covenants. The default remains rung 0. A loosened match becomes a
 * resolved claim and a review task. It does not become publishable here.
 *
...
const { MANIFEST: REGISTRY_SUBSTRATE_MANIFEST } = require('../../vocab/resolution/registry-substrate-manifest');
const {
  PARTY_CAPACITY_LEXICON,
  PARTY_CAPACITY_SEGMENT_LEXICON,
  PARTY_CAPACITY_SIDE_OF,
  JOINT_MULTI_PARTY_CAPACITY,
} = require('../../vocab/resolution/party-capacity-registry');
const {
  CAPITAL_STRUCTURE_MATERIALITY_TIER,
  MATERIALITY_TABLE,
  GENERAL_COVENANT_PREFIX_TIER,
  UNCLASSIFIED_MATERIALITY,
  MATERIALITY_DEFINITION_KEY_OVERRIDES,
  KNOWLEDGE_STANDARD_PATTERNS,
...
} = require('../../vocab/resolution/material-contract-registry');
...
const {
  NUMERAL_LADDER_RUNGS,
  parseDurationAtNumeralRung,
  validateNumeralLadderRung,
  wordsToNumber,
  trailingNumberWordsPhrase,
} = require('../../normalize-numeric');
const { TERMF_TRIGGER_META } = require('../../taxonomy');
...
const {
  LADDER_VERSION: CORROBORATION_LADDER_VERSION,
  MAX_RUNTIME_CORROBORATION_RUNG,
  assessDeterministicCorroborationAtRung,
} = require('./corroboration-ladder');
const {
  rubricPresentationCodeMatches,
} = require('../../vocab/general-covenant-rubric-presentation');
const {
...
const TAX_COOPERATION_LEGACY_OPTS = Object.freeze({ taxCooperationLegacyKindMatches });
const {
  annotateResolutionStructureContexts,
  canonicalEvidenceForEntry,
} = require('./structure-placement');
// Step 2X free-phase (DECISIONS.md §15): structural half of the
// termination limb chapeau finder adapts onto governing-structure.js.
// Direction grammar + capacity comparison stay in this file.
const {
  buildSectionOutline,
...
const {
  SCHEMA_VERSION: CANDIDATE_GOVERNING_CONTEXT_SCHEMA,
  createCandidateGoverningContext,
} = require('./candidate-governing-context');
const {
  buildSemanticSpan,
...
const {
  OUTCOMES: SAME_DEAL_DEFINED_TERM_OUTCOMES,
  validateSameDealDefinedTermIndex,
  resolveSameDealDefinedTerm,
} = require('./same-deal-defined-term-resolution');
...
const {
  MODES: DUPLICATE_SUPPRESSION_MODES,
  applyDuplicateSuppression,
  duplicateSuppressionReceipt: buildDuplicateSuppressionReceipt,
...
} = require('./representation-topic-ladder');

const REPRESENTATION_QUALIFIER_DISPATCH_MODES = Object.freeze({
  OFF: 'OFF',
  REPORT_ONLY: 'REPORT_ONLY',
  ENFORCE: 'ENFORCE',
});
const REPRESENTATION_QUALIFIER_LOOKBACK_SIGNAL = /^\s*(?:since|from)\b/i;
const CONTEXT_LADDER_SCHEMA = 'CONTEXT_LADDER/V1';
const CONTEXT_LADDER_MODES = Object.freeze({
  OFF: 'OFF',
  REPORT_ONLY: 'REPORT_ONLY',
});
const CONTEXT_RUNGS = Object.freeze({
  RENDERED_LINE: 0,
  PARAGRAPH: 1,
  ENUMERATED_SIBLING_GROUP: 2,
  WHOLE_SECTION: 3,
  SECTION_AND_FULL_CHAPEAU_CHAIN: 4,
});
const {
  QUALIFIER_CLAIM_KEY,
...
const MAPPING_TABLE_VERSION = 21;
...
function generalCovenantJointPartyCapacityOverride({ claim, section, admittedSourceContext }) {
  const attributes = claim?.attributes || {};
  if (attributes.covenant_code !== 'COV-TAKEOVER') return null;
  const actor = attributes.covenant_obligor;
  const quote = claim.raw_value;
  if (typeof actor !== 'string' || typeof quote !== 'string') return null;
  if (actor === 'None of the Parties' && quote.startsWith(`${actor} `)) {
    return JOINT_MULTI_PARTY_CAPACITY;
  }
  if (actor !== 'each of them') return null;
  const sectionText = utf8Slice(admittedSourceContext.canonical_text.text, section.start, section.end);
  const quoteIndex = sectionText.indexOf(quote);
  if (quoteIndex < 0 || sectionText.indexOf(quote, quoteIndex + quote.length) >= 0) return null;
  const governingPrefix = sectionText.slice(0, quoteIndex);
  return /\bNone of the Parties\b[^.]*,\s+and\s+$/.test(governingPrefix)
    ? JOINT_MULTI_PARTY_CAPACITY
    : null;
}
...
function sourceCitationForClaim(entry, claim) {
  return typeof claim?.source_citation === 'string' && claim.source_citation.length > 0
    ? claim.source_citation
    : sourceDerivedCitation(entry);
}
...
// admitted text. Structural half (paragraph-initial lettered marker;
// head-bound at first `:`/`;`/newline; fail closed on 0 or >1 markers;
// fail closed when governing-structure returns UNDETERMINED) lives in
// `resolveTerminationLimbChapeauViaStructure` — see DECISIONS.md §15 /
// Step 2X free-phase. This wrapper only maps the adapter's section-
// relative UTF-8 bytes onto the document-absolute `{ chapeau_text, span }`
// contract callers already consume. Direction grammar stays below in
// `parseTerminationLimbDirection`; capacity comparison stays in
// `findTerminationLimbGrantContext`. Never falls back to a local
// structural guess when the adapter returns null.
function findTerminationLimbChapeau({
  ...
  const sectionText = utf8Slice(admittedSourceContext.canonical_text.text, section.start, section.end);
  const viaStructure = resolveTerminationLimbChapeauViaStructure({
    sectionText,
    letter,
  });
  if (!viaStructure) return null;
  const start = section.start + viaStructure.start_byte;
  const end = section.start + viaStructure.end_byte;
  return Object.freeze({
    chapeau_text: viaStructure.chapeau_text,
    span: buildSemanticSpan(admittedSourceContext, start, end),
  });
}
...
const PROXY_MEETING_KIND_CORROBORATION = Object.freeze({
  ...
  ADJOURNMENT_REASON: /\b(?:quorum|insufficient\s+(?:affirmative\s+)?votes?|supplemental(?:\s+or\s+amended)?\s+disclosure|supplement\w*\s+(?:the\s+)?proxy|required\s+by\s+(?:applicable\s+)?law|legal\s+requirement)\b/i,
  PARENT_APPROVAL: /\b(?:obtain(?:ing)?\s+(?:the\s+)?Parent\s+(?:Stockholder|Shareholder)\s+Approval|Parent\s+(?:Stockholder|Shareholder)\s+Approval[\s\S]{0,120}(?:obtain|approve|adopt|vote|consent)\w*|(?:Parent(?:'s|’s)?\s+(?:stockholders?|shareholders?)|(?:stockholders?|shareholders?)\s+of\s+Parent)\s+(?:shall|will|must)\s+(?:approve|adopt|vote|consent)\w*)\b/i,
...
const PROXY_MEETING_COMPATIBLE_KIND_PAIRS = Object.freeze([
  ...
  // Concho 6.6 and TopBuild 4.5 state the meeting, convening duty and Parent
  // approval in one sentence. Each fact remains independently true.
  Object.freeze(['MEETING_DEADLINE', 'CONVENE_OBLIGATION', 'PARENT_APPROVAL']),
]);

function proxyMeetingKindsAreCompatible(kinds) {
  if (!Array.isArray(kinds) || kinds.length < 2 || new Set(kinds).size !== kinds.length) return false;
  return PROXY_MEETING_COMPATIBLE_KIND_PAIRS.some((compatibleKinds) => (
    compatibleKinds.length === kinds.length && compatibleKinds.every((kind) => kinds.includes(kind))
  ));
}
...
const APPROVAL_MECHANISM_PATTERNS = Object.freeze({
  ...
  SOLE_HOLDER_WRITTEN_CONSENT: /\bsole\s+(?:stock)?holder\b[\s\S]{0,80}\bwritten\s+consent\b/i,
...
// EITHER_PRINCIPAL_PARTY (spec section 4, audit C-1): a governed capacity
// value retained for non-termination families (regulatory mutual obligors,
// closing-condition beneficiaries). Termination mutual rights (Step 2X-I,
// Ben ruling 2) mint TWO assertion rows via resolveMutualPrincipalParties
// below -- one TARGET and one BUYER -- never a single EITHER row as product
// output.
...
// Ref-only either patterns (no leading "by ") for terminating_party_ref
// strings the model quotes without the grant verb.
const TERMINATION_EITHER_ONE_HAND_REF_PATTERN = new RegExp(
  `\\beither\\s+(${TERMINATION_PARTY_NAME_ALTERNATION}),?\\s*on\\s+the\\s+one\\s+hand,?\\s*or\\s+(${TERMINATION_PARTY_NAME_ALTERNATION}),?\\s*on\\s+the\\s+other\\s+hand\\b`, 'i',
);
const TERMINATION_PLAIN_EITHER_REF_PATTERN = new RegExp(
  `\\beither\\s+(${TERMINATION_PARTY_NAME_ALTERNATION})\\s+or\\s+(${TERMINATION_PARTY_NAME_ALTERNATION})\\b`, 'i',
);

function mintPrincipalTerminationParty(partyRef) {
  if (typeof partyRef !== 'string') return null;
  const trimmed = partyRef.trim();
  if (!trimmed) return null;
  const capacity = resolvePartyCapacity(trimmed);
  if (capacity !== 'TARGET' && capacity !== 'BUYER') return null;
  return Object.freeze({
    role: TERMINATION_RIGHT_HOLDER_ROLE,
    value: trimmed,
    capacity,
...
function pairFromEitherMatch(match) {
  const partyA = mintPrincipalTerminationParty(match[1]);
  const partyB = mintPrincipalTerminationParty(match[2]);
  if (!partyA || !partyB || partyA.capacity === partyB.capacity) return null;
  return Object.freeze([partyA, partyB]);
}

// Step 2X-I (Ben ruling 2): a mutual termination right mints one assertion
// row per principal party (TARGET and BUYER), never a single
// EITHER_PRINCIPAL_PARTY row. Returns null when the ref cannot be split
// into two distinct principal capacities -- callers queue, never fall back
// to mintEitherPrincipalParty.
function resolveMutualPrincipalParties(terminatingPartyRef, partySupportQuote) {
  const ref = typeof terminatingPartyRef === 'string' ? terminatingPartyRef.trim() : '';
  if (!ref) return null;
  const texts = [ref];
  if (typeof partySupportQuote === 'string' && partySupportQuote.trim()) texts.push(partySupportQuote.trim());
  const eitherPatterns = [
    TERMINATION_LIMB_EITHER_PATTERN,
    TERMINATION_LIMB_PLAIN_EITHER_PATTERN,
    TERMINATION_EITHER_ONE_HAND_REF_PATTERN,
    TERMINATION_PLAIN_EITHER_REF_PATTERN,
  ];
  for (const text of texts) {
    for (const pattern of eitherPatterns) {
      const match = pattern.exec(text);
      if (match) {
        const pair = pairFromEitherMatch(match);
        if (pair) return pair;
      }
    }
  }
  ...
  for (const segment of segmentPartyListString(ref)) {
    const party = mintPrincipalTerminationParty(segment);
    if (party && !seenCapacities.has(party.capacity)) {
      seenCapacities.add(party.capacity);
      parties.push(party);
    }
  }
  return parties.length === 2 && parties[0].capacity !== parties[1].capacity
...
const NO_SHOP_WAVE_B_CORROBORATION_TABLE = Object.freeze({
  CEASE_ACTION: Object.freeze({
    ...
    // Step 2X-G promotion (2026-08-08): corpus recurrence across six deals
    // (concho/metsera/redhat/skechers/skywater/topbuild). Accept instruct/
    // direct as well as request; also the bare "return (or destroy" cease
    // action shape (Metsera) that omits the request verb in the narrowed
    // quote. Still requires a return/destroy marker — never a bare instruct.
    REQUEST_RETURN_OR_DESTRUCTION_OF_INFORMATION: {
      test(quote) {
        if (typeof quote !== 'string' || quote.length === 0) return false;
        const hasReturnDestroy = /\b(?:return|destruction|destroy)\w*\b/i.test(quote);
        if (!hasReturnDestroy) return false;
        if (/\b(?:request|instruct|direct)\w*\b/i.test(quote)) return true;
        return /\breturn\b[\s\S]{0,80}\bdestroy/i.test(quote)
          || /\bdestroy\b[\s\S]{0,80}\breturn/i.test(quote);
      },
    },
  }),
  REPRESENTATIVE_CONTROL_STANDARD: Object.freeze({
 ...
 * @param {number} [args.corroboration_rung=0]    deterministic corroboration
 *   rung. Rung 0 preserves the current resolver. Rungs 1-2 may resolve an
 *   otherwise empty General Covenants check into the review queue. Model
 *   rungs 3-4 are not accepted by this resolver.
 * @returns {{
 *   resolved: object[],
 *   review_queue: object[],
 *   open_world: object[],
...
function resolveCandidates({
  ...
  // Step 2X-C report-only: when set, multi-code corroboration collisions
  // are recorded here and never change resolution behaviour.
  corroboration_collision_report: corroborationCollisionReportInput = null,
  // Stage 2Y is opt-in. An omitted index preserves the existing receipt and
  // every existing route byte-for-byte.
  same_deal_defined_terms: sameDealDefinedTermsInput = null,
  // ENFORCE is admitted only at this resolver-finalisation seam. The live
  // extraction runner still permits OFF and REPORT_ONLY only.
  duplicate_suppression_mode: duplicateSuppressionMode = DUPLICATE_SUPPRESSION_MODES.OFF,
  representation_qualifier_dispatch_mode: representationQualifierDispatchMode = REPRESENTATION_QUALIFIER_DISPATCH_MODES.OFF,
  representation_topic_ladder_mode: representationTopicLadderMode = REPRESENTATION_TOPIC_LADDER_MODES.OFF,
  representation_topic_rung: representationTopicRung = REPRESENTATION_TOPIC_LADDER_RUNGS.OFF,
  numeral_rung: numeralRung = NUMERAL_LADDER_RUNGS.LEGACY_DIGIT_ONLY,
  context_rung: contextRung = CONTEXT_RUNGS.RENDERED_LINE,
  context_ladder_mode: contextLadderMode = CONTEXT_LADDER_MODES.OFF,
  corroboration_rung: corroborationRung = 0,
} = {}) {
  ...
  const sameDealDefinedTerms = sameDealDefinedTermsInput == null
    ? null
    : validateSameDealDefinedTermIndex(sameDealDefinedTermsInput, runReceipt.document_hash);
  if (!Object.values(DUPLICATE_SUPPRESSION_MODES).includes(duplicateSuppressionMode)) {
    throw new TypeError('duplicate_suppression_mode must be OFF, REPORT_ONLY, or ENFORCE in resolveCandidates');
  }
  if (!Object.values(REPRESENTATION_QUALIFIER_DISPATCH_MODES).includes(representationQualifierDispatchMode)) {
    throw new TypeError('representation_qualifier_dispatch_mode must be OFF, REPORT_ONLY, or ENFORCE in resolveCandidates');
  }
  if (![REPRESENTATION_TOPIC_LADDER_MODES.OFF, REPRESENTATION_TOPIC_LADDER_MODES.REPORT_ONLY].includes(representationTopicLadderMode)) {
    throw new TypeError('representation_topic_ladder_mode must be OFF or REPORT_ONLY in resolveCandidates');
  }
  validateRepresentationTopicRung(representationTopicRung);
  if (representationTopicLadderMode === REPRESENTATION_TOPIC_LADDER_MODES.OFF
    && representationTopicRung !== REPRESENTATION_TOPIC_LADDER_RUNGS.OFF) {
    throw new TypeError('representation_topic_rung must be rung 0 when representation_topic_ladder_mode is OFF');
  }
  validateNumeralLadderRung(numeralRung);
  if (!Number.isInteger(contextRung) || contextRung < CONTEXT_RUNGS.RENDERED_LINE
    || contextRung > CONTEXT_RUNGS.SECTION_AND_FULL_CHAPEAU_CHAIN) {
    throw new TypeError('context_rung must be an integer from 0 through 4 in resolveCandidates');
  }
  if (![CONTEXT_LADDER_MODES.OFF, CONTEXT_LADDER_MODES.REPORT_ONLY].includes(contextLadderMode)) {
    throw new TypeError('context_ladder_mode must be OFF or REPORT_ONLY in resolveCandidates');
  }
  if (!Number.isInteger(corroborationRung) || corroborationRung < 0
    || corroborationRung > MAX_RUNTIME_CORROBORATION_RUNG) {
    throw new TypeError(`corroboration_rung must be an integer from 0 through ${MAX_RUNTIME_CORROBORATION_RUNG} in resolveCandidates`);
  }
  ...
  const candidateGoverningContext = createCandidateGoverningContext({
  ...
  let openWorld = [];
  const residuals = [];
  const representationQualifierDispatchDecisions = [];
  const representationTopicLadderRecords = [];
  const contextLadderRecords = [];
  const contextLadderRecordByCandidate = new Map();
  const relationships = [];
  ...
  function hasExpressExclusionCue(quote, detail) {
    let offset = quote.indexOf(detail);
    while (offset !== -1) {
      const prefix = quote.slice(Math.max(0, offset - 80), offset);
      if (/(?:\bother\s+than|\bexcluding|\bexcept(?:\s+for)?)\s*$/i.test(prefix)) return true;
      offset = quote.indexOf(detail, offset + detail.length);
    }
    return false;
  }
  function materialContractGroundingFailure(claim) {
    ...
    const scopeExclusions = attributes.scope_exclusions === undefined
      ? []
      : attributes.scope_exclusions;
    ...
    if (!Array.isArray(scopeExclusions)
      || scopeExclusions.some((value) => typeof value !== 'string' || !value.trim() || value !== value.trim())
      || new Set(scopeExclusions).size !== scopeExclusions.length
      || scopeExclusions.some((value) => !hasExpressExclusionCue(quote, value))) {
      return 'MATERIAL_CONTRACT_EXCLUSION_UNCORROBORATED';
    }
  ...
  const corroborationCollisionReport = corroborationCollisionReportInput == null
    ? null
    : requirePlainObject(corroborationCollisionReportInput, 'corroboration_collision_report');

  function recordCorroborationCollision(family, entry) {
    if (!corroborationCollisionReport) return;
    if (!Array.isArray(corroborationCollisionReport[family])) {
      corroborationCollisionReport[family] = [];
    }
    corroborationCollisionReport[family].push(Object.freeze(entry));
  }

  function generalCovenantCorroboration(claim) {
    const quote = String(claim?.raw_value || '');
    const code = claim?.attributes?.covenant_code;
    // Inject the Stage-4 HOLD scaffold matcher so general-covenant-
    // corroboration.js stays a zero-dependency PURE_ANALYSIS leaf.
    return corroborateGeneralCovenantCode({
      quote,
      code,
      rubricPresentationCodeMatches,
    });
  ...
  function generalCovenantGroundingFailure(claim) {
    ...
    // general-covenant-corroboration.js's own operative-text patterns first,
    // then lib/vocab/general-covenant-rubric-presentation.js on a primary
    // miss only (Step 2X-B). Rubric label/alias strings alone failed on
    // Modiv (Step 3G) but remain the legacy vocabulary for second chance.
    const corroboration = generalCovenantCorroboration(claim);
    if (corroboration.outcome !== 'RESOLVED') {
      const selected = corroborationRung === 0
        || corroboration.reason !== 'GENERAL_COVENANT_CODE_UNCORROBORATED'
        || (Array.isArray(corroboration.matches)
          && corroboration.matches.some((matchedCode) => matchedCode !== code))
        ? null : assessDeterministicCorroborationAtRung({
          family: 'GENERAL_COVENANTS',
          quote,
          code,
          selected_rung: corroborationRung,
        });
      if (selected?.derivation === 'MATCH') return null;
      return corroboration.reason === 'AMBIGUOUS_GENERAL_COVENANT_CODE'
        ? 'AMBIGUOUS_GENERAL_COVENANT_CODE'
    ...
    if (Object.hasOwn(waveBDefinitionByKind, kind)) {
      ...
      if (kind === 'STOCKHOLDER_APPROVAL') {
        ...
        const patterns = Object.freeze({
          ...
          PRIMARILY_CAUSED: /primarily caused/i,
  ...
  function handleIocRestrictionCandidate(entry) {
    ...
    if ((category === 'ACQUISITIONS_BUSINESS_COMBINATIONS' && /purchasing an equity interest in or portion of the assets|joint venture/i.test(claim.raw_value))
      || (category === 'MATERIAL_CONTRACTS' && /agree, authorize or commit to do any of the foregoing/i.test(claim.raw_value))) {
    ...
    if (!component) {
      const orderedSpans = runReceipt.compiled_candidates
        .filter((candidateEntry) => candidateEntry?.ok === true
          ...
          && normaliseRestrictionCategory(
            candidateEntry.candidate.claim.attributes?.restriction_category,
            candidateEntry.candidate.claim.raw_value,
          ) === category)
  ...
  function handleProxyMeetingCandidate(entry) {
    ...
    // These Boolean facts prove that an action exists. They do not identify
    // a specific meeting, so a null meeting_ref carries no missing meaning.
    const presenceFact = assertionKind === 'RECORD_DATE_ESTABLISHMENT'
      || assertionKind === 'BROKER_SEARCH_OBLIGATION';
    const refField = approvalKind || presenceFact
      ? null
  ...
  function recordRepresentationQualifierDispatch({ entry, claim, conceptKey, qualifierKind }) {
    if (representationQualifierDispatchMode === REPRESENTATION_QUALIFIER_DISPATCH_MODES.OFF
      || !['THRESHOLD', 'TEMPORAL'].includes(qualifierKind)) return;

    const record = (decision) => {
      const frozen = Object.freeze(decision);
      representationQualifierDispatchDecisions.push(frozen);
      return frozen;
    };

    const quote = claim.raw_value;
    const classification = classifyQualifierQuote({ quote, modelKind: qualifierKind });
    const base = {
      closure_id: claim.closure_id || null,
      section_reference: entry.section_reference,
    ...
    if (classification.outcome === 'REVIEW' && classification.reason === QUALIFIER_KIND_DISAGREEMENT) {
      return record({
        ...base,
        disposition: 'REVIEW',
        dispatch: 'MODEL_KIND_CONFLICTS_WITH_MAE_TOLERANCE',
        reason: QUALIFIER_KIND_DISAGREEMENT,
      });
    }
    if (qualifierKind === 'THRESHOLD'
      && classification.outcome === 'CLASSIFIED'
      && classification.kind === 'THRESHOLD') {
      return record({
        ...base,
        disposition: 'SAFE',
        dispatch: 'THRESHOLD_MATERIALITY',
        proposed_claim: Object.freeze({
    ...
    if (qualifierKind === 'TEMPORAL' && REPRESENTATION_QUALIFIER_LOOKBACK_SIGNAL.test(quote)) {
      const parsed = parseDefinedMeasurementDate(quote);
      if (parsed.outcome === 'RESOLVED') {
        return record({
          ...base,
          disposition: 'SAFE',
    ...
    if (qualifierKind === 'TEMPORAL'
      && classification.outcome === 'CLASSIFIED'
      && classification.kind === 'TEMPORAL'
      && classification.measurementDateEligible === true) {
      const parsed = parseDefinedMeasurementDate(quote);
      if (parsed.outcome === 'RESOLVED') {
        return record({
          ...base,
          ...
          proposed_claim: Object.freeze({
            ...
            concept_key: conceptKey,
            canonical_value: parsed.iso_date,
          }),
        });
      }
      if (parsed.outcome === 'PERIOD_DETECTED') parseMeasurementPeriod({ quote, agreement_date: agreementDate });
    }
    return record({
      ...base,
      disposition: 'HOLD',
      dispatch: qualifierKind === 'THRESHOLD' ? 'THRESHOLD_HELD' : 'TEMPORAL_HELD',
      reason: qualifierKind === 'THRESHOLD' ? 'THRESHOLD_FORM_NOT_GOVERNED' : 'TEMPORAL_FORM_NOT_GOVERNED',
    });
  }

  function recordRepresentationTopicLadder({ entry, claim }) {
    if (representationTopicLadderMode !== REPRESENTATION_TOPIC_LADDER_MODES.REPORT_ONLY
      || claim.claim_definition_key !== LIMB_ASSERTION_CLAIM_KEY) return;
    const section = findPinnedParentSection(sectionsByReference, entry.section_reference);
    if (!section || section.section_family !== 'REPRESENTATIONS'
      || !SECTION_FAMILY_REAL_PROVENANCES.has(section.section_family_provenance)) return;

    let classifierInput = {
      family: section.section_family,
      generic_claim_key: claim.claim_definition_key,
      raw_value: claim.raw_value,
      subject: claim.attributes?.subject ?? null,
      evidence: null,
    };
    if (representationTopicRung !== REPRESENTATION_TOPIC_LADDER_RUNGS.OFF) {
      ...
      if (typeof sourceText === 'string' && canonicalEvidence.status === 'OK') {
        ...
        if (sectionText != null) {
          classifierInput = { ...classifierInput, evidence: { source_text: sectionText, spans: canonicalEvidence.evidence } };
        }
      }
    }
    const classification = classifyRepresentationTopicAtRung(classifierInput, representationTopicRung);
    const party = resolveParty({
      attributes: claim.attributes,
      mapping: { party_field: 'party_making', party_role: 'REPRESENTATION_MAKER' },
    });
    const partySide = party ? sideForCapacity(party.capacity) : null;
    const representationSide = partySide === 'TARGET_SIDE'
      ? 'TARGET'
      : partySide === 'BUYER_SIDE'
        ? 'PARENT'
        : null;
    const conceptKey = representationSide === 'TARGET'
      ? 'REP-T-TOPIC'
      : representationSide === 'PARENT'
        ? 'REP-B-TOPIC'
        : null;
    const proposedClaim = classification.state === 'CLASSIFIED' && conceptKey
      ? Object.freeze({
      ...
      : null;
    representationTopicLadderRecords.push(Object.freeze({
      schema_version: REPRESENTATION_TOPIC_LADDER_SCHEMA,
      section_reference: entry.section_reference,
      section_family: section.section_family,
      section_family_provenance: section.section_family_provenance,
      original_claim_occurrence_id: claim.claim_occurrence_id || null,
      closure_id: claim.closure_id || null,
      raw_value: claim.raw_value,
      subject: claim.attributes?.subject ?? null,
      party,
      ...
      disposition: proposedClaim ? 'WOULD_CLASSIFY' : 'OPEN_WORLD',
      activation_blocker: 'ORDINARY_REVIEW_PENDING',
    }));
  }

  function recordsForContextRung(context) {
    if (context.status !== 'RESOLVED') return Object.freeze([]);
    if (contextRung === CONTEXT_RUNGS.RENDERED_LINE) return Object.freeze([context.rendered_line]);
    if (contextRung === CONTEXT_RUNGS.PARAGRAPH) return Object.freeze([context.leaf]);
    if (contextRung === CONTEXT_RUNGS.ENUMERATED_SIBLING_GROUP) {
      return Object.freeze([context.leaf, ...context.siblings]);
    }
    if (contextRung === CONTEXT_RUNGS.WHOLE_SECTION) return Object.freeze([context.section]);
    const deduped = new Map();
    for (const record of [context.section, ...context.chapeau]) {
      deduped.set(`${record.start_byte}:${record.end_byte}`, record);
    }
    return Object.freeze([...deduped.values()]);
  }

  function recordContextLadder({ entry, claimRow, route, reason, sourceCitation }) {
    if (contextLadderMode !== CONTEXT_LADDER_MODES.REPORT_ONLY
      || contextLadderRecordByCandidate.has(entry)) return;
    const context = candidateGoverningContext.forCandidate({ entry, claim: claimRow });
    const host = context.status === 'RESOLVED'
      ? (context.item || context.section)
      : null;
    const record = Object.freeze({
      schema_version: CONTEXT_LADDER_SCHEMA,
      ...
      original_claim_occurrence_id: claimRow.claim_occurrence_id || null,
      closure_id: claimRow.closure_id || null,
      evidence: claimRow.evidence,
      fragment_binding: Object.freeze(context.status === 'RESOLVED'
        ? {
          status: 'BOUND',
          candidate_is_fragment: context.operative.text !== context.leaf.text,
          host_kind: context.item ? 'LEAF' : 'SECTION',
          host,
        }
        : { status: 'UNDETERMINED', reason: context.reason }),
      selected_context: Object.freeze({
        status: context.status,
        ...(context.status === 'RESOLVED'
          ? { scope: Object.keys(CONTEXT_RUNGS).find((key) => CONTEXT_RUNGS[key] === contextRung), records: recordsForContextRung(context) }
          : { reason: context.reason, records: Object.freeze([]) }),
      }),
    });
    contextLadderRecordByCandidate.set(entry, record);
    contextLadderRecords.push(record);
  }

  function sourceParagraphForCandidate(entry, claim) {
    const sourceText = admittedSourceContext.canonical_text?.text;
    const section = findPinnedParentSection(sectionsByReference, entry?.section_reference);
    const originalClaim = entry?.candidate?.claim || claim;
    const canonicalEvidence = canonicalEvidenceForEntry({
      claim: originalClaim,
      evidence: entry?.evidence,
    });
    if (typeof sourceText !== 'string' || typeof claim?.raw_value !== 'string'
      || !section || canonicalEvidence.status !== 'OK'
      || canonicalEvidence.evidence.length !== 1) return null;

    const [edge] = canonicalEvidence.evidence;
    ...
    try {
      sectionText = utf8Slice(sourceText, section.start, section.end);
      quotedText = utf8Slice(sectionText, edge.absolute_start, edge.absolute_end);
    } catch (_error) {
      return null;
    }
    if (quotedText !== originalClaim.raw_value || !quotedText.includes(claim.raw_value)) return null;
    ...
    if (governing.status !== 'RESOLVED') return null;

    const bytes = Buffer.from(sectionText, 'utf8');
    let paragraphStart = edge.absolute_start;
    while (paragraphStart > 0 && bytes[paragraphStart - 1] !== 0x0A) paragraphStart -= 1;
    let paragraphEnd = edge.absolute_end;
    while (paragraphEnd < bytes.length && bytes[paragraphEnd] !== 0x0A) paragraphEnd += 1;
    try {
      return utf8Slice(sectionText, paragraphStart, paragraphEnd);
    } catch (_error) {
      return null;
    }
  }
  ...
  function pushOpenWorld({ entry, claimRow, reason }) {
    const isSoleRemedyEvidence = claimRow.attributes?.structured_mechanic?.surface === 'SOLE_REMEDY_EVIDENCE';
    const sourceCitation = isSoleRemedyEvidence ? sourceCitationForClaim(entry, claimRow) : sourceDerivedCitation(entry);
    recordContextLadder({
      entry,
      claimRow,
      route: 'OPEN_WORLD',
    ...
    openWorld.push(Object.freeze({
      ...
      evidence: claimRow.evidence,
      ...(isSoleRemedyEvidence ? { original_claim_occurrence_id: claimRow.claim_occurrence_id } : {}),
  ...
  function pushReviewUnresolved({
    ...
    const augmentedReasons = sectionFamilyReason && !reasons.includes(sectionFamilyReason)
      ...
      : reasons;
    const sourceCitation = claimRow.claim_definition_key === IOC_RESTRICTION_CLAIM_KEY
      ? sourceCitationForClaim(entry, claimRow)
      : sourceDerivedCitation(entry);
    recordContextLadder({
      entry,
      claimRow,
      route: 'REVIEW_QUEUE',
      reason: Object.freeze(augmentedReasons),
      sourceCitation,
    });
  ...
  function finalizeResolvedCandidate({
    ...
    recordContextLadder({
      entry,
      claimRow: entry.candidate.claim,
      route: 'RESOLVED_PASS',
      reason: Object.freeze(reasons),
      sourceCitation: publishedCitation,
    });
    ...
    if (!autoPass) {
      reviewQueue.push(Object.freeze({
        ...
        ...(genericKey === IOC_RESTRICTION_CLAIM_KEY ? {
          source_citation: sourceCitationForClaim(entry, entry.candidate.claim),
          raw_value: entry.candidate.claim.raw_value,
          canonical_value: entry.candidate.claim.canonical_value,
          original_claim_occurrence_id: entry.candidate.claim.claim_occurrence_id,
          evidence: entry.candidate.claim.evidence,
        } : { evidence: rebuiltClaim.evidence }),
  ...
  function handleNoShopWaveBCandidate(entry) {
    ...
    // Step 2X-G (2026-08-08): REQUEST_RETURN_OR_DESTRUCTION_OF_INFORMATION
    // was held to open-world in d04c5584 while source-context corroboration
    // closed. It is already a registered NO_SHOP_CEASE_ACTION value under
    // NOSOL-CEASE. Corpus recurrence (≥3 deals, here six) + confidence +
    // non-collision with 2X-J CONSUME cleared the promotion gate — do not
    // re-open the shape. Corroboration (widened for instruct/direct and the
    // bare return/destroy shape) and the obligor check below still apply.
  ...
  function handleRegulatoryEffortsCandidate(entry) {
    ...
    if (kind === 'HSR_FILING_DEADLINE' || kind === 'REGULATORY_FILING_DEADLINE') {
      ...
      const parsed = parseFilingDeadlineDays(quote, { numeralRung });
  ...
  function quoteContainsEntityRef(quote, value) {
    if (value == null || value === '') return true;
    if (typeof value !== 'string') return false;
    return quote.includes(value);
  }
  function handleMergerTransactionStepCandidate(entry) {
    const claim = entry.candidate.claim;
    const attrs = claim.attributes || {};
    const quote = String(claim.raw_value || '');
    const stepOrder = Number(attrs.step_order);
    if (!Number.isInteger(stepOrder) || stepOrder < 1) {
      pushOpenWorld({ entry, claimRow: claim, reason: 'MERGER_TRANSACTION_STEP_ORDER_INVALID' });
      return;
    }
    if (!TRANSACTION_STEP_KINDS.includes(attrs.step_kind)) {
      pushOpenWorld({ entry, claimRow: claim, reason: 'MERGER_TRANSACTION_STEP_KIND_OUT_OF_ENUM' });
      return;
    }
    if (attrs.concurrency != null && !TRANSACTION_STEP_CONCURRENCY.includes(attrs.concurrency)) {
      pushOpenWorld({ entry, claimRow: claim, reason: 'MERGER_TRANSACTION_STEP_CONCURRENCY_OUT_OF_ENUM' });
      return;
    }
    if (!quoteContainsEntityRef(quote, attrs.disappearing_entity)
      || !quoteContainsEntityRef(quote, attrs.surviving_entity)) {
      pushOpenWorld({ entry, claimRow: claim, reason: 'MERGER_TRANSACTION_STEP_ENTITY_NOT_IN_QUOTE' });
      return;
    }
    const parentEntity = quoteContainsEntityRef(quote, attrs.parent_entity)
      ? attrs.parent_entity ?? null
      : null;
    finalizeM3StructuralCarrier({
      entry,
      genericKey: MERGER_TRANSACTION_STEP_CLAIM_KEY,
      conceptKey: 'MERGER-STRUCTURE',
      definitionKey: 'MERGER_TRANSACTION_STEP',
      canonicalValue: true,
      attributes: {
        ...
        concurrency: attrs.concurrency ?? 'SEQUENTIAL',
      },
    });
  }
  ...
  function handleRepresentationQualifierCarrier(entry) {
    ...
    const dispatchDecision = recordRepresentationQualifierDispatch({
      entry, claim, conceptKey, qualifierKind,
    });
    if (representationQualifierDispatchMode === REPRESENTATION_QUALIFIER_DISPATCH_MODES.ENFORCE
      && dispatchDecision) {
      if (dispatchDecision.disposition === 'REVIEW') {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
          mapping: null,
          conceptFamily: conceptKey,
          reasons: [dispatchDecision.dispatch],
      ...
      if (dispatchDecision.disposition === 'HOLD') {
        pushOpenWorld({
          entry,
          claimRow: claim,
          reason: 'REPRESENTATION_QUALIFIER_KIND_NOT_GOVERNED',
        });
        return;
      }
      const proposed = dispatchDecision.proposed_claim;
      finalizeRepresentationQualifier({
        entry,
        claim,
        conceptKey: proposed.concept_key,
        definitionKey: proposed.claim_definition_key,
        canonicalValue: proposed.canonical_value,
        party,
        attributes: {
          party_making: party.value,
          ...
          qualifier_kind: qualifierKind,
          attachment,
          qualifier_dispatch: dispatchDecision.dispatch,
          ...(dispatchDecision.dispatch === 'TEMPORAL_RETROSPECTIVE_LOOKBACK'
    ...
    if (qualifierKind === 'KNOWLEDGE') {
      ...
      const modelCode = claim.canonical_value || knowledge.knowledge_standard || null;
      const definedTerm = sameDealDefinedTerms
        ? resolveSameDealDefinedTerm({
          index: sameDealDefinedTerms,
          term_ref: 'Knowledge',
          use_party: party.capacity,
          requested_kind: 'KNOWLEDGE_STANDARD',
          use_evidence: claim.evidence,
        })
        : null;
      if (definedTerm?.outcome === SAME_DEAL_DEFINED_TERM_OUTCOMES.RESOLVED
        && modelCode != null && modelCode !== definedTerm.value) {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
          mapping: null,
          conceptFamily: conceptKey,
          reasons: ['REPRESENTATION_KNOWLEDGE_STANDARD_CONFLICT'],
          materiality: materialityFor({ conceptKey }),
          normalisedPhrase: normaliseForMatching(claim.raw_value),
          attachmentPosition: attachment.position,
        });
      ...
      if (definedTerm && ![
        SAME_DEAL_DEFINED_TERM_OUTCOMES.RESOLVED,
        SAME_DEAL_DEFINED_TERM_OUTCOMES.NO_DEFINITION,
        SAME_DEAL_DEFINED_TERM_OUTCOMES.INERT,
      ].includes(definedTerm.outcome)) {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
          mapping: null,
          conceptFamily: conceptKey,
          reasons: [`REPRESENTATION_DEFINED_TERM_${definedTerm.outcome}`],
          materiality: materialityFor({ conceptKey }),
          normalisedPhrase: normaliseForMatching(claim.raw_value),
          attachmentPosition: attachment.position,
        });
      ...
      const effectiveCode = definedTerm?.outcome === SAME_DEAL_DEFINED_TERM_OUTCOMES.RESOLVED
        ? definedTerm.value
        : definedTerm?.outcome === SAME_DEAL_DEFINED_TERM_OUTCOMES.NO_DEFINITION
          && ['ACTUAL', 'CONSTRUCTIVE', 'AFTER_INQUIRY'].includes(modelCode)
          ? modelCode
          : knowledge.knowledge_standard;
      if (definedTerm?.outcome === SAME_DEAL_DEFINED_TERM_OUTCOMES.NO_DEFINITION
        && knowledge.knowledge_standard != null && modelCode != null
        && knowledge.knowledge_standard !== modelCode) {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
          mapping: null,
      ...
      if (claim.canonical_value != null
        && effectiveCode !== claim.canonical_value) {
      ...
      finalizeRepresentationQualifier({
        ...
        attributes: {
          ...
          attachment,
          ...(effectiveCode
            ? { knowledge_standard: effectiveCode }
            : {}),
          ...(definedTerm?.outcome === SAME_DEAL_DEFINED_TERM_OUTCOMES.RESOLVED
            ? { defined_term_resolution: {
              schema_version: 'SAME_DEAL_DEFINED_TERM_RESOLUTION/V1',
              term_identity: definedTerm.definition_identity,
              outcome: definedTerm.outcome,
              effective_code: definedTerm.value,
              model_code: modelCode,
              definition_claim_occurrence_ids: definedTerm.provenance.source_claim_occurrence_ids,
              definition_evidence_ids: definedTerm.provenance.definition_evidence_ids,
              calibration_id: definedTerm.provenance.calibration_id,
            } }
            : {}),
        },
      });
  ...
  function singleNumber(quote, unit, { acceptSpelled = false, convertYearsToMonths = false } = {}) {
    if (!acceptSpelled) {
      const found = [...quote.matchAll(new RegExp(`(?:\\(\\s*)?\\b(\\d+)\\b(?:\\s*\\))?\\s*(?:-\\s*)?${unit}`, 'ig'))].map((match) => match[1]);
      return found.length === 1 ? found[0] : null;
    }

    const candidates = [];
    for (const unitMatch of quote.matchAll(new RegExp(`\\b(${unit})\\b`, 'ig'))) {
      const before = quote.slice(0, unitMatch.index);
      const parenthetical = before.match(/\(\s*(\d+)\s*\)\s*$/);
      const digit = parenthetical ? parenthetical[1] : before.match(/\b(\d+)\b\s*$/)?.[1] || null;
      const spelledPhrase = parenthetical
        ? trailingNumberWordsPhrase(before.slice(0, before.lastIndexOf('(')))
        : (digit === null ? trailingNumberWordsPhrase(before) : null);
      const spelled = spelledPhrase === null ? null : wordsToNumber(spelledPhrase);

      if (spelledPhrase !== null && spelled === null) continue;
      if (digit !== null && spelled !== null && Number(digit) !== spelled) return null;
      const value = digit === null ? spelled : Number(digit);
      if (!Number.isFinite(value)) continue;
      candidates.push(convertYearsToMonths && /^years?$/i.test(unitMatch[1]) ? String(value * 12) : String(value));
    }
  ...
  function handleDnoCandidate(entry) {
    ...
    const [conceptKey, definitionKey, pattern] = map[key]; if (!pattern.test(quote)) { reviewEmployeeDno({ entry, claim, reason: 'DNO_KIND_UNCORROBORATED', conceptKey }); return; } let value = true;
    if (key === 'INDEM_SURVIVAL_PERIOD' || key === 'TAIL_PERIOD') {
      if (numeralRung === NUMERAL_LADDER_RUNGS.LEGACY_DIGIT_ONLY) {
        value = singleNumber(quote, 'years?');
      } else {
        const parsed = parseDurationAtNumeralRung(quote, { numeralRung });
        value = parsed?.unit === 'years' ? String(parsed.value) : null;
      }
    }
    if (key === 'TAIL_CAP_PERCENT') value = singleNumber(quote, '%'); if (value == null) { reviewEmployeeDno({ entry, claim, reason: key === 'TAIL_CAP_PERCENT' ? 'PERCENT_UNRESOLVED' : 'YEAR_COUNT_UNRESOLVED', conceptKey }); return; }
  ...
  // (either via resolveParty for ONE_PARTY, or one of the two principal-
  // party rows from resolveMutualPrincipalParties for EITHER_PARTY) -- this
  // tail never resolves a party itself.
  ...
  function handleTerminationRightCandidate(entry) {
    ...
    if (partyScope === 'EITHER_PARTY') {
      ...
      // Step 2X-I (Ben ruling 2): mint one row per principal party (TARGET
      // and BUYER), never a single EITHER_PRINCIPAL_PARTY row.
      parties = resolveMutualPrincipalParties(terminatingPartyRef, partySupportQuote);
      if (!parties) {
        pushTerminationReview({ entry, claim: supportedClaim, reason: 'MUTUAL_PARTY_UNRESOLVED', normalisedPhrase, conceptFamily: conceptKey });
        return;
      }
  ...
  function handleTaxMattersCandidate(entry) {
    ...
    // Step 2X-C report-only: run BOTH cooperation kinds' primary tables
    // against this quote, whichever kind was asserted, and record a
    // collision when more than one fires. Never changes resolution below.
    if (kind === 'TAX_OPINION_COOPERATION' || kind === 'TRANSFER_COOPERATION') {
      const cooperationMatches = taxCooperationPrimaryMatchingKinds(q);
      if (cooperationMatches.length > 1) {
        recordCorroborationCollision('tax_cooperation', Object.freeze({
          run: dealKey,
          section_reference: entry.section_reference,
          asserted_kind: kind,
          matching_kinds: cooperationMatches,
          quote: q,
        }));
      }
    }
    ...
    // Step 3G).
    if (kind === 'TAX_OPINION_COOPERATION' && taxOpinionCooperationCorroborated(q, TAX_COOPERATION_LEGACY_OPTS)) {
      const attributes = {
    ...
    if (kind === 'TRANSFER_COOPERATION' && transferCooperationCorroborated(q, TAX_COOPERATION_LEGACY_OPTS)) { resolveTdaClaim(entry, claim, kind, 'TAXM-TRANSFER', 'TRANSFER_TAX_COOPERATION', true); return; }
  ...
  for (const entry of runReceipt.compiled_candidates) {
    ...
    if (provenance.proposal_kind === 'OPEN_WORLD') {
      if (candidate.kind === 'claim') {
        recordContextLadder({
          entry,
          claimRow: candidate.claim,
          route: 'GENERIC_FALLBACK',
          reason: 'NATIVE_OPEN_WORLD_PROPOSAL',
          sourceCitation: sourceDerivedCitation(entry),
        });
    ...
    // `let`: general-covenant specificity may REMAP NOTIFY → LITNOTIFY
    // (DECISIONS.md §16) before the shared finalize path reads these.
    let claim = candidate.claim;
    let genericKey = claim.claim_definition_key;
    ...
    if (genericKey === MERGER_TRANSACTION_STEP_CLAIM_KEY) { handleMergerTransactionStepCandidate(entry); continue; }
    ...
    if (genericKey.startsWith('NATIVE_GENERAL_COVENANT_')) {
      ...
      if (primaryMatches.length > 1) {
        recordCorroborationCollision('general_covenants', Object.freeze({
          run: dealKey,
          section_reference: entry.section_reference,
          asserted_code: ownCode,
          matching_codes: primaryMatches,
          quote,
        }));
      }
      ...
      // PLAN.md Step 3G review, condition 2b + DECISIONS.md §16: a quote
      // that corroborates two different-owner codes is held — EXCEPT the
      // ruled lex-specialis pair NOTIFY ⊂ LITNOTIFY, which remaps to one
      // COV-LITNOTIFY row rather than holding forever. Remap rebuilds a
      // local claim object; it does not mutate frozen compiled_candidates.
      const specificity = resolveGeneralCovenantSpecificity({
        quote,
        assertedCode: ownCode,
        owners: GENERAL_COVENANT_FOLLOW_ON_OWNERS,
      });
      if (specificity.action === 'HOLD') {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
          mapping: null,
      ...
      if (specificity.action === 'REMAP') {
        const newCode = specificity.code;
        const newKey = GENERAL_COVENANT_CLAIM_KEYS[newCode];
        if (!newKey) {
          pushReviewUnresolved({
            entry,
            claimRow: claim,
        ...
        claim = Object.freeze({
          ...claim,
          claim_definition_key: newKey,
          canonical_value: newCode,
          attributes: Object.freeze({
            ...(claim.attributes || {}),
            covenant_code: newCode,
            owner_id: GENERAL_COVENANT_FOLLOW_ON_OWNERS[newCode],
            specificity_precedence_from: specificity.from,
          }),
        });
        genericKey = newKey;
      }
    ...
    recordRepresentationTopicLadder({ entry, claim });
    ...
    const generalCovenantPartyCapacity = genericKey.startsWith('NATIVE_GENERAL_COVENANT_')
      ? generalCovenantJointPartyCapacityOverride({ claim, section, admittedSourceContext })
      : null;
    const partyMapping = generalCovenantPartyCapacity
      ? {
        ...mapping,
        party_capacity_overrides: Object.freeze({
          [claim.attributes.covenant_obligor]: generalCovenantPartyCapacity,
        }),
      }
      : mapping;
    const party = resolveParty({ attributes: claim.attributes, mapping: partyMapping });
  ...
  // This is the finalisation seam. It sees final rewiring, relationships,
  // final queue ordering and completed component trees, then updates only
  // final open-world output before receipt counts are calculated.
  const duplicateSuppression = applyDuplicateSuppression({
    mode: duplicateSuppressionMode,
    run_receipt: runReceipt,
    resolved,
    open_world: openWorld,
    limb_component_trees: limbComponentTrees,
    admitted_source_context: admittedSourceContext,
  });
  openWorld = duplicateSuppression.open_world;
  const duplicateSuppressionReceipt = duplicateSuppressionMode === DUPLICATE_SUPPRESSION_MODES.OFF
    ? null
    : buildDuplicateSuppressionReceipt({
      mode: duplicateSuppressionMode,
  ...
  const receiptBody = {
    ...
    mapping_table_version: MAPPING_TABLE_VERSION,
    qualifier_kind_lexicon_version: QUALIFIER_KIND_LEXICON_VERSION,
    registry_substrate: REGISTRY_SUBSTRATE_MANIFEST,
    ...
    ...(duplicateSuppressionReceipt ? { duplicate_suppression: duplicateSuppressionReceipt } : {}),
    ...(representationQualifierDispatchMode === REPRESENTATION_QUALIFIER_DISPATCH_MODES.ENFORCE
      ? { representation_qualifier_dispatch_mode: representationQualifierDispatchMode }
      : {}),
    ...(corroborationRung > 0 ? {
      corroboration_ladder: Object.freeze({
        schema_version: CORROBORATION_LADDER_VERSION,
        selected_rung: corroborationRung,
  ...
  const sortedRepresentationQualifierDispatchDecisions = representationQualifierDispatchDecisions
    .slice()
    .sort((left, right) => (
      left.section_reference.localeCompare(right.section_reference)
      || String(left.closure_id || '').localeCompare(String(right.closure_id || ''))
      || left.raw_value.localeCompare(right.raw_value)
    ));
  const sortedRepresentationTopicLadderRecords = representationTopicLadderRecords
    .slice()
    .sort((left, right) => (
      left.section_reference.localeCompare(right.section_reference)
      || String(left.original_claim_occurrence_id || '').localeCompare(String(right.original_claim_occurrence_id || ''))
      || String(left.closure_id || '').localeCompare(String(right.closure_id || ''))
      || left.raw_value.localeCompare(right.raw_value)
    ));
  const sortedContextLadderRecords = contextLadderRecords
    .slice()
    .sort((left, right) => (
      left.section_reference.localeCompare(right.section_reference)
      || String(left.original_claim_occurrence_id || '').localeCompare(String(right.original_claim_occurrence_id || ''))
  ...
  return Object.freeze({
    ...
    ...(duplicateSuppressionMode !== DUPLICATE_SUPPRESSION_MODES.OFF
      ? { duplicate_suppression_records: duplicateSuppression.records }
      : {}),
    ...(representationQualifierDispatchMode !== REPRESENTATION_QUALIFIER_DISPATCH_MODES.OFF
      ? { representation_qualifier_dispatch_decisions: Object.freeze(sortedRepresentationQualifierDispatchDecisions) }
      : {}),
    ...(representationTopicLadderMode === REPRESENTATION_TOPIC_LADDER_MODES.REPORT_ONLY
      ? {
        representation_topic_ladder: Object.freeze({
          schema_version: REPRESENTATION_TOPIC_LADDER_SCHEMA,
          topic_rung: representationTopicRung,
          registry_binding: REPRESENTATION_TOPIC_REGISTRY_BINDING_V1,
          activation: Object.freeze({ eligible: false, blocker: 'ORDINARY_REVIEW_PENDING' }),
          records: Object.freeze(sortedRepresentationTopicLadderRecords),
        }),
      }
      : {}),
    ...(contextLadderMode === CONTEXT_LADDER_MODES.REPORT_ONLY
      ? {
        context_ladder: Object.freeze({
          schema_version: CONTEXT_LADDER_SCHEMA,
          context_rung: contextRung,
          candidate_governing_context_schema: CANDIDATE_GOVERNING_CONTEXT_SCHEMA,
          records: Object.freeze(sortedContextLadderRecords),
        }),
      }
      : {}),
