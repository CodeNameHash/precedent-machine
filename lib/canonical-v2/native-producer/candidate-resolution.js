...
const MAPPING_TABLE_VERSION = 14;
...
const APPRAISAL_AVAILABLE_PATTERNS = Object.freeze([
  /\bSection 262\b/,
  /\bDissenting (?:Stockholder|Shares|Company Shares)\b/,
  /\bDissenter Rights\b/,
]);
...
function appraisalStatusCorroboration(quote, status) {
  ...
  const available = typeof quote === 'string'
    && APPRAISAL_AVAILABLE_PATTERNS.some((pattern) => pattern.test(quote));
  return available
    ? Object.freeze({ corroborated: true, reason: null })
    : Object.freeze({ corroborated: false, reason: 'APPRAISAL_STATUS_UNCORROBORATED' });
}
...
function resolveCandidates({
  ...
  function handleProxyMeetingCandidate(entry) {
    ...
    const compatiblePair = corroboratedKinds.length === 2
      && corroboratedKinds.includes('MEETING_DEADLINE')
      && corroboratedKinds.includes('CONVENE_OBLIGATION');
    ...
    if (corroboratedKinds.length > 1 && !compatiblePair) {
      pushProxyMeetingReview({ entry, claim, reason: 'AMBIGUOUS_PROXY_MEETING_KIND', conceptFamily: mapping.concept_key });
      return;
    }

    const refField = assertionKind === 'FILING_DEADLINE' || assertionKind === 'RECOMMENDATION_INCLUSION'
      ? 'document_ref'
      : 'meeting_ref';
    const refValue = typeof attrs[refField] === 'string' && attrs[refField].length > 0 ? attrs[refField] : null;
    if (!refValue || !quote.includes(refValue)) {
  ...
  function handleConsiderationCandidate(entry) {
    ...
    const corroboration = appraisalStatusCorroboration(quote, appraisalStatus);
