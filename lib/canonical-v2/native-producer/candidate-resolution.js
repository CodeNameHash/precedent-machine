...
const MAPPING_TABLE_VERSION = 10;
...
const MATERIALITY_TABLE = Object.freeze([
  ...
  Object.freeze({ rank: 65, label: 'REGULATORY_EFFORTS', concept_key_prefixes: Object.freeze(['ANTI-']) }),
...
const REGULATORY_MUTUAL_OBLIGOR_PATTERN = /\beach of (?:Parent and the Company|the parties)\b|\beach of the parties hereto\b|\bthe parties (?:hereto )?(?:shall|agree)\b|\bneither (?:Parent|the Company|[A-Z][a-zA-Z]+) nor\b/i;
...
const REGULATORY_ASSERTION_MAP = Object.freeze({
  ...
  TIMING_RESTRICTION: Object.freeze({ concept_key: 'ANTI-TIMING', definition_key: 'REGULATORY_TIMING_RESTRICTION' }),
...
function resolveCandidates({
  ...
  function handleRegulatoryEffortsCandidate(entry) {
    ...
    if (kind === 'HSR_FILING_DEADLINE') {
      ...
      if (attrs.filing_regime_ref !== 'HSR Act') { pushRegulatoryReview({ entry, claim, reason: 'NON_HSR_FILING_REGIME', conceptFamily: kindMap.concept_key }); return; }
    ...
    if (!regulatoryValueCorroborated(kind, proposedValue, quote, attrs)) { pushRegulatoryReview({ entry, claim, reason: kind === 'EFFORTS_STANDARD' ? 'EFFORTS_STANDARD_UNCORROBORATED' : kind === 'BURDEN_COMMITMENT' ? 'BURDEN_COMMITMENT_UNCORROBORATED' : kind === 'LITIGATION_OBLIGATION' ? 'LITIGATION_OBLIGATION_UNCORROBORATED' : 'TIMING_RESTRICTION_UNCORROBORATED', conceptFamily: kindMap.concept_key }); return; }
