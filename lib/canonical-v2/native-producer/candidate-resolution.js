...
// one paragraph-initial occurrence of the letter, and on no colon found to
// bound the chapeau's end -- never guesses a boundary. The chapeau ends at
// the colon that opens its own numbered sub-list, the same "chapeau ends at
// the next colon" convention findIocChapeau/findTerminationGrantContext
// already use one level up, for the section's own chapeau.
function findTerminationLimbChapeau({
  ...
  const chapeauText = sectionText.slice(markerStart, colonIndex + 1);
...
// Two recognised forms, both ordinary M&A termination drafting. Anything
// else -- including a limb with no recognisable direction at all -- is
...
function parseTerminationLimbDirection(chapeauText) {
  ...
  if (either) return Object.freeze({ outcome: 'EITHER_PARTY', party_a: either[1], party_b: either[2] });
...
const MATERIAL_CONTRACT_ANY_THRESHOLD_CONTRADICTION = /\$[\d,]+(?:\.\d+)?|\d{1,3}(?:\.\d+)?\s?%|\btop\s+\d+\b/i;
...
function resolveCandidates({
  ...
  function handleRepresentationQualifierCarrier(entry) {
    ...
    if (qualifierKind === 'ACCURACY') {
      ...
      // classifier recognised qualifier-marker text and deliberately
      // declined to pick a single family for it (genuine ambiguity, or
      // explicit carve-out doubt) -- never that the text is not a
      // qualifier. Folding REVIEW into REPRESENTATION_QUALIFIER_KIND_NOT_EXACT
      // (open world) answered "is this a qualifier at all?" when the
      // classifier had actually answered "yes, but which one needs a
      // human". modiv-representations-20260807-replay: 10 of its 28
      // open-world entries were exactly this -- ACCURACY-tagged MAE-
      // qualification clauses ("except where the failure ... would not ...
      // reasonably be expected to have a Company Material Adverse Effect")
      // whose marker text the lexicon's four families do not recognise, so
      // classifyQualifierQuote returns REVIEW:QUALIFIER_KIND_UNCLASSIFIED
      // for every one of them (docs/core/PLAN.md Step 3G). Genuinely
      // ungoverned text -- classification.outcome === 'OPEN_WORLD', the
      // lexicon found NO marker at all, not even a disagreement -- still
      // falls through to REPRESENTATION_QUALIFIER_KIND_NOT_EXACT below
      // unchanged; only REVIEW is redirected.
      if (classification.outcome === 'REVIEW') {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
      ...
      if (attachment.position !== 'CHAPEAU') {
        pushOpenWorld({ entry, claimRow: claim, reason: 'REPRESENTATION_ACCURACY_NOT_CHAPEAU' });
        return;
      }
