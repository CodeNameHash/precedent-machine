...
function resolveCandidates({
  ...
  function resolveQualifierPart({
    entry, provenance, partOriginalClaim, part, attachmentPosition, governsPath,
  }) {
    ...
    // the QUALIFIER, never the representation -- it never even attempts a
    // parse, and routes open-world with a typed reason distinct from a
    // plain "unmapped" generic key.
    ...
    if (!mapping) {
      ...
      pushOpenWorld({ entry, claimRow: partOriginalClaim, reason: 'UNMAPPED_GENERIC_CLAIM_KEY' });
      return;
    }

    let resolvedCanonicalValue;
    const attributesExtra = { deterministic_kind: kind };
    if (kind === 'ACCURACY') {
    ...
    const resolvedEntry = finalizeResolvedCandidate({
      entry, provenance, genericKey, mapping, party, provision, rebuiltClaim, claimDefinition, defectMatch, materiality,
    });

    // ASSERTION_SCOPE_AMBIGUOUS (and any other subject-assignment reason)
    // is a triage-blocking finding independent of the structural gates
    // finalizeResolvedCandidate already evaluated -- surfaced here rather
    // than duplicated into that shared helper, which has no notion of
    // qualifier attachment at all.
    if (subjectAssignment.reasons.length > 0) {
      const mergedReasons = Object.freeze([...resolvedEntry.triage.reasons, ...subjectAssignment.reasons]);
      const alreadyQueued = reviewQueue.find((item) => item.closure_id === rebuiltClaim.closure_id);
      if (alreadyQueued) {
        reviewQueue[reviewQueue.indexOf(alreadyQueued)] = Object.freeze({ ...alreadyQueued, reasons: mergedReasons, auto_pass: false });
      } else {
        reviewQueue.push(Object.freeze({
          section_reference: entry.section_reference,
          generic_claim_key: genericKey,
  ...
  function handleQualifierCandidate(entry) {
    ...
    if (rulingApplied) {
      ...
      // own, so a ruled TEMPORAL is treated as eligible (the human ruling
      // IS the eligibility determination for that exact phrase).
      parts = [{
        kind: rulingApplied.kind,
        code: rulingApplied.code,
        measurementDateEligible: rulingApplied.kind === 'TEMPORAL' ? true : null,
