...
const TERMINATION_LIMB_BARE_BY_PATTERN = new RegExp(
  `^\\([a-zA-Z]\\)\\s*by\\s+(${TERMINATION_PARTY_NAME_ALTERNATION})\\s*(?=[,:;]|\\s+(?:if|at)\\b)`, 'i',
);
...
function resolveCandidates({
  ...
  // tree (today true for every family except CAPITALISATION, whose
  // producer is the only one emitting LIMB_ASSERTION candidates -- see the
  // Stage 5 finding), ambiguous attachment, zero or MULTIPLE derived
  ...
  function handleTerminationRightCandidate(entry) {
    ...
    if (section && (!partyScope || !terminatingPartyRef)) {
      ...
      if (!terminatingPartyRef && derivationDirection) {
        if (derivationDirection.outcome === 'EITHER_PARTY' && partyScope === 'EITHER_PARTY'
          && typeof derivationDirection.either_phrase === 'string') {
          terminatingPartyRef = derivationDirection.either_phrase;
        } else if (derivationDirection.outcome === 'ONE_PARTY' && partyScope === 'ONE_PARTY') {
