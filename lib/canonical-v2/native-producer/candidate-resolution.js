...
function resolveCandidates({
  ...
  function mintCitationTriggeredByRelationship({
    ...
    citedReference, citedClaim, conceptKey, party, feeSide, relationshipOrdinal,
  }) {
    ...
    try {
      ...
      citedProvision = mintProvision({
        section: citedSection, conceptKey, party, ordinal: 1, admittedSourceContext, contractVocabulary,
      });
    } catch (error) {
      return null;
    }
  ...
  function handleFeeTriggerCandidate(entry) {
    ...
    // (design doc Part 6.4). Only attempted once the citing claim above
    // has actually been finalized -- provisionsByGroupKey now holds its
    // provision, minted or reused by finalizeTerminationFeeClaim's own
    // call to mintProvision, keyed the identical way (groupKeyFor). A
    // relationship failing to mint (missing evidence span, missing
    // section metadata) never un-publishes the trigger-code claim above --
    // that claim's own gates already ran, unmodified, before this point.
    if (citationFollowup) {
      const citingGroupKey = groupKeyFor({ sectionReference: entry.section_reference, conceptKey, party });
      const citingProvision = provisionsByGroupKey.get(citingGroupKey);
      if (citingProvision) {
        citationFollowup.citedCandidates.forEach(({ citedReference, claim: citedClaim }, index) => {
          const minted = mintCitationTriggeredByRelationship({
            citingSectionReference: entry.section_reference,
            citingProvision,
            citingClaim: claim,
            citedReference,
            citedClaim,
  ...
  function handleTaxMattersCandidate(entry) {
    ...
    if (kind === 'TAX_OPINION_COOPERATION'
      && /tax opinion/i.test(q)
      && /\b(?:shall|will|agree to)\b[\s\S]{0,300}\b(?:cooperate|provide|furnish|deliver|execute)\b|\b(?:cooperate|provide|furnish|deliver|execute)\b[\s\S]{0,300}\b(?:tax opinion|tax counsel)\b/i.test(q)) {
      const attributes = {
