...
function resolveJointPartyCapacities(rawValue) {
  ...
  for (const segment of segments) {
    const match = PARTY_CAPACITY_LEXICON.find((entry) => entry.pattern.test(segment));
    if (!match) continue;
...
function resolvePartyCapacity(rawValue) {
  ...
  // Joint-obligation check (Step 3F): only ever consulted when the string
  // actually splits into more than one segment -- a single segment (no
  // comma/"and"/"or"/"&" present at all, e.g. "Parent OpCo" alone) falls
  // straight through to the ordinary whole-string scan below, unchanged from
  // before this function had a joint path. That is the load-bearing
  // property behind BOTH commit-34059a2f traps this fix must not disturb:
  // "Parent OpCo" still matches `parent` (BUYER) via array order, and a bare
  // "Company Merger Sub" is still not "currently reached by any single-party
  // candidate" (unchanged, since a single-segment string never enters this
  // branch at all).
  ...
  if (segments.length > 1) {
    ...
    for (const segment of segments) {
      const match = PARTY_CAPACITY_LEXICON.find((entry) => entry.pattern.test(segment));
      if (match) distinctSides.add(sideForCapacity(match.capacity));
    }
