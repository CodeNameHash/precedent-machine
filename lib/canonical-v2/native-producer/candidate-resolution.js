...
function resolveCandidates({
  ...
  for (const entry of runReceipt.compiled_candidates) {
    ...
    if (genericKey.startsWith('NATIVE_GENERAL_COVENANT_')) {
      ...
      // generalCovenantGroundingFailure already ran corroborateGeneralCovenantCode
      // above; do not mutate frozen compiled_candidates entries here.
      const doubleFireCode = generalCovenantDoubleFireCode(quote, ownCode);
      if (doubleFireCode) {
        pushReviewUnresolved({
          entry,
          claimRow: claim,
