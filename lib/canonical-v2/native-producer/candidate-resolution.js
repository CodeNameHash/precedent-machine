...
const MATERIALITY_TABLE = Object.freeze([
  ...
  Object.freeze({ rank: 85, label: 'DNO_INDEMNIFICATION', concept_key_prefixes: Object.freeze(['DNO-INDEM', 'DNO-TAIL', 'DNO-BENEF']) }),
...
function resolveCandidates({
  ...
  // ---------------------------------------------------------------------
  function preferredCapitalisationLimbClaim(claim, entry) {
    const attrs = claim.attributes || {};
    const limbPath = attrs.limb_path;
    ...
    if (!Array.isArray(limbPath) || limbPath.length === 0 || !evidence
      || !countKind || !SHARE_COUNT_KINDS.includes(countKind) || !shareClassRef
      || typeof claim.raw_value !== 'string') return claim;
    ...
    if (clippedParse.outcome !== 'RESOLVED') return claim;
    const matches = runReceipt.compiled_candidates
      .filter((candidateEntry) => candidateEntry.ok === true
        && candidateEntry.section_reference === entry.section_reference
        && candidateEntry.candidate?.kind === 'claim'
        && candidateEntry.candidate.claim?.claim_definition_key === LIMB_ASSERTION_CLAIM_KEY)
      .map((candidateEntry) => candidateEntry.candidate.claim)
      .filter((limbClaim) => {
        const limbAttrs = limbClaim.attributes || {};
        const limbEvidence = operativeEvidenceEdge(limbClaim);
        if (typeof limbClaim.raw_value !== 'string') return false;
        const limbParse = parseShareCount({
          quote: limbClaim.raw_value, count_kind: countKind, share_class_ref: shareClassRef, plan_ref: planRef,
        });
        return JSON.stringify(limbAttrs.limb_path) === JSON.stringify(limbPath)
          && limbClaim.raw_value.includes(claim.raw_value)
          && limbClaim.raw_value.includes(shareClassRef)
          && (planRef == null || limbClaim.raw_value.includes(planRef))
          && limbParse.outcome === 'RESOLVED'
          && limbParse.canonical_value === clippedParse.canonical_value
          && limbEvidence
          && limbEvidence.absolute_start <= evidence.absolute_start
          && evidence.absolute_end <= limbEvidence.absolute_end;
      });
    if (matches.length !== 1) return claim;
    const [limbClaim] = matches;
    return Object.freeze({
      ...claim,
      raw_value: limbClaim.raw_value,
      evidence_ids: limbClaim.evidence_ids,
      evidence: limbClaim.evidence,
    });
  }
  ...
  function handleShareCountCandidate(entry) {
    const { candidate } = entry;
    const provenance = candidate.extraction_provenance;
    const claim = preferredCapitalisationLimbClaim(candidate.claim, entry);
    const attrs = claim.attributes || {};
    const countKind = typeof attrs.count_kind === 'string' ? attrs.count_kind : null;
    const normalisedPhrase = normaliseForMatching(claim.raw_value);
  ...
  function handleNoShopActionCandidate(entry) {
    ...
    if (actionCode === 'FURNISH_NONPUBLIC_INFORMATION' && isFurnishParentheticalExample(entry, claim)) {
      pushNoShopReview({
  ...
  function handleEmployeeMattersCandidate(entry) {
    ...
    if (key === 'ITEM_STANDARD') {
      const items = { BASE_SALARY: /base salary|wage rate|wages/i, TARGET_BONUS: /target annual cash bonus|short-term incentive|commission|incentive compensation target/i, EQUITY_AWARD_OPPORTUNITY: /equity award opportunit/i, SEVERANCE: /\bseverance\b/i, EMPLOYEE_BENEFITS: /employee benefit plans|employee welfare|employee benefits/i };
      const standards = { NO_LESS_FAVORABLE: /no less favorable|not less than/i, SUBSTANTIALLY_COMPARABLE: /substantially comparable/i, SUBSTANTIALLY_SIMILAR: /substantially similar/i, DEFERRED_TO_SCHEDULE: /set forth (?:on|in) schedule/i };
      if (!items[a.comp_item] || !standards[a.standard_kind]) { pushOpenWorld({ entry, claimRow: claim, reason: 'EMPLOYEE_ITEM_OR_STANDARD_OUT_OF_VOCABULARY' }); return; }
      if (!items[a.comp_item].test(quote) || !standards[a.standard_kind].test(quote)) { reviewEmployeeDno({ entry, claim, reason: 'ITEM_OR_STANDARD_UNCORROBORATED', conceptKey: 'COV-EMPLOYEE' }); return; }
      finalizeEmployeeDno({ entry, claim, genericKey: EMPLOYEE_MATTERS_CLAIM_KEY, conceptKey: 'COV-EMPLOYEE', definitionKey: map[key], canonicalValue: true, attributes: { comp_item: a.comp_item, standard_kind: a.standard_kind, aggregation: a.aggregation, benchmark: a.benchmark } }); return;
    }
    ...
    const patterns = { SERVICE_CREDIT: /service.*credit|credit.*service/i, WELFARE_RELIEF: /pre-existing condition|waiting periods?|deductible|coinsurance|co-pay|out-of-pocket/i, TPB_DISCLAIMER: /no .*third party beneficiar|no .*rights/i };
    if (!patterns[key].test(quote)) { reviewEmployeeDno({ entry, claim, reason: 'EMPLOYEE_KIND_UNCORROBORATED', conceptKey: 'COV-EMPLOYEE' }); return; }
    finalizeEmployeeDno({ entry, claim, genericKey: EMPLOYEE_MATTERS_CLAIM_KEY, conceptKey: 'COV-EMPLOYEE', definitionKey: map[key], canonicalValue: true, attributes: key === 'WELFARE_RELIEF' ? { relief_kind: a.relief_kind } : {} });
  }
  function handleDnoCandidate(entry) {
    ...
    const [conceptKey, definitionKey, pattern] = map[key]; if (!pattern.test(quote)) { reviewEmployeeDno({ entry, claim, reason: 'DNO_KIND_UNCORROBORATED', conceptKey }); return; }
    let value = true; if (key === 'INDEM_SURVIVAL_PERIOD' || key === 'TAIL_PERIOD') value = singleNumber(quote, 'years?'); if (key === 'TAIL_CAP_PERCENT') value = singleNumber(quote, '%');
    if (value == null) { reviewEmployeeDno({ entry, claim, reason: key === 'TAIL_CAP_PERCENT' ? 'PERCENT_UNRESOLVED' : 'YEAR_COUNT_UNRESOLVED', conceptKey }); return; }
    const attrs = key === 'TAIL_CAP_PERCENT' ? { cap_basis_ref: a.cap_basis, ...(a.cap_term ? { cap_term_ref: a.cap_term } : {}) } : key === 'TAIL_CAP_OFF_AGREEMENT' ? { schedule_reference_phrase: a.schedule_reference_phrase, ...(a.cap_term ? { cap_term_ref: a.cap_term } : {}) } : {};
    finalizeEmployeeDno({ entry, claim, genericKey: DNO_CLAIM_KEY, conceptKey, definitionKey, canonicalValue: value, attributes: attrs });
  }
