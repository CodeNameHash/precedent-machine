...
const { resolveStructuredPerShareCashValue } = require('./structured-per-share-cash-source-parser');
...
const MATERIALITY_TABLE = Object.freeze([
  ...
  Object.freeze({ rank: 81, label: 'TAX_MATTERS', concept_key_prefixes: Object.freeze(['TAXM-TREATMENT', 'TAXM-TRANSFER', 'TAXM-FIRPTA']) }),
...
function uniqueQuotedDefinedTermValue(admittedSourceContext, term) {
  const text = admittedSourceContext?.canonical_text?.text;
  if (typeof text !== 'string' || typeof term !== 'string' || !term) return null;
  const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const matches = [...text.matchAll(new RegExp(`(?:“|\\")${escaped}(?:”|\\")\\s+means\\s+([^;\\r\\n]+)`, 'g'))];
  if (matches.length !== 1) return null;
  return matches[0][1].trim().replace(/\.$/, '');
}
...
const TERMINATION_TRIGGER_KIND_CORROBORATION_TABLE = Object.freeze({
  ...
  BREACH: /\bbreached\b|\bfailed to perform\b|\bbreach of\b|\bbreach by\b|\bthere has been a breach\b/i,
  ...
  RECOMMENDATION_CHANGE: /\bAdverse Recommendation\b|\bIntervening Event Recommendation Change\b|\bChange of Recommendation\b|\bChange in Recommendation\b|\bCompany Board Recommendation\b/i,
  NO_SOLICITATION_BREACH: /materially\s+breaches?\s+its\s+obligations\s+under\s+Section[\s\u200e\u200f]*4\.4/i,
});
...
const REGULATORY_MUTUAL_OBLIGOR_PATTERN = /\beach of (?:Parent and the Company|the parties)\b|\beach of the parties hereto\b|\beach party hereto\s+(?:shall|will|agrees)\b|\b(?:Parent and (?:the )?Company|the parties) (?:hereto )?(?:shall|agree)\b|\bneither (?:Parent|the Company|[A-Z][a-zA-Z]+) nor\b/i;
...
function regulatoryValueCorroborated(kind, value, quote, attrs) {
  ...
  if (kind === 'NON_IMPEDIMENT_COVENANT') {
    return value === true
      && /\b(?:shall not|neither .{0,120} shall|no party shall)\b/i.test(quote)
      && /\b(?:prevent|delay|impede)\b[\s\S]{0,240}\b(?:consummation|closing|transactions?)\b/i.test(quote);
  }
  return true;
}
...
const MAE_CARVEOUT_CORROBORATION_TABLE = Object.freeze({
  ECONOMY_GENERAL: allPatterns(/\bgeneral economic\b/i, /\bconditions?\b/i),
...
function resolveCandidates({
  ...
  function finalizeResolvedCandidate({
    ...
    const resolvedEntry = Object.freeze({
      ...
      governing_context_quote: governingContextQuote,
  ...
  function corroboratingShareCountQuote({ entry, claim, countKind }) {
    ...
    if (!sectionReference || typeof claim.raw_value !== 'string') return null;
    ...
    for (const supportingEntry of runReceipt.compiled_candidates) {
      ...
      if (typeof supportingAttrs.section_reference !== 'string'
        ...
        || typeof supportingClaim.raw_value !== 'string'
        || !supportingClaim.raw_value.includes(claim.raw_value)
        || !shareCountKindCorroborated(countKind, supportingClaim.raw_value)) continue;
      quotes.add(supportingClaim.raw_value);
    }
  ...
  function handleNoOtherRepsCandidate(entry) {
    ...
    const rebuiltClaim = rebuildClaim({
      ...
      newOrdinal: claimOrdinal,
      newCanonicalValue: true,
      attributesReplace: governedAttributes,
    });
    finalizeResolvedCandidate({
      entry, provenance, genericKey, mapping, party, provision, rebuiltClaim, claimDefinition, defectMatch, materiality,
    });
  ...
  function handleFeeTriggerCandidate(entry) {
    ...
    const fullPaymentQuote = directSides.length === 0
      ...
      : null;
    const corroboratedSides = fullPaymentQuote ? [feeSide] : directSides;
    if (corroboratedSides.length >= 2) {
      pushReviewUnresolved({
        entry, claimRow: claim, mapping: null, conceptFamily: TERMF_PENDING_CONCEPT_FAMILY,
        reasons: ['AMBIGUOUS_FEE_SIDE'],
  ...
  function handleNoShopActionCandidate(entry) {
    ...
    for (const resolvedActionCode of actionCodes) {
      finalizeNoShopClaim({
  ...
  function handleRepresentationQualifierCarrier(entry) { const claim = entry.candidate.claim; const attrs = claim.attributes || {}; const side = attrs.representation_side; if (!['TARGET', 'BUYER'].includes(side)) { pushOpenWorld({ entry, claimRow: claim, reason: 'REPRESENTATION_SIDE_UNRESOLVED' }); return; } if (!['ACCURACY', 'KNOWLEDGE'].includes(attrs.qualifier_kind) || typeof claim.canonical_value !== 'string' || !claim.raw_value) { pushOpenWorld({ entry, claimRow: claim, reason: 'REPRESENTATION_QUALIFIER_UNRESOLVED' }); return; } finalizeM3StructuralCarrier({ entry, genericKey: REPRESENTATION_QUALIFIER_CLAIM_KEY, conceptKey: side === 'TARGET' ? 'REP-T-QUALIFIER' : 'REP-B-QUALIFIER', definitionKey: 'REPRESENTATION_QUALIFIER_STANDARD', canonicalValue: claim.canonical_value, attributes: { representation_side: side, qualifier_kind: attrs.qualifier_kind, attachment: attrs.attachment } }); }
  ...
  function handleTerminationRightCandidate(entry) {
    ...
    const grantContextAttributes = grantContext
      ...
      : Object.freeze({});
    const supportedClaim = grantContext
      ? Object.freeze({
        ...claim,
        attributes: Object.freeze({ ...attrs, ...grantContextAttributes }),
      })
      : claim;
  ...
  function handleConsiderationCandidate(entry) {
    ...
    if (assertionKind === 'PER_SHARE_CASH') {
      ...
      if (parsed.outcome !== 'RESOLVED') {
        const definedValue = uniqueQuotedDefinedTermValue(admittedSourceContext, considerationTermRef);
        const definedAmount = definedValue && /\$([0-9]+(?:\.[0-9]{1,2})?)/.exec(definedValue);
        if (definedAmount && !/fractional shares/i.test(quote)) {
          finalizeConsiderationClaim({ entry, claim, provenance, registeredClaimDefinitionKey: claimDefinitionKey, conceptKey,
            canonicalValue: definedAmount[1], extraAttributes: { consideration_term_ref: considerationTermRef, defined_term_value: definedValue, per_share_cash_parse_version: PER_SHARE_CASH_PARSE_VERSION } });
          return;
        }
    ...
    if (assertionKind === 'EXCHANGE_RATIO') {
      ...
      if (parsed.outcome !== 'RESOLVED') {
        const definedValue = uniqueQuotedDefinedTermValue(admittedSourceContext, ratioTermRef);
        if (definedValue && /^\d+(?:\.\d+)?$/.test(definedValue)) {
          finalizeConsiderationClaim({ entry, claim, provenance, registeredClaimDefinitionKey: claimDefinitionKey, conceptKey,
            canonicalValue: definedValue, extraAttributes: { ratio_term_ref: ratioTermRef, issuer_stock_ref: issuerStockRef, defined_term_value: definedValue, exchange_ratio_parse_version: EXCHANGE_RATIO_PARSE_VERSION } });
          return;
        }
        review(parsed.reason);
        return;
  ...
  function handleTaxMattersCandidate(entry) {
    ...
    if (kind === 'TRANSFER_COOPERATION' && /cooperate.*(?:preparation|filing)|(?:preparation|filing).*cooperate/i.test(q) && /Transfer Taxes|sales, transfer, stamp/i.test(q)) { resolveTdaClaim(entry, claim, kind, 'TAXM-TRANSFER', 'TRANSFER_TAX_COOPERATION', true); return; }
  ...
  function handleDividendsCandidate(entry) {
    ...
    if (kind === 'SPECIAL_DIVIDEND' && /(?:pre-closing|special) dividend/i.test(q) && a.dividend_term_ref && q.includes(a.dividend_term_ref)) { resolveTdaClaim(entry, claim, kind, 'DIVD-SPECIAL', 'PRE_CLOSING_SPECIAL_DIVIDEND', true, { dividend_term_ref: a.dividend_term_ref }); const amount = q.match(/\$\s*(\d+(?:\.\d+)?)\s*(?:per\s+share)?/i); if (amount && /per\s+share/i.test(q)) resolveTdaClaim(entry, claim, kind, 'DIVD-SPECIAL', 'PRE_CLOSING_SPECIAL_DIVIDEND_PER_SHARE_AMOUNT', amount[1], { dividend_term_ref: a.dividend_term_ref, currency: 'USD' }); return; }
