...
function resolveCandidates({
  ...
  function preferredCapitalisationLimbClaim(claim, entry) {
    ...
    if (!Array.isArray(limbPath) || limbPath.length === 0 || !evidence) return claim;
    const matches = runReceipt.compiled_candidates
      ...
      .filter((limbClaim) => {
        ...
        return JSON.stringify(limbAttrs.limb_path) === JSON.stringify(limbPath)
          && typeof limbClaim.raw_value === 'string'
          && limbClaim.raw_value.includes(claim.raw_value)
          && limbClaim.raw_value.includes(attrs.share_class_ref)
          && (attrs.plan_ref == null || limbClaim.raw_value.includes(attrs.plan_ref))
          && limbEvidence
