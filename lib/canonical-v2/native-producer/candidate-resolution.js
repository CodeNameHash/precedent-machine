  ...
  while (match) {
    ...
    if (end <= beforeAbsoluteStart) {
      ...
      if (party && (covenantSide === null || party.capacity === covenantSide)) {
        nearest = Object.freeze({ party, span: buildSemanticSpan(admittedSourceContext, start, end) });
      }
    }
...
function resolveCandidates({
  ...
  // ---------------------------------------------------------------------
  function corroboratingShareCountQuote({ claim, countKind }) {
    const attrs = claim.attributes || {};
  ...
  function handleShareCountCandidate(entry) {
    ...
    const kindCorroboratingQuote = shareCountKindCorroborated(countKind, claim.raw_value)
      ? claim.raw_value
      : corroboratingShareCountQuote({ claim, countKind });
  ...
  for (const entry of runReceipt.compiled_candidates) {
    ...
    if (genericKey === SPECIFIC_PERFORMANCE_REMEDY_CLAIM_KEY) { handlePresenceCarrier(entry, { genericKey, conceptKey: 'REMEDY-SPECIFIC-PERFORMANCE', definitionKey: 'SPECIFIC_PERFORMANCE_REMEDY_PRESENT', allowedKinds: REMEDY_ASSERTION_KINDS }); continue; }
