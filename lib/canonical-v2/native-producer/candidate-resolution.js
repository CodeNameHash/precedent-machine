...
// Two lookup indices: entries that gate on a specific qualifier_kind, and
// entries that apply regardless of it (qualifier_kind: null). A lookup tries
// the kind-specific index first, then falls back to the unconditional one --
// never the reverse, so a kind-specific gap (e.g. QUALIFIER_CLAIM_KEY +
// 'TEMPORAL') can never accidentally fall through to an unconditional entry
// that was only ever meant for a DIFFERENT generic_claim_key.
const GENERIC_CLAIM_KEY_RESOLUTION_BY_KIND = new Map(
  GENERIC_CLAIM_KEY_RESOLUTION_TABLE
    .filter((entry) => entry.qualifier_kind != null)
    .map((entry) => [`${entry.generic_claim_key}::${entry.qualifier_kind}`, entry]),
);
const GENERIC_CLAIM_KEY_RESOLUTION_UNCONDITIONAL = new Map(
  GENERIC_CLAIM_KEY_RESOLUTION_TABLE
    .filter((entry) => entry.qualifier_kind == null)
    .map((entry) => [entry.generic_claim_key, entry]),
);

function lookupGenericClaimKeyMapping(genericClaimKey, qualifierKind) {
  if (typeof qualifierKind === 'string' && qualifierKind.length > 0) {
    const keyed = GENERIC_CLAIM_KEY_RESOLUTION_BY_KIND.get(`${genericClaimKey}::${qualifierKind}`);
    if (keyed) return keyed;
  }
  return GENERIC_CLAIM_KEY_RESOLUTION_UNCONDITIONAL.get(genericClaimKey) || null;
}
...
function resolveCandidates({
  ...
  for (const entry of runReceipt.compiled_candidates) {
    ...
    const qualifierKind = claim.attributes && claim.attributes.qualifier_kind;
    const mapping = lookupGenericClaimKeyMapping(genericKey, qualifierKind);
    if (!mapping) {
    ...
    const deterministicGatesPassed = reasons.length === 0;
    ...
    const autoPass = deterministicGatesPassed && unevaluatedConditions.length === 0;
