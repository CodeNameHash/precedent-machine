...
const INSTRUCTIONS = `Extract only positive closing-condition facts. Copy every quote exactly. Split compound conditions into one fact per entry. For an officer certificate, preserve every cited condition section in certified_condition_refs, but do not claim that the cited conditions resolve locally. Dissent thresholds remain open world until a corpus quote grounds the shape: never emit DISSENT_THRESHOLD. Use open_world_candidates when a fact does not fit or a controlled code is uncertain. Never infer a party, threshold role, or controlled code. Return JSON only.`;
