 ...
 * `provision_instance_id` the caller already minted. It performs no I/O, no
 * model calls, and does not itself get wired into candidate-resolution.js
 * -- that integration is a later task. Every output is frozen. Content-id
 * derivation follows the existing `contentId(domain, payload)` convention
 * with new `/V1` domain strings, per the spec's "Pinned implementation
 * decisions".
 */
