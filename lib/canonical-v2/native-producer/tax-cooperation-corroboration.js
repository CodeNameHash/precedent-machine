 ...
 * both still refuse).
 *
 * Step 2X-B: when the primary tables miss, a second chance may CONSULT a
 * caller-injected V1 legacy matcher (lib/vocab/tax-cooperation-legacy.js)
 ...
 * The matcher is injected so this file stays a PRODUCTION_PATH_PURE_ANALYSIS
 * leaf (zero module deps). Step 2X-C: `taxCooperationPrimaryMatchingKinds`
 * runs both primary tables; multi-kind refusal is enforced.
 */
...
function resolveLegacyKindMatches(quote, taxCooperationLegacyKindMatches) {
  if (typeof taxCooperationLegacyKindMatches !== 'function') return [];
  return taxCooperationLegacyKindMatches(quote) || [];
}

/**
 * @param {string} quote
 * @param {{ taxCooperationLegacyKindMatches?: (quote: string) => string[] }} [opts]
 */
function corroborateTaxOpinionCooperation(quote, opts = {}) {
  if (typeof quote !== 'string' || quote.length === 0) {
    throw new TypeError('quote must be a non-empty string');
  }
  ...
  const v1Kinds = resolveLegacyKindMatches(quote, opts.taxCooperationLegacyKindMatches);
  if (v1Kinds.length > 1) {
    return Object.freeze({
      outcome: 'REVIEW',
      reason: 'AMBIGUOUS_TAX_COOPERATION_KIND',
 ...
 * @param {{ taxCooperationLegacyKindMatches?: (quote: string) => string[] }} [opts]
 */
function corroborateTransferCooperation(quote, opts = {}) {
  if (typeof quote !== 'string' || quote.length === 0) {
    throw new TypeError('quote must be a non-empty string');
  }
...
function taxOpinionCooperationCorroborated(quote, opts = {}) {
  return corroborateTaxOpinionCooperation(quote, opts).outcome === 'RESOLVED';
}

function transferCooperationCorroborated(quote, opts = {}) {
  return corroborateTransferCooperation(quote, opts).outcome === 'RESOLVED';
}
