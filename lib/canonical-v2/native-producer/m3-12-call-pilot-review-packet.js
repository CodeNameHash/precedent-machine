...
function outputCounts(workResult) {
  if (workResult.status !== 'SUCCEEDED') return null;
  const receipt = workResult.run_receipt;
  const resolution = workResult.resolution;
  const resolutionCounts = resolution?.resolution_receipt?.counts;
  const values = {
    compiled_candidate_count: receipt?.compiled_candidate_count,
    open_world_count: resolutionCounts?.open_world,
    review_queue_count: resolutionCounts?.review_queue,
  };
  if (!Object.values(values).every(nonNegativeInteger)) {
    fail('INVALID_OUTPUT_COUNTS', 'A successful work result must retain non-negative compiled and unresolved-output counts.', {
      work_item_id: workResult.work_item_id,
    });
  }
  if (!Array.isArray(receipt.compiled_candidates)
    || !Array.isArray(resolution.open_world)
    || !Array.isArray(resolution.review_queue)
    || receipt.compiled_candidate_count !== receipt.compiled_candidates.filter((entry) => entry?.ok).length
    || resolutionCounts.open_world !== resolution.open_world.length
    || resolutionCounts.review_queue !== resolution.review_queue.length) {
    fail('OUTPUT_COUNT_MISMATCH', 'A successful work result output count does not match its retained output.', {
      work_item_id: workResult.work_item_id,
    });
  }
  return Object.freeze(values);
...
function buildPilotReviewPackets({ manifest, execution_result: executionResult, root_dir: rootDir = process.cwd() } = {}) {
  ...
  for (const item of semanticItems) {
    ...
    const counts = outputCounts(workResult);
    const qualityRecord = Object.freeze({
      ...
      compiled_candidate_count: counts?.compiled_candidate_count ?? null,
      open_world_count: counts?.open_world_count ?? null,
      review_queue_count: counts?.review_queue_count ?? null,
    });
    const packetBody = {
