...
const PILOT_WORK_ITEM_SET_ID = 'd3acd0057df3c86733260eff20c856a50a72ddd066172ee7ada6db83b5514814';
...
function evaluatePilotQualityGate({ quality_records: qualityRecords, review_findings: reviewFindings, root_dir: rootDir = process.cwd() } = {}) {
  ...
  const findings = workItems.map((expected) => {
    ...
    if (reviewDuplicates.has(expected.work_item_id)) {
      escalation.set(expected.work_item_id, ['DUPLICATE_INDEPENDENT_REVIEW']);
    } else if (review.escalation_code) {
      escalation.set(expected.work_item_id, [review.escalation_code]);
      ...
      escalation.set(expected.work_item_id, ['INDEPENDENT_REVIEW_NEEDS_ADJUDICATION']);
    }
