...
const { validatePilotAdjudication } = require('./m3-pilot-adjudication');
...
function adjudicationState({ record, reviewPacket, qualityRecord, review }) {
  if (!record) return { accepted: false, invalid: false };
  if (!reviewPacket || review.status !== 'PASS') return { accepted: false, invalid: true };
  const output = reviewPacket.resolved_output;
  if (!output || output.open_world?.length !== qualityRecord.open_world_count
    || output.review_queue?.length !== qualityRecord.review_queue_count) {
    return { accepted: false, invalid: true };
  }
  const validation = validatePilotAdjudication({ adjudication: record, review_packet: reviewPacket });
  return { accepted: validation.valid, invalid: !validation.valid };
}

function evaluatePilotQualityGate({
  ...
  const { indexed: packets, duplicates: packetDuplicates } = indexByWorkItem(reviewPackets, 'review_packets', workItems);
  const { indexed: adjudicationByWorkItem, duplicates: adjudicationDuplicates } = indexByWorkItem(adjudications, 'adjudications', workItems);
  const rerun = new Map();
  ...
  const findings = workItems.map((expected) => {
    ...
    const adjudication = adjudicationState({
      record: adjudicationByWorkItem.get(expected.work_item_id),
      reviewPacket: packets.get(expected.work_item_id),
      qualityRecord: records.get(expected.work_item_id),
      review,
    });
    const mayClearUnresolved = deterministicCodes.length === 0
      && !reviewDuplicates.has(expected.work_item_id)
      && review.escalation_code == null
      && review.status === 'PASS';
    if (unresolvedCodes.length > 0 && !(mayClearUnresolved && adjudication.accepted)) {
      addEscalation(escalation, expected.work_item_id, unresolvedCodes);
      if (adjudication.invalid || adjudicationDuplicates.has(expected.work_item_id) || packetDuplicates.has(expected.work_item_id)) {
        addEscalation(escalation, expected.work_item_id, ['INVALID_PILOT_ADJUDICATION']);
      }
    }
