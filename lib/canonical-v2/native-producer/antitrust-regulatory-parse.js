...
const { wordsToNumber, NUMERAL_LADDER_RUNGS, validateNumeralLadderRung } = require('../../normalize-numeric');

const ANTITRUST_REGULATORY_PARSE_VERSION = 3;
...
function parseFilingDeadlineDays(quote, { numeralRung = NUMERAL_LADDER_RUNGS.LEGACY_DIGIT_ONLY } = {}) {
  ...
  for (const match of quote.matchAll(DAY_TOKEN_PATTERN)) {
    ...
    if (!digit && numeralRung < NUMERAL_LADDER_RUNGS.SPELLED_ALONE) return Object.freeze({ outcome: 'ABSTAIN', reason: 'NON_LITERAL_NUMERAL' });
...
function parseFilingDeadlineDaysAtNumeralRung(quote, numeralRung) {
  return parseFilingDeadlineDays(quote, { numeralRung });
}

module.exports = { ANTITRUST_REGULATORY_PARSE_VERSION, CURRENCY_CODES, isHsrRegimeRef, parseDivestitureCapAmount, parseFilingDeadlineDays, parseFilingDeadlineDaysAtNumeralRung };
