 ...
 * Resolved rows read claim.evidence; review and open-world rows read
 * top-level evidence. A row carrying both forms is refused when they differ.
...
/**
 * Resolution entries use claim.evidence while open-world and review rows
 * transport the same array at entry.evidence. If both forms appear, they
 * must agree exactly. Placement never searches source text to fill a gap.
 */
function canonicalEvidenceForEntry(entry) {
  const claimEvidence = Array.isArray(entry?.claim?.evidence) ? entry.claim.evidence : null;
  const entryEvidence = Array.isArray(entry?.evidence) ? entry.evidence : null;
  if (claimEvidence && entryEvidence && !isDeepStrictEqual(claimEvidence, entryEvidence)) {
    return Object.freeze({ status: 'CONFLICT', evidence: null });
  }
  const evidence = claimEvidence || entryEvidence;
  return Object.freeze({
    status: evidence && evidence.length > 0 ? 'OK' : 'MISSING',
    evidence: evidence || null,
  });
}

function evidenceSpanForEvidence(edges) {
  if (!Array.isArray(edges) || edges.length === 0) return null;
  let start = null;
  let end = null;
  for (const edge of edges) {
    if (!Number.isInteger(edge.absolute_start) || !Number.isInteger(edge.absolute_end)
      || edge.absolute_end <= edge.absolute_start) return null;
...
function evidenceSpanForEntry(entry) {
  const canonical = canonicalEvidenceForEntry(entry);
  return canonical.status === 'OK' ? evidenceSpanForEvidence(canonical.evidence) : null;
}
...
function annotateEntryStructureContext(entry, {
  ...
  const canonicalEvidence = canonicalEvidenceForEntry(entry);
  if (canonicalEvidence.status === 'CONFLICT') {
    return Object.freeze({
      ...entry,
      structure_context: undeterminedContext(UNDETERMINED_REASONS.EVIDENCE_CONFLICT),
    });
  }
  const span = canonicalEvidence.status === 'OK'
    ? evidenceSpanForEvidence(canonicalEvidence.evidence)
    : null;
  if (!span) {
    return Object.freeze({
      ...entry,
...
/**
 * Drop `structure_context` for bridge-facing product-projection open_items.
 *
 * Resolution entries keep the annotation (PLAN.md Step 2X-A). Dark bridges
 * still enforce a closed open-item field set (ADR-001 scaffolding) and must
 * not silently absorb new native fields — strip at the projection boundary.
 */
function withoutStructureContext(entry) {
  if (!entry || typeof entry !== 'object' || !Object.hasOwn(entry, 'structure_context')) {
    return entry;
  }
  const { structure_context: _ignored, ...rest } = entry;
  return rest;
}
