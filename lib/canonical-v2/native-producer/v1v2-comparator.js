...
const V1V2_COMPARISON_DIAGNOSTIC_SCHEMA = 'V1V2_COMPARISON_DIAGNOSTIC/V1';

// ---------------------------------------------------------------------------
// FAMILY_MAPPING_TABLE (Tier 1) -- v1 `(provision_type,
// provision_subtype)` -> v2 `concept_key`, one explicit entry per pair,
// NO FUZZY MATCHING (spec
...
//
// VERSION 2 (2026-08-02, comparator-wiring slice): adds the SEVEN Ben-
// ratified identity rows from docs/acks/FAMILY-MAPPING-RULINGS-2026-08-02.md
// (Ben's verbatim MAP rulings, all identity -- same "v1 subtype string
// already follows the v2 concept_key convention" grounds as every other row
// in this table):
//   - REP-B-VOTE      -- Ben: MAP (buyer-side stockholder vote rep).
//   - REP-T-CONTROLS  -- Ben: MAP (target internal-controls rep).
//   - REP-T-NOLIAB    -- Ben: MAP (target no-undisclosed-liabilities rep).
//   - REP-B-NOLIAB    -- Ben: MAP (buyer no-undisclosed-liabilities rep).
//   - REP-T-PROXY     -- Ben: MAP (target proxy-statement rep).
//   - REP-T-RPT       -- Ben: MAP (target related-party-transactions rep).
//   - REP-T-SANCTIONS -- Ben: MAP (sanctions/anti-corruption disambiguation
//                        deferred to the v1 backlog; the mapping itself is
//                        not contingent on that split).
// EXPLICITLY NOT ADDED this slice (documented, not an oversight): three
// subtypes Ben ruled OPEN and Fable resolved into SPLIT designs, held back
// for the v1 reclassification slice (same ack doc, "Fable rulings"
// section) -- their cards stay typed `V1_CARD_UNMAPPED` here, expected and
// counted, until the split subtypes exist on real cards:
//   - REP-T-CONSENT      -- splits into REP-T-STOCKAPPROVAL / REP-T-GOVAPPROVAL (R1).
//   - REP-T-REGSTATUS    -- splits into REP-T-40ACT / REP-T-ADVISERSACT / REP-T-INSREG / REP-T-CFIUS (R2).
//   - REP-B-ANTIRELIANCE -- splits into REP-B-NOOTHERREPS / REP-B-NONRELIANCE / REP-B-INDEPINVEST / REP-B-FRAUDCARVEOUT (R3).
// ---------------------------------------------------------------------------
const FAMILY_MAPPING_TABLE_VERSION = 3;
const REPRESENTATION_MAPPING_SUBTYPES = Object.freeze([
  ...
  'REP-B-NOINTEREST', 'REP-B-NOLIAB', 'REP-B-NOREP', 'REP-B-ORG', 'REP-B-PREAMBLE', 'REP-B-PROXY',
  'REP-B-SEC', 'REP-B-SOLVENCY', 'REP-B-TAX', 'REP-B-VOTE',
  'REP-T-AUTH', 'REP-T-BENEFITS', 'REP-T-BROKERS', 'REP-T-CAP', 'REP-T-COMPLY',
  'REP-T-CONTRACTS', 'REP-T-CONTROLS', 'REP-T-ENV', 'REP-T-FAIRNESS', 'REP-T-INSURANCE', 'REP-T-IP',
  'REP-T-LABOR', 'REP-T-LIT', 'REP-T-MATERIAL-CONTRACTS', 'REP-T-NOCHANGE',
  'REP-T-NOCONFLICT', 'REP-T-NOLIAB', 'REP-T-NOREP', 'REP-T-ORG', 'REP-T-PREAMBLE', 'REP-T-PROPERTY',
  'REP-T-PROXY', 'REP-T-RPT', 'REP-T-SANCTIONS',
  'REP-T-SEC', 'REP-T-TAKEOVER', 'REP-T-TAX', 'REP-T-TOP-CUSTOMERS',
]);
const TERMINATION_FEE_MAPPING_SUBTYPES = Object.freeze([
...
const FAMILY_MAPPING_TABLE = Object.freeze([
  ...REPRESENTATION_MAPPING_SUBTYPES.map((subtype) => Object.freeze({
    v1_provision_type: 'REPRESENTATION',
    v1_provision_subtype: subtype,
    concept_key: subtype,
  })),
  ...TERMINATION_FEE_MAPPING_SUBTYPES.map((subtype) => Object.freeze({
    v1_provision_type: 'TERMINATION_FEE',
    v1_provision_subtype: subtype,
    concept_key: subtype,
  })),
]);

function familyMappingKey(provisionType, provisionSubtype) {
  return `${provisionType}\u0000${provisionSubtype}`;
}

const FAMILY_MAPPING_BY_TYPE_AND_SUBTYPE = new Map(
  FAMILY_MAPPING_TABLE.map((entry) => [
    familyMappingKey(entry.v1_provision_type, entry.v1_provision_subtype),
    entry.concept_key,
  ]),
);
...
function computeTier1({ v1Cards, v2Provisions, attemptedSectionScope }) {
  ...
  for (const card of v1Cards) {
    ...
    const conceptKey = typeof card.provision_type === 'string' && typeof subtype === 'string'
      ? FAMILY_MAPPING_BY_TYPE_AND_SUBTYPE.get(familyMappingKey(card.provision_type, subtype))
      : undefined;
  ...
  const familiesWithV1Cards = new Set(
    v1Cards
      .map((card) => (
        typeof card.provision_type === 'string' && typeof card.provision_subtype === 'string'
          ? FAMILY_MAPPING_BY_TYPE_AND_SUBTYPE.get(familyMappingKey(card.provision_type, card.provision_subtype))
          : undefined
      ))
      .filter((key) => key != null),
  );
...
// ---------------------------------------------------------------------------
// Snapshot validation. A diagnostic validates the immutable snapshot shape
// and content id only. It never verifies the asserted deal identity and must
// never be used by a consumer that needs an authoritative comparison receipt.
// ---------------------------------------------------------------------------
function validateV1SnapshotShape(snapshot) {
  ...
  const { snapshot_id: snapshotId, ...body } = snapshot;
  if (snapshotId !== contentId(V1_PROVISION_SNAPSHOT_SCHEMA, body)) {
    fail('INVALID_SNAPSHOT', 'v1_snapshot.snapshot_id must match the complete closed snapshot payload.');
  }
  return snapshot;
}

function validateV1Snapshot(snapshot) {
  validateV1SnapshotShape(snapshot);
  requirePlainObject(snapshot.snapshot_identity_evidence, 'v1_snapshot.snapshot_identity_evidence');
  requireNonEmptyString(snapshot.snapshot_identity_evidence.manifest_id, 'v1_snapshot.snapshot_identity_evidence.manifest_id');
  requireIssuedIdentityConsumerEvidence({
    allocation_receipt: snapshot.snapshot_identity_evidence.issued_allocation_receipt,
    reviewed_bridge: snapshot.snapshot_identity_evidence.issued_reviewed_bridge,
    production_deal_id: snapshot.deal_identity_bridge.production_deal_id,
    governed_deal_key: snapshot.deal_identity_bridge.governed_deal_key,
    manifest_id: snapshot.snapshot_identity_evidence.manifest_id,
  });
  return snapshot;
}
...
function buildComparisonArtifact({
  v1Snapshot, v2Side, attemptedSectionScopeInput, v2CanonicalText,
  schemaVersion, identifierField, validateSnapshot, authority,
} = {}) {
  validateSnapshot(v1Snapshot);
  ...
  const artifactBody = {
    ...
    ...(authority === undefined ? { deal_identity_bridge: Object.freeze({ ...v1Snapshot.deal_identity_bridge }) } : {}),
  ...
  const artifactId = contentId(schemaVersion, artifactBody);
...
function buildV1V2ComparisonReceipt({
  ...
  return buildComparisonArtifact({
    v1Snapshot, v2Side, attemptedSectionScopeInput, v2CanonicalText,
    schemaVersion: V1V2_COMPARISON_RECEIPT_SCHEMA,
    identifierField: 'v1v2_comparison_receipt_id',
    validateSnapshot: validateV1Snapshot,
  });
}

/**
 * Pure legal-comparison output for local testing and investigation. This is
 * deliberately not a receipt: `authority: NONE` states that the snapshot's
 * asserted identity has not been verified or admitted for production use.
 */
function buildV1V2ComparisonDiagnostic({
  v1_snapshot: v1Snapshot, v2_side: v2Side, attempted_section_scope: attemptedSectionScopeInput,
  v2_canonical_text: v2CanonicalText,
} = {}) {
  return buildComparisonArtifact({
    v1Snapshot, v2Side, attemptedSectionScopeInput, v2CanonicalText,
    schemaVersion: V1V2_COMPARISON_DIAGNOSTIC_SCHEMA,
    identifierField: 'v1v2_comparison_diagnostic_id',
    validateSnapshot: validateV1SnapshotShape,
    authority: 'NONE',
  });
}
