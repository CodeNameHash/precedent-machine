...
const FAMILY_IDS = Object.freeze([
  ...
  // CAPITALISATION was previously untracked here despite having the
  // largest extraction prompt in the repository (capitalisation-producer-
  // prompt.js), resolving three claim types (qualifier, limb-assertion and
  // bring-down-tier candidates), and being the only component with real
  // recorded model output (the F28/Modiv/Skechers live-run handoffs). Added
  // 2026-08-05 at the completion state the evidence actually supports:
  // FOLLOW_ON_OPEN, not FAMILY_COMPLETE -- see its wave_a checks and single
  // product surface in m3-family-parity-register.json.
  'CAPITALISATION',
  'CLOSING_CONDITIONS',
  'CONSIDERATION',
  'DIVIDENDS',
...
function validateSurface(surface, label) {
  // `adapter_set` and `server_stamped_field` are the only optional members, independently.
  // Ruling 1 (Ben, 2026-08-05): a surface naming several adapters requires exact consumer
  // proof for every one of them. Omitting adapter_set therefore means ALL_REQUIRED; the
  // express value ALTERNATIVES is the sole opt-out. server_stamped_field is the HTTP-
  // boundary correspondence evidence (docs/codex-program/notes/serving-path-proof.md):
  // omitting it leaves a surface's visibility exactly as it was before this evidence shape
  // existed; naming it holds the surface to the additional serving-boundary proof below.
  if (!exactKeys(surface, SURFACE_KEYS)
    && !exactKeys(surface, [...SURFACE_KEYS, 'adapter_set'])
    && !exactKeys(surface, [...SURFACE_KEYS, 'server_stamped_field'])
    && !exactKeys(surface, [...SURFACE_KEYS, 'adapter_set', 'server_stamped_field'])) {
  ...
  // Validated only once source_path itself is confirmed a safe, real, resolving repository
  // file above -- client_function's own resolution check below depends on that already
  // holding.
  if (surface.server_stamped_field !== undefined) {
    validateServerStampedField(surface.server_stamped_field, surface.source_path, `${label}.server_stamped_field`);
  }
...
// Two more 503/redirect containment shapes, outside QUERY_CONTAINED_ROUTE_FILES, that must
// not lend serving proof either (docs/codex-program/notes/family-rollout-mechanics.md, Part
// 1). Named explicitly, the same way QUERY_CONTAINED_ROUTE_FILES is, rather than detected
// structurally: a general getServerSideProps/getStaticProps unconditional-redirect sweep is
// real, separate follow-up work, not a safe inline heuristic to add opportunistically.
//
// pages/api/market-stats.js's entire body is marketStatsContainedHandler
// (lib/market-stats-containment.js), an unconditional 503 MARKET_STATS_DISABLED responder -- a
// second containment mechanism the query-containment module does not model.
const MARKET_STATS_CONTAINED_ROUTE_FILES = Object.freeze([
  'pages/api/market-stats.js',
]);
// pages/query/whats-market/adhoc.js's entire getServerSideProps is an unconditional
// server-side redirect to '/'. No real request ever renders this component or reaches
// anything it imports.
const UNCONDITIONAL_REDIRECT_ROUTE_FILES = Object.freeze([
...
function servedModules() {
  ...
  const contained = new Set([
    ...Object.values(QUERY_CONTAINED_ROUTE_FILES),
    ...MARKET_STATS_CONTAINED_ROUTE_FILES,
    ...UNCONDITIONAL_REDIRECT_ROUTE_FILES,
  ]);
...
// A module top-level `const NAME = '<literal>';` value, recorded alongside `bindings`'s own
// kind classification for the same declarator. Only a direct string-literal initialiser is
// recorded -- an expression, a call, a template, or a reference to something else is not.
// This does not by itself decide whether NAME is safe to trust: `bindings` already marks a
// name declared more than once as AMBIGUOUS, and `resolvedConstantString` (server-stamped-
// field proof, below) checks that before ever reading this map, so a shadowed or duplicated
// name is never silently folded.
function recordConstantStringBindings(constantStrings, declaration) {
  if (!declaration || declaration.type !== 'VariableDeclaration' || declaration.kind !== 'const') return;
  for (const declarator of declaration.declarations) {
    if (declarator.id.type !== 'Identifier') continue;
    if (declarator.init && declarator.init.type === 'Literal' && typeof declarator.init.value === 'string') {
      constantStrings.set(declarator.id.name, declarator.init.value);
    }
  }
}
...
function analyseModuleSource(source, filePath) {
  ...
  for (const statement of program.body) {
    ...
    recordConstantStringBindings(constantStrings, declaration);
  ...
  return {
    bindings, functions, exportLocals, topLevelReferences, constantStrings,
  };
}
...
// ---------------------------------------------------------------------------------------
// Server-stamped-field proof.
//
// docs/codex-program/notes/family-rollout-mechanics.md ("Watch for this" #1) and
// docs/codex-program/notes/compare-locator-fix.md (Part 4) both found, and neither built,
// the same gap: every proof above this point walks ONE module graph, the same
// require()/import specifiers a bundler would follow. A family's real serving switch
...
// SEPARATE module graph reads that same field name off the fetched payload. No import edge
// connects the two ends, only an agreed-upon string, so a surface can report NATIVE_VISIBLE
// today while proving nothing whatsoever about whether V2 data reaches a user.
//
// This does not, and cannot, execute an HTTP request or run the server -- see
// docs/codex-program/notes/serving-path-proof.md for why that question needs a different
// kind of check than anything in this file. What IS provable from the committed source,
// mechanically, without a human looking at a page:
//   1. the named server-side stamping function is itself on a reached path from a real,
//      uncontained route -- the exact same call-graph proof every other surface in this
//      register already relies on (ruling 2), applied to the server side of the boundary
//      instead of the client side;
//   2. that function's own reachable code genuinely computes the exact wire-field literal
//      the surface claims, not a same-shaped decoy living elsewhere in the file;
//   3. the client-side function named on the surface's own already-governed source_path
//      genuinely computes that SAME literal.
// A rename on either side that is not mirrored on the other breaks the match and the proof
// fails closed -- see the hostile disagreement tests in
// tests/canonical-v2-parity-serving-boundary.test.js.
//
// What this deliberately does NOT prove, on the record rather than left implicit: that the
// named client_function sits on the EXACT executed call path from a live page down to the
// field read. Every *.config.js in this repository dispatches through a generic
// `config.selectRows(reviewDeal)` / `column.renderCell(row, ctx)` call the intra-module call
// graph cannot see through (family-rollout-mechanics.md Part 2 documents this for
// marketSubterms/selectRows; it is the identical shape for
// isCanonicalTerminationFeeServingEnabled/selectRows). Solving that is a different, already-
// named, already-deferred problem, not this one. So this proof anchors trust one notch short
// of full reachability: client_function must be a real, resolvable, top-level symbol in the
// surface's own source_path (already governed by exactLocatorResolves, validated above), and
// it must be referenced beyond its own declaration somewhere in that SAME file
// (referencedBeyondImport -- the identical "genuinely used, not dead code" bar ruling 2
// already applies to consumer imports, applied within one file instead of across two) -- but
// not, additionally, transitively reachable from the surface's own locator. A register
// author who names a client_function that exists, is called from SOMEWHERE in the file, and
// evaluates the right literal, but from code that is itself unreachable from any served
// page, would slip past this specific check. That is a real, narrower version of the same
// gap ruling 3 exists to close elsewhere, recorded rather than hidden.
// ---------------------------------------------------------------------------------------

const SERVER_STAMPED_FIELD_KEYS = Object.freeze([
  'client_function', 'kind', 'server_function', 'server_path', 'server_route', 'wire_field',
]);

function validateServerStampedField(shape, clientPath, label) {
  if (!exactKeys(shape, SERVER_STAMPED_FIELD_KEYS) || shape.kind !== 'server_stamped_field') {
    fail('INVALID_PARITY_REGISTER', `${label} must match the closed server_stamped_field contract.`);
  }
  text(shape.wire_field, `${label}.wire_field`);
  text(shape.server_path, `${label}.server_path`);
  text(shape.server_function, `${label}.server_function`);
  text(shape.server_route, `${label}.server_route`);
  text(shape.client_function, `${label}.client_function`);
  repositoryPath(shape.server_path, `${label}.server_path`);
  repositoryPath(shape.server_route, `${label}.server_route`);
  if (!shape.server_route.startsWith('pages/')) {
    fail('INVALID_PARITY_REGISTER', `${label}.server_route must name a Next.js route file under pages/.`);
  }
  if (!exactLocatorResolves(shape.server_path, shape.server_function, `${label}.server_function`)) {
    fail('PARITY_LOCATOR_NOT_FOUND', `${label}.server_function does not resolve in server_path.`);
  }
  if (!exactLocatorResolves(clientPath, shape.client_function, `${label}.client_function`)) {
    fail('PARITY_LOCATOR_NOT_FOUND', `${label}.client_function does not resolve in source_path.`);
  }
}

// A module top-level `const NAME = 'literal';` value, resolved with confidence: NAME must be
// bound exactly once at module scope (bindings.get(name) === 'VALUE', never AMBIGUOUS,
// FUNCTION or IMPORTED) and initialised directly by a string literal. Anything looser
// resolves to null rather than a guess. This is the one-hop constant fold both ends of the
// boundary need: the server and client files each declare their own copy of the wire-field
// string exactly this way (see termination-fee-serving-source.js and
// termination-fees.config.js, which re-declares the server's constants independently on the
// client side, by design -- the client cannot import a server module that pulls in node:fs
// and the extraction pipeline).
function resolvedConstantString(analysis, name) {
  if (analysis.bindings.get(name) !== 'VALUE') return null;
  return analysis.constantStrings.has(name) ? analysis.constantStrings.get(name) : null;
}

// Every exact string a reachable function body evaluates: string literals directly (already
// in `.tokens`, via collectTokens), plus one-hop constant folds of identifier tokens that
// resolve to a literal at module scope. Deliberately the same generosity as ruling 3's
// NON_FUNCTION_EVALUATION_SITE rule (ambient in a reachable function's body is enough; a
// top-level-only appearance is not, "being defined is not being served") plus exactly one
// new capability: it only credits a function that is ITSELF reachable from entrySymbols
// (reachableNames, ruling 3's own closure), and it only follows a constant reference exactly
// one hop, so an indirect or computed field name still fails closed instead of being guessed
// at.
function evaluatesStringOnReachablePath(analysis, entrySymbols, value) {
  for (const name of reachableNames(analysis, entrySymbols)) {
    const fn = analysis.functions.get(name);
    if (!fn) continue;
    if (fn.tokens.has(value)) return true;
    for (const token of fn.tokens) {
      if (resolvedConstantString(analysis, token) === value) return true;
    }
  }
  return false;
}

// The server half. server_route must be a live, uncontained entry point -- never take the
// register's word for it, servedModules() is the same real walk every other surface's proof
// already depends on. server_function must be an export of server_path that server_route
// genuinely imports AND references beyond the import line (executedAdapterExportNames,
// ruling 2's exact bar, unweakened here). server_function's own reachable code must
// genuinely compute the exact wire_field literal, not a same-shaped decoy living elsewhere
// in server_path.
function serverStampsField(shape) {
  if (!servedModules().has(shape.server_route)) {
    return Object.freeze({ proven: false, reason: 'SERVER_ROUTE_NOT_SERVED' });
  }
  if (!executedAdapterExportNames(shape.server_route, shape.server_path).has(shape.server_function)) {
    return Object.freeze({ proven: false, reason: 'SERVER_FUNCTION_NOT_EXECUTED_BY_ROUTE' });
  }
  const analysis = analyseModule(shape.server_path);
  if (!analysis) {
    return Object.freeze({ proven: false, reason: 'SERVER_MODULE_NOT_PARSEABLE' });
  }
  if (!evaluatesStringOnReachablePath(analysis, [shape.server_function], shape.wire_field)) {
    return Object.freeze({ proven: false, reason: 'WIRE_FIELD_NOT_STAMPED_ON_REACHABLE_PATH' });
  }
  return Object.freeze({ proven: true, reason: null });
}

// The client half. client_function must be a real top-level function binding in the
// surface's own source_path (validateServerStampedField already confirmed the symbol
// resolves textually; here it must be a registered FUNCTION binding, not a value or an
// import), genuinely referenced beyond its own declaration somewhere in that file
// (referencedBeyondImport -- the same "not dead code" bar ruling 2 already applies to
// consumer imports, applied within a single file instead of across two), and its own
// reachable code must compute the exact same wire_field literal the server half stamps. See
// the section header above for what this does NOT additionally require, and why.
function clientReadsField(surface, shape) {
  const analysis = analyseModule(surface.source_path);
  if (!analysis || !analysis.functions.has(shape.client_function)) {
    return Object.freeze({ proven: false, reason: 'CLIENT_FUNCTION_NOT_FOUND' });
  }
  const source = readStrippedSource(surface.source_path);
  if (source === null || !referencedBeyondImport(source, shape.client_function)) {
    return Object.freeze({ proven: false, reason: 'CLIENT_FUNCTION_NEVER_CALLED_IN_FILE' });
  }
  if (!evaluatesStringOnReachablePath(analysis, [shape.client_function], shape.wire_field)) {
    return Object.freeze({ proven: false, reason: 'WIRE_FIELD_NOT_READ_ON_REACHABLE_PATH' });
  }
  return Object.freeze({ proven: true, reason: null });
}

// The whole-boundary answer for one surface. Not applicable when the surface carries no
// server_stamped_field evidence -- every surface registered before this task names none, so
// liveProductVisibility must treat those exactly as before, byte-identical. When a surface
// does carry this evidence, BOTH halves must independently prove the SAME literal, anchored
// to the one wire_field the surface names: there is no separate "extract, then compare"
// step, because both halves are already checked against that one shared, register-declared
// value. A surface that mis-names either half, or whose two ends genuinely disagree about
// which field carries the value, fails here.
function servingBoundaryProof(surface) {
  const shape = surface.server_stamped_field;
  if (!shape) return Object.freeze({ applicable: false, proven: false, reason: null });
  const server = serverStampsField(shape);
  if (!server.proven) return Object.freeze({ applicable: true, proven: false, reason: server.reason });
  const client = clientReadsField(surface, shape);
  if (!client.proven) return Object.freeze({ applicable: true, proven: false, reason: client.reason });
  return Object.freeze({ applicable: true, proven: true, reason: null });
}
...
function liveProductVisibility(surface) {
  ...
  if (surface.disposition === 'APPROVED_DERIVED' || surface.disposition === 'NATIVE_COMPLETE') {
    ...
    // Additive, opt-in gate: only a surface that names server_stamped_field evidence is held
    // to it, and (like ruling 3 above) it only ever withdraws a positive claim -- it cannot
    // promote a row the earlier gates already rejected, and it runs last so a surface still
    // blocked on consumer proof or locator reachability keeps that more specific reason. No
    // surface registered before this evidence shape existed names it, so this is a byte-
    // identical no-op for all of them. See the section above servingBoundaryProof for why
    // even a NATIVE_VISIBLE/DERIVED_VISIBLE answer without this evidence proves only that
    // rendering plumbing is reached, never that V2 data crosses the HTTP boundary.
    if (surface.server_stamped_field && !servingBoundaryProof(surface).proven) {
      return derived ? 'DERIVED_SERVING_BOUNDARY_UNVERIFIED' : 'NATIVE_SERVING_BOUNDARY_UNVERIFIED';
    }
