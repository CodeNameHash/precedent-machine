...
const MODULE_SPECIFIER = /(?:require\s*\(\s*['"]([^'"]+)['"]\s*\)|from\s+['"]([^'"]+)['"]|import\s*\(\s*['"]([^'"]+)['"]\s*\))/g;
...
// so this can only under-report serving.
...
function servedModules() {
  ...
  // served entry point and must not lend serving proof to anything it imports.
  ...
  const pending = [...files].filter((file) => file.startsWith('pages/')
    && !contained.has(file)
    && !designGuarded(file));
  while (pending.length > 0) {
    ...
    let source;
    try {
      source = stripCodeComments(fs.readFileSync(path.join(REPOSITORY_ROOT, file), 'utf8'));
    } catch {
      continue;
    }
    for (const match of source.matchAll(MODULE_SPECIFIER)) {
      const resolved = resolveRelativeSpecifier(file, match[1] || match[2] || match[3], files);
