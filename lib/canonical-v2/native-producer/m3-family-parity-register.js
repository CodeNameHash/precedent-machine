...
const REGISTER_SCHEMA = 'CANONICAL_V2_M3_FAMILY_PARITY_REGISTER/V2';
const STATUS_SCHEMA = 'CANONICAL_V2_M3_FAMILY_PARITY_STATUS/V2';
...
function validateSupplementalOwner(owner, index) {
  ...
  if (!['NATIVE_FAMILY', 'ROUTING_UMBRELLA'].includes(owner.owner_kind)) {
    fail('INVALID_PARITY_REGISTER', `${label}.owner_kind is not governed.`);
  }
  text(owner.ownership_path, `${label}.ownership_path`);
  if (!exactKeys(owner.first_slice, ['evidence_paths', 'state'])) {
    fail('INVALID_PARITY_REGISTER', `${label}.first_slice must match the closed first-slice contract.`);
  }
  const firstSliceState = state(owner.first_slice.state, `${label}.first_slice.state`);
  evidencePaths(owner.first_slice.evidence_paths, `${label}.first_slice.evidence_paths`, firstSliceState);
  if (!Array.isArray(owner.product_surfaces) || owner.product_surfaces.length === 0) {
    fail('INVALID_PARITY_REGISTER', `${label}.product_surfaces must not be empty.`);
  }
  owner.product_surfaces.forEach((surface, surfaceIndex) => (
    validateSurface(surface, `${label}.product_surfaces[${surfaceIndex}]`)
  ));
}
...
function validateM3FamilyParityRegister(register) {
  ...
  if (!Array.isArray(register.supplemental_owners)
    || register.supplemental_owners.length !== SUPPLEMENTAL_OWNER_IDS.length) {
    fail('INVALID_PARITY_REGISTER', 'The M3 family parity register must contain exactly three supplemental owners.');
  }
  register.supplemental_owners.forEach(validateSupplementalOwner);
  const supplementalOwnerIds = register.supplemental_owners.map((owner) => owner.owner_id).sort();
  ...
  const surfaceIds = [
    ...
    ...register.supplemental_owners.flatMap((owner) => owner.product_surfaces.map((surface) => surface.surface_id)),
...
function supplementalOwnerCompletionState(owner) {
  ...
  if (owner.product_surfaces.some((surface) => surface.state !== 'PASS')) return 'FOLLOW_ON_OPEN';
