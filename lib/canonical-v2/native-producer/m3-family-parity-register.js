...
// A product surface remains a blocker until it has a terminal disposition
// backed by evidence. OPEN_WORLD evidence is deliberately not a disposition.
function listM3ProductParityBlockers(register) {
  ...
  const blockers = register.families.flatMap((family) => family.product_surfaces
    .filter((surface) => surface.state !== 'PASS')
  ...
  const unassigned = register.unassigned_product_surfaces.map((surface) => Object.freeze({
    family_id: null,
    surface_id: surface.surface_id,
    source_kind: surface.source_kind,
    source_path: surface.source_path,
