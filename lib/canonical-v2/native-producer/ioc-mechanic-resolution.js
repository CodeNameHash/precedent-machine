...
const NORMALISED_IGNORABLE_CHARACTERS = new Set([
  '\u200B', '\u200C', '\u200D', '\u200E', '\u200F', '\uFEFF', '\u00AD',
]);

function normalisedUnitsWithSourceSpans(value, absoluteStart = 0) {
  if (typeof value !== 'string') throw new TypeError('normalised containment text must be a string');
  if (!Number.isInteger(absoluteStart) || absoluteStart < 0) {
    throw new TypeError('normalised containment absolute start must be a non-negative integer');
  }
  const units = [];
  let byteOffset = absoluteStart;
  for (const character of value) {
    const nextByteOffset = byteOffset + utf8ByteLength(character);
    if (NORMALISED_IGNORABLE_CHARACTERS.has(character)) {
      byteOffset = nextByteOffset;
      continue;
    }
    if (/\s/u.test(character)) {
      const previous = units.at(-1);
      if (previous?.value === ' ') previous.end = nextByteOffset;
      else units.push({ value: ' ', start: byteOffset, end: nextByteOffset });
      byteOffset = nextByteOffset;
      continue;
    }
    units.push({ value: character, start: byteOffset, end: nextByteOffset });
    byteOffset = nextByteOffset;
  }
  return units;
}

function locateNormalisedContainment(sourceText, quote, absoluteStart = 0) {
  if (typeof quote !== 'string' || quote.length === 0) {
    return Object.freeze({ outcome: 'ZERO', spans: Object.freeze([]) });
  }
  const sourceUnits = normalisedUnitsWithSourceSpans(sourceText, absoluteStart);
  const quoteUnits = normalisedUnitsWithSourceSpans(quote).map((unit) => unit.value);
  if (quoteUnits.length === 0) return Object.freeze({ outcome: 'ZERO', spans: Object.freeze([]) });
  const spans = [];
  for (let start = 0; start <= sourceUnits.length - quoteUnits.length; start += 1) {
    let matches = true;
    for (let offset = 0; offset < quoteUnits.length; offset += 1) {
      if (sourceUnits[start + offset].value !== quoteUnits[offset]) {
        matches = false;
        break;
      }
    }
    if (matches) {
      spans.push(Object.freeze({
        absolute_start: sourceUnits[start].start,
        absolute_end: sourceUnits[start + quoteUnits.length - 1].end,
      }));
    }
  }
  return Object.freeze({
    outcome: spans.length === 1 ? 'UNIQUE' : spans.length === 0 ? 'ZERO' : 'MULTIPLE',
    spans: Object.freeze(spans),
  });
}

function normalisedUnitsEqual(left, right) {
  const leftUnits = normalisedUnitsWithSourceSpans(left).map((unit) => unit.value);
  const rightUnits = normalisedUnitsWithSourceSpans(right).map((unit) => unit.value);
  return leftUnits.length === rightUnits.length
    && leftUnits.every((unit, index) => unit === rightUnits[index]);
}

function uniqueNormalisedSectionOccurrence(section, sourceText, quote) {
  if (!section) return null;
  const sectionText = utf8Slice(sourceText, section.start, section.end);
  const result = locateNormalisedContainment(sectionText, quote, section.start);
  return result.outcome === 'UNIQUE' ? result.spans[0] : null;
}
...
function limbAttachment({ mechanic, item, components, entriesByComponentId, sourceText }) {
  ...
  const mechanicContainment = locateNormalisedContainment(item.raw_value, targetQuote);
  if (mechanicContainment.outcome !== 'UNIQUE') {
    return { attachment: null, reason: 'IOC_ATTACHMENT_TARGET_QUOTE_OUTSIDE_MECHANIC' };
  }
  const matches = components.flatMap((component) => {
    const text = componentText(component, sourceText);
    const containment = locateNormalisedContainment(text, targetQuote, component.absolute_start);
    if (!normalisedUnitsEqual(text, targetQuote)) return [];
    return containment.spans.map((span) => ({ component, span }));
  });
  ...
  const { component, span } = matches[0];
...
function parentAttachment({ item, section, sectionEntries, sourceText }) {
  if (!uniqueNormalisedSectionOccurrence(section, sourceText, item.raw_value)) {
...
function resolveIocMechanics({
  ...
  if (!openWorld.some(isIocMechanic)) {
    // Same hazard as sole-remedy-resolution.js's disabled branch: `openWorld`
    // is the CALLER's own live array, not a copy. `freeze()` recurses through
    // every value here and calls Object.freeze on each -- passing the array
    // through by reference would freeze the caller's still-mutable structure
    // in place. Copy it, matching the `enabled: true` branch below, which
    // already returns its own freshly-built `nextOpenWorld` array.
