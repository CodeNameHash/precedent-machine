...
const SCHEMA_VERSION = 'CANDIDATE_GOVERNING_CONTEXT/V2';
...
function createCandidateGoverningContext({
  ...
  function forCandidate({ entry = null, claim: suppliedClaim = null } = {}) {
    ...
    try {
      ...
      const siblingPath = parentPath(governing.leaf.path);
      const siblings = outline.leaves
        .filter((item) => item.path != null && item.path !== governing.leaf.path && parentPath(item.path) === siblingPath)
        .map((item) => recordForSpan(sectionText, item.start_byte, item.end_byte, {
          path: item.path,
          depth: item.depth,
        }));
      ...
      let lineStart = edge.absolute_start;
      while (lineStart > 0 && sectionBytes[lineStart - 1] !== 0x0A) lineStart -= 1;
      let lineEnd = edge.absolute_end;
      while (lineEnd < sectionBytes.length && sectionBytes[lineEnd] !== 0x0A) lineEnd += 1;
      const renderedLine = recordForSpan(sectionText, lineStart, lineEnd, {});
      const sectionRecord = recordForSpan(sectionText, 0, Buffer.byteLength(sectionText, 'utf8'), {});
      return Object.freeze({
        status: 'RESOLVED',
        schema_version: SCHEMA_VERSION,
