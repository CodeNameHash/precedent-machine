...
// patterns are the design-spec conjunctive tables. Step 2X-B CONSULTS
// lib/vocab/guaranty-assertion-legacy.js on a primary miss but does NOT
// resolve (DECISIONS.md §15 HOLD — report-only until a corpus-pass quote
// report is reviewed). Hits are tagged `second_chance_would_resolve: true`.
// Primary hits stay byte-identical. Cross-kind ambiguity (PERFORMANCE vs
// delivery kinds) is enforced on both passes.
...
function corroborateGuarantyKind({ quote, asserted_kind: assertedKind } = {}) {
  ...
  // Step 2X-B HOLD (DECISIONS.md §15): relaxed legacy scaffolds must not
  // resolve until a corpus pass with quotes is reviewed. Record the hit as a
  // would-resolve candidate; keep the primary-miss path at REVIEW.
  if (v1Matches.length === 1 && v1Matches[0] === assertedKind) {
    return Object.freeze({
      outcome: 'REVIEW',
      reason: 'GTY_KIND_UNCORROBORATED',
