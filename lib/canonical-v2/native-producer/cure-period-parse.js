 ...
 * word->value table (one...ninety, the plausible cure-period range) and
 * its value differs from the digit -> ABSTAIN `SPELLED_DIGIT_MISMATCH`.
 * A spelled numeral with NO adjacent digit -> ABSTAIN `NON_LITERAL_
...
// NUMERAL rather than the generic NO_DAY_COUNT. "one" through "ninety",
// the plausible cure-period range (spec section 2b).
const NON_LITERAL_DAY_PATTERN =
  /\b(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)\s+(?:business\s+)?days?\b/i;
