...
function reviewItem(resolved, item) {
  return freeze({
    section_reference: resolved.section_reference,
    source_citation: item.source_citation || item.section_reference || null,
    ...
    original_claim_occurrence_id: item.original_claim_occurrence_id || null,
...
function resolveSoleRemedyOpenWorld({
  ...
  for (const item of openWorld) {
    ...
    reviewQueue.push(reviewItem(effectResolved, item));
    ...
    for (const [carveOutOrdinal, carveOut] of (Array.isArray(mechanic.carve_outs) ? mechanic.carve_outs : []).entries()) {
      ...
      reviewQueue.push(reviewItem(resolvedCarveOut, item));
    }
  }
