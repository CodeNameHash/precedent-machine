...
const STRICT_INDEPENDENT_REVIEW_SCHEMA = 'M3_12_CALL_FINAL_PILOT_STRICT_INDEPENDENT_REVIEW_INPUT/V3';
const FINAL_RE_REVIEW_FINDINGS_SCHEMA = 'M3_12_CALL_FINAL_PILOT_RE_REVIEW_FINDINGS/V2';
...
const FINAL_RE_REVIEW_RUBRIC = Object.freeze({
  ...
  required_failures: Object.freeze([
    ...
    'MISSING_FORMULA',
    'MISSING_FORMULA_SOURCE_CITATION',
...
function reviewSource(item) {
  if (item.source_kind === 'ADJUDICATED_FIRST_PASS_REPLAY') {
    return {
      ...
      additional_binding: {
        first_work_result: item.first_work_result,
        adjudication_binding: item.adjudication_binding,
      },
    };
  }
  if (item.source_kind === 'REPLAY_ONLY_CURRENT_RESOLVER_REPLAY') {
    return {
      source_output_id: item.repaired_replay?.repaired_replay_id,
      provider_recording_id: item.repaired_replay?.provider_recording?.provider_recording_id,
      source_output: item.repaired_replay,
      additional_binding: {
        first_work_result: item.first_work_result,
        prior_replay_result: item.prior_replay_result,
      },
    };
  }
  if (item.source_kind === 'PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY') {
    return {
      source_output_id: item.repaired_replay?.repaired_replay_id,
      provider_recording_id: item.repaired_replay?.provider_recording?.provider_recording_id,
      source_output: item.repaired_replay,
      additional_binding: { iteration_2_work_result: item.iteration_2_work_result },
    };
  }
  ...
  return {
    ...
    provider_recording_id: item.repaired_replay?.provider_recording?.provider_recording_id,
    source_output: item.repaired_replay,
    additional_binding: additionalBindingForCurrentResolverReplay(item),
  };
}

function additionalBindingForCurrentResolverReplay(item) {
  if (item.source_kind === 'ADJUDICATED_FIRST_PASS_REPLAY') {
    return { first_work_result: item.first_work_result, adjudication_binding: item.adjudication_binding };
  }
  if (item.source_kind === 'REPLAY_ONLY_CURRENT_RESOLVER_REPLAY') {
    return { first_work_result: item.first_work_result, prior_replay_result: item.prior_replay_result };
  }
  if (item.source_kind === 'PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY') {
    return { iteration_2_work_result: item.iteration_2_work_result };
  }
  return null;
}
...
function explicitFormulaSourceCitations(formula) {
  return Object.freeze(
    (Array.isArray(formula.source_citations) ? formula.source_citations : [])
      .filter((citation) => typeof citation === 'string' && citation.trim())
      .sort(),
  );
}

function formulaReviewRows(resolution, exactSourceBytes) {
  const conditionalTerminationFeeValues = (resolution.conditional_termination_fee_values || []).map((formula) => Object.freeze({
    conditional_termination_fee_value_id: formula.conditional_termination_fee_value_id || null,
    formula: Object.freeze({ ...formula }),
    source_citations: explicitFormulaSourceCitations(formula),
    exact_source_bytes: exactSourceBytes,
  }));
  const structuredPerShareCashValues = (resolution.structured_per_share_cash_values || []).map((formula) => Object.freeze({
    structured_per_share_cash_value_id: formula.structured_per_share_cash_value_id || null,
    formula: Object.freeze({ ...formula }),
    source_citations: explicitFormulaSourceCitations(formula),
    exact_source_bytes: exactSourceBytes,
  }));
...
function rowsForReview(item, exactSourceBytes) {
  ...
  const formulaRows = formulaReviewRows(resolution, exactSourceBytes);
  return Object.freeze({
    resolved_claims: Object.freeze((resolution.resolved || []).map((entry) => Object.freeze({
      ...
      attributes: Object.freeze({ ...(entry.claim?.attributes || {}) }),
      citation_context: entry.citation_context || null,
      exact_source_bytes: exactSourceBytes,
    }))),
    ...
    conditional_termination_fee_values: formulaRows.conditional_termination_fee_values,
    structured_per_share_cash_values: formulaRows.structured_per_share_cash_values,
  });
}
...
function hasExactFormulaText(row) {
  const formula = row.formula || {};
  const source = row.exact_source_bytes?.text;
  return typeof formula.raw_formula === 'string' && formula.raw_formula.trim()
    && typeof source === 'string' && source.includes(formula.raw_formula)
    && (!formula.operative_quote || source.includes(formula.operative_quote));
}

function validateFormulaReviews({
  finding,
  reviewItem,
  rows,
  rowKey,
  ...
  const expectedIds = exactIds(rows, rowKey, `sealed ${label}`, reviewItem.work_item_id);
  const observedRows = finding[reviewKey];
  const observedIds = exactIds(observedRows, rowKey, `${label} reviews`, reviewItem.work_item_id);
  equalIds(observedIds, expectedIds, `${label} reviews`, reviewItem.work_item_id);
  const formulasById = new Map(rows.map((row) => [row[rowKey], row]));
  for (const review of observedRows) {
    if (!exactKeys(review, [rowKey, 'value_status', 'formula_status', 'citation_lineage_status', 'status'])) {
      fail('INVALID_RE_REVIEW_FINDINGS', 'A formula review has an invalid closed shape.', { work_item_id: reviewItem.work_item_id });
    }
    for (const field of ['value_status', 'formula_status', 'citation_lineage_status', 'status']) {
      requirePassOrFail(review[field], `formula review ${field}`, reviewItem.work_item_id);
    }
    const formulaRow = formulasById.get(review[rowKey]);
    const formula = formulaRow.formula || {};
    const unsupportedValue = formula.base_amount == null || typeof formula.currency !== 'string' || !formula.currency.trim();
    const missingFormula = !hasExactFormulaText(formulaRow);
    const missingCitationOrLineage = !nonEmptyStrings(formulaRow.source_citations)
      || !nonEmptyStrings(formula.defined_term_lineage);
    const hardFailure = unsupportedValue || missingFormula || missingCitationOrLineage;
    if ((unsupportedValue && review.value_status !== 'FAIL')
      || (missingFormula && review.formula_status !== 'FAIL')
      || (missingCitationOrLineage && review.citation_lineage_status !== 'FAIL')) {
      fail('RE_REVIEW_RUBRIC_VIOLATION', 'Every unsupported formula value, formula, citation, or lineage must be recorded in its review field.', {
        work_item_id: reviewItem.work_item_id,
        formula_id: review[rowKey],
      });
    }
    if (hardFailure && review.status !== 'FAIL') {
      fail('RE_REVIEW_RUBRIC_VIOLATION', 'A formula with an unsupported value, missing formula, citation, or lineage cannot pass.', {
        work_item_id: reviewItem.work_item_id,
        formula_id: review[rowKey],
      });
    }
    if (review.status === 'FAIL' && finding.status !== 'FAIL') {
      fail('RE_REVIEW_RUBRIC_VIOLATION', 'A failed formula cannot receive a PASS finding.', {
        work_item_id: reviewItem.work_item_id,
        formula_id: review[rowKey],
      });
    }
...
function requireDistinctFormulaIds(reviewItem) {
  const formulaIds = [
    ...(reviewItem.review_rows.conditional_termination_fee_values || [])
      .map((row) => row.conditional_termination_fee_value_id),
    ...(reviewItem.review_rows.structured_per_share_cash_values || [])
      .map((row) => row.structured_per_share_cash_value_id),
  ];
  if (new Set(formulaIds).size !== formulaIds.length) {
    fail('INVALID_RE_REVIEW_INPUT', 'Formula identities must be unique across all formula sidecars.', { work_item_id: reviewItem.work_item_id });
  }
}

function exactSourceBytesByDocumentHashFromInput({ packet, input }) {
  const packetItems = new Map(packet.work_items.map((item) => [item.work_item_id, item]));
  const bytesByDocumentHash = {};
  for (const reviewItem of input.review_items) {
    const packetItem = packetItems.get(reviewItem.work_item_id);
    const source = packetItem ? reviewSource(packetItem) : null;
    const documentHash = source?.source_output?.run_receipt?.document_hash;
    const sourceSha256 = source?.source_output?.run_receipt?.source_sha256;
    if (!digest(documentHash) || !digest(sourceSha256)) {
      fail('EXACT_SOURCE_BYTES_REQUIRED', 'Every sealed review item must bind exact source bytes.', { work_item_id: reviewItem.work_item_id });
    }
    const rows = [
      ...(reviewItem.review_rows?.resolved_claims || []),
      ...(reviewItem.review_rows?.conditional_termination_fee_values || []),
      ...(reviewItem.review_rows?.structured_per_share_cash_values || []),
    ];
    const sources = rows.map((row) => row.exact_source_bytes).filter(Boolean);
    if (sources.length === 0) {
      fail('EXACT_SOURCE_BYTES_REQUIRED', 'Every sealed review item must embed exact source bytes.', { work_item_id: reviewItem.work_item_id });
    }
    for (const exactSource of sources) {
      if (exactSource.document_hash !== documentHash || exactSource.source_sha256 !== sourceSha256
        || typeof exactSource.text !== 'string'
        || sha256Hex(Buffer.from(exactSource.text, 'utf8')) !== sourceSha256) {
        fail('EXACT_SOURCE_BYTES_INVALID', 'Embedded source bytes do not match the final packet source binding.', { work_item_id: reviewItem.work_item_id });
      }
      if (bytesByDocumentHash[documentHash] && bytesByDocumentHash[documentHash] !== exactSource.text) {
        fail('EXACT_SOURCE_BYTES_INVALID', 'The sealed review input has conflicting exact source bytes.', { work_item_id: reviewItem.work_item_id });
      }
      bytesByDocumentHash[documentHash] = exactSource.text;
    }
  }
  return bytesByDocumentHash;
}

function validateSealedStrictReviewInput({ packet, input }) {
  validatePacket(packet);
  if (!input || input.schema_version !== STRICT_INDEPENDENT_REVIEW_SCHEMA
    || !digest(input.strict_independent_review_input_id)) {
    fail('INVALID_RE_REVIEW_INPUT', 'The strict review input must have the current schema and a content identity.');
  }
  const body = { ...input };
  delete body.strict_independent_review_input_id;
  if (contentId(STRICT_INDEPENDENT_REVIEW_SCHEMA, body) !== input.strict_independent_review_input_id) {
    fail('STRICT_REVIEW_INPUT_ID_MISMATCH', 'The strict review input does not match its content identity.');
  }
  const rebuilt = buildFinalPilotStrictIndependentReviewInput({
    final_review_packet: packet,
    exact_source_bytes_by_document_hash: exactSourceBytesByDocumentHashFromInput({ packet, input }),
  });
  if (canonicalJson(rebuilt) !== canonicalJson(input)) {
    fail('STRICT_REVIEW_INPUT_REBUILD_MISMATCH', 'The strict review input does not exactly rebuild from the sealed final packet and source bytes.');
  }
  return rebuilt;
}

function validateFindingsArtifactBinding({ artifact, input, packet, index, artifactCount }) {
  if (!artifact || artifact.schema_version !== FINAL_RE_REVIEW_FINDINGS_SCHEMA || !Array.isArray(artifact.findings)
    || (artifactCount === 1 && artifact.findings.length !== 12)
    || (artifactCount === 2 && artifact.findings.length !== 6)
    || artifact.final_review_packet_id !== packet.final_review_packet_id
    || artifact.strict_independent_review_input_id !== input.strict_independent_review_input_id
    || !digest(artifact.independent_legal_review_findings_id)) {
    fail('RE_REVIEW_FINDINGS_COVERAGE', `Findings artifact ${index + 1} has invalid coverage or bindings.`);
  }
  const body = { ...artifact };
  delete body.independent_legal_review_findings_id;
  if (contentId(FINAL_RE_REVIEW_FINDINGS_SCHEMA, body) !== artifact.independent_legal_review_findings_id) {
    fail('RE_REVIEW_FINDINGS_ID_MISMATCH', `Findings artifact ${index + 1} does not match its content identity.`);
  }
...
function validateFinding(finding, reviewItem) {
  const expectedKeys = [
    ...
    'open_world_reviews',
    'conditional_termination_fee_value_reviews',
    'structured_per_share_cash_value_reviews',
  ];
  ...
  validateFormulaReviews({
    finding,
    reviewItem,
    rows: reviewItem.review_rows.conditional_termination_fee_values || [],
    rowKey: 'conditional_termination_fee_value_id',
    reviewKey: 'conditional_termination_fee_value_reviews',
    label: 'conditional termination-fee value',
  });
  validateFormulaReviews({
    finding,
    reviewItem,
    rows: reviewItem.review_rows.structured_per_share_cash_values || [],
    rowKey: 'structured_per_share_cash_value_id',
    reviewKey: 'structured_per_share_cash_value_reviews',
    label: 'structured per-share cash value',
  });
  requireDistinctFormulaIds(reviewItem);
...
function validateFinalPilotReReviewFindings({
  final_review_packet: packet,
  strict_independent_review_input: input,
  finding_artifacts: artifacts,
} = {}) {
  const sealedInput = validateSealedStrictReviewInput({ packet, input });
  if (!Array.isArray(sealedInput.review_items) || sealedInput.review_items.length !== 12) {
  ...
  const findings = artifacts.flatMap((artifact, index) => validateFindingsArtifactBinding({
    artifact,
    input: sealedInput,
    packet,
    index,
  ...
  const reviewItems = new Map(sealedInput.review_items.map((item) => [item.work_item_id, item]));
