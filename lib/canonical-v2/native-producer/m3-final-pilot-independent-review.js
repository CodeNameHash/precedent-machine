...
function explicitFormulaSourceCitations(resolution, formula) {
  const citations = new Set(
    (Array.isArray(formula.source_citations) ? formula.source_citations : [])
      .filter((citation) => typeof citation === 'string' && citation.trim()),
  );
  if (typeof formula.operative_quote === 'string' && formula.operative_quote.trim()) {
    for (const entry of [
      ...(resolution.resolved || []),
      ...(resolution.review_queue || []),
      ...(resolution.open_world || []),
    ]) {
      if (typeof entry?.source_citation !== 'string' || !entry.source_citation.trim()) continue;
      const sourceQuote = entry.claim?.raw_value || entry.raw_value || '';
      if (sourceQuote.includes(formula.operative_quote)) citations.add(entry.source_citation);
    }
  }
  return Object.freeze([...citations].sort());
}
...
function formulaReviewRows(resolution, exactSourceBytes) {
  const conditionalTerminationFeeValues = (resolution.conditional_termination_fee_values || []).map((formula) => Object.freeze({
    ...
    source_citations: explicitFormulaSourceCitations(resolution, formula),
...
function validateFinalPilotReReviewFindings({ strict_independent_review_input: input, finding_artifacts: artifacts } = {}) {
  if (!input || input.schema_version !== STRICT_INDEPENDENT_REVIEW_SCHEMA
    || !Array.isArray(input.review_items) || input.review_items.length !== 12) {
