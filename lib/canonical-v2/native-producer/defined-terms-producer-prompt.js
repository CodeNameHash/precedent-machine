...
PRESERVE THE NOVEL. For every definition that is not a controlled assertion, emit a definition_envelopes entry. It keeps exact source evidence and a term identity for side-by-side Review and Query only. Do not treat same-named terms as equivalent, infer an owner, or publish a market statistic. A cross_reference_target is allowed only when it appears in definition_body_quote. All-or-substantially-all thresholds, uncertain kinds or codes, and any shape outside the controlled vocabulary go to open_world_candidates as well when no envelope can state it faithfully. Do not force a nearest fit.
