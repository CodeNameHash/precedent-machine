...
const RESPONSE_SHAPE = `{
  ...
  "definition_envelopes": [{
    "section_reference": "verbatim section reference or null",
    "defined_term": "exact term as printed",
    "definition_kind": "TERM_DEFINITION | CROSS_REFERENCE | INLINE_PARENTHETICAL",
    "definition_head_quote": "exact contiguous definition-head quote",
    "definition_body_quote": "exact contiguous definition body quote",
    "cross_reference_target": null,
    "owner_hint": null
  }],
  "mechanics": [{
    "surface": "DEF_PRODUCT_OUTPUT | ACQUISITION_THRESHOLD | SUPERIOR_THRESHOLD | ORDINARY_COURSE_STRUCTURE | EFFECTIVE_THRESHOLD_RELATIONSHIP",
    "quote": "exact contiguous deferred mechanism",
    "detail": "factual description only"
  }],
...
BOUNDARIES. The Material Adverse Effect definition belongs to another producer. Emit no assertion and no open-world candidate for it. Termination-fee amounts and triggers, no-shop procedure and day counts, caps that create an undue burden, and closing conditions also belong to other families.

NEVER ASSERT A NEGATIVE FROM SILENCE. Never emit an absent definition, NA knowledge standard, or absent exclusion.

PRESERVE THE NOVEL. For every definition that is not a controlled assertion, emit a definition_envelopes entry. It keeps exact source evidence and a term identity for side-by-side Review and Query only. Tax, Tax Return, Made Available and Ordinary Course use this exact-envelope route. For those terms, report no inferred legal meaning, subtype, equivalence or market statistic. A cross_reference_target is allowed only when it appears in definition_body_quote. Put a deferred product-output or threshold-relationship mechanism in mechanics only when the controlled assertion or neutral envelope cannot state it faithfully. A mechanics surface label routes exact evidence only and is not a governed legal code. Never duplicate a fact across assertions, envelopes and mechanics. MAE disproportionality, its comparison baseline, its source form and its carve-out relationship belong only to the MAE-definition producer. Never emit them in mechanics. All-or-substantially-all thresholds, uncertain kinds or codes, and any shape outside the controlled vocabulary go to open_world_candidates as well when no envelope can state it faithfully. Do not force a nearest fit.
