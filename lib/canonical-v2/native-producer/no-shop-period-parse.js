...
const NO_SHOP_PERIOD_PARSE_VERSION = 1;
...
// spelled-restatement close-paren), optional whitespace, then the unit
// phrase itself -- anchored at the start of the remainder so ANY other
// intervening character (in particular a defined-term parenthetical opening
// with '(') fails the match entirely, never partially.
const UNIT_ADJACENCY_PATTERN = /^\)?\s*((?:business|calendar)\s+days?|days?|hours?)\b/i;
...
// UNSPECIFIED_DAYS / HOURS / UNCORROBORATED, plus the verbatim unit_phrase
...
function classifyUnitAdjacency(quote, token) {
  ...
  else unitOutcome = 'UNSPECIFIED_DAYS';
...
function parseNoShopPeriod(quote) {
  ...
  const dayCandidates = classified.filter(
    (entry) => entry.unit_outcome === 'BUSINESS_DAYS'
      ...
      || entry.unit_outcome === 'UNSPECIFIED_DAYS',
  );
