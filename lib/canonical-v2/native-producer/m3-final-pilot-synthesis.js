...
const REPAIRED_REPLAY_SCHEMA = 'M3_12_CALL_FINAL_PILOT_REPAIRED_REPLAY/V2';
const DIGEST_RE = /^[a-f0-9]{64}$/;
const COMMIT_RE = /^[a-f0-9]{40}$/;
const RETAINED_IDS = new Set(['modiv-consideration-2-1', 'modiv-termination-fee-7-3']);
const ADJUDICATED_FIRST_PASS_REPLAY = 'ADJUDICATED_FIRST_PASS_REPLAY';
const REPLAY_ONLY_CURRENT_RESOLVER_REPLAY = 'REPLAY_ONLY_CURRENT_RESOLVER_REPLAY';
const PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY = 'PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY';
...
const FIRST_PASS_REPAIR_IDS = new Set([
...
async function replayProviderRecording({
  work_item,
  prior_result: prior,
  source_execution_kind: sourceExecutionKind,
  manifest,
  ...
  const semanticItems = semanticManifest?.work_items || validateUnifiedRunManifest({ manifest, root_dir: rootDir }).semantic_manifest.work_items;
  const semantic = semanticItems.find((item) => item.work_item_id === work_item.work_item_id);
  ...
  const runReceipt = await runNativeExtraction({
    ...
    source_tree_limb_citations: semantic.family_id === 'CAPITALISATION',
    ...
    provider: async ({ governed_scope: governedScope }) => {
      try {
        return reshapeRecordedProviderOutput({
          ...
          section_family: semantic.family_id,
...
async function buildFinalPilotSynthesis({
  ...
  contract_bundle: contractBundle = compileFixtureContractV34(),
  definitions = Object.freeze({ known_definitions: Object.freeze([]) }),
  replay_recording = replayProviderRecording,
} = {}) {
  ...
  const manifestValidation = validateUnifiedRunManifest({ manifest: manifestToUse, root_dir: rootDir });
  ...
  for (const workItemId of [...repairs].sort()) {
    const sourceExecutionKind = FIRST_PASS_REPAIR_IDS.has(workItemId) ? 'FIRST_PASS' : 'ITERATION_2';
    const priorResult = sourceExecutionKind === 'FIRST_PASS'
      ? firstById.get(workItemId)
      : iteration2ById.get(workItemId);
    const replay = await replay_recording({
      work_item: manifestById.get(workItemId),
      prior_result: priorResult,
      source_execution_kind: sourceExecutionKind,
      manifest: manifestToUse,
      semantic_manifest: manifestValidation.semantic_manifest,
      root_dir: rootDir,
      contract_bundle: contractBundle,
      definitions,
    });
    ...
    if (replay.work_item_id !== workItemId || replay.model_call_count !== 0
      || replay.source_execution_kind !== sourceExecutionKind
      || replay.source_work_result_id !== priorResult.work_result_id
      || replay.provider_recording?.provider_recording_id !== priorResult.provider_recording.provider_recording_id) {
      fail('INVALID_REPAIRED_REPLAY', 'A repaired replay does not retain its explicitly selected provider recording.');
    }
    repairedById.set(workItemId, replay);
  }
  const retainedReplaysById = new Map();
  for (const workItemId of [...RETAINED_IDS].sort()) {
    const priorResult = firstById.get(workItemId);
    const replay = await replay_recording({
      work_item: manifestById.get(workItemId),
      prior_result: priorResult,
    ...
    sealedBody(replay, 'repaired_replay_id', REPAIRED_REPLAY_SCHEMA, 'INVALID_ADJUDICATED_FIRST_PASS_REPLAY');
    if (replay.work_item_id !== workItemId || replay.model_call_count !== 0
      || replay.source_execution_kind !== 'FIRST_PASS'
      || replay.source_work_result_id !== priorResult.work_result_id
      || replay.provider_recording?.provider_recording_id !== priorResult.provider_recording.provider_recording_id) {
      fail('INVALID_ADJUDICATED_FIRST_PASS_REPLAY', 'An adjudicated first-pass replay does not retain its exact first-pass recording.');
    }
    retainedReplaysById.set(workItemId, replay);
  }
  const replayOnlyCurrentResolverById = new Map();
  for (const workItemId of [...REPLAY_ONLY_IDS].sort()) {
    const priorResult = firstById.get(workItemId);
    const replay = await replay_recording({
      work_item: manifestById.get(workItemId),
      prior_result: priorResult,
      source_execution_kind: 'REPLAY_ONLY_FIRST_PASS',
      manifest: manifestToUse,
      semantic_manifest: manifestValidation.semantic_manifest,
      root_dir: rootDir,
      contract_bundle: contractBundle,
    ...
    sealedBody(replay, 'repaired_replay_id', REPAIRED_REPLAY_SCHEMA, 'INVALID_REPLAY_ONLY_CURRENT_RESOLVER_REPLAY');
    if (replay.work_item_id !== workItemId || replay.model_call_count !== 0
      || replay.source_execution_kind !== 'REPLAY_ONLY_FIRST_PASS'
      || replay.source_work_result_id !== priorResult.work_result_id
      || replay.provider_recording?.provider_recording_id !== priorResult.provider_recording.provider_recording_id) {
      fail('INVALID_REPLAY_ONLY_CURRENT_RESOLVER_REPLAY', 'A replay-only item does not retain its exact first-pass provider recording.');
    }
    replayOnlyCurrentResolverById.set(workItemId, replay);
  }
  const passedIteration2Result = iteration2ById.get(PASSED_ITERATION_2_ID);
  const passedIteration2CurrentResolverReplay = await replay_recording({
    work_item: manifestById.get(PASSED_ITERATION_2_ID),
    prior_result: passedIteration2Result,
    source_execution_kind: 'PASSED_ITERATION_2',
    manifest: manifestToUse,
    semantic_manifest: manifestValidation.semantic_manifest,
    root_dir: rootDir,
    contract_bundle: contractBundle,
  ...
  sealedBody(passedIteration2CurrentResolverReplay, 'repaired_replay_id', REPAIRED_REPLAY_SCHEMA,
    'INVALID_PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY');
  if (passedIteration2CurrentResolverReplay.work_item_id !== PASSED_ITERATION_2_ID
    || passedIteration2CurrentResolverReplay.model_call_count !== 0
    || passedIteration2CurrentResolverReplay.source_execution_kind !== 'PASSED_ITERATION_2'
    || passedIteration2CurrentResolverReplay.source_work_result_id !== passedIteration2Result.work_result_id
    || passedIteration2CurrentResolverReplay.provider_recording?.provider_recording_id
      !== passedIteration2Result.provider_recording.provider_recording_id) {
    fail('INVALID_PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY', 'The passed iteration-2 item does not retain its exact selected provider recording.');
  }
  const workItems = [...expectedIds].sort().map((workItemId) => {
    if (RETAINED_IDS.has(workItemId)) return Object.freeze({
      work_item_id: workItemId, source_kind: ADJUDICATED_FIRST_PASS_REPLAY, model_call_count: 0,
      first_work_result: firstById.get(workItemId), adjudication_binding: adjudications.get(workItemId),
      repaired_replay: retainedReplaysById.get(workItemId),
    ...
    if (REPLAY_ONLY_IDS.has(workItemId)) return Object.freeze({
      work_item_id: workItemId, source_kind: REPLAY_ONLY_CURRENT_RESOLVER_REPLAY, model_call_count: 0,
      first_work_result: firstById.get(workItemId), prior_replay_result: replayOnlyById.get(workItemId),
      repaired_replay: replayOnlyCurrentResolverById.get(workItemId),
      legal_disposition: 'NOT_DETERMINED', independent_review_state: 'PENDING_INDEPENDENT_LEGAL_REVIEW',
    });
    if (workItemId === PASSED_ITERATION_2_ID) return Object.freeze({
      work_item_id: workItemId, source_kind: PASSED_ITERATION_2_CURRENT_RESOLVER_REPLAY, model_call_count: 0,
      iteration_2_work_result: passedIteration2Result,
      repaired_replay: passedIteration2CurrentResolverReplay,
      legal_disposition: 'NOT_DETERMINED', independent_review_state: 'PENDING_INDEPENDENT_LEGAL_REVIEW',
    });
