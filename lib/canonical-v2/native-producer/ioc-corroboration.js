...
const {
  ...
  LEGACY_RESTRICTION_CATEGORY_QUOTE_ALIASES,
...
function normaliseRestrictionCategory(category, quote = null) {
  ...
  if (compatibility?.source_quote === quote) return compatibility.canonical_category;
  return LEGACY_RESTRICTION_CATEGORY_QUOTE_ALIASES.find((entry) => (
    entry.legacy_category === category && entry.source_quote === quote
...
function corroborateRestrictionCategory({ quote, asserted_category: assertedCategory } = {}) {
  ...
  const compatibilityCategory = LEGACY_RESTRICTION_CATEGORY_QUOTE_ALIASES.find((entry) => (
    entry.canonical_category === assertedCategory && entry.source_quote === quote
  ))?.canonical_category;
  const matches = [...new Set([...categoryMatches(quote), ...(compatibilityCategory ? [compatibilityCategory] : [])])].sort();
  if (matches.length > 1) {
