...
//   - Step 2X-A: `buildMarkerTree` / `findMarkerCandidates` are also the
//     line-anchored second input to `resolveGoverningStructure`
//     corroboration tiers (CORROBORATED / LINE_ANCHORED_ONLY /
//     PERMISSIVE_ONLY). Parentage agreement with segmentSubClauses does
//     NOT confirm tree shape — both mis-nest restarting lists.
...
// Letter-track continuation, covering both the single-letter run ("a".."z")
// and the doubled/tripled/... "repeated-letter" overflow convention real
// drafting uses once a list runs past 26 items -- "...x, y, z, aa, bb, ...,
// zz, aaa, bbb, ...". Confirmed against two real, independent corpus filings
// (docs/codex-program/notes/nested-lettering-collision.md): Modiv Section
// 8.12 (a 74-entry definitions list) and Skechers Section 1.1 (a 108-entry
// definitions list) both print exactly this shape once their own outer
// letter list runs past "(z)".
//
// Previously, a single-letter value of "z"/"Z" returned null ("no next"),
// and a repeated-letter value (e.g. "aa") returned one MORE repetition of
// the same letter ("aaa") rather than the next letter at the same length
// ("bb") -- so the real printed sequence "(z)(aa)(bb)(cc)..." could never be
// recognised as ONE continuing list: "(z)" had no defined successor at all,
// and even "(aa)" itself did not expect "(bb)" next. Every marker from the
// 27th item onward therefore matched no open sequence and fell through to
// the "open a brand-new nested child" path in buildMarkerTree, one level
// deeper each time -- misparenting the rest of the list as an ever-deepening
// chain of descendants of the 26th item (see the design note for the full
// before/after tree dump). `value` here is always either a single letter or
// a run of one repeated letter -- the only two shapes `classifyOpeningKind`/
// `kindMatchesLabel` ever accept for this kind.
...
function expectedNext(kind, value) {
  switch (kind) {
    case 'LOWER_LETTER':
    case 'UPPER_LETTER':
      return nextLetterSequence(value);
    case 'LOWER_ROMAN':
...
// SEC conversions also render a real heading as `1.1.` followed by a line
// break and the title. The optional terminal dot is deliberately limited to
// that heading separator shape, not a general decimal-number match.
//
// Title-length bound (docs/codex-program/notes/swallowed-headings.md): this
// was previously {1,78}?, which silently dropped whole sections on real
// filings whose titles simply run long -- TopBuild's real Section 4.5 title
// ("Company Stockholder Meeting and Parent Stockholder Meeting; Form S-4 and
// Joint Proxy Statement/Prospectus") is 105 characters. A too-long title
// doesn't just fail to match ITSELF: because the trailing `.` here is
// required (not optional) and the whole match fails when the title capture
// can't complete, the candidate is silently never added to the raw candidate
// list at all -- not even as a recorded rejection (contrast a LONE_CANDIDATE
// or NON_MONOTONIC_SEQUENCE rejection, both of which post to
// rejected_inline_heading_candidates). Below applySequenceGate's >=2-member
// rule, that can additionally drag down an otherwise-fine SHORT sibling
// title in the same article (TopBuild ARTICLE III has only two sections;
// 3.2's title is 84 characters, so with the old cap 3.2 never became a
// candidate, leaving 3.1 -- whose own title fits easily -- alone, and
// alone is exactly what LONE_CANDIDATE rejects).
//
// The bound itself is not what protects against false positives: the `?`
// makes this quantifier LAZY, and the character class here excludes `.`/`!`/
// `?` entirely, so the match already stops at the FIRST period-then-
// whitespace/end it finds regardless of how high the cap is set -- a long
// cap cannot make this regex run on into unrelated body prose and swallow
// it as a "title". The cap is only a defensive upper bound on regex-engine
// work per attempt, not a false-positive gate; the real false-positive
// gates are the line-start anchor (genuine cross-reference prose is never
// written to open a line on a bare "N.M"), the required uppercase start,
// and the terminal-period-before-whitespace shape. Measured empirically
// (see swallowed-headings.md) across every real filing in this repo's
// corpus (Modiv, TopBuild, Skechers, Landos): raising the cap from 78 to
// 200 -- comfortably above the longest real title observed anywhere in that
// corpus (105 characters) -- adds zero new candidates in Modiv, Skechers or
// Landos, and in TopBuild adds exactly the 4 real titles the old cap
// silently dropped (1.8, 3.2, 4.5, 5.2; 3.1 then recovers too, once 3.2
// rejoins it as a second candidate and the sequence gate has two members
// again).
const INLINE_DECIMAL_HEADING_RE = /(?:^|\n)(\d{1,3})\.(\d{1,3})\.?[ \t\n]+([A-Z][A-Za-z0-9 ,;:'&()/-]{1,200}?)\.(?=\s|$)/g;
const INLINE_DECIMAL_MARKER_HEADING_RE = /(?:^|\n)(\d{1,3})\.(\d{1,3})\.?[ \t]+([A-Z][A-Za-z0-9 ,;:'&()/-]{1,200}?)(?=\n[ \t]*\([A-Za-z0-9]{1,9}\))/g;
...
function contentsTitlesBeforeArticle(clean, articleStart) {
  const prefix = clean.slice(0, articleStart);
  const contentsStart = prefix.lastIndexOf('TABLE OF CONTENTS');
  if (contentsStart < 0) return new Map();
  const lines = prefix.slice(contentsStart).split('\n').map((line) => line.trim());
  const titles = new Map();
  for (let index = 0; index < lines.length - 1; index += 1) {
    if (!/^\d{1,3}\.\d{1,3}$/.test(lines[index])) continue;
    const title = lines[index + 1] || '';
    if (/^[A-Z][^\n]{2,200}$/.test(title)) titles.set(lines[index], title);
  }
  return titles;
}

function scanInlineDecimalHeadingCandidates(body, articleArabic, contentsTitles = new Map()) {
  const candidates = [];
  for (const pattern of [INLINE_DECIMAL_HEADING_RE, INLINE_DECIMAL_MARKER_HEADING_RE]) {
    pattern.lastIndex = 0;
    let match;
    while ((match = pattern.exec(body)) !== null) {
      ...
      if (candidates.some((candidate) => candidate.localStart === localStart)) continue;
      candidates.push({
  ...
  const bareHeading = new RegExp(`(?:^|\\n)(${articleArabic})\\.(\\d{1,3})[ \\t]+([^\\n]+)`, 'g');
  let match;
  while ((match = bareHeading.exec(body)) !== null) {
    const reference = `${articleArabic}.${parseInt(match[2], 10)}`;
    const title = contentsTitles.get(reference);
    if (!title || !(match[3] === title || match[3].startsWith(`${title} `))) continue;
    const localStart = match.index + (match[0].startsWith('\n') ? 1 : 0);
    if (candidates.some((candidate) => candidate.localStart === localStart)) continue;
    candidates.push({
      reference,
  ...
  return candidates.sort((left, right) => left.localStart - right.localStart);
}
...
function applySequenceGate(candidates) {
  ...
  if (accepted.length === 1 && accepted[0].sectionNumber === 1) return { accepted, rejected };
...
// ─── Chapeau/INTRO boundary reconciliation (additive-pass follow-up) ───
//
// parseStructure's own article-chapeau detection (see structural.js's
// parseSections, the "Article chapeau" block) finds the gap between an
// ARTICLE heading and the first SECTION heading its own classifier can see,
// and mints a synthetic "<article>-INTRO" node for that gap. When every
// section in the article uses the bare, non-blank-line-anchored inline
// numbering this file's INLINE_DECIMAL_HEADING_RE pass exists for (see the
// block comment above it), parseStructure's classifier sees NO section
// headings anywhere in the article, so its "no interior boundary found"
// fallback treats the ENTIRE article body as chapeau. The pass below then
// mints the real, tightly-bounded interior sections on top of that —
// correctly — but without this reconciliation the stale "<article>-INTRO"
// node (and any marker-tree subsection children the main build derived
// from its now-wrong body) stays in the tree, silently overlapping the
// real section(s) it turns out to precede: a "nothing from a neighbouring
// section" violation, confirmed on real filings — TopBuild and Skechers
// both mint an INTRO node that swallows the article's first several real
// sections whole, with phantom subsection nodes misattributed to the
// INTRO reference instead of their real section.
//
// Fix: once a run of real inline sections is accepted for an article,
// clip any pre-existing direct child of that article whose span straddles
// the new first section's start down to that boundary, and rebuild its
// (now much smaller, usually empty) subsection tree from the corrected
// span. This never removes real chapeau prose — only the START of the
// clipped node is ever reused, so genuine lead-in text before the first
// real section (the "Representations" lead-in this mechanism exists for)
// is preserved exactly as before; only the erroneous OVER-EXTENSION past
// a boundary we now know about is corrected.
function collectSectionSubtreeIds(nodes, rootSectionId) {
  const ids = new Set([rootSectionId]);
  let grew = true;
  while (grew) {
    grew = false;
    for (const node of nodes) {
      if (ids.has(node.parent_section_id) && !ids.has(node.section_id)) {
        ids.add(node.section_id);
        grew = true;
      }
    }
  }
...
function reconcileStaleArticleChildren({
  out, documentHash, sourceText, layers, clean, articleSpan, firstAcceptedCleanStart,
}) {
  const firstAcceptedRawChar = layers.mapCleanToRaw(firstAcceptedCleanStart);
  const firstAcceptedByteStart = charToByteOffset(sourceText, firstAcceptedRawChar);

  const straddlers = out.filter((node) => (
    node.parent_section_id === articleSpan.node.section_id
    && node.start < firstAcceptedByteStart
    && node.end > firstAcceptedByteStart
  ));

  for (const stale of straddlers) {
    const subtreeIds = collectSectionSubtreeIds(out, stale.section_id);
    for (let i = out.length - 1; i >= 0; i--) {
      if (subtreeIds.has(out[i].section_id)) out.splice(i, 1);
    }

    // The only shape we know how to rebuild with confidence is
    // parseSections' own article-chapeau node, which is ALWAYS minted
    // starting exactly at the article's own start (`startChar: artStart`
    // in structural.js). If some other straddling direct child ever shows
    // up with a different start, rebuilding it from that assumption would
    // fabricate a span we did not actually verify — worse than the
    // overlap this function exists to remove. Leave it removed and let
    // findSectionByReference fail loudly on that reference instead of
    // resolving it to an approximate boundary.
    if (stale.start !== articleSpan.node.start) continue;
    ...
    const rebuilt = makeNode({
      ...
      sourceText,
      rawStartChar: layers.mapCleanToRaw(articleSpan.cleanStart),
      rawEndChar: firstAcceptedRawChar,
      depth: stale.depth,
      parentId: stale.parent_section_id,
      required: false,
    });
    if (!rebuilt) continue; // clipped down to nothing real — correctly dropped, nothing lost

    const rebuiltCleanText = clean.slice(articleSpan.cleanStart, firstAcceptedCleanStart);
    const markerTree = buildMarkerTree(rebuiltCleanText);
    out.push(rebuilt);
    if (markerTree.length > 0) {
      appendMarkerNodes({
        ...
        containerCleanStart: articleSpan.cleanStart,
        containerRawStart: layers.mapCleanToRaw(articleSpan.cleanStart),
        parentReference: rebuilt.reference,
        parentId: rebuilt.section_id,
        depth: rebuilt.depth + 1,
        containerText: rebuiltCleanText,
      });
    }
  }
}
...
function appendInlineDecimalHeadingSections({
  ...
  for (const articleSpan of articleSpans) {
    ...
    const rawCandidates = scanInlineDecimalHeadingCandidates(
      body,
      articleArabic,
      contentsTitlesBeforeArticle(clean, articleSpan.cleanStart),
    );
    const { accepted, rejected } = applySequenceGate(rawCandidates);

    if (accepted.length > 0) {
      reconcileStaleArticleChildren({
        out,
        documentHash,
        sourceText,
        layers,
        clean,
        articleSpan,
        firstAcceptedCleanStart: articleSpan.cleanStart + accepted[0].localStart,
      });
    }
...
// ─── Swallowed-heading tripwire (additive detection only — never mutates
// the tree, never rejects/accepts anything itself) ───
//
// docs/codex-program/notes/swallowed-headings.md. The sibling-tiling
// invariant this codebase already checks (assertSiblingsTileExactly in
// tests/canonical-v2-native-sectionizer.test.js) is structurally blind to a
// heading that is never recognised as a candidate AT ALL: its text is
// silently absorbed by the preceding sibling, which produces neither an
// overlap nor a gap, so the tree looks perfect and is wrong. The 78-character
// title cap that used to sit on INLINE_DECIMAL_HEADING_RE (see the comment
// above it) is the one concrete mechanism found and fixed for this corpus,
// but the property this pass guards is broader than that one mechanism: ANY
// future reason a genuine "N.M" heading fails to become a candidate — a
// different shape gate, a character this file's title character class
// doesn't allow, a regression in this pass entirely — would be exactly as
// invisible to every other check in this module. This pass is the backstop:
// it does not try to be the fix, only to make the failure loud.
//
// Deliberately looser than INLINE_DECIMAL_HEADING_RE itself: no title-shape
// requirement at all, just a bare line-start "N.M" whose N matches the
// enclosing article's own number. Verified empirically (swallowed-headings.md)
// against every real filing in this repo's corpus (Modiv, TopBuild, Skechers,
// Landos): this loose a pattern produces ZERO false positives anywhere in
// four full real merger agreements — the same reasoning that justifies the
// line-start anchor elsewhere in this file applies here too (genuine
// cross-reference prose is always prefixed, e.g. "Section 3.1", and is never
// drafted to open a line on a bare decimal number), so tightening the shape
// further was measured and found unnecessary, not assumed.
//
// A hit is reported as a residual only when its reference is BOTH absent
// from the final tree AND absent from rejected_inline_heading_candidates —
// a recorded rejection (LONE_CANDIDATE, NON_MONOTONIC_SEQUENCE) is already
// visible by design and is deliberately not re-reported here.
const SWALLOWED_HEADING_TRIPWIRE_RE = /(?:^|\n)(\d{1,3})\.(\d{1,3})\b(?!\()/g;

function detectSwallowedHeadingResiduals({
  nodes, rejectedInlineHeadingCandidates, clean, layers, sourceText, articleSpans,
}) {
  const knownReferences = new Set(nodes.filter((n) => n.reference).map((n) => n.reference));
  const rejectedReferences = new Set(rejectedInlineHeadingCandidates.map((entry) => entry.reference));
  ...
  for (const articleSpan of articleSpans) {
    ...
    if (!Number.isFinite(articleArabic)) continue;
    const body = clean.slice(articleSpan.cleanStart, articleSpan.cleanEnd);
    SWALLOWED_HEADING_TRIPWIRE_RE.lastIndex = 0;
    let match;
    while ((match = SWALLOWED_HEADING_TRIPWIRE_RE.exec(body)) !== null) {
      const articleDigits = parseInt(match[1], 10);
      if (articleDigits !== articleArabic) continue; // belongs to a different article's numbering (e.g. a cross-reference) — not this article's concern
      const reference = `${articleDigits}.${parseInt(match[2], 10)}`;
      if (knownReferences.has(reference) || rejectedReferences.has(reference)) continue; // already a node, or already a recorded (non-silent) rejection
      const localStart = match.index + (match[0].startsWith('\n') ? 1 : 0);
      const rawOffset = layers.mapCleanToRaw(articleSpan.cleanStart + localStart);
      residuals.push(Object.freeze({
        article: articleSpan.number,
        reference,
        reason: 'UNACCOUNTED_LINE_START_DECIMAL',
        start: charToByteOffset(sourceText, rawOffset),
      }));
    }
  }
...
function topLevelExhibitScopes(clean, parsedSections) {
  const mainSections = (parsedSections || []).filter((section) => (
    !section.isAnnex && /^\d{1,3}\.\d{1,3}/.test(String(section.number || ''))
  ));
  if (mainSections.length === 0) return [];
  const afterMain = Math.max(...mainSections.map((section) => section.endChar));
  const candidates = [];
  const pattern = /^[ \t]*EXHIBIT[ \t]+([A-Z])[ \t]*$/gim;
  for (const match of clean.matchAll(pattern)) {
    const start = match.index + (match[0].match(/^[ \t]*/)?.[0].length || 0);
    if (start >= afterMain) candidates.push({ label: match[1].toUpperCase(), cleanStart: start });
  }
  const accepted = [];
  let expected = 'A'.charCodeAt(0);
  for (const candidate of candidates) {
    if (candidate.label.charCodeAt(0) !== expected) continue;
    accepted.push(candidate);
    expected += 1;
  }
  return accepted.map((scope, index) => ({
    ...scope,
    cleanEnd: accepted[index + 1]?.cleanStart ?? clean.length,
    reference: `Exhibit-${scope.label}`,
  }));
}

function appendScopedExhibits({ out, documentHash, sourceText, layers, clean, rootNode, scopes }) {
  for (const scope of scopes) {
    const rawStart = layers.mapCleanToRaw(scope.cleanStart);
    const rawEnd = layers.mapCleanToRaw(scope.cleanEnd);
    const scopeText = clean.slice(scope.cleanStart, scope.cleanEnd);
    const scopeNode = makeNode({
      documentHash,
      kind: 'SECTION',
      reference: scope.reference,
      heading: scopeText.split('\n').slice(1).map((line) => line.trim()).find(Boolean) || null,
      sourceText,
      rawStartChar: rawStart,
      rawEndChar: rawEnd,
      depth: 1,
    ...
    const nested = parseStructure(scopeText);
    const realArticles = [...(nested.articles || [])]
      .filter((article) => !article.synthesized)
      .sort((left, right) => left.startChar - right.startChar);
    const nestedArticleByNumber = new Map();
    for (let index = 0; index < realArticles.length; index += 1) {
      const article = realArticles[index];
      const localEnd = realArticles[index + 1]?.startChar ?? nested.diagnostics.bodyEnd;
      if (!(localEnd > article.startChar)) continue;
      const built = makeNode({
        documentHash,
        kind: 'ARTICLE',
        reference: `${scope.reference}::ARTICLE ${article.number}`,
        heading: article.title || null,
        sourceText,
        rawStartChar: layers.mapCleanToRaw(scope.cleanStart + article.startChar),
        rawEndChar: layers.mapCleanToRaw(scope.cleanStart + localEnd),
        depth: 2,
        parentId: scopeNode.section_id,
        required: false,
      });
      if (!built) continue;
      out.push(built);
      nestedArticleByNumber.set(String(article.number || '').toUpperCase(), built);
    }

    for (const section of [...(nested.sections || [])].sort((left, right) => left.startChar - right.startChar)) {
      const parentArticle = nestedArticleByNumber.get(String(section.articleNumber || '').toUpperCase());
      if (!parentArticle || !(section.endChar > section.startChar)) continue;
      const reference = `${scope.reference}::${section.number}`;
      const built = makeNode({
        documentHash,
        kind: 'SECTION',
        reference,
        ...
        sourceText,
        rawStartChar: layers.mapCleanToRaw(scope.cleanStart + section.startChar),
        rawEndChar: layers.mapCleanToRaw(scope.cleanStart + section.endChar),
        depth: 3,
        parentId: parentArticle.section_id,
        required: false,
      });
      if (!built) continue;
      out.push(built);
      const sectionCleanText = scopeText.slice(section.startChar, section.endChar);
      const markerTree = buildMarkerTree(sectionCleanText);
      if (markerTree.length > 0) {
        appendMarkerNodes({
          out,
          ...
          layers,
          markerNodes: markerTree,
          containerCleanStart: scope.cleanStart + section.startChar,
          containerRawStart: layers.mapCleanToRaw(scope.cleanStart + section.startChar),
          parentReference: reference,
          parentId: built.section_id,
          depth: 4,
          containerText: sectionCleanText,
...
function sectionizeAdmittedSource({
  ...
  if (sourceText.length === 0) {
    return Object.freeze({
      ...
      swallowed_heading_residuals: Object.freeze([]),
    });
  }
  ...
  const exhibitScopes = topLevelExhibitScopes(clean, parsed.sections);
  const scopedExhibitReferences = new Set(exhibitScopes.map((scope) => scope.reference.toLowerCase()));
  ...
  const sortedArticles = [...(parsed.articles || [])]
    .filter((article) => !article.synthesized && !article.isAnnex)
    .sort((a, b) => a.startChar - b.startChar);
  for (let i = 0; i < sortedArticles.length; i++) {
    const article = sortedArticles[i];
    const cleanStart = article.startChar;
    // The final main-agreement article ends at the first retained exhibit
    // scope. Without that boundary its span would overlap every exhibit.
    const cleanEnd = i + 1 < sortedArticles.length
      ? sortedArticles[i + 1].startChar
      : (exhibitScopes[0]?.cleanStart ?? clean.length);
  ...
  for (const section of sortedSections) {
    if (section.isAnnex && scopedExhibitReferences.has(String(section.number || '').toLowerCase())) continue;
  ...
  appendScopedExhibits({
    ...
    clean,
    rootNode,
    scopes: exhibitScopes,
  });
  if (exhibitScopes.length > 0) anySectionOrSubsection = true;
  ...
  // Runs last, over the fully-built node list (including everything the
  // inline-decimal-heading pass and the whole-document marker-tree fallback
  // above added) — see the tripwire's own header comment for why this exists
  // and what it deliberately does NOT do (never mutates nodes, never gates
  // anything, only reports).
  const swallowedHeadingResiduals = detectSwallowedHeadingResiduals({
    nodes,
    rejectedInlineHeadingCandidates,
    clean,
    layers,
    sourceText,
    articleSpans,
  ...
  return Object.freeze({
    ...
    // Typed, never-silent record of every line-start "N.M" this module found
    // inside an article's own span, matching that article's own number, that
    // is neither a node anywhere in `nodes` above nor already explained by
    // `rejected_inline_heading_candidates` — i.e. a heading the source text
    // itself implies but that this module's own passes lost track of
    // entirely, for any reason. Always empty on a healthy tree.
    swallowed_heading_residuals: Object.freeze(
      swallowedHeadingResiduals.map((entry) => Object.freeze(entry)),
    ),
  });
}
...
module.exports = {
  ...
  // Step 2X-A corroboration second input (marker existence tiers).
