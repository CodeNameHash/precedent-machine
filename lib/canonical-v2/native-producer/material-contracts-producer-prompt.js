  ...
  "material_contract_criteria": [
    {
      ...
      "scope_exclusions": ["<concise extracted detail for each expressly excluded subject>"],
...
THRESHOLDS. Emit USD, PERCENT or TOP_N only when the same quote contains the exact value. Every discrete contract-inclusion list item without a quantitative floor is an ANY threshold: emit threshold_kind ANY and threshold_value "Any", even if the smallest quote does not contain the word "any". Use null only when the extracted clause makes no threshold assertion. Do not derive a threshold from another clause.

DEFINED TERMS. List a defined term or cross-reference only when its unresolved meaning can change the selected bucket or threshold. Never list a term merely because it is capitalised, appears in the quote, or affects only the parties or scope. The resolver will keep a listed term open-world. Do not infer the definition.

EXCLUSIONS. Put each expressly excluded subject in scope_exclusions as concise extracted detail, not as a second verbatim quote. Each detail must occur exactly inside the criterion quote. Use [] when the criterion has no express exclusion. Do not infer an exclusion.
