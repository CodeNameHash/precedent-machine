...
function admitRecordedSource(source, rootDir) {
  let recorded;
  try {
  ...
  let reference;
  try {
    reference = buildAdmittedSourceReference(context);
  } catch (error) {
    fail('INVALID_RECORDED_CONTEXT', `source ${source.source_id} recorded context failed closed-shape validation.`);
  }
  ...
  if (canonicalJson(reference) !== canonicalJson(source.admitted_source_reference)) {
    fail('RECORDED_SOURCE_REFERENCE_MISMATCH', `source ${source.source_id} recorded context does not match its admitted source reference pin.`);
  }
  return Object.freeze({
...
function admitSource(source, rootDir) {
  ...
  if (source.disposition === 'ADMITTED_RECORDED') return admitRecordedSource(source, rootDir);
...
function loadAdmittedSourceForExecution({ source, root_dir: rootDir = process.cwd() } = {}) {
  ...
  const admitted = admitSource(source, rootDir);
  if (admitted.disposition === 'BLOCKED_SOURCE_PIN') {
    fail('BLOCKED_SOURCE_PIN', `source ${source.source_id} is not admitted for extraction.`);
  }
  return admitted;
}
