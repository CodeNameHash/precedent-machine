...
function shapeMaterialContractCriterion(criterion, sourceBytes, ordinalFor, dropCounter) {
  ...
  let scopeExclusions = criterion.scope_exclusions;
  if (!Object.hasOwn(criterion, 'scope_exclusions')) {
    const extracted = [];
    const pattern = /\((?:other\s+than|excluding|except\s+for)\s+([^()]+?)\)/gi;
    let match;
    while ((match = pattern.exec(String(criterion.quote || ''))) !== null) {
      const detail = match[1].trim();
      if (detail && !extracted.includes(detail)) extracted.push(detail);
    }
    scopeExclusions = extracted.length > 0 ? extracted : undefined;
  }
  ...
  const common = {
    ...
    attributes: {
      ...
      ...(scopeExclusions === undefined ? {} : { scope_exclusions: scopeExclusions }),
