...
function shapeRepresentationInstance(instance, sourceBytes, ordinalFor, dropCounter) {
  ...
  for (const limb of (Array.isArray(instance.limbs) ? instance.limbs : [])) {
    ...
    const limbReference = limb.limb_reference ?? null;
    ...
    if (assertionEvidence) {
      proposals.push({
        ...
        attributes: {
          section_reference: sectionReference,
          party_making: partyMaking,
          limb_reference: limbReference,
          subject: limb.subject ?? null,
        },
        allowed_attributes: ['section_reference', 'party_making', 'limb_reference', 'subject'],
        taxonomy_codes: {},
        codebooks: {},
    ...
    for (const qualifier of (Array.isArray(limb.qualifiers) ? limb.qualifiers : [])) {
      ...
      proposals.push({
        ...
        attributes: {
          ...
          attachment: qualifier.attachment ?? null,
        },
        allowed_attributes: [
          'section_reference', 'party_making', 'limb_reference', 'qualifier_kind', 'attachment',
        ],
...
function shapeProposals(parsed, sourceText) {
  ...
  return { proposals, dropped_quote_mismatches: dropped };
}
...
function createAnthropicProvider({
  ...
  return async function anthropicProvider({ governed_scope: governedScope, definitions } = {}) {
    ...
    const { proposals, dropped_quote_mismatches: droppedQuoteMismatches } =
    ...
    return {
      ...
      proposals,
      dropped_quote_mismatches: droppedQuoteMismatches,
