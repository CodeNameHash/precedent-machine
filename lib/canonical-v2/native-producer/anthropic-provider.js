...
function isIncompleteSpecificPerformanceGrant(assertion) {
...
function shapeSpecificPerformanceRemedyProposals(parsed, sourceText) {
  ...
  const incompleteGrants = assertions.filter(isIncompleteSpecificPerformanceGrant);
  const shaped = shapePresenceAssertions({
    parsed: {
      ...parsed,
      remedy_assertions: assertions.filter((assertion) => !isIncompleteSpecificPerformanceGrant(assertion)),
    },
    sourceText,
    ...
    allowedKinds: REMEDY_ASSERTION_KINDS,
