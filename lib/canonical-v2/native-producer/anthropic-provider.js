...
function representationSideFor(partyMaking) { if (typeof partyMaking !== 'string') return null; const target = /\b(?:company|target|seller|partnership)\b/i.test(partyMaking); const buyer = /\b(?:parent|buyer|acquir)/i.test(partyMaking); return target === buyer ? null : (target ? 'TARGET' : 'BUYER'); }
  ...
  return async function anthropicProvider({
    ...
    for (let attempt = 0; attempt < totalAttempts; attempt += 1) {
      ...
      try {
        ...
        // the same prompt against the same document will not shrink it, so
        // stop immediately rather than spending a retry (and its backoff
        // wait) on a failure mode retries cannot fix.
