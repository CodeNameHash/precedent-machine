...
function shapeMiscBoilerplateProposals(parsed, sourceText) { return shapePresenceAssertions({ parsed, sourceText, listKey: 'boilerplate_assertions', claimKey: MISC_BOILERPLATE_CLAIM_KEY, proposalKind: MISC_BOILERPLATE_PROPOSAL_KIND, allowedKinds: MISC_ASSERTION_KINDS, subjectKind: 'MISC_BOILERPLATE' }); }
...
function shapeGovernedTdaAssertions(parsed, sourceText, listKey, claimKey, proposalKind) {
  ...
  const proposals = assertions.map((assertion) => {
    ...
    const attributes = Object.fromEntries(['section_reference', 'assertion_kind', 'treatment_scope_ref', 'treatment_term_ref', 'bearer_ref', 'dividend_term_ref', 'statute_ref'].map((key) => [key, assertion[key] ?? null]));
...
function shapeMergerStructureProposals(parsed, sourceText) {
  return shapePresenceAssertions({ parsed, sourceText, listKey: 'structure_assertions', claimKey: MERGER_STRUCTURE_CLAIM_KEY, proposalKind: MERGER_STRUCTURE_PROPOSAL_KIND, allowedKinds: STRUCTURE_SURFACES, subjectKind: 'MERGER_STRUCTURE' });
}
...
function deriveZeroShareCountAssertions(representationInstances, assertions) {
  const derived = [];
  const seen = new Set();
  const identityFor = ({ section_reference: sectionReference, party_making: partyMaking, count_kind: countKind, share_class: shareClass, limb_path: limbPath }) => canonicalJson({
    section_reference: sectionReference ?? null,
    party_making: partyMaking ?? null,
    count_kind: countKind ?? null,
    share_class: shareClass ?? null,
    limb_path: isNonEmptyLimbPath(limbPath) ? limbPath : null,
  });

  for (const assertion of assertions) {
    if (!assertion || typeof assertion !== 'object' || assertion.count_kind !== 'ISSUED_OUTSTANDING') continue;
    seen.add(identityFor(assertion));
  }

  const add = (assertion) => {
    const identity = identityFor(assertion);
    ...
    for (const limb of (Array.isArray(instance.limbs) ? instance.limbs : [])) {
      ...
      if (noneOutstanding) {
        add({
          ...
          quote: `shares of ${noneOutstanding[1]}, ${noneOutstanding[2]}`,
        });
      }
    }
  }
...
function shapeConsiderationProposals(parsed, sourceText) {
  ...
  for (const mechanic of (Array.isArray(parsed.consideration_mechanics)
    ...
    const alternatives = Array.isArray(mechanic?.alternatives)
      ? mechanic.alternatives.map(exact).filter(Boolean) : [];
    const structuredMechanic = {
      surface,
      ...(alternatives.length ? { alternatives } : {}),
      ...Object.fromEntries(ELECTION_MECHANIC_FIELDS.slice(1).map((field) => [field, exact(mechanic?.[field])]).filter(([, value]) => value)),
    };
    const proposal = shapeOpenWorldCandidate({
      observed_quote: mechanic?.quote,
...
function shapeProposals(parsed, sourceText) {
  ...
  const shareCountAssertions = Array.isArray(parsed[SHARE_COUNT_ASSERTIONS_KEY])
    ...
    : [];
  const zeroShareCountAssertions = deriveZeroShareCountAssertions(
    parsed.representation_instances,
    shareCountAssertions,
  );
...
function shapeRepresentationQualifierProposals(parsed, sourceText) {
  ...
  for (const instance of (Array.isArray(parsed.representation_instances) ? parsed.representation_instances : [])) { const side = representationSideFor(instance?.party_making); for (const qualifier of (Array.isArray(instance?.qualifiers) ? instance.qualifiers : [])) { const evidence = evidenceFromQuote(sourceBytes, qualifier?.quote); if (!evidence) { if (qualifier?.quote) dropCounter.record('REPRESENTATION_QUALIFIER_QUOTE_UNVERIFIED', qualifier.quote); continue; } const attachment = qualifier.attachment && typeof qualifier.attachment === 'object' ? qualifier.attachment : null; const subjectId = mintSubjectId({ kind: 'REPRESENTATION_QUALIFIER', section_reference: instance?.section_reference ?? null, side, quote: qualifier.quote }); proposals.push({ kind: 'claim', proposal_kind: REPRESENTATION_QUALIFIER_PROPOSAL_KIND, subject_occurrence_id: subjectId, claim_definition_key: REPRESENTATION_QUALIFIER_CLAIM_KEY, claim_definition_version: 1, ordinal: ordinalFor(`${subjectId}:${REPRESENTATION_QUALIFIER_CLAIM_KEY}`), state: 'PRESENT', raw_value: qualifier.quote, canonical_value: qualifier.code ?? null, attributes: { section_reference: instance?.section_reference ?? null, representation_side: side, qualifier_kind: qualifier.kind ?? null, attachment }, allowed_attributes: ['section_reference', 'representation_side', 'qualifier_kind', 'attachment'], taxonomy_codes: qualifier.code ? { qualifier_code: qualifier.code } : {}, codebooks: { qualifier_code: QUALIFIER_CODES }, evidence: [evidence], extraction_version: RESPONSE_VERSION, normalisation_version: RESPONSE_VERSION, derivation_version: RESPONSE_VERSION }); } }
  for (const candidate of (Array.isArray(parsed.open_world_candidates) ? parsed.open_world_candidates : [])) { const proposal = shapeOpenWorldCandidate(candidate, sourceBytes, ordinalFor, dropCounter); if (proposal) proposals.push(proposal); }
  return { proposals, evidence_residuals: dropped };
}
...
const FAMILY_RESPONSE_SHAPERS = Object.freeze({
  ...
  EMPLOYEE_MATTERS: shapeGovernedEmployeeMattersProposals,
  DNO_INDEMNIFICATION: shapeGovernedDnoProposals,
  TAX_MATTERS: shapeGovernedTaxMattersProposals,
  DIVIDENDS: shapeGovernedDividendsProposals,
  APPRAISAL_DISSENTERS_RIGHTS: shapeGovernedAppraisalProposals,
