...
function resolvedParty(entry) {
  ...
  if (entryParty && provisionParty && canonicalJson(entryParty) !== canonicalJson(provisionParty)) {
    throw new TypeError('resolved General Covenants party conflicts with its provision party');
  }
  ...
  if (!party
    ...
    throw new TypeError('resolved General Covenants party is ambiguous');
  }
  return party;
}
...
function projectGeneralCovenantsProductSurfaces({ resolution, deal_id: dealId, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId, publication_evaluation_time: publicationEvaluationTime } = {}) {
  ...
  for (const entry of filterResolvedEntriesForPublication(resolution.resolved, publicationFilter, releaseReceiptId, publicationEvaluationTime)) {
    ...
    if (canonicalJson(groups.get(provisionId).party) !== canonicalJson(party)) {
      throw new TypeError('resolved General Covenants claims for one provision have conflicting parties');
    }
