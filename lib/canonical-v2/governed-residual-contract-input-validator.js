...
const GOVERNED_RESIDUAL_CONTRACT_DEFINITIONS = Object.freeze({
  ...
  GOVERNED_RESIDUAL_PRODUCER_REGISTRY: Object.freeze({
    ...
    definition_digest:
      'ee9360bd495e4b3b796a63f72b2c46d92c4a588c683b28a011030a8417cde84f',
  }),
  ...
  GOVERNED_RESIDUAL_UNIVERSE: Object.freeze({
    ...
    definition_digest:
      '9347d73e85e6ba5defa88f770ead64b91ed08c8f1baa782bb948c3ab20896206',
  }),
...
function validateProducer(definition) {
  ...
  const producerBoundaryKeys = registry.closed_producer_entries.flatMap(
    (entry) => entry.residual_capable_boundaries.map(
      (boundary) => `${entry.producer_kind}/${entry.producer_version}/${boundary}`,
    ),
  );
  if (new Set(producerBoundaryKeys).size !== producerBoundaryKeys.length) {
    fail(code, 'Each producer-boundary identity must have one registry entry.');
  }
  requireExact(
    registry.producer_boundary_entry_identity_fields,
    [
      'producer_kind',
      'producer_version',
      'boundary_kind',
    ...
    'Producer-boundary entry identity',
    code,
  );
  requireFlags(
    registry,
    [
      'every_registered_producer_boundary_pair_has_exactly_one_entry',
      'every_entry_boundary_has_exactly_one_producer_boundary_identity',
      'producer_boundary_universe_bidirectional_equality_required',
      'shared_residual_carrier_schema_permitted',
    ],
    [
      'carrier_schema_uniqueness_required',
      'unregistered_producer_permitted',
      'unregistered_producer_or_boundary_permitted',
...
function validateUniverse(definition) {
  ...
  const terminalJoin = definition.terminal_residual_join_contract;
  ...
  if (terminalJoin.required_terminal_kind !== 'GOVERNED_RESIDUAL_OBSERVATION') {
    fail(code, 'The universe join must select residual-observation terminals.');
  }
  requireExact(
    terminalJoin.required_join_bindings,
    [
      'terminal_consumption_record_id',
      'terminal_consumption_record_payload_digest',
      'terminal_kind',
      'terminal_residual_observation_id',
      'terminal_residual_observation_payload_digest',
      'universe_residual_id',
      'universe_residual_payload_digest',
    ],
    'Terminal residual universe join bindings',
    code,
  );
  requireFlags(
    terminalJoin,
    [
      'every_governed_residual_terminal_consumption_joins_exactly_one_universe_residual',
      'every_universe_residual_joins_exactly_one_governed_residual_terminal_consumption',
      'terminal_and_universe_residual_identity_equality_required',
      'terminal_and_universe_payload_digest_equality_required',
    ],
    ['terminal_kind_substitution_permitted', 'stale_terminal_payload_permitted'],
    'Terminal residual universe join',
    code,
  );
  requireExact(
    terminalJoin.required_empty_difference_roots,
    [
      'terminal_residual_join_missing_root',
      'terminal_residual_join_extra_root',
      'terminal_residual_join_conflict_root',
    ],
    'Terminal residual universe join difference roots',
    code,
  );
  requireExact(
    ...
    [
      ...
      'terminal_residual_join_reconciliation_id',
      'terminal_residual_join_reconciliation_payload_digest',
  ...
  requireExact(
    identity.required_manifest_identity_inputs,
    [
      'frozen_contract_pair',
      'scope_code',
      'pre_scope_predecessor_manifest_id_and_payload_digest_or_genesis',
      'captured_terminal_head_tuple',
      'both_enumerator_authority_root_ids_and_payload_digests',
      'universe_reconciliation_id_and_payload_digest',
      'producer_registry_id_and_payload_digest',
      'admission_receipt_root_set_id_and_payload_digest',
      'terminal_consumption_root_set_id_and_payload_digest',
      'consumption_reconciliation_set_root_id_and_payload_digest',
      'terminal_residual_join_reconciliation_id_and_payload_digest',
      'ordered_residual_id_and_payload_digest_set',
      'required_empty_difference_roots',
    ],
    'Residual universe manifest identity',
    code,
  );
}
...
function validateFamilySemantics(values) {
  ...
  requireExact(
    Object.keys(
      impact.final_residual_impact_contract.branch_by_final_disposition,
    ).sort(),
    [...disposition.disposition_contract.closed_final_dispositions].sort(),
