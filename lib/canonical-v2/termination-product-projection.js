...
const { validatePublicationDispositionV2Identity } = require('./publication-disposition');
...
// Mutual termination concepts whose EITHER_PARTY scope is stored as two
// principal-party rows (TARGET + BUYER) after Step 2X-I -- each row still
// projects as PARTY_MUTUAL for TERMINATION_PARTY-shaped fields.
const MUTUAL_TERMINATION_CONCEPTS = new Set([
  'TERMR-MUTUAL', 'TERMR-OUTSIDE', 'TERMR-NOVOTE', 'TERMR-LEGAL',
]);

function partyCode(entry) {
  const capacity = entry?.party?.capacity || entry?.provision_instance?.party?.capacity;
  const scope = entry?.claim?.attributes?.terminating_party_scope;
  if (capacity === 'EITHER_PRINCIPAL_PARTY') return 'PARTY_MUTUAL';
  if (scope === 'EITHER_PARTY' && MUTUAL_TERMINATION_CONCEPTS.has(entry?.concept_key)
    && (capacity === 'TARGET' || capacity === 'BUYER')) {
    return 'PARTY_MUTUAL';
  }
...
function publicationDispositionAllows(entry, publicationFilter, releaseReceiptId) {
  if (publicationFilter === undefined || publicationFilter === null) return true;
  const sidecar = entry?.publication_disposition;
  if (!sidecar || sidecar.claim_revision_id !== entry?.claim?.claim_revision_id) return false;
  try {
    validatePublicationDispositionV2Identity(sidecar);
  } catch {
    return false;
  }
  if (publicationFilter === 'CANDIDATE_ELIGIBLE') {
    return sidecar.disposition === 'ELIGIBLE' || sidecar.disposition === 'PUBLISHED';
  }
  if (publicationFilter === 'REQUIRE_PUBLISHED') {
    return sidecar.disposition === 'PUBLISHED'
      && typeof releaseReceiptId === 'string'
      && sidecar.release_receipt_id === releaseReceiptId;
  }
  return false;
}

function project({ resolution, dealId, family, definitions, evidenceSurfaces, codeFor, featuresFor, type, projectionName, extraGroups, publicationFilter, releaseReceiptId }) {
  ...
  for (const entry of resolution.resolved) {
    if (!publicationDispositionAllows(entry, publicationFilter, releaseReceiptId)) continue;
  ...
  const openEntries = (publicationFilter === undefined || publicationFilter === null)
    ? resolution.open_world.filter((entry) => evidenceSurfaces.has(evidenceSurface(entry)))
    : [];
...
function projectTerminationRightsProductSurfaces({ resolution, deal_id: dealId, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId } = {}) {
  return project({
    resolution,
    dealId,
...
function projectTerminationFeeProductSurfaces({
  ...
  return project({
    ...
    extraGroups: publicationFilter === undefined || publicationFilter === null
      ? conditionalFeeExtraGroups(resolution, dealId)
      : [],
    publicationFilter,
    releaseReceiptId,
  });
