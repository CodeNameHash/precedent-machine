...
function waveBEvidenceFeatures(entry, family) {
  ...
  if (mechanic.surface === 'SOLE_REMEDY_EVIDENCE') return {
    // The Remedies family owns the legal-effect claim.  Fee projections
    // retain only the payment context and exact carve-out evidence.
    remediesOwnedSoleRemedyEvidence: clone(mechanic),
  };
  if (mechanic.surface === 'LATE_PAYMENT_INTEREST') return {
