...
const { publicationDispositionAllows } = require('./publication-serving-filter');
...
function fixedDuration(value, unit, trigger) {
  return { value, unit, trigger };
}
...
function rightFeatures(entry) {
  ...
  if (definition === 'TERMINATION_CURE_PERIOD_DAYS') {
    addFeature(features, 'curePeriod', fixedDuration(
...
function feeFeatures(entry, { feeRequiredCrossCheck } = {}) {
  ...
  if (definition === 'TERMINATION_FEE_AMOUNT') {
    ...
    addFeature(features, 'tailProvision', {
      period_months: entry.claim.canonical_value,
      trigger: 'qualifying_termination',
    });
    addFeature(features, 'tailFeeWindowMonths', fixedDuration(
      entry.claim.canonical_value,
      'months',
      'qualifying_termination',
    ));
  }
  return features;
}
...
function project({ resolution, dealId, family, definitions, evidenceSurfaces, codeFor, featuresFor, type, projectionName, extraGroups, publicationFilter, releaseReceiptId, publicationEvaluationTime }) {
  ...
  for (const entry of resolution.resolved) {
    if (!publicationDispositionAllows(entry, publicationFilter, releaseReceiptId, publicationEvaluationTime)) continue;
...
function projectTerminationRightsProductSurfaces({ resolution, deal_id: dealId, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId, publication_evaluation_time: publicationEvaluationTime } = {}) {
  return project({
    resolution,
    dealId,
