 ...
 * QXO reads a committed fixture. Modiv uses this permitted staging reader.
 *
...
// issues can only ever be one of these nine, matching the nine functions
...
function createLocalStagingQueryInterface(client) {
  return {
    ...
    async claimPublicationDispositionsByClaimRevisionIds(claimRevisionIds, decisionId) {
      return queryPayloads(
        client,
        `SELECT canonical_payload FROM canonical_v2_staging.claim_publication_dispositions
         WHERE claim_revision_id = ANY($1::text[])
           AND decision_id = $2`,
        [claimRevisionIds, decisionId],
      );
    },
  };
}
...
function createHostedStagingQueryInterface(client) {
  ...
  return {
    ...
    claimPublicationDispositionsByClaimRevisionIds: (claimRevisionIds, decisionId) => call(
      'canonical_v2_staging_read_claim_publication_dispositions', {
        p_claim_revision_ids: claimRevisionIds,
...
async function readDealFromLocalCanonicalV2Staging({
  ...
  env = process.env,
  expectNonEmpty = true,
  publicationDispositionDecisionId,
}) {
  ...
  requireClient(client);
  requireDocumentHash(documentHash);
  if (publicationDispositionDecisionId !== undefined
    && (typeof publicationDispositionDecisionId !== 'string'
      || !/^[a-f0-9]{64}$/.test(publicationDispositionDecisionId))) {
    throw new TypeError('publicationDispositionDecisionId must be an immutable SHA-256 decision id');
  }
  ...
  let resolvedWithDispositions = resolved;
  if (publicationDispositionDecisionId !== undefined) {
    const q = createStagingQueryInterface(client);
    const sidecars = await q.claimPublicationDispositionsByClaimRevisionIds(
      resolved.map((entry) => entry.claim.claim_revision_id), publicationDispositionDecisionId,
    );
    const byClaimId = new Map(sidecars.map((sidecar) => [sidecar.claim_revision_id, sidecar]));
    resolvedWithDispositions = resolved.map((entry) => Object.freeze({
      ...entry,
      publication_disposition: byClaimId.get(entry.claim.claim_revision_id) || null,
    }));
  }
  ...
  return Object.freeze({
    document_hash: documentHash,
    resolved: Object.freeze(resolvedWithDispositions),
