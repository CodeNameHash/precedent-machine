...
async function importRunEvidence({
  ...
  // sources. A run directory is not a database: it carries its own
  // `admitted_source_contexts`, one per source reference, and they are the
  // authoritative record of what that run was actually run against.
  //
  // So the bridge decorates the repository with a resolver backed by the
  // run's own contexts, rather than requiring every repository to learn how
  // to read an evidence directory. A repository that already resolves them
  // keeps its own implementation -- decorating, not overriding, so a
  // Postgres repository importing a run it already knows about is unaffected.
  // ALWAYS decorate, never defer to the repository's own resolver. An earlier
  // version deferred when the repository already had one, which silently
  // picked InMemoryCanonicalRepository's empty implementation and failed with
  // "every source reference must resolve exactly once". More importantly it
  // was wrong in principle: the repository has not seen this run yet -- that
  // is what importing means -- so its resolver cannot know these sources. The
  // run's own contexts are the authority for the run, and they are the ones
  // the validator just checked.
  const resolvingRepository = Object.create(repository, {
      resolveAdmittedSourceReferences: {
        value: async (sourceReferences) => {
          const contexts = evidence.admitted_source_contexts;
          if (!Array.isArray(contexts) || contexts.length !== sourceReferences.length) {
            throw new EvidenceBridgeError(
              'SOURCE_CONTEXT_COUNT_MISMATCH',
              `${evidence.run_directory} has ${Array.isArray(contexts) ? contexts.length : 0} admitted `
              + `source context(s) for ${sourceReferences.length} source reference(s). The writer requires `
              + 'exactly one each; a mismatch means the run and its sources disagree.',
            );
          }
          return contexts;
        },
