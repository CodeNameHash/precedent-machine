...
// deal-scope run and concluded every committed run was unimportable. They are
// not. An extraction run is a `DEAL_SCOPE_RUN` and is checked by
// `validateResolvedCanonicalWriteSet` against `DEAL_SCOPE_WRITE_SET_KEYS`.
//
// For the current numbers read `evidence/canonical-v2/baseline-manifest.json`
// rather than this comment. As of 2026-08-07 it is 25 importable runs of 49
// directories -- the 24 refusals are the pre-2026-08-07 runs, which cannot
// rebuild their source chain. An earlier version of this header said "23 of
// 24 pass", which was true when written and stopped being true the same day.
...
async function importRunEvidence({
  ...
  // called the generic one and concluded every committed run was
  // unimportable. They are not; see baseline-manifest.json for the count.
