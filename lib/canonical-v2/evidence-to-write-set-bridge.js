...
async function importRunEvidence({
  ...
  // itself published and refuse a shortfall. Without this the composition
  // above could regress to reading the adapter's write-set directly and every
  // test would still pass except one.
  ...
  const resolvingRepository = Object.create(repository, {
    resolveAdmittedSourceReferences: {
      value: createRebuildingSourceReferenceResolver({ runDirectory: evidence.run_directory }),
    },
  });
