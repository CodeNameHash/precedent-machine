...
async function importRunEvidence({
  ...
  try {
    // Shape and contract conformance. The resolved-form validator additionally
    // needs the admitted source contexts, which a run directory does not
    // carry, so it is the writer's own call to make during the write rather
    // than something this bridge can pre-empt.
    validateCanonicalWriteSet(evidence.write_set, contractBundle);
  } catch (err) {
    throw new EvidenceBridgeError(
      'REVALIDATION_FAILED',
