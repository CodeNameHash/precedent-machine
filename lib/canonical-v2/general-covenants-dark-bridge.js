...
function assertGeneralCovenantCardKind(card, lineage, index) {
  ...
  if (lineage.source === EVIDENCE_SOURCE) {
    if (card.party !== null) {
      fail('INVALID_PARTY', 'Open-world General Covenants evidence must not claim an obligor party.', { index });
    }
  } else {
    requireObject(card.party, 'INVALID_PARTY', 'A native General Covenants card needs its resolved obligor party.');
    assertExactFields(card.party, ['role', 'value', 'capacity'], 'card party');
    if (card.party.role !== 'COVENANT_OBLIGOR'
      || !nonEmptyString(card.party.value)
      || !['TARGET', 'BUYER', 'JOINT_MULTI_PARTY'].includes(card.party.capacity)) {
      fail('INVALID_PARTY', 'A native General Covenants card has an invalid resolved obligor party.', { index });
    }
  }
