...
function assembleCanonicalContractBundlePreReviewPackage(input = {}) {
  ...
  try {
    ...
    fail(
      'INVALID_CONTRACT_FREEZE_ATTESTATION_IDENTITY',
      'The derived contract freeze identity does not satisfy the existing schema.',
      { validation_error: error.message },
    );
  }
  const frozenPairDigest = domainDigest(
    'PROGRAMME_GATE_FROZEN_CONTRACT_PAIR/V1',
    {
      ...
      contract_freeze_attestation_id:
        attestationIdentity.contract_freeze_attestation_id,
    },
  );
