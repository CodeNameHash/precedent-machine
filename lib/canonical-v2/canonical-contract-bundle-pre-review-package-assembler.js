...
const SPECIFICATION_ROOT_MEMBER_COUNT = 7;
...
function validateSpecificationManifest(manifest, members) {
  ...
  if (
    ...
    || members.length !== SPECIFICATION_ROOT_MEMBER_COUNT
  ) {
    fail(
      'INVALID_GOVERNING_SPECIFICATION_MANIFEST',
...
function compileGoverningSpecificationMembers(values) {
  if (!Array.isArray(values) || values.length !== SPECIFICATION_ROOT_MEMBER_COUNT) {
    fail(
      'INVALID_GOVERNING_SPECIFICATION_MEMBER_SET',
      'The existing governing specification root requires exactly seven ordered members.',
    );
  }
...
module.exports = {
  ...
  SPECIFICATION_ROOT_MEMBER_COUNT,
