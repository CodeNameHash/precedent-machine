...
// Derived from the manifest rather than hardcoded, since 2026-08-06.
//
// These were the literals 6 and 7. When four prose documents were unpinned the
// manifest dropped to two files, and this module kept insisting on six,
// producing a specification root that disagreed with the verifier's. Nothing
// was wrong with either; they simply disagreed about what the set was.
//
// That is the failure the documentation audit named: a hardcoded list inside a
// JavaScript file rots exactly as a stale paragraph does, and is harder to
// notice because it looks like code. The manifest is the single declaration of
// membership, so read it. The member count is the files plus the manifest
// itself, which is a member of its own root.
function specificationDeclaredFileCount(manifest) {
  return Array.isArray(manifest?.files) ? manifest.files.length : 0;
}
function specificationRootMemberCount(manifest) {
  return specificationDeclaredFileCount(manifest) + 1;
}
...
function validateSpecificationManifest(manifest, members) {
  ...
  if (
    ...
    || members.length !== specificationRootMemberCount(manifest)
  ) {
    fail(
      'INVALID_GOVERNING_SPECIFICATION_MANIFEST',
...
function compileGoverningSpecificationMembers(values) {
  // The member set is the manifest plus every file it declares. The manifest
  // is always the first member and is a member of its own root, so a valid set
  // is at least two entries and its size is whatever the manifest says plus
  // one. Previously this required exactly seven, which silently stopped being
  // true when the pinned set was narrowed.
  if (!Array.isArray(values) || values.length < 2) {
    fail(
      'INVALID_GOVERNING_SPECIFICATION_MEMBER_SET',
      'The governing specification root requires the manifest and at least one declared file.',
    );
  }
...
function assembleCanonicalContractBundlePreReviewPackage(input = {}) {
  ...
  const aggregateSuccessorProjection = validateProjection(
    firstCompilation.canonical_contract_bundle_projection,
    'aggregate_successor_contract_bundle_projection',
  );
  const successorProjection = validateProjection(
    [
      ...aggregateSuccessorProjection,
      ...generatedTopology.generated_contract_bundle_projection,
  ...
  const exactReviewPackage = {
    ...
    governed_topology_input_identity:
      clone(generatedTopology.governed_topology_input_identity),
    ...
    generated_contract_bundle_members:
      clone(freezeCandidate.generated_contract_bundle_members),
