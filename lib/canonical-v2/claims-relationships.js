...
const CLAIM_REVISION_KEYS = Object.freeze([
  'schema_version',
  'claim_revision_id',
  'claim_occurrence_id',
...
const RELATIONSHIP_REVISION_KEYS = Object.freeze([
  'schema_version',
...
function revisionForValidation(row, label, allowClosure) {
  if (!allowClosure || !Object.hasOwn(row, 'closure_id')) return row;
  requireContentId(row.closure_id, `${label}.closure_id`);
  const revision = { ...row };
  delete revision.closure_id;
  return revision;
}

function requirePublicationContainers(row, label, requirePublication) {
  if (!row.attributes
    || typeof row.attributes !== 'object'
    || Array.isArray(row.attributes)
    || !row.taxonomy_codes
    || typeof row.taxonomy_codes !== 'object'
    || Array.isArray(row.taxonomy_codes)) {
    throw new TypeError(`${label} attributes and taxonomy codes must be objects`);
  }
  if (!Array.isArray(row.retained_residuals)
    || row.retained_residuals.length > MAX_GOVERNED_ITEMS) {
    throw new TypeError(`${label} retained residuals must be a bounded array`);
  }
  if (requirePublication
    && (row.publication_state !== 'VALIDATED'
      || row.retained_residuals.length !== 0
      || row.quarantine !== null)) {
    throw new TypeError(`${label} is not validated for publication`);
  }
  if (!requirePublication
    && row.publication_state === 'VALIDATED'
    && (row.retained_residuals.length !== 0 || row.quarantine !== null)) {
    throw new TypeError(`${label} validated state is inconsistent`);
  }
  if (!requirePublication
    && row.publication_state === 'QUARANTINED'
    && (row.retained_residuals.length === 0
      || !row.quarantine
      || typeof row.quarantine !== 'object'
      || Array.isArray(row.quarantine))) {
    throw new TypeError(`${label} quarantine state is inconsistent`);
  }
  if (!['VALIDATED', 'QUARANTINED'].includes(row.publication_state)) {
    throw new TypeError(`${label} publication state is invalid`);
  }
}
...
function isGovernedRelationshipEffect(value) {
  return value != null
    && typeof value === 'object'
    && !Array.isArray(value)
    && ['NON_SEMANTIC', 'TYPED_LEGAL_EFFECT'].includes(value.effect_mode)
    && typeof value.legal_operation === 'string'
    && value.legal_operation.length > 0;
}
...
function normalizeEvidence(evidence, occurrenceId, kind) {
  if (evidence != null && !Array.isArray(evidence)) {
    throw new TypeError(`${kind} evidence must be an array`);
  }
  const suppliedEvidence = [...(evidence ?? [])];
  if (suppliedEvidence.length > MAX_GOVERNED_ITEMS) {
    throw new TypeError(`${kind} evidence exceeds the governed maximum`);
  }
...
function hasTypedStateDetail(input, state) {
  if (state === 'NOT_APPLICABLE') {
    return typeof input.applicability?.rule === 'string'
      ...
      && input.applicability.source_fact_ids.length <= MAX_GOVERNED_ITEMS
...
function buildRelationshipRevision(input) {
  ...
  requireGovernedKey(input.relationship_definition_key, 'relationship_definition_key');
  const relationshipDefinitionVersion = input.relationship_definition_version ?? 1;
  const ordinal = input.ordinal ?? 0;
  const resolverVersion = governedVersion(input.resolver_version, 'resolver_version');
  requireSafeInteger(relationshipDefinitionVersion, 'relationship_definition_version', 1);
  requireSafeInteger(ordinal, 'relationship ordinal');
  if (input.target_occurrence_ids != null
    && !Array.isArray(input.target_occurrence_ids)) {
    throw new TypeError('target_occurrence_ids must be an array');
  }
  const targetOccurrenceIds = [...(input.target_occurrence_ids ?? [])];
  if (targetOccurrenceIds.length > MAX_GOVERNED_ITEMS) {
    throw new TypeError('target_occurrence_ids exceeds the governed maximum');
  }
  for (const targetId of targetOccurrenceIds) requireContentId(targetId, 'target_occurrence_id');
  const relationshipOccurrenceId = contentId('RELATIONSHIP_OCCURRENCE/V1', {
    ...
    relationship_definition_version: relationshipDefinitionVersion,
    ordinal,
  });
  ...
  if (input.state === 'PRESENT' && evidence.length === 0) residuals.push({ reason: 'PRESENT_WITHOUT_EVIDENCE' });
  if (input.state === 'PRESENT' && !targetOccurrenceIds.length) residuals.push({ reason: 'PRESENT_WITHOUT_RESOLVED_TARGET' });
  if (input.state === 'PRESENT' && !isGovernedRelationshipEffect(input.effect)) {
    residuals.push({ reason: 'PRESENT_WITHOUT_EFFECT' });
  }
  if (input.state === 'ABSENT' && !completeScope(input.scope)) residuals.push({ reason: 'ABSENT_WITHOUT_COMPLETE_SCOPE' });
  if (input.state !== 'PRESENT' && (targetOccurrenceIds.length || input.effect != null)) {
...
function validateEvidenceIdentity(row, kind) {
  ...
  if (!Array.isArray(row.evidence)
    ...
    || row.evidence.length > MAX_GOVERNED_ITEMS
    || row.evidence_ids.length > MAX_GOVERNED_ITEMS) {
    throw new TypeError(`${kind} evidence must be bounded arrays`);
  }
...
function validateClaimRevisionIdentity(
  row,
  { allowClosure = false, requirePublication = true } = {},
) {
  if (!row || row.schema_version !== 'CLAIM_REVISION/V1' || !CLAIM_STATES.includes(row.state)) {
    throw new TypeError('invalid ClaimRevision');
  }
  row = revisionForValidation(row, 'ClaimRevision', allowClosure);
  requireExactKeys(row, CLAIM_REVISION_KEYS, 'ClaimRevision');
  ...
  requirePublicationContainers(row, 'ClaimRevision', requirePublication);
...
function validateRelationshipRevisionIdentity(
  row,
  { allowClosure = false, requirePublication = true } = {},
) {
  ...
  row = revisionForValidation(row, 'RelationshipRevision', allowClosure);
  requireExactKeys(row, RELATIONSHIP_REVISION_KEYS, 'RelationshipRevision');
  requireContentId(row.source_occurrence_id, 'source_occurrence_id');
  requireGovernedKey(row.relationship_definition_key, 'relationship_definition_key');
  requireSafeInteger(row.relationship_definition_version, 'relationship_definition_version', 1);
  requireSafeInteger(row.ordinal, 'relationship ordinal');
  requireGovernedVersion(row.resolver_version, 'resolver_version');
  requirePublicationContainers(row, 'RelationshipRevision', requirePublication);
  if (!Array.isArray(row.target_occurrence_ids)
    || row.target_occurrence_ids.length > 4096) {
    throw new TypeError('target_occurrence_ids must be a bounded array');
  }
  ...
  if (requirePublication) {
    if (row.state === 'PRESENT' && (!row.evidence.length
      || !row.target_occurrence_ids.length
      || !isGovernedRelationshipEffect(row.effect))) {
