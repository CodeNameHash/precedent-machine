...
function meetingParty(entry) {
  ...
  return entry.party?.capacity || null;
}
...
function projectProxyMeetingProductSurfaces({ resolution, deal_id: dealId } = {}) {
  ...
  for (const entry of resolution.resolved) {
    ...
    if (!['COV-PROXY', 'COV-MEETING'].includes(entry.concept_key)) continue;
  ...
  const cards = [...groups.values()].map((group) => {
    ...
    return {
      ...
      short_title: group.conceptKey === 'COV-PROXY' ? 'Proxy statement covenant' : 'Stockholder meeting covenant',
      primary_quote: quote,
