...
const ALLOWED_OPERATIONS = new Set(['FIXTURE_DEAL_EXTRACTION_RUN', 'INTAKE_CAPTURE']);
const WRITE_ORDER = [
  'excerpts',
...
function createCanonicalWriter({ repository, contractBundle }) {
  ...
  return {
    async write({ operation, idempotencyKey, dryRun = false, writeSet } = {}) {
      ...
      if (operation !== 'INTAKE_CAPTURE') validateContractBundle(contractBundle);
      const validation = operation === 'INTAKE_CAPTURE'
      ...
      const publishableClosureIds = isIntake ? new Set() : new Set(WRITE_ORDER.flatMap(
