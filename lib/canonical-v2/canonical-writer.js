...
const WRITE_ORDER = ['excerpts', 'provisions', 'components', 'claims', 'relationships'];
  ...
  return {
    async write({ operation, idempotencyKey, dryRun = false, writeSet } = {}) {
      ...
      const committed = await repository.transaction(async (transaction) => {
        ...
        await transaction.writeSource(validation.publishableWriteSet.source);
        await transaction.writeSourceAdmission(validation.publishableWriteSet.source_admission);
