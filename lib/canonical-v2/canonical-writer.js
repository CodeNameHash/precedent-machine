...
const OPTIONAL_AUDIT_WRITE_ORDER = [
...
const OBJECT_ID_FIELDS = Object.freeze({
  ...
  publication_calibration_authorities: 'calibration_authority_id',
  publication_dispositions: 'decision_id',
});
...
function assertDealScopeWriteSetShape(writeSet, contractBundle) {
  ...
  const legacyKeys = DEAL_SCOPE_WRITE_SET_KEYS.filter((key) => key !== 'condition_groups');
  // Optional extras, enumerated as a powerset rather than hand-listed, which
  // is how validate-write-set.js:507 already does it. `write_set_origin` was
  // added there as an optional key and never here, so this check rejected
  // every native-producer run -- all 24 committed ones -- while the validator
  // passed them. The validator does not merely tolerate the key, it USES it
  // (line ~2102: answer_provenance is required for claims only when
  // write_set_origin === 'NATIVE_PRODUCER'), so dropping the key upstream
  // would weaken validation rather than simplify it.
  //
  // `conditional_termination_fee_values` (PLAN.md Step 4A3, Ben's ruling in
  // DECISIONS.md decision 3) added the same way, for the identical reason:
  // Step 4A2 made validate-write-set.js's assertResolvedWriteSetShape accept
  // it as a third independently-optional key, and Step 4A2's own note named
  // this exact omission as the follow-up this file still owed -- "whoever
  // wires the adapter next will need to widen that key set too, or
  // DEAL_SCOPE_RUN calls through the ordinary JS path will keep silently
  // dropping this field." Without this, `native-write-set-adapter.js`'s own
  // write set (now carrying this key -- see that module) is rejected here as
  // INVALID_DEAL_SCOPE_WRITE_SET before validateResolvedCanonicalWriteSet
  // -- and therefore this kind's own shape check -- ever runs.
  const optionalKeys = [
  ...
  const acceptedKeySets = new Set();
  for (const base of [DEAL_SCOPE_WRITE_SET_KEYS, ...(allowsLegacyConditionGroups ? [legacyKeys] : [])]) {
    for (let mask = 0; mask < (1 << optionalKeys.length); mask += 1) {
      const keys = [...base];
      optionalKeys.forEach((key, index) => {
        if (mask & (1 << index)) keys.push(key);
      });
      acceptedKeySets.add(canonicalJson(keys.slice().sort()));
    }
  }
  if (!writeSet || Array.isArray(writeSet)
    || Object.getPrototypeOf(writeSet) !== Object.prototype
    || !acceptedKeySets.has(canonicalJson(actualKeys))
    ...
    || (writeSet.conditional_termination_fee_values !== undefined
      && !Array.isArray(writeSet.conditional_termination_fee_values))
    || (writeSet.publication_dispositions !== undefined
      && !Array.isArray(writeSet.publication_dispositions))
    || (writeSet.publication_calibration_authorities !== undefined
      && !Array.isArray(writeSet.publication_calibration_authorities))) {
    throw new CanonicalWriterError(
...
async function writeDealScopeRun({
  ...
  const committed = await repository.transaction(async (transaction) => {
    ...
    for (const kind of OPTIONAL_AUDIT_WRITE_ORDER) {
      for (const row of transactionValidation.publishableWriteSet[kind] || []) {
        await transaction.writeObject(kind, row);
      }
    }
...
async function writeProductResultCandidateRun({
  repository,
  idempotencyKey,
  dryRun,
  writeSet,
  processAuthorityContext,
  processAuthorityInput,
  ...
  if (!validation.materialisable) {
    throw new CanonicalWriterError(
      'UNMATERIALISED_AGREEMENT_CANDIDATE_CARRIER',
      'The admitted Agreement carrier requires an exact Product query, release binding, and complete result write set before persistence.',
      { adapterIdentifier: validation.adapterIdentifier },
    );
  }
  ...
  const committed = await repository.transaction(async (transaction) => {
    const concurrent = await transaction.getReceipt(
      'PRODUCT_RESULT_CANDIDATE_RUN',
      idempotencyKey,
    );
    if (concurrent) {
    ...
    if (typeof transaction.writeProductCandidateResult !== 'function') {
      throw new CanonicalWriterError(
        'PRODUCT_CANDIDATE_RESULT_WRITER_UNAVAILABLE',
        'The canonical repository cannot persist Product candidate results.',
      );
    }
    const receipt = buildCanonicalWriteReceipt({
      operation: 'PRODUCT_RESULT_CANDIDATE_RUN',
...
function createCanonicalWriter({ repository, contractBundle }) {
  ...
  return {
    async write({
      ...
      if (operation === 'PRODUCT_RESULT_CANDIDATE_RUN') {
        return writeProductResultCandidateRun({
          repository,
          idempotencyKey: key,
          dryRun,
          writeSet,
...
class InMemoryCanonicalRepository {
  ...
  async transaction(work) {
    ...
    const transaction = {
      ...
      writeProductCandidateResult: async (row) => {
        this._maybeFail('product_candidate_result');
        uniquePush(
          'productCandidateResults',
          row,
