...
function assertDealScopeWriteSetShape(writeSet) {
  if (!writeSet || Array.isArray(writeSet)
    || Object.getPrototypeOf(writeSet) !== Object.prototype
    || canonicalJson(Object.keys(writeSet).sort())
      !== canonicalJson([...DEAL_SCOPE_WRITE_SET_KEYS].sort())
...
async function writeDealScopeRun({
  ...
  assertDealScopeWriteSetShape(writeSet);
  ...
  const validateResolved = async (resolver) => {
    const admittedSourceContexts = await resolveAdmittedSemanticContexts(
      resolver,
      writeSet.source_references,
    );
  ...
  if (existing || dryRun === true) {
    const validation = await validateResolved(
      repository.resolveAdmittedSourceReferences?.bind(repository),
    );
    if (existing) {
  ...
  const committed = await repository.transaction(async (transaction) => {
    ...
    if (concurrent) {
      ...
      const validation = await validateResolved(
        transaction.resolveAdmittedSourceReferences?.bind(transaction),
      );
    ...
    for (const kind of WRITE_ORDER) {
      for (const row of validation.publishableWriteSet[kind]) {
    ...
    return { receipt, replayed: false, validation };
  });
