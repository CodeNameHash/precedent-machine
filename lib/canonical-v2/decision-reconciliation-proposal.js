...
const BINDING_PROFILES = Object.freeze([
  ...
  Object.freeze({
    ...
    // lib/canonical-v2/merger-structure-product-projection.js and its test were
    // removed 2026-08-05: the module keyed on STRUCTURE_CLAIMS/concepts that were
    // never registered in contract-bundle.js, so it could never receive a claim
    // the real resolver (candidate-resolution.js, MERGER-STRUCTURE concept,
    // MERGER_STRUCTURE_MECHANIC_PRESENT) actually emits. See the note at that
    // resolver site for the full account. This binding profile now carries only
    // the appraisal half of its original pairing.
    code_paths: Object.freeze(['lib/canonical-v2/native-producer/appraisal-producer-prompt.js']),
    ...
    test_paths: Object.freeze(['tests/appraisal-rights-codebook.test.js']),
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    // The self-verifying p9-acceptance-* proposal layer (definition-authority,
    // evidence-inventory, definition-proposal-layer) was deleted per D3/decision
    // 10: its validator compared its own output to itself and caught nothing.
    // What remains real for these two rulings is the governing registry itself.
    code_paths: Object.freeze([
