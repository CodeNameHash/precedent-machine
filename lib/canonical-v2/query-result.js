...
const QUERY_METRICS = Object.freeze([FEE_METRIC, IOC_CAPEX_METRIC, MATERIAL_CONTRACT_METRIC]);
...
const COLUMN_DEFINITIONS = Object.freeze({
  ...
  percent_of_deal_value: Object.freeze({ label: '% of deal value', value_kind: 'DECIMAL_PERCENT', refinement: { operator: 'BETWEEN', request_field: 'column_filters.percent_of_deal_value' } }),
  deal_value_basis: Object.freeze({ label: 'Deal-value basis', value_kind: 'DENOMINATOR', refinement: null }),
  source: Object.freeze({ label: 'Source', value_kind: 'EXACT_SOURCE_ACTION', refinement: null }),
  fee_side: Object.freeze({ label: 'Fee side', value_kind: 'CODED_VALUE', metrics: [FEE_METRIC], refinement: { operator: 'EQ', request_field: 'column_filters.fee_side' } }),
  payer: Object.freeze({ label: 'Payer', value_kind: 'PARTY', metrics: [FEE_METRIC], refinement: { operator: 'EQ', request_field: 'column_filters.payer_capacity' } }),
  payee: Object.freeze({ label: 'Payee', value_kind: 'PARTY', metrics: [FEE_METRIC], refinement: { operator: 'EQ', request_field: 'column_filters.payee_capacity' } }),
  triggers: Object.freeze({ label: 'Triggers and timing', value_kind: 'CANONICAL_SET', metrics: [FEE_METRIC], refinement: { operator: 'CONTAINS', request_field: 'column_filters.trigger_code' } }),
...
function normaliseColumnFilters(filters = {}, metricKey) {
  ...
  const feeKeys = ['fee_side', 'payer_capacity', 'payee_capacity', 'trigger_code'];
  const materialKeys = [
    'criterion_code',
    'contract_scope_code',
    'cash_flow_direction_code',
  ...
  if (metricKey !== FEE_METRIC && feeKeys.some((key) => result[key] !== null)) {
    throw new CanonicalQueryError('INVALID_REQUEST', 'fee refinements require the seller termination fee metric.');
  }
...
function compileCanonicalQueryRequest(request) {
  ...
  if (request.contract_fingerprint !== FIXTURE_CONTRACT_FINGERPRINT) {
    throw new CanonicalQueryError('INVALID_REQUEST', 'contract_fingerprint does not match the frozen serving contract.');
  }
  ...
  if (!QUERY_METRICS.includes(cohort.metric_key)) {
    throw new CanonicalQueryError('INVALID_REQUEST', 'the fixture query contract supports only its governed deal-relative money metrics.');
  }
  ...
  const querySemanticsDigest = contentId('CANONICAL_QUERY_SEMANTICS/V1', semantics);
  const pagePositionDigest = contentId('CANONICAL_QUERY_PAGE_POSITION/V1', cursor || 'FIRST_PAGE');
  const cacheKey = contentId('CANONICAL_QUERY_PAGE_CACHE/V1', {
    serving_namespace_id: cohort.serving_namespace_id,
    corpus_release_id: cohort.corpus_release_id,
...
function triggerValues(component) {
  return component.bounded_relationship_effects
    ...
    .map((relationship) => ({
      trigger_code: relationship.effect.trigger_code,
      terminating_party: relationship.effect.terminating_party,
      payment_timing: relationship.effect.payment_timing,
      conditions: [...relationship.effect.conditions],
    }))
    .sort((left, right) => left.trigger_code.localeCompare(right.trigger_code));
}

function projectSharedServingRowRecord({ row, serving_namespace_id }) {
  ...
  if (!QUERY_METRICS.includes(market.metric_key)) {
    throw new CanonicalQueryError('INVALID_REQUEST', 'the fixture query projection supports only its governed money metrics.');
  }
  ...
  return Object.freeze({
    ...
    canonical_numeric_value: component.canonical_value,
    ...
    trigger_codes: triggerValues(component).map((trigger) => trigger.trigger_code),
...
function rowMatchesRequest(row, request) {
  ...
  if ((columnFilters.min_percent_of_deal_value
    ...
    || (columnFilters.trigger_code
      && !triggerValues(component).some((trigger) => trigger.trigger_code === columnFilters.trigger_code))) return false;
  return true;
}
...
function validateCanonicalQueryPage(result, request) {
  ...
  if (result.schema_version !== 'CANONICAL_QUERY_PAGE_RESULT/V1'
...
function buildCanonicalQueryResultView(result, request) {
  ...
  return Object.freeze({
    ...
    columns,
    refinements: columns.filter((column) => column.refinement).map((column) => ({
...
async function queryCanonicalResultPage({
  ...
  const compiled = compileCanonicalQueryRequest(request);
  ...
  try {
    response = await withDeadline(
      Promise.resolve(client.rpc('canonical_v2_query_page', rpcParams(compiled))),
      timeoutMs,
    );
  } catch (error) {
  ...
  if (!response || response.error) throw new CanonicalQueryError('DATA_SOURCE_ERROR', 'The canonical query page could not be read.');
  const result = validateCanonicalQueryPage(response.data, compiled);
  if (cache && typeof cache.set === 'function') await cache.set(compiled.cache_key, result, cacheTtlSeconds);
  return { request: compiled, result: buildCanonicalQueryResultView(result, compiled), cache: 'MISS' };
}
