...
function buildOfflineInterpretedQueryProjectionRecord({
  row,
  observation,
} = {}) {
  ...
  if (!observation
    ...
    throw new TypeError('offline query projection does not close over its observation and row');
  }
  const context = row.canonical_result.comparability_context;
  const body = {
    schema_version: 'QUERY_PROJECTION_RECORD/V3',
    authority_scope: 'OFFLINE_CANDIDATE_ONLY',
    integration_admission_id: row.integration_admission_id,
    ...
    comparability_class_digest:
      context.comparability_class_digest,
    clarity_state: context.clarity_state,
    ambiguity_dimension_codes: context.ambiguity_dimension_codes,
    comparability_effect_code: context.comparability_effect_code,
    primary_semantic_class: context.primary_semantic_class,
    alternative_semantic_classes:
      context.alternative_semantic_classes,
    lawyer_warning: context.lawyer_warning,
  ...
  validateOfflineInterpretedQueryProjectionRecord(record);
  return Object.freeze(record);
}

function validateOfflineInterpretedQueryProjectionRecord(record) {
  const {
  ...
  if (!record
    ...
    || record.schema_version !== 'QUERY_PROJECTION_RECORD/V3'
    || record.authority_scope !== 'OFFLINE_CANDIDATE_ONLY'
    || record.integration_admission_id
      !== QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID
    || record.activation_eligibility
      !== 'INELIGIBLE_CONTRACT_NOT_SERVING_ADMITTED'
    || record.metric_key !== 'NO_SHOP_COPY_DELIVERY_PERIOD_DAYS'
    || record.source_action_state !== 'AVAILABLE') {
    throw new TypeError('invalid offline query projection record');
  }
  const { canonical_payload_digest: digest, ...body } = record;
  if (digest !== contentId('QUERY_PROJECTION_RECORD_PAYLOAD/V3', body)) {
    throw new TypeError('offline query projection identity has drifted');
  }
  return true;
}
