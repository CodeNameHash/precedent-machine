...
function sortedTriggerFieldValues(triggers, field) {
  const values = field === 'conditions'
    ? triggers.flatMap((trigger) => trigger.conditions)
...
function projectSharedServingRowRecord({ row, serving_namespace_id }) {
  ...
  return Object.freeze({
    ...
    trigger_conditions: sortedTriggerFieldValues(triggers, 'conditions'),
...
function rowMatchesRequest(row, request) {
  ...
  if ((columnFilters.min_percent_of_deal_value
    ...
    || (columnFilters.trigger_condition
      && !triggers.some((trigger) => trigger.conditions.includes(columnFilters.trigger_condition)))) return false;
  return true;
}
