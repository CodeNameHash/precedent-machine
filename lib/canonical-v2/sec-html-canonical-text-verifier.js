...
const CONVERSION_KEYS = Object.freeze([
  ...
  'canonical_text_byte_length', 'source_map_digest', 'canonical_text_id',
  'input_regions', 'output_mappings',
]);
const REGION_KEYS = Object.freeze(['input_start', 'input_end', 'region_kind']);
const TAGGED_REGION_KEYS = Object.freeze([...REGION_KEYS, 'tag_name']);
const MAPPING_KEYS = Object.freeze([
  'output_start', 'output_end', 'input_start', 'input_end', 'mapping_kind',
]);
...
function assertClosedConversion(conversion) {
  if (!sameKeys(conversion, CONVERSION_KEYS)
    || conversion.schema_version !== 'SEC_HTML_CANONICAL_TEXT_CONVERSION/V1'
    ...
    || !Array.isArray(conversion.input_regions)
    || !Array.isArray(conversion.output_mappings)) {
    fail('INVALID_CONVERSION_CONTRACT', 'conversion must match the closed conversion-only contract.');
  }
  for (const region of conversion.input_regions) {
    const tagged = Object.hasOwn(region || {}, 'tag_name');
    if (!sameKeys(region, tagged ? TAGGED_REGION_KEYS : REGION_KEYS)) {
      fail('INVALID_INPUT_REGION', 'every input region must match its closed shape.');
    }
  }
  if (!conversion.output_mappings.every((mapping) => sameKeys(mapping, MAPPING_KEYS))) {
    fail('INVALID_OUTPUT_MAPPING', 'every output mapping must match its closed shape.');
  }
}
...
function verifySecHtmlCanonicalText({ capture, conversion } = {}) {
  ...
  assertRegionCoverage(conversion.input_regions, bytes.length);
  if (canonicalJson(conversion.input_regions) !== canonicalJson(expectedRegions)) {
  ...
  assertMappingCoverage(conversion.output_mappings, bytes.length, conversion.canonical_text_byte_length);
  if (canonicalJson(conversion.output_mappings) !== canonicalJson(rebuilt.mappings)) {
  ...
  const sourceMapDigest = contentId('SEC_CANONICAL_TEXT_SOURCE_MAP/V1', {
    input_regions: expectedRegions,
    output_mappings: rebuilt.mappings,
  });
  const identity = {
  ...
  if (conversion.source_map_digest !== sourceMapDigest
    || conversion.canonical_text_id !== contentId('SEC_CANONICAL_TEXT/V1', identity)) {
