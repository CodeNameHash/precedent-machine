...
function collect({ cohort, loadReceipt, activeBundle = {} }) {
  ...
  for (const [label, entries] of byLabel) {
    ...
    if (rawDealIds.length < 3 || eligible.length === 0) continue;
    ...
    const payload = {
      ...
      recurrence: { raw_deal_ids: rawDealIds, raw_deal_count: rawDealIds.length, raw_occurrence_count: entries.length, eligible_deal_ids: [...new Set(eligible.map((row) => row.receipt.deal_id))].sort(byteOrder), eligible_deal_count: new Set(eligible.map((row) => row.receipt.deal_id)).size, eligible_occurrence_count: eligible.length, threshold: 3, strength: 'NEW' },
