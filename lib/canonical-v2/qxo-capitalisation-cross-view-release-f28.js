...
function emptyMarketContext(bindings, admissionByOrdinal, terminalByOrdinal) {
  return {
    ...
    subject_deal_included: false,
    subject_exclusion_state: 'APPLIED_AND_VERIFIED',
    state: 'EMPTY_COHORT',
    reason_code: 'NO_INDEPENDENT_COMPARABLE_DEALS',
    counts: {
      eligible_deals: 1,
      independent_comparable_deals: 0,
      excluded_subject_deals: 1,
    },
    metric_results: bindings.map((binding) => ({
      ...
      comparison_class_key: binding.comparison_class_key,
      metric_serving_admission_id:
        admissionByOrdinal.get(binding.display_ordinal)
          .metric_serving_admission_id,
      subject_metric_output_id: terminalId(
        terminalByOrdinal.get(binding.display_ordinal),
      ),
      subject_state:
        terminalByOrdinal.get(binding.display_ordinal).state,
      result_state: 'EMPTY_COHORT',
      state_groups: [],
      value_groups: [],
...
function validateReleaseEnvelope(release) {
  ...
  for (const [index, subrow] of row.subrows.entries()) {
    ...
    if (
      ...
      || subrow.market_context?.subject_deal_included !== false
      || subrow.market_context?.subject_exclusion_state
        !== 'APPLIED_AND_VERIFIED'
      || subrow.market_context?.state !== 'EMPTY_COHORT'
    ...
    for (const [metricIndex, metric] of
      ...
      if (
        ...
        || metric.result_state !== 'EMPTY_COHORT'
        || metric.reason_code !== 'NO_INDEPENDENT_COMPARABLE_DEALS'
        || metric.state_groups.length !== 0
        || metric.value_groups.length !== 0
        || metric.numeric_summary !== null
      ) {
...
function validateSubjectExclusion(value, request, binding) {
  return exactKeys(value, [
    ...
    'residual_subject_metric_outputs',
  ])
    && value.subject_deal_key === request.subject_deal_key
    && value.subject_metric_output_id === binding.subject_metric_output_id
    && value.exclusion_predicate === request.subject_exclusion_predicate
    && value.state === 'APPLIED_AND_VERIFIED'
    && value.residual_subject_deals === 0
    && value.residual_subject_metric_outputs === 0;
}
...
function validCanonicalGroupValue(value, binding) {
  ...
  if (binding.value_dimension === 'SIGNED_DURATION' && presentDeals > 0) {
...
function validateQxoCapitalisationF28MarketResult(result, request) {
  ...
  for (const [index, slot] of result.slot_results.entries()) {
    ...
    if (
      ...
      || !validateSubjectExclusion(
        slot.subject_exclusion,
        request,
        binding,
      )
      || !['READY', 'EMPTY_COHORT', 'FAILED'].includes(slot.result_state)
      ...
      || slot.counts.comparable_deals + slot.counts.excluded_deals
    ...
    if (
      ...
      && (
        ...
        || slot.value_groups.length !== 0
        || slot.numeric_summary !== null
        || slot.empty_cohort_reason_code !== null
...
function buildTypedEmptyF28MarketResult(request) {
  const body = {
    ...
    slot_results: request.metric_bindings.map((binding) => ({
      ...
      subject_exclusion: {
        ...
        residual_subject_metric_outputs: 0,
      },
      result_state: 'EMPTY_COHORT',
      counts: {
        eligible_deals: 1,
      ...
      value_groups: [],
      numeric_summary: null,
      empty_cohort_reason_code: 'NO_INDEPENDENT_COMPARABLE_DEALS',
