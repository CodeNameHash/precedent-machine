...
const SUBJECT_DIMENSIONS = Object.freeze({
  sector: null,
  buyer: 'QXO',
  merger_form: null,
  adviser_either: null,
...
function selectedNarrowerCohortReason(filters) {
  const direct = [
    ['sector', 'SECTOR_MISMATCH'],
    ['buyer', 'BUYER_MISMATCH'],
    ['merger_form', 'MERGER_FORM_MISMATCH'],
    ['adviser_either', 'ADVISER_MISMATCH'],
    ['lawyer_either', 'LAWYER_MISMATCH'],
  ];
  for (const [key, reason] of direct) {
    if (
      filters[key] !== null
      && filters[key] !== SUBJECT_DIMENSIONS[key]
    ) {
      return `SELECTED_NARROWER_COHORT:${reason}`;
    }
  }
  if (
    filters.year_from !== null
    && (
      SUBJECT_DIMENSIONS.announce_year === null
      || SUBJECT_DIMENSIONS.announce_year < filters.year_from
    )
  ) return 'SELECTED_NARROWER_COHORT:ANNOUNCE_YEAR_OUT_OF_RANGE';
  if (
    filters.year_to !== null
    && (
      SUBJECT_DIMENSIONS.announce_year === null
      || SUBJECT_DIMENSIONS.announce_year > filters.year_to
    )
  ) return 'SELECTED_NARROWER_COHORT:ANNOUNCE_YEAR_OUT_OF_RANGE';
  if (
    filters.min_value_usd !== null
    && (
      SUBJECT_DIMENSIONS.deal_value_usd === null
      || compareCanonicalDecimals(
        SUBJECT_DIMENSIONS.deal_value_usd,
        filters.min_value_usd,
      ) < 0
    )
  ) return 'SELECTED_NARROWER_COHORT:DEAL_VALUE_OUT_OF_RANGE';
  if (
    filters.max_value_usd !== null
    && (
      SUBJECT_DIMENSIONS.deal_value_usd === null
      || compareCanonicalDecimals(
        SUBJECT_DIMENSIONS.deal_value_usd,
        filters.max_value_usd,
      ) > 0
    )
  ) return 'SELECTED_NARROWER_COHORT:DEAL_VALUE_OUT_OF_RANGE';
  return null;
...
function subjectDisposition(terminal, filters) {
  ...
  const narrowerReason = selectedNarrowerCohortReason(filters);
  return narrowerReason === null
    ? { status: 'INCLUDED', exclusion_reason: null }
    : { status: 'EXCLUDED', exclusion_reason: narrowerReason };
}

function subjectCohortMembership({
  terminal,
  filters,
  cohortDigest,
  counts,
}) {
  const disposition = subjectDisposition(terminal, filters);
  const body = {
    ...disposition,
    cohort_digest: cohortDigest,
    subject_deal_key: terminal.governed_deal_key,
...
function requestSubjectDisposition(request, binding) {
  if (binding.subject_comparability.status !== 'COMPARABLE') {
    return {
      status: 'EXCLUDED',
      exclusion_reason: binding.subject_comparability.reason,
    };
  }
  const narrowerReason = selectedNarrowerCohortReason(request.filters);
  return narrowerReason === null
    ? { status: 'INCLUDED', exclusion_reason: null }
    : { status: 'EXCLUDED', exclusion_reason: narrowerReason };
}

function requestSubjectCohortMembership(request, binding, counts) {
  const body = {
    ...requestSubjectDisposition(request, binding),
    cohort_digest: request.selected_cohort_digest,
...
function validSubjectCohortMembership(value) {
  ...
  const { membership_receipt_id: receiptId, ...body } = value;
  if (
    !SHA256_RE.test(value.cohort_digest)
    || !SHA256_RE.test(value.subject_metric_output_id)
    || !SHA256_RE.test(value.counts_digest)
    || receiptId !== contentId(
      'SUBJECT_COHORT_MEMBERSHIP_RECEIPT/V1',
      body,
    )
...
function subjectMetricDistribution(
  terminal,
  valueDimension,
  cohortDigest,
  filters,
  ...
  const disposition = subjectDisposition(terminal, filters);
  const counts = disposition.status === 'EXCLUDED'
    ? {
        eligible_deals: 1,
        comparable_deals: 0,
        excluded_deals: 1,
  ...
  const membership = subjectCohortMembership({
    terminal,
    filters,
    cohortDigest,
    counts,
...
function emptyMarketContext(
  ...
  const cohortDigest = contentId(
    'QXO_CAPITALISATION_SELECTED_COHORT_F28/V1',
    {
      release_id: releaseId,
  ...
  const subrowMembership = subjectCohortMembership({
    terminal: terminalByOrdinal.get(bindings[0].display_ordinal),
    filters,
    cohortDigest,
    counts: {
      eligible_deals: 1,
  ...
  return {
    ...
    subject_cohort_membership: subrowMembership,
...
function validateReleaseEnvelope(release) {
  ...
  const defaultCohortDigest = contentId(
    'QXO_CAPITALISATION_SELECTED_COHORT_F28/V1',
    {
  ...
  for (const [index, subrow] of row.subrows.entries()) {
    ...
    for (const [metricIndex, metric] of
      ...
      const expectedMembership = expectedProjection
...
function compileQxoCapitalisationF28MarketRequest({
  ...
  const metricBindings = release.admissions.map((admission) => {
    ...
    return {
      ...
      subject_comparability: terminal.schema_version
        === 'MARKET_OBSERVATION_F28/V1'
        ? { status: 'COMPARABLE', reason: null }
        : {
  ...
  const selectedCohortDigest = contentId(
    'QXO_CAPITALISATION_SELECTED_COHORT_F28/V1',
    {
      release_id: release.release_id,
  ...
  const base = {
    ...
    broad_claim_or_card_loading: false,
    subject_membership_policy: SUBJECT_INCLUSION_POLICY,
    selected_cohort_digest: selectedCohortDigest,
...
function validateSubjectMembershipVerification(
  value,
  request,
  binding,
  membership,
) {
  const included = membership.status === 'INCLUDED';
  return exactKeys(value, [
    ...
    && value.membership_receipt_id === membership.membership_receipt_id
    && value.state === (
  ...
  for (const [index, slot] of result.slot_results.entries()) {
    ...
    if (
      ...
      || canonicalJson(slot.subject_cohort_membership)
        !== canonicalJson(requestSubjectCohortMembership(
          request,
          binding,
          slot.counts,
        ))
      || !validateSubjectMembershipVerification(
        ...
        request,
        binding,
        slot.subject_cohort_membership,
      )
      || ![
        'READY',
        'SUBJECT_INCLUDED_NO_INDEPENDENT_PEERS',
        'SUBJECT_EXCLUDED_SELECTED_NARROWER_COHORT',
        'SUBJECT_EXCLUDED_TYPED_NON_COMPARABILITY',
        'FAILED',
      ].includes(
        slot.result_state,
    ...
    if (
      ...
      && (
        ...
        || !ALLOWED_MARKET_METRIC_FAILURE_REASON_CODES.includes(
          slot.failure_reason_code,
        )
      )
    ) {
    ...
    if (
      ...
      const subjectStateGroup = slot.state_groups.find(
        (entry) => entry.state === binding.subject_state,
      );
      const subjectValueGroup = slot.value_groups.find(
        (entry) => canonicalJson(entry.canonical_value)
          === canonicalJson(binding.subject_canonical_value),
      );
      if (
        !subjectStateGroup
        || subjectStateGroup.deal_count < 1
        || (
          binding.subject_state === 'PRESENT'
          && (!subjectValueGroup || subjectValueGroup.deal_count < 1)
        )
      ) {
        throw new TypeError(
          'F28 included subject does not contribute its exact state and value',
        );
      }
    }
  }
...
function buildSubjectOnlyF28MarketResult(request) {
  const body = {
    ...
    slot_results: request.metric_bindings.map((binding) => {
      const disposition = requestSubjectDisposition(request, binding);
      const included = disposition.status === 'INCLUDED';
      ...
      const membership = requestSubjectCohortMembership(
        request,
        binding,
        counts,
      );
      return {
        ...
        result_state: included
          ? 'SUBJECT_INCLUDED_NO_INDEPENDENT_PEERS'
          : disposition.exclusion_reason.startsWith(
            'SELECTED_NARROWER_COHORT:',
          )
            ? 'SUBJECT_EXCLUDED_SELECTED_NARROWER_COHORT'
            : 'SUBJECT_EXCLUDED_TYPED_NON_COMPARABILITY',
