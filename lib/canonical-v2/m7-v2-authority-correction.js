...
const CORRECTION_PATH =
  `${MIGRATION_CONTROL_ROOT}/m7-v2-repair-candidate-replacement-authority-correction-1.json`;
...
const APPROVAL_KEYS = Object.freeze(['approved_on', 'ben_approval_id', 'state']);
...
const CONTRACT_CHANGE_CORRECTION_KEYS = Object.freeze([
  'code', 'contract_change', 'rationale',
]);
...
function validateApproval(approval) {
  ...
  assert(typeof approval.ben_approval_id === 'string'
    && approval.ben_approval_id.length > 0
...
function validateCorrections(corrections) {
  assert(Array.isArray(corrections)
    && same(corrections.map((entry) => entry?.code), CORRECTION_CODES),
  'CORRECTION_CONTRACT_DRIFT', 'corrections roster');
  const [argvCorrection, renameCorrection, contractCorrection] = corrections;
