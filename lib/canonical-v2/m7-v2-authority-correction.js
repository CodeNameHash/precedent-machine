...
//
// Three appliers, one per correction code this schema can express. A record
// carries a SUBSET of those codes, not all three, so an applier whose
// correction is absent is the identity. Each takes the value the authority
// declares and returns the value that is in force:
//
...
// intended callers of the first and third. Nothing in the tree consumes the
// contract change today either: approving it authorises a change to the
// sealed contract, it does not make one. Nothing here reads or writes
