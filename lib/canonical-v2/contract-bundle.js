...
const {
  REGISTRY_VERSION: REPRESENTATION_TOPIC_REGISTRY_VERSION,
  REGISTRY_DIGEST: REPRESENTATION_TOPIC_REGISTRY_DIGEST,
  TOPIC_RULES: REPRESENTATION_TOPIC_RULES,
...
// Ben-approved 2026-07-23 (docs/archive/handoffs/SPEC-CONTRACT-AMENDMENT-PATH-2026-07-23.md),
...
// field. Option B", cross-checked against decision 6). `payment_timing`
// conflated two things: TWO_BUSINESS_DAYS_AFTER_TERMINATION encodes an
// event AND a delay; UPON_EARLIER_OF_SIGNING_OR_CONSUMMATION encodes an
// event and implies a zero delay. Modiv's real drafting has a pattern
// (7.3(b)(iii): "within two (2) Business Days after the earlier of ...")
// that is the SECOND code's event with the FIRST code's delay -- unreachable
// under the old enum without inventing a third code, which would only
// reopen the same question on the next novel delay. TERMINATION_FEE_TRIGGER_
// PATH_SCHEMA_V2 above is frozen and untouched (`shared-serving-row.js`,
// `canonical-contract-technical-relationship-validator.js`,
// `qxo-buyer-termination-fee-trigger-detail.js` and their tests all still
// read it, unmodified) -- this is a new, additive schema VERSION, exactly
// the discipline this file's own top-of-file comment already requires for
// vocabulary growth. `termination-fee-trigger-path.js`'s
// `validateTerminationFeeTriggerEffect` is schema-version-aware: schema_version
// 2 effects are validated exactly as before, schema_version 3 effects
// against `allowed_payment_trigger_events` / `allowed_payment_delays`
// instead of `allowed_payment_timings`.
//
// Migration, not duplication: PAYMENT_TIMING_SPLIT_MIGRATION_V1 below is the
// exact, exhaustive map from both of QXO's legacy `payment_timing` codes to
// the new pair; V3's own `pathway_constraints` are DERIVED from V2's by
// running every entry through it, never hand-retyped, so the migration
// cannot silently drift from the values it replaces. No entry below carries
// a `payment_timing` key alongside the new pair -- the old single-field form
// does not appear anywhere in V3.
const PAYMENT_TIMING_SPLIT_MIGRATION_V1 = Object.freeze({
...
const TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3 = Object.freeze((() => {
  const {
    allowed_payment_timings: _legacyAllowedPaymentTimings,
    pathway_constraints: legacyPathwayConstraints,
  ...
  return {
    ...v2Rest,
    schema_version: 3,
    pathway_constraints: Object.freeze(legacyPathwayConstraints.map((entry) => {
      const { payment_timing: legacyPaymentTiming, ...rest } = entry;
      const split = PAYMENT_TIMING_SPLIT_MIGRATION_V1[legacyPaymentTiming];
      if (!split) {
        throw new Error(`no split-payment-timing migration recorded for legacy code: ${legacyPaymentTiming}`);
      }
      return Object.freeze({ ...rest, ...split });
    })),
    // The vocabulary the split requires and nothing beyond it: three
    // trigger events (TERMINATION, EARLIER_OF_SIGNING_OR_CONSUMMATION and
    // CONCURRENT_WITH_TERMINATION -- Modiv 7.3(b)(ii)'s "prior to or
    // substantially concurrently with such termination", decision 6's
    // condition-precedent concept on the fee side) and two delays (NONE,
    // TWO_BUSINESS_DAYS). All five real patterns known today (both QXO
    // codes, all three Modiv patterns) are combinations of these six values
    // -- see tests/canonical-v2-contract-bundle-v39.test.js.
    allowed_payment_trigger_events: Object.freeze([
      'TERMINATION',
      'EARLIER_OF_SIGNING_OR_CONSUMMATION',
      'CONCURRENT_WITH_TERMINATION',
    ]),
    allowed_payment_delays: Object.freeze(['NONE', 'TWO_BUSINESS_DAYS']),
  };
})());

// Step 3J2 (PLAN.md; DECISIONS.md decision 5, "RULED 2026-08-07... two
// further rulings"). Two independent gaps in TERMINATION_FEE_TRIGGER_PATH_
// SCHEMA_V3, found on a THIRD real agreement (Skechers) sitting in this
// repository's own fixtures -- neither is a widening of V3's existing
// vocabulary, so this is a new schema VERSION, not an edit to V3 (which
// stays frozen, exactly the discipline V2 -> V3 already established):
//
// 1. Skechers Section 8.3(b)(i): "the Company will concurrently with the
//    consummation of such Acquisition Transaction pay ... the Company
//    Termination Fee" -- consummation ALONE, not "earlier of signing or
//    consummation". Encoding it as EARLIER_OF_SIGNING_OR_CONSUMMATION
//    inverts the term: under that code the fee is owed at SIGNING even if
//    the later deal collapses; under Skechers' actual drafting it is owed
//    only at CLOSING. `CONSUMMATION` is a fourth, additional trigger event
//    -- TERMINATION, EARLIER_OF_SIGNING_OR_CONSUMMATION and CONCURRENT_
//    WITH_TERMINATION all carry forward from V3 unchanged.
// 2. `payment_delay` becomes a MEASURED, structured value --
//    `{ count, unit, bound_type }` -- rather than a wider enum. Decision 5:
//    "within three Business Days" (this repository's own financing corpus,
//    tests/fixtures/canonical-v2/financing-covenants-fixtures/
//    corpus-cards.json) fits neither of V3's two delay codes; and V3's
//    TWO_BUSINESS_DAYS conflates an EXACT period with an OUTER BOUND --
//    Skechers writes both "promptly (and in any event within two Business
//    Days)" (Section 8.3(b)(ii)) and "promptly, but in no event later than
//    two Business Days" (Section 8.3(c)) for its own two fee payments, and
//    both are outer bounds on a promptness covenant, not an exact two-day
//    term -- a distinction V3's single TWO_BUSINESS_DAYS code could not
//    express and never documented either way. Per decision 5, **adding a
//    duration is not a codebook decision requiring Ben** ("a duration is a
//    measurement, not a concept"); the trigger-event vocabulary (1 above)
//    stays reserved.
//
// PAYMENT_DELAY_STRUCTURE_MIGRATION_V1 is the exact, exhaustive map from
// both of V3's `payment_delay` tokens to the V4 structured form -- V4's
// `pathway_constraints` are DERIVED from V3's by running every entry
// through it below, never hand-retyped. `bound_type: 'OUTER_BOUND'` for
// TWO_BUSINESS_DAYS is not a guess: every real quote this programme has
// actually read for that code -- QXO's and Modiv's alike -- uses "within",
// which is an outer bound by its own ordinary meaning (see
// tests/canonical-v2-payment-timing-split.test.js's real, parsed Modiv
// quotes, and decision 5's own text: "[TWO_BUSINESS_DAYS] squeeze[s] into
// ... only if that code silently means outer bound").
const PAYMENT_DELAY_STRUCTURE_MIGRATION_V1 = Object.freeze({
  NONE: Object.freeze({ count: 0, unit: 'BUSINESS_DAYS', bound_type: 'EXACT' }),
  TWO_BUSINESS_DAYS: Object.freeze({ count: 2, unit: 'BUSINESS_DAYS', bound_type: 'OUTER_BOUND' }),
});
const TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V4 = Object.freeze((() => {
  const {
    allowed_payment_delays: _v3AllowedPaymentDelays,
    pathway_constraints: v3PathwayConstraints,
    schema_version: _v3SchemaVersion,
    ...v3Rest
  } = TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3;
  return {
    ...v3Rest,
    schema_version: 4,
    pathway_constraints: Object.freeze(v3PathwayConstraints.map((entry) => {
      const { payment_delay: v3PaymentDelay, ...rest } = entry;
      const structuredDelay = PAYMENT_DELAY_STRUCTURE_MIGRATION_V1[v3PaymentDelay];
      if (!structuredDelay) {
        throw new Error(`no payment-delay structure migration recorded for V3 delay: ${v3PaymentDelay}`);
      }
      return Object.freeze({ ...rest, payment_delay: structuredDelay });
    })),
    // `payment_trigger_event`: V3's three events plus CONSUMMATION -- no
    // existing pathway_constraint uses it yet (Skechers was never part of
    // the QXO-derived pathway lineage V2/V3's 9 constraints come from), so
    // it is declared here as governed vocabulary a future producer can
    // target, the same relationship V3's own CONCURRENT_WITH_TERMINATION
    // already has to this schema's pathway_constraints (declared, not yet
    // matched by any of the 9). `payment_delay` is no longer a closed enum:
    // `allowed_payment_delay_units` / `allowed_payment_delay_bound_types`
    // bound the SHAPE (unit and bound type), and `count` is validated as a
    // non-negative integer by termination-fee-trigger-path.js -- open to
    // any measured duration, per decision 5's "adding a duration is not a
    // codebook decision".
    allowed_payment_trigger_events: Object.freeze([
      'TERMINATION',
      'EARLIER_OF_SIGNING_OR_CONSUMMATION',
    ...
    allowed_payment_delay_units: Object.freeze(['BUSINESS_DAYS']),
    allowed_payment_delay_bound_types: Object.freeze(['EXACT', 'OUTER_BOUND']),
  };
})());
...
const DISCLOSURE_SCHEDULE_CARVEOUT_CLAIM_DEFINITION_V1 = Object.freeze({
...
const PERFORMANCE_VESTING_ASSUMPTION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'PERFORMANCE_VESTING_ASSUMPTION',
  version: 1,
  allowed_canonical_values: Object.freeze(['MAXIMUM', 'TARGET']),
  canonical_value_required_when_present: true,
});

const FIXTURE_CONTRACT_INPUT_V18 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V17,
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V17.claim_definitions,
    DISCLOSURE_SCHEDULE_CARVEOUT_CLAIM_DEFINITION_V1,
    PERFORMANCE_VESTING_ASSUMPTION_CLAIM_DEFINITION_V1,
  ]),
});

const REGULATORY_EFFORTS_STANDARD_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_EFFORTS_STANDARD', version: 1,
  allowed_canonical_values: Object.freeze(['REASONABLE_BEST_EFFORTS', 'BEST_EFFORTS', 'COMMERCIALLY_REASONABLE_EFFORTS', 'REASONABLE_EFFORTS', 'FLAT_OBLIGATION']),
  canonical_value_required_when_present: true,
});
const REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_BURDEN_COMMITMENT', version: 1,
  allowed_canonical_values: Object.freeze(['ANTI_HOHW', 'BURDENSOME_CONDITION', 'CAPPED_QUANTITATIVE', 'EXPRESS_HOHW', 'NO_LITIGATION_ONLY', 'NAMED_ASSET_CARVEOUT']),
  canonical_value_required_when_present: true,
});
const REGULATORY_DIVESTITURE_CAP_AMOUNT_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'REGULATORY_DIVESTITURE_CAP_AMOUNT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });
const REGULATORY_LITIGATION_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'REGULATORY_LITIGATION_OBLIGATION', version: 1, allowed_canonical_values: Object.freeze(['MANDATORY_DEFEND', 'TIME_BOUNDED', 'EXPRESS_NONE']), canonical_value_required_when_present: true });
const REGULATORY_TIMING_RESTRICTION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'REGULATORY_TIMING_RESTRICTION', version: 1, allowed_canonical_values: Object.freeze(['BARRED_MUTUAL_CONSENT', 'NOT_UNREASONABLY_WITHHELD', 'BUYER_ONLY_RESTRICTED', 'BUYER_OPTION_CONSULT']), canonical_value_required_when_present: true });
const HSR_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'HSR_FILING_DEADLINE_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });

const FIXTURE_CONTRACT_INPUT_V19 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V18,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V18.concepts,
    Object.freeze({ concept_key: 'ANTI-EFFORTS', version: 1 }), Object.freeze({ concept_key: 'ANTI-BURDEN', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-LITIGATION', version: 1 }), Object.freeze({ concept_key: 'ANTI-TIMING', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-FILING', version: 1 })]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V18.claim_definitions,
    REGULATORY_EFFORTS_STANDARD_CLAIM_DEFINITION_V1, REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V1,
    REGULATORY_DIVESTITURE_CAP_AMOUNT_CLAIM_DEFINITION_V1, REGULATORY_LITIGATION_OBLIGATION_CLAIM_DEFINITION_V1,
    REGULATORY_TIMING_RESTRICTION_CLAIM_DEFINITION_V1, HSR_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1]),
});

const REGULATORY_TIMING_AGREEMENT_RESTRICTION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_TIMING_AGREEMENT_RESTRICTION', version: 1,
  allowed_canonical_values: Object.freeze(['BARRED_MUTUAL_CONSENT', 'NOT_UNREASONABLY_WITHHELD', 'BUYER_ONLY_RESTRICTED']),
  canonical_value_required_when_present: true,
});
const REGULATORY_WITHDRAWAL_REFILING_RESTRICTION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_WITHDRAWAL_REFILING_RESTRICTION', version: 1,
  allowed_canonical_values: Object.freeze(['MUTUAL_CONSENT', 'SELLER_CONSENT_GATED', 'BUYER_UNILATERAL_GF']),
  canonical_value_required_when_present: true,
});
const REGULATORY_STRATEGY_CONTROL_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_STRATEGY_CONTROL', version: 1,
  allowed_canonical_values: Object.freeze(['PARENT_CONTROL', 'COMPANY_CONTROL', 'SHARED_CONTROL', 'BUYER_LEAD', 'BUYER_WITH_SETTLEMENT_GAG', 'PRINCIPAL_WITH_VETO', 'JURISDICTION_SPLIT', 'SELLER_LED']),
  canonical_value_required_when_present: true,
});
const REGULATORY_CONSULTATION_RIGHT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_CONSULTATION_RIGHT', version: 1,
  allowed_canonical_values: Object.freeze(['NOTICE_CONSULT', 'GOOD_FAITH_VIEWS', 'INCORPORATE_COMMENTS', 'PARTICIPATE', 'CONSENT_NOT_UNREASONABLE', 'CONSENT_SOLE']),
  canonical_value_required_when_present: true,
});
const REGULATORY_FILING_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_FILING_OBLIGATION', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});
const REGULATORY_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_FILING_DEADLINE_DAYS', version: 1,
  canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true,
});
const REGULATORY_NON_IMPEDIMENT_COVENANT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_NON_IMPEDIMENT_COVENANT', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});

const FIXTURE_CONTRACT_INPUT_V20 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V19,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V19.concepts,
    Object.freeze({ concept_key: 'ANTI-AGREEMENTS', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-STRATEGY', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-CONSULT', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-NOACTION', version: 1 })]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V19.claim_definitions,
    ...
    REGULATORY_FILING_OBLIGATION_CLAIM_DEFINITION_V1,
    REGULATORY_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1,
    REGULATORY_NON_IMPEDIMENT_COVENANT_CLAIM_DEFINITION_V1]),
});

const NO_SHOP_WAVE_B_CLAIM_DEFINITIONS_V1 = Object.freeze([
  Object.freeze({ claim_definition_key: 'NO_SHOP_CEASE_ACTION', version: 1, allowed_canonical_values: Object.freeze(['CEASE_EXISTING_DISCUSSIONS_OR_NEGOTIATIONS', 'CEASE_FURNISHING_INFORMATION', 'TERMINATE_DATA_ROOM_ACCESS', 'REQUEST_RETURN_OR_DESTRUCTION_OF_INFORMATION']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_REPRESENTATIVE_CONTROL_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['CAUSE_NOT_TO', 'REASONABLE_BEST_EFFORTS', 'INSTRUCT_NOT_TO']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_REPRESENTATIVE_BREACH_ATTRIBUTION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_STANDSTILL_ACTION', version: 1, allowed_canonical_values: Object.freeze(['ENFORCE', 'NO_WAIVER_RELEASE_OR_MODIFICATION', 'PERMIT_WAIVER_FOR_CONFIDENTIAL_PROPOSAL', 'PERMIT_WAIVER_FIDUCIARY_DUTY_GATED']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_FIDUCIARY_ENGAGEMENT_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['CONSTITUTES_OR_COULD_REASONABLY_BE_EXPECTED_TO_LEAD', 'COULD_REASONABLY_BE_EXPECTED_TO_LEAD', 'REASONABLY_LIKELY_TO_LEAD', 'CONSTITUTES_SUPERIOR_PROPOSAL']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_RECOMMENDATION_CHANGE_FIDUCIARY_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['INCONSISTENT_WITH_FIDUCIARY_DUTIES', 'BREACH_OF_FIDUCIARY_DUTIES', 'REASONABLY_LIKELY_BREACH']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_RECOMMENDATION_CHANGE_ACTION', version: 1, allowed_canonical_values: Object.freeze(['WITHDRAW_QUALIFY_OR_MODIFY_RECOMMENDATION', 'APPROVE_ENDORSE_OR_RECOMMEND_ACQUISITION_PROPOSAL', 'FAIL_TO_INCLUDE_RECOMMENDATION', 'FAIL_TO_REAFFIRM_RECOMMENDATION', 'TENDER_OFFER_NO_REJECTION', 'ENTER_ALTERNATIVE_AGREEMENT', 'RESOLVE_OR_AGREE_TO_FOREGOING']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_RECOMMENDATION_CHANGE_TRIGGER', version: 1, allowed_canonical_values: Object.freeze(['SUPERIOR_PROPOSAL', 'INTERVENING_EVENT']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_SHOP_RECOMMENDATION_SAFE_DISCLOSURE', version: 1, allowed_canonical_values: Object.freeze(['STOP_LOOK_AND_LISTEN', 'RULE_14D9_OR_14E2_DISCLOSURE', 'FACTUAL_DISCLOSURE_REAFFIRMING_RECOMMENDATION', 'DISCLOSURE_REQUIRED_BY_LAW']), canonical_value_required_when_present: true }),
]);
const FIXTURE_CONTRACT_INPUT_V21 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V20,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V20.concepts,
    Object.freeze({ concept_key: 'NOSOL-CEASE', version: 1 }), Object.freeze({ concept_key: 'NOSOL-ENFORCE', version: 1 }),
    Object.freeze({ concept_key: 'NOSOL-WAIVER', version: 1 }), Object.freeze({ concept_key: 'NOSOL-RECOMMEND', version: 1 })]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V20.claim_definitions, ...NO_SHOP_WAVE_B_CLAIM_DEFINITIONS_V1]),
});

const PER_SHARE_CASH_CONSIDERATION_CLAIM_DEFINITION_V1 = Object.freeze({
...
const EXCHANGE_RATIO_VALUE_CLAIM_DEFINITION_V1 = Object.freeze({
...
const APPRAISAL_RIGHTS_STATUS_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'APPRAISAL_RIGHTS_STATUS',
  version: 1,
  allowed_canonical_values: Object.freeze(['AVAILABLE', 'NOT_AVAILABLE']),
  canonical_value_required_when_present: true,
});

const FIXTURE_CONTRACT_INPUT_V22 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V21,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V21.concepts,
    Object.freeze({ concept_key: 'CONS-PERSHARE', version: 1 }),
    Object.freeze({ concept_key: 'CONS-RATIO', version: 1 }),
    Object.freeze({ concept_key: 'CONS-DISSENT', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V21.claim_definitions,
...
const NO_MAE_CONDITION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'NO_MAE_CONDITION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const NO_MAE_CONDITION_CONTINUING_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'NO_MAE_CONDITION_CONTINUING', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const COVENANT_COMPLIANCE_STANDARD_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'COVENANT_COMPLIANCE_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['MAT_ALL_MATERIAL']), canonical_value_required_when_present: true });
const REGULATORY_APPROVAL_CONDITION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'REGULATORY_APPROVAL_CONDITION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const FIXTURE_CONTRACT_INPUT_V23 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V22,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V22.concepts,
    Object.freeze({ concept_key: 'COND-S-REP', version: 1 }), Object.freeze({ concept_key: 'COND-MAE', version: 1 }),
    Object.freeze({ concept_key: 'COND-COV', version: 1 }), Object.freeze({ concept_key: 'COND-REG', version: 1 })]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V22.claim_definitions,
    NO_MAE_CONDITION_CLAIM_DEFINITION_V1, NO_MAE_CONDITION_CONTINUING_CLAIM_DEFINITION_V1,
    COVENANT_COMPLIANCE_STANDARD_CLAIM_DEFINITION_V1, REGULATORY_APPROVAL_CONDITION_CLAIM_DEFINITION_V1]),
});

const CLOSING_CONDITION_PRESENCE_CLAIM_KEYS_V24 = Object.freeze([
  'STOCKHOLDER_APPROVAL_CONDITION',
  'LEGAL_RESTRAINT_CONDITION',
  'GOVERNMENT_PROCEEDING_CONDITION',
  'LISTING_CONDITION',
  'FUNDS_AVAILABILITY_CONDITION',
  'OFFICER_CERTIFICATE_REQUIRED',
  'BURDENSOME_CONDITION',
]);
const CLOSING_CONDITION_PRESENCE_CLAIM_DEFINITIONS_V24 = Object.freeze(
  CLOSING_CONDITION_PRESENCE_CLAIM_KEYS_V24.map((claimDefinitionKey) => Object.freeze({
...
const S4_CONDITION_COMPONENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'S4_CONDITION_COMPONENT', version: 1,
  allowed_canonical_values: Object.freeze(['FORM_EFFECTIVE', 'NO_STOP_ORDER', 'NO_PENDING_OR_THREATENED_PROCEEDING']),
  canonical_value_required_when_present: true,
});
const FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'FRUSTRATION_CAUSATION_STANDARD', version: 1,
  allowed_canonical_values: Object.freeze(['CAUSED_BY_BREACH', 'PROXIMATELY_CAUSED', 'PRINCIPALLY_CAUSED', 'PRIMARILY_RESULTED']),
  canonical_value_required_when_present: true,
});
const FRUSTRATION_BREACH_STANDARD_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'FRUSTRATION_BREACH_STANDARD', version: 1,
  allowed_canonical_values: Object.freeze(['BREACH', 'MATERIAL_BREACH', 'FAILURE_TO_PERFORM']),
  canonical_value_required_when_present: true,
});
const CONDITION_DOLLAR_THRESHOLD_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'CONDITION_DOLLAR_THRESHOLD', version: 1,
  canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true,
});
const FIXTURE_CONTRACT_INPUT_V24 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V23,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V23.concepts,
    ...['COND-M-STOCKHOLDER', 'COND-M-LEGAL', 'COND-M-S4', 'COND-M-LISTING',
      'COND-S-FUNDS', 'COND-B-CERT', 'COND-S-CERT', 'COND-FRUSTRATE']
      .map((conceptKey) => Object.freeze({ concept_key: conceptKey, version: 1 }))]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V23.claim_definitions,
    ...CLOSING_CONDITION_PRESENCE_CLAIM_DEFINITIONS_V24,
    S4_CONDITION_COMPONENT_CLAIM_DEFINITION_V1,
    FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V1,
    FRUSTRATION_BREACH_STANDARD_CLAIM_DEFINITION_V1,
    CONDITION_DOLLAR_THRESHOLD_CLAIM_DEFINITION_V1]),
});

const IOC_RESTRICTION_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'IOC_RESTRICTION_PRESENT',
  version: 1,
  allowed_canonical_values: Object.freeze([true]),
  canonical_value_required_when_present: true,
});
const IOC_RESTRICTION_CONCEPT_KEYS_V25 = Object.freeze([
  'IOC-MERGE', 'IOC-CONTRACT', 'IOC-COMP', 'IOC-DEBT', 'IOC-TAX',
  'IOC-CHARTER', 'IOC-ISSUE', 'IOC-ACCOUNTING', 'IOC-SETTLE', 'IOC-DIVIDEND',
  'IOC-LIEN', 'IOC-REPURCHASE', 'IOC-HIRE', 'IOC-IP', 'IOC-INSURANCE',
  'IOC-REALPROP', 'IOC-AFFILIATE', 'IOC-REGAUTH', 'IOC-ORDINARY',
]);
const FIXTURE_CONTRACT_INPUT_V25 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V24,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V24.concepts,
    ...IOC_RESTRICTION_CONCEPT_KEYS_V25.map(
      (conceptKey) => Object.freeze({ concept_key: conceptKey, version: 1 }),
    ),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V24.claim_definitions,
    IOC_RESTRICTION_PRESENT_CLAIM_DEFINITION_V1,
  ]),
});

const PROXY_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'PROXY_FILING_DEADLINE_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });
const MEETING_DEADLINE_DAYS_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MEETING_DEADLINE_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });
const MEETING_ADJOURNMENT_MAX_COUNT_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MEETING_ADJOURNMENT_MAX_COUNT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });
const MEETING_ADJOURNMENT_MAX_DAYS_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MEETING_ADJOURNMENT_MAX_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true });
const BOARD_RECOMMENDATION_INCLUSION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'BOARD_RECOMMENDATION_INCLUSION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const MEETING_CONVENE_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MEETING_CONVENE_OBLIGATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const PROXY_MEETING_CLAIM_DEFINITIONS_V26 = Object.freeze([
  PROXY_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1,
  MEETING_DEADLINE_DAYS_CLAIM_DEFINITION_V1,
  MEETING_ADJOURNMENT_MAX_COUNT_CLAIM_DEFINITION_V1,
  MEETING_ADJOURNMENT_MAX_DAYS_CLAIM_DEFINITION_V1,
  BOARD_RECOMMENDATION_INCLUSION_CLAIM_DEFINITION_V1,
  MEETING_CONVENE_OBLIGATION_CLAIM_DEFINITION_V1,
]);
const FIXTURE_CONTRACT_INPUT_V26 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V25,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V25.concepts,
    Object.freeze({ concept_key: 'COV-PROXY', version: 1 }),
    Object.freeze({ concept_key: 'COV-MEETING', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V25.claim_definitions,
    ...PROXY_MEETING_CLAIM_DEFINITIONS_V26,
  ]),
});

const MATERIAL_CONTRACT_BUCKET_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'MATERIAL_CONTRACT_BUCKET_PRESENT',
  version: 1,
  allowed_canonical_values: Object.freeze([
...
const MATERIAL_CONTRACT_THRESHOLD_STRUCTURE_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'MATERIAL_CONTRACT_THRESHOLD_STRUCTURE',
  version: 1,
  allowed_canonical_values: Object.freeze(['ANY', 'PERCENT', 'TOP_N', 'USD']),
  canonical_value_required_when_present: true,
});

const FIXTURE_CONTRACT_INPUT_V27 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V26,
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V26.claim_definitions,
    MATERIAL_CONTRACT_BUCKET_PRESENT_CLAIM_DEFINITION_V1,
    MATERIAL_CONTRACT_THRESHOLD_STRUCTURE_CLAIM_DEFINITION_V1,
  ]),
});

const GENERAL_COVENANT_CODES_V1 = Object.freeze([
  'COV-16B',
  'COV-ACCESS',
  'COV-CONSENT',
  'COV-CVR',
...
const GENERAL_COVENANT_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'GENERAL_COVENANT_PRESENT',
  version: 1,
  allowed_canonical_values: GENERAL_COVENANT_CODES_V1,
  canonical_value_required_when_present: true,
});

const FIXTURE_CONTRACT_INPUT_V28 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V27,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V27.concepts,
    ...GENERAL_COVENANT_CODES_V1.map((conceptKey) => Object.freeze({
      concept_key: conceptKey,
      version: 1,
    })),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V27.claim_definitions,
    GENERAL_COVENANT_PRESENT_CLAIM_DEFINITION_V1,
  ]),
});

const RECORD_DATE_ESTABLISHMENT_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'RECORD_DATE_ESTABLISHMENT_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const BROKER_SEARCH_OBLIGATION_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'BROKER_SEARCH_OBLIGATION_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const PARENT_APPROVAL_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'PARENT_APPROVAL_OBLIGATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const MERGER_SUB_APPROVAL_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MERGER_SUB_APPROVAL_OBLIGATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true });
const MEETING_ADJOURNMENT_REASON_CLAIM_DEFINITION_V1 = Object.freeze({ claim_definition_key: 'MEETING_ADJOURNMENT_REASON', version: 1, allowed_canonical_values: Object.freeze(['QUORUM_ABSENT', 'INSUFFICIENT_VOTES', 'SUPPLEMENTAL_DISCLOSURE', 'LEGAL_REQUIREMENT']), canonical_value_required_when_present: true });
const PROXY_MEETING_FOLLOW_ON_CLAIM_DEFINITIONS_V29 = Object.freeze([
  RECORD_DATE_ESTABLISHMENT_PRESENT_CLAIM_DEFINITION_V1,
  BROKER_SEARCH_OBLIGATION_PRESENT_CLAIM_DEFINITION_V1,
  PARENT_APPROVAL_OBLIGATION_CLAIM_DEFINITION_V1,
  MERGER_SUB_APPROVAL_OBLIGATION_CLAIM_DEFINITION_V1,
  MEETING_ADJOURNMENT_REASON_CLAIM_DEFINITION_V1,
]);
const FIXTURE_CONTRACT_INPUT_V29 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V28,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V28.concepts,
    Object.freeze({ concept_key: 'COV-PROXY-PARENT-APPROVAL', version: 1 }),
    Object.freeze({ concept_key: 'COV-PROXY-MERGERSUB-APPROVAL', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V28.claim_definitions,
    ...PROXY_MEETING_FOLLOW_ON_CLAIM_DEFINITIONS_V29,
  ]),
});

const FINANCING_GUARANTY_CLAIM_DEFINITIONS_V30 = Object.freeze([
  Object.freeze({ claim_definition_key: 'FINANCING_OBTAIN_EFFORTS_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['REASONABLE_BEST_EFFORTS', 'COMMERCIALLY_REASONABLE_EFFORTS']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'FINANCING_COOPERATION_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'NO_FINANCING_CONDITION_ACKNOWLEDGMENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'PAYOFF_DELIVERY_LEAD_TIME_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'MARKETING_PERIOD_LENGTH_DAYS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'PARENT_PERFORMANCE_GUARANTY', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'LIMITED_GUARANTY_DELIVERED', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'LIMITED_GUARANTY_IN_EFFECT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
]);
const FIXTURE_CONTRACT_INPUT_V30 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V29,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V29.concepts,
    Object.freeze({ concept_key: 'COV-FINANCING', version: 1 }),
    Object.freeze({ concept_key: 'COV-MARKETING', version: 1 }),
    Object.freeze({ concept_key: 'COV-PAYOFF', version: 1 }),
    Object.freeze({ concept_key: 'GTY-DELIVERY', version: 1 }),
    Object.freeze({ concept_key: 'GTY-PERF', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V29.claim_definitions,
    ...FINANCING_GUARANTY_CLAIM_DEFINITIONS_V30,
  ]),
});

const KEY_DEFINED_TERM_CONCEPT_KEYS_V31 = Object.freeze([
  'DEF-ACQPROPOSAL', 'DEF-SUPERIOR', 'DEF-INTERVENING', 'DEF-KNOWLEDGE', 'DEF-WILLFUL',
]);
const KEY_DEFINED_TERM_CLAIM_DEFINITIONS_V31 = Object.freeze([
  Object.freeze({ claim_definition_key: 'ACQUISITION_PROPOSAL_THRESHOLD_PERCENT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'SUPERIOR_PROPOSAL_THRESHOLD_PERCENT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'DEFINED_TERM_THRESHOLD_SUBSTITUTION', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'SUPERIOR_PROPOSAL_QUALIFIER', version: 1, allowed_canonical_values: Object.freeze(['FINANCIAL_FAVORABILITY', 'CONSUMMATION_LIKELIHOOD']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'INTERVENING_EVENT_DEFINITION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'INTERVENING_EVENT_EXCLUSION', version: 1, allowed_canonical_values: Object.freeze(['ACQUISITION_PROPOSAL_RECEIPT', 'STOCK_PRICE_CHANGE', 'NON_MAE_EFFECT']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'KNOWLEDGE_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['ACTUAL', 'AFTER_INQUIRY', 'CONSTRUCTIVE']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'KNOWLEDGE_PERSON_SOURCE', version: 1, allowed_canonical_values: Object.freeze(['NAMED_INDIVIDUALS', 'SCHEDULE_REFERENCE', 'TITLE_CLASS']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'WILLFUL_BREACH_DEFINITION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'WILLFUL_BREACH_KNOWLEDGE_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['ACTUAL', 'ACTUAL_OR_CONSTRUCTIVE']), canonical_value_required_when_present: true }),
]);
const TAX_DIVIDENDS_APPRAISAL_CLAIM_DEFINITIONS_V31 = Object.freeze([
  Object.freeze({ claim_definition_key: 'INTENDED_TAX_TREATMENT_KIND', version: 1, allowed_canonical_values: Object.freeze(['REORG_368A', 'TRANSFER_351']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TAX_TREATMENT_PROTECTION_COVENANT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TRANSFER_TAX_ALLOCATION', version: 1, allowed_canonical_values: Object.freeze(['PARENT', 'SPLIT_EVEN', 'SURVIVING_CORPORATION']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TRANSFER_TAX_COOPERATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'FIRPTA_CERTIFICATE_DELIVERY', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'DIVIDEND_COORDINATION_COVENANT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'PRE_CLOSING_SPECIAL_DIVIDEND', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'PRE_CLOSING_SPECIAL_DIVIDEND_PER_SHARE_AMOUNT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'APPRAISAL_SETTLEMENT_CONSENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'APPRAISAL_WITHDRAWAL_RECONVERSION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
]);
const FIXTURE_CONTRACT_INPUT_V31 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V30,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V30.concepts,
    ...KEY_DEFINED_TERM_CONCEPT_KEYS_V31.map((concept_key) => Object.freeze({ concept_key, version: 1 })),
    ...['TAXM-TREATMENT', 'TAXM-TRANSFER', 'TAXM-FIRPTA', 'DIVD-COORD', 'DIVD-SPECIAL', 'APPR-SETTLE', 'APPR-WITHDRAW'].map((concept_key) => Object.freeze({ concept_key, version: 1 })),
  ]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V30.claim_definitions, ...KEY_DEFINED_TERM_CLAIM_DEFINITIONS_V31, ...TAX_DIVIDENDS_APPRAISAL_CLAIM_DEFINITIONS_V31]),
});

const EMPLOYEE_DNO_CLAIM_DEFINITIONS_V32 = Object.freeze([
  Object.freeze({ claim_definition_key: 'EMPLOYEE_COMP_ITEM_STANDARD', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'BENEFITS_CONTINUATION_PERIOD_MONTHS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'EMPLOYEE_SERVICE_CREDIT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'WELFARE_PLAN_TRANSITION_RELIEF', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'EMPLOYEE_MATTERS_TPB_DISCLAIMER', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'INDEMNIFICATION_CONTINUATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'CHARTER_PROTECTION_CONTINUATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'ADVANCEMENT_OF_EXPENSES', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'INDEMNIFICATION_SURVIVAL_YEARS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TAIL_POLICY_OBLIGATION', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TAIL_POLICY_PERIOD_YEARS', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TAIL_PREMIUM_CAP_PERCENT', version: 1, canonical_value_type: 'NON_NEGATIVE_DECIMAL_STRING', canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'TAIL_PREMIUM_CAP_OFF_AGREEMENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'COVERED_PERSON_TPB_RIGHTS', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
]);
const FIXTURE_CONTRACT_INPUT_V32 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V31,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V31.concepts,
    Object.freeze({ concept_key: 'COV-EMPLOYEE', version: 1 }),
    Object.freeze({ concept_key: 'DNO-BENEF', version: 1 }),
    Object.freeze({ concept_key: 'DNO-INDEM', version: 1 }),
    Object.freeze({ concept_key: 'DNO-TAIL', version: 1 })]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V31.claim_definitions, ...EMPLOYEE_DNO_CLAIM_DEFINITIONS_V32]),
});
const M3_REPRESENTATIONS_MERGER_REMEDIES_MISC_CLAIM_DEFINITIONS_V33 = Object.freeze([
  Object.freeze({ claim_definition_key: 'REPRESENTATION_QUALIFIER_STANDARD', version: 1, allowed_canonical_values: Object.freeze(['ACTUAL', 'CONSTRUCTIVE', 'AFTER_INQUIRY', 'MAT_ALL_RESPECTS', 'MAT_ALL_RESPECTS_DE_MINIMIS', 'MAT_ALL_MATERIAL', 'MAT_MATERIAL_TO_COMPANY', 'MAT_MAE_QUALIFIED']), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'MERGER_STRUCTURE_MECHANIC_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'SPECIFIC_PERFORMANCE_REMEDY_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
  Object.freeze({ claim_definition_key: 'MISC_BOILERPLATE_MECHANIC_PRESENT', version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true }),
]);
const FIXTURE_CONTRACT_INPUT_V33 = Object.freeze({ ...FIXTURE_CONTRACT_INPUT_V32,
  concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V32.concepts, ...['REP-T-QUALIFIER', 'REP-B-QUALIFIER', 'MERGER-STRUCTURE', 'REMEDY-SPECIFIC-PERFORMANCE', 'MISC-BOILERPLATE'].map((concept_key) => Object.freeze({ concept_key, version: 1 }))]),
  claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V32.claim_definitions, ...M3_REPRESENTATIONS_MERGER_REMEDIES_MISC_CLAIM_DEFINITIONS_V33]),
});
const NO_OTHER_REPS_FRAUD_CONCEPT_KEYS_V34 = Object.freeze(['REP-B-FRAUDCARVEOUT', 'REP-B-INDEPINVEST', 'REP-B-NOOTHERREPS', 'REP-B-NONRELIANCE', 'REP-T-FRAUDCARVEOUT', 'REP-T-INDEPINVEST', 'REP-T-NOOTHERREPS', 'REP-T-NONRELIANCE']);
const NO_OTHER_REPS_FRAUD_CLAIM_DEFINITIONS_V34 = Object.freeze(['NO_OTHER_REPRESENTATIONS_DISCLAIMER_PRESENT', 'NON_RELIANCE_ACKNOWLEDGMENT_PRESENT', 'EXTRA_CONTRACTUAL_RELIANCE_DISCLAIMER_PRESENT', 'INDEPENDENT_INVESTIGATION_ACKNOWLEDGMENT_PRESENT', 'FRAUD_CARVEOUT_PRESENT', 'WILLFUL_BREACH_DEFINITION_PRESENT'].map((claim_definition_key) => Object.freeze({ claim_definition_key, version: 1, allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true })));
const FIXTURE_CONTRACT_INPUT_V34 = Object.freeze({ ...FIXTURE_CONTRACT_INPUT_V33, concepts: Object.freeze([...FIXTURE_CONTRACT_INPUT_V33.concepts, ...NO_OTHER_REPS_FRAUD_CONCEPT_KEYS_V34.map((concept_key) => Object.freeze({ concept_key, version: 1 }))]), claim_definitions: Object.freeze([...FIXTURE_CONTRACT_INPUT_V33.claim_definitions, ...NO_OTHER_REPS_FRAUD_CLAIM_DEFINITIONS_V34]) });
const RECORDED_DEFINED_TERM_CONCEPT_KEYS_V35 = Object.freeze([
  'DEF-TAX', 'DEF-TAX-RETURN', 'DEF-MADE-AVAILABLE', 'DEF-ORDINARY-COURSE',
]);
const RECORDED_DEFINED_TERM_CLAIM_DEFINITIONS_V35 = Object.freeze([
...
].map((claim_definition_key) => Object.freeze({
  claim_definition_key,
  version: 1,
  allowed_canonical_values: Object.freeze([true]),
  canonical_value_required_when_present: true,
})));
const FIXTURE_CONTRACT_INPUT_V35 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V34,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V34.concepts,
    ...RECORDED_DEFINED_TERM_CONCEPT_KEYS_V35.map((concept_key) => Object.freeze({ concept_key, version: 1 })),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V34.claim_definitions,
    ...RECORDED_DEFINED_TERM_CLAIM_DEFINITIONS_V35,
  ]),
});

const REGULATORY_COOPERATION_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_COOPERATION_OBLIGATION', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});
const REGULATORY_INFORMATION_SHARING_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_INFORMATION_SHARING_OBLIGATION', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});
const REGULATORY_NOTIFICATION_OBLIGATION_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_NOTIFICATION_OBLIGATION', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});
const REGULATORY_FILING_TIMING_STANDARD_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REGULATORY_FILING_TIMING_STANDARD', version: 1,
  allowed_canonical_values: Object.freeze([
    'AS_PROMPTLY_AS_PRACTICABLE',
    'AS_PROMPTLY_AS_REASONABLY_PRACTICABLE',
    'AS_SOON_AS_PRACTICABLE',
    'AS_SOON_AS_REASONABLY_PRACTICABLE',
    'PROMPTLY',
    'FIXED_DATE',
  ]),
  canonical_value_required_when_present: true,
});
const EXPANDED_ANTITRUST_CLAIM_DEFINITIONS_V36 = Object.freeze([
  REGULATORY_COOPERATION_OBLIGATION_CLAIM_DEFINITION_V1,
  REGULATORY_INFORMATION_SHARING_OBLIGATION_CLAIM_DEFINITION_V1,
  REGULATORY_NOTIFICATION_OBLIGATION_CLAIM_DEFINITION_V1,
  REGULATORY_FILING_TIMING_STANDARD_CLAIM_DEFINITION_V1,
]);
const REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V36 = Object.freeze({
  ...REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V1,
  allowed_canonical_values: Object.freeze(
    REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V1.allowed_canonical_values
      .filter((value) => value !== 'NAMED_ASSET_CARVEOUT'),
  ),
});
const RETIRED_ANTITRUST_CONCEPT_KEYS_V36 = Object.freeze(['ANTI-TIMING', 'ANTI-FOREIGN', 'ANTI-INTERIM']);
const RETIRED_ANTITRUST_CLAIM_KEYS_V36 = Object.freeze(['REGULATORY_TIMING_RESTRICTION']);
const FIXTURE_CONTRACT_INPUT_V36 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V35,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V35.concepts.filter(({ concept_key }) => !RETIRED_ANTITRUST_CONCEPT_KEYS_V36.includes(concept_key)),
    Object.freeze({ concept_key: 'ANTI-COOPERATE', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-INFO', version: 1 }),
    Object.freeze({ concept_key: 'ANTI-NOTIFY', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V35.claim_definitions.filter(({ claim_definition_key }) => (
      !RETIRED_ANTITRUST_CLAIM_KEYS_V36.includes(claim_definition_key)
      && claim_definition_key !== 'REGULATORY_BURDEN_COMMITMENT'
    )),
    REGULATORY_BURDEN_COMMITMENT_CLAIM_DEFINITION_V36,
    ...EXPANDED_ANTITRUST_CLAIM_DEFINITIONS_V36,
  ]),
});
const TAX_OPINION_COOPERATION_CLAIM_DEFINITION_V37 = Object.freeze({
  claim_definition_key: 'TAX_OPINION_COOPERATION_COVENANT', version: 1,
  allowed_canonical_values: Object.freeze([true]), canonical_value_required_when_present: true,
});
const FIXTURE_CONTRACT_INPUT_V37 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V36,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V36.concepts,
    Object.freeze({ concept_key: 'TAXM-OPINION', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V36.claim_definitions,
    TAX_OPINION_COOPERATION_CLAIM_DEFINITION_V37,
  ]),
});

const SOLE_REMEDY_CLAIM_DEFINITIONS_V38 = Object.freeze([
  Object.freeze({
    claim_definition_key: 'SOLE_REMEDY_LEGAL_EFFECT_PRESENT',
    version: 1,
    allowed_canonical_values: Object.freeze([true]),
    canonical_value_required_when_present: true,
  }),
  Object.freeze({
    claim_definition_key: 'SOLE_REMEDY_CARVEOUT_KIND',
    version: 1,
    allowed_canonical_values: Object.freeze(['FRAUD', 'WILLFUL_BREACH']),
    canonical_value_required_when_present: true,
  }),
]);
const FIXTURE_CONTRACT_INPUT_V38 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V37,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V37.concepts,
    Object.freeze({ concept_key: 'REM-SOLE', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V37.claim_definitions,
    ...SOLE_REMEDY_CLAIM_DEFINITIONS_V38,
  ]),
});

// Step 3J: strictly additive spread of V38 -- zero concept or claim-
// definition additions, `serving_trigger_path_schemas` gains
// TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3 alongside the existing (unedited)
// V2, so this is the dual-numbering case PLAN.md's own knowledge table
// warns about: fixture-input version moves to V39, concept-keys version
// stays at V24 (no new concept), claim-keys version stays at V38 (no new
// claim definition -- the split fields are schema-embedded, like
// `terminating_party`, not governed claim definitions).
const FIXTURE_CONTRACT_INPUT_V39 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V38,
  serving_trigger_path_schemas: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V38.serving_trigger_path_schemas,
    TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3,
  ]),
});

// Step 3J2: same dual-numbering case as V39 -- strictly additive spread of
// V39, zero concept or claim-definition additions (concepts stay at V24,
// claims stay at V38: the CONSUMMATION event and the structured delay are
// schema-embedded fields, not governed claim definitions, exactly as V3's
// own fields were), `serving_trigger_path_schemas` gains TERMINATION_FEE_
// TRIGGER_PATH_SCHEMA_V4 alongside the existing, unedited V2 and V3.
const FIXTURE_CONTRACT_INPUT_V40 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V39,
  serving_trigger_path_schemas: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V39.serving_trigger_path_schemas,
    TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V4,
  ]),
});

// Stage 3 (docs/codex-program/notes/structural-inheritance-diagnosis.md;
// Ben ruled directly that materiality standards are DISTINCT facts -- "in
// any material respect" and "in all material respects" carry separate
// codes, never aliased -- which is the codebook approval this change
// implements). REPRESENTATION_ACCURACY_STANDARD's allowed value set widens
// by MAT_MATERIAL_INLINE ("in any material respect" -- materiality inline
// within the rep), consumed from lib/taxonomy.js MATERIALITY_CODES.
// Step 2X-D / DECISIONS.md entry 14 (Ben, 2026-08-08): MAT_MAE_AGGREGATE
// was briefly admitted here under the Stage 3 never-alias split; REMOVED
// because that phrase is a drafting variant of MAT_MAE_QUALIFIED, not a
// separate legal standard. This was true on the branch that authored
// 2X-D (cursor/step-2x-d-mae-revert-945e), which had no committed V2
// resolution.json carrying the retired code -- but is NOT true on this
// branch: evidence/canonical-v2/redhat-representations-20260808-2xl-replay/
// (Step 2X-L, committed here before 2X-D was integrated) carries
// MAT_MAE_AGGREGATE as canonical_value on 13 REPRESENTATION_ACCURACY_STANDARD
// claims, in resolution.json, validation.json and adapter-result.json.
// Narrowing this list in place does NOT delete or corrupt that evidence --
// it is a static record of a run that happened under the pre-revert
// lexicon -- but it does mean any later re-validation of that exact
// resolution.json against this V41 list produces INVALID_CANONICAL_VALUE
// residuals and quarantines those 13 claims (lib/canonical-v2/
// validate-write-set.js canonicalValueAllowed / structuralResiduals), not
// silent acceptance. Confirmed by replaying that run (zero model calls,
// same fixtures) against this post-revert code: resolved count is
// unchanged at 32, all 13 reclassify as MAT_MAE_QUALIFIED, 0 residuals, 0
// quarantines, validation accepted. The committed 2xl-replay directory
// itself is left as the historical record pending its own regeneration by
// whoever owns that step, per DECISIONS.md entry 14's normal remedy for
// stale evidence -- rerun, don't hand-edit. V1 MATERIALITY_CODES keeps the key for legacy data/UI. WIDENING (of
// MAT_MATERIAL_INLINE) stays in place, claim_definition version STAYS 1,
// deliberately: the pre-existing values keep their exact meaning, so
// every already-resolved accuracy claim's identity (which folds
// claim_definition_version) must not re-mint. The entry is REPLACED
// rather than appended because claim_definition_key is a unique key in
// the compiled vocabulary -- this is the one deliberate exception to the
// plain append pattern, stated here rather than hidden in the map call.
const REPRESENTATION_ACCURACY_STANDARD_ALLOWED_VALUES_V41 = Object.freeze([
  'MAT_ALL_RESPECTS',
  'MAT_ALL_RESPECTS_DE_MINIMIS',
  'MAT_ALL_MATERIAL',
  'MAT_MAE_QUALIFIED',
...
const FIXTURE_CONTRACT_INPUT_V41 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V40,
  claim_definitions: Object.freeze(FIXTURE_CONTRACT_INPUT_V40.claim_definitions.map((definition) => (
    definition.claim_definition_key === 'REPRESENTATION_ACCURACY_STANDARD'
      ? Object.freeze({
        ...definition,
        allowed_canonical_values: REPRESENTATION_ACCURACY_STANDARD_ALLOWED_VALUES_V41,
      })
      : definition
  ))),
});
const MERGER_TRANSACTION_STEP_CLAIM_DEFINITION_V42 = Object.freeze({
  claim_definition_key: 'MERGER_TRANSACTION_STEP',
  version: 1,
  allowed_canonical_values: Object.freeze([true]),
  canonical_value_required_when_present: true,
});
const FIXTURE_CONTRACT_INPUT_V42 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V41,
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V41.claim_definitions,
    MERGER_TRANSACTION_STEP_CLAIM_DEFINITION_V42,
  ]),
});

const REPRESENTATION_TOPIC_CODES_V1 = Object.freeze(
  REPRESENTATION_TOPIC_RULES
    .map(({ id }) => id.replace('REP_TOPIC_V1_', ''))
    .sort(),
);
const REPRESENTATION_TOPIC_REGISTRY_BINDING_V1 = Object.freeze({
  registry_version: REPRESENTATION_TOPIC_REGISTRY_VERSION,
  registry_digest: REPRESENTATION_TOPIC_REGISTRY_DIGEST,
});
const REPRESENTATION_TOPIC_PRESENT_CLAIM_DEFINITION_V1 = Object.freeze({
  claim_definition_key: 'REPRESENTATION_TOPIC_PRESENT',
  version: 1,
  allowed_canonical_values: REPRESENTATION_TOPIC_CODES_V1,
  canonical_value_required_when_present: true,
});
const FIXTURE_CONTRACT_INPUT_V43 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V42,
  representation_topic_registry_binding: REPRESENTATION_TOPIC_REGISTRY_BINDING_V1,
  concepts: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V42.concepts,
    Object.freeze({ concept_key: 'REP-T-TOPIC', version: 1 }),
    Object.freeze({ concept_key: 'REP-B-TOPIC', version: 1 }),
  ]),
  claim_definitions: Object.freeze([
    ...FIXTURE_CONTRACT_INPUT_V42.claim_definitions,
    REPRESENTATION_TOPIC_PRESENT_CLAIM_DEFINITION_V1,
  ]),
});

// V44 widens one existing enum without rewriting V24 through V43. The
// definition version remains 1 because every old value keeps the same legal
// meaning and existing claims must keep their identity. Only the new
// PRIMARILY_CAUSED value becomes admissible under this contract version.
const FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V44 = Object.freeze({
  ...FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V1,
  allowed_canonical_values: Object.freeze([
    ...FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V1.allowed_canonical_values,
    'PRIMARILY_CAUSED',
  ]),
});
const FIXTURE_CONTRACT_INPUT_V44 = Object.freeze({
  ...FIXTURE_CONTRACT_INPUT_V43,
  claim_definitions: Object.freeze(FIXTURE_CONTRACT_INPUT_V43.claim_definitions.map((definition) => (
    definition.claim_definition_key === 'FRUSTRATION_CAUSATION_STANDARD'
      ? FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V44
      : definition
  ))),
});
...
const EXPECTED_CONCEPT_KEYS_V6 = [...EXPECTED_CONCEPT_KEYS_V5, 'ANTI-EFFORTS', 'ANTI-BURDEN', 'ANTI-LITIGATION', 'ANTI-TIMING', 'ANTI-FILING'].sort();
const EXPECTED_CONCEPT_KEYS_V7 = [...EXPECTED_CONCEPT_KEYS_V6, 'ANTI-AGREEMENTS', 'ANTI-CONSULT', 'ANTI-NOACTION', 'ANTI-STRATEGY'].sort();
const EXPECTED_CONCEPT_KEYS_V8 = [...EXPECTED_CONCEPT_KEYS_V7, 'NOSOL-CEASE', 'NOSOL-ENFORCE', 'NOSOL-WAIVER', 'NOSOL-RECOMMEND'].sort();
const EXPECTED_CONCEPT_KEYS_V9 = [
  ...EXPECTED_CONCEPT_KEYS_V8,
  'CONS-PERSHARE',
  'CONS-RATIO',
  'CONS-DISSENT',
].sort();
const EXPECTED_CONCEPT_KEYS_V10 = [...EXPECTED_CONCEPT_KEYS_V9, 'COND-S-REP', 'COND-MAE', 'COND-COV', 'COND-REG'].sort();
const EXPECTED_CONCEPT_KEYS_V11 = [...EXPECTED_CONCEPT_KEYS_V10,
  'COND-M-STOCKHOLDER', 'COND-M-LEGAL', 'COND-M-S4', 'COND-M-LISTING',
  'COND-S-FUNDS', 'COND-B-CERT', 'COND-S-CERT', 'COND-FRUSTRATE'].sort();
const EXPECTED_CONCEPT_KEYS_V12 = [
  ...EXPECTED_CONCEPT_KEYS_V11,
  ...IOC_RESTRICTION_CONCEPT_KEYS_V25,
].sort();
const EXPECTED_CONCEPT_KEYS_V13 = [...EXPECTED_CONCEPT_KEYS_V12, 'COV-PROXY', 'COV-MEETING'].sort();
const EXPECTED_CONCEPT_KEYS_V14 = [
  ...EXPECTED_CONCEPT_KEYS_V13,
  ...GENERAL_COVENANT_CODES_V1,
].sort();
const EXPECTED_CONCEPT_KEYS_V15 = [
  ...EXPECTED_CONCEPT_KEYS_V14,
  'COV-PROXY-PARENT-APPROVAL',
  'COV-PROXY-MERGERSUB-APPROVAL',
].sort();
const EXPECTED_CONCEPT_KEYS_V16 = [
  ...EXPECTED_CONCEPT_KEYS_V15,
  'COV-FINANCING', 'COV-MARKETING', 'COV-PAYOFF', 'GTY-DELIVERY', 'GTY-PERF',
].sort();
const EXPECTED_CONCEPT_KEYS_V17 = [...EXPECTED_CONCEPT_KEYS_V16,
  ...KEY_DEFINED_TERM_CONCEPT_KEYS_V31,
  'TAXM-TREATMENT', 'TAXM-TRANSFER', 'TAXM-FIRPTA', 'DIVD-COORD', 'DIVD-SPECIAL',
  'APPR-SETTLE', 'APPR-WITHDRAW',
].sort();
const EXPECTED_CONCEPT_KEYS_V18 = [...EXPECTED_CONCEPT_KEYS_V17,
  'COV-EMPLOYEE', 'DNO-BENEF', 'DNO-INDEM', 'DNO-TAIL',
].sort();
const EXPECTED_CONCEPT_KEYS_V19 = [...EXPECTED_CONCEPT_KEYS_V18, 'REP-T-QUALIFIER', 'REP-B-QUALIFIER', 'MERGER-STRUCTURE', 'REMEDY-SPECIFIC-PERFORMANCE', 'MISC-BOILERPLATE'].sort();
const EXPECTED_CONCEPT_KEYS_V20 = [...EXPECTED_CONCEPT_KEYS_V19, ...NO_OTHER_REPS_FRAUD_CONCEPT_KEYS_V34].sort();
const EXPECTED_CONCEPT_KEYS_V21 = [...EXPECTED_CONCEPT_KEYS_V20, ...RECORDED_DEFINED_TERM_CONCEPT_KEYS_V35].sort();
const EXPECTED_CONCEPT_KEYS_V22 = [
  ...EXPECTED_CONCEPT_KEYS_V21.filter((conceptKey) => !RETIRED_ANTITRUST_CONCEPT_KEYS_V36.includes(conceptKey)),
  'ANTI-COOPERATE', 'ANTI-INFO', 'ANTI-NOTIFY',
].sort();
const EXPECTED_CONCEPT_KEYS_V23 = [...EXPECTED_CONCEPT_KEYS_V22, 'TAXM-OPINION'].sort();
const EXPECTED_CONCEPT_KEYS_V24 = [...EXPECTED_CONCEPT_KEYS_V23, 'REM-SOLE'].sort();
const EXPECTED_CONCEPT_KEYS_V25 = [...EXPECTED_CONCEPT_KEYS_V24, 'REP-T-TOPIC', 'REP-B-TOPIC'].sort();
...
const EXPECTED_CLAIM_KEYS_V18 = [
  ...EXPECTED_CLAIM_KEYS_V17,
  'DISCLOSURE_SCHEDULE_CARVEOUT',
  'PERFORMANCE_VESTING_ASSUMPTION',
].sort();
const EXPECTED_CLAIM_KEYS_V19 = [...EXPECTED_CLAIM_KEYS_V18, 'REGULATORY_EFFORTS_STANDARD', 'REGULATORY_BURDEN_COMMITMENT', 'REGULATORY_DIVESTITURE_CAP_AMOUNT', 'REGULATORY_LITIGATION_OBLIGATION', 'REGULATORY_TIMING_RESTRICTION', 'HSR_FILING_DEADLINE_DAYS'].sort();
const EXPECTED_CLAIM_KEYS_V20 = [...EXPECTED_CLAIM_KEYS_V19, 'REGULATORY_TIMING_AGREEMENT_RESTRICTION', 'REGULATORY_WITHDRAWAL_REFILING_RESTRICTION', 'REGULATORY_STRATEGY_CONTROL', 'REGULATORY_CONSULTATION_RIGHT', 'REGULATORY_FILING_OBLIGATION', 'REGULATORY_FILING_DEADLINE_DAYS', 'REGULATORY_NON_IMPEDIMENT_COVENANT'].sort();
const EXPECTED_CLAIM_KEYS_V21 = [...EXPECTED_CLAIM_KEYS_V20, ...NO_SHOP_WAVE_B_CLAIM_DEFINITIONS_V1.map((definition) => definition.claim_definition_key)].sort();
const EXPECTED_CLAIM_KEYS_V22 = [
  ...EXPECTED_CLAIM_KEYS_V21,
  'PER_SHARE_CASH_CONSIDERATION',
  'EXCHANGE_RATIO_VALUE',
  'APPRAISAL_RIGHTS_STATUS',
].sort();
const EXPECTED_CLAIM_KEYS_V23 = [...EXPECTED_CLAIM_KEYS_V22, 'NO_MAE_CONDITION', 'NO_MAE_CONDITION_CONTINUING', 'COVENANT_COMPLIANCE_STANDARD', 'REGULATORY_APPROVAL_CONDITION'].sort();
const EXPECTED_CLAIM_KEYS_V24 = [...EXPECTED_CLAIM_KEYS_V23,
  ...CLOSING_CONDITION_PRESENCE_CLAIM_KEYS_V24, 'S4_CONDITION_COMPONENT',
  'FRUSTRATION_CAUSATION_STANDARD', 'FRUSTRATION_BREACH_STANDARD',
  'CONDITION_DOLLAR_THRESHOLD'].sort();
const EXPECTED_CLAIM_KEYS_V25 = [...EXPECTED_CLAIM_KEYS_V24, 'IOC_RESTRICTION_PRESENT'].sort();
const EXPECTED_CLAIM_KEYS_V26 = [
  ...EXPECTED_CLAIM_KEYS_V25,
  ...PROXY_MEETING_CLAIM_DEFINITIONS_V26.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V27 = [
  ...EXPECTED_CLAIM_KEYS_V26,
  'MATERIAL_CONTRACT_BUCKET_PRESENT',
  'MATERIAL_CONTRACT_THRESHOLD_STRUCTURE',
].sort();
const EXPECTED_CLAIM_KEYS_V28 = [
  ...EXPECTED_CLAIM_KEYS_V27,
  'GENERAL_COVENANT_PRESENT',
].sort();
const EXPECTED_CLAIM_KEYS_V29 = [
  ...EXPECTED_CLAIM_KEYS_V28,
  ...PROXY_MEETING_FOLLOW_ON_CLAIM_DEFINITIONS_V29.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V30 = [
  ...EXPECTED_CLAIM_KEYS_V29,
  ...FINANCING_GUARANTY_CLAIM_DEFINITIONS_V30.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V31 = [
  ...EXPECTED_CLAIM_KEYS_V30,
  ...KEY_DEFINED_TERM_CLAIM_DEFINITIONS_V31.map((definition) => definition.claim_definition_key),
  ...TAX_DIVIDENDS_APPRAISAL_CLAIM_DEFINITIONS_V31.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V32 = [...EXPECTED_CLAIM_KEYS_V31,
  ...EMPLOYEE_DNO_CLAIM_DEFINITIONS_V32.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V33 = [...EXPECTED_CLAIM_KEYS_V32, ...M3_REPRESENTATIONS_MERGER_REMEDIES_MISC_CLAIM_DEFINITIONS_V33.map((definition) => definition.claim_definition_key)].sort();
const EXPECTED_CLAIM_KEYS_V34 = [...EXPECTED_CLAIM_KEYS_V33, ...NO_OTHER_REPS_FRAUD_CLAIM_DEFINITIONS_V34.map((definition) => definition.claim_definition_key)].sort();
const EXPECTED_CLAIM_KEYS_V35 = [...EXPECTED_CLAIM_KEYS_V34, ...RECORDED_DEFINED_TERM_CLAIM_DEFINITIONS_V35.map((definition) => definition.claim_definition_key)].sort();
const EXPECTED_CLAIM_KEYS_V36 = [
  ...EXPECTED_CLAIM_KEYS_V35.filter((claimKey) => !RETIRED_ANTITRUST_CLAIM_KEYS_V36.includes(claimKey)),
  ...EXPANDED_ANTITRUST_CLAIM_DEFINITIONS_V36.map((definition) => definition.claim_definition_key),
].sort();
const EXPECTED_CLAIM_KEYS_V37 = [...EXPECTED_CLAIM_KEYS_V36, 'TAX_OPINION_COOPERATION_COVENANT'].sort();
const EXPECTED_CLAIM_KEYS_V38 = [...EXPECTED_CLAIM_KEYS_V37, ...SOLE_REMEDY_CLAIM_DEFINITIONS_V38.map((definition) => definition.claim_definition_key)].sort();
const EXPECTED_CLAIM_KEYS_V39 = [...EXPECTED_CLAIM_KEYS_V38, 'MERGER_TRANSACTION_STEP'].sort();
const EXPECTED_CLAIM_KEYS_V40 = [...EXPECTED_CLAIM_KEYS_V39, 'REPRESENTATION_TOPIC_PRESENT'].sort();
...
// Step 3J: V39 adds TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3 alongside V2.
const EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V5 = [
  ...EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
  'TERMINATION_FEE_TRIGGER_PATH/V3',
].sort();
// Step 3J2: V40 adds TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V4 alongside V2
// and V3.
const EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6 = [
  ...EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V5,
  'TERMINATION_FEE_TRIGGER_PATH/V4',
].sort();
...
const KNOWN_VERSION_SHAPES = Object.freeze([
  ...
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V5,
    claims: EXPECTED_CLAIM_KEYS_V18,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V18.claim_definitions,
    relationship_definitions:
      FIXTURE_CONTRACT_INPUT_V18.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V18.result_definitions,
    market_metric_definitions:
      FIXTURE_CONTRACT_INPUT_V18.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V6,
    claims: EXPECTED_CLAIM_KEYS_V19,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V19.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V19.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V19.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V19.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V7,
    claims: EXPECTED_CLAIM_KEYS_V20,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V20.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V20.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V20.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V20.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V8,
    claims: EXPECTED_CLAIM_KEYS_V21,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V21.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V21.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V21.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V21.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V9,
    claims: EXPECTED_CLAIM_KEYS_V22,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V22.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V22.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V22.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V22.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V10,
    claims: EXPECTED_CLAIM_KEYS_V23,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V23.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V23.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V23.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V23.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V11,
    claims: EXPECTED_CLAIM_KEYS_V24,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V24.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V24.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V24.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V24.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V12,
    claims: EXPECTED_CLAIM_KEYS_V25,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V25.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V25.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V25.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V25.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V13,
    claims: EXPECTED_CLAIM_KEYS_V26,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V26.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V26.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V26.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V26.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V13,
    claims: EXPECTED_CLAIM_KEYS_V27,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V27.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V27.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V27.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V27.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V14,
    claims: EXPECTED_CLAIM_KEYS_V28,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V28.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V28.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V28.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V28.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V15,
    claims: EXPECTED_CLAIM_KEYS_V29,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V29.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V29.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V29.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V29.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V16,
    claims: EXPECTED_CLAIM_KEYS_V30,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V30.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V30.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V30.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V30.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V17,
    claims: EXPECTED_CLAIM_KEYS_V31,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V31.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V31.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V31.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V31.market_metric_definitions,
  }),
  Object.freeze({
    concepts: EXPECTED_CONCEPT_KEYS_V18,
    claims: EXPECTED_CLAIM_KEYS_V32,
    claim_definitions: FIXTURE_CONTRACT_INPUT_V32.claim_definitions,
    relationship_definitions: FIXTURE_CONTRACT_INPUT_V32.relationship_definitions,
    exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3,
    metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4,
    trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4,
    money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1,
    ...
    brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1,
    capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1,
    result_definitions: FIXTURE_CONTRACT_INPUT_V32.result_definitions,
    market_metric_definitions: FIXTURE_CONTRACT_INPUT_V32.market_metric_definitions,
  }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V19, claims: EXPECTED_CLAIM_KEYS_V33, claim_definitions: FIXTURE_CONTRACT_INPUT_V33.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V33.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V33.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V33.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V20, claims: EXPECTED_CLAIM_KEYS_V34, claim_definitions: FIXTURE_CONTRACT_INPUT_V34.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V34.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V34.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V34.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V21, claims: EXPECTED_CLAIM_KEYS_V35, claim_definitions: FIXTURE_CONTRACT_INPUT_V35.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V35.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V35.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V35.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V22, claims: EXPECTED_CLAIM_KEYS_V36, claim_definitions: FIXTURE_CONTRACT_INPUT_V36.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V36.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V36.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V36.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V23, claims: EXPECTED_CLAIM_KEYS_V37, claim_definitions: FIXTURE_CONTRACT_INPUT_V37.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V37.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V37.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V37.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V24, claims: EXPECTED_CLAIM_KEYS_V38, claim_definitions: FIXTURE_CONTRACT_INPUT_V38.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V4, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
  // Step 3J / V39: dual-numbering case -- concepts and claims stay at their
  // V24/V38 shapes (zero additions), only trigger_path_schemas grows.
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V24, claims: EXPECTED_CLAIM_KEYS_V38, claim_definitions: FIXTURE_CONTRACT_INPUT_V38.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V5, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
  // Step 3J2 / V40: same dual-numbering case again -- concepts and claims
  // stay at V24/V38, only trigger_path_schemas grows again (V2, V3, V4).
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V24, claims: EXPECTED_CLAIM_KEYS_V38, claim_definitions: FIXTURE_CONTRACT_INPUT_V38.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),

  // Stage 3 / V41: concepts stay at V24, claim KEY set stays at V38 (no new
  // claim definition -- the set of keys is unchanged); the one difference
  // from the V40 shape is REPRESENTATION_ACCURACY_STANDARD's widened
  // allowed_canonical_values (see FIXTURE_CONTRACT_INPUT_V41's own comment
  // for the Ben ruling behind it and why the definition version stays 1).
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V24, claims: EXPECTED_CLAIM_KEYS_V38, claim_definitions: FIXTURE_CONTRACT_INPUT_V41.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
  // Step 2X-I / V42: one new claim definition for model-extracted transaction
  // steps. Concepts stay at V24; only the claim key set grows.
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V24, claims: EXPECTED_CLAIM_KEYS_V39, claim_definitions: FIXTURE_CONTRACT_INPUT_V42.claim_definitions, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V25, claims: EXPECTED_CLAIM_KEYS_V40, claim_definitions: FIXTURE_CONTRACT_INPUT_V43.claim_definitions, representation_topic_registry_binding: REPRESENTATION_TOPIC_REGISTRY_BINDING_V1, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
  Object.freeze({ concepts: EXPECTED_CONCEPT_KEYS_V25, claims: EXPECTED_CLAIM_KEYS_V40, claim_definitions: FIXTURE_CONTRACT_INPUT_V44.claim_definitions, representation_topic_registry_binding: REPRESENTATION_TOPIC_REGISTRY_BINDING_V1, relationship_definitions: FIXTURE_CONTRACT_INPUT_V38.relationship_definitions, exact_detail_actions: EXPECTED_EXACT_DETAIL_ACTION_KEYS_V3, metric_operation_bindings: EXPECTED_METRIC_OPERATION_BINDING_KEYS_V4, trigger_path_schemas: EXPECTED_TRIGGER_PATH_SCHEMA_KEYS_V6, money_denominator_precision_policy: MONEY_DENOMINATOR_PRECISION_POLICY_V1, no_shop_semantic_schemas: NO_SHOP_SEMANTIC_SCHEMAS_V6, claim_interpretation_policy: CLAIM_INTERPRETATION_POLICY_V2, definition_use_effect_schema: USES_DEFINITION_EFFECT_SCHEMA_V3, brings_down_effect_schema: BRINGS_DOWN_EFFECT_SCHEMA_V1, capitalisation_semantic_schema: CAPITALISATION_REPRESENTATION_SCHEMA_V1, result_definitions: FIXTURE_CONTRACT_INPUT_V38.result_definitions, market_metric_definitions: FIXTURE_CONTRACT_INPUT_V38.market_metric_definitions }),
]);
...
function validateInput(input) {
  ...
  if (input.representation_topic_registry_binding != null) {
    assertExact(
      input.representation_topic_registry_binding,
  ...
  const versionShape = KNOWN_VERSION_SHAPES.find((shape) => (
    ...
    && canonicalJson(input.representation_topic_registry_binding || null)
      === canonicalJson(shape.representation_topic_registry_binding || null)
  ));
  if (!versionShape) {
  ...
  if (input.serving_trigger_path_schemas != null) {
    // Step 3J / 3J2: V4 (fixture-input numbering) through V38 carry exactly
    // [V2] (inherited unchanged, never re-declared); V39 adds V3 alongside
    // it; V40 adds V4 (trigger-path-schema numbering) alongside both. Branch
    // on the input's own length rather than on a version number so this
    // still refuses anything that is not one of those three exact, frozen
    // shapes.
    const expectedTriggerPathSchemas = input.serving_trigger_path_schemas.length > 2
      ? [TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V2, TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3, TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V4]
      : input.serving_trigger_path_schemas.length > 1
        ? [TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V2, TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V3]
        : [TERMINATION_FEE_TRIGGER_PATH_SCHEMA_V2];
    assertExact(input.serving_trigger_path_schemas, expectedTriggerPathSchemas, 'serving trigger-path schemas');
  }
...
function compileFixtureContract(input = FIXTURE_CONTRACT_INPUT_V1) {
  ...
  if (input.representation_topic_registry_binding != null) {
    payload.representation_topic_registry_binding =
      input.representation_topic_registry_binding;
  }
...
function validateContractBundle(bundle) {
  ...
  if (payload.representation_topic_registry_binding != null) {
    reconstructedInput.representation_topic_registry_binding =
      payload.representation_topic_registry_binding;
  }
  ...
  if (expected.representation_topic_registry_binding != null) {
    expectedPayload.representation_topic_registry_binding =
      expected.representation_topic_registry_binding;
  }
...
function compileFixtureContractV18() {
  return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V18);
}

function compileFixtureContractV19() {
  return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V19);
}

function compileFixtureContractV20() {
  return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V20);
}
function compileFixtureContractV21() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V21); }
function compileFixtureContractV22() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V22); }
function compileFixtureContractV23() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V23); }
function compileFixtureContractV24() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V24); }
function compileFixtureContractV25() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V25); }
function compileFixtureContractV26() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V26); }
function compileFixtureContractV27() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V27); }
function compileFixtureContractV28() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V28); }
function compileFixtureContractV29() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V29); }
function compileFixtureContractV30() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V30); }
function compileFixtureContractV31() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V31); }
function compileFixtureContractV32() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V32); }
function compileFixtureContractV33() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V33); }
function compileFixtureContractV34() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V34); }
function compileFixtureContractV35() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V35); }
function compileFixtureContractV36() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V36); }
function compileFixtureContractV37() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V37); }
function compileFixtureContractV38() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V38); }
function compileFixtureContractV39() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V39); }
function compileFixtureContractV40() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V40); }
function compileFixtureContractV41() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V41); }
function compileFixtureContractV42() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V42); }
function compileFixtureContractV43() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V43); }
function compileFixtureContractV44() { return compileFixtureContract(FIXTURE_CONTRACT_INPUT_V44); }
...
const FIXTURE_CONTRACT_FINGERPRINT_V18 =
  compileFixtureContract(FIXTURE_CONTRACT_INPUT_V18).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V19 =
  compileFixtureContract(FIXTURE_CONTRACT_INPUT_V19).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V20 =
  compileFixtureContract(FIXTURE_CONTRACT_INPUT_V20).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V21 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V21).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V22 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V22).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V23 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V23).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V24 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V24).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V25 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V25).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V26 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V26).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V27 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V27).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V28 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V28).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V29 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V29).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V30 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V30).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V31 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V31).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V32 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V32).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V33 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V33).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V34 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V34).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V35 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V35).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V36 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V36).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V37 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V37).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINT_V38 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V38).fingerprint;
// Step 3J: deliberately NOT added to FIXTURE_CONTRACT_FINGERPRINTS /
// FIXTURE_CONTRACTS_BY_FINGERPRINT below. Those two structures are what
// shared-serving-row.js and market-cohort-query.js (neither owned by this
// step) check membership against to decide "is this a fingerprint real
// serving code should trust" -- registering V39 there is a production-
// wiring decision (whether Canonical V2 should actually serve the new
// schema version to a reviewer) this step does not make; nothing yet
// produces a V39-fingerprinted bundle outside this file's own tests. V39
// still compiles, still validates (`validateContractBundle`), and is fully
// exercised by tests/canonical-v2-contract-bundle-v39.test.js -- it is
// simply not yet in the "currently served" set.
const FIXTURE_CONTRACT_FINGERPRINT_V39 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V39).fingerprint;
// Step 3J2: same reasoning as V39 above, one version further -- deliberately
// NOT added to FIXTURE_CONTRACT_FINGERPRINTS / FIXTURE_CONTRACTS_BY_
// FINGERPRINT. Nothing produces a V4-trigger-path-schema-shaped bundle
// outside this file's own tests; see docs/core/GRAVEYARD.md entry 16 (V3's
// unreachability, which this schema inherits unchanged) for the concrete
// remaining steps and why registering ahead of a producer was already tried
// once for V39 and reverted.
const FIXTURE_CONTRACT_FINGERPRINT_V40 = compileFixtureContract(FIXTURE_CONTRACT_INPUT_V40).fingerprint;
const FIXTURE_CONTRACT_FINGERPRINTS = Object.freeze([
  ...
  FIXTURE_CONTRACT_FINGERPRINT_V18,
  FIXTURE_CONTRACT_FINGERPRINT_V19,
  FIXTURE_CONTRACT_FINGERPRINT_V20,
  FIXTURE_CONTRACT_FINGERPRINT_V21,
  FIXTURE_CONTRACT_FINGERPRINT_V22,
  FIXTURE_CONTRACT_FINGERPRINT_V23,
  FIXTURE_CONTRACT_FINGERPRINT_V24,
  FIXTURE_CONTRACT_FINGERPRINT_V25,
  FIXTURE_CONTRACT_FINGERPRINT_V26,
  FIXTURE_CONTRACT_FINGERPRINT_V27,
  FIXTURE_CONTRACT_FINGERPRINT_V28,
  FIXTURE_CONTRACT_FINGERPRINT_V29,
  FIXTURE_CONTRACT_FINGERPRINT_V30,
  FIXTURE_CONTRACT_FINGERPRINT_V31,
  FIXTURE_CONTRACT_FINGERPRINT_V32,
  FIXTURE_CONTRACT_FINGERPRINT_V33,
  FIXTURE_CONTRACT_FINGERPRINT_V34,
  FIXTURE_CONTRACT_FINGERPRINT_V35,
  FIXTURE_CONTRACT_FINGERPRINT_V36,
  FIXTURE_CONTRACT_FINGERPRINT_V37,
  FIXTURE_CONTRACT_FINGERPRINT_V38,
]);
...
const FIXTURE_CONTRACTS_BY_FINGERPRINT = new Map([
  ...
  [
    FIXTURE_CONTRACT_FINGERPRINT_V18,
    compileFixtureContract(FIXTURE_CONTRACT_INPUT_V18),
  ],
  [
    FIXTURE_CONTRACT_FINGERPRINT_V19,
    compileFixtureContract(FIXTURE_CONTRACT_INPUT_V19),
  ],
  [
    FIXTURE_CONTRACT_FINGERPRINT_V20,
    compileFixtureContract(FIXTURE_CONTRACT_INPUT_V20),
  ],
  [FIXTURE_CONTRACT_FINGERPRINT_V21, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V21)],
  [FIXTURE_CONTRACT_FINGERPRINT_V22, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V22)],
  [FIXTURE_CONTRACT_FINGERPRINT_V23, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V23)],
  [FIXTURE_CONTRACT_FINGERPRINT_V24, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V24)],
  [FIXTURE_CONTRACT_FINGERPRINT_V25, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V25)],
  [FIXTURE_CONTRACT_FINGERPRINT_V26, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V26)],
  [FIXTURE_CONTRACT_FINGERPRINT_V27, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V27)],
  [FIXTURE_CONTRACT_FINGERPRINT_V28, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V28)],
  [FIXTURE_CONTRACT_FINGERPRINT_V29, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V29)],
  [FIXTURE_CONTRACT_FINGERPRINT_V30, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V30)],
  [FIXTURE_CONTRACT_FINGERPRINT_V31, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V31)],
  [FIXTURE_CONTRACT_FINGERPRINT_V32, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V32)],
  [FIXTURE_CONTRACT_FINGERPRINT_V33, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V33)],
  [FIXTURE_CONTRACT_FINGERPRINT_V34, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V34)],
  [FIXTURE_CONTRACT_FINGERPRINT_V35, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V35)],
  [FIXTURE_CONTRACT_FINGERPRINT_V36, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V36)],
  [FIXTURE_CONTRACT_FINGERPRINT_V37, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V37)],
  [FIXTURE_CONTRACT_FINGERPRINT_V38, compileFixtureContract(FIXTURE_CONTRACT_INPUT_V38)],
  // Step 3J: V39 deliberately not registered here either -- see the comment
  // by FIXTURE_CONTRACT_FINGERPRINT_V39's declaration above.
]);
...
module.exports = {
  ...
  FIXTURE_CONTRACT_FINGERPRINT_V37,
  FIXTURE_CONTRACT_FINGERPRINT_V38,
  FIXTURE_CONTRACT_FINGERPRINT_V39,
  FIXTURE_CONTRACT_FINGERPRINT_V40,
  ...
  REGULATORY_FILING_TIMING_STANDARD_CLAIM_DEFINITION_V1,
  EXPANDED_ANTITRUST_CLAIM_DEFINITIONS_V36,
  RETIRED_ANTITRUST_CONCEPT_KEYS_V36,
  RETIRED_ANTITRUST_CLAIM_KEYS_V36,
  IOC_RESTRICTION_PRESENT_CLAIM_DEFINITION_V1,
  IOC_RESTRICTION_CONCEPT_KEYS_V25,
  PROXY_FILING_DEADLINE_DAYS_CLAIM_DEFINITION_V1,
  MEETING_DEADLINE_DAYS_CLAIM_DEFINITION_V1,
  ...
  CLOSING_CONDITION_PRESENCE_CLAIM_KEYS_V24,
  CLOSING_CONDITION_PRESENCE_CLAIM_DEFINITIONS_V24,
  S4_CONDITION_COMPONENT_CLAIM_DEFINITION_V1,
  FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V1,
  FRUSTRATION_CAUSATION_STANDARD_CLAIM_DEFINITION_V44,
  ...
  FINANCING_GUARANTY_CLAIM_DEFINITIONS_V30,
  KEY_DEFINED_TERM_CONCEPT_KEYS_V31,
  KEY_DEFINED_TERM_CLAIM_DEFINITIONS_V31,
  TAX_DIVIDENDS_APPRAISAL_CLAIM_DEFINITIONS_V31,
  TAX_OPINION_COOPERATION_CLAIM_DEFINITION_V37,
  EMPLOYEE_DNO_CLAIM_DEFINITIONS_V32,
  M3_REPRESENTATIONS_MERGER_REMEDIES_MISC_CLAIM_DEFINITIONS_V33,
  MERGER_TRANSACTION_STEP_CLAIM_DEFINITION_V42,
  REPRESENTATION_TOPIC_CODES_V1,
  REPRESENTATION_TOPIC_REGISTRY_BINDING_V1,
  REPRESENTATION_TOPIC_PRESENT_CLAIM_DEFINITION_V1,
  NO_OTHER_REPS_FRAUD_CONCEPT_KEYS_V34,
  NO_OTHER_REPS_FRAUD_CLAIM_DEFINITIONS_V34,
  BRINGS_DOWN_EFFECT_SCHEMA_V1,
  CAPITALISATION_REPRESENTATION_SCHEMA_V1,
  TARGET_CAPITALISATION_BRING_DOWN_RESULT_DEFINITION_V2,
  ...
  PAYMENT_DELAY_STRUCTURE_MIGRATION_V1,
  USES_DEFINITION_EFFECT_SCHEMA_V1,
  USES_DEFINITION_EFFECT_SCHEMA_V2,
  USES_DEFINITION_EFFECT_SCHEMA_V3,
