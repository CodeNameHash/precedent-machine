...
// by exactly two members of lib/taxonomy.js's own MATERIALITY_CODES
// vocabulary (consumed, not invented): MAT_MATERIAL_INLINE ("in any
// material respect" -- materiality inline within the rep) and
// MAT_MAE_AGGREGATE (the "individually or in the aggregate" MAE-qualifier
// form, kept distinct from the bare MAT_MAE_QUALIFIED under the same
// never-alias ruling). WIDENING IN PLACE, claim_definition version STAYS 1,
// deliberately: the four pre-existing values keep their exact meaning, so
// every already-resolved accuracy claim's identity (which folds
// claim_definition_version) must not re-mint; a version bump here would
// re-key every committed accuracy claim for a change that does not touch
// them. The entry is REPLACED rather than appended because
// claim_definition_key is a unique key in the compiled vocabulary -- this
// is the one deliberate exception to the plain append pattern, stated here
// rather than hidden in the map call.
