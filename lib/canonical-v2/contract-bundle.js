...
const KNOWN_CONCEPT_KEY_VERSIONS = [EXPECTED_CONCEPT_KEYS_V1, EXPECTED_CONCEPT_KEYS_V2];
...
const EXPECTED_EXACT_DETAIL_ACTION_KEYS = [
...
function validateInput(input) {
  ...
  const conceptsMatchKnownVersion = KNOWN_CONCEPT_KEY_VERSIONS
    .some((expected) => canonicalJson(concepts) === canonicalJson(expected));
  if (!conceptsMatchKnownVersion) {
    throw new Error('concept keys do not match any frozen fixture contract version (V1 or V2)');
  }
