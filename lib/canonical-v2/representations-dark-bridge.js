...
const {
  ...
  KNOWLEDGE_STANDARDS,
  GOVERNED_CLAIMS,
  isGovernedCanonicalValue,
} = require('./representations-product-projection');
const { canonicalJson, contentId } = require('./canonical-bytes');
const { REPRESENTATION_TOPIC_REGISTRY_BINDING_V1 } = require('./contract-bundle');
...
const GENERIC_QUALIFIER_CONCEPTS = Object.freeze(new Set(['REP-T-QUALIFIER', 'REP-B-QUALIFIER']));
const ATTRIBUTE_BY_CLAIM_DEFINITION = Object.freeze({
  ...
  GENERAL_MATERIALITY_QUALIFIER: 'generalMaterialityQualifier',
  REPRESENTATION_MEASUREMENT_DATE: 'representationMeasurementDate',
  RETROSPECTIVE_LOOKBACK: 'retrospectiveLookback',
  [TOPIC_CLAIM_KEY]: 'representationTopic',
});
...
const QUALIFIER_DISPATCH_DIMENSION_FIELDS = Object.freeze([
  ...DIMENSIONS_BASE_FIELDS,
  'representation_party', 'attachment', 'qualifier_dispatch',
]);
const TOPIC_DIMENSION_FIELDS = Object.freeze([
  'representation_side', 'representation_maker', 'representation_maker_capacity',
  'topic_registry_version', 'topic_registry_digest',
]);
...
const EXPECTED_MARKET_SHAPE = Object.freeze({
  ...
  GENERAL_MATERIALITY_QUALIFIER: Object.freeze({
    metric_key: 'GENERAL_MATERIALITY_QUALIFIER_PRESENCE',
    value_dimension: 'BOOLEAN_PRESENCE',
    canonical_unit: 'PRESENT_TRUE',
    per_deal_rollup: 'ANY_TRUE',
    weighting: 'DEAL',
  }),
  REPRESENTATION_MEASUREMENT_DATE: Object.freeze({
    metric_key: 'REPRESENTATION_MEASUREMENT_DATE',
    value_dimension: 'DATE',
    canonical_unit: 'ISO_8601_DATE_STRING',
    per_deal_rollup: 'DISTINCT_VALUE_SET',
    weighting: 'DEAL',
  }),
  RETROSPECTIVE_LOOKBACK: Object.freeze({
    metric_key: 'RETROSPECTIVE_LOOKBACK_PRESENCE',
    value_dimension: 'BOOLEAN_PRESENCE',
    canonical_unit: 'PRESENT_TRUE',
    per_deal_rollup: 'ANY_TRUE',
    weighting: 'DEAL',
...
function validateGovernedRecord(record, index) {
  ...
  if (!definition) fail('UNGOVERNED_CLAIM', 'Only governed representation claims can be bridged.', { index, claim_definition_key: claimDefinitionKey });
  if ((claimDefinitionKey === TOPIC_CLAIM_KEY) !== TOPIC_CONCEPTS.has(conceptKey)) {
    fail('TOPIC_CONCEPT_MISMATCH', 'The representation topic claim and topic concept must appear together.', { index });
  }
  ...
  if (!isGovernedCanonicalValue(claimDefinitionKey, productValue.canonical_value)) fail('INVALID_PROJECTION', 'The record canonical value is not governed.', { index });
  ...
  if (claimDefinitionKey === TOPIC_CLAIM_KEY) {
    assertExactFields(dimensions, TOPIC_DIMENSION_FIELDS, 'record dimensions');
  } else if (claimDefinitionKey === 'REPRESENTATION_ACCURACY_STANDARD') {
    assertExactFields(dimensions, DIMENSIONS_BASE_FIELDS, 'record dimensions');
  } else if (claimDefinitionKey === 'KNOWLEDGE_QUALIFIER') {
    assertFieldSet(dimensions, DIMENSIONS_BASE_FIELDS, ['knowledge_standard'], 'record dimensions');
  } else if (claimDefinitionKey === 'RETROSPECTIVE_LOOKBACK') {
    assertFieldSet(dimensions, QUALIFIER_DISPATCH_DIMENSION_FIELDS, ['temporal_form'], 'record dimensions');
  } else {
    assertExactFields(dimensions, QUALIFIER_DISPATCH_DIMENSION_FIELDS, 'record dimensions');
  }
  ...
  if (claimDefinitionKey === TOPIC_CLAIM_KEY) {
    if (dimensions.topic_registry_version !== REPRESENTATION_TOPIC_REGISTRY_BINDING_V1.registry_version
      || dimensions.topic_registry_digest !== REPRESENTATION_TOPIC_REGISTRY_BINDING_V1.registry_digest) {
      fail('TOPIC_REGISTRY_BINDING_MISMATCH', 'The topic record does not carry the governed registry identity.', { index });
    }
  } else {
  ...
  if (['GENERAL_MATERIALITY_QUALIFIER', 'REPRESENTATION_MEASUREMENT_DATE', 'RETROSPECTIVE_LOOKBACK'].includes(claimDefinitionKey)) {
    const expectedDispatch = {
      GENERAL_MATERIALITY_QUALIFIER: 'THRESHOLD_MATERIALITY',
      REPRESENTATION_MEASUREMENT_DATE: 'TEMPORAL_MEASUREMENT_DATE',
      RETROSPECTIVE_LOOKBACK: 'TEMPORAL_RETROSPECTIVE_LOOKBACK',
    }[claimDefinitionKey];
    if (!dimensions.representation_party
      || dimensions.representation_party.role !== 'REPRESENTATION_MAKER'
      || dimensions.representation_party.value !== dimensions.representation_maker
      || dimensions.representation_party.capacity !== dimensions.representation_maker_capacity) {
      fail('ALTERED_CLAIM_CONTENT', 'The retained representation party does not match the claim dimensions.', { index });
    }
    if (!dimensions.attachment || canonicalJson(dimensions.attachment.position) !== canonicalJson(dimensions.qualifier_position)) {
      fail('ALTERED_CLAIM_CONTENT', 'The retained qualifier attachment does not match its position.', { index });
    }
    if (dimensions.qualifier_dispatch !== expectedDispatch) fail('ALTERED_CLAIM_CONTENT', 'The qualifier dispatch does not match its claim definition.', { index });
    if (claimDefinitionKey === 'RETROSPECTIVE_LOOKBACK' && dimensions.temporal_form !== 'LOOKBACK') {
      fail('ALTERED_CLAIM_CONTENT', 'A retrospective lookback must retain its temporal form.', { index });
    }
  }
...
function adaptProvisionGroupToLegacyCard(group, groupIndex, legacyIndex, identities) {
  ...
  const representationSide = first.representation_side;
  const topicConceptKey = representationSide === 'TARGET' ? 'REP-T-TOPIC' : 'REP-B-TOPIC';
  const concreteConceptKeys = [...new Set(group
    .map((record) => record.query.value.concept_key)
    .filter((conceptKey) => conceptKey !== topicConceptKey && !GENERIC_QUALIFIER_CONCEPTS.has(conceptKey)))];
  if (concreteConceptKeys.length > 1) {
    fail('PROVISION_GROUP_MISMATCH', 'Records grouped under one provision disagree on their concrete representation topic.', { groupIndex });
  }
  const joinedConceptKey = String(legacyCard.provision_subtype || '').trim().toUpperCase();
  const conceptKey = concreteConceptKeys[0] || joinedConceptKey;
  if (!conceptKey.startsWith(EXPECTED_SIDE_PREFIX[representationSide]) || TOPIC_CONCEPTS.has(conceptKey)) {
    fail('TOPIC_ROUTE_NOT_FOUND', 'The exact legacy provision join does not supply an existing representation topic route.', {
      groupIndex, provision_instance_id: provisionInstanceId,
    });
  }
  if (EXCLUDED_CONCEPTS.has(conceptKey)) {
    fail('EXCLUDED_FAMILY_OWNERSHIP', 'The exact legacy provision join points to a representation topic owned by another family.', {
      groupIndex, concept_key: conceptKey,
    });
  }
  ...
  for (const record of group) {
    ...
    if (record.representation_side !== representationSide) {
    ...
    if (record.query.value.concept_key !== conceptKey
      && record.query.value.concept_key !== topicConceptKey
      && !GENERIC_QUALIFIER_CONCEPTS.has(record.query.value.concept_key)) {
  ...
  for (const record of group) {
    ...
    let canonicalFeatureValue;
    if (attribute === 'knowledgeQualifier') {
      canonicalFeatureValue = true;
    } else if (['generalMaterialityQualifier', 'representationMeasurementDate', 'retrospectiveLookback'].includes(attribute)) {
      canonicalFeatureValue = deepFreeze({
        value: productValue.canonical_value,
        text: record.evidence.quote,
        party: productValue.dimensions.representation_party,
        attachment: productValue.dimensions.attachment,
        qualifier_dispatch: productValue.dimensions.qualifier_dispatch,
        ...(attribute === 'retrospectiveLookback' ? { temporal_form: productValue.dimensions.temporal_form } : {}),
      });
    } else {
      canonicalFeatureValue = Object.freeze({ code: productValue.canonical_value, text: record.evidence.quote });
    }
    features[attribute] = canonicalFeatureValue;
    featureClaimRevisionIds[attribute] = Object.freeze([record.claim_revision_id]);
  ...
  const card = deepFreeze({
    ...
    canonical_v2_lineage: {
      source: NATIVE_SOURCE,
      claim_revision_ids: sortedClaimRevisionIds,
      feature_claim_revision_ids: deepFreeze({ ...featureClaimRevisionIds }),
    },
...
function assertCardShape(card, index) {
  ...
  assertExactFields(lineage, isNative
    ? ['source', 'claim_revision_ids', 'feature_claim_revision_ids']
    : ['source', 'reason', 'claim_revision_ids'], 'card lineage');
  ...
  if (isNative) {
    ...
    const featureClaimRevisionIds = requireObject(
      lineage.feature_claim_revision_ids,
      'INVALID_LINEAGE',
      'A native governed card needs feature-level claim lineage.',
    );
    ...
    if (canonicalJson(Object.keys(featureClaimRevisionIds).sort()) !== canonicalJson([...featureKeys].sort())) {
      fail('INVALID_LINEAGE', 'Feature-level lineage must name every governed card feature exactly once.', { index });
    }
    for (const featureKey of featureKeys) {
      const revisions = featureClaimRevisionIds[featureKey];
      if (!Array.isArray(revisions) || revisions.length !== 1
        || !LOWERCASE_SHA256.test(revisions[0]) || !claimRevisionIds.includes(revisions[0])) {
        fail('INVALID_LINEAGE', 'Each governed card feature must bind to one source claim revision.', { index, feature_key: featureKey });
      }
    }
    ...
    if (featureKeys.includes('representationTopic')) {
      const value = card.features.representationTopic;
      if (!value || typeof value !== 'object'
        || !GOVERNED_CLAIMS[TOPIC_CLAIM_KEY].values.includes(value.code)
        || !nonEmptyString(value.text)) {
        fail('INVALID_NATIVE_FEATURES', 'A representation topic feature is not governed and source-grounded.', { index });
      }
    }
    for (const featureKey of ['generalMaterialityQualifier', 'representationMeasurementDate', 'retrospectiveLookback']) {
      if (!featureKeys.includes(featureKey)) continue;
      const value = card.features[featureKey];
      const expectedClaimKey = {
        generalMaterialityQualifier: 'GENERAL_MATERIALITY_QUALIFIER',
        representationMeasurementDate: 'REPRESENTATION_MEASUREMENT_DATE',
        retrospectiveLookback: 'RETROSPECTIVE_LOOKBACK',
      }[featureKey];
      if (!value || typeof value !== 'object'
        || !isGovernedCanonicalValue(expectedClaimKey, value.value)
        || !nonEmptyString(value.text)
        || !value.party || !value.attachment || !nonEmptyString(value.qualifier_dispatch)) {
        fail('INVALID_NATIVE_FEATURES', 'A dispatched representation qualifier feature is not exact and source-grounded.', { index, feature_key: featureKey });
      }
      if (featureKey === 'retrospectiveLookback' && value.temporal_form !== 'LOOKBACK') {
        fail('INVALID_NATIVE_FEATURES', 'A retrospective lookback feature must retain its temporal form.', { index });
      }
    }
...
function assertClaimShape(claim, index, cardsByProvisionInstanceId) {
  ...
  if (['materialityQualifier', 'representationTopic', 'generalMaterialityQualifier', 'representationMeasurementDate', 'retrospectiveLookback'].includes(claim.attribute)
    && claim.canonical.text !== claim.verbatim) {
    fail('INVALID_CLAIM_CONTRACT', 'A coded representation feature must retain the claim\'s exact verbatim quote.', { index });
  }
  ...
  if (canonicalJson(card.canonical_v2_lineage.feature_claim_revision_ids[claim.attribute])
    !== canonicalJson(revisionIds)) {
    fail('INVALID_LINEAGE', 'The claim provenance must match its card feature-level lineage.', { index });
  }
  return card;
}
