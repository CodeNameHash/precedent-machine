...
//      word-boundary-aligned negation drop on its own -- that is exactly
//      what tier 1's carried-forward span exists to catch instead.
...
// anchored occurrence (a boundary is required only on a side where the
// needle's OWN edge is a word character, so a needle starting or ending in
// punctuation is not penalised on that side) -- or null if `candidate` is
// not grounded in `sourceText` at all. This is the SAME computation used
// both to accept/reject a fresh quote at build time and to compute the
// span carried forward for later slice re-verification, so a record can
// never pass the build-time check at one position while a different,
// inconsistent position is recorded as its "verified" span.
function locateGrounding(candidate, sourceText) {
  ...
  for (;;) {
    ...
    if (leftOk && rightOk) return { start: index, end: index + candidate.length };
    from = index + 1;
  }
}
