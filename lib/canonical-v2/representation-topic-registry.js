...
/*
 * Inert V1 representation-topic registry. This module classifies recorded,
 * byte-verified REPRESENTATIONS limb candidates only. It has no resolver,
 * provider, writer, or serving dependency.
 */
...
// Anchors are deliberately data, not resolver-side regular expressions. Every
// pattern is matched after NFC/case/whitespace normalisation.
...
// Specific topics always beat a general rule. Pairs not listed here are not
// silently ordered: they are an open-world, non-hierarchical tie.
...
// These are the only non-hierarchical pairs that a source subject can decide.
// The raw quote must already anchor both candidates.
...
// A textual exclusion is narrower than precedence. It prevents a positive
// anchor from treating a defined MAE term or an adviser-fee disclosure as the
// substantive representation topic named by that nearby word.
...
function subjectBreaksTie(matches, subject) {
  if (matches.length !== 2 || !SUBJECT_TIE_SET.has(pairKey(...matches.map((item) => CODE_BY_RULE_ID[item.entry.id])))) return null;
  const text = normaliseForMatching(subject);
  if (!text) return null;
  const corroborated = matches.filter((item) => item.entry.subject_anchors.some((anchor) => testAnchor(text, anchor)));
...
function classifyRepresentationTopic({ subject = null, raw_value, family, generic_claim_key, claim_definition_key, evidence } = {}) {
  ...
  const matched = ruleMatches(normaliseForMatching(raw_value));
  if (!matched.length) return unclassified(UNCLASSIFIED_REASONS.NO_RAW_ANCHOR);
  const surviving = removeGeneralMatches(matched);
  const selected = surviving.length === 1 ? surviving[0] : subjectBreaksTie(surviving, subject);
  if (!selected) return unclassified(UNCLASSIFIED_REASONS.NON_HIERARCHICAL_TIE, surviving.map((item) => item.entry.id));
  const code = CODE_BY_RULE_ID[selected.entry.id];
  return result({ state: CLASSIFIED, canonical_value: code, reason: null, matched_rule_ids: Object.freeze(surviving.map((item) => item.entry.id).sort()), topic_match_rule_id: selected.entry.id, raw_anchor_matches: Object.freeze(selected.matched.sort()), subject_corroborated: selected.entry.subject_anchors.some((anchor) => testAnchor(normaliseForMatching(subject), anchor)) });
}

module.exports = Object.freeze({
  REGISTRY_VERSION, REGISTRY_DIGEST, NATIVE_LIMB_KEY, CLASSIFIED, UNCLASSIFIED, UNCLASSIFIED_REASONS,
