...
function buildProductAdmission(input = {}) {
  const realBridge = input.process_phrasebook_admission;
  const bridgeInput = realBridge === undefined ? null : realBridge;
  if (bridgeInput !== null) {
    requireExactKeys(input, ['process_phrasebook_admission'], 'Metsera Product admission input');
    try {
  ...
  const processAdmissionInput = bridgeInput === null
    ? input.process_admission_input
    : bridgeInput.process_admission_input;
  const processAdmissionReceipt = bridgeInput === null
    ? input.process_admission_receipt
    : bridgeInput.process_admission_receipt;
  try {
    validateProcessPhrasebookResultAdmissionReceipt(
  ...
  const body = {
    ...
    materialisation_receipt_id: bridgeInput === null
      ? null
      : bridgeInput.materialisation_receipt_id,
    ...intended,
    corpus_release_id: null,
    admission_input: clone(processAdmissionInput),
    admission_receipt: clone(processAdmissionReceipt),
    process_phrasebook_admission: bridgeInput === null ? null : clone(bridgeInput),
...
function validateMetseraExclusivityProductAdmission(value) {
  const rebuilt = value?.process_phrasebook_admission === null
    ? buildProductAdmission({
      process_admission_input: value?.admission_input,
      process_admission_receipt: value?.admission_receipt,
    })
    : buildProductAdmission({
