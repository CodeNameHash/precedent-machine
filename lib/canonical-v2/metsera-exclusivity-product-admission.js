...
function buildProductAdmission(input = {}, sourceBytesByIdentity) {
  requireExactKeys(
    input,
...
function compileMetseraExclusivityProductAdmission(input, sourceBytesByIdentity) {
  return deepFreeze(buildProductAdmission(clone(input), sourceBytesByIdentity));
}
