...
const F23_MANIFEST_DIGEST =
  '05931a9f9fac898223b4c3a05234cc5fb4960b780d213f50d9e7ce799989f6d4';
const F26_RELEASE_ID =
  'c3e00b8046bc10dc7120525bb3ce5734aea30ed9a5edc57779aa6a46128597b7';
const F26_RELEASE_DIGEST =
  'c1aff2fff67c70c37664d7fc9c3e0b89692e803c10c0bdb8da94a0a432be0a48';
const F26_MANIFEST_ID =
  '81adc296436ffcebfa4a185293124c8441d97ba9fa8d63ae59e48b065098923e';
const F26_MANIFEST_DIGEST =
  'a3c197f72b9b79a9e345d6c6191dbc0de234ac59fb739386ff416d7d2dcede76';
const F26_REGISTRY_ID =
  '7337f24c6ca498608f369bdffc0e9a8c6902a756e98924bb0de51e5a0c2aeeda';
const F26_PROVISION_ROW_ID =
  'c8bc07dfc8b06c0824e9dcd63b95be59f7a7405189cbfc592e184df1acda9411';
  ...
  return {
    ...
    counts: {
      ...
      ...(Object.prototype.hasOwnProperty.call(
        ...
        ? {
          excluded_slots:
            context.counts.excluded_slots
            + context.counts.observation_slots,
        }
        : {}),
    },
    distribution: [],
...
function copyDeliverySubrow(validated, admission) {
  ...
  const claimAttributes =
    f23.market_observations[0].claim_attributes;
  const presentation =
  ...
  return subrow({
    ...
    subject: {
      ...
      timing_semantics: {
        promptly_qualifier:
          claimAttributes.promptly_qualifier,
        outside_cap_semantic_code:
          claimAttributes.outside_cap_semantic_code,
      },
      items: [],
    },
...
function validateSubrow(row, admission) {
  ...
  if (row.comparison_kind === 'DURATION') {
    ...
    if (row.metric_key
        === 'NO_SHOP_COPY_DELIVERY_PERIOD_DAYS'
      && (row.subject.timing_semantics?.promptly_qualifier
          !== true
        || row.subject.timing_semantics
          .outside_cap_semantic_code
          !== 'PROMPTLY_AND_NO_LATER_THAN_STATED_DURATION')) {
      throw new TypeError(
        'F26 copy-delivery timing semantics are incomplete',
      );
    }
...
function compileNoShopF26MarketRequest({
  ...
  const base = {
    ...
    subject_deal_key:
      release.provision_row.governed_deal_key,
    execution_shape:
...
function validateNoShopF26MarketResult(result, request) {
  ...
  for (const metricResult of result.metric_results) {
    ...
    if (!exactKeys(metricResult, [
      ...
      || !exactKeys(metricResult.subject_exclusion, [
        'subject_deal_key',
        'exclusion_predicate',
        'state',
      ...
      || metricResult.subject_exclusion.subject_deal_key
        !== request.subject_deal_key
      || metricResult.subject_exclusion.exclusion_predicate
        !== 'GOVERNED_DEAL_KEY_NOT_EQUAL_SUBJECT_DEAL_KEY'
      || metricResult.subject_exclusion.state
        !== 'APPLIED_AND_VERIFIED'
      || metricResult.subject_exclusion.residual_subject_deals
        !== 0
      || metricResult.subject_exclusion
