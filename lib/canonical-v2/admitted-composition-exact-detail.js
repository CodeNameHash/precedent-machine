...
// Admitted counterpart of exact-detail.js's buildFixtureClaimEvidenceDetailPackage
// (SPEC-QXO-TERMF-F2-CANDIDATE-OPTION-A-2026-07-24.md). It serves the SAME
// frozen action (RESULT_COMPONENT_CLAIM_EVIDENCE, same response schema and
// byte bound — no contract change) but closes over an ADMITTED source context
// and V2 admission manifest instead of the reviewed fixture shapes. Needed
// because a complete result whose relationship evidence spans many long
// excerpts cannot fit the RESULT_COMPOSITION_EVIDENCE response inside its
// frozen 16KB bound; the live Landos termination-fee row set the precedent
// that such rows serve their claim's operative evidence as exact detail.
const CLAIM_EVIDENCE_ACTION_SLOT_KEY = 'RESULT_COMPONENT_CLAIM_EVIDENCE';

function claimEvidenceAction(contractBundle) {
  const actions = contractBundle.serving_exact_detail_action_definitions || [];
  const action = actions.find((entry) => entry.action_slot_key === CLAIM_EVIDENCE_ACTION_SLOT_KEY);
  if (!action) throw new TypeError('the frozen claim-evidence exact-detail action is unavailable');
  return action;
}

function admittedClaimEvidenceInputs({
  contractBundle,
  row,
  source,
  sourceAdmission,
  ...
  if (row.row_kind !== 'CANONICAL_RESULT'
    ...
    throw new TypeError('admitted claim-evidence detail may attach only to a validated base canonical row');
  }
  ...
  if (evidence.excerpt_id !== excerpt.excerpt_id
    ...
    || evidence.document_ordinal !== source.source_ordinal) {
    throw new TypeError('claim evidence and admitted source-backed excerpt do not agree');
  }
  return { action: claimEvidenceAction(contractBundle), component, evidence };
}

function admittedClaimEvidencePath({ source, sourceAdmission, excerpt, claim, evidence }) {
  const semanticSpan = {
    schema_version: 'SEMANTIC_SPAN/V1',
    canonical_text_id: source.canonical_text_id,
    absolute_start: excerpt.absolute_start,
...
function buildAdmittedClaimEvidenceDetailPackage({
  contract_bundle: contractBundle,
  row,
  source,
  source_admission: sourceAdmission,
  ...
  const { action, component, evidence } = admittedClaimEvidenceInputs({
    contractBundle,
    row,
    source,
    sourceAdmission,
  ...
  const orderedPath = admittedClaimEvidencePath({ source, sourceAdmission, excerpt, claim, evidence });
  const lineage = sourceLineage(source, sourceAdmission, contractBundle);
  const responseBody = {
    schema_version: action.response_schema,
    detail_kind: action.detail_kind,
    ...
    excerpt: excerptProjection(excerpt),
    source_lineage: lineage,
  };
  const responseBodyDigest = contentId('SERVING_EXACT_DETAIL_RESPONSE_BODY/V1', responseBody);
  const encodedByteLength = utf8ByteLength(canonicalJson(responseBody));
  if (encodedByteLength > action.maximum_encoded_bytes) {
    throw new TypeError(`admitted claim-evidence exact-detail response is ${encodedByteLength} bytes and exceeds its frozen byte bound`);
  }
  const payloadBody = {
    schema_version: 'SERVING_EXACT_DETAIL_PAYLOAD/V1',
    corpus_release_id: row.corpus_release_id,
...
function validateAdmittedClaimEvidenceDetailPackage({
  package: candidate,
  contract_bundle: contractBundle,
  source,
  source_admission: sourceAdmission,
  ...
  requireExactKeys(candidate, PACKAGE_KEYS, 'admitted claim-evidence exact-detail package');
  if (candidate.schema_version !== 'EXACT_DETAIL_ATOMIC_PACKAGE/V1'
    || !Array.isArray(candidate.action_definitions) || candidate.action_definitions.length !== 1
    || !Array.isArray(candidate.detail_payloads) || candidate.detail_payloads.length !== 1
    || !Array.isArray(candidate.references) || candidate.references.length !== 1
    || !Array.isArray(candidate.parent_edges) || candidate.parent_edges.length !== 1) {
    throw new TypeError('admitted claim-evidence exact-detail package is incomplete');
  }
  validateSharedServingRow(candidate.row);
  if (!Array.isArray(claim?.evidence)) {
    throw new TypeError('admitted claim-evidence validation requires the governed claim');
  }
  const evidenceOrdinal = claim.evidence.findIndex((edge) => edge.excerpt_id === excerpt?.excerpt_id);
  if (evidenceOrdinal < 0) {
    throw new TypeError('admitted claim-evidence excerpt is not a governed claim evidence edge');
  }
  const expected = buildAdmittedClaimEvidenceDetailPackage({
    contract_bundle: contractBundle,
    row: baseRowFromAttached(candidate.row),
    source,
    source_admission: sourceAdmission,
  ...
  if (canonicalJson(candidate) !== canonicalJson(expected)) {
    throw new TypeError('admitted claim-evidence exact-detail identity or source closure mismatch');
  }
  return true;
}

function buildOfflineInterpretedResultCompositionDetailPackage({
  contract_bundle: contractBundle,
  row,
  source,
  source_admission: sourceAdmission,
  ...
  if (!Array.isArray(components) || components.length !== 1) {
    throw new TypeError('offline interpreted exact detail requires one component');
  }
  const entry = components[0];
  requireExactKeys(
    entry,
    ['component', 'claim', 'interpretation', 'relationships'],
    'offline interpreted components[0]',
  );
  if (entry.relationships.length !== 0) {
    throw new TypeError('copy-delivery exact detail does not admit unrelated relationships');
  }
  validateClaimInterpretation({
    claim_interpretation: entry.interpretation,
  ...
  validateInterpretedPresentClaimRevision({
    ...
    policy: contractBundle.claim_interpretation_policy_definition,
    allowed_attributes: Object.keys(entry.claim.attributes),
    codebooks: {},
    expected_attributes: entry.claim.attributes,
  });
  const rowComponent = row.canonical_result.components[0];
  if (entry.component.component_occurrence_id
      !== rowComponent.component_occurrence_id
    || entry.component.component_revision_id
      !== rowComponent.component_revision_id
    || entry.claim.claim_occurrence_id
      !== rowComponent.claim_occurrence_id
    || entry.claim.claim_revision_id !== rowComponent.claim_revision_id
    || entry.interpretation.interpretation_payload_id
      !== rowComponent.interpretation_payload_id
    || entry.interpretation.interpretation_payload_id
      !== row.provenance.interpretation_payload_id) {
    throw new TypeError('offline exact-detail component does not close over its shared row');
  }
  const ordered = [...excerpts].sort(sourceOrder);
  if (canonicalJson(ordered) !== canonicalJson(excerpts)
    || ordered.length !== 4) {
    throw new TypeError('offline interpreted exact detail requires exactly four source-ordered excerpts');
  }
  const excerptIds = ordered.map((excerpt) => {
    validateExcerpt({ source, excerpt });
    return excerpt.excerpt_id;
  });
  const requiredIds = [...new Set([
    ...entry.claim.evidence.map((edge) => edge.excerpt_id),
    ...entry.interpretation.evidence_excerpt_ids,
    ...row.canonical_result.comparability_context
      .governing_evidence_excerpt_ids,
  ])].sort();
  if (canonicalJson([...excerptIds].sort()) !== canonicalJson(requiredIds)) {
    throw new TypeError('offline interpreted exact detail does not close over claim and interpretation evidence');
  }
  const responseBody = {
    schema_version: 'RESULT_COMPOSITION_EVIDENCE_RESPONSE/V2',
    ...
    components: [{
      governed_ordinal: 0,
      component_slot_key:
        row.canonical_result.components[0].component_slot_key,
      component_occurrence_id: entry.component.component_occurrence_id,
      component_revision_id: entry.component.component_revision_id,
      claim: clone(entry.claim),
      interpretation: clone(entry.interpretation),
    }],
    excerpts: ordered.map(excerptProjection),
  ...
  const payloadBody = {
    schema_version: 'SERVING_EXACT_DETAIL_PAYLOAD/V2',
  ...
  attachedBody.source_actions = [{
    ...
    source_detail_payload_id: payload.source_detail_payload_id,
    governed_ordinal: 0,
  }];
  attachedBody.canonical_result.source_detail_state = {
    state: 'AVAILABLE',
    reason_code: 'EXACT_DETAIL_CERTIFIED_OFFLINE',
  };
  const attachedRow = {
    ...attachedBody,
    canonical_payload_digest: contentId(
  ...
  validateOfflineCandidateSharedServingRow(attachedRow);
  return Object.freeze({
    schema_version: 'EXACT_DETAIL_ATOMIC_PACKAGE/V2',
    authority_scope: 'OFFLINE_CANDIDATE_ONLY',
    activation_eligibility:
      'INELIGIBLE_CONTRACT_NOT_SERVING_ADMITTED',
    row: Object.freeze(attachedRow),
    detail_payloads: Object.freeze([Object.freeze(payload)]),
  });
}

function validateOfflineInterpretedResultCompositionDetailPackage({
  package: candidate,
  ...inputs
} = {}) {
  const expected = buildOfflineInterpretedResultCompositionDetailPackage({
    ...inputs,
    row: (() => {
      const base = clone(candidate.row);
      delete base.canonical_payload_digest;
      base.source_actions = [];
      base.canonical_result.source_detail_state = {
        state: 'UNAVAILABLE',
        reason_code: 'EXACT_DETAIL_PROJECTION_NOT_BUILT',
      };
      return {
  ...
  if (canonicalJson(candidate) !== canonicalJson(expected)) {
    throw new TypeError('offline interpreted exact-detail package has drifted');
  }
  return true;
}

function validateOfflineInterpretedResultCompositionDetailPackageStructure(
  candidate,
) {
  requireExactKeys(candidate, [
    ...
    'row',
    'detail_payloads',
  ], 'offline exact-detail package');
  if (candidate.schema_version !== 'EXACT_DETAIL_ATOMIC_PACKAGE/V2'
    || candidate.authority_scope !== 'OFFLINE_CANDIDATE_ONLY'
    || candidate.activation_eligibility
      !== 'INELIGIBLE_CONTRACT_NOT_SERVING_ADMITTED'
    || candidate.detail_payloads.length !== 1) {
    throw new TypeError('invalid offline exact-detail package structure');
  }
  validateOfflineCandidateSharedServingRow(candidate.row);
  const payload = candidate.detail_payloads[0];
  requireExactKeys(payload, [
    ...
    'activation_eligibility',
    'corpus_release_id',
    'parent_row_serving_key',
    'detail_kind',
    'response_body',
    'response_body_digest',
    'source_detail_payload_id',
    'canonical_payload_digest',
  ], 'offline exact-detail payload');
  const { source_detail_payload_id: payloadId, canonical_payload_digest: digest, ...body } =
    payload;
  const response = payload.response_body;
  const rowComponent = candidate.row.canonical_result.components[0];
  requireExactKeys(response, [
    ...
    'excerpts',
    'source_lineage',
  ], 'offline exact-detail response');
  requireExactKeys(response.source_lineage, [
    'source_content_id',
  ...
  ], 'offline exact-detail source lineage');
  const sourceLineageDigests = [
    'source_content_id',
    'source_occurrence_id',
    'canonical_text_id',
    'immutable_source_document_id',
  ...
  if (payload.corpus_release_id !== candidate.row.corpus_release_id
    || payload.parent_row_serving_key !== candidate.row.row_serving_key
    || payload.detail_kind !== 'RESULT_COMPOSITION_EVIDENCE'
    || payload.response_body_digest !== contentId(
      'SERVING_EXACT_DETAIL_RESPONSE_BODY/V2',
      response,
    )
    || payloadId !== contentId('SERVING_EXACT_DETAIL_PAYLOAD/V2', body)
    || digest !== contentId('SERVING_EXACT_DETAIL_PAYLOAD_BODY/V2', body)
    || response?.schema_version
      !== 'RESULT_COMPOSITION_EVIDENCE_RESPONSE/V2'
    || response.detail_kind !== 'RESULT_COMPOSITION_EVIDENCE'
    || response.derived_result_occurrence_id
      !== candidate.row.canonical_result.derived_result_occurrence_id
    || response.derived_result_revision_id
      !== candidate.row.canonical_result.derived_result_revision_id
    || response.components?.length !== 1
    || response.excerpts?.length !== 4
    || candidate.row.source_actions?.length !== 1
    || candidate.row.source_actions[0].source_detail_payload_id
      !== payloadId
    || response.source_lineage.deal_admission_id
      !== candidate.row.deal_admission_id
    || response.source_lineage.contract_fingerprint
      !== candidate.row.provenance.contract_fingerprint
    || !Number.isInteger(response.source_lineage.source_byte_length)
    || response.source_lineage.source_byte_length < 1
    || sourceLineageDigests.some(
      (key) => !SHA256_RE.test(response.source_lineage[key] || ''),
    )
    || response.components[0].component_occurrence_id
      !== rowComponent.component_occurrence_id
    || response.components[0].component_revision_id
      !== rowComponent.component_revision_id
    || response.components[0].claim?.claim_revision_id
      !== rowComponent.claim_revision_id
    || response.components[0].interpretation?.interpretation_payload_id
      !== rowComponent.interpretation_payload_id) {
    throw new TypeError('offline exact-detail payload does not close over its row');
  }
  const excerptIds = response.excerpts.map((excerpt) => {
    const identity = {
      excerpt_definition_key: excerpt.excerpt_definition_key,
      excerpt_definition_version: excerpt.excerpt_definition_version,
      excerpt_definition_payload_digest:
        excerpt.excerpt_definition_payload_digest,
      ordered_component_assignments:
        excerpt.ordered_component_assignments,
      excerpt_purpose: excerpt.excerpt_purpose,
      transformation_or_redaction_version:
        excerpt.transformation_or_redaction_version,
      output_text_hash: sha256Hex(
        Buffer.from(excerpt.exact_text, 'utf8'),
      ),
    };
    if (excerpt.excerpt_id !== contentId('EXCERPT/V1', identity)
      || excerpt.exact_bytes_digest !== identity.output_text_hash) {
      throw new TypeError('offline exact-detail excerpt bytes or identity have drifted');
    }
    return excerpt.excerpt_id;
  });
  const required = [...new Set([
    ...response.components[0].claim.evidence.map(
      (edge) => edge.excerpt_id,
    ),
    ...response.components[0].interpretation.evidence_excerpt_ids,
    ...candidate.row.canonical_result.comparability_context
      .governing_evidence_excerpt_ids,
  ])].sort();
  if (canonicalJson([...excerptIds].sort()) !== canonicalJson(required)
    || !response.excerpts.some((excerpt) => (
      excerpt.absolute_start === 207783
        && excerpt.absolute_end === 208859
        && excerpt.exact_text.includes(
          'after receipt of any Company Acquisition Proposal or any Company Request',
        )
        && excerpt.exact_text.includes(
          'would reasonably be expected to give rise to or result in a Company Acquisition Proposal',
        )
    ))) {
    throw new TypeError('offline exact-detail legal evidence closure has drifted');
  }
  return true;
}
