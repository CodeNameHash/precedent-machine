...
const PROCESS_SCOPE_ENUMERATION_SCHEMA = 'PROCESS_SCOPE_ENUMERATION/V1';
const PROCESS_SCOPE_RECEIPT_DOMAIN = 'PROCESS_SCOPE_ENUMERATION_RECEIPT/V1';
...
function compareRecords(left, right) {
  return compareText(left.source_unit_id, right.source_unit_id)
    || (left.evidence?.start_utf8_byte ?? -1) - (right.evidence?.start_utf8_byte ?? -1)
    || (left.evidence?.end_utf8_byte ?? -1) - (right.evidence?.end_utf8_byte ?? -1)
    || compareText(left.slot_key || '', right.slot_key || '')
    || compareText(left.candidate_key || left.outcome_key, right.candidate_key || right.outcome_key);
}

function validateScopeReceipt(scopeReceipt, expectedScopeReceiptId) {
  const code = 'INVALID_PROCESS_LEXICAL_SCOPE_RECEIPT';
  requireDigest(expectedScopeReceiptId, 'expected_scope_receipt_id', code);
  requireExactKeys(scopeReceipt, [
    'schema_version',
    'scope_receipt_id',
  ...
  if (scopeReceipt.schema_version !== PROCESS_SCOPE_ENUMERATION_SCHEMA) {
    fail(code, 'scope_receipt schema_version is not supported.');
  }
  requireDigest(scopeReceipt.scope_receipt_id, 'scope_receipt.scope_receipt_id', code);
  if (scopeReceipt.scope_receipt_id !== expectedScopeReceiptId) {
    fail(code, 'scope_receipt does not match the trusted expected receipt ID.');
  }
  requireExactKeys(scopeReceipt.limits, ['max_scope_records', 'max_slots'], 'scope_receipt.limits', code);
  for (const key of ['max_scope_records', 'max_slots']) {
    if (!Number.isSafeInteger(scopeReceipt.limits[key]) || scopeReceipt.limits[key] < 1 || scopeReceipt.limits[key] > 1024) {
      fail(code, `scope_receipt.limits.${key} must be a bounded positive integer.`);
    }
  }
  if (!Array.isArray(scopeReceipt.expected_occurrence_slots)
    || scopeReceipt.expected_occurrence_slots.length > scopeReceipt.limits.max_slots) {
    fail(code, 'scope_receipt.expected_occurrence_slots must be bounded.');
  }
  function validateScopeEvidence(scopeEvidence, label) {
    if (!Array.isArray(scopeEvidence) || scopeEvidence.length === 0 || scopeEvidence.length > 64) {
    ...
    const evidence = scopeEvidence.map((item, evidenceIndex) => {
      requireExactKeys(item, expected, `${label}[${evidenceIndex}]`, code);
      for (const key of [
        'source_document_identity', 'source_revision_id', 'document_hash', 'exact_text_digest',
      ]) requireDigest(item[key], `${label}[${evidenceIndex}].${key}`, code);
      if (!Number.isSafeInteger(item.start_utf8_byte) || item.start_utf8_byte < 0
        || !Number.isSafeInteger(item.end_utf8_byte) || item.end_utf8_byte <= item.start_utf8_byte) {
        fail(code, `${label}[${evidenceIndex}] has an invalid source interval.`);
      }
      return cloneCanonical(item, `${label}[${evidenceIndex}]`, code);
    });
    const ordered = [...evidence].sort((left, right) => (
      compareText(left.source_document_identity, right.source_document_identity)
      || compareText(left.source_revision_id, right.source_revision_id)
      || left.start_utf8_byte - right.start_utf8_byte
      || left.end_utf8_byte - right.end_utf8_byte
      || compareText(left.exact_text_digest, right.exact_text_digest)
    ));
    if (canonicalJson(evidence) !== canonicalJson(ordered)
      || new Set(evidence.map(canonicalJson)).size !== evidence.length) {
      fail(code, `${label} must be ordered and unique.`);
    }
    return evidence;
  }
  const slots = scopeReceipt.expected_occurrence_slots.map((slot, index) => {
    ...
    return {
      ...cloneCanonical(slot, `scope_receipt.expected_occurrence_slots[${index}]`, code),
      scope_evidence: validateScopeEvidence(
        slot.scope_evidence,
        `scope_receipt.expected_occurrence_slots[${index}].scope_evidence`,
      ),
    };
  });
  const keys = slots.map((slot) => slot.slot_key);
  if (new Set(keys).size !== keys.length) fail(code, 'scope_receipt slot keys must be unique.');
  if (!Array.isArray(scopeReceipt.residuals)) {
    fail(code, 'scope_receipt.residuals must be an array.');
  }
  const residuals = scopeReceipt.residuals.map((residual, index) => {
    requireExactKeys(residual, [
      'record_key', 'residual_code', 'deal_id', 'scope_evidence',
    ], `scope_receipt.residuals[${index}]`, code);
    requireNonEmptyString(residual.record_key, `scope_receipt.residuals[${index}].record_key`, code);
    requireNonEmptyString(residual.residual_code, `scope_receipt.residuals[${index}].residual_code`, code);
    requireDigest(residual.deal_id, `scope_receipt.residuals[${index}].deal_id`, code);
    return {
      ...cloneCanonical(residual, `scope_receipt.residuals[${index}]`, code),
      scope_evidence: validateScopeEvidence(
        residual.scope_evidence,
        `scope_receipt.residuals[${index}].scope_evidence`,
      ),
    };
  });
  const residualRecordKeys = residuals.map((residual) => residual.record_key);
  const orderedResiduals = [...residuals].sort((left, right) => (
    compareText(left.record_key, right.record_key)
  ));
  if (new Set(residualRecordKeys).size !== residualRecordKeys.length
    || canonicalJson(residuals) !== canonicalJson(orderedResiduals)) {
    fail(code, 'scope_receipt residual record keys must be unique canonical UTF-8 order.');
  }
  const totalRecords = slots.length + residuals.length;
  if (totalRecords < 1 || totalRecords > scopeReceipt.limits.max_scope_records) {
    fail(code, 'scope_receipt must contain a governed bounded record set.');
  }
  const dealIds = new Set([
    ...slots.map((slot) => slot.deal_id),
    ...residuals.map((residual) => residual.deal_id),
  ]);
  if (dealIds.size !== 1) {
    fail(code, 'scope_receipt must bind one governed deal.');
  }
  const expectedReceiptId = contentId(PROCESS_SCOPE_RECEIPT_DOMAIN, {
    governed_deal_admission_id: [...dealIds][0],
    expected_occurrence_slots: slots,
    residuals,
    limits: cloneCanonical(scopeReceipt.limits, 'scope_receipt.limits', code),
  });
  if (scopeReceipt.scope_receipt_id !== expectedReceiptId) {
    fail(code, 'scope_receipt ID does not bind its complete validated payload.');
  }
  return {
...
function validateSourceUnit(value, index, slotByKey) {
  ...
  if (!Array.isArray(value.lexical_observations)
    || value.lexical_observations.length === 0) {
    fail(code, `source_units[${index}].lexical_observations must be a non-empty array.`);
  }
...
function evidenceFor(unit, observation, scopeEvidence = []) {
  const evidence = {
  ...
  if (observation.evidence_role_key !== null) {
    evidence.evidence_role_key = observation.evidence_role_key;
  }
  if (scopeEvidence.length > 0) evidence.scope_evidence = scopeEvidence;
  return Object.freeze(evidence);
}

function outcomeFor(unit, observation, outcomeKind, reasonCode) {
  const evidence = evidenceFor(unit, observation, observation.mapped_slot?.scope_evidence);
  const body = {
...
function buildScopeResidual(scopeReceiptId, residual) {
  const body = {
    scope_receipt_id: scopeReceiptId,
    outcome_kind: 'SCOPE_RESIDUAL',
    reason_code: residual.residual_code,
    source_unit_id: residual.record_key,
    slot_key: null,
    evidence: null,
    lexical_payload: null,
    scope_evidence: residual.scope_evidence,
  };
  return Object.freeze({
    schema_version: PROCESS_LEXICAL_OUTCOME_SCHEMA,
    outcome_key: contentId(PROCESS_LEXICAL_OUTCOME_SCHEMA, body),
...
function enumerateProcessLexicalCandidates(input, expectedScopeReceiptId) {
  ...
  const scopeReceipt = validateScopeReceipt(
    input.scope_receipt,
  ...
  const candidates = [];
  const rejections = [];
  const residuals = scopeReceipt.residuals.map((residual) => (
    buildScopeResidual(scopeReceipt.scope_receipt_id, residual)
  ));
