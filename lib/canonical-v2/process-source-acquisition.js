...
function verifiedSourceBytes(
  ...
  if (!Buffer.isBuffer(bytes)) {
    fail(code, 'Source bytes are missing for the frozen source document identity.');
  }
  if (sha256Hex(bytes) !== expectedDigest) {
    fail(code, 'Frozen source bytes do not match source_text_digest.');
  }
  return bytes;
}

function validateSource(source, expectedIds, sourceBytesByIdentity) {
  ...
  return {
    source: canonicalClone(source, 'frozen source', code),
    sourceBytes,
  };
}
...
function acquireProcessSources(input, sourceBytesByIdentity) {
  ...
  const intakeOutcomes = input.frozen_sources.map((source) => {
    const { source: valid, sourceBytes } = validateSource(
      source,
      expectedIds,
      sourceBytesByIdentity,
    );
