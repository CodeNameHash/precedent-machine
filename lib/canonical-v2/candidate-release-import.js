...
function projectCandidateCorrectionInputSeal({ seal, manifest }) {
  ...
  const correctionInputRoot = contentId('FIXTURE_RELEASE_CORRECTION_INPUT/V1', {
...
function buildCandidateReleaseImportPlan({ release, environment = 'staging' } = {}) {
  ...
  const body = {
    schema_version: 'CANDIDATE_RELEASE_IMPORT_PLAN/V3',
    environment,
    release_record: {
      corpus_release_id: manifest.corpus_release_id,
  ...
  const plan = Object.freeze({
    ...body,
    candidate_release_import_plan_id: contentId('CANDIDATE_RELEASE_IMPORT_PLAN/V3', body),
  });
...
function validateCandidateReleaseImportPlan(plan) {
  ...
  if (plan.schema_version !== 'CANDIDATE_RELEASE_IMPORT_PLAN/V3' || plan.environment !== 'staging') {
  ...
  if (planId !== contentId('CANDIDATE_RELEASE_IMPORT_PLAN/V3', body)) {
...
function validateCandidateReleaseImportReceipt(receipt, plan) {
  ...
  if (receipt.schema_version !== 'CANDIDATE_RELEASE_IMPORT_RECEIPT/V3'
