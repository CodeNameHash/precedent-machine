...
} = require('./product-query-result-release-partition');
const {
...
function projectionBindingForManifest(manifest) {
  if (manifest?.schema_version === 'FIXTURE_CANDIDATE_RELEASE_MANIFEST/V3') {
    if (manifest.serving_projection_version !== SERVING_PROJECTION_VERSION_V2
      || manifest.query_projection_contract_digest !== QUERY_PROJECTION_CONTRACT_DIGEST_V2) {
      throw new TypeError('projection-bound manifest does not select the frozen v2 serving contract');
    }
  ...
  if (['FIXTURE_CANDIDATE_RELEASE_MANIFEST/V1', 'FIXTURE_CANDIDATE_RELEASE_MANIFEST/V2']
    .includes(manifest?.schema_version)) {
    return Object.freeze({
      projection_version: SERVING_PROJECTION_VERSION_V1,
      query_projection_contract_digest: null,
    });
  }
  throw new TypeError('candidate release manifest has no governed serving projection');
}
...
function buildCandidateReleaseImportPlan({
  release,
  ...
  const carriesProductResults = productResultReleasePartition !== undefined;
  if (carriesProductResults) {
    validateProductCandidateReleaseBundle(productResultReleasePartition);
    if (canonicalJson(
      productResultReleasePartition.base_candidate_release_manifest,
    ) !== canonicalJson(release.manifest)) {
      throw new TypeError(
        'Product result release partition does not bind the imported candidate release.',
      );
    }
  }
  const { manifest } = release;
  const isV5 = release.schema_version === 'FIXTURE_CANDIDATE_RELEASE_BUNDLE/V2';
  const isV6 = release.schema_version === 'FIXTURE_CANDIDATE_RELEASE_BUNDLE/V3';
  const carriesIncompletePartition = isV5 || isV6;
  const projectionBinding = projectionBindingForManifest(manifest);
  const incompleteCanonicalRecords = carriesIncompletePartition
    ? release.incomplete_canonical_rows.map((row) => (
  ...
  const body = {
    schema_version: carriesProductResults
    ...
    release_record: {
      ...
      contract_fingerprint: manifest.contract_fingerprint,
      projection_version: projectionBinding.projection_version,
      ...(isV6 ? {
        query_projection_contract_digest: projectionBinding.query_projection_contract_digest,
      } : {}),
      ...(carriesProductResults ? {
    ...
    ...(carriesIncompletePartition ? { incomplete_canonical_records: incompleteCanonicalRecords } : {}),
    ...
    expected_counts: {
      ...
      ...(carriesIncompletePartition ? {
        incomplete_canonical_records: incompleteCanonicalRecords.length,
      } : {}),
      ...
      ...(carriesProductResults ? {
        product_query_result_release_partition_manifests: 1,
        product_query_result_serving_records:
          productResultReleasePartition
            .product_query_result_serving_records.length,
      } : {}),
    },
  };
...
function validateCandidateReleaseImportPlan(plan) {
  ...
  const isV6 = plan?.schema_version === 'CANDIDATE_RELEASE_IMPORT_PLAN/V6';
  const isV7 = plan?.schema_version === 'CANDIDATE_RELEASE_IMPORT_PLAN/V7';
  const carriesProductResults = isV7;
  const carriesIncompletePartition = isV5 || isV6 || isV7;
  requireExactKeys(plan, [
    ...
    'market_exclusions',
    'query_records',
    ...(carriesIncompletePartition ? ['incomplete_canonical_records'] : []),
  ...
  requireExactKeys(plan.release_record, [
    ...
    'contract_fingerprint',
    'projection_version',
    ...((isV6 || isV7) ? ['query_projection_contract_digest'] : []),
    ...(carriesProductResults ? [
      'product_query_result_release_partition_manifest_id',
      'product_query_result_release_partition_manifest_payload_digest',
      'product_query_result_record_root',
  ...
  if (carriesProductResults) {
    ...
    if (
      !Number.isInteger(
        plan.release_record.product_query_result_record_count,
      )
      || plan.release_record.product_query_result_record_count < 1
    ) {
      throw new TypeError(
        'release_record.product_query_result_record_count must be positive.',
      );
    }
  }
  const manifestProjection = projectionBindingForManifest(plan.release_record.canonical_payload);
  if (plan.release_record.projection_version !== manifestProjection.projection_version
    || ((isV6 || isV7) && plan.release_record.query_projection_contract_digest
      !== manifestProjection.query_projection_contract_digest)) {
    throw new TypeError('release record serving projection does not equal its certified manifest');
  }
  if (plan.release_record.response_schema_version !== 'MARKET_COHORT_RESULT/V1') {
    throw new TypeError('response_schema_version is not governed');
  }
  ...
  if (isV5 !== (plan.release_record.canonical_payload.schema_version
    ...
    || (isV6 || isV7) !== (plan.release_record.canonical_payload.schema_version
      === 'FIXTURE_CANDIDATE_RELEASE_MANIFEST/V3')
    || (carriesIncompletePartition && plan.expected_counts.incomplete_canonical_records
  ...
  if (
    carriesProductResults
    && plan.expected_counts.product_query_result_release_partition_manifests !== 1
  ) {
    throw new TypeError(
      'candidate release import requires exactly one Product result release partition.',
    );
  }
  ...
  if (carriesProductResults) {
    const productPartition = {
      schema_version: 'PRODUCT_QUERY_RESULT_RELEASE_PARTITION_BUNDLE/V1',
      base_candidate_release_manifest:
        plan.release_record.canonical_payload,
      product_query_result_release_partition_manifest:
        plan.product_query_result_release_partition_manifest,
      product_query_result_serving_records:
        plan.product_query_result_serving_records,
    };
    validateProductCandidateReleaseBundle(productPartition);
    const partition =
      plan.product_query_result_release_partition_manifest;
    if (
      plan.release_record
      ...
      || plan.release_record
        .product_query_result_release_partition_manifest_payload_digest
        !== partition.canonical_payload_digest
      || plan.release_record.product_query_result_record_root
        !== partition.product_query_result_record_root
      || plan.release_record.product_query_result_record_count
        !== partition.product_query_result_record_count
      || plan.expected_counts.product_query_result_serving_records
        !== partition.product_query_result_record_count
    ) {
      throw new TypeError(
        'Product result partition does not equal the certified import plan.',
      );
    }
  }
  ...
  const releaseBoundCollections = collections.filter(
    (key) => key !== 'product_query_result_serving_records',
  );
  for (const record of [
    plan.correction_input_seal_record,
    ...releaseBoundCollections.flatMap((key) => plan[key]),
  ]) {
...
function validateCandidateReleaseImportReceipt(receipt, plan) {
  ...
  const expectedReceiptSchema = plan.schema_version === 'CANDIDATE_RELEASE_IMPORT_PLAN/V7'
    ? 'CANDIDATE_RELEASE_IMPORT_RECEIPT/V7'
    : plan.schema_version === 'CANDIDATE_RELEASE_IMPORT_PLAN/V6'
      ? 'CANDIDATE_RELEASE_IMPORT_RECEIPT/V6'
    : plan.schema_version === 'CANDIDATE_RELEASE_IMPORT_PLAN/V5'
