...
function formatCellValue(columnKey, value, options = {}) {
  ...
  if (columnKey === 'percent_of_deal_value') {
    ...
    if (!formatted) return '—';
    return options.denominatorPrecision === 'APPROXIMATE'
  ...
  if (columnKey === 'deal_value_basis') {
    ...
    const prefix = options.denominatorPrecision === 'APPROXIMATE' ? 'Approximately ' : '';
    return `${prefix}${value.currency} ${amount} · ${basis}`;
  }
...
function mapCanonicalRowForRender(row, columns) {
  ...
  try {
    ...
    const denominatorPrecision = row.display_metadata?.denominator_precision;
