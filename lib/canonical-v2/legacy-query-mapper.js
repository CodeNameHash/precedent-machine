...
// request shape (see docs/archive/handoffs/SPEC-CANONICAL-QUERY-UI-SLICE-2026-07-22.md),
// widened by docs/archive/handoffs/SPEC-QUERY-METRICS-WIDENING-2026-07-23.md to a
...
// (docs/archive/handoffs/ANALYSIS-D2-ADVISER-LAWYER-ENTITY-CLASSES-2026-07-23.md):
...
const SUPPORTED_BUYER_FEE_METRIC_KEY = 'BUYER_TERMINATION_FEE_PERCENT_OF_DEAL_VALUE';
const SUPPORTED_BUYER_FEE_METRIC_VERSION = 1;
const SUPPORTED_BUYER_FEE_CONCEPT_KEY = 'TERMF-REVERSE';
const SUPPORTED_BUYER_FEE_PARTY = Object.freeze({ role: 'FEE_PAYER', value: 'PARENT', capacity: 'BUYER' });
...
function isSupportedSellerTerminationFeeRequest(payload) {
  return payload.provision_type === 'TERMINATION_FEE' && payload.field_path === 'feePctOfDealValue';
}

function isSupportedBuyerTerminationFeeRequest(payload) {
  return payload.provision_type === 'TERMINATION_FEE' && payload.field_path === 'reverseFeePctOfDealValue';
}

function isSupportedTerminationFeeRequest(payload) {
  return isSupportedSellerTerminationFeeRequest(payload) || isSupportedBuyerTerminationFeeRequest(payload);
}
...
function mapLegacyRequestToCanonical(payload) {
  ...
  // Only one of these predicates can ever be true here — the caller
  ...
  // exactly one supported request shape.
  const isNoShopMatch = isSupportedNoShopMatchRequest(payload || {});
  const isBuyerFee = isSupportedBuyerTerminationFeeRequest(payload || {});
  const metricKey = isNoShopMatch
    ? SUPPORTED_NO_SHOP_MATCH_METRIC_KEY
    : isBuyerFee ? SUPPORTED_BUYER_FEE_METRIC_KEY : SUPPORTED_METRIC_KEY;
  const metricVersion = isNoShopMatch
    ? SUPPORTED_NO_SHOP_MATCH_METRIC_VERSION
    : isBuyerFee ? SUPPORTED_BUYER_FEE_METRIC_VERSION : SUPPORTED_METRIC_VERSION;
  const conceptKey = isNoShopMatch
    ? SUPPORTED_NO_SHOP_MATCH_CONCEPT_KEY
    : isBuyerFee ? SUPPORTED_BUYER_FEE_CONCEPT_KEY : SUPPORTED_CONCEPT_KEY;
  const party = isNoShopMatch
    ? SUPPORTED_NO_SHOP_MATCH_PARTY
    : isBuyerFee ? SUPPORTED_BUYER_FEE_PARTY : SUPPORTED_PARTY;
...
function buildRefinedCanonicalRequest(baseBody, columnFilters) {
  ...
  if (['trigger_code', 'payment_timing', 'trigger_condition']
    .filter((key) => next[key] !== undefined).length > 1) {
    throw new Error('Only one trigger-pathway refinement may be applied at a time.');
  }
...
function humanizeRefinementLabel(key) {
  if (key === 'trigger_condition') return 'Pathway factor';
...
// from the full-cohort summary where one exists, otherwise from the current
// page. Values never come from a hardcoded list.
function valuesForRefinementKey(view, filterKey) {
  ...
  const fullCohortValues = facetKey
    ? view?.cohort_summary?.facets?.[facetKey]
    : null;
  if (Array.isArray(fullCohortValues)) {
    return fullCohortValues
  ...
  for (const row of rows) {
    ...
    for (const trigger of triggers) {
      ...
      if (filterKey === 'trigger_condition' && Array.isArray(trigger.indexed_facts)) {
        for (const fact of trigger.indexed_facts) {
          if (fact) values.add(fact);
        }
      }
    }
  }
...
// from the governed response. No hardcoded vocabulary list anywhere in this function. Percent
...
function refinementOptionsFromView(view) {
  ...
  for (const refinement of refinements) {
    ...
    options.push(Object.freeze({
      ...
      values: Object.freeze(valuesForRefinementKey(view, filterKey)),
    }));
  }
  return Object.freeze(options);
}

function hasPercentRangeRefinement(view) {
  return Array.isArray(view?.refinements) && view.refinements.some((refinement) => (
    refinement?.operator === 'BETWEEN'
      && refinement?.request_field === 'column_filters.percent_of_deal_value'
  ));
}
...
const CANONICAL_VIEW_IDENTITY_KEYS = Object.freeze([
  'schema_version',
  'release_selector',
  'pointer_id',
...
function sameCanonicalValue(left, right) {
  ...
  return leftKeys.length === rightKeys.length
    && leftKeys.every((key, index) => (
      key === rightKeys[index] && sameCanonicalValue(left[key], right[key])
    ));
}

function appendCanonicalPage(existingView, nextView, requestedCursor) {
  ...
  if (CANONICAL_VIEW_IDENTITY_KEYS.some((key) => existingView[key] !== nextView[key])
    || !sameCanonicalValue(existingView.party, nextView.party)
    || !sameCanonicalValue(existingView.columns, nextView.columns)
    || !sameCanonicalValue(existingView.refinements, nextView.refinements)
    || !sameCanonicalValue(existingView.cohort_summary, nextView.cohort_summary)) {
    throw new Error('Cannot append a canonical query page from a different governed query.');
  }
  if (!requestedCursor || !sameCanonicalValue(existingView.next_cursor, requestedCursor)) {
    throw new Error('Cannot append a canonical query page from a discontinuous cursor.');
  }
  ...
  const mergedRows = [...existingRows, ...appended];
  if (mergedRows.length > nextView.total_count
    || (nextView.next_cursor === null && mergedRows.length !== nextView.total_count)
    || (nextView.next_cursor !== null && mergedRows.length >= nextView.total_count)) {
    throw new Error('Cannot append a canonical query page that truncates the governed result.');
  }
  return Object.freeze({
    ...existingView,
    rows: Object.freeze(mergedRows),
...
// fake to test.
function resolveCanonicalQueryPageUpdate(isShowMore, existingView, nextView, requestedCursor = null) {
  return isShowMore ? appendCanonicalPage(existingView, nextView, requestedCursor) : nextView;
}
...
async function runCanonicalRefinementRequest({
  ...
  try {
    return Object.freeze({
      ok: true,
      view: resolveCanonicalQueryPageUpdate(
        isShowMore,
        existingView,
        outcome.view,
        isShowMore ? body?.cursor : null,
      ),
    });
  } catch {
...
} = require('./denominator-precision-presentation');
...
function formatTriggerCell(trigger) {
  ...
  for (const key of [
    'pathway_label',
    'trigger_label',
    'terminating_party_label',
    'payment_timing_label',
  ]) {
    if (typeof trigger[key] !== 'string' || !trigger[key]) {
      throw new TypeError(`triggers[] entry is missing ${key}`);
    }
  }
  return Object.freeze({
    display_kind: 'TERMINATION_FEE_PATHWAY',
    pathway_code: trigger.pathway_code || null,
    pathway_label: trigger.pathway_label,
    trigger_label: trigger.trigger_label,
    terminating_party_label: trigger.terminating_party_label,
    payment_timing_label: trigger.payment_timing_label,
    condition_expression_text: trigger.condition_expression_text || trigger.trigger_label,
  });
}
...
// "[object Object]".
function formatCellValue(columnKey, value, options = {}) {
  if (value === null || value === undefined) return '—';
  if (columnKey === 'percent_of_deal_value') {
    const formatted = formatPercentOfDealValue(value);
    if (!formatted) return '—';
    return qualifyDerivedValue(
      formatted,
      normaliseDenominatorPrecision(options.denominatorPrecision),
    );
  }
  ...
  if (columnKey === 'duration') {
    if (!value || typeof value !== 'object') throw new TypeError('duration cell must be an object');
    const magnitude = value.canonical_value;
    const unit = String(value.canonical_unit || '').toLowerCase();
    if (typeof magnitude !== 'string' || !unit) throw new TypeError('duration cell is incomplete');
    const canonical = `${magnitude} ${unit.replace(/s$/, '')}${magnitude === '1' ? '' : 's'}`;
    if (!value.raw_magnitude || !value.raw_unit) return canonical;
    const rawUnit = String(value.raw_unit).toLowerCase().replace(/s$/, '');
    const raw = `${value.raw_magnitude} ${rawUnit}${value.raw_magnitude === '1' ? '' : 's'}`;
    return canonical === raw ? canonical : `${canonical} (source: ${raw})`;
  }
  if (columnKey === 'deal_value_basis') {
    if (!value || typeof value !== 'object'
      || typeof value.value !== 'string'
      || typeof value.currency !== 'string'
      || typeof value.basis !== 'string') {
      throw new TypeError('deal-value basis cell is incomplete');
    }
    const amount = value.value.replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    const basis = value.basis.toLowerCase().replaceAll('_', ' ');
    return qualifyDealValueBasis(
      `${value.currency} ${amount} · ${basis}`,
      normaliseDenominatorPrecision(options.denominatorPrecision),
    );
  }
...
function mapCanonicalRowForRender(row, columns) {
  ...
  try {
    ...
    const denominatorPrecision = normaliseDenominatorPrecision(
      row.display_metadata?.denominator_precision,
    );
    const cells = columnKeys.map((key) => ({
      column_key: key,
      display: formatCellValue(
        key,
        row.cells ? row.cells[key] : undefined,
