...
function validateFrozenSlots(values, scopeReceiptId) {
  ...
  if (new Set(keys).size !== keys.length || typeof scopeReceiptId !== 'string') {
    fail(code, 'Frozen slots must have unique keys and a scope receipt.');
  }
  return new Set(keys);
}
...
function validateInput(input) {
  ...
  requireExactKeys(input, [
    'schema_version', 'scope_receipt_id', 'frozen_slots', 'source_units', 'limits',
  ...
  requireDigest(input.scope_receipt_id, 'Scope receipt ID', code);
  ...
  const slotKeys = validateFrozenSlots(input.frozen_slots, input.scope_receipt_id);
