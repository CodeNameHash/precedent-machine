...
function validateFrozenSlots(values) {
  ...
  if (!Array.isArray(values) || values.length > MAX_SOURCE_UNITS) {
    fail(code, 'Frozen slots must be a bounded array.');
  }
...
function validateScopeReceipt(value, expectedScopeReceiptId) {
  const code = 'INVALID_PROCESS_SEMANTIC_SCOPE_RECEIPT';
  requireDigest(expectedScopeReceiptId, 'Expected semantic scope receipt ID', code);
  requireExactKeys(value, [
  ...
  if (value.scope_receipt_id !== expectedScopeReceiptId) {
    fail(code, 'Semantic scope receipt does not match the trusted expected ID.');
  }
  ...
  if (!Array.isArray(value.residuals)
    || value.expected_occurrence_slots.length + value.residuals.length < 1
...
function validateInput(input, expectedScopeReceiptId) {
  ...
  const scope = validateScopeReceipt(
    input.scope_receipt,
    expectedScopeReceiptId,
  );
  const slotKeys = scope.slot_keys;
  if (!Array.isArray(input.source_units)
    || input.source_units.length > input.limits.max_source_units) {
    fail(code, 'Semantic source units must be a bounded array.');
  }
...
function enumerateProcessSemantics(input, expectedScopeReceiptId) {
  const value = validateInput(
    clone(input, 'Semantic enumeration input', 'INVALID_PROCESS_SEMANTIC_ENUMERATOR_INPUT'),
    expectedScopeReceiptId,
  );
