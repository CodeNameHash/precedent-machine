...
function projectMarketMetricSlot(input) {
  ...
  const relationships = (input.relationships || []).filter(
    (row) => row.source_occurrence_id === input.claim.subject_occurrence_id
      && (!definition.required_relationship_key
        || row.relationship_definition_key === definition.required_relationship_key),
  );
  if (relationships.length > 1) throw new TypeError('metric slot has more than one governed owner relationship');
