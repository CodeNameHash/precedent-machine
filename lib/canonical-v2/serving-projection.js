...
function exclusionReason(input, definition, relationship) {
  ...
  if (!definition.allowed_canonical_values.includes(input.claim.canonical_value)) {
...
function projectMarketMetricSlot(input) {
  ...
  const relationships = (input.relationships || []).filter(
    (row) => row.relationship_definition_key === definition.required_relationship_key
      && row.source_occurrence_id === input.claim.subject_occurrence_id,
  );
  if (relationships.length > 1) throw new TypeError('metric slot has more than one governed BRINGS_DOWN relationship');
  ...
  const evidenceIds = sortedDigests([
    ...
    ...(relationship ? relationship.evidence_ids || [] : []),
  ], 'evidence_id');
  const relationshipRevisionIds = relationship
    ? [requireDigest(relationship.relationship_revision_id, 'relationship.relationship_revision_id')]
    : [];
  const relationshipEffectDigests = relationship && relationship.effect
    ? [contentId('RELATIONSHIP_EFFECT/V1', relationship.effect)]
    : [];
  const common = {
  ...
  const observationBody = {
    ...
    basis_key: definition.canonical_unit,
...
function assertMetricSlotPartition(outputs) {
  ...
  for (const output of outputs || []) {
    ...
    if (output.observation && output.observation.metric_slot_key !== output.metric_slot_key) {
      throw new TypeError('observation metric slot mismatch');
    }
    if (output.exclusion && output.exclusion.metric_slot_key !== output.metric_slot_key) {
      throw new TypeError('exclusion metric slot mismatch');
    }
  }
  return true;
}
