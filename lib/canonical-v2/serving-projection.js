...
function projectOfflineInterpretedMarketMetricSlot(input) {
  if (!input || input.authority_scope !== 'OFFLINE_CANDIDATE_ONLY') {
    throw new TypeError('offline interpreted projection requires explicit offline candidate authority');
  }
  const {
  ...
  if (input.integration_admission_id
      !== QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID) {
    throw new TypeError('offline projection lacks the pinned F19 integration admission');
  }
  ...
  validateClaimInterpretation({
    claim_interpretation: input.claim_interpretation,
    policy: input.contract_bundle.claim_interpretation_policy_definition,
  });
  validateInterpretedPresentClaimRevision({
    claim: input.claim,
    interpretation: input.claim_interpretation,
    policy: input.contract_bundle.claim_interpretation_policy_definition,
    allowed_attributes: Object.keys(input.claim.attributes),
    codebooks: {},
    expected_attributes: input.claim.attributes,
  });
  if (input.claim.interpretation_payload_id
      !== input.comparability_context?.interpretation_payload_id
    || input.comparability_context?.comparability_class_digest == null) {
    throw new TypeError('offline projection requires typed interpretation-keyed comparability context');
  }
  const definition = requireMetric(input.metric_key);
  if (definition.metric_key !== 'NO_SHOP_COPY_DELIVERY_PERIOD_DAYS'
    || input.concept_key !== definition.concept_key
    || input.claim.claim_definition_key
      !== definition.required_claim_definition_key
    || input.claim.canonical_value !== '1'
    || input.claim.unit !== definition.canonical_unit
    || input.claim.day_basis !== 'ELAPSED'
    || input.claim.attributes?.trigger !== definition.trigger
    || !copyDeliveryInterpretationAllowed(
      input.claim.attributes,
      definition,
    )) {
    throw new TypeError('offline interpreted value is outside the exact copy-delivery metric contract');
  }
  const expectedResult = buildFixtureResultComponent({
    deal_admission_id: input.deal.deal_admission_id,
    result_key: input.result.result_key,
  ...
  if (canonicalJson(expectedResult) !== canonicalJson(input.result)
    || input.result.completeness !== 'COMPLETE'
    || input.result.comparability !== 'COMPARABLE') {
    throw new TypeError('offline interpreted result does not close over the V2 claim');
  }
  const party = requireParty(input.party);
  const dimensions = validateDimensions(input.deal.dimensions);
  const slot = slotIdentity(input, definition, party);
  ...
  const occurrenceBody = {
    ...
    result_version: slot.result_version,
    owner_type: slot.owner_type,
    owner_occurrence_id: input.result.component_occurrence_id,
    scope_type: slot.scope_type,
    scope_id: input.result.component_occurrence_id,
    value_slot_key: slot.value_slot_key,
    ordinal: slot.ordinal,
    comparability_class_digest:
      input.comparability_context.comparability_class_digest,
  };
  const occurrenceId = contentId('METRIC_OBSERVATION_OCCURRENCE/V2', occurrenceBody);
  const body = {
    schema_version: 'MARKET_OBSERVATION/V2',
    authority_scope: 'OFFLINE_CANDIDATE_ONLY',
    activation_eligibility: 'INELIGIBLE_CONTRACT_NOT_SERVING_ADMITTED',
    integration_admission_id: input.integration_admission_id,
    corpus_release_id: input.corpus_release_id,
    contract_fingerprint: input.contract_bundle.fingerprint,
    metric_definition_admission_id:
    ...
    value_slot_key: slot.value_slot_key,
    ordinal: slot.ordinal,
    claim_schema_version: input.claim.schema_version,
    claim_occurrence_id: input.claim.claim_occurrence_id,
    claim_revision_id: input.claim.claim_revision_id,
    claim_definition_key: input.claim.claim_definition_key,
    claim_definition_version: input.claim.claim_definition_version,
    claim_evidence_ids: [...input.claim.evidence_ids],
    claim_attributes: clone(input.claim.attributes),
    claim_scope_closure_id: input.claim.scope.scope_closure_id,
    claim_scope: clone(input.claim.scope),
    composition_scope_closure_id:
      input.result.composition_scope_closure_id,
    result_input_lineage_digest: input.result.input_lineage_digest,
    relationship_revision_ids: [],
    relationship_effect_digests: [],
    relationship_effects: [],
    evidence_ids: [...input.claim.evidence_ids],
    state: input.claim.state,
    raw_value: input.claim.raw_value,
    canonical_value: input.claim.canonical_value,
    unit: input.claim.unit,
    ...
    examination_state: 'EXAMINED',
    presence_state: 'PRESENT',
    comparability_state: 'COMPARABLE_WITH_GOVERNED_AMBIGUITY',
    interpretation_payload_id: input.claim.interpretation_payload_id,
    comparability_context: clone(input.comparability_context),
    ...dimensions,
  };
  const observation = {
    market_observation_serving_key: contentId('MARKET_OBSERVATION/V2', {
      corpus_release_id: input.corpus_release_id,
      metric_observation_occurrence_id: occurrenceId,
    }),
    ...body,
    canonical_payload_digest: contentId('MARKET_OBSERVATION_PAYLOAD/V2', body),
  };
  return Object.freeze({
    metric_slot_key: metricSlotKey,
...
function validateOfflineInterpretedMarketMetricSlot(output) {
  if (!output?.observation || output.exclusion !== null
    || output.observation.schema_version !== 'MARKET_OBSERVATION/V2'
    || output.observation.authority_scope !== 'OFFLINE_CANDIDATE_ONLY'
    || output.observation.activation_eligibility
      !== 'INELIGIBLE_CONTRACT_NOT_SERVING_ADMITTED') {
    throw new TypeError('invalid offline interpreted metric projection');
  }
  const {
    QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID,
  } = require('./candidate-metric-definition-policy');
  const { market_observation_serving_key: servingKey, canonical_payload_digest, ...body } =
    output.observation;
  const definition = requireMetric(body.metric_key);
  const expectedSlot = contentId('MARKET_METRIC_SLOT/V1', {
    schema_version: 'MARKET_METRIC_SLOT/V1',
    metric_definition_id: metricDefinitionId(definition),
    deal_key: body.deal_key,
    deal_admission_id: body.deal_admission_id,
    ...
    owner_occurrence_id: body.owner_occurrence_id,
    scope_type: body.scope_type,
    scope_id: body.scope_id,
    value_slot_key: body.value_slot_key,
    ordinal: body.ordinal,
  });
  const expectedOccurrence = contentId(
    'METRIC_OBSERVATION_OCCURRENCE/V2',
    {
      ...
      ordinal: body.ordinal,
      comparability_class_digest:
        body.comparability_context?.comparability_class_digest,
    },
  );
  if (canonical_payload_digest !== contentId('MARKET_OBSERVATION_PAYLOAD/V2', body)
    || body.integration_admission_id
      !== QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID
    || body.claim_schema_version !== 'CLAIM_REVISION/V2'
    || body.metric_key !== 'NO_SHOP_COPY_DELIVERY_PERIOD_DAYS'
    || body.metric_slot_key !== expectedSlot
    || body.metric_observation_occurrence_id !== expectedOccurrence
    || body.canonical_value !== '1'
    || body.unit !== 'DAYS'
    || body.day_basis !== 'ELAPSED'
    || body.interpretation_payload_id
      !== body.comparability_context?.interpretation_payload_id
    || body.owner_revision_id == null
    || body.result_input_lineage_digest == null
    || body.comparability_context?.comparability_class_digest == null
    || servingKey !== contentId('MARKET_OBSERVATION/V2', {
      corpus_release_id: body.corpus_release_id,
      metric_observation_occurrence_id: body.metric_observation_occurrence_id,
    })) {
    throw new TypeError('offline interpreted observation identity has drifted');
  }
  return true;
}
