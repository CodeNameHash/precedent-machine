...
const PROCESS_PREDICATE_DEFINITION_CONTRACTS = Object.freeze({
  ...
  PROCESS_EXCLUSIVITY_PREDICATE_CATALOGUE: Object.freeze({
    ...
    expected_version: 2,
    definition_digest: '8c5e9f11fa67860c71c9b0ff1c8813763c4af76481ddc63631e5e670c46f4f9a',
...
function validateExactPredecessorContract(definition, code) {
  const predecessor = definition?.predecessor_contract;
  requireExactKeys(predecessor, [
    'catalogue_version',
    'definition_digest',
    'exact_raw_contract_bytes',
    'exact_raw_contract_bytes_digest',
    'exact_raw_contract_bytes_encoding',
    'schema_version',
    'stable_id',
  ], 'Process exclusivity predecessor contract', code);
  if (
    predecessor.stable_id !== 'PROCESS_EXCLUSIVITY_PREDICATE_CATALOGUE'
    || predecessor.schema_version !== PROCESS_PREDICATE_CONTRACT_INPUT_SCHEMA
    || predecessor.catalogue_version !== 1
    || predecessor.exact_raw_contract_bytes_encoding !== 'BASE64'
  ) {
    fail(code, 'The Process exclusivity predecessor identity is invalid.');
  }
  let rawBytes;
  let exactContract;
  try {
    rawBytes = Buffer.from(predecessor.exact_raw_contract_bytes, 'base64');
    if (rawBytes.toString('base64') !== predecessor.exact_raw_contract_bytes) {
      fail(code, 'The Process exclusivity predecessor bytes are not canonical base64.');
    }
    exactContract = JSON.parse(rawBytes.toString('utf8'));
  } catch (error) {
    fail(code, 'The exact Process exclusivity predecessor bytes are invalid.', {
      cause: error.message,
    });
  }
  const rawDigest = `sha256:${sha256Hex(rawBytes)}`;
  const definitionDigest = `sha256:${sha256Hex(Buffer.from(
    canonicalJson(exactContract.definition),
    'utf8',
  ))}`;
  if (
    rawDigest !== predecessor.exact_raw_contract_bytes_digest
    || definitionDigest !== predecessor.definition_digest
    || exactContract.object_kind !== PROCESS_PREDICATE_CONTRACT_INPUT_KIND
    || exactContract.stable_id !== predecessor.stable_id
    || exactContract.schema_version !== predecessor.schema_version
    || exactContract.definition?.catalogue_version !== 1
    || canonicalJson(
      exactContract.definition?.ordinary_question_contract
        ?.mandatory_predicate_keys,
    ) !== canonicalJson(
      definition.ordinary_question_contract.mandatory_predicate_keys.slice(0, 23),
    )
  ) {
    fail(code, 'The Process exclusivity predecessor bytes do not bind the 23 preserved predicates.');
  }
}
...
function validateAuthoredProcessPredicateInputs(authoredMembers) {
  ...
  for (const member of members) {
    ...
    if (value.stable_id === 'PROCESS_EXCLUSIVITY_PREDICATE_CATALOGUE') {
      validateExactPredecessorContract(value.definition, contract.error_code);
