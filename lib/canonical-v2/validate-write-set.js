const { canonicalJson, contentId } = require('./canonical-bytes');
const {
  moneyDenominatorPrecisionPolicyForClaim,
  validateContractBundle,
} = require('./contract-bundle');
const {
  validateExcerpt,
  validateFixtureSourceAdmission,
  validateImmutableSource,
} = require('./source-structure');
const { validateSharedServingRow } = require('./shared-serving-row');
const { validateValidatedDefinitionGraph } = require('./definition-graph');
const { buildAdmittedSourceReference } = require('./admitted-semantic-source');
const {
  validateClaimRevisionIdentity,
  validateRelationshipRevisionIdentity,
} = require('./claims-relationships');
const {
  PUBLICATION_DISPOSITIONS,
  validateCalibrationAuthority,
  validatePublicationDispositionV2,
} = require('./publication-disposition');

const WRITE_SET_KEYS = new Set([
  'source',
  'source_admission',
  'sources',
  'source_admissions',
  'deal',
  'excerpts',
  'provisions',
  'components',
  'condition_groups',
  'claims',
  'relationships',
  'validated_semantic_graphs',
  'open_world_candidates',
  'open_world_candidate_occurrences',
  'open_world_evidence_references',
  'open_world_candidate_dispositions',
  'open_world_primitives',
  'semantic_impact_closures',
  'reviewed_source_specific_rows',
  'incomplete_canonical_result_rows',
  'write_set_origin',
  'publication_dispositions',
  'publication_calibration_authorities',
]);
const CANONICAL_COLLECTION_KEYS = [
  'excerpts',
  'definition_occurrences',
  'provisions',
  'components',
  'claims',
  'relationships',
];
const CONDITION_GROUP_COLLECTION_KEYS = ['condition_groups'];
const SEMANTIC_GRAPH_COLLECTION_KEYS = ['validated_semantic_graphs'];
const OPEN_WORLD_COLLECTION_KEYS = [
  'open_world_candidates',
  'open_world_candidate_occurrences',
  'open_world_evidence_references',
  'open_world_candidate_dispositions',
  'open_world_primitives',
  'semantic_impact_closures',
  'reviewed_source_specific_rows',
  'incomplete_canonical_result_rows',
];
const COLLECTION_KEYS = [
  ...CANONICAL_COLLECTION_KEYS,
  ...CONDITION_GROUP_COLLECTION_KEYS,
  ...SEMANTIC_GRAPH_COLLECTION_KEYS,
  ...OPEN_WORLD_COLLECTION_KEYS,
];
const ADMITTED_SOURCE_REFERENCE_KEYS = Object.freeze([
  'schema_version',
  'immutable_source_document_id',
  'source_admission_manifest_id',
  'semantic_extraction_input_envelope_id',
  'canonical_text_id',
  'governed_deal_key',
  'deal_admission_id',
  'source_ordinal',
]);
const DEAL_SCOPE_WRITE_SET_KEYS = Object.freeze([
  'source_references',
  'deal',
  ...COLLECTION_KEYS,
]);
const RETAINED_OBJECT_REFERENCE_KEYS = Object.freeze([
  'schema_version',
  'object_kind',
  'object_id',
  'stored_closure_id',
  'canonical_payload_digest',
  'validation_closure_id',
]);
const ID_FIELDS = Object.freeze({
  excerpts: 'excerpt_id',
  definition_occurrences: 'definition_occurrence_id',
  provisions: 'provision_instance_id',
  components: 'provision_component_id',
  condition_groups: 'condition_group_revision_id',
  claims: 'claim_revision_id',
  relationships: 'relationship_revision_id',
  validated_semantic_graphs: 'validated_semantic_graph_id',
  open_world_candidates: 'candidate_id',
  open_world_candidate_occurrences: 'open_world_candidate_occurrence_id',
  open_world_evidence_references: 'evidence_reference_id',
  open_world_candidate_dispositions: 'final_disposition_id',
  open_world_primitives: 'primitive_id',
  semantic_impact_closures: 'semantic_impact_closure_id',
  reviewed_source_specific_rows: 'reviewed_source_specific_row_serving_key',
  incomplete_canonical_result_rows: 'incomplete_result_review_row_serving_key',
});
const PROVISION_INSTANCE_KEYS = Object.freeze([
  'schema_version',
  'source_occurrence_id',
  'canonical_text_id',
  'document_hash',
  'absolute_start',
  'absolute_end',
  'concept_key',
  'party',
  'ordinal',
  'source_anchor_id',
  'provision_instance_id',
  'closure_id',
]);
const STRUCTURAL_PROVISION_INSTANCE_KEYS = Object.freeze([
  'schema_version',
  'source_occurrence_id',
  'canonical_text_id',
  'document_hash',
  'absolute_start',
  'absolute_end',
  'concept_key',
  'ordinal',
  'source_anchor_id',
  'provision_instance_id',
  'closure_id',
]);
const PROVISION_COMPONENT_KEYS = Object.freeze([
  'schema_version',
  'parent_provision_instance_id',
  'canonical_text_id',
  'absolute_start',
  'absolute_end',
  'component_key',
  'ordinal',
  'source_anchor_id',
  'provision_component_id',
  'closure_id',
]);
const DEFINITION_OCCURRENCE_KEYS = Object.freeze([
  'schema_version',
  'definition_occurrence_id',
  'document_hash',
  'canonical_text_id',
  'exact_declaration_span',
  'ordered_body_spans',
  'neutral_definition_key',
  'governed_ordinal',
  'declaration_evidence_excerpt_id',
  'body_evidence_excerpt_ids',
  'closure_id',
]);
const CONDITION_GROUP_KEYS = Object.freeze([
  'schema_version',
  'document_hash',
  'parent_provision_instance_id',
  'canonical_text_id',
  'absolute_start',
  'absolute_end',
  'component_key',
  'party',
  'governed_ordinal',
  'condition_group_occurrence_id',
  'comparison_class_key',
  'source_clause_code',
  'target_limb_ordinals',
  'evidence_excerpt_id',
  'review_version',
  'condition_group_revision_id',
  'closure_id',
]);
// PLAN.md Step 4A3 (Ben's ruling, DECISIONS.md decision 3). The frozen
// 11-key contract `conditional-termination-fee-value.js`'s
// `buildConditionalTerminationFeeValue` builds and validates -- this is the
// JS-side mirror of the SQL shape check Step 4A2 added to
// `supabase/canonical-v2-foundation.sql` (`supplied_conditional_termination_
// fee_values`/`typed_conditional_termination_fee_values`), read from that
// same SQL rather than inferred. Unlike every CANONICAL_COLLECTION_KEYS row,
// this kind carries no `closure_id` -- the JS shape has none, so this file
// must not invent one (see `validateConditionalTerminationFeeValueRow`,
// which never expects it).
const CONDITIONAL_TERMINATION_FEE_VALUE_SCHEMA = 'CONDITIONAL_TERMINATION_FEE_VALUE/V1';
const CONDITIONAL_TERMINATION_FEE_VALUE_SIDES = Object.freeze(['SELLER', 'BUYER']);
const CONDITIONAL_TERMINATION_FEE_VALUE_BRANCHES = Object.freeze([
  '7.3(b)(i)', '7.3(b)(ii)', '7.3(b)(iii)', '7.3(b)(iv)', '7.3(b)(v)', '7.3(c)',
]);
const CONDITIONAL_TERMINATION_FEE_VALUE_KEYS = Object.freeze([
  'schema_version',
  'fee_side',
  'triggering_branch',
  'base_amount',
  'currency',
  'operator',
  'reit_limit_cap_term_ref',
  'defined_term_lineage',
  'source_citations',
  'raw_formula',
  'conditional_termination_fee_value_id',
]);
const SHA256_RE = /^[a-f0-9]{64}$/;
const STRUCTURAL_KEY_RE = /^[A-Z0-9][A-Z0-9_-]*$/;
const MAX_GOVERNED_ITEMS = 4096;
const MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS = 16384;

// Validator origin discriminator (docs/superpowers/specs/2026-08-01-claim-
// identity-provenance-design.md, "Pinned implementation decisions"): the
// write-set envelope MAY carry `write_set_origin`, set by the native
// producer's adapter (native-write-set-adapter.js). Reviewed-slice write
// sets never set it -- its absence is the untouched, pre-existing path.
const WRITE_SET_ORIGINS = Object.freeze(['NATIVE_PRODUCER', 'REVIEWED_SLICE']);

// Three-tag provenance model (spec section 5). `answer_provenance` lives at
// `row.attributes.answer_provenance` on `claims`/`relationships` rows (the
// only two canonical collections whose frozen identity contract -- see
// claims-relationships.js's CLAIM_REVISION_KEYS/RELATIONSHIP_REVISION_KEYS,
// which this file must never touch -- has room for it: inside the existing,
// schema-free `attributes` object, exactly like every other non-identity
// extra this pipeline already stores there).
const ANSWER_PROVENANCE_TAGS = Object.freeze(['MECHANICAL', 'AI', 'VERIFIED']);
const ANSWER_PROVENANCE_COLLECTION_KINDS = Object.freeze(['claims', 'relationships']);

function isNonEmptyString(value) {
  return typeof value === 'string' && value.length > 0;
}

// Pin-shape rules per tag (spec section 5's "Pins the rule and its version"/
// "Pins model, prompt version, skill/ruleset version"/"Pins reviewer, time,
// AND the source it was verified against"). The resolver itself never mints
// VERIFIED (only review flows do), but the shape is validated here whenever
// one is present, per the plan's Task 5 acceptance criteria.
function validateAnswerProvenancePins(tag, pins, label) {
  if (!pins || typeof pins !== 'object' || Array.isArray(pins)) {
    throw new CanonicalValidationError(`${label}.pins must be an object.`, { path: `${label}.pins` });
  }
  if (tag === 'MECHANICAL') {
    if (!Number.isSafeInteger(pins.mapping_table_version)) {
      throw new CanonicalValidationError(
        `${label}.pins.mapping_table_version is required for a MECHANICAL tag.`,
        { path: `${label}.pins.mapping_table_version` },
      );
    }
    return;
  }
  if (tag === 'AI') {
    if (!isNonEmptyString(pins.model_id) || !isNonEmptyString(pins.prompt_id) || !isNonEmptyString(pins.prompt_version)) {
      throw new CanonicalValidationError(
        `${label}.pins must carry model_id, prompt_id and prompt_version for an AI tag.`,
        { path: `${label}.pins` },
      );
    }
    return;
  }
  // VERIFIED (audit finding A5): pins the reviewer, when, and the exact
  // source it was verified against -- the canonical_text hash and evidence
  // excerpt id verified-pin-sweep.js compares on source re-admission.
  if (!isNonEmptyString(pins.reviewer) || !isNonEmptyString(pins.verified_at)
    || !isNonEmptyString(pins.canonical_text_hash) || !isNonEmptyString(pins.evidence_excerpt_id)) {
    throw new CanonicalValidationError(
      `${label}.pins must carry reviewer, verified_at, canonical_text_hash and evidence_excerpt_id for a VERIFIED tag.`,
      { path: `${label}.pins` },
    );
  }
}

// Validates ONE row's `attributes.answer_provenance`, staged per spec:
// validated whenever present; required only when `required` is true (the
// caller passes that only for `claims` rows on a NATIVE_PRODUCER-origin
// write set -- never for reviewed-slice/legacy rows, which stay untouched).
// The identity invariant ("tags cover answers, never identity") is absolute
// and NOT staged: a claims/relationships row already carries a registered
// claim_definition_key/relationship_definition_key -- its OWN identity --
// so an AI tag here is rejected unconditionally, whether or not the field
// is required at all.
function validateAnswerProvenanceRow({
  kind, row, objectId, required,
}) {
  const label = `writeSet.${kind}[${objectId}].attributes.answer_provenance`;
  const attributes = row && typeof row.attributes === 'object' && row.attributes !== null ? row.attributes : null;
  const provenance = attributes ? attributes.answer_provenance : undefined;
  if (provenance === undefined) {
    if (required) {
      throw new CanonicalValidationError(
        `${label} is required: this write set originates from the native producer (write_set_origin: 'NATIVE_PRODUCER').`,
        { path: label },
      );
    }
    return;
  }
  if (!provenance || typeof provenance !== 'object' || Array.isArray(provenance)
    || !ANSWER_PROVENANCE_TAGS.includes(provenance.tag)) {
    throw new CanonicalValidationError(`${label} must be a { tag, pins } object with a governed tag.`, { path: label });
  }
  if (provenance.tag === 'AI') {
    throw new CanonicalValidationError(
      `${label} may never carry an AI tag: which registered key a claim/relationship resolves to is never AI-assigned (spec section 5's invariant).`,
      { path: label },
    );
  }
  validateAnswerProvenancePins(provenance.tag, provenance.pins, label);
}

class CanonicalValidationError extends Error {
  constructor(message, details = {}) {
    super(message);
    this.name = 'CanonicalValidationError';
    this.code = 'CANONICAL_VALIDATION_ERROR';
    this.details = details;
  }
}

function canonicalise(value) {
  return JSON.parse(canonicalJson(value));
}

function nonEmptyString(value, path) {
  if (typeof value !== 'string' || !value.trim()) {
    throw new CanonicalValidationError(`${path} must be a non-empty string.`, { path });
  }
  return value.trim();
}

function digestId(value, path) {
  const id = nonEmptyString(value, path);
  if (!SHA256_RE.test(id)) {
    throw new CanonicalValidationError(`${path} must be a full SHA-256 content ID.`, { path });
  }
  return id;
}

function list(value, path) {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw new CanonicalValidationError(`${path} must be an array.`, { path });
  return value;
}

function boundedList(value, path) {
  const items = list(value, path);
  if (items.length > MAX_GOVERNED_ITEMS) {
    throw new CanonicalValidationError(`${path} exceeds the governed maximum.`, { path });
  }
  return items;
}

// PLAN.md Step 4A3. Validates one `conditional_termination_fee_values` row
// against the exact 11-key contract `buildConditionalTerminationFeeValue`
// (conditional-termination-fee-value.js) builds and would itself refuse --
// not inferred from a sibling collection, since this kind has none of the
// evidence/closure/party machinery every CANONICAL_COLLECTION_KEYS row
// carries. Mirrors `supabase/canonical-v2-foundation.sql`'s
// `typed_conditional_termination_fee_values` CTE (Step 4A2) field for field,
// including recomputing the row's own content-addressed identity rather
// than trusting the id it arrived with -- the same "never trust a supplied
// id" discipline every other kind in this file already applies. Returns the
// verified object id so the caller can detect duplicates the same way every
// other collection does.
function validateConditionalTerminationFeeValueRow(row, label) {
  if (!row || typeof row !== 'object' || Array.isArray(row)
    || canonicalJson(Object.keys(row).sort())
      !== canonicalJson([...CONDITIONAL_TERMINATION_FEE_VALUE_KEYS].sort())
    || row.schema_version !== CONDITIONAL_TERMINATION_FEE_VALUE_SCHEMA
    || !CONDITIONAL_TERMINATION_FEE_VALUE_SIDES.includes(row.fee_side)
    || !CONDITIONAL_TERMINATION_FEE_VALUE_BRANCHES.includes(row.triggering_branch)
    || typeof row.base_amount !== 'string' || !/^\d+(?:\.\d+)?$/.test(row.base_amount)
    || row.currency !== 'USD'
    || row.operator !== 'LOWER_OF'
    || typeof row.reit_limit_cap_term_ref !== 'string' || row.reit_limit_cap_term_ref.length === 0
    || !Array.isArray(row.defined_term_lineage) || row.defined_term_lineage.length < 2
    || row.defined_term_lineage.some((entry) => typeof entry !== 'string' || entry.length === 0)
    || !Array.isArray(row.source_citations) || row.source_citations.length < 2
    || row.source_citations.some((entry) => typeof entry !== 'string' || entry.length === 0)
    || typeof row.raw_formula !== 'string' || row.raw_formula.length === 0) {
    throw new CanonicalValidationError(
      `${label} conditional termination fee value shape is invalid.`,
      { path: label },
    );
  }
  const expectedId = contentId(CONDITIONAL_TERMINATION_FEE_VALUE_SCHEMA, {
    schema_version: row.schema_version,
    fee_side: row.fee_side,
    triggering_branch: row.triggering_branch,
    base_amount: row.base_amount,
    currency: row.currency,
    operator: row.operator,
    reit_limit_cap_term_ref: row.reit_limit_cap_term_ref,
    defined_term_lineage: row.defined_term_lineage,
    source_citations: row.source_citations,
    raw_formula: row.raw_formula,
  });
  const objectId = digestId(
    row.conditional_termination_fee_value_id,
    `${label}.conditional_termination_fee_value_id`,
  );
  if (objectId !== expectedId) {
    throw new CanonicalValidationError(
      `${label} conditional termination fee value identity does not match its content.`,
      { path: label },
    );
  }
  return objectId;
}

function assertSemanticGraphRequestBudget(rows) {
  let memberCount = 0;
  let identifierCount = 0;
  for (const [index, row] of rows.entries()) {
    if (!row || typeof row !== 'object' || Array.isArray(row)
      || !Array.isArray(row.definition_cue_ids)
      || !Array.isArray(row.definition_use_cue_ids)
      || !Array.isArray(row.definition_cues)
      || !Array.isArray(row.definition_use_cues)
      || row.definition_cue_ids.length > MAX_GOVERNED_ITEMS
      || row.definition_use_cue_ids.length > MAX_GOVERNED_ITEMS
      || row.definition_cues.length > MAX_GOVERNED_ITEMS
      || row.definition_use_cues.length > MAX_GOVERNED_ITEMS) {
      throw new CanonicalValidationError(
        'Validated semantic graph request has an invalid or unbounded child collection.',
        { path: `writeSet.validated_semantic_graphs[${index}]` },
      );
    }
    memberCount += row.definition_cues.length + row.definition_use_cues.length;
    identifierCount += row.definition_cue_ids.length + row.definition_use_cue_ids.length;
    if (memberCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS) break;
    for (const [cueIndex, cue] of row.definition_cues.entries()) {
      if (!cue || typeof cue !== 'object' || Array.isArray(cue)
        || !Array.isArray(cue.body_spans)
        || !Array.isArray(cue.body_span_ids)
        || cue.body_spans.length > MAX_GOVERNED_ITEMS
        || cue.body_span_ids.length > MAX_GOVERNED_ITEMS) {
        throw new CanonicalValidationError(
          'Validated semantic graph request has an invalid or unbounded body collection.',
          {
            path: `writeSet.validated_semantic_graphs[${index}].definition_cues[${cueIndex}]`,
          },
        );
      }
      memberCount += cue.body_spans.length;
      identifierCount += cue.body_span_ids.length;
      if (memberCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS
        || identifierCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS) break;
    }
    if (memberCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS
      || identifierCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS) break;
  }
  if (memberCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS) {
    throw new CanonicalValidationError(
      'Validated semantic graph request exceeds the governed nested-member maximum.',
      { path: 'writeSet.validated_semantic_graphs', memberCount },
    );
  }
  if (identifierCount > MAX_SEMANTIC_GRAPH_REQUEST_MEMBERS) {
    throw new CanonicalValidationError(
      'Validated semantic graph request exceeds the governed identifier-member maximum.',
      { path: 'writeSet.validated_semantic_graphs', identifierCount },
    );
  }
}

function sourceSet(writeSet) {
  const hasSingular = writeSet.source !== undefined || writeSet.source_admission !== undefined;
  const hasPlural = writeSet.sources !== undefined || writeSet.source_admissions !== undefined;
  if (hasSingular === hasPlural) {
    throw new CanonicalValidationError(
      'writeSet must use exactly one complete singular or plural source contract.',
      { path: 'writeSet' },
    );
  }
  if (hasSingular) {
    if (!writeSet.source || !writeSet.source_admission) {
      throw new CanonicalValidationError('writeSet.source and writeSet.source_admission are both required.', {
        path: 'writeSet',
      });
    }
    return { mode: 'SINGULAR', sources: [writeSet.source], admissions: [writeSet.source_admission] };
  }
  const sources = list(writeSet.sources, 'writeSet.sources');
  const admissions = list(writeSet.source_admissions, 'writeSet.source_admissions');
  if (sources.length === 0 || admissions.length === 0 || sources.length !== admissions.length) {
    throw new CanonicalValidationError(
      'writeSet.sources and writeSet.source_admissions must be non-empty arrays of equal length.',
      { path: 'writeSet' },
    );
  }
  return { mode: 'PLURAL', sources, admissions };
}

function contractVocabulary(contractBundle) {
  validateContractBundle(contractBundle);
  return {
    concepts: new Set(contractBundle.concepts.map((entry) => entry.concept_key)),
    components: new Set(contractBundle.component_definitions.map((entry) => entry.component_key)),
    claims: new Map(contractBundle.claim_definitions.map((entry) => [entry.claim_definition_key, entry])),
    relationships: new Map(contractBundle.relationship_definitions.map((entry) => [entry.relationship_key, entry])),
    capitalisationSemanticSchema: contractBundle.capitalisation_semantic_schema_definition || null,
    noShopSemanticSchemas: new Map(
      (contractBundle.no_shop_semantic_schema_definitions || []).map(
        (entry) => [entry.semantic_schema_key, entry],
      ),
    ),
    states: new Set(contractBundle.claim_states),
    residualReasons: new Set(contractBundle.residual_reason_codes),
    moneyDenominatorPrecisionPolicyForClaim: (claimDefinitionKey) => (
      moneyDenominatorPrecisionPolicyForClaim(contractBundle, claimDefinitionKey)
    ),
  };
}

function assertWriteSetShape(writeSet, contractBundle) {
  if (!writeSet || typeof writeSet !== 'object' || Array.isArray(writeSet)) {
    throw new CanonicalValidationError('writeSet must be an object.', { path: 'writeSet' });
  }
  const unknownKeys = Object.keys(writeSet).filter((key) => !WRITE_SET_KEYS.has(key));
  if (unknownKeys.length) {
    throw new CanonicalValidationError('writeSet contains keys outside the fixed contract.', {
      path: 'writeSet',
      unknownKeys: unknownKeys.sort(),
    });
  }
  if (!writeSet.deal || typeof writeSet.deal !== 'object' || Array.isArray(writeSet.deal)) {
    throw new CanonicalValidationError('writeSet.deal must be an object.', { path: 'writeSet.deal' });
  }
  const admittedSources = sourceSet(writeSet);
  nonEmptyString(writeSet.deal.deal_key, 'writeSet.deal.deal_key');
  digestId(writeSet.deal.deal_admission_id, 'writeSet.deal.deal_admission_id');
  digestId(writeSet.deal.document_hash, 'writeSet.deal.document_hash');
  const sourceIds = new Set();
  const canonicalTextIds = new Set();
  const sourceOrdinals = new Set();
  const sourceById = new Map();
  for (const [index, source] of admittedSources.sources.entries()) {
    validateImmutableSource(source);
    if (sourceIds.has(source.immutable_source_document_id) || canonicalTextIds.has(source.canonical_text_id)) {
      throw new CanonicalValidationError('writeSet contains a duplicate immutable source identity.', {
        path: `writeSet.sources[${index}]`,
      });
    }
    sourceIds.add(source.immutable_source_document_id);
    canonicalTextIds.add(source.canonical_text_id);
    sourceById.set(source.immutable_source_document_id, source);
  }
  for (const [index, admission] of admittedSources.admissions.entries()) {
    const source = sourceById.get(admission?.immutable_source_document_id);
    if (!source) {
      throw new CanonicalValidationError('source admission does not name an immutable source in this write set.', {
        path: `writeSet.source_admissions[${index}]`,
      });
    }
    if (sourceOrdinals.has(admission.source_ordinal)) {
      throw new CanonicalValidationError('source admissions must have unique governed source ordinals.', {
        path: `writeSet.source_admissions[${index}].source_ordinal`,
      });
    }
    sourceOrdinals.add(admission.source_ordinal);
    validateFixtureSourceAdmission({
      source,
      admission,
      dealKey: writeSet.deal.deal_key,
      dealAdmissionId: writeSet.deal.deal_admission_id,
      contractFingerprint: contractBundle.fingerprint,
    });
  }
  if (!admittedSources.sources.some((source) => writeSet.deal.document_hash === source.document_hash)) {
    throw new CanonicalValidationError('writeSet.deal.document_hash must name an admitted immutable source.', {
      path: 'writeSet.deal.document_hash',
    });
  }
  for (const key of COLLECTION_KEYS) boundedList(writeSet[key], `writeSet.${key}`);
  return {
    ...admittedSources,
    byCanonicalTextId: new Map(admittedSources.sources.map((source) => [source.canonical_text_id, source])),
  };
}

function assertResolvedWriteSetShape(writeSet, admittedSourceContexts, contractBundle) {
  const actualKeys = Object.keys(writeSet).sort();
  const allowsLegacyConditionGroups =
    contractBundle.capitalisation_semantic_schema_definition == null;
  const baseKeys = [...DEAL_SCOPE_WRITE_SET_KEYS];
  const legacyBaseKeys = DEAL_SCOPE_WRITE_SET_KEYS.filter((key) => key !== 'condition_groups');
  // Three INDEPENDENTLY optional keys: `persisted_object_references` (pre-
  // existing), `write_set_origin` (the staged validator discriminator), and
  // `conditional_termination_fee_values` (PLAN.md Step 4A3, Ben's ruling in
  // DECISIONS.md decision 3 -- "OPTIONAL, not required" the same way
  // `supabase/canonical-v2-foundation.sql`'s Step 4A2 OPTIONAL-key
  // subtraction list treats it: most deals never produce this kind). A
  // caller may supply any subset of the three, so every combination is a
  // distinct allowed key set -- generated rather than hand-enumerated, so
  // this stays correct as more optional keys are ever added.
  const optionalKeys = ['persisted_object_references', 'write_set_origin', 'conditional_termination_fee_values', 'publication_dispositions', 'publication_calibration_authorities'];
  const allowedKeySetJson = new Set();
  for (const keys of [baseKeys, ...(allowsLegacyConditionGroups ? [legacyBaseKeys] : [])]) {
    for (let mask = 0; mask < (1 << optionalKeys.length); mask += 1) {
      const variant = [...keys];
      for (let bit = 0; bit < optionalKeys.length; bit += 1) {
        if (mask & (1 << bit)) variant.push(optionalKeys[bit]);
      }
      allowedKeySetJson.add(canonicalJson(variant.sort()));
    }
  }
  if (!writeSet || typeof writeSet !== 'object' || Array.isArray(writeSet)
    || !allowedKeySetJson.has(canonicalJson(actualKeys))) {
    throw new CanonicalValidationError('DEAL_SCOPE_RUN writeSet fields do not match the reference-only contract.', {
      path: 'writeSet',
    });
  }
  if (writeSet.write_set_origin !== undefined && !WRITE_SET_ORIGINS.includes(writeSet.write_set_origin)) {
    throw new CanonicalValidationError('writeSet.write_set_origin must be NATIVE_PRODUCER or REVIEWED_SLICE when present.', {
      path: 'writeSet.write_set_origin',
    });
  }
  if (!Array.isArray(admittedSourceContexts)
    || admittedSourceContexts.length === 0
    || !Array.isArray(writeSet.source_references)
    || writeSet.source_references.length !== admittedSourceContexts.length) {
    throw new CanonicalValidationError(
      'DEAL_SCOPE_RUN requires one resolved admitted semantic context per source reference.',
      { path: 'writeSet.source_references' },
    );
  }
  if (!writeSet.deal || typeof writeSet.deal !== 'object' || Array.isArray(writeSet.deal)) {
    throw new CanonicalValidationError('writeSet.deal must be an object.', { path: 'writeSet.deal' });
  }
  const dealKey = nonEmptyString(writeSet.deal.deal_key, 'writeSet.deal.deal_key');
  const dealAdmissionId = digestId(
    writeSet.deal.deal_admission_id,
    'writeSet.deal.deal_admission_id',
  );
  const documentHash = digestId(writeSet.deal.document_hash, 'writeSet.deal.document_hash');
  for (const key of COLLECTION_KEYS) boundedList(writeSet[key], `writeSet.${key}`);
  boundedList(
    writeSet.persisted_object_references,
    'writeSet.persisted_object_references',
  );
  boundedList(
    writeSet.conditional_termination_fee_values,
    'writeSet.conditional_termination_fee_values',
  );
  boundedList(writeSet.publication_dispositions, 'writeSet.publication_dispositions');
  boundedList(writeSet.publication_calibration_authorities, 'writeSet.publication_calibration_authorities');

  const sourceIds = new Set();
  const canonicalTextIds = new Set();
  const sourceOrdinals = new Set();
  for (const [index, reference] of writeSet.source_references.entries()) {
    const context = admittedSourceContexts[index];
    if (!reference || typeof reference !== 'object' || Array.isArray(reference)
      || canonicalJson(Object.keys(reference).sort())
        !== canonicalJson([...ADMITTED_SOURCE_REFERENCE_KEYS].sort())
      || reference.schema_version !== 'ADMITTED_SOURCE_REFERENCE/V1') {
      throw new CanonicalValidationError('source reference fields do not match the frozen contract.', {
        path: `writeSet.source_references[${index}]`,
      });
    }
    if (!context || context.schema_version !== 'ADMITTED_SEMANTIC_SOURCE_CONTEXT/V1') {
      throw new CanonicalValidationError('source reference did not resolve to a prevalidated semantic context.', {
        path: `admittedSourceContexts[${index}]`,
      });
    }
    if (canonicalJson(buildAdmittedSourceReference(context)) !== canonicalJson(reference)) {
      throw new CanonicalValidationError('prevalidated semantic context identity does not match its source reference.', {
        path: `admittedSourceContexts[${index}]`,
      });
    }
    for (const field of ADMITTED_SOURCE_REFERENCE_KEYS.slice(1)) {
      if (reference[field] !== context[field]) {
        throw new CanonicalValidationError('resolved semantic context does not match its source reference.', {
          path: `writeSet.source_references[${index}].${field}`,
        });
      }
    }
    if (reference.governed_deal_key !== dealKey
      || reference.deal_admission_id !== dealAdmissionId) {
      throw new CanonicalValidationError('source references must belong to the authoritative deal admission.', {
        path: `writeSet.source_references[${index}]`,
      });
    }
    if (!Number.isInteger(reference.source_ordinal) || reference.source_ordinal < 0
      || sourceOrdinals.has(reference.source_ordinal)) {
      throw new CanonicalValidationError('source references require unique non-negative source ordinals.', {
        path: `writeSet.source_references[${index}].source_ordinal`,
      });
    }
    for (const [value, seen, label] of [
      [reference.immutable_source_document_id, sourceIds, 'immutable source'],
      [reference.canonical_text_id, canonicalTextIds, 'canonical text'],
    ]) {
      digestId(value, `writeSet.source_references[${index}].${label}`);
      if (seen.has(value)) {
        throw new CanonicalValidationError(`writeSet contains a duplicate ${label} identity.`, {
          path: `writeSet.source_references[${index}]`,
        });
      }
      seen.add(value);
    }
    sourceOrdinals.add(reference.source_ordinal);
  }
  if (!admittedSourceContexts.some((source) => source.document_hash === documentHash)) {
    throw new CanonicalValidationError('writeSet.deal.document_hash must name an admitted immutable source.', {
      path: 'writeSet.deal.document_hash',
    });
  }
  return {
    mode: 'REFERENCES',
    sources: admittedSourceContexts,
    sourceReferences: writeSet.source_references,
    byCanonicalTextId: new Map(admittedSourceContexts.map((source) => [
      source.canonical_text_id,
      source,
    ])),
    sourceOrdinalByCanonicalTextId: new Map(admittedSourceContexts.map((source) => [
      source.canonical_text_id,
      source.source_ordinal,
    ])),
  };
}

function materialiseRetainedEntries(writeSet, retainedCanonicalObjects) {
  const references = list(
    writeSet.persisted_object_references,
    'writeSet.persisted_object_references',
  );
  if (references.length === 0) return [];
  if (!Array.isArray(retainedCanonicalObjects)
    || retainedCanonicalObjects.length !== references.length) {
    throw new CanonicalValidationError(
      'Every retained canonical object reference must resolve exactly once.',
      { path: 'retainedCanonicalObjects' },
    );
  }
  const seen = new Set();
  return references.map((reference, index) => {
    if (!reference || typeof reference !== 'object' || Array.isArray(reference)
      || canonicalJson(Object.keys(reference).sort())
        !== canonicalJson([...RETAINED_OBJECT_REFERENCE_KEYS].sort())
      || reference.schema_version !== 'PERSISTED_CANONICAL_OBJECT_REFERENCE/V1'
      || !COLLECTION_KEYS.includes(reference.object_kind)) {
      throw new CanonicalValidationError(
        'Retained canonical object reference fields do not match the frozen contract.',
        { path: `writeSet.persisted_object_references[${index}]` },
      );
    }
    const objectId = digestId(
      reference.object_id,
      `writeSet.persisted_object_references[${index}].object_id`,
    );
    const validationClosureId = digestId(
      reference.validation_closure_id,
      `writeSet.persisted_object_references[${index}].validation_closure_id`,
    );
    const storedClosureId = digestId(
      reference.stored_closure_id,
      `writeSet.persisted_object_references[${index}].stored_closure_id`,
    );
    const canonicalPayloadDigest = digestId(
      reference.canonical_payload_digest,
      `writeSet.persisted_object_references[${index}].canonical_payload_digest`,
    );
    const resolved = retainedCanonicalObjects[index];
    if (!resolved
      || resolved.object_kind !== reference.object_kind
      || resolved.object_id !== objectId
      || resolved.closure_id !== storedClosureId
      || resolved.canonical_payload_digest !== canonicalPayloadDigest
      || !resolved.canonical_payload
      || resolved.canonical_payload[ID_FIELDS[reference.object_kind]] !== objectId
      || resolved.canonical_payload.closure_id !== storedClosureId) {
      throw new CanonicalValidationError(
        'Retained canonical object reference did not resolve to exact stored content.',
        { path: `retainedCanonicalObjects[${index}]` },
      );
    }
    const key = `${reference.object_kind}:${objectId}`;
    if (seen.has(key)) {
      throw new CanonicalValidationError(
        'Retained canonical object references must be unique.',
        { path: `writeSet.persisted_object_references[${index}]` },
      );
    }
    seen.add(key);
    return {
      kind: reference.object_kind,
      row: canonicalise({
        ...resolved.canonical_payload,
        closure_id: validationClosureId,
      }),
      objectId,
      closureId: validationClosureId,
      storedClosureId,
      retained: true,
    };
  });
}

function completeScope(scope) {
  if (!scope || scope.coverage_status !== 'COMPLETE' || !SHA256_RE.test(scope.scope_closure_id || '')) return false;
  if (!Array.isArray(scope.required_interval_ids)
    || !Array.isArray(scope.examined_interval_ids)
    || scope.required_interval_ids.length > MAX_GOVERNED_ITEMS
    || scope.examined_interval_ids.length > MAX_GOVERNED_ITEMS) {
    return false;
  }
  const required = [...scope.required_interval_ids].sort();
  const examined = [...scope.examined_interval_ids].sort();
  return required.length > 0
    && required.every((id) => SHA256_RE.test(id))
    && examined.every((id) => SHA256_RE.test(id))
    && canonicalJson(required) === canonicalJson(examined);
}

function hasAssertedValue(row) {
  return ['raw_value', 'canonical_value', 'unit', 'day_basis', 'denominator']
    .some((key) => row[key] !== undefined && row[key] !== null);
}

function hasTypedStateDetail(row, state) {
  if (state === 'NOT_APPLICABLE') {
    return typeof row.applicability?.rule === 'string'
      && row.applicability.rule.length > 0
      && Array.isArray(row.applicability.source_fact_ids)
      && row.applicability.source_fact_ids.length > 0
      && row.applicability.source_fact_ids.length <= MAX_GOVERNED_ITEMS
      && row.applicability.source_fact_ids.every((id) => SHA256_RE.test(id));
  }
  if (state === 'NOT_EXAMINED') {
    return typeof row.not_examined?.reason === 'string'
      && row.not_examined.reason.length > 0
      && row.not_examined.intended_scope != null;
  }
  if (state === 'FAILED') {
    return typeof row.failure?.failure_code === 'string'
      && row.failure.failure_code.length > 0
      && typeof row.failure.attempted_extractor === 'string'
      && row.failure.attempted_extractor.length > 0;
  }
  return true;
}

function pick(row, fields) {
  return Object.fromEntries(fields.map((field) => [field, row[field]]));
}

function canonicalValueAllowed(definition, value) {
  if (Array.isArray(definition.allowed_canonical_values)) {
    return definition.allowed_canonical_values.some(
      (allowed) => canonicalJson(allowed) === canonicalJson(value),
    );
  }
  if (definition.canonical_value_type === 'NON_NEGATIVE_DECIMAL_STRING') {
    return typeof value === 'string' && /^(0|[1-9]\d*)(\.\d+)?$/.test(value);
  }
  if (definition.canonical_value_type === 'ISO_8601_DATE_STRING') {
    return typeof value === 'string'
      && /^\d{4}-\d{2}-\d{2}$/.test(value)
      && Number.isFinite(Date.parse(`${value}T00:00:00.000Z`));
  }
  if (definition.canonical_value_type === 'SCHEDULE_REFERENCE_STRING') {
    // P2 section 3: three-place lockstep, byte-identical regex.
    return typeof value === 'string' && /^\d+\.\d+(\([0-9A-Za-z]+\))*@DISCLOSURE_LETTER$/.test(value);
  }
  return false;
}

function assertStructuralRowShape(kind, row, source, label) {
  const partylessProvision = kind === 'provisions'
    && row.schema_version === 'STRUCTURAL_PROVISION_INSTANCE/V1';
  const keys = kind === 'provisions'
    ? (partylessProvision ? STRUCTURAL_PROVISION_INSTANCE_KEYS : PROVISION_INSTANCE_KEYS)
    : PROVISION_COMPONENT_KEYS;
  const schemaVersion = kind === 'provisions'
    ? (partylessProvision ? 'STRUCTURAL_PROVISION_INSTANCE/V1' : 'PROVISION_INSTANCE/V1')
    : 'PROVISION_COMPONENT/V1';
  if (canonicalJson(Object.keys(row).sort()) !== canonicalJson([...keys].sort())
    || row.schema_version !== schemaVersion) {
    throw new CanonicalValidationError(`${label} fields do not match the structural write contract.`, {
      path: label,
    });
  }
  for (const field of [
    ...(kind === 'provisions'
      ? ['source_occurrence_id', 'document_hash', 'provision_instance_id']
      : ['parent_provision_instance_id', 'provision_component_id']),
    'canonical_text_id',
    'source_anchor_id',
    'closure_id',
  ]) digestId(row[field], `${label}.${field}`);
  if (!Number.isSafeInteger(row.absolute_start)
    || !Number.isSafeInteger(row.absolute_end)
    || row.absolute_start < 0
    || row.absolute_end < row.absolute_start
    || (source && row.absolute_end > source.canonical_text_byte_length)
    || !Number.isSafeInteger(row.ordinal)
    || row.ordinal < 1) {
    throw new CanonicalValidationError(`${label} has invalid offsets or ordinal.`, { path: label });
  }
  if (kind === 'provisions') {
    nonEmptyString(row.concept_key, `${label}.concept_key`);
    if (!STRUCTURAL_KEY_RE.test(row.concept_key)) {
      throw new CanonicalValidationError(`${label}.concept_key must be a governed ASCII token.`, {
        path: `${label}.concept_key`,
      });
    }
    if (!partylessProvision && (!row.party || typeof row.party !== 'object' || Array.isArray(row.party)
      || canonicalJson(Object.keys(row.party).sort())
        !== canonicalJson(['capacity', 'role', 'value'])
      || ['role', 'value', 'capacity'].some(
        (field) => typeof row.party[field] !== 'string' || row.party[field].length === 0,
      ))) {
      throw new CanonicalValidationError(`${label}.party must match its closed contract.`, {
        path: `${label}.party`,
      });
    }
  } else {
    nonEmptyString(row.component_key, `${label}.component_key`);
    if (!STRUCTURAL_KEY_RE.test(row.component_key)) {
      throw new CanonicalValidationError(`${label}.component_key must be a governed ASCII token.`, {
        path: `${label}.component_key`,
      });
    }
  }
}

function expectedObjectId(kind, row, source) {
  if (kind === 'excerpts') {
    try {
      validateExcerpt({ source, excerpt: row });
      return row.excerpt_id;
    } catch {
      return null;
    }
  }
  if (kind === 'provisions') {
    const partylessProvision = row.schema_version === 'STRUCTURAL_PROVISION_INSTANCE/V1';
    const payload = pick(row, [
      'schema_version',
      'source_occurrence_id',
      'canonical_text_id',
      'document_hash',
      'absolute_start',
      'absolute_end',
      'concept_key',
      ...(!partylessProvision ? ['party'] : []),
      'ordinal',
    ]);
    const spanPayload = {
      schema_version: 'SEMANTIC_SPAN/V1',
      canonical_text_id: row.canonical_text_id,
      absolute_start: row.absolute_start,
      absolute_end: row.absolute_end,
    };
    if (row.source_anchor_id !== contentId('SEMANTIC_SPAN/V1', spanPayload)) return null;
    return contentId(partylessProvision
      ? 'STRUCTURAL_PROVISION_INSTANCE/V1'
      : 'PROVISION_INSTANCE/V1', payload);
  }
  if (kind === 'components') {
    const payload = pick(row, [
      'schema_version',
      'parent_provision_instance_id',
      'canonical_text_id',
      'absolute_start',
      'absolute_end',
      'component_key',
      'ordinal',
    ]);
    const spanPayload = {
      schema_version: 'SEMANTIC_SPAN/V1',
      canonical_text_id: row.canonical_text_id,
      absolute_start: row.absolute_start,
      absolute_end: row.absolute_end,
    };
    if (row.source_anchor_id !== contentId('SEMANTIC_SPAN/V1', spanPayload)) return null;
    return contentId('PROVISION_COMPONENT/V1', payload);
  }
  if (kind === 'definition_occurrences') {
    return row.definition_occurrence_id;
  }
  if (kind === 'claims') {
    const expectedOccurrenceId = contentId('CLAIM_OCCURRENCE/V1', {
      subject_occurrence_id: row.subject_occurrence_id,
      claim_definition_key: row.claim_definition_key,
      claim_definition_version: row.claim_definition_version,
      ordinal: row.ordinal,
    });
    if (row.claim_occurrence_id !== expectedOccurrenceId) return null;
    const payload = pick(row, [
      'claim_occurrence_id',
      'subject_occurrence_id',
      'claim_definition_key',
      'claim_definition_version',
      'ordinal',
      'state',
      'raw_value',
      'canonical_value',
      'unit',
      'day_basis',
      'denominator',
      'scope',
      'applicability',
      'not_examined',
      'failure',
      'evidence_ids',
      'attributes',
      'taxonomy_codes',
      'extraction_version',
      'normalisation_version',
      'derivation_version',
    ]);
    return contentId('CLAIM_REVISION/V1', payload);
  }
  const expectedOccurrenceId = contentId('RELATIONSHIP_OCCURRENCE/V1', {
    source_occurrence_id: row.source_occurrence_id,
    relationship_definition_key: row.relationship_definition_key,
    relationship_definition_version: row.relationship_definition_version,
    ordinal: row.ordinal,
  });
  if (row.relationship_occurrence_id !== expectedOccurrenceId) return null;
  const payload = pick(row, [
    'relationship_occurrence_id',
    'source_occurrence_id',
    'relationship_definition_key',
    'relationship_definition_version',
    'ordinal',
    'state',
    'raw_scope',
    'scope',
    'applicability',
    'not_examined',
    'failure',
    'target_occurrence_ids',
    'effect',
    'evidence_ids',
    'attributes',
    'taxonomy_codes',
    'resolver_version',
  ]);
  return contentId('RELATIONSHIP_REVISION/V1', payload);
}

function validateConditionGroupRows({ rows, admittedSources, vocabulary, indexed }) {
  if (rows.length === 0) return;
  const semantic = vocabulary.capitalisationSemanticSchema;
  if (!semantic
    || semantic.condition_group_component_schema !== 'CAPITALISATION_CONDITION_GROUP/V1'
    || !Array.isArray(semantic.condition_group_contracts)
    || !semantic.party_contract?.result_party) {
    throw new CanonicalValidationError(
      'condition_groups require the frozen capitalisation semantic contract.',
      { path: 'writeSet.condition_groups' },
    );
  }
  if (rows.length !== semantic.condition_group_contracts.length) {
    throw new CanonicalValidationError(
      'condition_groups must contain every frozen capitalisation condition group exactly once.',
      { path: 'writeSet.condition_groups' },
    );
  }
  const provisions = new Map(indexed
    .filter((entry) => entry.kind === 'provisions')
    .map((entry) => [entry.objectId, entry]));
  const excerpts = new Map(indexed
    .filter((entry) => entry.kind === 'excerpts')
    .map((entry) => [entry.objectId, entry]));
  const closureIds = new Set();
  const identities = new Set();
  for (const [index, row] of rows.entries()) {
    const label = `writeSet.condition_groups[${index}]`;
    if (!row || typeof row !== 'object' || Array.isArray(row)
      || canonicalJson(Object.keys(row).sort())
        !== canonicalJson([...CONDITION_GROUP_KEYS].sort())
      || row.schema_version !== 'CAPITALISATION_CONDITION_GROUP/V1') {
      throw new CanonicalValidationError(
        `${label} fields do not match the condition-group structural write contract.`,
        { path: label },
      );
    }
    for (const field of [
      'document_hash', 'parent_provision_instance_id', 'canonical_text_id',
      'condition_group_occurrence_id', 'evidence_excerpt_id',
      'condition_group_revision_id', 'closure_id',
    ]) digestId(row[field], `${label}.${field}`);
    if (!Number.isSafeInteger(row.absolute_start)
      || !Number.isSafeInteger(row.absolute_end)
      || row.absolute_start < 0
      || row.absolute_end <= row.absolute_start
      || !Number.isSafeInteger(row.governed_ordinal)
      || row.governed_ordinal < 1) {
      throw new CanonicalValidationError(`${label} has invalid offsets or ordinal.`, { path: label });
    }
    const source = admittedSources.byCanonicalTextId.get(row.canonical_text_id);
    if (!source
      || row.document_hash !== source.document_hash
      || row.absolute_end > source.canonical_text_byte_length) {
      throw new CanonicalValidationError(`${label} does not name one admitted source document.`, {
        path: label,
      });
    }
    const contract = semantic.condition_group_contracts[index];
    if (row.component_key !== semantic.condition_group_component_key
      || row.comparison_class_key !== contract.comparison_class_key
      || row.source_clause_code !== contract.source_clause_code
      || canonicalJson(row.target_limb_ordinals)
        !== canonicalJson(contract.required_limb_ordinals)
      || row.governed_ordinal !== index + 1
      || row.review_version !== 'QXO_CAPITALISATION_BRING_DOWN_F27/V1'
      || canonicalJson(row.party) !== canonicalJson(semantic.party_contract.result_party)) {
      throw new CanonicalValidationError(`${label} is outside the governed condition-group contract.`, {
        path: label,
      });
    }
    const parent = provisions.get(row.parent_provision_instance_id);
    const evidence = excerpts.get(row.evidence_excerpt_id);
    if (!parent
      || parent.row.concept_key !== semantic.condition_concept_key
      || canonicalJson(parent.row.party) !== canonicalJson(semantic.party_contract.result_party)
      || parent.row.canonical_text_id !== row.canonical_text_id
      || parent.row.document_hash !== row.document_hash
      || parent.closureId !== row.closure_id
      || row.absolute_start < parent.row.absolute_start
      || row.absolute_end > parent.row.absolute_end
      || !evidence
      || evidence.row.canonical_text_id !== row.canonical_text_id
      || evidence.row.document_hash !== row.document_hash
      || evidence.closureId !== row.closure_id
      || evidence.row.absolute_start !== row.absolute_start
      || evidence.row.absolute_end !== row.absolute_end) {
      throw new CanonicalValidationError(`${label} does not resolve to its governed parent and evidence.`, {
        path: label,
      });
    }
    const occurrenceBody = pick(row, [
      'document_hash', 'parent_provision_instance_id', 'canonical_text_id',
      'absolute_start', 'absolute_end', 'component_key', 'party', 'governed_ordinal',
    ]);
    const expectedOccurrenceId = contentId(
      'CAPITALISATION_CONDITION_GROUP_OCCURRENCE/V1',
      occurrenceBody,
    );
    const revisionBody = pick(row, [
      'condition_group_occurrence_id', 'comparison_class_key', 'source_clause_code',
      'target_limb_ordinals', 'evidence_excerpt_id', 'review_version',
    ]);
    const expectedRevisionId = contentId(
      'CAPITALISATION_CONDITION_GROUP_REVISION/V1',
      revisionBody,
    );
    if (row.condition_group_occurrence_id !== expectedOccurrenceId
      || row.condition_group_revision_id !== expectedRevisionId) {
      throw new CanonicalValidationError(`${label} has a forged condition-group identity.`, {
        path: label,
      });
    }
    if (identities.has(row.condition_group_revision_id)) {
      throw new CanonicalValidationError('condition_groups must not repeat a revision identity.', {
        path: label,
      });
    }
    identities.add(row.condition_group_revision_id);
    closureIds.add(row.closure_id);
  }
  if (closureIds.size !== 1) {
    throw new CanonicalValidationError('condition_groups must close as one immutable condition-group set.', {
      path: 'writeSet.condition_groups',
    });
  }
}

function definitionSpan(span, label) {
  if (!span || typeof span !== 'object' || Array.isArray(span)
    || canonicalJson(Object.keys(span).sort())
      !== canonicalJson(['canonical_text_id', 'absolute_start', 'absolute_end'].sort())
    || !/^[a-f0-9]{64}$/.test(span.canonical_text_id || '')
    || !Number.isSafeInteger(span.absolute_start)
    || !Number.isSafeInteger(span.absolute_end)
    || span.absolute_start < 0
    || span.absolute_end <= span.absolute_start) {
    throw new CanonicalValidationError(`${label} is not an exact UTF-8 source span.`, {
      path: label,
    });
  }
}

function sameDefinitionSpan(left, right) {
  return left.canonical_text_id === right.canonical_text_id
    && left.absolute_start === right.absolute_start
    && left.absolute_end === right.absolute_end;
}

function validateDefinitionOccurrenceRow({ row, admittedSources, indexed, label }) {
  if (!row || typeof row !== 'object' || Array.isArray(row)
    || canonicalJson(Object.keys(row).sort())
      !== canonicalJson([...DEFINITION_OCCURRENCE_KEYS].sort())
    || row.schema_version !== 'DEFINITION_OCCURRENCE/V1') {
    throw new CanonicalValidationError(
      `${label} fields do not match the definition-occurrence write contract.`,
      { path: label },
    );
  }
  for (const field of [
    'definition_occurrence_id', 'document_hash', 'canonical_text_id', 'closure_id',
    'declaration_evidence_excerpt_id',
  ]) digestId(row[field], `${label}.${field}`);
  if (typeof row.neutral_definition_key !== 'string' || !row.neutral_definition_key
    || !Number.isSafeInteger(row.governed_ordinal) || row.governed_ordinal < 0
    || !Array.isArray(row.ordered_body_spans) || row.ordered_body_spans.length === 0
    || !Array.isArray(row.body_evidence_excerpt_ids)
    || row.body_evidence_excerpt_ids.length !== row.ordered_body_spans.length
    || new Set(row.body_evidence_excerpt_ids).size !== row.body_evidence_excerpt_ids.length) {
    throw new CanonicalValidationError(`${label} has invalid definition occurrence members.`, {
      path: label,
    });
  }
  definitionSpan(row.exact_declaration_span, `${label}.exact_declaration_span`);
  if (row.exact_declaration_span.canonical_text_id !== row.canonical_text_id) {
    throw new CanonicalValidationError(`${label} declaration does not name its canonical text.`, {
      path: label,
    });
  }
  const source = admittedSources.byCanonicalTextId.get(row.canonical_text_id);
  if (!source || row.document_hash !== source.document_hash
    || row.exact_declaration_span.absolute_end > source.canonical_text_byte_length) {
    throw new CanonicalValidationError(`${label} does not name an admitted source document.`, {
      path: label,
    });
  }
  let previousEnd = -1;
  for (const [index, span] of row.ordered_body_spans.entries()) {
    definitionSpan(span, `${label}.ordered_body_spans[${index}]`);
    digestId(row.body_evidence_excerpt_ids[index], `${label}.body_evidence_excerpt_ids[${index}]`);
    if (span.canonical_text_id !== row.canonical_text_id
      || span.absolute_end > source.canonical_text_byte_length
      || span.absolute_start < previousEnd) {
      throw new CanonicalValidationError(`${label} body spans are outside the admitted source order.`, {
        path: label,
      });
    }
    previousEnd = span.absolute_end;
  }
  const excerpts = new Map(indexed
    .filter((entry) => entry.kind === 'excerpts')
    .map((entry) => [entry.objectId, entry]));
  const declarationEvidence = excerpts.get(row.declaration_evidence_excerpt_id);
  if (!declarationEvidence
    || declarationEvidence.closureId !== row.closure_id
    || declarationEvidence.row.document_hash !== row.document_hash
    || !sameDefinitionSpan(declarationEvidence.row, row.exact_declaration_span)) {
    throw new CanonicalValidationError(`${label} declaration evidence is not the exact declaration span.`, {
      path: label,
    });
  }
  for (const [index, span] of row.ordered_body_spans.entries()) {
    const evidence = excerpts.get(row.body_evidence_excerpt_ids[index]);
    if (!evidence || evidence.closureId !== row.closure_id
      || evidence.row.document_hash !== row.document_hash
      || !sameDefinitionSpan(evidence.row, span)) {
      throw new CanonicalValidationError(`${label} body evidence is not the exact body span.`, {
        path: `${label}.body_evidence_excerpt_ids[${index}]`,
      });
    }
  }
  const identity = {
    document_hash: row.document_hash,
    exact_declaration_span: row.exact_declaration_span,
    ordered_body_spans: row.ordered_body_spans,
    neutral_definition_key: row.neutral_definition_key,
    governed_ordinal: row.governed_ordinal,
  };
  if (row.definition_occurrence_id !== contentId('DEFINITION_OCCURRENCE/V1', identity)) {
    throw new CanonicalValidationError(`${label} has a forged definition occurrence identity.`, {
      path: label,
    });
  }
}

function residualFor({ kind, objectId, closureId, reasonCode, contractKey = null, row, upstream = null }) {
  const payload = {
    source_kind: kind,
    source_object_id: objectId,
    closure_id: closureId,
    reason_code: reasonCode,
    contract_key: contractKey,
    upstream_residual: upstream,
    source_object: canonicalise(row),
  };
  return {
    residual_id: contentId('CANONICAL_RESIDUAL/V1', payload),
    ...payload,
  };
}

function structuralResiduals(kind, row, objectId, closureId, vocabulary, source) {
  const residuals = [];
  if (kind === 'provisions') {
    const conceptKey = nonEmptyString(row.concept_key, `provision ${objectId}.concept_key`);
    if (!vocabulary.concepts.has(conceptKey)) {
      residuals.push(residualFor({
        kind, objectId, closureId, reasonCode: 'INVALID_TAXONOMY_CODE', contractKey: conceptKey, row,
      }));
    }
  }
  if (kind === 'components') {
    const componentKey = nonEmptyString(row.component_key, `component ${objectId}.component_key`);
    if (!vocabulary.components.has(componentKey)) {
      residuals.push(residualFor({
        kind, objectId, closureId, reasonCode: 'INVALID_TAXONOMY_CODE', contractKey: componentKey, row,
      }));
    }
  }
  if (kind === 'claims') {
    const claimKey = nonEmptyString(row.claim_definition_key, `claim ${objectId}.claim_definition_key`);
    const state = nonEmptyString(row.state, `claim ${objectId}.state`);
    if (!vocabulary.states.has(state)) {
      throw new CanonicalValidationError(`claim ${objectId}.state is outside the frozen contract.`, { objectId, state });
    }
    const claimDefinition = vocabulary.claims.get(claimKey);
    if (!claimDefinition) {
      residuals.push(residualFor({
        kind, objectId, closureId, reasonCode: 'UNKNOWN_ATTRIBUTE', contractKey: claimKey, row,
      }));
    }
    if (state === 'PRESENT' && claimDefinition
      && (row.canonical_value == null || !canonicalValueAllowed(claimDefinition, row.canonical_value))) {
      residuals.push(residualFor({
        kind, objectId, closureId, reasonCode: 'INVALID_CANONICAL_VALUE', contractKey: claimKey, row,
      }));
    }
    const moneyDenominatorPrecisionPolicy =
      vocabulary.moneyDenominatorPrecisionPolicyForClaim(claimKey);
    if (state === 'PRESENT' && moneyDenominatorPrecisionPolicy) {
      const authoritativePrecision = row.denominator?.precision;
      const compatibilityPrecision = row.attributes?.denominator_precision;
      if (!moneyDenominatorPrecisionPolicy.allowed_precision_values.includes(authoritativePrecision)
        || compatibilityPrecision !== authoritativePrecision) {
        residuals.push(residualFor({
          kind,
          objectId,
          closureId,
          reasonCode: 'INVALID_DENOMINATOR_PRECISION',
          contractKey: claimKey,
          row,
        }));
      }
    }
    if (state === 'PRESENT' && boundedList(row.evidence, `claim ${objectId}.evidence`).length === 0) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'PRESENT_WITHOUT_EVIDENCE', row }));
    }
    if (state === 'ABSENT' && !completeScope(row.scope)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'ABSENT_WITHOUT_COMPLETE_SCOPE', row }));
    }
    if (state !== 'PRESENT' && hasAssertedValue(row)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'NON_PRESENT_ASSERTED_VALUE', row }));
    }
    if (!hasTypedStateDetail(row, state)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'STATE_DETAIL_REQUIRED', row }));
    }
  }
  if (kind === 'relationships') {
    const relationshipKey = nonEmptyString(
      row.relationship_definition_key,
      `relationship ${objectId}.relationship_definition_key`,
    );
    const relationshipDefinition = vocabulary.relationships.get(relationshipKey);
    if (!relationshipDefinition) {
      residuals.push(residualFor({
        kind, objectId, closureId, reasonCode: 'INVALID_TAXONOMY_CODE', contractKey: relationshipKey, row,
      }));
    }
    const state = nonEmptyString(row.state, `relationship ${objectId}.state`);
    if (!vocabulary.states.has(state)) {
      throw new CanonicalValidationError(`relationship ${objectId}.state is outside the frozen contract.`, {
        objectId,
        state,
      });
    }
    if (state === 'PRESENT' && boundedList(
      row.evidence,
      `relationship ${objectId}.evidence`,
    ).length === 0) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'PRESENT_WITHOUT_EVIDENCE', row }));
    }
    if (state === 'PRESENT' && boundedList(
      row.target_occurrence_ids,
      `relationship ${objectId}.target_occurrence_ids`,
    ).length === 0) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'PRESENT_WITHOUT_RESOLVED_TARGET', row }));
    }
    const effectMode = relationshipDefinition?.effect_mode;
    if (state === 'PRESENT'
      && (!row.effect
        || typeof row.effect !== 'object'
        || Array.isArray(row.effect)
        || row.effect.effect_mode !== effectMode
        || typeof row.effect.legal_operation !== 'string'
        || row.effect.legal_operation.length === 0)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'PRESENT_WITHOUT_EFFECT', row }));
    }
    if (state === 'ABSENT' && !completeScope(row.scope)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'ABSENT_WITHOUT_COMPLETE_SCOPE', row }));
    }
    if (state !== 'PRESENT' && ((row.target_occurrence_ids || []).length || row.effect != null)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'NON_PRESENT_ASSERTED_VALUE', row }));
    }
    if (!hasTypedStateDetail(row, state)) {
      residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'STATE_DETAIL_REQUIRED', row }));
    }
  }
  if (kind === 'provisions'
    && (!source
      || row.source_occurrence_id !== source.source_occurrence_id
      || row.canonical_text_id !== source.canonical_text_id
      || row.document_hash !== source.document_hash)) {
    residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'CANONICAL_IDENTITY_MISMATCH', row }));
  }
  if (expectedObjectId(kind, row, source) !== objectId) {
    residuals.push(residualFor({ kind, objectId, closureId, reasonCode: 'CANONICAL_IDENTITY_MISMATCH', row }));
  }
  return residuals;
}

function semanticIdentityResiduals(kind, row, objectId, closureId) {
  if (!['claims', 'relationships'].includes(kind)) return [];
  try {
    if (kind === 'claims') {
      validateClaimRevisionIdentity(row, {
        allowClosure: true,
        requirePublication: false,
      });
    } else {
      validateRelationshipRevisionIdentity(row, {
        allowClosure: true,
        requirePublication: false,
      });
    }
    return [];
  } catch {
    return [residualFor({
      kind,
      objectId,
      closureId,
      reasonCode: 'CANONICAL_IDENTITY_MISMATCH',
      row,
    })];
  }
}

function semanticReferenceResiduals(indexed) {
  const residuals = [];
  const provisions = new Map(indexed
    .filter((entry) => entry.kind === 'provisions')
    .map((entry) => [entry.objectId, entry]));
  const components = new Map(indexed
    .filter((entry) => entry.kind === 'components')
    .map((entry) => [entry.objectId, entry]));
  const definitions = new Map(indexed
    .filter((entry) => entry.kind === 'definition_occurrences')
    .map((entry) => [entry.objectId, entry]));
  const relationshipOccurrences = new Map(indexed
    .filter((entry) => entry.kind === 'relationships')
    .map((entry) => [entry.row.relationship_occurrence_id, entry]));
  const conditionGroups = new Map(indexed
    .filter((entry) => entry.kind === 'condition_groups')
    .map((entry) => [entry.row.condition_group_occurrence_id, entry]));
  const occurrences = new Map([
    ...provisions,
    ...components,
    ...definitions,
    ...conditionGroups,
    ...relationshipOccurrences,
  ]);
  const add = (entry) => residuals.push(residualFor({
    kind: entry.kind,
    objectId: entry.objectId,
    closureId: entry.closureId,
    reasonCode: 'SEMANTIC_REFERENCE_UNRESOLVED',
    row: entry.row,
  }));

  for (const entry of indexed) {
    if (entry.kind === 'components') {
      const parent = provisions.get(entry.row.parent_provision_instance_id);
      if (!parent
        || entry.row.canonical_text_id !== parent.row.canonical_text_id
        || entry.row.absolute_start < parent.row.absolute_start
        || entry.row.absolute_end > parent.row.absolute_end) add(entry);
    }
    if (entry.kind === 'claims') {
      const subject = occurrences.get(entry.row.subject_occurrence_id);
      if (!subject) add(entry);
    }
    if (entry.kind === 'relationships') {
      const source = occurrences.get(entry.row.source_occurrence_id);
      const targets = boundedList(
        entry.row.target_occurrence_ids,
        `relationship ${entry.objectId}.target_occurrence_ids`,
      ).map((id) => occurrences.get(id));
      if (!source
        || targets.some((target) => !target)) add(entry);
      if (entry.row.state === 'PRESENT'
        && entry.row.relationship_definition_key === 'USES_DEFINITION') {
        const selectedDefinitionId = entry.row.effect?.selected_definition_occurrence_id;
        if (targets.length !== 1
          || targets[0]?.kind !== 'definition_occurrences'
          || selectedDefinitionId !== entry.row.target_occurrence_ids[0]) add(entry);
      }
    }
  }
  return residuals;
}

function inlinePermissionEffectResiduals(indexed, vocabulary) {
  const operation = 'PERMITS_INFORMING_PERSONS_OF_NO_SHOP_PROVISIONS';
  const schema = vocabulary.noShopSemanticSchemas.get(
    'NO_SHOP_INLINE_PERMISSION_EFFECT',
  );
  const byId = new Map(indexed.map((entry) => [entry.objectId, entry]));
  const relationships = indexed.filter((entry) => {
    if (entry.kind !== 'relationships'
      || entry.row.state !== 'PRESENT'
      || entry.row.relationship_definition_key !== 'EXCEPTED_BY'
      || entry.row.relationship_definition_version !== 2) return false;
    const source = byId.get(entry.row.source_occurrence_id);
    return source?.kind === 'provisions'
      && source.row.concept_key === 'NOSOL-PROHIBIT';
  });
  if (relationships.length === 0) return [];
  const claimsBySubject = new Map();
  for (const entry of indexed.filter((candidate) => candidate.kind === 'claims')) {
    const subjectId = entry.row.subject_occurrence_id;
    if (!claimsBySubject.has(subjectId)) claimsBySubject.set(subjectId, []);
    claimsBySubject.get(subjectId).push(entry);
  }
  const residuals = [];
  for (const entry of relationships) {
    const row = entry.row;
    try {
      if (row.effect?.legal_operation !== operation) {
        throw new Error(
          'the no-shop prohibition has an unrecognised exception effect',
        );
      }
      if (!schema
        || schema.relationship_key !== 'EXCEPTED_BY'
        || schema.relationship_definition_version !== 2
        || schema.effect_mode !== 'TYPED_LEGAL_EFFECT'
        || schema.legal_operation !== operation) {
        throw new Error('the frozen inline-permission effect schema is unavailable');
      }
      const requiredEffectKeys = [
        'effect_mode',
        ...schema.required_effect_fields,
      ].sort();
      if (row.relationship_definition_key !== schema.relationship_key
        || row.relationship_definition_version
          !== schema.relationship_definition_version
        || canonicalJson(Object.keys(row.effect).sort())
          !== canonicalJson(requiredEffectKeys)
        || row.effect.effect_mode !== schema.effect_mode
        || row.effect.permitted_action_code !== schema.permitted_action_code
        || row.effect.information_subject_code
          !== schema.information_subject_code
        || row.effect.temporal_scope_inheritance
          !== schema.temporal_scope_inheritance) {
        throw new Error('the inline-permission effect is outside its closed schema');
      }
      const qualifiedIds = boundedList(
        row.effect.qualified_action_occurrence_ids,
        `relationship ${entry.objectId}.effect.qualified_action_occurrence_ids`,
      );
      if (qualifiedIds.length < schema.minimum_qualified_action_occurrences
        || qualifiedIds.length > schema.maximum_qualified_action_occurrences
        || new Set(qualifiedIds).size !== qualifiedIds.length
        || qualifiedIds.some((id) => !SHA256_RE.test(id))) {
        throw new Error('the inline-permission action scope is invalid');
      }
      const source = byId.get(row.source_occurrence_id);
      const targets = row.target_occurrence_ids.map((id) => byId.get(id));
      const qualified = qualifiedIds.map((id) => byId.get(id));
      if (source?.kind !== 'provisions'
        || source.row.concept_key !== 'NOSOL-PROHIBIT'
        || targets.length !== 1
        || targets[0]?.kind !== 'components'
        || targets[0].row.component_key !== 'EXCEPTION_LIMB'
        || targets[0].row.parent_provision_instance_id !== source.objectId
        || qualified.some((candidate) => (
          candidate?.kind !== 'components'
          || candidate.row.component_key !== 'RESTRICTED_ACTION'
          || candidate.row.parent_provision_instance_id !== source.objectId
        ))
        || qualified.some((candidate, index) => (
          index > 0
          && candidate.row.ordinal <= qualified[index - 1].row.ordinal
        ))) {
        throw new Error('the inline-permission relationship topology is invalid');
      }
      if (canonicalJson(row.effect.permitted_actor_party)
          !== canonicalJson(source.row.party)
        || canonicalJson(Object.keys(row.effect.permitted_actor_party).sort())
          !== canonicalJson(['capacity', 'role', 'value'])) {
        throw new Error('the inline-permission actor does not match its prohibition');
      }
      const qualifiedActionClaimRevisionIds = [];
      for (const qualifiedEntry of qualified) {
        const actionClaims = (claimsBySubject.get(qualifiedEntry.objectId) || [])
          .filter((claim) => (
            claim.row.claim_definition_key === 'NO_SHOP_PROHIBITED_ACTION'
            && claim.row.claim_definition_version === 2
            && claim.row.state === 'PRESENT'
          ));
        if (actionClaims.length !== 1) {
          throw new Error('a qualified action has no unique typed positive claim');
        }
        qualifiedActionClaimRevisionIds.push(
          actionClaims[0].row.claim_revision_id,
        );
      }
      if (canonicalJson(
        row.attributes?.qualified_action_claim_revision_ids,
      ) !== canonicalJson(qualifiedActionClaimRevisionIds)) {
        throw new Error('the qualified action claims are not bound exactly');
      }
      const relationshipEvidenceExcerptIds = row.evidence.map(
        (edge) => edge.excerpt_id,
      );
      const effectEvidenceIds = boundedList(
        row.effect.evidence_excerpt_ids,
        `relationship ${entry.objectId}.effect.evidence_excerpt_ids`,
      );
      if (effectEvidenceIds.length < schema.minimum_evidence_excerpts
        || effectEvidenceIds.length > schema.maximum_evidence_excerpts
        || new Set(effectEvidenceIds).size !== effectEvidenceIds.length
        || canonicalJson(effectEvidenceIds)
          !== canonicalJson(relationshipEvidenceExcerptIds)
        || !effectEvidenceIds.includes(row.effect.cross_reference_excerpt_id)
        || row.effect.scope_closure_id !== row.scope?.scope_closure_id
        || row.scope?.coverage_status !== 'PARTIAL_POSITIVE_GRAPH'
        || canonicalJson(row.scope?.required_interval_ids)
          !== canonicalJson(effectEvidenceIds)
        || canonicalJson(row.scope?.examined_interval_ids)
          !== canonicalJson(effectEvidenceIds)
        || canonicalJson(row.scope?.target_interval_ids)
          !== canonicalJson([row.effect.cross_reference_excerpt_id])) {
        throw new Error('the inline-permission evidence or scope is incomplete');
      }
      const crossReference = byId.get(row.effect.cross_reference_excerpt_id);
      if (crossReference?.kind !== 'excerpts'
        || crossReference.row.canonical_text_id
          !== targets[0].row.canonical_text_id
        || crossReference.row.absolute_start !== targets[0].row.absolute_start
        || crossReference.row.absolute_end !== targets[0].row.absolute_end
        || crossReference.row.ordered_component_assignments[0]?.semantic_span_id
          !== targets[0].row.source_anchor_id) {
        throw new Error('the inline-permission target is not the exact cross-reference');
      }
    } catch {
      residuals.push(residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: 'PRESENT_WITHOUT_EFFECT',
        row: entry.row,
      }));
    }
  }
  return residuals;
}

function evidenceResiduals(entry, excerptsById, sourceOrdinalByCanonicalTextId) {
  if (!['claims', 'relationships'].includes(entry.kind)) return [];
  const occurrenceId = entry.kind === 'claims'
    ? entry.row.claim_occurrence_id
    : entry.row.relationship_occurrence_id;
  const evidenceIdField = entry.kind === 'claims' ? 'claim_evidence_id' : 'relationship_evidence_id';
  const evidenceDomain = entry.kind === 'claims' ? 'CLAIM_EVIDENCE/V1' : 'RELATIONSHIP_EVIDENCE/V1';
  const evidence = boundedList(entry.row.evidence, `${entry.kind} ${entry.objectId}.evidence`);
  if (entry.kind === 'claims'
    && entry.row.denominator != null
    && (entry.row.denominator.value != null || entry.row.denominator.currency != null)) {
    const lineageIds = boundedList(
      entry.row.denominator.source_lineage_ids,
      `claims ${entry.objectId}.denominator.source_lineage_ids`,
    );
    const derivationEvidenceExcerptIds = new Set(evidence
      .filter((edge) => edge.evidence_role === 'DERIVATION_INPUT')
      .map((edge) => edge.excerpt_id));
    if (lineageIds.length === 0
      || lineageIds.some((id) => !excerptsById.has(id) || !derivationEvidenceExcerptIds.has(id))) {
      return [residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: 'EVIDENCE_REFERENCE_UNRESOLVED',
        row: entry.row,
      })];
    }
  }
  const validIds = [];
  for (const [ordinal, edge] of evidence.entries()) {
    const excerpt = excerptsById.get(edge.excerpt_id);
    const expectedEdgeId = contentId(evidenceDomain, {
      occurrence_id: occurrenceId,
      evidence_role: edge.evidence_role,
      excerpt_id: edge.excerpt_id,
      ordinal,
    });
    const valid = excerpt
      && edge.document_ordinal === sourceOrdinalByCanonicalTextId.get(excerpt.canonical_text_id)
      && edge[evidenceIdField] === expectedEdgeId
      && edge.ordinal === ordinal
      && edge.absolute_start === excerpt.absolute_start
      && edge.absolute_end === excerpt.absolute_end;
    if (!valid) {
      return [residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: 'EVIDENCE_REFERENCE_UNRESOLVED',
        row: entry.row,
      })];
    }
    validIds.push(expectedEdgeId);
  }
  if (canonicalJson(validIds) !== canonicalJson(entry.row.evidence_ids || [])) {
    return [residualFor({
      kind: entry.kind,
      objectId: entry.objectId,
      closureId: entry.closureId,
      reasonCode: 'EVIDENCE_REFERENCE_UNRESOLVED',
      row: entry.row,
    })];
  }
  return [];
}

function dependencyResiduals(indexed, initialResiduals) {
  const entriesById = new Map(indexed.map((entry) => [entry.objectId, entry]));
  const quarantinedClosures = new Set(initialResiduals.map((residual) => residual.closure_id));
  const residuals = [];

  function dependencies(entry) {
    if (entry.kind === 'components') {
      return [{ kind: 'SEMANTIC', entry: entriesById.get(entry.row.parent_provision_instance_id) }];
    }
    if (entry.kind === 'claims') {
      return [
        { kind: 'SEMANTIC', entry: entriesById.get(entry.row.subject_occurrence_id) },
        ...(entry.row.evidence || []).map((edge) => ({
          kind: 'EVIDENCE', entry: entriesById.get(edge.excerpt_id),
        })),
      ];
    }
    if (entry.kind === 'relationships') {
      return [
        { kind: 'SEMANTIC', entry: entriesById.get(entry.row.source_occurrence_id) },
        ...(entry.row.target_occurrence_ids || []).map((id) => ({
          kind: 'SEMANTIC', entry: entriesById.get(id),
        })),
        ...(entry.row.evidence || []).map((edge) => ({
          kind: 'EVIDENCE', entry: entriesById.get(edge.excerpt_id),
        })),
      ];
    }
    if (entry.kind === 'open_world_evidence_references') {
      return [{ kind: 'EVIDENCE', entry: entriesById.get(entry.row.excerpt_id) }];
    }
    return [];
  }

  let changed = true;
  while (changed) {
    changed = false;
    for (const entry of indexed) {
      if (quarantinedClosures.has(entry.closureId)) continue;
      const blocked = dependencies(entry).find((dependency) => (
        dependency.entry && quarantinedClosures.has(dependency.entry.closureId)
      ));
      if (!blocked) continue;
      const residual = residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: blocked.kind === 'EVIDENCE'
          ? 'EVIDENCE_REFERENCE_UNRESOLVED'
          : 'SEMANTIC_REFERENCE_UNRESOLVED',
        row: entry.row,
      });
      residuals.push(residual);
      quarantinedClosures.add(entry.closureId);
      changed = true;
    }
  }
  return residuals;
}

function upstreamResiduals(kind, row, objectId, closureId, vocabulary) {
  const retained = list(row.retained_residuals, `${kind} ${objectId}.retained_residuals`);
  if (row.publication_state === 'QUARANTINED' && retained.length === 0) {
    throw new CanonicalValidationError(`${kind} ${objectId} is quarantined without retained residuals.`, { objectId });
  }
  return retained.map((upstream) => {
    const reasonCode = nonEmptyString(upstream.reason, `${kind} ${objectId}.retained_residual.reason`);
    if (!vocabulary.residualReasons.has(reasonCode)) {
      throw new CanonicalValidationError(`retained residual reason ${reasonCode} is outside the frozen contract.`, {
        objectId,
        reasonCode,
      });
    }
    return residualFor({
      kind,
      objectId,
      closureId,
      reasonCode,
      contractKey: upstream.attribute || upstream.dimension || null,
      row,
      upstream,
    });
  });
}

function exactKeys(value, keys, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    || canonicalJson(Object.keys(value).sort()) !== canonicalJson([...keys].sort())) {
    throw new CanonicalValidationError(`${label} fields do not match the open-world write contract.`, { path: label });
  }
}

// Native-producer open-world closure (DECISIONS.md decision 2, PLAN.md Step
// 2B). A SECOND, DELIBERATELY LIGHTER open-world grammar, distinct from the
// QXO F16-F26 certification chain's rigorous one below (`NOVEL_CONCEPT_
// CANDIDATE/V1`, market-authority, canonical-result closure). DECISIONS.md
// decision 13 rules that certification chain "not extended to any other
// family"; decision 2 asks only that an open-world fact be written and
// flagged as ungoverned, not certified against a canonical result. So this is
// a real second shape, not a relaxation of the first -- discriminated by the
// candidate row's own `schema_version`, checked by the caller before this
// runs, and it never touches the QXO branch's logic or its ~70-file test
// surface.
//
// `native-write-set-adapter.js`'s `buildOpenWorldWriteRows` is this grammar's
// one producer. See that module's header for what mints the marker
// (`evidence_governance: NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER`) this
// function checks is present, verbatim, on every row kind.
const NATIVE_OPEN_WORLD_CANDIDATE_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE/V1';
const NATIVE_OPEN_WORLD_OCCURRENCE_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE_OCCURRENCE/V1';
const NATIVE_OPEN_WORLD_EVIDENCE_REFERENCE_DOMAIN = 'NATIVE_OPEN_WORLD_EVIDENCE_REFERENCE/V1';
const NATIVE_OPEN_WORLD_DISPOSITION_SCHEMA = 'NATIVE_OPEN_WORLD_CANDIDATE_DISPOSITION/V1';
const NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER = 'UNGOVERNED_OPEN_WORLD_EVIDENCE';

function nativeOpenWorldClosureResiduals({ byKind, excerptsById }) {
  for (const kind of ['open_world_candidates', 'open_world_candidate_occurrences', 'open_world_candidate_dispositions']) {
    if (byKind[kind].length !== 1) throw new CanonicalValidationError(`${kind} must contain exactly one closure member.`);
  }
  if (byKind.open_world_evidence_references.length < 1) {
    throw new CanonicalValidationError('open-world evidence collection must be non-empty.');
  }
  for (const kind of [
    'open_world_primitives', 'semantic_impact_closures',
    'reviewed_source_specific_rows', 'incomplete_canonical_result_rows',
  ]) {
    if (byKind[kind].length !== 0) {
      throw new CanonicalValidationError(`${kind} must be empty for a native-producer open-world closure -- that machinery belongs to the QXO certification chain only.`);
    }
  }

  const candidate = byKind.open_world_candidates[0].row;
  exactKeys(candidate, [
    'schema_version', 'candidate_id', 'evidence_governance', 'document_hash',
    'section_reference', 'attempted_claim_definition_key', 'reason_code',
    'raw_value', 'canonical_value', 'closure_id',
  ], 'native_open_world_candidate');
  if (candidate.evidence_governance !== NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER) {
    throw new CanonicalValidationError('native open-world candidate is missing its ungoverned-evidence marker.');
  }
  const expectedCandidateId = contentId(NATIVE_OPEN_WORLD_CANDIDATE_SCHEMA, {
    document_hash: candidate.document_hash,
    section_reference: candidate.section_reference,
    attempted_claim_definition_key: candidate.attempted_claim_definition_key,
    reason_code: candidate.reason_code,
    raw_value: candidate.raw_value,
    canonical_value: candidate.canonical_value,
    closure_id: candidate.closure_id,
  });
  if (candidate.candidate_id !== expectedCandidateId) {
    throw new CanonicalValidationError('native open-world candidate identity mismatch.');
  }

  const occurrence = byKind.open_world_candidate_occurrences[0].row;
  exactKeys(occurrence, [
    'schema_version', 'open_world_candidate_occurrence_id', 'candidate_id', 'evidence_governance',
    'document_hash', 'raw_value', 'canonical_value', 'attributes', 'closure_id',
  ], 'native_open_world_candidate_occurrence');
  if (occurrence.evidence_governance !== NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER) {
    throw new CanonicalValidationError('native open-world occurrence is missing its ungoverned-evidence marker.');
  }
  const expectedOccurrenceId = contentId(NATIVE_OPEN_WORLD_OCCURRENCE_SCHEMA, {
    candidate_id: occurrence.candidate_id,
    document_hash: occurrence.document_hash,
    raw_value: occurrence.raw_value,
    canonical_value: occurrence.canonical_value,
    attributes: occurrence.attributes,
    closure_id: occurrence.closure_id,
  });
  if (occurrence.candidate_id !== candidate.candidate_id
    || occurrence.closure_id !== candidate.closure_id
    || occurrence.open_world_candidate_occurrence_id !== expectedOccurrenceId) {
    throw new CanonicalValidationError('native open-world candidate occurrence identity mismatch.');
  }

  const evidenceIds = new Set();
  for (const entry of byKind.open_world_evidence_references) {
    const evidence = entry.row;
    exactKeys(evidence, [
      'evidence_reference_id', 'open_world_candidate_occurrence_id', 'evidence_governance',
      'evidence_role', 'excerpt_id', 'document_ordinal', 'absolute_start', 'absolute_end',
      'ordinal', 'closure_id',
    ], 'native_open_world_evidence_reference');
    if (evidence.evidence_governance !== NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER) {
      throw new CanonicalValidationError('native open-world evidence reference is missing its ungoverned-evidence marker.');
    }
    const excerpt = excerptsById.get(evidence.excerpt_id);
    const expectedEvidenceId = contentId(NATIVE_OPEN_WORLD_EVIDENCE_REFERENCE_DOMAIN, {
      open_world_candidate_occurrence_id: evidence.open_world_candidate_occurrence_id,
      evidence_role: evidence.evidence_role,
      excerpt_id: evidence.excerpt_id,
      document_ordinal: evidence.document_ordinal,
      absolute_start: evidence.absolute_start,
      absolute_end: evidence.absolute_end,
      ordinal: evidence.ordinal,
    });
    if (evidence.evidence_reference_id !== expectedEvidenceId
      || evidence.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
      || evidence.closure_id !== candidate.closure_id
      || !excerpt
      || excerpt.absolute_start !== evidence.absolute_start
      || excerpt.absolute_end !== evidence.absolute_end
      || evidenceIds.has(evidence.evidence_reference_id)) {
      throw new CanonicalValidationError('native open-world evidence reference does not resolve to admitted exact source.');
    }
    evidenceIds.add(evidence.evidence_reference_id);
  }

  const disposition = byKind.open_world_candidate_dispositions[0].row;
  exactKeys(disposition, [
    'schema_version', 'final_disposition_id', 'evidence_governance',
    'open_world_candidate_occurrence_id', 'disposition_code', 'reason_code', 'closure_id',
  ], 'native_open_world_candidate_disposition');
  if (disposition.evidence_governance !== NATIVE_OPEN_WORLD_EVIDENCE_GOVERNANCE_MARKER) {
    throw new CanonicalValidationError('native open-world disposition is missing its ungoverned-evidence marker.');
  }
  const expectedDispositionId = contentId(NATIVE_OPEN_WORLD_DISPOSITION_SCHEMA, {
    open_world_candidate_occurrence_id: disposition.open_world_candidate_occurrence_id,
    disposition_code: disposition.disposition_code,
    reason_code: disposition.reason_code,
    closure_id: disposition.closure_id,
  });
  if (disposition.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
    || disposition.closure_id !== candidate.closure_id
    || disposition.final_disposition_id !== expectedDispositionId) {
    throw new CanonicalValidationError('native open-world candidate disposition identity mismatch.');
  }
}

function openWorldGraphResiduals({ indexed, excerptsById, admittedSources }) {
  const openWorldEntries = indexed.filter((entry) => OPEN_WORLD_COLLECTION_KEYS.includes(entry.kind));
  if (openWorldEntries.length === 0) return [];
  const residuals = [];
  const sourceByCanonicalTextId = admittedSources.byCanonicalTextId;
  const byClosure = new Map();
  for (const entry of openWorldEntries) {
    if (!byClosure.has(entry.closureId)) byClosure.set(entry.closureId, []);
    byClosure.get(entry.closureId).push(entry);
  }

  for (const [closureId, entries] of byClosure) {
    try {
      const byKind = Object.fromEntries(OPEN_WORLD_COLLECTION_KEYS.map((kind) => [
        kind,
        entries.filter((entry) => entry.kind === kind),
      ]));

      // Dispatch on the candidate row's OWN schema_version, per this
      // function's header -- not on write_set_origin, so a closure's shape
      // is self-describing even if inspected apart from the write set it
      // arrived in.
      if (byKind.open_world_candidates.length === 1
        && byKind.open_world_candidates[0].row?.schema_version === NATIVE_OPEN_WORLD_CANDIDATE_SCHEMA) {
        nativeOpenWorldClosureResiduals({ byKind, excerptsById });
        continue;
      }

      for (const kind of [
        'open_world_candidates',
        'open_world_candidate_occurrences',
        'open_world_candidate_dispositions',
        'semantic_impact_closures',
      ]) {
        if (byKind[kind].length !== 1) throw new CanonicalValidationError(`${kind} must contain exactly one closure member.`);
      }
      const terminalRows = [
        ...byKind.reviewed_source_specific_rows,
        ...byKind.incomplete_canonical_result_rows,
      ];
      if (terminalRows.length !== 1
        || byKind.reviewed_source_specific_rows.length > 1
        || byKind.incomplete_canonical_result_rows.length > 1) {
        throw new CanonicalValidationError('open-world closure must contain exactly one reviewed terminal row.');
      }
      if (byKind.open_world_evidence_references.length < 1 || byKind.open_world_primitives.length < 1) {
        throw new CanonicalValidationError('open-world evidence and primitive collections must be non-empty.');
      }

      const candidate = byKind.open_world_candidates[0].row;
      exactKeys(candidate, [
        'schema_version', 'candidate_id', 'candidate_kind', 'document_hash',
        'ordered_proposition_evidence_reference_ids', 'observed_party_token_digest',
        'governed_ordinal', 'neutral_proposition_digest', 'closure_id',
      ], 'open_world_candidate');
      if (candidate.schema_version !== 'NOVEL_CONCEPT_CANDIDATE/V1'
        || !['CONCEPT', 'ATTRIBUTE_OR_QUESTION'].includes(candidate.candidate_kind)) {
        throw new CanonicalValidationError('open-world candidate kind is outside the frozen contract.');
      }
      const expectedCandidateId = contentId('NOVEL_CONCEPT_CANDIDATE/V1', {
        schema_version: candidate.schema_version,
        candidate_kind: candidate.candidate_kind,
        document_hash: candidate.document_hash,
        ordered_proposition_evidence_reference_ids: candidate.ordered_proposition_evidence_reference_ids,
        observed_party_token_digest: candidate.observed_party_token_digest,
        governed_ordinal: candidate.governed_ordinal,
        neutral_proposition_digest: candidate.neutral_proposition_digest,
      });
      if (candidate.candidate_id !== expectedCandidateId) throw new CanonicalValidationError('open-world candidate identity mismatch.');

      const occurrence = byKind.open_world_candidate_occurrences[0].row;
      exactKeys(occurrence, [
        'schema_version', 'open_world_candidate_occurrence_id', 'candidate_id', 'candidate_kind',
        'admission_state', 'document_hash', 'ordered_proposition_evidence_reference_ids',
        'observed_party_token_digest', 'governed_ordinal', 'neutral_proposition_digest', 'closure_id',
      ], 'open_world_candidate_occurrence');
      const expectedOccurrenceId = contentId('OPEN_WORLD_CANDIDATE_OCCURRENCE/V1', {
        schema_version: occurrence.schema_version,
        candidate_id: occurrence.candidate_id,
        admission_state: occurrence.admission_state,
        document_hash: occurrence.document_hash,
        ordered_proposition_evidence_reference_ids: occurrence.ordered_proposition_evidence_reference_ids,
        observed_party_token_digest: occurrence.observed_party_token_digest,
        governed_ordinal: occurrence.governed_ordinal,
        neutral_proposition_digest: occurrence.neutral_proposition_digest,
      });
      if (occurrence.schema_version !== 'OPEN_WORLD_CANDIDATE_OCCURRENCE/V1'
        || occurrence.candidate_id !== candidate.candidate_id
        || occurrence.candidate_kind !== candidate.candidate_kind
        || occurrence.admission_state !== 'ADMITTED_SEMANTIC'
        || occurrence.document_hash !== candidate.document_hash
        || occurrence.observed_party_token_digest !== candidate.observed_party_token_digest
        || occurrence.governed_ordinal !== candidate.governed_ordinal
        || occurrence.neutral_proposition_digest !== candidate.neutral_proposition_digest
        || canonicalJson(occurrence.ordered_proposition_evidence_reference_ids)
          !== canonicalJson(candidate.ordered_proposition_evidence_reference_ids)
        || occurrence.open_world_candidate_occurrence_id !== expectedOccurrenceId) {
        throw new CanonicalValidationError('open-world candidate occurrence identity mismatch.');
      }

      const evidenceIds = new Set();
      for (const entry of byKind.open_world_evidence_references) {
        const evidence = entry.row;
        exactKeys(evidence, [
          'evidence_reference_id', 'evidence_role', 'excerpt_id', 'semantic_span_id',
          'document_hash', 'absolute_start', 'absolute_end', 'exact_bytes_digest',
          'governed_ordinal', 'open_world_candidate_occurrence_id', 'canonical_text_id', 'closure_id',
        ], 'open_world_evidence_reference');
        const excerpt = excerptsById.get(evidence.excerpt_id);
        const source = sourceByCanonicalTextId.get(evidence.canonical_text_id);
        const evidenceBody = pick(evidence, [
          'evidence_role', 'excerpt_id', 'semantic_span_id', 'document_hash',
          'absolute_start', 'absolute_end', 'exact_bytes_digest', 'governed_ordinal',
        ]);
        if (evidence.evidence_reference_id !== contentId('OPEN_WORLD_EVIDENCE_REFERENCE/V1', evidenceBody)
          || evidence.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
          || !excerpt
          || !source
          || excerpt.canonical_text_id !== evidence.canonical_text_id
          || excerpt.document_hash !== evidence.document_hash
          || excerpt.absolute_start !== evidence.absolute_start
          || excerpt.absolute_end !== evidence.absolute_end
          || excerpt.exact_bytes_digest !== evidence.exact_bytes_digest
          || excerpt.ordered_component_assignments[0].semantic_span_id !== evidence.semantic_span_id
          || evidenceIds.has(evidence.evidence_reference_id)) {
          throw new CanonicalValidationError('open-world evidence reference does not resolve to admitted exact source.');
        }
        evidenceIds.add(evidence.evidence_reference_id);
      }
      if (canonicalJson(occurrence.ordered_proposition_evidence_reference_ids) !== canonicalJson(
        candidate.ordered_proposition_evidence_reference_ids,
      ) || occurrence.ordered_proposition_evidence_reference_ids.some((id) => !evidenceIds.has(id))) {
        throw new CanonicalValidationError('open-world candidate proposition evidence is incomplete.');
      }

      const disposition = byKind.open_world_candidate_dispositions[0].row;
      exactKeys(disposition, [
        'schema_version', 'open_world_candidate_occurrence_id', 'final_disposition_id',
        'disposition_code', 'review_state', 'reviewed_display_label', 'non_comparable_reason', 'closure_id',
      ], 'open_world_candidate_disposition');
      const expectedDispositionId = contentId('OPEN_WORLD_CANDIDATE_FINAL_DISPOSITION/V1', {
        schema_version: disposition.schema_version,
        open_world_candidate_occurrence_id: disposition.open_world_candidate_occurrence_id,
        disposition_code: disposition.disposition_code,
        review_state: disposition.review_state,
        reviewed_display_label: disposition.reviewed_display_label,
        non_comparable_reason: disposition.non_comparable_reason,
      });
      if (disposition.schema_version !== 'OPEN_WORLD_CANDIDATE_FINAL_DISPOSITION/V1'
        || disposition.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
        || disposition.disposition_code !== 'REVIEWED_SOURCE_SPECIFIC'
        || disposition.review_state !== 'FINAL'
        || disposition.final_disposition_id !== expectedDispositionId) {
        throw new CanonicalValidationError('open-world candidate has no final publishable disposition.');
      }

      const primitiveIds = new Set();
      for (const entry of byKind.open_world_primitives) {
        const primitive = entry.row;
        exactKeys(primitive, [
          'open_world_candidate_occurrence_id', 'primitive_id', 'primitive_kind', 'governed_ordinal',
          'raw_value', 'interpreted_value', 'evidence_reference_ids', 'closure_id',
        ], 'open_world_primitive');
        const primitiveBody = pick(primitive, [
          'primitive_kind', 'governed_ordinal', 'raw_value', 'interpreted_value', 'evidence_reference_ids',
        ]);
        if (primitive.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
          || primitive.primitive_id !== contentId('OPEN_WORLD_LEGAL_PRIMITIVE/V1', {
            open_world_candidate_occurrence_id: occurrence.open_world_candidate_occurrence_id,
            ...primitiveBody,
          })
          || primitive.evidence_reference_ids.some((id) => !evidenceIds.has(id))
          || primitiveIds.has(primitive.primitive_id)) {
          throw new CanonicalValidationError('open-world primitive identity or evidence mismatch.');
        }
        primitiveIds.add(primitive.primitive_id);
      }

      const impact = byKind.semantic_impact_closures[0].row;
      exactKeys(impact, [
        'schema_version', 'open_world_candidate_occurrence_id', 'semantic_impact_closure_id',
        'impact_value', 'affected_canonical_result_occurrence_ids', 'market_authority', 'closure_id',
      ], 'semantic_impact_closure');
      const expectedImpactId = contentId('SEMANTIC_IMPACT_CLOSURE/V1', {
        schema_version: impact.schema_version,
        open_world_candidate_occurrence_id: impact.open_world_candidate_occurrence_id,
        impact_value: impact.impact_value,
        affected_canonical_result_occurrence_ids: impact.affected_canonical_result_occurrence_ids,
        market_authority: impact.market_authority,
      });
      if (impact.schema_version !== 'SEMANTIC_IMPACT_CLOSURE/V1'
        || impact.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
        || impact.market_authority !== 'NO_MARKET_AUTHORITY'
        || !Array.isArray(impact.affected_canonical_result_occurrence_ids)
        || impact.semantic_impact_closure_id !== expectedImpactId) {
        throw new CanonicalValidationError('open-world semantic impact closure is invalid.');
      }

      if (byKind.reviewed_source_specific_rows.length === 1) {
        if (candidate.candidate_kind !== 'CONCEPT'
          || impact.impact_value !== 'REVIEW_ONLY_SOURCE_SPECIFIC'
          || impact.affected_canonical_result_occurrence_ids.length !== 0) {
          throw new CanonicalValidationError('source-specific concept has invalid semantic impact.');
        }
        const rowRecord = byKind.reviewed_source_specific_rows[0].row;
        exactKeys(rowRecord, [
          'reviewed_source_specific_row_serving_key', 'shared_serving_row', 'closure_id',
        ], 'reviewed_source_specific_row');
        validateSharedServingRow(rowRecord.shared_serving_row);
        const servingBody = rowRecord.shared_serving_row.reviewed_source_specific;
        if (rowRecord.shared_serving_row.row_kind !== 'REVIEWED_SOURCE_SPECIFIC'
          || rowRecord.reviewed_source_specific_row_serving_key !== rowRecord.shared_serving_row.row_serving_key
          || servingBody.candidate_occurrence.open_world_candidate_occurrence_id !== occurrence.open_world_candidate_occurrence_id
          || servingBody.final_disposition.final_disposition_id !== disposition.final_disposition_id
          || servingBody.semantic_impact_closure.semantic_impact_closure_id !== impact.semantic_impact_closure_id
          || servingBody.primitive_collection.total !== byKind.open_world_primitives.length
          || canonicalJson(servingBody.evidence_references.map((row) => row.evidence_reference_id).sort())
            !== canonicalJson([...evidenceIds].sort())) {
          throw new CanonicalValidationError('reviewed source-specific row does not close over its open-world graph.');
        }
      } else {
        const primitiveKinds = byKind.open_world_primitives.map((entry) => entry.row.primitive_kind);
        if (candidate.candidate_kind !== 'ATTRIBUTE_OR_QUESTION'
          || canonicalJson(primitiveKinds) !== canonicalJson(['TIME'])
          || impact.impact_value !== 'AFFECTS_CANONICAL_RESULT'
          || impact.affected_canonical_result_occurrence_ids.length !== 1) {
          throw new CanonicalValidationError('incomplete canonical result has invalid open-world semantic impact.');
        }
        const rowRecord = byKind.incomplete_canonical_result_rows[0].row;
        exactKeys(rowRecord, [
          'incomplete_result_review_row_serving_key', 'shared_serving_row', 'closure_id',
        ], 'incomplete_canonical_result_row');
        validateSharedServingRow(rowRecord.shared_serving_row);
        const servingBody = rowRecord.shared_serving_row.incomplete_canonical_result;
        const intersectingCandidates = servingBody.intersecting_candidates;
        if (rowRecord.shared_serving_row.row_kind !== 'INCOMPLETE_CANONICAL_RESULT'
          || rowRecord.incomplete_result_review_row_serving_key !== rowRecord.shared_serving_row.row_serving_key
          || impact.affected_canonical_result_occurrence_ids[0] !== servingBody.derived_result_occurrence_id
          || intersectingCandidates.length !== 1
          || intersectingCandidates[0].open_world_candidate_occurrence_id
            !== occurrence.open_world_candidate_occurrence_id
          || intersectingCandidates[0].final_disposition_id !== disposition.final_disposition_id
          || intersectingCandidates[0].semantic_impact_closure_id !== impact.semantic_impact_closure_id
          || intersectingCandidates[0].impact_value !== impact.impact_value) {
          throw new CanonicalValidationError('incomplete canonical result row does not close over its open-world graph.');
        }
      }
    } catch (error) {
      const payload = {
        closure_id: closureId,
        open_world_objects: entries.map((entry) => ({
          kind: entry.kind,
          object_id: entry.objectId,
          canonical_payload_digest: contentId('OPEN_WORLD_WRITE_OBJECT/V1', entry.row),
        })),
        validation_error: error.message,
      };
      residuals.push(residualFor({
        kind: 'open_world_semantic_closure',
        objectId: closureId,
        closureId,
        reasonCode: 'CANONICAL_IDENTITY_MISMATCH',
        row: payload,
      }));
    }
  }
  return residuals;
}

function semanticGraphResiduals({ indexed, admittedSources }) {
  const graphEntries = indexed.filter((entry) => entry.kind === 'validated_semantic_graphs');
  const residuals = [];
  const entriesByIdentity = new Map();
  for (const entry of graphEntries) {
    if (!entriesByIdentity.has(entry.objectId)) entriesByIdentity.set(entry.objectId, []);
    entriesByIdentity.get(entry.objectId).push(entry);
  }
  for (const entries of entriesByIdentity.values()) {
    if (entries.length < 2) continue;
    for (const entry of entries) {
      residuals.push(residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: 'CANONICAL_IDENTITY_MISMATCH',
        row: entry.row,
      }));
    }
  }
  for (const entry of graphEntries) {
    try {
      const source = admittedSources.byCanonicalTextId.get(entry.row.canonical_text_id);
      if (!source) throw new Error('ValidatedSemanticGraph names an unadmitted canonical text');
      validateValidatedDefinitionGraph({ source, graph: entry.row });
    } catch (error) {
      residuals.push(residualFor({
        kind: entry.kind,
        objectId: entry.objectId,
        closureId: entry.closureId,
        reasonCode: 'CANONICAL_IDENTITY_MISMATCH',
        row: {
          ...entry.row,
          validation_error: error.message,
        },
      }));
    }
  }
  return residuals;
}

function validateWithAdmittedSources(
  writeSet,
  contractBundle,
  admittedSources,
  retainedCanonicalObjects = [],
  publicationGate,
) {
  const vocabulary = contractVocabulary(contractBundle);
  const indexed = [];
  const residualsById = new Map();
  const retainedEntries = materialiseRetainedEntries(writeSet, retainedCanonicalObjects);
  const retainedByKind = new Map(COLLECTION_KEYS.map((kind) => [
    kind,
    retainedEntries.filter((entry) => entry.kind === kind),
  ]));

  for (const kind of CANONICAL_COLLECTION_KEYS) {
    const supplied = list(writeSet[kind], `writeSet.${kind}`);
    const rows = [
      ...supplied.map((row) => ({ row, retained: false })),
      ...retainedByKind.get(kind).map((entry) => ({ row: entry.row, retained: true })),
    ];
    for (const [index, candidate] of rows.entries()) {
      const { row, retained } = candidate;
      if (!row || typeof row !== 'object' || Array.isArray(row)) {
        throw new CanonicalValidationError(`writeSet.${kind}[${index}] must be an object.`, {
          path: `writeSet.${kind}[${index}]`,
        });
      }
      if (kind === 'provisions' || kind === 'components') {
        assertStructuralRowShape(
          kind,
          row,
          admittedSources.byCanonicalTextId.get(row.canonical_text_id),
          `writeSet.${kind}[${index}]`,
        );
      } else if (kind === 'definition_occurrences') {
        validateDefinitionOccurrenceRow({
          row,
          admittedSources,
          indexed,
          label: `writeSet.${kind}[${index}]`,
        });
      }
      const objectId = digestId(row[ID_FIELDS[kind]], `writeSet.${kind}[${index}].${ID_FIELDS[kind]}`);
      const closureId = digestId(row.closure_id, `writeSet.${kind}[${index}].closure_id`);
      const source = admittedSources.byCanonicalTextId.get(row.canonical_text_id);
      if (ANSWER_PROVENANCE_COLLECTION_KINDS.includes(kind)) {
        // Staged requirement (spec: "Citation guard and validator staging"):
        // REQUIRED only for freshly supplied `claims` rows, on a
        // NATIVE_PRODUCER-origin write set, whose claim_definition_key is
        // itself a REGISTERED key in the supplied vocabulary. That last
        // condition matters: native-write-set-adapter.js is also exercised
        // directly on PRE-resolution compiled candidates (generic
        // producer keys like `NATIVE_CAPITALISATION_QUALIFIER_CANDIDATE`,
        // never registered in any contract vocabulary -- see its own test
        // suite) that never went through candidate-resolution.js's resolver
        // at all, so no MECHANICAL/AI tag was ever minted for them and none
        // should be demanded. Retained/backfilled rows and every
        // `relationships` row (the native producer emits none today) are
        // validated whenever the field is present, never required --
        // reviewed-slice write sets and every pre-existing fixture stay
        // green, untouched.
        validateAnswerProvenanceRow({
          kind,
          row,
          objectId,
          required: !retained && kind === 'claims' && writeSet.write_set_origin === 'NATIVE_PRODUCER'
            && vocabulary.claims.has(row.claim_definition_key),
        });
      }
      const identityResiduals = semanticIdentityResiduals(
        kind,
        row,
        objectId,
        closureId,
      );
      const found = [
        ...identityResiduals,
        ...structuralResiduals(
          kind,
          row,
          objectId,
          closureId,
          vocabulary,
          source,
        ),
        ...(identityResiduals.length === 0
          ? upstreamResiduals(kind, row, objectId, closureId, vocabulary)
          : []),
      ];
      indexed.push({ kind, row: canonicalise(row), objectId, closureId, retained });
      for (const residual of found) residualsById.set(residual.residual_id, residual);
    }
  }
  for (const kind of CONDITION_GROUP_COLLECTION_KEYS) {
    const supplied = boundedList(writeSet[kind], `writeSet.${kind}`);
    const rows = [
      ...supplied.map((row) => ({ row, retained: false })),
      ...retainedByKind.get(kind).map((entry) => ({ row: entry.row, retained: true })),
    ];
    validateConditionGroupRows({
      rows: rows.map((entry) => entry.row),
      admittedSources,
      vocabulary,
      indexed,
    });
    for (const [index, candidate] of rows.entries()) {
      const { row, retained } = candidate;
      const objectId = digestId(
        row.condition_group_revision_id,
        `writeSet.${kind}[${index}].condition_group_revision_id`,
      );
      const closureId = digestId(row.closure_id, `writeSet.${kind}[${index}].closure_id`);
      indexed.push({ kind, row: canonicalise(row), objectId, closureId, retained });
    }
  }
  for (const kind of SEMANTIC_GRAPH_COLLECTION_KEYS) {
    const supplied = boundedList(writeSet[kind], `writeSet.${kind}`);
    const rows = [
      ...supplied.map((row) => ({ row, retained: false })),
      ...retainedByKind.get(kind).map((entry) => ({ row: entry.row, retained: true })),
    ];
    assertSemanticGraphRequestBudget(rows.map((entry) => entry.row));
    for (const [index, candidate] of rows.entries()) {
      const { row, retained } = candidate;
      if (!row || typeof row !== 'object' || Array.isArray(row)) {
        throw new CanonicalValidationError(`writeSet.${kind}[${index}] must be an object.`, {
          path: `writeSet.${kind}[${index}]`,
        });
      }
      const objectId = digestId(row[ID_FIELDS[kind]], `writeSet.${kind}[${index}].${ID_FIELDS[kind]}`);
      const closureId = digestId(row.closure_id, `writeSet.${kind}[${index}].closure_id`);
      indexed.push({ kind, row: canonicalise(row), objectId, closureId, retained });
    }
  }
  for (const kind of OPEN_WORLD_COLLECTION_KEYS) {
    const supplied = list(writeSet[kind], `writeSet.${kind}`);
    const rows = [
      ...supplied.map((row) => ({ row, retained: false })),
      ...retainedByKind.get(kind).map((entry) => ({ row: entry.row, retained: true })),
    ];
    for (const [index, candidate] of rows.entries()) {
      const { row, retained } = candidate;
      if (!row || typeof row !== 'object' || Array.isArray(row)) {
        throw new CanonicalValidationError(`writeSet.${kind}[${index}] must be an object.`, {
          path: `writeSet.${kind}[${index}]`,
        });
      }
      const objectId = digestId(row[ID_FIELDS[kind]], `writeSet.${kind}[${index}].${ID_FIELDS[kind]}`);
      const closureId = digestId(row.closure_id, `writeSet.${kind}[${index}].closure_id`);
      indexed.push({ kind, row: canonicalise(row), objectId, closureId, retained });
    }
  }

  // PLAN.md Step 4A3 (Ben's ruling, DECISIONS.md decision 3).
  // `conditional_termination_fee_values` is deliberately NOT one of the
  // `COLLECTION_KEYS` loops above: it carries no `closure_id`
  // (`native-write-set-adapter.js` never mints one for this kind, matching
  // `resolveModivConditionalFees`'s own rows), so it takes no part in the
  // closure/residual/quarantine machinery those loops feed -- exactly the
  // independent, always-published treatment
  // `supabase/canonical-v2-foundation.sql`'s Step 4A2 branch gives it,
  // excluded from every generic closure-keyed loop there for the identical
  // reason. Validated for shape and identity, checked for duplicate ids
  // (the one integrity rule that DOES apply to it), and copied straight into
  // `publishableWriteSet` when supplied -- never quarantined, never dropped.
  const conditionalTerminationFeeValueRows = boundedList(
    writeSet.conditional_termination_fee_values,
    'writeSet.conditional_termination_fee_values',
  );
  const conditionalTerminationFeeValueIds = new Set();
  for (const [index, row] of conditionalTerminationFeeValueRows.entries()) {
    const label = `writeSet.conditional_termination_fee_values[${index}]`;
    const objectId = validateConditionalTerminationFeeValueRow(row, label);
    if (conditionalTerminationFeeValueIds.has(objectId)) {
      throw new CanonicalValidationError(
        'writeSet contains a duplicate conditional termination fee value.',
        { path: label },
      );
    }
    conditionalTerminationFeeValueIds.add(objectId);
  }

  const retainedIdentityKeys = new Set(
    indexed
      .filter((entry) => entry.retained)
      .map((entry) => `${entry.kind}:${entry.objectId}`),
  );
  if (indexed.some((entry) => (
    !entry.retained && retainedIdentityKeys.has(`${entry.kind}:${entry.objectId}`)
  ))) {
    throw new CanonicalValidationError(
      'A canonical object cannot be both retained and newly published in one write.',
      { path: 'writeSet.persisted_object_references' },
    );
  }
  const retainedStoredClosureIds = new Set(
    retainedEntries.map((entry) => entry.storedClosureId),
  );
  if (indexed.some((entry) => !entry.retained && retainedStoredClosureIds.has(entry.closureId))) {
    throw new CanonicalValidationError(
      'A later run cannot extend a persisted semantic closure through object references.',
      { path: 'writeSet.persisted_object_references' },
    );
  }

  const excerptsById = new Map(indexed
    .filter((entry) => entry.kind === 'excerpts')
    .map((entry) => [entry.objectId, entry.row]));
  const sourceOrdinalByCanonicalTextId = admittedSources.sourceOrdinalByCanonicalTextId
    || new Map(admittedSources.admissions.map((admission) => {
      const source = admittedSources.sources.find(
        (row) => row.immutable_source_document_id === admission.immutable_source_document_id,
      );
      return [source.canonical_text_id, admission.source_ordinal];
    }));
  for (const entry of indexed) {
    for (const residual of evidenceResiduals(entry, excerptsById, sourceOrdinalByCanonicalTextId)) {
      residualsById.set(residual.residual_id, residual);
    }
  }

  // Publication disposition is an audit sidecar. It does not change claim
  // identity, resolution, review routing, or this validation-clean set.
  const publicationDispositions = writeSet.publication_dispositions === undefined
    ? []
    : boundedList(writeSet.publication_dispositions, 'writeSet.publication_dispositions');
  const publicationAuthorities = writeSet.publication_calibration_authorities === undefined
    ? []
    : boundedList(writeSet.publication_calibration_authorities, 'writeSet.publication_calibration_authorities');
  const publicationAuthoritiesById = new Map();
  for (const [index, authority] of publicationAuthorities.entries()) {
    try {
      validateCalibrationAuthority(authority);
    } catch (error) {
      throw new CanonicalValidationError(`publication calibration authority is invalid: ${error.message}`, {
        path: `writeSet.publication_calibration_authorities[${index}]`, code: error.code,
      });
    }
    if (publicationAuthoritiesById.has(authority.calibration_authority_id)) {
      throw new CanonicalValidationError('publication calibration authority appears more than once.', {
        path: `writeSet.publication_calibration_authorities[${index}]`,
      });
    }
    publicationAuthoritiesById.set(authority.calibration_authority_id, authority);
  }
  const claimsById = new Map(indexed
    .filter((entry) => entry.kind === 'claims' && !entry.retained)
    .map((entry) => [entry.objectId, entry]));
  if (writeSet.publication_dispositions !== undefined
    && publicationDispositions.length !== claimsById.size) {
    throw new CanonicalValidationError('publication dispositions must be one-to-one with supplied claims.', {
      path: 'writeSet.publication_dispositions',
    });
  }
  const dispositionsByClaimId = new Map();
  for (const [index, sidecar] of publicationDispositions.entries()) {
    if (!sidecar || !claimsById.has(sidecar.claim_revision_id)
      || dispositionsByClaimId.has(sidecar.claim_revision_id)) {
      throw new CanonicalValidationError('publication disposition does not name one supplied claim exactly once.', {
        path: `writeSet.publication_dispositions[${index}]`,
      });
    }
    if (sidecar.authority_id !== null
      && !publicationAuthoritiesById.has(sidecar.authority_id)) {
      throw new CanonicalValidationError('publication disposition authority is not supplied immutably with the write set.', {
        path: `writeSet.publication_dispositions[${index}].authority_id`,
      });
    }
    try {
      validatePublicationDispositionV2(sidecar, {
        calibration_authority: publicationGate?.calibration_authority,
        publication_input: publicationGate?.publication_inputs_by_claim_revision_id?.[sidecar.claim_revision_id]
          || publicationGate?.publication_input,
        rebuild: true,
      });
    } catch (error) {
      throw new CanonicalValidationError(`publication disposition is invalid: ${error.message}`, {
        path: `writeSet.publication_dispositions[${index}]`, code: error.code,
      });
    }
    dispositionsByClaimId.set(sidecar.claim_revision_id, canonicalise(sidecar));
  }
  for (const residual of semanticReferenceResiduals(indexed)) {
    residualsById.set(residual.residual_id, residual);
  }
  for (const residual of inlinePermissionEffectResiduals(indexed, vocabulary)) {
    residualsById.set(residual.residual_id, residual);
  }
  for (const residual of openWorldGraphResiduals({ indexed, excerptsById, admittedSources })) {
    residualsById.set(residual.residual_id, residual);
  }
  for (const residual of semanticGraphResiduals({ indexed, admittedSources })) {
    residualsById.set(residual.residual_id, residual);
  }
  for (const residual of dependencyResiduals(indexed, [...residualsById.values()])) {
    residualsById.set(residual.residual_id, residual);
  }

  const residuals = [...residualsById.values()];
  const retainedValidationClosureIds = new Set(
    retainedEntries.map((entry) => entry.closureId),
  );
  if (residuals.some((residual) => retainedValidationClosureIds.has(residual.closure_id))) {
    throw new CanonicalValidationError(
      'A persisted canonical object reference failed current-run semantic validation.',
      { path: 'writeSet.persisted_object_references' },
    );
  }
  const quarantinedClosureIds = new Set(residuals.map((residual) => residual.closure_id));
  // Compatibility name: `publishableWriteSet` means validation-clean. It is
  // not a public-availability or release decision; publication is the V2
  // sidecar below.
  const publishableWriteSet = {
    deal: canonicalise(writeSet.deal),
    ...Object.fromEntries(COLLECTION_KEYS.map((key) => [key, []])),
  };
  if (admittedSources.mode === 'REFERENCES') {
    publishableWriteSet.source_references = canonicalise(admittedSources.sourceReferences);
    if (writeSet.persisted_object_references !== undefined) {
      publishableWriteSet.persisted_object_references = canonicalise(
        writeSet.persisted_object_references,
      );
    }
    if (writeSet.conditional_termination_fee_values !== undefined) {
      publishableWriteSet.conditional_termination_fee_values = canonicalise(
        conditionalTerminationFeeValueRows,
      );
    }
  } else if (admittedSources.mode === 'SINGULAR') {
    publishableWriteSet.source = canonicalise(admittedSources.sources[0]);
    publishableWriteSet.source_admission = canonicalise(admittedSources.admissions[0]);
  } else {
    publishableWriteSet.sources = canonicalise(admittedSources.sources);
    publishableWriteSet.source_admissions = canonicalise(admittedSources.admissions);
  }
  for (const entry of indexed) {
    if (!entry.retained && !quarantinedClosureIds.has(entry.closureId)) {
      publishableWriteSet[entry.kind].push(entry.row);
    }
  }
  const acceptedClaimIds = new Set(publishableWriteSet.claims.map((claim) => claim.claim_revision_id));
  if (writeSet.publication_dispositions !== undefined) {
    publishableWriteSet.publication_dispositions = canonicalise(publicationDispositions
      .filter((sidecar) => acceptedClaimIds.has(sidecar.claim_revision_id)));
  }
  if (writeSet.publication_calibration_authorities !== undefined) {
    const referencedAuthorityIds = new Set((publishableWriteSet.publication_dispositions || [])
      .map((sidecar) => sidecar.authority_id).filter(Boolean));
    publishableWriteSet.publication_calibration_authorities = canonicalise(
      publicationAuthorities
        .filter((authority) => referencedAuthorityIds.has(authority.calibration_authority_id)),
    );
  }

  const quarantines = [...quarantinedClosureIds].sort().map((closureId) => {
    const affectedObjects = indexed
      .filter((entry) => entry.closureId === closureId)
      .map((entry) => ({ kind: entry.kind, object_id: entry.objectId, source_object: entry.row }));
    const closureResiduals = residuals.filter((residual) => residual.closure_id === closureId);
    const payload = {
      closure_id: closureId,
      affected_objects: affectedObjects,
      residual_ids: closureResiduals.map((entry) => entry.residual_id).sort(),
    };
    return {
      quarantine_id: contentId('CANONICAL_QUARANTINE/V1', payload),
      reason_code: 'UNRESOLVED_RESIDUAL',
      ...payload,
    };
  });

  const eligibleClaimIds = new Set((publishableWriteSet.publication_dispositions || [])
    .filter((sidecar) => sidecar.disposition === PUBLICATION_DISPOSITIONS.ELIGIBLE)
    .map((sidecar) => sidecar.claim_revision_id));
  const unattendedPublicationCandidateWriteSet = {
    claims: publishableWriteSet.claims.filter((claim) => eligibleClaimIds.has(claim.claim_revision_id)),
    publication_dispositions: (publishableWriteSet.publication_dispositions || [])
      .filter((sidecar) => eligibleClaimIds.has(sidecar.claim_revision_id)),
  };

  return {
    accepted: true,
    publishableWriteSet,
    unattendedPublicationCandidateWriteSet,
    residuals,
    quarantines,
    quarantinedClosureIds: [...quarantinedClosureIds].sort(),
    counts: {
      // Includes `conditional_termination_fee_values` (Step 4A3): it is a
      // real write, not a reference, the same reasoning
      // `supabase/canonical-v2-foundation.sql`'s Step 4A2
      // `publishable_object_count` sum already applies -- see that query's
      // own comment for why it, alone among the closure-keyed exclusions,
      // keeps counting this kind.
      publishable: COLLECTION_KEYS.reduce((total, key) => total + publishableWriteSet[key].length, 0)
        + (publishableWriteSet.conditional_termination_fee_values || []).length,
      residuals: residuals.length,
      quarantinedClosures: quarantines.length,
    },
  };
}

function validateCanonicalWriteSet(writeSet, contractBundle) {
  validateContractBundle(contractBundle);
  return validateWithAdmittedSources(
    writeSet,
    contractBundle,
    assertWriteSetShape(writeSet, contractBundle),
  );
}

function validateResolvedCanonicalWriteSet({
  writeSet,
  contractBundle,
  admittedSourceContexts,
  retainedCanonicalObjects = [],
  publicationGate,
} = {}) {
  validateContractBundle(contractBundle);
  return validateWithAdmittedSources(
    writeSet,
    contractBundle,
    assertResolvedWriteSetShape(writeSet, admittedSourceContexts, contractBundle),
    retainedCanonicalObjects,
    publicationGate,
  );
}

module.exports = {
  CanonicalValidationError,
  canonicalise,
  validateCanonicalWriteSet,
  validateResolvedCanonicalWriteSet,
  // Exported for direct provenance-tag tests (spec section 5 / plan Task 5).
  WRITE_SET_ORIGINS,
  ANSWER_PROVENANCE_TAGS,
  validateAnswerProvenanceRow,
  // Exported so the P2 three-place lockstep test (docs/superpowers/specs/
  // 2026-08-02-p2-qualifier-kinds-design.md section 3) can assert this
  // copy agrees byte-for-byte with candidate-resolution.js's own copy,
  // mirroring that module's existing export of the same function.
  canonicalValueAllowed,
};
