...
const WRITE_SET_KEYS = new Set(['deal', 'excerpts', 'provisions', 'claims', 'relationships']);
...
function assertWriteSetShape(writeSet) {
...
function expectedObjectId(kind, row) {
  if (kind === 'excerpts') {
    const payload = pick(row, ['schema_version', 'canonical_text_id', 'absolute_start', 'absolute_end']);
    return row.excerpt_id === row.semantic_span_id
      && SHA256_RE.test(row.exact_bytes_digest || '')
      && row.semantic_span_id === contentId('SEMANTIC_SPAN/V1', payload)
      ? row.excerpt_id
      : null;
  }
  if (kind === 'provisions') {
...
function structuralResiduals(kind, row, objectId, closureId, vocabulary) {
  const residuals = [];
  if (kind === 'provisions') {
  ...
  if (expectedObjectId(kind, row) !== objectId) {
...
function validateCanonicalWriteSet(writeSet, contractBundle) {
  ...
  for (const kind of COLLECTION_KEYS) {
    for (const [index, row] of list(writeSet[kind], `writeSet.${kind}`).entries()) {
      ...
      const found = [
        ...structuralResiduals(kind, row, objectId, closureId, vocabulary),
