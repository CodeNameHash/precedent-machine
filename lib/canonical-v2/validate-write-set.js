...
function assertResolvedWriteSetShape(writeSet, admittedSourceContexts, contractBundle) {
  ...
  // Two INDEPENDENTLY optional keys: `persisted_object_references` (pre-
  // existing) and `write_set_origin` (this task's addition -- the staged
  // validator discriminator). A caller may supply neither, either, or both,
  // so every combination is a distinct allowed key set -- generated rather
  // than hand-enumerated, so this stays correct as more optional keys are
  // ever added.
  const optionalKeys = ['persisted_object_references', 'write_set_origin'];
