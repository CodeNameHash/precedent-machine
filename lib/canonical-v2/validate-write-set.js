...
const COLLECTION_KEYS = [...CANONICAL_COLLECTION_KEYS, ...OPEN_WORLD_COLLECTION_KEYS];
...
function semanticReferenceResiduals(indexed) {
  ...
  for (const entry of indexed) {
    if (entry.kind === 'components') {
      ...
      if (!parent
        || parent.closureId !== entry.closureId
    ...
    if (entry.kind === 'claims') {
      ...
      if (!subject || subject.closureId !== entry.closureId) add(entry);
    }
    if (entry.kind === 'relationships') {
      ...
      if (!source
        || source.closureId !== entry.closureId
        || targets.some((target) => !target || target.closureId !== entry.closureId)) add(entry);
    }
  }
  return residuals;
}
...
function validateCanonicalWriteSet(writeSet, contractBundle) {
  validateContractBundle(contractBundle);
  const admittedSources = assertWriteSetShape(writeSet, contractBundle);
  ...
  const sourceOrdinalByCanonicalTextId = new Map(admittedSources.admissions.map((admission) => {
  ...
  if (admittedSources.mode === 'SINGULAR') {
