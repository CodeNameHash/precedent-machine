...
const PROCESS_CONTROLLED_CODE_REGISTRY_INPUT_KIND =
  'PROCESS_CONTROLLED_CODE_REGISTRY_INPUT';
const PROCESS_CONTROLLED_CODE_REGISTRY_INPUT_SCHEMA =
...
const PROCESS_EXPECTED_OCCURRENCE_SLOT_IDS = Object.freeze([
...
const PROCESS_EVENT_SLOT_BINDING_DEFINITION_DIGEST =
  '27a8787abc0e2fd754c56f3bf09ac88aff17fe19ea3f1183711a322d36468d4d';
const PROCESS_CONTROLLED_CODE_REGISTRY_DEFINITION_DIGEST =
  '19856718d288b3fdd98eeead43ec97b350bcf915da781e8b3787bc6b81c022ff';
...
const PROCESS_OBJECT_DEFINITION_LOCKS = Object.freeze({
  ...
  PROCESS_PREDICATE_WITNESS: Object.freeze({
    ...
    definition_digest: 'f3369a32c298af91b3762bbfc817f94b734cf15b98905738ed58d0586ef74a53',
    error_code: 'INVALID_PROCESS_PREDICATE_WITNESS_INPUT',
  }),
  PROCESS_RELATIONSHIP: Object.freeze({
    logical_type: 'ProcessRelationship',
    logical_type_version: 2,
    definition_digest: 'ff5884c843d21f7a009436e78d7ad1ea59835856fe6ce4a0ef39afb4d641f8c9',
...
function validateLockedProcessObject(member, stableId) {
  ...
  if (
    ...
    || value.definition?.logical_type_version
      !== (contract.logical_type_version || 1)
  ) {
    fail(
...
function validateEventSlotBinding(member) {
  const code = 'INVALID_PROCESS_EVENT_SLOT_BINDING_INPUT';
  const value = member.canonical_value;
  requireExactKeys(value, [
    'object_kind',
    'stable_id',
    'schema_version',
    'definition',
  ], 'Process event-slot binding input', code);
  if (
    value.object_kind !== PROCESS_EXPECTED_OCCURRENCE_SLOT_INPUT_KIND
    || value.stable_id !== 'PROCESS_EVENT_SLOT_BINDING'
    || value.schema_version !== PROCESS_EXPECTED_OCCURRENCE_SLOT_INPUT_SCHEMA
    || value.definition?.domain_key !== 'PROCESS'
    || value.definition?.slot_key !== 'PROCESS_EVENT_SLOT_BINDING'
    || value.definition?.slot_version !== 1
    || value.definition?.slot_schema
      !== 'PROCESS_EVENT_EXPECTED_OCCURRENCE_SLOT_BINDING/V1'
    || value.definition?.runtime_logical_type !== 'ExpectedOccurrenceSlot'
    || value.definition?.owner_logical_type !== 'ProcessEvent'
    || value.definition?.owner_definition_stable_id !== 'PROCESS_EVENT'
    || value.definition?.consumer_logical_type !== 'ProcessParticipant'
    || value.definition?.consumer_definition_stable_id
      !== 'PROCESS_PARTICIPANT'
    || value.definition?.cardinality !== 'REPEATABLE'
  ) {
    fail(
      'UNREGISTERED_PROCESS_EXPECTED_OCCURRENCE_SLOT',
      'The Process event-slot binding identity is not registered.',
    );
  }
  let actualDigest;
  try {
    ...
    fail(code, 'The Process event-slot binding is not canonical JSON.', {
      cause: error.message,
    });
  }
  if (actualDigest !== PROCESS_EVENT_SLOT_BINDING_DEFINITION_DIGEST) {
    fail(
      code,
      'The Process event-slot binding does not match its governed definition.',
      {
        expected_definition_digest:
          PROCESS_EVENT_SLOT_BINDING_DEFINITION_DIGEST,
        actual_definition_digest: actualDigest,
      },
    );
  }
...
function validateControlledCodeRegistry(member) {
  const code = 'INVALID_PROCESS_CONTROLLED_CODE_REGISTRY_INPUT';
  const value = member.canonical_value;
  requireExactKeys(value, [
    'object_kind',
    'stable_id',
    'schema_version',
    'definition',
  ], 'Process controlled-code registry input', code);
  if (
    value.object_kind !== PROCESS_CONTROLLED_CODE_REGISTRY_INPUT_KIND
    || value.stable_id !== 'PROCESS_CONTROLLED_CODE_REGISTRY'
    || value.schema_version !== PROCESS_CONTROLLED_CODE_REGISTRY_INPUT_SCHEMA
    || value.definition?.domain_key !== 'PROCESS'
    || value.definition?.registry_version !== 2
    || value.definition?.registry_state !== 'CLOSED'
  ) {
    fail(
      'UNREGISTERED_PROCESS_CONTROLLED_CODE_REGISTRY',
      'The Process controlled-code registry identity is not registered.',
    );
  }
  let actualDigest;
  try {
    ...
    fail(code, 'The Process controlled-code registry is not canonical JSON.', {
      cause: error.message,
    });
  }
  if (actualDigest !== PROCESS_CONTROLLED_CODE_REGISTRY_DEFINITION_DIGEST) {
    fail(
      code,
      'The Process controlled-code registry does not match its governed definition.',
      {
        expected_definition_digest:
          PROCESS_CONTROLLED_CODE_REGISTRY_DEFINITION_DIGEST,
        actual_definition_digest: actualDigest,
      },
    );
  }
...
function validateAuthoredProcessInputs(authoredMembers) {
  ...
  const controlledCodeRegistryMembers = processMembers.filter(
    (member) => member.object_kind
      === PROCESS_CONTROLLED_CODE_REGISTRY_INPUT_KIND,
  );
  if (controlledCodeRegistryMembers.length !== 1) {
    fail(
      'PROCESS_CONTROLLED_CODE_REGISTRY_CARDINALITY_MISMATCH',
      'The Process contract requires one controlled-code registry.',
      {
        controlled_code_registry_count:
          controlledCodeRegistryMembers.length,
      },
    );
  }
  validateControlledCodeRegistry(controlledCodeRegistryMembers[0]);
  ...
  const actualSlotIds = slotMembers
    .map((member) => member.canonical_value?.stable_id)
    .sort();
  if (
    domainMembers.length !== 1
    || canonicalJson(actualSlotIds)
      !== canonicalJson(PROCESS_EXPECTED_OCCURRENCE_SLOT_IDS)
  ) {
    fail(
      'PROCESS_BASE_CONTRACT_CARDINALITY_MISMATCH',
      'The Process base contract requires one domain registry and its complete expected-slot set.',
      {
        domain_registry_count: domainMembers.length,
        actual_slot_ids: actualSlotIds,
        expected_slot_ids: PROCESS_EXPECTED_OCCURRENCE_SLOT_IDS,
      },
    );
  }
  validateDomainRegistry(domainMembers[0]);
  validateEventSlotBinding(slotMembers.find(
    (member) => member.canonical_value.stable_id
      === 'PROCESS_EVENT_SLOT_BINDING',
  ));
  validateNarrationSlot(slotMembers.find(
    (member) => member.canonical_value.stable_id
      === 'PROCESS_NARRATION',
  ));
  ...
  const narrationSlot = slotMembers.find(
    (member) => member.canonical_value.stable_id === 'PROCESS_NARRATION',
  ).canonical_value;
  const eventSlotBinding = slotMembers.find(
    (member) => member.canonical_value.stable_id
      === 'PROCESS_EVENT_SLOT_BINDING',
  ).canonical_value;
  if (
    narration.definition.expected_occurrence_slot_key
      !== narrationSlot.stable_id
    || narrationSlot.definition.owner_definition_stable_id
      !== narration.stable_id
    || canonicalJson(narration.definition.occurrence_identity.stable_id_inputs)
      !== canonicalJson(narrationSlot.definition.identity_inputs)
  ) {
    fail(
  ...
  const event = logicalMembers.find(
    (member) => member.canonical_value.stable_id === 'PROCESS_EVENT',
  ).canonical_value.definition;
  const participant = logicalMembers.find(
    (member) => member.canonical_value.stable_id
      === 'PROCESS_PARTICIPANT',
  ).canonical_value.definition;
  if (
    event.composition_contract.event_slot_binding_definition_stable_id
      !== eventSlotBinding.stable_id
    || participant.event_binding_contract
      .event_slot_binding_definition_stable_id
      !== eventSlotBinding.stable_id
    || eventSlotBinding.definition.owner_definition_stable_id
      !== 'PROCESS_EVENT'
    || eventSlotBinding.definition.consumer_definition_stable_id
      !== 'PROCESS_PARTICIPANT'
    || canonicalJson(
      eventSlotBinding.definition
        .process_event_identity_binding_contract.stable_id_inputs,
    ) !== canonicalJson(event.event_identity.stable_id_inputs)
  ) {
    fail(
      'PROCESS_EVENT_SLOT_BINDING_MISMATCH',
      'The Process event, participant, and expected event slot do not bind exactly.',
    );
  }

  const fieldCatalogueMember = processMembers.find(
  ...
  if (fieldCatalogueMember) {
    const actualReferences = [...new Set(
      fieldCatalogueMember.canonical_value.definition.field_definitions
        .map((field) => field.required_value_registry),
    )].sort();
    const expectedReferences = controlledCodeRegistryMembers[0]
      .canonical_value.definition
      .field_catalogue_registry_reference_contract
      .required_registry_references;
    if (canonicalJson(actualReferences) !== canonicalJson(expectedReferences)) {
      fail(
        'PROCESS_FIELD_CATALOGUE_REGISTRY_REFERENCE_MISMATCH',
        'The Process field catalogue and controlled-code registry references do not reconcile exactly.',
        {
          actual_registry_references: actualReferences,
          expected_registry_references: expectedReferences,
        },
      );
    }
  }
