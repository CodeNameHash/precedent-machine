...
const {
  ...
  validateReadOnlyNoShopProposalBatch,
} = require('../parser-v2/canonical-proposals');
...
const REVIEWED_SECTION = Object.freeze({
  ...
  proposal_batch_id: '66960757085c2ea56f7836aeef270269056a5b2c1c23cc6b2ebd5e5fd3a30934',
  proposal_id: '81b4c13bf357cbb078b88745e402a5abad86ae4b353fef8c290e616bf760bf3a',
  detector_key: 'PARSER_V2_TRY_DETERMINISTIC_NO_SHOP/V1',
  legacy_provision_type_hint: 'NOSOL',
});
...
function requireSpansWithinProposal(spans, proposal) {
  const anchor = proposal.evidence_anchor;
  for (const [label, span] of Object.entries(spans)) {
    if (span.canonical_text_id !== anchor.canonical_text_id
      || span.absolute_start < anchor.absolute_start
      || span.absolute_end > anchor.absolute_end) {
      throw new TypeError(`reviewed ${label} evidence escapes the source-backed Section 5.3 proposal`);
    }
  }
}

function buildReviewedNoShopSlice({
  sourceText,
  contractBundle,
  corpusReleaseId,
  proposalBatch,
  ...
  const resolvedProposalBatch = proposalBatch || buildReadOnlyNoShopProposalBatch({
  ...
  validateReadOnlyNoShopProposalBatch({
    proposal_batch: resolvedProposalBatch,
    contract_bundle: contractBundle,
    source,
    source_admission: sourceAdmission,
    governed_deal_key: DEAL_KEY,
  ...
  if (resolvedProposalBatch.proposal_batch_id !== REVIEWED_SECTION.proposal_batch_id) {
    throw new TypeError('reviewed no-shop proposal batch has drifted');
  }
  const sectionMatches = resolvedProposalBatch.proposals.filter(
  ...
  if (section.section_title !== REVIEWED_SECTION.title
    || section.source_backed_provision_proposal_id !== REVIEWED_SECTION.proposal_id
    || section.detector_key !== REVIEWED_SECTION.detector_key
    || section.legacy_provision_type_hint !== REVIEWED_SECTION.legacy_provision_type_hint) {
  ...
  requireSpansWithinProposal(allSpans, section);
  ...
  const reviewedMapping = {
    ...
    review_version: REVIEW_VERSION,
    document_hash: source.document_hash,
    proposal_batch_id: resolvedProposalBatch.proposal_batch_id,
    source_backed_provision_proposal_ids: [section.source_backed_provision_proposal_id],
  ...
  return Object.freeze({
    ...
    source,
    sourceAdmission,
    proposalBatch: resolvedProposalBatch,
    section,
