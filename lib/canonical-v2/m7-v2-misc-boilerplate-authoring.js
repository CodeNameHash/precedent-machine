...
/**
 * Family-local M7 V2 repair authoring for MISC_BOILERPLATE (N1 family #24).
 *
 ...
 *
 * The 114 profiles are claim-scale, one per governed comparator M4 claim across the
 * six comparator deals (Concho, Metsera, Modiv, Skechers, Skywater, TopBuild). Claim scale rather than
 * section scale because TopBuild Article VII carries twenty-six independently operative limbs;
 ...
 * claim definition key under all twelve subtype buckets — so every profile
 * carries LEGAL_GROUPING_REVIEW_REQUIRED, 12 rows additionally
 * carry SUBTYPE_PARTITION_DIVERGES_FROM_CALIBRATION_PROPOSED_SUBTYPE, and 102 rows
...
const MISC_BOILERPLATE_PROFILE_COUNT = 114;
const MISC_BOILERPLATE_SUBTYPE_DIVERGENCE_PROFILE_COUNT = 12;
const MISC_BOILERPLATE_OUTSIDE_CALIBRATION_PROFILE_COUNT = 102;
const MISC_BOILERPLATE_REGISTERED_SUBTYPE_BUCKET_COUNT = 12;
const MISC_BOILERPLATE_POPULATED_SUBTYPE_BUCKET_COUNT = 12;

const MISC_BOILERPLATE_REVIEW_FLAGS = Object.freeze({
...
const MISC_BOILERPLATE_PHASE2_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_MISC_BOILERPLATE_AUTHORING_PHASE2_AUTHORITY/V2';
const MISC_BOILERPLATE_PHASE2_AUTHORITY_ID =
  'f18f30c873ed97fb4112cdeb727350cc49726578ffc7a817a824baff77a2e84e';
const MISC_BOILERPLATE_PHASE2_AUTHORITY_PATH =
...
const MISC_BOILERPLATE_PHASE2_AUTHORITY_BYTES = 275394;
const MISC_BOILERPLATE_PHASE2_AUTHORITY_SHA256 =
  '531194f0ee3ac3199eeca1542584b8c6f8d88ec63eb327c397410969239ae45b';

const MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES = Object.freeze({
  AUTHORITY: 'M7_V2_MISC_BOILERPLATE_PHASE2_AUTHORITY',
  CONTRACT: 'M7_V2_MISC_BOILERPLATE_PHASE2_PROPOSAL_CONTRACT',
  COVERAGE: 'M7_V2_MISC_BOILERPLATE_PHASE2_SOURCE_COVERAGE',
});

const MISC_BOILERPLATE_PHASE2_PROPOSAL_KEYS = Object.freeze([
  'schema_version',
  'proposal_id',
  'family_key',
  'proposal_state',
...
function validateMiscBoilerplateProposalAuthority(envelope) {
  const code = MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES.AUTHORITY;
  validateEnvelopeShape(envelope, code, 'Phase2 authority');
  validateBoundRecord(envelope, code, 'Phase2 authority');
  const { binding, record } = envelope;
  if (
    binding.byte_length !== MISC_BOILERPLATE_PHASE2_AUTHORITY_BYTES
    || binding.path !== MISC_BOILERPLATE_PHASE2_AUTHORITY_PATH
    || binding.record_id !== MISC_BOILERPLATE_PHASE2_AUTHORITY_ID
    || binding.record_id_field !== 'misc_boilerplate_authoring_phase2_authority_id'
    || binding.schema_version !== MISC_BOILERPLATE_PHASE2_AUTHORITY_SCHEMA
    || binding.sha256 !== MISC_BOILERPLATE_PHASE2_AUTHORITY_SHA256
  ) {
    fail(code, 'Phase2 authority binding drift.');
  }
  if (
    record.schema_version !== MISC_BOILERPLATE_PHASE2_AUTHORITY_SCHEMA
    || record.misc_boilerplate_authoring_phase2_authority_id
      !== MISC_BOILERPLATE_PHASE2_AUTHORITY_ID
  ) {
    fail(code, 'Phase2 authority identity drift.');
  }
  const unsigned = { ...record };
  delete unsigned.misc_boilerplate_authoring_phase2_authority_id;
  if (contentId(record.schema_version, unsigned) !== MISC_BOILERPLATE_PHASE2_AUTHORITY_ID) {
    fail(code, 'Phase2 authority self identity drift.');
  }
  return deepFreeze(clone(envelope));
}

function miscBoilerplateAgreementSources(authority, agreementEvidenceByAgreementId) {
  const code = MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES.COVERAGE;
...
function validateMiscBoilerplateProposalGovernedSources(authority, governedSources) {
  const code = MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES.COVERAGE;
  exactKeysOrFail(
    governedSources,
    [
  ...
  return miscBoilerplateAgreementSources(
    authority,
    governedSources.agreementEvidenceByAgreementId,
  );
}

function validateMiscBoilerplateProposalSourceCoverage(authority, agreements) {
  const code = MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES.COVERAGE;
...
function miscBoilerplateProposalCoverageRecords(authority, coverage) {
...
function miscBoilerplateProposalPartition(coverage) {
...
function miscBoilerplateProposalInventoryDigest(coverage, proposedPartition) {
...
function prepareMiscBoilerplatePhase2FamilyProposal(input) {
  const contractCode = MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES.CONTRACT;
  exactKeysOrFail(
    input,
    ['miscBoilerplateAuthoringPhase2Authority', 'governedSources'],
    contractCode,
    'Misc Boilerplate proposal input',
  );
  const authorityEnvelope = validateMiscBoilerplateProposalAuthority(
    input.miscBoilerplateAuthoringPhase2Authority,
  );
  const authority = authorityEnvelope.record;
  const agreements = validateMiscBoilerplateProposalGovernedSources(
    authority,
    input.governedSources,
  );
  const coverage = validateMiscBoilerplateProposalSourceCoverage(authority, agreements);
  const accounting = miscBoilerplateProposalCoverageRecords(authority, coverage);
  const proposedPartition = miscBoilerplateProposalPartition(coverage);
  ...
  const unresolvedItems = [
    ...
    MISC_BOILERPLATE_REVIEW_FLAGS.LEGAL_GROUPING,
  ].sort(compareStrings);
  const unsignedProposal = {
    ...
    inventory_digest: miscBoilerplateProposalInventoryDigest(coverage, proposedPartition),
...
const MISC_BOILERPLATE_PHASE4_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_MISC_BOILERPLATE_AUTHORING_PHASE4_FAMILY_PROFILE_PACKAGE_REVIEW_AUTHORITY/V1';
const MISC_BOILERPLATE_PHASE4_AUTHORITY_ID =
  '97567a49a02dd9af4d391138345a343b9347b88b05263cd2ae24155079fce66b';
const MISC_BOILERPLATE_PHASE4_AUTHORITY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-misc-boilerplate-authoring-phase4-family-profile-package-review-authority.json';
const MISC_BOILERPLATE_PHASE4_AUTHORITY_BYTES = 123651;
const MISC_BOILERPLATE_PHASE4_AUTHORITY_SHA256 =
  '5fe6c60ef492656b3de806f32be8526682cdfc02a3cbe2feae41a65b480f2bd4';
const MISC_BOILERPLATE_PHASE4_SCHEDULE_SHA256 =
  '5362b737b15389c5b16a777be42722b47a49f17cd8f402d30193ab15e3e73869';
const MISC_BOILERPLATE_PHASE4_CANDIDATE_SCHEMA =
  'M7_V2_MISC_BOILERPLATE_PHASE4_FAMILY_PROFILE_PACKAGE_REVIEW_CANDIDATE/V1';
const MISC_BOILERPLATE_PHASE4_CANDIDATE_STATE =
  'REVIEW_ONLY_114_PROFILES_UNAPPROVED_AWAITING_BEN_INVENTORY';

const MISC_BOILERPLATE_PHASE4_REVIEW_CODES = Object.freeze({
  CONTRACT: 'M7_V2_MISC_BOILERPLATE_PHASE4_REVIEW_CONTRACT',
  AUTHORITY: 'M7_V2_MISC_BOILERPLATE_PHASE4_REVIEW_AUTHORITY',
  PHASE2_PROPOSAL: 'M7_V2_MISC_BOILERPLATE_PHASE4_PHASE2_PROPOSAL',
  PROFILE_SCHEDULE: 'M7_V2_MISC_BOILERPLATE_PHASE4_PROFILE_SCHEDULE',
  REVIEW_OUTPUT: 'M7_V2_MISC_BOILERPLATE_PHASE4_REVIEW_OUTPUT',
});

const MISC_BOILERPLATE_PHASE4_REVIEW_INPUT_KEYS = Object.freeze([
  'miscBoilerplateAuthoringPhase4FamilyProfilePackageReviewAuthority',
  'miscBoilerplateAuthoringPhase2Authority',
  'governedSources',
]);

const MISC_BOILERPLATE_PHASE4_REVIEW_OUTPUT_KEYS = Object.freeze([
  'schema_version',
  'review_candidate_id',
  'family_key',
  'candidate_state',
...
const MISC_BOILERPLATE_PHASE4_AUTHORITY_ROOT_KEYS = Object.freeze([
  'authority_classification',
  'authority_state',
  'candidate_output_contract',
  'design_basis',
  'execution_schedule',
  'misc_boilerplate_authoring_phase4_family_profile_package_review_authority_id',
...
function miscBoilerplatePhase4ExpectedParentBindings() {
  return {
    misc_boilerplate_authoring_phase2_authority: {
      byte_length: MISC_BOILERPLATE_PHASE2_AUTHORITY_BYTES,
      path: MISC_BOILERPLATE_PHASE2_AUTHORITY_PATH,
      record_id: MISC_BOILERPLATE_PHASE2_AUTHORITY_ID,
      record_id_field: 'misc_boilerplate_authoring_phase2_authority_id',
      schema_version: MISC_BOILERPLATE_PHASE2_AUTHORITY_SCHEMA,
      sha256: MISC_BOILERPLATE_PHASE2_AUTHORITY_SHA256,
    },
  };
}

function miscBoilerplateContainsForbiddenKey(value, forbiddenKeys, seen = new Set()) {
  if (!value || typeof value !== 'object') return false;
  if (seen.has(value)) return false;
  seen.add(value);
  if (Array.isArray(value)) {
    return value.some((member) => miscBoilerplateContainsForbiddenKey(
      member,
      forbiddenKeys,
      seen,
    ));
  ...
  for (const [key, member] of Object.entries(value)) {
    ...
    if (miscBoilerplateContainsForbiddenKey(member, forbiddenKeys, seen)) return true;
  }
  return false;
}

function validateMiscBoilerplatePhase4FamilyProfilePackageReviewAuthority(envelope) {
  const code = MISC_BOILERPLATE_PHASE4_REVIEW_CODES.AUTHORITY;
  validateEnvelopeShape(envelope, code, 'Phase4 family profile package review authority');
  const { binding, record } = envelope;
  if (
    binding.byte_length !== MISC_BOILERPLATE_PHASE4_AUTHORITY_BYTES
    || binding.path !== MISC_BOILERPLATE_PHASE4_AUTHORITY_PATH
    || binding.record_id !== MISC_BOILERPLATE_PHASE4_AUTHORITY_ID
    || binding.record_id_field
      !== 'misc_boilerplate_authoring_phase4_family_profile_package_review_authority_id'
    || binding.schema_version !== MISC_BOILERPLATE_PHASE4_AUTHORITY_SCHEMA
    || binding.sha256 !== MISC_BOILERPLATE_PHASE4_AUTHORITY_SHA256
  ) {
  ...
  if (
    !exactKeys(record, MISC_BOILERPLATE_PHASE4_AUTHORITY_ROOT_KEYS)
    || record.schema_version !== MISC_BOILERPLATE_PHASE4_AUTHORITY_SCHEMA
    || record.misc_boilerplate_authoring_phase4_family_profile_package_review_authority_id
      !== MISC_BOILERPLATE_PHASE4_AUTHORITY_ID
  ) {
    fail(code, 'Phase4 family profile package review authority identity drift.');
  }
  const unsigned = { ...record };
  delete unsigned.misc_boilerplate_authoring_phase4_family_profile_package_review_authority_id;
  if (contentId(record.schema_version, unsigned) !== MISC_BOILERPLATE_PHASE4_AUTHORITY_ID) {
  ...
  const expectedErrorCodes = Object.values(MISC_BOILERPLATE_PHASE4_REVIEW_CODES);
  let scheduleBytes;
  try {
  ...
  if (
    ...
    || !sameValue(
      ...
      miscBoilerplatePhase4ExpectedParentBindings(),
    )
    || !sameValue(
      implementation.exact_outer_input_keys,
      MISC_BOILERPLATE_PHASE4_REVIEW_INPUT_KEYS,
    )
    || implementation.exported_function !== 'prepareMiscBoilerplateFamilyProfilePackageReview'
    || implementation.phase2_internal_function
      !== 'prepareMiscBoilerplatePhase2FamilyProposal'
    ...
    || output.schema_version !== MISC_BOILERPLATE_PHASE4_CANDIDATE_SCHEMA
    || output.record_id_field !== 'review_candidate_id'
    || output.candidate_state !== MISC_BOILERPLATE_PHASE4_CANDIDATE_STATE
    || output.profile_approval_state !== 'UNAPPROVED'
    || !sameValue(output.exact_keys, MISC_BOILERPLATE_PHASE4_REVIEW_OUTPUT_KEYS)
    || schedule.length !== MISC_BOILERPLATE_PROFILE_COUNT
    || scheduleContract.exact_profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || scheduleContract.exact_complete_profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || scheduleContract.exact_incomplete_profile_count !== 0
    || scheduleContract.schedule_canonical_json_sha256
      !== MISC_BOILERPLATE_PHASE4_SCHEDULE_SHA256
    || sha256Hex(scheduleBytes) !== MISC_BOILERPLATE_PHASE4_SCHEDULE_SHA256
...
function miscBoilerplatePhase4ValidatePhase2Proposal(proposal) {
  const code = MISC_BOILERPLATE_PHASE4_REVIEW_CODES.PHASE2_PROPOSAL;
  if (
    !isObject(proposal)
    || proposal.schema_version !== 'M7_V2_MISC_BOILERPLATE_FAMILY_PROPOSAL/V1'
    || proposal.family_key !== 'MISC_BOILERPLATE'
    || proposal.profile_approval_state !== 'UNAPPROVED'
    || proposal.source_terminal_coverage.accounted_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || proposal.m4_claim_accounting.accounted_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || proposal.derived_profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || !Array.isArray(proposal.proposed_partition.proposed_profiles)
    || proposal.proposed_partition.proposed_profiles.length !== MISC_BOILERPLATE_PROFILE_COUNT
    || proposal.proposed_partition.source_unit_assignment_count
      !== MISC_BOILERPLATE_PROFILE_COUNT
    || proposal.proposed_partition.m4_claim_assignment_count !== MISC_BOILERPLATE_PROFILE_COUNT
  ) {
...
function miscBoilerplatePhase4DeriveProfiles(authority, phase2Proposal) {
  const scheduleCode = MISC_BOILERPLATE_PHASE4_REVIEW_CODES.PROFILE_SCHEDULE;
...
function prepareMiscBoilerplateFamilyProfilePackageReview(input) {
  const contractCode = MISC_BOILERPLATE_PHASE4_REVIEW_CODES.CONTRACT;
  exactKeysOrFail(
    input,
    MISC_BOILERPLATE_PHASE4_REVIEW_INPUT_KEYS,
    contractCode,
    'Misc Boilerplate Phase4 package review input',
  );
  const authorityEnvelope = validateMiscBoilerplatePhase4FamilyProfilePackageReviewAuthority(
    input.miscBoilerplateAuthoringPhase4FamilyProfilePackageReviewAuthority,
  );
  const authority = authorityEnvelope.record;
  const phase2AuthorityEnvelope = validateMiscBoilerplateProposalAuthority(
    input.miscBoilerplateAuthoringPhase2Authority,
  );
  if (
    phase2AuthorityEnvelope.binding.record_id
      !== authority.immutable_parent_bindings
        .misc_boilerplate_authoring_phase2_authority.record_id
  ) {
    fail(
      MISC_BOILERPLATE_PHASE4_REVIEW_CODES.AUTHORITY,
      'Phase4 parent Phase2 authority pin drift.',
    );
  }
  validateMiscBoilerplateProposalGovernedSources(
  ...
  let phase2Proposal;
  try {
    phase2Proposal = prepareMiscBoilerplatePhase2FamilyProposal({
      miscBoilerplateAuthoringPhase2Authority: input.miscBoilerplateAuthoringPhase2Authority,
      governedSources: input.governedSources,
    });
  } catch (error) {
    fail(
      MISC_BOILERPLATE_PHASE4_REVIEW_CODES.PHASE2_PROPOSAL,
      'Phase4 fresh Phase2 proposal failed.',
      { cause_code: typeof error.code === 'string' ? error.code : null },
    );
  }
  miscBoilerplatePhase4ValidatePhase2Proposal(phase2Proposal);
  const proposedProfiles = miscBoilerplatePhase4DeriveProfiles(authority, phase2Proposal);
  const authorityBinding = {
    path: authorityEnvelope.binding.path,
    schema_version: authorityEnvelope.binding.schema_version,
    record_id_field: authorityEnvelope.binding.record_id_field,
  ...
  const unsignedCandidate = {
    schema_version: MISC_BOILERPLATE_PHASE4_CANDIDATE_SCHEMA,
    family_key: 'MISC_BOILERPLATE',
  ...
  if (
    !exactKeys(candidate, MISC_BOILERPLATE_PHASE4_REVIEW_OUTPUT_KEYS)
    || proposedProfiles.length !== MISC_BOILERPLATE_PROFILE_COUNT
    || proposedProfiles.some((profile) => (
      ...
      || !profile.review_flags.includes(MISC_BOILERPLATE_REVIEW_FLAGS.LEGAL_GROUPING)
    ))
    || proposedProfiles.filter((profile) => profile.review_flags.includes(
      MISC_BOILERPLATE_REVIEW_FLAGS.SUBTYPE_DIVERGENCE,
    )).length !== MISC_BOILERPLATE_SUBTYPE_DIVERGENCE_PROFILE_COUNT
    || proposedProfiles.filter((profile) => profile.review_flags.includes(
      MISC_BOILERPLATE_REVIEW_FLAGS.OUTSIDE_CALIBRATION,
    )).length !== MISC_BOILERPLATE_OUTSIDE_CALIBRATION_PROFILE_COUNT
    ...
    || miscBoilerplateContainsForbiddenKey(candidate, forbiddenKeys)
  ) {
    fail(
      MISC_BOILERPLATE_PHASE4_REVIEW_CODES.REVIEW_OUTPUT,
...
const MISC_BOILERPLATE_WORK3_CONTROL_PATH =
 ...
 * binds, not by a Misc Boilerplate-specific ruling note. Nothing here invents a
 * lawyer decision.
 */
const MISC_BOILERPLATE_WORK3_RULINGS_BINDING = Object.freeze({
  byte_length: 1519,
  path: `${MISC_BOILERPLATE_WORK3_CONTROL_PATH}/m5-programme-rulings.json`,
  sha256: '2711dc5c958da271bfd86a154712c251978ac1f1aec713d22302946bf8f87497',
});
const MISC_BOILERPLATE_WORK3_INVENTORY_AUTHORITY_BINDING = Object.freeze({
  byte_length: 2134,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-work3-misc-boilerplate-unapproved-inventory-review-authority.json',
  record_id: '0428eb40c3a9f50c49e6e7e7a6c06cbc48dd81205cdc27c3c68ecb95d16cbef4',
  record_id_field: 'work3_misc_boilerplate_unapproved_inventory_review_authority_id',
  schema_version: 'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MISC_BOILERPLATE_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1',
  sha256: 'fd9eee531da1074e7a993c578d5ab20dcf577c3ea8f2a7fac65ddd1aaca2ba2c',
});
const MISC_BOILERPLATE_WORK3_PACKET_BINDING = Object.freeze({
  byte_length: 136861,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-misc-boilerplate-114-profile-inventory-review-packet-draft.json',
  record_id: '440a469bde98ab29c2953e0067584810859fa33c5e6090260172f1a03ab38267',
  record_id_field: 'inventory_review_packet_id',
  schema_version: 'STAGE_2Y_M7_V2_MISC_BOILERPLATE_114_PROFILE_INVENTORY_REVIEW_PACKET/V1',
  sha256: '0a6e819bfb9deb4dd619ae7abef36c59f9d0189ded550cbc86146836f709d1c5',
});
const MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING = Object.freeze({
  byte_length: 46199,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-misc-boilerplate-114-profile-inventory-disposition.json',
  record_id: 'd8bad05c39b5fb080a49de760eae376e6068b7feb7f409266619cf56273fb37c',
  record_id_field: 'inventory_disposition_id',
  schema_version: 'STAGE_2Y_M7_V2_MISC_BOILERPLATE_114_PROFILE_INVENTORY_DISPOSITION/V1',
  sha256: '90ef3243f75591c70ab5464a33b15c27e9b8736b7df1be4470a569478bb7d0bd',
});
const MISC_BOILERPLATE_WORK3_SESSION_BINDING = Object.freeze({
  byte_length: 1133,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-misc-boilerplate-ben-inventory-session-receipt.json',
  record_id: 'dbbafa54bbe5c748ddd87c4527f34e045f3fc9739c46fccd4c42e2c33568a2a2',
  record_id_field: 'ben_inventory_session_receipt_id',
  schema_version: 'STAGE_2Y_M7_V2_MISC_BOILERPLATE_BEN_INVENTORY_SESSION_RECEIPT/V1',
  sha256: '33ad0736be70f02ed7f908ca7dafcc8bef72a09151de391803ebe000cd880560',
});
const MISC_BOILERPLATE_WORK3_BEN_AUTHORITY_BINDING = Object.freeze({
  byte_length: 2829,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-work3-misc-boilerplate-ben-inventory-session-successor-authority.json',
  record_id: 'e48198ced27fec441c96ae9881381c2e2ab6a58e55f28804faad0834797047b8',
  record_id_field: 'work3_misc_boilerplate_ben_inventory_session_successor_authority_id',
  schema_version: 'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MISC_BOILERPLATE_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY/V1',
  sha256: '2d987baa55283c24f4157bb9f74c5ea1fff66db4b515801c7ef39cd29d1eaa3a',
});
const MISC_BOILERPLATE_WORK3_SEAL_AUTHORITY_BINDING = Object.freeze({
  byte_length: 3346,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-work3-misc-boilerplate-family-package-seal-successor-authority.json',
  record_id: '63175830b4ba18cac57c87e407bd9c4aad652eba7f428f53977df892d075ad4a',
  record_id_field: 'work3_misc_boilerplate_family_package_seal_successor_authority_id',
  schema_version: 'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MISC_BOILERPLATE_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY/V1',
  sha256: 'ef33f4c17fba24c5bfde749657e6712011d2eca35d75b7c02958bd018b15a9a0',
});
const MISC_BOILERPLATE_WORK3_SEAL_RECEIPT_BINDING = Object.freeze({
  byte_length: 2299,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-misc-boilerplate-family-package-seal-receipt.json',
  record_id: 'dfc53b08fe37bf091e7a8bec778178746b38cd872d89b9906cd7313c3ef66e45',
  record_id_field: 'misc_boilerplate_family_package_seal_receipt_id',
  schema_version: 'STAGE_2Y_M7_V2_MISC_BOILERPLATE_FAMILY_PACKAGE_SEAL_RECEIPT/V1',
  sha256: '3a1ea1be524951dde0bf1ea848502b5fcf73675c77336fccb992576173d881fc',
});
const MISC_BOILERPLATE_WORK3_REGISTRATION_AUTHORITY_BINDING = Object.freeze({
  byte_length: 2877,
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-work3-misc-boilerplate-registration-successor-authority.json',
  record_id: '8a7873dae675acda60225d633882ed22cb6d7d8f8653ff93fc860dc64bbd569b',
  record_id_field: 'work3_misc_boilerplate_registration_successor_authority_id',
  schema_version: 'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MISC_BOILERPLATE_REGISTRATION_SUCCESSOR_AUTHORITY/V1',
  sha256: 'b37acd61c252d9493341b06513d184dfc7d079597487f440f323000e08cfaf14',
});

const MISC_BOILERPLATE_WORK3_CODES = Object.freeze({
  CONTRACT: 'M7_V2_MISC_BOILERPLATE_WORK3_CONTRACT',
  AUTHORITY: 'M7_V2_MISC_BOILERPLATE_WORK3_AUTHORITY',
  INVENTORY: 'M7_V2_MISC_BOILERPLATE_WORK3_INVENTORY',
  DISPOSITION: 'M7_V2_MISC_BOILERPLATE_WORK3_DISPOSITION',
  RECEIPT: 'M7_V2_MISC_BOILERPLATE_WORK3_RECEIPT',
  OUTPUT: 'M7_V2_MISC_BOILERPLATE_WORK3_OUTPUT',
});

const MISC_BOILERPLATE_WORK3_WITHHELD_FIELDS = Object.freeze([
  'activation_id',
...
function miscBoilerplateWork3ValidatePinnedEnvelope(envelope, expected, label) {
  validateEnvelopeShape(envelope, MISC_BOILERPLATE_WORK3_CODES.AUTHORITY, label);
  if (!sameValue(envelope.binding, expected)) {
    fail(MISC_BOILERPLATE_WORK3_CODES.AUTHORITY, `${label} binding drift.`);
  }
  validateBoundRecord(envelope, MISC_BOILERPLATE_WORK3_CODES.AUTHORITY, label);
  ...
  if (contentId(envelope.record.schema_version, unsigned) !== expected.record_id) {
    fail(MISC_BOILERPLATE_WORK3_CODES.AUTHORITY, `${label} self identity drift.`);
  }
  return deepFreeze(clone(envelope));
}

function miscBoilerplateWork3ValidateInput(input, outerKeys, evidenceKey, evidenceKeys) {
  exactKeysOrFail(
    input,
    outerKeys,
    MISC_BOILERPLATE_WORK3_CODES.CONTRACT,
    'Misc Boilerplate Work3 input',
  );
  ...
  exactKeysOrFail(
    ...
    evidenceKeys,
    MISC_BOILERPLATE_WORK3_CODES.CONTRACT,
    'Misc Boilerplate Work3 evidence bundle',
  );
  for (const key of evidenceKeys) {
    if (
      ...
      fail(
        MISC_BOILERPLATE_WORK3_CODES.CONTRACT,
        `Misc Boilerplate Work3 ${key} envelope drift.`,
      );
    }
  }
  return evidence;
...
function miscBoilerplateWork3Phase4(input) {
  try {
    return prepareMiscBoilerplateFamilyProfilePackageReview(input);
  } catch (error) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.INVENTORY,
      'Misc Boilerplate Work3 Phase4 review derivation failed.',
      { cause_code: typeof error.code === 'string' ? error.code : null },
    );
  }
}

function validateMiscBoilerplateUnapprovedInventoryReviewEvidence(evidence) {
  if (
    !isObject(evidence)
    || evidence.profile_approval_state !== 'UNAPPROVED'
    || evidence.profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || evidence.complete_profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || evidence.incomplete_profile_count !== 0
    || !Array.isArray(evidence.proposed_profiles)
    || evidence.proposed_profiles.length !== MISC_BOILERPLATE_PROFILE_COUNT
    ...
    || sortedUnique(evidence.proposed_profiles.map((profile) => profile.proposed_profile_key))
      .length !== MISC_BOILERPLATE_PROFILE_COUNT
  ) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.INVENTORY,
      'Misc Boilerplate unapproved inventory review evidence census drift.',
    );
  }
  return deepFreeze({
    schema_version: 'M7_V2_MISC_BOILERPLATE_UNAPPROVED_INVENTORY_REVIEW_VALIDATOR_ACCEPTANCE/V1',
    status: 'PASS',
    profile_count: MISC_BOILERPLATE_PROFILE_COUNT,
    complete_profile_count: MISC_BOILERPLATE_PROFILE_COUNT,
    incomplete_profile_count: 0,
    retained_source_gap_count: 0,
  });
}

function prepareMiscBoilerplateWork3UnapprovedInventoryReview(input) {
  const evidence = miscBoilerplateWork3ValidateInput(
    input,
    [
      'miscBoilerplateWork3UnapprovedInventoryReviewEvidence',
      'miscBoilerplatePhase4ReviewInput',
    ],
    'miscBoilerplateWork3UnapprovedInventoryReviewEvidence',
    ['work3MiscBoilerplateUnapprovedInventoryReviewAuthority'],
  );
  const authorityEnvelope = miscBoilerplateWork3ValidatePinnedEnvelope(
    evidence.work3MiscBoilerplateUnapprovedInventoryReviewAuthority,
    MISC_BOILERPLATE_WORK3_INVENTORY_AUTHORITY_BINDING,
    'Misc Boilerplate Work3 inventory authority',
  );
  const phase4 = miscBoilerplateWork3Phase4(input.miscBoilerplatePhase4ReviewInput);
  const validator = validateMiscBoilerplateUnapprovedInventoryReviewEvidence({
  ...
  const unsigned = {
    ...
    withheld_work3_fields: clone(MISC_BOILERPLATE_WORK3_WITHHELD_FIELDS),
...
function miscBoilerplateWork3ValidatePacket(envelope) {
  miscBoilerplateWork3ValidatePinnedEnvelope(
    envelope,
    MISC_BOILERPLATE_WORK3_PACKET_BINDING,
    'Misc Boilerplate inventory packet',
  );
  const record = envelope.record;
  const items = record.profile_review_items;
  if (
    record.profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || record.complete_profile_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || record.incomplete_profile_count !== 0
    || record.retained_source_gap_count !== 0
    || !Array.isArray(items)
    || items.length !== MISC_BOILERPLATE_PROFILE_COUNT
    ...
    fail(
      MISC_BOILERPLATE_WORK3_CODES.INVENTORY,
      'Misc Boilerplate inventory packet census drift.',
    );
  }
  return record;
}

function miscBoilerplateWork3ValidateDisposition(envelope) {
  miscBoilerplateWork3ValidatePinnedEnvelope(
    envelope,
    MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING,
    'Misc Boilerplate Ben disposition',
  );
  const record = envelope.record;
  ...
  if (
    ...
    || record.packet_digest !== MISC_BOILERPLATE_WORK3_PACKET_BINDING.sha256
    || record.ben_rulings_digest !== MISC_BOILERPLATE_WORK3_RULINGS_BINDING.sha256
    || !Array.isArray(rows)
    || rows.length !== MISC_BOILERPLATE_PROFILE_COUNT
    || rows.filter((row) => row.disposition === 'APPROVE').length
      !== MISC_BOILERPLATE_PROFILE_COUNT
    || summary.approved_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || summary.hold_count !== 0
    || summary.legal_grouping_review_pending_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || summary.subtype_partition_divergence_count
      !== MISC_BOILERPLATE_SUBTYPE_DIVERGENCE_PROFILE_COUNT
    || summary.outside_calibration_example_count
      !== MISC_BOILERPLATE_OUTSIDE_CALIBRATION_PROFILE_COUNT
    || summary.subtype_grouping_pending_legal !== true
    || summary.taxonomy_expansion_acknowledged !== true
  ) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.DISPOSITION,
      'Misc Boilerplate Ben inventory disposition drift.',
    );
  }
  return record;
}

function prepareMiscBoilerplateWork3BenInventorySessionDisposition(input) {
  const evidenceKeys = [
    'work3MiscBoilerplateUnapprovedInventoryReviewAuthority',
    'work3MiscBoilerplateBenInventorySessionSuccessorAuthority',
  ...
  const evidence = miscBoilerplateWork3ValidateInput(
    input,
    [
      'miscBoilerplateWork3BenInventorySessionDispositionEvidence',
      'miscBoilerplatePhase4ReviewInput',
    ],
    'miscBoilerplateWork3BenInventorySessionDispositionEvidence',
    evidenceKeys,
  ...
  const authorityEnvelope = miscBoilerplateWork3ValidatePinnedEnvelope(
    evidence.work3MiscBoilerplateBenInventorySessionSuccessorAuthority,
    MISC_BOILERPLATE_WORK3_BEN_AUTHORITY_BINDING,
    'Misc Boilerplate Ben inventory authority',
  );
  miscBoilerplateWork3ValidatePacket(evidence.inventoryReviewPacketDraft);
  const disposition = miscBoilerplateWork3ValidateDisposition(
    evidence.benAuthoredInventoryDisposition,
  );
  const inventory = prepareMiscBoilerplateWork3UnapprovedInventoryReview({
    miscBoilerplateWork3UnapprovedInventoryReviewEvidence: {
      work3MiscBoilerplateUnapprovedInventoryReviewAuthority:
        evidence.work3MiscBoilerplateUnapprovedInventoryReviewAuthority,
    },
    miscBoilerplatePhase4ReviewInput: input.miscBoilerplatePhase4ReviewInput,
  });
  const contract = authorityEnvelope.record.schema_review_candidate_contract;
  const unsigned = {
    schema_version: contract.schema_version,
    ...
    disposition_binding: {
      path: MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING.path,
      inventory_disposition_id: disposition.inventory_disposition_id,
      packet_digest: disposition.packet_digest,
      profile_disposition_count: MISC_BOILERPLATE_PROFILE_COUNT,
      session_summary: clone(disposition.session_summary),
    },
    packet_binding: clone(MISC_BOILERPLATE_WORK3_PACKET_BINDING),
    ben_rulings_binding: clone(MISC_BOILERPLATE_WORK3_RULINGS_BINDING),
    session_receipt_reference: {
      schema_version: MISC_BOILERPLATE_WORK3_SESSION_BINDING.schema_version,
...
function miscBoilerplateWork3ValidateSessionReceipt(envelope) {
  miscBoilerplateWork3ValidatePinnedEnvelope(
    envelope,
    MISC_BOILERPLATE_WORK3_SESSION_BINDING,
    'Misc Boilerplate Ben session receipt',
  );
  const record = envelope.record;
  if (
    record.completion_state !== 'COMPLETE'
    || record.disposition_binding.inventory_disposition_id
      !== MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING.record_id
    || record.zero_effect_boundary.work3_identity_count !== 0
  ) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.RECEIPT,
      'Misc Boilerplate Ben session receipt drift.',
    );
  }
  return record;
}

function prepareMiscBoilerplateWork3FamilyPackageSeal(input) {
  const evidenceKeys = [
    'work3MiscBoilerplateUnapprovedInventoryReviewAuthority',
    'work3MiscBoilerplateBenInventorySessionSuccessorAuthority',
    'work3MiscBoilerplateFamilyPackageSealSuccessorAuthority',
    'inventoryReviewPacketDraft',
    'benAuthoredInventoryDisposition',
  ...
  const evidence = miscBoilerplateWork3ValidateInput(
    input,
    [
      'miscBoilerplateWork3FamilyPackageSealEvidence',
      'miscBoilerplatePhase4ReviewInput',
    ],
    'miscBoilerplateWork3FamilyPackageSealEvidence',
    evidenceKeys,
  ...
  const authorityEnvelope = miscBoilerplateWork3ValidatePinnedEnvelope(
    evidence.work3MiscBoilerplateFamilyPackageSealSuccessorAuthority,
    MISC_BOILERPLATE_WORK3_SEAL_AUTHORITY_BINDING,
    'Misc Boilerplate family package seal authority',
  );
  const dispositionCandidate = prepareMiscBoilerplateWork3BenInventorySessionDisposition({
    miscBoilerplateWork3BenInventorySessionDispositionEvidence: {
      work3MiscBoilerplateUnapprovedInventoryReviewAuthority:
        evidence.work3MiscBoilerplateUnapprovedInventoryReviewAuthority,
      work3MiscBoilerplateBenInventorySessionSuccessorAuthority:
  ...
  miscBoilerplateWork3ValidateSessionReceipt(evidence.benInventorySessionReceipt);
  if (
    ...
    fail(
      MISC_BOILERPLATE_WORK3_CODES.RECEIPT,
      'Misc Boilerplate session receipt identity drift.',
    );
  }
  const contract = authorityEnvelope.record.schema_review_candidate_contract;
  const unsigned = {
    ...
    inventory_session_disposition_reference: {
      inventory_disposition_id: MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING.record_id,
      candidate_state: dispositionCandidate.candidate_state,
    },
    ben_rulings_binding: clone(MISC_BOILERPLATE_WORK3_RULINGS_BINDING),
    disposition_binding: clone(MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING),
    session_receipt_binding: clone(MISC_BOILERPLATE_WORK3_SESSION_BINDING),
    legal_grouping_disposition_binding: {
      ...clone(MISC_BOILERPLATE_WORK3_RULINGS_BINDING),
      disposition_status: 'PENDING_LEGAL_REVIEW',
      legal_grouping_review_pending_count: MISC_BOILERPLATE_PROFILE_COUNT,
      outside_calibration_example_count: MISC_BOILERPLATE_OUTSIDE_CALIBRATION_PROFILE_COUNT,
      populated_subtype_bucket_count: MISC_BOILERPLATE_POPULATED_SUBTYPE_BUCKET_COUNT,
      registered_subtype_bucket_count: MISC_BOILERPLATE_REGISTERED_SUBTYPE_BUCKET_COUNT,
      subtype_partition_divergence_count: MISC_BOILERPLATE_SUBTYPE_DIVERGENCE_PROFILE_COUNT,
    },
    review_accounting: clone(contract.review_accounting_exact_values),
    withheld_work3_fields: clone(MISC_BOILERPLATE_WORK3_WITHHELD_FIELDS),
    next_governance_stop: {
...
function miscBoilerplateWork3ValidateSealReceipt(envelope) {
  miscBoilerplateWork3ValidatePinnedEnvelope(
    envelope,
    MISC_BOILERPLATE_WORK3_SEAL_RECEIPT_BINDING,
    'Misc Boilerplate family seal receipt',
  );
  const record = envelope.record;
  if (
    record.completion_state !== 'COMPLETE'
    ...
    fail(
      MISC_BOILERPLATE_WORK3_CODES.RECEIPT,
      'Misc Boilerplate family seal receipt drift.',
    );
  }
  return record;
}

function prepareMiscBoilerplateWork3FamilyPackageRegistration(input) {
  const evidenceKeys = [
    'work3MiscBoilerplateUnapprovedInventoryReviewAuthority',
    'work3MiscBoilerplateBenInventorySessionSuccessorAuthority',
    'work3MiscBoilerplateFamilyPackageSealSuccessorAuthority',
    'work3MiscBoilerplateRegistrationSuccessorAuthority',
    'inventoryReviewPacketDraft',
    'benAuthoredInventoryDisposition',
    'benInventorySessionReceipt',
  ...
  const evidence = miscBoilerplateWork3ValidateInput(
    input,
    [
      'miscBoilerplateWork3FamilyPackageRegistrationEvidence',
      'miscBoilerplatePhase4ReviewInput',
    ],
    'miscBoilerplateWork3FamilyPackageRegistrationEvidence',
    evidenceKeys,
  ...
  const authorityEnvelope = miscBoilerplateWork3ValidatePinnedEnvelope(
    evidence.work3MiscBoilerplateRegistrationSuccessorAuthority,
    MISC_BOILERPLATE_WORK3_REGISTRATION_AUTHORITY_BINDING,
    'Misc Boilerplate registration authority',
  );
  const sealCandidate = prepareMiscBoilerplateWork3FamilyPackageSeal({
    miscBoilerplateWork3FamilyPackageSealEvidence: {
      work3MiscBoilerplateUnapprovedInventoryReviewAuthority:
        evidence.work3MiscBoilerplateUnapprovedInventoryReviewAuthority,
      work3MiscBoilerplateBenInventorySessionSuccessorAuthority:
  ...
  const sealReceipt = miscBoilerplateWork3ValidateSealReceipt(
    evidence.familyPackageSealReceipt,
  );
  if (sealReceipt.family_package_seal_id !== sealCandidate.family_package_seal_id) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.RECEIPT,
      'Misc Boilerplate family seal candidate and receipt identity drift.',
    );
  }
  const phase4 = miscBoilerplateWork3Phase4(input.miscBoilerplatePhase4ReviewInput);
  ...
  const registeredProfiles = phase4.proposed_profiles.map((profile) => {
    ...
    if (!disposition) {
      fail(
        MISC_BOILERPLATE_WORK3_CODES.OUTPUT,
        'Misc Boilerplate registration disposition missing.',
      );
    }
    const identityInput = {
      family_key: 'MISC_BOILERPLATE',
    ...
    return {
      profile_id: contentId(
        'M7_V2_MISC_BOILERPLATE_WORK3_REGISTERED_PROFILE_IDENTITY/V1',
        identityInput,
      ),
      proposed_profile_key: profile.proposed_profile_key,
      phase3_profile_key: null,
  ...
  const packageUnsigned = {
    ...
    profile_id_count: MISC_BOILERPLATE_PROFILE_COUNT,
    profile_ids: registeredProfiles.map((profile) => profile.profile_id),
    inventory_disposition_id: MISC_BOILERPLATE_WORK3_DISPOSITION_BINDING.record_id,
    family_package_seal_receipt_id: MISC_BOILERPLATE_WORK3_SEAL_RECEIPT_BINDING.record_id,
    legal_grouping_disposition_state: 'PENDING_LEGAL_REVIEW',
  };
  const packageIdentity = {
    family_profile_package_id: contentId(
      'M7_V2_MISC_BOILERPLATE_WORK3_FAMILY_PROFILE_PACKAGE_IDENTITY/V1',
      packageUnsigned,
    ),
  ...
  const unsigned = {
    ...
    candidate_state: contract.candidate_state,
    authority_binding: clone(authorityEnvelope.binding),
    family_package_seal_receipt_binding: clone(MISC_BOILERPLATE_WORK3_SEAL_RECEIPT_BINDING),
    ...
    next_governance_stop: {
      state: 'STOP_AFTER_MISC_BOILERPLATE_FAMILY_PACKAGE_REGISTRATION_BEFORE_ACTIVATION',
  ...
  if (
    result.registered_profile_identities.length !== MISC_BOILERPLATE_PROFILE_COUNT
    || result.review_accounting.profile_identity_count !== MISC_BOILERPLATE_PROFILE_COUNT
    || result.review_accounting.work3_identity_count !== MISC_BOILERPLATE_PROFILE_COUNT + 1
    || result.zero_effect_boundary.activation_count !== 0
    || miscBoilerplateContainsForbiddenKey(result, new Set(['activation_id']))
  ) {
    fail(
      MISC_BOILERPLATE_WORK3_CODES.OUTPUT,
      'Misc Boilerplate family registration boundary drift.',
    );
  }
  return result;
}
...
module.exports = {
  MISC_BOILERPLATE_OUTSIDE_CALIBRATION_PROFILE_COUNT,
  MISC_BOILERPLATE_PHASE2_AUTHORITY_BYTES,
  MISC_BOILERPLATE_PHASE2_AUTHORITY_ID,
  MISC_BOILERPLATE_PHASE2_AUTHORITY_PATH,
  MISC_BOILERPLATE_PHASE2_AUTHORITY_SCHEMA,
  MISC_BOILERPLATE_PHASE2_AUTHORITY_SHA256,
  MISC_BOILERPLATE_PHASE2_PROPOSAL_CODES,
  MISC_BOILERPLATE_PHASE2_PROPOSAL_KEYS,
  MISC_BOILERPLATE_PHASE4_AUTHORITY_BYTES,
  MISC_BOILERPLATE_PHASE4_AUTHORITY_ID,
  MISC_BOILERPLATE_PHASE4_AUTHORITY_PATH,
  MISC_BOILERPLATE_PHASE4_AUTHORITY_SCHEMA,
  MISC_BOILERPLATE_PHASE4_AUTHORITY_SHA256,
  MISC_BOILERPLATE_PHASE4_CANDIDATE_SCHEMA,
  MISC_BOILERPLATE_PHASE4_CANDIDATE_STATE,
  MISC_BOILERPLATE_PHASE4_REVIEW_CODES,
  MISC_BOILERPLATE_PHASE4_REVIEW_INPUT_KEYS,
  MISC_BOILERPLATE_PHASE4_REVIEW_OUTPUT_KEYS,
  MISC_BOILERPLATE_PHASE4_SCHEDULE_SHA256,
  MISC_BOILERPLATE_POPULATED_SUBTYPE_BUCKET_COUNT,
  MISC_BOILERPLATE_PROFILE_COUNT,
  MISC_BOILERPLATE_REGISTERED_SUBTYPE_BUCKET_COUNT,
  MISC_BOILERPLATE_REVIEW_FLAGS,
  MISC_BOILERPLATE_SUBTYPE_DIVERGENCE_PROFILE_COUNT,
  miscBoilerplateProposalPartition,
  prepareMiscBoilerplateFamilyProfilePackageReview,
  prepareMiscBoilerplatePhase2FamilyProposal,
  prepareMiscBoilerplateWork3BenInventorySessionDisposition,
  prepareMiscBoilerplateWork3FamilyPackageRegistration,
  prepareMiscBoilerplateWork3FamilyPackageSeal,
  prepareMiscBoilerplateWork3UnapprovedInventoryReview,
  validateMiscBoilerplateUnapprovedInventoryReviewEvidence,
};
