...
const PURE_PROPOSAL_SOURCES = Object.freeze([
  ...
  // Execution-manifest validator: reads files and delegates its fixed
  // read-only Git observations to the Work3 validator's exported seam; it
  // holds no child-process authority of its own.
  ...
  // Programme roadmap page (local-only admin tool, not a Canonical V2
  // production path). lib/programme/roadmap.js is a plain data module: no
  // function bodies, no I/O, only literal objects/arrays with document
  // citations. lib/programme/derive.js is pure graph functions over that
  // data (topological order, cycle detection, next-executable set,
  // parallel sets, blocked-by, date status) plus localOnlyGate(env), which
  // reads process.env but performs no capability -- it returns a plain
  // object describing whether the page should 404, never calls fs/network/
  // database/signing/child-process/deployment itself. pages/admin/
  // programme.js's own getServerSideProps calls only these two modules and
  // reads process.env; its client-side code uses localStorage and a
  // Blob/URL download, neither of which is one of the seven scanned
  // capabilities (browser-only, discarded per profile, never touching this
  // repository or a server -- same reasoning as clauseSearchMode.js above).
  // It writes nothing anywhere; proposed edits stay in browser state and an
  // exported file the user downloads. Cannot be PRODUCTION_PATH_PURE_
  // ANALYSIS because that class forbids any module dependency and this
  // page imports react and next/head.
  'lib/programme/derive.js',
  'lib/programme/roadmap.js',
  'pages/admin/programme.js',
]);
...
const READ_ONLY_GIT_INSPECTORS = Object.freeze([
  ...
  // Work 7 independent verifier (external workstream, PR #485): read-only
  // cat-file, ls-tree and rev-parse through one allowlisted seam; no writes.
...
const READ_ONLY_GIT_ARTIFACT_WRITERS = Object.freeze([
  ...
  // Same shape for the named evidence gates: Git tree and blob reads only, one
  // local CI cache marker write.
