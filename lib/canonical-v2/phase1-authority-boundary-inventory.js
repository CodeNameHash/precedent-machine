...
const PURE_PROPOSAL_SOURCES = Object.freeze([
  ...
  // Programme roadmap page (admin tool, not a Canonical V2 production
  // path). lib/programme/roadmap.js is a plain data module: no
  ...
  // parallel sets, blocked-by, date status) and nothing else -- it calls no
  // fs/network/database/signing/child-process/deployment capability. It
  // held a localOnlyGate(env) until 2026-09-04, when Ben ruled the page may
  // serve on Vercel previews and production (DECISIONS.md #27). pages/admin/
  // programme.js's own getServerSideProps calls only these two modules; its
  // client-side code uses localStorage and a
