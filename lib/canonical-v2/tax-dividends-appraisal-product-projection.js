...
// Confirmed live on TAX_MATTERS: every resolved TAX_MATTERS_CLAIM_KEY entry
// in the committed modiv-tax-matters-20260807-replay run binds to a
// partyless provision -- 'STRUCTURAL_PROVISION_INSTANCE/V1', the kind Step
// 4A1 taught the SQL writer -- because candidate-resolution.js's
// GENERIC_CLAIM_KEY_RESOLUTION_TABLE gives TAX_MATTERS_CLAIM_KEY,
// DIVIDENDS_CLAIM_KEY and APPRAISAL_CLAIM_KEY `party_field: null`: none of
// the three families this module owns is ever party-bound. The strict-equal
// check rejected every one of them; DIVIDENDS and APPRAISAL are latent only
// because the committed modiv-dividends/-appraisal-20260807-replay runs
// resolved zero claims, not because their provisions differ.
// validate-write-set.js's assertStructuralRowShape/expectedObjectId admit
// both real kinds, so the fix accepts the union rather than swapping one
// literal for another.
...
const TAX_CLAIMS = Object.freeze({
  ...
  TAX_OPINION_COOPERATION_COVENANT: Object.freeze({
    concept: 'TAXM-OPINION', label: 'Tax opinion cooperation covenant', value_kind: 'PRESENCE',
    values: Object.freeze([true]), allowed_attributes: Object.freeze(['tax_opinion_ref', 'representation_letter_ref']),
  }),
...
const DIVIDEND_CLAIMS = Object.freeze({
  ...
  PRE_CLOSING_SPECIAL_DIVIDEND: Object.freeze({
    concept: 'DIVD-SPECIAL', label: 'Pre-closing special dividend', value_kind: 'PRESENCE',
    values: Object.freeze([true]), allowed_attributes: Object.freeze(['dividend_term_ref']),
  }),
  PRE_CLOSING_SPECIAL_DIVIDEND_PER_SHARE_AMOUNT: Object.freeze({
    concept: 'DIVD-SPECIAL', label: 'Pre-closing special dividend per-share amount', value_kind: 'DECIMAL',
    values: Object.freeze([]), allowed_attributes: Object.freeze(['dividend_term_ref', 'currency']),
  }),
});
...
// Found alongside defect 4, same investigation, same committed
// modiv-tax-matters-20260807-replay run: the tax/dividends/appraisal
// producer's structured response is one fixed schema shared across every
// assertion_kind in the family (candidate-resolution.js's
// handleTaxMattersCandidate/handleDividendsCandidate/handleAppraisalCandidate,
// via finalizeConsiderationClaim's `attributesExtra` merge, which by design
// "never removes or overwrites an existing key the producer supplied").
// A TAX_OPINION_COOPERATION_COVENANT claim's raw attributes therefore
// arrive with every OTHER tax/dividend/appraisal ref field present and
// null (bearer_ref, dividend_term_ref, statute_ref, ...), plus
// assertion_kind, section_reference and answer_provenance -- all system/
// routing metadata this module already reads from entry.concept_key,
// entry.section_reference and the resolved claim_definition_key, never
// from these attribute keys. Before this fix, rejectUngovernedAttributes
// treated every one of those as an "ungoverned attribute" and the whole
// family failed closed on the very first real claim (TAX_MATTERS,
// confirmed) -- a schema-version fix alone was not enough to make
// TAX_MATTERS actually project. FINANCING_COVENANTS and
// GUARANTY_FINANCING_PARTY carry the identical answer_provenance key
// (finalizeFinancingGuarantyClaim's attributesReplace, financing-guaranty-
// product-projection.js) and would have hit the same failure the first
// time either resolves a claim, so that module's rejectUngovernedAttributes
// got the identical fix.
//
// Stripped once, here, rather than threaded through every consumer:
// validateAttributes' `Object.keys(attributes).length === 0` checks (the
// DIVIDEND_COORDINATION_COVENANT/FIRPTA_CERTIFICATE_DELIVERY/TRANSFER_TAX_
// COOPERATION zero-attribute branches) and taxDimensions/appraisalDimensions
// all read `attributes` downstream of this call and need the same
// "system keys and unset placeholders are not attributes" view
// rejectUngovernedAttributes enforces -- sanitizing once keeps that
// definition in one place instead of three.
...
function validateAttributes(family, key, attributes) {
  if (family === 'TAX_MATTERS') {
    ...
    if (key === 'TAX_OPINION_COOPERATION_COVENANT') {
      return optionalNonEmpty(attributes.tax_opinion_ref)
        && optionalNonEmpty(attributes.representation_letter_ref);
    }
    if (key === 'TRANSFER_TAX_ALLOCATION') return nonEmpty(attributes.bearer_ref);
    return Object.keys(attributes).length === 0;
  }
  if (family === 'DIVIDENDS') {
    if (key === 'DIVIDEND_COORDINATION_COVENANT') return Object.keys(attributes).length === 0;
    return nonEmpty(attributes.dividend_term_ref)
      && (key !== 'PRE_CLOSING_SPECIAL_DIVIDEND_PER_SHARE_AMOUNT' || attributes.currency === 'USD');
  }
...
function taxDimensions(key, attributes) {
  ...
  if (key === 'TAX_OPINION_COOPERATION_COVENANT') {
    return {
      claim_definition_key: key,
      ...(attributes.tax_opinion_ref ? { tax_opinion_ref: attributes.tax_opinion_ref } : {}),
      ...(attributes.representation_letter_ref ? { representation_letter_ref: attributes.representation_letter_ref } : {}),
    };
  }
...
function marketValue(family, key, definition, canonicalValue, dimensions) {
  if (definition.value_kind === 'ENUM' || definition.value_kind === 'DECIMAL') {
    return {
      metric_key: key === 'INTENDED_TAX_TREATMENT_KIND'
        ? 'INTENDED_TAX_TREATMENT_KIND'
        : key === 'TRANSFER_TAX_ALLOCATION' ? 'TRANSFER_TAX_ALLOCATION' : 'PRE_CLOSING_SPECIAL_DIVIDEND_PER_SHARE_AMOUNT',
      value_dimension: definition.value_kind === 'DECIMAL' ? 'DECIMAL' : 'ENUM', canonical_unit: definition.value_kind === 'DECIMAL' ? 'USD_PER_SHARE' : 'CODE', canonical_value: canonicalValue,
...
function projectEntry(entry) {
  ...
  if (!claim || claim.state !== 'PRESENT' || claim.claim_definition_key !== key
    || !nonEmpty(claim.claim_revision_id) || (definition.value_kind === 'DECIMAL'
      ? !/^\d+(?:\.\d+)?$/.test(claim.canonical_value)
...
function projectTaxDividendsAppraisalClaims({ resolved_entries: resolvedEntries, publication_filter: publicationFilter, release_receipt_id: releaseReceiptId, publication_evaluation_time: publicationEvaluationTime } = {}) {
