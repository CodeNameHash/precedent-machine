...
function validateMetseraAdmission(value, sourceBytesByIdentity) {
  ...
  try {
    validateMetseraExclusivityProductAdmission(value, sourceBytesByIdentity);
  } catch (error) {
    fail(
      `the full materialisation-backed Product admission is invalid: ${
...
function buildMetseraProductRow(
  admission,
  authorityContext,
  authorityInput,
  sourceBytesByIdentity,
) {
  validateMetseraAdmission(admission, sourceBytesByIdentity);
  const authority = authorityBoundContext(
    admission,
    authorityContext,
    authorityInput,
