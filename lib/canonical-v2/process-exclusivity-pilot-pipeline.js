...
function verifiedSourceBytes(
  ...
  if (!Buffer.isBuffer(bytes)) {
    fail(code, 'Source bytes are missing for the admitted source document identity.');
  }
  if (sha256Hex(bytes) !== expectedDigest) {
    fail(code, 'Admitted source bytes do not match source_text_digest.');
  }
  return bytes;
}
...
function validateEvidenceAgainstSources(
  evidence,
  sources,
  sourceBytesByIdentity,
  label,
  ...
  const sourceBytes = verifiedSourceBytes(
    sourceBytesByIdentity,
    source.source_identity.source_document_identity,
  ...
  if (Object.hasOwn(evidence, 'scope_evidence')) {
    evidence.scope_evidence.forEach((value, index) => (
      validateEvidenceAgainstSources(
        ...
        sources,
        sourceBytesByIdentity,
        `${label} scope evidence ${index}`,
      )
    ));
  }
}

function validateGraphEvidence(candidateGraph, sources, sourceBytesByIdentity) {
...
function compileProcessExclusivityPilotPipeline(
  input,
  trustedContext,
  sourceBytesByIdentity,
) {
