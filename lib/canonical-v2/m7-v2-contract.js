...
const DNO_ITEM42_SUCCESSOR_PACKAGE_BINDING = Object.freeze({
  byte_length: 431970,
  git_blob_oid: 'e8bb49c1ec87953903a21c46ea971d96e7ac1a1c',
  path: DNO_ITEM42_SUCCESSOR_PACKAGE_PATH,
...
const FAMILY_GROUPING_SUCCESSOR_CONTRACTS = Object.freeze([
  {
    family_key: 'DIVIDENDS',
    profile_count: 1,
    registration_authority_id:
      '07dc997eb5789f7c7e19e3c050d35ce649deca5f54f5f465bda3291f304d7ee7',
    successor_package_binding: {
      byte_length: 18198,
      git_blob_oid: '0ef2e9b4b2ee2910b542610a786e10df72755185',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-dividends-grouping-successor-2026-09-01B.json',
      record_id: '181a09058a5d247226269a9fb73786595afcd7a3dddbc8a7c01929db7b1cc7a4',
      record_id_field: 'family_profile_package_id',
      schema_version: FAMILY_PROFILE_PACKAGE_SCHEMA,
      sha256: '2b915ab01f26d7a908f084640fa98c51ebec801ea30b2f6b675cc2bb41cbc8d2',
    },
  },
  {
    ...
    profile_count: 4,
    registration_authority_id:
      'c4b20e87dd0643d8690d56ad6d4e78da9aae4e42c7784716f609558861038e0c',
    successor_package_binding: {
      byte_length: 55362,
      git_blob_oid: '03aa4e895b1fc089e7cf53e6db2362231c5af80c',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-mae-definition-grouping-successor-2026-09-01B.json',
  ...
  },
  {
    ...
    profile_count: 5,
    registration_authority_id:
      'ad1efb8ad4b499e5f08f05fae77cc86b70b9f27325dd8d15beab87a799c94d71',
    successor_package_binding: {
      byte_length: 70377,
      git_blob_oid: 'f8c7bc8ab13c94c4537a628ccc8e0d6c219f5250',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-guaranty-financing-party-grouping-successor-2026-09-01B.json',
  ...
  },
  {
    ...
    profile_count: 5,
    registration_authority_id:
      '344ba0d71455ac95b24cb20e66fbc7859a1f26810a0432a914451d2066189ac0',
    successor_package_binding: {
      byte_length: 71128,
      git_blob_oid: 'a6b98629e46560ec0dd9823cdcd5bda02662f448',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-appraisal-dissenters-rights-grouping-successor-2026-09-01B.json',
  ...
  },
  {
    ...
    profile_count: 5,
    registration_authority_id:
      'acb932208647c30746addd949cdbe097411a480e5ab7bd58ad156ad68fae6e44',
    successor_package_binding: {
      byte_length: 68960,
      git_blob_oid: 'e8a09d2335f6b4fb50d27d996afff21b85e61ed8',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-financing-covenants-grouping-successor-2026-09-01B.json',
  ...
  },
  {
    ...
    profile_count: 7,
    registration_authority_id:
      'f5f4fd5704d65d08a0dde56f643d9858af29ef75f0328a368a287cc16b019b43',
    successor_package_binding: {
      byte_length: 92618,
      git_blob_oid: 'ac0e3424fdac2a94362562cdaafd4538b4d34a00',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-consideration-grouping-successor-2026-09-01B.json',
  ...
  },
  {
    ...
    profile_count: 113,
    registration_authority_id:
      'ef248f31b6edfaf290718a47b275eda7251da751fefa7f339cdf540764086941',
    successor_package_binding: {
      byte_length: 1435829,
      git_blob_oid: '99d8dc0bbe6523bd7bd6cabaf4ef147350c1ea9b',
      path: MIGRATION_ROOT + '/control/m7-v2-repair-family-work3-profile-package-interim-operating-grouping-successor-2026-09-01B.json',
...
// Candidate replacement authority (Ben, 2026-09-03). A registration taken out
// under that authority may bind it, and must then also declare the static
// import closure of its bound code. Both members are OPTIONAL on the schema:
// the registrations that predate the authority carry neither, and they must
// keep validating unchanged. The binding is pinned member for member, so a
// registration cannot name a different file, a different record or different
// bytes and still claim the authority.
const CANDIDATE_REPLACEMENT_AUTHORITY_BINDING = Object.freeze({
  path: 'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-candidate-replacement-authority.json',
...
// An import-closure member binds bytes at a path and nothing else: it names no
// record, because a module is not a record.
const CANDIDATE_IMPORT_CLOSURE_KEYS = Object.freeze([
  'path',
  'byte_length',
  'sha256',
  'git_blob_oid',
]);
// `supersedes_candidate_registration_id` joins them for an INTERIM
// registration: the authority's `interim_registration_policy` mints one before
// every evidence run, each superseding the last, and the chain is carried on
// the registrations themselves rather than in a mutable index. It is optional
// like the other two, and it may not appear without the authority binding that
// licenses it.
const CANDIDATE_OPTIONAL_KEYS = Object.freeze([
  'candidate_replacement_authority_binding',
  'import_closure_bindings',
...
const CANDIDATE_WORK3_SPLIT_TEST_PATH =
  'tests/stage-2y-structure-m7-v2-repair-work3-mae.test.js';
const CANDIDATE_WORK4_DISPATCH_TEST_PATH =
...
function buildFamilyPackageRegistry(profileSet, resolveBinding, code, contract = null) {
  const familyKeys = contract?.familyKeys ?? FAMILY_KEYS;
  const expectedBindings = contract?.packageBindings ?? null;
  const bindings = array(
    profileSet?.family_profile_package_bindings, code, 'family profile package bindings',
  );
  if (bindings.length !== familyKeys.length) {
    fail(code, 'approved profile set does not bind the exact physical family package set');
  }
  ...
  for (let index = 0; index < familyKeys.length; index += 1) {
    ...
    const hasExpectedBinding = expectedBindings !== null;
    if ((hasExpectedBinding && !same(binding, expectedBindings[index]))
        || (!hasExpectedBinding
          && binding.path !== FAMILY_PROFILE_PACKAGE_PATH_BY_FAMILY.get(familyKey))
  ...
  const benApprovalIds = familyKeys.map((familyKey) => {
  ...
  return {
    ...
    byFamily,
    familyKeys,
    parkedFamilyKeys: contract?.parkedFamilyKeys ?? [],
    memberContracts: PACKAGE_MEMBER_FIELDS,
  };
}
...
function candidateFamilyPackageContract(registration, code) {
  const work3Index = registration.predecessor_receipt_bindings.findIndex(
    (entry) => entry.work === 'WORK3',
  );
  const work3Binding = work3Index === -1
    ? null : registration.predecessor_receipt_bindings[work3Index].binding;
  if (work3Binding?.schema_version !== WORK3_RECEIPT_V2_SCHEMA) {
    return {
      familyKeys: FAMILY_KEYS,
      packagePaths: FAMILY_PROFILE_PACKAGE_PATHS,
      verificationChecks: CANDIDATE_VERIFICATION_CHECKS,
      work3V2: false,
    };
  }
  if (work3Index !== 2
      || work3Binding.path !== WORK3_RECEIPT_V2_PATH
      || work3Binding.record_id_field !== 'work3_receipt_id') {
    fail(code, 'candidate Work3 V2 predecessor is outside the exact closed lineage');
  }
  return {
    familyKeys: WORK3_PHYSICAL_FAMILY_KEYS,
    packagePaths: null,
    verificationChecks: CANDIDATE_VERIFICATION_CHECKS.map((checkId) => (
...
function validateWork3V2FamilyPackageContract(
  receipt, profileSetBinding, profileSet, candidateContract, code,
) {
  const work3Receipt = object(receipt, code, 'Work3 V2 predecessor receipt');
  if (!candidateContract.work3V2
      || work3Receipt.schema_version !== WORK3_RECEIPT_V2_SCHEMA
      || work3Receipt.work !== 'WORK3'
      || work3Receipt.stage !== 'M7_V2_REPAIR_WORK3'
      || work3Receipt.state !== 'PASS_WORK3_BUILD_ONLY_NULL_CANDIDATE'
      || work3Receipt.status !== 'PASS'
      || work3Receipt.candidate_registration_id !== null
      || work3Receipt.candidate_transition !== null) {
    fail(code, 'physical family registry lacks the exact passed Work3 V2 predecessor');
  }
  const evidence = object(
    work3Receipt.family_profile_evidence, code, 'Work3 V2 family profile evidence',
  );
  exactKeys(evidence, [
  ...
  ], code, 'Work3 V2 family profile evidence');
  const packageBindings = array(
    evidence.family_profile_package_bindings, code, 'Work3 V2 family package bindings',
  );
  if (!same(evidence.approved_family_profile_set_binding, profileSetBinding)
      || !same(packageBindings, profileSet.family_profile_package_bindings)
      || !same(evidence.governed_family_keys, FAMILY_KEYS)
      || !same(evidence.sealed_package_family_keys, WORK3_PHYSICAL_FAMILY_KEYS)
      || !same(candidateContract.familyKeys, WORK3_PHYSICAL_FAMILY_KEYS)) {
    fail(code, 'Work3 V2 family evidence does not bind the exact 24-package profile set');
  }
  const parked = object(
    evidence.parked_family_evidence, code, 'Work3 V2 parked family evidence',
  );
  exactKeys(parked, [
    'family_key',
    'on_disk_package_binding',
    'on_disk_package_present',
    'parked_state',
    'planned_package_path',
    'product_activation_permitted',
    'serving_permitted',
    'stage_9f_authority_required',
    'synthetic_validation_package',
  ], code, 'Work3 V2 parked family evidence');
  const synthetic = object(
    parked.synthetic_validation_package, code, 'Work3 V2 synthetic parked package',
  );
  exactKeys(synthetic, [
    'package_id', 'profile_count', 'state',
  ], code, 'Work3 V2 synthetic parked package');
  assertHex(synthetic.package_id, 64, code, 'Work3 V2 synthetic parked package ID');
  if (parked.family_key !== 'CAPITALISATION'
      || parked.on_disk_package_binding !== null
      || parked.on_disk_package_present !== false
      || parked.parked_state !== 'PARKED'
      || parked.planned_package_path
        !== FAMILY_PROFILE_PACKAGE_PATH_BY_FAMILY.get('CAPITALISATION')
      || parked.product_activation_permitted !== false
      || parked.serving_permitted !== false
      || parked.stage_9f_authority_required !== true
      || synthetic.profile_count !== 1
      || synthetic.state !== 'IN_MEMORY_VALIDATION_ONLY_NOT_A_SEALED_PACKAGE'
      || packageBindings.some((binding) => binding.path === parked.planned_package_path)) {
    fail(code, 'CAPITALISATION is not parked at the exact Work3 physical boundary');
  }
  return {
    familyKeys: WORK3_PHYSICAL_FAMILY_KEYS,
    packageBindings,
    parkedFamilyKeys: ['CAPITALISATION'],
  };
}

// The optional replacement-authority members. Presence of the authority
// binding is what makes an import closure mandatory: a registration taken out
// under the replacement authority must say what its bound code can load, and
// the closure must cover every code path the registration binds. A closure
// declared without the authority binding is still checked for shape, order,
// coverage and — via `resolveBinding`, hash-verified against the bytes at its
// own path exactly as every other bound record binding is in `validateBinding`
// above — its own byte_length/sha256/git_blob_oid: an unsorted, partial or
// drifted closure is a defect wherever it appears.
function candidateCodePaths(codeBindings, code) {
  const paths = [];
  for (const key of CANDIDATE_CODE_KEYS) {
    const value = codeBindings[key];
    if (Array.isArray(value)) {
      for (const binding of value) {
        paths.push(string(object(binding, code, 'candidate code binding').path,
          code, 'candidate code binding path'));
      }
      continue;
    }
    paths.push(string(object(value, code, 'candidate code binding').path,
      code, 'candidate code binding path'));
  }
  return paths;
}

function validateCandidateReplacementMembers(registration, resolveBinding, code) {
  const declaresClosure = Object.hasOwn(registration, 'import_closure_bindings');
  const declaresAuthority = Object.hasOwn(
  ...
  if (declaresAuthority) {
    ...
    exactKeys(binding, BINDING_KEYS, code, 'candidate replacement authority binding');
    if (!same(binding, CANDIDATE_REPLACEMENT_AUTHORITY_BINDING)) {
      fail(code, 'candidate replacement authority binding is not the pinned authority');
    }
    if (!declaresClosure) {
      fail(code, 'candidate replacement authority binding requires an import closure');
    }
  }
  if (Object.hasOwn(registration, 'supersedes_candidate_registration_id')) {
    if (!declaresAuthority) {
      fail(code, 'candidate supersession requires the replacement authority binding');
    }
    const superseded = registration.supersedes_candidate_registration_id;
    assertHex(superseded, 64, code, 'candidate supersession target');
    if (superseded === registration.candidate_registration_id) {
      fail(code, 'candidate registration supersedes itself');
    }
  }
  if (!declaresClosure) return;
  const closure = array(registration.import_closure_bindings, code, 'candidate import closure');
  if (closure.length === 0) fail(code, 'candidate import closure is empty');
  // Each member's own declared byte_length/sha256/git_blob_oid is re-derived
  // from the bytes `resolveBinding` returns for it, exactly as every other
  // bound record binding is checked in `validateBinding` above — a closure
  // member is not exempt from being what it claims to be.
  const paths = closure.map((member) => {
    exactKeys(member, CANDIDATE_IMPORT_CLOSURE_KEYS, code, 'candidate import closure member');
    string(member.path, code, 'candidate import closure member path');
    if (!Number.isInteger(member.byte_length) || member.byte_length < 0) {
      fail(code, `candidate import closure member ${member.path}: byte_length is invalid`);
    }
    assertHex(member.sha256, 64, code, 'candidate import closure member digest');
    assertHex(member.git_blob_oid, 40, code, 'candidate import closure member blob');
    let selected;
    try {
      selected = resolveBinding(member);
    } catch {
      fail(code, `candidate import closure member ${member.path} could not be resolved`);
    }
    const selectedBytes = bytes(
      selected, code, `candidate import closure member ${member.path}`,
    );
    if (selectedBytes.length !== member.byte_length
        || sha256Hex(selectedBytes) !== member.sha256
        || gitBlobOid(selectedBytes) !== member.git_blob_oid) {
      fail(code, `candidate import closure member ${member.path}: bytes do not match their binding`);
    }
    return member.path;
  });
  unique(paths, code, 'candidate import closure');
  if (!same(paths, [...paths].sort(codeUnitCompare))) {
    fail(code, 'candidate import closure is not sorted by path');
  }
  const declared = new Set(paths);
  for (const path of candidateCodePaths(
    object(registration.code_bindings, code, 'candidate registration code bindings'), code,
  )) {
    if (!declared.has(path)) {
      fail(code, `candidate import closure omits the bound code path ${path}`);
    }
  }
}

function validateCandidateVerification(
  verification, candidate, registrationBinding, candidateContract,
) {
  ...
  if (!same(checks, candidateContract.verificationChecks.map(
...
function validateCandidateRegistrationBytes(selectedBytes, governance, resolveBinding) {
  const code = 'M7_V2_GOVERNANCE';
  let registration;
  try {
  ...
  exactKeys(registration, [
    ...CANDIDATE_KEYS,
    ...CANDIDATE_OPTIONAL_KEYS.filter((key) => Object.hasOwn(registration, key)),
  ], code, 'candidate registration');
  exactKeys(registration.code_bindings, CANDIDATE_CODE_KEYS, code,
    'candidate registration code bindings');
  validateCandidateReplacementMembers(registration, resolveBinding, code);
  ...
  const familyContract = candidateFamilyPackageContract(registration, code);
  ...
  const requiredTests = [
    ...
    ...(predecessorCount >= 2 ? [CANDIDATE_WORK3_SPLIT_TEST_PATH] : []),
    ...(predecessorCount >= 3 ? [CANDIDATE_WORK4_DISPATCH_TEST_PATH] : []),
  ...
  if (!same(subtypeFamilies, familyContract.familyKeys)) {
    fail(code, 'candidate does not bind one ordered subtype tree for the physical family set');
  }
  unique(subtypeContainerPaths, code, 'candidate subtype-tree package paths');
  if (familyContract.packagePaths !== null
      && !same(subtypeContainerPaths, familyContract.packagePaths)) {
  ...
  return { registration, familyContract };
}
...
function validateGovernance(governance, resolveBinding) {
  ...
  const { registration: candidate, familyContract: candidateFamilyContract } =
    validateCandidateRegistrationBytes(candidateBytes, governance, resolveBinding);
  validateCandidateVerification(governance.candidate_registration_verification, candidate,
    governance.candidate_registration_binding, candidateFamilyContract);
  ...
  const candidatePredecessorEntries = candidate.predecessor_receipt_bindings.map((entry) => {
    exactKeys(entry, ['work', 'binding'], code, 'candidate predecessor');
    if (!/^WORK[1-7]$/.test(entry.work)) fail(code, 'candidate predecessor work is invalid');
    return entry;
  });
  if (!same(candidatePredecessorEntries.map((entry) => entry.binding),
    governance.predecessor_receipt_bindings)) {
  ...
  for (let index = 0; index < governance.predecessor_receipt_bindings.length; index += 1) {
    const binding = governance.predecessor_receipt_bindings[index];
    ...
    predecessorRecords.set(candidatePredecessorEntries[index].work, receipt);
  }
  unique(predecessorPaths, code, 'predecessor receipt paths');
  const profileSet = inputRecords.get('APPROVED_FAMILY_PROFILE_SET');
  const registryContract = candidateFamilyContract.work3V2
    ? validateWork3V2FamilyPackageContract(
      predecessorRecords.get('WORK3'),
      governance.family_profile_set_binding,
      profileSet,
      candidateFamilyContract,
      'M7_V2_PROFILE_GATE',
    )
    : null;
  const packageRegistry = buildFamilyPackageRegistry(
    profileSet, resolveBinding, 'M7_V2_PROFILE_GATE', registryContract,
  );
  const approvedTreeBindings = profileSet.subtype_tree_bindings;
  ...
  for (let index = 0; index < candidate.subtype_tree_bindings.length; index += 1) {
    ...
    if (entry.family_key !== packageRegistry.familyKeys[index]) {
      fail(code, 'candidate subtype trees are not in the authorised physical family order');
    }
...
function applyDnoItem42PackageSuccessorAuthority(
  ...
  if (!same(successorBinding, DNO_ITEM42_SUCCESSOR_PACKAGE_BINDING)) {
    fail(code, 'D&O item-42 successor package binding is not the approved record');
  }
...
function applyFamilyGroupingPackageSuccessorAuthorities(
  authority,
  work3Authority,
  ...
  if (successorAuthorities === null) return authority;
  array(successorAuthorities, code, 'family grouping package successor authorities');
  if (successorAuthorities.length !== FAMILY_GROUPING_SUCCESSOR_CONTRACTS.length) {
    fail(code, 'family grouping package successor authority set is incomplete');
  }
  const contracts = new Map(
    FAMILY_GROUPING_SUCCESSOR_CONTRACTS.map((contract) => [contract.family_key, contract]),
  );
  const seen = new Set();
  const packagePaths = [...authority.packagePaths];
  const work3Bytes = Buffer.from(canonicalJson(work3Authority) + '\n', 'utf8');
  const expectedWork3Binding = {
    byte_length: work3Bytes.length,
    git_blob_oid: gitBlobOid(work3Bytes),
    path:
  ...
  for (const successorAuthority of successorAuthorities) {
    object(successorAuthority, code, 'family grouping package successor authority');
    exactKeys(successorAuthority, [
      'schema_version',
      'grouping_registration_successor_authority_id',
      'applied_on',
      'authority_state',
      'exact_grouping_stamp_clearance_ordinals',
      'exact_held_ambiguous_ordinals',
      'exact_mapped_without_predecessor_grouping_stamp_ordinals',
      'family_key',
      'family_package_seal_receipt_binding',
      'predecessor_package_binding',
      'production_activation_permitted',
      'profile_count',
      'ruling_locator',
      'ruling_receipt_binding',
      'stamp_clearance_permitted',
      'successor_disposition_binding',
      'successor_package_binding',
      'successor_session_receipt_binding',
      'work3_entry_correction_authority_binding',
      'zero_effect_boundary',
    ], code, 'family grouping package successor authority');
    const contract = contracts.get(successorAuthority.family_key);
    if (contract === undefined || seen.has(successorAuthority.family_key)) {
      fail(code, 'family grouping package successor authority family set is false');
    }
    seen.add(successorAuthority.family_key);
    const schema = 'N1_GROUPING_REGISTRATION_SUCCESSOR_AUTHORITY/V1';
    const unsigned = { ...successorAuthority };
    delete unsigned.grouping_registration_successor_authority_id;
    if (successorAuthority.schema_version !== schema
        || successorAuthority.grouping_registration_successor_authority_id
          !== contentId(schema, unsigned)
        || successorAuthority.grouping_registration_successor_authority_id
          !== contract.registration_authority_id
        || successorAuthority.applied_on !== '2026-09-01'
        || successorAuthority.authority_state !== 'REGISTERED_ZERO_PRODUCT_WRITE_EFFECT'
        || successorAuthority.profile_count !== contract.profile_count
        || successorAuthority.production_activation_permitted !== false
        || successorAuthority.stamp_clearance_permitted !== true
        || successorAuthority.exact_held_ambiguous_ordinals.length !== 0
        || !same(successorAuthority.ruling_locator, {
          family_key: contract.family_key,
          ruling_ordinal: FAMILY_GROUPING_SUCCESSOR_CONTRACTS.indexOf(contract) + 1,
        })
        || !same(successorAuthority.successor_package_binding,
          contract.successor_package_binding)
        || !same(successorAuthority.work3_entry_correction_authority_binding,
          expectedWork3Binding)
        || !same(successorAuthority.zero_effect_boundary, {
          database_write_count: 0,
          product_write_count: 0,
          serving_change_count: 0,
        })) {
      fail(code, 'family grouping package successor authority is not the approved record');
    }
    const familyIndex = authority.familyKeys.indexOf(contract.family_key);
    const suppliedSource = familyPackageSources[familyIndex];
    if (familyIndex === -1
        || !suppliedSource
        || !same(suppliedSource.binding, contract.successor_package_binding)
        || suppliedSource.record?.family_key !== contract.family_key
        || suppliedSource.record?.profiles?.length !== contract.profile_count
        || suppliedSource.record?.dimension_evidence?.length !== contract.profile_count) {
      fail(code, 'family grouping successor package bytes do not match its authority');
    }
    packagePaths[familyIndex] = contract.successor_package_binding.path;
  }
...
function validateProfileSnapshots(
  ...
  if (!same([...approvedFamilies].sort(), [...familyKeys].sort())) {
    fail(code, 'approved family profile set does not cover the authorised physical families');
  }
  ...
  if (ownedTreeBindings.length !== orderedFamilies.length) {
    fail(code, 'approved profile set does not own the exact physical subtype trees');
  }
...
function validateFamilyProfilePackageSetForWork3(input) {
  ...
  exactKeys(input, [
    ...
    'familyGroupingSuccessorAuthorities',
    'familyProfileSet',
    'familyPackageSources',
    'familyPacketSet',
  ...
  const dnoSuccessorAuthority = applyDnoItem42PackageSuccessorAuthority(
    baseAuthority,
  ...
  const authority = applyFamilyGroupingPackageSuccessorAuthorities(
    dnoSuccessorAuthority,
    input.work3Authority,
    input.familyGroupingSuccessorAuthorities,
    input.familyPackageSources,
    code,
  );
...
const WORK3_PHYSICAL_FAMILY_KEYS = Object.freeze([
  'ANTITRUST_REGULATORY',
  'APPRAISAL_DISSENTERS_RIGHTS',
  'CLOSING_CONDITIONS',
  'CONSIDERATION',
...
const WORK3_PHYSICAL_PACKAGE_KEYS = Object.freeze([
  'schema_version',
...
const WORK3_PHYSICAL_PROFILE_SET_KEYS = Object.freeze([
  'schema_version',
...
const WORK3_RECEIPT_V2_KEYS = Object.freeze([
  'schema_version',
  'work3_receipt_id',
  'work',
  'stage',
...
const WORK3_RECEIPT_V2_FAMILY_EVIDENCE_KEYS = Object.freeze([
  'family_profile_package_bindings',
  'approved_family_profile_set_binding',
  'governed_family_keys',
  'sealed_package_family_keys',
...
const WORK3_APPLICATION_RECEIPT_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_WORK3_CLOSURE_AMENDMENT_APPLICATION_RECEIPT/V1';
const WORK3_APPLICATION_RECEIPT_KEYS = Object.freeze([
  'schema_version',
...
function validateWork3ReceiptV2Binding(binding, resolveBinding, label) {
  const code = 'WORK3_ARTIFACT_BINDING_DRIFT';
  exactKeys(binding, BINDING_KEYS, code, label);
  let selectedBytes;
  try {
    selectedBytes = bytes(resolveBinding(binding), code, label);
  } catch {
    fail(code, `${label} resolution`);
  }
  if (!Number.isSafeInteger(binding.byte_length) || binding.byte_length < 0
      || typeof binding.sha256 !== 'string'
      || !/^[0-9a-f]{64}$/u.test(binding.sha256)
      || typeof binding.git_blob_oid !== 'string'
      || !/^[0-9a-f]{40}$/u.test(binding.git_blob_oid)
      || selectedBytes.length !== binding.byte_length
      || sha256Hex(selectedBytes) !== binding.sha256
      || gitBlobOid(selectedBytes) !== binding.git_blob_oid) {
  ...
  if (binding.schema_version === null) {
    if (binding.record_id_field !== null || binding.record_id !== null) {
      fail(code, `${label} raw binding identity`);
    }
    return { bytes: selectedBytes, record: null };
  }
  if (typeof binding.record_id_field !== 'string'
      || typeof binding.record_id !== 'string') {
    fail(code, `${label} record identity`);
  }
  const record = validateCanonicalRecordBytes(selectedBytes, binding, code, label);
  return { bytes: selectedBytes, record };
}

function expectedWork3Successor(
  amendment,
  predecessor,
  amendmentBinding,
  externalBinding,
  ...
  const overlay = object(
    amendment.successor_manifest_contract_overlay,
    'WORK3_RECEIPT_INVALID',
    'successor manifest overlay',
  );
...
function validateWork3ReceiptV2ParentBinding(binding, resolveBinding) {
  const code = 'WORK3_ARTIFACT_BINDING_DRIFT';
  exactKeys(binding, [
  ...
  let selectedBytes;
  try {
    selectedBytes = bytes(resolveBinding(binding), code, 'parent authority');
  } catch {
    fail(code, 'parent authority resolution');
  }
  let record;
  try {
    record = JSON.parse(selectedBytes.toString('utf8'));
  } catch {
    fail(code, 'parent authority JSON');
  }
  if (selectedBytes.length !== binding.byte_length
      || sha256Hex(selectedBytes) !== binding.sha256
      || !selectedBytes.equals(Buffer.from(`${canonicalJson(record)}\n`, 'utf8'))
    ...
    fail(code, 'parent authority bytes');
  }
}

function validateWork3ReceiptV2(options) {
  const code = 'WORK3_RECEIPT_INVALID';
  exactKeys(options, [
    'amendment', 'pathExists', 'physicalValidation', 'receipt', 'resolveBinding',
    'work3EntryAuthority',
  ], code, 'Work3 V2 receipt validation input');
  if (typeof options.resolveBinding !== 'function'
      || typeof options.pathExists !== 'function') fail(code, 'receipt callbacks');
  const amendment = object(options.amendment, code, 'closure amendment');
  const physical = object(options.physicalValidation, code, 'physical validation');
  const receipt = object(options.receipt, code, 'Work3 V2 receipt');
  const work3EntryAuthority = object(
    options.work3EntryAuthority, code, 'Work3 entry authority',
  );
  const contract = object(
    amendment.receipt_contract_overlay, code, 'closure-qualified receipt contract',
  );
  if (contract.schema_version !== WORK3_RECEIPT_V2_SCHEMA
      || contract.top_level_key_count !== 31
      || !same(contract.exact_keys, WORK3_RECEIPT_V2_KEYS)) {
    fail(code, 'closure-qualified receipt contract');
  }
  exactKeys(receipt, WORK3_RECEIPT_V2_KEYS, code, 'Work3 V2 receipt');
  const unsignedReceipt = { ...receipt };
  delete unsignedReceipt.work3_receipt_id;
  if (receipt.schema_version !== WORK3_RECEIPT_V2_SCHEMA
      || receipt.work3_receipt_id
        !== contentId(WORK3_RECEIPT_V2_SCHEMA, unsignedReceipt)
      || receipt.work !== 'WORK3'
    ...
    fail(code, 'Work3 receipt envelope');
  }

  const closure = object(
    amendment.effective_family_package_closure, code, 'physical family closure',
  );
  const sealedPackages = array(
    closure.sealed_family_packages, code, 'sealed family packages',
  );
  if (sealedPackages.length !== 24) fail(code, 'sealed package authority');
  const sealedBindings = sealedPackages.map((entry) => entry.package_binding);
  exactKeys(
    receipt.family_profile_evidence,
    WORK3_RECEIPT_V2_FAMILY_EVIDENCE_KEYS,
    code,
    'family profile evidence',
  );
  if (!same(receipt.family_profile_evidence.family_profile_package_bindings,
    sealedBindings)
      || !same(receipt.family_profile_evidence.governed_family_keys,
        closure.governed_family_keys)
      || !same(receipt.family_profile_evidence.sealed_package_family_keys,
        closure.sealed_family_keys)
      || !same(receipt.family_profile_evidence.parked_family_evidence,
        closure.capitalisation)) {
    fail(code, 'family profile evidence closure');
  }

  const artifactContract = object(
    contract.artifact_bindings_contract, code, 'artifact binding contract',
  );
  const artifactPaths = array(artifactContract.paths, code, 'artifact paths');
  if (artifactContract.count !== 52
      || artifactPaths.length !== 52
      || new Set(artifactPaths).size !== 52
      || !same(artifactPaths, [...artifactPaths].sort())
      || !Array.isArray(receipt.artifact_bindings)
      || receipt.artifact_bindings.length !== 52
      || !same(receipt.artifact_bindings.map((binding) => binding.path), artifactPaths)) {
    fail('WORK3_ARTIFACT_BINDING_DRIFT', 'exact artifact paths');
  }
  const artifacts = new Map();
  for (const binding of receipt.artifact_bindings) {
    if (artifacts.has(binding.path)) {
      fail('WORK3_ARTIFACT_BINDING_DRIFT', 'duplicate artifact path');
    }
    const selected = validateWork3ReceiptV2Binding(
      binding, options.resolveBinding, binding.path,
    );
    artifacts.set(binding.path, { binding, ...selected });
  }
  const categoryContracts = array(
    artifactContract.record_id_categories,
    code,
    'artifact record identity categories',
  );
  const categorizedPaths = new Set();
  for (const category of categoryContracts) {
    object(category, code, 'artifact record identity category');
    const specifications = Array.isArray(category.schema_and_id_fields)
      ? category.schema_and_id_fields
      : Array.isArray(category.paths)
        ? category.paths.map((selectedPath) => ({
          path: selectedPath,
    ...
    for (const specification of specifications) {
      const selectedPath = specification.path;
      if (!artifactPaths.includes(selectedPath) || categorizedPaths.has(selectedPath)) {
        fail('WORK3_ARTIFACT_BINDING_DRIFT', 'artifact identity category coverage');
      }
      categorizedPaths.add(selectedPath);
      const binding = artifacts.get(selectedPath)?.binding;
      if (binding?.schema_version !== specification.schema_version
          || binding?.record_id_field !== specification.record_id_field
          || typeof binding?.record_id !== 'string') {
        fail('WORK3_ARTIFACT_BINDING_DRIFT', `artifact identity category ${selectedPath}`);
      }
    }
  }
  for (const selectedPath of artifactPaths) {
    if (categorizedPaths.has(selectedPath)) continue;
    const binding = artifacts.get(selectedPath)?.binding;
    if (binding?.schema_version !== null
        || binding?.record_id_field !== null
        || binding?.record_id !== null) {
      fail('WORK3_ARTIFACT_BINDING_DRIFT', `raw artifact identity ${selectedPath}`);
    }
  }
  if (receipt.artifact_set_digest
      !== sha256Hex(Buffer.from(canonicalJson(receipt.artifact_bindings), 'utf8'))) {
    fail('WORK3_ARTIFACT_BINDING_DRIFT', 'artifact set digest');
  }
  const exactArtifact = (binding, label) => {
    exactKeys(binding, BINDING_KEYS, 'WORK3_ARTIFACT_BINDING_DRIFT', label);
    if (!same(artifacts.get(binding.path)?.binding, binding)) {
      fail('WORK3_ARTIFACT_BINDING_DRIFT', label);
    }
  };
  validateWork3ReceiptV2ParentBinding(
    receipt.parent_authority_binding, options.resolveBinding,
  );
  for (const [field, label] of [
    ['activation_receipt_binding', 'activation receipt'],
    ['predecessor_receipt_binding', 'predecessor receipt'],
    ['candidate_ordering_correction_authority_binding', 'ordering authority'],
  ]) validateWork3ReceiptV2Binding(receipt[field], options.resolveBinding, label);
  for (const [field, label] of [
    ['work3_entry_correction_authority_binding', 'Work3 entry authority'],
    ['closure_amendment_binding', 'closure amendment'],
    ['external_review_receipt_binding', 'external review receipt'],
    ['closure_application_receipt_binding', 'closure application receipt'],
    ['successor_execution_manifest_binding', 'successor execution manifest'],
  ]) exactArtifact(receipt[field], label);
  exactArtifact(
    receipt.family_profile_evidence.approved_family_profile_set_binding,
    'approved family profile set',
  );
  exactArtifact(receipt.structure_disposition_set_binding, 'structure disposition set');
  const amendmentArtifact = artifacts.get(receipt.closure_amendment_binding.path);
  if (amendmentArtifact?.binding.record_id !== amendment.closure_amendment_id
      || !same(amendmentArtifact?.record, amendment)) {
    fail('WORK3_ARTIFACT_BINDING_DRIFT', 'closure amendment identity');
  }
  if (!same(amendment.predecessor_work3_entry_correction_authority_binding,
    receipt.work3_entry_correction_authority_binding)
      || !same(
        artifacts.get(receipt.work3_entry_correction_authority_binding.path)?.record,
        work3EntryAuthority,
      )) {
    fail('WORK3_ARTIFACT_BINDING_DRIFT', 'Work3 entry authority identity');
  }
  const externalReview = artifacts.get(
    receipt.external_review_receipt_binding.path,
  )?.record;
  const externalContract = object(
    amendment.external_review_receipt_contract,
    code,
    'external review receipt contract',
  );
  exactKeys(externalReview, externalContract.exact_keys, code, 'external review receipt');
  if (externalReview.schema_version !== externalContract.schema_version
      || externalReview.status !== externalContract.status_exact_value
      || externalReview.review_state !== externalContract.review_state_exact_value
      || !same(externalReview.base_commit_binding, externalContract.base_commit_binding)
      || !same(externalReview.checks, externalContract.checks_exact_value)
      || !same(externalReview.findings, externalContract.findings_exact_value)
      || !same(externalReview.authority_boundary,
        externalContract.authority_boundary_exact_value)
      || !same(externalReview.independence_attestation,
        externalContract.independence_attestation_exact_value)
      || !same(externalReview.zero_effect_boundary,
        externalContract.zero_effect_boundary_exact_value)
      || !same(externalReview.reviewed_artifact_bindings?.amendment_binding,
        receipt.closure_amendment_binding)) {
    fail(code, 'external review receipt lineage');
  }
  exactKeys(
    externalReview.reviewed_artifact_bindings,
    externalContract.reviewed_artifact_bindings_exact_keys,
    code,
    'reviewed artifact bindings',
  );
  if (!same(externalReview.reviewed_artifact_bindings.generator_binding,
    externalContract.reviewed_artifact_binding_contracts.generator_binding)
      || !same(externalReview.reviewed_artifact_bindings.test_binding,
        externalContract.reviewed_artifact_binding_contracts.test_binding)) {
    fail(code, 'external review packet bindings');
  }
  exactKeys(
    externalReview.reviewer_identity,
    externalContract.reviewer_identity_contract.exact_keys,
    code,
    'external reviewer identity',
  );
  const reviewer = externalReview.reviewer_identity;
  if (Object.values(reviewer).some(
    (value) => typeof value !== 'string' || value.trim() !== value || value.length === 0,
  )
      || reviewer.authoring_vendor_id
        !== externalContract.reviewer_identity_contract.authoring_vendor_id_exact_value
      || reviewer.reviewer_model_id
        !== externalContract.reviewer_identity_contract.reviewer_model_id_contract.exact_value
      || !/^[A-Z0-9]+(?:_[A-Z0-9]+)*$/u.test(reviewer.reviewer_vendor_id)
      || externalContract.reviewer_identity_contract
        .reviewer_vendor_aliases_forbidden_case_insensitively
        .map((alias) => alias.replace(/[^A-Z0-9]/giu, '').toUpperCase())
        .some((alias) => reviewer.reviewer_vendor_id
          .replace(/[^A-Z0-9]/giu, '')
          .toUpperCase()
          .includes(alias))) {
    fail(code, 'external reviewer identity');
  }
  const targetContract = externalContract.review_target_commit_binding_contract;
  const target = externalReview.review_target_commit_binding;
  exactKeys(target, targetContract.exact_keys, code, 'review target commit binding');
  if (target.branch !== targetContract.branch_exact_value
      ...
      || target.commit_sha !== target.live_remote_commit_sha
      || target.commit_sha !== target.remote_ref_commit_sha
      || !/^[0-9a-f]{40}$/u.test(target.commit_sha)
      || !/^[0-9a-f]{40}$/u.test(target.tree_sha)
      || !/^\d{4}-\d{2}-\d{2}$/u.test(externalReview.reviewed_on)) {
    fail(code, 'review target commit binding');
  }
  const reviewedBindingsByPath = new Map(
    Object.values(externalReview.reviewed_artifact_bindings)
      .map((binding) => [binding.path, binding]),
  );
  if (!Array.isArray(target.path_blob_bindings)
      || target.path_blob_bindings.length !== target.changed_paths.length
      || !same(target.path_blob_bindings.map((binding) => binding.path),
        target.changed_paths)
      || target.path_blob_bindings.some((binding) => (
        !isObject(binding)
        || !same(Object.keys(binding).sort(), ['git_blob_oid', 'path'])
        || binding.git_blob_oid !== reviewedBindingsByPath.get(binding.path)?.git_blob_oid
      ))) {
    fail(code, 'review target path-blob bindings');
  }
  const application = artifacts.get(
    receipt.closure_application_receipt_binding.path,
  )?.record;
  exactKeys(application, WORK3_APPLICATION_RECEIPT_KEYS, code, 'application receipt');
  if (application.schema_version !== WORK3_APPLICATION_RECEIPT_SCHEMA
      || application.state !== 'IMMUTABLE_ZERO_EFFECT_APPLICATION'
      || !same(application.closure_amendment_binding, receipt.closure_amendment_binding)
      || !same(application.external_review_receipt_binding,
        receipt.external_review_receipt_binding)
      || !same(application.zero_effect_boundary, amendment.zero_effect_boundary)) {
    fail(code, 'application receipt lineage');
  }
  const successor = artifacts.get(receipt.successor_execution_manifest_binding.path)?.record;
  const predecessor = validateResolvedRecordBinding(
    amendment.predecessor_work3_execution_manifest_binding,
    options.resolveBinding,
    'predecessor execution manifest',
    'WORK3_ARTIFACT_BINDING_DRIFT',
  );
  const expectedSuccessor = expectedWork3Successor(
    amendment,
    predecessor,
    receipt.closure_amendment_binding,
    receipt.external_review_receipt_binding,
    receipt.closure_application_receipt_binding,
  );
  if (!isObject(successor)
      || !same(successor, expectedSuccessor)
      || receipt.execution_manifest_id
        !== receipt.successor_execution_manifest_binding.record_id
      || successor.execution_manifest_id !== receipt.execution_manifest_id
      || successor.execution_manifest_digest !== receipt.execution_manifest_digest
      || !same(successor.parent_authority_binding, receipt.parent_authority_binding)
      || !same(successor.activation_receipt_binding, receipt.activation_receipt_binding)
      || !same(successor.predecessor_receipt_binding, receipt.predecessor_receipt_binding)
      || !same(successor.candidate_ordering_correction_authority_binding,
        receipt.candidate_ordering_correction_authority_binding)
      || !same(successor.work3_entry_correction_authority_binding,
        receipt.work3_entry_correction_authority_binding)
      || !same(successor.closure_amendment_binding, receipt.closure_amendment_binding)
      || !same(successor.external_review_receipt_binding,
        receipt.external_review_receipt_binding)
      || !same(successor.closure_application_receipt_binding,
        receipt.closure_application_receipt_binding)) {
    fail(code, 'successor manifest lineage');
  }

  const approvedProfileSet = artifacts.get(
    receipt.family_profile_evidence.approved_family_profile_set_binding.path,
  )?.record;
  let recomputedPhysical;
  try {
    recomputedPhysical = validateWork3PhysicalClosureV2({
      closure,
      familyProfileSet: approvedProfileSet,
      resolveBinding: options.resolveBinding,
      pathExists: options.pathExists,
    });
  } catch (error) {
    fail(error.code ?? code, 'physical closure recomputation');
  }
  if (!same(recomputedPhysical, physical)) {
    fail(code, 'physical validation is not independently recomputed');
  }

  const nativeContract = object(
    work3EntryAuthority.work3_scope_contract?.rich_work3_receipt_contract
      ?.candidate_native_set_evidence_contract,
    code,
    'candidate native-set evidence contract',
  );
  exactKeys(
    receipt.candidate_native_set_evidence,
    nativeContract.exact_keys,
    code,
    'candidate native-set evidence',
  );
  ...
  if (!same([
    ...
    fail(code, 'candidate native-set authority');
  }
  const nativeSetContracts = array(
    work3EntryAuthority.agreement_index_set_authority?.sets,
    code,
    'native set authority records',
  );
  if (nativeSetContracts.length !== 3) fail(code, 'native set authority records');
  for (const [index, field] of [
    [0, 'work3_agreement_index_set_binding'],
    [1, 'work3_context_compilation_set_binding'],
    [2, 'work3_agreement_analysis_set_binding'],
  ]) {
    const selectedBinding = nativeEvidence[field];
    const bindingContract = nativeContract.work3_binding_contracts[index];
    if (selectedBinding.path !== bindingContract.path
        || selectedBinding.schema_version !== bindingContract.schema_version
        || selectedBinding.record_id_field !== bindingContract.record_id_field) {
      fail(code, `candidate native-set binding ${index}`);
    }
    exactArtifact(selectedBinding, `candidate native-set binding ${index}`);
    const nativeRecord = artifacts.get(selectedBinding.path)?.record;
    const setContract = nativeSetContracts.find(
      (entry) => entry.path === selectedBinding.path,
    );
    if (!setContract
        || setContract.schema_version !== selectedBinding.schema_version
        || setContract.record_id_field !== selectedBinding.record_id_field
        || !Array.isArray(setContract.members)
        || setContract.members.length !== 10) {
      fail(code, `candidate native-set contract ${index}`);
    }
    const unsignedNativeRecord = {
      schema_version: setContract.schema_version,
      members: structuredClone(setContract.members),
    };
    const expectedNativeRecord = {
      ...unsignedNativeRecord,
      [setContract.record_id_field]: contentId(
        setContract.schema_version,
        unsignedNativeRecord,
      ),
    };
    if (!same(nativeRecord, expectedNativeRecord)) {
      fail(code, `candidate native-set members ${index}`);
    }
  }
  for (const [field, label] of [
    ['work2_agreement_analysis_set_binding', 'Work2 analysis set'],
    ['work2_context_compilation_set_binding', 'Work2 context set'],
  ]) validateWork3ReceiptV2Binding(nativeEvidence[field], options.resolveBinding, label);

  const countsContract = object(contract.counts_contract, code, 'counts contract');
  exactKeys(receipt.counts, countsContract.exact_keys, code, 'receipt counts');
  for (const [key, value] of Object.entries(countsContract.exact_values)) {
    if (receipt.counts[key] !== value) fail(code, `receipt count ${key}`);
  }
  if (physical.status !== 'PASS_WORK3_PHYSICAL_CLOSURE_V2') {
    fail(code, 'physical validation status');
  }
  const physicalCounts = {
    family_profile_package_count: physical.family_package_count,
    parked_family_count: physical.parked_family_count,
    profile_count: physical.profile_count,
    dimension_evidence_count: physical.dimension_evidence_count,
    subtype_tree_binding_count: physical.subtype_tree_count,
    match_fixture_count: physical.match_fixture_count,
    structure_fixture_member_count: physical.structure_fixture_member_count,
    package_member_count: physical.package_member_count,
  };
  for (const [key, value] of Object.entries(physicalCounts)) {
    if (receipt.counts[key] !== value) fail(code, `physical count ${key}`);
  }
  const structure = artifacts.get(receipt.structure_disposition_set_binding.path)?.record;
  if (!Array.isArray(structure?.members)
      || receipt.counts.structure_disposition_member_count !== structure.members.length) {
    fail(code, 'structure disposition member count');
  }
  const lawfulFixtureContract = object(
    amendment.lawful_fixture,
    code,
    'lawful fixture contract',
  );
  const lawfulFixtureBinding = lawfulFixtureContract.fixture_binding;
  exactKeys(lawfulFixtureBinding, [
    'path', 'schema_version', 'byte_length', 'sha256', 'git_blob_oid',
  ], code, 'lawful fixture binding');
  let lawfulFixtureBytes;
  try {
    lawfulFixtureBytes = bytes(
      options.resolveBinding(lawfulFixtureBinding),
      code,
      'lawful fixture',
    );
  } catch {
    fail(code, 'lawful fixture resolution');
  }
  if (lawfulFixtureBinding.schema_version !== null
      || lawfulFixtureBytes.length !== lawfulFixtureBinding.byte_length
      || sha256Hex(lawfulFixtureBytes) !== lawfulFixtureBinding.sha256
      || gitBlobOid(lawfulFixtureBytes) !== lawfulFixtureBinding.git_blob_oid) {
    fail(code, 'lawful fixture physical binding');
  }
  let lawfulFixture;
  try {
    lawfulFixture = JSON.parse(gunzipSync(
      Buffer.from(lawfulFixtureBytes.toString('utf8').trim(), 'base64'),
    ).toString('utf8'));
  } catch {
    fail(code, 'lawful fixture encoding');
  }
  const unsignedLawfulFixture = { ...lawfulFixture };
  delete unsignedLawfulFixture.fixture_digest;
  const ambiguousFixtureRecord = lawfulFixture.generated_native_source_records?.find(
    (entry) => entry.binding?.path
      === contract.create_once_output_paths[0],
  )?.record;
  let expectedStructure;
  try {
    expectedStructure = rebindWork3StructureDispositionSet({
      packageBindings: sealedBindings,
      resolveBinding: options.resolveBinding,
      structureDispositionSet: lawfulFixture.structure_disposition_set,
    });
  } catch (error) {
    fail(error.code ?? code, 'lawful fixture structure rebinding');
  }
  if (lawfulFixture.schema_version !== lawfulFixtureContract.fixture_schema_version
      || lawfulFixture.fixture_digest !== lawfulFixtureContract.fixture_digest
      || lawfulFixture.fixture_digest
        !== sha256Hex(Buffer.from(canonicalJson(unsignedLawfulFixture), 'utf8'))
      || !same(expectedStructure, structure)
      || !same(
        artifacts.get(contract.create_once_output_paths[0])?.record,
        ambiguousFixtureRecord,
      )) {
    fail(code, 'lawful fixture output closure');
  }
  const packageRegistry = {
    byPath: new Map(sealedBindings.map((binding) => [
      binding.path,
      {
        binding,
        record: validateResolvedRecordBinding(
          binding,
          options.resolveBinding,
          `structure package ${binding.path}`,
          code,
        ),
      },
    ])),
  ...
  let structureFixtureReferenceCount = 0;
  for (const [memberIndex, member] of structure.members.entries()) {
    const inclusion = array(
      member.inclusion_fixture_bindings,
      code,
      `structure member ${memberIndex} inclusion fixtures`,
    );
    const exclusion = array(
      member.exclusion_fixture_bindings,
      code,
      `structure member ${memberIndex} exclusion fixtures`,
    );
    if (inclusion.length === 0 || exclusion.length === 0) {
      fail(code, `structure member ${memberIndex} fixture closure`);
    }
    for (const [kind, bindings] of [['inclusion', inclusion], ['exclusion', exclusion]]) {
      for (const [fixtureIndex, binding] of bindings.entries()) {
        resolvePackageMemberBinding(
          binding,
          packageRegistry,
          'match_fixtures',
          `structure member ${memberIndex} ${kind} fixture ${fixtureIndex}`,
          code,
        );
      }
    }
    const repeatBindings = member.inline_list_overlay?.ambiguous_repeat_fixture_bindings ?? [];
    for (const [fixtureIndex, binding] of repeatBindings.entries()) {
      resolvePackageMemberBinding(
        binding,
        packageRegistry,
        'structure_fixture_members',
        `structure member ${memberIndex} repeat fixture ${fixtureIndex}`,
        code,
      );
      structureFixtureReferenceCount += 1;
    }
  }
  if (structureFixtureReferenceCount !== 1) {
    fail(code, 'structure fixture package-member closure');
  }

  const repositoryContract = object(
    contract.repository_precondition_contract, code, 'repository precondition contract',
  );
  exactKeys(
    receipt.repository_precondition,
    repositoryContract.exact_keys,
    code,
    'repository precondition',
  );
  if (!repositoryContract.exact_keys.every(
    (key) => same(receipt.repository_precondition[key], repositoryContract[key]),
  )
      || receipt.repository_precondition.effective_work3_paths.length !== 53
      || !receipt.repository_precondition.effective_work3_paths.includes(
        contract.work3_receipt_path,
      )
      || artifacts.has(contract.work3_receipt_path)
      || !same(receipt.repository_precondition.immutable_pre_existing_package_bindings,
        sealedBindings)
      || !same(receipt.repository_precondition.generated_paths_absent,
        contract.create_once_output_paths)
      || contract.create_once_output_paths.length !== 7) {
    fail(code, '53-path repository closure');
  }
  exactKeys(receipt.effects, contract.effects_contract.exact_keys, code, 'effects');
  exactKeys(receipt.next_work, contract.next_work_contract.exact_keys, code, 'next work');
  if (!same(receipt.effects, contract.effects_contract.exact_values)
      || !same(receipt.checks, contract.checks_contract.exact_ordered_checks)
      || !same(receipt.next_work, contract.next_work_contract.exact_values)) {
    fail(code, 'checks, effects, or next work');
  }
  const ledgerContract = object(
    contract.command_execution_ledger_contract, code, 'command ledger contract',
  );
  const executionFixtureContract = object(
    amendment.work3_execution_fixture_contract_overlay?.effective_contract,
    code,
    'Work3 execution fixture contract',
  );
  const executionFixtureArtifact = artifacts.get(executionFixtureContract.path);
  if (!executionFixtureArtifact || executionFixtureArtifact.record !== null) {
    fail(code, 'Work3 execution fixture artifact');
  }
  let executionFixture;
  try {
    executionFixture = JSON.parse(executionFixtureArtifact.bytes.toString('utf8'));
  } catch {
    fail(code, 'Work3 execution fixture JSON');
  }
  exactKeys(
    executionFixture,
    executionFixtureContract.exact_keys,
    code,
    'Work3 execution fixture',
  );
  if (executionFixtureArtifact.binding.schema_version !== null
      || !executionFixtureArtifact.bytes.equals(
        Buffer.from(`${canonicalJson(executionFixture)}\n`, 'utf8'),
      )
      || executionFixture.schema_version !== executionFixtureContract.schema_version
      || executionFixture.state !== executionFixtureContract.state
      || !same(executionFixture.case_ids, executionFixtureContract.case_ids)
      || !same(executionFixture.combined_test_result,
        executionFixtureContract.combined_test_result)
      || !Array.isArray(executionFixture.command_run_counts)
      || executionFixture.command_run_counts.length !== 21) {
    fail(code, 'Work3 execution fixture contract');
  }
  if (!Array.isArray(receipt.command_execution_ledger)
      || receipt.command_execution_ledger.length !== 21
      || ledgerContract.entry_count !== 21
      || !same(receipt.command_execution_ledger.map((entry) => entry?.argv),
        ledgerContract.argv_order)
      || receipt.command_execution_ledger.some((entry) => (
        !isObject(entry)
        || !same(Object.keys(entry).sort(), ledgerContract.entry_exact_keys.slice().sort())
        || !Array.isArray(entry.argv)
        || !Number.isSafeInteger(entry.run_count)
        || entry.run_count < 0
        || typeof entry.state !== 'string'
      ))) {
    fail(code, 'command or test ledger');
  }
  for (const range of ledgerContract.state_ranges) {
    for (let index = range.indices[0]; index <= range.indices[1]; index += 1) {
      if (receipt.command_execution_ledger[index]?.state !== range.state) {
        fail(code, `command ledger state ${index}`);
      }
    }
  }
  if (!same(
    ...
    executionFixture.command_run_counts,
      ...
      || !same(receipt.combined_test_result, executionFixture.combined_test_result)) {
    fail(code, 'command ledger or combined result differs from the bound fixture');
  }
  for (let index = 0; index <= 3; index += 1) {
    if (receipt.command_execution_ledger[index].run_count
        < ledgerContract.bootstrap_counts_minimum) {
      fail(code, `bootstrap command count ${index}`);
    }
  }
  for (let index = 4; index <= 18; index += 1) {
    if (receipt.command_execution_ledger[index].run_count
        < ledgerContract.pre_receipt_work3_command_counts_minimum) {
      fail(code, `pre-receipt command count ${index}`);
    }
  }
  const runLimits = array(
    amendment.successor_manifest_contract_overlay?.exact_argv_with_run_limits,
    code,
    'successor command run limits',
  );
  if (runLimits.length !== 17) fail(code, 'successor command run limits');
  for (let index = 0; index < runLimits.length; index += 1) {
    const limit = object(runLimits[index], code, `successor command limit ${index}`);
    const ledgerEntry = receipt.command_execution_ledger[index + 4];
    if (!same(limit.argv, ledgerEntry.argv)
        || !Number.isSafeInteger(limit.max_runs)
        || limit.max_runs < 0
        || ledgerEntry.run_count > limit.max_runs) {
      fail(code, `successor command limit ${index}`);
    }
  }
  if (receipt.command_execution_ledger[19].run_count
    ...
    fail(code, 'final command counts');
  }
  const combinedContract = object(
    executionFixtureContract.combined_test_result, code, 'combined test contract',
  );
  exactKeys(
    receipt.combined_test_result,
    Object.keys(combinedContract),
    code,
    'combined test result',
  );
  if (!same(receipt.combined_test_result, combinedContract)) {
    fail(code, 'combined test result');
  }
  return deepFreezeTransient({
    schema_version: 'STAGE_2Y_M7_V2_REPAIR_WORK3_VALIDATION/V2',
    status: 'PASS',
    work3_receipt_id: receipt.work3_receipt_id,
    family_package_count: receipt.counts.family_profile_package_count,
    profile_count: receipt.counts.profile_count,
    artifact_binding_count: receipt.counts.artifact_binding_count,
    effective_path_count: receipt.counts.effective_path_count,
    create_once_output_count: receipt.counts.create_once_output_count,
  });
}

function work3PhysicalMemberBinding(containerPath, field, index, record, contract) {
  ...
  return {
    ...
    member_field: field,
    member_index: index,
    member_schema_version: contract.schema,
    member_record_id_field: contract.idField,
    member_record_id: record[contract.idField],
    member_byte_length: memberBytes.length,
    member_sha256: sha256Hex(memberBytes),
  };
}

function rebindWork3StructureDispositionSet(options) {
  const code = 'M7_V2_WORK3_PHYSICAL_CLOSURE';
  exactKeys(options, [
    'packageBindings', 'resolveBinding', 'structureDispositionSet',
  ], code, 'structure disposition rebinding input');
  if (!Array.isArray(options.packageBindings)
      || typeof options.resolveBinding !== 'function') {
    fail(code, 'structure disposition rebinding callbacks');
  }
  const sourceSet = object(
    options.structureDispositionSet,
    code,
    'source structure disposition set',
  );
  const packagesByPath = new Map(options.packageBindings.map((binding) => [
    binding.path,
    validateResolvedRecordBinding(
      binding,
      options.resolveBinding,
  ...
  if (packagesByPath.size !== options.packageBindings.length) {
    fail(code, 'structure package registry');
  }
  const rebind = (binding, expectedField, label) => {
    const memberContract = validatePackageMemberBindingShape(
      binding,
      label,
      code,
      PACKAGE_MEMBER_FIELDS,
    );
    if (binding.member_field !== expectedField) {
      fail(code, `${label} uses the wrong package member field`);
    }
    const container = packagesByPath.get(binding.container_path);
    if (!container) fail(code, `${label} has no sealed package container`);
    const members = container[expectedField];
    ...
    if (matches.length !== 1
        || memberContentId(matches[0].member, memberContract)
          !== binding.member_record_id) {
      fail(code, `${label} does not identify one sealed package member`);
    }
    return work3PhysicalMemberBinding(
  ...
  const members = array(sourceSet.members, code, 'source structure dispositions').map(
    (sourceMember, memberIndex) => {
      const unsigned = structuredClone(sourceMember);
      delete unsigned.structure_disposition_id;
      unsigned.inclusion_fixture_bindings = array(
        sourceMember.inclusion_fixture_bindings,
        code,
        `structure member ${memberIndex} inclusion fixtures`,
      ).map((binding, fixtureIndex) => rebind(
        binding,
        'match_fixtures',
        `structure member ${memberIndex} inclusion fixture ${fixtureIndex}`,
      ));
      unsigned.exclusion_fixture_bindings = array(
        sourceMember.exclusion_fixture_bindings,
        code,
        `structure member ${memberIndex} exclusion fixtures`,
      ).map((binding, fixtureIndex) => rebind(
        binding,
        'match_fixtures',
        `structure member ${memberIndex} exclusion fixture ${fixtureIndex}`,
      ));
      if (unsigned.inline_list_overlay !== null) {
        unsigned.inline_list_overlay.ambiguous_repeat_fixture_bindings = array(
          sourceMember.inline_list_overlay?.ambiguous_repeat_fixture_bindings,
          code,
          `structure member ${memberIndex} repeat fixtures`,
        ).map((binding, fixtureIndex) => rebind(
          binding,
          'structure_fixture_members',
          `structure member ${memberIndex} repeat fixture ${fixtureIndex}`,
      ...
      return {
        ...unsigned,
        structure_disposition_id: contentId(unsigned.schema_version, unsigned),
      };
    },
  );
  const unsignedSet = { ...sourceSet, members };
  delete unsignedSet.structure_disposition_set_id;
  return deepFreezeTransient({
    ...unsignedSet,
    structure_disposition_set_id: contentId(unsignedSet.schema_version, unsignedSet),
  });
}

function validateWork3PhysicalPackageMembers(record, binding, familyKey, code) {
  const fields = [
    ['profiles', false],
    ['dimension_evidence', false],
    ['match_fixtures', false],
    ['structure_fixture_members', false],
  ];
  const members = {};
  for (const [field] of fields) {
    const contract = PACKAGE_MEMBER_FIELDS[field];
    const selected = array(record[field], code, `${familyKey} ${field}`);
    const ids = [];
    selected.forEach((member, index) => {
      object(member, code, `${familyKey} ${field} member`);
      if (member.schema_version !== contract.schema) {
        fail(code, `${familyKey} ${field} member schema is invalid`);
      }
      assertHex(member[contract.idField], 64, code, `${familyKey} ${field} member ID`);
      ids.push(member[contract.idField]);
      if (field === 'profiles'
          && (member.family_key !== familyKey
            || member.profile_set_version !== record.profile_set_version)) {
        fail(code, `${familyKey} profile member is outside its package`);
      }
      if (field === 'dimension_evidence' && member.family_key !== familyKey) {
        fail(code, `${familyKey} dimension evidence is outside its package`);
      }
    });
    unique(ids, code, `${familyKey} ${field} member IDs`);
    members[field] = {
      records: selected,
      bindings: selected.map((member, index) => work3PhysicalMemberBinding(
        binding.path,
        field,
        index,
        member,
  ...
  const tree = object(record.subtype_tree, code, `${familyKey} subtype tree`);
  const treeContract = PACKAGE_MEMBER_FIELDS.subtype_tree;
  if (tree.schema_version !== treeContract.schema
      || tree.family_key !== familyKey
      || tree.profile_set_version !== record.profile_set_version) {
    fail(code, `${familyKey} subtype tree is outside its package`);
  }
  assertHex(tree[treeContract.idField], 64, code, `${familyKey} subtype-tree ID`);
  members.subtype_tree = {
    record: tree,
    binding: work3PhysicalMemberBinding(
      binding.path,
      'subtype_tree',
      null,
      tree,
...
function validateWork3PhysicalClosureV2(options) {
  const code = 'M7_V2_WORK3_PHYSICAL_CLOSURE';
  exactKeys(options, [
    'closure',
    'familyProfileSet',
    'resolveBinding',
    'pathExists',
  ], code, 'Work3 physical closure validation input');
  if (typeof options.resolveBinding !== 'function'
      || typeof options.pathExists !== 'function') {
    fail(code, 'physical closure callbacks are invalid');
  }
  const closure = object(options.closure, code, 'physical closure');
  exactKeys(closure, [
    'capitalisation',
    'family_count',
    'full_set_validation',
    ...
    'sealed_member_inventory',
    'sealed_profile_count',
  ], code, 'physical closure');
  const governedFamilyKeys = [...WORK3_PHYSICAL_FAMILY_KEYS];
  governedFamilyKeys.splice(2, 0, 'CAPITALISATION');
  if (closure.family_count !== 25
      || closure.parked_family_count !== 1
      || closure.sealed_family_package_count !== 24
      || closure.sealed_profile_count !== 1382
      || !same(closure.governed_family_keys, governedFamilyKeys)
      || !same(closure.sealed_family_keys, WORK3_PHYSICAL_FAMILY_KEYS)
      || !same(closure.full_set_validation, {
        dimension_evidence_count: 1383,
        family_package_count: 25,
        profile_count: 1383,
        state: 'PASS_WITH_SYNTHETIC_CAPITALISATION_VALIDATION_INPUT_ONLY',
      })) {
    fail(code, 'physical family closure is not the exact 24 plus one parked estate');
  }
  const capitalisation = object(
    closure.capitalisation, code, 'parked CAPITALISATION family',
  );
  exactKeys(capitalisation, [
    ...
    'stage_9f_authority_required',
    'synthetic_validation_package',
  ], code, 'parked CAPITALISATION family');
  const syntheticCapitalisation = object(
    capitalisation.synthetic_validation_package,
    code,
    'synthetic CAPITALISATION validation package',
  );
  exactKeys(syntheticCapitalisation, [
    'package_id', 'profile_count', 'state',
  ], code, 'synthetic CAPITALISATION validation package');
  assertHex(
    syntheticCapitalisation.package_id,
    64,
    code,
    'synthetic CAPITALISATION package ID',
  );
  if (capitalisation.family_key !== 'CAPITALISATION'
      || capitalisation.on_disk_package_binding !== null
      || capitalisation.on_disk_package_present !== false
      || capitalisation.parked_state !== 'PARKED'
      || typeof capitalisation.planned_package_path !== 'string'
      || capitalisation.planned_package_path.length === 0
      || capitalisation.product_activation_permitted !== false
      || capitalisation.serving_permitted !== false
      || capitalisation.stage_9f_authority_required !== true
      || syntheticCapitalisation.profile_count !== 1
      || syntheticCapitalisation.state
        !== 'IN_MEMORY_VALIDATION_ONLY_NOT_A_SEALED_PACKAGE') {
    fail(code, 'CAPITALISATION is not parked at the exact physical boundary');
  }
  let capitalisationPresent;
  try {
    capitalisationPresent = options.pathExists(capitalisation.planned_package_path);
  } catch {
    fail(code, 'CAPITALISATION presence could not be resolved');
  }
  if (capitalisationPresent !== false) {
    fail(code, 'parked CAPITALISATION has a physical package');
  }

  const sealedPackages = array(
    closure.sealed_family_packages, code, 'sealed family packages',
  );
  if (sealedPackages.length !== WORK3_PHYSICAL_FAMILY_KEYS.length) {
    fail(code, 'sealed family package count is invalid');
  }
  ...
  for (let index = 0; index < WORK3_PHYSICAL_FAMILY_KEYS.length; index += 1) {
    const familyKey = WORK3_PHYSICAL_FAMILY_KEYS[index];
    const packageEntry = sealedPackages[index];
    exactKeys(packageEntry, [
      'family_key', 'package_binding', 'profile_count',
    ], code, `sealed package ${familyKey}`);
    if (packageEntry.family_key !== familyKey
        || !Number.isSafeInteger(packageEntry.profile_count)
        || packageEntry.profile_count < 1) {
      fail(code, `sealed package ${familyKey} is reordered or empty`);
    }
    const binding = packageEntry.package_binding;
    exactKeys(binding, BINDING_KEYS, code, `sealed package ${familyKey} binding`);
    if (binding.schema_version !== FAMILY_PROFILE_PACKAGE_SCHEMA
        || binding.record_id_field !== 'family_profile_package_id'
        || containerPaths.has(binding.path)
        || binding.path === capitalisation.planned_package_path) {
      fail(code, `sealed package ${familyKey} binding is invalid`);
    }
    containerPaths.add(binding.path);
    const record = validateResolvedRecordBinding(
      binding,
      options.resolveBinding,
      `sealed package ${familyKey}`,
      code,
    );
    exactKeys(record, WORK3_PHYSICAL_PACKAGE_KEYS, code, `sealed package ${familyKey}`);
    if (record.schema_version !== FAMILY_PROFILE_PACKAGE_SCHEMA
        || record.state !== 'BEN_APPROVED_FAMILY_PROFILE_PACKAGE'
        || record.family_key !== familyKey
        || !Number.isSafeInteger(record.profile_set_version)
        || record.profile_set_version < 1
        || record.profiles.length !== packageEntry.profile_count
        || !isObject(record.family_approval)
        || typeof record.family_approval.ben_approval_id !== 'string'
        || record.family_approval.ben_approval_id.length === 0) {
      fail(code, `sealed package ${familyKey} envelope is invalid`);
    }
    const nestedPackage = { ...record };
    delete nestedPackage.family_profile_package_id;
    if (containsExactNestedValue(nestedPackage, binding)
      ...
      fail(code, `sealed package ${familyKey} contains its own outer binding`);
    }
    packageBindings.push(binding);
    packageRecords.push(record);
    memberSets.push(validateWork3PhysicalPackageMembers(
      record, binding, familyKey, code,
    ));
    approvalIds.push(record.family_approval.ben_approval_id);
  }
  unique(approvalIds, code, 'sealed package Ben approval IDs');

  const profileSet = object(
    options.familyProfileSet, code, 'approved physical family profile set',
  );
  exactKeys(
    profileSet,
    WORK3_PHYSICAL_PROFILE_SET_KEYS,
    code,
    'approved physical family profile set',
  );
  const unsignedProfileSet = { ...profileSet };
  delete unsignedProfileSet.family_profile_set_id;
  if (profileSet.schema_version !== INPUT_SCHEMAS.APPROVED_FAMILY_PROFILE_SET
      ...
      || !same(profileSet.family_profile_package_bindings, packageBindings)) {
    fail(code, 'approved physical family profile set envelope is invalid');
  }
  const expectedProfiles = packageRecords.flatMap((record) => record.profiles);
  const expectedDimensions = memberSets.flatMap(
    (members) => members.dimension_evidence.bindings,
  );
  const expectedTrees = WORK3_PHYSICAL_FAMILY_KEYS.map((familyKey, index) => ({
    family_key: familyKey,
    binding: memberSets[index].subtype_tree.binding,
  }));
  if (!same(profileSet.profiles, expectedProfiles)
      || !same(profileSet.dimension_evidence_bindings, expectedDimensions)
      || !same(profileSet.subtype_tree_bindings, expectedTrees)) {
    fail(code, 'approved physical set differs from exact sealed package members');
  }
  unique(expectedProfiles.map((profile) => profile.profile_id), code, 'physical profile IDs');
  unique(
    packageRecords.flatMap((record) => record.dimension_evidence)
      .map((member) => member.dimension_evidence_id),
    code,
    'physical dimension-evidence IDs',
  );
  unique(
    packageRecords.flatMap((record) => record.match_fixtures)
      .map((member) => member.match_fixture_id),
    code,
    'physical match-fixture IDs',
  );
  unique(
    packageRecords.map((record) => record.subtype_tree.subtype_tree_id),
    code,
    'physical subtype-tree IDs',
  );
  unique(
    packageRecords.flatMap((record) => record.structure_fixture_members)
      .map((member) => member.fixture_id),
    code,
    'physical structure-fixture IDs',
  );

  const counts = {
    family_package_count: packageRecords.length,
    parked_family_count: 1,
    profile_count: expectedProfiles.length,
    dimension_evidence_count: expectedDimensions.length,
    subtype_tree_count: expectedTrees.length,
    match_fixture_count: packageRecords.reduce(
      (total, record) => total + record.match_fixtures.length,
      0,
    ),
    structure_fixture_member_count: packageRecords.reduce(
      (total, record) => total + record.structure_fixture_members.length,
      0,
    ),
  };
  counts.package_member_count = counts.profile_count
    + counts.dimension_evidence_count
    + counts.subtype_tree_count
    + counts.match_fixture_count
    + counts.structure_fixture_member_count;
  const expectedMemberInventory = {
    by_family: packageRecords.map((record) => {
      const selected = {
    ...
    totals: {
      dimension_evidence_count: counts.dimension_evidence_count,
      match_fixture_count: counts.match_fixture_count,
      profile_count: counts.profile_count,
      structure_fixture_member_count: counts.structure_fixture_member_count,
      subtype_tree_count: counts.subtype_tree_count,
      total_package_member_count: counts.package_member_count,
    },
  };
  if (!same(closure.sealed_member_inventory, expectedMemberInventory)) {
    fail(code, 'physical package member inventory differs from sealed authority');
  }
  if (!same(counts, {
    ...
    fail(code, 'physical package member totals are not exact');
  }
  return deepFreezeTransient({
...
function validateSources(analysis, governance, resolveBinding, semanticInputs) {
  ...
  for (const closure of closures.values()) {
    ...
    if (Object.hasOwn(closure, 'context_spans')) closureKeys.push('context_spans');
    exactKeys(closure, closureKeys, closureCode, 'source closure');
    ...
    if (Object.hasOwn(closure, 'context_spans')) {
      const contextSpans = array(closure.context_spans, closureCode, 'context spans');
      for (const contextSpan of contextSpans) {
        exactKeys(contextSpan, [
          'kind',
          'node_occurrence_id',
          'start_byte',
          'end_byte',
          'text_sha256',
        ], closureCode, 'context span');
        if (!['PARENT_CHAIN', 'GOVERNING_CHAPEAU', 'ARTICLE_CHAPEAU'].includes(contextSpan.kind)
            || !Number.isInteger(contextSpan.start_byte)
            || !Number.isInteger(contextSpan.end_byte)
            || contextSpan.start_byte < 0
            || contextSpan.end_byte <= contextSpan.start_byte
            || contextSpan.end_byte > sourceBytes.length
            || sha256Hex(sourceBytes.subarray(
              contextSpan.start_byte, contextSpan.end_byte,
            )) !== contextSpan.text_sha256) {
          fail(closureCode, 'context span is not hash-verified against sealed M2 bytes');
        }
        string(contextSpan.node_occurrence_id, closureCode, 'context span node');
      }
    }
...
function validateCoverage(
  ...
  for (const closure of sources.closures.values()) {
    ...
    for (let index = 0; index < closure.spans.length; index += 1) {
      ...
      } else if (entry.treatment_kind === 'REVIEW_RESIDUE') {
        const reviewOnly = array(analysis.dispositions, code, 'dispositions').some(
          (disposition) => disposition.source_closure_id === closure.source_closure_id
            && disposition.output_disposition === 'REVIEW_ONLY',
        );
        const emittingRule = array(analysis.rules, code, 'rules').find(
          (rule) => rule.source_closure_id === closure.source_closure_id,
        );
        if (!reviewOnly
            || (emittingRule && !['REVIEW_ONLY'].includes(emittingRule.validation?.output_disposition))
            || entry.owner_id !== null
            || typeof entry.reason_code !== 'string'
            || entry.reason_code.length === 0
            || entry.authority_id !== null) {
          fail(code, `review residue ${span.span_id} is not confined to a review-only closure`);
        }
...
function validateAnalysisV2({
  ...
  const { profiles, trees } = validateProfileSnapshots(
    ...
    semanticInputs,
    governance.packageRegistry.familyKeys,
  );
...
function validateProjectionRows(projection, analysis, viewPolicy, policy) {
  ...
  for (const row of rows.values()) {
    ...
    for (const layout of array(row.layouts, 'M7_V2_LAYOUT_RECONCILIATION', 'row layouts')) {
      ...
      const requiredLevels = layoutPolicy.required_classification_levels;
      if (levels.length < requiredLevels.length
          || !same(levels.slice(0, requiredLevels.length), requiredLevels)) {
