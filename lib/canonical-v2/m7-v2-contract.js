...
// declared without the authority binding is still checked for shape, order and
// coverage — an unsorted or partial closure is a defect wherever it appears.
...
function validateCandidateReplacementMembers(registration, code) {
  ...
  const paths = closure.map((member) => {
    ...
    return string(member.path, code, 'candidate import closure member path');
  });
...
function validateCandidateRegistrationBytes(selectedBytes, governance) {
  ...
  validateCandidateReplacementMembers(registration, code);
...
function validateGovernance(governance, resolveBinding) {
  ...
  const { registration: candidate, familyContract: candidateFamilyContract } =
    validateCandidateRegistrationBytes(candidateBytes, governance);
