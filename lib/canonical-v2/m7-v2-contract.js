...
const OPERATORS = Object.freeze(new Map([
  ['ALL_OF', { min: 2, max: Infinity, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['ANY_OF', { min: 2, max: Infinity, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['NOT', { min: 1, max: 1, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['IF_THEN', { min: 2, max: 2, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['EXCEPTION_TO', { min: 2, max: 2, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['OVERRIDES', { min: 2, max: 2, childKinds: ['RULE', 'EXPRESSION'] }],
  ['DEEMS_AS', { min: 2, max: 2, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['EARLIER_OF', { min: 2, max: Infinity, childKinds: ['FACT', 'EXPRESSION'] }],
  ['LATER_OF', { min: 2, max: Infinity, childKinds: ['FACT', 'EXPRESSION'] }],
  ['TO_EXTENT', { min: 2, max: 2, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
  ['CONSEQUENCE_MODIFIER', { min: 2, max: 2, childKinds: ['FACT', 'RULE', 'EXPRESSION'] }],
]));
...
function validateCandidateRegistrationBytes(selectedBytes, governance) {
  ...
  for (const entry of registration.subtype_tree_bindings) {
    ...
    subtypeFamilies.push(entry.family_key);
    subtypePaths.push(entry.binding.path);
    exactKeys(entry.binding, BINDING_KEYS, code, 'candidate subtype-tree file binding');
    if (entry.binding.schema_version !== SUBTYPE_TREE_SCHEMA
        || entry.binding.record_id_field !== 'subtype_tree_id') {
      fail(code, 'candidate subtype-tree binding has the wrong schema');
    }
    assertHex(entry.binding.record_id, 64, code, 'candidate subtype-tree ID');
  }
  if (!same(subtypeFamilies, [...FAMILY_KEYS].sort())) {
    fail(code, 'candidate does not bind one ordered subtype tree for all 25 families');
  }
  unique(subtypePaths, code, 'candidate subtype-tree paths');
...
function validateGovernance(governance, resolveBinding) {
  ...
  return { candidate, inputByRole, inputRecords };
}

function validateFamilyPacketAuthority(familyPacketSet, governance, resolveBinding) {
  ...
  if (fixedMembers.length !== 50 || fixedByOrdinal.size !== 50
      ...
      || rulingFamilies.length !== FAMILY_KEYS.length
...
function validateSemanticInputConsumption(analysis, governance, resolveBinding) {
  ...
  for (const member of baseMembers) {
    exactKeys(member, ['agreement_id', 'governed_input_occurrence_ids'], code,
      'base analysis member');
    baseAgreementIds.push(string(member.agreement_id, code, 'base agreement ID'));
    const occurrenceIds = array(member.governed_input_occurrence_ids, code,
      'base governed occurrences');
    occurrenceIds.forEach((value) => string(value, code, 'base governed occurrence ID'));
    unique(occurrenceIds, code, 'base governed occurrences');
  }
  unique(baseAgreementIds, code, 'base agreement IDs');
  const baseMember = baseMembers.find((member) => member.agreement_id === analysis.agreement_id);
  if (!baseMember || !same([...baseMember.governed_input_occurrence_ids].sort(),
    [...analysis.governed_input_occurrence_ids].sort())) {
  ...
  for (const member of contextMembers) {
    exactKeys(member, ['agreement_id', 'edges'], code, 'context compilation member');
    contextAgreementIds.push(string(member.agreement_id, code, 'context agreement ID'));
    const edgeIds = [];
    for (const edge of array(member.edges, code, 'context edges')) {
      exactKeys(edge, [
        'edge_id', 'edge_type', 'target_id', 'state', 'source_support_ids',
      ], code, 'context edge');
      edgeIds.push(string(edge.edge_id, code, 'context edge ID'));
      string(edge.edge_type, code, 'context edge type');
      if (edge.target_id !== null) string(edge.target_id, code, 'context edge target');
      if (!['RESOLVED', 'UNRESOLVED', 'AMBIGUOUS'].includes(edge.state)) {
        fail(code, 'context edge state is invalid');
      }
      const supports = array(edge.source_support_ids, code, 'context edge supports');
      supports.forEach((value) => string(value, code, 'context edge support'));
      unique(supports, code, 'context edge supports');
    }
    unique(edgeIds, code, 'context edge IDs');
  }
  unique(contextAgreementIds, code, 'context agreement IDs');
  const contextMember = contextMembers.find((member) => member.agreement_id === analysis.agreement_id);
  if (!contextMember) fail(code, 'agreement is absent from the context compilation set');
  const contextEdges = indexBy(contextMember.edges, 'edge_id', code, 'context edge');
  ...
  const structureMembers = indexBy(array(structureSet.members, code,
    'structure disposition members'), 'structure_disposition_id', code,
  ...
  for (const member of structureMembers.values()) {
    ...
    if (member.kind === 'BEN_AUTHORED_INLINE_LIST_OVERLAY') {
      ...
      if (repeatBindings.length === 0) fail(code, 'overlay lacks an ambiguous-repeat negative');
      const repeatFixtures = repeatBindings.map((binding) => {
        exactKeys(binding, BINDING_KEYS, code, 'ambiguous-repeat fixture binding');
        if (binding.schema_version !== STRUCTURE_OVERLAY_FIXTURE_SCHEMA
            || binding.record_id_field !== 'fixture_id') {
          fail(code, 'overlay ambiguous-repeat fixture binding has another schema');
        }
        return validateResolvedRecordBinding(binding, resolveBinding,
    ...
    for (const field of ['inclusion_fixture_bindings', 'exclusion_fixture_bindings']) {
      ...
      for (const binding of bindings) {
        exactKeys(binding, BINDING_KEYS, code, field);
        if (binding.schema_version !== MATCH_FIXTURE_SCHEMA
            || binding.record_id_field !== 'match_fixture_id') {
          fail(code, `structure ${field} uses a non-V2 fixture`);
        }
        const fixture = validateResolvedRecordBinding(binding, resolveBinding,
...
function validateProfileSnapshots(analysis, candidate, resolveBinding, profileSet, semanticInputs) {
  const code = 'M7_V2_PROFILE_GATE';
  exactKeys(profileSet, [
    'schema_version', 'family_profile_set_id', 'state', 'profiles',
    'dimension_evidence_bindings', 'subtype_tree_bindings',
  ...
  const approvedProfiles = indexBy(array(profileSet.profiles, code, 'approved profiles'),
    'profile_id', code, 'approved profile');
  const approvedProfileKeys = indexBy(array(profileSet.profiles, code, 'approved profiles'),
    'profile_key', code, 'approved profile key');
  const dimensionEvidenceByProfile = new Map(
    [...approvedProfiles.keys()].map((profileId) => [profileId, []]),
  );
  const dimensionEvidencePaths = [];
  ...
  for (const evidenceBinding of array(
    ...
    exactKeys(evidenceBinding, BINDING_KEYS, code, 'dimension evidence binding');
    if (evidenceBinding.schema_version !== DIMENSION_EVIDENCE_SCHEMA
        || evidenceBinding.record_id_field !== 'dimension_evidence_id') {
      fail(code, 'dimension evidence binding schema is invalid');
    }
    dimensionEvidencePaths.push(evidenceBinding.path);
    dimensionEvidenceIds.push(evidenceBinding.record_id);
    const evidence = validateResolvedRecordBinding(
      evidenceBinding, resolveBinding, 'dimension evidence', code,
    );
    exactKeys(evidence, [
    ...
    if (!profile || !Array.isArray(profile.legal_authority_ids)
        ...
        || !profile.legal_authority_ids.includes(evidence.lawyer_ruling_id)
        || !semanticInputs.packetRulingIds.has(evidence.lawyer_ruling_id)) {
      fail(code, 'dimension evidence is outside its approved profile and ruling');
    }
    exactKeys(evidence.evidence_binding, BINDING_KEYS, code,
      'dimension source or fixture binding');
    if (evidence.evidence_binding.schema_version !== MATCH_FIXTURE_SCHEMA
        || evidence.evidence_binding.record_id_field !== 'match_fixture_id'
    ...
    const fixture = validateResolvedRecordBinding(
      evidence.evidence_binding, resolveBinding, 'dimension evidence fixture', code,
    );
    validateFixtureRecord(fixture, proof);
    dimensionEvidenceByProfile.get(evidence.profile_id).push({ evidence, fixture });
  }
  unique(dimensionEvidencePaths, code, 'dimension evidence paths');
  unique(dimensionEvidenceIds, code, 'dimension evidence IDs');
  if (!same(dimensionEvidencePaths, [...dimensionEvidencePaths].sort())) {
    fail(code, 'dimension evidence bindings are not in canonical path order');
  }
  ...
  for (const member of approvedProfiles.values()) {
    ...
    if (member.schema_version !== FAMILY_PROFILE_SCHEMA
        || !FAMILY_KEYS.includes(member.family_key)) {
    ...
    if (member.legal_authority_ids.length === 0
        || member.legal_authority_ids.some(
          (rulingId) => !semanticInputs.packetRulingIds.has(rulingId),
        )) fail(code, 'approved profile does not trace to exact family-packet rulings');
    ...
    for (const proof of array(member.fixture_proofs, code, 'profile fixture proofs')) {
      ...
      exactKeys(proof.fixture_binding, BINDING_KEYS, code, 'profile fixture binding');
      if (proof.fixture_binding.schema_version !== MATCH_FIXTURE_SCHEMA
          || proof.fixture_binding.record_id_field !== 'match_fixture_id') {
      ...
      if (proof.expected_selected_profile_key !== null) {
        ...
        if (!approvedProfileKeys.has(proof.expected_selected_profile_key)) {
  ...
  if (!same([...approvedFamilies].sort(), [...FAMILY_KEYS].sort())) {
  ...
  const orderedFamilies = [...FAMILY_KEYS].sort();
  ...
  for (let index = 0; index < ownedTreeBindings.length; index += 1) {
    ...
    exactKeys(entry.binding, BINDING_KEYS, code, 'profile-set subtype-tree record binding');
    if (entry.binding.schema_version !== SUBTYPE_TREE_SCHEMA
        || entry.binding.record_id_field !== 'subtype_tree_id') {
      fail(code, 'profile-set subtype-tree binding schema is invalid');
    }
    treePaths.push(entry.binding.path);
    treeIds.push(entry.binding.record_id);
    const tree = validateResolvedRecordBinding(
      entry.binding, resolveBinding, `profile-set subtype tree ${entry.family_key}`, code,
    );
    exactKeys(tree, [
      'schema_version',
      'subtype_tree_id',
    ...
    if (tree.schema_version !== SUBTYPE_TREE_SCHEMA
        || tree.subtype_tree_id !== entry.binding.record_id
    ...
    for (const node of nodes.values()) {
      ...
      const profile = approvedProfileKeys.get(node.profile_key);
    ...
    if (rootCount !== 1) fail(code, 'profile-set subtype tree must have exactly one root');
    treesByFamily.set(entry.family_key, tree);
  }
  unique(treePaths, code, 'profile-set subtype-tree paths');
  ...
  for (const profile of profiles.values()) {
    ...
    if (!FAMILY_KEYS.includes(profile.family_key)) fail(code, 'profile family is not approved');
    ...
    const validateFieldRequirement = (requirement, allowedCardinalities, label) => {
      ...
      if (!profile.legal_authority_ids.includes(requirement.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(requirement.lawyer_ruling_id)) {
        fail(code, `${label} lacks an exact family-packet ruling`);
      }
    ...
    for (const condition of array(
      ...
      if (!profile.legal_authority_ids.includes(condition.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(condition.lawyer_ruling_id)) {
        fail(code, 'conditional requirement lacks an exact family-packet ruling');
      }
    }
    ...
    for (const sourceType of allowedSourceTypes) {
      ...
      if (!profile.legal_authority_ids.includes(sourceType.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(sourceType.lawyer_ruling_id)) {
        fail(code, 'allowed source type lacks an exact family-packet ruling');
      }
    }
    ...
    for (const dependencyType of allowedDependencyTypes) {
      ...
      if (!profile.legal_authority_ids.includes(dependencyType.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(dependencyType.lawyer_ruling_id)) {
        fail(code, 'allowed dependency type lacks an exact family-packet ruling');
      }
    }
    ...
    for (const requirement of array(profile.child_rule_profiles, code,
      ...
      if (!profile.legal_authority_ids.includes(requirement.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(requirement.lawyer_ruling_id)) {
        fail(code, 'child rule requirement lacks an exact family-packet ruling');
      }
    }
    ...
    for (const slot of EQUIVALENCE_SIGNATURE_SLOTS) {
      ...
      if (fieldKeys.some((fieldKey) => !declaredOrDelegatedFieldKeys.includes(fieldKey))
          ...
          || !profile.legal_authority_ids.includes(mapping.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(mapping.lawyer_ruling_id)) {
    ...
    if (compatibleProfiles.some((profileId) => !approvedProfiles.has(profileId)
        ...
        || (profile.grouping_policy.allowed && (
          !profile.legal_authority_ids.includes(profile.grouping_policy.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(profile.grouping_policy.lawyer_ruling_id)
        ))
        || (!profile.grouping_policy.allowed && (
    ...
    for (const dimension of array(profile.known_relevant_dimensions, code,
      ...
      if (!['CALIBRATION', 'ADVERSARIAL'].includes(dimension.source)
          || !profile.legal_authority_ids.includes(dimension.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(dimension.lawyer_ruling_id)) {
    ...
    if (boundTree.schema_version !== SUBTYPE_TREE_SCHEMA
        || boundTree.subtype_tree_id !== profile.tree_binding.record_id
    ...
    if (rootCount !== 1) fail(code, 'profile subtype tree must have exactly one root');
    ...
    if (profile.no_comparison_policy !== null) {
      ...
      if (!profile.legal_authority_ids.includes(profile.no_comparison_policy.lawyer_ruling_id)
          || !semanticInputs.packetRulingIds.has(profile.no_comparison_policy.lawyer_ruling_id)) {
...
function validateCandidateSets(analysis) {
  ...
  for (const candidateSet of candidateSets.values()) {
    ...
    if (!same([...candidateSet.considered_family_keys].sort(), [...FAMILY_KEYS].sort())) {
...
function validateFixtureRecord(record, proof) {
...
function validateProfileFixtures(profiles, resolveBinding, semanticInputs) {
  ...
  for (const profile of profiles.values()) {
    for (const proof of profile.fixture_proofs) {
      let fixture = evaluatedBindings.get(proof.fixture_binding.path)?.fixture;
      let evaluated = evaluatedBindings.get(proof.fixture_binding.path)?.evaluated;
      const cachedBinding = evaluatedBindings.get(proof.fixture_binding.path)?.binding;
      if (cachedBinding && !same(cachedBinding, proof.fixture_binding)) {
        fail(code, 'one fixture path carries two different byte bindings');
      }
      if (!fixture) {
        fixture = validateResolvedRecordBinding(
          proof.fixture_binding, resolveBinding, `match fixture ${proof.fixture_id}`, code,
        );
        const context = validateFixtureRecord(fixture, proof);
        evaluated = evaluateApprovedProfiles(profiles, context);
        evaluatedBindings.set(proof.fixture_binding.path, {
      ...
      if (!semanticInputs.packetRulingIds.has(proof.lawyer_ruling_id)
          || !profile.legal_authority_ids.includes(proof.lawyer_ruling_id)) {
        fail(code, `fixture ${proof.fixture_id} lacks an exact family-packet ruling`);
      }
    }
  }
...
function validateAmbiguousRepeatOverlayFixture(
  ...
  if (fixture.schema_version !== STRUCTURE_OVERLAY_FIXTURE_SCHEMA
      ...
      || fixture.expected_selected_candidate_tree_id !== null
      || fixture.expected_output_disposition !== 'REVIEW_ONLY'
      || fixture.lawyer_ruling_id !== ITEM39_DECISION_ID) {
  ...
  if (fixture.agreement_index_binding.schema_version !== AGREEMENT_INDEX_SCHEMA
      || fixture.agreement_index_binding.record_id_field !== 'agreement_index_id') {
    fail(code, 'ambiguous-repeat fixture uses another structural evidence schema');
  }
...
function validateGenericOutputAuthority(authority, rule, profile, tree, candidate, governed) {
  const code = 'M7_V2_PROFILE_GATE';
  ...
  const genericFixturePaths = new Set();
  for (const [field, expectedMatch] of [
    ...
    for (const fixtureBinding of bindings) {
      exactKeys(fixtureBinding, BINDING_KEYS, code, `generic ${field} binding`);
      const proof = profile.fixture_proofs.find(
      ...
      if (!proof || proof.expected_match !== expectedMatch
          ...
          || genericFixturePaths.has(fixtureBinding.path)) {
        fail(code, `generic ${field} differs from evaluated profile/version/ruling fixtures`);
      }
      genericFixturePaths.add(fixtureBinding.path);
    }
  }
  ...
  for (const rule of rules.values()) {
    ...
    if (['NORMAL', 'APPROVED_LIMITED', 'NO_COMPARISON'].includes(output)) {
      ...
      validateGenericOutputAuthority(
        candidate.effect.generic_level_output_authority,
        rule, profile, trees.get(rule.profile_id), candidate, governed,
      );
    }
...
function validateAnalysisV2({ analysis, resolveBinding } = {}) {
...
function validateViewPolicy(viewPolicy) {
  ...
  for (const layout of layouts.values()) {
    ...
    array(layout.required_classification_levels, code, 'required classification levels');
    array(layout.required_field_keys, code, 'required layout fields');
    array(layout.permitted_omission_rule_ids, code, 'permitted omission rules');
...
function validateProjectionV2(options = {}) {
  ...
  const analysisDispositions = indexBy(analysis.dispositions, 'input_occurrence_id',
    'M7_V2_LAYOUT_RECONCILIATION', 'analysis disposition');
  const storedAnalysisValidation = ANALYSIS_VALIDATION_RESULTS.get(analysis);
  if (!storedAnalysisValidation) {
    fail('M7_V2_LAYOUT_RECONCILIATION',
  ...
  if (projection.view_policy_id !== viewPolicy.view_policy_id
      || !same(projection.view_policy_binding, analysis.governance.view_policy_binding)
      || projection.view_policy_binding.schema_version !== VIEW_POLICY_SCHEMA
      || projection.view_policy_binding.record_id_field !== 'view_policy_id'
      || projection.view_policy_binding.record_id !== viewPolicy.view_policy_id
      || projection.view_policy_binding.byte_length !== viewPolicyBytes.length
      || projection.view_policy_binding.sha256 !== sha256Hex(viewPolicyBytes)
      || projection.view_policy_binding.git_blob_oid !== gitBlobOid(viewPolicyBytes)) {
