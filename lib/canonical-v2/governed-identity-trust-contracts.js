...
const APPROVAL_ATTESTATION_REQUEST_SCHEMA = 'DEAL_IDENTITY_APPROVAL_ATTESTATION/V2';
const NAMESPACE_APPROVAL_REQUEST_SCHEMA = 'GOVERNED_IDENTITY_NAMESPACE_APPROVAL_REQUEST/V1';
const EXTERNAL_NAMESPACE_BINDING_SCHEMA = 'GOVERNED_IDENTITY_EXTERNAL_NAMESPACE_BINDING/V1';
const SERIALISABLE_CAS_PROOF_SCHEMA = 'GOVERNED_IDENTITY_SERIALISABLE_CAS_PROOF_PROPOSAL/V1';
const SIGNED_ALLOCATION_RECEIPT_SCHEMA = 'GOVERNED_IDENTITY_SIGNED_ALLOCATION_RECEIPT/V2';
const MAPPING_SET_SCHEMA = 'GOVERNED_IDENTITY_40_DEAL_41_OCCURRENCE_MAPPING_SET/V1';
const BEN_MAPPING_APPROVAL_REQUEST_SCHEMA = 'GOVERNED_IDENTITY_BEN_MAPPING_APPROVAL_REQUEST/V1';
const REVIEWED_BRIDGE_SCHEMA = 'GOVERNED_IDENTITY_REVIEWED_BRIDGE/V2';
const AUTHORITY_BUNDLE_SCHEMA = 'GOVERNED_IDENTITY_AUTHORITY_BUNDLE_PROPOSAL/V1';
const GOVERNED_DEAL_KEY_DOMAIN = 'GOVERNED_IDENTITY_DEAL_KEY/V1';
const MANIFEST_DOMAIN = 'GOVERNED_IDENTITY_MANIFEST/V1';
...
const NAMESPACE_RATIFICATION_BLOCKER = 'PENDING_BEN_RULING_IDENTITY_NAMESPACE_AND_KEY_DOMAIN';
...
const FROZEN_KEY_REGISTRY_ROLE_DOMAIN_GRANTS = Object.freeze([
  ...
  Object.freeze({
    ...
    permitted_action: 'SIGN_DEAL_IDENTITY_APPROVAL_ATTESTATION',
    permitted_domain: APPROVAL_SIGNATURE_DOMAIN,
    role: 'BEN_APPROVER',
  }),
  Object.freeze({
...
function validateNamespaceApprovalRequest(value) {
  exactKeys(value, [
    'approval_authority', 'approval_request_id', 'approval_signature', 'namespace_value', 'pending_ben_ruling_id',
    'request_nonce', 'schema_version', 'state',
  ], 'Namespace approval request');
  if (value.schema_version !== NAMESPACE_APPROVAL_REQUEST_SCHEMA || value.state !== 'REQUESTED_EXTERNAL_BEN_RATIFICATION_REQUIRED'
    || value.approval_authority !== 'NONE' || value.approval_signature !== null) {
    fail('IDENTITY_NAMESPACE_APPROVAL_REQUEST_INVALID', 'Namespace approval remains an unsigned external-ratification request.');
  }
  requireText(value.namespace_value, 'Namespace value'); requireDigest(value.pending_ben_ruling_id, 'Pending Ben ruling'); requireNonce(value.request_nonce, 'Namespace request nonce');
...
function buildNamespaceApprovalRequest({ namespace_value: namespaceValue, pending_ben_ruling_id: rulingId, request_nonce: nonce } = {}) {
  const body = {
    schema_version: NAMESPACE_APPROVAL_REQUEST_SCHEMA, state: 'REQUESTED_EXTERNAL_BEN_RATIFICATION_REQUIRED',
    approval_authority: 'NONE', approval_signature: null, namespace_value: namespaceValue,
    pending_ben_ruling_id: rulingId, request_nonce: nonce,
  };
...
function validateExternalNamespaceBinding(value, { namespace_approval_request: request } = {}) {
  ...
  if (value.schema_version !== EXTERNAL_NAMESPACE_BINDING_SCHEMA || value.state !== 'EXTERNALLY_RATIFIED_REFERENCE_ONLY_NOT_AUTHORITY'
...
function bindExternallyRatifiedNamespace({ namespace_approval_request: request, external_ruling_id: rulingId } = {}) {
  const body = {
    ...
    state: 'EXTERNALLY_RATIFIED_REFERENCE_ONLY_NOT_AUTHORITY',
...
function validateSerialisableCasProof(value) {
  ...
  ['mapping_seed_root_id', 'nonce_registry_predecessor_id', 'serialisable_isolation_root_id', 'slot_predecessor_id', 'transaction_id'].forEach((key) => requireDigest(value[key], key));
...
function validateApprovalAttestationSigningRequest(value) {
  exactKeys(value, [
    'allocation_receipt_id', 'approval_attestation_request_id', 'governed_deal_key', 'manifest_id',
    'schema_version', 'seed_digest', 'signature', 'signing_authority', 'state', 'transaction_id',
  ], 'Approval-attestation signing request');
  if (value.schema_version !== APPROVAL_ATTESTATION_REQUEST_SCHEMA || value.state !== 'UNSIGNED_PROPOSAL_ONLY'
    || value.signing_authority !== 'NONE' || value.signature !== null) {
    fail('IDENTITY_APPROVAL_ATTESTATION_REQUEST_INVALID', 'Approval attestation request must remain an unsigned, non-authorising proposal.');
  }
  ['allocation_receipt_id', 'governed_deal_key', 'manifest_id', 'seed_digest', 'transaction_id'].forEach((key) => requireDigest(value[key], `Approval attestation request ${key}`));
  const { approval_attestation_request_id: identifier, ...body } = value;
  if (identifier !== contentId(APPROVAL_ATTESTATION_REQUEST_SCHEMA, body)) fail('IDENTITY_APPROVAL_ATTESTATION_REQUEST_ID_INVALID', 'Approval attestation request identity is invalid.');
  return cloneFreeze(value);
}

function buildApprovalAttestationSigningRequest({ transaction_id: transactionId, seed_digest: seedDigest, governed_deal_key: governedDealKeyValue, manifest_id: manifest, allocation_receipt_id: allocationReceiptId } = {}) {
  const body = {
    schema_version: APPROVAL_ATTESTATION_REQUEST_SCHEMA,
    state: 'UNSIGNED_PROPOSAL_ONLY', signing_authority: 'NONE', signature: null,
    transaction_id: transactionId, seed_digest: seedDigest, governed_deal_key: governedDealKeyValue,
    manifest_id: manifest, allocation_receipt_id: allocationReceiptId,
  };
  return validateApprovalAttestationSigningRequest({ ...body, approval_attestation_request_id: contentId(APPROVAL_ATTESTATION_REQUEST_SCHEMA, body) });
}
...
function normaliseDealMappings(rows, binding) {
  ...
  const output = rows.map((row, index) => {
    ...
    ['allocation_receipt_id', 'governed_deal_key', 'manifest_id', 'seed_digest', 'transaction_id'].forEach((key) => requireDigest(row[key], `Deal mapping ${key}`));
...
function normaliseOccurrenceMappings(rows, transactionIds) {
  ...
  const output = rows.map((row, index) => {
    ...
    requireText(row.occurrence_id, 'Occurrence ID'); requireDigest(row.transaction_id, 'Occurrence transaction ID');
...
function validateBenMappingApprovalRequest(value, mappingSet, namespaceRequest) {
  ...
  exactKeys(value, [
    'approval_authority', 'approval_request_id', 'approval_signature', 'mapping_set_id', 'namespace_binding',
...
function validateAuthorityBundle(value) {
  exactKeys(value, [
    'allocation_receipts', 'approval_attestation_requests', 'authority_bundle_id', 'ben_mapping_approval_request', 'blocker_codes', 'bridge',
  ...
  if (value.schema_version !== AUTHORITY_BUNDLE_SCHEMA || value.state !== NOT_ISSUED
    || !Array.isArray(value.blocker_codes) || !value.blocker_codes.includes(NAMESPACE_RATIFICATION_BLOCKER)
    || !value.blocker_codes.includes(REGISTRY_AND_PERSISTENCE_BLOCKER)) {
    fail('IDENTITY_AUTHORITY_BUNDLE_INVALID', 'Authority bundle must remain blocked on namespace ratification, registry amendment, and persistence.');
  }
  ...
  if (!Array.isArray(value.cas_proofs) || !Array.isArray(value.allocation_receipts) || !Array.isArray(value.approval_attestation_requests)
    || value.cas_proofs.length !== REQUIRED_DEAL_COUNT || value.allocation_receipts.length !== REQUIRED_DEAL_COUNT
    || value.approval_attestation_requests.length !== REQUIRED_DEAL_COUNT) {
    fail('IDENTITY_AUTHORITY_BUNDLE_COHORT_INCOMPLETE', 'Authority bundle requires distinct complete sets of 40 CAS proposals, allocation receipts, and approval-attestation signing requests.');
  }
  ...
  const attestations = value.approval_attestation_requests.map(validateApprovalAttestationSigningRequest);
  for (const key of ['transaction_id', 'seed_digest', 'governed_deal_key', 'manifest_id', 'allocation_receipt_id']) {
    unique(attestations.map((attestation) => attestation[key]), 'IDENTITY_AUTHORITY_BUNDLE_DUPLICATE', `Approval attestation request ${key}`);
  }
  if (!same(attestations.map((attestation) => attestation.allocation_receipt_id).sort(), receipts.map((receipt) => receipt.allocation_receipt_id).sort())) {
    fail('IDENTITY_APPROVAL_ATTESTATION_SET_MISMATCH', 'Approval-attestation signing requests must cover the exact allocation receipt set.');
  }
  for (const attestation of attestations) {
    const receipt = receiptById.get(attestation.allocation_receipt_id);
    const mapping = value.mapping_set.deal_mappings.find((row) => row.allocation_receipt_id === attestation.allocation_receipt_id);
    if (!receipt || !mapping
      || attestation.transaction_id !== receipt.transaction_id || attestation.seed_digest !== receipt.seed_digest
      || attestation.governed_deal_key !== receipt.governed_deal_key || attestation.manifest_id !== receipt.manifest_id
      || attestation.transaction_id !== mapping.transaction_id || attestation.seed_digest !== mapping.seed_digest
      || attestation.governed_deal_key !== mapping.governed_deal_key || attestation.manifest_id !== mapping.manifest_id) {
      fail('IDENTITY_APPROVAL_ATTESTATION_EXPLICIT_JOIN_REQUIRED', 'Approval attestation requests must join their receipt and mapping by transaction, seed, governed key, manifest, and allocation receipt, never by position.');
    }
  }
...
function buildAuthorityBundle(input = {}) {
  const body = { schema_version: AUTHORITY_BUNDLE_SCHEMA, state: NOT_ISSUED, blocker_codes: Object.freeze([NAMESPACE_RATIFICATION_BLOCKER, REGISTRY_AND_PERSISTENCE_BLOCKER]), ...input };
...
module.exports = {
  APPROVAL_ATTESTATION_REQUEST_SCHEMA, APPROVAL_SIGNATURE_DOMAIN, AUTHORITY_BUNDLE_SCHEMA, BEN_APPROVER_KEY_ID, BEN_MAPPING_APPROVAL_REQUEST_SCHEMA, EXTERNAL_NAMESPACE_BINDING_SCHEMA,
  FROZEN_KEY_REGISTRY_AMENDMENT_SCHEMA, GOVERNED_DEAL_KEY_DOMAIN, MAPPING_SET_SCHEMA, MANIFEST_DOMAIN,
  NAMESPACE_APPROVAL_REQUEST_SCHEMA, NAMESPACE_RATIFICATION_BLOCKER, NOT_ISSUED, REGISTRY_AND_PERSISTENCE_BLOCKER,
  ...
  buildApprovalAttestationSigningRequest,
  ...
  requireRegistryAmendmentAndPersistence, validateAllocationReceipt, validateApprovalAttestationSigningRequest, validateAuthorityBundle,
