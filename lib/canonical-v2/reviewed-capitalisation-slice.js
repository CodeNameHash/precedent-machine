...
const REVIEWED_SECTIONS = Object.freeze({
  '3.3': Object.freeze({
    ...
    proposal_id: 'c902543faadec364b46ee15c7c5c5c13e1f253413f5921dad08ee0677fe5eb31',
  }),
  '6.3': Object.freeze({
    title: 'Conditions to Obligations of Parent and Merger Sub to Effect the Closing',
    proposal_id: '14e8690a00ede041d72bb1198b177cde466d236052f73b3a29d947c7d97ce752',
  }),
});
...
function buildReviewedCapitalisationSlice({ sourceText, contractBundle, corpusReleaseId } = {}) {
  ...
  const representationSubjects = [
    { label: 'Capitalization, Section 3.3(a)', section: '3.3(a)' },
    { label: 'Capitalization, Section 3.3(c), first sentence', section: '3.3(c)' },
  ];
  const knowledgeClaims = representationComponents.map((component, index) => {
    ...
    return buildClaimRevision({
      ...
      state: 'ABSENT',
      attributes: {
        representation_subject_label: representationSubjects[index].label,
        representation_section: representationSubjects[index].section,
      },
      allowed_attributes: ['representation_subject_label', 'representation_section'],
      scope: {
  ...
  const contextComponents = [
    ...
    ...knowledgeClaims.map((claim, index) => ({
      component: buildFixtureResultComponent({
        deal_admission_id: dealAdmissionId,
        result_key: 'TARGET_CAPITALISATION_BRING_DOWN',
        result_version: 1,
        concept_key: 'COND-B-REP',
        party: CONDITION_PARTY,
        value_slot_key: `KNOWLEDGE_QUALIFIER_${index + 1}`,
        ordinal: index + 2,
        claim,
        relationships: [],
  ...
  return Object.freeze({
    ...
    canonicalWriteSet,
    result,
    contextComponents: Object.freeze(contextComponents),
    projection,
  });
}
...
function buildReviewedCapitalisationServingRow({ slice, contractBundle } = {}) {
  ...
  const row = buildCanonicalResultServingRow({
    ...
    context_components: slice.contextComponents,
  });
