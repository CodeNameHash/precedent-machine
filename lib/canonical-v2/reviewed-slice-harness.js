 ...
 * explicitly via the switches above. See docs/handoffs/
...
function buildProvisions({ sourcesRegistry, spansRegistry, provisionsInput }) {
  const registry = freshRegistry();
  for (const entry of requireArray(provisionsInput, 'provisions')) {
    ...
    const provision = buildProvisionInstance({
      source, span, conceptKey: entry.conceptKey, party: entry.party, ordinal: entry.ordinal,
    });
...
function buildComponents({ sourcesRegistry, spansRegistry, provisionsRegistry, componentsInput }) {
  const registry = freshRegistry();
  for (const entry of requireArray(componentsInput, 'components')) {
    ...
    const component = buildProvisionComponent({
      source, parentProvision: provision, span, componentKey: entry.componentKey, ordinal: entry.ordinal,
    });
...
function buildClaims({ reviewVersion, componentsRegistry, excerptsRegistry, claimsInput }) {
  const registry = freshRegistry();
  for (const entry of requireArray(claimsInput, 'claims')) {
    ...
    if (kind === 'money_relative_to_deal_value') {
      ...
      requireExactKeys(entry.normalisation.denominator, DENOMINATOR_KEYS, 'claims[].normalisation.denominator');
...
function buildReviewedSlice(input) {
  ...
  const provisionsRegistry = buildProvisions({ sourcesRegistry, spansRegistry, provisionsInput: input.provisions });
  const componentsRegistry = buildComponents({
    sourcesRegistry, spansRegistry, provisionsRegistry, componentsInput: input.components,
  });
  const claimsRegistry = buildClaims({ reviewVersion, componentsRegistry, excerptsRegistry, claimsInput: input.claims });
