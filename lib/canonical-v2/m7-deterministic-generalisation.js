...
function governingChapeau(index, container, selected) {
  if (selected.node_kind === 'CHAPEAU') return null;
...
function authoredUnitsWithin(index, node, unitTest) {
  const sentences = descendants(index, node)
    ...
    .sort(compareNodes);
  if (sentences.length === 0) return [node];
  const onTopic = unitTest
    ? sentences.filter((child) => unitTest.test(sectionText(index, child)))
    : [];
  return onTopic.length > 0 ? onTopic : sentences;
}
...
function representativeLegalUnit(index, section, familyKey, matcher) {
  ...
  if (familyKey === 'REPRESENTATIONS') {
    const organisation = allCandidates.find((node) => ORGANISATION_EXISTENCE.test(sectionText(index, node)));
    if (organisation) return { node: organisation, mode: SELECTION_MODES.AUTHORED_UNIT_MATCH };
  }
...
function selectFamilySections(index, familyOrder) {
  ...
  return familyOrder.map((family) => {
    ...
    if (sourceNode && sourceNode.node_kind === 'CHAPEAU') {
      // A disclosure chapeau states a carve-out, never a rule. It cannot stand
      // as a complete comparison proposition on its own.
    ...
    return {
      ...
      authored_units: sourceNode
        ? authoredUnitsWithin(
          index,
          sourceNode,
          family.family_key === 'REPRESENTATIONS' ? ORGANISATION_EXISTENCE : matcher,
        ).map((unit) => ({
