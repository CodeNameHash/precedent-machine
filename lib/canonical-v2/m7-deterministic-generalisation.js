...
const FAMILY_MATCHERS = Object.freeze([
  ...
  ['REPRESENTATIONS', /\b(?:authority; binding|authorization; valid and binding)\b/i],
...
function representativeLegalUnit(index, section, familyKey, matcher) {
  ...
  if (allCandidates.length === 0) return section;
  ...
  const matchingCandidates = matcher
    ...
    : [];
  const candidates = matchingCandidates.length > 0 ? matchingCandidates : allCandidates;
...
function selectFamilySections(index, familyOrder) {
  ...
  return familyOrder.map((family) => {
    ...
    const sourceNode = match ? representativeLegalUnit(index, match.node, family.family_key, matcher) : null;
    return {
