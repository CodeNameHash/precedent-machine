...
function governedContract(contractBundle) {
  const contract = contractBundle || FIXTURE_CONTRACT;
  validateContractBundle(contract);
  return contract;
}

function buildProvisionInstance({
  source,
  span,
  conceptKey,
  party,
  ...
  const conceptKeys = new Set(
    governedContract(contractBundle).concepts.map((entry) => entry.concept_key),
  );
  if (!conceptKeys.has(conceptKey)) {
    throw new Error(`conceptKey is outside the frozen fixture contract governed by this bundle: ${conceptKey}`);
  }
  assertParty(party);
...
function buildStructuralProvisionInstance({
  source,
  span,
  conceptKey,
  ordinal,
  ...
  return Object.freeze({
    ...payload,
    source_anchor_id: span.semantic_span_id,
    provision_instance_id: contentId('STRUCTURAL_PROVISION_INSTANCE/V1', payload),
  });
}

function buildProvisionComponent({
  source,
  parentProvision,
  span,
  componentKey,
  ...
  const componentKeys = new Set(
    governedContract(contractBundle).component_definitions.map((entry) => entry.component_key),
  );
  if (!componentKeys.has(componentKey)) {
    throw new Error(`componentKey is outside the frozen fixture contract governed by this bundle: ${componentKey}`);
  }
  assertOrdinal(ordinal);
  const payload = {
