...
function compileMarketCohortRequest(request) {
  ...
  // SPEC-VERSIONED-CONTRACT-2026-07-23: accept any currently-supported fixture
  // contract version (F1 default, or F2 once a reviewed artifact compiles
  // under it) rather than only the module-load-time default, so a versioned
  // candidate keeps building without loosening this to "any fingerprint".
  if (!FIXTURE_SERVING_CONTRACT_FINGERPRINTS.includes(request.contract_fingerprint)) {
    throw new MarketCohortQueryError('INVALID_REQUEST', 'contract_fingerprint does not match a frozen serving contract.');
  }
  if (isRejectedServingContractFingerprint(request.contract_fingerprint)) {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'The rejected contract cannot be used for market serving.',
    );
  }
  const contractBundle = fixtureContractForFingerprint(request.contract_fingerprint);
  if (!contractBundle.concepts.some((entry) => entry.concept_key === metric.concept_key)
    || !contractBundle.claim_definitions.some(
      (entry) => entry.claim_definition_key === metric.required_claim_definition_key,
    )) {
    throw new MarketCohortQueryError('INVALID_REQUEST', 'metric is outside the supplied frozen contract version.');
  }
  if (metric.metric_key === 'BUYER_TERMINATION_FEE_PERCENT_OF_DEAL_VALUE'
    && !contractBundle.serving_metric_operation_bindings?.some((binding) => (
      binding.metric_key === metric.metric_key
      && binding.metric_version === metric.metric_version
      && binding.concept_key === metric.concept_key
      && binding.required_claim_definition_key === metric.required_claim_definition_key
      && binding.legal_operation === 'CREATES_BUYER_TERMINATION_FEE_PAYMENT_TRIGGER'
      && binding.trigger_path_schema_key === 'TERMINATION_FEE_TRIGGER_PATH'
      && binding.trigger_path_schema_version === 2
    ))) {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'metric is not bound to its governed legal operation in the supplied frozen contract version.',
    );
  }
  const compiled = {
    ...
    basis_key: metric.basis_key || metric.canonical_unit,
...
function compileOfflineInterpretedMarketCohortRequest(request) {
  if (!request || request.authority_scope !== 'OFFLINE_CANDIDATE_ONLY') {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline interpreted cohort compilation requires explicit offline authority.',
    );
  }
  const allowedKeys = [
    'authority_scope',
  ...
  if (unknown.length) {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      `Unsupported offline request fields: ${unknown.sort().join(', ')}.`,
    );
  }
  const metric = METRIC_DEFINITIONS[request.metric_key];
  const {
  ...
  if (!metric
    || request.metric_version !== metric.metric_version
    || request.concept_key !== metric.concept_key
    || request.metric_key !== 'NO_SHOP_COPY_DELIVERY_PERIOD_DAYS') {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline metric, version and concept do not match the governed copy-delivery contract.',
    );
  }
  if (request.integration_admission_id
      !== QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID) {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline cohort lacks the pinned F19 integration admission.',
    );
  }
  const contract = fixtureContractForFingerprint(request.contract_fingerprint);
  if (!contract
    || FIXTURE_SERVING_CONTRACT_FINGERPRINTS.includes(request.contract_fingerprint)
    || contract.claim_interpretation_policy_definition?.schema_version
      !== 'CLAIM_INTERPRETATION_POLICY/V2') {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline interpreted cohorts require a frozen non-serving V12 contract.',
    );
  }
  const compiled = {
    schema_version: 'MARKET_COHORT_REQUEST/V2',
    authority_scope: 'OFFLINE_CANDIDATE_ONLY',
    integration_admission_id:
      QXO_COPY_DELIVERY_F19_INTEGRATION_ADMISSION_ID,
    serving_namespace_id: requireDigest(
      request.serving_namespace_id,
      'serving_namespace_id',
    ),
    corpus_release_id: requireDigest(
      request.corpus_release_id,
      'corpus_release_id',
    ),
    contract_fingerprint: requireDigest(
    ...
    concept_key: metric.concept_key,
    party: normaliseParty(request.party),
    subject_deal_key: optionalText(
      request.subject_deal_key,
      'subject_deal_key',
    ),
    basis_key: metric.basis_key,
    comparability_class_digest: requireDigest(
      request.comparability_class_digest,
      'comparability_class_digest',
    ),
    filters: normaliseFilters(request.filters),
  };
  const { serving_namespace_id: _namespace, ...logicalCohort } = compiled;
  const cohortDigest = contentId('MARKET_COHORT/V2', logicalCohort);
  return Object.freeze({
    ...compiled,
    cohort_digest: cohortDigest,
    cache_key: contentId('MARKET_COHORT_CACHE/V2', {
      serving_namespace_id: compiled.serving_namespace_id,
      corpus_release_id: compiled.corpus_release_id,
      cohort_digest: cohortDigest,
      comparability_class_digest:
...
function validateOfflineInterpretedMarketCohortRequest(request) {
  if (!request || request.schema_version !== 'MARKET_COHORT_REQUEST/V2') {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline cohort request must use V2.',
    );
  }
  const base = {
  ...
  const expected = compileOfflineInterpretedMarketCohortRequest(base);
  if (JSON.stringify(request) !== JSON.stringify(expected)) {
    throw new MarketCohortQueryError(
      'INVALID_REQUEST',
      'offline cohort request identity has drifted.',
    );
  }
  return true;
}

function validateOfflineInterpretedMarketCohortResult(result, request) {
  const compiled = request.schema_version === 'MARKET_COHORT_REQUEST/V2'
    ? request
    : compileOfflineInterpretedMarketCohortRequest(request);
  ...
  requireExactResponseKeys(result, [
    ...
    'distribution',
    'exclusions',
  ], 'offline market cohort response');
  if (!result || result.schema_version !== 'MARKET_COHORT_RESULT/V2'
    || result.authority_scope !== 'OFFLINE_CANDIDATE_ONLY'
    || result.integration_admission_id
      !== compiled.integration_admission_id
    || result.serving_namespace_id !== compiled.serving_namespace_id
    || result.corpus_release_id !== compiled.corpus_release_id
    || result.contract_fingerprint !== compiled.contract_fingerprint
    || result.cohort_digest !== compiled.cohort_digest
    || result.comparability_class_digest
      !== compiled.comparability_class_digest
    || result.metric_key !== compiled.metric_key
    || result.metric_version !== compiled.metric_version
    || result.concept_key !== compiled.concept_key
    || result.subject_deal_key !== compiled.subject_deal_key
    || result.execution_state !== 'OFFLINE_FIXTURE_ONLY'
    || !result.counts
    || !Array.isArray(result.distribution)
    || !Array.isArray(result.exclusions)) {
    throw new MarketCohortQueryError(
      'INVALID_RESPONSE',
      'offline interpreted cohort result does not match its class-keyed request.',
    );
  }
  requireExactResponseKeys(result.counts, [
    'eligible_deals',
    'applicable_deals',
    'examined_deals',
    'present_deals',
  ...
  Object.entries(result.counts).forEach(([key, value]) => {
    nonNegativeInteger(value, `offline counts.${key}`);
  });
  result.distribution.forEach((row, index) => {
    requireExactResponseKeys(
      row,
      ['canonical_value', 'subject_count', 'deal_count'],
      `offline distribution[${index}]`,
    );
  });
  return result;
}
