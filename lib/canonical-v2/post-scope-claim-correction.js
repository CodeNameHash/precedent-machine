...
} = require('./correction-value-evidence');
...
function applyApprovedPostScopeClaimCorrection({ writeSet, correction, approval, contractBundle } = {}) {
  ...
  let successor;
  try {
    const successorInput = claimInput(predecessor, selectedCorrection.patch);
    const valueEvidenceProjection = deriveCorrectionValueEvidenceProjection({
      writeSet: inputWriteSet,
      correction: selectedCorrection,
      predecessor,
      successor: successorInput,
      contractBundle,
    });
    if (valueEvidenceProjection) {
      successorInput.attributes = clone(valueEvidenceProjection.expected.projection.normalisation_attributes);
      successorInput.allowed_attributes = Object.keys(successorInput.attributes);
    }
    const built = buildClaimRevision(successorInput);
    ...
    successor = {
      ...
      ...(predecessor.closure_id === undefined ? {} : { closure_id: predecessor.closure_id }),
    };
  } catch (error) {
    if (error instanceof CorrectionValueEvidenceError) throw error;
  ...
  const valueEvidenceProof = buildCorrectionValueEvidenceProof({
    writeSet: inputWriteSet,
    correction: selectedCorrection,
    application,
    predecessor,
  ...
  return deepFreeze({
    ...
    application,
    lineage,
    value_evidence_proof: valueEvidenceProof,
