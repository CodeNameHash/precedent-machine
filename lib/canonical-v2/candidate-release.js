...
function releaseWideMarketContext({
  ...
  if (subjectMembership?.status === 'EXCLUDED') {
    if (subjectMembership.cohort_digest !== compiled.cohort_digest) {
      throw new TypeError('release-wide cohort cannot carry a stale subject exclusion receipt');
    }
    let expectedExcludedReceipt;
    try {
      expectedExcludedReceipt = buildSubjectCohortMembershipReceipt({
        ...
        status: 'EXCLUDED',
        exclusion_reason: subjectMembership.exclusion_reason,
      });
    } catch {
      throw new TypeError(
        'release-wide subject exclusion reason is invalid or has no selected narrower filter',
      );
    }
    if (canonicalJson(subjectMembership) !== canonicalJson(expectedExcludedReceipt)) {
      throw new TypeError('release-wide subject exclusion receipt identity mismatch');
    }
  }
  const cohort = [
  ...
  const subjectIsIncluded = comparable.some((item) => (
    item.terminal.deal_key === row.governed_deal_key
  ));
  const releaseSubjectMembership = subjectIsIncluded
    ? buildSubjectCohortMembershipReceipt({
      cohort_request: cohortRequest,
      observation: {
        deal_key: row.governed_deal_key,
