...
function validateObservationMember({
  ...
  const compositionDetail = member.exact_detail.package?.row?.source_actions?.[0]?.action_slot_key
    === COMPOSITION_ACTION_SLOT_KEY;
  requireExactKeys(member.exact_detail, compositionDetail ? [
    'package',
    'source',
    'source_admission',
    'components',
...
function materialiseCandidateCorrectionInputs({
  ...
  if (!Array.isArray(expectedActiveApplicationIds)) {
    throw new TypeError('expected_active_correction_application_ids is mandatory, including when empty');
  }
  if (!Array.isArray(correctionMaterialisations)) {
    throw new TypeError('correction_materialisations is mandatory, including when empty');
  }
  const sealedMaterialisations = correctionMaterialisations.map((materialisation, index) => {
    requireExactKeys(materialisation, ['correction_output'], `correction_materialisations[${index}]`);
    ...
    const discharge = buildCorrectionDischarge({
      ...
      consistency_output_refs: correctionConsistencyRefs({
        ...
        detailPackage: member.exact_detail.package,
        successorClaimRevisionId: successor.claim_revision_id,
      }),
    });
    return { correction_output: output, correction_discharge: discharge };
  }).sort((left, right) => (
  ...
  const seal = buildCandidateCorrectionInputSeal({
    ...
    correction_materialisations: sealedMaterialisations,
  });
...
function validateReleaseCorrectionInputs({
  ...
  for (const entry of seal.correction_entries) {
    ...
    if (canonicalJson(discharge.ordered_consistency_output_refs) !== canonicalJson(expectedRefs)) {
      throw new TypeError('candidate correction discharge does not bind the corrected result, row and exact detail');
    }
  }
  return true;
}
...
function buildFixtureCandidateRelease({
  ...
  const candidateCorrectionInputs = materialiseCandidateCorrectionInputs({
    contractBundle,
    expectedActiveApplicationIds: expectedActiveCorrectionApplicationIds,
    correctionMaterialisations,
    releaseMembers,
    queryRecords,
  });
