...
function selectMostSpecificProfile(profiles, profileResults) {
  ...
  if (mostSpecific.length !== 1) {
    fail(`expected one unique most-specific approved profile match, received ${mostSpecific.length}`);
  }
...
function generateAnalysisV2({ baseAnalysis, agreementIndex, contextCompilation,
  ...
  for (const claim of baseAnalysis.claims) {
    if (!claim || typeof claim !== 'object' || Array.isArray(claim)
        ...
        || !Array.isArray(claim.source_node_occurrence_ids)
        || claim.source_node_occurrence_ids.length !== 1
        || typeof claim.source_node_occurrence_ids[0] !== 'string'
        || claim.source_node_occurrence_ids[0].length === 0
        || nodeIds.has(claim.source_node_occurrence_ids[0])) {
      fail('each governed claim must have a unique ID and one unique M2 node');
    }
    occurrenceIds.add(claim.claim_occurrence_id);
    nodeIds.add(claim.source_node_occurrence_ids[0]);
  }
  ...
  const analysisBody = {
    ...
    counts: {
      ...
      source_closures: occurrenceCount,
