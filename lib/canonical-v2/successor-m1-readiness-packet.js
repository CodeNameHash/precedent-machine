...
const { PROPOSAL_SCHEMA: TRUSTED_KEY_DOMAIN_EXTENSION_SCHEMA } = require('./deal-identity-trusted-key-registry-proposal');
const {
...
const {
  CURRENT_LIVE_P9_GATE_IDS: ACCEPTANCE_DEFINITION_P9_GATE_IDS,
  GOVERNING_INVENTORY_CONFLICT,
  compileP9AcceptanceDefinitionAuthority,
  validateP9AcceptanceDefinitionAuthority,
} = require('../programme-gates/p9-acceptance-definition-authority');
const {
  compileP9AcceptanceEvidenceInventory,
  validateP9AcceptanceEvidenceInventory,
} = require('../programme-gates/p9-acceptance-evidence-inventory');
const {
  buildP9DefinitionProposalLayer,
  proposalBinding: p9DefinitionProposalBinding,
  validateP9DefinitionProposalLayer,
} = require('../programme-gates/p9-definition-proposal-layer');
const {
  COMPLETION_GATE_ID,
...
function currentP9Binding(repositoryRoot) {
  const registry = createGoverningRegistryAuthority();
  if (canonicalJson(ACCEPTANCE_DEFINITION_P9_GATE_IDS) !== canonicalJson(GOVERNING_REGISTRY_P9_GATE_IDS)) {
    fail('P9_GOVERNING_REGISTRY_BINDING_CONFLICT', 'The P9 acceptance proposal and the v2 governing registry disagree on live gate identifiers.');
  }
  const authority = compileP9AcceptanceDefinitionAuthority({ p9GateIds: GOVERNING_REGISTRY_P9_GATE_IDS });
  validateP9AcceptanceDefinitionAuthority(authority, { p9GateIds: GOVERNING_REGISTRY_P9_GATE_IDS });
  const inventory = compileP9AcceptanceEvidenceInventory({ p9GateIds: GOVERNING_REGISTRY_P9_GATE_IDS, repositoryRoot });
  validateP9AcceptanceEvidenceInventory(inventory, { p9GateIds: GOVERNING_REGISTRY_P9_GATE_IDS, repositoryRoot });
  const definitionLayer = buildP9DefinitionProposalLayer({ repository_root: repositoryRoot });
  validateP9DefinitionProposalLayer(definitionLayer, { repository_root: repositoryRoot });
  const definitionBinding = p9DefinitionProposalBinding(definitionLayer);
  if (canonicalJson(inventory.definition_proposal_binding) !== canonicalJson(definitionBinding)) {
    fail('P9_DEFINITION_PROPOSAL_BINDING_CONFLICT', 'The P9 evidence inventory and definition proposal layer disagree.');
  }
  const databaseSoak = inventory.leaves.find((leaf) => leaf.gate_id === 'P9_DATABASE_SOAK');
  return Object.freeze({
    authority_id: authority.authority_id,
    inventory_id: inventory.inventory_id,
    live_gate_count: authority.live_p9_gate_ids.length,
    recovered_candidate_gate_count: authority.recovered_candidate_p9_gate_ids.length,
    ...
    registry_live_gate_count: registry.live_p9_gate_ids.length,
    phase_12_security_gate_id: registry.phase_12_security_gate_id,
    completion_gate_id: registry.completion_gate_id,
    completion_gate_live: registry.completion_gate_live,
    evidence_map_state: 'CORRECTED_CURRENT_CATALOGUE_REFERENCES_PROPOSAL_ONLY',
    database_soak_scenario_ids: Object.freeze(databaseSoak.hostile_test_references.map((reference) => reference.test_id)),
    acceptance_definition_and_predicate_state: 'UNADOPTED_NO_EXECUTABLE_PREDICATES',
    definition_proposal_layer_id: definitionBinding.definition_proposal_layer_id,
    definition_proposal_source_sha256: definitionBinding.source_module_sha256,
    definition_live_record_count: definitionBinding.live_record_count,
    definition_completion_candidate_count: definitionBinding.completion_candidate_count,
    definition_threshold_blocker_gate_ids: definitionBinding.threshold_blocker_gate_ids,
    definition_formal_definition_authority: definitionBinding.formal_definition_authority,
    definition_predicate_authority: definitionBinding.predicate_authority,
    definition_pass_authority: definitionBinding.pass_authority,
  });
}
...
function currentFamilyBinding(decisionReconciliation) {
  ...
  if (CURRENT_M3_FAMILY_PARITY_REGISTER.families.length !== 21) {
    fail('FAMILY_REGISTER_INCOMPLETE', 'The successor packet requires all 21 registered families.');
  }
  const conditional = bindDecisionConditionalFamilyStatus(status, decisionReconciliation);
  return Object.freeze({
    register_id: contentId('CANONICAL_V2_M3_FAMILY_PARITY_REGISTER/V2', CURRENT_M3_FAMILY_PARITY_REGISTER),
...
function fixedBlockers(policyBindings, v1TrustedCapture, governedIdentity, sourceIntakeAuthority, sourceExceptionApproval, decisionReconciliation) {
  const entries = [
    ['PENDING_BEN_RULING_P9_COMPLETION_LEAF_RATIFICATION', 'Ben must ratify the missing P9 completion leaf.'],
    ...
    ['DECISION_RECONCILIATION_REQUIRED', `Decision-register freeze is blocked by missing rulings ${decisionReconciliation.missing_ruling_ids.join(', ') || 'none'}, implementation gaps ${decisionReconciliation.implementation_gap_ruling_ids.join(', ') || 'none'} and authority ${decisionReconciliation.decision_register_freeze_authority}.`],
...
function buildCurrentSuccessorM1ReadinessPacket({ repository_root: repositoryRoot = REPOSITORY_ROOT } = {}) {
  ...
  const decisionReconciliationProposal = compileDecisionReconciliationProposal({ repository_root: root });
  validateDecisionReconciliationProposal(decisionReconciliationProposal, { repository_root: root });
  ...
  const p9 = currentP9Binding(root);
  ...
  const body = {
    ...
    v2_identity_binding: Object.freeze({
      ...
      approval_signature_domain: APPROVAL_SIGNATURE_DOMAIN,
      trusted_key_domain_extension_schema: TRUSTED_KEY_DOMAIN_EXTENSION_SCHEMA,
      authority_status: 'PROPOSED_PENDING_BEN_RULING_AND_REAL_VERIFIER_NOT_ACTIVE',
    }),
