...
function indexAgreement(exactSourceInput, structuralPolicyInput) {
  ...
  const internalTempIds = new Set(internalNodes.map((node) => node.temp_id));
  for (const node of internalNodes) {
    if (node.parent_temp_id === null || internalTempIds.has(node.parent_temp_id)) continue;
    const priorParentTempId = node.parent_temp_id ?? null;
    const repairedParent = internalNodes
      .filter((candidate) => candidate.temp_id !== node.temp_id
        && candidate.start <= node.start
        && candidate.end >= node.end
        && (candidate.start < node.start || candidate.end > node.end))
      .sort((left, right) => (left.end - left.start) - (right.end - right.start)
        || right.start - left.start
        || left.temp_id.localeCompare(right.temp_id))[0];
    if (!repairedParent) fail('INVALID_PARENT', `${node.node_kind}:${node.start}:${node.end}`);
    node.parent_temp_id = repairedParent.temp_id;
    addDiagnostic(
      'PARENT_REPAIRED',
      span(node.start, node.end),
      {
        node_kind: node.node_kind,
        prior_parent_temp_id: priorParentTempId,
        repaired_parent_temp_id: repairedParent.temp_id,
        reason: 'MISSING_INTERNAL_PARENT',
      },
      'INFO',
    );
  }
  ...
  for (const [ordinal, node] of internalNodes.entries()) {
    const key = `${node.node_kind}\0${node.start}`;
    if (!collisionGroups.has(key)) collisionGroups.set(key, []);
    collisionGroups.get(key).push({ node, ordinal });
  }
  const collisionRedirects = new Map();
  for (const group of collisionGroups.values()) {
    if (group.length < 2) continue;
    const ordered = [...group].sort((left, right) => right.node.end - left.node.end
      || left.ordinal - right.ordinal);
    const retained = ordered[0].node;
    for (const entry of ordered.slice(1)) {
      collisionRedirects.set(entry.node.temp_id, retained.temp_id);
      addDiagnostic(
        'NODE_COLLISION_DROPPED',
        span(entry.node.start, entry.node.end),
        {
          node_kind: entry.node.node_kind,
          retained_temp_id: retained.temp_id,
          dropped_temp_id: entry.node.temp_id,
          reason: 'DUPLICATE_NODE_KIND_AND_START_BYTE',
        },
        'REVIEW',
      );
    }
  }
  const resolveCollisionTempId = (tempId) => {
    let resolved = tempId;
    const seen = new Set();
    while (collisionRedirects.has(resolved)) {
      if (seen.has(resolved)) fail('NODE_COLLISION', resolved);
      seen.add(resolved);
      resolved = collisionRedirects.get(resolved);
    }
    return resolved;
  };
  if (collisionRedirects.size > 0) {
    for (const node of internalNodes) {
      if (node.parent_temp_id !== null) {
        node.parent_temp_id = resolveCollisionTempId(node.parent_temp_id);
      }
    }
    for (const annotation of annotations) {
      if (annotation.owner_temp_id) annotation.owner_temp_id = resolveCollisionTempId(annotation.owner_temp_id);
    }
    for (const draft of inlineMarkerDispositionDrafts) {
      draft.parent_temp_id = resolveCollisionTempId(draft.parent_temp_id);
      draft.produced_temp_ids = draft.produced_temp_ids.map(resolveCollisionTempId);
    }
    for (const draft of inlineMarkerCandidateDrafts) {
      draft.owner_temp_id = resolveCollisionTempId(draft.owner_temp_id);
    }
    for (const draft of inlineAmbiguityDrafts) {
      draft.parent_temp_id = resolveCollisionTempId(draft.parent_temp_id);
    }
    for (let index = 0; index < inlineScannedTempIds.length; index += 1) {
      inlineScannedTempIds[index] = resolveCollisionTempId(inlineScannedTempIds[index]);
    }
    const retainedNodes = internalNodes.filter((node) => !collisionRedirects.has(node.temp_id));
    internalNodes.splice(0, internalNodes.length, ...retainedNodes);
  }
  ...
  for (const node of internalNodes) {
    ...
    if (seenOccurrenceIds.has(node.node_occurrence_id)) fail('NODE_COLLISION', node.node_occurrence_id);
  ...
  const publicNodes = internalNodes
    ...
    .map((node) => {
      ...
      const undefinedRevisionField = Object.entries(revisionPayload)
        .find(([, value]) => value === undefined || (Array.isArray(value) && value.includes(undefined)));
      if (undefinedRevisionField) {
        throw new TypeError(`undefined source node revision field ${undefinedRevisionField[0]} for ${node.node_kind} at ${node.start}:${node.end}`);
      }
      const structureRevisionId = contentId('AGREEMENT_SOURCE_NODE_REVISION/V1', revisionPayload);
