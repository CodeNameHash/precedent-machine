...
function assembleCanonicalContractBundleFreezeCandidate(input = {}) {
  ...
  const generatedProjection =
    clone(generatedTopology.generated_contract_bundle_projection);
  const completeProjection = [...aggregateProjection, ...generatedProjection]
    .sort((left, right) => compareStrings(left.member_key, right.member_key));
  const completeMemberRoot = contentId(
    'CANONICAL_CONTRACT_BUNDLE_MEMBER_ROOT/V3',
    {
      aggregate_members: firstCompilation.canonical_contract_bundle_members,
      generated_members: generatedTopology.generated_contract_bundle_members,
    },
  );
  ...
  const unsignedReceiptPayload = {
    ...
    canonical_contract_bundle_member_count:
      firstCompilation.canonical_contract_bundle_members.length
        + generatedTopology.generated_contract_bundle_members.length,
  ...
  const candidatePayload = {
    ...
    generated_contract_bundle_members:
      clone(generatedTopology.generated_contract_bundle_members),
    aggregate_canonical_contract_bundle_projection: aggregateProjection,
