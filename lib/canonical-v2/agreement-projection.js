...
// Byte containment against the proposition's own authored extents. Offsets are
// UTF-8 throughout, so this is numeric and never a string index.
...
function actorTexts(proposition) {
  ...
  // Only a DIRECT entry whose span lies inside this proposition's own authored
  // source may be displayed. The unprovenanced fallback that used to sit here
  // showed synthetic obligated_party and control_party values carrying no span
  // at all, so nothing tied them to the unit the row states.
