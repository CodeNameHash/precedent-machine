...
function actorTexts(proposition) {
  ...
  const directActors = stableText(proposition.roles.ACTOR_OR_PARTIES
    .filter((value) => value && typeof value === 'object' && value.provenance_kind === 'DIRECT'));
  if (directActors.length > 0) return directActors.slice(0, 1);
  return stableText(proposition.roles.ACTOR_OR_PARTIES
    .filter((value) => typeof value === 'string'
      || (value && typeof value === 'object' && !value.provenance_kind))).slice(0, 1);
}
