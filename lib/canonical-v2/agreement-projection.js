...
const PROJECTION_EXECUTION_LANES = Object.freeze({
  M6_V1_HISTORICAL_REPLAY: 'M6_V1_HISTORICAL_REPLAY',
  M7_V1_HISTORICAL_REPLAY: 'M7_V1_HISTORICAL_REPLAY',
  M7_V2_REPAIR_REGISTERED_CANDIDATE: 'M7_V2_REPAIR_REGISTERED_CANDIDATE',
});
...
const V2_FORMATTERS = Object.freeze({
...
function buildV2ViewPolicy(profileSnapshots) {
  if (!Array.isArray(profileSnapshots) || profileSnapshots.length === 0) {
    fail('M7_V2_VIEW_POLICY', 'approved profile snapshots are required');
  }
  const fieldKeys = new Set();
  let groupingAllowed = false;
  for (const profile of profileSnapshots) {
    if (!profile || typeof profile !== 'object'
        || !Array.isArray(profile.required_fields)
        || !Array.isArray(profile.optional_fields)
        || !Array.isArray(profile.excluded_or_delegated_dimensions)
        || typeof profile.grouping_policy?.allowed !== 'boolean') {
      fail('M7_V2_VIEW_POLICY', 'approved profile snapshot is malformed');
    }
    for (const requirement of [...profile.required_fields, ...profile.optional_fields]) {
      if (typeof requirement?.field_key !== 'string' || requirement.field_key.length === 0) {
        fail('M7_V2_VIEW_POLICY', 'approved profile field is malformed');
      }
      fieldKeys.add(requirement.field_key);
    }
    for (const dimension of profile.excluded_or_delegated_dimensions) {
      if (dimension?.disposition === 'DELEGATED') {
        if (typeof dimension.dimension_key !== 'string' || dimension.dimension_key.length === 0) {
          fail('M7_V2_VIEW_POLICY', 'delegated profile field is malformed');
        }
        fieldKeys.add(dimension.dimension_key);
      }
    }
    groupingAllowed ||= profile.grouping_policy.allowed;
  }
  for (const requiredField of ['APPLIES_TO', 'LEGAL_EFFECT']) {
    if (!fieldKeys.has(requiredField)) {
      fail('M7_V2_VIEW_POLICY', `approved profiles omit ${requiredField}`);
    }
  }
  const unsigned = {
    schema_version: 'STAGE_2Y_M7_V2_VIEW_POLICY/V1',
    labels: [...fieldKeys].sort().map((fieldKey) => ({
    ...
    formatters: Object.entries(V2_FORMATTERS).map(([valueType, formatterId]) => ({
  ...
  const viewPolicy = deepFreeze({
    view_policy_id: contentId('STAGE_2Y_M7_V2_VIEW_POLICY/V1', unsigned),
    ...unsigned,
  });
...
function failDispatch(code, detail) {
  const error = new TypeError(`${code}: ${detail}`);
  error.code = code;
  throw error;
}

function dispatchAgreementProjection(options) {
  ...
  if (canonicalJson(keys) !== canonicalJson([
    ...
    failDispatch('PROJECTION_DISPATCH_OPTIONS', 'exact dispatch options are required');
  }
  const {
    analysis,
    candidateRegistration,
  ...
  ].includes(executionLane)) {
    if (analysis?.schema_version !== 'AGREEMENT_ANALYSIS/V1'
        || viewPolicy?.schema_version !== 'STAGE_2Y_M6_VIEW_POLICY/V1'
        || candidateRegistration !== null) {
      failDispatch('PROJECTION_DISPATCH_SCHEMA', 'historical replay requires V1 inputs');
    }
    return projectLegacyAgreementV1(analysis, viewPolicy);
  }
  if (executionLane !== PROJECTION_EXECUTION_LANES.M7_V2_REPAIR_REGISTERED_CANDIDATE) {
    failDispatch('PROJECTION_DISPATCH_LANE', String(executionLane));
  }
  if (analysis?.schema_version !== 'AGREEMENT_ANALYSIS/V2'
      || viewPolicy?.schema_version !== 'STAGE_2Y_M7_V2_VIEW_POLICY/V1') {
    failDispatch('PROJECTION_DISPATCH_SCHEMA', 'registered candidate execution requires V2 inputs');
  }
  if (candidateRegistration?.schema_version
        !== 'STAGE_2Y_M7_V2_CANDIDATE_REGISTRATION/V1'
      || candidateRegistration.stage !== 'M7_V2_REPAIR'
      || candidateRegistration.lifecycle_state !== 'CANDIDATE_PENDING_REVIEW') {
    failDispatch('PROJECTION_DISPATCH_CANDIDATE', 'registered M7 V2 candidate is required');
  }
  const policyBytes = Buffer.from(`${canonicalJson(viewPolicy)}\n`, 'utf8');
  const binding = candidateRegistration.view_policy_binding;
  if (binding?.schema_version !== viewPolicy.schema_version
      || binding?.record_id_field !== 'view_policy_id'
      || binding?.record_id !== viewPolicy.view_policy_id
      || binding?.byte_length !== policyBytes.length
      || binding?.sha256 !== sha256Hex(policyBytes)) {
    failDispatch('PROJECTION_DISPATCH_POLICY', 'candidate view policy binding does not match');
  }
  return projectAgreement(analysis, viewPolicy);
}
