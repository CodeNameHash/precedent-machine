...
function resultBody(row) {
  ...
  throw new TypeError('result-composition detail requires a canonical result row');
}
...
function compositionInputs({
  ...
  if (!Array.isArray(relationshipTargets)
    || (row.row_kind === 'CANONICAL_RESULT' && relationshipTargets.length < 1)
  ...
  components.forEach((entry, index) => {
    ...
    if (canonicalJson(entry.component) !== canonicalJson(expected)
      || body.components[index].governed_ordinal !== index
  ...
  if ((row.row_kind === 'CANONICAL_RESULT' && orderedRelationships.length < 1)
    || orderedRelationships.length > MAX_RELATIONSHIPS) {
