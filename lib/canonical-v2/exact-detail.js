...
} = require('./composition-exact-detail');
const {
...
const {
  ACTION_SLOT_KEY: TERMINATION_FEE_TRIGGER_ACTION_SLOT_KEY,
...
function validateInputs({
  ...
  validateClaimRevisionIdentity(claim, { allowClosure: true });
...
function baseRowFromAttached(row) {
  ...
  const rowBody = body.row_kind === 'CANONICAL_RESULT'
    ...
    : body.row_kind === 'INCOMPLETE_CANONICAL_RESULT'
      ? body.incomplete_canonical_result
...
function validateFixtureExactDetailPackage({
  ...
  const actionSlotKey = candidate?.row?.source_actions?.[0]?.action_slot_key;
  ...
  if (candidate?.row?.row_kind === 'INCOMPLETE_CANONICAL_RESULT'
    && actionSlotKey !== COMPOSITION_ACTION_SLOT_KEY) {
    throw new TypeError('incomplete canonical result exact detail requires result-composition evidence');
  }
  if (actionSlotKey === TERMINATION_FEE_TRIGGER_ACTION_SLOT_KEY) {
    return validateQxoBuyerTerminationFeeTriggerDetailPackage({
      package: candidate,
      contract_bundle: contractBundle,
      source,
      source_admission: sourceAdmission,
  ...
  if (actionSlotKey === COMPOSITION_ACTION_SLOT_KEY) {
    if (Array.isArray(sources)) {
      return validateAdmittedMultiSourceResultCompositionDetailPackage({
        package: candidate,
        contract_bundle: contractBundle,
        sources,
        components,
    ...
    if (source?.schema_version === 'ADMITTED_SEMANTIC_SOURCE_CONTEXT/V1') {
      return validateAdmittedResultCompositionDetailPackage({
        package: candidate,
        contract_bundle: contractBundle,
        source,
        source_admission: sourceAdmission,
    ...
    return validateFixtureResultCompositionDetailPackage({
      package: candidate,
      contract_bundle: contractBundle,
      source,
      source_admission: sourceAdmission,
  ...
  if (actionSlotKey !== ACTION_SLOT_KEY && actionSlotKey !== GENERAL_ACTION_SLOT_KEY) {
    throw new TypeError('claim-evidence exact detail requires a governed claim-evidence action');
  }
  if (source?.schema_version === 'ADMITTED_SEMANTIC_SOURCE_CONTEXT/V1') {
    return validateAdmittedClaimEvidenceDetailPackage({
      package: candidate,
      contract_bundle: contractBundle,
      source,
      source_admission: sourceAdmission,
