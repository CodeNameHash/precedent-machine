...
function exactDetailAction(contractBundle) {
  const actions = contractBundle.serving_exact_detail_action_definitions || [];
  const action = actions.find((entry) => entry.action_slot_key === ACTION_SLOT_KEY);
  if (!action || actions.length !== 1) throw new TypeError('the frozen exact-detail action is unavailable');
  return action;
}
...
function validateInputs({ contract_bundle: contractBundle, row, source, source_admission: sourceAdmission, excerpt, claim }) {
  ...
  if (component.claim_occurrence_id !== claim.claim_occurrence_id
    ...
    throw new TypeError('selected result component does not own exactly one accuracy claim evidence edge');
  }
  ...
  return { action: exactDetailAction(contractBundle), component, evidence };
}
