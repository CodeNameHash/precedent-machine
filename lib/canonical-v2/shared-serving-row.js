...
const { compileFixtureContract, validateContractBundle, FIXTURE_CONTRACT_FINGERPRINTS } = require('./contract-bundle');
const {
...
function validateIncompleteCanonicalResultBody(body, row) {
  ...
  requireExactKeys(component.denominator, ['value', 'currency', 'basis', 'source_lineage_ids'], 'incomplete component denominator');
...
function validateCanonicalResultBody(body, row) {
  ...
  const relationshipEffectDigests = component.bounded_relationship_effects.map((relationship, index) => {
    ...
    // QXO_TAIL_CONDITIONS (PROPOSAL-TERMF-TRIGGER-VOCABULARY-2026-07-23, Step 1): the
    // three tail-composition conditions shared by every QXO §6.5(b)(iii) ground, per
    // the design rule that tail mechanics are trigger_condition codes composed onto a
    // ground trigger_code, not separate ground codes.
    ...
    // feeTriggerTerms maps each frozen trigger_code to the array of exact
    // (terminating_party, payment_timing, conditions) tuples a relationship using
    // that code is allowed to carry. Most codes name exactly one legal pathway
    // (array length 1, unchanged since the Landos freeze). NO_SOLICIT_BREACH_
    // TERMINATION is the vocabulary amendment's one ground that composes onto TWO
    // distinct pathways in the same deal (QXO's immediate §6.4(a)(ii) termination and
    // its tail §6.5(b)(iii)(C)(1) gateway) — the "grounds-as-codes" design rule from
    // PROPOSAL-TERMF-TRIGGER-VOCABULARY-2026-07-23.md means one ground code can
    // legitimately have more than one allowed tuple, distinguished by conditions/timing.
    const feeTriggerTerms = {
      ...
      // QXO termination-fee amendment (SPEC-QXO-TERMF-AMENDMENT-2026-07-23, Step 1):
      // immediate pathway (§6.4(a)(ii) via §6.5(b)(i)) and tail pathway (general-breach
      // gateway, §6.5(b)(iii)(C)(1)) — same ground, two compositions.
      ...
      // §6.5(b)(iii)(A)/(B) — tail-only in this deal; carries the extra
      // STOCKHOLDER_APPROVAL_NOT_YET_OBTAINED condition the text requires.
      STOCKHOLDER_APPROVAL_FAILURE_TERMINATION: [{
        terminating_party: 'EITHER_OR_PARENT_AS_SPECIFIED',
        payment_timing: 'UPON_EARLIER_OF_SIGNING_OR_CONSUMMATION',
        conditions: [...QXO_TAIL_CONDITIONS, 'STOCKHOLDER_APPROVAL_NOT_YET_OBTAINED'],
      }],
    ...
    const validFeeTrigger = isFeeTrigger
      && relationship.effect.legal_operation === 'CREATES_SELLER_TERMINATION_FEE_PAYMENT_TRIGGER'
...
function validateSharedServingRow(row) {
  ...
  if (!FIXTURE_CONTRACT_FINGERPRINTS.includes(row.provenance.contract_fingerprint)) {
