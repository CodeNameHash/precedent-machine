...
const EXACT_DETAIL_ACTION = FIXTURE_CONTRACT.serving_exact_detail_action_definitions[0];
...
function servingContract(contractFingerprint) {
  return {
    ...
    embedded_reference_allowlist_digest: contentId('SERVING_EMBEDDED_REFERENCE_ALLOWLIST/V1', {
      contract_fingerprint: contractFingerprint,
      source_actions: ['ACCURACY_STANDARD_CLAIM_EVIDENCE'],
    }),
  };
}
...
function validateSourceActions(row, body) {
  ...
  if (action.action_slot_key !== EXACT_DETAIL_ACTION.action_slot_key
    || action.action_version !== EXACT_DETAIL_ACTION.action_version
    || action.action_definition_id !== EXACT_DETAIL_ACTION.action_definition_id
    || action.action_definition_payload_digest !== EXACT_DETAIL_ACTION.action_definition_payload_digest
    || action.detail_kind !== EXACT_DETAIL_ACTION.detail_kind
...
function validateCanonicalResultBody(body, row) {
  ...
  const relationshipEffectDigests = component.bounded_relationship_effects.map((relationship, index) => {
    ...
    requireExactKeys(relationship.effect, [
      'effect_mode', 'legal_operation', 'accuracy_standard',
    ], `component.bounded_relationship_effects[${index}].effect`);
    if (relationship.relationship_definition_key !== 'BRINGS_DOWN'
      || relationship.effect.effect_mode !== 'TYPED_LEGAL_EFFECT'
      || relationship.effect.legal_operation !== 'TEST_ACCURACY_AT_CLOSING') {
  ...
  if (!metric
    ...
    || subject.basis_key !== metric.canonical_unit
    || !metric.allowed_canonical_values.includes(component.canonical_value)
