...
function validateCanonicalResultBody(body, row) {
  ...
  const relationshipEffectDigests = component.bounded_relationship_effects.map((relationship, index) => {
    ...
    const expectedFeeTrigger = feeTriggerTerms[relationship.effect.trigger_code];
    const validFeeTrigger = isFeeTrigger
      && relationship.effect.legal_operation === 'CREATES_SELLER_TERMINATION_FEE_PAYMENT_TRIGGER'
      && expectedFeeTrigger
      && relationship.effect.terminating_party === expectedFeeTrigger.terminating_party
      && relationship.effect.payment_timing === expectedFeeTrigger.payment_timing
      && canonicalJson(relationship.effect.conditions) === canonicalJson(expectedFeeTrigger.conditions);
