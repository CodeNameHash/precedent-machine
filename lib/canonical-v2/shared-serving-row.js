...
const SUBJECT_COHORT_MEMBERSHIP_KEYS = Object.freeze([
  'status',
  'exclusion_reason',
  'cohort_digest',
  'subject_deal_key',
...
const SUBJECT_COHORT_EXCLUSION_REASONS = Object.freeze([
  'FILTER_SECTOR_MISMATCH',
  'FILTER_BUYER_MISMATCH',
  'FILTER_MERGER_FORM_MISMATCH',
  'FILTER_ADVISER_MISMATCH',
  'FILTER_LAWYER_MISMATCH',
  'FILTER_ANNOUNCE_YEAR_OUT_OF_RANGE',
  'FILTER_DEAL_VALUE_OUT_OF_RANGE',
  'NOT_COMPARABLE_UNDER_COHORT_RULE',
]);

const SUBJECT_COHORT_FILTERS_BY_REASON = Object.freeze({
  FILTER_SECTOR_MISMATCH: Object.freeze(['sector']),
  FILTER_BUYER_MISMATCH: Object.freeze(['buyer']),
  FILTER_MERGER_FORM_MISMATCH: Object.freeze(['merger_form']),
  FILTER_ADVISER_MISMATCH: Object.freeze(['adviser_either']),
  FILTER_LAWYER_MISMATCH: Object.freeze(['lawyer_either']),
  FILTER_ANNOUNCE_YEAR_OUT_OF_RANGE: Object.freeze(['year_from', 'year_to']),
  FILTER_DEAL_VALUE_OUT_OF_RANGE: Object.freeze(['min_value_usd', 'max_value_usd']),
});
...
function validateSubjectCohortMembership({
  membership,
  counts,
  request,
  cohort_digest,
  ...
  requireExactKeys(
    membership,
    SUBJECT_COHORT_MEMBERSHIP_KEYS,
    'canonical_result.market_context.subject_cohort_membership',
  );
  requireDigest(membership.cohort_digest, 'subject cohort membership.cohort_digest');
  requireText(membership.subject_deal_key, 'subject cohort membership.subject_deal_key');
  requireDigest(
    membership.market_observation_serving_key,
    'subject cohort membership.market_observation_serving_key',
  );
  requireDigest(
    membership.membership_receipt_id,
    'subject cohort membership.membership_receipt_id',
  );
  const {
    membership_receipt_id: membershipReceiptId,
    ...receiptBody
  } = membership;
  if (membershipReceiptId !== contentId('SUBJECT_COHORT_MEMBERSHIP_RECEIPT/V1', receiptBody)) {
    throw new TypeError('subject cohort membership receipt identity mismatch');
  }
  if (request && membership.cohort_digest !== request.cohort_digest) {
    throw new TypeError('subject cohort membership receipt does not match the selected cohort');
  }
  if (cohort_digest && membership.cohort_digest !== cohort_digest) {
    throw new TypeError('subject cohort membership receipt does not match the stored cohort');
  }
  if (observation && (
    membership.subject_deal_key !== observation.deal_key
    || membership.market_observation_serving_key !== observation.market_observation_serving_key
  )) {
    throw new TypeError('subject cohort membership receipt does not bind the selected observation');
  }
  if (membership.status === 'INCLUDED') {
    if (membership.exclusion_reason !== null
      || counts.eligible_deals < 1
      || counts.applicable_deals < 1
      || counts.examined_deals < 1
      || counts.present_deals < 1
      || counts.comparable_deals < 1
      || counts.distribution_deals < 1) {
      throw new TypeError('included subject must be present in the comparable cohort');
    }
    return true;
  }
  if (membership.status !== 'EXCLUDED'
    || !SUBJECT_COHORT_EXCLUSION_REASONS.includes(membership.exclusion_reason)) {
    throw new TypeError('subject cohort membership requires an included state or typed exclusion reason');
  }
  if (request) {
    const requiredFilters = SUBJECT_COHORT_FILTERS_BY_REASON[membership.exclusion_reason];
    if (requiredFilters && !requiredFilters.some((key) => request.filters[key] !== null)) {
      throw new TypeError('subject cohort filter exclusion requires its selected narrower filter');
    }
  }
  return true;
}

function buildSubjectCohortMembershipReceipt({
  cohort_request,
  observation,
  status,
  exclusion_reason,
} = {}) {
  const request = compileMarketCohortRequest(cohort_request);
  if (!observation?.deal_key || !observation?.market_observation_serving_key) {
    throw new TypeError('subject cohort membership receipt requires an observation');
  }
  const receiptBody = {
    status,
    exclusion_reason,
  ...
  const membership = {
    ...receiptBody,
    membership_receipt_id: contentId('SUBJECT_COHORT_MEMBERSHIP_RECEIPT/V1', receiptBody),
  };
  validateSubjectCohortMembership({
    membership,
    counts: {
      eligible_deals: status === 'INCLUDED' ? 1 : 0,
      applicable_deals: status === 'INCLUDED' ? 1 : 0,
      examined_deals: status === 'INCLUDED' ? 1 : 0,
      present_deals: status === 'INCLUDED' ? 1 : 0,
      comparable_deals: status === 'INCLUDED' ? 1 : 0,
      distribution_deals: status === 'INCLUDED' ? 1 : 0,
      excluded_deals: 0,
      observation_slots: status === 'INCLUDED' ? 1 : 0,
      excluded_slots: 0,
    },
    request,
    observation,
...
function buildCanonicalResultServingRow({
  ...
  if (subject_cohort_membership != null) {
    validateSubjectCohortMembership({
      membership: subject_cohort_membership,
      request: compiledRequest,
      result: cohort_result,
      counts: cohort_result.counts,
      observation: projection_output.observation,
    });
  }
  const subjectCohortMembership = subject_cohort_membership == null
    ? null
    : clone(subject_cohort_membership);
  ...
  const canonicalResult = {
    ...
    market_context: {
      ...
      ...(subjectCohortMembership
        ? { subject_cohort_membership: subjectCohortMembership }
        : {}),
      denominators: {
        prevalence: {
...
function validateCanonicalResultBody(body, row) {
  ...
  requireExactKeys(body.market_context, [
    'metric_definition_id', 'metric_key', 'metric_version', 'subject_observation', 'cohort',
    ...(body.market_context.subject_cohort_membership
      ? ['subject_cohort_membership']
      : []),
    'denominators',
  ...
  if (body.market_context.subject_cohort_membership != null) {
    validateSubjectCohortMembership({
      membership: body.market_context.subject_cohort_membership,
      counts,
      cohort_digest: body.market_context.cohort.cohort_digest,
      observation: {
        deal_key: row.governed_deal_key,
        market_observation_serving_key: subject.market_observation_serving_key,
      },
    });
  }
