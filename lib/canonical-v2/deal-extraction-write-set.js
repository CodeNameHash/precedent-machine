...
function composeReferenceOnlyDealScopeWriteSet({ writeSets, deal } = {}) {
  if (!Array.isArray(writeSets) || writeSets.length < 1) {
    throw new TypeError('writeSets must contain at least one family write set');
  }
  if (!deal || typeof deal !== 'object' || Array.isArray(deal)) {
  ...
  for (const writeSet of writeSets) {
    assertCompatibleDeal(writeSet?.deal, deal);
    if (!Array.isArray(writeSet?.source_references) || writeSet.source_references.length < 1) {
      throw new TypeError('each reference-only family write set must carry admitted source references');
    }
    for (const sourceReference of writeSet.source_references) {
      insertExact(
        sourceReferences,
        sourceReference?.immutable_source_document_id,
        sourceReference,
        'admitted source reference',
      );
    }
    for (const [collection, identityField] of Object.entries(COLLECTION_IDENTITIES)) {
      const rows = writeSet?.[collection] || [];
