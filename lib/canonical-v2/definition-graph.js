...
const MAX_GRAPH_ITEMS = 4096;
const MAX_GRAPH_NESTED_ITEMS = 16384;

function requireSpan(span, name) {
  if (!span || !SHA256_RE.test(span.semantic_span_id || '')
...
function requireBoundedArray(value, name, { nonEmpty = false } = {}) {
  if (!Array.isArray(value)
    || value.length > MAX_GRAPH_ITEMS
    || (nonEmpty && value.length === 0)) {
    throw new TypeError(`${name} must be a bounded${nonEmpty ? ' non-empty' : ''} array`);
  }
  return value;
}

function requireBoundedGraphMembers(definitionCues, definitionUseCues) {
  const bodySpanCount = definitionCues.reduce((total, cue) => (
    total + (Array.isArray(cue?.body_spans) ? cue.body_spans.length : 0)
  ), 0);
  const identifierCount = definitionCues.length + definitionUseCues.length
    + definitionCues.reduce((total, cue) => (
      total + (Array.isArray(cue?.body_span_ids) ? cue.body_span_ids.length : 0)
    ), 0);
  if (definitionCues.length + definitionUseCues.length + bodySpanCount
    > MAX_GRAPH_NESTED_ITEMS) {
    throw new TypeError('semantic graph nested members exceed the bounded limit');
  }
  if (identifierCount > MAX_GRAPH_NESTED_ITEMS) {
    throw new TypeError('semantic graph identifier members exceed the bounded limit');
  }
}
...
function validateStoredSpanIdentity(span, canonicalTextId, name) {
  ...
  if (span.schema_version !== 'SEMANTIC_SPAN/V1'
    ...
    || !SHA256_RE.test(span.semantic_span_id || '')
    || !Number.isSafeInteger(span.absolute_start)
    || !Number.isSafeInteger(span.absolute_end)
    || span.absolute_start < 0
    || span.absolute_end <= span.absolute_start
...
function validateDefinitionCueIdentity(cue, graph) {
  ...
  if (cue.schema_version !== 'DEFINITION_CUE/V1'
    ...
    || !cue.raw_term
    || !SHA256_RE.test(cue.definition_cue_id || '')
    || !SHA256_RE.test(cue.source_anchor_id || '')
    || !SHA256_RE.test(cue.term_span_id || '')
    || !SHA256_RE.test(cue.raw_term_digest || '')
    || !SYNTACTIC_ROLES.has(cue.syntactic_role)
    || !Number.isSafeInteger(cue.source_order_ordinal)
    || cue.source_order_ordinal < 0
    || !Array.isArray(cue.body_spans)
    || cue.body_spans.length === 0
    || cue.body_spans.length > MAX_GRAPH_ITEMS
    || !Array.isArray(cue.body_span_ids)
    || cue.body_span_ids.length > MAX_GRAPH_ITEMS) {
...
function validateDefinitionUseCueIdentity(use, graph, cueIds) {
  ...
  if (use.schema_version !== 'DEFINITION_USE_CUE/V1'
    ...
    || !SHA256_RE.test(use.definition_use_cue_id || '')
    || !SHA256_RE.test(use.definition_cue_id || '')
    || !SHA256_RE.test(use.use_span_id || '')
    || !cueIds.has(use.definition_cue_id)
    || !USE_ROLES.has(use.use_role)
    || !Number.isSafeInteger(use.source_order_ordinal)
...
function buildDefinitionCue({ source, termSpan, bodySpans, rawTerm, syntacticRole, ordinal = 0 }) {
  ...
  requireBoundedArray(bodySpans, 'bodySpans', { nonEmpty: true });
  ...
  if (!rawTerm || !SYNTACTIC_ROLES.has(syntacticRole)
...
function buildDefinitionUseCue({ source, definitionCue, useSpan, useRole = 'OPERATIVE_REFERENCE', ordinal = 0 }) {
  ...
  if (!USE_ROLES.has(useRole) || !Number.isSafeInteger(ordinal) || ordinal < 0) {
...
function buildValidatedDefinitionGraph({ source, definitionCues, definitionUseCues }) {
  ...
  requireBoundedArray(definitionCues, 'definitionCues');
  requireBoundedArray(definitionUseCues, 'definitionUseCues');
  requireBoundedGraphMembers(definitionCues, definitionUseCues);
...
function validateValidatedDefinitionGraphIdentity({ graph }) {
  ...
  requireBoundedArray(graphBody.definition_cue_ids, 'definition_cue_ids');
  requireBoundedArray(
    graphBody.definition_use_cue_ids,
    'definition_use_cue_ids',
  );
  requireBoundedArray(graphBody.definition_cues, 'definition_cues');
  requireBoundedArray(graphBody.definition_use_cues, 'definition_use_cues');
  requireBoundedGraphMembers(graphBody.definition_cues, graphBody.definition_use_cues);
