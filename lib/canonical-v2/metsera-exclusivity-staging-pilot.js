...
const SELECTED_EVENT_DATE = '2025-08-17';
const DIRECT_GRANTOR_TEXT = 'Metsera did not grant Party 2 exclusivity';
const DIRECT_BENEFICIARY_TEXT = 'grant Party 2 exclusivity';
const GRANT_EFFECTIVE_TEXT = 'Later on August&nbsp;17, 2025';
const EXCLUSIVITY_EXPIRY_TEXT =
...
function verifiedSourceBytes(
  ...
  if (!(sourceBytesByIdentity instanceof Map)) {
    fail('source bytes must be supplied as a Map keyed by source document identity');
  }
  const bytes = sourceBytesByIdentity.get(sourceDocumentIdentity);
  if (!Buffer.isBuffer(bytes)) {
    fail(`source bytes are missing for ${sourceDocumentIdentity}`);
  }
  if (sha256Hex(bytes) !== expectedDigest) {
    fail(`source bytes do not match source_text_digest for ${sourceDocumentIdentity}`);
  }
  return bytes;
}
...
function validateSealedGrantEvidence({
  grantorEvidenceText,
  beneficiaryEvidenceText,
  grantEffectiveText,
  expiryText,
}) {
  if (
    ...
    || !grantorEvidenceText.includes('Metsera')
    || !grantorEvidenceText.includes('grant Party 2 exclusivity')
  ) {
    fail('grantor and beneficiary evidence must directly bind both entities to the grant');
  }
  if (
    grantEffectiveText !== GRANT_EFFECTIVE_TEXT
    || expiryText !== EXCLUSIVITY_EXPIRY_TEXT
    || grantEffectiveText.includes('September')
    || expiryText.includes('August')
  ) {
    fail('grant effective time and exclusivity expiry must remain distinct exact expressions');
  }
}
...
function buildMaterialisationInput({
  ...
  if (!passage
    ...
    || passage.date !== SELECTED_EVENT_DATE
    || canonicalJson(rolePassage.partyIds)
      !== canonicalJson(['metsera', 'party-2', 'party-1'])) {
  ...
  const directGrantorBeneficiaryInterval = subInterval(
    rolePassage,
    sourceByAccession,
    DIRECT_GRANTOR_TEXT,
  );
  ...
  const grantEffectiveInterval = subInterval(
    passage,
    sourceByAccession,
    GRANT_EFFECTIVE_TEXT,
  );
  const expiryInterval = subInterval(
    passage,
    sourceByAccession,
      ...
      temporal_slots: [
        {
          expected_temporal_slot_key:
            'METSERA_EXCLUSIVITY_GRANT_EFFECTIVE_TIME',
          governed_ordinal: 0,
          governed_subject: {
            logical_type: 'ProcessEvent',
            slot_key: 'METSERA_PARTY_2_EXCLUSIVITY_GRANT_EVENT',
...
function scopeEvidence(interval, sourceByIdentity, sourceBytesByIdentity) {
  const source = sourceByIdentity.get(interval.source_document_identity);
  const exactBytes = verifiedSourceBytes(
    sourceBytesByIdentity,
...
function buildPipelineInput(
  ...
  const exactEvidence = scopeEvidence(
    narrationInterval,
    sourceByIdentity,
    sourceBytesByIdentity,
  );
  ...
  return {
    ...
    trustedContext: {
      expected_acquisition_receipt_id:
        acquireProcessSources(sourceAcquisitionInput, sourceBytesByIdentity)
