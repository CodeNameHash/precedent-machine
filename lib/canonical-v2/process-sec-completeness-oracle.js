...
const PROCESS_SEC_COMPLETENESS_ORACLE_RECEIPT_SCHEMA =
  'PROCESS_SEC_COMPLETENESS_ORACLE_RECEIPT/V2';
...
function accessionInventoryDigest(governedScope, records) {
  return contentId('PROCESS_SEC_ACCESSION_INVENTORY/V2', {
...
function buildProcessSecCompletenessOracle(input) {
  ...
  const inventoryDigest = accessionInventoryDigest(governedScope, records);
  const body = {
    ...
    accession_inventory: records,
    accession_inventory_digest: inventoryDigest,
    limits,
...
function validateProcessSecCompletenessOracleReceipt(receipt) {
  ...
  const governedScope = validateScope(receipt.governed_scope);
  if (receipt.governed_scope_id !== governedScope.scope_id
    || receipt.governed_scope_digest !== governedScope.scope_digest
    || receipt.issuer_cik !== governedScope.issuer_cik) {
    fail(code, 'The receipt governed scope projections do not bind the retained scope.');
  }
  ...
  if (!Array.isArray(receipt.accession_inventory)
    || receipt.accession_inventory.length === 0
    || receipt.accession_inventory.length > limits.maximum_records) {
    fail(code, 'The receipt accession inventory must be a bounded non-empty array.');
  }
  const accessions = new Set();
  ...
  for (const record of receipt.accession_inventory) {
    let validRecord;
    try {
      validRecord = validateRecord(record, governedScope);
    } catch (error) {
      if (error instanceof ProcessSecCompletenessOracleError) {
        fail(code, 'The receipt has an invalid governed accession inventory record.', {
          cause: error.code,
        });
      }
      throw error;
    }
    if (accessions.has(validRecord.accession_number)
      || (previousAccession !== null
        && compareText(previousAccession, validRecord.accession_number) >= 0)) {
      fail(code, 'The receipt accession inventory is not unique canonical order.');
    }
    accessions.add(validRecord.accession_number);
    previousAccession = validRecord.accession_number;
    totalBytes += Buffer.byteLength(canonicalJson(validRecord), 'utf8');
  }
  if (totalBytes > limits.maximum_total_bytes || totalBytes > limits.maximum_memory_bytes) {
    fail(code, 'The receipt accession inventory exceeds a governed resource limit.');
  }
  const expectedInventory = accessionInventoryDigest(
    governedScope,
...
function reconcileExactProcessSecAccessionMembership(leftReceipt, rightReceipt) {
  ...
  for (const key of ['governed_scope_id', 'governed_scope_digest', 'issuer_cik']) {
    if (left[key] !== right[key]) {
      fail('PROCESS_SEC_SCOPE_MISMATCH',
        'Exact accession reconciliation requires identical governed scope identity.', {
          field: key,
          left: left[key],
          right: right[key],
        });
