...
const NO_INDEPENDENT_COMPARABLE_DEALS = 'NO_INDEPENDENT_COMPARABLE_DEALS';
...
function validateReviewedIocCapexServingRow(row) {
  ...
  const cohort = row?.canonical_result?.market_context?.cohort;
  if (
    row.governed_deal_key !== DEAL_KEY
    || !cohort
    || Object.values(cohort.counts || {}).some((count) => count !== 0)
    || cohort.distribution?.length !== 0
    || cohort.exclusions?.length !== 1
    || cohort.exclusions[0].reason_code !== NO_INDEPENDENT_COMPARABLE_DEALS
    || cohort.exclusions[0].slot_count !== 0
    || cohort.exclusions[0].deal_count !== 0
  ) {
    throw new TypeError(
      'reviewed IOC market cohort must exclude its subject deal',
    );
  }
  return true;
}
