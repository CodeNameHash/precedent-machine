...
function buildReviewedIocCapexServingRow({ slice, contractBundle } = {}) {
  ...
  validateReviewedIocCapexServingRow(row);
  return row;
}

function validateReviewedIocCapexServingRow(row) {
  validateSharedServingRow(row);
  const market = row?.canonical_result?.market_context;
  const cohort = market?.cohort;
  const subject = market?.subject_observation;
  if (
    row.governed_deal_key !== DEAL_KEY
    || !cohort
    || market.subject_cohort_membership?.status !== 'INCLUDED'
    || market.subject_cohort_membership?.exclusion_reason !== null
    || Object.entries(cohort.counts || {}).some(([key, count]) => (
      key === 'excluded_deals' || key === 'excluded_slots' ? count !== 0 : count !== 1
    ))
    || cohort.distribution?.length !== 1
    || cohort.distribution[0].canonical_value !== subject?.canonical_value
    || cohort.distribution[0].subject_count !== 1
    || cohort.distribution[0].deal_count !== 1
    || cohort.exclusions?.length !== 0
  ) {
    throw new TypeError(
      'reviewed IOC market cohort must include its subject deal',
    );
  }
  return true;
}
