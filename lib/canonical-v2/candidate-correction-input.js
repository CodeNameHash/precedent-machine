...
const { validateCorrectionValueEvidenceProof } = require('./correction-value-evidence');
const {
...
function validateValueEvidenceProofBinding(correction, application, predecessor, successor, proof) {
  if (proof === null) return null;
  try {
    validateCorrectionValueEvidenceProof(proof);
  } catch (error) {
    fail('STALE_CORRECTION_INPUT', 'Correction value evidence proof identity is invalid.', {
  ...
  if (proof.correction_id !== correction.correction_id
    || proof.correction_application_id !== application.correction_application_id
    || proof.claim_occurrence_id !== successor.claim_occurrence_id
    || proof.predecessor_claim_revision_id !== predecessor.claim_revision_id
    || proof.successor_claim_revision_id !== successor.claim_revision_id
    || proof.claim_definition_key !== successor.claim_definition_key
    || proof.claim_definition_version !== successor.claim_definition_version
    || canonicalJson(proof.expected_successor_projection) !== canonicalJson(expectedProjection)) {
    fail('STALE_CORRECTION_INPUT', 'Correction value evidence proof does not bind the exact correction successor.');
  }
  return proof;
}
...
function reproduceCorrectionOutput(correctionOutput, contractBundle) {
  ...
  validateValueEvidenceProofBinding(
    correction,
    application,
    predecessor,
    successor,
...
function correctionValueEvidenceProofRef(proof) {
  if (proof === null) return null;
  validateCorrectionValueEvidenceProof(proof);
  return buildCorrectionOutputRef({
    role: 'CONSISTENCY',
    logical_type: 'CORRECTION_VALUE_EVIDENCE_PROOF',
    stable_key: proof.correction_application_id,
    immutable_id: proof.correction_value_evidence_proof_id,
    canonical_payload_digest: proof.canonical_payload_digest,
  });
}
...
function buildCorrectionDischarge({
  ...
  const valueEvidenceProof = selected.value_evidence_proof;
  const valueEvidenceProofRef = correctionValueEvidenceProofRef(valueEvidenceProof);
  const suppliedConsistencyRefs = sortedOutputRefs(consistencyOutputRefs, 'CONSISTENCY');
  const suppliedProofRefs = suppliedConsistencyRefs.filter((ref) => (
    ref.logical_type === 'CORRECTION_VALUE_EVIDENCE_PROOF'
  ));
  if ((!valueEvidenceProofRef && suppliedProofRefs.length > 0)
    || suppliedProofRefs.some((ref) => (
      ref.correction_output_ref_id !== valueEvidenceProofRef.correction_output_ref_id
    ))) {
    fail('INVALID_CORRECTION_OUTPUT_REF', 'Correction value evidence proof refs must match the reproduced correction proof.');
  }
  const consistencyRefs = [
    ...suppliedConsistencyRefs.filter((ref) => (
      !valueEvidenceProofRef || ref.correction_output_ref_id !== valueEvidenceProofRef.correction_output_ref_id
    )),
    ...(valueEvidenceProofRef ? [valueEvidenceProofRef] : []),
  ].sort((left, right) => left.correction_output_ref_id.localeCompare(right.correction_output_ref_id));
  ...
  const beforePatchAfterProofDigest = contentId('CORRECTION_BEFORE_PATCH_AFTER_PROOF/V1', {
    ...
    correction_value_evidence_proof_id: valueEvidenceProof?.correction_value_evidence_proof_id || null,
    correction_value_evidence_proof_payload_digest: valueEvidenceProof?.canonical_payload_digest || null,
  });
  const body = {
    ...
    successor_claim_revision_id: successor.claim_revision_id,
    successor_claim_payload_digest: claimPayloadDigest(successor),
    correction_value_evidence_proof: valueEvidenceProof,
...
function validateCorrectionDischargeIdentity(discharge) {
  ...
  let expectedProofRef;
  try {
    expectedProofRef = correctionValueEvidenceProofRef(discharge.correction_value_evidence_proof);
  } catch (error) {
    fail('INVALID_CORRECTION_DISCHARGE', 'Embedded correction value evidence proof is invalid.', {
      cause_code: error.code || error.name,
      cause_message: error.message,
    });
  }
  const actualProofRefs = sortedConsistencyRefs.filter((ref) => (
    ref.logical_type === 'CORRECTION_VALUE_EVIDENCE_PROOF'
  ));
  if ((expectedProofRef === null && actualProofRefs.length !== 0)
    || (expectedProofRef !== null && (actualProofRefs.length !== 1
      || canonicalJson(actualProofRefs[0]) !== canonicalJson(expectedProofRef)))
    || (expectedProofRef !== null
      && (discharge.correction_value_evidence_proof.correction_id !== discharge.correction_id
        || discharge.correction_value_evidence_proof.correction_application_id
          !== discharge.correction_application_id
        || discharge.correction_value_evidence_proof.predecessor_claim_revision_id
          !== discharge.predecessor_claim_revision_id
        || discharge.correction_value_evidence_proof.successor_claim_revision_id
          !== discharge.successor_claim_revision_id))) {
    fail(
      'INVALID_CORRECTION_DISCHARGE',
      'Correction discharge proof output ref does not match its embedded immutable proof.',
    );
  }
