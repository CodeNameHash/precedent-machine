...
function projectEntry(entry) {
  ...
  if (!definition) {
    fail('UNGOVERNED_CLAIM', 'Only governed representation accuracy and knowledge claims can be projected.', {
  ...
  const body = {
    ...
    evidence: {
      ...
      category: claim.attributes.qualifier_kind,
...
function projectRepresentationClaims({
  ...
  if (!Array.isArray(resolvedEntries) || !Array.isArray(openWorldEntries)
    || (resolvedEntries.length === 0 && openWorldEntries.length === 0)) {
