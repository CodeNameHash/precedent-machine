...
const GENERAL_MATERIALITY_CLAIM_KEY = 'GENERAL_MATERIALITY_QUALIFIER';
const MEASUREMENT_DATE_CLAIM_KEY = 'REPRESENTATION_MEASUREMENT_DATE';
const RETROSPECTIVE_LOOKBACK_CLAIM_KEY = 'RETROSPECTIVE_LOOKBACK';
...
const GOVERNED_CLAIMS = Object.freeze({
  ...
  [GENERAL_MATERIALITY_CLAIM_KEY]: Object.freeze({
    label: 'General materiality qualifier',
    value_kind: 'PRESENCE',
    values: Object.freeze([true]),
  }),
  [MEASUREMENT_DATE_CLAIM_KEY]: Object.freeze({
    label: 'Representation measurement date',
    value_kind: 'ISO_8601_DATE_STRING',
  }),
  [RETROSPECTIVE_LOOKBACK_CLAIM_KEY]: Object.freeze({
...
function isGovernedCanonicalValue(key, value) {
  if (key === MEASUREMENT_DATE_CLAIM_KEY) return isIsoDate(value);
  return GOVERNED_CLAIMS[key]?.values?.includes(value) === true;
}
...
function dimensionsFor(key, side, party, attributes) {
  ...
  const attachment = validateAttachment(attributes);
  const qualifierPosition = attachment.position;
  ...
  if ([GENERAL_MATERIALITY_CLAIM_KEY, MEASUREMENT_DATE_CLAIM_KEY, RETROSPECTIVE_LOOKBACK_CLAIM_KEY].includes(key)) {
    const dispatchByClaim = {
      [GENERAL_MATERIALITY_CLAIM_KEY]: 'THRESHOLD_MATERIALITY',
      [MEASUREMENT_DATE_CLAIM_KEY]: 'TEMPORAL_MEASUREMENT_DATE',
      [RETROSPECTIVE_LOOKBACK_CLAIM_KEY]: 'TEMPORAL_RETROSPECTIVE_LOOKBACK',
    };
    if (attributes.qualifier_dispatch !== dispatchByClaim[key]) {
      fail('INVALID_QUALIFIER_DISPATCH', 'The qualifier dispatch does not match its governed claim definition.');
    }
    dimensions.representation_party = clone(party);
    dimensions.attachment = clone(attachment);
    dimensions.qualifier_dispatch = attributes.qualifier_dispatch;
    if (key === RETROSPECTIVE_LOOKBACK_CLAIM_KEY) {
      if (attributes.temporal_form !== 'LOOKBACK') {
        fail('INVALID_TEMPORAL_FORM', 'A retrospective lookback must retain its governed temporal form.');
      }
      dimensions.temporal_form = attributes.temporal_form;
    }
  }
...
function marketValue(key, canonicalValue, dimensions) {
  ...
  if (key === MEASUREMENT_DATE_CLAIM_KEY) {
    return {
      metric_key: MEASUREMENT_DATE_CLAIM_KEY,
  ...
  if (key === GENERAL_MATERIALITY_CLAIM_KEY || key === RETROSPECTIVE_LOOKBACK_CLAIM_KEY) {
    return {
      metric_key: `${key}_PRESENCE`,
...
function projectEntry(entry) {
  ...
  if (!definition) {
    fail('UNGOVERNED_CLAIM', 'Only governed representation claims can be projected.', {
  ...
  if (!claim || claim.state !== 'PRESENT' || claim.claim_definition_key !== key
    || !nonEmpty(claim.claim_revision_id) || !isGovernedCanonicalValue(key, claim.canonical_value)
