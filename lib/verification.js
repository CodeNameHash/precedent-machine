/* ─────────────────────────────────────────────────────────────────────────
   ...
   1. QUOTE VERIFICATION — every citable feature stores model-emitted "verbatim"
      ...
      against the deal's source text ONLY. A match against the provision's
      own full_text is recorded on a failure as a weaker triage signal
      (in_provision_text) — never as verification — because a quote that
      lands in the provision text but not the reconstructed source usually
      means the source drifted, not that the quote is safe. Its mutating
      sibling sanitizeFeatureQuotes(), which strips unverifiable quotes at
      ingest, is deliberately looser: it accepts a match against EITHER the
      source text or the supplied provision text. Unverified quotes are the
      hallucination surface — they get flagged, not trusted.
...
function normalizeForMatch(s) {
  ...
  return s
    ...
    .replace(/[‘’‛'“”„"]/g, '')
    .replace(/[–—]/g, '-')
    // Invisible directional/zero-width marks — both the undecoded NAMED
    // entity form ("&lrm;", present verbatim in older stored full_texts:
    // Summit Materials carries 99 of them) and the raw characters. Neither
    // carries content; the model quotes around them inconsistently, so both
    // sides must drop them before comparison.
    .replace(/&(?:lrm|rlm|zwnj|zwj|ZeroWidthSpace);/gi, '')
    // Older stored texts carry other undecoded named entities the current
    // stripHtml now handles at ingest — decode the ones quotes reproduce as
    // glyphs (Whole Foods: source "&sect;" vs quoted "§").
    .replace(/&sect;/gi, '§')
    .replace(/[\u200B-\u200F\u2060\uFEFF]/g, '')
    ...
    // Left gate includes ")" — subsection ranges wrap as "4.07(e)(i) -(iv)"
    // (United Homes) and the range continuation starts with "(", so the
    // lookahead admits "(" too when the left side is a close-paren.
    .replace(/([a-zA-Z0-9)])\s+-(?=[a-zA-Z0-9(])/g, '$1-')
    // OCR/text-layer artifacts can split the final digit of decimal legal
    // citations, e.g. FAR Section 45.10 1 in the source vs 45.101 in the
    // extracted provision. Close only a decimal + single trailing digit.
...
function quoteAppearsIn(normQuote, normSource) {
  ...
  // Head fallback for a quote whose TAIL diverges from the source. The old
  // rule forgave anything past the first 80 chars, so a genuine 80-char head
  // followed by a fabricated tail verified with zero flags (EXT-2). Only a
  // SHORT divergent tail is a truncation/elision artifact; a long one is
  // fabrication. Require the source to contain all but the last
  // HEAD_TAIL_SLACK chars — but never below the old 80-char floor, so this
  // only ever tightens (identical for quotes <= ~104 chars, stricter above).
  // Measured corpus-wide 2026-07-15: flips exactly 2 borderline claims of
  // 2,565 verified (one a confirmed hallucination), no false-flag flood.
  const q = frags[0] || normQuote;
  const HEAD_TAIL_SLACK = 24;
  const HEAD_FLOOR = 80;
  const keep = Math.max(HEAD_FLOOR, q.length - HEAD_TAIL_SLACK);
  if (q.length >= MIN_QUOTE_CHARS && leftGroundedContainment(q.slice(0, keep), normSource)) return true;
  if (crossRefAppearsIn(normQuote, normSource)) return true;
  return false;
}

// Matches the *-dark-bridge.js isWordChar convention exactly (including the
// underscore) so "word boundary" means the same thing everywhere this
// repository checks it.
function isWordChar(character) {
  return /[A-Za-z0-9_]/.test(character || '');
}

// True when `needle`'s OWN left edge sits at a word boundary in `haystack`
// at `index` (no boundary required when the needle's own edge is punctuation
// — mirrors the *-dark-bridge.js anchoredContains convention so a needle
// starting with e.g. "$" is not penalised on that side).
function leftBoundaryOk(haystack, index, needle) {
  const before = index > 0 ? haystack[index - 1] : '';
  return !isWordChar(needle[0]) || !isWordChar(before);
}

function rightBoundaryOk(haystack, index, needle) {
  const after = haystack[index + needle.length] || '';
  return !isWordChar(needle[needle.length - 1]) || !isWordChar(after);
}

// Word-boundary-anchored containment (both edges) whose accepted occurrence
// is also required to NOT sit just past an un-included governing negation
// (lib/negation-boundary-guard.js) — closes the "have a Company Material
// Adverse Effect" cut out of "...would not have a Company Material Adverse
// Effect..." class: that fragment is a genuine, word-boundary-aligned
// substring, so plain containment (the previous check here) accepted it.
// Tries every occurrence, not just the first, since a real quote can
// legitimately recur (boilerplate) even where one particular occurrence's
// boundary looks trimmed.
function groundedContainment(needle, haystack) {
  if (!needle || !haystack) return false;
  let from = 0;
  for (;;) {
    const index = haystack.indexOf(needle, from);
    if (index === -1) return false;
    if (leftBoundaryOk(haystack, index, needle) && rightBoundaryOk(haystack, index, needle)
      && !hasUnclosedNegationBeforeSpan(haystack, index)) return true;
    from = index + 1;
  }
}

// Same negation/left-boundary discipline as groundedContainment, but for the
// HEAD-fallback caller above: `needle` is a caller-chosen truncation of a
// longer quote (an arbitrary cut point, by design — see EXT-2 above), so its
// RIGHT edge is deliberately not boundary-checked, only its LEFT edge (where
// a trimmed negation would actually hide) and the negation guard itself.
function leftGroundedContainment(needle, haystack) {
  if (!needle || !haystack) return false;
  let from = 0;
  for (;;) {
    const index = haystack.indexOf(needle, from);
    if (index === -1) return false;
    if (leftBoundaryOk(haystack, index, needle) && !hasUnclosedNegationBeforeSpan(haystack, index)) return true;
    from = index + 1;
  }
}
...
//
// Both containment checks are WORD-BOUNDARY-ANCHORED and NEGATION-GUARDED
// (groundedContainment), not plain `.includes()`: plain containment accepts
// any substring that occurs anywhere in the source regardless of whether it
// preserves the source's meaning — a mid-word slice ("accurate" out of
// "inaccurate") or a fragment with its governing negation trimmed off the
// front both pass plain containment and both must not pass this.
function fragmentAppears(frag, normSource) {
  if (groundedContainment(frag, normSource)) return true;
  const stripped = frag.replace(/\.+$/, '');
  return stripped.length !== frag.length
    && stripped.length >= MIN_QUOTE_CHARS
    && groundedContainment(stripped, normSource);
}
...
const ANCILLARY_AGREEMENT_TITLES = [
  ...
  'escrow agreement',
  // Sponsor equity-backstop attachments on take-privates (M.D.C.'s tail is a
  // 47k limited guarantee). The PREAMBLE qualifier keeps body mentions of
  // "guarantee" from ever matching.
  'limited guarantee',
  'limited guaranty',
];
...
const CHARTER_EXHIBIT_TITLES = [
  ...
  'bylaws of',
  'by-laws of',
  // Maryland-entity charter exhibits (Forest City's tail is an 11.5k
  // "Articles of Amendment" to a Maryland REIT charter).
  'articles of amendment',
  'articles supplementary',
];
const CHARTER_BOILERPLATE = /(registered (?:office|agent)|name of the corporation|authorized[\s\S]{0,60}shares|series\s+[a-z]\s+(?:cumulative\s+)?(?:redeemable\s+)?preferred stock|does hereby certify|principal office|offices[\s\S]{0,80}(?:within or without|without the state)|article i\b|state department of assessments and taxation|(?:charter|articles) of the (?:company|corporation|trust)[\s\S]{0,60}(?:is|are) hereby amended)/;
...
function looksLikeKeptDefinitionAppendix(normSource, idx, phrase) {
  const after = normSource.slice(idx, idx + 160);
  if (phrase === 'certain definitions' && /\bcertain definitions\s+for purposes of\s+(?:this\s+)?article\b/.test(after)) {
    return false;
  }
  // "certain definitions section 2. demand registration section 3. ..." is
  // the TOC of an ATTACHED agreement (Kraft's registration rights agreement),
  // not the merger agreement's definitions appendix — two numbered
  // section-title entries in a row is the tell.
  if (/section \d{1,2}\.[\s\S]{0,80}?section \d{1,2}\./.test(after)) {
    return false;
  }
  return true;
}
...
function detectAncillaryRegions(normSource) {
  ...
  // Take the EARLIEST signature marker of either shape, not the first pattern
  // that happens to match anywhere. Cox / M.D.C. / Forest City open their
  // signature stacks with "[Signature Page to ...]" markers tens of thousands
  // of chars BEFORE the first "in witness whereof" string (which there belongs
  // to a later attached agreement) — pattern-priority matching anchored the
  // exclusion far too late and left 30-50k of signature pages in the
  // denominator.
  const witnessMatch = normSource.match(/in witness whereof[\s\S]{0,400}?(?:executed|caused this[\s\S]{0,80}?to be executed)/);
  // Bracket REQUIRED: unbracketed "signature page to this agreement by
  // facsimile" is a counterparts clause in the §10 boilerplate (Forest City
  // anchored its whole tail exclusion on one, swallowing operative text).
  const sigPageMatch = normSource.match(/\[\s*signature page to [^\]\n]{3,120}\]/);
  let sigMatch = witnessMatch;
  if (sigPageMatch && (!witnessMatch || sigPageMatch.index < witnessMatch.index)) sigMatch = sigPageMatch;
  if (!sigMatch) return [];
  ...
  // A title hit is only a HEADING if it isn't sitting mid-sentence. In
  // normalized space no line structure survives, so use lexical guards:
  // a definition of the ancillary agreement ('tax receivable agreement
  // means that certain tax receivable agreement, dated as of ...' — Redfin)
  // or a prose cross-reference ('the/that certain/such <title>') must not
  // anchor the exclusion. 2026-07-17 audit flags.
  const midSentenceTitle = (i, title) => {
    const before = normSource.slice(Math.max(0, i - 24), i);
    // NOTE: no bare "a"/"any"/"each" here — the canonical heading form is
    // "Exhibit A <title>", and its trailing "a" would false-positive.
    // "or"/"and"/comma catch list continuations ("the charter, articles or
    // certificate of incorporation" — Concho's Organizational Documents
    // definition).
    if (/(?:\b(?:the|that certain|such|this|or|and)\s|,\s*)$/.test(before)) return true;
    const after = normSource.slice(i + title.length, i + title.length + 24);
    if (/^\s*(?:means|shall mean|as defined|referred to)\b/.test(after)) return true;
    return false;
  };

  // Real attached agreement iff its own preamble follows the title — the
  // "THIS <Title> is made and entered into / dated as of" opening a
  // standalone executed document carries. A definition ("Voting Agreement
  // means the voting and support agreement, dated as of ...") has the
  // dated-as-of phrase but not the "this <title>" restatement — M.D.C.'s
  // Annex A anchored a 97k exclusion on exactly that. Anchor "this" to the
  // title's own leading words so a nearby "the date of this agreement"
  // inside a definition can't qualify.
  // A document-opening restatement near the title: "THIS <Title> (this
  // 'Agreement'), is made and entered into / dated as of ...". The window
  // reaches BEFORE the match too — the qualifying occurrence often IS the
  // restatement ("this amended and restated registration rights agreement
  // (this agreement), is made and entered into" — Kraft's RRA). Verb list
  // deliberately omits bare "by and among": a definition ("voting agreement
  // means the ... agreement, dated as of the date of this agreement, by and
  // among ...") carries that phrase after "this agreement" without being a
  // document opening (M.D.C.'s Annex A).
  const attachedPreambleAfter = (i) => {
    const vicinity = normSource.slice(Math.max(0, i - 80), i + 320);
    return /\bthis [a-z&.\- ]{0,70}?(?:agreement|guarantee|guaranty)\s*(?:\([^)]{0,50}\)\s*)?,?\s*(?:is (?:made and )?entered into|is made and entered|dated as of|is being executed)/.test(vicinity);
  };

  // Earliest QUALIFIED title at/after `from` across both title families —
  // used for the exclusion anchor AND for terminating the definitions-
  // appendix carve-back (a raw indexOf there ended the carve at the first
  // MENTION of "voting agreement" inside the definitions themselves,
  // excluding the rest of the appendix).
  const findQualifiedTitle = (from, opts = {}) => {
    let best = -1;
    let bestLabel = null;
    for (const title of ANCILLARY_AGREEMENT_TITLES) {
      let cursor = from;
      let i;
      while ((i = normSource.indexOf(title, cursor)) !== -1) {
        if (!midSentenceTitle(i, title) && attachedPreambleAfter(i)) {
          if (best === -1 || i < best) { best = i; bestLabel = title; }
          break; // earliest occurrence of THIS title is enough
        }
        cursor = i + title.length;
      }
    }
    // Charter/bylaws exhibit forms: header + charter boilerplate within ~600
    // chars qualifies (no agreement preamble exists on these).
    if (!opts.ancillaryOnly) {
      for (const title of CHARTER_EXHIBIT_TITLES) {
        let cursor = from;
        let i;
        while ((i = normSource.indexOf(title, cursor)) !== -1) {
          if (!midSentenceTitle(i, title) && CHARTER_BOILERPLATE.test(normSource.slice(i, i + 600))) {
            if (best === -1 || i < best) { best = i; bestLabel = title; }
            break;
          }
          cursor = i + title.length;
    ...
    return { index: best, label: bestLabel };
  };

  const anchor = findQualifiedTitle(scanFrom);
  let firstStart = anchor.index;
  let firstLabel = anchor.label;
  // Form-of exhibit stubs: "exhibit a-1 form of subscription agreement
  // [attached]" (Noble Africa's 12-exhibit trailer). These carry neither an
  // executed-document preamble nor charter boilerplate, so no title family
  // qualifies — but an "Exhibit <designator> Form of ..." heading after the
  // signature block is unambiguous (mirrors findBodyEnd's FORM-OF rule).
  const formOf = /\bexhibit [a-z0-9][a-z0-9.-]{0,3} form of [a-z]/;
  const formOfMatch = normSource.slice(scanFrom).match(formOf);
  if (formOfMatch) {
    const idx = scanFrom + formOfMatch.index;
    if (firstStart === -1 || idx < firstStart) { firstStart = idx; firstLabel = 'form-of exhibit'; }
  }
  ...
  // Search for the kept definitions appendix from the EXCLUSION start, not
  // from the first exhibit title: Concho / M.D.C. put "Annex A — Certain
  // Definitions" between the signature stack and the first exhibit, so a
  // firstStart-anchored search skipped right past it and the whole tail —
  // operative definitions included — got excluded (2026-07-17 audit flags).
  let defStart = -1;
  for (const ph of DEF_APPENDIX_HEADERS) {
    let from = exclusionStart;
    let di;
  ...
  // The kept appendix must belong to the MERGER agreement: once an attached
  // ancillary agreement has begun, any later "certain definitions" is that
  // document's own section (Kraft's registration rights agreement §1) and
  // stays excluded. Charter exhibits don't break the chain — Kraft's real
  // index follows its charter exhibit.
  if (defStart !== -1) {
    const firstAncillary = findQualifiedTitle(exclusionStart, { ancillaryOnly: true });
    if (firstAncillary.index !== -1 && firstAncillary.index < defStart) defStart = -1;
  }
  if (defStart === -1) {
  ...
  // Appendix end: the next exhibit-like header after it, capped. The cap is
  // a backstop for tails where no later title matches (Kraft: index →
  // by-laws → tax opinions) — but it must clear a FULL "Annex A — Certain
  // Definitions" (Concho's runs well past 25k), so it is generous; the
  // next-title search is the real terminator.
  const DEF_APPENDIX_MAX_CHARS = 60000;
  let defEnd = Math.min(srcLen, defStart + DEF_APPENDIX_MAX_CHARS);
  const nextExhibit = findQualifiedTitle(defStart + 40);
  if (nextExhibit.index !== -1 && nextExhibit.index < defEnd) defEnd = nextExhibit.index;
...
/**
 * Detect HEAD matter to exclude from the coverage denominator: the cover page
 * and table of contents that precede the operative preamble. Anchor: the
 * ENACTING phrase of the real preamble ("now, therefore ... agree as
 * follows" / "witnesseth" / "recitals"); the operative preamble sentence is
 * the LAST "this agreement is entered into / dated as of" match before it
 * (cover pages repeat the same phrases, so first-match anchors too early).
 * Everything before that sentence — filename header, cover, TOC — is not
 * provision text. Recitals and the preamble itself stay IN the denominator.
 * Conservative by design: no enacting phrase in the first 30% -> exclude
 * nothing; the LLM coverage audit (scripts/coverage-audit.js) reviews every
 * excluded region afterwards, so a false exclusion is caught, not silent.
 */
function detectHeadMatter(normSource) {
  const srcLen = normSource.length;
  const scanCap = Math.floor(srcLen * 0.3);
  const enact = normSource.slice(0, scanCap).search(/\b(?:now,? therefore|witnesse?th|recitals\b|the parties(?: hereto)? .{0,60}agree as follows)/);
  if (enact <= 0) return [];
  const window = normSource.slice(0, enact);
  const preambleRe = /(?:this (?:agreement|transaction agreement|amendment)|agreement and plan of merger|transaction agreement)[^]{0,240}?(?:is (?:made and )?entered into|made and entered into|is dated as of|dated as of [a-z]+ \d)/g;
  let headEnd = -1;
  let m;
  while ((m = preambleRe.exec(window)) !== null) headEnd = m.index;
  if (headEnd < 2000) return []; // no cover/TOC of consequence before the preamble
  return [[0, headEnd, 'cover/toc']];
}
...
function computeCoverage(provisions, sourceText, opts = {}) {
  ...
  const ancillary = opts.includeAncillary
    ? []
    : [...detectHeadMatter(normSource), ...detectAncillaryRegions(normSource)];
