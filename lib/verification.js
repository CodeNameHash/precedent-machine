...
function quoteAppearsIn(normQuote, normSource) {
  ...
  const frags = normQuote.split(/\s*(?:\.\.\.|…)\s*/).filter((f) => f.length >= MIN_QUOTE_CHARS);
  if (frags.length > 1) {
    return frags.every((f) => normSource.includes(f));
  }
...
function detectAncillaryRegions(normSource) {
  ...
  // starts within it (carve it back so integral defs stay counted).
  let end = srcLen;
  for (const ph of DEF_APPENDIX_HEADERS) {
    const di = normSource.indexOf(ph, firstStart);
    if (di !== -1 && di < end) end = di;
  }
  if (end - firstStart < 2000) return []; // too small to matter
  return [[firstStart, end, firstLabel]];
}
