...
// Identify which party can terminate to invoke a trigger, from phrasing like
// "Termination by the Company …" / "by Parent …" / "by either Parent or the
// Company …". Returns a short label or null.
function deriveTriggerParty(s) {
  if (typeof s !== 'string') return null;
  // "either Parent or the Company" / "either party" / "Parent or the Company"
  if (/by\s+either\b/i.test(s) || /\b(?:Parent|Company)\s+or\s+(?:the\s+)?(?:Parent|Company)\b/i.test(s)) {
    return 'Either party';
  }
  if (/by\s+(?:the\s+)?Company\b/i.test(s)) return 'Company / Target';
  if (/by\s+(?:Parent|Buyer|Acquiror|Acquirer|Merger\s+Sub)\b/i.test(s)) return 'Parent / Buyer';
  return null;
}
