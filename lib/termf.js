...
function deriveTriggerParty(s) {
  ...
  // 1. "<Party> terminates" — the grammatical subject of the termination verb.
  ...
  // 2. "either Parent or the Company" / "by either …"
  if (/by\s+either\b/i.test(s) || /\beither\s+(?:Parent|the\s+Company)\s+or\s+(?:the\s+)?(?:Parent|Company)\b/i.test(s)) {
    return 'Either party';
  }
