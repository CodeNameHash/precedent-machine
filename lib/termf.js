...
// shape { name, terminationClauses, party, feeAmount, feeAmountPct }.
function parseTriggerStrings(arr) {
  ...
  for (const s of arr) {
    ...
    // string that is really the tail descriptor ("Tail fee: …") would otherwise
    // surface as a confusing party-less extra row in the company-fee matrix.
    if (/^\s*tail[\s-]*fee\b/i.test(s)) continue;
    const refs = Array.from(new Set(s.match(SECTION_REF_RE) || []));
    const name = deriveTriggerName(s, refs);
    out.push({
      name,
      terminationClauses: refs,
      party: deriveTriggerParty(s) || inferTriggerPartyFromName(name),
      feeAmount: null,
      feeAmountPct: null,
      sourceText: s.trim(),
    });
