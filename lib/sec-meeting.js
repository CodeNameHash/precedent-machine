...
export function secMeetingDisplayState(provisions) {
  ...
  const rows = [
    { key: 'proxyFilingDeadline', label: 'Filing deadline', kind: 'deadline', value: summary.proxyFilingDeadline },
    { key: 'mailingDeadline', label: 'Mailing', kind: 'deadline', value: summary.mailingDeadline },
    { key: 'meetingDeadline', label: 'Meeting', kind: 'deadline', value: summary.meetingDeadline },
    ...summary.adjournmentRights.map((right, idx) => ({
      key: `adjournmentRights-${idx}`,
      label: `Adjournment rights${right.party ? ` - ${enumLabel(right.party)}` : ''}`,
      kind: 'adjournment',
      value: right,
    })),
  ];
