/* ─────────────────────────────────────────────────────────────────────────
   ...
   Closed unit vocabulary (spec P3-numeric, Fable decision — do not extend
   without a new spec): USD, percent, days, business_days, months, years,
   shares.
...
const CLOSED_UNITS = new Set(['USD', 'percent', 'days', 'business_days', 'months', 'years', 'shares']);
...
const UNIT_ALTERNATION = '(business\\s+days|days|months|years|shares)';

function normalizeUnitWord(raw) {
  const w = String(raw).toLowerCase().replace(/\s+/g, ' ').trim();
  return w === 'business days' ? 'business_days' : w;
}
...
function parseDuration(text) {
  ...
  while ((m = unitOccurrenceRe.exec(text)) !== null) {
    ...
    const parenTail = before.match(/([a-z]+(?:-[a-z]+)?)?\s*\((\d+(?:\.\d+)?)\)\s*$/i);
    // A bare number immediately before the unit word (no parens).
    const bareTail = !parenTail ? before.match(/(\d+(?:\.\d+)?)\s*$/) : null;
...
function classifyNullReason(text) {
  ...
  const hasUnitContext = /\$|%|\bpercent\b|\b(business\s+days|days|months|years|shares)\b/i.test(trimmed);
