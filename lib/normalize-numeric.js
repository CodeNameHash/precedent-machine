...
const NUMERAL_LADDER_SCHEMA = 'NUMERAL_LADDER/V1';
const NUMERAL_LADDER_RUNGS = Object.freeze({
  LEGACY_DIGIT_ONLY: 0,
  SPELLED_AND_DIGIT: 1,
  SPELLED_ALONE: 2,
  UNIT_CONVERSION: 3,
...
function validateNumeralLadderRung(numeralRung) {
  if (!Number.isInteger(numeralRung) || numeralRung < NUMERAL_LADDER_RUNGS.LEGACY_DIGIT_ONLY || numeralRung > NUMERAL_LADDER_RUNGS.UNIT_CONVERSION) throw new TypeError('numeral rung must be an integer from 0 through 3');
  return numeralRung;
}
...
function parseDurationAtNumeralRung(text, { numeralRung = NUMERAL_LADDER_RUNGS.LEGACY_DIGIT_ONLY, convertToUnit = null } = {}) {
  validateNumeralLadderRung(numeralRung);
  if (convertToUnit !== null && convertToUnit !== 'months') throw new TypeError('only conversion to months is supported');
  ...
  while ((m = unitOccurrenceRe.exec(text)) !== null) {
    ...
    const parenTail = before.match(/\(\s*(\d+(?:\.\d+)?)(?:st|nd|rd|th)?\s*\)[\s-]*$/i);
    // A bare number immediately before the unit word (no parens).
    const bareTail = !parenTail ? before.match(/(\d+(?:\.\d+)?)(?:st|nd|rd|th)?[\s-]*$/i) : null;
    const spelledPhrase = trailingNumberWordsPhrase(parenTail ? before.slice(0, before.lastIndexOf('(')) : (bareTail ? before.slice(0, before.length - bareTail[0].length) : before));

    let numericVal = null;
    let spelledVal = null;
    if (parenTail) {
      numericVal = parseFloat(parenTail[1]);
      if (spelledPhrase !== null) spelledVal = wordsToNumber(spelledPhrase);
    } else if (bareTail) {
      numericVal = parseFloat(bareTail[1]);
    } else if (spelledPhrase !== null) {
      spelledVal = wordsToNumber(spelledPhrase);
    }
    ...
    if (numericVal === null && numeralRung < NUMERAL_LADDER_RUNGS.SPELLED_ALONE) continue;
    const value = numericVal !== null ? numericVal : spelledVal;
    matches.push({ value, unit });
  }
  ...
  for (const mm of matches) distinct.set(`${mm.value}|${mm.unit}`, mm);
  if (distinct.size > 1) return null; // more than one distinct duration mentioned -> ambiguous
  const result = [...distinct.values()][0];
  if (numeralRung >= NUMERAL_LADDER_RUNGS.UNIT_CONVERSION && convertToUnit === 'months' && result.unit === 'years') return { value: result.value * 12, unit: 'months' };
  return result;
}
