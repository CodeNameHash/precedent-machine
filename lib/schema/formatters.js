...
function genericObjectValue(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  for (const key of ['value', 'label', 'text', 'quote', 'summary', 'name']) {
    if (!isNil(value[key])) return value[key];
  }
  if (!isNil(value.code)) return humanizeEnum(value.code);
  return null;
}
...
function formatGenericValue(value) {
  ...
  if (typeof value === 'object') {
    const simple = genericObjectValue(value);
    if (simple !== null) return formatGenericValue(simple);
    return Object.entries(value)
      .map(([key, item]) => {
        const rendered = formatGenericValue(item);
        return rendered ? `${labelKey(key)}: ${rendered}` : null;
      })
      .filter(Boolean)
      .join('; ');
  }
...
function formatValue(formatterName, value, ctx = {}) {
  ...
  if (!formatter) return formatGenericValue(value);
