...
/**
 * Categorize an IOC covenant from its BODY text (not a clean heading).
 *
 * canonicalIocCategoryFromHeading uses fixed array-priority first-match,
 * which is right for short headings but mis-fires on multi-sentence
 * sub-clause bodies: a debt-securities covenant that also names an
 * "acquisition" carve-out would match ACQUISITIONS before INDEBTEDNESS; a
 * loans covenant that later says "accounting" would match ACCOUNTING. In a
 * "shall not [verb] [OBJECT] ..." covenant the operative OBJECT that fixes
 * the category sits near the start, before cross-references and carve-outs —
 * so pick the category whose keyword appears EARLIEST in the text, breaking
 * ties by array priority. Returns null when nothing matches.
 */
function iocCategoryFromBody(body) {
  const text = String(body || '');
  if (!text.trim()) return null;
  let best = null;
  for (let i = 0; i < HEADING_TO_IOC_CATEGORY.length; i += 1) {
    const entry = HEADING_TO_IOC_CATEGORY[i];
    const pos = text.search(entry.pattern);
    if (pos < 0) continue;
    if (!best || pos < best.pos) best = { key: entry.key, pos, order: i };
    // strictly-earlier wins; equal position keeps the earlier array entry
  }
