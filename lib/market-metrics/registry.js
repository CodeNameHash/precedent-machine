...
function manifestMaterialContractMetrics(row, rowKey, label) {
  ...
  if (/\b(?:per\s+annum|per\s+year|annual(?:ly)?|in\s+(?:19|20)\d{2})\b/i.test(cadenceText)) cadence = 'annual';
  ...
  const metricBase = `material-contracts.bucket.${itemCode.toLowerCase()}.${cadence.replace(/_/g, '-')}`;
