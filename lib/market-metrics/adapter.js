...
import darkAuthorityFence from '../query/dark-authority-fence.js';

const { FEATURES } = featuresModule;
const { assertRowIsServable } = darkAuthorityFence;
...
export function resolveMarketMetricRow(row, context = {}) {
  assertRowIsServable(row, 'Market metrics cannot admit a VALIDATED_NOT_SERVED source card.');
  if (row?.marketState === 'OPEN_NATIVE_FIELD') {
    ...
    return {
      ...
      metrics: [nonComparableMetric({
        rowKey,
        metricKey: `${rowKey}.evidence-only`,
        label,
        code: 'unstructured_source',
        detail: 'Evidence retained without a governed comparable field.',
      })],
      errors: [],
    };
  }
