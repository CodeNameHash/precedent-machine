...
function semanticKeyForDynamicRow(row, label, context) {
  ...
  if (id.startsWith('material-contracts-') && row?.code) {
    ...
    if (/\b(?:per\s+annum|per\s+year|annual(?:ly)?|in\s+(?:19|20)\d{2})\b/i.test(text)) cadence = 'annual';
    ...
    return `material-contracts:${slug(row.code)}:${cadence}`;
  }
...
function semanticsForFeature(kind, definition) {
  if (kind === 'money') {
...
function registryMetrics(row, context, rowKey, label) {
  ...
  const definition = FEATURES[primaryKey] || null;
  const cohort = featureCohort(featureDefs, context, sourceCardsForMarketRow(row));
  const metricBase = metricBaseForFeature(context, primaryKey, rowKey);
  const valueKind = valueKindForFeature(primaryKey, definition);
  ...
  if (valueKind === 'duration') {
    ...
    return [
      ...
      nonComparableMetric({
        ...
        label,
        code: 'ambiguous_time_basis',
        detail: 'The feature registry does not identify both the legal trigger and an unambiguous clock for this duration.',
      }),
    ];
  }
  if (valueKind === 'object') {
    ...
    return [
      ...
      nonComparableMetric({
        ...
        metricKey: `${metricBase}.value`,
        label,
        code: 'unstructured_source',
        detail: 'This object-valued feature needs an explicit field path before its values can be compared.',
      }),
    ];
  }
  return featureMetricSet({
    ...
    kind: valueKind,
    cohort,
    semantics: semanticsForFeature(valueKind, definition),
  });
}
