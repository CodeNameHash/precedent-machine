...
const CALENDAR_BASES = new Set(['elapsed', 'business', 'calendar']);
const CADENCES = new Set(['aggregate', 'annual', 'per_event', 'per_day', 'per_month', 'one_time', 'unknown']);
...
function validateSemantics(spec, errors) {
  ...
  if (kind === 'duration') {
    if (!['elapsed_hours', 'business_days', 'calendar_days', 'months', 'years'].includes(unit)) {
