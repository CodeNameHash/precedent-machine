...
function previewRouteActions(records, label = 'preview route-action observation') {
  if (!Array.isArray(records)
    ...
    || records.some((entry) => !REQUIRED_PREVIEW_ACTION_CLASSES.includes(entry.action_class))
...
function actionIdentities(records) {
...
function actionInventoryRoot(actions) {
  return domainDigest('PROGRAMME_GATE_PREVIEW_ROUTE_ACTION_INVENTORY/V1', actions);
}
...
function buildIsolationObservationSource(input) {
  ...
  const sourceActions = actionIdentities(previewRouteActions(
    input.sourcePreviewRouteActions,
    'source preview route-action inventory',
  ));
  const builtActions = actionIdentities(previewRouteActions(
    input.builtPreviewRouteActions,
    'built preview route-action inventory',
  ));
  const runtimeActions = actionIdentities(accessActions);
  return Object.freeze({
    ...
    source_route_action_inventory: sourceActions,
    source_route_action_inventory_root: actionInventoryRoot(sourceActions),
    built_route_action_inventory: builtActions,
    built_route_action_inventory_root: actionInventoryRoot(builtActions),
    runtime_route_action_inventory: runtimeActions,
    runtime_route_action_inventory_root: actionInventoryRoot(runtimeActions),
