...
const COMPLETION_RATIFICATION_BLOCKER = 'P9_COMPLETION_LEAF_RATIFICATION_REQUIRED';
...
function recordFor(inventory, leaf) {
  ...
  const body = {
    ...
    partition: completion ? 'RECOVERED_COMPLETION_CANDIDATE_NOT_LIVE' : 'LIVE_P9_LEAF',
    ...
    adoption_blocker: completion ? COMPLETION_RATIFICATION_BLOCKER : ADOPTION_BLOCKER,
...
function buildP9DefinitionProposalLayerFromInventory({ inventory, repository_root: repositoryRoot } = {}) {
  if (!inventory || typeof inventory !== 'object' || !Array.isArray(inventory.leaves)
    ...
    || canonicalJson(inventory.recovered_candidate_p9_gate_ids) !== canonicalJson(RECOVERED_CANDIDATE_P9_GATE_IDS)
    || inventory.leaves.length !== RECOVERED_CANDIDATE_P9_GATE_IDS.length) {
    fail('P9_DEFINITION_PROPOSAL_INVENTORY_INVALID', 'P9 definition proposal layer requires the corrected 21-plus-one inventory.');
  }
  const body = {
...
function validateP9DefinitionProposalLayerFromInventory(value, { inventory, repository_root: repositoryRoot } = {}) {
  exactKeys(value, [
    ...
    'predicate_authority', 'records', 'recovered_candidate_p9_gate_ids', 'schema_version', 'state',
  ...
  if (value.schema_version !== SCHEMA || value.state !== STATE || value.inventory_id !== inventory.inventory_id
    ...
    || canonicalJson(value.recovered_candidate_p9_gate_ids) !== canonicalJson(RECOVERED_CANDIDATE_P9_GATE_IDS)
    || !Array.isArray(value.records) || value.records.length !== RECOVERED_CANDIDATE_P9_GATE_IDS.length) {
...
function proposalBinding(layer) {
  return Object.freeze({
    ...
    live_record_count: layer.records.filter((record) => record.partition === 'LIVE_P9_LEAF').length,
    completion_candidate_count: layer.records.filter((record) => record.partition === 'RECOVERED_COMPLETION_CANDIDATE_NOT_LIVE').length,
...
module.exports = {
  ADOPTION_BLOCKER,
  COMPLETION_RATIFICATION_BLOCKER,
  EVIDENCE_OBJECT_MAP,
  RECORD_SCHEMA,
  SCHEMA,
  STATE,
