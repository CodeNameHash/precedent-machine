...
const P0_TEST_FILES = Object.freeze([
...
async function executeP0Test(runtime, root, codeCommit, environment) {
  ...
  const executableMembers = P0_TEST_FILES.map((file) => ({
    path: file,
    sha256: sha256(runtime.readFile(runtime.resolve(root, file))),
  }));
...
async function collectContainmentEvidence({
  ...
  const p0Test = await executeP0Test(fixedRuntime, root, fixedCodeCommit, fixedEnvironment);
