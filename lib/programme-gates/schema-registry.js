...
function catalogueReviewPayload(catalogueKind, reviewScope) {
  return closedObject({
    ...
    reasoning_level: { enum: ['high', 'provider_default'] },
...
const generatedContractBundleMemberSchema = closedObject({
  schema_version: { const: 'CanonicalGeneratedContractBundleMember/V1' },
  member_key: nonEmptyString,
...
const schemas = Object.freeze({
  'CanonicalGeneratedContractBundleMember/V1': {
    $id: 'CanonicalGeneratedContractBundleMember/V1',
    ...generatedContractBundleMemberSchema,
  },
  ...
  'P1ContractFreezeReviewRegistration/V1': {
    $id: 'P1ContractFreezeReviewRegistration/V1',
    ...closedObject({
      schema_version: { const: 'P1ContractFreezeReviewRegistration/V1' },
      registration_id: digest,
      gate_id: { const: 'P1_CONTRACT_FREEZE_ATTESTED' },
      exact_review_package_fingerprint: digest,
      exact_review_package_payload_digest: digest,
      code_commit: codeCommit,
      ...
      lanes: {
        ...
        items: closedObject({
          ...
          expected_authenticated_result_digest: digest,
        }),
      },
      controller_key_id: token,
      signature_algorithm: { const: 'Ed25519' },
      controller_signature: { type: 'string', pattern: BASE64, minLength: 1 },
    }),
  },

  'P1ContractFreezeAuthenticatedResult/V1': {
    $id: 'P1ContractFreezeAuthenticatedResult/V1',
    ...closedObject({
      schema_version: { const: 'P1ContractFreezeAuthenticatedResult/V1' },
      registration_id: digest,
      lane_id: token,
      task_id: nonEmptyString,
      result_payload_digest: digest,
      authenticated_result_digest: digest,
      controller_key_id: token,
      signature_algorithm: { const: 'Ed25519' },
      controller_signature: { type: 'string', pattern: BASE64, minLength: 1 },
    }),
  ...
  'ReviewerIndependenceAttestation/V1': {
    ...
    ...closedObject({
      ...
      source_control_history_scope: {
        const: 'REVIEWED_COMMIT_ANCESTRY_FROM_REPOSITORY_GENESIS',
      },
  ...
  'ColdReviewOutput/V1': {
    $id: 'ColdReviewOutput/V1',
    ...closedObject({
      disposition: { enum: ['PASS', 'FAIL'] },
      findings: {
        type: 'array',
        uniqueItems: true,
        items: closedObject({
  ...
  'ExactDigestReviewSetAttestation/V2': {
    $id: 'ExactDigestReviewSetAttestation/V2',
    ...closedObject({
      review_set_evidence_id: digest,
      reviewed_root: digest,
      reviewed_code_commit: codeCommit,
      review_artifact_sha256: digest,
      review_artifact_byte_size: { type: 'integer', minimum: 1 },
      lane_outcomes: {
        type: 'array',
        minItems: 5,
        maxItems: 5,
      ...
      full_review_disposition: { enum: ['PASS', 'FAIL'] },
      full_review_pass_claimed: { const: false },
    }),
  },
  ...
  'BenSpecificationApproval/V2': {
    $id: 'BenSpecificationApproval/V2',
    ...closedObject({
      schema_version: { const: 'BenSpecificationApproval/V2' },
      approved_root: digest,
      approved_code_commit: codeCommit,
      review_set_evidence_id: digest,
      reviewed_root: digest,
      reviewed_code_commit: codeCommit,
      review_artifact_sha256: digest,
      review_artifact_byte_size: { type: 'integer', minimum: 1 },
      full_review_pass_claimed: { const: false },
      authorisation_scope: { const: 'ISOLATED_STAGING_CANONICAL_IMPLEMENTATION' },
      permitted_actions: {
        const: [
      ...
      review_findings_disposition: { const: 'ACKNOWLEDGED_NOT_RESOLVED_OR_WAIVED' },
      p1_p9_gate_state: { const: 'OPEN' },
      governance_diff: closedObject({
        basis_code_commit: codeCommit,
        authorised_code_commit: codeCommit,
        changed_paths: stringSet,
        allowed_path_set_digest: digest,
        patch_digest: digest,
        unexpected_paths: {
          type: 'array',
          maxItems: 0,
      ...
      github_actor_login: { const: 'CodeNameHash' },
      github_actor_id: { const: '264183176' },
      github_run_id: nonEmptyString,
      approval_intent: {
        const: 'AUTHORISE_ISOLATED_STAGING_CANONICAL_IMPLEMENTATION',
      },
      approver_identity: { const: 'BEN_GOODCHILD' },
      conditions: {
        type: 'array',
  ...
  'BenSpecificationApprovalEvidence/V2': {
    $id: 'BenSpecificationApprovalEvidence/V2',
    ...closedObject({
      approval_evidence_id: digest,
      approved_root: digest,
      approved_code_commit: codeCommit,
  ...
  'ContractFreezeAttestationIdentity/V1': {
    ...
    ...closedObject({
      ...
      predecessor_canonical_contract_bundle_member_root: digest,
  ...
  'PROCESS_VERTICAL_SLICE_ATTESTATION/V1': {
    $id: 'PROCESS_VERTICAL_SLICE_ATTESTATION/V1',
    ...closedObject({
      schema_version: { const: 'PROCESS_VERTICAL_SLICE_ATTESTATION/V1' },
      gate_id: { const: 'PROCESS_VERTICAL_SLICE_PASS' },
      status: { const: 'PASS' },
      attestation_id: digest,
      payload_digest: digest,
      frozen_pair_id: digest,
      canonical_contract_bundle_id: digest,
      canonical_contract_bundle_payload_digest: digest,
      agreement_shared_and_process_input_root: digest,
      code_commit: codeCommit,
      candidate_release_manifest_id: digest,
      candidate_release_manifest_payload_digest: digest,
      pm_data_version: nonEmptyString,
      product_field_catalogue_id: digest,
      product_field_catalogue_payload_digest: digest,
      navigation_catalogue_id: digest,
      navigation_catalogue_payload_digest: digest,
      source_universe_manifest_id: digest,
      source_universe_manifest_payload_digest: digest,
      sealed_gold_package_id: digest,
      sealed_gold_package_payload_digest: digest,
      release_scope_decision_id: digest,
      release_scope_decision_payload_digest: digest,
      benchmark_manifest_id: digest,
      benchmark_manifest_payload_digest: digest,
      acceptance_item_evidence: {
        ...
        items: closedObject({
          item_id: token,
          certification_id: digest,
        }),
      },
      predicate_certification_identities: digestSet,
      field_certification_identities: digestSet,
      adversarial_rejection_receipts: {
        type: 'array',
        minItems: 1,
        uniqueItems: true,
        items: closedObject({
          rejection_code: token,
          test_id: { const: 'VERTICAL-SLICE-01' },
          test_execution_id: digest,
        }),
      },
      attester_key_id: token,
      signature_algorithm: { const: 'Ed25519' },
  ...
  'ContractDiffReviewAttestation/V1': {
    ...
    ...closedObject({
      ...
      predecessor_canonical_contract_bundle_members: {
        ...
        minItems: 1,
        uniqueItems: true,
        items: immutableContractBundleMemberSchema,
      },
      contract_bundle_id: digest,
      contract_bundle_digest: digest,
      ...
      exact_review_input_schema_version: {
        const: 'CANONICAL_CONTRACT_BUNDLE_EXACT_REVIEW_INPUT/V2',
      },
      reviewed_contract_source_set_digest: digest,
      frozen_contract_pair_digest: digest,
