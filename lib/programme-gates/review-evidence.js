...
function verifyIndependence(attestation, record, authority, at) {
  ...
  if (attestation.reviewer_principal_id !== record.reviewer_principal_id
    ...
    || attestation.source_control_history_scope !== 'ALL_REFS_FROM_REPOSITORY_GENESIS'
...
function projectReviewSetAttestation(verification) {
  ...
  validateSchema('ExactDigestReviewSetAttestation/V1', attestation);
...
function projectBenApprovalEvidence(verification, record) {
  ...
  const evidence = {
    ...
    passing_review_set_evidence_id:
      verification.facts.passing_review_set_evidence_id,
    conditions: [...record.conditions],
  };
  validateSchema('BenSpecificationApprovalEvidence/V1', evidence);
