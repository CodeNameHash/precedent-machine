...
function runPilotIntegrationPreflight(input, cache = new Map()) {
  ...
  check(8, 'deployment metadata', input.candidate
    && /^[a-f0-9]{40}$/.test(input.candidate.head || '')
    && input.deployment && input.deployment.present === true
    && input.deployment.code_commit === input.candidate.head
