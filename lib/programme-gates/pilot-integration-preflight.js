...
function authorisedChangedPaths(actual, allowed) {
...
function authorisedPaths(allowlist) {
  if (!allowlist || typeof allowlist !== 'object' || Array.isArray(allowlist)) {
    throw new TypeError('phase allowlist must be an object');
  }
  const keys = ['allowed', 'allowedWrites'].filter((key) => Object.hasOwn(allowlist, key));
  if (keys.length !== 1 || !Array.isArray(allowlist[keys[0]])
    || allowlist[keys[0]].some((value) => typeof value !== 'string')
    || new Set(allowlist[keys[0]]).size !== allowlist[keys[0]].length) {
    throw new Error('phase allowlist has no closed permission set');
  }
  return Object.freeze([...allowlist[keys[0]]]);
}
...
function runPilotIntegrationPreflight(input, cache = new Map()) {
  ...
  check(3, 'allowed paths', input.allowlist_valid !== false && authorisedChangedPaths(input.changed_paths, input.allowed_paths)
    ? pass() : failure('PATH_VIOLATION', input.allowlist_valid === false
      ? 'an applicable phase allowlist has an unknown permission schema'
      : `unauthorised changed paths: ${(input.changed_paths || []).filter((value) => !(input.allowed_paths || []).includes(value)).join(', ')}`));
