...
function validateSource(source, authority) {
  ...
  if (source.schema_version !== 'ProgrammeGateIsolationObservationSource/V1'
    ...
    || ![401, 403, 302].includes(source.unauthenticated_status)
    || (source.unauthenticated_status === 302
      && source.unauthenticated_redirect_origin !== 'https://vercel.com/sso-api')
    || ([401, 403].includes(source.unauthenticated_status)
      && source.unauthenticated_redirect_origin !== null)
    || !Number.isInteger(source.authorised_status)
    || source.authorised_status < 200
    || source.authorised_status >= 300
...
function evidenceObjects(source) {
  return Object.freeze([
    ...
    Object.freeze({
      unauthenticated_status: source.unauthenticated_status,
      unauthenticated_redirect_origin: source.unauthenticated_redirect_origin,
      authorised_status: source.authorised_status,
    }),
  ]);
}
...
function buildIsolationReadiness(input) {
  requireExactKeys(
    input,
    ['authority', 'source', 'gateTestResult', 'deployCutoverTestResult'],
  ...
  const candidates = evidenceObjects(source).map((evidenceObject, index) => {
    ...
    const testResult = gate.required_adversarial_tests[0] === 'DEPLOY-CUTOVER-01'
      ? input.deployCutoverTestResult
    ...
    return Object.freeze({
      ...
      test_results: Object.freeze([testResult]),
