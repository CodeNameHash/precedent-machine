...
function validateSource(source) {
  ...
  requireTimestamp(source.observed_at, 'source observed_at');
  return source;
}
...
function evidenceObjects(source, authority, sourceDigest) {
  ...
  return Object.freeze([
    ...
    Object.freeze({
      ...
      replacement_activation_verified_at: source.observed_at,
      secret_field_count: 0,
    }),
    Object.freeze({
      ...
      rotation_verified_at: source.observed_at,
...
function buildSecurityDispositionReadiness(input) {
  ...
  if (Date.parse(source.observed_at) > Date.parse(authority.verificationTime)) {
    throw new Error('security-disposition source is newer than the trusted verification time');
  }
  validateTestResult(input.testResult, authority, source.observed_at);
