...
function contractDiffReviewIsMechanicallyBound(review, evidence, context) {
  ...
  const predecessorSourceFacts = sourceFacts(
...
function contractFreezeAttestationIdentityIsValid(evidence, context, identity, review) {
  ...
  const predecessorRecords = review?.predecessor_canonical_contract_bundle_members;
  if (!Array.isArray(predecessorRecords) || predecessorRecords.length === 0) return false;
