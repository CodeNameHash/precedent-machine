...
const RAW_EVIDENCE_KEYS = Object.freeze({
  ...
  G0_STAGING_ACCESS_PROTECTED: Object.freeze([
    ...
    'unauthenticated_redirect_origin',
    'authorised_status',
  ]),
  ...
  switch (gateId) {
    ...
    case 'G0_STAGING_ACCESS_PROTECTED':
      return Number.isInteger(evidence.unauthenticated_status)
        && (evidence.unauthenticated_redirect_origin === null
          || isNonEmptyString(evidence.unauthenticated_redirect_origin))
...
function unauthenticatedAccessDenied(evidence) {
  return evidence.unauthenticated_status === 401
    || evidence.unauthenticated_status === 403
    || (
      evidence.unauthenticated_status === 302
      && evidence.unauthenticated_redirect_origin === 'https://vercel.com/sso-api'
    );
}
...
function verifiedContractFreeze(evidence, context) {
  const verified = context && context.verifiedContractFreeze;
  return isRecord(verified)
    && evidence.specification_root === context.expectedSpecificationRoot
    && verified.contract_bundle_id === evidence.contract_bundle_id
    && verified.contract_bundle_digest === evidence.contract_bundle_digest
    && verified.frozen_contract_pair_digest === evidence.frozen_contract_pair_digest
    && verified.compilation_receipt_id === evidence.compilation_receipt_id
    && verified.semantic_identity_review_id === evidence.semantic_identity_review_id
    && verified.freeze_gate_approval_id === evidence.freeze_gate_approval_id
    && verified.status_generation === evidence.status_generation
    && verified.status_payload_digest === evidence.status_payload_digest;
}

function bundleCompiles(evidence, context) {
  return verifiedContractFreeze(evidence, context)
    && context.verifiedContractFreeze.bundle_compiles === true;
}

function semanticAndIdentityDiffReviewed(evidence, context) {
  return verifiedContractFreeze(evidence, context)
    && context.verifiedContractFreeze.semantic_and_identity_diff_reviewed === true;
}

function freezeGateApproved(evidence, context) {
  return verifiedContractFreeze(evidence, context)
    && context.verifiedContractFreeze.freeze_gate_approved === true;
}

function statusGenerationMatches(evidence, context) {
  return verifiedContractFreeze(evidence, context)
    && Number.isInteger(context.expectedStatusGeneration)
    && evidence.status_generation === context.expectedStatusGeneration
    && context.verifiedContractFreeze.status_generation_matches === true;
}
