...
function reviewSetFacts(evidence, context) {
  ...
  if (runtimeIdentities.size !== 1
    || laneMembers.some((member) => (
      ...
      || member.controller_record.reasoning_level !== REVIEW_CONTROLLER_POLICY.reasoning_level
    ))) return null;
...
function contractFreezeMembers(evidence, context) {
  ...
  return compilation && review && approval && status
    ? { compilation, review, approval, status }
    : null;
}
...
function bundleCompiles(evidence, context) {
  ...
  return Boolean(members)
    ...
    && members.compilation.output_root === evidence.contract_bundle_digest;
}
...
function semanticAndIdentityDiffReviewed(evidence, context) {
  ...
  return Boolean(members)
    ...
    && members.review.semantic_diff_reviewed === true
    && members.review.identity_diff_reviewed === true
