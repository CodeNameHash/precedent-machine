...
const NESTED_RAW_EVIDENCE_KEYS = Object.freeze({
  ...
  test: Object.freeze(['test_id', 'state']),
  route: Object.freeze([
    'route_id',
    'is_broad',
    'contained',
...
function methodProbeRecordsAreValid(probes) {
  return Array.isArray(probes)
    ...
    && probes.every((probe) => (
      ...
      && (probe.retry_after === null || isNonNegativeInteger(probe.retry_after))
    ))
    && new Set(probes.map((probe) => probe.method)).size === probes.length;
}
...
function routeRecordsAreValid(routes) {
  return Array.isArray(routes)
    ...
    && routes.every((route) => (
      ...
      && typeof route.is_broad === 'boolean'
      && typeof route.contained === 'boolean'
...
function liveRouteZeroCorpusReads(evidence) {
  return evidence.method_probes.every((probe) => (
    probe.status === 503
...
function containmentTestPass(evidence) {
  return evidence.tests.every((entry) => entry.state === 'PASS');
}
...
function everyBroadRouteContained(evidence) {
  return sameClosedStringSet(
    evidence.source_route_ids,
    evidence.routes.map((route) => route.route_id),
  )
    && evidence.routes.every((route) => route.is_broad === true && route.contained === true)
    && evidence.tests.every((entry) => entry.state === 'PASS');
}
