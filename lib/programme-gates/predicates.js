...
const G0_PERMITTED_ACTIONS = Object.freeze([
...
const G0_PROHIBITED_ACTIONS = Object.freeze([
  ...
  switch (gateId) {
    ...
    case 'G0_EXACT_DIGEST_REVIEW_SET':
      return isDigest(evidence.review_set_evidence_id)
        ...
        && isCommit(evidence.reviewed_code_commit)
        && isDigest(evidence.review_artifact_sha256)
        && Number.isInteger(evidence.review_artifact_byte_size)
        && evidence.review_artifact_byte_size > 0
        && Array.isArray(evidence.lane_outcomes)
        && evidence.lane_outcomes.length === REVIEW_LANES.length
        && ['PASS', 'FAIL'].includes(evidence.full_review_disposition)
        && evidence.full_review_pass_claimed === false;
    case 'G0_BEN_SPEC_APPROVAL':
      return isDigest(evidence.approval_evidence_id)
        && isDigest(evidence.approved_root)
        && isCommit(evidence.approved_code_commit)
        && isDigest(evidence.review_set_evidence_id)
        && isDigest(evidence.reviewed_root)
        && isCommit(evidence.reviewed_code_commit)
        && isDigest(evidence.review_artifact_sha256)
        && Number.isInteger(evidence.review_artifact_byte_size)
        && evidence.full_review_pass_claimed === false
        && evidence.authorisation_scope === 'ISOLATED_STAGING_CANONICAL_IMPLEMENTATION'
        && Array.isArray(evidence.permitted_actions)
        && Array.isArray(evidence.prohibited_actions)
        && evidence.review_findings_disposition === 'ACKNOWLEDGED_NOT_RESOLVED_OR_WAIVED'
        && evidence.p1_p9_gate_state === 'OPEN'
        && isDigest(evidence.governance_diff_digest)
        && evidence.github_actor_login === 'CodeNameHash'
        && evidence.github_actor_id === '264183176'
        && isNonEmptyString(evidence.github_run_id)
        && evidence.approval_intent
...
function processVerticalSliceEvidenceIsInternallyBound(evidence) {
  if (!evidence || typeof evidence !== 'object' || Array.isArray(evidence)) return false;
  try {
    validateSchema(PROCESS_VERTICAL_SLICE_GATE_BINDING.evidence_schema_id, evidence);
  } catch {
    return false;
  }
  ...
  if (evidence.attestation_id !== contextlessDomainDigest(
    'PROCESS_VERTICAL_SLICE_ATTESTATION_ID/V1',
    unsigned,
  ) || evidence.payload_digest !== contextlessDomainDigest(
    'PROCESS_VERTICAL_SLICE_ATTESTATION_PAYLOAD/V1',
    unsigned,
  )) return false;
  if (!sameClosedStringSet(
    evidence.acceptance_item_evidence.map((entry) => entry.item_id),
    PROCESS_VERTICAL_SLICE_GATE_BINDING.required_acceptance_items,
  ) || !sameClosedStringSet(
    evidence.adversarial_rejection_receipts.map((entry) => entry.rejection_code),
    PROCESS_VERTICAL_SLICE_GATE_BINDING.required_adversarial_rejections,
  )) return false;
  return evidence.predicate_certification_identities.length > 0
    && evidence.field_certification_identities.length > 0;
}

function processVerticalSliceEvidenceMatchesRelease(evidence, release) {
  if (!processVerticalSliceEvidenceIsInternallyBound(evidence)
    || !release || typeof release !== 'object' || Array.isArray(release)) return false;
  const bindings = [
    'frozen_pair_id',
    'canonical_contract_bundle_id',
    'canonical_contract_bundle_payload_digest',
    'agreement_shared_and_process_input_root',
    'code_commit',
    ...
    'source_universe_manifest_payload_digest',
    'sealed_gold_package_id',
    'sealed_gold_package_payload_digest',
    'release_scope_decision_id',
    'release_scope_decision_payload_digest',
    'benchmark_manifest_id',
    'benchmark_manifest_payload_digest',
  ];
  return bindings.every((key) => evidence[key] === release[key]);
}

function processVerticalSliceEvidenceCanExecute(evidence) {
  return PROCESS_VERTICAL_SLICE_GATE_BINDING.evidence_execution_state === 'ACTIVE'
    && processVerticalSliceEvidenceIsInternallyBound(evidence);
}
...
function reviewSetFacts(evidence, context) {
  ...
  const legacyStrictReview = !Object.hasOwn(evidence, 'reviewed_code_commit');
  const reviewContext = {
    ...context,
    expectedSpecificationRoot: evidence.reviewed_root,
    expectedCodeCommit: legacyStrictReview
      ? context.expectedCodeCommit
      : evidence.reviewed_code_commit,
  };
  const laneMembers = [];
  ...
  for (const lane of REVIEW_LANES) {
    ...
    const reviewOutput = legacyStrictReview
      ? null
      : memberFor(
        context,
        `output:${lane.lane_id}`,
    ...
    if (!record
      || (!legacyStrictReview && !reviewOutput)
      || !independence
      ...
      || record.cold_review_prompt_digest
        !== REVIEW_CONTROLLER_POLICY.prompt_digests[lane.lane_id]
      || !frozenReviewContextIsValid(record, lane, reviewContext)
      || record.exact_specification_root !== evidence.reviewed_root
      || (legacyStrictReview
        ? record.reviewer_disposition !== 'PASS'
        : (
          record.reviewer_disposition !== reviewOutput.disposition
          || record.review_output_digest
            !== context.domainDigest('PROGRAMME_GATE_REVIEW_OUTPUT/V1', reviewOutput)
        ))
      ...
      || !reviewIndependenceIsRecomputed(record, independence, reviewContext)
    ...
    laneMembers.push({
      ...
      ...(legacyStrictReview ? {} : { review_output: reviewOutput }),
  ...
  if (legacyStrictReview) {
    const legacyEvidenceId = context.domainDigest(
      'PROGRAMME_GATE_REVIEW_SET/V1',
      laneMembers,
    );
    if (evidence.review_set_evidence_id !== legacyEvidenceId
      || evidence.reviewed_root !== context.expectedSpecificationRoot) return null;
    return {
      laneMembers,
      laneOutcomes: laneMembers.map((member) => ({
        lane_id: member.lane_id,
        disposition: 'PASS',
      })),
      fullReviewDisposition: 'PASS',
      legalReviewerEligible: laneMembers.some((member) => (
        member.lane_id === 'LEGAL_SEMANTIC'
      )),
    };
  }
  const laneOutcomes = laneMembers.map((member) => ({
    lane_id: member.lane_id,
  ...
  const fullReviewDisposition = laneOutcomes.every(
    (outcome) => outcome.disposition === 'PASS',
  ) ? 'PASS' : 'FAIL';
  const evidenceId = context.domainDigest('PROGRAMME_GATE_REVIEW_OUTCOME_SET/V1', {
    reviewed_code_commit: evidence.reviewed_code_commit,
    reviewed_root: evidence.reviewed_root,
    review_artifact_sha256: evidence.review_artifact_sha256,
    review_artifact_byte_size: evidence.review_artifact_byte_size,
    ordered_members: laneMembers,
  });
  if (evidence.review_set_evidence_id !== evidenceId
    || evidence.full_review_pass_claimed !== false
    || evidence.full_review_disposition !== fullReviewDisposition
    || context.domainDigest(
      'PROGRAMME_GATE_REVIEW_LANE_OUTCOME_SET/V1',
      evidence.lane_outcomes,
    ) !== context.domainDigest(
      'PROGRAMME_GATE_REVIEW_LANE_OUTCOME_SET/V1',
      laneOutcomes,
    )) return null;
...
function passingReviewSetFacts(evidence, context) {
  const facts = reviewSetFacts(evidence, context);
  if (!facts
    || facts.fullReviewDisposition !== 'PASS'
    || facts.laneOutcomes.some((lane) => lane.disposition !== 'PASS')) return null;
  return facts;
}

function fiveNamedLaneOutcomesRecordedSameRoot(evidence, context) {
  return reviewSetFacts(evidence, context) !== null;
}

function reviewOutputsMatchSignedControllerDigests(evidence, context) {
  return reviewSetFacts(evidence, context) !== null;
}
...
function benApprovalFacts(evidence, context) {
  if (!isRecord(context) || typeof context.domainDigest !== 'function') return null;
  if (Object.hasOwn(evidence, 'passing_review_set_evidence_id')) {
    const legacyRecord = memberFor(
      context,
      `approval:${evidence.approval_evidence_id}`,
      'BenSpecificationApproval',
    );
    const legacyReviewSet = memberFor(
      context,
    ...
    if (!legacyRecord
      || !legacyReviewSet
      || legacyRecord.approver_identity !== 'BEN_GOODCHILD'
      || legacyRecord.approved_root !== context.expectedSpecificationRoot
      || legacyRecord.approved_root !== evidence.approved_root
      || legacyRecord.passing_review_set_evidence_id
        !== evidence.passing_review_set_evidence_id
      || legacyReviewSet.review_set_evidence_id
        !== evidence.passing_review_set_evidence_id
      || legacyReviewSet.reviewed_root !== evidence.approved_root
      || legacyRecord.conditions.length !== 0
      || evidence.conditions.length !== 0
      || context.domainDigest('PROGRAMME_GATE_BEN_APPROVAL_ID/V1', legacyRecord)
        !== evidence.approval_evidence_id
      || !signatureIsValid(context, {
        record: legacyRecord,
        signatureField: 'signature',
        keyId: legacyRecord.approver_key_id,
        role: 'BEN_APPROVER',
        domain: 'PROGRAMME_GATE_BEN_APPROVAL/V1',
      })) return null;
    return { record: legacyRecord, reviewSet: legacyReviewSet };
  }
  const record = memberFor(
    context,
    `approval:${evidence.approval_evidence_id}`,
  ...
  const reviewSet = memberFor(
    context,
    `review-set:${evidence.review_set_evidence_id}`,
    'ExactDigestReviewSetAttestation',
  );
  if (!record
    || !reviewSet
    ...
    || record.approved_code_commit !== context.expectedCodeCommit
    || record.approved_code_commit !== evidence.approved_code_commit
    || record.review_set_evidence_id !== evidence.review_set_evidence_id
    || reviewSet.review_set_evidence_id !== evidence.review_set_evidence_id
    || record.reviewed_root !== reviewSet.reviewed_root
    || record.reviewed_code_commit !== reviewSet.reviewed_code_commit
    || record.review_artifact_sha256 !== reviewSet.review_artifact_sha256
    || record.review_artifact_byte_size !== reviewSet.review_artifact_byte_size
    || evidence.reviewed_root !== reviewSet.reviewed_root
    || evidence.reviewed_code_commit !== reviewSet.reviewed_code_commit
    || evidence.review_artifact_sha256 !== reviewSet.review_artifact_sha256
    || evidence.review_artifact_byte_size !== reviewSet.review_artifact_byte_size
    || record.full_review_pass_claimed !== false
    || evidence.full_review_pass_claimed !== false
    || reviewSet.full_review_pass_claimed !== false
    || record.authorisation_scope !== 'ISOLATED_STAGING_CANONICAL_IMPLEMENTATION'
    || JSON.stringify(record.permitted_actions) !== JSON.stringify(G0_PERMITTED_ACTIONS)
    || JSON.stringify(record.prohibited_actions) !== JSON.stringify(G0_PROHIBITED_ACTIONS)
    || JSON.stringify(evidence.permitted_actions) !== JSON.stringify(G0_PERMITTED_ACTIONS)
    || JSON.stringify(evidence.prohibited_actions) !== JSON.stringify(G0_PROHIBITED_ACTIONS)
    ...
    || record.approval_intent
      !== 'AUTHORISE_ISOLATED_STAGING_CANONICAL_IMPLEMENTATION'
    || context.domainDigest('PROGRAMME_GATE_GOVERNANCE_DIFF/V1', record.governance_diff)
      !== evidence.governance_diff_digest
...
function approvedRootAndCommitExact(evidence, context) {
  return benApprovalFacts(evidence, context) !== null;
}

function reviewOutcomeSetAcknowledged(evidence, context) {
  return benApprovalFacts(evidence, context) !== null;
}
...
function authorisationScopeBounded(evidence, context) {
  return benApprovalFacts(evidence, context) !== null;
}

function fullReviewPassNotClaimed(evidence, context) {
  if (evidence?.full_review_pass_claimed !== false) return false;
  if (evidence?.approval_evidence_id) return benApprovalFacts(evidence, context) !== null;
  return reviewSetFacts(evidence, context) !== null;
}
...
function g0EnvelopeFacts({
  ...
  const orderedClaimKeys = descriptor.ordered_claim_keys;
  if (!envelope
    ...
    || envelope.exact_acceptance_claims.length !== orderedClaimKeys.length
    || envelope.exact_acceptance_claims.some(
      (claim, index) => claim.claim_key !== orderedClaimKeys[index],
    )
...
function contractFreezeAuthorityIsComplete(evidence, context, manifest) {
  ...
  if (!g0Review
    ...
    || (
      g0Approval.review_set_evidence_id
        ?? g0Approval.passing_review_set_evidence_id
    ) !== g0Review.review_set_evidence_id
    || g0Approval.conditions.length !== 0) return false;

  const reviewFacts = passingReviewSetFacts(g0Review, context);
...
function canonicalPredecessorSourceFacts(records) {
  try {
    const facts = validateCanonicalPredecessorBundleMembers(records);
    if (!facts.source_kinds.every((sourceKind) => (
      sourceKind === 'CANONICAL_AGGREGATE'
      || sourceKind === 'LEGACY_F1'
    ))) return null;
    return {
      records: facts.members,
      projection: facts.projection,
      sourceKinds: facts.source_kinds,
    };
  } catch {
    return null;
  }
...
function contractDiffReviewIsMechanicallyBound(review, evidence, context) {
  ...
  const sourceFacts = (records) => {
    ...
    for (const record of records) {
      ...
      const bytes = Buffer.from(record.source_bytes_base64, 'base64');
      if (
        bytes.toString('base64') !== record.source_bytes_base64
    ...
    const orderedKeys = [...memberKeys].sort((left, right) => left.localeCompare(right));
    if (
      new Set(memberKeys).size !== memberKeys.length
      || JSON.stringify(memberKeys) !== JSON.stringify(orderedKeys)
    ) return null;
    return { records, projection };
  };
  const predecessorSourceFacts = canonicalPredecessorSourceFacts(
    review.predecessor_canonical_contract_bundle_members,
  );
  const successorSourceFacts = sourceFacts(
    review.canonical_contract_bundle_members,
  );
  const activeBundleFacts = canonicalContractBundleFacts(context);
  if (!predecessorSourceFacts || !successorSourceFacts || !activeBundleFacts) return false;
  const projectionDigest = (projection) => context.domainDigest(
    'PROGRAMME_GATE_CONTRACT_DIFF_REVIEW_PROJECTION/V1',
    projection,
  );
  if (
    projectionDigest(predecessorSourceFacts.projection)
      !== projectionDigest(review.predecessor_contract_members)
    || projectionDigest(successorSourceFacts.projection)
      !== projectionDigest(review.contract_bundle_members)
    || context.domainDigest(
      'PROGRAMME_GATE_CANONICAL_CONTRACT_BUNDLE_MEMBER_ROOT/V1',
      successorSourceFacts.records,
    ) !== activeBundleFacts.memberRoot
  ) return false;
  ...
  const reviewedContractSourceSetDigest = context.domainDigest(
    'PROGRAMME_GATE_CONTRACT_DIFF_REVIEW_SOURCE_SET/V1',
    {
      ...
      predecessor_canonical_contract_bundle_members:
        predecessorSourceFacts.records,
      canonical_contract_bundle_members: successorSourceFacts.records,
    },
  );
  ...
  return review.predecessor_contract_bundle_digest === predecessorDigest
    ...
    && review.reviewed_contract_source_set_digest === reviewedContractSourceSetDigest
    && review.exact_input_context_digest === exactInputContextDigest;
}

function contractFreezeAttestationIdentityIsValid(evidence, context, identity, review) {
  try {
  ...
  if (!predecessorSourceFacts) return false;
  const predecessorMemberRoot = context.domainDigest(
    'PROGRAMME_GATE_CANONICAL_CONTRACT_BUNDLE_MEMBER_ROOT/V1',
    predecessorSourceFacts.records,
  );
  return identity.contract_freeze_attestation_id === expectedId
    ...
    && identity.predecessor_canonical_contract_bundle_member_root === predecessorMemberRoot
    && identity.predecessor_canonical_contract_bundle_member_count
      === predecessorSourceFacts.records.length
...
const ACCEPTANCE_PREDICATES = Object.freeze({
  ...
  G0_EXACT_DIGEST_REVIEW_SET: Object.freeze({
    five_named_lane_outcomes_recorded_same_root: fiveNamedLaneOutcomesRecordedSameRoot,
    review_outputs_match_signed_controller_digests:
      reviewOutputsMatchSignedControllerDigests,
    eligible_legal_reviewer: eligibleLegalReviewer,
    reviewer_independence_recomputed: reviewerIndependenceRecomputed,
    full_review_pass_not_claimed: fullReviewPassNotClaimed,
  }),
  G0_BEN_SPEC_APPROVAL: Object.freeze({
    approved_root_and_commit_exact: approvedRootAndCommitExact,
    review_outcome_set_acknowledged: reviewOutcomeSetAcknowledged,
    ben_identity_and_signature_valid: benIdentityAndSignatureValid,
    authorisation_scope_bounded: authorisationScopeBounded,
    full_review_pass_not_claimed: fullReviewPassNotClaimed,
  }),
  P1_CONTRACT_FREEZE_ATTESTED: Object.freeze({
...
module.exports = {
  ...
  featureGateOff,
  fiveNamedLaneOutcomesRecordedSameRoot,
  fullReviewPassNotClaimed,
  ...
  reviewOutcomeSetAcknowledged,
  reviewOutputsMatchSignedControllerDigests,
  authorisationScopeBounded,
  rotationVerifiedOrRecognisedTrafficNaWithBenApproval,
