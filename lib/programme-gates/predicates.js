...
function catalogueAuthorityChainIsValid(authorities, prefix) {
...
function contractAuthorityTopologyIsValid(authorities) {
  if (!catalogueAuthorityChainIsValid(authorities, 'SEMANTIC_QUESTION')
    || !catalogueAuthorityChainIsValid(authorities, 'COMPOSITION')) return false;
  ...
  const stage = oneAuthority(authorities, 'PRE_FREEZE_SEMANTIC_STAGE_OUTPUT_SET');
  ...
  return Boolean(
    reconciliation && neutral && stage && projected
    ...
    && stage.authority_payload.input_projection_digest
      === neutral.authority_payload_digest
    && exactStringSet(stage.related_authority_ids, [neutral.authority_evidence_id])
    && projected.authority_payload.semantic_stage_output_set_root
      === stage.authority_payload_digest
    && exactStringSet(projected.related_authority_ids, [stage.authority_evidence_id])
...
function contractFreezeAuthorityIsComplete(evidence, context, manifest) {
  ...
  if (authorities.some((member) => !authorityRecordIsValid(context, member))
    || !contractAuthorityTopologyIsValid(authorities)) return false;
  ...
  const eligibleIdentities = reviewerEligibility.payload.authority_payload.eligible_reviewers
    .map((reviewer) => reviewer.reviewer_identity);
...
function contractReviewerEligibilityIsBound(review, context) {
  ...
  return Boolean(eligibility)
    && eligibility.authority_payload_digest === review.reviewer_eligibility_digest
