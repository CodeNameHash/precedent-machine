...
function recomputeReviewApprovalVerifications(authority, bundle) {
  ...
  const reviewVerification = verifyReviewSetEvidence({
    expected_code_commit: reviewCandidate.evidence_object.reviewed_code_commit,
    expected_specification_root: reviewCandidate.evidence_object.reviewed_root,
    review_artifact_byte_size: reviewCandidate.evidence_object.review_artifact_byte_size,
    review_artifact_sha256: reviewCandidate.evidence_object.review_artifact_sha256,
  ...
  const approvalVerification = verifyBenApprovalEvidence({
    ...
    expected_code_commit: bundle.code_commit,
  ...
  if (bundle.review_set_evidence_id !== reviewVerification.evidence_id
    ...
    throw new Error('review or approval evidence identity has drifted');
  }
  return true;
}
...
function validateCandidate({
  ...
  if (candidate.evidence_subject.gate_id !== gate.id
    ...
    || (
      ...
      && (
        ...
        || candidate.evidence_subject.reviewed_code_commit
          !== candidate.evidence_object.reviewed_code_commit
    ...
    || (
      ...
      && (
        ...
        || candidate.evidence_subject.approved_code_commit
          !== candidate.evidence_object.approved_code_commit
  ...
  const recomputedClaims = evaluateAcceptanceClaims({
    ...
    context: {
      ...
      immutableMembers: orderedMembers,
...
function buildReviewApprovalSigningRequest(input) {
  ...
  recomputeReviewApprovalVerifications(authority, bundle);
...
function preflightReviewApprovalSignature(input) {
  ...
  try {
    ...
    recomputeReviewApprovalVerifications(authority, input.bundle);
  } catch {
    return deepFreeze({
