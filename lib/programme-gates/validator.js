...
} = require('./test-execution-attestation');
...
function validateTests({
  ...
  if (!clock || typeof clock.now !== 'function') {
    fail(
      ...
      'a trusted clock is required to authenticate adversarial test executions',
    );
  }
  ...
  try {
    ...
    fail(
      'ADVERSARIAL_TEST_ATTESTATION_NOT_TRUSTED',
      'trusted clock could not authenticate adversarial test executions',
    );
  }
  for (const entry of testResults) {
    ...
    if (entry.attester_key_id !== envelope.validator_key_id) {
      fail(
        'ADVERSARIAL_TEST_ATTESTATION_NOT_TRUSTED',
        'adversarial test execution attester does not match the evidence validator',
      );
    }
    try {
      const verified = verifySignature({
        keyRegistry,
        keyId: entry.attester_key_id,
        role: TEST_EXECUTION_SIGNATURE_ROLE,
        domain: TEST_EXECUTION_SIGNATURE_DOMAIN,
        payload: unsignedTestExecutionRecord(entry),
        signature: entry.execution_signature,
        at: verificationTime,
      });
      if (verified !== true) {
        fail(
          'ADVERSARIAL_TEST_ATTESTATION_NOT_TRUSTED',
          'adversarial test execution signature is not trusted',
        );
      }
    } catch {
      fail(
