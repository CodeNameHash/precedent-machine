...
function validateFreezeCandidate(candidate, successor, frozenPairDigest) {
  ...
  if (
    ...
    fail(
      'INVALID_FREEZE_CANDIDATE',
      'The freeze candidate roots do not match the exact governed aggregates.',
    );
  }
  ...
  const compilationPayload = {
    ...
    canonical_contract_bundle_projection: successor.projection,
  ...
  const expectedReceipt = {
    ...
    canonical_contract_bundle_member_root: memberRoot,
    canonical_contract_bundle_member_count: successor.records.length,
    canonical_contract_bundle_required_kind_set_root: requiredKindRoot,
    compile_errors: [],
    cycle_errors: [],
    drift_errors: [],
...
function validateExactReviewPackage(input) {
  ...
  const successor = validateSuccessorAggregates(
    packageValue.canonical_contract_bundle_members,
  );
  const successorProjection = successor.projection;
  if (
...
function validateP1ContractFreezeReviewResults({
  ...
  return deepFreeze({
    ...
    reason_code: 'P1_SIGNED_REVIEW_REGISTRATION_UNAVAILABLE',
    reason:
    ...
    result_authentication: 'NOT_REGISTERED',
