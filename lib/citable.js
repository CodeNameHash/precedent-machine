...
import { taxonomyForFeatureKey, labelForCode } from './taxonomy.js';
...
export function getCitableQuotes(v) {
  ...
  if (Array.isArray(v.quotes)) {
    const cleaned = v.quotes
      .filter((q) => typeof q === 'string')
      .map((q) => q.trim())
      .filter(Boolean);
    // Item 10: stored tagged items are almost always `quotes: []` (empty,
    // not absent) with the real per-item verbatim sitting on `.text`. The
    // old code only fell through to `.text` when `quotes` was ABSENT, so an
    // empty array short-circuited straight to nothing -- and callers fell
    // all the way back to the whole card's primary_quote/region_full_text
    // (the head of the definition/clause) instead of this item's own text.
...
// Item 10: per-item quote for a TAGGED item ({code, label, text, quotes}),
// used ahead of the generic citable/source fallbacks so a pill's own
// verbatim wins over the card-level evidence (the whole definition's head).
// Never returns an "echo" -- `.text` that's merely the code or label
// repeated back verbatim (some backfills store the code as `.text` when no
// distinguishing quote was captured), since that carries no evidentiary
// value over just showing the label.
export function getTaggedItemQuote(item) {
  if (!isTaggedItem(item)) return null;
  if (Array.isArray(item.quotes)) {
    const hit = item.quotes.find((q) => typeof q === 'string' && q.trim());
    if (hit) return hit.trim();
  }
  if (typeof item.text === 'string') {
    const t = item.text.trim();
    if (t && t !== String(item.code || '').trim() && t !== String(item.label || '').trim()) return t;
  }
  return null;
}
...
/* ── evidenceHover — GD 11 repeat fix ──
 *  Root cause: a table cell showing a DERIVED value (Yes/No, a pill, a
 *  resolved label) hovers a HoverSource popover whose `quote` is set to the
 *  evidence excerpt itself, with no `highlight` — so the popover body IS the
 *  entire quote and there is nothing to bold against (the earlier "empty
 *  cell" bolding fix only ever wired `highlight` through for the fallback
 *  full_text path, never for a populated citable/derived value).
 *
 *  Fix: when the resolved evidence excerpt is a genuine, strictly-shorter
 *  substring of the owning provision's full_text (true for both a real
 *  citable quote AND the sentence-focused full_text fallback), render the
 *  FULL provision text as the popover body with the excerpt bolded inside
 *  it — the reader sees the supporting phrase in its surrounding context
 *  instead of an undifferentiated blob. Falls back to the bare excerpt
 *  (no wider context available, nothing to bold) when there's no provision
 *  full_text, or the excerpt already equals it verbatim. Never invents
 *  text — the highlight is always a literal substring of the quote shown. */
export function evidenceHover(rawValue, opts = {}) {
  const { provision = null } = opts;
  const { primaryQuote } = resolveEvidence(rawValue, opts);
  if (!primaryQuote) return { quote: null, highlight: null, primaryQuote: null };
  const full = provision && typeof provision.full_text === 'string' ? provision.full_text.trim() : '';
  if (full && full.length > primaryQuote.length && full.includes(primaryQuote)) {
    return { quote: full, highlight: primaryQuote, primaryQuote };
  }
  return { quote: primaryQuote, highlight: null, primaryQuote };
}
