...
/* The producer's comparison_group enum reaches the review page verbatim when
 * the model supplies it, so a reviewer sees TARGET_PRE_CLOSING rather than
 * "Company pre-closing arrangements" — the human label that already exists two
 * lines above and is only reachable through the standard_code fallback.
 * Reported by Ben, 2026-08-10, reviewing rendered rows: "can we tidy up the
 * machine text of TARGET_PRE_CLOSING". Maps the enum onto the SAME two
 * owner-specified groups; anything unrecognised is returned unchanged rather
 * than guessed at, so a new enum value shows up as itself instead of being
 * silently relabelled as one of these two. */
const COMPARISON_GROUP_LABELS = new Map([
  ['TARGET_PRE_CLOSING', COMPARISON_GROUP_PRE_CLOSING],
  ['COMPANY_PRE_CLOSING', COMPARISON_GROUP_PRE_CLOSING],
  ['BUYER_SIMILARLY_SITUATED', COMPARISON_GROUP_BUYER_EMPLOYEES],
  ['SIMILARLY_SITUATED_BUYER', COMPARISON_GROUP_BUYER_EMPLOYEES],
]);

export function comparisonGroupLabel(rawGroup) {
  if (!rawGroup) return null;
  const raw = String(rawGroup).trim();
  return COMPARISON_GROUP_LABELS.get(raw.toUpperCase()) || raw;
}
