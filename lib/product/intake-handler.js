...
const { activeSubmissionVersion } = require('./legal-schema-selection');
...
function createProductIntakeHandler({
  ...
  return async function productIntakeHandler(req, res) {
    ...
    try {
      ...
      // The server decides the schema and prompt bundle for a new submission
      // (lib/product/legal-schema-selection.js); client-sent values are
      // ignored so a stale bundle cannot start a run on the wrong schema.
      const submissionVersion = activeSubmissionVersion();
      // Runtime log line for the private preview: shows which schema the
      // server chose and whether the selection variables reached the runtime.
      console.log(`product intake schema ${submissionVersion.schemaVersion} (NEXT_PUBLIC set: ${Boolean(process.env.NEXT_PUBLIC_PRODUCT_LEGAL_SCHEMA_VERSION)}, PRODUCT set: ${Boolean(process.env.PRODUCT_LEGAL_SCHEMA_VERSION)})`);
      const run = await foundation.submit({ ...req.body, ...submissionVersion, modelConfig: modelConfigResolver() });
