...
function rowLabelProblem(table, rowLabel) {
  if (table.rows_are !== 'fixed list') return null;
  const legal = [
...
function validateFactConclusions(fact, { tableShapes } = {}) {
  ...
  conclusions.cells.forEach((cell, index) => {
    ...
    const fillFrom = new Set(column.fill_from || []);
    ...
    for (const componentId of componentIds) {
      ...
      if (!fillFrom.has(component.kind)) {
        problems.push(`${where}: component ${componentId} kind ${component.kind} is outside ${cell.column_id}'s fill_from`);
        continue;
      }
    ...
    if (column.render === 'vocabulary') {
      ...
      if (!cell.value || typeof cell.value !== 'object' || cell.value.canonical === undefined) {
        ...
        const parsed = parseComponentValue(component.kind, component.text);
      ...
      else if (!citedComponents.some((component) => component.text === cell.text)) {
        problems.push(`${where}: text does not match any cited component's verbatim text`);
      }
...
function renderConclusionCells(fact, tableShapes) {
  ...
  if (!resolved) return [];
  const { table } = resolved;
  const cellsByColumn = new Map((fact.conclusions?.cells || []).map((cell) => [cell.column_id, cell]));
