...
function defaultConclusions(fact, tableShapes) {
  ...
  // The table's as-drafted column (display fact_text) shows the fact's
  // summary, so the defaulted readout cites the fact's own components for
  // it (generation 7, 5.01: the specific-restrictions carve-out row read
  // blank).
  const drafted = (table.columns || []).find((column) => column.display === 'fact_text');
  const own = [...walk(fact.components || [])].map(([component]) => component).filter((component) => component && (component.origin === 'OWN' || !component.origin) && component.component_id);
  const cells = drafted && own.length ? [{ column_id: drafted.column_id, text: own[0].text, component_ids: own.map((component) => component.component_id) }] : [];
  return { table_key: table.table_key, row_label: rowOf(table), cells };
}

// The codes only the intro row (General Exceptions) of a table may carry:
// the codes of its own list that no other row's list names.
function introRowOnlyCodes(table, column) {
  const introRow = table?.intro_facts_row;
  const byRow = column?.vocabulary_by_row;
  if (!introRow || !byRow || !Array.isArray(byRow[introRow])) return new Set();
  const elsewhere = new Set(Object.entries(byRow).filter(([row]) => row !== introRow).flatMap(([, codes]) => codes));
  return new Set(byRow[introRow].filter((code) => !elsewhere.has(code)));
}

// A representation row never repeats the article introduction's exceptions
// ("Except as disclosed in SEC filings" on Organization; Metsera generation
// 7, 3.01): those codes belong to the General Exceptions row, which Ben
// moved them to ("move it over to what we had in the old version"). Cells
// carrying one on any other row are dropped; the dropped codes are returned
// so the extractor can note them (CONCLUSIONS_CELLS_DROPPED).
function stripIntroRowCodes(conclusions, table) {
  if (!conclusions || !Array.isArray(conclusions.cells) || !table?.intro_facts_row) return { conclusions, dropped: [] };
  if (conclusions.row_label === table.intro_facts_row) return { conclusions, dropped: [] };
  const dropped = [];
  const cells = conclusions.cells.filter((cell) => {
    const column = (table.columns || []).find((candidate) => candidate.column_id === cell?.column_id);
    if (!column || !cell.code || !introRowOnlyCodes(table, column).has(cell.code)) return true;
    dropped.push({ column_id: cell.column_id, code: cell.code });
    return false;
  });
  return { conclusions: dropped.length ? { ...conclusions, cells } : conclusions, dropped };
}
...
function formatValue(value, valueKind) {
  ...
  // "$47.50", not "$47.5"; a whole-dollar amount stays "$190,000,000"
  // (generation 7, 2.01).
  if (valueKind === 'AMOUNT') {
    const amount = Number(value.canonical);
    return `$${amount.toLocaleString('en-US', Number.isInteger(amount) ? {} : { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  }
...
module.exports = {
  defaultConclusions, stripIntroRowCodes, introRowOnlyCodes,
  contract,
  columnVocabulary,
  findTableByKey,
  tablesForFamily,
