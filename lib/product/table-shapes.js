...
function validateColumnV3(column, where, seenColumnIds, componentKindSet, sharedVocabularyIds) {
  ...
  if (column.display !== undefined && !['resolved_reference', 'fact_text', 'party'].includes(column.display)) fail('TABLE_SHAPES_COLUMN_DISPLAY', `${where}: ${column.display}`);
  ...
  if (column.derived !== undefined) {
    ...
    if (!['cross_reference', 'presence'].includes(spec.join)) fail('TABLE_SHAPES_DERIVED_JOIN', `${where}: ${spec.join}`);
