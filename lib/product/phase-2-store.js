...
// Attaches `conclusions` to proposals with a stored conclusions row (Part 3,
// FACT_CONCLUSIONS/V1); every other proposal reads back unchanged.
function withFactConclusions(proposalPayloads, conclusionRowsByProposal) {
  return proposalPayloads.map((payload) => {
    const { conclusions, ...rest } = payload;
