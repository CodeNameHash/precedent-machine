...
const { RESIDUAL_PASS_VERSION, SECTION_DRAFT_VERSION } = require('./agreement-draft');
...
class ProductPhase2Store extends ProductPhase1Store {
  ...
  async loadCompletedSectionResults(runId) {
    ...
    return rows.product_section_results.sort((left, right) => {
      ...
      const residualCall = modelCalls.find((call) => call.call_kind === 'RESIDUAL');
      if (!residualCall || !Array.isArray(residualCall.response?.paragraphs)) {
      ...
      const residualBody = {
        ...
        model_call_id: residualCall.model_call_id,
        dispositions: residualCall.response.paragraphs.map((item, paragraphIndex) => ({
      ...
      return {
        ...
        residual_pass: { ...residualBody, residual_pass_id: contentId(RESIDUAL_PASS_VERSION, residualBody) },
        spans: sectionSpans,
