...
const { validateFactComponents, walk, VALUE_KINDS } = require('./fact-components');
const { validateFactConclusions } = require('./fact-conclusions');
...
const RESULT_TABLE_ORDER = Object.freeze({
  ...
  // Every order must be total within a run (it ends in the table's own
  // key): loadCompletedSectionResults pages with range(), and Postgres may
  // order tied rows differently on each page. Metsera generation 7,
  // 2026-09-14: component rows were paged by (proposal_id, ordinal) alone;
  // a child and a top-level sibling share an ordinal, the tie fell across
  // the 2500-row page boundary, one row came back twice and one never, and
  // draft finalisation failed with DRAFT_NESTED_IDENTITY on that proposal.
  product_fact_components: ['proposal_id', 'ordinal', 'component_id'],
  product_fact_headlines: ['proposal_id'],
  product_fact_conclusions: ['proposal_id'],
});
...
function makeFactConclusionsIssue({ problems, familyKey, subtypeKey, nodeId, proposalId, sourceClosureId, sourceSpanIds }) {
  const body = {
    schema_version: ISSUE_VERSION,
    kind: 'NOTE',
    code: 'CONCLUSIONS_DROPPED',
    message: JSON.stringify(problems),
    family_key: familyKey ?? null,
    subtype_key: subtypeKey ?? null,
    structure_node_id: nodeId ?? null,
...
function prepareSectionResultForCommit(result) {
  ...
  const proposals = result.proposals.map((proposal) => {
    ...
    // summary (nullable column, migration 20260914140000): the extractor's
    // one-line summary of the provision, written with the headline so the
    // Other provisions table reads it back from the store (Ben, 2026-09-14:
    // "it needs to be a summary of the provision on the right etc - like in
    // the normal course. Not just a sec ref...!"; "1. for now - yes";
    // "everything to be driven by coding and not just a layer on top so all
    // edits need to be repeatable across corpus").
    factHeadlineRows.push({
      ...
      summary: typeof proposal.headline.summary === 'string' && proposal.headline.summary.trim() ? proposal.headline.summary : null,
      coverage_only: proposal.coverage_only === true,
    });
    if (proposal.conclusions) {
      const conclusionProblems = validateFactConclusions(proposal, { tableShapes });
      if (conclusionProblems.length > 0) {
        issues.push(makeFactConclusionsIssue({
          problems: conclusionProblems,
          familyKey: proposal.family_key,
          subtypeKey: proposal.subtype_key,
          nodeId: result.node_id,
          proposalId: proposal.proposal_id,
        ...
        // Safety net behind the compiler's own check: the readout row is
        // not written and a note records why; the proposal payload (whose
        // id was hashed over its conclusions) is stored exactly as compiled.
        return proposal;
      }
      factConclusionRows.push({
        proposal_id: proposal.proposal_id,
        table_key: proposal.conclusions.table_key,
        row_label: proposal.conclusions.row_label,
        cells: proposal.conclusions.cells,
      });
    }
    return proposal;
  });
  return {
    ...
    ...(factConclusionRows.length ? { fact_conclusion_rows: factConclusionRows } : {}),
  };
}
...
function buildComponentTree(rows) {
  ...
  const build = (parentId) => (byParent.get(parentId) || [])
    ...
    .map((row) => ({
      ...
      origin: row.origin,
      ...(row.origin !== 'OWN' ? { origin_structure_node_id: row.origin_structure_node_id ?? null } : {}),
      gap_before: row.gap_before === true,
      // Mirror the writer's key rules (fact-components buildComponentTree)
      // exactly: a value kind always carries `value`, a reference kind always
      // carries `resolves_to`, a litany always carries `members`, and each
      // may be null. Dropping a null key changes the proposal's content id
      // (NCS generation 4, 2026-09-13: PERIOD with value null).
      ...(VALUE_KINDS.has(row.kind) ? { value: row.value ?? null } : {}),
      ...(row.kind === 'LITANY' ? { members: Array.isArray(row.members) ? row.members : [] } : {}),
      ...(row.kind === 'CROSS_REFERENCE' || row.kind === 'DEFINED_TERM' ? { resolves_to: row.resolves_to ?? null } : {}),
      children: build(row.component_id),
    }));
  return build(null);
}

// Attaches `components` and `headline` from the stored rows to proposals
// that have them. Every other proposal reads back exactly as it was written:
// a V1 proposal has neither key, and a held V2 proposal (tree rejected, no
// rows written) keeps the tree in its payload because its proposal_id was
// hashed over it (NCS generation 4, 2026-09-13: stripping the keys broke
// DRAFT_NESTED_IDENTITY on the first held proposal).
function withFactComponents(proposalPayloads, componentRowsByProposal, headlineByProposal) {
  return proposalPayloads.map((payload) => {
    const { coverage_only, ...rest } = payload;
    ...
    return {
      ...
      headline: {
        ...
        // Older generations have no summary; the key is absent, not null,
        // so a stored V2 fact reads back exactly as it was written.
        ...(typeof headlineRow.summary === 'string' && headlineRow.summary.trim() ? { summary: headlineRow.summary } : {}),
      },
...
// A proposal's `conclusions` are compiled into it before its id is computed,
// so the payload is the truth and reads back exactly as written. The stored
// conclusions row (Part 3, FACT_CONCLUSIONS/V1) is attached only to a payload
// that carries no conclusions of its own (none today; kept for a future
// write path that stores the readout outside the payload).
function withFactConclusions(proposalPayloads, conclusionRowsByProposal) {
  return proposalPayloads.map((payload) => {
    if (Object.hasOwn(payload, 'conclusions')) return payload;
    const row = conclusionRowsByProposal.get(payload.proposal_id);
    if (!row) return payload;
    return {
      ...payload,
      conclusions: { table_key: row.table_key, row_label: row.row_label, cells: row.cells },
    };
  });
}
...
class ProductPhase2Store extends ProductPhase1Store {
  ...
  async loadCompletedSectionResults(runId) {
    ...
    const conclusionsByProposal = new Map(rows.product_fact_conclusions.map((row) => [row.proposal_id, row]));
...
module.exports = { ProductPhase2Store, crossSectionRelationshipStagingInput, withFactComponents, withFactConclusions, RESULT_TABLE_ORDER };
