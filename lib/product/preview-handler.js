...
function buildPreviewWorkspace({ run, sourceDocument, agreementStructure, sections, review = null }) {
  ...
  return {
    ...
    // The run's generation rides along for the page header (Ben, 2026-09-14:
    // the metadata line shows "what the run knows").
    progress: { status: run.status, stage: run.stage, completed: sections.length, total, generation: run.source_generation ?? null },
  };
}
