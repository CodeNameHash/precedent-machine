...
function compileExtraction({ sourceDocument, legalSchema, node, closure, call, response, routedFamilies, ownedNodeIds }) {
  ...
  for (const proposal of response.proposals) {
    ...
    if (!rawGroup && !skippedGroup) fail('PROPOSAL_GROUP', clientRef);
    const subtype = family.subtypes.find((item) => item.subtype_key === proposal.subtype_key);
    if (!subtype || skippedGroup) {
      ...
      issues.push(makeIssue({
        kind: 'VALIDATION', code: skippedGroup
...
async function buildAgreementSectionDraft({
  ...
  const extractionCall = await invokeModel(model, {
    kind: 'EXTRACTION', promptVersion: 'PRODUCT_ALL_FAMILY_EXTRACTOR/V4', request: extractionRequest,
    nodeId: node.node_id, attemptToken, onModelCall,
  });
