...
function compileProposalComponents({ sourceDocument, closure, proposal, subtype, factOccurrenceId, spansById }) {
  ...
  return { headline, components: tree.components, problems: [...new Set(problems)] };
}

function compileExtraction({ sourceDocument, legalSchema, node, closure, call, response, routedFamilies, ownedNodeIds }) {
  const families = schemaFamilies(legalSchema);
...
async function buildAgreementSectionDraft({
  ...
  const extractionCall = await invokeModel(model, {
    kind: 'EXTRACTION', promptVersion: layered ? 'PRODUCT_ALL_FAMILY_EXTRACTOR/V8' : 'PRODUCT_ALL_FAMILY_EXTRACTOR/V6', request: extractionRequest,
    nodeId: node.node_id, attemptToken, onModelCall,
  });
  const compiled = compileExtraction({
    sourceDocument, legalSchema, node, closure, call: extractionCall.call,
    response: extractionCall.response, routedFamilies: routing.families, ownedNodeIds,
  });
