...
// (the family's coverage_only flag and, from FACT_CONCLUSIONS/V1, the
// conclusions row). They are not part of the proposal's content identity.
// 2026-09-13: NCS generation 4 failed draft finalisation with
// DRAFT_NESTED_IDENTITY because coverage_only was hashed in.
const PROPOSAL_READ_ANNOTATIONS = Object.freeze(['coverage_only', 'conclusions']);
      ...
      if (tableShapes && componentResult.problems.length === 0
        ...
        if (rawConclusions) {
          ...
          if (conclusionProblems.length > 0) {
            temporaryProposal.validation_status = 'INVALID';
            issues.push(makeIssue({
              kind: 'VALIDATION', code: 'INVALID_FACT_CONCLUSIONS',
