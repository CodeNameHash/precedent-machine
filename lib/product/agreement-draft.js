...
async function buildAgreementSectionDraft({
  ...
  if (layered) {
    ...
    extractionRequest.response_contract.proposals[0].components = [{ ref: 'string', kind: 'FACT_COMPONENTS/V2 kind', label: 'string', quote: 'exact string', source_span_id: 'closure component span ID', occurrence: 'zero-based integer', origin: 'OWN|CHAPEAU|INTRO|DEFINITION', gap_before: 'boolean', members: ['only for LITANY'], resolves_to: { structure_node_id: 'only for CROSS_REFERENCE or DEFINED_TERM', text: 'referenced words' }, children: ['nested components'] }];
  }
  const extractionCall = await invokeModel(model, {
    kind: 'EXTRACTION', promptVersion: layered ? 'PRODUCT_ALL_FAMILY_EXTRACTOR/V7' : 'PRODUCT_ALL_FAMILY_EXTRACTOR/V6', request: extractionRequest,
    nodeId: node.node_id, attemptToken, onModelCall,
  });
