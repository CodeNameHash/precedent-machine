...
function compileResidualPass({ response, call, node, closure, paragraphs }) {
  ...
  if (response.paragraphs.length !== paragraphs.length) fail('RESIDUAL_PARAGRAPH_COVERAGE', node.reference);
  const seen = new Set();
  const dispositions = response.paragraphs.map((item, responseIndex) => {
    const entry = byId.get(item.source_span_id);
    if (!entry || seen.has(item.source_span_id)) fail('RESIDUAL_PARAGRAPH_SET', node.reference);
    if (entry.paragraphIndex !== responseIndex) fail('RESIDUAL_PARAGRAPH_ORDER', node.reference);
    ...
    const familyKeys = uniqueStrings(item.family_keys || [], 'residual.family_keys');
...
function sealSectionResult(result) {
  ...
  const spans = sorted(result.spans, 'span_id');
  const body = {
    schema_version: SECTION_DRAFT_VERSION,
    ...result,
    source_closure: { ...result.source_closure, spans },
    model_calls: sorted(result.model_calls, 'model_call_id'),
    spans,
    proposals: sorted(result.proposals, 'proposal_id'),
    groups: sorted(result.groups, 'proposition_group_id'),
    links: sorted(result.links, 'fact_link_id'),
    issues: sorted(result.issues, 'issue_id'),
    coverage: sorted(result.coverage, 'coverage_assertion_id'),
  };
  return deepFreeze({ ...body, section_result_id: contentId(SECTION_DRAFT_VERSION, body) });
}

async function invokeModel(model, { kind, promptVersion, request, nodeId }) {
  ...
  return deepFreeze({ call: { ...body, model_call_id: contentId(MODEL_CALL_VERSION, body) }, response });
}
...
function compileRouting({ response, call, node, deterministicEvidence }) {
  const families = uniqueStrings(response.families, 'routing.families');
  if (families.some((family) => !TARGET_FAMILIES.includes(family))) fail('ROUTING_FAMILY', families.join(','));
  if (!SECTION_DISPOSITIONS.has(response.disposition)) fail('ROUTING_DISPOSITION', String(response.disposition));
  if ((families.length > 0) !== (response.disposition === 'FAMILY_ASSIGNED')) {
    fail('ROUTING_CONSISTENCY', node.reference);
  }
  const disagreements = Array.isArray(response.deterministic_disagreements)
    ? response.deterministic_disagreements : [];
...
function exactQuoteSpan(sourceDocument, closure, evidence) {
  ...
  if (!Number.isSafeInteger(occurrence) || occurrence < 0 || occurrence >= unique.length) {
    fail('PROPOSAL_EXACT_SPAN', quote.slice(0, 80));
  }
...
function compileExtraction({ sourceDocument, legalSchema, node, closure, call, response, routedFamilies }) {
  ...
  for (const proposal of response.proposals) {
    ...
    const evidenceSpans = evidence.map((item) => exactQuoteSpan(sourceDocument, closure, item));
    ...
    const occurrenceBody = {
      ...
      source_span_ids: evidenceSpans.map((span) => span.span_id).sort(),
    };
    ...
    const temporaryProposal = {
      ...
      source_span_ids: evidenceSpans.map((span) => span.span_id),
      source_closure_id: closure.source_closure_id,
      model_call_id: call.model_call_id,
      fact_occurrence_id: factOccurrenceId,
      structure_node_id: node.node_id,
      state: PROPOSAL_STATES.has(proposal.state) ? proposal.state : 'PROPOSED',
      validation_status: missingRoles.length === 0 && !normalized.issue ? 'VALID' : 'INVALID',
    };
  ...
  const links = response.links.map((link) => {
    ...
    if (!RELATIONSHIP_TYPES.includes(link.relationship_type)) fail('FACT_LINK_TYPE', link.relationship_type);
    ...
    if (!fromSubtype.relationships.includes(link.relationship_type)) {
      issues.push(makeIssue({ kind: 'VALIDATION', code: 'RELATIONSHIP_NOT_ALLOWED', message: link.relationship_type, familyKey: from.family_key, subtypeKey: from.subtype_key, nodeId: node.node_id, proposalId: from.proposal_id }));
    }
    const body = {
      ...
      source_span_ids: uniqueStrings(link.source_span_ids || [], 'link.source_span_ids'),
    };
    if (body.source_span_ids.some((id) => !spansById.has(id))) fail('FACT_LINK_SPAN', link.relationship_type);
...
async function buildAgreementSectionDraft({ sourceDocument, agreementStructure, legalSchema, model, node }) {
  ...
  const routingRequest = {
    instruction: 'Classify this complete substantive section semantically. Return every applicable family. Deterministic labels are evidence only.',
  ...
  const routingCall = await invokeModel(model, { kind: 'ROUTING', promptVersion: 'PRODUCT_MULTI_LABEL_ROUTER/V1', request: routingRequest, nodeId: node.node_id });
  ...
  const residualCall = await invokeModel(model, { kind: 'RESIDUAL', promptVersion: 'PRODUCT_PARAGRAPH_RESIDUAL/V1', request: residualRequest, nodeId: node.node_id });
  ...
  const extractionRequest = {
    instruction: 'Propose atomic legal facts. Cite exact source words. Keep conditions, exceptions, thresholds and timing as roles or linked facts. Do not infer absence.',
  ...
  const extractionCall = await invokeModel(model, { kind: 'EXTRACTION', promptVersion: 'PRODUCT_ALL_FAMILY_EXTRACTOR/V1', request: extractionRequest, nodeId: node.node_id });
