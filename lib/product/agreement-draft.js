...
function extractionDeclined(response, routedFamilies) {
  ...
  const coverage = plainObject(response.coverage) ? response.coverage : {};
...
async function buildAgreementSectionDraft({
  ...
  if (familiesWithTables.length > 0) {
    ...
    extractionRequest.table_shapes = Object.fromEntries(familiesWithTables.map((familyKey) => [
      ...
      tablesForFamily(familyKey, tableShapes).map(({ table }) => ({
        ...
        columns: (table.columns || []).filter((column) => !column.derived).map((column) => ({
  ...
  if (routing.families.length > 0 && extractionDeclined(extractionCall.response, routing.families)) {
    fail('EXTRACTION_DECLINED', `${node.reference}: no proposals and every routed family UNRESOLVED (${routing.families.join(', ')})`);
  }
  const compiled = compileExtraction({
    sourceDocument, legalSchema, node, closure, call: extractionCall.call,
    response: extractionCall.response, routedFamilies: routing.families, ownedNodeIds, tableShapes,
  });
