...
function evaluateSupervisedRelease({ inventory, reconciliation, analysis, reviewState, legalSchema, citationAssessments, elapsedMinutes, developerAssisted, processingStartedAt, processingCompletedAt }) {
  if (!Array.isArray(inventory) || !Array.isArray(reconciliation) || !analysis || !reviewState || !legalSchema || !Array.isArray(citationAssessments)) fail('EVALUATION_INPUT', 'inventory, reconciliation, analysis, review, legal schema and citations are required');
  ...
  const contradictions = new Set((analysis.issues || []).filter((item) => /CONTRADICT/i.test(item.code || '') && item.state !== 'CLOSED')
  ...
  for (const item of (analysis.coverage_assertions || []).filter((candidate) => ['UNRESOLVED', 'NOT_RUN'].includes(candidate.state))) {
  ...
  const citationsSufficient = publishedFacts.length > 0 && citationAssessments.length === publishedFacts.length && publishedFacts.every((fact) => {
    ...
    return fact.source_span_ids?.length > 0 && assessment?.exact === true && assessment?.legally_sufficient === true && assessment?.narrow === true;
  });
  ...
  const bars = {
    ...
    citations_sufficient_and_narrow: citationsSufficient,
    ...
    review_within_ninety_minutes_without_developer: Number.isFinite(effectiveElapsedMinutes)
      && elapsedMinutes >= 0 && effectiveElapsedMinutes <= 90
      && Number.isFinite(measuredReviewSeconds) && measuredReviewSeconds >= 0 && measuredReviewSeconds <= 5400
      && developerAssisted === false,
  };
  return Object.freeze({
    schema_version: 'PRODUCT_SUPERVISED_RELEASE_EVALUATION/V1',
    diagnostics: {
