...
function evaluateSupervisedRelease({ referenceSamples, analysis, reviewState, legalSchema, citationAssessments, findingResolutions = [], elapsedMinutes, developerAssisted, processingStartedAt, processingCompletedAt }) {
  if (!Array.isArray(referenceSamples) || !analysis || !reviewState || !legalSchema
    || !Array.isArray(citationAssessments) || !Array.isArray(findingResolutions)) {
    fail('EVALUATION_INPUT', 'reference samples, analysis, review, legal schema, citations and finding resolutions are required');
  }
  const referenceSampleIds = new Set();
  for (const item of referenceSamples) {
    if (!item?.reference_sample_id || referenceSampleIds.has(item.reference_sample_id)
      || !WEIGHTS[item.severity] || !['FOUND', 'MISSED', 'INCORRECT', 'UNRESOLVED'].includes(item.assessment)
      || item.reviewed_by_role !== 'LAWYER'
      || typeof item.source_document_id !== 'string' || item.source_document_id.trim() === ''
      || typeof item.source_url !== 'string' || !isSafeHttpsUrl(item.source_url)
      || typeof item.source_section !== 'string' || item.source_section.trim() === ''
      || typeof item.description !== 'string' || item.description.trim() === ''
      || (item.assessment === 'FOUND' && (typeof item.comparison !== 'string' || item.comparison.trim() === ''))
      || (item.assessment !== 'FOUND'
        && (typeof item.reviewed_limitation !== 'string' || item.reviewed_limitation.trim() === ''))) {
      fail('REFERENCE_SAMPLE', item?.reference_sample_id || 'missing');
    }
    referenceSampleIds.add(item.reference_sample_id);
  }
  ...
  const referenceSampleWeight = referenceSamples.reduce((sum, item) => sum + WEIGHTS[item.severity], 0);
  const successfulReferenceSampleWeight = referenceSamples
    .filter((item) => item.assessment === 'FOUND')
  ...
  const referenceSamplesReviewed = referenceSamples.length > 0
    && referenceSamples.every((item) => item.assessment !== 'UNRESOLVED');
  ...
  const bars = {
    reference_samples_reviewed: referenceSamplesReviewed,
  ...
  return Object.freeze({
    schema_version: 'PRODUCT_SUPERVISED_RELEASE_EVALUATION/V3',
    diagnostics: {
      reference_sample_success_rate: ratio(successfulReferenceSampleWeight, referenceSampleWeight),
      reference_sample_count: referenceSamples.length,
      reference_sample_missed_count: referenceSamples.filter((item) => item.assessment === 'MISSED').length,
      reference_sample_incorrect_count: referenceSamples.filter((item) => item.assessment === 'INCORRECT').length,
      reference_sample_unresolved_count: referenceSamples.filter((item) => item.assessment === 'UNRESOLVED').length,
