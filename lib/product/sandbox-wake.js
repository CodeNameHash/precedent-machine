...
async function wakeSandboxProductRun({
  ...
  sandboxApi = null,
  now = Date.now,
  startupDelay = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)),
}) {
  ...
  await startupDelay(STARTUP_CHECK_DELAY_MS);
  const startup = await sandbox.getCommand(command.cmdId);
  if (Number.isInteger(startup.exitCode) && startup.exitCode !== 0) {
