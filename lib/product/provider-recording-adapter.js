...
function activeExtractionFromProviderEvidence({ evidence, familyContracts, sourceClosure }) {
  ...
  const ownedStructureNodeIds = new Set(sourceClosure.owned_structure_node_ids
    || [sourceClosure.full_section.structure_node_id]);
  const proposals = [];
  const groups = [];
  const seen = new Set();
  ...
  for (const contract of familyContracts) {
    ...
    for (const observation of observations) {
      ...
      const matching = components.filter((item) => item.exact_text.includes(observation.exact_quote));
      const component = matching.find((item) => (
        ownedStructureNodeIds.has(item.structure_node_id)
      )) || matching[0];
