...
function parseProductNoShopPeriod(quote) {
  ...
  const excluded = [SECTION_REFERENCE_PATTERN, CLOCK_TIME_PATTERN, CALENDAR_DATE_PATTERN, SEC_RULE_CITATION_PATTERN, /\bArticle\s+\d+\b/gi]
    .flatMap((pattern) => [...quote.matchAll(new RegExp(pattern.source, pattern.flags.includes('g') ? pattern.flags : `${pattern.flags}g`))]
      .map((match) => [match.index, match.index + match[0].length]));
  const tokens = [...quote.matchAll(/\d(?:[\d,]*\d)?(?:\.\d+)?/g)]
  ...
  if (tokens.length === 0 && legacy.reason === 'PERIOD_UNIT_UNCORROBORATED') {
    return { outcome: 'ABSTAIN', reason: 'NO_NUMERIC_LITERAL' };
  }
