...
function createCodexCliProductModel({
  ...
  const adapterFor = (selection) => {
    const selectedModelId = `${selection.model};reasoning=${selection.reasoning_effort}`;
    if (adapters.has(selectedModelId)) return adapters.get(selectedModelId);
    ...
    adapters.set(selectedModelId, adapter);
    return adapter;
  };
  return {
    async complete(input) {
      const selection = resolveCodexCallModel(modelConfig, input?.call_kind);
      return adapterFor(selection).complete(input);
    },
  };
}
