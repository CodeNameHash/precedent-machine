...
function partyIdentity(source) {
  ...
  const embeddedRole = /(?:(?<=\))\s+(?:and\s+)?|;\s+(?:and\s+)?|,\s+(?:and\s+)?|\bamong\s*:?\s+)([A-Z][A-Za-z0-9&.' -]{2,180}?(?:,\s+(?:Inc\.?|LLC|L\.L\.C\.?|Ltd\.?|Corporation|Corp\.?|L\.P\.|Public Limited Company))?),\s+(?:an?|the)\s+[^()]{1,160}\(\s*(?:the\s+)?["“](Company|Target|Parent|Buyer|Purchaser|Merger Sub)["”]\s*\)/gi;
...
const ENTITY_SUFFIX_ONLY = /^(?:incorporated|inc\.?|corporation|corp\.?|llc|l\.l\.c\.?|ltd\.?|l\.p\.?)$/i;

function partyIdentityNeedsDisplayRepair(parties) {
  return Array.isArray(parties) && parties.length >= 2 && parties.every((party) => (
    party && typeof party.name === 'string' && ENTITY_SUFFIX_ONLY.test(party.name.trim())
      && typeof party.role === 'string' && party.role.length > 0
  ));
}

function recoverPartyIdentityForDisplay(parties, canonicalText) {
  if (!partyIdentityNeedsDisplayRepair(parties) || typeof canonicalText !== 'string' || canonicalText.length === 0) return null;
  const recovered = partyIdentity(canonicalText);
  if (recovered.conflicting || recovered.parties.length !== parties.length
    || recovered.parties.some((party) => ENTITY_SUFFIX_ONLY.test(party.name.trim()))) return null;
  const roles = (items) => items.map((party) => party.role).sort((left, right) => left.localeCompare(right));
  if (canonicalJson(roles(recovered.parties)) !== canonicalJson(roles(parties))) return null;
  return recovered.parties;
}
...
module.exports = {
  ProductSecIntakeError,
  createSecIntakeAdapter,
  extractDocumentIdentity,
  parseSecExhibitUrl,
  partyIdentityNeedsDisplayRepair,
  recoverPartyIdentityForDisplay,
};
