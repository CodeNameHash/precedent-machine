...
function partyIdentity(source) {
  ...
  for (const line of lines) {
    ...
    const match = asRole || (/^(?:this\s+)?agreement\b/i.test(line) ? null
      : line.match(/^(?:by\s+and\s+among\s+)?([A-Z][A-Z0-9&.' -]{2,180}?)(?:,\s+[^()]*)?\(\s*(?:the\s+)?["“](Company|Target|Parent|Buyer|Purchaser|Merger Sub)["”]\s*\)/i));
    if (!match) continue;
    const party = { name: match[1].trim(), role: match[2].toUpperCase().replace(/\s+/g, '_') };
  ...
  const embeddedRole = /(?:,\s+(?:and\s+)?|\bamong\s+)([A-Z][A-Za-z0-9&.' -]{2,120}(?:,\s+(?:Inc\.?|LLC|L\.L\.C\.|Ltd\.?|Corporation|Corp\.?|L\.P\.))?),\s+(?:an?|the)\s+[^()]{1,160}\(\s*(?:the\s+)?["“](Company|Target|Parent|Buyer|Purchaser)["”]\s*\)/g;
