...
function authoredSectionHeading({ node, routing, sourceClosure, spans = [] }) {
  if (typeof node?.title === 'string' && node.title.trim()) return node.title.trim();
  const reference = node?.reference;
  if (typeof reference !== 'string' || !reference
    || routing?.structure_node_id !== node?.node_id
    || routing?.section_reference !== reference
    || sourceClosure?.structure_node_id !== node?.node_id
    || sourceClosure?.section_reference !== reference
    || typeof sourceClosure?.source_closure_id !== 'string'
    || typeof sourceClosure?.full_section_span_id !== 'string') return 'Agreement section';
  const fullSection = spans.find((span) => (
    span.span_id === sourceClosure.full_section_span_id
    && span.kind === 'FULL_SECTION'
    && span.structure_node_id === node.node_id
    && (span.source_closure_ids || []).includes(sourceClosure.source_closure_id)
  ));
  if (typeof fullSection?.exact_text !== 'string') return 'Agreement section';
  const prefix = new RegExp(`^(?:Section\\s+)?${escapeRegExp(reference)}(?:\\.\\s+|\\s+)`, 'i');
  const opening = fullSection.exact_text.trimStart();
  if (!prefix.test(opening)) return 'Agreement section';
  const heading = opening.replace(prefix, '')
    .match(new RegExp(`^([^\\r\\n.]{1,${SECTION_HEADING_MAX_LENGTH}})\\.(?:\\s|$)`))?.[1]?.trim();
  return heading || 'Agreement section';
}
