...
const { validateFactComponents } = require('./fact-components');
...
// A closure's spans, mapped by span ID, the same set the extraction path
// resolves component quotes against (`closure.spans` in agreement-draft.js).
function componentsSpansById(analysis, closureId) {
  const ids = closureSpanIds(analysis, closureId);
  const map = new Map();
  for (const span of analysis.spans || []) {
    if (ids && ids.has(span.span_id)) map.set(span.span_id, span);
  }
  return map;
}

// Reconstructs enough of the canonical text to byte-check a fact's
// components: each closure span's exact text placed at its own absolute
// byte offset. The review state never holds the whole document, so bytes
// outside a known span decode as filler; every component byte range a
// lawyer's edit can produce falls inside a closure span, which is what this
// buffer actually attests to.
function closureSourceText(analysis, closureId) {
  const ids = closureSpanIds(analysis, closureId);
  if (!ids) return null;
  const relevant = (analysis.spans || []).filter((span) => ids.has(span.span_id)
    && Number.isSafeInteger(span.start_byte) && Number.isSafeInteger(span.end_byte)
    && typeof span.exact_text === 'string');
  if (relevant.length === 0) return null;
  const maxEnd = Math.max(...relevant.map((span) => span.end_byte));
...
function decideItem(state, command, timestamp, legalSchema, analysis) {
  ...
  if (command.decision === 'EDITED') {
    ...
    if (Object.hasOwn(command, 'components')) {
      if (!Array.isArray(command.components)) fail('REVIEW_FACT_COMPONENTS', 'components must be an array');
      const effectiveHeadline = command.headline !== undefined ? command.headline
        : current.edited_headline || current.original.headline;
      const problems = validateFactComponents({ headline: effectiveHeadline, components: command.components }, {
        sourceText: closureSourceText(analysis, current.source_closure_id),
        spansById: componentsSpansById(analysis, current.source_closure_id),
      });
      if (problems.length > 0) fail('REVIEW_FACT_COMPONENTS', problems.join('; '));
      componentsEdit = { components: command.components, headline: command.headline !== undefined ? command.headline : null };
    }
  }
  const replacement = {
    ...current,
    ...
    edited_components: componentsEdit ? componentsEdit.components : current.edited_components ?? null,
    edited_headline: componentsEdit ? componentsEdit.headline : current.edited_headline ?? null,
...
// Returns an item to PENDING, discarding any edit, and keeps its comment.
// Used when a lawyer made an edit as a note rather than a correction.
function resetItem(state, command, timestamp) {
  const index = state.items.findIndex((item) => item.item_id === command.item_id);
  if (index < 0) fail('REVIEW_ITEM_NOT_FOUND', command.item_id);
  const current = state.items[index];
  const { edited_proposition_group_id, edited_relationship, reviewed_at, decided_by_role, ...rest } = current;
  const items = [...state.items];
  items[index] = {
    ...rest,
    decision: 'PENDING',
    ...
    edited_components: null,
    edited_headline: null,
    source_span_ids: current.kind === 'PROPOSAL' ? [...current.original.source_span_ids] : current.source_span_ids,
    reset_at: timestamp,
  };
  return { ...state, items };
}

function commentItem(state, command, timestamp) {
  const index = state.items.findIndex((item) => item.item_id === command.item_id);
  if (index < 0) fail('REVIEW_ITEM_NOT_FOUND', command.item_id);
  if (command.comment !== null && typeof command.comment !== 'string') fail('REVIEW_COMMENT', 'string or null required');
  const text = typeof command.comment === 'string' ? command.comment.trim() : '';
  if (text.length > COMMENT_MAX_LENGTH) fail('REVIEW_COMMENT', `longer than ${COMMENT_MAX_LENGTH} characters`);
  const items = [...state.items];
  const { comment, commented_at, ...rest } = state.items[index];
  items[index] = text ? { ...rest, comment: text, commented_at: timestamp } : rest;
  return { ...state, items };
}
...
function addMissingFact(state, command, analysis, timestamp, legalSchema) {
  ...
  if (new Set(command.source_span_ids).size !== command.source_span_ids.length) fail('MISSING_FACT_SOURCE', 'duplicate span');
...
// Extra keys for a summary fact whose proposal or user fact carries a
// FACT_COMPONENTS/V2 tree (Phase 5B.5). A fact without `components` and
// `headline` gets none of these keys, so a V1 summary body is unchanged.
function layeredFactExtras(item, legalSchema, analysis) {
  const original = item.original;
  const components = item.edited_components || original.components;
  const headline = item.edited_headline || original.headline;
  if (!Array.isArray(components) || !headline) return {};
  const family = (legalSchema?.families || [])
    .find((candidate) => candidate.family_key === item.family_key);
  const node = (analysis?.agreement_structure?.nodes || [])
    .find((candidate) => candidate.node_id === item.structure_node_id);
  return {
    components,
    headline,
    coverage_only: family?.coverage_only === true,
...
function compileReviewSummary(state, legalSchema, analysis) {
  ...
  const facts = state.items.filter((item) => ['PROPOSAL', 'USER_FACT'].includes(item.kind)
    ...
    ...layeredFactExtras(item, legalSchema, analysis),
...
function prepareReferenceSamples(value, referenceSources) {
  if (!Array.isArray(value) || value.length === 0) fail('RELEASE_REFERENCE_SAMPLES', 'at least one reference sample is required');
  if (!Array.isArray(referenceSources)) fail('RELEASE_REFERENCE_SOURCES', 'verified reference sources are required');
  const sourcesById = new Map(referenceSources.map((source) => [source.source_document_id, source]));
  const seen = new Set();
  return value.map((item) => {
    const referenceSampleId = typeof item?.reference_sample_id === 'string' ? item.reference_sample_id.trim() : '';
    const sourceDocumentId = typeof item?.source_document_id === 'string' ? item.source_document_id.trim() : '';
    const sourceUrl = typeof item?.source_url === 'string' ? item.source_url.trim() : '';
    const sourceSection = typeof item?.source_section === 'string' ? item.source_section.trim() : '';
    const description = typeof item?.description === 'string' ? item.description.trim() : '';
    const comparison = typeof item?.comparison === 'string' ? item.comparison.trim() : '';
    const reviewedLimitation = typeof item?.reviewed_limitation === 'string' ? item.reviewed_limitation.trim() : '';
    let parsedSourceUrl;
    try { parsedSourceUrl = new URL(sourceUrl); } catch {}
    const safeSourceUrl = parsedSourceUrl?.protocol === 'https:' && parsedSourceUrl.username === ''
      && parsedSourceUrl.password === '' && parsedSourceUrl.hostname !== '';
    const verifiedSource = sourcesById.get(sourceDocumentId);
    if (!referenceSampleId || seen.has(referenceSampleId) || !sourceDocumentId || !safeSourceUrl
      || verifiedSource?.retrieval_url !== sourceUrl
      || !sourceSection || !description || !['CRITICAL', 'MATERIAL'].includes(item.severity)
      || !['FOUND', 'MISSED', 'INCORRECT', 'UNRESOLVED'].includes(item.assessment)
      || (item.assessment === 'FOUND' && !comparison)
      || (item.assessment !== 'FOUND' && !reviewedLimitation)) {
      fail('RELEASE_REFERENCE_SAMPLE', referenceSampleId || 'missing');
    }
    seen.add(referenceSampleId);
    return {
      reference_sample_id: referenceSampleId,
      ...
      ...(item.assessment === 'FOUND' ? { comparison } : { reviewed_limitation: reviewedLimitation }),
      reviewed_by_role: 'LAWYER',
    };
  });
}

function applyReviewCommand(state, command, { analysis, legalSchema, clock, timing, reviewHistory, referenceSources } = {}) {
  ...
  if (command.type === 'EVALUATE_RELEASE') {
    ...
    if (command.lawyer_attestation !== true || command.reference_samples_attestation !== true
    ...
    const referenceSamples = prepareReferenceSamples(command.reference_samples, referenceSources);
    ...
    const releaseEvaluationInput = {
      ...
      reference_samples_attested: true,
    };
  ...
  else if (command.type === 'COMMENT_ITEM') next = commentItem(state, command, timestamp);
  else if (command.type === 'RESET_ITEM') next = resetItem(state, command, timestamp);
