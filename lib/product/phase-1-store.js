...
class ProductPhase1Store {
  ...
  async findStructureBySource(sourceDocumentId) {
    const { data, error } = await this.client.from('product_agreement_structures')
      .select('payload').eq('source_document_id', sourceDocumentId).maybeSingle();
    if (error) throw rpcError('findStructureBySource', error);
    return data?.payload || null;
  }
