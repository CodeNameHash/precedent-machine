...
// The DECIDE_ITEM/EDITED payload for a component edit. The database
// validates an edited proposal as a whole: it must carry a statement and
// an object of roles, so a component-only edit keeps the saved statement
// and roles rather than sending the tree alone.
function componentEditPayload({ proposal, item, components }) {
  return {
    statement: item?.edited_statement || proposal.statement,
    roles: item?.edited_roles || proposal.roles || {},
    components,
  };
}

module.exports = { buildEditedComponents, byteOffsetsForText, componentEditPayload, locateUniqueText };
