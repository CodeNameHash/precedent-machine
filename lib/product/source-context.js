...
function enclosingArticleIntroduction(node, nodesById) {
  let current = node;
  const seen = new Set();
  while (current && !seen.has(current.node_id)) {
    seen.add(current.node_id);
    if (current.kind === 'ARTICLE') {
      return [...nodesById.values()].find((candidate) => (
        candidate.kind === 'SECTION'
        && candidate.parent_id === current.node_id
        && INTRO_REFERENCE.test(String(candidate.reference || ''))
      )) || null;
    }
...
function canonicalCrossScopeReferences(entries) {
...
function buildSourceClosure({ sourceDocument, agreementStructure, nodeId }) {
  ...
  const localChapeauSpans = chapeauRange.end > chapeauRange.start
    ? [makeSpan(sourceDocument, chapeauRange.start, chapeauRange.end, 'CHAPEAU', node.node_id)]
    : [];

  const definitions = [];
  const references = [];
  const articleIntroductions = [];
  const crossScopeResolutions = [];
  const unresolvedReferences = new Set();
  const traversed = new Set([section.node_id]);
  const queue = [section];
  const enqueueArticleIntroduction = (candidate) => {
    const introduction = enclosingArticleIntroduction(candidate, nodesById);
    if (!introduction || introduction.node_id === section.node_id) return;
    articleIntroductions.push(introduction);
    if (!traversed.has(introduction.node_id)) {
      traversed.add(introduction.node_id);
      queue.push(introduction);
    }
  };
  enqueueArticleIntroduction(section);
  ...
  while (queue.length > 0) {
    ...
    for (const annotation of currentAnnotations.filter((item) => item.kind === 'SECTION_REFERENCE')) {
      ...
      enqueueArticleIntroduction(targetSection);
    }
  ...
  const inheritedChapeauSpans = uniqueNodes(articleIntroductions).map((candidate) => makeSpan(
    sourceDocument, candidate.span.start_byte, candidate.span.end_byte, 'CHAPEAU', candidate.node_id,
  ));
  const chapeauSpans = [...new Map([...localChapeauSpans, ...inheritedChapeauSpans]
  ...
  const body = {
    ...
    context_diagnostics: {
      ...
      cross_scope_section_references: canonicalCrossScopeReferences(crossScopeResolutions),
    },
  };
  return deepFreeze({
    ...body,
