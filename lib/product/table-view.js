...
function conclusionCellsForFact(fact, table, tableShapes) {
  if (factConclusions && typeof factConclusions.renderConclusionCells === 'function') {
    const rendered = factConclusions.renderConclusionCells(fact, tableShapes);
    if (rendered) return normalizeConclusionCells(rendered, fact);
  }
  if (fact.conclusions && fact.conclusions.cells) {
