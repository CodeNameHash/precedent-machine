...
function derivePresenceCell({ column, facts }) {
  ...
  const sources = (facts || []).filter((fact) => fact.family_key === spec.from_family
    && (!spec.from_subtype || fact.subtype_key === spec.from_subtype));
  let derived = null;
  for (const source of sources) {
    ...
    const incoming = column.display === 'fact_text'
      ? { column_id: column.column_id, kind: 'text', label: factOwnText(source), tone: 'neutral', component_ids: own.map((component) => component.component_id), fact_ids: [factIdOf(source)] }
