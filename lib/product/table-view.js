...
function cellValue(cell) {
  return { label: cell.label, ...(cell.code ? { code: cell.code } : {}), kind: cell.kind, tone: cell.tone, component_ids: cell.component_ids || [], fact_ids: cell.fact_ids || [] };
}
...
function buildTableView({ facts, tableShapes, legalSchema = null }) {
  ...
  for (const fact of facts || []) {
    ...
    const subject = table.rows_are === 'one per agreement' ? (table.subject_label || 'The deal') : subjectForFact(fact);
    ...
    const detail = typeof fact.conclusions?.row_detail === 'string' && fact.conclusions.row_detail.trim() ? fact.conclusions.row_detail.trim() : null;
    if (detail) {
