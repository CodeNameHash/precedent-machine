...
function cellValue(cell) {
  return { label: cell.label, ...(cell.code ? { code: cell.code } : {}), ...(cell.link_section ? { link_section: cell.link_section } : {}), kind: cell.kind, tone: cell.tone, component_ids: cell.component_ids || [], fact_ids: cell.fact_ids || [] };
}
...
function footerEntryForFact(fact, table, tableShapes) {
  ...
  const text = verbatim ? verbatim.label : components.filter((component) => component.origin === 'OWN' || !component.origin).map((component) => component.text).filter(Boolean).join(' ');
  return {
    fact_id: factIdOf(fact),
    section_reference: fact.section_reference || null,
...
function buildTableView({ facts, tableShapes, legalSchema = null }) {
  ...
  for (const fact of facts || []) {
    ...
    const subject = table.rows_are === 'one per agreement' ? (table.subject_label || 'The deal') : (subtypeRow || subjectForFact(fact));
  ...
  for (const section of tableShapes.sections || []) {
    ...
    sections.push({ section_key: section.section_key, title: section.title, tables: sectionTables, facts_without_readout: withoutReadout });
  }
