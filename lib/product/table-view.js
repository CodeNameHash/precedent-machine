...
// The Term of an other-provision row: a summary built from the labels the
// extractor gave the fact's own components (Ben, 2026-09-14, on the first
// table: "it needs to be a summary of the provision on the right etc -
// like in the normal course. Not just a sec ref...!"). The labels of the
// ACTOR, OPERATION and OBJECT components in that order (the standard,
// condition, exception, trigger or term labels when the words carry none
// of those), generic labels dropped and a leading "Subject:" / "Operation:"
// tag stripped, at most three, joined with a middle dot; the headline label
...
function groupOtherProvisions(facts) {
  ...
  for (const group of groups.values()) {
    ...
    const branches = members.map((member) => {
      ...
      const rest = member.own.filter((component) => !commonTexts.has(component.text.trim()));
      return { fact_id: factIdOf(member.fact), section_reference: member.fact.section_reference || null, text: joinInSourceOrder(core.length ? core : rest), term: provisionTerm(member.fact, rest) };
    });
