...
function factSummaryText(fact) {
  const own = (fact?.components || []).filter((component) => component && (component.origin === 'OWN' || !component.origin) && component.text);
  ...
  let chosen = own.filter((component) => SUMMARY_CORE_KINDS.has(component.kind) || distinguishing.has(component.component_id));
  if (chosen.length === 0) chosen = own;
  const anchored = chosen.every((component) => Number.isSafeInteger(component.start_byte) && Number.isSafeInteger(component.end_byte));
  const ordered = anchored ? [...chosen].sort((left, right) => left.start_byte - right.start_byte) : chosen;
  const parts = [];
  let previous = null;
...
function derivePresenceCell({ column, facts }) {
  ...
  for (const source of sources) {
    ...
    const incoming = column.display === 'fact_text'
      ...
      : { column_id: column.column_id, kind: 'pill', label: 'Yes', code: 'YES', tone: TONE_BY_RENDER[column.render] || 'neutral', component_ids: own.slice(0, 1).map((component) => component.component_id), fact_ids: [factIdOf(source)] };
