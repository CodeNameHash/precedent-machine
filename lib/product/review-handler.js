...
function createProductReviewHandler({ getClient, storeFactory = (client) => new ProductPhase3Store({ client }), actorResolver = getProductActor } = {}) {
  ...
  return async function productReviewHandler(req, res) {
    ...
    try {
      ...
      if (body.command.type === 'RESTORE') {
        ...
        const state = applyReviewCommand(review.state, command, { analysis, legalSchema, timing });
