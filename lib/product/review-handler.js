...
const {
  applyReviewCommand, initialiseReviewState, normaliseReviewRelationships,
  relationshipReviewIsCoherent, ProductReviewError,
...
const { legalSchemaForVersion } = require('./legal-schema-selection');
...
function projectReviewRelationships(review, analysis) {
  if (!review || review.state?.status !== 'DRAFT') return review;
  const state = normaliseReviewRelationships(review.state, analysis);
  return {
    ...review,
    state: {
      ...state,
      relationship_review_coherent: relationshipReviewIsCoherent(state, analysis, legalSchemaForVersion(analysis.legal_schema_version)),
    },
  };
}

async function presentAnalysisIdentity(analysis, store, runId) {
  const parties = analysis.source_document?.parties;
  if (!partyIdentityNeedsDisplayRepair(parties)) return analysis;
  ...
  if (!displayParties) return analysis;
  return {
    ...analysis,
    source_document: { ...analysis.source_document, display_parties: displayParties },
  };
}
...
function createProductReviewHandler({ getClient, storeFactory = (client) => new ProductPhase3Store({ client }), actorResolver = getProductActor, clock } = {}) {
  ...
  return async function productReviewHandler(req, res) {
    ...
    try {
      ...
      if (!review) {
        await store.initialiseReview({ runId, state: initialiseReviewState(analysis), actor });
        review = await store.getReview({ runId, actor });
      }
      const referenceSources = typeof store.listReferenceSources === 'function'
        ? await store.listReferenceSources({ actor }) : [];
      if (req.method === 'GET') return res.status(200).json({
        schema_version: 'PRODUCT_REVIEW_WORKSPACE/V1', analysis: await presentAnalysisIdentity(analysis, store, runId),
        review: projectReviewRelationships(review, analysis), reference_sources: referenceSources,
      });
      const body = req.body || {};
      ...
      if (body.command.type === 'RESTORE') {
        ...
        const state = applyReviewCommand(review.state, command, {
          analysis, legalSchema: legalSchemaForVersion(analysis.legal_schema_version), timing, clock, reviewHistory: reviewTimingHistory(review), referenceSources,
        });
...
module.exports = { createProductReviewHandler, presentAnalysisIdentity, projectReviewRelationships };
