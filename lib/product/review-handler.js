...
function projectReviewRelationships(review, analysis) {
  ...
  return {
    ...
    state: {
      ...state,
      relationship_review_coherent: relationshipReviewIsCoherent(state, analysis, legalSchema),
    },
  };
}
    ...
    try {
      ...
      if (body.command.type === 'RESTORE') {
        ...
        const state = applyReviewCommand(review.state, command, {
          analysis, legalSchema, timing, clock, reviewHistory: reviewTimingHistory(review), referenceSources,
        });
