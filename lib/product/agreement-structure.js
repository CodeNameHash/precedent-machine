...
function isExternalSectionCitation(canonicalText, matchIndex, matchLength) {
...
function buildAnnotations(canonicalText, sourceBytes, sourceSha256, nodes) {
  ...
  for (const [kind, pattern] of patterns) {
    for (const match of canonicalText.matchAll(pattern)) {
      if (kind === 'SECTION_REFERENCE' && isExternalSectionCitation(canonicalText, match.index, match[0].length)) continue;
