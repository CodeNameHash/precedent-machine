...
const LEGAL_SCHEMA_VERSION_V2 = 'LEGAL_SCHEMA/V2';
const V2_GENERIC_OPTIONAL = ['TEMPORAL_OR_TRIGGER_SCOPE', 'QUALIFICATIONS', 'FORUM'];
...
// V2 (layered fact model, plan Phase 5B) adds per-family headline and layer
// rules, a coverage_only flag, subtype labels and optional timing,
// qualification and forum roles. Everything V1 requires still holds.
function validateV2Additions(schema) {
  if (schema.component_contract !== 'FACT_COMPONENTS/V2' || schema.roles?.empty_role_allowed !== true) {
    fail('LEGAL_SCHEMA_V2_CONTRACT', 'V2 must bind FACT_COMPONENTS/V2 and allow empty roles');
  }
  for (const family of schema.families) {
    if (typeof family.coverage_only !== 'boolean' || typeof family.layer_rules !== 'string' || !family.layer_rules
      || !family.headline || family.headline.label_source !== 'subtype_label'
      || !Array.isArray(family.headline.distinguishing_kinds) || family.headline.distinguishing_kinds.length === 0) {
      fail('LEGAL_SCHEMA_V2_FAMILY', family.family_key);
    }
    for (const subtype of family.subtypes) {
      if (typeof subtype.label !== 'string' || !subtype.label.trim()) fail('LEGAL_SCHEMA_V2_LABEL', `${family.family_key}.${subtype.subtype_key}`);
      const forced = V2_GENERIC_OPTIONAL.filter((role) => subtype.required_roles.includes(role));
      if (forced.length) fail('LEGAL_SCHEMA_V2_FORCED_ROLE', `${family.family_key}.${subtype.subtype_key}: ${forced.join(',')}`);
    }
  }
}

function validateLegalSchema(schema) {
  if (!schema || ![LEGAL_SCHEMA_VERSION, LEGAL_SCHEMA_VERSION_V2].includes(schema.schema_version)) {
    fail('LEGAL_SCHEMA_VERSION', 'expected LEGAL_SCHEMA/V1 or LEGAL_SCHEMA/V2');
  }
  ...
  if (schema.schema_version === LEGAL_SCHEMA_VERSION_V2) validateV2Additions(schema);
  return schema;
}
