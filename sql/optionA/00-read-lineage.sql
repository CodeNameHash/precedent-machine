-- Block 00 — read-only lineage read for the QXO termination candidate (F2).
-- SPEC-QXO-TERMF-F2-CANDIDATE-OPTION-A-2026-07-24.md. Run in the STAGING
-- Supabase SQL Editor (project sjumbznveyyiizhwvixj). Copy the single JSON
-- value this returns and paste it back to the Claude session; it becomes the
-- input file for scripts/canonical-v2-staging-qxo-termination-optionA.mjs.
-- Nothing here writes. The two big fields (response_bytes_base64 and the
-- conversion text) are excluded — the generator refetches the SEC bytes and
-- reproves every hash, so the paste-back cannot smuggle altered lineage.
