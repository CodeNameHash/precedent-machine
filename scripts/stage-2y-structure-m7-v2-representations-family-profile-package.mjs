...
/**
 * Regenerate the on-disk REPRESENTATIONS Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-representations.json`;
const WORK3_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'REPRESENTATIONS';
...
const APPROVAL_TEXT =
  ...
  + `(${PROFILE_COUNT} APPROVE / 0 HOLD; subtype partition across the six sealed M5 buckets `
  + `remains LEGAL_GROUPING_REVIEW_REQUIRED on every row, and ${KNOWLEDGE_COUNT} knowledge-qualifier `
...
function phase4Fixture() {
  ...
  return {
    representationsAuthoringPhase4FamilyProfilePackageReviewAuthority: boundEnvelope(pinned(
...
function buildProfileFromTemplate(templateProfile, {
  ...
  profile.match_test = {
    ...
    tokens: ['familyrepresentations', profileMatchToken(requiredExpressionSignature)],
  };
  return profile;
}
...
function buildPackageRecord() {
  ...
  const phase4Review = repsAuthoring.prepareRepresentationsFamilyProfilePackageReview(
    phase4Fixture(),
  );
...
function main() {
  ...
  console.log(JSON.stringify({
    ...
    hold_count: 0,
    knowledge_link_only_count: KNOWLEDGE_COUNT,
