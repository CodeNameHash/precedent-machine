...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-closing-conditions.json`;
const WORK3_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'CLOSING_CONDITIONS';
...
const APPROVAL_TEXT =
  ...
  + `(${APPROVE_COUNT} APPROVE / ${HOLD_COUNT} HOLD on subtype partition reconciliation).`;
...
function phase4Fixture() {
  ...
  return {
    closingConditionsAuthoringPhase4FamilyProfilePackageReviewAuthority: boundEnvelope(pinned(
...
function buildProfileFromTemplate(templateProfile, {
  ...
  profile.match_test = {
    ...
    tokens: ['familyclosingconditions', profileMatchToken(requiredExpressionSignature)],
  };
  return profile;
}
...
function buildPackageRecord() {
  ...
  const phase4Review = ccAuthoring.prepareClosingConditionsFamilyProfilePackageReview(
    phase4Fixture(),
  );
