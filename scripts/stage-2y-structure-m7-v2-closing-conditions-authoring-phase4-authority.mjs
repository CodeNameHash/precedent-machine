/**
 * Emit CLOSING_CONDITIONS Phase 4 family profile package review authority from
 * the Phase 2 partition (57 comparator-derived profiles; no Phase 3 chain).
 */
...
import closingConditionsAuthoring from '../lib/canonical-v2/m7-v2-closing-conditions-authoring.js';
...
const PHASE2_PATH = closingConditionsAuthoring.CLOSING_CONDITIONS_PHASE2_AUTHORITY_PATH;
const PROFILE_COUNT = closingConditionsAuthoring.CLOSING_CONDITIONS_PROFILE_COUNT;

const PHASE2_BINDING = Object.freeze({
  path: PHASE2_PATH,
  schema_version: closingConditionsAuthoring.CLOSING_CONDITIONS_PHASE2_AUTHORITY_SCHEMA,
  record_id_field: 'closing_conditions_authoring_phase2_authority_id',
  record_id: closingConditionsAuthoring.CLOSING_CONDITIONS_PHASE2_AUTHORITY_ID,
  byte_length: closingConditionsAuthoring.CLOSING_CONDITIONS_PHASE2_AUTHORITY_BYTES,
  sha256: closingConditionsAuthoring.CLOSING_CONDITIONS_PHASE2_AUTHORITY_SHA256,
});
...
function phase2Fixture() {
  const closingConditionsAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = closingConditionsAuthoringPhase2Authority.record;
...
function packageProfileKey(classificationBucket, proposedProfileKey) {
  return ['PROFILE', 'CLOSING_CONDITIONS', classificationBucket, proposedProfileKey].join(':');
}

function reviewFlagsForBucket(bucket) {
  ...
  if (bucket === 'FRUSTRATION') {
    flags.push('METSERA_FRUSTRATION_BRANCH_DISPOSITION_REQUIRED');
  }
  if (bucket === 'NO_MAE') {
    flags.push('CROSS_FAMILY_LINK_ONLY_MAE_DEFINITION');
  }
  if (bucket === 'COVENANT_COMPLIANCE') {
    flags.push('CROSS_FAMILY_LINK_ONLY_REPRESENTATIONS_BRINGDOWN');
  }
  if (bucket === 'OFFICER_CERTIFICATE') {
    flags.push('OFFICER_CERTIFICATE_BUNDLE_COMPLETENESS_REVIEW');
  }
...
function buildSchedule(phase2Proposal) {
  ...
  return profiles.map((profile) => {
    ...
    return {
      ...
      review_flags: reviewFlagsForBucket(bucket),
      missing_required_field_keys: [],
    };
  });
}
...
function main() {
  const phase2Proposal = closingConditionsAuthoring.prepareClosingConditionsPhase2FamilyProposal(
    phase2Fixture(),
  );
  ...
  const holdFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.some((flag) => (
      closingConditionsAuthoring.CLOSING_CONDITIONS_WORK3_HOLD_REVIEW_FLAGS.includes(flag)
    )),
  ).length;
  if (holdFlagCount !== closingConditionsAuthoring.CLOSING_CONDITIONS_WORK3_HOLD_COUNT) {
    throw new Error(
      `Expected ${closingConditionsAuthoring.CLOSING_CONDITIONS_WORK3_HOLD_COUNT} hold rows, got ${holdFlagCount}`,
    );
  }
  ...
  const unsigned = {
    ...
    design_basis: {
      ...
      public_facade: 'prepareClosingConditionsFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      closing_conditions_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      exported_function: 'prepareClosingConditionsFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-closing-conditions-authoring.js',
      phase2_internal_function: 'prepareClosingConditionsPhase2FamilyProposal',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 CLOSING_CONDITIONS family profile package review returns unapproved proposals without Work3 identities',
  ...
  const record = {
    ...unsigned,
    closing_conditions_authoring_phase4_family_profile_package_review_authority_id: recordId,
  };
