#!/usr/bin/env node
/**
 * Emit TAX_MATTERS Phase 2 authoring authority v2.
 *
 * Seventeen governed M4 claims across four comparator deals (Concho, Skechers,
 * Skywater, TopBuild). Claim-scale partition — one terminal per governed claim.
 * Closing Conditions tax-opinion receipt-only conditions stay link-only (Q02).
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/TAX_MATTERS.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/TAX_MATTERS.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-tax-matters-authoring.js';
const TEST_PATH = 'tests/stage-2y-structure-m7-v2-repair-tax-matters-work3.test.js';
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  INTENDED_TAX_TREATMENT_KIND: 'INTENDED_TAX_TREATMENT',
  TAX_TREATMENT_PROTECTION_COVENANT: 'TAX_TREATMENT_PROTECTION',
  TAX_OPINION_COOPERATION_COVENANT: 'TAX_OPINION_COOPERATION',
  TRANSFER_TAX_ALLOCATION: 'TRANSFER_TAX_ALLOCATION',
};
...
const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    agreement_id: null,
    deal: null,
    disposition: 'LINK_ONLY_NOT_A_TAX_MATTERS_TERMINAL',
    owner_family_key: 'CLOSING_CONDITIONS',
    ruling_id: 'M5-RULING-ONE-SEMANTIC-OWNER',
    section_reference: null,
    subject: 'TAX_OPINION_RECEIPT_ONLY_CLOSING_CONDITION',
  },
];
...
function requiredExpressionSignature(claim, bucket) {
  ...
  return `TAX_MATTERS::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'TAX_MATTERS');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_TAX_MATTERS_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for TAX_MATTERS Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_TAX_MATTERS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_TAX_MATTERS_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_FOUR_EXAMPLES_INTENDED_TAX_TREATMENT_WHILE_CLAIM_KEYS_POPULATE_FOUR_SUBTYPE_BUCKETS',
    ...
    implementation_contract: {
      ...
      public_error_contract: {
        missing_facade: 'TAX_MATTERS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareTaxMattersPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved TAX_MATTERS partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'tax_matters_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'Tax Matters first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    tax_matters_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'tax_matters_authoring_phase2_authority_id',
    record_id: record.tax_matters_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
