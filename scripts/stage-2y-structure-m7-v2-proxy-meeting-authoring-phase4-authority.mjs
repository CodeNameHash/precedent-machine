#!/usr/bin/env node
/**
 * Emit PROXY_MEETING Phase 4 family profile package review authority from
...
const SCHEMA = proxyMeetingAuthoring.PROXY_MEETING_PHASE4_AUTHORITY_SCHEMA;
const OUT_PATH = proxyMeetingAuthoring.PROXY_MEETING_PHASE4_AUTHORITY_PATH;
const PHASE2_PATH = proxyMeetingAuthoring.PROXY_MEETING_PHASE2_AUTHORITY_PATH;
...
function phase2Fixture() {
  const proxyMeetingAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = proxyMeetingAuthoringPhase2Authority.record;
...
function main() {
  ...
  const terminalsBySourceUnitKey = new Map(
    fixture.proxyMeetingAuthoringPhase2Authority.record
  ...
  const unsigned = {
    ...
    design_basis: {
      ...
      public_facade: 'prepareProxyMeetingFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      proxy_meeting_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      exported_function: 'prepareProxyMeetingFamilyProfilePackageReview',
      module_path: MODULE_PATH,
      phase2_internal_function: 'prepareProxyMeetingPhase2FamilyProposal',
    ...
    candidate_output_contract: {
      candidate_state: proxyMeetingAuthoring.PROXY_MEETING_PHASE4_CANDIDATE_STATE,
      exact_keys: [
      ...
      schema_version:
        proxyMeetingAuthoring.PROXY_MEETING_PHASE4_CANDIDATE_SCHEMA,
      unresolved_items: [
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 PROXY_MEETING family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_31_PROFILES_31_M4_CLAIMS_0_SILENT_AS_PHASE4_PARENT',
  ...
  const record = {
    ...unsigned,
    proxy_meeting_authoring_phase4_family_profile_package_review_authority_id: recordId,
  };
