...
/**
 * Emit the NO_OTHER_REPS_FRAUD Ben inventory disposition, session receipt, family
 * package seal receipt and the three Work3 successor authorities.
 *
 * All 36 rows are APPROVE. The unresolved four-bucket subtype partition is carried as
 * LEGAL_GROUPING_REVIEW_REQUIRED with seal disposition_status PENDING_LEGAL_REVIEW, and
 * the two link censuses are acknowledged per row, not held.
 */
...
const DISPOSITION_PATH =
  `${CONTROL}/m7-v2-repair-no-other-reps-fraud-36-profile-inventory-disposition.json`;
const SESSION_PATH =
  `${CONTROL}/m7-v2-repair-no-other-reps-fraud-ben-inventory-session-receipt.json`;
const BEN_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-no-other-reps-fraud-ben-inventory-session-successor-authority.json`;
const SEAL_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-no-other-reps-fraud-family-package-seal-successor-authority.json`;
const SEAL_RECEIPT_PATH =
  `${CONTROL}/m7-v2-repair-no-other-reps-fraud-family-package-seal-receipt.json`;
const REGISTRATION_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-no-other-reps-fraud-registration-successor-authority.json`;
const RULINGS_PATH =
  'docs/codex-program/notes/NO-OTHER-REPS-FRAUD-BEN-RULINGS-Q01-Q03-2026-08-24.md';

const DISPOSITION_SCHEMA =
  'STAGE_2Y_M7_V2_NO_OTHER_REPS_FRAUD_36_PROFILE_INVENTORY_DISPOSITION/V1';
const SESSION_SCHEMA =
  'STAGE_2Y_M7_V2_NO_OTHER_REPS_FRAUD_BEN_INVENTORY_SESSION_RECEIPT/V1';
const BEN_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_NO_OTHER_REPS_FRAUD_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY/V1';
const SEAL_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_NO_OTHER_REPS_FRAUD_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY/V1';
const SEAL_RECEIPT_SCHEMA =
  'STAGE_2Y_M7_V2_NO_OTHER_REPS_FRAUD_FAMILY_PACKAGE_SEAL_RECEIPT/V1';
const REGISTRATION_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_NO_OTHER_REPS_FRAUD_REGISTRATION_SUCCESSOR_AUTHORITY/V1';
const SEAL_CANDIDATE_SCHEMA =
  'M7_V2_NO_OTHER_REPS_FRAUD_WORK3_FAMILY_PACKAGE_SEAL_CANDIDATE/V1';
const DISPOSITION_CANDIDATE_SCHEMA =
  'M7_V2_NO_OTHER_REPS_FRAUD_WORK3_BEN_INVENTORY_SESSION_DISPOSITION_CANDIDATE/V1';
const DISPOSITION_CANDIDATE_STATE =
  'BEN_36_PROFILE_INVENTORY_DISPOSITION_CAPTURED_PACKAGE_SEAL_NOT_RECORDED';
...
const REGISTERED_SUBTYPE_BUCKET_COUNT = 4;
...
const profileDispositions = packet.profile_review_items.map((item) => ({
  ...
  shared_source_citation_link_only: item.review_flags.includes(
    'SHARED_SOURCE_CITATION_LINK_ONLY',
  ),
  cross_family_representations_link_only: item.review_flags.includes(
    'CROSS_FAMILY_REPRESENTATIONS_LINK_ONLY',
  ),
}));
const approvedCount = profileDispositions.filter((row) => row.disposition === 'APPROVE').length;
const sharedCitationCount = profileDispositions.filter(
  (row) => row.shared_source_citation_link_only,
).length;
const crossFamilyCount = profileDispositions.filter(
  (row) => row.cross_family_representations_link_only,
).length;
if (
  ...
  throw new Error(
    `disposition census drift: ${approvedCount} approve / ${sharedCitationCount} shared / ${crossFamilyCount} cross`,
  );
}
...
const sessionUnsigned = {
  ...
  session_classification: 'NO_OTHER_REPS_FRAUD_36_PROFILE_INVENTORY_BEN_REVIEW',
...
const benAuthority = authority(
  ...
  'work3_no_other_reps_fraud_ben_inventory_session_successor_authority_id',
  {
    authority_classification:
      'WORK3_NO_OTHER_REPS_FRAUD_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY',
    authority_state:
    ...
    implementation_contract: {
      exported_function: 'prepareNoOtherRepsFraudWork3BenInventorySessionDisposition',
...
const sealAuthority = authority(
  ...
  'work3_no_other_reps_fraud_family_package_seal_successor_authority_id',
  {
    authority_classification:
      'WORK3_NO_OTHER_REPS_FRAUD_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareNoOtherRepsFraudWork3FamilyPackageSeal',
      exact_outer_input_keys: [
    ...
    schema_review_candidate_contract: {
      ...
      candidate_state:
        'BEN_NO_OTHER_REPS_FRAUD_FAMILY_PACKAGE_SEAL_CAPTURED_REGISTRATION_NOT_RECORDED',
      review_accounting_exact_values: {
        cross_family_representations_link_only_count: CROSS_FAMILY_COUNT,
        inventory_review_count: 1,
...
const sealReceiptUnsigned = {
  ...
  seal_classification: 'NO_OTHER_REPS_FRAUD_WORK3_FAMILY_PACKAGE_SEAL',
  completion_state: 'COMPLETE',
  reviewer: 'BEN_GOODCHILD',
...
const sealReceipt = {
  ...sealReceiptUnsigned,
  no_other_reps_fraud_family_package_seal_receipt_id:
...
const registrationAuthority = authority(
  ...
  'work3_no_other_reps_fraud_registration_successor_authority_id',
  {
    authority_classification: 'WORK3_NO_OTHER_REPS_FRAUD_REGISTRATION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareNoOtherRepsFraudWork3FamilyPackageRegistration',
      exact_outer_input_keys: [
    ...
    schema_review_candidate_contract: {
      schema_version:
        'M7_V2_NO_OTHER_REPS_FRAUD_WORK3_FAMILY_PACKAGE_REGISTRATION_CANDIDATE/V1',
      candidate_state:
