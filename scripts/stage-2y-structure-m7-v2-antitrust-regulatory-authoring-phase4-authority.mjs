/**
 * Emit ANTITRUST_REGULATORY Phase 4 family profile package review authority from
 * the Phase 2 partition (70 claim-scale profiles; no Phase 3 chain).
 */
...
const PHASE2_PATH = antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_PATH;
const PROFILE_COUNT = antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PROFILE_COUNT;

const PHASE2_BINDING = Object.freeze({
  path: PHASE2_PATH,
  schema_version: antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_SCHEMA,
  record_id_field: 'antitrust_regulatory_authoring_phase2_authority_id',
  record_id: antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_ID,
  byte_length: antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_BYTES,
  sha256: antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_SHA256,
});

/** Buckets with no sealed M5 subtype label — every row in them is an honest hold. */
const BUCKETS_WITHOUT_SEALED_M5_LABEL = Object.freeze([
  'FILING_TIMING_STANDARD',
  'WITHDRAWAL_REFILING',
  'NOTIFICATION',
]);

const PHASE2_AUTHORITY_PATH =
...
function loadPhase2Residuals() {
  const phase2 = readJson(PHASE2_AUTHORITY_PATH);
  return {
    nonHsrKeys: new Set(phase2.filing_regime_residuals.non_hsr_regime_source_unit_keys),
    oneSidedKeys: new Set(phase2.obligor_capacity_residuals.one_sided_obligor_source_unit_keys),
  };
}
...
function phase2Fixture() {
  const antitrustRegulatoryAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = antitrustRegulatoryAuthoringPhase2Authority.record;
...
function packageProfileKey(classificationBucket, proposedProfileKey) {
  return ['PROFILE', 'ANTITRUST_REGULATORY', classificationBucket, proposedProfileKey].join(':');
}

function reviewFlagsForProfile(profile, residuals) {
  const bucket = profile.canonical_tuple.classification_path[1];
  const sourceUnitKey = profile.source_unit_keys[0];
  const flags = ['LEGAL_GROUPING_REVIEW_REQUIRED'];
  if (BUCKETS_WITHOUT_SEALED_M5_LABEL.includes(bucket)) {
    flags.push('M5_SUBTYPE_BUCKET_PARTITION_UNRESOLVED');
  }
  if (residuals.nonHsrKeys.has(sourceUnitKey)) {
    flags.push('NON_HSR_FILING_REGIME');
  }
  if (residuals.oneSidedKeys.has(sourceUnitKey)) {
    flags.push('ONE_SIDED_OBLIGOR_CAPACITY');
  }
  return flags.sort(compareStrings);
}

function buildSchedule(phase2Proposal, residuals) {
  ...
  return profiles.map((profile) => {
    ...
    return {
      ...
      review_flags: reviewFlagsForProfile(profile, residuals),
      missing_required_field_keys: [],
    };
  });
}
...
function main() {
  const phase2Proposal = antitrustRegulatoryAuthoring.prepareAntitrustRegulatoryPhase2FamilyProposal(
    phase2Fixture(),
  );
  const residuals = loadPhase2Residuals();
  const profileReviewSchedule = buildSchedule(phase2Proposal, residuals);
  ...
  const m5SubtypeFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.includes('M5_SUBTYPE_BUCKET_PARTITION_UNRESOLVED'),
  ).length;
  const nonHsrFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.includes('NON_HSR_FILING_REGIME'),
  ).length;
  const oneSidedFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.includes('ONE_SIDED_OBLIGOR_CAPACITY'),
  ).length;
  if (
    m5SubtypeFlagCount !== antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_M5_SUBTYPE_UNRESOLVED_PROFILE_COUNT
    || nonHsrFlagCount !== antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_NON_HSR_FILING_REGIME_PROFILE_COUNT
    || oneSidedFlagCount !== antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_ONE_SIDED_OBLIGOR_PROFILE_COUNT
  ) {
    throw new Error(
      `Flag census drift: m5=${m5SubtypeFlagCount} non_hsr=${nonHsrFlagCount} one_sided=${oneSidedFlagCount}`,
    );
  }
  ...
  const firstLegalStop = {
    ...
    m5_subtype_bucket_partition_unresolved_flag_count: m5SubtypeFlagCount,
    non_hsr_filing_regime_flag_count: nonHsrFlagCount,
    one_sided_obligor_capacity_flag_count: oneSidedFlagCount,
  ...
  const unsigned = {
    ...
    design_basis: {
      ...
      public_facade: 'prepareAntitrustRegulatoryFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      antitrust_regulatory_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      exported_function: 'prepareAntitrustRegulatoryFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-antitrust-regulatory-authoring.js',
      phase2_internal_function: 'prepareAntitrustRegulatoryPhase2FamilyProposal',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 ANTITRUST_REGULATORY family profile package review returns unapproved proposals without Work3 identities',
  ...
  const record = {
    ...unsigned,
    antitrust_regulatory_authoring_phase4_family_profile_package_review_authority_id: recordId,
  };
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      legal_grouping_review_flag_count: legalGroupingFlagCount,
      m5_subtype_flag_count: m5SubtypeFlagCount,
      non_hsr_flag_count: nonHsrFlagCount,
      one_sided_flag_count: oneSidedFlagCount,
