#!/usr/bin/env node
/**
 * Emit the PROXY_MEETING Ben inventory disposition, session receipt, and the
 * three remaining Work3 successor authorities plus the family package seal receipt.
 *
 * Disposition is APPROVE on all 31 profiles with the subtype-grouping question
 * stamped as an open hold. Q01-Q03 reuse the sealed M5 programme rulings the family
 * role schema already binds; no Proxy / Meeting-specific ruling is invented.
 */
import { readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import canonicalModule from '../lib/canonical-v2/canonical-bytes.js';
import proxyMeetingAuthoring from '../lib/canonical-v2/m7-v2-proxy-meeting-authoring.js';
...
const PACKET_PATH =
  `${CONTROL}/m7-v2-repair-proxy-meeting-31-profile-inventory-review-packet-draft.json`;
const INVENTORY_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-proxy-meeting-unapproved-inventory-review-authority.json`;
const DISPOSITION_PATH =
  `${CONTROL}/m7-v2-repair-proxy-meeting-31-profile-inventory-disposition.json`;
const SESSION_PATH =
  `${CONTROL}/m7-v2-repair-proxy-meeting-ben-inventory-session-receipt.json`;
const BEN_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-proxy-meeting-ben-inventory-session-successor-authority.json`;
const SEAL_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-proxy-meeting-family-package-seal-successor-authority.json`;
const SEAL_RECEIPT_PATH =
  `${CONTROL}/m7-v2-repair-proxy-meeting-family-package-seal-receipt.json`;
const REGISTRATION_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-proxy-meeting-registration-successor-authority.json`;
const RULINGS_PATH = `${CONTROL}/m5-programme-rulings.json`;
const ROLE_SCHEMA_PATH = `${CONTROL}/family-role-schemas/PROXY_MEETING.json`;

const DISPOSITION_SCHEMA =
  'STAGE_2Y_M7_V2_PROXY_MEETING_31_PROFILE_INVENTORY_DISPOSITION/V1';
const SESSION_SCHEMA =
  'STAGE_2Y_M7_V2_PROXY_MEETING_BEN_INVENTORY_SESSION_RECEIPT/V1';
const BEN_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_PROXY_MEETING_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY/V1';
const SEAL_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_PROXY_MEETING_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY/V1';
const SEAL_RECEIPT_SCHEMA =
  'STAGE_2Y_M7_V2_PROXY_MEETING_FAMILY_PACKAGE_SEAL_RECEIPT/V1';
const REGISTRATION_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_PROXY_MEETING_REGISTRATION_SUCCESSOR_AUTHORITY/V1';
const SEAL_CANDIDATE_SCHEMA =
  'M7_V2_PROXY_MEETING_WORK3_FAMILY_PACKAGE_SEAL_CANDIDATE/V1';
...
const SEAL_CANDIDATE_STATE =
  'BEN_PROXY_MEETING_FAMILY_PACKAGE_SEAL_CAPTURED_REGISTRATION_NOT_RECORDED';
...
const DIVERGENCE_COUNT =
  proxyMeetingAuthoring.PROXY_MEETING_SUBTYPE_DIVERGENCE_PROFILE_COUNT;
const OUTSIDE_CALIBRATION_COUNT =
  proxyMeetingAuthoring.PROXY_MEETING_OUTSIDE_CALIBRATION_PROFILE_COUNT;
const REGISTERED_BUCKETS =
  proxyMeetingAuthoring.PROXY_MEETING_REGISTERED_SUBTYPE_BUCKET_COUNT;
const POPULATED_BUCKETS =
  proxyMeetingAuthoring.PROXY_MEETING_POPULATED_SUBTYPE_BUCKET_COUNT;
...
const inventoryAuthorityBinding = binding(
  ...
  inventoryAuthority,
  'work3_proxy_meeting_unapproved_inventory_review_authority_id',
);
...
const sessionUnsigned = {
  ...
  session_classification: 'PROXY_MEETING_31_PROFILE_INVENTORY_BEN_REVIEW',
...
const benAuthority = authority(
  ...
  'work3_proxy_meeting_ben_inventory_session_successor_authority_id',
  {
    authority_classification:
      'WORK3_PROXY_MEETING_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareProxyMeetingWork3BenInventorySessionDisposition',
    ...
    schema_review_candidate_contract: {
      schema_version:
        'M7_V2_PROXY_MEETING_WORK3_BEN_INVENTORY_SESSION_DISPOSITION_CANDIDATE/V1',
...
const sealAuthority = authority(
  ...
  'work3_proxy_meeting_family_package_seal_successor_authority_id',
  {
    authority_classification: 'WORK3_PROXY_MEETING_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareProxyMeetingWork3FamilyPackageSeal',
      exact_outer_input_keys: [
...
// Mirrors prepareProxyMeetingWork3FamilyPackageSeal exactly; the test asserts
...
const sealReceiptUnsigned = {
  ...
  seal_classification: 'PROXY_MEETING_WORK3_FAMILY_PACKAGE_SEAL',
  completion_state: 'COMPLETE',
  reviewer: 'BEN_GOODCHILD',
...
const sealReceipt = {
  ...sealReceiptUnsigned,
  proxy_meeting_family_package_seal_receipt_id:
...
const registrationAuthority = authority(
  ...
  'work3_proxy_meeting_registration_successor_authority_id',
  {
    authority_classification: 'WORK3_PROXY_MEETING_REGISTRATION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareProxyMeetingWork3FamilyPackageRegistration',
      exact_outer_input_keys: [
    ...
    schema_review_candidate_contract: {
      schema_version: 'M7_V2_PROXY_MEETING_WORK3_FAMILY_PACKAGE_REGISTRATION_CANDIDATE/V1',
      candidate_state:
