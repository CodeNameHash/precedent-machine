/**
 * Emit GUARANTY_FINANCING_PARTY Phase 4 family profile package review authority
 * from the Phase 2 partition (five provision-example profiles; no Phase 3 chain).
 */
...
const OUT_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-contract-guaranty-financing-party-authoring-phase4-family-profile-package-review-authority.json';
const PHASE2_PATH =
...
function phase2Fixture() {
  const guarantyFinancingPartyAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = guarantyFinancingPartyAuthoringPhase2Authority.record;
...
function reviewFlagsForProfile() {
  return ['LEGAL_GROUPING_REVIEW_REQUIRED'];
}
  ...
  return profiles.map((profile) => {
    ...
    return {
      ...
      proposed_validation: {
        ...
        issue_codes: [],
        no_comparison_authority: profile.m4_claim_ids.length === 0
          ? 'M4_SILENT_PROVISION_EXAMPLE'
          : null,
      },
      review_flags: reviewFlagsForProfile(),
      missing_required_field_keys: [],
    };
  });
}
...
function main() {
  ...
  if (profileCount !== 5) {
    throw new Error(`Expected 5 profiles, got ${profileCount}`);
  }
  ...
  const unsigned = {
    ...
    design_basis: {
      ...
      public_facade: 'prepareGuarantyFinancingPartyFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      guaranty_financing_party_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      error_precedence: [
        {
          code: 'M7_V2_GUARANTY_FINANCING_PARTY_PHASE4_REVIEW_CONTRACT',
          condition: 'NULL_NON_OBJECT_MISSING_EXTRA_OR_REORDERED_OUTER_INPUT_KEYS',
          order: 1,
        },
        {
          code: 'M7_V2_GUARANTY_FINANCING_PARTY_PHASE4_REVIEW_AUTHORITY',
          condition: 'AUTHORITY_ENVELOPE_BINDING_PHYSICAL_BYTES_SELF_ID_OR_PARENT_PIN_DRIFT',
          order: 2,
        },
        {
          code: 'M7_V2_GUARANTY_FINANCING_PARTY_PHASE4_PHASE2_PROPOSAL',
          condition: 'FRESH_PHASE2_PROPOSAL_FAILS_OR_5_PROFILE_BINDING_DRIFTS',
          order: 3,
        },
        {
          code: 'M7_V2_GUARANTY_FINANCING_PARTY_PHASE4_PROFILE_SCHEDULE',
          condition: 'TUPLE_PROFILE_SOURCE_CLAIM_OR_PROFILE_ORDER_DRIFT',
          order: 4,
        },
        {
          code: 'M7_V2_GUARANTY_FINANCING_PARTY_PHASE4_REVIEW_OUTPUT',
      ...
      exported_function: 'prepareGuarantyFinancingPartyFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-guaranty-financing-party-authoring.js',
      phase2_internal_function: 'prepareGuarantyFinancingPartyPhase2FamilyProposal',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 GUARANTY_FINANCING_PARTY family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_5_PROFILES_1_M4_CLAIM_4_SILENT_AS_PHASE4_PARENT',
