#!/usr/bin/env node
/**
 * Regenerate the on-disk CONSIDERATION Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-consideration.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'CONSIDERATION';
...
const APPROVAL_TEXT =
  'Ben approves the CONSIDERATION 7-profile Work3 package inventory; subtype grouping '
  + 'review pending.';
...
function buildPhase4Review() {
  ...
  return considerationAuthoring.prepareConsiderationFamilyProfilePackageReview({
    considerationAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    considerationAuthoringPhase2Authority: phase2Authority,
