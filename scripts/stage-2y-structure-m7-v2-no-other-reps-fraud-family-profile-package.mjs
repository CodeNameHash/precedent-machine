...
/**
 * Regenerate the on-disk NO_OTHER_REPS_FRAUD Work3 family profile package from the
 * Phase 4 review candidate (36 profiles) and the lawful Work3 package template.
 */
...
const PACKAGE_PATH =
  `${CONTROL}/m7-v2-repair-family-work3-profile-package-no-other-reps-fraud.json`;
const WORK3_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'NO_OTHER_REPS_FRAUD';
...
const APPROVAL_TEXT =
  ...
  + `(${PROFILE_COUNT} APPROVE / 0 HOLD; the subtype partition across the four sealed M5 `
  + 'buckets remains LEGAL_GROUPING_REVIEW_REQUIRED on every row, '
  + `${SHARED_CITATION_COUNT} rows share an authored citation with another governed claim, `
  + `and ${CROSS_FAMILY_COUNT} rows are link-only to REPRESENTATIONS on a shared printed section).`;
...
function phase4Fixture() {
  ...
  return {
    noOtherRepsFraudAuthoringPhase4FamilyProfilePackageReviewAuthority: boundEnvelope(pinned(
...
function buildPackageRecord() {
  ...
  const phase4Review = norAuthoring.prepareNoOtherRepsFraudFamilyProfilePackageReview(
    phase4Fixture(),
  );
