#!/usr/bin/env node
/**
 * Emit INTERIM_OPERATING Phase 2 authoring authority v2.
 *
 * One hundred thirteen governed M4 claims across six comparator deals (Concho, Metsera, Red Hat, Skechers, Skywater, TopBuild). Claim-scale partition — one terminal per governed claim. Dividends coordination covenant and dividend-restriction limbs embedded in interim operating articles stay owned here under Q02.
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/INTERIM_OPERATING.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/INTERIM_OPERATING.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-interim-operating-authoring.js';
const TEST_PATH = 'tests/stage-2y-structure-m7-v2-repair-interim-operating-work3.test.js';
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  IOC_RESTRICTION_PRESENT: 'RESTRICTIVE_COVENANT',
};
...
function classificationPath(bucket) {
  return ['INTERIM_OPERATING', bucket];
}
...
function discriminatorToken(claim) {
  ...
  const category = typeof attributes.restriction_category === 'string'
    ? attributes.restriction_category
    : 'UNSPECIFIED';
  return `${category}_${claim.claim_occurrence_id.slice(0, 8)}`;
}
...
function requiredExpressionSignature(claim, bucket, sourceCitation) {
  ...
  return `INTERIM_OPERATING::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'INTERIM_OPERATING');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_INTERIM_OPERATING_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for INTERIM_OPERATING Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_INTERIM_OPERATING_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_INTERIM_OPERATING_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      sparse_comparator_stress: 'ONE_HUNDRED_THIRTEEN_GOVERNED_CLAIMS_ACROSS_SIX_COMPARATOR_DEALS',
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_SIX_EXAMPLES_RESTRICTIVE_COVENANT_WHILE_SEALED_SCHEMA_ADMITS_FIVE_SUBTYPE_BUCKETS',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareInterimOperatingPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade: 'INTERIM_OPERATING Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareInterimOperatingPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved INTERIM_OPERATING partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'interim_operating_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'Interim Operating first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    interim_operating_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'interim_operating_authoring_phase2_authority_id',
    record_id: record.interim_operating_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
