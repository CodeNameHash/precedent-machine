#!/usr/bin/env node
/**
 * scripts/canonical-v2-modiv-first-live-extraction-run.mjs
 *
 * Breadth slice: THIRD-DEAL live native extraction run, against Modiv
 * Industrial, Inc. / Global Net Lease, Inc. (REIT stock-for-stock merger),
 * parameterized from scripts/canonical-v2-skechers-first-live-extraction-run.mjs
 ...
 *
 * PINNING FETCH. This is the FIRST fetch of the Modiv/Global Net Lease
 * filing in this repo -- there is no pinned hash yet. This script performs
 * the pinning fetch itself: it records the response byte length and
 * SHA-256 of the raw HTML it receives, refuses to proceed on a non-200
 * status or any redirect (SEC EDGAR intake capture rules -- see
 ...
 *
 * SECTION LOCATION IS NOT ASSUMED. Unlike Skechers/QXO (dense,
 * blank-line-free Article III prose that defeated the blank-line-anchored
 * heading detector), a mechanical pre-flight scan of this filing's
 * sectionizer tree (470 nodes: 1 ROOT, 8 ARTICLE, 101 SECTION, 360
 * SUBSECTION) found real, cleanly-scoped decimal SECTION nodes throughout,
 * including a SECTION node at reference "3.2" whose heading is exactly
 * "Capitalization" (Article III, Representations and Warranties of the
 * Company Parties). This driver still does NOT assume that outcome: it
 * requires a caller-supplied `--section-ref` that must resolve, via the
 * real findSectionByReference(), to a SECTION node whose heading matches
 * /capital/i before any model call is made, and aborts with the full tree
 * shape printed if that assertion fails.
 *
 ...
 *
 * Usage:
 *   node scripts/canonical-v2-modiv-first-live-extraction-run.mjs \
 *     --raw-html <path to fetched .htm file, OR omit to fetch live> \
 *     --retrieval-url <the exact https://www.sec.gov/... URL> \
 *     --section-ref <section reference to extract, e.g. "3.2"> \
 ...
 * against MODIV_URL below (refusing redirects/non-200), and writes the
 * fetched bytes to <out-dir>/modiv-raw-fetched.htm for reproducibility.
 */
...
// NO PIN EXISTS YET for this filing -- this run performs the pinning fetch
// itself (see header note). Unlike the Skechers/QXO scripts, there is no
// EXPECTED_RAW_BYTES_SHA256 to check the fetch against; the value computed
// below IS the pin, printed prominently for a follow-up commit to record
// into tests/fixtures/canonical-v2/modiv-first-live-run/intake-pin.json.

function parseArgs(argv) {
  const out = { model: 'sonnet', sectionRef: '3.2' };
  ...
  if (!out.retrievalUrl) out.retrievalUrl = MODIV_URL;
  return out;
}
    ...
    messages: {
      async create(params) {
        ...
        writeFileSync(resolve(fixtureOutPath), JSON.stringify({
          ...
          note: 'Modiv Industrial / Global Net Lease FIRST live run (third deal, breadth slice), '
            + 'PROMPT_VERSION 4. request_messages omitted here; raw_response_text is the model\'s '
            + 'literal output including its ```json fence, byte-for-byte as returned.',
...
async function fetchPinned(url, outDir) {
  ...
  const savedPath = resolve(outDir, 'modiv-raw-fetched.htm');
...
async function main() {
  ...
  process.stderr.write(`[modiv-run1] PROMPT_VERSION = ${PROMPT_VERSION}\n`);
  ...
  if (args.rawHtml) {
    ...
    process.stderr.write(`[modiv-run1] performing PINNING FETCH of ${args.retrievalUrl}\n`);
    ...
    process.stderr.write(`[modiv-run1] saved raw bytes to ${fetched.savedPath}\n`);
  }
  const bytesSha256 = sha256Hex(rawBytes);
  process.stderr.write(`[modiv-run1] *** PIN *** raw bytes length = ${rawBytes.length}, sha256 = ${bytesSha256}\n`);
  process.stderr.write('[modiv-run1] NO PRE-EXISTING PIN to check against -- this run IS the pinning fetch. '
    + 'Record the length/sha256 above into tests/fixtures/canonical-v2/modiv-first-live-run/intake-pin.json in a follow-up commit.\n');
  ...
  const dealKey = `deal:modiv-first-live-run:${sha256Hex(args.retrievalUrl).slice(0, 16)}`;
  const dealAdmissionId = sha256Hex(`deal-admission-modiv1:${args.retrievalUrl}`);
  ...
  process.stderr.write(`[modiv-run1] document_hash (raw fetched-file sha256) = ${documentHash}\n`);
  process.stderr.write(`[modiv-run1] canonical_text_sha256 = ${conversion.canonical_text_sha256}\n`);
  process.stderr.write(`[modiv-run1] canonical_text_byte_length = ${conversion.canonical_text_byte_length}\n`);

  writeFileSync(resolve(outDir, 'intake-pin.json'), JSON.stringify({
    retrieval_url: args.retrievalUrl,
    status_code: 200,
    redirect_count: 0,
    raw_bytes_length: rawBytes.length,
    raw_bytes_sha256: bytesSha256,
    retrieved_at: new Date().toISOString(),
    ...
    note: 'PINNING FETCH -- first fetch of this filing in this repo, no prior pin existed to verify against.',
  }, null, 2));
  ...
  process.stderr.write(`[modiv-run1] sectionizer node count = ${tree.nodes.length}, by kind = ${JSON.stringify(kindCounts)}\n`);
  writeFileSync(resolve(outDir, 'section-location-scan.json'), JSON.stringify({
    node_count: tree.nodes.length,
    kind_counts: kindCounts,
    article_and_section_nodes: tree.nodes
      .filter((n) => n.kind === 'ARTICLE' || n.kind === 'SECTION')
      .map((n) => ({
  ...
  if (node.kind !== 'SECTION' || !/capital/i.test(node.heading || '')) {
    throw new Error(
      ...
      + 'Expected a SECTION node with a heading matching /capital/i. Aborting.',
    );
  }
  process.stderr.write(`[modiv-run1] resolved section ${args.sectionRef}: kind=${node.kind} heading=${JSON.stringify(node.heading)} start=${node.start} end=${node.end} toc_corroborated=${node.toc_corroborated}\n`);
  const sourceBuf = Buffer.from(fullText, 'utf8');
  const governedSectionText = sourceBuf.slice(node.start, node.end).toString('utf8');
  process.stderr.write(`[modiv-run1] section opening excerpt: ${JSON.stringify(governedSectionText.slice(0, 200))}\n`);
  ...
  process.stderr.write('[modiv-run1] starting LIVE extraction call...\n');
  ...
  process.stderr.write(`[modiv-run1] extraction run complete in ${runWallClockMs}ms\n`);
  ...
  if (!args.agreementDate) {
    process.stderr.write('[modiv-run1] WARNING: no --agreement-date supplied; passing agreement_date=null (open world for TEMPORAL family)\n');
  }
  ...
  process.stderr.write('[modiv-run1] === SUMMARY ===\n');
...
main().catch((err) => {
  process.stderr.write(`[modiv-run1] FAILED: ${err && err.stack ? err.stack : err}\n`);
  process.exitCode = 1;
});
