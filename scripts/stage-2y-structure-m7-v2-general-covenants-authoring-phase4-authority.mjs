/**
 * Emit GENERAL_COVENANTS Phase 4 family profile package review authority from
 * the Phase 2 partition (54 comparator-derived profiles; no Phase 3 reference chain).
 */
...
function phase2Fixture() {
  const generalCovenantsAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = generalCovenantsAuthoringPhase2Authority.record;
...
function reviewFlagsForProfile(terminal) {
  if (terminal.classification_bucket === 'ACCESS') {
    return ['ACCESS_SCOPE_WORK1_ITEM_44_REVIEW_UNAPPROVED'];
  }
  return [];
}

function buildSchedule(phase2Proposal, phase2Authority) {
  const terminals = phase2Authority.source_terminal_successor_contract
  ...
  return profiles.map((profile) => {
    ...
    if (!terminal) {
      throw new Error(`Missing terminal for ${profile.proposed_profile_key}`);
    }
    const bucket = profile.canonical_tuple.classification_path[2];
    return {
      ...
      review_flags: reviewFlagsForProfile(terminal),
      missing_required_field_keys: [],
    };
  });
}
...
function main() {
  ...
  const phase2Authority = readJson(PHASE2_PATH);
  const profileReviewSchedule = buildSchedule(phase2Proposal, phase2Authority);
  const scheduleBytes = Buffer.from(canonicalJson(profileReviewSchedule), 'utf8');
  const profileCount = profileReviewSchedule.length;
  const accessReviewFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.length > 0,
  ).length;
  if (profileCount !== EXPECTED_PROFILE_COUNT) {
    throw new Error(`Expected ${EXPECTED_PROFILE_COUNT} profiles, got ${profileCount}`);
  }
  ...
  const unsigned = {
    ...
    authority_state:
      'TEMP_ONLY_ZERO_EFFECT_AUTHORITY_54_REVIEW_PROPOSALS_UNAPPROVED_AWAITING_BEN_INVENTORY',
    design_basis: {
      ...
      public_facade: 'prepareGeneralCovenantsFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      general_covenants_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      error_precedence: [
        {
          code: 'M7_V2_GENERAL_COVENANTS_PHASE4_REVIEW_CONTRACT',
          condition: 'NULL_NON_OBJECT_MISSING_EXTRA_OR_REORDERED_OUTER_INPUT_KEYS',
          order: 1,
        },
        {
          code: 'M7_V2_GENERAL_COVENANTS_PHASE4_REVIEW_AUTHORITY',
          condition: 'AUTHORITY_ENVELOPE_BINDING_PHYSICAL_BYTES_SELF_ID_OR_PARENT_PIN_DRIFT',
          order: 2,
        },
        {
          code: 'M7_V2_GENERAL_COVENANTS_PHASE4_PHASE2_PROPOSAL',
          condition: 'FRESH_PHASE2_PROPOSAL_FAILS_OR_54_PROFILE_BINDING_DRIFTS',
          order: 3,
        },
        {
          code: 'M7_V2_GENERAL_COVENANTS_PHASE4_PROFILE_SCHEDULE',
          condition: 'TUPLE_PROFILE_SOURCE_CLAIM_OR_PROFILE_ORDER_DRIFT',
          order: 4,
        },
        {
          code: 'M7_V2_GENERAL_COVENANTS_PHASE4_REVIEW_OUTPUT',
      ...
      exported_function: 'prepareGeneralCovenantsFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-general-covenants-authoring.js',
      phase2_internal_function: 'prepareGeneralCovenantsPhase2FamilyProposal',
      phase3_internal_function: null,
    },
    profile_review_schedule_contract: {
      canonical_tuple_exact_keys: [
        'classification_path',
      ...
      access_scope_review_flag_count: accessReviewFlagCount,
    ...
    candidate_output_contract: {
      ...
      first_legal_stop: {
        ...
        current_required_disposition:
          'KEEP_ALL_54_PROPOSALS_UNAPPROVED_AND_REVIEW_ONLY_AT_PACKAGE_LEVEL',
        family_key: 'GENERAL_COVENANTS',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 GENERAL_COVENANTS family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_54_PROFILES_ADMITTED_AS_PHASE4_PARENT',
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      access_review_flag_count: accessReviewFlagCount,
    }, null, 2)}\n`,
  );
}
