#!/usr/bin/env node
/**
 * Emit the SPECIFIC_PERFORMANCE_REMEDIES Work3 unapproved inventory review authority and the
 * 8-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = specificPerformanceRemediesAuthoring.SPECIFIC_PERFORMANCE_REMEDIES_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_SPECIFIC_PERFORMANCE_REMEDIES_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_SPECIFIC_PERFORMANCE_REMEDIES_8_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'specific_performance_remedies_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'specific_performance_remedies_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  specificPerformanceRemediesAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  specificPerformanceRemediesAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = specificPerformanceRemediesAuthoring
  .prepareSpecificPerformanceRemediesFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_SPECIFIC_PERFORMANCE_REMEDIES_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    specificPerformanceRemedies_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareSpecificPerformanceRemediesWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_SPECIFIC_PERFORMANCE_REMEDIES_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
    ...
    review_accounting_exact_values: {
      ...
      owner_family_disposition_hold_flag_count: flagCount(FLAGS.OWNER_FAMILY),
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three SpecificPerformanceRemedies claim keys under all eight '
      + 'subtype buckets. This packet proposes equitable-relief and remedy-coordination limbs on shared remedy sections claims under BOND_SECURITY_WAIVER and the '
      + 'no-financing-condition acknowledgment under REMEDY_COORDINATION, which diverges from '
      + 'the calibration pack tagging every provision example SPECIFIC_PERFORMANCE_REMEDIES_PACKAGE. The divergence is '
