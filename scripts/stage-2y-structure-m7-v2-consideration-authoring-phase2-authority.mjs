#!/usr/bin/env node
/**
 * Emit CONSIDERATION Phase 2 authoring authority v2.
 *
 * Seven governed M4 claims across four comparator deals (Metsera, Red Hat,
 * Skechers, TopBuild). Claim-scale partition — one terminal per governed claim.
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/CONSIDERATION.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/CONSIDERATION.json';
...
const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    agreement_id: null,
    deal: null,
    disposition: 'LINK_ONLY_NOT_A_CONSIDERATION_TERMINAL',
...
function discriminatorToken(claim) {
  ...
  if (typeof attributes.consideration_term_ref === 'string') {
    return attributes.consideration_term_ref.replace(/[^0-9A-Za-z]/g, '_');
  }
...
function requiredExpressionSignature(claim, bucket) {
  ...
  return `CONSIDERATION::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'CONSIDERATION');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_CONSIDERATION_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for CONSIDERATION Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_CONSIDERATION_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_CONSIDERATION_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_FOUR_EXAMPLES_CONSIDERATION_PACKAGE_WHILE_CLAIM_KEYS_POPULATE_FOUR_SUBTYPE_BUCKETS',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareConsiderationPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade: 'CONSIDERATION Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareConsiderationPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved CONSIDERATION partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'consideration_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'Consideration first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    consideration_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'consideration_authoring_phase2_authority_id',
    record_id: record.consideration_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
