/**
 * Emit DNO_INDEMNIFICATION Phase 4 family profile package review authority from
 * the Phase 2 partition (31 comparator-derived profiles; no Phase 3 reference chain).
 */
...
function phase2Fixture() {
  const dnoIndemnificationAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = dnoIndemnificationAuthoringPhase2Authority.record;
...
function reviewFlagsForProfile(terminal) {
  if (terminal.agreement_id === METSERA_AGREEMENT_ID) {
    return ['LINKED_DUTY_SHARED_SOURCE_REVIEW_UNAPPROVED'];
  }
  return [];
}
...
function main() {
  ...
  if (profileCount !== 31) {
    throw new Error(`Expected 31 profiles, got ${profileCount}`);
  }
  ...
  const unsigned = {
    ...
    authority_state:
      'TEMP_ONLY_ZERO_EFFECT_AUTHORITY_31_REVIEW_PROPOSALS_UNAPPROVED_AWAITING_BEN_INVENTORY',
    design_basis: {
      ...
      public_facade: 'prepareDnoIndemnificationFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      dno_indemnification_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      error_precedence: [
        {
          code: 'M7_V2_DNO_INDEMNIFICATION_PHASE4_REVIEW_CONTRACT',
          condition: 'NULL_NON_OBJECT_MISSING_EXTRA_OR_REORDERED_OUTER_INPUT_KEYS',
          order: 1,
        },
        {
          code: 'M7_V2_DNO_INDEMNIFICATION_PHASE4_REVIEW_AUTHORITY',
          condition: 'AUTHORITY_ENVELOPE_BINDING_PHYSICAL_BYTES_SELF_ID_OR_PARENT_PIN_DRIFT',
          order: 2,
        },
        {
          code: 'M7_V2_DNO_INDEMNIFICATION_PHASE4_PHASE2_PROPOSAL',
          condition: 'FRESH_PHASE2_PROPOSAL_FAILS_OR_31_PROFILE_BINDING_DRIFTS',
          order: 3,
        },
        {
          code: 'M7_V2_DNO_INDEMNIFICATION_PHASE4_PROFILE_SCHEDULE',
          condition: 'TUPLE_PROFILE_SOURCE_CLAIM_OR_PROFILE_ORDER_DRIFT',
          order: 4,
        },
        {
          code: 'M7_V2_DNO_INDEMNIFICATION_PHASE4_REVIEW_OUTPUT',
      ...
      exported_function: 'prepareDnoIndemnificationFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-dno-indemnification-authoring.js',
      phase2_internal_function: 'prepareDnoIndemnificationPhase2FamilyProposal',
    ...
    profile_review_schedule_contract: {
      ...
      metsera_linked_duty_review_flag_count: profileReviewSchedule.filter(
    ...
    candidate_output_contract: {
      ...
      first_legal_stop: {
        ...
        current_required_disposition:
          'KEEP_ALL_31_PROPOSALS_UNAPPROVED_AND_REVIEW_ONLY_AT_PACKAGE_LEVEL',
        family_key: 'DNO_INDEMNIFICATION',
      ...
      review_accounting_exact_values: {
        complete_profile_count: profileCount,
        incomplete_profile_count: 0,
        linked_duty_review_flag_count: profileReviewSchedule.filter(
          (item) => item.review_flags.length > 0,
        ).length,
        proposed_profile_count: profileCount,
      ...
      unresolved_items: [
        'DNO_INDEMNIFICATION_Q01_Q02_Q03_RECORDED_AWAITING_INVENTORY_SESSION',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 DNO_INDEMNIFICATION family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_31_PROFILES_ADMITTED_AS_PHASE4_PARENT',
