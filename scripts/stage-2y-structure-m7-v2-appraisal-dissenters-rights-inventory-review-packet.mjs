#!/usr/bin/env node
/**
 * Emit the APPRAISAL_DISSENTERS_RIGHTS Work3 unapproved inventory review authority and the
 * five-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE2_PATH = appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_PHASE2_AUTHORITY_PATH;
const PHASE4_PATH = appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_APPRAISAL_DISSENTERS_RIGHTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_APPRAISAL_DISSENTERS_RIGHTS_5_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'appraisal_dissenters_rights_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'appraisal_dissenters_rights_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  appraisalDissentersRightsAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  appraisalDissentersRightsAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = appraisalDissentersRightsAuthoring
  .prepareAppraisalDissentersRightsFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_APPRAISAL_DISSENTERS_RIGHTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    appraisal_dissenters_rights_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareAppraisalDissentersRightsWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_APPRAISAL_DISSENTERS_RIGHTS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Appraisal / dissenters-rights claim keys under all seven '
      + 'subtype buckets. This packet proposes withdrawal-reconversion and settlement-consent claims under WITHDRAWAL_RECONVERSION and the '
      + 'no-financing-condition acknowledgment under NEGOTIATION_CONTROL, which diverges from '
      + 'the calibration pack tagging every provision example APPRAISAL_STATUS. The divergence is '
