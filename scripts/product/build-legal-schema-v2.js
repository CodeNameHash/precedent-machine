...
const OVERLAY = {
  ...
  NO_SHOP: {
    layers: 'One fact per duty. Prohibited actions are one LIST with one LIST_ELEMENT per verb (solicit, initiate, endorse, encourage, facilitate). Clean-up duties are their own subtypes. Notice duties carry the notice contents as LIST_ELEMENTs. Fiduciary exceptions carry each prerequisite as a CONDITION component. Periods are PERIOD components with canonical values. The words that bind the Company\'s Representatives ("shall cause its Representatives not to"; "shall not authorize or permit") are a PROHIBITED_ACTION fact of their own (the representative control standard). The superior-proposal path and the intervening-event path each carry their own NOTICE_PERIOD and INITIAL_MATCH_PERIOD facts when the agreement states them separately, each fact naming its path in the headline. The definition of an adverse recommendation change (the acts that count as one) is a RECOMMENDATION_CHANGE fact wherever the definition sits; what the agreement says is not such a change is a second RECOMMENDATION_CHANGE fact (decision 34).',
  ...
  EMPLOYEE_MATTERS: {
    layers: 'One fact per independently operative unit. Components follow the sentence: actor, operation, object, then each condition, exception, threshold, period and qualifier as its own component; lists that vary between deals as LIST with LIST_ELEMENTs; synonym litanies as one LITANY. Nothing is invented for an absent timing, qualification or forum. A compensation covenant that lists several benefit elements (base salary, target cash incentive, equity or long-term incentive opportunities, other employee benefits, severance) is one fact per element, each carrying the comparison group, the standard and the protection period from the sentence, so the benefits table has a row per element (Ben, 2026-09-13, Metsera 6.04). EMPLOYEE_MATTERS is the post-closing compensation and benefits covenant only. The treatment of equity awards at the Effective Time (options, RSUs, PSUs, restricted stock, the ESPP: cancellation, conversion, cash-out, CVR entitlement, vesting, payroll payment, withholding on those amounts, board resolutions to effect it) is CONSIDERATION/EQUITY_AWARD, never EMPLOYEE_MATTERS (Metsera generation 5, 2.03). The disclaimers inside the employee covenant (no third-party beneficiaries, no plan amendment, no right to continued employment, no duplication of benefits) are EMPLOYEE_MATTERS facts on the other-protections rows, never MISC_BOILERPLATE (Metsera generation 5, 6.04).',
  },
  INTERIM_OPERATING: {
  ...
  MAE_DEFINITION: {
    layers: 'A Material Adverse Effect definition with two limbs (an effect on the business; the ability to consummate the transaction) is two DEFINITION_PRONG facts, one per limb, each citing that limb\'s own words; the effect-on-the-business limb is never omitted in favour of the consummation limb (Metsera generation 7, 9.03: only the consummation limb was returned and the page read the Company definition as one limb). The definition prong is one fact with components: the affected-matters LITANY, the aggregation standard ("individually or in the aggregate"), the probability standard ("is or would reasonably be expected to be"), the materiality term, the affected-business LIST (business, assets, liabilities, condition, results of operations), and "taken as a whole". Each carve-out is one fact whose distinguishing component is the carve-out subject; list elements are LIST_ELEMENTs with nested inclusions nested. A carve-out that is itself one lettered element of the exclusion list is a fact whose root component is that LIST_ELEMENT (its own words), never the whole list. The disproportionality carve-back lists the clauses it applies to as CROSS_REFERENCEs.',
  ...
  REPRESENTATIONS: {
    layers: 'Each representation limb is one fact. Components: the subject, the standard, each materiality or knowledge qualifier, thresholds, look-back periods and carve-outs as separate components; disclosure-letter and SEC-document limitations inherited from the article intro with origin INTRO. A qualifier (knowledge, materiality, a look-back date, a disclosure exception) is never a fact of its own: it is a component of every limb it qualifies, and a limb\'s tree runs from the qualifiers through the subject to the operative words and their object, so the tree says what the representation is about. The subtype follows the representation\'s topic (its heading: anti-corruption, sanctions, permits and laws are COMPLIANCE_REPRESENTATION; organization, standing, authority and capitalization are STATUS_REPRESENTATION), never the grammatical form of a limb ("is the subject of an investigation" in an anti-corruption representation is compliance, not status) (Ben, 2026-09-13, Metsera 3.25). The article introduction ("Except as disclosed in the Filed Company SEC Documents ... or as set forth in the Company Disclosure Letter ..., the Company represents and warrants") is never a status or document representation: it is one REPRESENTATION_QUALIFICATION fact per source of exception (the SEC filings exception with its window and its exclusions as LIST elements: risk factors, forward-looking statements, exhibits, the historical-facts carve-back; the Disclosure Letter exception with its arrangement and cross-section rule), so the page can show the reader the categories first and the detail on opening (Ben, 2026-09-14: "we should be able to show the reader the general categories of the exceptions (SEC filings) and as they click into deeper levels show more detail (last X days) etc").',
    headline: { distinguishing: ['TERM', 'THRESHOLD', 'MATERIALITY_QUALIFIER'], note: 'subject plus qualifier plus threshold' },
    add: [
      { subtype_key: 'REPRESENTATION_QUALIFICATION', label: 'General qualification', required_roles: ['qualification_source', 'qualified_scope'], optional_roles: ['window', 'exclusions', 'standard'], relationships: ['QUALIFIES', 'EXCEPTS'] },
    ],
  },
};
