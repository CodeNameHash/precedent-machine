...
function renderReviewerHtml({ reviewPacket, ledger, gate }) {
  const cards = reviewPacket.cards.map((card) => {
    const contextRows = [...card.governing_context.chapeau, card.governing_context.governing_sentence]
      .map((span, index) => `<tr><td>${index < card.governing_context.chapeau.length ? 'Chapeau' : 'Governing sentence'}</td><td>${span.start_byte}</td><td>${span.end_byte}</td><td><pre>${escapeHtml(span.text)}</pre></td></tr>`).join('');
    return `<article class="card" data-family="${escapeHtml(card.family)}" data-error-class="${escapeHtml(card.error_class)}">
...
<h3>Governing context</h3><div class="table-scroll"><table><thead><tr><th>Part</th><th>Section byte start</th><th>Section byte end</th><th>Exact text</th></tr></thead><tbody>${contextRows}</tbody></table></div>
...
<h3>Question</h3><p>${escapeHtml(card.question.prompt)}</p><p><strong>Choose one:</strong> ${card.question.response_options.map(escapeHtml).join(' / ')}</p>
<details><summary>Decision template</summary><pre>${escapeHtml(canonicalJson({ decision_key: card.decision_key, reviewer_id: '<reviewer>', reviewed_at: '<UTC timestamp>', verdict: card.question.response_options.join(' | ') }))}</pre></details>
</article>`;
  }).join('\n');
