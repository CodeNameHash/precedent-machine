#!/usr/bin/env node
/**
 * Emit APPRAISAL_DISSENTERS_RIGHTS Phase 2 authoring authority v2.
 *
 * Five governed M4 claims across three comparator deals (Skechers, Skywater,
 * TopBuild). Claim-scale partition — one terminal per governed claim. Skechers
 * §2.7 and TopBuild §2.1 each carry two independently operative limbs
 * (withdrawal-reconversion + settlement-consent); Q01 requires one profile per
 * limb, not one per section.
 *
 * Consideration (#15) owns appraisal-rights availability on shared printed
 * sections; those rows stay Q02 link-only and never become Appraisal terminals.
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/APPRAISAL_DISSENTERS_RIGHTS.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/APPRAISAL_DISSENTERS_RIGHTS.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-appraisal-dissenters-rights-authoring.js';
const TEST_PATH =
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  APPRAISAL_WITHDRAWAL_RECONVERSION: 'WITHDRAWAL_RECONVERSION',
  APPRAISAL_SETTLEMENT_CONSENT: 'SETTLEMENT_CONSENT',
};
...
const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    ...
    deal: 'skechers',
    disposition: 'LINK_ONLY_NOT_AN_APPRAISAL_DISSENTERS_RIGHTS_TERMINAL',
    owner_family_key: 'CONSIDERATION',
    ruling_id: 'M5-RULING-ONE-SEMANTIC-OWNER',
    section_reference: '2.7',
    subject: 'APPRAISAL_RIGHTS_STATUS',
  },
  {
...
function classificationPath(bucket) {
  return ['APPRAISAL_DISSENTERS_RIGHTS', bucket];
}
...
function requiredExpressionSignature(claim, bucket) {
  ...
  return `APPRAISAL_DISSENTERS_RIGHTS::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter(
      (claim) => claim.family === 'APPRAISAL_DISSENTERS_RIGHTS',
    );
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_APPRAISAL_DISSENTERS_RIGHTS_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose:
        'Claim-scale comparator terminal registry for APPRAISAL_DISSENTERS_RIGHTS Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_APPRAISAL_DISSENTERS_RIGHTS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
      ...
      required_expression_signature_semantics:
        'COMPARATOR_CLAIM_BOUND_SIGNATURE_PER_TERMINAL',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_APPRAISAL_DISSENTERS_RIGHTS_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_THREE_EXAMPLES_APPRAISAL_STATUS_WHILE_CLAIM_KEYS_SPLIT_WITHDRAWAL_RECONVERSION_AND_SETTLEMENT_CONSENT',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareAppraisalDissentersRightsPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade:
          'APPRAISAL_DISSENTERS_RIGHTS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareAppraisalDissentersRightsPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved APPRAISAL_DISSENTERS_RIGHTS partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'appraisal_dissenters_rights_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale:
        'Appraisal / dissenters-rights first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    appraisal_dissenters_rights_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'appraisal_dissenters_rights_authoring_phase2_authority_id',
    record_id: record.appraisal_dissenters_rights_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
