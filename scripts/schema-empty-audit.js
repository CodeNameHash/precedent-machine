...
function candidateFlags(feature, row) {
  const flags = [];
  if (!feature || !row) return flags;
  if (FORCE_SILENT.has(feature.key)) return flags;
  const total = row.total || 0;
  const populated = row.populated || 0;
  const populatedPct = total ? populated / total : 0;

  if (FORCE_EXTRACTION_PENDING.has(feature.key)) flags.push('curated_extraction_pending');
  if (FORCE_NEEDS_REVIEW.has(feature.key)) flags.push('curated_needs_review');
  if (feature.presence === 'conditional' || feature.presenceCondition) flags.push('conditional_or_structural');
  if (feature.benchmarkable && total >= 10 && populated === 0) flags.push('benchmarkable_zero_populated');
  if (feature.benchmarkable && total >= 25 && populatedPct < 0.05) flags.push('benchmarkable_low_population');
  if ((feature.requiredEvidence || feature.citable) && total >= 10 && populated === 0) flags.push('citable_zero_populated');
...
function buildAudit({ deals, provisions }, opts = {}) {
  ...
  const rows = Object.values(FEATURES).map((feature) => {
    ...
    const flags = candidateFlags(feature, row);
    return {
      feature_key: feature.key,
      label: feature.label,
...
function renderMarkdown(audit) {
  ...
  const candidates = audit.rows.filter((row) => row.candidate_flags && row.candidate_flags.length);
  ...
  return [
    ...
    '',
    candidates.length ? markdownTable(candidates.slice(0, 80)) : 'No candidate signals.',
    '',
    '## Top Audit Rows',
    '',
    markdownTable(top),
