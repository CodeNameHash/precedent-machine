#!/usr/bin/env node
/**
 * Emit TAX_MATTERS Phase 4 family profile package review authority from
 * the Phase 2 partition (seventeen claim-scale profiles; no Phase 3 reference chain).
 *
...
const SCHEMA = taxMattersAuthoring.TAX_MATTERS_PHASE4_AUTHORITY_SCHEMA;
const OUT_PATH = taxMattersAuthoring.TAX_MATTERS_PHASE4_AUTHORITY_PATH;
const PHASE2_PATH = taxMattersAuthoring.TAX_MATTERS_PHASE2_AUTHORITY_PATH;
...
function phase2Fixture() {
  const taxMattersAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = taxMattersAuthoringPhase2Authority.record;
...
function main() {
  ...
  const terminalsBySourceUnitKey = new Map(
    fixture.taxMattersAuthoringPhase2Authority.record
  ...
  const firstLegalStop = {
    ...
    current_required_disposition:
      'KEEP_ALL_17_PROPOSALS_UNAPPROVED_AND_REVIEW_ONLY_AT_PACKAGE_LEVEL',
  ...
  const unsigned = {
    ...
    authority_state:
      'TEMP_ONLY_ZERO_EFFECT_AUTHORITY_17_REVIEW_PROPOSALS_UNAPPROVED_AWAITING_BEN_INVENTORY',
    design_basis: {
      ...
      public_facade: 'prepareTaxMattersFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      tax_matters_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      error_precedence: [
        {
          code: 'M7_V2_TAX_MATTERS_PHASE4_REVIEW_CONTRACT',
          condition: 'NULL_NON_OBJECT_MISSING_EXTRA_OR_REORDERED_OUTER_INPUT_KEYS',
          order: 1,
        },
        {
          code: 'M7_V2_TAX_MATTERS_PHASE4_REVIEW_AUTHORITY',
          condition: 'AUTHORITY_ENVELOPE_BINDING_PHYSICAL_BYTES_SELF_ID_OR_PARENT_PIN_DRIFT',
          order: 2,
        },
        {
          code: 'M7_V2_TAX_MATTERS_PHASE4_PHASE2_PROPOSAL',
          condition: 'FRESH_PHASE2_PROPOSAL_FAILS_OR_17_PROFILE_BINDING_DRIFTS',
          order: 3,
        },
        {
          code: 'M7_V2_TAX_MATTERS_PHASE4_PROFILE_SCHEDULE',
          condition: 'TUPLE_PROFILE_SOURCE_CLAIM_OR_PROFILE_ORDER_DRIFT',
          order: 4,
        },
        {
          code: 'M7_V2_TAX_MATTERS_PHASE4_REVIEW_OUTPUT',
      ...
      exported_function: 'prepareTaxMattersFamilyProfilePackageReview',
      module_path: MODULE_PATH,
      phase2_internal_function: 'prepareTaxMattersPhase2FamilyProposal',
    ...
    candidate_output_contract: {
      candidate_state: taxMattersAuthoring.TAX_MATTERS_PHASE4_CANDIDATE_STATE,
      exact_keys: [
      ...
      schema_version:
        taxMattersAuthoring.TAX_MATTERS_PHASE4_CANDIDATE_SCHEMA,
      unresolved_items: [
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 TAX_MATTERS family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_17_PROFILES_5_M4_CLAIMS_0_SILENT_AS_PHASE4_PARENT',
  ...
  const record = {
    ...unsigned,
    tax_matters_authoring_phase4_family_profile_package_review_authority_id: recordId,
  };
