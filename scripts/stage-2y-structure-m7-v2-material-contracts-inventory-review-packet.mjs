#!/usr/bin/env node
/**
 * Emit the MATERIAL_CONTRACTS Work3 unapproved inventory review authority and the
 * 116-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MATERIAL_CONTRACTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_MATERIAL_CONTRACTS_116_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'material_contracts_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'material_contracts_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  materialContractsAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  materialContractsAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = materialContractsAuthoring
  .prepareMaterialContractsFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_MATERIAL_CONTRACTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    material_contracts_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareMaterialContractsWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_MATERIAL_CONTRACTS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Material Contracts claim keys under all seven '
      + 'subtype buckets. This packet proposes meeting-covenant claims under MATERIAL_CONTRACT_BREACH_TERMINATION_RIGHT and the '
      + 'proxy/meeting subtype partition under MATERIAL_CONTRACT_BREACH_TERMINATION_RIGHT, which diverges from '
      + 'the calibration pack tagging every provision example MATERIAL_CONTRACT_CATEGORY_CRITERION. The divergence is '
