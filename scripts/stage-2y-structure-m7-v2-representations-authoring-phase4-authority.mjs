/**
 * Emit REPRESENTATIONS Phase 4 family profile package review authority from the
 * Phase 2 partition (70 claim-scale profiles; no Phase 3 reference chain).
 *
 * Every profile carries LEGAL_GROUPING_REVIEW_REQUIRED because the six M5 subtype
 * buckets are unresolved. Knowledge-qualifier profiles additionally carry
 * CROSS_FAMILY_KNOWLEDGE_DEFINITION_LINK_ONLY: the knowledge-person definition is
 * owned by KEY_DEFINED_TERMS under the Q02 one-owner ruling.
 */
...
import representationsAuthoring from '../lib/canonical-v2/m7-v2-representations-authoring.js';
...
const PHASE2_PATH = representationsAuthoring.REPRESENTATIONS_PHASE2_AUTHORITY_PATH;
const PHASE2_BINDING = Object.freeze({
  path: PHASE2_PATH,
  schema_version: representationsAuthoring.REPRESENTATIONS_PHASE2_AUTHORITY_SCHEMA,
  record_id_field: 'representations_authoring_phase2_authority_id',
  record_id: representationsAuthoring.REPRESENTATIONS_PHASE2_AUTHORITY_ID,
  byte_length: representationsAuthoring.REPRESENTATIONS_PHASE2_AUTHORITY_BYTES,
  sha256: representationsAuthoring.REPRESENTATIONS_PHASE2_AUTHORITY_SHA256,
});

const PROFILE_COUNT = representationsAuthoring.REPRESENTATIONS_PROFILE_COUNT;
const CANDIDATE_STATE = representationsAuthoring.REPRESENTATIONS_PHASE4_CANDIDATE_STATE;
const CANDIDATE_SCHEMA = representationsAuthoring.REPRESENTATIONS_PHASE4_CANDIDATE_SCHEMA;
...
const OUTPUT_EXACT_KEYS = Object.freeze([
  ...representationsAuthoring.REPRESENTATIONS_PHASE4_REVIEW_OUTPUT_KEYS,
]);
...
function phase2Fixture() {
  const representationsAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = representationsAuthoringPhase2Authority.record;
...
function packageProfileKey(classificationBucket, proposedProfileKey) {
  return ['PROFILE', 'REPRESENTATIONS', classificationBucket, proposedProfileKey].join(':');
}

function reviewFlagsForSignature(signature) {
  const flags = ['LEGAL_GROUPING_REVIEW_REQUIRED'];
  if (signature.includes(',KNOWLEDGE_QUALIFIER,')) {
    flags.push('CROSS_FAMILY_KNOWLEDGE_DEFINITION_LINK_ONLY');
  }
...
function buildSchedule(phase2Proposal) {
  ...
  return profiles.map((profile) => {
    ...
    return {
      ...
      review_flags: reviewFlagsForSignature(
        profile.canonical_tuple.required_expression_signature,
      ),
      missing_required_field_keys: [],
    };
  });
...
function main() {
  const phase2Proposal = representationsAuthoring.prepareRepresentationsPhase2FamilyProposal(
    phase2Fixture(),
  );
  ...
  if (
    crossFamilyFlagCount
    !== representationsAuthoring.REPRESENTATIONS_KNOWLEDGE_QUALIFIER_PROFILE_COUNT
  ) {
    throw new Error(`Unexpected knowledge-qualifier profile count: ${crossFamilyFlagCount}`);
  }
  ...
  const unsigned = {
    ...
    design_basis: {
      ...
      phase3_skip_basis:
        'ALL_SIX_CALIBRATION_PROVISION_EXAMPLES_HAVE_EMPTY_M3_DEPENDENCY_IDS_AND_NO_TERMINAL_CARRIES_A_BLOCKING_REFERENCE_EDGE',
      public_facade: 'prepareRepresentationsFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      representations_authoring_phase2_authority: PHASE2_BINDING,
    },
    ...
    profile_review_schedule_contract: {
      ...
      cross_family_knowledge_definition_flag_count: crossFamilyFlagCount,
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 REPRESENTATIONS family profile package review returns unapproved proposals without Work3 identities',
  ...
  const record = {
    ...unsigned,
    representations_authoring_phase4_family_profile_package_review_authority_id: recordId,
  };
