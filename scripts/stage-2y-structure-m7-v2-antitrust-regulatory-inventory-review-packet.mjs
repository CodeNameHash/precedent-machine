...
/**
 * Emit the ANTITRUST_REGULATORY Work3 unapproved inventory review authority and the
 ...
 * M5_SUBTYPE_BUCKET_PARTITION_UNRESOLVED are acknowledged flags, not holds.
 */
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_ANTITRUST_REGULATORY_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_ANTITRUST_REGULATORY_70_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
function phase4ReviewInput() {
  const phase2 = boundEnvelope(pinnedBinding(
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_PATH,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_SCHEMA,
    'antitrust_regulatory_authoring_phase2_authority_id',
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_ID,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_BYTES,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE2_AUTHORITY_SHA256,
  ));
  const phase4 = boundEnvelope(pinnedBinding(
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE4_AUTHORITY_PATH,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE4_AUTHORITY_SCHEMA,
    'antitrust_regulatory_authoring_phase4_family_profile_package_review_authority_id',
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE4_AUTHORITY_ID,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE4_AUTHORITY_BYTES,
    antitrustRegulatoryAuthoring.ANTITRUST_REGULATORY_PHASE4_AUTHORITY_SHA256,
  ));
  return {
    antitrustRegulatoryAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
    antitrustRegulatoryAuthoringPhase2Authority: phase2,
...
function buildShapeSummary(profile) {
  ...
  return {
    ...
    concept_key: signatureField(signature, 1),
    m4_claim_id: signatureField(signature, 3),
...
const review = antitrustRegulatoryAuthoring.prepareAntitrustRegulatoryFamilyProfilePackageReview(phase4Input);
const dealMap = dealByM4ClaimIdMap();

const authorityUnsigned = {
  schema_version: AUTHORITY_SCHEMA,
  authority_classification: 'WORK3_ANTITRUST_REGULATORY_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    antitrust_regulatory_phase4_review_authority:
      phase4Input.antitrustRegulatoryAuthoringPhase4FamilyProfilePackageReviewAuthority.binding,
  },
  implementation_contract: {
    exported_function: 'prepareAntitrustRegulatoryWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_ANTITRUST_REGULATORY_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const m5SubtypeCount = profileReviewItems.filter(
...
if (
  ...
  throw new Error(
    `packet census drift: ${profileReviewItems.length} profiles / ${m5SubtypeCount} m5-subtype / ${legalGroupingCount} legal`,
  );
}
