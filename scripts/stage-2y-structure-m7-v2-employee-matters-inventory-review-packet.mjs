#!/usr/bin/env node
/**
 * Emit the EMPLOYEE_MATTERS Work3 unapproved inventory review authority and the
 * 27-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = employeeMattersAuthoring.EMPLOYEE_MATTERS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_EMPLOYEE_MATTERS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_EMPLOYEE_MATTERS_27_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'employee_matters_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'employee_matters_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  employeeMattersAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  employeeMattersAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = employeeMattersAuthoring
  .prepareEmployeeMattersFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_EMPLOYEE_MATTERS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    employee_matters_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareEmployeeMattersWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_EMPLOYEE_MATTERS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Employee Matters claim keys under all seven '
      + 'subtype buckets. This packet proposes meeting-covenant claims under RETIREMENT_PLAN_ACTION and the '
      + 'proxy/meeting subtype partition under RETIREMENT_PLAN_ACTION, which diverges from '
      + 'the calibration pack tagging every provision example EMPLOYEE_COMPENSATION. The divergence is '
