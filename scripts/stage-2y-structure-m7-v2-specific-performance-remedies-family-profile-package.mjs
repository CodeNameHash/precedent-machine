#!/usr/bin/env node
/**
 * Regenerate the on-disk SPECIFIC_PERFORMANCE_REMEDIES Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-specific-performance-remedies.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'SPECIFIC_PERFORMANCE_REMEDIES';
...
const APPROVAL_TEXT =
  `Ben approves the SPECIFIC_PERFORMANCE_REMEDIES ${PROFILE_COUNT}-profile Work3 package inventory `
  + `(${APPROVE_COUNT} APPROVE / ${HOLD_COUNT} HOLD on Termination Fee sole-remedy owner-family `
  + 'question); subtype grouping review pending.';
...
function buildProfileFromTemplate(templateProfile, {
  ...
  profile.match_test = {
    ...
    tokens: [
      'familyspecificperformanceremedies',
...
function buildPhase4Review() {
  ...
  return specificPerformanceRemediesAuthoring.prepareSpecificPerformanceRemediesFamilyProfilePackageReview({
    specificPerformanceRemediesAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    specificPerformanceRemediesAuthoringPhase2Authority: phase2Authority,
