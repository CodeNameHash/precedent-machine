#!/usr/bin/env node
/**
 * Regenerate the on-disk APPRAISAL_DISSENTERS_RIGHTS Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-appraisal-dissenters-rights.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'APPRAISAL_DISSENTERS_RIGHTS';
...
const APPROVAL_TEXT =
  'Ben approves the APPRAISAL_DISSENTERS_RIGHTS five-profile Work3 package inventory; subtype grouping '
  + 'review pending.';
...
function buildProfileFromTemplate(templateProfile, {
  ...
  profile.match_test = {
    ...
    tokens: [
      'familyappraisaldissentersrights',
...
function buildPhase4Review() {
  ...
  return appraisalDissentersRightsAuthoring.prepareAppraisalDissentersRightsFamilyProfilePackageReview({
    appraisalDissentersRightsAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    appraisalDissentersRightsAuthoringPhase2Authority: phase2Authority,
