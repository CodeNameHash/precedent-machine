#!/usr/bin/env node
/**
 * Emit the APPRAISAL_DISSENTERS_RIGHTS Ben inventory disposition, session receipt, and the
 * three remaining Work3 successor authorities plus the family package seal receipt.
 *
 * Disposition is APPROVE on all five profiles with the subtype-grouping question
 * stamped as an open hold. Q01-Q03 reuse the sealed M5 programme rulings the family
 * role schema already binds; no Appraisal / dissenters-rights-specific ruling is invented.
 */
import { readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import canonicalModule from '../lib/canonical-v2/canonical-bytes.js';
import appraisalDissentersRightsAuthoring from '../lib/canonical-v2/m7-v2-appraisal-dissenters-rights-authoring.js';
...
const PACKET_PATH =
  `${CONTROL}/m7-v2-repair-appraisal-dissenters-rights-5-profile-inventory-review-packet-draft.json`;
const INVENTORY_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-appraisal-dissenters-rights-unapproved-inventory-review-authority.json`;
const DISPOSITION_PATH =
  `${CONTROL}/m7-v2-repair-appraisal-dissenters-rights-5-profile-inventory-disposition.json`;
const SESSION_PATH =
  `${CONTROL}/m7-v2-repair-appraisal-dissenters-rights-ben-inventory-session-receipt.json`;
const BEN_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-appraisal-dissenters-rights-ben-inventory-session-successor-authority.json`;
const SEAL_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-appraisal-dissenters-rights-family-package-seal-successor-authority.json`;
const SEAL_RECEIPT_PATH =
  `${CONTROL}/m7-v2-repair-appraisal-dissenters-rights-family-package-seal-receipt.json`;
const REGISTRATION_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-appraisal-dissenters-rights-registration-successor-authority.json`;
const RULINGS_PATH = `${CONTROL}/m5-programme-rulings.json`;
const ROLE_SCHEMA_PATH = `${CONTROL}/family-role-schemas/APPRAISAL_DISSENTERS_RIGHTS.json`;

const DISPOSITION_SCHEMA =
  'STAGE_2Y_M7_V2_APPRAISAL_DISSENTERS_RIGHTS_5_PROFILE_INVENTORY_DISPOSITION/V1';
const SESSION_SCHEMA =
  'STAGE_2Y_M7_V2_APPRAISAL_DISSENTERS_RIGHTS_BEN_INVENTORY_SESSION_RECEIPT/V1';
const BEN_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_APPRAISAL_DISSENTERS_RIGHTS_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY/V1';
const SEAL_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_APPRAISAL_DISSENTERS_RIGHTS_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY/V1';
const SEAL_RECEIPT_SCHEMA =
  'STAGE_2Y_M7_V2_APPRAISAL_DISSENTERS_RIGHTS_FAMILY_PACKAGE_SEAL_RECEIPT/V1';
const REGISTRATION_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_APPRAISAL_DISSENTERS_RIGHTS_REGISTRATION_SUCCESSOR_AUTHORITY/V1';
const SEAL_CANDIDATE_SCHEMA =
  'M7_V2_APPRAISAL_DISSENTERS_RIGHTS_WORK3_FAMILY_PACKAGE_SEAL_CANDIDATE/V1';
const BEN_CANDIDATE_STATE =
  'BEN_5_PROFILE_INVENTORY_DISPOSITION_CAPTURED_PACKAGE_SEAL_NOT_RECORDED';
const SEAL_CANDIDATE_STATE =
  'BEN_APPRAISAL_DISSENTERS_RIGHTS_FAMILY_PACKAGE_SEAL_CAPTURED_REGISTRATION_NOT_RECORDED';
...
const DIVERGENCE_COUNT =
  appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_SUBTYPE_DIVERGENCE_PROFILE_COUNT;
const OUTSIDE_CALIBRATION_COUNT =
  appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_OUTSIDE_CALIBRATION_PROFILE_COUNT;
const REGISTERED_BUCKETS =
  appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_REGISTERED_SUBTYPE_BUCKET_COUNT;
const POPULATED_BUCKETS =
  appraisalDissentersRightsAuthoring.APPRAISAL_DISSENTERS_RIGHTS_POPULATED_SUBTYPE_BUCKET_COUNT;
...
const inventoryAuthorityBinding = binding(
  ...
  inventoryAuthority,
  'work3_appraisal_dissenters_rights_unapproved_inventory_review_authority_id',
);
...
const sessionUnsigned = {
  ...
  session_classification: 'APPRAISAL_DISSENTERS_RIGHTS_5_PROFILE_INVENTORY_BEN_REVIEW',
...
const benAuthority = authority(
  ...
  'work3_appraisal_dissenters_rights_ben_inventory_session_successor_authority_id',
  {
    authority_classification:
      'WORK3_APPRAISAL_DISSENTERS_RIGHTS_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareAppraisalDissentersRightsWork3BenInventorySessionDisposition',
    ...
    schema_review_candidate_contract: {
      schema_version:
        'M7_V2_APPRAISAL_DISSENTERS_RIGHTS_WORK3_BEN_INVENTORY_SESSION_DISPOSITION_CANDIDATE/V1',
...
const sealAuthority = authority(
  ...
  'work3_appraisal_dissenters_rights_family_package_seal_successor_authority_id',
  {
    authority_classification: 'WORK3_APPRAISAL_DISSENTERS_RIGHTS_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareAppraisalDissentersRightsWork3FamilyPackageSeal',
      exact_outer_input_keys: [
...
// Mirrors prepareAppraisalDissentersRightsWork3FamilyPackageSeal exactly; the test asserts
...
const sealReceiptUnsigned = {
  ...
  seal_classification: 'APPRAISAL_DISSENTERS_RIGHTS_WORK3_FAMILY_PACKAGE_SEAL',
  completion_state: 'COMPLETE',
  reviewer: 'BEN_GOODCHILD',
...
const sealReceipt = {
  ...sealReceiptUnsigned,
  appraisal_dissenters_rights_family_package_seal_receipt_id:
...
const registrationAuthority = authority(
  ...
  'work3_appraisal_dissenters_rights_registration_successor_authority_id',
  {
    authority_classification: 'WORK3_APPRAISAL_DISSENTERS_RIGHTS_REGISTRATION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareAppraisalDissentersRightsWork3FamilyPackageRegistration',
      exact_outer_input_keys: [
    ...
    schema_review_candidate_contract: {
      schema_version: 'M7_V2_APPRAISAL_DISSENTERS_RIGHTS_WORK3_FAMILY_PACKAGE_REGISTRATION_CANDIDATE/V1',
      candidate_state:
