/**
 * Emit MAE_DEFINITION Phase 4 family profile package review authority from
 * the Phase 2 partition (four comparator-derived profiles; no Phase 3 reference chain).
 */
...
const PHASE2_BINDING = Object.freeze({
  ...
  record_id: '55e4bb2c26ac676d813181ef5be94398339014879df294a46d59d7991d06b308',
  byte_length: 28635,
  sha256: 'cb9dde5b355219633781f73da31a5c340d5ea69cf3973568141d249f1765d0e5',
});
...
function phase2Fixture() {
  const maeDefinitionAuthoringPhase2Authority = sourceEnvelope(PHASE2_BINDING);
  const authority = maeDefinitionAuthoringPhase2Authority.record;
...
function reviewFlagsForProfile(profile) {
  const flags = ['MAE_DEFINITION_SELF_CONTAINMENT_UNPROVEN'];
  if (profile.canonical_tuple.classification_path[1] === 'EXCLUSION') {
    flags.push('MAE_DEFINITION_SUBJECT_TERM_MISMATCH');
  }
...
function buildSchedule(phase2Proposal) {
  ...
  return profiles.map((profile) => {
    ...
    return {
      ...
      review_flags: reviewFlagsForProfile(profile),
      missing_required_field_keys: [],
    };
  });
}
...
function main() {
  const phase2Proposal = maeAuthoring.prepareMaeDefinitionFamilyProposal(
    phase2Fixture(),
  );
  ...
  const selfContainmentFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.includes('MAE_DEFINITION_SELF_CONTAINMENT_UNPROVEN'),
  ).length;
  const subjectTermFlagCount = profileReviewSchedule.filter(
    (item) => item.review_flags.includes('MAE_DEFINITION_SUBJECT_TERM_MISMATCH'),
  ).length;
  ...
  const unsigned = {
    ...
    authority_state:
      'TEMP_ONLY_ZERO_EFFECT_AUTHORITY_4_REVIEW_PROPOSALS_UNAPPROVED_AWAITING_BEN_INVENTORY',
    design_basis: {
      ...
      public_facade: 'prepareMaeDefinitionFamilyProfilePackageReview',
      public_input_key_count: 3,
    },
    immutable_parent_bindings: {
      mae_definition_authoring_phase2_authority: PHASE2_BINDING,
    },
    implementation_contract: {
      ...
      error_precedence: [
        {
          code: 'M7_V2_MAE_DEFINITION_PHASE4_REVIEW_CONTRACT',
          condition: 'NULL_NON_OBJECT_MISSING_EXTRA_OR_REORDERED_OUTER_INPUT_KEYS',
          order: 1,
        },
        {
          code: 'M7_V2_MAE_DEFINITION_PHASE4_REVIEW_AUTHORITY',
          condition: 'AUTHORITY_ENVELOPE_BINDING_PHYSICAL_BYTES_SELF_ID_OR_PARENT_PIN_DRIFT',
          order: 2,
        },
        {
          code: 'M7_V2_MAE_DEFINITION_PHASE4_PHASE2_PROPOSAL',
          condition: 'FRESH_PHASE2_PROPOSAL_FAILS_OR_4_PROFILE_BINDING_DRIFTS',
          order: 3,
        },
        {
          code: 'M7_V2_MAE_DEFINITION_PHASE4_PROFILE_SCHEDULE',
          condition: 'TUPLE_PROFILE_SOURCE_CLAIM_OR_PROFILE_ORDER_DRIFT',
          order: 4,
        },
        {
          code: 'M7_V2_MAE_DEFINITION_PHASE4_REVIEW_OUTPUT',
      ...
      exported_function: 'prepareMaeDefinitionFamilyProfilePackageReview',
      module_path: 'lib/canonical-v2/m7-v2-mae-definition-authoring.js',
      phase2_internal_function: 'prepareMaeDefinitionFamilyProposal',
    ...
    profile_review_schedule_contract: {
      ...
      exclusion_carveout_tuple_key: 'mae_carveout_code',
      exact_complete_profile_count: profileCount,
      exact_incomplete_profile_count: 0,
      exact_profile_count: profileCount,
      mae_self_containment_review_flag_count: selfContainmentFlagCount,
      mae_subject_term_review_flag_count: subjectTermFlagCount,
    ...
    candidate_output_contract: {
      ...
      first_legal_stop: {
        ...
        current_required_disposition:
          'KEEP_ALL_4_PROPOSALS_UNAPPROVED_AND_REVIEW_ONLY_AT_PACKAGE_LEVEL',
        family_key: 'MAE_DEFINITION',
    ...
    execution_schedule: {
      ...
      focused_test_name:
        'Phase4 MAE_DEFINITION family profile package review returns unapproved proposals without Work3 identities',
      prerequisite_binding: PHASE2_BINDING,
      prerequisite_state:
        'PHASE2_PARTITION_GREEN_4_PROFILES_108_CLAIMS_ADMITTED_AS_PHASE4_PARENT',
  ...
  const absoluteOutPath = path.join(REPO_ROOT, OUT_PATH);
  writeFileSync(absoluteOutPath, bytes);
