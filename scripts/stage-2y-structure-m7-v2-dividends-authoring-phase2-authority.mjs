#!/usr/bin/env node
/**
 * Emit DIVIDENDS Phase 2 authoring authority v2.
 *
 * One governed M4 claim on the Concho comparator deal. Claim-scale partition —
 * one terminal per governed claim. Consideration and Interim Operating Q02
 * link-only boundaries apply only when those rows surface elsewhere.
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/DIVIDENDS.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/DIVIDENDS.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-dividends-authoring.js';
const TEST_PATH = 'tests/stage-2y-structure-m7-v2-repair-dividends-work3.test.js';
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  DIVIDEND_COORDINATION_COVENANT: 'DIVIDEND_COORDINATION',
};
...
function requiredExpressionSignature(claim, bucket) {
  ...
  return `DIVIDENDS::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'DIVIDENDS');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_DIVIDENDS_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for DIVIDENDS Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_DIVIDENDS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_DIVIDENDS_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      sparse_comparator_stress: 'ONE_GOVERNED_CLAIM_ON_CONCHO_COMPARATOR_DEAL',
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_FIVE_EXAMPLES_DIVIDEND_COORDINATION_WHILE_ONE_COMPARATOR_CLAIM_POPULATES_DIVIDEND_COORDINATION',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareDividendsPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade: 'DIVIDENDS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareDividendsPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved DIVIDENDS partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'dividends_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'Dividends first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    dividends_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'dividends_authoring_phase2_authority_id',
    record_id: record.dividends_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
