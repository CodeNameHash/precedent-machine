#!/usr/bin/env node
/**
 * Emit the DIVIDENDS Work3 unapproved inventory review authority and the
 * 1-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = dividendsAuthoring.DIVIDENDS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_DIVIDENDS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_DIVIDENDS_1_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'dividends_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'dividends_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  dividendsAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  dividendsAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = dividendsAuthoring
  .prepareDividendsFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_DIVIDENDS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    dividends_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareDividendsWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_DIVIDENDS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Dividends claim keys under all seven '
      + 'subtype buckets. This packet proposes dividend-coordination covenant claims under UNPAID_DECLARED_DISTRIBUTION and the '
      + 'no-financing-condition acknowledgment under INTERIM_RESTRICTION_LINK, which diverges from '
      + 'the calibration pack tagging every provision example DIVIDEND_COORDINATION. The divergence is '
