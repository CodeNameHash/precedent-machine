#!/usr/bin/env node
/**
 * Emit the MERGER_STRUCTURE_CLOSING Work3 unapproved inventory review authority and the
 * 103-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = mergerStructureClosingAuthoring.MERGER_STRUCTURE_CLOSING_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MERGER_STRUCTURE_CLOSING_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_MERGER_STRUCTURE_CLOSING_103_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'merger_structure_closing_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'merger_structure_closing_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  mergerStructureClosingAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  mergerStructureClosingAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = mergerStructureClosingAuthoring
  .prepareMergerStructureClosingFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_MERGER_STRUCTURE_CLOSING_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    merger_structure_closing_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareMergerStructureClosingWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_MERGER_STRUCTURE_CLOSING_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Merger Structure / Closing claim keys under all seven '
      + 'subtype buckets. This packet proposes meeting-covenant claims under EFFECTIVE_TIME and the '
      + 'proxy/meeting subtype partition under EFFECTIVE_TIME, which diverges from '
      + 'the calibration pack tagging every provision example TRANSACTION_STEP. The divergence is '
