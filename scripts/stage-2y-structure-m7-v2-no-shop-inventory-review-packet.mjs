#!/usr/bin/env node
/**
 * Emit the NO_SHOP Work3 unapproved inventory review authority and the
 * 365-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = noShopAuthoring.NO_SHOP_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_NO_SHOP_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_NO_SHOP_365_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'no_shop_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'no_shop_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  noShopAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  noShopAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = noShopAuthoring
  .prepareNoShopFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_NO_SHOP_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    no_shop_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareNoShopWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_NO_SHOP_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all thirteen NoShop claim keys under all eight '
      + 'subtype buckets. This packet proposes profiles across six populated buckets '
      + '(RESTRICTION, STANDSTILL, RECOMMENDATION_CHANGE, REPRESENTATIVE_CONTROL, '
      + 'SAFE_DISCLOSURE, ENGAGEMENT_PERMISSION), which diverges from the calibration '
      + 'pack tagging every provision example RESTRICTION. The divergence is stamped as '
      + 'a hold, not resolved. KEY_DEFINED_TERMS, TERMINATION_FEE, PROXY_MEETING, and '
      + 'TERMINATION cross-refs remain Q02 link-only.',
    sealed_ruling_reuse:
