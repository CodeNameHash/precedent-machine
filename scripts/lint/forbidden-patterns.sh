...
function isHumanAnchorMachinePacket(rel, src) {
  if (rel !== HUMAN_ANCHOR_MACHINE_PACKET_FILE) return false;
  try {
    return JSON.parse(src).schema_version === HUMAN_ANCHOR_MACHINE_PACKET_SCHEMA;
  } catch (_) {
    return false;
  }
}
...
for (const rel of changedFiles()) {
  ...
  for (const pattern of globalPatterns) {
    ...
    // These Stage 2Y evidence ledgers are deterministic copies of recorded
