#!/usr/bin/env node
/**
 * Regenerate the on-disk NO_SHOP Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-no-shop.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'NO_SHOP';
...
const APPROVAL_TEXT =
  'Ben approves the NO_SHOP 365-profile Work3 package inventory; subtype grouping '
  + 'review pending.';
...
function buildPhase4Review() {
  ...
  return noShopAuthoring.prepareNoShopFamilyProfilePackageReview({
    noShopAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    noShopAuthoringPhase2Authority: phase2Authority,
