#!/usr/bin/env node
/**
 * Emit SPECIFIC_PERFORMANCE_REMEDIES Phase 2 authoring authority v2.
 *
 * Eight governed M4 claims across six comparator deals (Concho, Metsera, Red Hat,
 * Skechers, Skywater, TopBuild). Claim-scale partition — one terminal per governed
 * claim. Metsera §9.10 and TopBuild §7.6 each carry two independently operative
 * limbs; Q01 requires one profile per limb, not one per section.
 *
 * Termination Fee (#10) sole-remedy rows on fee sections stay Q02 link-only.
 * Five comparator deals carry comparator-resolved owner_family tension with this
 * family; those terminals stamp COMPARATOR_OWNER_FAMILY_DISPOSITION_REQUIRED and
 * remain honest holds until Ben resolves the open sole-remedy ownership question.
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/SPECIFIC_PERFORMANCE_REMEDIES.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/SPECIFIC_PERFORMANCE_REMEDIES.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-specific-performance-remedies-authoring.js';
const TEST_PATH =
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  SPECIFIC_PERFORMANCE_REMEDY_PRESENT: 'GENERAL_EQUITABLE_RELIEF',
};

const TF_SOLE_REMEDY_OWNER_FAMILY_CONFLICT_AGREEMENT_IDS = new Set([
...
const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    ...
    deal: 'concho',
    disposition: 'LINK_ONLY_NOT_A_SPECIFIC_PERFORMANCE_REMEDIES_TERMINAL',
...
function classificationPath(bucket) {
  return ['SPECIFIC_PERFORMANCE_REMEDIES', bucket];
}
...
function requiredExpressionSignature(claim, bucket) {
  ...
  return `SPECIFIC_PERFORMANCE_REMEDIES::${bucket}::${parts.join('_')}`;
}
...
function buildTerminal(claim, node, example) {
  ...
  if (TF_SOLE_REMEDY_OWNER_FAMILY_CONFLICT_AGREEMENT_IDS.has(claim.agreement_id)) {
    unresolvedItems.push('COMPARATOR_OWNER_FAMILY_DISPOSITION_REQUIRED');
  }
  return {
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'SPECIFIC_PERFORMANCE_REMEDIES');
...
function buildAuthority() {
  ...
  const ownerFamilyHoldCount = terminals.filter((terminal) => terminal.unresolved_items.includes(
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_SPECIFIC_PERFORMANCE_REMEDIES_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose:
        'Claim-scale comparator terminal registry for SPECIFIC_PERFORMANCE_REMEDIES Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_SPECIFIC_PERFORMANCE_REMEDIES_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_SPECIFIC_PERFORMANCE_REMEDIES_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      sparse_comparator_stress: 'EIGHT_GOVERNED_CLAIMS_ACROSS_SIX_COMPARATOR_DEALS',
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_SIX_EXAMPLES_GENERAL_EQUITABLE_RELIEF_WHILE_TERMINATION_FEE_SOLE_REMEDY_OWNER_FAMILY_REMAINS_OPEN',
      temporal_graph_materialisation: 'DEFERRED_NOT_TERMINATION_FRONTIER',
      termination_fee_sole_remedy_owner_family_hold_count: ownerFamilyHoldCount,
    },
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareSpecificPerformanceRemediesPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade:
          'SPECIFIC_PERFORMANCE_REMEDIES Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareSpecificPerformanceRemediesPhase2FamilyProposal',
      ...
      required_unresolved_items: [
        ...
        'TERMINATION_FEE_SOLE_REMEDY_OWNER_FAMILY_QUESTION_OPEN_FOR_BEN',
      ],
      source_inventory: {
        admitted_m4_claim_count: m4ClaimIds.length,
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved SPECIFIC_PERFORMANCE_REMEDIES partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'specific_performance_remedies_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale:
        'Specific performance remedies first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    specific_performance_remedies_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'specific_performance_remedies_authoring_phase2_authority_id',
    record_id: record.specific_performance_remedies_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
    ...
    owner_family_hold: record.policy_overlay.termination_fee_sole_remedy_owner_family_hold_count,
  }, null, 2)}\n`);
}
