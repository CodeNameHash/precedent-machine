#!/usr/bin/env node
/**
 * Emit the TAX_MATTERS Work3 unapproved inventory review authority and the
 * 17-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = taxMattersAuthoring.TAX_MATTERS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_TAX_MATTERS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_TAX_MATTERS_17_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'tax_matters_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'tax_matters_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  taxMattersAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  taxMattersAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = taxMattersAuthoring
  .prepareTaxMattersFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_TAX_MATTERS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    tax_matters_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareTaxMattersWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_TAX_MATTERS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Tax Matters claim keys under all seven '
      + 'subtype buckets. This packet proposes multi-limb opinion cooperation claims under TAX_OPINION_COOPERATION and the '
      + 'no-financing-condition acknowledgment under TRANSFER_TAX_ALLOCATION, which diverges from '
      + 'the calibration pack tagging every provision example INTENDED_TAX_TREATMENT. The divergence is '
