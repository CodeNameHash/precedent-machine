...
/**
 * Regenerate the on-disk ANTITRUST_REGULATORY Work3 family profile package from the
 * Phase 4 review candidate (70 profiles) and the lawful Work3 package template.
 */
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-antitrust-regulatory.json`;
const WORK3_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'ANTITRUST_REGULATORY';
...
const APPROVAL_TEXT =
  ...
  + `(${PROFILE_COUNT} APPROVE / 0 HOLD; subtype partition across the twelve sealed M5 buckets and fourteen comparator buckets `
  + `remains LEGAL_GROUPING_REVIEW_REQUIRED on every row, and ${M5_SUBTYPE_COUNT} M5-subtype-partition-unresolved `
  + 'rows are link-only to KEY_DEFINED_TERMS).';
...
function phase4Fixture() {
  ...
  return {
    antitrustRegulatoryAuthoringPhase4FamilyProfilePackageReviewAuthority: boundEnvelope(pinned(
...
function buildProfileFromTemplate(templateProfile, {
  ...
  profile.match_test = {
    ...
    tokens: ['familyantitrustregulatory', profileMatchToken(requiredExpressionSignature)],
  };
  return profile;
}
...
function buildPackageRecord() {
  ...
  const phase4Review = antitrustRegulatoryAuthoring.prepareAntitrustRegulatoryFamilyProfilePackageReview(
    phase4Fixture(),
  );
...
function main() {
  ...
  console.log(JSON.stringify({
    ...
    hold_count: 0,
    m5_subtype_unresolved_count: M5_SUBTYPE_COUNT,
