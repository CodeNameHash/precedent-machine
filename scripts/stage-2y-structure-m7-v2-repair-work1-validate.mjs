...
const EXPECTED_TEMPORAL_SHARED_SOURCE = Object.freeze({
...
const EXPECTED_SEMANTIC_ROLES = Object.freeze([
...
const EXPECTED_SEMANTIC_INPUT_CONTRACT = Object.freeze({
  count: 6,
  roles: EXPECTED_SEMANTIC_ROLES,
...
const EXPECTED_SEMANTIC_INPUT_CONSUMPTION = Object.freeze({
...
const EXPECTED_EXPRESSION_CONTRACT = Object.freeze({
...
const EXPECTED_SCHEMA_VERSIONS = Object.freeze({
...
const EXPECTED_STATE_COMBINATIONS = Object.freeze([
...
const EXPECTED_SOURCE_COVERAGE = Object.freeze({
...
const EXPECTED_SOURCE_STATUS_DERIVATION = Object.freeze({
...
const EXPECTED_PROFILE_GATE = Object.freeze({
...
const EXPECTED_LINKED_RULE_STATE = Object.freeze({
...
const EXPECTED_SEMANTIC_IDENTITY = Object.freeze({
...
const EXPECTED_NORMALISATION = Object.freeze({
...
const EXPECTED_PROFILE_FIXTURES = Object.freeze({
...
const EXPECTED_EQUIVALENCE_SIGNATURE = Object.freeze({
  mapping_slots: [
...
const EXPECTED_EFFECT_LOCAL_PROVENANCE = Object.freeze({
...
const EXPECTED_EFFECT_LEDGER = Object.freeze({
  entry_members: [
...
const EXPECTED_NO_OUTPUT = Object.freeze({
...
const EXPECTED_STRUCTURE_DISPOSITIONS = Object.freeze({
  required_members: [
...
const EXPECTED_PROJECTION_POLICY = Object.freeze({
...
const EXPECTED_CANDIDATE_POLICY = Object.freeze({
...
const EXPECTED_GENERATOR_POLICY = Object.freeze({
...
export class Work1ValidationError extends Error {
  constructor(code, detail = '') {
    super(detail ? `${code}: ${detail}` : code);
    this.name = 'Work1ValidationError';
    this.code = code;
  }
}

function fail(code, detail = '') {
  throw new Work1ValidationError(code, detail);
}
...
function resolvePath(root, repositoryPath, allowMissingLeaf = false) {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    if (stat.isSymbolicLink() || (index < parts.length - 1 && !stat.isDirectory())
      || (index === parts.length - 1 && !stat.isFile())) fail('PATH_INVALID', repositoryPath);
  }
  return current;
}

function readBytes(root, repositoryPath) {
  return readFileSync(resolvePath(root, repositoryPath));
}

function parseOptionalRecord(root, repositoryPath, idField) {
  let absolute;
  try {
    absolute = resolvePath(root, repositoryPath, true);
  } catch {
    fail('RECOVERY_PATH_SCOPE', repositoryPath);
  }
  if (!existsSync(absolute)) return null;
  try {
    return parseRecord(root, repositoryPath, null, idField);
  } catch (error) {
    if (error instanceof Work1ValidationError && error.code === 'PATH_INVALID') {
      fail('RECOVERY_PATH_SCOPE', repositoryPath);
    }
...
function parseRecord(root, repositoryPath, digestField = null, idField = null) {
  const bytes = readBytes(root, repositoryPath);
  let record;
  try { record = JSON.parse(bytes.toString('utf8')); } catch { fail('RECORD_INVALID', repositoryPath); }
  if (!bytes.equals(canonicalBytes(record))) fail('RECORD_INVALID', `${repositoryPath}:canonical`);
  if (digestField && idField) {
    ...
    if (record[digestField] !== digest || record[idField] !== contentId(record.schema_version, withDigest)) {
      fail('SELF_ID_DRIFT', repositoryPath);
    }
  } else if (idField) {
    const unsigned = JSON.parse(JSON.stringify(record));
    delete unsigned[idField];
...
function parentWork1Paths(inputs) {
  ...
  if (!Array.isArray(paths) || paths.length !== 13 || new Set(paths).size !== 13
    ...
    fail('AUTHORITY_BINDING_DRIFT', 'Work1 paths');
  }
  return paths;
}
...
function validateCorrectionAuthority(root, correctionAuthority, inputs) {
  ...
  try {
    ...
    ].map((repositoryPath) => binding(repositoryPath, readBytes(root, repositoryPath)));
  } catch {
...
function validateWork0EvidenceBindings(inputs) {
  ...
  for (const [repositoryPath, input] of generatedRecords) {
    ...
    const recordBinding = inputs.work0.record.work0_record_bindings.find(
      (entry) => entry.path === repositoryPath,
    );
    const snapshotBinding = inputs.work0.record.snapshot_bindings.find(
      (entry) => entry.path === repositoryPath,
    );
  ...
  if (!packetBinding || packetBinding.role !== 'LAWYER_REVIEW_PACKET'
    ...
    fail('AUTHORITY_BINDING_DRIFT', REVIEW_PACKET_PATH);
  }
}

function validateContractPolicy(policy, inputs) {
  const policyKeys = [
    'schema_version', 'contract_policy_id', 'contract_policy_digest', 'stage', 'state', 'version',
    'work0_evidence_root_binding', 'work1_7_authority_binding', 'activation_receipt_binding',
    'public_interfaces', 'semantic_input_contract', 'semantic_input_consumption_contract',
    'schema_versions', 'typed_value_kinds',
    'operator_vocabulary', 'legal_expression_contract', 'allowed_state_combinations',
    'source_coverage', 'source_status_derivation_contract', 'profile_gate', 'ownership',
    'linked_rule_state_contract',
    'semantic_identity_contract',
    'fact_normalisation_contract', 'equivalence_signature_contract',
    'effect_local_provenance_contract', 'effect_ledger_contract',
    'temporal_shared_source_contract',
    'profile_fixture_contract', 'no_output_contract',
    'structure_disposition_contract', 'projection', 'candidate_registration',
    'deterministic_generator_policy',
  ];
  if (inputs.correctionAuthority) {
    policyKeys.push('work1_correction_recovery_contract', 'correction_authority_binding');
  }
  if (!exactKeys(policy, policyKeys)
    || policy.schema_version !== CONTRACT_SCHEMA
    || policy.stage !== 'M7_V2_REPAIR_WORK1'
    || policy.state !== 'TECHNICAL_CONTRACT_ORACLE_NOT_LEGAL_PROFILE_APPROVAL'
    || policy.version !== 1
    || !same(policy.public_interfaces, [
    ...
    || !same(policy.semantic_input_contract, EXPECTED_SEMANTIC_INPUT_CONTRACT)
    || !same(policy.semantic_input_consumption_contract, EXPECTED_SEMANTIC_INPUT_CONSUMPTION)
    || !same(policy.schema_versions, EXPECTED_SCHEMA_VERSIONS)
    || !same(policy.operator_vocabulary, EXPECTED_OPERATOR_VOCABULARY)
    || !same(policy.legal_expression_contract, EXPECTED_EXPRESSION_CONTRACT)
    || !same(policy.typed_value_kinds, EXPECTED_TYPED_VALUE_KINDS)
    || !same(policy.allowed_state_combinations, EXPECTED_STATE_COMBINATIONS)
    || !same(policy.source_coverage, EXPECTED_SOURCE_COVERAGE)
    || !same(policy.source_status_derivation_contract, EXPECTED_SOURCE_STATUS_DERIVATION)
    || !same(policy.profile_gate, EXPECTED_PROFILE_GATE)
    || !same(policy.ownership, EXPECTED_OWNERSHIP)
    || !same(policy.linked_rule_state_contract, EXPECTED_LINKED_RULE_STATE)
    || !same(policy.semantic_identity_contract, EXPECTED_SEMANTIC_IDENTITY)
    || !same(policy.fact_normalisation_contract, EXPECTED_NORMALISATION)
    || !same(policy.equivalence_signature_contract, EXPECTED_EQUIVALENCE_SIGNATURE)
    || !same(policy.effect_local_provenance_contract, EXPECTED_EFFECT_LOCAL_PROVENANCE)
    || !same(policy.effect_ledger_contract, EXPECTED_EFFECT_LEDGER)
    || !same(policy.temporal_shared_source_contract, EXPECTED_TEMPORAL_SHARED_SOURCE)
    || !same(policy.profile_fixture_contract, EXPECTED_PROFILE_FIXTURES)
    || !same(policy.no_output_contract, EXPECTED_NO_OUTPUT)
    || !same(policy.structure_disposition_contract, EXPECTED_STRUCTURE_DISPOSITIONS)
    || !same(policy.projection, EXPECTED_PROJECTION_POLICY)
    || !same(policy.candidate_registration, EXPECTED_CANDIDATE_POLICY)
    || !same(policy.deterministic_generator_policy, EXPECTED_GENERATOR_POLICY)) {
    fail('CONTRACT_POLICY_INVALID');
  }
  if (inputs.correctionAuthority
    && !same(policy.work1_correction_recovery_contract, WORK1_CORRECTION_RECOVERY_CONTRACT)) {
    fail('RECOVERY_AUTHORITY_INVALID', 'contract policy');
  }
  if (inputs.correctionAuthority
    && !same(policy.correction_authority_binding, binding(
      CORRECTION_AUTHORITY_PATH,
      inputs.correctionAuthority.bytes,
      inputs.correctionAuthority.record,
    ))) {
    fail('RECOVERY_BINDING_DRIFT', 'contract policy');
  }
  if (!same(policy.work0_evidence_root_binding, binding(WORK0_PATH, inputs.work0.bytes, inputs.work0.record))
    || !same(policy.work1_7_authority_binding, binding(AUTHORITY_PATH, inputs.authority.bytes, inputs.authority.record))
    || !same(policy.activation_receipt_binding, binding(ACTIVATION_PATH, inputs.activation.bytes, inputs.activation.record))) {
    fail('CONTRACT_POLICY_BINDING_DRIFT');
  }
}

function validateFamilyPacketSet(packetSet, inputs) {
  ...
  const rulingByFamily = new Map(inputs.rulingMap.record.families.map((item) => [item.family_key, item]));
  const packetKeys = [
    'schema_version', 'family_packet_set_id', 'family_packet_set_digest', 'stage', 'state',
    'work0_evidence_root_binding', 'fixed_sample_identity_binding', 'repair_baseline_binding',
    'calibration_ruling_map_binding', 'lawyer_review_packet_binding', 'coverage', 'constraints',
    'families', 'structure_ambiguity_members',
  ];
  ...
  if (!exactKeys(packetSet, packetKeys)
    || packetSet.schema_version !== FAMILY_PACKET_SCHEMA
    || packetSet.stage !== 'M7_V2_REPAIR_WORK1'
    || packetSet.state !== 'LEGAL_EVIDENCE_ORACLE_NOT_EXECUTABLE_PROFILE_AUTHORITY'
    || !same(packetSet.coverage, expectedCoverage)
    || !same(packetSet.constraints, expectedConstraints)
    || !Array.isArray(packetSet.families) || packetSet.families.length !== 25
    || !same(
      packetSet.families.map((family) => family.family_key),
      inputs.rulingMap.record.families.map((family) => family.family_key),
    )) {
    fail('FAMILY_PACKET_INVALID');
  }
  const expectedBindings = [
    ['work0_evidence_root_binding', WORK0_PATH, inputs.work0],
    ['fixed_sample_identity_binding', FIXED_SAMPLE_PATH, inputs.fixed],
    ['repair_baseline_binding', BASELINE_PATH, inputs.baseline],
    ['calibration_ruling_map_binding', RULING_MAP_PATH, inputs.rulingMap],
    ['lawyer_review_packet_binding', REVIEW_PACKET_PATH, inputs.packet],
  ];
  for (const [field, repositoryPath, input] of expectedBindings) {
    if (!same(packetSet[field], binding(repositoryPath, input.bytes, input.record))) {
      fail('FAMILY_PACKET_BINDING_DRIFT', field);
    }
  }
  const seenOrdinals = [];
  const memberKeys = [
    'sample_ordinal', 'review_item_id', 'agreement_id', 'item_kind', 'prior_row_id',
    'source_node_occurrence_ids', 'ambiguity_id', 'source_spans', 'source_excerpt_sha256',
    'repair_membership', 'repair_class', 'original_decision', 'original_note',
    'lawyer_decision_id', 'reviewer',
  ...
  const validateMember = (member, expectedFamilyKey) => {
    const fixed = fixedByOrdinal.get(member.sample_ordinal);
    const baseline = baselineByOrdinal.get(member.sample_ordinal);
    const packet = packetByOrdinal.get(member.sample_ordinal);
    ...
    if (!exactKeys(member, memberKeys)
      || !exactKeys(member.focused_expectation, ['state', 'invariant_id', 'note_application'])
      || !fixed || !baseline || !packet || fixed.family_key !== expectedFamilyKey
      || packet.family_key !== expectedFamilyKey
      || member.review_item_id !== fixed.review_item_id
      || member.agreement_id !== fixed.agreement_id
      || member.item_kind !== fixed.item_kind
      || member.prior_row_id !== fixed.prior_row_id
      || !same(member.source_node_occurrence_ids, fixed.source_node_occurrence_ids)
      || member.ambiguity_id !== fixed.ambiguity_id
      || !same(member.source_spans, fixed.source_spans)
      || member.source_excerpt_sha256 !== fixed.source_excerpt_sha256
      || member.original_decision !== baseline.original_decision
      || member.original_note !== baseline.original_note
      || member.lawyer_decision_id !== baseline.lawyer_decision_id
      || member.reviewer !== baseline.reviewer
      || member.repair_membership !== baseline.repair_membership
      || member.repair_class !== baseline.repair_class
      || member.fresh_work5_question_required !== baseline.requires_fresh_work5_question
      || member.linked_point_annotation !== LINKED_POINT_ORDINALS.includes(member.sample_ordinal)
      || member.broad_legal_meaning_question !== 'Does the V2 result preserve every important legal effect, condition, exception, timing term, standard and qualification in this source?'
      || member.family_and_subtype_question !== `Is ${expectedFamilyKey ?? 'the post-overlay result'} assigned to the correct family and most-specific supported subtype?`
      || !same(member.focused_expectation, expectedFocused)) {
      fail('FAMILY_PACKET_SAMPLE_DRIFT', `${member.sample_ordinal}`);
    }
    seenOrdinals.push(member.sample_ordinal);
  };
  for (const family of packetSet.families) {
    const source = rulingByFamily.get(family.family_key);
    const expectedMappings = source?.question_mappings.map((mapping) => ({
    ...
    const expectedSampleOrdinals = inputs.fixed.record.members
      .filter((item) => item.family_key === family.family_key)
      .map((item) => item.sample_ordinal);
    if (!exactKeys(family, [
      ...
      || family.programme_question_mappings.some((mapping) => !exactKeys(mapping, [
        'family_question_id', 'programme_question_id', 'ruling_id', 'selection', 'legal_rule',
      ]))
      || !source || family.wave !== source.wave || family.executable_matcher_present !== false
      ...
      || !same(family.calibration_pack_binding, source.calibration_pack_binding)
      || !same(family.programme_question_mappings, expectedMappings)
      || !same(family.sample_members.map((member) => member.sample_ordinal), expectedSampleOrdinals)) {
      fail('FAMILY_PACKET_INVALID', family.family_key);
    }
    for (const member of family.sample_members) validateMember(member, family.family_key);
  }
  if (!Array.isArray(packetSet.structure_ambiguity_members)
    || packetSet.structure_ambiguity_members.length !== 1
    || packetSet.structure_ambiguity_members[0].sample_ordinal !== 39) {
    fail('FAMILY_PACKET_INVALID', 'structure ambiguity');
  }
  validateMember(packetSet.structure_ambiguity_members[0], null);
  if (!same(seenOrdinals.sort((a, b) => a - b), Array.from({ length: 50 }, (_, index) => index + 1))) {
    fail('FAMILY_PACKET_COVERAGE_DRIFT');
  }
}

function validateStaticBoundary(root) {
  const contractSource = readBytes(root, 'lib/canonical-v2/m7-v2-contract.js').toString('utf8');
  const builderSource = readBytes(root, 'scripts/stage-2y-structure-m7-v2-repair-register-candidate.mjs').toString('utf8');
  const verifierSource = readBytes(root, 'scripts/stage-2y-structure-m7-v2-repair-verify-candidate.mjs').toString('utf8');
  const manifestSource = readBytes(root, 'scripts/stage-2y-structure-m7-v2-repair-execution-manifest-validate.mjs').toString('utf8');
  const dependencySpecifiers = (source) => {
    const specifiers = [];
    const staticImport = /^\s*import\s+(?:[^'";]*?\s+from\s+)?['"]([^'"]+)['"]\s*;?/gm;
    const commonJsRequire = /\brequire\s*\(\s*(['"])([^'"]+)\1\s*\)/g;
    let match;
    while ((match = staticImport.exec(source)) !== null) specifiers.push(match[1]);
    while ((match = commonJsRequire.exec(source)) !== null) specifiers.push(match[2]);
    return specifiers;
  };
  const allowed = (source, allowedNodeModules) => {
    if (/\bimport\s*\(/.test(source) || /\beval\s*\(/.test(source) || /\bnew\s+Function\s*\(/.test(source)
      || /\bfetch\s*\(/.test(source) || /\brequire\s*\(\s*[^'"\s]/.test(source)
      || /\b(?:const|let|var)\s+[A-Za-z_$][\w$]*\s*=\s*require\b/.test(source)
      || /\bmodule\s*\.\s*require\s*\(/.test(source)
      || /\bprocess\s*\.\s*getBuiltinModule\s*\(/.test(source)
      || /\bFunction\s*\(/.test(source)) return false;
    return dependencySpecifiers(source).every((specifier) => allowedNodeModules.includes(specifier)
      || /(?:^|\/)canonical-bytes(?:\.js)?$/.test(specifier));
  };
  const esmExports = (source) => [...source.matchAll(
    /^export\s+(?:async\s+)?(?:function|class|const|let|var)\s+([A-Za-z_$][\w$]*)/gm,
  )].map((match) => match[1]).sort();
  const contractExportsExactly = /module\.exports\s*=\s*\{\s*validateAnalysisV2\s*,\s*validateProjectionV2\s*,?\s*\}\s*;/.test(contractSource)
    && (contractSource.match(/module\.exports\s*=/g) ?? []).length === 1;
  if (!allowed(contractSource, ['node:assert'])
    || !allowed(builderSource, ['node:crypto', 'node:fs', 'node:path'])
    || !allowed(verifierSource, ['node:child_process', 'node:crypto', 'node:fs', 'node:path', 'node:url'])
    || !allowed(manifestSource, ['node:crypto', 'node:fs', 'node:path', 'node:url'])
    || !contractExportsExactly
    || !same(esmExports(builderSource), ['registerCandidate'])
    || !same(esmExports(verifierSource), ['verifyRegisteredCandidate'])
    || !same(esmExports(manifestSource), ['WorkExecutionManifestValidationError', 'validateExecutionManifest'].sort())
    || /\bfs\s*\.\s*(?:write|append|truncate|rename|unlink|rm|mkdir|rmdir|link|symlink|copyFile|chmod|chown|utimes|createWriteStream|open)\w*\s*\(/.test(verifierSource)
    || /\b(?:writeFile|appendFile|truncate|rename|unlink|rm|mkdir|rmdir|link|symlink|copyFile|chmod|chown|utimes|createWriteStream|open)(?:Sync)?\b/.test(verifierSource)
    || /\b(?:const|let|var)\s+[A-Za-z_$][\w$]*\s*=\s*fs\b(?!\s*(?:\.|\[))/.test(verifierSource)) {
    fail('STATIC_BOUNDARY_INVALID');
  }
}

function loadInputs(root) {
  const inputs = {
    work0: parseRecord(root, WORK0_PATH, null, 'evidence_root_id'),
    authority: parseRecord(root, AUTHORITY_PATH, 'authority_digest', 'authority_id'),
    activation: parseRecord(root, ACTIVATION_PATH, 'activation_receipt_digest', 'activation_receipt_id'),
    fixed: parseRecord(root, FIXED_SAMPLE_PATH, null, 'fixed_sample_identity_manifest_id'),
    baseline: parseRecord(root, BASELINE_PATH, null, 'repair_baseline_ledger_id'),
    rulingMap: parseRecord(root, RULING_MAP_PATH, null, 'calibration_question_ruling_map_id'),
    packet: parseRecord(root, REVIEW_PACKET_PATH),
    correctionAuthority: parseOptionalRecord(
      root, CORRECTION_AUTHORITY_PATH, null,
    ),
    contract: parseRecord(root, CONTRACT_POLICY_PATH, 'contract_policy_digest', 'contract_policy_id'),
    familyPacket: parseRecord(root, FAMILY_PACKET_PATH, 'family_packet_set_digest', 'family_packet_set_id'),
    receipt: parseRecord(root, RECEIPT_PATH, 'work1_contract_receipt_digest', 'work1_contract_receipt_id'),
  };
  ...
  if (boundWork0?.path !== WORK0_PATH || boundWork0?.evidence_root_id !== WORK0_ID
    ...
    fail('AUTHORITY_BINDING_DRIFT', WORK0_PATH);
  }
  validateWork0EvidenceBindings(inputs);
  validateCorrectionAuthority(root, inputs.correctionAuthority, inputs);
  return inputs;
}

function validateReceipt(root, inputs) {
  const receipt = inputs.receipt.record;
  ...
  const expectedActivationCommit = {
    commit: ACTIVATION_COMMIT,
    branch: 'codex/recover-m7-20260812',
    authority_activation_receipt_id: ACTIVATION_ID,
  };
  const expectedCombinedTest = {
    argv: commandLedger[6].argv,
    status: 'PASS',
    test_file_count: 3,
    semantic_run_count: 12,
  };
  const expectedRepositoryPrecondition = {
    ...
    required_commit_and_push: {
      ...
      required_origin_ref: 'refs/remotes/origin/codex/recover-m7-20260812',
    },
  };
  const recovery = recoveryPrecondition(inputs);
  if (recovery) expectedRepositoryPrecondition.recovery = recovery;
  const receiptKeys = [
    ...
    'command_execution_ledger', 'drafting_command_audit', 'combined_test_result', 'repository_precondition',
    'counts', 'checks', 'effects', 'next_work',
  ];
  if (inputs.correctionAuthority) {
    const actualRecovery = receipt.repository_precondition?.recovery;
    if (!exactKeys(actualRecovery, RECOVERY_PRECONDITION_KEYS)
      || actualRecovery.recovery_run_count
        !== RECOVERY_LEDGER_COUNTS_PENDING_FINAL_AUDIT.recovery_run_count
      || actualRecovery.finaliser_cumulative_run_count
        !== RECOVERY_LEDGER_COUNTS_PENDING_FINAL_AUDIT.work1_finaliser_cumulative_run_count
      || actualRecovery.validator_cumulative_run_count
        !== RECOVERY_LEDGER_COUNTS_PENDING_FINAL_AUDIT.work1_validator_cumulative_run_count
      || actualRecovery.backup_state !== 'REMOVED_AFTER_VALIDATOR_PASS'
      || actualRecovery.rollback_state !== 'AVAILABLE_DURING_TRANSACTION_ONLY'
      || !same(actualRecovery.recovery_argv, CORRECTION_COMMAND_EXTENSION.recovery_argv)) {
      fail('RECOVERY_AUTHORITY_INVALID', 'receipt precondition');
    }
    if (!same(actualRecovery.correction_authority_binding,
      expectedRepositoryPrecondition.recovery.correction_authority_binding)
      || !same(actualRecovery.superseded_receipt_binding, STALE_OUTPUT_BINDINGS[2])) {
      fail('RECOVERY_BINDING_DRIFT', 'receipt precondition');
    }
    if (!same(actualRecovery.replaced_output_paths, RECOVERY_TARGETS)) {
      fail('RECOVERY_PATH_SCOPE', 'receipt precondition');
    }
  }
  if (!exactKeys(receipt, receiptKeys)
    || receipt.schema_version !== RECEIPT_SCHEMA || receipt.stage !== 'M7_V2_REPAIR_WORK1'
    || receipt.state !== 'PASS_WORK1_CONTRACTS' || receipt.status !== 'PASS'
    || !same(receipt.activation_commit_binding, expectedActivationCommit)
    ...
    || !same(receipt.command_execution_ledger, commandLedger)
    || !same(receipt.drafting_command_audit, DRAFTING_COMMAND_AUDIT)
    || !same(receipt.combined_test_result, expectedCombinedTest)
    || !same(receipt.repository_precondition, expectedRepositoryPrecondition)) {
    fail('RECEIPT_INVALID');
  }
  const expectedInputs = [
    ['work0_evidence_root_binding', WORK0_PATH, inputs.work0],
    ['work1_7_authority_binding', AUTHORITY_PATH, inputs.authority],
    ['activation_receipt_binding', ACTIVATION_PATH, inputs.activation],
    ['contract_policy_binding', CONTRACT_POLICY_PATH, inputs.contract],
    ['family_packet_set_binding', FAMILY_PACKET_PATH, inputs.familyPacket],
  ];
  for (const [field, repositoryPath, input] of expectedInputs) {
    if (!same(receipt[field], binding(repositoryPath, input.bytes, input.record))) fail('RECEIPT_BINDING_DRIFT', field);
  }
  const expectedPaths = authorityPaths.filter((repositoryPath) => repositoryPath !== RECEIPT_PATH);
  if (!same(receipt.artifact_bindings.map((item) => item.path), expectedPaths)) {
    fail(inputs.correctionAuthority ? 'RECOVERY_PATH_SCOPE' : 'RECEIPT_BINDING_DRIFT', 'paths');
  }
  const expectedBindings = expectedPaths.map((repositoryPath) => {
    const bytes = readBytes(root, repositoryPath);
    let record = null;
  ...
  if (!same(receipt.artifact_bindings, expectedBindings)
    || receipt.artifact_set_digest !== sha256Hex(canonicalJson(expectedBindings))) {
    fail(inputs.correctionAuthority ? 'RECOVERY_BINDING_DRIFT' : 'RECEIPT_BINDING_DRIFT', 'artifact set');
  }
}

export function validateWork1(options = {}) {
  if (options === null || typeof options !== 'object' || Array.isArray(options)
      || Object.keys(options).some((key) => key !== 'repoRoot')) {
    fail('INVALID_OPTIONS');
  }
  const repoRoot = options.repoRoot ?? REPO_ROOT;
  if (typeof repoRoot !== 'string') fail('INVALID_OPTIONS');
  ...
  validateContractPolicy(inputs.contract.record, inputs);
  validateFamilyPacketSet(inputs.familyPacket.record, inputs);
  validateStaticBoundary(root);
  validateReceipt(root, inputs);
  return {
    schema_version: 'STAGE_2Y_M7_V2_REPAIR_WORK1_VALIDATION/V1',
    status: 'PASS_WORK1_CONTRACTS',
    contract_policy_id: inputs.contract.record.contract_policy_id,
    family_packet_set_id: inputs.familyPacket.record.family_packet_set_id,
    work1_contract_receipt_id: inputs.receipt.record.work1_contract_receipt_id,
    counts: inputs.receipt.record.counts,
    effects: inputs.receipt.record.effects,
  };
}
...
if (process.argv[1] && realpathSync(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    process.stdout.write(`${canonicalJson(validateWork1())}\n`);
  } catch (error) {
    const code = error instanceof Work1ValidationError ? error.code : 'VALIDATION_FAILED';
