#!/usr/bin/env node
/**
 * Emit the FINANCING_COVENANTS Work3 unapproved inventory review authority and the
...
const PHASE4_PATH = financingCovenantsAuthoring.FINANCING_COVENANTS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_FINANCING_COVENANTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_FINANCING_COVENANTS_5_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'financing_covenants_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'financing_covenants_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  financingCovenantsAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  financingCovenantsAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = financingCovenantsAuthoring
  .prepareFinancingCovenantsFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_FINANCING_COVENANTS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    financing_covenants_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareFinancingCovenantsWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_FINANCING_COVENANTS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Financing Covenants claim keys under all seven '
      + 'subtype buckets. This packet proposes payoff lead-time claims under PAYOFF and the '
      + 'no-financing-condition acknowledgment under NO_FINANCING_CONDITION, which diverges from '
      + 'the calibration pack tagging every provision example OBTAIN_FINANCING. The divergence is '
