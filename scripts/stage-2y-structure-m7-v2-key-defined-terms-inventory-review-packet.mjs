#!/usr/bin/env node
/**
 * Emit the KEY_DEFINED_TERMS Work3 unapproved inventory review authority and the
 * 76-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE2_PATH = keyDefinedTermsAuthoring.KEY_DEFINED_TERMS_PHASE2_AUTHORITY_PATH;
const PHASE4_PATH = keyDefinedTermsAuthoring.KEY_DEFINED_TERMS_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_KEY_DEFINED_TERMS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_KEY_DEFINED_TERMS_76_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'key_defined_terms_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'key_defined_terms_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  keyDefinedTermsAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  keyDefinedTermsAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = keyDefinedTermsAuthoring
  .prepareKeyDefinedTermsFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_KEY_DEFINED_TERMS_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    key_defined_terms_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareKeyDefinedTermsWork3UnapprovedInventoryReview',
    exact_outer_input_keys: [
      'key_defined_termsWork3UnapprovedInventoryReviewEvidence',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_KEY_DEFINED_TERMS_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three KeyDefinedTerms claim keys under all seventy-six '
      + 'subtype buckets. This packet proposes knowledge-person and standard definition content with Representations link-only boundary claims under KNOWLEDGE and the '
      + 'no-financing-condition acknowledgment under WILLFUL_BREACH, which diverges from '
      + 'the calibration pack tagging every provision example KEY_DEFINED_TERMS_PACKAGE. The divergence is '
