#!/usr/bin/env node
/**
 * Emit MATERIAL_CONTRACTS Phase 2 authoring authority v2.
 *
 * One hundred sixteen governed M4 claims across six comparator deals (Concho,
 * Metsera, Modiv, Red Hat, Skywater, TopBuild). Claim-scale partition — one
 * terminal per governed claim. Representations status reps and Termination
 * breach-termination-right subtypes on shared sections stay link-only under Q02.
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/MATERIAL_CONTRACTS.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/MATERIAL_CONTRACTS.json';
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  MATERIAL_CONTRACT_BUCKET_PRESENT: 'MATERIAL_CONTRACT_CATEGORY_CRITERION',
  MATERIAL_CONTRACT_THRESHOLD_STRUCTURE: 'MATERIAL_CONTRACT_CATEGORY_CRITERION',
};

const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    disposition: 'LINK_ONLY_NOT_A_MATERIAL_CONTRACTS_TERMINAL',
    owner_family_key: 'REPRESENTATIONS',
    ruling_id: 'M5-RULING-ONE-SEMANTIC-OWNER',
    subject: 'MATERIAL_CONTRACT_STATUS_REPRESENTATION_ON_SHARED_SECTION',
  },
  {
    disposition: 'LINK_ONLY_NOT_A_MATERIAL_CONTRACTS_TERMINAL',
    owner_family_key: 'TERMINATION',
    ruling_id: 'M5-RULING-ONE-SEMANTIC-OWNER',
    subject: 'MATERIAL_CONTRACT_BREACH_TERMINATION_RIGHT_ON_SHARED_SECTION',
  },
];
...
function classificationPath(bucket) {
  return ['MATERIAL_CONTRACTS', bucket];
}
...
function discriminatorToken(claim) {
  ...
  if (typeof attributes.bucket_label === 'string') return attributes.bucket_label;
  if (typeof attributes.threshold_kind === 'string') return attributes.threshold_kind;
...
function requiredExpressionSignature(claim, bucket, sourceCitation) {
  ...
  return `MATERIAL_CONTRACTS::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'MATERIAL_CONTRACTS');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_MATERIAL_CONTRACTS_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for MATERIAL_CONTRACTS Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_MATERIAL_CONTRACTS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_MATERIAL_CONTRACTS_FIRST_SLICE',
    },
    policy_overlay: {
    ...
    implementation_contract: {
      ...
      public_error_contract: {
        missing_facade: 'MATERIAL_CONTRACTS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareMaterialContractsPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved MATERIAL_CONTRACTS partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'material_contracts_authoring_phase2_authority_id',
  ...
  return {
    ...unsigned,
    material_contracts_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'material_contracts_authoring_phase2_authority_id',
    record_id: record.material_contracts_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
