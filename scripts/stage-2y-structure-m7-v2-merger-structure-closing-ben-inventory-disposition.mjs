#!/usr/bin/env node
/**
 * Emit the MERGER_STRUCTURE_CLOSING Ben inventory disposition, session receipt, and the
 * three remaining Work3 successor authorities plus the family package seal receipt.
 *
 * Disposition is APPROVE on all 103 profiles with the subtype-grouping question
 * stamped as an open hold. Q01-Q03 reuse the sealed M5 programme rulings the family
 * role schema already binds; no Merger Structure / Closing-specific ruling is invented.
 */
import { readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import canonicalModule from '../lib/canonical-v2/canonical-bytes.js';
import mergerStructureClosingAuthoring from '../lib/canonical-v2/m7-v2-merger-structure-closing-authoring.js';
...
const PACKET_PATH =
  `${CONTROL}/m7-v2-repair-merger-structure-closing-103-profile-inventory-review-packet-draft.json`;
const INVENTORY_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-merger-structure-closing-unapproved-inventory-review-authority.json`;
const DISPOSITION_PATH =
  `${CONTROL}/m7-v2-repair-merger-structure-closing-103-profile-inventory-disposition.json`;
const SESSION_PATH =
  `${CONTROL}/m7-v2-repair-merger-structure-closing-ben-inventory-session-receipt.json`;
const BEN_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-merger-structure-closing-ben-inventory-session-successor-authority.json`;
const SEAL_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-merger-structure-closing-family-package-seal-successor-authority.json`;
const SEAL_RECEIPT_PATH =
  `${CONTROL}/m7-v2-repair-merger-structure-closing-family-package-seal-receipt.json`;
const REGISTRATION_AUTHORITY_PATH =
  `${CONTROL}/m7-v2-repair-contract-work3-merger-structure-closing-registration-successor-authority.json`;
const RULINGS_PATH = `${CONTROL}/m5-programme-rulings.json`;
const ROLE_SCHEMA_PATH = `${CONTROL}/family-role-schemas/MERGER_STRUCTURE_CLOSING.json`;

const DISPOSITION_SCHEMA =
  'STAGE_2Y_M7_V2_MERGER_STRUCTURE_CLOSING_103_PROFILE_INVENTORY_DISPOSITION/V1';
const SESSION_SCHEMA =
  'STAGE_2Y_M7_V2_MERGER_STRUCTURE_CLOSING_BEN_INVENTORY_SESSION_RECEIPT/V1';
const BEN_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MERGER_STRUCTURE_CLOSING_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY/V1';
const SEAL_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MERGER_STRUCTURE_CLOSING_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY/V1';
const SEAL_RECEIPT_SCHEMA =
  'STAGE_2Y_M7_V2_MERGER_STRUCTURE_CLOSING_FAMILY_PACKAGE_SEAL_RECEIPT/V1';
const REGISTRATION_AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_MERGER_STRUCTURE_CLOSING_REGISTRATION_SUCCESSOR_AUTHORITY/V1';
const SEAL_CANDIDATE_SCHEMA =
  'M7_V2_MERGER_STRUCTURE_CLOSING_WORK3_FAMILY_PACKAGE_SEAL_CANDIDATE/V1';
const BEN_CANDIDATE_STATE =
  'BEN_103_PROFILE_INVENTORY_DISPOSITION_CAPTURED_PACKAGE_SEAL_NOT_RECORDED';
const SEAL_CANDIDATE_STATE =
  'BEN_MERGER_STRUCTURE_CLOSING_FAMILY_PACKAGE_SEAL_CAPTURED_REGISTRATION_NOT_RECORDED';
...
const inventoryAuthorityBinding = binding(
  ...
  inventoryAuthority,
  'work3_merger_structure_closing_unapproved_inventory_review_authority_id',
);
...
const sessionUnsigned = {
  ...
  session_classification: 'MERGER_STRUCTURE_CLOSING_103_PROFILE_INVENTORY_BEN_REVIEW',
...
const benAuthority = authority(
  ...
  'work3_merger_structure_closing_ben_inventory_session_successor_authority_id',
  {
    authority_classification:
      'WORK3_MERGER_STRUCTURE_CLOSING_BEN_INVENTORY_SESSION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareMergerStructureClosingWork3BenInventorySessionDisposition',
    ...
    schema_review_candidate_contract: {
      schema_version:
        'M7_V2_MERGER_STRUCTURE_CLOSING_WORK3_BEN_INVENTORY_SESSION_DISPOSITION_CANDIDATE/V1',
...
const sealAuthority = authority(
  ...
  'work3_merger_structure_closing_family_package_seal_successor_authority_id',
  {
    authority_classification: 'WORK3_MERGER_STRUCTURE_CLOSING_FAMILY_PACKAGE_SEAL_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareMergerStructureClosingWork3FamilyPackageSeal',
      exact_outer_input_keys: [
...
// Mirrors prepareMergerStructureClosingWork3FamilyPackageSeal exactly; the test asserts
...
const sealReceiptUnsigned = {
  ...
  seal_classification: 'MERGER_STRUCTURE_CLOSING_WORK3_FAMILY_PACKAGE_SEAL',
  completion_state: 'COMPLETE',
  reviewer: 'BEN_GOODCHILD',
...
const sealReceipt = {
  ...sealReceiptUnsigned,
  merger_structure_closing_family_package_seal_receipt_id:
...
const registrationAuthority = authority(
  ...
  'work3_merger_structure_closing_registration_successor_authority_id',
  {
    authority_classification: 'WORK3_MERGER_STRUCTURE_CLOSING_REGISTRATION_SUCCESSOR_AUTHORITY',
    ...
    implementation_contract: {
      exported_function: 'prepareMergerStructureClosingWork3FamilyPackageRegistration',
      exact_outer_input_keys: [
    ...
    schema_review_candidate_contract: {
      schema_version: 'M7_V2_MERGER_STRUCTURE_CLOSING_WORK3_FAMILY_PACKAGE_REGISTRATION_CANDIDATE/V1',
      candidate_state:
