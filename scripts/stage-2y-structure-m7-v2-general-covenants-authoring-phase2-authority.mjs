/**
 * Emit GENERAL_COVENANTS Phase 2 authoring authority v2 from the M5 calibration
 * pack and seven comparator resolution runs (54 admitted M4 claim terminals).
 */
...
const CONCEPT_TO_BUCKET = Object.freeze({
  'COV-ACCESS': 'ACCESS',
  'COV-LITNOTIFY': 'LITIGATION_NOTIFICATION',
  'COV-NOTIFY': 'GENERAL_NOTIFICATION',
  'COV-16B': 'SECTION_16',
  'COV-DELIST': 'DELISTING',
  'COV-TAKEOVER': 'TAKEOVER_LAW',
  'COV-MERGESUB': 'MERGER_SUB_OBLIGATION',
  'COV-PUBLICITY': 'PUBLICITY',
  'COV-RESIGN': 'RESIGNATION',
  'COV-CVR': 'CVR',
  'COV-LIST': 'LISTING',
});
...
function classificationPath(bucket) {
  return ['GENERAL_COVENANTS', 'TOPIC_CLASSIFICATION', bucket];
}
...
function buildTerminal({
  ...
  const sourceUnitKey = contentId('GENERAL_COVENANTS_TERMINAL_SOURCE_UNIT/V1', {
    agreement_id: agreementId,
    concept_key: conceptKey,
    m4_claim_id: m4ClaimId,
    claim_revision_id: claimRevisionId,
  });
  const signature = `ALL_OF(GENERAL_COVENANTS,${conceptKey},${deal.toUpperCase()},${m4ClaimId})`;
...
function buildTerminals(calibrationPack) {
  ...
  for (const run of calibrationPack.comparator_run_bindings) {
    ...
    const gcClaims = m4.claims.filter((claim) => claim.family === 'GENERAL_COVENANTS');
    const claimsByOccurrence = new Map(
      gcClaims.map((claim) => [claim.claim_occurrence_id, claim]),
    );
    for (const entry of resolution.resolved) {
      const conceptKey = entry.concept_key;
      const claimOccurrenceId = entry.claim.claim_occurrence_id;
      const m4Claim = claimsByOccurrence.get(claimOccurrenceId);
      if (!m4Claim) {
        throw new Error(
          `${run.deal}:${conceptKey} missing from M4 GENERAL_COVENANTS claims`,
        );
      }
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_GENERAL_COVENANTS_PHASE2_PARTITION_AUTHORITY',
      ...
      item_44_additive_identity_preserved: true,
      legal_ruling_count: 0,
      proposed_on: '2026-08-24',
      purpose:
        'Governed comparator-derived terminal registry for GENERAL_COVENANTS Phase 2 partition only',
    },
    ...
    immutable_parent_bindings: {
      ...
      family_role_policy: fileBinding(
        'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/GENERAL_COVENANTS.json',
        'family_policy_id',
      ),
      calibration_pack: {
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_GENERAL_COVENANTS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_GC_FIRST_SLICE',
    },
    policy_overlay: {
      family_key: 'GENERAL_COVENANTS',
      topic_classification_stress: 'COVENANT_CODE_TO_SUBTYPE_BUCKET',
      access_scope_work1_item_44_binding: 'DEFERRED_REVIEW_ONLY',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        public_seam: 'prepareGeneralCovenantsPhase2FamilyProposal',
      ...
      public_error_contract: {
        missing_facade:
          'GENERAL_COVENANTS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareGeneralCovenantsPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved GENERAL_COVENANTS partition',
      module_path: 'lib/canonical-v2/m7-v2-general-covenants-authoring.js',
    },
    record_identity_contract: {
      authority_id_field: 'general_covenants_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'GC first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    general_covenants_authoring_phase2_authority_id: authorityId,
  };
}
...
function main() {
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      byte_length: bytes.byteLength,
      sha256: sha256Hex(bytes),
      record_id: record.general_covenants_authoring_phase2_authority_id,
