#!/usr/bin/env node
/**
 * Regenerate the on-disk FINANCING_COVENANTS Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-financing-covenants.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'FINANCING_COVENANTS';
...
const APPROVAL_TEXT =
  'Ben approves the FINANCING_COVENANTS five-profile Work3 package inventory; subtype grouping '
  + 'review pending.';
...
function buildPhase4Review() {
  ...
  return financingCovenantsAuthoring.prepareFinancingCovenantsFamilyProfilePackageReview({
    financingCovenantsAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    financingCovenantsAuthoringPhase2Authority: phase2Authority,
