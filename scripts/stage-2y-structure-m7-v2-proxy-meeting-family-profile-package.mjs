#!/usr/bin/env node
/**
 * Regenerate the on-disk PROXY_MEETING Work3 family profile package from the
...
const PACKAGE_PATH = `${CONTROL}/m7-v2-repair-family-work3-profile-package-proxy-meeting.json`;
const WORK3_AUTHORITY_PATH = `${CONTROL}/m7-v2-repair-contract-work3-entry-correction-authority.json`;

const FAMILY_KEY = 'PROXY_MEETING';
...
const APPROVAL_TEXT =
  'Ben approves the PROXY_MEETING 31-profile Work3 package inventory; subtype grouping '
  + 'review pending.';
...
function buildPhase4Review() {
  ...
  return proxyMeetingAuthoring.prepareProxyMeetingFamilyProfilePackageReview({
    proxyMeetingAuthoringPhase4FamilyProfilePackageReviewAuthority:
      boundEnvelope(PHASE4_BINDING),
    proxyMeetingAuthoringPhase2Authority: phase2Authority,
