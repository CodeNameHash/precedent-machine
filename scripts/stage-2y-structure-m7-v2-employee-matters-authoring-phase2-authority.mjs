#!/usr/bin/env node
/**
 * Emit EMPLOYEE_MATTERS Phase 2 authoring authority v2.
 *
 * Twenty-seven governed M4 claims across six comparator deals (Concho, Metsera, Red Hat, Skechers, Skywater, TopBuild). Claim-scale partition — one terminal per governed claim. TopBuild benefit-plan accuracy rep (section 3.1(h)) stays link-only under Representations (Q02).
 */
...
const FAMILY_ROLE_POLICY_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-specific-role-policies-v2/EMPLOYEE_MATTERS.json';
const SEALED_ROLE_SCHEMA_PATH =
  'evidence/canonical-v2/stage-2y-structure-migration/control/family-role-schemas/EMPLOYEE_MATTERS.json';
...
const MODULE_PATH = 'lib/canonical-v2/m7-v2-employee-matters-authoring.js';
const TEST_PATH = 'tests/stage-2y-structure-m7-v2-repair-employee-matters-work3.test.js';
...
const SUBTYPE_BY_CLAIM_DEFINITION_KEY = {
  EMPLOYEE_COMP_ITEM_STANDARD: 'EMPLOYEE_COMPENSATION',
  EMPLOYEE_SERVICE_CREDIT: 'SERVICE_CREDIT',
  WELFARE_PLAN_TRANSITION_RELIEF: 'WELFARE_RELIEF',
};
...
const CROSS_FAMILY_LINK_ONLY_BOUNDARIES = [
  {
    ...
    deal: 'topbuild',
    disposition: 'LINK_ONLY_NOT_AN_EMPLOYEE_MATTERS_TERMINAL',
    owner_family_key: 'REPRESENTATIONS',
    ruling_id: 'M5-RULING-ONE-SEMANTIC-OWNER',
    section_reference: '3.1(h)',
    subject: 'BENEFIT_PLAN_ACCURACY_REPRESENTATION',
  },
];
...
function classificationPath(bucket) {
  return ['EMPLOYEE_MATTERS', bucket];
}
...
function discriminatorToken(claim) {
  ...
  if (typeof attributes.comp_item === 'string') return attributes.comp_item;
  if (typeof attributes.relief_kind === 'string') return attributes.relief_kind;
...
function citationToken(sourceCitation) {
  return (sourceCitation ?? claimSection(claim)).replace(/[^0-9A-Za-z]/g, '_');
}

function claimSection(claim) {
  return claim.section_reference;
}
...
function requiredExpressionSignature(claim, bucket, sourceCitation) {
  ...
  return `EMPLOYEE_MATTERS::${bucket}::${parts.join('_')}`;
}
...
function buildTerminals(calibrationPack, agreementBindings) {
  ...
  for (const agreementId of comparatorAgreementIds) {
    ...
    const claims = m4.claims.filter((claim) => claim.family === 'EMPLOYEE_MATTERS');
...
function buildAuthority() {
  ...
  const unsigned = {
    ...
    approval_basis: {
      authority_kind: 'ZERO_EFFECT_EMPLOYEE_MATTERS_PHASE2_PARTITION_AUTHORITY',
      ...
      purpose: 'Claim-scale comparator terminal registry for EMPLOYEE_MATTERS Phase 2 partition only',
    ...
    source_terminal_successor_contract: {
      schema_version: 'STAGE_2Y_EMPLOYEE_MATTERS_TERMINAL_SUCCESSOR_CONTRACT/V1',
      state: 'COMPLETE',
    ...
    synthetic_component_contract: {
      component_count: 0,
      state: 'NOT_REQUIRED_FOR_EMPLOYEE_MATTERS_FIRST_SLICE',
    },
    policy_overlay: {
      ...
      sparse_comparator_stress: 'TWENTY_SEVEN_GOVERNED_CLAIMS_ACROSS_SIX_COMPARATOR_DEALS',
      subtype_partition_stress:
        'CALIBRATION_PACK_TAGS_ALL_SIX_EXAMPLES_EMPLOYEE_COMPENSATION_WHILE_CLAIM_KEYS_POPULATE_THREE_SUBTYPE_BUCKETS',
    ...
    implementation_contract: {
      ...
      compiler_phase2_dispatch_contract: {
        ...
        public_seam: 'prepareTaxMattersPhase2FamilyProposal',
      },
      ...
      public_error_contract: {
        missing_facade: 'EMPLOYEE_MATTERS Phase2 proposal facade export is missing.',
      },
      public_seam: 'prepareTaxMattersPhase2FamilyProposal',
    ...
    execution_scope: {
      ...
      first_red_test_pattern:
        'Phase2 proposal derives a deterministic unapproved EMPLOYEE_MATTERS partition',
      module_path: MODULE_PATH,
    },
    ...
    record_identity_contract: {
      authority_id_field: 'employee_matters_authoring_phase2_authority_id',
    ...
    state_transition_only_exclusions: {
      excluded_state_keys: [],
      rationale: 'Employee Matters first slice has no temporal state machines in Phase 2 authority',
    },
  ...
  return {
    ...unsigned,
    employee_matters_authoring_phase2_authority_id: contentId(SCHEMA, unsigned),
  };
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    schema_version: SCHEMA,
    record_id_field: 'employee_matters_authoring_phase2_authority_id',
    record_id: record.employee_matters_authoring_phase2_authority_id,
    byte_length: bytes.length,
    sha256: sha256Hex(bytes),
