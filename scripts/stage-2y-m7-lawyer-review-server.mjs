...
<body>
  ...
  <main><div class="intro"><h1>Does the proposed result preserve the law?</h1><p>A comparison row is Corpus's structured statement of one legal rule. It is not the source quote. Read the source first, then check the stated parties, rule, timing and qualifications. Style comments do not fail a legally correct result.</p></div><div id="app"></div></main>
  <script>
    const familyNames = {
    ...
    function render() {
      ...
      document.getElementById('app').innerHTML = '<div class="card">' +
        ...
        '<div class="section"><p class="eyebrow">What Corpus proposes</p><div class="proposal ' + (item.item_kind === 'SOURCE_TO_ROW' ? '' : 'none') + '">' + esc(explanation(item)) + '</div>' + (item.compact_row && item.compact_row !== item.expanded_row ? '<details><summary>Show the short summary</summary><pre>' + esc(item.compact_row) + '</pre></details>' : '') + '</div>' +
