#!/usr/bin/env node
/**
 * Emit the CONSIDERATION Work3 unapproved inventory review authority and the
 * 7-profile inventory review packet draft Ben reads before disposition.
 */
...
const PHASE4_PATH = considerationAuthoring.CONSIDERATION_PHASE4_AUTHORITY_PATH;
...
const AUTHORITY_SCHEMA =
  'STAGE_2Y_M7_V2_REPAIR_CONTRACT_WORK3_CONSIDERATION_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY/V1';
const PACKET_SCHEMA =
  'STAGE_2Y_M7_V2_CONSIDERATION_7_PROFILE_INVENTORY_REVIEW_PACKET/V1';
...
const phase2 = envelope(PHASE2_PATH, 'consideration_authoring_phase2_authority_id');
const phase4 = envelope(
  PHASE4_PATH,
  'consideration_authoring_phase4_family_profile_package_review_authority_id',
);
const phase4Input = {
  considerationAuthoringPhase4FamilyProfilePackageReviewAuthority: phase4,
  considerationAuthoringPhase2Authority: phase2,
  governedSources: governedSources(phase2.record),
};
const review = considerationAuthoring
  .prepareConsiderationFamilyProfilePackageReview(phase4Input);
...
const authorityUnsigned = {
  ...
  authority_classification: 'WORK3_CONSIDERATION_UNAPPROVED_INVENTORY_REVIEW_AUTHORITY',
  authority_state: 'AUTHORISED_UNAPPROVED_INVENTORY_REVIEW_ONLY_ZERO_WORK3_OUTPUT_EFFECT',
  immutable_parent_bindings: {
    consideration_phase4_review_authority: phase4.binding,
  },
  implementation_contract: {
    exported_function: 'prepareConsiderationWork3UnapprovedInventoryReview',
  ...
  schema_review_candidate_contract: {
    schema_version: 'M7_V2_CONSIDERATION_WORK3_UNAPPROVED_INVENTORY_REVIEW_CANDIDATE/V1',
    candidate_state:
...
const packetUnsigned = {
  ...
  review_workflow: {
    ...
    open_question_for_ben:
      'The sealed M5 role schema admits all three Consideration claim keys under all seven '
      + 'subtype buckets. This packet proposes election-branch per-share cash and appraisal linkage claims under CVR_COMPONENT and the '
      + 'no-financing-condition acknowledgment under ELECTION, which diverges from '
      + 'the calibration pack tagging every provision example CONSIDERATION_PACKAGE. The divergence is '
