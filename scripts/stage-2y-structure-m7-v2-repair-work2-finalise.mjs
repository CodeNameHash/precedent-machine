...
export class Work2FinalisationError extends Error {
  constructor(code, detail) {
    super(detail ? `${code}: ${detail}` : code);
    this.name = 'Work2FinalisationError';
    this.code = code;
  }
}

function fail(code, detail) {
  throw new Work2FinalisationError(code, detail);
}
...
function repositoryPath(value) {
  if (typeof value !== 'string' || value.length === 0 || path.posix.isAbsolute(value)
    ...
    fail('WORK2_OUTPUT_SAFETY', String(value));
  }
  return value;
}
...
function rootPath(repoRoot) {
  ...
  try {
    ...
    fail('WORK2_OUTPUT_SAFETY', 'repoRoot');
  }
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) fail('WORK2_OUTPUT_SAFETY', 'repoRoot');
  return resolved;
}

function resolveExisting(root, selectedPath, code = 'WORK2_SOURCE_INPUT_DRIFT') {
...
function targetPath(root, selectedPath) {
  repositoryPath(selectedPath);
  const parent = resolveExistingDirectory(root, path.posix.dirname(selectedPath));
  return path.join(parent, path.posix.basename(selectedPath));
}

function resolveExistingDirectory(root, selectedPath) {
  let current = root;
  for (const part of repositoryPath(selectedPath).split('/')) {
    current = path.join(current, part);
    let stat;
    try {
      stat = lstatSync(current);
    } catch {
      fail('WORK2_OUTPUT_SAFETY', selectedPath);
    }
...
function readBytes(root, selectedPath, code = 'WORK2_SOURCE_INPUT_DRIFT') {
  return readFileSync(resolveExisting(root, selectedPath, code));
}

function readCanonical(root, selectedPath, code = 'WORK2_SOURCE_INPUT_DRIFT') {
...
function recoveryPrecondition(recovery) {
  ...
  if (!exactKeys(value, RECEIPT_RECOVERY_KEYS)) {
    fail('WORK2_RECOVERY_BINDING_DRIFT', 'receipt recovery');
  }
  return value;
}

function recordBinding(
  selectedPath, bytes, record, idField, code = 'WORK2_SOURCE_INPUT_DRIFT',
) {
...
function validateStandardRecordBinding(root, claimed, expectedPath, code) {
...
function validateParentAuthorityBinding(root, claimed) {
...
function selectedNativeBindings(receipt, schemaVersion, excludedSchema, excludedPath) {
  if (receipt.schema_version !== 'STAGE_2Y_STRUCTURE_MIGRATION_PACKET_RECEIPT/V1'
    ...
    fail('WORK2_SOURCE_INPUT_DRIFT', `${receipt.packet_id} receipt state`);
  }
  ...
  if (selected.length !== 7 || excluded.length !== 1
    ...
    fail('WORK2_SOURCE_INPUT_DRIFT', `${receipt.packet_id} output selection`);
  }
  return selected;
}

function loadSourceSets(root) {
  const m3 = readCanonical(root, M3_RECEIPT_PATH);
  const m4 = readCanonical(root, M4_RECEIPT_PATH);
  if (m3.record.packet_id !== 'stage-2y-structure-m3-context-compilation'
    ...
    fail('WORK2_SOURCE_INPUT_DRIFT', 'native receipt packet IDs');
  }
  ...
  for (const receiptBinding of m3Bindings) {
    const source = readCanonical(root, receiptBinding.path);
    ...
    if (source.record.schema_version !== receiptBinding.schema_version
      ...
      fail('WORK2_SOURCE_INPUT_DRIFT', `${receiptBinding.path} binding`);
    }
    contexts.set(receiptBinding.agreement_id, {
      receiptBinding,
      entry: { record: source.record, binding: selectedBinding },
    });
  }
  const baseAnalyses = [];
  ...
  for (const receiptBinding of m4Bindings) {
    ...
    if (!context || source.record.schema_version !== receiptBinding.schema_version
      ...
      fail('WORK2_SOURCE_INPUT_DRIFT', `${receiptBinding.path} continuity`);
    }
    baseAnalyses.push({ record: source.record, binding: selectedBinding });
    contextCompilations.push(context.entry);
  }
  let sourceSets;
  try {
    sourceSets = buildSourceSets({ baseAnalyses, contextCompilations });
  } catch (error) {
    fail('WORK2_SOURCE_INPUT_DRIFT', error.message);
  }
  return { m3, m4, sourceSets };
}
...
function validateWork0SourceContinuity(root, work1, m3, m4) {
  ...
  const work0 = readCanonical(root, WORK0_PATH, 'WORK2_SOURCE_INPUT_DRIFT');
  const expectedWork0Binding = recordBinding(
    WORK0_PATH, work0.bytes, work0.record, 'evidence_root_id',
    'WORK2_SOURCE_INPUT_DRIFT',
  );
  if (!same(work1.record.work0_evidence_root_binding, expectedWork0Binding)
    ...
    fail('WORK2_SOURCE_INPUT_DRIFT', 'Work0 evidence-root continuity');
  }
  const expected = [
    {
      ordinal: 11,
  ...
  for (const spec of expected) {
    ...
    if (matches.length !== 1 || entry.role !== spec.role || entry.path !== spec.path
      ...
      fail('WORK2_SOURCE_INPUT_DRIFT', `${spec.role} Work0 binding`);
    }
  }
}

function validateBuildOnlyState(root, manifest, orderingAuthority) {
  ...
  if (manifest.work !== 'WORK2' || manifest.state !== 'PRE_WORK_BOOTSTRAP_ONLY'
    ...
    fail('WORK2_MANIFEST_BINDING_DRIFT', 'build-only manifest or authority');
  }
...
function commandLedger(manifest, executionFixture, recoveryAuthority) {
...
function buildOutputs(root) {
  ...
  validateBuildOnlyState(root, manifest, orderingAuthority);
  if (!same(
    ...
    orderingAuthority.work2_entry_correction_authority_binding,
    recordBinding(ENTRY_AUTHORITY_PATH, entryInput.bytes, entryInput.record,
      'correction_authority_id', 'WORK2_AUTHORITY_BINDING_DRIFT'),
  )) {
  ...
  validateParentAuthorityBinding(root, manifest.parent_authority_binding);
  validateStandardRecordBinding(
    root, manifest.activation_receipt_binding,
    ACTIVATION_RECEIPT_PATH, 'WORK2_AUTHORITY_BINDING_DRIFT',
  );
  const work1Input = validateStandardRecordBinding(
    root, manifest.predecessor_receipt_binding,
    WORK1_RECEIPT_PATH, 'WORK2_PREDECESSOR_BINDING_DRIFT',
  );
  ...
  const { m3, m4, sourceSets } = loadSourceSets(root);
  validateWork0SourceContinuity(root, work1Input, m3, m4);
  const agreementSet = sourceSets.agreementAnalysisSet;
  const contextSet = sourceSets.contextCompilationSet;
  const agreementBytes = canonicalBytes(agreementSet);
  const contextBytes = canonicalBytes(contextSet);
  const inMemory = new Map([
    [AGREEMENT_SET_PATH, { bytes: agreementBytes, record: agreementSet }],
    [CONTEXT_SET_PATH, { bytes: contextBytes, record: contextSet }],
  ]);
  ...
  for (const selectedPath of effectivePaths.filter((entry) => entry !== RECEIPT_PATH)) {
    ...
    if (memory) {
      artifactBindings.push(artifactIdentity(selectedPath, memory.bytes, memory.record));
      continue;
    }
    const selected = readBytes(root, selectedPath, 'WORK2_ARTIFACT_BINDING_DRIFT');
    let record = null;
    if ([ORDERING_AUTHORITY_PATH, ENTRY_AUTHORITY_PATH, RECOVERY_AUTHORITY_PATH,
      MANIFEST_PATH].includes(selectedPath)) {
  ...
  const sourceSetEvidence = {
    m3_context_compilation_receipt_binding: binding(M3_RECEIPT_PATH, m3.bytes),
    m4_agreement_analysis_receipt_binding: binding(M4_RECEIPT_PATH, m4.bytes),
  ...
  const compilerEvidence = {
    ...
    source_set_ids: {
      agreement_analysis_set_id: agreementSet.agreement_analysis_set_id,
      context_compilation_set_id: contextSet.context_compilation_set_id,
    },
    proof_states: {
  ...
  const ledger = commandLedger(manifest, executionFixture, recovery);
  const exactGitArgv = effectiveCommitArgv(manifest, effectivePaths, recovery);
  const repositoryPrecondition = {
    ...
    exact_git_commit_and_push_argv: structuredClone(exactGitArgv),
    required_validator_argv: structuredClone(manifest.exact_argv_with_run_limits[13].argv),
  };
  const recoveryMember = recoveryPrecondition(recovery);
  if (recoveryMember) repositoryPrecondition.recovery = recoveryMember;
  const receipt = identified(RECEIPT_SCHEMA, 'work2_receipt_id', {
    work: 'WORK2',
    stage: 'M7_V2_REPAIR_WORK2',
    state: 'PASS_WORK2_BUILD_ONLY_NULL_CANDIDATE',
    status: 'PASS',
    execution_manifest_id: manifest.execution_manifest_id,
    execution_manifest_digest: manifest.execution_manifest_digest,
    parent_authority_binding: structuredClone(manifest.parent_authority_binding),
    ...
    work2_entry_correction_authority_binding:
      structuredClone(orderingAuthority.work2_entry_correction_authority_binding),
...
function preflightOutputs(root) {
  const states = OUTPUT_PATHS.map((selectedPath) => {
    ...
    if (!existsSync(absolute)) return { selectedPath, absolute, exists: false };
    const stat = lstatSync(absolute);
    if (stat.isSymbolicLink() || !stat.isFile()) fail('WORK2_OUTPUT_SAFETY', selectedPath);
    return { selectedPath, absolute, exists: true };
  });
  if (states[2].exists) fail('WORK2_ALREADY_FINALISED', RECEIPT_PATH);
  if (states.some((entry) => entry.exists)) fail('WORK2_OUTPUT_STATE_DRIFT', 'partial outputs');
  return states;
}
...
function writeAll(fd, bytes) {
  ...
  while (offset < bytes.length) {
    ...
    if (written <= 0) fail('WORK2_WRITE_FAILED', 'short write');
    offset += written;
  }
}

function fsyncParent(absolute) {
  const fd = openSync(path.dirname(absolute), fsConstants.O_RDONLY);
  try {
    fsyncSync(fd);
  } finally {
    closeSync(fd);
...
function writeOutputs(root, outputs) {
  ...
  const bytesByPath = new Map([
    [AGREEMENT_SET_PATH, outputs.agreementBytes],
    [CONTEXT_SET_PATH, outputs.contextBytes],
    [RECEIPT_PATH, outputs.receiptBytes],
  ]);
  const created = [];
  try {
    for (const target of targets) {
      ...
      try {
        writeAll(fd, bytesByPath.get(target.selectedPath));
        fsyncSync(fd);
      } finally {
        closeSync(fd);
      }
    ...
    if (rollbackError) fail('WORK2_ROLLBACK_FAILED', rollbackError.message);
    if (error instanceof Work2FinalisationError) throw error;
    fail('WORK2_WRITE_FAILED', error.message);
  }
}

function resultFor(outputs, write) {
  return {
    status: write ? 'PASS_WORK2_FINALISATION' : 'PASS_WORK2_FINALISATION_PREVIEW',
    work2_receipt_id: outputs.receipt.work2_receipt_id,
    agreement_analysis_set_id: outputs.agreementSet.agreement_analysis_set_id,
    context_compilation_set_id: outputs.contextSet.context_compilation_set_id,
    target_paths: [...OUTPUT_PATHS],
    effects: {
      files_written: write ? 3 : 0,
      source_set_writes: write ? 2 : 0,
...
export function finaliseWork2(options = {}) {
  if (options === null || typeof options !== 'object' || Array.isArray(options)) {
    fail('WORK2_RECEIPT_INVALID', 'options');
  }
  const keys = Object.keys(options);
  if (keys.some((key) => !['repoRoot', 'write'].includes(key))) {
    fail('WORK2_RECEIPT_INVALID', 'options');
  }
  const root = rootPath(options.repoRoot ?? REPO_ROOT);
  const write = options.write ?? true;
  if (typeof write !== 'boolean') fail('WORK2_RECEIPT_INVALID', 'write option');
  preflightOutputs(root);
  const outputs = buildOutputs(root);
  if (write) writeOutputs(root, outputs);
  return resultFor(outputs, write);
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    if (process.argv.length !== 2) fail('WORK2_RECEIPT_INVALID', 'CLI arguments');
    process.stdout.write(`${JSON.stringify(finaliseWork2())}\n`);
  } catch (error) {
