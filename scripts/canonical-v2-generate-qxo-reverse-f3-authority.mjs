...
const OUTPUT_DIR = join(ROOT, 'sql', 'qxo-reverse-f3');

function sqlJson(value) {
  const json = JSON.stringify(value);
  const tag = `$qxo_f3_${createHash('sha256').update(json).digest('hex').slice(0, 16)}$`;
...
function verificationSql(genesis) {
  ...
  return `DO $qxo_f3_authority_gate$
BEGIN
...
const genesis = buildQxoReverseF3AuthorityGenesis();
const attestation = Object.freeze({
  schema_version: 'QXO_REVERSE_F3_AUTHORITY_GENESIS_ATTESTATION/V1',
...
writeFileSync(join(OUTPUT_DIR, '01-f3-authority-genesis-dry-run.sql'), transaction(genesis, false));
writeFileSync(join(OUTPUT_DIR, '02-f3-authority-genesis-apply.sql'), transaction(genesis, true));
writeFileSync(
