...
test('F28 third-run recorded fixture: replaying through the adapter WITH a resolution context leaves the CHAPEAU-attached resolved claim byte-identical to replaying WITHOUT one', () => {
  ...
  // Matches this run doc's own headline at the time this test was written:
  // the CHAPEAU claim publishes alone without this design's component rows,
  // because its ITEM-attached siblings' subjects resolve against nothing.
  ...
  // Re-verified on the phantom-citation re-derivation (docs/codex-program/
  // notes/phantom-citation-rederivation.md): this fixture was regenerated by
  // replaying the SAME recorded model response through the corrected
  // sectionizer, resolver and validator, as that note describes. The counts
  // below moved, but NOT because of the citation fix -- a controlled
  // before/after-sectionizer comparison (documented in that note) proves
  // this exact shape already holds under today's resolver with the OLD,
  // pre-fix sectionizer too. The real cause is independent resolver
  // evolution since this test was last calibrated (mapping_table_version
  // rose from 3 to 20 across many unrelated slices): the resolver's pick
  // among several near-duplicate "as of April 17, 2026"-style qualifiers
  // for the measurement-date claim has changed, which changes which limb
  // paths the two ITEM-attached claims attach to. Both now resolve to
  // ASSERTION-node subjects -- there is no longer a PATH-node-subject claim
  // in this recording to exercise "path nodes never mint" against. That
  // invariant is a property of the write-set adapter itself, not of this
  // one fixture, and stays covered by tests/canonical-v2-native-write-set-
  // adapter.test.js's synthetic "a PATH-node claim subject is never minted
  // a component" test; this real-fixture block only ever asserted what THIS
  // recording's own claims happen to do.
  ...
  assert.equal(withResolution.write_set.components.length, 2, 'both ITEM-attached claims now resolve to ASSERTION-node subjects and mint');
  assert.equal(componentValidation.publishableWriteSet.claims.length, 3, 'the CHAPEAU claim plus both now-resolvable ASSERTION-node claims');
  ...
  assert.deepEqual(componentValidation.residuals, [], 'nothing left typed-unresolved -- both ITEM-attached claims mint and publish');
});
