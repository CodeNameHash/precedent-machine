...
//
// The two F28 closure_id literals below were re-pinned on the phantom-
...
// section reference, so both open_world entries' closure_id moved from
// III-INTRO(b)-derived to 3.1(b)-derived even though raw_value (the literal
// quoted text loadOpenWorldQuote reads) did not change by one byte. Re-pinned
// by matching each old closure_id's raw_value to its new closure_id in the
// regenerated fixture, never by re-typing the quote.
// ---------------------------------------------------------------------------
...
test('F28 C1: AUTHORIZED and ISSUED_OUTSTANDING sub-quotes resolve end to end with exact canonical values, correct definitions, materiality rank 52', async () => {
  ...
  const parentQuote = loadOpenWorldQuote(
    'f28-third-live-run', 'd877983e6dd53c5b0b59df0fbc62d2050c9198ad57030f479089a66c017dcb0c',
  );
...
test('F28: the preferred-stock parent quote (idx2), proposed as ISSUED_OUTSTANDING, ABSTAINs AMBIGUOUS_LITERAL_AND_ZERO and routes to REVIEW with the typed reason (Fable review F-1)', async () => {
  const parentQuote = loadOpenWorldQuote(
    'f28-third-live-run', '52f806073cc547325a3f11cd8c0ad6ab8c6dd2e11f9be8b8609512f087becd09',
  );
...
test('MAPPING_TABLE_VERSION is 20; GENERIC_CLAIM_KEY_RESOLUTION_TABLE carries exactly ONE SHARE_COUNT entry and no duplicate generic keys anywhere (audit M-2)', () => {
