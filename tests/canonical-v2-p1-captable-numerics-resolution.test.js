...
test('MAPPING_TABLE_VERSION is 6 (bumped 5 -> 6 by the family-no-shop slice); GENERIC_CLAIM_KEY_RESOLUTION_TABLE carries exactly ONE SHARE_COUNT entry and no duplicate generic keys anywhere (audit M-2)', () => {
...
test('the resolution receipt carries share_count_parse_version and zero_pattern_table_version', async () => {
  ...
  assert.equal(resolution.resolution_receipt.mapping_table_version, 6);
});
