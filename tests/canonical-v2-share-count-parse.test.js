...
//
// The five F28 closure_id literals below were re-pinned on the phantom-
// citation re-derivation (docs/codex-program/notes/
...
// recorded model response through the sectionizer fix in 63a1fe3a (Sections
// 3.1/3.2 no longer swallowed into a phantom "III-INTRO(b)" chapeau child).
// closure_id is content-addressed over the governed scope, which embeds the
// section reference, so every open_world entry's closure_id moved from
// III-INTRO(b)-derived to 3.1(b)-derived even though its raw_value (the
// literal quoted text this file reads) did not change by one byte -- see
// that note for the full field-by-field accounting. Re-pinned by matching
// each old closure_id's raw_value to its new closure_id in the regenerated
// fixture, never by re-typing the quote.
// ---------------------------------------------------------------------------
...
test('F28 C1: AUTHORIZED sub-quote resolves 250000000 (Company Shares, real fixture bytes)', () => {
  ...
  const parentQuote = loadOpenWorldQuote(
    F28_DIR, 'd877983e6dd53c5b0b59df0fbc62d2050c9198ad57030f479089a66c017dcb0c',
  );
...
test('F28 C1: RESERVED sub-quote resolves 1600514, excluding "2015" inside the plan_ref span', () => {
  ...
  const parentQuote = loadOpenWorldQuote(
    F28_DIR, '328f9744839edbeab9feec1b0e953e03f3008022fb468a8e5afd52772c2bcd7e',
  );
  assertAnchoredOverlap(text, subQuote, parentQuote);
  const result = parseShareCount({
...
test('F28: the preferred-stock parent quote (idx2) is the flagship AMBIGUOUS_LITERAL_AND_ZERO case when count_kind is ISSUED_OUTSTANDING', () => {
  const text = loadCanonicalText(F28_DIR);
  const parentQuote = loadOpenWorldQuote(
    F28_DIR, '52f806073cc547325a3f11cd8c0ad6ab8c6dd2e11f9be8b8609512f087becd09',
  );
...
test('exclusion: LTR-mark-tolerant section reference, on the LITERAL F28 fixture bytes ("Section ‎3.1(b)(ii)"), abstains NO_NUMERIC_LITERAL rather than misreading the section number as a count', () => {
  ...
  const fullQuote = loadOpenWorldQuote(
    F28_DIR, '9b1157b410268f70947841ce569eb96255840db020ceb9dc16ecba994e1b7019',
  );
...
test('FIXED (Fable review finding F-3): a "Sections X and Y" continuation excludes BOTH references, real F28 bytes, real closure_id -- no longer a live wrong-number path', () => {
  const fullQuote = loadOpenWorldQuote(
    F28_DIR, '694f86db4152c320b43564d80761e3e979602f7dcf1fc853875e5092e246c92b',
  );
