...
let materialContractsAuthoring;
try {
  materialContractsAuthoring = require('../lib/canonical-v2/m7-v2-material-contracts-authoring.js');
} catch (error) {
  throw new Error('MATERIAL_CONTRACTS Phase2 proposal facade export is missing.');
}
...
for (const facade of [
  ...
  if (typeof materialContractsAuthoring[facade] !== 'function') {
    throw new Error(`MATERIAL_CONTRACTS ${facade} facade export is missing.`);
  }
}

const PROFILE_COUNT = materialContractsAuthoring.MATERIAL_CONTRACTS_PROFILE_COUNT;
const FLAGS = materialContractsAuthoring.MATERIAL_CONTRACTS_REVIEW_FLAGS;
...
const PHASE2_AUTHORITY_BINDING = Object.freeze({
  path: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_AUTHORITY_PATH,
  schema_version: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_AUTHORITY_SCHEMA,
  record_id_field: 'material_contracts_authoring_phase2_authority_id',
  record_id: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_AUTHORITY_ID,
  byte_length: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_AUTHORITY_BYTES,
  sha256: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_AUTHORITY_SHA256,
});

const PHASE4_AUTHORITY_BINDING = Object.freeze({
  path: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_PATH,
  schema_version: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_SCHEMA,
  record_id_field:
    'material_contracts_authoring_phase4_family_profile_package_review_authority_id',
  record_id: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_ID,
  byte_length: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_BYTES,
  sha256: materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_AUTHORITY_SHA256,
});

const WORK3_BINDINGS = Object.freeze({
  inventoryAuthority: {
    byte_length: 2155,
    path: `${CONTROL}/m7-v2-repair-contract-work3-material-contracts-unapproved-inventory-review-authority.json`,
  ...
  packet: {
    byte_length: 153430,
    path: `${CONTROL}/m7-v2-repair-material-contracts-116-profile-inventory-review-packet-draft.json`,
  ...
  disposition: {
    byte_length: 40310,
    path: `${CONTROL}/m7-v2-repair-material-contracts-116-profile-inventory-disposition.json`,
  ...
  session: {
    byte_length: 1141,
    path: `${CONTROL}/m7-v2-repair-material-contracts-ben-inventory-session-receipt.json`,
  ...
  benAuthority: {
    byte_length: 2856,
    path: `${CONTROL}/m7-v2-repair-contract-work3-material-contracts-ben-inventory-session-successor-authority.json`,
  ...
  sealAuthority: {
    byte_length: 3381,
    path: `${CONTROL}/m7-v2-repair-contract-work3-material-contracts-family-package-seal-successor-authority.json`,
  ...
  sealReceipt: {
    byte_length: 2308,
    path: `${CONTROL}/m7-v2-repair-material-contracts-family-package-seal-receipt.json`,
  ...
  registrationAuthority: {
    byte_length: 2913,
    path: `${CONTROL}/m7-v2-repair-contract-work3-material-contracts-registration-successor-authority.json`,
...
const FAMILY_PROFILE_PACKAGE_BINDING = Object.freeze({
  path: `${CONTROL}/m7-v2-repair-family-work3-profile-package-material-contracts.json`,
  schema_version: 'STAGE_2Y_M7_V2_FAMILY_PROFILE_PACKAGE/V2',
  record_id_field: 'family_profile_package_id',
  record_id: '3db2136e77cfd01b48a0d053d969d3557f3909032fac6af33a75571fca4fc731',
  byte_length: 1497422,
  sha256: '24945092f4dfa84900fe9aadca19c51f3a774ed7d5c2ac6fc3916113d8c762ba',
});
...
function phase2Fixture() {
  ...
  const materialContractsAuthoringPhase2Authority = sourceEnvelope(PHASE2_AUTHORITY_BINDING);
  return {
    materialContractsAuthoringPhase2Authority,
    governedSources: phase2GovernedSources(
      materialContractsAuthoringPhase2Authority.record,
    ),
  };
}
...
function phase4Fixture() {
  ...
  return {
    ...
    materialContractsAuthoringPhase2Authority:
      fixture.materialContractsAuthoringPhase2Authority,
...
test('Phase2 proposal derives a deterministic unapproved MATERIAL_CONTRACTS partition', () => {
  const fixture = phase2Fixture();
  const authority = fixture.materialContractsAuthoringPhase2Authority.record;
  ...
  const result = materialContractsAuthoring
    .prepareMaterialContractsPhase2FamilyProposal(fixture);

  assertExactKeys(
    result,
    materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE2_PROPOSAL_KEYS,
    'proposal keys',
  );
  assert.deepEqual({
...
test('Material Contracts Milestone A terminals do not collide with sealed Representations package identities', () => {
  const materialPhase2 = readRecord(PHASE2_AUTHORITY_BINDING.path);
  const representationsPackage = readRecord(
    `${CONTROL}/m7-v2-repair-family-work3-profile-package-representations.json`,
  );
  const representationsPhase2 = readRecord(
    `${CONTROL}/m7-v2-repair-contract-representations-authoring-phase2-authority-v2.json`,
  );

  const materialClaimIds = new Set(
    materialPhase2.source_terminal_successor_contract.terminal_rule_registry
      .flatMap((terminal) => terminal.m4_claim_ids),
  );
  const representationsClaimIds = new Set(
    representationsPhase2.source_terminal_successor_contract.terminal_rule_registry
      .flatMap((terminal) => terminal.m4_claim_ids),
  );
  const materialSignatures = new Set(
    materialPhase2.source_terminal_successor_contract.terminal_rule_registry
      .map((terminal) => terminal.required_expression_signature),
  );
  const representationsSignatures = new Set(
    representationsPackage.profiles.map((profile) => profile.required_expression_signature),
  );

  for (const claimId of materialClaimIds) {
    assert.equal(representationsClaimIds.has(claimId), false, `M4 claim collision: ${claimId}`);
  }
  for (const signature of materialSignatures) {
    assert.equal(
      representationsSignatures.has(signature),
      false,
      `Expression signature collision: ${signature}`,
    );
  }
});

test('Phase4 MATERIAL_CONTRACTS family profile package review returns unapproved proposals without Work3 identities', () => {
  ...
  const phase2Proposal = materialContractsAuthoring
    .prepareMaterialContractsPhase2FamilyProposal({
      materialContractsAuthoringPhase2Authority:
        fixture.materialContractsAuthoringPhase2Authority,
      governedSources: fixture.governedSources,
  ...
  const result = materialContractsAuthoring
    .prepareMaterialContractsFamilyProfilePackageReview(fixture);

  assertExactKeys(
    result,
    materialContractsAuthoring.MATERIAL_CONTRACTS_PHASE4_REVIEW_OUTPUT_KEYS,
  ...
  for (const profile of result.proposed_profiles) {
    ...
    assert.equal(profile.package_profile_key.startsWith('PROFILE:MATERIAL_CONTRACTS:'), true);
    assert.equal(Object.hasOwn(profile, 'work3_profile_id'), false);
  }
});

test('Work3 MATERIAL_CONTRACTS unapproved inventory review proves validator acceptance without Work3 identity or package approval', () => {
  ...
  const result = materialContractsAuthoring
    .prepareMaterialContractsWork3UnapprovedInventoryReview({
      ...
      materialContractsPhase4ReviewInput: phase4Fixture(),
    });

  assert.equal(
    result.candidate_state,
    'UNAPPROVED_116_PROFILE_INVENTORY_REVIEW_PACKET_ONLY_BEN_APPROVAL_NOT_RECORDED',
  );
...
test('Work3 MATERIAL_CONTRACTS Ben inventory session disposition captures Ben-authored file without Work3 identity or package seal', () => {
  ...
  assert.deepEqual(result.disposition_binding.session_summary, {
    ...
    populated_subtype_bucket_count:
      materialContractsAuthoring.MATERIAL_CONTRACTS_POPULATED_SUBTYPE_BUCKET_COUNT,
    registered_subtype_bucket_count:
      materialContractsAuthoring.MATERIAL_CONTRACTS_REGISTERED_SUBTYPE_BUCKET_COUNT,
...
test('Work3 MATERIAL_CONTRACTS family package seal captures Ben seal without Work3 identity or premature registration', () => {
  physicalBytes(WORK3_BINDINGS.session);
  physicalBytes(WORK3_BINDINGS.sealAuthority);
  const evidence = work3Evidence();
  const result = materialContractsAuthoring.prepareMaterialContractsWork3FamilyPackageSeal({
...
test('Work3 MATERIAL_CONTRACTS family package registration binds seal receipt without activation', () => {
  physicalBytes(WORK3_BINDINGS.sealReceipt);
  physicalBytes(WORK3_BINDINGS.registrationAuthority);
  const evidence = work3Evidence();
  const result = materialContractsAuthoring
    .prepareMaterialContractsWork3FamilyPackageRegistration({
      materialContractsWork3FamilyPackageRegistrationEvidence: evidence,
      materialContractsPhase4ReviewInput: phase4Fixture(),
    });

  assert.equal(
    result.candidate_state,
    'BEN_MATERIAL_CONTRACTS_FAMILY_PACKAGE_REGISTERED_ZERO_PRODUCT_WRITE_EFFECT',
  );
...
test('Material Contracts Milestone A inventory packet draft carries shape summaries and honest holds', () => {
  physicalBytes(WORK3_BINDINGS.packet);
...
test('Material Contracts Milestone A disposition approves 116 profiles and reuses only sealed M5 rulings', () => {
  physicalBytes(WORK3_BINDINGS.disposition);
...
test('Material Contracts Milestone A family profile package on disk validates 116 registered profiles', () => {
  ...
  assert.equal(
    ...
    'BEN_APPROVAL:MATERIAL_CONTRACTS:PROFILE_SET_V1',
  );
  ...
  const phase4 = materialContractsAuthoring
    .prepareMaterialContractsFamilyProfilePackageReview(phase4Fixture());
  assert.deepEqual(
...
test('lawful Work3 fixture MATERIAL_CONTRACTS on-disk override tracks the sealed package bytes', () => {
  ...
  const override = fixture.on_disk_family_package_overrides.find(
    (entry) => entry.family_key === 'MATERIAL_CONTRACTS',
  );
  assert.ok(override, 'lawful Work3 fixture has no MATERIAL_CONTRACTS on-disk override');
