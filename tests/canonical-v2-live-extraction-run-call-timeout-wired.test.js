...
test('the live Codex client receives config.timeoutMs, with an explicit safe default (source pin)', () => {
  const source = fs.readFileSync(RUNNER, 'utf8');
  assert.match(
    source,
    /timeoutMs:\s*config\.timeoutMs\s*\|\|\s*DEFAULT_CODEX_CALL_TIMEOUT_MS/,
    'createCodexCliClient must receive config.timeoutMs when supplied, otherwise use the runner\'s explicit default',
  );
});
