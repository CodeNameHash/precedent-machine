...
test('FIXTURE PIN: the no-v1v2-input resolveCandidates() path reproduces the committed f28-third-live-run/resolution.json resolution_receipt_id byte-for-byte', () => {
  ...
  assert.equal(
    ...
    // UNCHANGED at this call site (this test resolves against
    // CONTRACT_BUNDLE_V13, a fixed pre-slice bundle snapshot the test
    // deliberately keeps stale; only MAPPING_TABLE_VERSION and the new
    // receipt field, which are resolver-module constants independent of
    // which contract_vocabulary is passed in, move). [family-no-shop-era
