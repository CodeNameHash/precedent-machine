...
test('FIXTURE PIN: the no-v1v2-input resolveCandidates() path reproduces the committed f28-third-live-run/resolution.json resolution_receipt_id byte-for-byte', () => {
  ...
  // The fixture was regenerated to be CURRENT, not stale: a fresh baseline
  // computed here now reproduces the committed fixture's resolution_receipt_id
  // byte-for-byte, so this assertion flips from notEqual to equal.
  assert.equal(
  ...
  assert.equal(resolutionFixture.resolution_receipt.resolution_receipt_id, '81a307dad8257628eeaa2773083ef7f8d9df41f29fe8da3f33e0b746b8756cce');
