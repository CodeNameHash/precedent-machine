...
test('Closing Conditions follow-on source pack grounds certificate references and retains the dissent evidence gap', () => {
  ...
  assert.equal(pack.dispositions.DISSENT_THRESHOLD, OPEN_NO_GROUNDED_PRODUCTION_EVIDENCE);
  assert.equal(recommendedDissentThresholdDisposition(pack), APPROVED_RETIRED_PENDING_BEN_APPROVAL);
