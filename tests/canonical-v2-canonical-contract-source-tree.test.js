...
test('the single successor source compiles twice byte-identically without claiming completeness', () => {
  ...
  assert.equal(first.authored_members.length, 118);
  assert.equal(
...
test('the manifest exactly closes the 118-file Agreement, shared and Process source tree', () => {
  ...
  assert.equal(manifest.members.length, 118);
  assert.equal(
