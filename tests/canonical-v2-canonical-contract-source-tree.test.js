...
test('the single successor source compiles twice byte-identically as a complete authored universe without freeze authority', () => {
  ...
  assert.equal(first.authored_members.length, 172);
  assert.equal(
...
test('the manifest exactly closes the 172-file Agreement, shared, Process, Product and governance source tree', () => {
  ...
  assert.equal(manifest.members.length, 172);
  assert.equal(
