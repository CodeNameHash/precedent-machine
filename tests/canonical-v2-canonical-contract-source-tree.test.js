...
test('the single successor source compiles twice byte-identically as a complete authored universe without freeze authority', () => {
  ...
  assert.equal(first.authored_members.length, 179);
  assert.equal(
...
test('the manifest exactly closes the 179-file Agreement, shared, Process, Product and governance source tree', () => {
  ...
  assert.equal(manifest.members.length, 179);
  assert.equal(
