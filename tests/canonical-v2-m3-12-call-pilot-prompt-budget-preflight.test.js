...
function diagnosticSource(workItem) {
  return loadSourceForDiagnostic({ source: sourceFor(workItem), root_dir: ROOT });
}

test('the pilot aggregate preflight remains blocked without trusted source admission', () => {
  assert.throws(
    () => buildPilotPromptBudgetPreflight({ root_dir: ROOT }),
    /Caller source declarations cannot create an admitted source for execution/,
  );
});

test('a byte-pinned diagnostic TopBuild source supports the offline prompt calculation', () => {
  const item = extract('topbuild-ioc-company-4-1');
  const call = buildPromptBudgetCall({
    source: sourceFor(item), work_item: item, root_dir: ROOT, admitted_source: diagnosticSource(item),
  });
...
test('a section pin or family mismatch fails before any provider path exists', () => {
  ...
  assert.throws(
    () => buildPromptBudgetCall({ source: sourceFor(sectionMismatch), work_item: sectionMismatch, root_dir: ROOT, admitted_source: diagnosticSource(sectionMismatch) }),
  ...
  assert.throws(
    () => buildPromptBudgetCall({ source: sourceFor(familyMismatch), work_item: familyMismatch, root_dir: ROOT, admitted_source: diagnosticSource(familyMismatch) }),
  ...
  assert.throws(
    () => buildPromptBudgetCall({ source: sourceFor(broadTopBuildScope), work_item: broadTopBuildScope, root_dir: ROOT, admitted_source: diagnosticSource(broadTopBuildScope) }),
    (error) => error instanceof PilotPromptBudgetPreflightError && error.code === 'SECTION_PIN_MISMATCH',
  );
});
...
test('the explicit prompt-byte ceiling fails closed', () => {
  ...
  assert.throws(
    () => buildPromptBudgetCall({ source: sourceFor(item), work_item: item, root_dir: ROOT, prompt_byte_ceiling: 1, admitted_source: diagnosticSource(item) }),
