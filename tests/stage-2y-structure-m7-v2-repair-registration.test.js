...
function makeFixture(t) {
  ...
  for (const repositoryPath of [AUTHORITY_PATH, ACTIVATION_PATH, WORK0_PATH]) {
    write(root, repositoryPath, fs.readFileSync(path.join(ROOT, repositoryPath)));
  }
  ...
  write(
    root,
    code.independent_verifier,
    fs.readFileSync(path.join(ROOT, code.independent_verifier)),
  );
  ...
  const predecessor_receipts = ['WORK1', 'WORK2', 'WORK3'].map((work) => {
    const number = Number(work.slice(4));
    const repositoryPath = number === 1
      ? WORK1_RECEIPT_PATH
      : `evidence/canonical-v2/stage-2y-structure-migration/receipts/stage-2y-structure-m7-v2-repair-${work.toLowerCase()}-fixture.json`;
    const schemaVersion = number === 1
      ? 'STAGE_2Y_M7_V2_REPAIR_WORK1_CONTRACT_RECEIPT/V1'
      : `STAGE_2Y_M7_V2_REPAIR_${work}_RECEIPT/V1`;
    const idField = number === 1 ? 'work1_contract_receipt_id' : `work${number}_receipt_id`;
    const descriptor = writeRecord(root, repositoryPath, schemaVersion, idField, {
      stage: `M7_V2_REPAIR_${work}`,
      state: number === 1 ? 'PASS_WORK1_CONTRACTS' : `PASS_${work}`,
      status: 'PASS',
      work,
    });
    if (number > 1) {
      writeDigestRecord(
        root,
        `evidence/canonical-v2/stage-2y-structure-migration/control/m7-v2-repair-${work.toLowerCase()}-execution-manifest.json`,
        ...
        'execution_manifest_id',
        {
          ...
          work_receipt_path: repositoryPath,
        },
      );
    }
...
test('M7 V2 candidate registration is immutable, content-addressed and independently verified', async (t) => {
  ...
  await t.test('path escape and symlink routes fail closed', () => {
    ...
    const target = path.join(fixture.root, fixture.specification.code.compiler);
