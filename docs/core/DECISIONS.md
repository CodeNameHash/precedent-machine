...
- **Whether `IMMUTABLE_SOURCE_DOCUMENT/V2` should be keyed on the
  uncompressed source map instead of on compressor output.**
  ...
  **The fix on the table.** Key the identity on `source_map_digest`, which is
  a `contentId` over the uncompressed structure, already computed, already
  stable, and already in the same lineage block. The compressed digest stays
  as a payload integrity check, where it belongs, rather than as identity.

  **Why it is not just done.** It changes a closed V2 schema, which means
  every committed id derived from it moves. Blast radius, fixture
  regeneration and whether a smaller fix gets the same property are under
  adversarial review; recorded here so the question is not rediscovered.

  **The interim rule.** A run must be imported by the environment that
  produced it, or regenerated by replay first. Replay costs no model calls.
  Do not re-derive `source_references` from a rebuild to make a run agree
  with itself — that is editing the evidence until it passes.
