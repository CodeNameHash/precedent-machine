...
- `lib/product/agreement-draft.js` `longSectionGroups` / `mergePartResponses`: a section over 12k bytes with four or more child limbs is extracted per group of limbs and the parts merged when the caller passes `longSectionSplit` (the analysis runner does; Q6, 2026-09-14). `tests/product-long-section-split.test.js`.
