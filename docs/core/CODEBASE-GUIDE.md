...
Phases 0 to 3 define the contracts, durable intake path, all-family draft analysis, lawyer decisions and immutable publication.
...
- `contracts/product/legal-schema.v2.json`: layered fact model variant of V1 (decided 2026-09-12). Adds per-family coverage_only, headline and layer rules, subtype labels and optional timing/qualification/forum roles. Deterministically built by `scripts/product/build-legal-schema-v2.js`.
- `lib/product/legal-schema.js`: active schema validation; accepts both V1 and V2, calls validateV2Additions for V2.
- `contracts/product/fact-components.v2.json`: contract for FACT_COMPONENTS/V2 model, the layered tree of verbatim components. Defines kinds, origins, byte-span tracking, value canonicals for thresholds and periods, and cross-reference resolution.
- `lib/product/fact-components.js`: validation of fact structure (R1-R9), headline rendering (label plus distinguishing components), layer flattening for display and uncovered-text detection.
...
- `lib/product/agreement-draft.js`: routes every substantive section, compiles proposals for all 25 families via prompt version PRODUCT_ALL_FAMILY_EXTRACTOR/V6, groups and links, validates source spans, values, roles, relationships and four-state coverage.
...
- `pages/review/product/[id].js` and `components/product/ReviewWorkspace.jsx`: source-ordered proposal review, compact coverage, exact source context, revision restore and all-family summary. Query params `?focus=` lists provisions, `?all=1` shows every section, `?brief=` applies targeted guidance.
- `lib/product/section-highlight.js`: splits section text into marked and unmarked parts via byte offsets; used by focused review to highlight cited words.
- `components/product/FocusedReview.jsx`: two-column review UI showing one provision's text beside its proposed facts, with decision controls, comments and section detail toggles.
- `lib/product/review-briefs.js`: retrieves briefed sections and guidance questions for a run, curated by analysis to show material or uncertain extraction.
- `lib/product/review-state.js`: typed review command model with DECIDE_ITEM, COMMENT_ITEM, RESET_ITEM and ADD_MISSING_FACT. COMMENT_ITEM (4000 char limit) and RESET_ITEM (discards edits, keeps comments) enable lawyer notes and undo.
...
- `supabase/migrations/20260912190000_product_service_role_statement_timeout.sql`: extends statement timeout for service role to 60s. product_phase3_save_review validates ~1,700 items and exceeded the 8s default.
...
- `tests/product-focused-review-ui.test.js`: byte-range marking for section highlights, focus resolution from query and brief, FocusedReview component rendering, FactRow decision controls and comment state.
- `tests/product-review-comment.test.js`: COMMENT_ITEM command validation and state, RESET_ITEM revert-to-pending semantics.
- `tests/product-fact-components-contract.test.js`: FACT_COMPONENTS/V2 contract validation (invented text, ellipsis, loose list elements, forced values, unresolved references), headline and layer rendering.
- `tests/product-legal-schema-v2.test.js`: LEGAL_SCHEMA/V2 validation and deterministic generation from build script; V1 subtype survival and no forced generic roles.
