...
**Date:** 2026-08-02. **Status:** DRAFT — awaiting adversarial audit per
...
**1158 cards / 40 deals / 33 tagged subtypes + null.** Load-bearing counts:
...
2. **`party_scope` is uniformly `'MUTUAL'` (1158/1158)** — zero
   ...
   strings (`IOC-T | …`, `IOC-B | …`, e.g. deal `cf32899a-…`). Any design
...
3. **Multi-category limbs are the NORM, not the edge.** Card `3a2c477b-…`
   ...
   dividends in one lettered limb; card `dd0c39da-…` (dividend cap) also
   restricts redemptions and repurchases; card `da8a7a2f-…` is labelled
   IOC-ACCOUNTING but quotes Material-Contract restriction text — a live
   label↔text defect inside this family. v1 labels are UNTRUSTED input;
   every corroboration table below exists because the corpus itself
   demonstrates the drift, and the `da8a7a2f-…` defect becomes a permanent
   regression fixture (test 4).
...
- `ioc_positive_obligations` / `ioc_negative_obligations` (Supabase, RLS
  on) both hold **0 rows** and carry `obligation_kind` enums far coarser
  than the 34-code rubric taxonomy (no REPURCHASE/SPLIT/REALPROP/INSURANCE/
  WAIVE/AFFILIATE/pharma codes — they'd all collapse to `'OTHER'`). Those
...
- The long tail (OTHER-AFFIRMATIVE, IP, NEWLINE, AFFILIATE, REPURCHASE,
  ...
  for a first slice; their drafting stays open world.
...
- Candidate token: `[$€£]` optionally followed by whitespace, then a
  ...
  figures straight into enumerations ("not to exceed, in the aggregate
  $10,000,000 for any single Action", card `9ca3960a-…`; "does not exceed
  $300,000,000;", card `ffe804d5-…`), so a candidate ending in `,` or `.`
  followed by non-digit strips the trailing punctuation from the TOKEN
  (tokenizer boundary handling, not value repair) — evidence-pack warning
...
The parser never computes: "in an amount not to exceed $2,000,000 over the
aggregate amount … set forth in the Company's budget" (card `f75d5aba-…`)
resolves as the literal `'2000000'` with the budget-relative structure
preserved in the quoted evidence — the "over budget" comparative is legal
text for the reviewer, not an operand.
...
- **ONE resolution-table entry** (P1 M-2: `RESOLUTION_UNCONDITIONAL` is a
  ...
  `NATIVE_IOC_RESTRICTION_CANDIDATE`, row shape pinned against the REAL
  SHARE_COUNT row — `deterministic_kind: null`, `attachment_position:
  null`, `registered_claim_definition_key: null` (the handler makes the
  definition split on `assertion_kind`), `concept_key: null` explicitly
  (with the build-time check that nothing reads `mapping.concept_key`
  before the handler's category map runs), `party_field: null`,
  `party_role: null` (no party is minted this slice — pinned so an
  implementer doesn't improvise one). The concept split is made in the
...
- **restriction_category corroboration table** (frozen resolver constant;
  ...
  - `CHARTER` ↔ /certificate of incorporation|certificate of formation|
    bylaws|governing documents|charter/i (`3a2c477b-…`, `4b9149f4-…`,
    `ab03e623-…`)
...
0. **Fixture commit:** IOC-section canonical text spanning at least four
   ...
   (`3a2c477b-…`), the per-share step-up dividend cap (`dd0c39da-…`), the
   mislabelled accounting/contract card (`da8a7a2f-…`), a Material
   Contract limb (`0d7aeabc-…`), and one buyer-side section (deal
   `dfaa71fa-…` card `ab03e623-…`, a Parent charter covenant) for the
   classifier side split. Committed as LITERAL bytes from production
...
1. **Parser, table-driven:** "$10,000,000" (with its literal trailing
   comma context) → `'10000000'`; "$150,000,000" → `'150000000'`;
   ...
   ("$300,000,000;" and "$10,000,000," resolve clean); zero literals →
...
5. **Provider + dispatch:** response missing `ioc_restriction_assertions`
   ...
   guards hold, and the classifier is validated against ALL deals' section
   titles; `covenant_side` stamps through to claim attributes.
