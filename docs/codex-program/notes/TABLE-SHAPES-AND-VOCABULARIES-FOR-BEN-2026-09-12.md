...
## Decision 34 proposal: no fragment columns anywhere (for Ben's sign-off before generation 4), 2026-09-14 00:10 UTC

Ben, on the Antitrust / Regulatory table: "while it does produce, it doesn't create terms with usable names. Also meaning is lost - see the first row ... it misses the 'shall be required to' which changes the meaning. To be clear I don't want that specifically there but I'm not sure what we're trying to present? Also why wasn't the ordering/provisions from the precedent used? Or from the mock up last night? I'm overall just confused about your approach here. I've had to point out and fix every row." And on Intervening Event: "this is completely misleading on intervening event - it doesn't talk about the match periods etc".

### What went wrong

1. The pass-2 harvest took the print's *row labels* for the sections TopBuild prints, but where the print shows a sentence in a cell it recorded a free-text `verbatim` column called Provision or Detail. The extractor fills a verbatim column with the words it cited, so the cell shows a fragment ("commit to or agree") and loses the operative words around it ("shall not be required to"). That is the meaning loss.
2. Three sections have no print at all (Antitrust / Regulatory, Advisers / Fees / Expenses, the No-Shop overview). The harvest gave them a generic one-row-per-subject Term / Provision / Detail table, so each fact becomes a row named by its grammatical subject with verbs as cells: the pattern Ben rejected on 13 September for covenants and benefits.
3. The redesign to the print (decisions 17 to 33) was done section by section as Ben pointed at each one. The sections he had not opened kept the harvest shape. That is why every row needed his fix.

### The rule from here

Every table is the precedent's row list, and every column is exactly one of: a coded vocabulary, a number, a yes / no, or the fact as drafted (its own words in full). No column shows a cited fragment. A "Provision: Present" pill with nothing beside it is not a readout.

### Proposed shapes (each a generator decision, applied to the whole corpus)

- **Antitrust / Regulatory**: one row for the agreement, attribute grid, rows from the schema's fact types: Efforts standard (coded: reasonable best efforts / best efforts / commercially reasonable efforts / hell or high water); HSR filing deadline (period); Other filings timing (as drafted); Remedy / burden commitment (coded: no divestiture required / limited to a stated threshold or carve-out / hell or high water, plus as drafted); Litigation obligation (yes / no); Strategy control (coded: Parent / Company / joint); Consultation and information sharing (yes / no); Timing agreements without consent (prohibited / permitted); Pull and refile without consent (prohibited / permitted); Non-impediment covenant (yes / no); Third-party consents standard (as drafted).
- **Intervening Event**: rows Intervening Event provision (yes / no); Definition (as drafted); Excluded events (coded from the corpus taxonomy: acquisition proposal related / meeting or exceeding projections / stock price change / credit rating change / agreement-compliance actions); Notice to Parent before a change (period); Match period (period); Standard for the change (coded: inconsistent with fiduciary duties / other, as drafted); Parent termination right on a change (yes / no). The notice and match facts carry which path they govern (superior proposal or intervening event) so both tables show their own periods.
- **Interim operating covenants, negative**: rows are the print's restriction categories (Mergers, acquisitions, dispositions; Issuance of securities; Indebtedness; Capital expenditures; Commitments; Compensation and benefits; Charter / bylaws amendments; Accounting changes; Settlement of claims; plus the corpus taxonomy's categories, open to a new one), columns Restriction as drafted, Threshold (amount), Exceptions (coded from the print's exception vocabulary), Consent standard (coded: not unreasonably withheld / sole discretion). The separate Exceptions and Other restrictions tables fold into those columns.
- **No-Shop overview** table removed (its content is the core mechanics, fiduciary-out, intervening and superior tables).
- **Advisers / Fees / Expenses**: attribute grid: Company financial adviser (term), Parent financial adviser (term), Fairness opinion (yes / no), Expenses borne by (coded: each party its own / specified sharing).
- **Miscellaneous / Boilerplate**: keep the coded Provision column; Detail becomes as drafted.
- **Every remaining Detail / Provision verbatim column** in the no-shop core mechanics, acquisition proposal definition, votes and general covenants tables becomes as drafted.

Ben decides the row and code names above; the code applies them across the corpus once he has.

## Decision 34 applied, 2026-09-14

Ben, 2026-09-14: "why would you use top build? You know it is missing many provisions. Use bain/envestnet but then look at all of the output yourself and analyze if you think it is right or wrong and if not, iterate to improve it"; "just go through Metsera first and fix it all to be perfect before you go off on another deal"; "I don't mind you dual tracking envestnet and metsera or whatever is best but I want you to get to an output that provides what we used to have but with the deterministic qualities you have developed."

The specification is the old app's Envestnet review page, replayed as data by `scripts/product/legacy-review-print.js` (deal `1f80bec7-294b-4590-b861-2c156e75aadc`). The reading of Metsera generation 3 is `METSERA-GENERATION-3-AUDIT-2026-09-14.md`. Decision 34 is `scripts/product/table-shapes-decision-34.js`, applied by the generator after decisions 1 to 33:

- **Rule**: every table is the precedent's row list; every column is a coded vocabulary, a number, a yes / no, or the fact as drafted (`display: fact_text`). A test holds it: no fact table has a verbatim column without an as-drafted or resolved-reference display.
- **New shape mechanisms** (`lib/product/table-shapes.js`): `vocabulary_by_row` (a Provision column's codes listed per row; rule C13 rejects a code from another row's list); `derived: { from_family, from_subtype, join: 'presence', rows }` (a cell the page fills from another family's facts as a Yes pill or as drafted; rule C14 rejects an extractor cell on it; the page creates the row when only the derived cell exists); fixed-list rows always render in the precedent's order.
- **Antitrust / Regulatory**: the Envestnet rows (efforts standard, filing deadline, other filings, remedy commitment, regulatory litigation, strategy control, consultation, information sharing, timing agreements, non-impediment, third-party consents, request response), the row from the subtype, a coded Provision per row, a period, as drafted.
- **No-shop**: overview section removed. Core mechanics (go-shop; cease discussions; return or destruction; data-room access; prohibited acts, one code per act; representative control standard; standstill enforcement) and notice (required; period; content, one code per item; ongoing updates). Fiduciary-out / engagement (engagement standard; board determination standard; unsolicited, no breach, acceptable confidentiality agreement, information parity as yes / no) and change of recommendation (board change right; prohibited actions, one code per act; not a change; force the vote; reaffirmation period). Intervening event (provision; definition as drafted from KEY_DEFINED_TERMS; excluded events coded; board-change standard; notice and matching periods; Parent's termination right derived from TERMINATION). Superior proposal (threshold; test coded; determiner; the Company's termination right derived), the Acquisition Proposal and Acceptable Confidentiality Agreement definitions as drafted, and the matching-rights periods with the row from the subtype.
- **Interim operating covenants**: negative covenants one row per category (the corpus IOC categories, open), as drafted, threshold, exceptions coded, consent; general terms (consent standard, general exceptions, ordinary-course standard); the affirmative table as drafted with coded efforts.
- **Conditions**: a frustration row; the buyer's and seller's tables as drafted beside the tiers.
- **Termination rights**: the row from the subtype per table (BREACH is Company breach in Parent's table, Parent breach in the Company's), an extended-to date, a fee-on-exercise and window on the Company's table, every right as drafted; an effect-of-termination table (void, release, willful-breach carve-out, surviving provisions, formalities). Specific performance moves to boilerplate.
- **Termination fees**: amount, payer, triggers coded (one per scenario), payment timing, sole remedy, willful carve-out, late interest coded, as drafted; the tail's window, threshold, scenarios and scope.
- **Employee benefits**: "substantially comparable in the aggregate" and "no less favorable than similarly situated Parent employees" added; other protections as drafted.
- **Miscellaneous / Boilerplate**: the Envestnet rows plus notices, entire agreement, survival and construction, each with its own codes (governing law, forum, beneficiaries, assignment, amendment formalities, notice methods) and as drafted; the coverage-only family now renders, collapsed by default.
- **D&O indemnification**: its own section, a row per limb from the subtype, period and premium cap as numbers.
- **Consideration**: the appraisal line derived from the APPRAISAL_DISSENTERS_RIGHTS provision; the "per" cell coded; contingency as drafted.
- **Representations**: the old app's full canonical row list; capitalisation facts as limbs of Capitalization; Subsidiaries.
- **Votes and general covenants**: the anchor coded; details as drafted; the covenant standard coded; Advisers / Fees / Expenses removed.
- **Extractor**: the request carries `subtype_rows`, `vocabulary_codes_by_row`, `derived_rows` and `display: fact_text`, with the instruction sentences for each (`lib/product/agreement-draft.js`).

Q6 (long sections) is applied alongside: a section over 12k bytes with four or more child limbs is extracted per group of limbs, each part checked for a decline, the parts merged (`longSectionGroups`, `mergePartResponses`).
