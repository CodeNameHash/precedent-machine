...
Q02 link-only boundaries recorded in Phase 2 authority: **KEY_DEFINED_TERMS** (Acquisition Proposal / Superior Proposal definitions), **TERMINATION_FEE** (recommendation-change fee triggers), **PROXY_MEETING** (Force-the-vote recommendation-change cross-reference only), **TERMINATION** (termination-for-superior-proposal cross-refs). No definition or fee content absorbed.
