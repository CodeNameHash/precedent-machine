...
- **Fixed.** All five package generators hard-coded `evidence_binding: matchFixtureBindings[1]`. For D&O that index is a wrong-subtype fixture its anchor profile does not prove; the other four hit a proof index by coincidence. The D&O generator now derives the binding from the anchor's POSITIVE `fixture_proofs` entry, so closure is structural. Re-sealed package: `byte_length` 144,505, `sha256` `19e290f180e9…`, id `d5956a512622…`, **1** dimension-evidence row (was 3). Profiles, `match_fixtures` and `subtype_tree` are byte-identical — the 31-profile inventory and 26 APPROVE / 5 HOLD disposition are untouched.
- **Termination Blocker A (2026-08-24).** Per-profile fixture proofs added across all 45 profiles; package re-sealed at `byte_length` 1,121,931 (was 519,840), `sha256` `1997e2c3…`, id `7e0d841d…`. Generator and `work3.test.js` pin match; Milestone A slice 19/19 pass.
