...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, substantive head `2ddf8002`; includes the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission and shared Product row |
...
| M2 vertical slice | ACTIVE. QXO F28 passed the real isolated-staging rollback proof. Metsera now passes source acquisition through the typed Process sidecar, signed Process phrasebook-result admission and shared Product Query result row. Result-set, presentation, source-reading and candidate-release wiring remain. |
...
| `PM-METSERA-PRODUCT-ROW-01` | P8 | Compile the admitted Metsera grant through the existing Product Query IR and shared Product result row. | PM implementation | Same branch; commit `2ddf8002`; one bounded row adapter plus the existing signed Product compilers. | 50 focused Product and bridge tests PASS. Real run: query `f9045a1f047dcb97beb6ca5ba2432ec06384a78fde00c2524c335bf4b76e6af5`; Product result `8a1d819c7e69c0806c6f861ac250c5158adc7a3a851e4434ccf02d4c36df9207`; exact outcome field `EXCLUSIVITY_GRANTED`. | COMPLETE | Feed the exact row and Process ordering projection to the existing result-set compiler. | No |
