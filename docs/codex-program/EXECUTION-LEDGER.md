...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`; latest committed code head `a603c9e9`. All three Stage 2 correction reviews pass. The complete suite and production build are next. Main remains fixed. |
...
| Latest complete suite on this branch | PASS on prior candidate code head `86e5bc70`. On current head `a603c9e9`, 54 fresh writer, contract-root and two-clean-compile checks pass. One complete suite remains due on the final combined candidate. |
...
staging run still stops at the required M1 acknowledgement check. The current
task is Stage 5: the final complete suite and production build. Clean bundle
compiles, independent exact-root reviews and one reserved bundle approval
follow.
...
| `PM-P8-GENERIC-WRITER-02` | P8 Stage 5 | Make F28 and IOC reach one immutable candidate-result insert through the existing canonical writer. | PM implementation | Active branch; commit `a603c9e9`; candidate writer, governed SQL extracts and focused tests. | The SQL path closes copied provision rows and membership objects. Each governed function comment matches its exact bytes. Three Stage 2 reviews and 54 fresh affected checks pass. | INTEGRATION | Run the complete suite and build, then preserve the validated SQL in the exact-root review candidate. | No |
