...
| Generic Agreement writer staging proof | SUPERSEDED. F28 and IOC reached the same writer, replay and rollback path, but Stage 2 found that the SQL function did not independently revalidate the complete nested Product chain. Commit `53bea82f` now rejects every Agreement Product SQL write before DML. Durable candidate rows and receipts remain zero and pointer generation 10 is unchanged. The proof must run again after the SQL-native validator passes. |
...
chain. The SQL path now fails closed. The current task is the missing SQL-native
validator and a new isolated-staging proof. The final exact contract root,
independent reviews and one reserved bundle approval follow.
