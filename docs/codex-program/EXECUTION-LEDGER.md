...
| Active milestone branch | `codex/agreement-candidate-envelope-v1`, based on pushed F28 head `ea4baf3` |
...
| Draft generic-envelope successor | 172 substantive contracts and 297 links. Focused compile and runtime chain PASS. This draft does not replace the acknowledged M1 bundle until its exact bytes pass review and the reserved contract-freeze approval. |
...
| `PM-GENERIC-ENVELOPE-01` | P8 exit | Prove that the second Agreement family can use one parameterised envelope contract. | PM implementation | `codex/agreement-candidate-envelope-v1`; one generic contract/runtime plus data profiles for capitalisation and IOC capex. QXO and Landos are source-validator bindings, not envelope forks. No copied F28 envelope file. | 392 affected contract, exact-root, manifest and runtime tests PASS. QXO retains 14 terminals. Landos capex retains one 0.07272727% terminal, raw dollars, denominator, exception relationship, evidence and Product row. Draft root: 172 contracts, 297 links, zero structural defect. | REVIEW | Complete the bounded authority review, commit the unit, then run exact-root reviews. | Reserved exact contract freeze only |
