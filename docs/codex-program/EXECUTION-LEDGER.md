...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, substantive head `7cecbe7e`; includes the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row and Product result set |
...
| M2 vertical slice | ACTIVE. QXO F28 passed the real isolated-staging rollback proof. Metsera now passes source acquisition through the typed Process sidecar, signed Process phrasebook-result admission, shared Product Query row and Process-ordered Product result set. Presentation, source-reading and candidate-release wiring remain. |
...
| `PM-METSERA-PRODUCT-ROW-01` | P8 | Compile the admitted Metsera grant through the existing Product Query IR and shared Product result row. | PM implementation | Same branch; commit `2ddf8002`, with exact-query sequencing correction in `7cecbe7e`; one bounded row adapter plus the existing signed Product compilers. | 50 focused Product and bridge tests PASS. Real run: query `0615e8766415ebddbc02b1551e3f125ddc9c1146e0ec04422044c9129ebabda7`; Product result `935eee3255dff95397822824365f72666b6fc5b9672882b6d9ac9ed6adf4d405`; exact outcome field `EXCLUSIVITY_GRANTED`. | COMPLETE | Feed the exact row and Process ordering projection to the existing result-set compiler. | No |
| `PM-METSERA-RESULT-SET-01` | P8 | Revalidate the Metsera Product row and preserve the Process-owned order in the existing Product result-set compiler. | PM implementation | Same branch; commit `7cecbe7e`; one bounded result-set adapter. | 33 focused result-set tests PASS. Real run produces one ordered valid result, zero typed failures and zero exclusions. Product Query is compiled before Process ordering, so both bind the same exact Query ID. | COMPLETE | Pass the exact slots and execution summary to the existing presentation compiler. | No |
| `PM-METSERA-PRODUCT-LINK-01` | P8 | Carry the exact Metsera Product row through result-set, presentation, Review, Compare, Corpus Context and source reading. | PM implementation | Same branch; existing Process result-set, presentation and source-reader compilers. | Same result, source, release, citation and identities across all surfaces. | ACTIVE | Build the narrow result-set and presentation handoff without a second source or result architecture. | No |

## 3. Next 48 hours

1. Feed the validated Metsera Product row into the existing result-set, presentation and source-reader compilers, then bind that same payload to Review, Compare and Corpus Context.
