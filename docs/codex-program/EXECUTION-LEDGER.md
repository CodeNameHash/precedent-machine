...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, pushed head `f15d93fa`; the working batch adds the one-writer Product candidate-result operation after the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row, Product result set, answer-first presentation and four-surface Product binding |
...
1. Resolve the same candidate release through the existing source-reader compiler without pretending that it is active.
2. Run the cross-view browser proof against the exact candidate payload.
3. Run the exact 173-contract successor-root checks and the reserved exact-bundle approval only after all checks pass.
