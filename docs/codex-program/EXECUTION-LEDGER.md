...
| `PM-GENERIC-ENVELOPE-01` | P8 exit | Prove that the second Agreement family can use one parameterised envelope contract. | PM implementation | `codex/agreement-candidate-envelope-v1`; commit `ab12f40`; one generic contract/runtime plus data profiles for capitalisation and IOC capex. QXO and Landos are source-validator bindings, not envelope forks. No copied F28 envelope file. | 392 affected contract, exact-root, manifest and runtime tests PASS. QXO retains 14 terminals. Landos capex retains one 0.07272727% terminal, raw dollars, denominator, exception relationship, evidence and Product row. Draft root: 172 contracts, 297 links, zero structural defect. | REVIEW | Run exact-root reviews against commit `ab12f40`, then run the complete candidate checks. | Reserved exact contract freeze only |
