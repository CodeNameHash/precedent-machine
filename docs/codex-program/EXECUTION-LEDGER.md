...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, substantive head `abf74476`; includes the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row, Product result set, answer-first presentation and one four-surface Product binding |
...
| `PM-METSERA-PERSISTENCE-GAP-01` | P8 | Persist the Process result without converting it into an Agreement metric row. | PM controller | Current staging schema and candidate importer. | NEEDS_MECHANICAL_CHECK. Staging has no Process or Product result table. The existing candidate importer accepts Agreement metric rows. The Process pilot contracts grant no writer authority. | BLOCKED | Define the smallest governed Product-result persistence path or prove that the existing candidate-membership carrier is sufficient for P8. Do not use `query_response_cache` as authoritative storage. | Reserved only if the contract bundle must change |

## 3. Next 48 hours

1. Feed the validated Metsera Product result and exact Process source sidecar into the existing source-reader compiler, then bind that same payload to Review, Compare and Corpus Context.
