...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, pushed head `d6005906`; the working tree adds real browser acceptance after the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row, Product result set, answer-first presentation, four-surface Product binding, one-writer candidate persistence and the inactive-candidate source-reader proof |
