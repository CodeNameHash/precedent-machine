...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`; head `163a0f95`. The batch includes the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row, Product result set, answer-first presentation, four-surface Product binding, one-writer candidate persistence, inactive-candidate source-reader proof, real browser acceptance and the exact Metsera Product row in the generic Product result release import path. |
...
| M2 vertical slice | ACTIVE. QXO F28 passed the real isolated-staging rollback proof. Metsera now passes source acquisition through the typed Process sidecar, signed Process phrasebook-result admission, shared Product Query row, Process-ordered Product result set, answer-first presentation, one exact Query/Review/Compare/Corpus Context binding, rollback-only candidate persistence, an honest inactive-release source-reader refusal and desktop/mobile browser acceptance. The exact Metsera Product row now passes the generic V7 release import in the same rollback transaction as its canonical write. The inactive Product query fails closed. The active Product query remains one indexed, release-pinned call. Exact-root review and reviewed staging activation remain. |
...
source-to-product rollback, browser, inactive-source and candidate-release
import checks. The exact Product Query result and its complete Process sidecar
now travel through the existing candidate-release transaction. The exact
successor root now needs the independent reviews, one reserved bundle approval
and reviewed staging activation.
...
1. Run the final successor-root checks.
2. Run architecture and identity, legal-semantic, and query, serving and release reviews against the same exact bytes.
