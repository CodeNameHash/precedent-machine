...
| Active milestone branch | `codex/p8-combined-pilot-integration-v1`, pushed head `03051ebe`; the working tree adds the inactive-candidate source-reader proof after the generic Agreement envelope, bounded PI pipeline connector, sealed Metsera source-to-sidecar run, signed Process result admission, shared Product row, Product result set, answer-first presentation, four-surface Product binding and one-writer candidate persistence |
...
| M2 vertical slice | ACTIVE. QXO F28 passed the real isolated-staging rollback proof. Metsera now passes source acquisition through the typed Process sidecar, signed Process phrasebook-result admission, shared Product Query row, Process-ordered Product result set, answer-first presentation, one exact Query/Review/Compare/Corpus Context binding, rollback-only candidate persistence and an honest inactive-release source-reader refusal. Browser acceptance and the reviewed candidate-release activation remain. |
...
1. Run the cross-view browser proof against the exact candidate payload.
2. Run the exact 173-contract successor-root checks and independent reviews.
3. Prepare the one reserved exact-bundle approval package.
4. After approval, activate only the reviewed staging candidate and rerun the exact source-reader request.
...
| `P8-VERTICAL-SLICES` | P8 | QXO and Metsera pass source-to-product staging runs. | PM controller | P1-P7 links. | QXO F28 real isolated-staging rollback proof PASS. Metsera passes real source through candidate persistence and an honest inactive-release source-reader refusal. Browser evidence and reviewed staging activation remain. | ACTIVE | Run browser acceptance, exact-root review and the reserved bundle approval. | Reserved exact contract freeze only |
