-- The recorded request is the provider request: the product request (and
-- its source_closure.extraction_part) sits as a JSON string in
-- messages[0].content, so 20260914125500 read the part from the wrong
-- place and every part was still refused. The part number is read from
-- either place.
...
BEGIN
  ...
  IF p_call->>'invocation_id' IS NOT NULL THEN
    extraction_part := product_private.product_phase2_model_call_extraction_part(p_call);
