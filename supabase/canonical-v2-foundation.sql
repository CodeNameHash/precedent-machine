...
CREATE TABLE IF NOT EXISTS canonical_v2_staging.deal_admission_records (
  deal_admission_id text PRIMARY KEY
    CHECK (deal_admission_id ~ '^[0-9a-f]{64}$'),
  deal_key text NOT NULL CHECK (length(trim(deal_key)) > 0),
  document_hash text NOT NULL CHECK (document_hash ~ '^[0-9a-f]{64}$'),
  canonical_payload jsonb NOT NULL,
  canonical_payload_digest text GENERATED ALWAYS AS (
    canonical_v2_staging.payload_digest(canonical_payload)
  ) STORED,
  UNIQUE (deal_key, deal_admission_id),
  CONSTRAINT canonical_v2_deal_admission_records_payload_check CHECK (
    jsonb_typeof(canonical_payload) = 'object'
    AND canonical_payload ?& ARRAY[
      'deal_key', 'deal_admission_id', 'document_hash'
    ]
    AND jsonb_typeof(canonical_payload->'deal_key') = 'string'
    AND jsonb_typeof(canonical_payload->'deal_admission_id') = 'string'
    AND jsonb_typeof(canonical_payload->'document_hash') = 'string'
    AND canonical_payload->>'deal_key' = deal_key
    AND canonical_payload->>'deal_admission_id' = deal_admission_id
    AND canonical_payload->>'document_hash' = document_hash
  )
);

ALTER TABLE canonical_v2_staging.deal_admission_records
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_constraint
    WHERE conrelid =
      'canonical_v2_staging.deal_admission_records'::regclass
      AND conname =
        'canonical_v2_deal_admission_records_payload_check'
  ) THEN
    ALTER TABLE canonical_v2_staging.deal_admission_records
      ADD CONSTRAINT canonical_v2_deal_admission_records_payload_check
      CHECK (
        jsonb_typeof(canonical_payload) = 'object'
        AND canonical_payload ?& ARRAY[
          'deal_key', 'deal_admission_id', 'document_hash'
...
BEGIN
  IF EXISTS (
    ...
    WHERE jsonb_typeof(canonical_payload) IS DISTINCT FROM 'object'
      OR NOT (canonical_payload ?& ARRAY[
        'deal_key', 'deal_admission_id', 'document_hash'
      ])
      OR jsonb_typeof(canonical_payload->'deal_key')
        IS DISTINCT FROM 'string'
      OR jsonb_typeof(canonical_payload->'deal_admission_id')
        IS DISTINCT FROM 'string'
      OR jsonb_typeof(canonical_payload->'document_hash')
        IS DISTINCT FROM 'string'
      OR canonical_payload->>'deal_key' IS DISTINCT FROM deal_key
      OR canonical_payload->>'deal_admission_id'
        !~ '^[0-9a-f]{64}$'
      OR canonical_payload->>'document_hash' !~ '^[0-9a-f]{64}$'
  ) THEN
    RAISE EXCEPTION 'legacy deal admission backfill preflight failed'
      USING ERRCODE = '23514';
  END IF;

  FOR legacy_deal IN
    SELECT
      canonical_payload->>'deal_admission_id' AS deal_admission_id,
      deal_key,
      canonical_payload->>'document_hash' AS document_hash,
      canonical_payload
    FROM canonical_v2_staging.deals
    ORDER BY canonical_payload->>'deal_admission_id'
  LOOP
    INSERT INTO canonical_v2_staging.deal_admission_records(
      ...
      legacy_deal.deal_admission_id,
      legacy_deal.deal_key,
      legacy_deal.document_hash,
      legacy_deal.canonical_payload
    ) ON CONFLICT (deal_admission_id) DO NOTHING;

    IF NOT EXISTS (
      SELECT 1
      FROM canonical_v2_staging.deal_admission_records admission
      WHERE admission.deal_admission_id = legacy_deal.deal_admission_id
        AND admission.deal_key = legacy_deal.deal_key
        AND admission.document_hash = legacy_deal.document_hash
        AND admission.canonical_payload_digest =
          canonical_v2_staging.payload_digest(legacy_deal.canonical_payload)
    )
    THEN
      RAISE EXCEPTION 'canonical legacy deal admission backfill conflict'
        USING ERRCODE = '23505';
    END IF;
  END LOOP;
END
...
CREATE TABLE IF NOT EXISTS canonical_v2_staging.definition_occurrences (
  definition_occurrence_id text PRIMARY KEY
    CHECK (definition_occurrence_id ~ '^[0-9a-f]{64}$'),
  closure_id text NOT NULL CHECK (closure_id ~ '^[0-9a-f]{64}$'),
  canonical_text_id text NOT NULL CHECK (canonical_text_id ~ '^[0-9a-f]{64}$'),
  document_hash text NOT NULL CHECK (document_hash ~ '^[0-9a-f]{64}$'),
  canonical_payload jsonb NOT NULL,
  canonical_payload_digest text GENERATED ALWAYS AS (
    canonical_v2_staging.payload_digest(canonical_payload)
...
CREATE TABLE IF NOT EXISTS canonical_v2_staging.condition_group_revisions (
  condition_group_revision_id text PRIMARY KEY
    CHECK (condition_group_revision_id ~ '^[0-9a-f]{64}$'),
  closure_id text NOT NULL CHECK (closure_id ~ '^[0-9a-f]{64}$'),
  canonical_payload jsonb NOT NULL,
  canonical_payload_digest text GENERATED ALWAYS AS (
    canonical_v2_staging.payload_digest(canonical_payload)
...
CREATE INDEX IF NOT EXISTS canonical_v2_definition_occurrences_closure_idx
  ON canonical_v2_staging.definition_occurrences(closure_id);
CREATE INDEX IF NOT EXISTS canonical_v2_definition_occurrences_source_idx
  ON canonical_v2_staging.definition_occurrences(canonical_text_id, document_hash);
...
CREATE INDEX IF NOT EXISTS canonical_v2_condition_groups_closure_idx
  ON canonical_v2_staging.condition_group_revisions(closure_id);
...
ALTER TABLE canonical_v2_staging.deal_admission_records ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE canonical_v2_staging.definition_occurrences ENABLE ROW LEVEL SECURITY;
      ...
      IF domain_carrier - ARRAY[
        ...
        OR EXISTS (
          ...
          FROM jsonb_array_elements(
            domain_carrier->'agreement_candidate_envelope'->'ordered_terminals'
          ) AS terminal(value)
          WHERE terminal.value->>'terminal_payload_digest'
              IS DISTINCT FROM canonical_v2_staging.payload_digest(
                terminal.value - ARRAY[
                  'schema_version', 'terminal_id', 'terminal_payload_digest',
                  'subject_terminal'
                ]::text[]
              )
            OR terminal.value->>'terminal_id'
              IS DISTINCT FROM canonical_v2_staging.content_id(
            ...
            OR terminal.value->>'subject_terminal_payload_digest'
              IS DISTINCT FROM canonical_v2_staging.payload_digest(
                terminal.value->'subject_terminal'
              )
        )
      THEN
