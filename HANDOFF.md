...
2026-07-08: WP-M2-06 Step 11: added the Fiduciary-Out config, `nosol-fiduciary`, for the schema-first `ProvisionTable` path. The config covers engagement/final standards, board-change right, notice and match mechanics, force-the-vote, termination right, representative controls, and buyer nonsolicit-breach termination.
2026-07-08: WP-M2-06 Step 12: added the Employee Benefits config, `employee-benefits`, for the schema-first `ProvisionTable` path. The config renders benefit type, comparison group, standard, exceptions, bundling, and text from structured `compensationItems`, with fallback rows for legacy flat employee-compensation standards.
2026-07-08: WP-M2-06 Step 13: added the shareholder meeting / proxy / tender-offer SEC matters config, `sec-meeting`, for the schema-first `ProvisionTable` path. The config covers proxy filing, mailing, meeting and adjournment mechanics, plus Schedule TO, Schedule 14D-9, stockholder-list and tender-offer minimum-condition rows.
2026-07-08: WP-M2-06 Step 14: added the No Other Reps / Fraud config, `no-other-reps-fraud`, for the schema-first `ProvisionTable` path. The config renders the Abry four-question summary, fraud carve-out/silence, and willful-breach definition using the existing Abry derivation helper.
2026-07-08: WP-M2-06 Step 15: added the Consideration hero config, `consideration-hero`, for the schema-first `ProvisionTable` path. The config summarises headline consideration form, per-share/offer price, exchange ratio, proration/election, collar, walk-away, CVR, appraisal, withholding, and award-treatment mechanics from schema cards.
2026-07-08: WP-M2-06 Step 16: reran the schema parity audit after all schema-first table configs landed. Result: 40/40 clean deals, 0 diffs, 24,803 cells compared. Step 15's schema-default restore was already satisfied by the schema-only renderer state on main; no render switch remained to restore.
2026-07-08: WP-M2-08 Step 1: added the layout-slot inventory audit and generated `docs/audit/legacy-layout-inventory.md` plus `docs/audit/m2-08-config-gaps.md`. The audit found 16 structured legacy surfaces and 9 config gaps for Step 2: structure mechanics, antitrust/regulatory, MAE definitions, termination rights, termination fees, general covenants, approvals/votes, advisers/fees/expenses, and representation qualifiers.
2026-07-08: WP-M2-08 Step 2: added the nine missing schema-first configs from the Step 1 gap report and wired them into `/review/[id]`: `structure-mechanics`, `antitrust-regulatory`, `mae-definitions`, `termination-rights`, `termination-fees`, `general-covenants`, `approvals-votes`, `advisers-fees-expenses`, and `representations-qualifiers`. Each config is under the 150-line cap.
2026-07-08: WP-M2-08 Step 3: added the layout-surface coverage audit and generated `docs/audit/m2-08-coverage.md`. Result: 16/16 legacy layout surfaces covered by mounted schema-first configs, including 9/9 Step 1 gaps; no intentionally uncovered legacy surface remains.
2026-07-08: WP-M2-09 Step 1a: added the AST-driven legacy JSX audit and generated `docs/audit/m2-09-legacy-inventory.md` plus `docs/audit/m2-09-primitives-needed.md`. Result: 20 live JSX table components audited; baseline primitives inferred: PillCell, ThresholdCellWithHoverQuote, CoverageChecklist, GroupedSubRows, RomanNumeralOrdinal, EvidenceHoverSource, EmptyStateBranch, and ComputedRollupHeader.
2026-07-08: WP-M2-09 Step 1b: added the schema-augmentation inventory and generated `docs/audit/m2-09-schema-augmentations.md`. Result: 15 table families inventoried with required baseline augmentations plus config-discovered structured fields; every rendered card-backed value remains slated for provenance-bundle hover-source treatment.
2026-07-08: WP-M2-09 Step 1c: added `docs/audit/m2-09-per-family-plan.md` and the canonical Review Queue entry `docs/review-queue/m2-09-per-family-rebuild-plan.json`. Steps 2-4 are gated on Ben approving the batch per-family rebuild plan at `/admin/review-queue`.
2026-07-08: WP-M2-09 Step 2: added reusable `ProvisionTable` render primitives for pill cells, threshold evidence cells, checklist coverage, grouped subrows, roman-numeral ordinals, empty branches, rollup headers, and hover-source evidence. `ProvisionTable` now passes the primitive bundle to every config `renderCell` context.
...
2026-07-08: WP-M2-09 Step 3, Termination Rights family: enriched termination-rights config with primitive-backed signals and hover-source rendering for party, triggers, outside date, extensions, vote failure, breach, and fiduciary termination rows.

2026-07-08: WP-M2-09 Step 3, SEC / Meeting family: enriched shareholder meeting, proxy, tender-offer SEC matters config with primitive-backed signals and hover-source rendering for filing deadlines, meeting deadlines, adjournment rights, and offer mechanics.
