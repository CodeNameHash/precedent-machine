...
import ProcessFilterEditor, { isAdmittedFilterValue } from './ProcessFilterEditor';
...
export default function ProcessResearchSurface({ presentation, navigation, filterFields, admittedOptionsProjection, relatedPassages, sourceReader, onAsk, onBrowse, onEditSubject, onEditFilter, onClearFilter, onOpenSource, onOpenDetail, onCopyExactPassage, onShare, onSelectForExport, onSave, onRerun, onCorrection, onCloseReader, onReaderContextAction, onOpenRelated }) {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-4 p-4 text-ink">
      ...
      {editing ? <ProcessFilterEditor fields={filterFields} selected={editorSelection(editingSegment)} activeSegments={sentence?.ordered_filter_segments || []} admittedOptionsProjection={admittedOptionsProjection || []} onApply={(filter) => { const activeField = filterFields.find((field) => field.field_reference?.field_key === filter.field_reference?.field_key && field.field_reference?.field_version === filter.field_reference?.field_version); if (!isAdmittedFilterValue({ active: activeField, activeSegments: sentence?.ordered_filter_segments || [], admittedOptionsProjection: admittedOptionsProjection || [], value: filter.value })) return; setFilterSentence((current) => { const currentSegments = current?.ordered_filter_segments || []; const matched = currentSegments.find((segment) => segment.field_reference?.field_key === filter.field_reference?.field_key && segment.field_reference?.field_version === filter.field_reference?.field_version); const nextOrdinal = Math.max(0, ...currentSegments.map((segment) => segment.segment_ordinal)) + 1; const nextSegment = { ...(matched || { segment_ordinal: nextOrdinal, practitioner_operator: 'is', editable: true, clearable: true }), field_reference: filter.field_reference, field_label: filter.field_label, value: filter.value }; return { ...current, ordered_filter_segments: matched ? currentSegments.map((segment) => segment === matched ? nextSegment : segment) : [...currentSegments, nextSegment] }; }); onEditFilter?.(filter); setEditing(false); }} onClose={() => setEditing(false)} /> : null}
