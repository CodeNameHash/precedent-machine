...
export default function ProcessPassageCard({ slot, onOpenSource, onOpenDetail, onCopyExactPassage, onShare, onSelectForExport, onCorrection }) {
  ...
  return (
    <article className="rounded border border-border bg-white p-4" aria-label={`Passage from ${sourceLabel(slot)}`}>
      ...
      <div className="mt-4 flex flex-wrap gap-x-3 gap-y-2">{actions.map(([kind, callback]) => { const target = actionTarget(slot, kind); if (!target || !callback) return null; const input = kind === 'COPY_EXACT_PASSAGE' ? exactCopyInput(slot) : actionInput(slot, kind); return <button key={kind} type="button" onClick={() => invoke(callback, input)} className="text-xs font-medium text-ink underline">{target.accessible_name}</button>; })}{onCorrection ? <button type="button" onClick={() => onCorrection(slot)} className="text-xs font-medium text-ink underline">Suggest correction</button> : null}</div>
    </article>
  );
}
