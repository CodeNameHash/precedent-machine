...
function FactRow({ fact, selected, onSelect, onDecision, onComment, onReset, onSource, busy, cardProps, onSelectComponent, selectedComponentId }) {
  const [open, setOpen] = useState(false);
  ...
  return <li className={`rounded border p-2 ${rowTone[decision] || rowTone.PENDING} ${selected ? 'ring-2 ring-amber-400' : ''}`} data-testid="focused-fact" data-decision={decision}>
    {hasComponents ? (
      <PublishedFact
        ...
        onSource={onSource}
        aids={aids}
        onComponentSelect={(selection) => onSelectComponent(proposal, selection)}
