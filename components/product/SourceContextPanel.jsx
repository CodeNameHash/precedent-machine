    ...
    <div className="fixed inset-0 z-50 bg-black/40 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-label="Exact agreement source" className="mx-auto flex h-full max-w-5xl flex-col rounded-xl bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b border-border px-5 py-3">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-inkLight">Exact source</p><p className="font-mono text-xs text-inkMid">{span ? `${displayReviewLabel(span.kind)} · bytes ${span.start_byte}-${span.end_byte}` : 'Loading'}</p></div>
        ...
        <div className="min-h-0 flex-1 overflow-auto p-6">
          ...
          {loading ? <p>Loading source…</p> : <>{parts ? <article className="mb-4 rounded-lg border border-amber-400 bg-amber-50 p-4" data-testid="highlighted-source-context"><p className="mb-2 text-[10px] font-bold uppercase tracking-wide text-inkLight">{surroundingLabel(contextSpan)} · {displayReviewLabel(contextSpan.kind)} · bytes {contextSpan.start_byte}-{contextSpan.end_byte}</p><pre className="whitespace-pre-wrap font-mono text-xs leading-6 text-ink">{parts.before}<mark ref={mark} className="bg-amber-200">{parts.selected}</mark>{parts.after}</pre></article> : null}<details><summary className="cursor-pointer text-xs font-semibold text-accent">Show all source passages in this closure</summary><div className="mt-3 space-y-4">{closureSpans.map((item) => <article key={item.span_id} className={`rounded-lg border p-4 ${item.span_id === span?.span_id ? 'border-amber-400 bg-amber-50' : 'border-border'}`}><p className="mb-2 text-[10px] font-bold uppercase tracking-wide text-inkLight">{displayReviewLabel(item.kind)} · bytes {item.start_byte}-{item.end_byte}</p><pre className="whitespace-pre-wrap font-mono text-xs leading-6 text-ink">{item.span_id === span?.span_id ? <mark className="bg-amber-200">{item.exact_text}</mark> : item.exact_text}</pre></article>)}</div></details></>}
