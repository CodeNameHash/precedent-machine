...
import FindingResolutionFields, { COLLISION_TITLE, CollisionDetail, findingResolutionCommand } from './FindingResolutionFields';
...
export function ReleaseEvaluation({ state, onEvaluate, busy, onSource }) {
  ...
  return <form onSubmit={submit} className="space-y-4 rounded-xl border border-border bg-white p-4" data-testid="release-evaluation-form">
    ...
    <FindingResolutionFields findings={findings} facts={facts} selections={findingSelections} busy={busy} onSource={onSource} onChange={(id, value) => setFindingSelections((current) => ({ ...current, [id]: value }))} />
