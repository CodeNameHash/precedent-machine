...
import { groupPublishedFacts } from '../../lib/product/published-layers';
...
function HeldDetail({ value, issueCode }) {
  ...
  const category = [value.family_key, value.subtype_key, value.fact_type]
    .map(scalarText).filter(Boolean).map(displayReviewLabel);
  ...
  const roles = plainObject(value.roles) ? Object.entries(value.roles).map(([key, role]) => {
    ...
    return text === null ? null : `${displayReviewLabel(key)}: ${text}`;
  }).filter(Boolean) : [];
  ...
  const linkType = displayReviewLabel(nestedText(value.relationship_type ?? value.type));
...
function RecordedReviewDetail({ message }) {
  const readable = (text) => text.replace(/\b[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+\b/g, displayReviewLabel);
  let parsed;
  try { parsed = JSON.parse(message); } catch {}
  if (!plainObject(parsed) && !Array.isArray(parsed)) {
    return <span className="text-inkLight">Recorded detail: {typeof message === 'string' ? readable(message) : message}</span>;
  }
  const explanation = plainObject(parsed) ? [parsed.explanation, parsed.message, parsed.description, parsed.rationale,
    parsed.unknown_row?.rationale].filter((value) => typeof value === 'string' && value.trim()) : [];
  return <div className="basis-full text-inkMid">
    {explanation.length ? [...new Set(explanation)].map((text, index) => <p key={index}>{readable(text)}</p>)
      : <p>Additional review information is available in the recorded detail below.</p>}
    <details className="mt-2"><summary className="cursor-pointer font-semibold">Show recorded detail</summary><pre className="mt-1 max-h-48 overflow-auto whitespace-pre-wrap">{message}</pre></details>
  </div>;
}

export function Requirement({ item, onDecision, onSource, busy, analysis }) {
  const title = item.kind === 'COVERAGE' ? `${displayReviewLabel(item.original.family_key) || 'Section'} ${displayReviewLabel(item.original.subject_kind)}`
    : item.kind === 'IMMATERIAL_ROUTING' ? `Confirm ${displaySectionReference(item.original.section_reference)} is immaterial`
      : item.kind === 'EXCEPTION_LINK' ? 'Review exception relationship'
        : item.original.code === 'DUPLICATE_FACT_OCCURRENCE' ? COLLISION_TITLE : displayReviewLabel(item.original.code || item.kind);
  ...
  return <div className="flex flex-wrap items-center gap-2 rounded border border-border bg-white p-3 text-sm"><span className="font-semibold text-ink">{title}</span>{status ? <span className="text-inkLight">Status: {displayReviewLabel(status)}</span> : null}<EmptyExtractionWarning item={item} analysis={analysis} />{item.original.code === 'DUPLICATE_FACT_OCCURRENCE' ? <CollisionDetail message={item.original.message} /> : hasHeldDetail ? <HeldContent parsed={held} issueCode={item.original.code} originalMessage={item.original.message} /> : null}{item.original.message && !hasHeldDetail && item.original.code !== 'DUPLICATE_FACT_OCCURRENCE' ? <RecordedReviewDetail message={item.original.message} /> : null}{item.relationship_context ? <p className="basis-full rounded bg-paper p-2 text-xs text-inkMid"><strong>Exception:</strong> {item.relationship_context.from}<br/><strong>Applies to:</strong> {item.relationship_context.to}</p> : null}{item.issue_context ? <div className="basis-full rounded bg-paper p-2 text-xs text-inkMid"><p>{item.issue_context.explanation}</p><p><strong>Recorded group:</strong> {displayReviewLabel(item.issue_context.group_mapping.family_key)} · {displayReviewLabel(item.issue_context.group_mapping.subtype_key)}</p><p><strong>Proposal mappings:</strong> {item.issue_context.member_mappings.map((mapping) => `${displayReviewLabel(mapping.family_key)} · ${displayReviewLabel(mapping.subtype_key)}`).join('; ') || 'Unavailable in the stored analysis'}</p></div> : null}{item.source_span_ids?.length ? <button type="button" onClick={() => onSource(item.source_closure_id, item.source_span_ids[0])} className="text-xs font-semibold text-accent">View complete source closure</button> : null}<span className="ml-auto text-xs font-bold">{displayReviewLabel(item.decision)}</span><><button disabled={busy} type="button" onClick={() => onDecision(item.item_id, 'ACCEPTED')} className="rounded border border-green-700 px-2 py-1 text-xs text-green-800">{item.kind === 'EXCEPTION_LINK' ? 'Accept' : item.kind === 'ISSUE' || item.original.state === 'UNRESOLVED' ? 'Acknowledge' : 'Reviewed'}</button>{item.kind === 'EXCEPTION_LINK' ? <button disabled={busy} type="button" onClick={() => onDecision(item.item_id, 'REJECTED')} className="rounded border border-slate-500 px-2 py-1 text-xs">Reject</button> : null}<button disabled={busy} type="button" onClick={() => onDecision(item.item_id, 'UNRESOLVED')} className="rounded border border-red-600 px-2 py-1 text-xs text-red-700">Unresolved</button></></div>;
}

function relationshipTypeLabel(value) {
  return displayReviewLabel(value);
}
...
export function RelationshipReviewCard({ item, factItems, analysis, onDecision, onSave, onSource, busy }) {
  ...
  return <article className="rounded border border-border bg-white p-3 text-sm">
    <div className="flex flex-wrap items-center gap-2"><strong>{relationshipTypeLabel(relationship.relationship_type)}</strong><span className="ml-auto text-xs font-bold">{displayReviewLabel(item.decision)}</span></div>
...
export function DraftReview({ workspace, view, busy, command, openSource, focus = [], allSectionsHref = null, brief = null }) {
  ...
  return <>
    {focused ? <FocusedReview view={view} analysis={analysis} focus={focus} busy={busy} command={command} openSource={openSource} cardPropsFor={cardPropsFor} allSectionsHref={allSectionsHref} brief={brief} /> : null}
    ...
    <section id="review-checks" aria-labelledby="review-checks-heading" className="scroll-mt-40 space-y-5">
      ...
      {view.agreement_items.length ? <section className="space-y-2"><h3 className="font-display text-lg text-ink">Agreement-level findings · {view.agreement_items.length}</h3>{view.agreement_items.map((item) => <Requirement key={item.item_id} item={item} analysis={analysis} busy={busy} onSource={openSource} onDecision={(itemId, decision) => command({ type: 'DECIDE_ITEM', item_id: itemId, decision })} />)}</section> : null}
      <RelationshipReview items={view.relationship_items} factItems={view.fact_items} analysis={analysis} busy={busy} onSource={openSource} onDecision={(itemId, decision) => command({ type: 'DECIDE_ITEM', item_id: itemId, decision })} onSave={(relationship) => command({ type: 'UPSERT_RELATIONSHIP', ...relationship })} />
    </section>
    <section className="rounded-xl border border-border bg-white p-4"><h2 className="font-display text-lg">Revision history</h2><div className="mt-2 flex flex-wrap gap-2">{workspace.review.revisions.filter((revision) => revision.event_type !== 'PUBLISH').map((revision) => <button disabled={busy || revision.version === workspace.review.version} type="button" key={revision.version} onClick={() => command({ type: 'RESTORE', restore_version: revision.version })} className="rounded border border-border px-2 py-1 text-xs">Restore version {revision.version} · {displayReviewLabel(revision.event_type)}</button>)}</div></section>
  </>;
}
...
export function AcceptedSummary({ summary, metrics, onSource, active }) {
  ...
  const hasLayeredFacts = facts.some(isLayeredFact);
  const layeredFacts = facts.filter(isLayeredFact).map((fact) => ({ ...fact, fact_id: fact.review_item_id }));
  const remainingFamilies = hasLayeredFacts
    ? summary.families.map((family) => ({ ...family, facts: family.facts.filter((fact) => !isLayeredFact(fact)) }))
    : summary.families;
  return <section className="space-y-4" data-testid="accepted-summary"><div className={`rounded-xl border p-4 ${active ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}><h2 className="font-display text-xl text-ink">{active ? 'Active key-provisions release' : 'Inactive release candidate'}</h2><p className="mt-1 text-sm text-inkMid">{metrics.proposal_count} proposals reviewed, {metrics.proposal_errors} corrected or rejected, {metrics.proposal_omissions} omissions added, {metrics.review_time_seconds}s review time.</p></div>{hasLayeredFacts ? <PublishedSummary groups={groupPublishedFacts(layeredFacts, legalSchema)} onSource={onSource} /> : null}{hasLayeredFacts ? <h3 className="mb-2 text-sm font-bold tracking-wide text-ink" data-testid="v1-facts-heading">Facts without layers (V1)</h3> : null}{remainingFamilies.map((family) => <div key={family.family_key}><h3 className="mb-2 text-sm font-bold tracking-wide text-ink">{displayReviewLabel(family.family_key)}</h3>{family.facts.length ? <div className="space-y-2">{family.facts.map((fact) => <article key={fact.review_item_id} className="rounded-lg border border-border bg-white p-4"><p className="font-medium text-ink">{fact.statement}</p><p className="mt-1 text-xs text-inkLight">{displayReviewLabel(fact.subtype_key)} · {displayReviewLabel(fact.fact_type)}</p><CitationSourceButtons sourceClosureId={fact.source_closure_id} sourceSpanIds={fact.source_span_ids} onSource={onSource} /></article>)}</div> : <p className="text-sm text-inkLight">No accepted facts.</p>}</div>)}{summary.relationships.length ? <div><h3 className="mb-2 text-sm font-bold tracking-wide text-ink">Accepted relationships</h3><div className="space-y-2">{summary.relationships.map((relationship) => <article key={relationship.review_item_id} className="rounded-lg border border-border bg-white p-4 text-sm"><p className="font-semibold">{relationshipTypeLabel(relationship.relationship_type)}</p><p><strong>From:</strong> {statement(relationship.from_proposal_id)}</p><p><strong>To:</strong> {statement(relationship.to_proposal_id)}</p><div className="mt-2"><CitationSourceButtons sourceClosureId={relationship.source_closure_id} sourceSpanIds={relationship.source_span_ids} onSource={onSource} /></div></article>)}</div></div> : null}</section>;
}

function referenceSourceLabel(source) {
  const parties = Array.isArray(source.parties) ? source.parties.filter((party) => party?.name) : [];
  return parties.length ? parties.map((party) => `${party.name}${party.role ? ` (${party.role})` : ''}`).join(' / ')
    : source.retrieval_url;
}

export function ReleaseEvaluation({ state, analysis, referenceSources = [], onEvaluate, busy, onSource }) {
  ...
  const [referenceSamples, setReferenceSamples] = useState(() => prior?.reference_samples || []);
  ...
  const [attested, setAttested] = useState(prior?.reference_samples_attested === true);
  const evaluation = state.release_evaluation;
  function updateReferenceSample(id, values) {
    setReferenceSamples((current) => current.map((item) => item.reference_sample_id === id ? { ...item, ...values } : item));
  }
  function addReferenceSample() {
    setReferenceSamples((current) => [...current, {
      reference_sample_id: crypto.randomUUID(), source_document_id: '', source_url: '', source_section: '',
      description: '', severity: '', assessment: '', comparison: '', reviewed_limitation: '',
    }]);
  }
  async function submit(event) {
    ...
    await onEvaluate({
      ...
      reference_samples_attestation: attested,
    });
  }
  return <form onSubmit={submit} className="space-y-4 rounded-xl border border-border bg-white p-4" data-testid="release-evaluation-form">
    <div><h2 className="font-display text-lg text-ink">Supervised candidate evaluation</h2><p className="text-sm text-inkLight">Review source-linked reference samples on their original agreements, assess every candidate citation, and certify the candidate decisions. Reference-sample results do not measure recall or precision for this agreement.</p></div>
    <div className="space-y-2">{referenceSamples.map((item, index) => <div key={item.reference_sample_id} className="grid gap-2 rounded border border-border p-3 sm:grid-cols-6">
      <select required aria-label={`Reference agreement ${index + 1}`} value={item.source_document_id} onChange={(event) => { const source = referenceSources.find((candidate) => candidate.source_document_id === event.target.value); updateReferenceSample(item.reference_sample_id, { source_document_id: source?.source_document_id || '', source_url: source?.retrieval_url || '' }); }} className="sm:col-span-5 rounded border border-border p-2 text-sm"><option value="">Select an accessible source agreement</option>{referenceSources.map((source) => <option key={source.source_document_id} value={source.source_document_id}>{referenceSourceLabel(source)}</option>)}</select>
      <input required aria-label={`Reference section ${index + 1}`} value={item.source_section} onChange={(event) => updateReferenceSample(item.reference_sample_id, { source_section: event.target.value })} placeholder="Section" className="rounded border border-border p-2 text-sm" />
      {item.source_url ? <a href={item.source_url} target="_blank" rel="noreferrer" className="sm:col-span-6 text-xs font-semibold text-accent">Open source agreement</a> : null}
      <textarea required aria-label={`Reference legal point ${index + 1}`} value={item.description} onChange={(event) => updateReferenceSample(item.reference_sample_id, { description: event.target.value })} placeholder="One atomic legal point from that source agreement" className="sm:col-span-3 rounded border border-border p-2 text-sm" />
      <select required aria-label={`Reference severity ${index + 1}`} value={item.severity} onChange={(event) => updateReferenceSample(item.reference_sample_id, { severity: event.target.value })} className="rounded border border-border p-2 text-xs"><option value="">Select severity</option><option>CRITICAL</option><option>MATERIAL</option></select>
      <select required aria-label={`Reference assessment ${index + 1}`} value={item.assessment} onChange={(event) => updateReferenceSample(item.reference_sample_id, { assessment: event.target.value, comparison: '', reviewed_limitation: '' })} className="sm:col-span-2 rounded border border-border p-2 text-xs"><option value="">Select assessment</option><option value="FOUND">Found correctly</option><option value="MISSED">Missed</option><option value="INCORRECT">Incorrect</option><option value="UNRESOLVED">Unresolved</option></select>
      {item.assessment === 'FOUND' ? <textarea required aria-label={`Reference comparison ${index + 1}`} value={item.comparison} onChange={(event) => updateReferenceSample(item.reference_sample_id, { comparison: event.target.value })} placeholder="Compare the source point with the product output on this source agreement" className="sm:col-span-5 rounded border border-border p-2 text-sm" /> : item.assessment ? <textarea required aria-label={`Reference limitation ${index + 1}`} value={item.reviewed_limitation} onChange={(event) => updateReferenceSample(item.reference_sample_id, { reviewed_limitation: event.target.value })} placeholder="Record the correction or reviewed limitation" className="sm:col-span-5 rounded border border-border p-2 text-sm" /> : null}
      <button type="button" onClick={() => setReferenceSamples((current) => current.filter((candidate) => candidate.reference_sample_id !== item.reference_sample_id))} className="text-xs text-red-700">Remove</button>
    </div>)}</div>
    <button type="button" onClick={addReferenceSample} className="text-xs font-semibold text-accent">+ Add reference sample</button>
    <div className="space-y-2"><h3 className="text-sm font-semibold">Published citation assessments</h3>{facts.map((fact) => <fieldset key={fact.review_item_id} className="rounded border border-border p-3 text-sm"><legend className="px-1 font-medium">{fact.statement}</legend><CitationSourceButtons sourceClosureId={fact.source_closure_id} sourceSpanIds={fact.source_span_ids} onSource={onSource} />{[['exact', 'Exact', null], ['legally_sufficient', 'Legally sufficient', null], ['narrow', 'Focused supporting words (diagnostic)', 'The full surrounding paragraph or clause remains visible. This diagnostic does not add a publication requirement.']].map(([key, label, help]) => <label key={key} className="mr-4 inline-flex items-start gap-1"><input type="checkbox" checked={citations[fact.review_item_id]?.[key] === true} onChange={(event) => setCitations((current) => ({ ...current, [fact.review_item_id]: { ...current[fact.review_item_id], [key]: event.target.checked } }))} /><span>{label}{help ? <span className="block max-w-sm text-xs font-normal text-inkLight">{help}</span> : null}</span></label>)}</fieldset>)}</div>
    ...
    <label className="flex items-start gap-2 rounded bg-paper p-3 text-sm"><input required type="checkbox" checked={attested} onChange={(event) => setAttested(event.target.checked)} />I certify that I am the lawyer reviewer, that each reference sample is linked to its original agreement and records my comparison or reviewed limitation, and that I made or approved the final candidate decisions, citation assessments, finding resolutions, and agreement-level coverage confirmation.</label>
    <button disabled={busy || referenceSamples.length === 0 || !attested} className="rounded bg-ink px-4 py-2 text-xs font-semibold text-white disabled:opacity-40">Evaluate candidate against fixed bars</button>
    {evaluation ? <div className={`rounded p-3 text-sm ${evaluation.passed ? 'bg-green-50 text-green-900' : 'bg-red-50 text-red-900'}`} data-testid="release-evaluation-result"><strong>{evaluation.passed ? 'All fixed release bars pass.' : 'Release is blocked.'}</strong><p className="mt-2">Entered total: {evaluation.diagnostics.review_time_minutes} minutes. Measured review duration: {evaluation.diagnostics.measured_review_time_seconds} seconds.</p><p>Measured processing: {Number.isFinite(evaluation.diagnostics.processing_minutes) ? evaluation.diagnostics.processing_minutes.toFixed(1) : 'unavailable'} minutes. Recorded total process and review time: {Number.isFinite(evaluation.diagnostics.effective_elapsed_minutes) ? evaluation.diagnostics.effective_elapsed_minutes.toFixed(1) : 'unavailable'} minutes.</p>{evaluation.schema_version === 'PRODUCT_SUPERVISED_RELEASE_EVALUATION/V3' ? <p>Reference samples: {evaluation.diagnostics.reference_sample_count}; missed: {evaluation.diagnostics.reference_sample_missed_count}; incorrect: {evaluation.diagnostics.reference_sample_incorrect_count}; unresolved: {evaluation.diagnostics.reference_sample_unresolved_count}; weighted success: {Number.isFinite(evaluation.diagnostics.reference_sample_success_rate) ? `${(evaluation.diagnostics.reference_sample_success_rate * 100).toFixed(1)}%` : 'unavailable'}. These sample-only results do not measure recall or precision for this agreement.</p> : null}{evaluation.schema_version === 'PRODUCT_SUPERVISED_RELEASE_EVALUATION/V1' ? <p>This historical evaluation used the 90-minute pass/fail limit.</p> : <p>There is no 90-minute pass/fail limit. Valid timing is still required.</p>}<ul className="mt-2 list-disc pl-5">{Object.entries(evaluation.bars).map(([bar, passed]) => <li key={bar}>{passed ? 'Pass' : 'Fail'}: {bar.replaceAll('_', ' ')}</li>)}</ul></div> : null}
  </form>;
}

export default function ReviewWorkspace({ runId, focus = [], allSectionsHref = null, brief = null }) {
